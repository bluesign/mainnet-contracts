...
pub contract PDS{
    ...
    pub resource SharedCapabilities {
        ...
        pub fun mintPackNFT(distId: UInt64, commitHashes: [String], issuer: Address, recvCap: &{NonFungibleToken.CollectionPublic}): [UInt64] {
            var i = 0
        ...
        access(self) fun releaseEscrow(nftIds: [UInt64], recvCap:  &{NonFungibleToken.CollectionPublic}, collectionProviderPath: PrivatePath ) {
            let pdsCollection = PDS.account.getCapability(collectionProviderPath).borrow<&{NonFungibleToken.Provider}>()
        ...
        access(self) fun fulfillFromIssuer(nftIds: [UInt64], recvCap:  &{NonFungibleToken.CollectionPublic}) {
    ...
    pub resource DistributionManager {
        ...
        pub fun mintPackNFT(distId: UInt64, commitHashes: [String], issuer: Address, recvCap: &{NonFungibleToken.CollectionPublic}): [UInt64] {
            assert(PDS.DistSharedCap.containsKey(distId), message: "No such distribution")
            let d <- PDS.DistSharedCap.remove(key: distId)!
            let mintedNftIDs = d.mintPackNFT(distId: distId, commitHashes: commitHashes, issuer: issuer, recvCap: recvCap)
            PDS.DistSharedCap[distId] <-! d
            return mintedNftIDs
        }
