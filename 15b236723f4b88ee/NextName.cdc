...
pub contract NextName: NonFungibleToken {
    ...
    pub struct SetData {
        ...
        init(setID: UInt32) {
            pre {
                NextName.sets[setID] != nil: "Cannot borrow Set: The Set doesn't exist"
            }
    ...
    pub resource Set {
        ...
        pub fun addCollectibleItem(collectibleItemID: UInt32) {
            pre {
                NextName.collectibleItemDatas[collectibleItemID] != nil: "Cannot add the CollectibleItem to Set: CollectibleItem doesn't exist."
            ...
            switch view {
                case Type<MetadataViews.Display>():
                    ...
                    var url = metadata["posterUrl"]
                                ?? "https://cloudflare-ipfs.com/ipfs/bafybeifbnckgntaaq3oknp4ygidnalwdupmv6fhdiip3cdqoaxekxhvfei/nextname-logo.png"
                    let prefixes = ["ipfs://", "http://", "https:/"]
                    if !prefixes.contains(url.slice(from: 0, upTo: 7)) {
                        url = prefixes[0].concat(url)
                    }
                    return MetadataViews.Display(
                        name: metadata["title"] ?? "NextName NFT",
                        description: metadata["description"] ?? "Official NextName NFT",
                        thumbnail: MetadataViews.HTTPFile(
                            url: url,
                        )
                    )
                case Type<MetadataViews.Royalties>():
                    let receiver = NextName
                        .account
                        .getCapability<&AnyResource{FungibleToken.Receiver}>(/public/flowTokenReceiver)
                    let royalty = MetadataViews.Royalty(
                        receiver: receiver,
                        cut: 0.05,
                        description: "NextName / Niftory Fees"
                    )
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(
                        storagePath: NextName.CollectionStoragePath,
                        publicPath: NextName.CollectionPublicPath,
                        providerPath: /private/NextNameCollection001,
                        publicCollection: Type<&NextName.Collection{NextName.NextNameCollectionPublic}>(),
                        publicLinkedType: Type<&NextName.Collection{NextName.NextNameCollectionPublic,NonFungibleToken.CollectionPublic,NonFungibleToken.Receiver,MetadataViews.ResolverCollection}>(),
                        providerLinkedType: Type<&NextName.Collection{NextName.NextNameCollectionPublic,NonFungibleToken.CollectionPublic,NonFungibleToken.Provider,MetadataViews.ResolverCollection}>(),
                ...
                case Type<MetadataViews.NFTCollectionDisplay>():
                    return MetadataViews.NFTCollectionDisplay(
                        name: "NextName",
                        description: "NextName NFT Collection. Support Your Players.",
                        externalURL: MetadataViews.ExternalURL(
                            url: "https://nextname.io/"
                        ),
                        squareImage: MetadataViews.Media(
                            file: MetadataViews.HTTPFile(
                                url: "https://cloudflare-ipfs.com/ipfs/bafybeifbnckgntaaq3oknp4ygidnalwdupmv6fhdiip3cdqoaxekxhvfei/nextname-logo.png",
                            ),
                            mediaType: "image/png"
                        ),
                        bannerImage: MetadataViews.Media(
                            file: MetadataViews.HTTPFile(
                                url: "https://cloudflare-ipfs.com/ipfs/bafybeihyk7jgd2bhkvztqogoilgfc42gbx2mmxnb5pwvijqquaj5gdemmu/banner.jpg",
                            ),
                            mediaType: "image/png"
                        ),
                        socials: {}
