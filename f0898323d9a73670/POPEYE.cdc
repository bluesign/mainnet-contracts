...
pub contract POPEYE: NonFungibleToken, Anique {
    // -----------------------------------------------------------------------
    // POPEYE contract Events
    // -----------------------------------------------------------------------

    // Events for Contract-Related actions
    //
    // Emitted when the POPEYE contract is created
    ...
    // -----------------------------------------------------------------------
    // POPEYE contract-level Composite Type definitions
    // -----------------------------------------------------------------------

    // The structure that represents Item
    // each digital content which POPEYE deal with on Flow
    //
    pub struct ItemData {
        ...
        init(itemID: UInt32) {
            let item = (&POPEYE.items[itemID] as &Item?)!
    ...
    pub resource NFT: NonFungibleToken.INFT, Anique.INFT {
        ...
        init(serialNumber: UInt32, itemID: UInt32) {
            ...
            POPEYE.totalSupply = POPEYE.totalSupply + 1

            // set id
            self.id = POPEYE.totalSupply
    ...
    // interface that represents POPEYE collections to public
    ...
    pub resource interface CollectionPublic {
        ...
        // borrow NFT as POPEYE token
        pub fun borrowPOPEYECollectible(id: UInt64): auth &NFT
    }
    ...
    pub resource Collection: CollectionPublic, NonFungibleToken.Receiver, NonFungibleToken.Provider, NonFungibleToken.CollectionPublic {
        ...
        pub fun deposit(token: @NonFungibleToken.NFT) {
            ...
            // Cast the deposited token as an POPEYE NFT to make sure
            // it is the correct type
            let token <- token as! @POPEYE.NFT
        ...
        // not any POPEYE specific data. Please use borrowCollectible to
        ...
        // borrowPOPEYECollectible returns a borrowed reference
        // to an POPEYE Collectible
        pub fun borrowPOPEYECollectible(id: UInt64): auth &NFT {
            pre {
    ...
    pub resource Admin {
        ...
        // and stores it in the Items dictionary field in the POPEYE smart contract
        //
        ...
        pub fun createItem(metadata: {String: String}): UInt32 {
            ...
            POPEYE.items[newItem.itemID] <-! newItem

            return itemId
        }

        // borrowItem returns a reference to a Item in the POPEYE
        ...
        pub fun borrowItem(itemID: UInt32): &Item {
            pre {
                POPEYE.items[itemID] != nil: "Cannot borrow Item: The Item doesn't exist"
            }

            return (&POPEYE.items[itemID] as &Item?)!
        }
    ...
    // -----------------------------------------------------------------------
    // POPEYE contract-level function definitions
    // -----------------------------------------------------------------------
    ...
    pub fun createEmptyCollection(): @NonFungibleToken.Collection {
        return <-create POPEYE.Collection()
    }
    ...
    // -----------------------------------------------------------------------
    // POPEYE initialization function
    // -----------------------------------------------------------------------
    //
    init() {
        ...
        self.collectionStoragePath     = /storage/POPEYECollection
        self.collectionPublicPath      =  /public/POPEYECollection
        self.collectionPrivatePath     = /private/POPEYECollection
        self.adminStoragePath          = /storage/POPEYEAdmin
        self.saleCollectionStoragePath = /storage/POPEYESaleCollection
        self.saleCollectionPublicPath  =  /public/POPEYESaleCollection
