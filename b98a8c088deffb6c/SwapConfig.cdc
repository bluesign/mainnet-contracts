...
pub contract SwapConfig {
    ...
    pub fun sl(_ l: [[UInt256]]): [[UInt256]] {
        for i, k in l { l[i].append(UInt256(i)) }
        var p = 0; var c = l[0]; var i = 1
        while (i < l.length) {
            p = i - 1; c = l[i]
            while(p >= 0 && (l[p][1]) / (l[p][0]) < (c[1]) / (c[0])) { l[p+1] = l[p]; p = p - 1}
            l[p+1] = c
            i = i + 1
        }
        return l
    ...
    access(self) fun balance(_ maxAmount: UInt256, _ pIn: UInt256, _ pOut: UInt256, _ sumIn: UInt256, _ sumOut: UInt256): UInt256 {
    ...
    pub fun splitAmountsIn(_ tokenInAmount: UFix64, _ tokenInKey: Int, _ tokenOutKey: Int, _ pools: [[UInt256]]): [UFix64] {
