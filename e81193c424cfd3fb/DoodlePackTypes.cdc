...
pub contract DoodlePackTypes {
	...
	access(contract) fun validateTemplateDistributions(amountOfTokens: UInt8, templateDistributions: [TemplateDistribution]) {
		pre {
			amountOfTokens > 0: "Amount of tokens must be greater than 0"
			templateDistributions.length > 0: "Template distributions must be greater than 0"
		}
	...
	pub struct TemplateDistribution {
		...
		init(id: UInt64, templateProbabilities: [TemplateProbability], minAmount: UInt8, maxAmount: UInt8, maxMint: UInt64?) {
			pre {
				templateProbabilities.length > 0: "Template probabilities must be greater than 0"
				maxAmount > 0: "Max amount must be greater than 0"
				maxAmount >= minAmount: "Max amount must be greater than or equal to min amount"
				maxMint == nil || maxMint! > 0: "Max mint must be greater than 0"
			}
			self.id = id
	...
	access(account) fun addPackType(
		...
		pre {
			name.length > 0: "Name must be greater than 0"
			description.length > 0: "Description must be greater than 0"
			amountOfTokens > 0: "Amount of tokens must be greater than 0"
			templateDistributions.length > 0: "Template distributions must be greater than 0"
		}

		DoodlePackTypes.validateTemplateDistributions(amountOfTokens: amountOfTokens, templateDistributions: templateDistributions)
	...
	access(account) fun addMintedCountToPackType(typeId: UInt64, amount: UInt64) {
		if (DoodlePackTypes.packTypes[typeId] == nil) {
			panic("No pack type found")
		}
		DoodlePackTypes.packTypesMintedCount[typeId] = DoodlePackTypes.getPackTypesMintedCount(typeId: typeId) + 1
	}

	access(account) fun addMintedCountToTemplateDistribution(typeId: UInt64, templateDistributionId: UInt64, amount: UInt64) {
		let packType: DoodlePackTypes.PackType = DoodlePackTypes.packTypes[typeId] ?? panic("No pack type found")
		if (packType.templateDistributions[templateDistributionId] == nil) {
			panic("No template distribution found")
		}

		let templateDistributionsMintedCount = DoodlePackTypes.templateDistributionsMintedCount[typeId] ?? {}
		templateDistributionsMintedCount[templateDistributionId] = (templateDistributionsMintedCount[templateDistributionId] ?? 0) + 1
	...
	pub fun getTemplateDistributionMintedCount(typeId: UInt64, templateDistributionId: UInt64): UInt64 {
		let templateDistributionsMintedCount = DoodlePackTypes.templateDistributionsMintedCount[typeId] ?? {}
