...
pub contract SGKCLDR_jormungand: NonFungibleToken, Anique {
    // -----------------------------------------------------------------------
    // SGKCLDR_jormungand contract Events
    // -----------------------------------------------------------------------

    // Events for Contract-Related actions
    //
    // Emitted when the SGKCLDR_jormungand contract is created
    ...
    // -----------------------------------------------------------------------
    // SGKCLDR_jormungand contract-level fields.
    ...
    // -----------------------------------------------------------------------
    // SGKCLDR_jormungand contract-level Composite Type definitions
    // -----------------------------------------------------------------------

    // The structure that represents Item
    // each digital content which SGKCLDR_jormungand deal with on Flow
    //
    pub struct ItemData {
        ...
        init(itemID: UInt32) {
            let item = (&SGKCLDR_jormungand.items[itemID] as &Item?)!
    ...
    pub resource Item {
        ...
        init(metadata: {String: String}) {
            ...
            self.itemID = SGKCLDR_jormungand.nextItemID
            self.metadata = metadata
            self.numberMintedPerItem = 0

            // increment the nextItemID so that it isn't used again
            SGKCLDR_jormungand.nextItemID = SGKCLDR_jormungand.nextItemID + 1
    ...
    pub resource NFT: NonFungibleToken.INFT, Anique.INFT {
        ...
        init(serialNumber: UInt32, itemID: UInt32) {
            ...
            SGKCLDR_jormungand.totalSupply = SGKCLDR_jormungand.totalSupply + 1

            // set id
            self.id = SGKCLDR_jormungand.totalSupply
    ...
    // interface that represents SGKCLDR_jormungand collections to public
    ...
    pub resource interface CollectionPublic {
        ...
        // borrow NFT as SGKCLDR_jormungand token
        pub fun borrowSGKCLDR_jormungandCollectible(id: UInt64): auth &NFT
    }
    ...
    pub resource Collection: CollectionPublic, NonFungibleToken.Receiver, NonFungibleToken.Provider, NonFungibleToken.CollectionPublic {
        ...
        pub fun deposit(token: @NonFungibleToken.NFT) {
            ...
            // Cast the deposited token as an SGKCLDR_jormungand NFT to make sure
            // it is the correct type
            let token <- token as! @SGKCLDR_jormungand.NFT
        ...
        // not any SGKCLDR_jormungand specific data. Please use borrowCollectible to
        ...
        // borrowSGKCLDR_jormungandCollectible returns a borrowed reference
        // to an SGKCLDR_jormungand Collectible
        pub fun borrowSGKCLDR_jormungandCollectible(id: UInt64): auth &NFT {
            pre {
    ...
    pub resource Admin {
        ...
        // and stores it in the Items dictionary field in the SGKCLDR_jormungand smart contract
        //
        ...
        pub fun createItem(metadata: {String: String}): UInt32 {
            ...
            SGKCLDR_jormungand.items[newItem.itemID] <-! newItem

            return itemId
        }

        // borrowItem returns a reference to a Item in the SGKCLDR_jormungand
        ...
        pub fun borrowItem(itemID: UInt32): &Item {
            pre {
                SGKCLDR_jormungand.items[itemID] != nil: "Cannot borrow Item: The Item doesn't exist"
            }

            return (&SGKCLDR_jormungand.items[itemID] as &Item?)!
        }
    ...
    // -----------------------------------------------------------------------
    // SGKCLDR_jormungand contract-level function definitions
    // -----------------------------------------------------------------------
    ...
    pub fun createEmptyCollection(): @NonFungibleToken.Collection {
        return <-create SGKCLDR_jormungand.Collection()
    }
    ...
    // -----------------------------------------------------------------------
    // SGKCLDR_jormungand initialization function
    // -----------------------------------------------------------------------
    //
    init() {
        ...
        self.collectionStoragePath     = /storage/SGKCLDR_jormungandCollection
        self.collectionPublicPath      =  /public/SGKCLDR_jormungandCollection
        self.collectionPrivatePath     = /private/SGKCLDR_jormungandCollection
        self.adminStoragePath          = /storage/SGKCLDR_jormungandAdmin
        self.saleCollectionStoragePath = /storage/SGKCLDR_jormungandSaleCollection
        self.saleCollectionPublicPath  =  /public/SGKCLDR_jormungandSaleCollection
