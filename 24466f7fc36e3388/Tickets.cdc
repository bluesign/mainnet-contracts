...
pub contract Tickets {
  ...
  */
  pub resource Admin {
    ...
    pub fun mintDispenser(dispenser_id: UInt32, address: Address): @Dispenser {
      pre {
        Tickets.dispenserOwners[dispenser_id] != nil : "Requested address is not in previously requested list."
        Tickets.dispenserOwners[dispenser_id]!.grant == false : "Requested address is already minted."
      }
  ...
  */
  pub resource CapabilityReceiverVault: IProxyCapabilityReceiverPublic {
    ...
    pub fun deposit(cap: Capability<&Admin>) {
      pre {
        Tickets.adminCapabilityHolder == false : "Admin capability vault is already assigned."
      }
  ...
  pub resource DispenserVault: IDispenserPrivate, IDispenserPublic {
    ...
    pub fun refund(dispenser_id: UInt32, address: Address, user_id: UInt32, repayment: @FlowToken.Vault) {
      pre {
        ...
        Tickets.UserFlowTokenVault[user_id] != nil: "The beneficiary has not yet set up the Vault."
        Tickets.ticketRequesters.containsKey(dispenser_id): "Sender has not right to refund."
        Tickets.ticketRequesters[dispenser_id]!.containsKey(user_id): "Sender has not right to refund."
        Tickets.ticketRequesters[dispenser_id]![user_id]!.paid >= repayment.balance: "refund is larger than paid amount."
      }
  ...
  pub resource TicketVault: ITicketPrivate, ITicketPublic {
    ...
    pub fun useTicket(dispenser_id: UInt32, token_id: UInt64, address: Address, payment: @FlowToken.Vault, fee: @FlowToken.Vault) {
      pre {
        ...
        Tickets.DispenserFlowTokenVault[dispenser_id] != nil: "Receiver is not set."
        Tickets.ticketRequesters.containsKey(dispenser_id): "Ticket is not requested."
        Tickets.ticketRequesters[dispenser_id]![self.user_id]!.crowdfunding == false : "crowdfunding cannot use ticket with fee."
      }
    ...
    pub fun crowdfunding(dispenser_id: UInt32, address: Address, payment: @FlowToken.Vault, fee: @FlowToken.Vault) {
      pre {
        fee.balance > (fee.balance + payment.balance) * 0.024: "fee is less than 2.5%."
        Tickets.DispenserFlowTokenVault[dispenser_id] != nil: "Receiver is not set."
        Tickets.ticketRequesters.containsKey(dispenser_id): "crowdfunding registration info is not set."
        Tickets.ticketRequesters[dispenser_id]![self.user_id]!.crowdfunding == true : "this ticket requester is not asking crowdfunding."
      }
    ...
    pub fun setRefundVault(flow_vault_receiver: Capability<&FlowToken.Vault{FungibleToken.Receiver}>) {
      pre {
        Tickets.UserFlowTokenVault[self.user_id] == nil: "You have already requested a refund once."
      }
    ...
    pub fun useCrowdfundingTicket(dispenser_id: UInt32, token_id: UInt64, address: Address) {
      pre {
        Tickets.DispenserFlowTokenVault[dispenser_id] != nil: "Receiver is not set."
        Tickets.ticketRequesters.containsKey(dispenser_id): "Ticket is not requested."
        Tickets.ticketRequesters[dispenser_id]![self.user_id]!.crowdfunding == true : "this ticket is not for crowdfunding."
      }
  ...
  */
  init() {
    ...
    self.account.save<@Tickets.Admin>( <- create Admin(), to: self.AdminPrivateStoragePath)
    self.account.link<&Tickets.Admin>(self.AdminPrivatePath, target: self.AdminPrivateStoragePath)
    self.account.save<@Tickets.AdminPublic>(<- create AdminPublic(), to: self.AdminPublicStoragePath)
    self.account.link<&Tickets.AdminPublic>(self.AdminPublicPath, target: self.AdminPublicStoragePath)
  }
}