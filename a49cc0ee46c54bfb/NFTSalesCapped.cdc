...
pub contract NFTSalesCapped : ContractVersion {
    ...
    pub resource Sales {
        ...
        init(
            ...
            pausableCap: Capability<&{Pausable.PausableExternal}>
                ...
                pre {
                    name.length > 0 : "name has zero length"
                    NFTSalesCapped.isEqual2DArrayLengths(cardTypeWeights, cardTypes) : "inconsistent initial card type weight array lengths"
                }
                self.id = self.uuid
        ...
        pub fun openPacksWithCreditCardPayment(
            ...
            pre {
                !self.pausableCap.borrow()!.isPaused(): "Invalid: Sales is paused"
                NFTSalesCapped.orderIds[orderId] == nil : "orderId already used" 
            }
            ...
            let isValid = NFTSalesCapped.isValidSignature(
    ...
    pub resource SalesCollection: SalesCollectionPublic {
        ...
        pub fun addSales(sales: @Sales) {
            pre {
                NFTSalesCapped.isPaymentReceiverCapSet() == true : "payment receiver is not set"
            }
