...
pub contract MotoGPCard: NonFungibleToken {
   ...
   access(account) var allCards: [UInt128]
    ...
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver {
        ...
        pub fun getViews(): [Type] {
            return [
                ...
                Type<MotoGPCardMetadata.Riders>() // custom type
            ]
        }

        pub fun resolveView(_ view: Type): AnyStruct? {
            return MotoGPCardMetadata.resolveView(
                view: view,
                id: self.id,
                cardID: self.cardID,
                serial: self.serial,
                // Below four arguments are passed to MotoGPCardMetadata (as opposed being hardcoded there) to avoid cyclic dependencies MotoGPCard <-> MotoGPCardMetadata
                publicCollectionType: Type<&MotoGPCard.Collection{MotoGPCard.ICardCollectionPublic}>(),
                publicLinkedType: Type<&MotoGPCard.Collection{MotoGPCard.ICardCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(),
                providerLinkedType: Type<&MotoGPCard.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection}>(),
                createEmptyCollectionFunction: (fun (): @NonFungibleToken.Collection {
                    return <- MotoGPCard.createEmptyCollection()
                })
            )
        }
    }
    ...
    // interact with Cards.
    //
    pub resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, ICardCollectionPublic, MetadataViews.ResolverCollection {
        ...
        pub fun borrowViewResolver(id: UInt64): &AnyResource{MetadataViews.Resolver} {
            ...
            return cardNFT as &AnyResource{MetadataViews.Resolver}
        }
