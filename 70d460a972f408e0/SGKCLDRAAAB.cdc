...
pub contract SGKCLDRAAAB: NonFungibleToken, Anique {
    // -----------------------------------------------------------------------
    // SGKCLDRAAAB contract Events
    // -----------------------------------------------------------------------

    // Events for Contract-Related actions
    //
    // Emitted when the SGKCLDRAAAB contract is created
    ...
    // -----------------------------------------------------------------------
    // SGKCLDRAAAB contract-level fields.
    ...
    // -----------------------------------------------------------------------
    // SGKCLDRAAAB contract-level Composite Type definitions
    // -----------------------------------------------------------------------

    // The structure that represents Item
    // each digital content which SGKCLDRAAAB deal with on Flow
    //
    pub struct ItemData {
        ...
        init(itemID: UInt32) {
            let item = (&SGKCLDRAAAB.items[itemID] as &Item?)!
    ...
    pub resource NFT: NonFungibleToken.INFT, Anique.INFT {
        ...
        init(serialNumber: UInt32, itemID: UInt32) {
            ...
            SGKCLDRAAAB.totalSupply = SGKCLDRAAAB.totalSupply + 1

            // set id
            self.id = SGKCLDRAAAB.totalSupply
    ...
    // interface that represents SGKCLDRAAAB collections to public
    ...
    pub resource interface CollectionPublic {
        ...
        // borrow NFT as SGKCLDRAAAB token
        pub fun borrowSGKCLDRAAABCollectible(id: UInt64): auth &NFT
    }
    ...
    pub resource Collection: CollectionPublic, NonFungibleToken.Receiver, NonFungibleToken.Provider, NonFungibleToken.CollectionPublic {
        ...
        pub fun deposit(token: @NonFungibleToken.NFT) {
            ...
            // Cast the deposited token as an SGKCLDRAAAB NFT to make sure
            // it is the correct type
            let token <- token as! @SGKCLDRAAAB.NFT
        ...
        // not any SGKCLDRAAAB specific data. Please use borrowCollectible to
        ...
        // borrowSGKCLDRAAABCollectible returns a borrowed reference
        // to an SGKCLDRAAAB Collectible
        pub fun borrowSGKCLDRAAABCollectible(id: UInt64): auth &NFT {
            pre {
    ...
    pub resource Admin {
        ...
        // and stores it in the Items dictionary field in the SGKCLDRAAAB smart contract
        //
        ...
        pub fun createItem(metadata: {String: String}): UInt32 {
            ...
            SGKCLDRAAAB.items[newItem.itemID] <-! newItem

            return itemId
        }

        // borrowItem returns a reference to a Item in the SGKCLDRAAAB
        ...
        pub fun borrowItem(itemID: UInt32): &Item {
            pre {
                SGKCLDRAAAB.items[itemID] != nil: "Cannot borrow Item: The Item doesn't exist"
            }

            return (&SGKCLDRAAAB.items[itemID] as &Item?)!
        }
    ...
    // -----------------------------------------------------------------------
    // SGKCLDRAAAB contract-level function definitions
    // -----------------------------------------------------------------------
    ...
    pub fun createEmptyCollection(): @NonFungibleToken.Collection {
        return <-create SGKCLDRAAAB.Collection()
    }
    ...
    // -----------------------------------------------------------------------
    // SGKCLDRAAAB initialization function
    // -----------------------------------------------------------------------
    //
    init() {
        ...
        self.collectionStoragePath     = /storage/SGKCLDRAAABCollection
        self.collectionPublicPath      =  /public/SGKCLDRAAABCollection
        self.collectionPrivatePath     = /private/SGKCLDRAAABCollection
        self.adminStoragePath          = /storage/SGKCLDRAAABAdmin
        self.saleCollectionStoragePath = /storage/SGKCLDRAAABSaleCollection
        self.saleCollectionPublicPath  =  /public/SGKCLDRAAABSaleCollection
