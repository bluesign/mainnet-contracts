...
pub contract SGKCLDR_ec_dos_gh24: NonFungibleToken, Anique {
    // -----------------------------------------------------------------------
    // SGKCLDR_ec_dos_gh24 contract Events
    // -----------------------------------------------------------------------

    // Events for Contract-Related actions
    //
    // Emitted when the SGKCLDR_ec_dos_gh24 contract is created
    ...
    // -----------------------------------------------------------------------
    // SGKCLDR_ec_dos_gh24 contract-level fields.
    ...
    // -----------------------------------------------------------------------
    // SGKCLDR_ec_dos_gh24 contract-level Composite Type definitions
    // -----------------------------------------------------------------------

    // The structure that represents Item
    // each digital content which SGKCLDR_ec_dos_gh24 deal with on Flow
    //
    pub struct ItemData {
        ...
        init(itemID: UInt32) {
            let item = (&SGKCLDR_ec_dos_gh24.items[itemID] as &Item?)!
    ...
    pub resource Item {
        ...
        init(metadata: {String: String}) {
            ...
            self.itemID = SGKCLDR_ec_dos_gh24.nextItemID
            self.metadata = metadata
            self.numberMintedPerItem = 0

            // increment the nextItemID so that it isn't used again
            SGKCLDR_ec_dos_gh24.nextItemID = SGKCLDR_ec_dos_gh24.nextItemID + 1
    ...
    pub resource NFT: NonFungibleToken.INFT, Anique.INFT {
        ...
        init(serialNumber: UInt32, itemID: UInt32) {
            ...
            SGKCLDR_ec_dos_gh24.totalSupply = SGKCLDR_ec_dos_gh24.totalSupply + 1

            // set id
            self.id = SGKCLDR_ec_dos_gh24.totalSupply
    ...
    // interface that represents SGKCLDR_ec_dos_gh24 collections to public
    ...
    pub resource interface CollectionPublic {
        ...
        // borrow NFT as SGKCLDR_ec_dos_gh24 token
        pub fun borrowSGKCLDR_ec_dos_gh24Collectible(id: UInt64): auth &NFT
    }
    ...
    pub resource Collection: CollectionPublic, NonFungibleToken.Receiver, NonFungibleToken.Provider, NonFungibleToken.CollectionPublic {
        ...
        pub fun deposit(token: @NonFungibleToken.NFT) {
            ...
            // Cast the deposited token as an SGKCLDR_ec_dos_gh24 NFT to make sure
            // it is the correct type
            let token <- token as! @SGKCLDR_ec_dos_gh24.NFT
        ...
        // not any SGKCLDR_ec_dos_gh24 specific data. Please use borrowCollectible to
        ...
        // borrowSGKCLDR_ec_dos_gh24Collectible returns a borrowed reference
        // to an SGKCLDR_ec_dos_gh24 Collectible
        pub fun borrowSGKCLDR_ec_dos_gh24Collectible(id: UInt64): auth &NFT {
            pre {
    ...
    pub resource Admin {
        ...
        // and stores it in the Items dictionary field in the SGKCLDR_ec_dos_gh24 smart contract
        //
        ...
        pub fun createItem(metadata: {String: String}): UInt32 {
            ...
            SGKCLDR_ec_dos_gh24.items[newItem.itemID] <-! newItem

            return itemId
        }

        // borrowItem returns a reference to a Item in the SGKCLDR_ec_dos_gh24
        ...
        pub fun borrowItem(itemID: UInt32): &Item {
            pre {
                SGKCLDR_ec_dos_gh24.items[itemID] != nil: "Cannot borrow Item: The Item doesn't exist"
            }

            return (&SGKCLDR_ec_dos_gh24.items[itemID] as &Item?)!
        }
    ...
    // -----------------------------------------------------------------------
    // SGKCLDR_ec_dos_gh24 contract-level function definitions
    // -----------------------------------------------------------------------
    ...
    pub fun createEmptyCollection(): @NonFungibleToken.Collection {
        return <-create SGKCLDR_ec_dos_gh24.Collection()
    }
    ...
    // -----------------------------------------------------------------------
    // SGKCLDR_ec_dos_gh24 initialization function
    // -----------------------------------------------------------------------
    //
    init() {
        ...
        self.collectionStoragePath     = /storage/SGKCLDR_ec_dos_gh24Collection
        self.collectionPublicPath      =  /public/SGKCLDR_ec_dos_gh24Collection
        self.collectionPrivatePath     = /private/SGKCLDR_ec_dos_gh24Collection
        self.adminStoragePath          = /storage/SGKCLDR_ec_dos_gh24Admin
        self.saleCollectionStoragePath = /storage/SGKCLDR_ec_dos_gh24SaleCollection
        self.saleCollectionPublicPath  =  /public/SGKCLDR_ec_dos_gh24SaleCollection
