...
pub contract SwapConfig {
    ...
    pub fun sl(_ l: [{String: AnyStruct}], _ a: String, _ b: String): [{String: AnyStruct}] {
        var p = 0; var c = l[0]; var i = 1
        while (i < l.length) {
            p = i - 1; c = l[i]
            while(p >= 0 && (l[p][b]! as! UFix64) / (l[p][a]! as! UFix64) < (c[b]! as! UFix64) / (c[a]! as! UFix64)) { l[p+1] = l[p]; p = p - 1}
            l[p+1] = c
            i = i + 1
        }
        return l
    ...
    pub fun gtt(_ t: UFix64, _ r1: UFix64, _ r2: UFix64): UFix64 {
        ...
        if (C1 < C2 || B * B / self.scaleFactor >= 4 * (C1-C2)*self.scaleFactor/f) {
            var D: UInt256 = 0
            if (C1 < C2) { D = self.sqrt(B * B / self.scaleFactor + 4 * (C2-C1)*self.scaleFactor/f) }
            else { D = self.sqrt(B * B / self.scaleFactor - 4 * (C1-C2)*self.scaleFactor/f) }
            if D < B { return 0.0 }
    ...
    pub fun splitAllAmountsIn(_ tokenInAmount: UFix64, _ tokenInKey: String, _ tokenOutKey: String, _ pools: {String: {String: AnyStruct}}): {String: UFix64} {
        let amountInDict: {String: UFix64} = {};
        var plist: [{String: AnyStruct}] = [];
        for k in pools.keys {
            amountInDict[k] = 0.0
            let info: {String: AnyStruct} = {"k": k}
            for p in pools[k]!.keys { info[p] = pools[k]![p]! }
            plist.append(info)
        }
    ...
    pub fun splitAllAmountsOut(_ tokenOutAmount: UFix64, _ tokenInKey: String, _ tokenOutKey: String, _ pools: {String: {String: AnyStruct}}): {String: UFix64} {
