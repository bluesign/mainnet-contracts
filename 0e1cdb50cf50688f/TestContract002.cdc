...
pub contract TestContract002: NonFungibleToken, Anique {
    // -----------------------------------------------------------------------
    // TestContract002 contract Events
    // -----------------------------------------------------------------------

    // Events for Contract-Related actions
    //
    // Emitted when the TestContract002 contract is created
    ...
    // -----------------------------------------------------------------------
    // TestContract002 contract-level fields.
    ...
    // -----------------------------------------------------------------------
    // TestContract002 contract-level Composite Type definitions
    // -----------------------------------------------------------------------

    // The structure that represents Item
    // each digital content which TestContract002 deal with on Flow
    //
    pub struct ItemData {
        ...
        init(itemID: UInt32) {
            let item = (&TestContract002.items[itemID] as &Item?)!
    ...
    pub resource Item {
        ...
        init(metadata: {String: String}) {
            ...
            self.itemID = TestContract002.nextItemID
            self.metadata = metadata
            self.numberMintedPerItem = 0

            // increment the nextItemID so that it isn't used again
            TestContract002.nextItemID = TestContract002.nextItemID + 1
    ...
    pub resource NFT: NonFungibleToken.INFT, Anique.INFT {
        ...
        init(serialNumber: UInt32, itemID: UInt32) {
            ...
            TestContract002.totalSupply = TestContract002.totalSupply + 1

            // set id
            self.id = TestContract002.totalSupply
    ...
    // interface that represents TestContract002 collections to public
    ...
    pub resource interface CollectionPublic {
        ...
        // borrow NFT as TestContract002 token
        pub fun borrowTestContract002Collectible(id: UInt64): auth &NFT
    }
    ...
    pub resource Collection: CollectionPublic, NonFungibleToken.Receiver, NonFungibleToken.Provider, NonFungibleToken.CollectionPublic {
        ...
        pub fun deposit(token: @NonFungibleToken.NFT) {
            ...
            // Cast the deposited token as an TestContract002 NFT to make sure
            // it is the correct type
            let token <- token as! @TestContract002.NFT
        ...
        // not any TestContract002 specific data. Please use borrowCollectible to
        ...
        // borrowTestContract002Collectible returns a borrowed reference
        // to an TestContract002 Collectible
        pub fun borrowTestContract002Collectible(id: UInt64): auth &NFT {
            pre {
    ...
    pub resource Admin {
        ...
        // and stores it in the Items dictionary field in the TestContract002 smart contract
        //
        ...
        pub fun createItem(metadata: {String: String}): UInt32 {
            ...
            TestContract002.items[newItem.itemID] <-! newItem

            return itemId
        }

        // borrowItem returns a reference to a Item in the TestContract002
        ...
        pub fun borrowItem(itemID: UInt32): &Item {
            pre {
                TestContract002.items[itemID] != nil: "Cannot borrow Item: The Item doesn't exist"
            }

            return (&TestContract002.items[itemID] as &Item?)!
        }
    ...
    // -----------------------------------------------------------------------
    // TestContract002 contract-level function definitions
    // -----------------------------------------------------------------------
    ...
    pub fun createEmptyCollection(): @NonFungibleToken.Collection {
        return <-create TestContract002.Collection()
    }
    ...
    // -----------------------------------------------------------------------
    // TestContract002 initialization function
    // -----------------------------------------------------------------------
    //
    init() {
        ...
        self.collectionStoragePath     = /storage/TestContract002Collection
        self.collectionPublicPath      =  /public/TestContract002Collection
        self.collectionPrivatePath     = /private/TestContract002Collection
        self.adminStoragePath          = /storage/TestContract002Admin
        self.saleCollectionStoragePath = /storage/TestContract002SaleCollection
        self.saleCollectionPublicPath  =  /public/TestContract002SaleCollection
