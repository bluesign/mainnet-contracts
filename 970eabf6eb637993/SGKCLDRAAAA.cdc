...
pub contract SGKCLDRAAAA: NonFungibleToken, Anique {
    // -----------------------------------------------------------------------
    // SGKCLDRAAAA contract Events
    // -----------------------------------------------------------------------

    // Events for Contract-Related actions
    //
    // Emitted when the SGKCLDRAAAA contract is created
    ...
    // -----------------------------------------------------------------------
    // SGKCLDRAAAA contract-level fields.
    ...
    // -----------------------------------------------------------------------
    // SGKCLDRAAAA contract-level Composite Type definitions
    // -----------------------------------------------------------------------

    // The structure that represents Item
    // each digital content which SGKCLDRAAAA deal with on Flow
    //
    pub struct ItemData {
        ...
        init(itemID: UInt32) {
            let item = (&SGKCLDRAAAA.items[itemID] as &Item?)!
    ...
    pub resource NFT: NonFungibleToken.INFT, Anique.INFT {
        ...
        init(serialNumber: UInt32, itemID: UInt32) {
            ...
            SGKCLDRAAAA.totalSupply = SGKCLDRAAAA.totalSupply + 1

            // set id
            self.id = SGKCLDRAAAA.totalSupply
    ...
    // interface that represents SGKCLDRAAAA collections to public
    ...
    pub resource interface CollectionPublic {
        ...
        // borrow NFT as SGKCLDRAAAA token
        pub fun borrowSGKCLDRAAAACollectible(id: UInt64): auth &NFT
    }
    ...
    pub resource Collection: CollectionPublic, NonFungibleToken.Receiver, NonFungibleToken.Provider, NonFungibleToken.CollectionPublic {
        ...
        pub fun deposit(token: @NonFungibleToken.NFT) {
            ...
            // Cast the deposited token as an SGKCLDRAAAA NFT to make sure
            // it is the correct type
            let token <- token as! @SGKCLDRAAAA.NFT
        ...
        // not any SGKCLDRAAAA specific data. Please use borrowCollectible to
        ...
        // borrowSGKCLDRAAAACollectible returns a borrowed reference
        // to an SGKCLDRAAAA Collectible
        pub fun borrowSGKCLDRAAAACollectible(id: UInt64): auth &NFT {
            pre {
    ...
    pub resource Admin {
        ...
        // and stores it in the Items dictionary field in the SGKCLDRAAAA smart contract
        //
        ...
        pub fun createItem(metadata: {String: String}): UInt32 {
            ...
            SGKCLDRAAAA.items[newItem.itemID] <-! newItem

            return itemId
        }

        // borrowItem returns a reference to a Item in the SGKCLDRAAAA
        ...
        pub fun borrowItem(itemID: UInt32): &Item {
            pre {
                SGKCLDRAAAA.items[itemID] != nil: "Cannot borrow Item: The Item doesn't exist"
            }

            return (&SGKCLDRAAAA.items[itemID] as &Item?)!
        }
    ...
    // -----------------------------------------------------------------------
    // SGKCLDRAAAA contract-level function definitions
    // -----------------------------------------------------------------------
    ...
    pub fun createEmptyCollection(): @NonFungibleToken.Collection {
        return <-create SGKCLDRAAAA.Collection()
    }
    ...
    // -----------------------------------------------------------------------
    // SGKCLDRAAAA initialization function
    // -----------------------------------------------------------------------
    //
    init() {
        ...
        self.collectionStoragePath     = /storage/SGKCLDRAAAACollection
        self.collectionPublicPath      =  /public/SGKCLDRAAAACollection
        self.collectionPrivatePath     = /private/SGKCLDRAAAACollection
        self.adminStoragePath          = /storage/SGKCLDRAAAAAdmin
        self.saleCollectionStoragePath = /storage/SGKCLDRAAAASaleCollection
        self.saleCollectionPublicPath  =  /public/SGKCLDRAAAASaleCollection
