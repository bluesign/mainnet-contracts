...
pub contract TouchstonePartyFlavorz: NonFungibleToken {
			...
			switch view {
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(
						storagePath: TouchstonePartyFlavorz.CollectionStoragePath,
						publicPath: TouchstonePartyFlavorz.CollectionPublicPath,
						providerPath: TouchstonePartyFlavorz.CollectionPrivatePath,
          ...
          return MetadataViews.ExternalURL("https://touchstone.city/discover/".concat(self.owner!.address.toString()).concat("/TouchstonePartyFlavorz"))
				case Type<MetadataViews.NFTCollectionDisplay>():
					let squareMedia = MetadataViews.Media(
						file: TouchstonePartyFlavorz.getCollectionAttribute(key: "image") as! MetadataViews.IPFSFile,
						mediaType: "image"
					)
					...
					return MetadataViews.NFTCollectionDisplay(
						name: TouchstonePartyFlavorz.getCollectionAttribute(key: "name") as! String,
						description: TouchstonePartyFlavorz.getCollectionAttribute(key: "description") as! String,
						externalURL: MetadataViews.ExternalURL("https://touchstone.city/discover/".concat(self.owner!.address.toString()).concat("/TouchstonePartyFlavorz")),
						squareImage: squareMedia,
						bannerImage: bannerMedia ?? squareMedia,
						socials: TouchstonePartyFlavorz.getCollectionAttribute(key: "socials") as! {String: MetadataViews.ExternalURL}
					)
	...
	pub resource Administrator {
		pub fun createNFTMetadata(name: String, description: String, imagePath: String, thumbnailPath: String?, ipfsCID: String, price: UFix64?, extra: {String: AnyStruct}, supply: UInt64) {
			TouchstonePartyFlavorz.metadatas[TouchstonePartyFlavorz.nextMetadataId] = NFTMetadata(
				_name: name,
		...
		pub fun mintNFT(metadataId: UInt64, serial: UInt64, recipient: Address) {
			pre {
				EmeraldPass.isActive(user: TouchstonePartyFlavorz.account.address): "You must have an active Emerald Pass subscription to airdrop NFTs. You can purchase Emerald Pass at https://pass.ecdao.org/"
			}
			...
			if let recipientCollection = getAccount(recipient).getCapability(TouchstonePartyFlavorz.CollectionPublicPath).borrow<&TouchstonePartyFlavorz.Collection{NonFungibleToken.CollectionPublic}>() {
				...
				if let storage = &TouchstonePartyFlavorz.nftStorage[recipient] as &{UInt64: NFT}? {
					storage[nft.id] <-! nft
				} else {
