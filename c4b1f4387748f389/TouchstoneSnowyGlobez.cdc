...
pub contract TouchstoneSnowyGlobez: NonFungibleToken {
			...
			switch view {
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(
						storagePath: TouchstoneSnowyGlobez.CollectionStoragePath,
						publicPath: TouchstoneSnowyGlobez.CollectionPublicPath,
						providerPath: TouchstoneSnowyGlobez.CollectionPrivatePath,
          ...
          return MetadataViews.ExternalURL("https://touchstone.city/discover/".concat(self.owner!.address.toString()).concat("/TouchstoneSnowyGlobez"))
				case Type<MetadataViews.NFTCollectionDisplay>():
					let squareMedia = MetadataViews.Media(
						file: TouchstoneSnowyGlobez.getCollectionAttribute(key: "image") as! MetadataViews.IPFSFile,
						mediaType: "image"
					)
					...
					return MetadataViews.NFTCollectionDisplay(
						name: TouchstoneSnowyGlobez.getCollectionAttribute(key: "name") as! String,
						description: TouchstoneSnowyGlobez.getCollectionAttribute(key: "description") as! String,
						externalURL: MetadataViews.ExternalURL("https://touchstone.city/discover/".concat(self.owner!.address.toString()).concat("/TouchstoneSnowyGlobez")),
						squareImage: squareMedia,
						bannerImage: bannerMedia ?? squareMedia,
						socials: TouchstoneSnowyGlobez.getCollectionAttribute(key: "socials") as! {String: MetadataViews.ExternalURL}
					)
	...
	pub resource Administrator {
		pub fun createNFTMetadata(name: String, description: String, imagePath: String, thumbnailPath: String?, ipfsCID: String, price: UFix64?, extra: {String: AnyStruct}, supply: UInt64) {
			TouchstoneSnowyGlobez.metadatas[TouchstoneSnowyGlobez.nextMetadataId] = NFTMetadata(
				_name: name,
		...
		pub fun mintNFT(metadataId: UInt64, serial: UInt64, recipient: Address) {
			pre {
				EmeraldPass.isActive(user: TouchstoneSnowyGlobez.account.address): "You must have an active Emerald Pass subscription to airdrop NFTs. You can purchase Emerald Pass at https://pass.ecdao.org/"
			}
			...
			if let recipientCollection = getAccount(recipient).getCapability(TouchstoneSnowyGlobez.CollectionPublicPath).borrow<&TouchstoneSnowyGlobez.Collection{NonFungibleToken.CollectionPublic}>() {
				...
				if let storage = &TouchstoneSnowyGlobez.nftStorage[recipient] as &{UInt64: NFT}? {
					storage[nft.id] <-! nft
				} else {
