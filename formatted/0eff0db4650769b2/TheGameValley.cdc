...
pub contract TheGameValley: FungibleToken, ViewResolver{ 
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, MetadataViews.Resolver{ 
        ...
        pub fun withdraw(amount: UFix64): @FungibleToken.Vault{ 
            ...
            if let owner: Address = self.owner?.address{ 
                TheGameValley.setBalance(address: owner, balance: self.balance)
            }
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                case Type<FungibleTokenMetadataViews.FTView>():
                    return TheGameValley.resolveView(view)
        ...
        destroy(){ 
            if self.balance > 0.0{ 
                ...
                TheGameValley.totalSupply = TheGameValley.totalSupply - self.balance
            }
        }
    }
    ...
    pub resource Minter: Toucans.Minter{ 
        pub fun mint(amount: UFix64): @Vault{ 
            post{ 
                TheGameValley.maxSupply == nil || TheGameValley.totalSupply <= TheGameValley.maxSupply!:
                    "Exceeded the max supply of tokens allowd."
            }
            TheGameValley.totalSupply = TheGameValley.totalSupply + amount
    ...
    pub fun resolveView(_ view: Type): AnyStruct?{ 
        switch view{ 
            ...
            case Type<FungibleTokenMetadataViews.FTDisplay>():
                let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreih4bte65ty7ke6uz4gasx37wpn6qmbrjkpsaa75ef4na3rfe7ildy"), mediaType: "image")
                let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreifb43kwtsuylnes47dez7pp6t7pp5zmikaka2damlz6orab6ruywu"), mediaType: "image")
                let medias = MetadataViews.Medias([media, bannerMedia])
                return FungibleTokenMetadataViews.FTDisplay(name: "The Game Valley", symbol: "GAME", description: "Explore the vast and beautiful game world of The Game Valley using the power of Flow BlockChain", externalURL: MetadataViews.ExternalURL("ecdao.org"), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("emerald_dao"), "discord": MetadataViews.ExternalURL("emerald-city")})
            case Type<FungibleTokenMetadataViews.FTVaultData>():
                return FungibleTokenMetadataViews.FTVaultData(storagePath: TheGameValley.VaultStoragePath, receiverPath: TheGameValley.ReceiverPublicPath, metadataPath: TheGameValley.VaultPublicPath, providerPath: /private/TheGameValleyVault, receiverLinkedType: Type<&Vault{FungibleToken.Receiver}>(), metadataLinkedType: Type<&Vault{FungibleToken.Balance, MetadataViews.Resolver}>(), providerLinkedType: Type<&Vault{FungibleToken.Provider}>(), createEmptyVaultFunction: fun (): @Vault{ 
                        return <-TheGameValley.createEmptyVault()
                    })
        }
        return nil
    }
    ...
    init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
        ...
        // Paths
        self.VaultStoragePath = /storage/TheGameValleyVault
        self.ReceiverPublicPath = /public/TheGameValleyReceiver
        self.VaultPublicPath = /public/TheGameValleyMetadata
        self.MinterStoragePath = /storage/TheGameValleyMinter
        self.AdministratorStoragePath = /storage/TheGameValleyAdmin
