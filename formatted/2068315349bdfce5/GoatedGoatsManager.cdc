...
pub contract GoatedGoatsManager{ 
    ...
    //  Trait
    // -----------------------------------------------------------------------
    priv let traitsMintedEditions:{ UInt64: Bool}
    
    priv var traitsSequentialMintMin: UInt64
    ...
    //  TraitPack
    // -----------------------------------------------------------------------
    priv let traitPacksMintedEditions:{ UInt64: Bool}
    
    priv let traitPacksByPackIdMintedEditions:{ UInt64:{ UInt64: Bool}}
    
    priv var traitPacksSequentialMintMin: UInt64
    
    priv var traitPacksByPackIdSequentialMintMin:{ UInt64: UInt64}
    ...
    //  Goat
    // -----------------------------------------------------------------------
    priv let goatsMintedEditions:{ UInt64: Bool}
    
    priv let goatsByGoatIdMintedEditions:{ UInt64: Bool}
    
    priv var goatsSequentialMintMin: UInt64
    ...
    pub resource Manager{ 
        ...
        pub fun updateTraitEditionMetadata(
            editionNumber: UInt64,
            metadata:{ 
                String: String
            }
        ){ 
            GoatedGoatsTrait.setEditionMetadata(
        ...
        pub fun mintTraitAtEdition(
            edition: UInt64,
            packID: UInt64
        ): @NonFungibleToken.NFT{ 
            emit AdminMintTrait(id: edition)
            return <-GoatedGoatsManager.mintTrait(
                edition: edition,
                packID: packID
            )
        }
        ...
        pub fun updateTraitPackEditionMetadata(
            editionNumber: UInt64,
            metadata:{ 
                String: String
            }
        ...
        pub fun mintTraitPackAtEdition(
            edition: UInt64,
            packID: UInt64,
            packEditionID: UInt64
        ): @NonFungibleToken.NFT{ 
        ...
        pub fun mintSequentialTraitPack(packID: UInt64): @NonFungibleToken.NFT{ 
            let trait <-
                GoatedGoatsManager.mintSequentialTraitPack(packID: packID)
        ...
        pub fun updateGoatEditionMetadata(
            goatID: UInt64,
            metadata:{ 
                String: String
            },
    ...
    priv fun mintSequentialTrait(packID: UInt64): @NonFungibleToken.NFT{ 
        ...
        let newTrait <-
            self.mintTrait(edition: UInt64(curEditionNumber), packID: packID)
        return <-newTrait
    }
    ...
    access(contract) fun mintTraitPack(
        edition: UInt64,
        packID: UInt64,
        packEditionID: UInt64
    ): @NonFungibleToken.NFT{ 
    ...
    priv fun mintSequentialTraitPack(packID: UInt64): @NonFungibleToken.NFT{ 
        // Grab the resource ID aka editionID
        var curEditionNumber = self.traitPacksSequentialMintMin
        while self.traitPacksMintedEditions.containsKey(
        ...
        var curPackEditionNumber =
            self.traitPacksByPackIdSequentialMintMin[packID]!
    ...
    //  TraitPack
    // -----------------------------------------------------------------------
    pub fun publicRedeemTraitPackWithVoucher(
        traitPackVoucher: @NonFungibleToken.NFT
    ...
    pub fun publicRedeemTraitPack(
        traitPack: @NonFungibleToken.NFT,
        address: Address
    ){ 
        pre{ 
            getCurrentBlock().timestamp >= self.traitPackRedeemStartTime:
    ...
    //  Goat
    // -----------------------------------------------------------------------
    pub fun publicRedeemGoatWithVoucher(
        goatVoucher: @NonFungibleToken.NFT,
        address: Address
    ): @NonFungibleToken.Collection{ 
        pre{ 
    ...
    pub resource GoatAndTraits{ 
        ...
        pub fun extractAllTraits(): @[NonFungibleToken.NFT]{ 
            ...
            assert(
                self.unequippedTraits.length == 0,
                message: "Couldn't extract all goats."
            )
            return <-assets
        }
        ...
        destroy(){ 
            ...
            assert(
                self.unequippedTraits.length == 0,
                message: "Can notdestroy with traits."
            )
            destroy self.goat
    ...
    pub fun updateGoatTraits(
        ...
        assert(
            newGoatInstance.traits.length
            <= Int(newGoatInstance.getTraitSlots()!),
        ...
        return <-create GoatAndTraits(
            goat: <-newGoatInstance,
            unequippedTraits: <-unequippedTraits
        )
    }
