...
import IBulkSales from "../0x9066631feda9e518/IBulkSales.cdc"
...
pub contract BulkPurchasing: IBulkSales{ 
    
    /// BulkPurchaseExecuted
    /// Event to notify when a user has executed a bulk purchase
    pub event BulkPurchaseExecuted(bulkOrder: ReadableBulkPurchaseOrder)
    ...
    pub let CommissionAdminPrivatePath: PrivatePath
    
    /// CommissionReaderPublicPath
    /// Public path for commission reader capability
    pub let CommissionReaderPublicPath: PublicPath
    
    /// CommissionReaderCapability
    /// Stored capability for commission reader
    pub let CommissionReaderCapability: Capability<&{IBulkSales.ICommissionReader}>
    
    /// DefaultExpirationDays
    /// Default number of days for a listing to expire
    pub var DefaultExpirationDays: UInt64
    
    /// PurchaseOrder
    /// All data necessary to purchase a single NFT listed for sale
    pub struct PurchaseOrder: IBulkSales.IReadable{ 
        ...
        pub fun getReadable():{ String: AnyStruct}{ 
            ...
            return{ "sellerAddress": self.ownerAddress.toString(), "listingID": self.listingID.toString(), "marketplaceType": storefrontType?.identifier ?? "nil", "buyerAddress": self.receiverAddress?.toString() ?? "nil", "salePrice": self.salePrice?.toString() ?? "nil", "saleCuts": self.saleCuts, "vaultIdentifier": self.vaultType?.identifier ?? "nil", "nftType": self.nftType?.identifier ?? "nil", "nftID": self.nftID?.toString() ?? "nil"}
        }
        ...
        access(contract) fun setStorefrontVersion(_ version: UInt8){ 
            pre{ 
                version == 1 || version == 2 || version == 11 || version == 13:
                    "storefront version must be 1, 2, 11 (TopShotMarketV1), or 13 (TopShotMarketV3)"
            }
            self.storefrontVersion = version
        }
        
        // Helper function to get the storefront type, used for getReadable functionality
        pub fun getStorefrontType(): Type?{ 
        ...
        access(contract) fun addSaleCut(recipient: Address?, amount: UFix64){ 
            pre{ 
                self.salePrice != nil:
                    "must set sale price before sale cuts"
                amount <= self.salePrice!:
                    "sale cut must be less than or equal to sale price"
            }
        ...
        access(contract) fun setVaultType(_ vaultType: Type){ 
            pre{ 
                vaultType.identifier.slice(from: vaultType.identifier.length - 5, upTo: vaultType.identifier.length) == "Vault":
                    "invalid vault type"
            }
        ...
        access(contract) fun setNftType(_ nftType: Type){ 
            pre{ 
                nftType.identifier.slice(from: nftType.identifier.length - 3, upTo: nftType.identifier.length) == "NFT":
                    "invalid NFT type"
            }
    ...
    pub struct ReadablePurchaseOrder{ 
        pub let sellerAddress: String
        
        pub let listingID: String
        
        pub let executed: String
        
        pub let marketplaceType: String
        
        pub let buyerAddress: String
        
        pub let salePrice: String
        
        pub let saleCuts:{ String: String}
        ...
        init(order: PurchaseOrder){ 
            ...
            self.marketplaceType = storefrontType?.identifier ?? "nil"
    ...
    /// A human-readable struct meant for use in the BulkPurchaseExecuted event
    pub struct ReadableBulkPurchaseOrder{ 
        pub let orders: [ReadablePurchaseOrder]
        
        // actual price totals of all executed orders
        pub let priceTotals:{ String: String}
    ...
    pub struct BulkPurchaseOrder: IBulkSales.IReadable{ 
        ...
        pub fun getReadable():{ String: AnyStruct}{ 
            ...
            return{ "orders": readableOrders, "priceTotals": self.priceTotals}
        }
        
        pub fun getPriceTotals():{ String: UFix64}{ 
            return self.priceTotals
        }
        
        // Helper function to add a sum to a vault total
        priv fun addToPriceTotal(vaultIdentifier: String, _ additionalAmount: UFix64){ 
        ...
        priv fun tabulateV1Listing(_ listing: &NFTStorefront.Listing{NFTStorefront.ListingPublic}, order: PurchaseOrder): PurchaseOrder{ 
        ...
        priv fun tabulateV2Listing(_ listing: &NFTStorefrontV2.Listing{NFTStorefrontV2.ListingPublic}, order: PurchaseOrder): PurchaseOrder{ 
            let listingDetails = listing.getDetails()
            assert(!listingDetails.purchased, message: "listing has already been purchased for ".concat(listingDetails.nftType.identifier))
            self.addToPriceTotal(vaultIdentifier: listingDetails.salePaymentVaultType.identifier, listingDetails.salePrice)
        ...
        priv fun tabulateTopShotListing(nftID: UInt64, salePublic: &{Market.SalePublic}, v3: Bool, order: PurchaseOrder): PurchaseOrder{ 
            pre{ 
                salePublic.cutPercentage >= 0.0 && salePublic.cutPercentage <= 1.0:
                    "cutPercentage must be between 0 and 1"
            }
        ...
        init(orders: [PurchaseOrder], unsafeMode: Bool?){ 
            ...
            for order in orders{ 
                ...
                if !self.topShotMarketV3Refs.containsKey(order.ownerAddress){ 
                    ...
                    if let marketV3 = getAccount(order.ownerAddress).getCapability(TopShotMarketV3.marketPublicPath).borrow<&TopShotMarketV3.SaleCollection{Market.SalePublic}>(){ 
                ...
                if !self.topShotMarketV1Refs.containsKey(order.ownerAddress){ 
                    ...
                    if let marketV1 = getAccount(order.ownerAddress).getCapability(/public/topshotSaleCollection).borrow<&Market.SaleCollection{Market.SalePublic}>(){ 
                ...
                if !self.storefrontV2Refs.containsKey(order.ownerAddress){ 
                    ...
                    let storefrontV2 = getAccount(order.ownerAddress).getCapability<&NFTStorefrontV2.Storefront{NFTStorefrontV2.StorefrontPublic}>(NFTStorefrontV2.StorefrontPublicPath).borrow()
                    if storefrontV2 != nil{ 
                ...
                if !self.storefrontV1Refs.containsKey(order.ownerAddress){ 
                    ...
                    let storefrontV1 = getAccount(order.ownerAddress).getCapability<&NFTStorefront.Storefront{NFTStorefront.StorefrontPublic}>(NFTStorefront.StorefrontPublicPath).borrow()
                    if storefrontV1 != nil{ 
    ...
    pub struct Royalty: IBulkSales.IRoyalty, IBulkSales.IReadable{ 
        ...
        pub fun getReadable():{ String: String}{ 
            return{ "receiverAddress": self.receiverAddress.toString(), "rate": self.rate.toString()}
        }
    ...
    pub resource interface PurchasingAdmin{ 
        pub fun toggleBulkPurchasing(_ value: Bool){} 
    }
    
    /// BulkPurchasingAdmin
    /// This object provides admin controls for commission receivers
    pub resource BulkPurchasingAdmin: PurchasingAdmin, IBulkSales.ICommissionAdmin, IBulkSales.ICommissionReader{ 
        
        // This contract's token receivers stored by vault type identifier
        priv let contractCommissionReceivers:{ String: Capability<&AnyResource{FungibleToken.Receiver}>}
        ...
        pub fun getCommissionReceiver(_ identifier: String): Capability<&AnyResource{FungibleToken.Receiver}>?{ 
            return self.contractCommissionReceivers[identifier]
        }
        
        // Add a commission receiver for sales facilitated by this contract
        pub fun addCommissionReceiver(_ receiver: Capability<&AnyResource{FungibleToken.Receiver}>){ 
    ...
    pub fun getCatalogEntryForNFT(nftTypeIdentifier: String, ownerAddress: Address?, nftID: UInt64?): NFTCatalog.NFTCatalogMetadata?{ 
    ...
    pub fun purchaseNFTs(bulkOrder: BulkPurchaseOrder, paymentVaultRefs:{ String: &FungibleToken.Vault}, nftReceiverCapabilities:{ String: Capability<&AnyResource{NonFungibleToken.Receiver}>}, expectedTotals:{ String: UFix64}?, preferredCommissionReceiverAddresses: [Address]?, unsafeMode: Bool?){ 
        pre{ 
            BulkPurchasing.AllowBulkPurchasing:
        ...
        for order in bulkOrder.orders{ 
            ...
            if order.storefrontVersion! == 11 || order.storefrontVersion! == 13{ 
                ...
                if bulkOrder.topShotMarketV3Refs[order.ownerAddress] != nil && (bulkOrder.topShotMarketV3Refs[order.ownerAddress]!).getIDs().contains(order.listingID){ 
                    topShotMarket = bulkOrder.topShotMarketV3Refs[order.ownerAddress]!
                } else if bulkOrder.topShotMarketV1Refs[order.ownerAddress] != nil && (bulkOrder.topShotMarketV1Refs[order.ownerAddress]!).getIDs().contains(order.listingID){ 
                ...
                assert(paymentVaultRef.getType() == Type<@DapperUtilityCoin.Vault>(), message: "payment vault type mismatch for ".concat(order.listingID.toString()))
                ...
                let receiverCapability = nftReceiverCapabilities[Type<@TopShot.NFT>().identifier] ?? panic("could not find receiver capability for TopShot")
                let receiver = receiverCapability.borrow() ?? panic("invalid or missing receiver for TopShot")
                ...
                let storefrontV2 = bulkOrder.storefrontV2Refs[order.ownerAddress] ?? panic("could not borrow NFTStorefrontV2 reference for ".concat(order.ownerAddress.toString()))
                ...
                if listingDetails.commissionAmount > 0.0{ 
                    if let allowedCommissionReceivers = (listing!).getAllowedCommissionReceivers(){ 
                        ...
                        for allowedReceiver in allowedCommissionReceivers{ 
                            ...
                            if commissionReceiverCapability == nil && preferredCommissionReceiverAddresses != nil && (preferredCommissionReceiverAddresses!).contains(allowedReceiver.address){ 
                                commissionReceiverCapability = allowedReceiver
                            }
                        }
                ...
                let storefrontV1 = bulkOrder.storefrontV1Refs[order.ownerAddress] ?? panic("could not borrow NFTStorefront reference for ".concat(order.ownerAddress.toString()))
