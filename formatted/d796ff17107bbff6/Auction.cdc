...
pub contract Auction{ 
    ...
    pub resource AuctionItem{ 
        ...
        access(contract) fun sendNFT(
            _ capability: Capability<&{Art.CollectionPublic}>
        ){ 
    ...
    pub resource AuctionCollection: AuctionPublic{ 
        ...
        pub fun createAuction(token:                                     
                                     // placeBid sends the bidder's tokens to the bid vault and updates the
                                     // currentPrice of the current auction item
                                     @Art.NFT, minimumBidIncrement: UFix64, auctionLength: UFix64                                                                                                 
                                                                                                 // Get the auction item resources
                                                                                                 , auctionStartTime: UFix64, startPrice: UFix64                                                                                                                                               // destroy the empty resources
                                                                                                                                               ...
                                                                                                                                               , collectionCap: Capability<&{Art.CollectionPublic}>, vaultCap: Capability<&{FungibleToken.Receiver}>){ 
            let                
                // create a new auction items resource container
                item <- Auction                               // createAuctionCollection returns a new AuctionCollection resource to the caller
                               .createStandaloneAuction(token: <-token, minimumBidIncrement: minimumBidIncrement, auctionLength: auctionLength, auctionStartTime: auctionStartTime, startPrice: startPrice, collectionCap: collectionCap, vaultCap: vaultCap)
