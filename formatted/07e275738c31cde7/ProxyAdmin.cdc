...
pub contract ProxyAdmin{ 
    ...
    pub resource MinterProxy: MinterProxyPublic{ 
        priv var minterCapability: Capability<&AFLPack.Pack>?
    ...
    init(){ 
        self.account.save(<-create MinterProxy(), to: /storage/proxy)
        self.account.link<&MinterProxy{MinterProxyPublic}>(
            /public/proxy,
