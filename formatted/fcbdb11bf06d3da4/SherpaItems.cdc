...
pub contract SherpaItems: NonFungibleToken{ 
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &SherpaItems.NFT?{ 
        let collection = (getAccount(from).getCapability(SherpaItems.CollectionPublicPath)!).borrow<&SherpaItems.Collection{SherpaItems.SherpaItemsCollectionPublic}>() ?? panic("Couldn't get collection")
