...
pub contract AFLMarketplace{ 
    ...
    access(contract) var marketplaceWallet: Capability<
    ...
    pub fun createSaleCollection(
        ownerVault: Capability<&FiatToken.Vault{FungibleToken.Receiver}>
    ): @SaleCollection{ 
    ...
    pub resource AFLMarketAdmin{ 
        ...
        pub fun changePercentage(_ newPercent: UFix64){ 
            ...
            emit CutPercentageChanged(
                newPercent: newPercent,
                owner: (self.owner!).address
            )
        }
        
        pub fun changeMarketplaceWallet(
            _ newCap: Capability<&FiatToken.Vault{FungibleToken.Receiver}>
        ){ 
    ...
    init(){ 
        ...
        self.account.save(
            <-create AFLMarketAdmin(),
            to: /storage/AFLMarketAdmin
        )
    }
}
