...
pub contract StorageHelper{ 
    ...
    pub fun availableAccountStorage(address: Address): UInt64{ 
        return getAccount(address).storageCapacity
        - getAccount(address).storageUsed
    }
    ...
    access(account) fun topUpAccount(address: Address){ 
        ...
        if StorageHelper.availableAccountStorage(address: address)
        > topUpThreshold{ 
            return
        }
        let vaultRef =
        ...
        let receiverRef =
            getAccount(address).getCapability(/public/flowTokenReceiver).borrow<
