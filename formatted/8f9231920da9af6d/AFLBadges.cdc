...
pub contract AFLBadges{ 
    
    /// @dev Every Template can have a list of badge ids associated with it
    access(contract) let badgeIdsByTemplateId:{ UInt64: [UInt64]}
    
    access(contract) let badges:{ UInt64: Badge}
    
    access(contract) var nextBadgeId: UInt64
    
    /// @dev AdminStoragePath is used to store the Admin resource in the contract account's storage
    pub let AdminStoragePath: StoragePath
    
    pub event BadgeCreated(
        id: UInt64,
        name: String,
        description: String,
        imageUrl: String
    ...
    pub event BadgeRemoved(id: UInt64)
    
    pub event BadgeAssignedToTemplate(badgeId: UInt64, templateId: UInt64)
    
    pub event BadgeRemovedFromTemplate(badgeId: UInt64, templateId: UInt64)
    ...
    pub fun getBadgesForTemplate(id: UInt64): [Badge]{ 
    ...
    pub fun getBadgesForTemplates(ids: [UInt64]):{ UInt64: [Badge]}{ 
    ...
    pub fun getTemplatesForBadge(id: UInt64): [UInt64]{ 
    ...
    pub resource Admin{ 
        ...
        pub fun addBadgeToTemplate(badgeId: UInt64, templateId: UInt64){ 
            pre{ 
        ...
        pub fun addBadgeToTemplates(badgeId: UInt64, templateIds: [UInt64]){ 
        ...
        pub fun removeBadgeFromTemplate(badgeId: UInt64, templateId: UInt64){ 
            pre{ 
                AFLBadges.badges[badgeId] != nil:
            ...
            (AFLBadges.badgeIdsByTemplateId[templateId]!).remove(
                at: (AFLBadges.badgeIdsByTemplateId[templateId]!).firstIndex(
                    of: badgeId
                )!
            )
            emit BadgeRemovedFromTemplate(
                badgeId: badgeId,
                templateId: templateId
            )
        }
        ...
        pub fun removeBadgeFromTemplates(
            badgeId: UInt64,
            templateIds: [
                UInt64
            ]
