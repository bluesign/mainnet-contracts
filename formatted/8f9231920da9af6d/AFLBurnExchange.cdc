...
pub contract AFLBurnExchange{ 
    priv let tokens: @{UInt64: NonFungibleToken.NFT} // nftId of token to be swapped -> new token 
    ...
    pub fun swap(token: @NonFungibleToken.NFT): @NonFungibleToken.NFT{ 
        ...
        // withdraw new token
        let newToken <-
            self.tokens.remove(key: token.id)
            ?? panic("No token found for exchange")
    ...
    // @token = new token
    access(account) fun addTokenForExchange(
        nftId: UInt64,
        token: @NonFungibleToken.NFT
    ){ 
        pre{ 
            self.tokens[nftId] == nil:
                "Token already exists: ".concat(nftId.toString())
        }
    ...
    access(account) fun withdrawToken(nftId: UInt64): @NonFungibleToken.NFT{ 
        pre{ 
            self.tokens[nftId] != nil:
                "No token found available in exchange for the provided token id: ".concat(nftId.toString())
        }
