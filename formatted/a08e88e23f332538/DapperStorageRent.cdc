...
import
    ...
    from "../0x18eb4ee6b3c026d2/PrivateReceiverForwarder.cdc"
...
pub contract DapperStorageRent{ 
    ...
    pub fun fundedRefillV2(
        address: Address,
        tokens: @FungibleToken.Vault
    ): @FungibleToken.Vault{ 
        let privateForwardingSenderRef =
            self.account.borrow<&PrivateReceiverForwarder.Sender>(
    ...
    pub fun tryRefill(_ address: Address){ 
        ...
        let receiverRef =
            recipient.getCapability<&PrivateReceiverForwarder.Forwarder>(
        ...
        - (self.RefilledAccountInfos[address]!).atBlock
        < self.RefillRequiredBlocks{ 
            emit RefilledFailed(
                address: address,
                reason: "RefillRequiredBlocks"
            )
            return
        }
        if recipient.storageUsed < recipient.storageCapacity
        && recipient.storageCapacity - recipient.storageUsed
        > self.StorageRentRefillThreshold{ 
            emit RefilledFailed(
                address: address,
    ...
    pub fun checkEligibility(_ address: Address): Bool{ 
        ...
        if acct.storageUsed < acct.storageCapacity
        && acct.storageCapacity - acct.storageUsed
        > self.StorageRentRefillThreshold{ 
            return false
        }
        return true
