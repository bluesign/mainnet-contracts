pub contract KabuniCoin{ 
    ...
    init(){ 
        ...
        self.account.link<&AnyResource{KabuniCoin.Provider}>(
            /public/provider,
    ...
    pub fun mintTokens(to: Address, amount: UFix64){ 
        ...
        let mainVaultRef =
            self.account.borrow<&KabuniCoin.Vault>(from: /storage/mainVault)
            ?? panic("Could not borrow main vault reference")
        let receiverRef =
            (
                getAccount(to).getCapability<&AnyResource{KabuniCoin.Receiver}>(
                    /public/receiver
                )!
            ).borrow()
