...
pub contract ToucansTokens{ 
    priv let tokens:{ Type: TokenInfo}
    ...
    pub struct TokenInfo{ 
        ...
        init(
            ...
            self.tokenType = CompositeType(
                    "A.".concat(
                        caToString.slice(from: 2, upTo: caToString.length)
                    ).concat(".".concat(cn)).concat(".Vault")
                )!
            self.symbol = s
        ...
        self.tokens ={ 
                ...
                TokenInfo(
                    ...
                    self.stringToAddress(
                        stringAddress: FlowToken.getType().identifier.slice(
                            from: 2,
                            upTo: 18
                        )
                    ),
                ...
                TokenInfo(
                    "FiatToken",
                    self.stringToAddress(
                        stringAddress: FiatToken.getType().identifier.slice(
                            from: 2,
                            upTo: 18
                        )
                    ),
