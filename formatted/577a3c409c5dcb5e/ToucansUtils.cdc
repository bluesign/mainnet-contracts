...
pub contract ToucansUtils{ 
    pub fun ownsNFTFromCatalogCollectionIdentifier(
        ...
        if let entry: NFTCatalog.NFTCatalogMetadata =
            ...
            let contractAddressToString: String =
                entry.contractAddress.toString()
            let constructedIdentifier: String =
                "A.".concat(
                    contractAddressToString.slice(
                        from: 2,
                        upTo: contractAddressToString.length
                    )
                ).concat(".").concat(entry.contractName).concat(".Collection")
            var addresses: [Address] = [user]
            if let discordID: String =
                EmeraldIdentity.getDiscordFromAccount(account: user){ 
                addresses = EmeraldIdentity.getEmeraldIDs(discordID: discordID)
                        .values
            }
            assert(
    ...
    pub fun depositTokensToAccount(
        funds: @FungibleToken.Vault,
        to: Address,
        publicPath: PublicPath
    ){ 
        let vault =
            getAccount(to).getCapability(publicPath).borrow<
    ...
    pub fun getAddressAndContractNameFromCollectionIdentifier(
        identifier: String
    ): [
        AnyStruct
    ]{ 
        let address: Address =
            self.stringToAddress(
                stringAddress: identifier.slice(from: 2, upTo: 18)
            )
        let contractName: String =
            identifier.slice(from: 19, upTo: identifier.length - 11)
    ...
    pub fun getEstimatedOut(amountIn: UFix64, tokenInKey: String): UFix64{ 
        // normal xyk pool
        let poolCapV1 =
            getAccount(0x396c0cda3302d8c5).getCapability<
                &{SwapInterfaces.PairPublic}
            >(/public/increment_swap_pair).borrow()!
        // stableswap pool with most liquidity
        let poolCapStable =
            getAccount(0xc353b9d685ec427d).getCapability<
                &{SwapInterfaces.PairPublic}
            >(/public/increment_swap_pair).borrow()!
        let estimatedSwapOutV1 =
            poolCapV1.getAmountOut(amountIn: amountIn, tokenInKey: tokenInKey)
        let estimatedSwapOutStable =
            poolCapStable.getAmountOut(
                amountIn: amountIn,
    ...
    pub fun swapTokensWithPotentialStake(
        ...
        let estimatedSwapOut =
            estimatedSwapOutStable > estimatedSwapOutV1
                ? estimatedSwapOutStable
                : estimatedSwapOutV1
        let estimatedStakeOut =
            LiquidStaking.calcStFlowFromFlow(flowAmount: amountIn)
        if tokenInKey == "A.1654653399040a61.FlowToken"
        && estimatedStakeOut > estimatedSwapOut{ 
            return <-LiquidStaking.stake(
                flowVault: <-(inVault as! @FlowToken.Vault)
            )
        }
        return <-estimatedSwapPoolCap.swap(
            vaultIn: <-inVault,
            exactAmountOut: nil
        )
    }
    
    pub fun getNFTCatalogCollectionIdentifierFromCollectionIdentifier(
        collectionIdentifier: String
    ): String{ 
        let nftTypeIdentifier: String =
            collectionIdentifier.slice(
                from: 0,
                upTo: collectionIdentifier.length - 10
            ).concat("NFT")
