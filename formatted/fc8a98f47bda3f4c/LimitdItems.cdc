...
pub contract LimitdItems: NonFungibleToken{ 
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &LimitdItems.NFT?{ 
        let collection = (getAccount(from).getCapability(LimitdItems.CollectionPublicPath)!).borrow<&LimitdItems.Collection{LimitdItems.LimitdItemsCollectionPublic}>() ?? panic("Couldn't get collection")
