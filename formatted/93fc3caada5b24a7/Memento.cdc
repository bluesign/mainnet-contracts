...
pub contract Memento: NonFungibleToken, ViewResolver{ 
    ...
    pub event Minted(id: UInt64, serial: UInt64, recipient: Address, creatorID: String)
    
    pub event MetadataSuccess(creatorID: String, description: String)
    ...
    pub resource MetadataStorage: MetadataStoragePublic{ 
        // List of Creator 
        pub var creatorsIds:{ String: [NFTMetadata]}
        
        init(){ 
            self.creatorsIds ={} 
        }
        
        access(account) fun creatorExist(_ creatorId: String){ 
            if self.creatorsIds[creatorId] == nil{ 
                self.creatorsIds[creatorId] = []
            }
        }
        
        access(account) fun metadataIsNew(_ creatorId: String, _ description: String): Bool{ 
        ...
        access(account) fun addMetadata(_ creatorId: String, _ metadata: NFTMetadata){ 
            if self.creatorsIds[creatorId] == nil{ 
                self.creatorsIds[creatorId] = []
            }
        ...
        access(account) fun updateMinted(_ creatorId: String, _ description: String){ 
            let metadataRef = self.findMetadataRef(creatorId, description)!
            metadataRef.updateMinted()
        }
        
        // Public Functions
        pub fun findMetadataRef(_ creatorId: String, _ description: String): &Memento.NFTMetadata?{ 
        ...
        // Public Functions
        pub fun findMetadata(_ creatorId: String, _ description: String): Memento.NFTMetadata?{ 
            let metadatas = self.creatorsIds[creatorId]!
            var i = metadatas.length - 1
            while i >= 0{ 
                if metadatas[i].description == description{ 
        ...
        pub fun getTimeRemaining(_ creatorID: String, _ description: String): UFix64?{ 
            let metadata = self.findMetadata(creatorID, description)!
    ...
    pub struct NFTMetadata{ 
        ...
        init(_creatorID: String, _creatorUsername: String, _creatorAddress: Address, _description: String, _image: MetadataViews.HTTPFile, _supply: UInt64, _extra:{ String: AnyStruct}, _MementoCreationDate: String, _contentCreationDate: String, _currentTime: UFix64, _lockdownTime: UFix64, _embededHTML: String){ 
            self.metadataId = 0
    ...
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver{ 
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            ...
            switch view{ 
                case Type<MetadataViews.Display>():
                    return MetadataViews.Display(creatorID: metadata.creatorUsername.concat(" ").concat(metadata.contentCreationDate), description: metadata.description, thumbnail: metadata.image)
                ...
                case Type<MetadataViews.Medias>():
                    if metadata.embededHTML != nil{ 
                        return MetadataViews.Medias(items: [MetadataViews.Media(file: MetadataViews.HTTPFile(url: metadata.embededHTML), mediaType: "html")])
                    }
                case Type<MetadataViews.Royalties>():
                    return MetadataViews.Royalties([MetadataViews.Royalty(recepient: getAccount(metadata.creatorAddress).getCapability<&FlowToken.Vault{FungibleToken.Receiver}>(/public/flowTokenReceiver), cut: 0.10, // 10% royalty on secondary sales description: "The creator of the original content gets 10% of every secondary sale.")])
        ...
        init(_creatorID: String, _description: String, _recipient: Address){ 
    ...
    pub resource Administrator{ 
        ...
        pub fun createNFTMetadata(channel: String, creatorID: String, creatorUsername: String, creatorAddress: Address, sourceURL: String, description: String, MementoCreationDate: String, contentCreationDate: String, lockdownOption: Int, supplyOption: UInt64, imgUrl: String, embededHTML: String){ 
        ...
        pub fun mintNFT(creatorId: String, description: String, recipient: Address){ 
            pre{ 
            ...
            } else if let storage = &Memento.nftStorage[recipient] as &{UInt64: NFT}?{ 
                storage[nft.id] <-! nft
            } else{ 
        ...
        access(account) fun isMintingAvailable(_ creatorId: String, _ description: String): Bool{ 
        ...
        access(account) fun getLockdownTime(_ lockdownOption: Int): UFix64{ 
            switch lockdownOption{ 
                ...
                case 3:
                    return 172800.0 // 48 Hours
                default:
                    return 0.0 // No lockdown time
            }
        }
    }
    ...
    pub fun resolveView(_ view: Type): AnyStruct?{ 
        switch view{ 
            case Type<MetadataViews.NFTCollectionData>():
                return MetadataViews.NFTCollectionData(storagePath: Memento.CollectionStoragePath, publicPath: Memento.CollectionPublicPath, providerPath: Memento.CollectionPrivatePath, publicCollection: Type<&Memento.Collection{Memento.MementoCollectionPublic}>(), publicLinkedType: Type<&Memento.Collection{Memento.MementoCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&Memento.Collection{Memento.MementoCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Provider, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
    ...
    pub fun getNFTMetadata(_ creatorId: String, _ description: String): Memento.NFTMetadata?{ 
    ...
    pub fun isMintingAvailable(_ creatorId: String, _ description: String): Bool{ 
        let metadata = Memento.getNFTMetadata(creatorId, description)!
        if metadata.unlimited{ 
            if metadata.lockdownTime != 0.0{ 
                let answer = getCurrentBlock().timestamp <= metadata.creationTime + metadata.lockdownTime
    ...
    init(){ 
        ...
        self.account.link<&MetadataStorage{MetadataStoragePublic}>(self.MetadataPublicPath, target: self.MetadataStoragePath)
