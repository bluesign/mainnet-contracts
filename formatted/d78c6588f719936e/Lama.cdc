...
import
    ...
    from "../0xea86b9b77d95aeea/FungibleTokenMetadataViews.cdc"
...
pub contract Lama: ViewResolver{ 
    ...
    pub resource Allowance: ParentAccess, ChildAccess{ 
        priv var allowances:{ PrivatePath: UFix64}
        
        priv var collected:{ PrivatePath: UFix64}
        
        priv var capabilities:{ PrivatePath: Capability<&AnyResource{FungibleToken.Provider, FungibleToken.Balance}>}
        ...
        pub fun collect(path: PrivatePath, receiverPath: PublicPath, receiver: Address){ 
            let childProvider: Capability<&AnyResource{FungibleToken.Provider, FungibleToken.Balance}> = self.capabilities[path] ?? panic("FungibleToken.Provider capability not found for provider path")
            let childVault: &AnyResource{FungibleToken.Provider, FungibleToken.Balance} = childProvider.borrow() ?? panic("Could not borrow FungibleToken.Provider")
            let parentVault: &AnyResource{FungibleToken.Receiver} = getAccount(receiver).getCapability<&AnyResource{FungibleToken.Receiver}>(receiverPath).borrow() ?? panic("Problem getting parent receiver for this public path")
        ...
        priv fun _setAllowance(path: PrivatePath, allowance: UFix64){ 
