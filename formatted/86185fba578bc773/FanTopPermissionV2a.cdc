...
pub contract FanTopPermissionV2a{ 
    ...
    pub struct User{ 
        pub fun sell(
            ...
            var signedData =
                agent.toBytes().concat(capability.address.toBytes()).concat(
                    orderId.utf8
                ).concat(refId.utf8).concat(nftId.toBigEndianBytes()).concat(
                    version.toBigEndianBytes()
                )
        ...
        pub fun cancel(account: AuthAccount, orderId: String){ 
            pre{ 
