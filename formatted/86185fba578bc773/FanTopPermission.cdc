...
pub contract FanTopPermission{ 
    ...
    pub resource Owner{ 
        ...
        pub fun addAdmin(receiver: &AnyResource{Receiver}){ 
            ...
            let capability =
                FanTopPermission.addPermission(recipient, as: Role.admin)
                as!
                Capability<&Admin>
    ...
    pub resource Admin{ 
        pub fun addOperator(receiver: &AnyResource{Receiver}){ 
            ...
            let recipient = (receiver.owner!).address
            let capability =
                FanTopPermission.addPermission(recipient, as: Role.operator)
                as!
                Capability<&Operator>
        ...
        pub fun addMinter(receiver: &AnyResource{Receiver}){ 
            ...
            let recipient = (receiver.owner!).address
            let capability =
                FanTopPermission.addPermission(recipient, as: Role.minter)
                as!
                Capability<&Minter>
    ...
    pub resource Holder: Receiver{ 
        ...
        priv let roles:{ Role: Capability}
        ...
        pub fun receive(as: Role, capability: Capability){ 
            pre{ 
                !self.roles.containsKey(as):
                    "Holder cannot receive roles it already owns"
            }
        ...
        pub fun borrowAdmin(by: AuthAccount): &Admin{ 
            pre{ 
                self.isValid():
                    "Invalid holder cannot be used"
                self.recipient == by.address:
                    "Only the owner can borrow"
                self.roles.containsKey(Role.admin):
                    "Roles not given cannot be borrowed"
                FanTopPermission.hasPermission(by.address, as: Role.admin):
                    "Roles without permission cannot be used"
            }
        ...
        pub fun borrowOperator(by: AuthAccount): &Operator{ 
            pre{ 
                ...
                self.recipient == by.address:
                    "Only the owner can borrow"
                self.roles.containsKey(Role.operator):
                    "Roles not given cannot be borrowed"
                FanTopPermission.hasPermission(by.address, as: Role.operator):
                    "Roles without permission cannot be used"
            }
        ...
        pub fun borrowMinter(by: AuthAccount): &Minter{ 
            pre{ 
                ...
                self.recipient == by.address:
                    "Only the owner can borrow"
                self.roles.containsKey(Role.minter):
                    "Roles not given cannot be borrowed"
                FanTopPermission.hasPermission(by.address, as: Role.minter):
                    "Roles without permission cannot be used"
            }
    ...
    priv let capabilities:{ Role: Capability}
    
    priv fun addPermission(_ address: Address, as: Role): Capability{ 
        pre{ 
            !FanTopPermission.hasPermission(address, as: as):
                "Existing roles are not added"
        }
    ...
    priv fun removePermission(_ address: Address, as: Role){ 
        pre{ 
            FanTopPermission.hasPermission(address, as: as):
                "Roles that do not exist cannot be removed"
        }
    ...
    init(){ 
        ...
        self.account.save<@Operator>(
            <-create Operator(),
            to: /storage/FanTopOperator
        )
        self.account.save<@Minter>(<-create Minter(), to: /storage/FanTopMinter)
        self.capabilities ={} 
        self.capabilities[Role.admin] = self.account.link<&Admin>(
                /private/FanTopAdmin,
                target: /storage/FanTopAdmin
            )!
        self.capabilities[Role.operator] = self.account.link<&Operator>(
                /private/FanTopOperator,
                target: /storage/FanTopOperator
            )!
        self.capabilities[Role.minter] = self.account.link<&Minter>(
                /private/FanTopMinter,
                target: /storage/FanTopMinter
            )!
    }
}
