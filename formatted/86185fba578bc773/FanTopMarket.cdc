...
pub contract FanTopMarket{ 
    pub event CapacityExtended(by: Address, capacity: Int)
    ...
    pub struct SellOrder{ 
        ...
        access(contract) fun withdraw(): @FanTopToken.NFT{ 
            let token <-
                (self.capability.borrow()!).withdraw(withdrawID: self.nftId)
                as!
                @FanTopToken.NFT
            return <-token
        }
    ...
    access(account) fun fulfill(
        agent: Address,
        orderId: String,
        version: UInt32,
        recipient: &AnyResource{FanTopToken.CollectionPublic}
    ){ 
        pre{ 
            self.containsOrder(orderId):
