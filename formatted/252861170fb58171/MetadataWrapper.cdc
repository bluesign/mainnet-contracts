...
pub contract MetadataWrapper{ 
        ...
        switch view{ 
            case Type<MetadataViews.Display>():
                return MetadataViews.Display(
                    ...
                    thumbnail: MetadataViews.HTTPFile(
                        url: attributes["_display.thumbnail"]! as! String
                    )
                )
            case Type<MetadataViews.Medias>():
                if let medias = attributes["_medias"]{ 
                    ...
                    return MetadataViews.Medias(items: items)
                }
                return nil
            ...
            case Type<MetadataViews.NFTView>():
                return MetadataViews.NFTView(
                    ...
                    display: self.buildView(
                        Type<MetadataViews.Display>(),
                        attributes
                    )
                    as!
                    MetadataViews.Display?,
                    externalURL: self.buildView(
                        Type<MetadataViews.ExternalURL>(),
                        attributes
                    )
                    as!
                    MetadataViews.ExternalURL?,
                    collectionData: self.buildView(
                        Type<MetadataViews.NFTCollectionData>(),
                        attributes
                    )
                    as!
                    MetadataViews.NFTCollectionData?,
                    collectionDisplay: self.buildView(
                        Type<MetadataViews.NFTCollectionDisplay>(),
                        attributes
                    )
                    as!
                    MetadataViews.NFTCollectionDisplay?,
                    royalties: self.buildView(
                        Type<MetadataViews.Royalties>(),
                        attributes
                    )
                    as!
                    MetadataViews.Royalties?,
                    traits: self.buildView(
                        Type<MetadataViews.Traits>(),
                        attributes
                    )
                    as!
    ...
    pub fun resolveViewsByPath(
        ...
        if let wrapper =
            getAccount(self.account.address).getCapability(path).borrow<
                &{WrapperInterface}
            >(){ 
            for id in ids{ 
    ...
    pub fun resolveViews(
        ...
        var res:{ UInt64: [AnyStruct]} ={} 
        if let wrapper =
            self.account.borrow<&{WrapperInterface}>(
                from: StoragePath(identifier: type)!
            ){ 
            for id in ids{ 
                wrapper.setData(address: address, id: id)
                var v: [AnyStruct] = []
