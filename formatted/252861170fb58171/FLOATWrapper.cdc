...
pub contract FLOATWrapper{ 
    pub fun getRef(_ account: Address, _ id: UInt64): &FLOAT.NFT?{ 
        if let collection =
            getAccount(account).getCapability(FLOAT.FLOATCollectionPublicPath)
    ...
    pub fun getNFTAttributes(_ float: &FLOAT.NFT?):{ String: AnyStruct}{ 
        ...
        return{ 
            ...
            //externalURL
            "_externalURL":
            "https://floats.city/".concat(
                (((float!).owner!).address as Address).toString()
    ...
    pub fun setup(){ 
        ...
        self.account.link<&{MetadataWrapper.WrapperInterface}>(
            /public/FLOAT,
            target: /storage/FLOAT
        )
