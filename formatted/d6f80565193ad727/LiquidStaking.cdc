...
pub contract LiquidStaking{ 
    ...
    pub event Stake(
        flowAmountIn: UFix64,
        stFlowAmountOut: UFix64,
        currProtocolEpoch: UInt64
    )
    
    pub event Unstake(
        stFlowAmountIn: UFix64,
        lockedFlowAmount: UFix64,
        currProtocolEpoch: UInt64,
        unlockProtocolEpoch: UInt64,
        voucherUUID: UInt64
    )
    
    pub event UnstakeQuickly(
        stFlowAmountIn: UFix64,
        flowAmountOut: UFix64,
        currProtocolEpoch: UInt64
    )
    
    pub event MigrateDelegator(
        uuid: UInt64,
        migratedFlowIn: UFix64,
        stFlowOut: UFix64
    )
    
    pub event BurnWithdrawVoucher(
        uuid: UInt64,
        amountFlowCashedout: UFix64,
        currProtocolEpoch: UInt64
    )
    ...
    pub fun stake(flowVault: @FlowToken.Vault): @stFlowToken.Vault{ 
        pre{ 
            // Pause check
            LiquidStakingConfig.isStakingPaused == false:
                LiquidStakingError.ErrorEncode(msg: "Staking is paused", err: LiquidStakingError.ErrorCode.STAKE_NOT_OPEN)
            // Flow blockchain staking state check
            FlowIDTableStaking.stakingEnabled():
                LiquidStakingError.ErrorEncode(msg: "Cannot stake as not in flowchain's staking auction period", err: LiquidStakingError.ErrorCode.STAKING_AUCTION_NOT_IN_PROGRESS)
            // Protocol epoch sync check
            FlowEpoch.currentEpochCounter == DelegatorManager.quoteEpochCounter:
                LiquidStakingError.ErrorEncode(msg: "Cannot stake until protocol epoch syncs with chain epoch", err: LiquidStakingError.ErrorCode.QUOTE_EPOCH_EXPIRED)
            // Min staking amount check
            flowVault.balance >= LiquidStakingConfig.minStakingAmount:
                LiquidStakingError.ErrorEncode(msg: "Stake amount must be greater than ".concat(LiquidStakingConfig.minStakingAmount.toString()), err: LiquidStakingError.ErrorCode.INVALID_PARAMETERS)
            // Staking amount cap check
            LiquidStakingConfig.stakingCap >= flowVault.balance + DelegatorManager.getTotalValidStakingAmount():
                LiquidStakingError.ErrorEncode(msg: "Exceed stake cap: ".concat(LiquidStakingConfig.stakingCap.toString()), err: LiquidStakingError.ErrorCode.EXCEED_STAKE_CAP)
        }
        let flowAmountToStake = flowVault.balance
        let stFlowAmountToMint =
            self.calcStFlowFromFlow(flowAmount: flowAmountToStake)
        ...
        emit Stake(
            flowAmountIn: flowAmountToStake,
            stFlowAmountOut: stFlowAmountToMint,
            currProtocolEpoch: DelegatorManager.quoteEpochCounter
        )
        return <-stFlowVault
    }
    ...
    pub fun unstake(stFlowVault: @stFlowToken.Vault): @WithdrawVoucher{ 
        pre{ 
            // Pause check
            LiquidStakingConfig.isUnstakingPaused == false:
                LiquidStakingError.ErrorEncode(msg: "Unstaking is paused", err: LiquidStakingError.ErrorCode.UNSTAKE_NOT_OPEN)
            // Unstaking amount check
            stFlowVault.balance > 0.0:
                LiquidStakingError.ErrorEncode(msg: "Unstake amount must be greater than 0.0", err: LiquidStakingError.ErrorCode.INVALID_PARAMETERS)
        }
        let stFlowAmountToBurn = stFlowVault.balance
        let flowAmountToUnstake =
            self.calcFlowFromStFlow(stFlowAmount: stFlowAmountToBurn)
        ...
        let stakingEndView =
            (FlowEpoch.getEpochMetadata(FlowEpoch.currentEpochCounter)!)
                .stakingEndView
        ...
        } else if FlowIDTableStaking.stakingEnabled() == false{ 
        ...
        let unstakeVoucher <-
            create WithdrawVoucher(
                lockedFlowAmount: flowAmountToUnstake,
                unlockEpoch: unlockEpoch
            )
        emit Unstake(
            stFlowAmountIn: stFlowAmountToBurn,
            lockedFlowAmount: flowAmountToUnstake,
            currProtocolEpoch: protocolEpoch,
            unlockProtocolEpoch: unlockEpoch,
            voucherUUID: unstakeVoucher.uuid
        )
    ...
    pub fun unstakeQuickly(stFlowVault: @stFlowToken.Vault): @FlowToken.Vault{ 
        pre{ 
            ...
            FlowIDTableStaking.stakingEnabled():
                LiquidStakingError.ErrorEncode(msg: "Cannot unstake as not in flowchain's staking auction period", err: LiquidStakingError.ErrorCode.STAKING_AUCTION_NOT_IN_PROGRESS)
            // Protocol epoch sync check
            FlowEpoch.currentEpochCounter == DelegatorManager.quoteEpochCounter:
                LiquidStakingError.ErrorEncode(msg: "Cannot unstake until protocol epoch syncs with chain epoch", err: LiquidStakingError.ErrorCode.QUOTE_EPOCH_EXPIRED)
            // Unstaking amount check
            stFlowVault.balance > 0.0:
                LiquidStakingError.ErrorEncode(msg: "Unstake amount must be greater than 0.0", err: LiquidStakingError.ErrorCode.INVALID_PARAMETERS)
        }
        ...
        let flowVault <-
            DelegatorManager.withdrawFromCommitted(amount: flowAmountToUnstake)
        
        // Deduct fast unstake protocol fee
        let feeVault <-
            flowVault.withdraw(
                amount: LiquidStakingConfig.quickUnstakeFee
                * flowAmountToUnstake
            )
        DelegatorManager.depositToProtocolFees(
            flowVault: <-feeVault,
            purpose: "fast unstake fee -> protocol fee"
        )
        emit UnstakeQuickly(
            stFlowAmountIn: stFlowAmountToBurn,
            flowAmountOut: flowVault.balance,
            currProtocolEpoch: DelegatorManager.quoteEpochCounter
        )
        return <-flowVault
    }
    ...
    pub fun cashoutWithdrawVoucher(
        voucher: @WithdrawVoucher
    ): @FlowToken.Vault{ 
        pre{ 
            DelegatorManager.quoteEpochCounter >= voucher.unlockEpoch:
                LiquidStakingError.ErrorEncode(msg: "Voucher is not redeemable yet", err: LiquidStakingError.ErrorCode.CANNOT_CASHOUT_WITHDRAW_VOUCHER)
        }
        let cashoutAmount = voucher.lockedFlowAmount
        let flowVault <-
            DelegatorManager.withdrawFromUnstaked(amount: cashoutAmount)
        emit BurnWithdrawVoucher(
            uuid: voucher.uuid,
            amountFlowCashedout: cashoutAmount,
            currProtocolEpoch: DelegatorManager.quoteEpochCounter
        )
        destroy voucher
        return <-flowVault
    ...
    pub fun migrate(
        ...
        pre{ 
            // Pause check
            LiquidStakingConfig.isMigratingPaused == false:
                LiquidStakingError.ErrorEncode(msg: "Migrating is paused", err: LiquidStakingError.ErrorCode.MIGRATE_NOT_OPEN)
            // Flowchain staking state check
            FlowIDTableStaking.stakingEnabled():
                LiquidStakingError.ErrorEncode(msg: "Cannot migrate as not in flowchain's staking auction period", err: LiquidStakingError.ErrorCode.STAKING_AUCTION_NOT_IN_PROGRESS)
            // Protocol epoch sync check
            FlowEpoch.currentEpochCounter == DelegatorManager.quoteEpochCounter:
                LiquidStakingError.ErrorEncode(msg: "Cannot migrate until protocol epoch syncs with chain epoch", err: LiquidStakingError.ErrorCode.QUOTE_EPOCH_EXPIRED)
        }
        let delegatroInfo =
        ...
        assert(
            LiquidStakingConfig.stakingCap
            >= delegatroInfo.tokensStaked
            + DelegatorManager.getTotalValidStakingAmount(),
            message: "Exceed stake cap"
        )
        assert(
            delegatroInfo.tokensUnstaking == 0.0,
            message: "Wait for the previous unstaking processing to complete"
        )
        assert(
            delegatroInfo.tokensRewarded == 0.0,
            message: "Please claim the reward before migrating"
        )
        assert(
            delegatroInfo.tokensUnstaked == 0.0,
            message: "Please withdraw the unstaked tokens before migrating"
        )
        assert(
            delegatroInfo.tokensRequestedToUnstake == 0.0,
            message: "Please cancel the unstake requests before migrating"
        )
        assert(
            delegatroInfo.tokensCommitted == 0.0,
            message: "Please cancel the stake requests before migrating"
        )
        assert(
            delegatroInfo.tokensStaked > 0.0,
            message: "No staked tokens need to be migrated."
        )
        let stakedFlowToMigrate = delegatroInfo.tokensStaked
        let stFlowAmountToMint =
            self.calcStFlowFromFlow(flowAmount: stakedFlowToMigrate)
        emit MigrateDelegator(
            uuid: delegator.uuid,
            migratedFlowIn: stakedFlowToMigrate,
            stFlowOut: stFlowAmountToMint
        )
    ...
    pub resource WithdrawVoucherCollection: WithdrawVoucherCollectionPublic{ 
        ...
        priv var vouchers: @[WithdrawVoucher]
        ...
        pub fun getVoucherInfos(): [AnyStruct]{ 
            ...
            var index = 0
            while index < self.vouchers.length{ 
                voucherInfos.append({"uuid": self.vouchers[index].uuid, "lockedFlowAmount": self.vouchers[index].lockedFlowAmount, "unlockEpoch": self.vouchers[index].unlockEpoch})
                index = index + 1
            }
            return voucherInfos
        }
    ...
    pub fun calcStFlowFromFlow(flowAmount: UFix64): UFix64{ 
        let currentEpochSnapshot =
            DelegatorManager.borrowCurrentQuoteEpochSnapshot()
        let scaledFlowPrice = currentEpochSnapshot.scaledQuoteFlowStFlow
        let scaledFlowAmount =
            LiquidStakingConfig.UFix64ToScaledUInt256(flowAmount)
        let stFlowAmount =
            LiquidStakingConfig.ScaledUInt256ToUFix64(
                scaledFlowPrice * scaledFlowAmount
                / LiquidStakingConfig.scaleFactor
            )
        return stFlowAmount
    }
    ...
    pub fun calcFlowFromStFlow(stFlowAmount: UFix64): UFix64{ 
        ...
        let scaledStFlowAmount =
            LiquidStakingConfig.UFix64ToScaledUInt256(stFlowAmount)
        let flowAmount =
            LiquidStakingConfig.ScaledUInt256ToUFix64(
                scaledStFlowPrice * scaledStFlowAmount
                / LiquidStakingConfig.scaleFactor
            )
        return flowAmount
    }
    ...
    init(){ 
        self
            .WithdrawVoucherCollectionPath = /storage/liquid_staking_withdraw_voucher_collection
        self
            .WithdrawVoucherCollectionPublicPath = /public/liquid_staking_withdraw_voucher_collection
