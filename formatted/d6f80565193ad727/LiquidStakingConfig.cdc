...
pub contract LiquidStakingConfig{ 
    ...
    pub fun calcEstimatedStakingPayout(stakedAmount: UFix64): UFix64{ 
        ...
        var scaledRewardAmount =
            scaledStakedAmount * scaledEpochTokenPayout
            / scaledSystemTotalStaked
        let scaledNodeCutAmount =
            scaledRewardAmount
            * self.UFix64ToScaledUInt256(
                FlowIDTableStaking.getRewardCutPercentage()
            )
            / self.scaleFactor
        // Deducting node cut
        scaledRewardAmount = scaledRewardAmount - scaledNodeCutAmount
        let scaledProtocolCutAmount =
            scaledRewardAmount * self.UFix64ToScaledUInt256(self.protocolFee)
            / self.scaleFactor
    ...
    pub resource Admin{ 
        ...
        pub fun setStakingCap(stakingCap: UFix64){ 
            emit ConfigStakingCap(
                newValue: stakingCap,
                oldValue: LiquidStakingConfig.stakingCap
            )
        ...
        pub fun setProtocolFee(protocolFee: UFix64){ 
            ...
            emit ConfigProtocolFee(
                newValue: protocolFee,
                oldValue: LiquidStakingConfig.protocolFee
            )
