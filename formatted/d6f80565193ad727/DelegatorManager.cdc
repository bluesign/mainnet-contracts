...
pub contract DelegatorManager{ 
    ...
    priv let migratedDelegatorIDs:{ String:{ UInt32: UInt64}}
    ...
    priv let totalRewardedVault: @FlowToken.Vault
    
    priv let totalUnstakedVault: @FlowToken.Vault
    
    /// All epoch snapshot history
    /// {epoch index -> snapshot}
    priv let epochSnapshotHistory:{ UInt64: EpochSnapshot}
    
    /// Vault of protocol fees
    priv let protocolFeeVault: @FlowToken.Vault
    ...
    pub event RewardsInfoCheckpointed(
        currEpoch: UInt64,
        received: UFix64,
        estimated: UFix64,
    ...
    pub event RestakeCanceledTokens(
        amount: UFix64,
        type: String,
        fromProtocolEpoch: UInt64
    )
    ...
    pub event DelegatorsCollected(
        startIndex: Int,
        endIndex: Int,
        uncollectedCount: Int
    )
    
    pub event StrategyTransferCommittedTokens(
        from: String,
        to: String,
        amount: UFix64
    )
    
    pub event StrategyProcessUnstakeRequest(
        amount: UFix64,
    ...
    pub event DelegatorRemoved(
        nodeID: String,
        delegatorID: UInt32,
        uuid: UInt64
    )
    
    pub event SetApprovedNodeList(
        nodeIDs:{ 
            String: UFix64
        },
        defaultNodeIDToStake: String
    )
    
    pub event UpsertApprovedNode(
        nodeID: String,
        oldWeight: UFix64?,
        newWeight: UFix64
    )
    ...
    pub struct EpochSnapshot{ 
        ...
        priv let delegatorInfoDict:{ 
            String:{ 
                UInt32: FlowIDTableStaking.DelegatorInfo
            }
        }
        
        /// { delegator uuid -> collected? }
        priv let delegatorCollected:{ UInt64: Bool}
        ...
        access(contract) fun upsertDelegatorInfo(
            nodeID: String,
            delegatorID: UInt32
        ){ 
            let delegatorInfo =
            ...
            self.allDelegatorStaked = self.allDelegatorStaked
                + delegatorInfo.tokensStaked
            self.allDelegatorCommitted = self.allDelegatorCommitted
                + delegatorInfo.tokensCommitted
            self.allDelegatorRequestedToUnstake = self
                    .allDelegatorRequestedToUnstake
                + delegatorInfo.tokensRequestedToUnstake
            (self.delegatorInfoDict[nodeID]!).insert(
                key: delegatorID,
                delegatorInfo
            )
        }
        ...
        access(contract) fun removeDelegatorInfo(
            nodeID: String,
            delegatorID: UInt32
        ){ 
            ...
            && (self.delegatorInfoDict[nodeID]!).containsKey(delegatorID){ 
                ...
                assert(
                    delegatorInfo.totalTokensInRecord()
                    + delegatorInfo.tokensRequestedToUnstake
                    == 0.0,
        ...
        access(contract) fun addRedelegatedTokensRequestToUnstake(
            amount: UFix64
        ){ 
            self.redelegatedTokensRequestToUnstake = self
                    .redelegatedTokensRequestToUnstake
                + amount
        }
        
        access(contract) fun addRedelegatedTokensUnderUnstaking(
            amount: UFix64
        ){ 
            self.redelegatedTokensUnderUnstaking = self
                    .redelegatedTokensUnderUnstaking
                + amount
        }
        
        access(contract) fun setStflowPrice(
            stFlowToFlow: UInt256,
            flowToStFlow: UInt256
        ){ 
        ...
        init(epochCounter: UInt64){ 
            ...
            self.scaledQuoteStFlowFlow = LiquidStakingConfig
                    .UFix64ToScaledUInt256(1.0)
            self.scaledQuoteFlowStFlow = LiquidStakingConfig
                    .UFix64ToScaledUInt256(1.0)
    ...
    priv fun stFlowQuote(){ 
        ...
        let totalStaked =
            nextEpochSnapshot.allDelegatorStaked
        ...
        nextEpochSnapshot.setStflowPrice(
            stFlowToFlow: stFlow_flow,
            flowToStFlow: flow_stFlow
        )
    }
    ...
    access(account) fun withdrawFromCommitted(
        amount: UFix64
    ): @FlowToken.Vault{ 
    ...
    access(account) fun requestWithdrawFromStaked(amount: UFix64){ 
        ...
        assert(
            currentEpochSnapshot.allDelegatorStaked
    ...
    access(account) fun withdrawFromUnstaked(amount: UFix64): @FlowToken.Vault{ 
        return <-(
            self.totalUnstakedVault.withdraw(amount: amount)
            as!
            @FlowToken.Vault
        )
    }
    ...
    access(account) fun migrateDelegator(
        delegator: @FlowIDTableStaking.NodeDelegator
    ){ 
    ...
    priv fun restakeCanceledTokens(){ 
        pre{ 
            FlowEpoch.currentEpochCounter > self.quoteEpochCounter:
    ...
    pub fun collectDelegatorsOnEpochStart(
        startIndex: Int,
        endIndex: Int,
        ifAdvanceEpoch: Bool
    ){ 
        ...
        if self.epochSnapshotHistory.containsKey(FlowEpoch.currentEpochCounter)
        == false{ 
            self.epochSnapshotHistory[
                FlowEpoch.currentEpochCounter
            ] = EpochSnapshot(epochCounter: FlowEpoch.currentEpochCounter)
        }
        let currEpochSnapshot = self.borrowCurrentQuoteEpochSnapshot()
        let nextEpochSnapshot = self.borrowCurrentChainEpochSnapshot()
    ...
    access(contract) fun insertMigratedDelegatorUUID(
        nodeID: String,
        delegatorID: UInt32,
        uuid: UInt64
    ){ 
        ...
        assert(
            (self.migratedDelegatorIDs[nodeID]!).containsKey(delegatorID)
            == false,
    ...
    priv fun snapshotRewardsInfo(){ 
        ...
        let nextEpochSnapshot = self.borrowCurrentChainEpochSnapshot()
        assert(
            nextEpochSnapshot.getCollectedDelegatorCount()
            == self.allDelegators.length,
        ...
        currEpochSnapshot.setReceivedReward(
            received: self.totalRewardedVault.balance
        )
        ...
        nextEpochSnapshot.setEstimatedReward(
            estimated: estimatedNextEpochAmount
        )
    ...
    priv fun filterApprovedNodeListOnEpochStart(){ 
        ...
        if self.defaultNodeIDToStake == ""
        && self.approvedNodeIDList.length > 0{ 
            self.defaultNodeIDToStake = self.approvedNodeIDList.keys[0]
        }
    }
    
    priv fun borrowManagedDelegator(
        uuid: UInt64
    ): &FlowIDTableStaking.NodeDelegator?{ 
        return &self.allDelegators[uuid] as &FlowIDTableStaking.NodeDelegator?
    }
    
    priv fun removeManagedDelegator(uuid: UInt64){ 
        let tmpDelegator <- self.allDelegators[uuid] <- nil
        destroy tmpDelegator
    }
    
    priv fun borrowApprovedDelegatorFromNode(
        _ nodeID: String
    ): &FlowIDTableStaking.NodeDelegator?{ 
        if (
    ...
    priv fun registerApprovedDelegator(
        _ nodeID: String,
        _ initialCommit: @FungibleToken.Vault
    ){ 
        pre{ 
    ...
    priv fun removeApprovedNodeID(nodeID: String){ 
        pre{ 
            self.approvedNodeIDList.containsKey(nodeID):
    ...
    pub fun getSlicedMigratedDelegatorIDList(
        nodeID: String,
        from: Int,
        to: Int
    ): [
        ...
        return (self.migratedDelegatorIDs[nodeID]!).keys.slice(
            from: from,
            upTo: upTo
        )
    }
    ...
    pub fun getDelegatorInfoByUUID(
        delegatorUUID: UInt64
    ): FlowIDTableStaking.DelegatorInfo{ 
        let delegator =
    ...
    pub fun getApprovedDelegatorInfoByNodeID(
        nodeID: String
    ): FlowIDTableStaking.DelegatorInfo{ 
        let delegator =
    ...
    pub resource DelegationStrategy{ 
        ...
        pub fun transferCommittedTokens(
            fromNodeID: String,
            toNodeID: String,
            amount: UFix64
        ){ 
            ...
            let transferVault <-
                fromDelegator.withdrawUnstakedTokens(amount: amount)
            
            // deposit committed
            toDelegator.delegateNewTokens(from: <-transferVault)
            
            // update snapshot
            DelegatorManager.borrowCurrentQuoteEpochSnapshot()
                .upsertDelegatorInfo(
            ...
            emit StrategyTransferCommittedTokens(
                from: fromNodeID,
                to: toNodeID,
                amount: amount
            )
        ...
        pub fun processUnstakeRequest(
            requestUnstakeAmount: UFix64,
            delegatorUUID: UInt64
        ){ 
            pre{ 
                FlowEpoch.currentEpochCounter == DelegatorManager.quoteEpochCounter:
            ...
            let delegator =
                DelegatorManager.borrowManagedDelegator(uuid: delegatorUUID)!
            let delegatorInfo =
                FlowIDTableStaking.DelegatorInfo(
                    nodeID: delegator.nodeID,
                    delegatorID: delegator.id
            ...
            DelegatorManager.requestedToUnstake = DelegatorManager
                    .requestedToUnstake
                - unstakeAmount
    ...
    pub resource Admin{ 
        ...
        pub fun initApprovedNodeIDList(
            nodeIDs:{ 
                String: UFix64
            },
            defaultNodeIDToStake: String
            ...
            emit SetApprovedNodeList(
                nodeIDs: nodeIDs,
                defaultNodeIDToStake: defaultNodeIDToStake
            )
        }
        
        pub fun upsertApprovedNodeID(nodeID: String, weight: UFix64){ 
            let oldWeight =
                DelegatorManager.approvedNodeIDList.insert(key: nodeID, weight)
            emit UpsertApprovedNode(
                nodeID: nodeID,
                oldWeight: oldWeight,
                newWeight: weight
            )
        ...
        pub fun redelegate(
            ...
            let uuid =
                DelegatorManager.getDelegatorUUIDByID(
                    nodeID: nodeID,
                    delegatorID: delegatorID
                )!
            let delegator = DelegatorManager.borrowManagedDelegator(uuid: uuid)!
            let delegatorInfo =
                DelegatorManager.getDelegatorInfoByUUID(delegatorUUID: uuid)
            ...
            DelegatorManager.borrowCurrentQuoteEpochSnapshot()
                .addRedelegatedTokensRequestToUnstake(amount: amount)
        ...
        pub fun registerApprovedDelegator(
            nodeID: String,
            initialCommit: @FungibleToken.Vault
        ){ 
    ...
    init(){ 
        ...
        self.protocolFeeVault <- FlowToken.createEmptyVault()
            as!
            @FlowToken.Vault
        self.totalUnstakedVault <- FlowToken.createEmptyVault()
            as!
            @FlowToken.Vault
        self.totalRewardedVault <- FlowToken.createEmptyVault()
            as!
            @FlowToken.Vault
