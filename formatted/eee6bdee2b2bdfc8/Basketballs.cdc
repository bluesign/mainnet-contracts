...
pub contract Basketballs: NonFungibleToken{ 
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &Basketballs.NFT?{ 
        let collection = getAccount(from).getCapability(Basketballs.CollectionPublicPath).borrow<&Basketballs.Collection{Basketballs.BasketballsCollectionPublic}>() ?? panic("Couldn't get collection")
