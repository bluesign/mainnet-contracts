        ...
        access(contract) fun sendBidTokens(
            ...
            if let vaultRef = capability.borrow(){ 
                ...
                if sale{ 
                    ...
                    let disruptartvaultRef = getAccount(DisruptArtMarketplace.marketAddress).getCapability(/public/fusdReceiver).borrow<&{FungibleToken.Receiver}>() ?? panic("failed to borrow reference to Marketplace vault")
                    
                    // let itemRef = (&self.auctionItems[id] as? &AuctionItem?)!
                    
                    let creatorvaultRef = getAccount(self.creator!!).getCapability(/public/fusdReceiver).borrow<&{FungibleToken.Receiver}>() ?? panic("failed to borrow reference to owner vault")
