...
pub contract DisruptArtMarketplaceFlow{ 
    ...
    pub resource SaleCollection: SalePublic{ 
        ...
        pub fun purchaseGroup(tokens: [UInt64], recipient: &{DisruptArt.DisruptArtCollectionPublic}, payment: @FungibleToken.Vault){ 
            ...
            let disruptartvaultRef = getAccount(DisruptArtMarketplaceFlow.marketAddress).getCapability(/public/flowTokenReceiver).borrow<&{FungibleToken.Receiver}>() ?? panic("failed to borrow reference to DisruptArtMarketplace vault")
        ...
        pub fun purchase(id: UInt64, recipient: &{DisruptArt.DisruptArtCollectionPublic}, payment: @FungibleToken.Vault){ 
            ...
            let creatorvaultRef = getAccount(owner).getCapability(/public/flowTokenReceiver).borrow<&{FungibleToken.Receiver}>() ?? panic("failed to borrow reference to owner vault")
        ...
        pub fun saleWithdrawn(tokens: [UInt64]){ 
            ...
            let receiver = getAccount(owner).getCapability(DisruptArt.disruptArtPublicPath).borrow<&{DisruptArt.DisruptArtCollectionPublic}>() ?? panic("Could not get receiver reference to the NFT Collection")
            while count < tokens.length{ 
    ...
    pub resource Admin{ 
        pub fun changeoperator(
            newOperator: Address,
            marketCommission: UFix64,
            royality: UFix64
        ){ 
    ...
    init(){ 
        ...
        self.account.save(
            <-create Admin(),
            to: /storage/DisruptArtFlowMarketAdmin
        )
    }
}
