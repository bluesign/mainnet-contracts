...
pub contract DisruptArtMarketplace{ 
    ...
    pub resource SaleCollection: SalePublic{ 
        ...
        pub fun purchaseGroup(tokens: [UInt64], recipient: &{DisruptArt.DisruptArtCollectionPublic}, payment: @FungibleToken.Vault){ 
            ...
            let disruptartvaultRef = getAccount(DisruptArtMarketplace.marketAddress).getCapability(/public/fusdReceiver).borrow<&{FungibleToken.Receiver}>() ?? panic("failed to borrow reference to DisruptArtMarketplace vault")
        ...
        pub fun purchase(id: UInt64, recipient: &{DisruptArt.DisruptArtCollectionPublic}, payment: @FungibleToken.Vault){ 
            ...
            let creatorvaultRef = getAccount(owner).getCapability(/public/fusdReceiver).borrow<&{FungibleToken.Receiver}>() ?? panic("failed to borrow reference to owner vault")
