...
pub contract DisruptArtAuctionFlow{ 
    ...
    pub resource AuctionItem{ 
        ...
        priv var recipientCollectionCap: Capability<
            &{DisruptArt.DisruptArtCollectionPublic}
        >
        ...
        access(contract) fun sendNFT(
            _ capability: Capability<&{DisruptArt.DisruptArtCollectionPublic}>
        ){ 
        ...
        access(contract) fun sendBidTokens(
            ...
            if let vaultRef = capability.borrow(){ 
                ...
                if sale{ 
                    ...
                    let disruptartvaultRef = getAccount(DisruptArtMarketplaceFlow.marketAddress).getCapability(/public/flowTokenReceiver).borrow<&{FungibleToken.Receiver}>() ?? panic("failed to borrow reference to Marketplace vault")
                    
                    // let itemRef = &self.auctionItems[id] as? &AuctionItem
                    
                    let creatorvaultRef = getAccount(self.creator!!).getCapability(/public/flowTokenReceiver).borrow<&{FungibleToken.Receiver}>() ?? panic("failed to borrow reference to owner vault")
