...
pub contract TrmAssetMSV1_0: NonFungibleToken{ 
    ...
    pub struct AssetData{ 
        ...
        access(contract) fun setValid(valid: Bool){ 
            self.valid = valid
        }
        ...
        access(contract) fun setKID(kID: String?){ 
            self.kID = kID
        }
        
        access(contract) fun setTitle(title: String?){ 
            self.title = title
        }
        ...
        access(contract) fun setThumbnailURL(thumbnailURL: String?){ 
            self.thumbnailURL = thumbnailURL
        }
        
        access(contract) fun setORH(orh: [String]?){ 
            self.orh = orh
        }
        
        access(contract) fun setISRC(isrc: String?){ 
            self.isrc = isrc
        }
        
        access(contract) fun setPerformers(performers: [String]?){ 
            self.performers = performers
        }
        
        access(contract) fun setLyricists(lyricists: [String]?){ 
            self.lyricists = lyricists
        }
        
        access(contract) fun setComposers(composers: [String]?){ 
            self.composers = composers
        }
        
        access(contract) fun setArrangers(arrangers: [String]?){ 
            self.arrangers = arrangers
        }
        
        access(contract) fun setReleaseDate(releaseDate: String?){ 
            self.releaseDate = releaseDate
        }
        
        access(contract) fun setPlayingTime(playingTime: String?){ 
            self.playingTime = playingTime
        }
        
        access(contract) fun setAlbumTitle(albumTitle: String?){ 
            self.albumTitle = albumTitle
        }
        
        access(contract) fun setCatalogNumber(catalogNumber: String?){ 
            self.catalogNumber = catalogNumber
        }
        
        access(contract) fun setTrackNumber(trackNumber: String?){ 
            self.trackNumber = trackNumber
        }
        
        access(contract) fun setOwnersHash(ownersHash: String?){ 
            self.ownersHash = ownersHash
        }
        
        access(contract) fun setRevenueHash(revenueHash: String?){ 
