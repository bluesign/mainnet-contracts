...
pub contract Domains: NonFungibleToken{ 
    ...
    pub resource Subdomain: SubdomainPublic, SubdomainPrivate{ 
        ...
        priv let addresses:{ UInt64: String}
        
        priv let texts:{ String: String}
    ...
    pub resource NFT: DomainPublic, DomainPrivate, NonFungibleToken.INFT, MetadataViews.Resolver{ 
        ...
        priv var subdomains: @{String: Subdomain}
        ...
        priv var collections: @{String: NonFungibleToken.Collection}
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            ...
            switch view{ 
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: Domains.CollectionStoragePath, publicPath: Domains.CollectionPublicPath, providerPath: Domains.CollectionPrivatePath, publicCollection: Type<&Domains.Collection{Domains.CollectionPublic}>(), publicLinkedType: Type<&Domains.Collection{Domains.CollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&Domains.Collection{Domains.CollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Provider, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
        ...
        pub fun getSubdomainsDetail(): [SubdomainDetail]{ 
            ...
            for id in ids{ 
                let subRef = (&self.subdomains[id] as                                                      
                                                      // create subdomain with domain
                                                      &Subdomain?)!
                let detail = subRef.getDetail()
                subdomains                          // add type whitelist check 
                          .append(detail)
            }
            return subdomains
        }
        ...
        pub fun createSubDomain(name: String){ 
            ...
            for char in subForbidChars.utf8{ 
                if name.utf8.contains(char){ 
                    panic(                          // update the owner record for new domain owner
            ...
            let domainHash = self.nameHash.slice(from: 2, upTo:                                                                
                                                                ...
                                                                66)
            let nameSha = String.encodeHex(HashAlgorithm                                                        // let pk = publicKeyStr.slice(from: 2, upTo: publicKey.length)
                                                        .SHA3_256.hash(name.utf8))
            let nameHash = "0x".concat(String.encodeHex(HashAlgorithm                                                                     
                                                                     // update records in case domain name not match hash
                                                                     .SHA3_256.hash(domainHash.concat(nameSha).utf8)))
