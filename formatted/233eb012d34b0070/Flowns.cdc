...
pub contract Flowns{ 
    ...
    pub event RootDomainMaxLengthUpdated(
        domainId: UInt64,
        before: Int,
        after: Int
    )
    
    pub event RootDomainCommissionRateUpdated(
        domainId: UInt64,
        before: UFix64,
        after: UFix64
    )
    
    pub event RootDomainMintDurationUpdated(
        domainId: UInt64,
        before: UFix64,
        after: UFix64
    )
    
    pub event DomainRegisterCommissionAllocated(
        domainId: UInt64,
        nameHash: String,
        amount: UFix64,
    ...
    pub resource RootDomain{ 
        ...
        priv var server: Capability<&Domains.Collection>?
        ...
        access(account) fun setMaxDomainLength(_ length: Int){ 
            ...
            emit RootDomainMaxLengthUpdated(
                domainId: self.id,
                before: oldLength,
                after: length
            )
        ...
        access(account) fun setCommissionRate(_ rate: UFix64){ 
            ...
            emit RootDomainCommissionRateUpdated(
                domainId: self.id,
                before: oldRate,
                after: rate
            )
        ...
        access(contract) fun _distributeCommission(
            ...
            let collectionCap =
                referAcc.getCapability<&{Domains.CollectionPublic}>(
        ...
        pub fun renewDomain(
            ...
            let expiredAt =
                Domains.getExpiredTime(domain.nameHash)! + UFix64(duration)
    ...
    pub resource interface RootDomainCollectionPublic{ 
        ...
        pub fun renewDomainWithNameHash(
            nameHash: String,
            duration: UFix64,
            feeTokens: @FungibleToken.Vault,
            refer: Address?
    ...
    pub resource interface RootDomainCollectionAdmin{ 
        ...
        access(account) fun changeRootDomainVault(
            domainId: UInt64,
            vault: @FungibleToken.Vault
        ){} 
        
        access(account) fun setPrices(
            domainId: UInt64,
            len: Int,
            price: UFix64
        ){} 
        
        access(account) fun setMinRentDuration(
            domainId: UInt64,
            duration: UFix64
        ){} 
        ...
        access(account) fun renewDomainWithAdmin(
            nameHash: String,
            duration: UFix64
        ){} 
    }
    ...
    pub resource RootDomainCollection:
        RootDomainCollectionPublic,
        RootDomainCollectionAdmin{
        ...
        pub fun renewDomainWithNameHash(
            ...
            let domain =
                Flowns.getDomain(nameHash: nameHash)
                ?? panic("Can not find domain by nameHash")
        ...
        access(account) fun withdrawVault(
            ...
            self.getRootDomain(domainId).withdrawVault(
                receiver: receiver,
                amount: amount
            )
        }
        ...
        access(account) fun renewDomainWithAdmin(
            ...
            emit RenewDomain(
                name: domain.name.concat(".").concat(domain.parent),
                nameHash: nameHash,
                duration: duration,
                price: 0.0
            )
    ...
    pub resource interface AdminPrivate{ 
        pub fun addCapability(
            _ cap: Capability<&Flowns.RootDomainCollection>
        ){} 
        
        pub fun addRootDomainCapability(
            domainId: UInt64,
            cap: Capability<&Domains.Collection>
        ){} 
    ...
    pub resource Admin: AdminPrivate{ 
        priv var server: Capability<&Flowns.RootDomainCollection>?
    ...
    pub fun getDomainNameHash(name: String, parentNameHash: String): String{ 
        ...
        let domainNameHash =
            Flowns.hash(
                node: parentNameHash.slice(from: 2, upTo: 66),
                lable: name
            )
    ...
    pub fun hash(node: String, lable: String): String{ 
        ...
        let lableHash =
            String.encodeHex(HashAlgorithm.SHA3_256.hash(lable.utf8))
        let hash =
            String.encodeHex(
                HashAlgorithm.SHA3_256.hash(prefixNode.concat(lableHash).utf8)
            )
        return hash
    }
    ...
    pub fun getDomain(nameHash: String): &{Domains.DomainPublic}?{ 
        ...
        let collectionCap =
            account.getCapability<&{Domains.CollectionPublic}>(
                Domains.CollectionPublicPath
            )!
        let collection = collectionCap.borrow()!
        ...
        if id != nil
        && !Domains.isDeprecated(nameHash: nameHash, domainId: id!){ 
    ...
    pub fun getRootDomainInfo(domainId: UInt64): RootDomainInfo?{ 
        ...
        let rootCollectionCap =
            account.getCapability<&{Flowns.RootDomainCollectionPublic}>(
    ...
    pub fun getRootVaultBalance(domainId: UInt64): UFix64{ 
        ...
        let collection =
            rootCollectionCap.borrow() ?? panic("Could not borrow collection ")
    ...
    init(){ 
        ...
        account.link<&Flowns.Admin{Flowns.AdminPrivate}>(
