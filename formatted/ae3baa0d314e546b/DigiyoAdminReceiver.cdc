...
pub contract DigiyoAdminReceiver{ 
    ...
    init(){ 
        ...
        if self.account.borrow<&DigiyoSplitCollection.SplitCollection>(
            from: self.splitCollectionPath
        )
        == nil{ 
            let collection <-
                DigiyoSplitCollection.createEmptyCollection(numBuckets: 32)
            self.account.save(<-collection, to: self.splitCollectionPath)
            self.account.link<&{Digiyo.DigiyoNFTCollectionPublic}>(
                Digiyo.collectionPublicPath,
                target: self.splitCollectionPath
            )
        }
    }
}
