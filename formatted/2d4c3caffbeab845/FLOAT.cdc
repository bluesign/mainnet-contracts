...
pub contract FLOAT: NonFungibleToken, ViewResolver{ 
    /***********************************************//******************** PATHS ********************//***********************************************/    pub let FLOATCollectionStoragePath: StoragePath
    ...
    /************************************************//******************** EVENTS ********************//************************************************/    pub event ContractInitialized()
    ...
    /***********************************************//******************** STATE ********************//***********************************************/// The total amount of FLOATs that have ever been
    ...
    /***********************************************//**************** FUNCTIONALITY ****************//***********************************************/// A helpful wrapper to contain an address, 
    ...
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver{ 
        ...
        pub fun getViews(): [Type]{ 
            let supportedViews = [Type<MetadataViews.Display>(), Type<MetadataViews.Royalties>(), Type<MetadataViews.ExternalURL>(), Type<MetadataViews.NFTCollectionData>(), Type<MetadataViews.NFTCollectionDisplay>(), Type<MetadataViews.Traits>(), Type<MetadataViews.Serial>(), Type<TokenIdentifier>()]
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                ...
                case Type<MetadataViews.Royalties>():
                    return MetadataViews.Royalties([MetadataViews.Royalty(recepient: getAccount(0x5643fd47a29770e7).getCapability<&FlowToken.Vault{FungibleToken.Receiver}>(/public/flowTokenReceiver), cut: 0.05, // 5% royalty on secondary sales description: "Emerald City DAO receives a 5% royalty from secondary sales because this NFT was created using FLOAT (https://floats.city/), a proof of attendance platform created by Emerald City DAO.")])
        ...
        destroy(){ 
            emit FLOATDestroyed(id: self.id, eventHost: self.eventHost, eventId: self.eventId, eventImage: self.eventImage, serial: self.serial)
        }
    }
    ...
    pub resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection, CollectionPublic{ 
        ...
        priv var events:{ UInt64:{ UInt64: Bool}}
    ...
    pub resource FLOATEvent: FLOATEventPublic, MetadataViews.Resolver{ 
        ...
        priv let verifiers:{ String: [{IVerifier}]}
        ...
        priv var extraMetadata:{ String: AnyStruct}
        
        // DEPRECATED, DO NOT USE
        priv var claimed:{ Address: TokenIdentifier}
        
        // DEPRECATED, DO NOT USE (used for storing claim info now)
        priv var currentHolders:{ UInt64: TokenIdentifier}
        
        // DEPRECATED, DO NOT USE
        priv var groups:{ String: Bool}
        ...
        priv fun setUserClaim(serial: UInt64, address: Address, floatId: UInt64){ 
        ...
        // Type: Contract Setter
        priv fun setExtraFloatMetadata(serial: UInt64, metadata:{ String: AnyStruct}){ 
        ...
        // Type: Contract Setter
        priv fun setSpecificExtraFloatMetadata(serial: UInt64, key: String, value: AnyStruct){ 
            if self.extraMetadata["extraFloatMetadatas"] == nil{ 
                let extraFloatMetadatas:{ UInt64: AnyStruct} ={} 
                self.extraMetadata["extraFloatMetadatas"] = extraFloatMetadatas
            }
        ...
        pub fun mint(recipient: &Collection{NonFungibleToken.CollectionPublic}, optExtraFloatMetadata:{ String: AnyStruct}?): UInt64{ 
            pre{ 
                self.userCanMint(address: (recipient.owner!).address):
                    "Only 1 FLOAT allowed per user, and this user already claimed their FLOAT!"
            }
            ...
            if let extraFloatMetadata:{ String: AnyStruct} = optExtraFloatMetadata{ 
                // ensure 
                assert(FLOAT.validateExtraFloatMetadata(data: extraFloatMetadata), message: "Extra FLOAT metadata is not proper. Check the `FLOAT.validateExtraFloatMetadata` function.")
        ...
        // the user can mint
        priv fun verifyAndMint(recipient: &Collection, params:{ String: AnyStruct}): UInt64{ 
        ...
        pub fun purchase(recipient: &Collection, params:{ String: AnyStruct}, payment: @FungibleToken.Vault){ 
            ...
            let EventHostVault = getAccount(self.host).getCapability(tokenInfo.path).borrow<&{FungibleToken.Receiver}>() ?? panic("Could not borrow the &{FungibleToken.Receiver} from the event host.")
            ...
            let EmeraldCityVault = getAccount(emeraldCityTreasury).getCapability(tokenInfo.path).borrow<&{FungibleToken.Receiver}>() ?? panic("Could not borrow the &{FungibleToken.Receiver} from Emerald City's Vault.")
    ...
    pub resource FLOATEvents: FLOATEventsPublic, MetadataViews.ResolverCollection{ 
        ...
        priv var events: @{UInt64: FLOATEvent}
        
        // DEPRECATED
        priv var groups: @{String: Group}
        ...
        pub fun createEvent(claimable: Bool, description: String, image: String, name: String, transferrable: Bool, url: String, verifiers: [{IVerifier}], allowMultipleClaim: Bool, certificateType: String, visibilityMode: String, extraMetadata:{ String: AnyStruct}): UInt64{ 
            pre{ 
                certificateType == "ticket" || certificateType == "medal" || certificateType == "certificate":
                    "You must either choose 'ticket', 'medal', or 'certificate' for certificateType. This is how your FLOAT will be displayed."
                visibilityMode == "certificate" || visibilityMode == "picture":
                    "You must either choose 'certificate' or 'picture' for visibilityMode. This is how your FLOAT will be displayed."
                extraMetadata["userClaims"] == nil:
                    "Cannot use userClaims key in extraMetadata."
                extraMetadata["extraFloatMetadatas"] == nil:
                    "Cannot use extraFloatMetadatas key in extraMetadata."
                extraMetadata["certificateImage"] == nil || (extraMetadata["certificateImage"]!).getType() == Type<String>():
                    "certificateImage must be a String or nil type."
                extraMetadata["backImage"] == nil || (extraMetadata["backImage"]!).getType() == Type<String>():
                    "backImage must be a String or nil type."
                extraMetadata["eventType"] == nil || (extraMetadata["eventType"]!).getType() == Type<String>():
                    "eventType must be a String or nil type."
            }
    ...
    pub fun resolveView(_ view: Type): AnyStruct?{ 
        switch view{ 
            case Type<MetadataViews.NFTCollectionData>():
                return MetadataViews.NFTCollectionData(storagePath: FLOAT.FLOATCollectionStoragePath, publicPath: FLOAT.FLOATCollectionPublicPath, providerPath: /private/FLOATCollectionPrivatePath, publicCollection: Type<&Collection{CollectionPublic}>(), publicLinkedType: Type<&Collection{CollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&Collection{CollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Provider, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
                        return <-FLOAT.createEmptyCollection()
                    })
            case Type<MetadataViews.NFTCollectionDisplay>():
                let squareMedia: MetadataViews.Media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://i.imgur.com/v0Njnnk.png"), mediaType: "image")
                let bannerMedia: MetadataViews.Media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://i.imgur.com/ETeVZZU.jpg"), mediaType: "image")
                return MetadataViews.NFTCollectionDisplay(name: "FLOAT", description: "FLOAT is a proof of attendance platform on the Flow blockchain.", externalURL: MetadataViews.ExternalURL("https://floats.city"), squareImage: squareMedia, bannerImage: bannerMedia, socials:{ "twitter": MetadataViews.ExternalURL("https://twitter.com/emerald_dao"), "discord": MetadataViews.ExternalURL("https://discord.gg/emeraldcity")})
        }
        return nil
    }
    ...
    init(){ 
        ...
        if self.account.borrow<&FLOAT.Collection>(from: FLOAT.FLOATCollectionStoragePath) == nil{ 
            ...
            self.account.link<&FLOAT.Collection{NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection, FLOAT.CollectionPublic}>(FLOAT.FLOATCollectionPublicPath, target: FLOAT.FLOATCollectionStoragePath)
        }
        if self.account.borrow<&FLOAT.FLOATEvents>(from: FLOAT.FLOATEventsStoragePath) == nil{ 
            ...
            self.account.link<&FLOAT.FLOATEvents{FLOAT.FLOATEventsPublic, MetadataViews.ResolverCollection}>(FLOAT.FLOATEventsPublicPath, target: FLOAT.FLOATEventsStoragePath)
        }
        let FLOATEvents = self.account.borrow<&FLOAT.FLOATEvents>(from: FLOAT.FLOATEventsStoragePath) ?? panic("Could not borrow the FLOATEvents from the signer.")
