...
pub contract FlowStakingCollection{ 
    ...
    /// Events
    pub event NodeAddedToStakingCollection(
        nodeID: String,
        role: UInt8,
    ...
    pub event DelegatorAddedToStakingCollection(
        nodeID: String,
        delegatorID: UInt32,
        amountCommitted: UFix64,
        address: Address?
    ...
    pub event NodeRemovedFromStakingCollection(
        nodeID: String,
        role: UInt8,
        address: Address?
    )
    
    pub event DelegatorRemovedFromStakingCollection(
        nodeID: String,
        delegatorID: UInt32,
        address: Address?
    )
    
    pub event MachineAccountCreated(
        nodeID: String,
        role: UInt8,
        address: Address
    )
    ...
    pub struct MachineAccountInfo{ 
        ...
        access(contract) let machineAccountVaultProvider: Capability<
            &FlowToken.Vault
        >
        ...
        init(
            nodeID: String,
            role: UInt8,
            machineAccountVaultProvider: Capability<&FlowToken.Vault>
        ){ 
            pre{ 
                machineAccountVaultProvider.check():
                    "Invalid Flow Token Vault Provider"
            }
            self.nodeID = nodeID
            self.role = role
    ...
    pub resource interface StakingCollectionPublic{ 
        ...
        pub fun addNodeObject(
            _ node: @FlowIDTableStaking.NodeStaker,
            machineAccountInfo: MachineAccountInfo?
        ){} 
        
        pub fun addDelegatorObject(
            _ delegator: @FlowIDTableStaking.NodeDelegator
        ){} 
    ...
    pub resource StakingCollection: StakingCollectionPublic{ 
        ...
        /// unlocked vault
        priv var unlockedVault: Capability<&FlowToken.Vault>
        
        /// locked vault
        /// will be nil if the account has no corresponding locked account
        priv var lockedVault: Capability<&FlowToken.Vault>?
        ...
        priv var nodeStakers: @{String: FlowIDTableStaking.NodeStaker}
        
        priv var nodeDelegators: @{String: FlowIDTableStaking.NodeDelegator}
        ...
        priv var tokenHolder: Capability<&LockedTokens.TokenHolder>?
        ...
        priv fun getTokens(amount: UFix64): @FungibleToken.Vault{ 
            ...
            if self.lockedVault != nil{ 
                ...
                assert(amount <= lockedBalance + unlockedBalance, message: "Insufficient total available Flow balance")
                ...
                assert(amount <= unlockedBalance, message: "Insufficient total Flow balance")
        ...
        priv fun depositTokens(from: @FungibleToken.Vault){ 
            pre{ 
                ...
                from.balance <= self.unlockedTokensUsed + self.lockedTokensUsed:
                    "Cannot deposit more than is already used"
            }
            let unlockedVault = self.unlockedVault.borrow()!
        ...
        pub fun removeNode(nodeID: String): @FlowIDTableStaking.NodeStaker?{ 
            pre{ 
                self.doesStakeExist(nodeID: nodeID, delegatorID: nil):
                    "Specified node does not exist in this collection"
                self.lockedTokensUsed == UFix64(0.0):
                    "Cannot remove node if locked tokens are used"
            }
        ...
        pub fun removeDelegator(nodeID: String, delegatorID: UInt32): @FlowIDTableStaking.NodeDelegator?{ 
            pre{ 
                self.doesStakeExist(nodeID: nodeID, delegatorID: delegatorID):
                    "Specified delegator does not exist in this collection"
                self.lockedTokensUsed == UFix64(0.0):
                    "Cannot remove delegator if locked tokens are used"
            }
        ...
        pub fun registerNode(id: String, role: UInt8, networkingAddress: String, networkingKey: String, stakingKey: String, amount: UFix64, payer: AuthAccount): AuthAccount?{ 
            ...
            let nodeReference = self.borrowNode(id) ?? panic("Could not borrow node reference")
        ...
        priv fun registerMachineAccount(nodeReference: &FlowIDTableStaking.NodeStaker, payer: AuthAccount): AuthAccount?{ 
            let nodeInfo = FlowIDTableStaking.NodeInfo(nodeID: nodeReference.id)
        ...
        pub fun addMachineAccountRecord(nodeID: String, machineAccount: AuthAccount){ 
            pre{ 
                self.doesStakeExist(nodeID: nodeID, delegatorID: nil):
                    "Cannot add a machine account record for a node that you do not own"
            }
            ...
            if nodeInfo.role == FlowEpoch.NodeRole.Collector.rawValue{ 
                let qcVoterRef = machineAccount.borrow<&FlowClusterQC.Voter>(from: FlowClusterQC.VoterStoragePath) ?? panic("Could not access QC Voter object from the provided machine account")
                assert(nodeID == qcVoterRef.nodeID, message: "QC Voter Object in machine account does not match machine node ID")
            } else if nodeInfo.role == FlowEpoch.NodeRole.Consensus.rawValue{ 
                let dkgParticipantRef = machineAccount.borrow<&FlowDKG.Participant>(from: FlowDKG.ParticipantStoragePath) ?? panic("Could not access DKG Participant object from the provided machine account")
                assert(nodeID == dkgParticipantRef.nodeID, message: "DKG Participant Object in machine account does not match machine node ID")
            }
        ...
        pub fun createMachineAccountForExistingNode(nodeID: String, payer: AuthAccount): AuthAccount?{ 
            ...
            if let nodeReference = self.borrowNode(nodeID){ 
                return self.registerMachineAccount(nodeReference: nodeReference, payer: payer)
            } else if let tokenHolderObj = self.tokenHolder{ 
                
                // borrow the main token manager object from the locked account 
                // to get access to the locked vault capability
                let lockedTokenManager = (tokenHolderObj.borrow()!).borrowTokenManager()
                let lockedNodeReference = lockedTokenManager.borrowNode() ?? panic("Could not borrow a node reference from the locked account")
        ...
        pub fun withdrawFromMachineAccount(nodeID: String, amount: UFix64){ 
            pre{ 
                self.doesStakeExist(nodeID: nodeID, delegatorID: nil):
                    "Specified stake does not exist in this collection"
            }
            if let machineAccountInfo = self.machineAccounts[nodeID]{ 
                let vaultRef = machineAccountInfo.machineAccountVaultProvider.borrow() ?? panic("Could not borrow reference to machine account vault")
        ...
        priv fun borrowNode(_ nodeID: String): &FlowIDTableStaking.NodeStaker?{ 
            if self.nodeStakers[nodeID] != nil{ 
        ...
        priv fun borrowDelegator(nodeID: String, delegatorID: UInt32): &FlowIDTableStaking.NodeDelegator?{ 
            if self.nodeDelegators[nodeID] != nil{ 
                let delegatorRef = (&self.nodeDelegators[nodeID] as &FlowIDTableStaking.NodeDelegator?)!
                if delegatorRef.id == delegatorID{ 
                    return delegatorRef
        ...
        pub fun closeStake(nodeID: String, delegatorID: UInt32?){ 
            ...
            if let delegatorID = delegatorID{ 
                ...
                assert(delegatorInfo.tokensStaked + delegatorInfo.tokensCommitted + delegatorInfo.tokensUnstaking == 0.0, message: "Cannot close a delegation until all tokens have been withdrawn, or moved to a withdrawable state.")
                ...
                if let machineAccountInfo = self.machineAccounts[nodeID]{ 
                    let vaultRef = machineAccountInfo.machineAccountVaultProvider.borrow() ?? panic("Could not borrow vault ref from machine account")
                    let unlockedVault = (self.unlockedVault!).borrow()!
                ...
                assert(stakeInfo.tokensStaked + stakeInfo.tokensCommitted + stakeInfo.tokensUnstaking == 0.0, message: "Cannot close a stake until all tokens have been withdrawn, or moved to a withdrawable state.")
    ...
    pub fun doesStakeExist(
        ...
        let stakingCollectionRef =
            account.getCapability<&StakingCollection{StakingCollectionPublic}>(
                self.StakingCollectionPublicPath
            ).borrow()
            ?? panic("Could not borrow ref to StakingCollection")
        return stakingCollectionRef.doesStakeExist(
            nodeID: nodeID,
    ...
    pub fun getAllDelegatorInfo(address: Address): [
