...
pub contract StarlyCardStakingClaims{ 
    ...
    pub resource RecentClaims{ 
        priv let timestamps: [UFix64]
        
        priv let claimAmounts: [UFix64]
        ...
        pub fun addClaim(
            ...
            let availableAmount =
                maxDailyClaimAmount.saturatingSubtract(alreadyClaimed)
            if availableAmount <= 0.0{ 
                return 0.0
            }
            let claimAmount =
                amountToClaim < availableAmount
                    ? amountToClaim
                    : availableAmount
        ...
        pub fun getClaimedAmount(): UFix64{ 
            let thresholdTimestamp =
                getCurrentBlock().timestamp - self.windowSizeSeconds
            var i = 0
    ...
    pub fun claim(ids: [UInt64], address: Address){ 
        ...
        let maxDailyClaimAmount =
            self.getDailyClaimAmountLimitByAddress(address: address)
        if !self.recentClaims.containsKey(address){ 
            self.recentClaims[address] <-! create RecentClaims()
        }
        let remainingResourceEditor =
            self.account.borrow<&{StarlyCardStaking.IEditor}>(
                from: StarlyCardStaking.EditorProxyStoragePath
            )
            ?? panic(
                "Could not borrow a reference to StarlyCardStaking.EditorProxyStoragePath!"
            )
        // get all staked cards
        let account = getAccount(address)
        let cardStakeCollectionRef =
            (account.getCapability(StakedStarlyCard.CollectionPublicPath)!)
                .borrow<
                &{
                    StakedStarlyCard.CollectionPublic,
                    NonFungibleToken.CollectionPublic
                }
            >()
            ?? panic(
                "Could not borrow capability from public StarlyTokenStaking collection!"
            )
        ...
        if payoutAmount > 0.0{ 
            ...
            let receiverRef = account.getCapability(StarlyToken.TokenPublicReceiverPath).borrow<&{FungibleToken.Receiver}>() ?? panic("Could not borrow StarlyToken receiver reference to the beneficiary's vault!")
    ...
    pub fun getRemainingDailyClaimAmountByAddress(address: Address): UFix64{ 
        return self.getDailyClaimAmountLimitByAddress(address: address)
            .saturatingSubtract(
    ...
    pub fun getDailyClaimAmountLimitByAddress(address: Address): UFix64{ 
        let stakeCollectionRef =
            (
                ...
                &{
                    StarlyTokenStaking.CollectionPublic,
                    NonFungibleToken.CollectionPublic
                }
            >()
            ?? panic(
    ...
    init(){ 
        ...
        if self.account.borrow<&StarlyToken.Vault>(
            from: StarlyToken.TokenStoragePath
        )
        == nil{ 
            self.account.save(
                <-StarlyToken.createEmptyVault(),
