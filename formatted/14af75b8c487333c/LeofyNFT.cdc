...
pub contract LeofyNFT: NonFungibleToken{ 
    ...
    pub struct LeofyNFTMetadataView{ 
        ...
        init(author: String, name: String, description: String, thumbnail: AnyStruct{MetadataViews.File}, itemID: UInt64, serialNumber: UInt32){ 
            self.author = author
            self.name = name
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            ...
            switch view{ 
                case Type<MetadataViews.Display>():
                    return MetadataViews.Display(name: itemCollection.getItemMetaDataByField(itemID: self.itemID, field: "name") ?? "", description: self.description(), thumbnail: MetadataViews.HTTPFile(itemCollection.getItemMetaDataByField(itemID: self.itemID, field: "thumbnail") ?? ""))
                case Type<LeofyNFTMetadataView>():
                    return LeofyNFTMetadataView(author: itemCollection.getItemMetaDataByField(itemID: self.itemID, field: "author") ?? "", name: itemCollection.getItemMetaDataByField(itemID: self.itemID, field: "name") ?? "", description: self.description(), thumbnail: MetadataViews.HTTPFile(itemCollection.getItemMetaDataByField(itemID: self.itemID, field: "thumbnail") ?? ""), itemID: self.itemID, serialNumber: self.serialNumber)
            }
            return nil
        }
    }
    ...
    pub fun getItemCollectionPublic(): &AnyResource{LeofyNFT.ItemCollectionPublic}{ 
        return self.account.getCapability(LeofyNFT.ItemPublicPath).borrow<&{LeofyNFT.ItemCollectionPublic}>() ?? panic("Could not borrow capability from public Item Collection")
    }
    
    pub fun getLeofyCoinVault(): &AnyResource{FungibleToken.Receiver}{ 
        return (self.account.getCapability(LeofyCoin.ReceiverPublicPath)!).borrow<&{FungibleToken.Receiver}>() ?? panic("Could not borrow receiver reference to the recipient's Vault")
    }
    ...
    init(){ 
        ...
        self.account.link<&LeofyNFT.ItemCollection{LeofyNFT.ItemCollectionPublic}>(self.ItemPublicPath, target: self.ItemStoragePath)
        ...
        self.account.link<&LeofyNFT.Collection{NonFungibleToken.CollectionPublic, LeofyNFT.LeofyCollectionPublic}>(self.CollectionPublicPath, target: self.CollectionStoragePath)
