...
pub contract LeofyMarketPlace{ 
    ...
    pub resource MarketplaceItem{ 
        ...
        priv var recipientCollectionCap: Capability<
            &{LeofyNFT.LeofyCollectionPublic}
        >?
        ...
        init(
            ...
            LeofyMarketPlace.totalMarketPlaceItems = LeofyMarketPlace
        ...
        access(contract) fun sendNFT(
            _ capability: Capability<&{LeofyNFT.LeofyCollectionPublic}>
        ){ 
        ...
        pub fun settleAuction(
            cutPercentage: UFix64,
            cutVault: Capability<&{FungibleToken.Receiver}>
        ){ 
            pre{ 
        ...
        pub fun timeRemaining(): Fix64{ 
            ...
            let remaining =
                Fix64(startTime + auctionLength) - Fix64(currentTime)
            return remaining
        }
        
        pub fun isAuctionExpired(): Bool{ 
            let timeRemaining = self.timeRemaining()
            if self.auctionStartTime == UFix64(0.0)
        ...
        pub fun placeBid(
            ...
            let amountYouAreBidding =
                bidTokens.balance
                + self.currentBidForUser(address: bidderAddress)
            ...
            < LeofyMarketPlace.extendsWhenTimeLowerThan{ 
        ...
        pub fun getMarketplaceStatus(): MarketplaceStatus{ 
            ...
            let view =
                self.NFT?.resolveView(Type<LeofyNFT.LeofyNFTMetadataView>())
    ...
    pub resource MarketplaceCollection: MarketplaceCollectionPublic{ 
        ...
        pub fun sellItem(token: @LeofyNFT.NFT, auctionStartTime: UFix64, auctionLength: UFix64, startPrice: UFix64, ownerCollectionCap: Capability<&{LeofyNFT.LeofyCollectionPublic}>, ownerVaultCap: Capability<&{FungibleToken.Receiver}>, purchasePrice: UFix64){ 
            pre{ 
        ...
        pub fun getMarketplaceStatuses():{ UInt64: MarketplaceStatus}{ 
            ...
            for id in self.marketplaceItems.keys{ 
                let itemRef = (&self.marketplaceItems[id] as                                                             
        ...
        pub fun getMarketplaceStatus(_ id:                                           
                                           // Get the auction item resources
                                           UInt64): MarketplaceStatus{ 
            pre{ 
    ...
    pub resource LeofyMarketPlaceAdmin{ 
        ...
        pub fun changeExtendsWhenTimeLowerThan(
            _ extendsWhenTimeLowerThan: Fix64
        ){ 
            LeofyMarketPlace.extendsWhenTimeLowerThan = extendsWhenTimeLowerThan
        }
        
        pub fun changeMarketplaceVault(
            _ marketplaceVault: Capability<&{FungibleToken.Receiver}>
        ){ 
            pre{ 
    ...
    init(){ 
        ...
        self.marketplaceVault = self.account.getCapability<
        ...
        destroy self.account.load<@MarketplaceCollection>(
        ...
        destroy self.account.load<@LeofyMarketPlaceAdmin>(
            from: self.AdminStoragePath
        )
        self.account.save<@LeofyMarketPlaceAdmin>(
            <-create LeofyMarketPlaceAdmin(),
            to: self.AdminStoragePath
        )
        self.account.link<
            &LeofyMarketPlace.MarketplaceCollection{
                LeofyMarketPlace.MarketplaceCollectionPublic
            }
