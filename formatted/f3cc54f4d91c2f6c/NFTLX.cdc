...
pub contract NFTLX{ 
    ...
    pub fun getSetsCount(): Int{ 
        let sets =
            self.setsCapability.borrow()
            ?? panic("Unable to load sets from capability")
        return sets.length
    }
    ...
    pub resource Admin{ 
        pub fun addNewSet(set: @{ISet}){ 
            var sets <-
                NFTLX.account.load<@{UInt32:{ ISet}}>(
                    from: NFTLX.SetsStoragePath
                )
    ...
    init(){ 
        ...
        if let oldAdmin <-
            self.account.load<@Admin>(from: self.AdminStoragePath){ 
            destroy oldAdmin
        }
        ...
        if self.account.borrow<auth &{UInt32:{ ISet}}>(
            from: self.SetsStoragePath
        )
        == nil{ 
        ...
        let setsCap =
            self.account.link<auth &{UInt32:{ ISet}}>(
