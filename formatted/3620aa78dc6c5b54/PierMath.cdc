...
pub contract PierMath{ 
    ...
    priv let scaleUI256: UInt256
    
    // fixed scaling factor 10^8 in UFix64
    priv let scaleUF64: UFix64
    ...
    pub fun UFix64ToRawUInt256(_ n: UFix64): UInt256{ 
        ...
        return UInt256(n) * self.scaleUI256
        + UInt256(fractionalPart * self.scaleUF64)
    }
    ...
    pub fun rawUInt256ToUFix64(_ n: UInt256): UFix64{ 
        ...
        return UFix64(n / self.scaleUI256)
        + UFix64(fractionalPart) / self.scaleUF64
    }
    ...
    pub fun computePriceCumulative(
        ...
        let newPriceCumulative =
            UInt256(lastPrice1Cumulative)
            + self.UFix64ToRawUInt256(reserve2)
            * self.UFix64ToRawUInt256(timeElapsed)
            / self.UFix64ToRawUInt256(reserve1)
    ...
    pub fun AddressToUInt64(address: Address): UInt64{ 
        ...
        assert(
            ...
            message: "Metapier PierMath: Address should be 64 bits"
        )
        return UInt64(addressBytes[0]) << 56 | UInt64(addressBytes[1]) << 48
