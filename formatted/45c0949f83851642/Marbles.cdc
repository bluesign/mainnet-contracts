...
pub contract Marbles: NonFungibleToken, TiblesApp, TiblesNFT, TiblesProducer{ 
    ...
    pub resource NFT: NonFungibleToken.INFT, TiblesNFT.INFT, MetadataViews.Resolver{ 
        ...
        pub fun displayData():{ String: String}{ 
            ...
            if metadata.containsKey("pack"){ 
                return{ "name": "Marbles pack", "description": "A Marbles pack", "imageUrl": "https://i.tibles.com/m/marbles-flow-icon.png"}
            }
            let set = metadata["set"]! as! &Marbles.Set
            let item = metadata["item"]! as! &Marbles.Item
            let variant = metadata["variant"]! as! &Marbles.Variant
            var edition: String = ""
        ...
        pub fun editions(): MetadataViews.Editions{ 
            ...
            let metadata = self.metadata() ?? panic("Missing NFT metadata")
            if metadata.containsKey("pack"){ 
                return MetadataViews.Editions([MetadataViews.Edition(name: "Marbles pack", number: UInt64(self.mintNumber), max: nil)])
            }
            let variant = metadata["variant"]! as! &Marbles.Variant
        ...
        pub fun externalURL(): MetadataViews.ExternalURL{ 
            return MetadataViews.ExternalURL("https://marbles.tibles.com/".concat(self.id.toString()))
        }
        
        pub fun nftCollectionData(): MetadataViews.NFTCollectionData{ 
            return MetadataViews.NFTCollectionData(storagePath: Marbles.CollectionStoragePath, publicPath: Marbles.PublicCollectionPath, providerPath: /private/MarblesCollection, publicCollection: Type<&Marbles.Collection{TiblesNFT.CollectionPublic}>(), publicLinkedType: Type<&Marbles.Collection{TiblesNFT.CollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&Marbles.Collection{TiblesNFT.CollectionPublic, NonFungibleToken.Provider, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
                    return <-Marbles.createEmptyCollection()
                })
        }
        
        pub fun nftCollectionDisplay(): MetadataViews.NFTCollectionDisplay{ 
            let squareMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://i.tibles.com/m/marbles-flow-icon.png"), mediaType: "image/svg+xml")
            let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://i.tibles.com/m/marbles-flow-collection-banner.png"), mediaType: "image/png")
            ...
            return MetadataViews.NFTCollectionDisplay(name: "Marbles Collection", description: "Marble collecting by Tibles", externalURL: MetadataViews.ExternalURL("https://app.marbles.com/"), squareImage: squareMedia, bannerImage: bannerMedia, socials: socials)
        }
    ...
    pub resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, TiblesNFT.CollectionPublic, MetadataViews.ResolverCollection{ 
        pub var ownedNFTs: @{UInt64: NonFungibleToken.NFT}
        
        pub fun deposit(token: @NonFungibleToken.NFT){ 
            let tible <- token as! @Marbles.NFT
        ...
        pub fun depositTible(tible: @AnyResource{TiblesNFT.INFT}){ 
            ...
            let token <- tible as! @Marbles.NFT
            let id = token.id
    ...
    pub fun createNewPackMinter(id: String, contentCapability: Capability): @PackMinter{ 
        assert(self.account.address == 0x45c0949f83851642, message: "wrong address")
        return <-create PackMinter(id: id, contentCapability: contentCapability)
    }
    
    init(){ 
        self.totalSupply = 0
        self.appId = "com.tibles.marbles"
        self.title = "Marbles"
        self.description = "Marble collecting by Tibles"
        self.ProducerStoragePath = /storage/TiblesMarblesProducer
        self.ProducerPath = /private/TiblesMarblesProducer
        self.ContentPath = /public/TiblesMarblesContent
        self.CollectionStoragePath = /storage/TiblesMarblesCollection
        self.PublicCollectionPath = /public/TiblesMarblesCollection
