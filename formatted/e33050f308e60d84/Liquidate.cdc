...
pub contract Liquidate{ 
    priv let poolNames: [String]
    
    priv let poolAddrs: [Address]
    
    priv let poolBalancePaths: [String]
    
    priv let poolVaultPaths: [String]
    
    priv let poolPenalties: [UFix64]
    ...
    pub fun repayAmount(
        ...
        let liquidity =
            comptrollerRef.getUserCrossMarketLiquidity(userAddr: borrowerAddr)
        if liquidity[0] >= liquidity[1]{ 
            return 0.0
        }
        let details =
            self.calculateLiquidation(
        ...
        var repayAmount =
            self.min(
                ...
                self.min(maxSupplyUsd, maxSize)
                / (1.0 + self.poolPenalties[maxSupplyIndex])
    ...
    pub fun liquidate(
        ...
        let repayPoolRef =
            getAccount(self.poolAddrs[maxBorrowIndex]).getCapability<
                &{LendingInterfaces.PoolPublic}
            >(LendingConfig.PoolPublicPublicPath).borrow()!
        let seizePoolRef =
            getAccount(self.poolAddrs[maxSupplyIndex]).getCapability<
                &{LendingInterfaces.PoolPublic}
            >(LendingConfig.PoolPublicPublicPath).borrow()!
        let preLpToken =
            LendingConfig.ScaledUInt256ToUFix64(
                seizePoolRef.getAccountLpTokenBalanceScaled(
