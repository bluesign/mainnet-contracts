...
pub contract Swap{ 
    ...
    pub var AllowSwapProposalCreation: Bool
    ...
    pub struct ProposedTradeAsset: Readable{ 
        pub let nftID: UInt64
        
        pub let type: Type
        
        pub let collectionData: Utils.StorableNFTCollectionData
        
        pub fun getReadable():{ String: String}{ 
            return{ "nftID": self.nftID.toString(), "type": self.type.identifier}
        }
        
        pub fun toFullyQualifiedIdentifier(): String{ 
    ...
    pub struct Fee: Readable{ 
        ...
        pub let feeGroup: UInt8
        
        pub let tokenType: Type
        
        init(receiver: Capability<&AnyResource{FungibleToken.Receiver}>, amount: UFix64, feeGroup: UInt8){ 
    ...
    pub struct UserOffer: Readable{ 
        ...
        pub fun toFullyQualifiedIdentifiers(): [String]{ 
            let response: [String] = []
    ...
    pub struct UserCapabilities{ 
        access(contract) let collectionReceiverCapabilities:{ 
            String: Capability<&{NonFungibleToken.Receiver}>
        }
        
        access(contract) let collectionProviderCapabilities:{ 
            String: Capability<&{NonFungibleToken.Provider}>
        }
        
        access(contract) let feeProviderCapabilities:{ 
    ...
    pub struct SwapProposal{ 
        ...
        access(contract) fun execute(rightUserCapabilities: UserCapabilities){ 
            ...
            let mapNfts =
                fun (_ array: [ProposedTradeAsset]): [SwapArchive.SwapNftData]{ 
                    var res: [SwapArchive.SwapNftData] = []
                    for item in array{ 
                        let nftData = SwapArchive.SwapNftData(id: item.nftID, type: item.type)
                        res.append(nftData)
                    }
                    return res
                }
            ...
            // archive swap
            SwapArchive.archiveSwap(
                id: id,
                SwapArchive.SwapData(
                    id: self.id,
            ...
            // update swap stats
            SwapStatsRegistry.addAccountStats(
                id: id,
                address: self.leftUserOffer.userAddress,
                SwapStatsRegistry.AccountSwapData(
                    partnerAddress: self.rightUserOffer.userAddress,
                    totalTradeVolumeSent: UInt(
                        self.leftUserOffer.proposedNfts.length
                    )!,
                    totalTradeVolumeReceived: UInt(
                        self.rightUserOffer.proposedNfts.length
                    )!
                )
            )
            SwapStatsRegistry.addAccountStats(
                id: id,
                address: self.rightUserOffer.userAddress,
                SwapStatsRegistry.AccountSwapData(
    ...
    pub resource SwapCollection: SwapCollectionManager, SwapCollectionPublic{ 
        ...
        pub fun createProposal(leftUserOffer: UserOffer, rightUserOffer: UserOffer, leftUserCapabilities: UserCapabilities, expirationOffsetMinutes: UFix64?, metadata:{ String: String}?): String{ 
    ...
    pub resource interface SwapProposalManager{ 
        ...
        pub fun updateMinExpiration(_ exp: UFix64){} 
        
        pub fun updateMaxExpiration(_ exp: UFix64){} 
        
        pub fun updateDefaultExpiration(_ exp: UFix64){} 
    }
    
    pub resource interface SwapFeeManager{ 
        pub fun addFee(fee: Fee){} 
        
        pub fun removeFeeGroup(feeGroup: UInt8){} 
    }
    
    pub resource interface SwapStatsManager{ 
        pub fun addAccountStats(
            id: String,
            address: Address,
            _ data: SwapStatsRegistry.AccountSwapData
        ){} 
        
        pub fun clearAccountStats(id: String, address: Address){} 
        
        pub fun clearAllAccountStatsById(id: String, limit: Int?){} 
        
        pub fun clearAllAccountStatsLookupById(id: String, limit: Int?){} 
        
        pub fun clearAllAccountStatsPartnersById(id: String, limit: Int?){} 
    }
    ...
    pub resource SwapAdmin:
        ...
        pub fun clearAllAccountStatsLookupById(id: String, limit: Int?){ 
            SwapStatsRegistry.clearAllAccountStatsLookupById(
                id: id,
                limit: limit
            )
        }
        ...
        pub fun clearAllAccountStatsPartnersById(id: String, limit: Int?){ 
            SwapStatsRegistry.clearAllAccountStatsPartnersById(
                id: id,
                limit: limit
            )
        }
    ...
    access(contract) fun verifyUserOffer(
        ...
        if fees.length > 0 && userCapabilities != nil{ 
            assert((userCapabilities!).feeProviderCapabilities != nil && ((userCapabilities!).feeProviderCapabilities!).length > 0, message: "feeProviderCapabilities dictionary cannot be empty if fees are required")
    ...
    init(){ 
        ...
        self.account.link<&SwapAdmin{SwapProposalManager, SwapFeeManager}>(
            self.SwapAdminPrivatePath,
            target: self.SwapAdminStoragePath
        )
    }
}
