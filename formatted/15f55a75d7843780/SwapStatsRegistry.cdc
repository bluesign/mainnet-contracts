pub contract SwapStatsRegistry{ 
    priv var accountSwapStats:{ String: [AccountStats]}
    
    priv var accountSwapStatsLookup:{ String:{ Address: Int}}
    
    priv var accountSwapPartners:{ String:{ Address: [Address]}}
    ...
    pub struct AccountStats{ 
        ...
        pub fun updateData(id: String, _ data: AccountSwapData){ 
            ...
            self.totalTradeVolumeSent = self.totalTradeVolumeSent
                + Int(data.totalTradeVolumeSent)
            self.totalTradeCount = self.totalTradeCount + 1
            if !((SwapStatsRegistry.accountSwapPartners[id]!)[self.address]!)
                .contains(data.partnerAddress){ 
                ((SwapStatsRegistry.accountSwapPartners[id]!)[self.address]!)
                    .append(data.partnerAddress)
    ...
    pub fun getAccountStats(id: String, address: Address): AccountStats{ 
    ...
    access(contract) fun sortStats(
        _ array: [
            AccountStats
        ],
        _ compare: ((AccountStats, AccountStats): Int)
    ): [
        AccountStats
    ]{ 
        if array.length < 2{ 
    ...
    priv fun resolveAccountStats(id: String): [AccountStats]{ 
        if !self.accountSwapStats.containsKey(id){ 
            return []
        }
        let response =
            self.sortStats(
                self.accountSwapStats[id]!,
                fun (a: AccountStats, b: AccountStats): Int{ 
    ...
    access(contract) fun getAccountStatsIndex(
        id: String,
        address: Address
    ): Int?{ 
        if !self.accountSwapStats.containsKey(id)
        || !self.accountSwapStatsLookup.containsKey(id){ 
            return nil
        }
    ...
    access(account) fun addAccountStats(
        id: String,
        address: Address,
        _ data: AccountSwapData
    ){ 
        assert(
            address != data.partnerAddress,
            message: "partner address not allowed"
        )
        if !self.accountSwapStats.containsKey(id){ 
    ...
    access(account) fun clearAllAccountStatsLookupById(
        id: String,
        limit: Int?
    ){ 
        if self.accountSwapStatsLookup.containsKey(id){ 
    ...
    access(account) fun clearAllAccountStatsPartnersById(
        id: String,
        limit: Int?
    ){ 
        if self.accountSwapPartners.containsKey(id){ 
