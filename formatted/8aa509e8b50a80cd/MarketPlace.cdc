...
pub contract MarketPlace{ 
    ...
    pub resource SaleCollection: SaleCollectionPublic{ 
        ...
        priv var forSale: @{UInt64: Collectible.NFT}
        ...
        priv fun handlePayments(tokenID: UInt64, buyTokens: @FUSD.Vault, price: UFix64){ 
            let vaultRef = self.ownerVault.borrow() ?? panic("Could not borrow reference to owner token vault")
        ...
        pub fun purchase(tokenID: UInt64, recipientCap: Capability<&Collectible.Collection{Collectible.CollectionPublic}>, buyTokens: @FUSD.Vault){ 
            pre{ 
    ...
    pub fun getSaleDatas(address: Address): [SaleData]{ 
        ...
        let CollectibleCollection =
            account.getCapability<
                &AnyResource{MarketPlace.SaleCollectionPublic}
            >(MarketPlace.CollectionPublicPath).borrow()
    ...
    pub fun createSaleCollection(
        ownerVault: Capability<&FUSD.Vault{FungibleToken.Receiver}>
    ): @SaleCollection{ 
