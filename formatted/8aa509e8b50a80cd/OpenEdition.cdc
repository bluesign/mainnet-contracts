...
pub contract OpenEdition{ 
    ...
    pub resource OpenEditionItem{ 
        ...
        init(
            ...
            OpenEdition.totalOpenEditions = OpenEdition.totalOpenEditions
                + 1 as UInt64
            self.price = price
            self.startTime = startTime
        ...
        priv fun sendCommissionPayments(
            ...
            let editionRef =
                OpenEdition.account.getCapability<
        ...
        pub fun purchase(
            buyerTokens: @FUSD.Vault,
            buyerCollectionCap: Capability<
                &Collectible.Collection{Collectible.CollectionPublic}
            >,
    ...
    pub resource OpenEditionCollection: OpenEditionCollectionPublic{ 
        ...
        pub fun createOpenEdition(price: UFix64, startTime: UFix64, saleLength: UFix64, editionNumber: UInt64, metadata: Collectible.Metadata, platformVaultCap: Capability<&FUSD.Vault{FungibleToken.Receiver}>){ 
            pre{ 
                saleLength > 0.00:
        ...
        pub fun purchase(id: UInt64, buyerTokens: @FUSD.Vault, collectionCap: Capability<&Collectible.Collection{Collectible.CollectionPublic}>){ 
            pre{ 
                self.openEditionsItems[id] != nil:
                    "Open Edition does not exist"
                collectionCap.check():
    ...
    init(){ 
        ...
        let openEdition <-
            OpenEdition.createOpenEditionCollection(minterCap: minterCap)
        self.account.save(<-openEdition, to: OpenEdition.CollectionStoragePath)
        self.account.link<&{OpenEdition.OpenEditionCollectionPublic}>(
            OpenEdition.CollectionPublicPath,
            target: OpenEdition.CollectionStoragePath
        )
    }
}
