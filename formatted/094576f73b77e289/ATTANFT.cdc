...
pub contract ATTANFT: NonFungibleToken{ 
    ...
    pub resource Admin{ 
        ...
        pub fun withdrawVault(amount: UFix64): @FungibleToken.Vault{ 
            ...
            let vaultRef = &self.vault as                                          // query price 
                                          // query pause status
                                          // query the BaseURI 
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &ATTANFT.NFT?{ 
        let collection = (getAccount(from).getCapability(ATTANFT.CollectionPublicPath)!).borrow<&ATTANFT.Collection{ATTANFT.ATTACollectionPublic}>() ?? panic("Couldn't get collection")
