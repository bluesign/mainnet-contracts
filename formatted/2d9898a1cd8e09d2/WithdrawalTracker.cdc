pub contract WithdrawalTracker{ 
    pub event WithdrawalTotalTrackerCreated(
        withdrawalLimit: UFix64,
        runningTotal: UFix64
    )
    
    pub event WithdrawalLimitSet(
        oldLimit: UFix64,
        newLimit: UFix64,
        runningTotal: UFix64
    )
    
    pub event RunningTotalUpdated(
        amount: UFix64,
        runningTotal: UFix64,
        withdrawalLimit: UFix64
    )
    ...
    pub resource WithdrawalTotalTracker:
        WithdrawalTotalChecker,
        SetWithdrawalLimit{
    
        priv var withdrawalLimit: UFix64
        
        priv var runningTotal: UFix64
        ...
        pub fun updateRunningTotal(withdrawalAmount: UFix64){ 
            ...
            emit RunningTotalUpdated(
                amount: withdrawalAmount,
                runningTotal: self.runningTotal,
                withdrawalLimit: self.withdrawalLimit
            )
        }
        
        pub fun setWithdrawalLimit(withdrawalLimit: UFix64){ 
            emit WithdrawalLimitSet(
                oldLimit: self.withdrawalLimit,
                newLimit: withdrawalLimit,
                runningTotal: self.runningTotal
            )
        ...
        init(initialLimit: UFix64, initialRunningTotal: UFix64){ 
            ...
            emit WithdrawalTotalTrackerCreated(
                withdrawalLimit: self.withdrawalLimit,
                runningTotal: self.runningTotal
            )
        }
    ...
    pub fun createWithdrawalTotalTracker(
        initialLimit: UFix64,
