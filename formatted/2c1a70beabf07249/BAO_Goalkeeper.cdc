...
//
pub contract BAO_Goalkeeper: NonFungibleToken{ 
    ...
    pub resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic{ 
        ...
        pub fun deposit(token: @NonFungibleToken.NFT){ 
            let token <- token as! @BAO_Goalkeeper.NFT
    ...
    pub resource NFTMinter{ 
        ...
        pub fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, mugenNFTID: UInt64){ 
            if BAO_Goalkeeper.mintedIds[mugenNFTID] == true{ 
                emit MintFailDuplicateId(id: mugenNFTID)
                panic("Duplicate token id")
            }
            if BAO_Goalkeeper.totalSupply == BAO_Goalkeeper.maxSupply{ 
            ...
            recipient.deposit(token: <-create BAO_Goalkeeper.NFT(initID: mugenNFTID))
            BAO_Goalkeeper.mintedIds[mugenNFTID] = true
            BAO_Goalkeeper.totalSupply = BAO_Goalkeeper.totalSupply + 1 as UInt64
        }
    }
    ...
    init(){ 
        // Mugen contract name
        // Set our named paths
        self.CollectionStoragePath = /storage/BAO_GoalkeeperCollection
        self.CollectionPublicPath = /public/BAO_GoalkeeperCollection
        self.MinterStoragePath = /storage/BAO_Goalkeeper
