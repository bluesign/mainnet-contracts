...
pub contract TheWolfofFlow: FungibleToken, ViewResolver{ 
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, MetadataViews.Resolver{ 
        ...
        pub fun withdraw(amount: UFix64): @FungibleToken.Vault{ 
            ...
            if let owner: Address = self.owner?.address{ 
                TheWolfofFlow.setBalance(address: owner, balance: self.balance)
            }
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                case Type<FungibleTokenMetadataViews.FTView>():
                    return TheWolfofFlow.resolveView(view)
        ...
        destroy(){ 
            if self.balance > 0.0{ 
                ...
                TheWolfofFlow.totalSupply = TheWolfofFlow.totalSupply - self.balance
            }
        }
    }
    ...
    pub resource Minter: Toucans.Minter{ 
        pub fun mint(amount: UFix64): @Vault{ 
            post{ 
                TheWolfofFlow.maxSupply == nil || TheWolfofFlow.totalSupply <= TheWolfofFlow.maxSupply!:
                    "Exceeded the max supply of tokens allowd."
            }
            TheWolfofFlow.totalSupply = TheWolfofFlow.totalSupply + amount
    ...
    pub fun resolveView(_ view: Type): AnyStruct?{ 
        switch view{ 
            ...
            case Type<FungibleTokenMetadataViews.FTDisplay>():
                let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafybeig3vjkn2xxqxm4f24jmyrfuahzj35pxiticfaqlzbznjjggcnzwdi"), mediaType: "image")
                let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafybeih4bwu6rmntnsenlxlsjtmioeblvdn7ujcs4tz45u3vwy4mul46u4"), mediaType: "image")
                let medias = MetadataViews.Medias([media, bannerMedia])
                return FungibleTokenMetadataViews.FTDisplay(name: "The Wolf of Flow", symbol: "WOLF", description: "$WOLF a fair launch meme coin on @flow_blockchain", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("thewolfofflow"), "discord": MetadataViews.ExternalURL("")})
            case Type<FungibleTokenMetadataViews.FTVaultData>():
                return FungibleTokenMetadataViews.FTVaultData(storagePath: TheWolfofFlow.VaultStoragePath, receiverPath: TheWolfofFlow.ReceiverPublicPath, metadataPath: TheWolfofFlow.VaultPublicPath, providerPath: /private/TheWolfofFlowVault, receiverLinkedType: Type<&Vault{FungibleToken.Receiver}>(), metadataLinkedType: Type<&Vault{FungibleToken.Balance, MetadataViews.Resolver}>(), providerLinkedType: Type<&Vault{FungibleToken.Provider}>(), createEmptyVaultFunction: fun (): @Vault{ 
                        return <-TheWolfofFlow.createEmptyVault()
                    })
        }
        return nil
    }
    ...
    init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
        ...
        // Paths
        self.VaultStoragePath = /storage/TheWolfofFlowVault
        self.ReceiverPublicPath = /public/TheWolfofFlowReceiver
        self.VaultPublicPath = /public/TheWolfofFlowMetadata
        self.MinterStoragePath = /storage/TheWolfofFlowMinter
        self.AdministratorStoragePath = /storage/TheWolfofFlowAdmin
