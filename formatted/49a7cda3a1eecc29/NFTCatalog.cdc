...
pub contract NFTCatalog{ 
    ...
    priv let catalogProposals:{ UInt64: NFTCatalogProposal} // { ProposalID : Metadata }
    ...
    pub resource NFTCatalogProposalManager: NFTCatalogProposalManagerPublic{ 
        priv var currentProposalEntry: String?
    ...
    pub resource Snapshot{ 
        ...
        pub fun setPartialSnapshot(
            _ snapshotKey: String,
            _ snapshotEntry: NFTCatalogMetadata
        ){ 
    ...
    pub fun getCatalog():{ String: NFTCatalogMetadata}{ 
        let snapshot =
            self.account.borrow<&NFTCatalog.Snapshot>(
                from: /storage/CatalogSnapshot
            )
        if snapshot != nil{ 
    ...
    access(account) fun removeCatalogEntryUnsafe(
        collectionIdentifier: String,
    ...
    access(account) fun updateCatalogProposal(
        proposalID: UInt64,
        proposalMetadata: NFTCatalogProposal
    ){ 
