...
pub contract VictoryNFTCollectionItem: NonFungibleToken{ 
    ...
    pub resource NFTMinter{ 
        ...
        //
        pub fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, owner: Address, typeID: UInt64, brandID: UInt64, dropID: UInt64, contentHash: UInt256, startIssueNum: UInt32, maxIssueNum: UInt32, totalIssueNum: UInt32){ 
            var i: UInt32 = 0
            while i < maxIssueNum{ 
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &VictoryNFTCollectionItem.NFT?{ 
        let collection = (getAccount(from).getCapability(VictoryNFTCollectionItem.CollectionPublicPath)!).borrow<&VictoryNFTCollectionItem.Collection{VictoryNFTCollectionItem.VictoryNFTCollectionItemCollectionPublic}>() ?? panic("Couldn't get collection")
