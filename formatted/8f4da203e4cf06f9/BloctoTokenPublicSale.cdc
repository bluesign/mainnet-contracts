...
pub contract BloctoTokenPublicSale{ 
    ...
    /****** Sale Enums ******/    pub enum PurchaseState: UInt8{ 
        pub case initial
    ...
    /****** Sale Resources ******/// BLT holder vault
    ...
    /****** Sale Variables ******/    access(contract) var isSaleActive: Bool
    ...
    pub resource Admin{ 
        ...
        pub fun distribute(address: Address, allocationAmount: UFix64){ 
            ...
            let bltVault <-
                BloctoTokenPublicSale.bltVault.withdraw(amount: bltAmount)
            ...
            emit Distributed(
                address: address,
                tusdtAmount: allocationAmount,
                bltAmount: bltAmount
            )
            
            // Refund the remaining amount
            if refundAmount > 0.0{ 
                let tUSDTReceiverRef = getAccount(address).getCapability(TeleportedTetherToken.TokenPublicReceiverPath).borrow<&{FungibleToken.Receiver}>() ?? panic("Could not borrow tUSDT vault receiver public reference")
        ...
        pub fun refund(address: Address){ 
            ...
            let receiverRef =
                ...
                ?? panic(
                    "Could not borrow tUSDT vault receiver public reference"
                )
            let purchaseInfo =
                BloctoTokenPublicSale.purchases[address]
                ?? panic("Count not get purchase info for the address")
            let tusdtVault <-
                BloctoTokenPublicSale.tusdtVault.withdraw(
