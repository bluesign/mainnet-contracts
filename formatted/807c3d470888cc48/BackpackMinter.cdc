...
pub contract BackpackMinter{ 
    ...
    pub event BackpackClaimed(
        FlunkTokenID: UInt64,
        BackpackTokenID: UInt64,
        signer: Address
    )
    
    pub let AdminStoragePath: StoragePath
    
    priv var backpackClaimedPerFlunkTokenID:{ UInt64: UInt64} // Flunk token ID: backpack token ID
    
    priv var backpackClaimedPerFlunkTemplate:{ UInt64: UInt64} // Flunks template ID: backpack token ID
    ...
    pub fun claimBackPack(tokenID: UInt64, signer: AuthAccount, setID: UInt64){ 
        ...
        let collection =
            getAccount(signer.address).getCapability<
                &Flunks.Collection{Flunks.FlunksCollectionPublic}
            >(Flunks.CollectionPublicPath).borrow()!
        ...
        let backpackReceiver =
            recipient.getCapability(Backpack.CollectionPublicPath).borrow<
                &{NonFungibleToken.CollectionPublic}
            >()
            ?? panic(
                "Could not get receiver reference to the Backpack NFT Collection"
            )
        
        // mint backpack to signer
        let admin =
            self.account.borrow<&Backpack.Admin>(
                from: Backpack.AdminStoragePath
            )
        ...
        BackpackMinter.backpackClaimedPerFlunkTemplate[
            templateID
        ] = backpackTokenID
    }
