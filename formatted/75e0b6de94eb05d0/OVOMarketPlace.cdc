...
pub contract OVOMarketPlace{ 
    ...
    pub resource Market: MarketController, MarketPublic{ 
        ...
        // save on sell NFTs
        priv var onSellNFTList: @{UInt64: NonFungibleToken.NFT}
        
        // order list
        priv var orderList:{ UInt64: orderData}
        
        // fee list, like "FUSD":0.05
        // 0.05 means 5%
        priv var transactionFeeList:{ String: UFix64}
        
        // record current orderId
        // it will increase 1 when a new order is created
        priv var orderId: UInt64
        
        // fees receiver
        priv var transactionFeeReceiverAddr: Address?
        ...
        pub fun buyNFT(orderId: UInt64, buyerAddr: Address, tokenName: String, buyerTokenVault: @FungibleToken.Vault){ 
            ...
            var sellerFUSDReceiver = getAccount((orderData!).sellerAddr).getCapability(/public/fusdReceiver).borrow<&{FungibleToken.Receiver}>() ?? panic("Unable to borrow seller receiver reference")
            var feeReceiverFUSDReceiver = getAccount(self.transactionFeeReceiverAddr!).getCapability(/public/fusdReceiver).borrow<&{FungibleToken.Receiver}>() ?? panic("Unable to borrow fee receiver reference")
            ...
            var buyerNFTCap = getAccount(buyerAddr).getCapability(NyatheesOVO.CollectionPublicPath).borrow<&{NyatheesOVO.NFTCollectionPublic}>() ?? panic("Unable to borrow NyatheesOVO Collection of the seller")
        ...
        pub fun sellNFT(sellerAddr: Address, tokenName: String, totalPrice: UFix64, tokenId: UInt64, sellerNFTProvider: &NyatheesOVO.Collection{NonFungibleToken.Provider, NyatheesOVO.NFTCollectionPublic}){ 
            pre{ 
                tokenName != "":
        ...
        pub fun cancelOrder(orderId: UInt64, sellerAddr: Address){ 
            ...
            var sellerNFTCap = getAccount(sellerAddr).getCapability(NyatheesOVO.CollectionPublicPath).borrow<&{NyatheesOVO.NFTCollectionPublic}>() ?? panic("Unable to borrow NyatheesOVO Collection of the seller!")
    ...
    init(){ 
        ...
        self.account.link<&OVOMarketPlace.Market{MarketPublic}>(
            self.MarketPublicPath,
            target: self.MarketControllerStoragePath
        )
