...
pub contract MysteryBox{ 
    ...
    pub resource interface MysteryBoxControllerPrivate{ 
        ...
        // are from here
        pub fun setMysteryBoxNFTPrividerCap(
            prividerCap: Capability<&NyatheesOVO.NFTMinter>
        ){} 
        ...
        pub fun checkInviteRelationAndSaveIt(
            buyerAddr: Address,
            referrerAddr: Address
        ){} 
        ...
        pub fun checkStockAndSendNFTToBuyer(
            buyerAddr: Address,
    ...
    pub resource interface MysteryBoxControllerPublic{ 
        ...
        pub fun getMysteryBoxTypeItem(
            typeId: UInt64
        ): MysteryBox.MysteryBoxType?{} 
    }
    
    pub resource MysteryBoxController:
        MysteryBoxControllerPrivate,
        MysteryBoxControllerPublic{
        ...
        priv var mainFusdReceiverAddr: Address
        
        //capability to transfer NFT to users when they buy mystery box
        priv var nftProviderCap: Capability<&NyatheesOVO.NFTMinter>?
        ...
        pub fun buyMysteryBox(
            ...
            pre{ 
                mysteryBoxAmount > 0 && mysteryBoxAmount <= MysteryBox.maxMysteryBoxToBuy:
                    "wrong mystery box amount"
                tokenName != nil && tokenName != "":
                    "wrong token name"
                buyerAddr != nil && buyerAddr != MysteryBox.ZERO_ADDRESS:
                    "wrong buyer address"
                referrerAddr != buyerAddr:
                    "you can not use your own address as a referrer"
                buyerTokenVault != nil:
                    "wrong buyer vault"
                MysteryBox.mysteryBoxTypeList[mysteryBoxTypeId] != nil:
                    "wrong mysterybox type"
            }
            // calculate totalPrice, if user buy 10 mysterybox, 90% discount
            var totalPrice =
                (MysteryBox.mysteryBoxTypeList[mysteryBoxTypeId]!).unitPrice
                * UFix64(mysteryBoxAmount)
            ...
            assert(
                totalPrice >= buyerTokenVault.balance,
                message: "not enough balance to buy mystery box"
            )
            
            // check the buyer account info, if not exist, create a new one
            if !MysteryBox.accountList.containsKey(buyerAddr){ 
                MysteryBox.accountList[buyerAddr] = MysteryBox.accountItem(referrerAddr: MysteryBox.ZERO_ADDRESS, active: false, inviteAmount: 0 as UInt64, promotes:{} , earned:{} )
            }
            self.checkInviteRelationAndSaveIt(
                buyerAddr: buyerAddr,
        ...
        pub fun sendCommission(
            ...
            if referrerAddr != MysteryBox.ZERO_ADDRESS{ 
                ...
                var referrerCap = getAccount(referrerAddr).getCapability(/public/fusdReceiver).borrow<&{FungibleToken.Receiver}>() ?? panic("Unable to borrow receiver fusd reference")
                ...
                if MysteryBox.accountList.containsKey(referrerAddr){ 
                    ...
                    MysteryBox.accountList[referrerAddr] = MysteryBox.accountItem(referrerAddr: (MysteryBox.accountList[referrerAddr]!).referrerAddr, active: (MysteryBox.accountList[referrerAddr]!).active, inviteAmount: (MysteryBox.accountList[referrerAddr]!).inviteAmount, promotes: tempPromotes, earned: tempEarned)
                    ...
                    if lreferrerAddr != MysteryBox.ZERO_ADDRESS{ 
                        var lreferrerCap = getAccount(lreferrerAddr).getCapability(/public/fusdReceiver).borrow<&{FungibleToken.Receiver}>() ?? panic("Unable to borrow receiver reference")
                        ...
                        if MysteryBox.accountList.containsKey(lreferrerAddr){ 
                            ...
                            MysteryBox.accountList[lreferrerAddr] = MysteryBox.accountItem(referrerAddr: (MysteryBox.accountList[lreferrerAddr]!).referrerAddr, active: (MysteryBox.accountList[lreferrerAddr]!).active, inviteAmount: (MysteryBox.accountList[lreferrerAddr]!).inviteAmount, promotes: tempPromotes, earned: tempEarned)
                        }
                    }
                }
            }
        ...
        pub fun checkInviteRelationAndSaveIt(
            ...
            pre{ 
                MysteryBox.accountList.containsKey(buyerAddr):
                    "buyer account not exist"
            }
            var buyerAccount = MysteryBox.accountList[buyerAddr]
            // if the buyer has no referrer, save it
            if (buyerAccount!).referrerAddr == MysteryBox.ZERO_ADDRESS
            && referrerAddr != MysteryBox.ZERO_ADDRESS{ 
                MysteryBox.accountList[buyerAddr] = MysteryBox.accountItem(
                        referrerAddr: referrerAddr,
                        active: (MysteryBox.accountList[buyerAddr]!).active,
                        inviteAmount: (MysteryBox.accountList[buyerAddr]!)
                            .inviteAmount,
                ...
                if MysteryBox.accountList.containsKey(referrerAddr){ 
                    MysteryBox.accountList[referrerAddr] = MysteryBox.accountItem(referrerAddr: (MysteryBox.accountList[referrerAddr]!).referrerAddr, active: (MysteryBox.accountList[referrerAddr]!).active, inviteAmount: (MysteryBox.accountList[referrerAddr]!).inviteAmount + 1, promotes: (MysteryBox.accountList[referrerAddr]!).promotes, earned: (MysteryBox.accountList[referrerAddr]!).earned)
                    ...
                    if lreferrerAddr != nil && lreferrerAddr != MysteryBox.ZERO_ADDRESS && MysteryBox.accountList.containsKey(lreferrerAddr){ 
                        MysteryBox.accountList[lreferrerAddr] = MysteryBox.accountItem(referrerAddr: (MysteryBox.accountList[lreferrerAddr]!).referrerAddr, active: (MysteryBox.accountList[lreferrerAddr]!).active, inviteAmount: (MysteryBox.accountList[lreferrerAddr]!).inviteAmount + 1, promotes: (MysteryBox.accountList[lreferrerAddr]!).promotes, earned: (MysteryBox.accountList[lreferrerAddr]!).earned)
                    }
                }
            }
        }
        ...
        pub fun checkStockAndSendNFTToBuyer(
            ...
            pre{ 
                ...
                buyerAddr != nil && buyerAddr != MysteryBox.ZERO_ADDRESS:
                    "wrong buyer address"
                mysteryBoxTypeId > 0:
                    "wrong mystery box type"
                MysteryBox.mysteryBoxTypeList.containsKey(mysteryBoxTypeId):
                    "mystery box not exist"
                (MysteryBox.mysteryBoxTypeList[mysteryBoxTypeId]!).stock >= mysteryBoxAmount:
                    "mystery box not enough"
            }
            var mysteryBoxTypeItem =
                MysteryBox.mysteryBoxTypeList[mysteryBoxTypeId]
            var stock = (MysteryBox.mysteryBoxTypeList[mysteryBoxTypeId]!).stock
            var receiverCap =
                getAccount(buyerAddr).getCapability(
                    NyatheesOVO.CollectionPublicPath
                ).borrow<&{NonFungibleToken.CollectionPublic}>()
                ?? panic("Unable to borrow NyatheesOVO Receiver!")
            ...
            MysteryBox.mysteryBoxTypeList[mysteryBoxTypeId] = MysteryBox
                    .MysteryBoxType(
                    typeId: (MysteryBox.mysteryBoxTypeList[mysteryBoxTypeId]!)
                        .typeId,
                    describe: (MysteryBox.mysteryBoxTypeList[mysteryBoxTypeId]!)
                        .describe,
                    stock: stock,
                    unitPrice: (
                        MysteryBox.mysteryBoxTypeList[mysteryBoxTypeId]!
        ...
        pub fun awardToTop50(
            ...
            pre{ 
                ...
                addresses.length == amounts.length:
                    "length of addresses not equal to length of amounts"
                addresses.length > 0 && amounts.length > 0:
                    "length of addresses or amounts < 0"
            }
            var count = addresses.length
            if paymentVault.balance >= total{ 
                var index = 0
                while count > index{ 
                    var receiverCap = getAccount(addresses[index]).getCapability(/public/fusdReceiver).borrow<&{FungibleToken.Receiver}>() ?? panic("Unable to borrow receiver reference")
            ...
            var payerCap =
                getAccount(payer).getCapability(/public/fusdReceiver).borrow<
        ...
        pub fun setMysteryBoxTypeToList(
            ...
            pre{ 
                typeId > 0:
                    "MysteryBox type should > 0"
                describe != "":
                    "MysteryBox describe can not be empty"
                stock >= 0:
                    "MysteryBox stock should >= 0"
                unitPrice > 0.0:
                    "MysteryBox type should > 0.0"
            }
            MysteryBox.mysteryBoxTypeList[typeId] = MysteryBoxType(
                    typeId: typeId,
                    describe: describe,
                    stock: stock,
                    unitPrice: unitPrice
    ...
    init(){ 
        ...
        self
            .MysteryBoxControllerPrivatePath = /private/MysteryBoxControllerPrivate
        ...
        self.account.save(
            ...
            to: self.MysteryBoxControllerStoragePath
        )
        self.account.link<
            &MysteryBox.MysteryBoxController{MysteryBoxControllerPrivate}
        >(
            self.MysteryBoxControllerPrivatePath,
            target: self.MysteryBoxControllerStoragePath
        )
        self.account.link<
            &MysteryBox.MysteryBoxController{MysteryBoxControllerPublic}
        >(
            self.MysteryBoxControllerPublicPath,
            target: self.MysteryBoxControllerStoragePath
        )
    }
}
