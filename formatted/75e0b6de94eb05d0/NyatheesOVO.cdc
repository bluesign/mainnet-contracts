...
pub contract NyatheesOVO: NonFungibleToken{ 
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &NyatheesOVO.NFT?{ 
        let collection = (getAccount(from).getCapability(NyatheesOVO.CollectionPublicPath)!).borrow<&NyatheesOVO.Collection{NyatheesOVO.NFTCollectionPublic}>() ?? panic("Couldn't get collection")
