...
pub contract Vevent{ 
    ...
    pub resource Project: ProjectPublic{ 
        ...
        access(account) fun purchase(user: Address, vault: @DapperUtilityCoin.Vault){ 
            ...
            let ownerVault = getAccount(owner).getCapability(/public/dapperUtilityCoinReceiver).borrow<&{FungibleToken.Receiver}>() ?? panic("This is not a Dapper Wallet account.")
    ...
    pub resource Collection: CollectionPublic{ 
        ...
        pub fun createProject(prices:{ UFix64: UInt64}, verifier: [{Verifier}]){ 
        ...
        pub fun purchase(projectOwner: Address, projectId: UInt64, payment: @DapperUtilityCoin.Vault){ 
            let collection: &Collection{CollectionPublic} = getAccount(projectOwner).getCapability(Vevent.CollectionPublicPath).borrow<&Collection{CollectionPublic}>() ?? panic("This project owner does not have a collection set up or linked properly.")
            let project: &Project{ProjectPublic} = collection.getProjectPublic(projectId: projectId) ?? panic("Project with this id does not exist.")
