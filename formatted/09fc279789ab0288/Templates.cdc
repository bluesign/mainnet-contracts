...
pub contract Templates{ 
    ...
    pub fun getCounter(_ name: String): UInt64{ 
        return Templates.counters[name] ?? 0
    }
    
    pub event CountersReset()
    
    access(account) fun createEditionInfoManually(
        name: String,
        counter: String,
        edition: UInt64?
    ): EditionInfo{ 
    ...
    pub struct interface Editionable{ 
        ...
        pub fun getCounter(): String{ 
            return self.getContract().concat("_").concat(self.getClassifier())
                .concat("_").concat(self.getCounterSuffix())
        }
    ...
    pub struct Royalty{ 
        ...
        pub fun getRoyalty(): MetadataViews.Royalty{ 
            let cap =
                getAccount(self.address).getCapability<
