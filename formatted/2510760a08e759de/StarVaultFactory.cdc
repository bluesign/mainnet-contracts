...
pub contract StarVaultFactory{ 
    ...
    priv let vaults: [Address]
    
    priv let vaultMap:{ String: Address}
    ...
    pub event VaultTemplateAddressChanged(
        oldTemplate: Address,
        newTemplate: Address
    )
    
    pub event VaultAccountPublicKeyChanged(
        oldPublicKey: String?,
        newPublicKey: String?
    )
    ...
    pub fun createVault(
        ...
        let vaultTemplateContract =
            getAccount(self.vaultTemplate).contracts.get(name: "StarVault")!
        ...
        emit NewVault(
            ...
            numVaults: self.vaults.length
        )
        return vaultAddress
    }
    
    pub resource VaultTokenCollection:
        StarVaultInterfaces.VaultTokenCollectionPublic{
        ...
        pub fun deposit(vault: Address, tokenVault: @FungibleToken.Vault){ 
            ...
            let vaultPublicRef =
                getAccount(vault).getCapability<
                    &{StarVaultInterfaces.VaultPublic}
                >(StarVaultConfig.VaultPublicPath).borrow()!
            assert(
                tokenVault.getType() == vaultPublicRef.getVaultTokenType(),
                message: "VaultTokenCollection: input token vault type mismatch with token vault"
            )
            if self.tokenVaults.containsKey(vault){ 
                ...
                self.tokenVaults[vault]                                        // Array.slice() is not supported yet.
                                        <-! tokenVault
            }
        }
    ...
    init(vaultTemplate: Address, vaultAccountPublicKey: String){ 
        ...
        self.account.save(
            <-create Admin(),
            to: StarVaultConfig.FactoryAdminStoragePath
        )
    }
}
