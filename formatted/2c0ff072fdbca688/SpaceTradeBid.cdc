...
pub contract SpaceTradeBid{ 
    ...
    pub resource BidFulfilledBundle{ 
        ...
        pub fun transferNFTs(
            receivers:{ 
                String: Capability<&{NonFungibleToken.CollectionPublic}>
            }
        ){ 
            var index = 0
            while index < self.nfts.length{ 
    ...
    pub struct NFTProposals{ 
        ...
        priv let providerCapability: Capability<&{NonFungibleToken.Provider}>
        ...
        init(
            ...
            self.nftType = (
                    SpaceTradeAssetCatalog.getNFTCollectionMetadata(
                        collectionName
                    )!
                ).nftType
    ...
    pub struct NFTRequests{ 
        ...
        priv let bidderCollectionPublicCapability: Capability<
        ...
        init(
            ...
            bidderCollectionPublicCapability: Capability<
                &{NonFungibleToken.CollectionPublic}
            >
        ){ 
            pre{ 
            ...
            self
                .bidderCollectionPublicCapability = bidderCollectionPublicCapability
        }
    ...
    pub struct FTProposals{ 
        ...
        init(
            ...
            amount: UFix64,
            providerCapability: Capability<
                &{FungibleToken.Balance, FungibleToken.Provider}
            >
        ){ 
            pre{ 
                amount > 0.0:
            ...
            self.amount = amount
            self.tokenName = tokenName
            self.vaultType = (SpaceTradeAssetCatalog.getFTMetadata(tokenName)!)
                    .vaultType
            self.providerCapability = providerCapability
        }
        ...
        access(contract) fun withdraw(): @FTBundle{ 
            let providerTokenRef =
                ...
                ?? panic(
                    "Could not borrow token provider from bidder for: ".concat(
                        self.tokenName
                    )
                )
            ...
            assert(
                ...
                message: "Withdraw fungible tokens from bidder is not expected type for: "
                    .concat(self.tokenName)
            )
            assert(
    ...
    pub struct FTRequests{ 
        ...
        access(contract) fun transfer(
            ...
            let providerRef =
                providerCapability.borrow()
                ?? panic(
                    "Could not borrow token provider capability from bidder for: "
                        .concat(self.tokenName)
                )
            ...
            let requesterCollectionRef =
                ...
                ?? panic(
                    "Could not borrow fungible token receiver capability from bidder for: "
                        .concat(self.tokenName)
                )
    ...
    pub resource interface BidAccept{ 
        pub fun accept(
            ...
            feeProviderCapability: Capability<
                &{FungibleToken.Balance, FungibleToken.Provider}
            >?
        ): @BidFulfilledBundle{} 
    }
    ...
    pub resource Bid: BidDetails, BidAccept{ 
        ...
        priv fun verifyFee(_ bidderFeeProviderCapability: Capability<&{FungibleToken.Balance, FungibleToken.Provider}>?){ 
        ...
        pub fun accept(nftProviderCapabilities:{ String: Capability<&{NonFungibleToken.Provider}>}, ftProviderCapabilities:{ String: Capability<&{FungibleToken.Provider}>}, feeProviderCapability: Capability<&{FungibleToken.Balance, FungibleToken.Provider}>?): @BidFulfilledBundle{ 
            pre{ 
                self.open == true:
                    "Bid is closed"
        ...
        priv fun satisfyFulfiller(): @BidFulfilledBundle{ 
        ...
        priv fun satisfyBidder(nftProviderCapabilities:{ String: Capability<&{NonFungibleToken.Provider}>}, ftProviderCapabilities:{ String: Capability<&{FungibleToken.Provider}>}){ 
        ...
        priv fun handleFees(acceptorFeeProviderCapability: Capability<&{FungibleToken.Provider}>?){ 
    ...
    pub fun createBid(
        ...
        bidderFeeProviderCapability: Capability<
            &{FungibleToken.Balance, FungibleToken.Provider}
        >?,
        expiration: UFix64,
        lockedUntil: UFix64
