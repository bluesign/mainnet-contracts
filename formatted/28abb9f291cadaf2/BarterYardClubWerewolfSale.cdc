...
pub contract BarterYardClubWerewolfSale{ 
    ...
    priv let stages:{ StageName: Stage}
    
    priv let nftsForSale:{ UInt16: WerewolfInfo}
    
    priv let usedMintPass:{ UInt64: UInt8}
    
    /// saleWallet is the vault where funds will be deposited when an NFT is bought
    priv let saleWallet: Capability<&FlowToken.Vault{FungibleToken.Receiver}>
    
    // minter is a capability to allow users to mint using smart contract minting function
    priv let minter: Capability<&BarterYardClubWerewolf.Admin>
    
    pub event ContractInitialized()
    
    pub event BarterYardClubWerewolfPurchased(
        id: UInt64,
        stage: UInt8,
        remaining: UInt16,
        to: Address
    ...
    pub fun mint(
        ...
        while minted < numberToMint{ 
            ...
            let random = UInt16(Fix64(unsafeRandom() % 100000).saturatingDivide(99999.0).saturatingMultiply(Fix64(availableNfts.length)))
        ...
        let refund =
            buyTokens.balance
            - UFix64(
                UInt16(numberToMint) * UInt16((self.stages[saleStage]!).price)
            )
    ...
    init(){ 
        ...
        self.stages ={ 
                StageName.Alpha:
                Stage(name: StageName.Alpha, price: 19, maxSupply: 1000),
                StageName.Beta:
                Stage(name: StageName.Beta, price: 25, maxSupply: 2300),
                StageName.Public:
                Stage(name: StageName.Public, price: 31, maxSupply: 6500)
            }
        self.nftsForSale ={} 
        self.usedMintPass ={} 
        ...
        self.saleWallet = self.account.getCapability<
                &FlowToken.Vault{FungibleToken.Receiver}
            >(/public/flowTokenReceiver)
        self.minter = self.account.getCapability<&BarterYardClubWerewolf.Admin>(
