...
pub contract ArleePartner: NonFungibleToken{ 
    ...
    /* Query Function (Can also be done in Arlee Contract) */// return true if the address holds the ArleePartner NFT
    ...
    /* Admin Function */// Add a new recipient as a partner to receive royalty cut
    ...
    access(account) fun setMarketplaceCut(cut: UFix64){ 
        ...
        royaltyRed.cut = cut
        emit RoyaltyUpdated(creditor: "Arlequin", previousCut: oldRoyalty                                                                         // panic if the specific partner minting is disabled
                                                                         , newCut: cut)
    }
    
    access(account) fun setPartnerCut(partner: String, cut: UFix64){ 
        pre{ 
            ArleePartner.allRoyalties.containsKey(partner)                                                          // panic if the specific partner is missing, regardless of whether its mintable.
                                                          :
                "This creditor does not exist"
        }
        let royaltyRed =                         // Paths
                         // Royalty
                         // Setup Account
                         &ArleePartner.allRoyalties[partner] as &Royalty
        let oldRoyalty = royaltyRed.cut
        royaltyRed.cut = cut
