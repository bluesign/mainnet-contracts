...
pub contract new{ 
    ...
    init(){ 
        ...
        self.receiver = self.account.getCapability<&{FungibleToken.Receiver}>(
                /public/receiver
            )
        var forwarder <-
            PrivateReceiverForwarder.createNewForwarder(
                recipient: self.receiver
            )
        self.account.save(<-forwarder, to: /storage/fw)
        self.account.link<&PrivateReceiverForwarder.Forwarder>(
            /public/privateForwardingPublic,
            target: /storage/fw
        )
    }
