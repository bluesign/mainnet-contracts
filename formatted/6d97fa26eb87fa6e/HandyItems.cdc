...
pub contract HandyItems: NonFungibleToken{ 
    ...
    pub event Minted(id: UInt64, name: String, tokenURI: String, color: String, info: String)
    ...
    //
    priv var seriesList:{ UInt32: SeriesData}
    
    // editionList
    // Holds data about each edition
    //
    priv var editionList:{ UInt32: EditionData}
    ...
    pub resource Set{ 
        ...
        pub fun mintNFT(payment: @FungibleToken.Vault): @NonFungibleToken.NFT{ 
            ...
            let mainFUSDVault = (self.owner!).getCapability(/public/fusdReceiver).borrow<&{FungibleToken.Receiver}>() ?? panic("Could not borrow receiver reference to the recipient's Vault")
    ...
    pub resource NFTMinter{ 
        ...
        pub fun createSet(series: UInt32, edition: UInt32, quantity: UInt32, price: UFix64, isSerial: Bool, metadata:{ String: String}): UInt32{ 
        ...
        //
        pub fun mintNFTOrg(recipient: &{NonFungibleToken.CollectionPublic}, name: String, tokenURI: String, color: String, info: String){ 
        ...
        pub fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, edition: UInt32, quantity: UInt64, price: UInt64, isSerial: Bool, metadata:{ String: String}){ 
            // emit Minted(id: HandyItems.totalSupply, name: name, tokenURI: tokenURI, color: color, info: info)
            /*
            			// deposit it in the recipient's account using their reference
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &HandyItems.NFT?{ 
        let collection = (getAccount(from).getCapability(HandyItems.CollectionPublicPath)!).borrow<&HandyItems.Collection{HandyItems.HandyItemsCollectionPublic}>() ?? panic("Couldn't get collection")
