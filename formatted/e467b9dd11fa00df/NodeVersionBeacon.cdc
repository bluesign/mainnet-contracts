...
pub contract NodeVersionBeacon{ 
    ...
    pub struct Semver{ 
        ...
        pub fun toString(): String{ 
            let semverCoreString =
                self.major.toString().concat(".").concat(self.minor.toString())
                    .concat(".").concat(self.patch.toString())
        ...
        /* Custom Comparators *//// Returns true if Semver core is greater than
    ...
    pub resource Admin{ 
        ...
        pub fun setVersionBoundary(versionBoundary: VersionBoundary){ 
            ...
            let exists =
                NodeVersionBeacon.versionBoundary[versionBoundary.blockHeight]
                != nil
            NodeVersionBeacon.versionBoundary[
                versionBoundary.blockHeight
            ] = versionBoundary
            if exists{ 
            ...
            while i > 1
            && NodeVersionBeacon.versionBoundaryBlockList[i - 1]
            > versionBoundary.blockHeight{ 
                i = i - 1
            }
            NodeVersionBeacon.versionBoundaryBlockList.insert(
                at: i,
                versionBoundary.blockHeight
            )
        ...
        pub fun deleteVersionBoundary(blockHeight: UInt64){ 
            pre{ 
                ...
                NodeVersionBeacon.versionBoundary.containsKey(blockHeight):
                    "No boundary defined at that blockHeight."
            }
            // Set the flag to true so the event will be emitted next time emitChanges is called
            NodeVersionBeacon.emitEventOnNextHeartbeat = true
            ...
            while i > 0
            && NodeVersionBeacon.versionBoundaryBlockList[i] > blockHeight{ 
                i = i - 1
            }
            assert(
                NodeVersionBeacon.versionBoundaryBlockList[i] == blockHeight,
            ...
            if NodeVersionBeacon.firstUpcomingBoundary != nil
            && NodeVersionBeacon.firstUpcomingBoundary!
            >= UInt64(NodeVersionBeacon.versionBoundaryBlockList.length){ 
        ...
        pub fun setVersionBoundaryFreezePeriod(newFreezePeriod: UInt64){ 
            post{ 
                NodeVersionBeacon.versionBoundaryFreezePeriod == newFreezePeriod:
                    "Update buffer was not properly set!"
            }
            ...
            let nextBlockBoundary =
                NodeVersionBeacon.versionBoundaryBlockList[
                    NodeVersionBeacon.firstUpcomingBoundary!
                ]
            ...
            assert(
                currentBlockHeight
                + NodeVersionBeacon.versionBoundaryFreezePeriod
                < nextBlockBoundary
            ...
            NodeVersionBeacon.versionBoundaryFreezePeriod = newFreezePeriod
            emit NodeVersionBoundaryFreezePeriodChanged(
    ...
    pub resource Heartbeat{ 
        ...
        pub fun heartbeat(){ 
            ...
            self.emitVersionBeaconEvent(
                versionBoundaries: NodeVersionBeacon
                    .getCurrentVersionBoundaries()
            )
        }
        
        priv fun emitVersionBeaconEvent(versionBoundaries: [VersionBoundary]){ 
            emit VersionBeacon(
            ...
            NodeVersionBeacon.nextVersionBeaconEventSequence = NodeVersionBeacon
                    .nextVersionBeaconEventSequence
                + 1
        }
        
        /// Check if the index pointing to the next version boundary needs to be moved.
        priv fun checkFirstUpcomingBoundary(){ 
            if NodeVersionBeacon.firstUpcomingBoundary == nil{ 
                return
            }
            let currentBlockHeight = getCurrentBlock().height
            ...
            while boundaryIndex
            < UInt64(NodeVersionBeacon.versionBoundaryBlockList.length)
            && NodeVersionBeacon.versionBoundaryBlockList[boundaryIndex]
            <= currentBlockHeight{ 
    ...
    pub fun getCurrentVersionBoundaries(): [VersionBoundary]{ 
        ...
        if NodeVersionBeacon.firstUpcomingBoundary == nil{ 
            ...
            tableUpdates.append(NodeVersionBeacon.versionBoundary[NodeVersionBeacon.versionBoundaryBlockList[NodeVersionBeacon.versionBoundaryBlockList.length - 1]]!)
            return tableUpdates
        }
    ...
    pub fun getVersionBoundary(
        effectiveAtBlockHeight: UInt64
    ): VersionBoundary{ 
        let block =
            self.searchForClosestHistoricalBlockBoundary(
                blockHeight: effectiveAtBlockHeight
            )
        return self.versionBoundary[block]!
    }
    ...
    pub fun getVersionBoundariesPage(
        page: Int,
        perPage: Int
    ): VersionBoundaryPage{ 
        pre{ 
            ...
            perPage > 0:
                "perPage must be greater than 0"
        }
        ...
        for block in self.versionBoundaryBlockList.slice(
            from: startIndex,
            upTo: endIndex
        ){ 
            values.append(NodeVersionBeacon.versionBoundary[block]!)
        }
        return VersionBoundaryPage(
            page: page,
            perPage: perPage,
    ...
    access(contract) fun searchForClosestHistoricalBlockBoundary(
        blockHeight: UInt64
    ): UInt64{ 
