...
pub contract FlowStorageFees{ 
    ...
    pub event StorageMegaBytesPerReservedFLOWChanged(
        _ storageMegaBytesPerReservedFLOW: UFix64
    )
    
    // Emitted when the minimum amount of Flow tokens that an account needs to have reserved for storage capacity changes.
    pub event MinimumStorageReservationChanged(
        _ minimumStorageReservation: UFix64
    )
    ...
    pub resource Administrator{ 
        ...
        pub fun setStorageMegaBytesPerReservedFLOW(
            _ storageMegaBytesPerReservedFLOW: UFix64
        ){ 
            if FlowStorageFees.storageMegaBytesPerReservedFLOW
            == storageMegaBytesPerReservedFLOW{ 
                return
            }
            FlowStorageFees
                .storageMegaBytesPerReservedFLOW = storageMegaBytesPerReservedFLOW
            emit StorageMegaBytesPerReservedFLOWChanged(
                storageMegaBytesPerReservedFLOW
            )
        }
        
        // Changes the minimum amount of FLOW an account has to have reserved.
        pub fun setMinimumStorageReservation(
            _ minimumStorageReservation: UFix64
        ){ 
            if FlowStorageFees.minimumStorageReservation
    ...
    pub fun calculateAccountCapacity(_ accountAddress: Address): UFix64{ 
        ...
        if let balanceRef =
            (
                getAccount(accountAddress).getCapability<
    ...
    pub fun getAccountsCapacityForTransactionStorageCheck(
        accountAddresses: [
            Address
        ],
        payer: Address,
        ...
        for accountAddress in accountAddresses{ 
            var balance = 0.0
            if let balanceRef = (getAccount(accountAddress).getCapability<&FlowToken.Vault{FungibleToken.Balance}>(/public/flowTokenBalance)!).borrow(){ 
    ...
    pub fun flowToStorageCapacity(_ amount: UFix64): UFix64{ 
        return amount.saturatingMultiply(
            FlowStorageFees.storageMegaBytesPerReservedFLOW
        )
    }
    ...
    pub fun convertUInt64StorageBytesToUFix64Megabytes(
        _ storage: UInt64
    ): UFix64{ 
        // safe convert UInt64 to UFix64 (without overflow)
        let f =
            UFix64(storage % 100000000 as UInt64) * 0.00000001 as UFix64
            + UFix64(storage / 100000000 as UInt64)
    ...
    pub fun defaultTokenAvailableBalance(_ accountAddress: Address): UFix64{ 
        ...
        var balance = 0.0
        if let balanceRef =
            acct.getCapability(/public/flowTokenBalance).borrow<
                &FlowToken.Vault{FungibleToken.Balance}
            >(){ 
            balance = balanceRef.balance
        }
    ...
    pub fun defaultTokenReservedBalance(_ accountAddress: Address): UFix64{ 
        let acct = getAccount(accountAddress)
        var reserved =
            self.storageCapacityToFlow(
                self.convertUInt64StorageBytesToUFix64Megabytes(
                    acct.storageUsed
                )
            )
