...
pub contract DayNFT: NonFungibleToken{ 
    ...
    pub resource ContractManager{ 
            ...
            if self.bestBid.date.equals(today) || self.bestBid.vault.balance == 0.0{ 
                if bid.vault.balance > self.bestBid.vault.balance{ 
                    if self.bestBid.vault.balance > 0.0{ 
                        ...
                        let rec = getAccount(self.bestBid.recipient).getCapability(/public/flowTokenReceiver).borrow<&FlowToken.Vault{FungibleToken.Receiver}>() ?? panic("Could not borrow a reference to the receiver")
                    ...
                    // Refund the new bid
                    let rec = getAccount(bid.recipient).getCapability(/public/flowTokenReceiver).borrow<&FlowToken.Vault{FungibleToken.Receiver}>() ?? panic("Could not borrow a reference to the receiver")
                    var tempVault <- FlowToken.createEmptyVault() as! @FlowToken.Vault
                    tempVault <-> bid.vault
                    rec.deposit(from: <-tempVault)
                }
        ...
        pub fun claimNFTsWithToday(address: Address, today: DateUtils.Date): Int{ 
            var res = 0
            let receiver = getAccount(address).getCapability(DayNFT.CollectionPublicPath).borrow<&{DayNFT.CollectionPublic}>() ?? panic("Could not get receiver reference to the NFT Collection")
            if self.NFTsDue[address] != nil{ 
                var a = 0
        ...
        pub fun tokensToClaim(address: Address): UFix64{ 
            ...
            let holder = getAccount(address).getCapability(DayNFT.CollectionPublicPath).borrow<&DayNFT.Collection{DayNFT.CollectionPublic}>() ?? panic("Could not get receiver reference to the NFT Collection")
        ...
        pub fun claimTokens(address: Address): UFix64{ 
            ...
            let receiver = getAccount(address).getCapability(/public/flowTokenReceiver).borrow<&FlowToken.Vault{FungibleToken.Receiver}>() ?? panic("Could not borrow a reference to the receiver")
        ...
        pub fun deposit(vault: @FungibleToken.Vault){ 
            ...
            let rec = getAccount(self.contractAddress).getCapability(/public/flowTokenReceiver).borrow<&FlowToken.Vault{FungibleToken.Receiver}>() ?? panic("Could not borrow a reference to the Flow receiver")
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: DayNFT.CollectionStoragePath, publicPath: DayNFT.CollectionPublicPath, providerPath: /private/DayNFTCollectionProvider, publicCollection: Type<&DayNFT.Collection{DayNFT.CollectionPublic}>(), publicLinkedType: Type<&DayNFT.Collection{DayNFT.CollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&DayNFT.Collection{DayNFT.CollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Provider, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
    ...
    pub fun makeBid(vault: @FlowToken.Vault, recipient: Address, title: String, date: DateUtils.Date){ 
    ...
    access(contract) fun makeBidWithToday(vault: @FlowToken.Vault, recipient: Address, title: String, date: DateUtils.Date, today: DateUtils.Date){ 
    ...
    init(){ 
        ...
        self.account.link<&DayNFT.Admin{FungibleToken.Receiver}>(self.AdminPublicPath, target: adminStoragePath)
        self.totalSupply = 0
