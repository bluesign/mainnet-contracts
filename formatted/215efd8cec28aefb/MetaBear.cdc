...
pub contract MetaBear: NonFungibleToken{ 
    ...
    pub resource NFTMinter{ 
        ...
        //
        pub fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, communityFeeVault: @FungibleToken.Vault, creatorFeeVault: @FungibleToken.Vault, platformFeeVault: @FungibleToken.Vault){ 
            pre{ 
            ...
            let ownerRef = getAccount(community).getCapability(/public/fusdReceiver).borrow<&FUSD.Vault{FungibleToken.Receiver}>() ?? panic("Could not borrow receiver reference to the owner's Vault")
            let creatorRef = getAccount(creator).getCapability(/public/fusdReceiver).borrow<&FUSD.Vault{FungibleToken.Receiver}>() ?? panic("Could not borrow receiver reference to the creator's Vault")
            let platformRef = getAccount(platform).getCapability(/public/fusdReceiver).borrow<&FUSD.Vault{FungibleToken.Receiver}>() ?? panic("Could not borrow receiver reference to the platform's Vault")
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &MetaBear.NFT?{ 
        let collection = getAccount(from).getCapability(MetaBear.CollectionPublicPath).borrow<&MetaBear.Collection{MetaBear.MetaBearCollectionPublic}>() ?? panic("Couldn't get collection")
    ...
    pub resource CollectionData: CollectionDataPublic{ 
        ...
        priv var settings:{ String: AnyStruct}
    ...
    init(){ 
        ...
        let collectionDataCap = self.account.getCapability<&{MetaBear.CollectionDataPublic}>(self.CollectionDataPublicPath) as Capability<&{MetaBear.CollectionDataPublic}>
