...
pub contract PopsyclAuction{ 
    ...
    // Events
    pub event TokenAddedToAuctionItems(
        auctionID: UInt64,
        startPrice: UFix64,
    ...
    pub resource AuctionItem{ 
        ...
        priv let influencer: Address?
        
        priv let royalty: UFix64
        
        // Recipient's Receiver Capabilities
        priv var recipientCollectionCap: Capability<
            &{Popsycl.PopsyclCollectionPublic}
        >
        ...
        init(
            ...
            influencer: Address?,
            royalty: UFix64,
            ownerCollectionCap: Capability<&{Popsycl.PopsyclCollectionPublic}>,
        ...
        access(contract) fun sendNFT(
            _ capability: Capability<&{Popsycl.PopsyclCollectionPublic}>
        ){ 
        ...
        access(contract) fun sendBidTokens(
            ...
            if let vaultRef = capability.borrow(){ 
                ...
                let influencerVaultRef = getAccount(self.influencer!!).getCapability(/public/flowTokenReceiver).borrow<&{FungibleToken.Receiver}>() ?? panic("failed to borrow reference to owner vault")
                var balance = 0.0
                if sale{ 
                    let marketShare = bidVaultRef.balance / 100.0 * PopsyclMarketplace.marketFee
                    let royalityShare = bidVaultRef.balance / 100.0 * self.royalty
                    ...
                    let PopsyclvaultRef = getAccount(PopsyclMarketplace.marketAddress).getCapability(/public/flowTokenReceiver).borrow<&{FungibleToken.Receiver}>() ?? panic("failed to borrow reference to Marketplace vault")
        ...
        pub fun placeBid(
            ...
            collectionCap: Capability<&{Popsycl.PopsyclCollectionPublic}>
        ){ 
            pre{ 
                !self.auctionCompleted:
                    "The auction is already settled"
            ...
            if bidTokens.balance
            < self.currentPrice + self.minimumBidIncrement{ 
    ...
    pub resource AuctionCollection: AuctionPublic{ 
        ...
        pub fun addTokenToAuctionItems(token: @NonFungibleToken.NFT, minimumBidIncrement: UFix64, startPrice: UFix64, bidVault: @FungibleToken.Vault, collectionCap: Capability<&{Popsycl.PopsyclCollectionPublic}>, vaultCap: Capability<&{FungibleToken.Receiver}>, endTime: Fix64){ 
            pre{ 
            ...
            let bidtoken <- token as! @Popsycl.NFT
            let tokenID = bidtoken.id
            let resale = bidtoken.creator == self.owner?.address ? false : true
            let creator = bidtoken.creator
            let influencer = bidtoken.influencer
            let royalty = bidtoken.royality
            let itemToken <- bidtoken as! @NonFungibleToken.NFT
            PopsyclAuction.totalAuctions = PopsyclAuction.totalAuctions + UInt64(1)
            let id = PopsyclAuction.totalAuctions
            ...
            let item <- create AuctionItem(NFT: <-itemToken, bidVault: <-bidVault, auctionID: id, minimumBidIncrement: minimumBidIncrement, startPrice: startPrice, auctionStartBlock: startBlock, startTime: startTime, endTime: endTime, resale: resale, creator: creator, influencer: influencer, royalty: royalty, ownerCollectionCap: collectionCap, ownerVaultCap: vaultCap)
        ...
        pub fun getAuctionStatus(_                                   
                                   // settleAuction sends the auction item to the highest bidder
                                   // and deposits the FungibleTokens into the auction owner's account
                                   id: UInt64): AuctionStatus{ 
            pre{ 
            ...
            let status                       
                       ...
                       // Get the auction item resources
                       = itemRef.getAuctionStatus()
            return status
        }
        
        pub            // destroy the empty resources
            fun getAuctionKeys()                                
        ...
        pub fun placeBid(id: UInt64, bidTokens: @FungibleToken.Vault, vaultCap: Capability<&{FungibleToken.Receiver}>, collectionCap: Capability<&{Popsycl.PopsyclCollectionPublic}>){ 
            pre{ 
                self.auctionItems[id] != nil:
                    "Auction doesn't exist"
            }
    ...
    init(){ 
        ...
        self.auctionStoragePath = /storage/PopsyclAuction
