...
pub contract PopsyclMarketplace{ 
    ...
    pub resource SaleCollection: SalePublic{ 
        ...
        priv let prices:{ UInt64: UFix64}
        
        // Dictionary of the prices for each NFT by ID
        priv let creators:{ UInt64: Address?}
        
        // Dictionary of the prices for each NFT by ID
        priv let influencer:{ UInt64: Address?}
        
        // Dictionary of sale type (either sale/resale)
        priv let resale:{ UInt64: Bool}
        
        priv let roylty:{ UInt64: UFix64}
        
        // Dictionary of token owner
        priv let seller:{ UInt64: Address?}
        ...
        pub fun purchaseGroup(tokens: [UInt64], recipient: &{Popsycl.PopsyclCollectionPublic}, payment: @FungibleToken.Vault){ 
            ...
            let PopsyclvaultRef = getAccount(PopsyclMarketplace.marketAddress).getCapability(/public/flowTokenReceiver).borrow<&{FungibleToken.Receiver}>() ?? panic("failed to borrow reference to PopsyclMarketplace vault")
        ...
        pub fun saleWithdrawn(tokens: [UInt64]){ 
            ...
            let receiver = getAccount(owner).getCapability(Popsycl.PopsyclPublicPath).borrow<&{Popsycl.PopsyclCollectionPublic}>() ?? panic("Could not get receiver reference to the NFT Collection")
            while count < tokens.length{ 
