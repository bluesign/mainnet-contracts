...
pub contract SportsIconNFTStorefront{ 
    ...
    pub struct ListingDetails{ 
        ...
        init(
            ...
            assert(
                ...
                message: "End time can not be set to a past time"
            )
        }
    }
    ...
    pub resource Listing: ListingPublic{ 
        ...
        pub fun purchase(payment: @FungibleToken.Vault): @NonFungibleToken.NFT{ 
            pre{ 
                ...
                getCurrentBlock().timestamp >= self.details.startTime:
                    "Sale has not started for this listing"
                getCurrentBlock().timestamp < self.details.endTime:
                    "Sale has expired"
            }
            ...
            emit ListingCompleted(storefrontAddress: self.owner?.address, listingResourceID: self.uuid, storefrontResourceID: self.details.storefrontID, nftType: self.details.nftType, nftID: self.details.nftID, ftVaultType: self.details.salePaymentVaultType, priceTransacted: self.details.salePrice, purchased: self.details.purchased)
            return <-nft
        }
        ...
        destroy(){ 
            ...
            if !self.details.purchased{ 
                emit ListingCompleted(storefrontAddress: self.owner?.address, listingResourceID: self.uuid, storefrontResourceID: self.details.storefrontID, nftType: self.details.nftType, nftID: self.details.nftID, ftVaultType: self.details.salePaymentVaultType, priceTransacted: 0.0, purchased: self.details.purchased)
            }
        }
        
        // initializer
        //
        init(nftProviderCapability: Capability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>, nftType: Type, nftID: UInt64, salePaymentVaultType: Type, saleCuts: [SaleCut], storefrontID: UInt64, startTime: UFix64, endTime: UFix64){ 
            // Store the sale information
            self.details = ListingDetails(nftType: nftType, nftID: nftID, salePaymentVaultType: salePaymentVaultType, saleCuts: saleCuts, storefrontID: storefrontID, startTime: startTime, endTime: endTime)
    ...
    pub resource Storefront: StorefrontManager, StorefrontPublic{ 
        ...
        //
        pub fun createListing(nftProviderCapability: Capability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>, nftType: Type, nftID: UInt64, salePaymentVaultType: Type, saleCuts: [SaleCut], startTime: UFix64, endTime: UFix64): UInt64{ 
            let listing <- create Listing(nftProviderCapability: nftProviderCapability, nftType: nftType, nftID: nftID, salePaymentVaultType: salePaymentVaultType, saleCuts: saleCuts, storefrontID: self.uuid, startTime: startTime, endTime: endTime)
