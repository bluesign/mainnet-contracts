/*
Central Smart Contract for NextName x Niftory Culture Icons Collectibles
...
pub contract NextName: NonFungibleToken{ 
    ...
    priv var seriesDatas:{ UInt32: Series}
    
    // Variable size dictionary of CollectibleItem structs
    priv var collectibleItemDatas:{ UInt32: CollectibleItem}
    ...
    pub struct CurrSeriesData{ 
        ...
        init(){ 
            var referencedSeries = (&NextName.seriesDatas[NextName.currentSeriesID] as &Series?)!
    ...
    pub struct CollectibleItem{ 
        ...
        init(metadata:{ String: String}, featuredArtists: [String]){ 
            ...
            self.collectibleItemID = NextName.nextCollectibleItemID
            self.metadata = metadata
            self.featuredArtists = featuredArtists
            
            // Increment the ID so that it isn't used again
            NextName.nextCollectibleItemID = NextName.nextCollectibleItemID + UInt32(1)
    ...
    pub struct SetData{ 
        ...
        init(setID: UInt32){ 
            pre{ 
                NextName.sets[setID] != nil:
                    "Cannot borrow Set: The Set doesn't exist"
            }
            let referencedSet = (&NextName.sets[setID] as &Set?)!
    ...
    pub resource Set{ 
        ...
        init(name: String, setIdentityURL: String?, description: String?){ 
            ...
            self.setID = NextName.nextSetID
            self.name = name
            self.setIdentityURL = setIdentityURL
            self.description = description
            self.series = NextName.seriesDatas[NextName.currentSeriesID]!
            self.collectibleItems = []
            self.retired ={} 
            self.locked = false
            self.numberMintedPerCollectibleItem ={} 
            NextName.nextSetID = NextName.nextSetID + UInt32(1)
        ...
        pub fun addCollectibleItem(collectibleItemID: UInt32){ 
            pre{ 
                NextName.collectibleItemDatas[collectibleItemID] != nil:
    ...
    pub resource Admin{ 
        ...
        // and stores it in the CollectibleItems dictionary in the NextName smart contract
        //
        ...
        pub fun createCollectibleItem(metadata:{ String: String}, featuredArtists: [String]): UInt32{ 
            ...
            NextName.collectibleItemDatas[newID] = newCollectibleItem
            return newID
        }
        
        // createSet creates a new Set resource and stores it
        // in the sets mapping in the NextName contract
        //
        ...
        pub fun createSet(name: String, setIdentityURL: String?, description: String?){ 
            ...
            NextName.sets[newSet.setID] <-! newSet
        }
        
        // borrowSet returns a reference to a set in the NextName
        ...
        pub fun borrowSet(setID: UInt32): &Set{ 
            ...
            return (&NextName.sets[setID] as &Set?)!
        }
        ...
        pub fun startNewSeries(name: String?, identityURL: String?): UInt32{ 
            ...
            // by incrementing the NextName series number
            let setIDs = NextName.sets.keys
            var i: Int = 0
            while i < setIDs.length{ 
                var currSet = SetData(setID: setIDs[i])
                if currSet.series.seriesID == NextName.currentSeriesID{ 
                    self.borrowSet(setID: setIDs[i]).retireAll()
                    self.borrowSet(setID: setIDs[i]).lock()
                }
                i = i + 1
            }
            var newSeries = Series(seriesID: NextName.currentSeriesID + UInt32(1), name: name, seriesIdentityURL: identityURL)
            NextName.currentSeriesID = newSeries.seriesID
            
            //put it in storage
            NextName.seriesDatas[NextName.currentSeriesID] = newSeries
            emit NewSeriesStarted(newCurrentSeries: NextName.currentSeriesID)
            return NextName.currentSeriesID
        }
    ...
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver{ 
        ...
        pub fun getViews(): [Type]{ 
            return [Type<MetadataViews.Display>(), Type<MetadataViews.Royalties>(), Type<MetadataViews.NFTCollectionData>(), Type<MetadataViews.NFTCollectionDisplay>(), Type<MetadataViews.ExternalURL>()]
        }
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                case Type<MetadataViews.Display>():
                    ...
                    let metadata = NextName.getCollectibleItemMetaData(collectibleItemID: collectibleItemID)!
                    
                    // The metadata URL may or may not have a prefix. If it doesn't, then
                    // assume `ipfs://`
                    var url = metadata["posterUrl"] ?? "https://cloudflare-ipfs.com/ipfs/bafybeifbnckgntaaq3oknp4ygidnalwdupmv6fhdiip3cdqoaxekxhvfei/nextname-logo.png"
                ...
                case Type<MetadataViews.Royalties>():
                    let receiver = NextName.account.getCapability<&AnyResource{FungibleToken.Receiver}>(/public/flowTokenReceiver)
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: NextName.CollectionStoragePath, publicPath: NextName.CollectionPublicPath, providerPath: /private/NextNameCollection001, publicCollection: Type<&NextName.Collection{NextName.NextNameCollectionPublic}>(), publicLinkedType: Type<&NextName.Collection{NextName.NextNameCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&NextName.Collection{NextName.NextNameCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Provider, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
                            return <-NextName.createEmptyCollection()
                        })
                case Type<MetadataViews.NFTCollectionDisplay>():
                    return MetadataViews.NFTCollectionDisplay(name: "NextName", description: "NextName NFT Collection. Support Your Players.", externalURL: MetadataViews.ExternalURL(url: "https://nextname.io/"), squareImage: MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://cloudflare-ipfs.com/ipfs/bafybeifbnckgntaaq3oknp4ygidnalwdupmv6fhdiip3cdqoaxekxhvfei/nextname-logo.png"), mediaType: "image/png"), bannerImage: MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://cloudflare-ipfs.com/ipfs/bafybeihyk7jgd2bhkvztqogoilgfc42gbx2mmxnb5pwvijqquaj5gdemmu/banner.jpg"), mediaType: "image/png"), socials:{} )
                case Type<MetadataViews.ExternalURL>():
                    return MetadataViews.ExternalURL(url: "https://nextname.io/")
            }
            return nil
        }
        ...
        init(serialNumber: UInt32, collectibleItemID: UInt32, setID: UInt32){ 
            ...
            NextName.totalSupply = NextName.totalSupply + UInt64(1)
            self.id = NextName.totalSupply
    ...
    pub resource interface NextNameCollectionPublic{ 
        ...
        pub fun borrowCollectible(id: UInt64): &NextName.NFT?{ 
    ...
    pub resource Collection: NextNameCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection{ 
        ...
        pub fun deposit(token: @NonFungibleToken.NFT){ 
            ...
            // Cast the deposited token as a NextName NFT to make sure
            // it is the correct type
            let token <- token as! @NextName.NFT
        ...
        // not any NextName specific data. Please use borrowCollectible to
    ...
    pub fun createEmptyCollection(): @NonFungibleToken.Collection{ 
        return <-create NextName.Collection()
    }
    
    // getAllCollectibleItems returns all the collectibleItems in NextName
    //
    // Returns: An array of all the collectibleItems that have been created
    pub fun getAllCollectibleItems(): [NextName.CollectibleItem]{ 
        return NextName.collectibleItemDatas.values
    }
    ...
    pub fun getCollectibleItemMetaDataByField(collectibleItemID: UInt32, field: String): String?{ 
        ...
        if let collectibleItem = NextName.collectibleItemDatas[collectibleItemID]{ 
    ...
    pub fun getCollectibleItemsInSet(setID: UInt32): [UInt32]?{ 
        ...
        return NextName.sets[setID]?.collectibleItems
    }
    ...
    pub fun isEditionRetired(setID: UInt32, collectibleItemID: UInt32): Bool?{ 
        ...
        if let setToRead <- NextName.sets.remove(key: setID){ 
    ...
    pub fun isSetLocked(setID: UInt32): Bool?{ 
        // Don't force a revert if the setID is invalid
        return NextName.sets[setID]?.locked
    }
    ...
    init(){ 
        ...
        self.seriesDatas ={} 
        self.seriesDatas[self.currentSeriesID] = Series(seriesID: NextName.currentSeriesID, name: nil, seriesIdentityURL: nil)
        ...
        // initialize paths
        // Set our named paths
        self.CollectionStoragePath = /storage/NextNameCollection001
        self.CollectionPublicPath = /public/NextNameCollection001
        self.AdminStoragePath = /storage/NextNameAdmin005
        
        // Put a new Collection in storage
        self.account.save<@Collection>(<-create Collection(), to: self.CollectionStoragePath)
        
        // Create a public capability for the Collection
        self.account.link<&{NextNameCollectionPublic}>(self.CollectionPublicPath, target: self.CollectionStoragePath)
