...
pub contract DynamicNFT: NonFungibleToken{ 
    ...
    pub struct NFTMetadata{ 
        ...
        priv let metadata: TraderflowScores.TradeMetadata
    ...
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver{ 
        ...
        priv let trades: TraderflowScores.TradeScores
    ...
    pub resource Administrator{ 
        pub fun mintNFT(recipient: &Collection{NonFungibleToken.Receiver}, name: String, description: String, thumbnail: String){ 
        ...
        pub fun pushTrade(id: UInt64, currentOwner: Address, trade: TraderflowScores.Trade){ 
            let ownerCollection = getAccount(currentOwner).getCapability(DynamicNFT.CollectionPublicPath).borrow<&Collection{CollectionPublic}>() ?? panic("This person does not have a DynamicNFT Collection set up properly.")
        ...
        pub fun pushEquity(id: UInt64, currentOwner: Address, equity: UFix64){ 
            let ownerCollection = getAccount(currentOwner).getCapability(DynamicNFT.CollectionPublicPath).borrow<&Collection{CollectionPublic}>() ?? panic("This person does not have a DynamicNFT Collection set up properly.")
            let nftRef = ownerCollection.borrowAuthNFT(id: id) ?? panic("This account does not own an NFT with this id.")
            let tradeRef = nftRef.borrowTradesRef()
        ...
        pub fun updateArtwork(id: UInt64, currentOwner: Address, ipfs: String){ 
            let ownerCollection = getAccount(currentOwner).getCapability(DynamicNFT.CollectionPublicPath).borrow<&Collection{CollectionPublic}>() ?? panic("This person does not have a DynamicNFT Collection set up properly.")
            let nftRef = ownerCollection.borrowAuthNFT(id: id) ?? panic("This account does not own an NFT with this id.")
