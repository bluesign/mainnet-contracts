...
pub contract TraderflowScores{ 
    ...
    pub struct TradeScores{ 
        /* Log of trades and counts */        priv var historical: [Trade]
        ...
        /* Moving average of % profit */        access(contract) var average_long_profit_ema150: UFix64
        ...
        /* Log of equity and totals */        priv var historical_equity: [Equity]
        ...
        pub fun pushEquity(_equity: UFix64): TradeMetadataRebuild{ 
            ...
            /* Determine if the NFT needs to be rebuilt */            let newMetadata = self.metadata()
        ...
        pub fun pushTrade(_trade: Trade): TradeMetadataRebuild{ 
            ...
            /* Calculate the running totals for completed trades */            if _trade.closeTime != 0{ // Has the trade completed 
                ...
                if open != nil{ 
                    ...
                    /* Find minimum and maximum equity value for the duration of the open trade */                    var minmax: [UFix64] = self.equityMinMaxBetween(start: start, end: end)
                    _trade.minEquity = minmax[0]
                    _trade.maxEquity = minmax[1]
                    /* Calculate 150 trade exponential moving average percentage for profit and loss */                    let ema = 2.0 / 150.0
                    let invEma = 1.0 - ema
            ...
            /* Every trade is added to the historical data to allow for verification */            self.historical.append(_trade)
            /* Determine if the NFT needs to be rebuilt */            let newMetadata = self.metadata()
            return TradeMetadataRebuild(
                _metadata: newMetadata,
                _rebuild: !oldMetadata.equal(md: newMetadata)
        ...
        /* INTERMEDIATE CALCULATIONS *//* Return last recorded equity value */        pub fun Equity(): UFix64{ 
        ...
        pub fun WinRate(): UFix64{ 
            var ptotal: UFix64 =
                UFix64(self.positive_long_total + self.positive_short_total)
            var total: UFix64 =
                ptotal
                + UFix64(self.negative_long_total + self.negative_short_total)
        ...
        /* ACHIEVEMENTS */        pub fun Provisional_Achievement(): Bool{ 
            /* Duration of 60 days in seconds */            let sixty_days: UFix64 = 60.0 * 24.0 * 60.0 * 60.0
            /* If a user has less than 50 trades they are provisional */            if self.historical.length < 50{ 
                return true
            }
            /* Additionally if they have less than 60 days of trades on chain they are provisional */            if self.historical[0].onchain + sixty_days
