...
pub contract BridgeManager{ 
    ...
    pub resource LockUp: LockUpPublic, LockUpPrivate{ 
        priv let holder: Address
        
        priv var releasedAt: UInt64
        
        priv var createdAt: UInt64
        
        priv var name: String
        
        priv var description: String
        
        priv var recipient: Address
        
        priv let ftLockUps:{ String: FTLockUp}
        
        priv let nftLockUps:{ String: NFTLockUp}
        ...
        pub fun withdrawFT(identifier: String, amount: UFix64, receiver: Capability<&{FungibleToken.Receiver}>, feeTokens: @FungibleToken.Vault){ 
        ...
        pub fun withdrawNFT(identifier: String, receiver: Capability<&{NonFungibleToken.Receiver}>, feeTokens: @FungibleToken.Vault, nftIDs: [UInt64]?){ 
            let currentTime = UInt64(getCurrentBlock().timestamp)
            // if self.releasedAt > currentTime {
            //     panic("The assets are still in lock-up period!")
            // }
        ...
        pub fun lockFT(identifier: String, vault: Capability<&FungibleToken.Vault>, balance: UFix64?){ 
        ...
        pub fun lockNFT(identifier: String, collection: Capability<&NonFungibleToken.Collection>, nftIDs: [UInt64]?){ 
            var IDs: [UInt64] = []
            if let ids = nftIDs{ 
        ...
        priv fun checkNFTExistence(collection: Capability<&NonFungibleToken.Collection>, nftIDs: [UInt64]){ 
    ...
    pub resource Admin{ 
        ...
        access(contract) let fungibleTokenInfoMapping:{ 
            String: FungibleTokenInfo
        }
        
        access(contract) let nonFungibleTokenInfoMapping:{ 
            String: NonFungibleTokenInfo
        }
        ...
        pub fun addNonFungibleTokenInfo(
            identifier: String,
    ...
    access(contract) fun addToLockUpsMapping(
        holder: Address,
        recipient: Address
    ){ 
    ...
    access(contract) fun removeFromLockUpsMapping(
        holder: Address,
        recipient: Address
    ){ 
        if !self.lockUpsMapping.containsKey(recipient){ 
    ...
    access(contract) fun getAdmin(): &Admin{ 
        let admin =
            self.account.getCapability(self.AdminPrivatePath).borrow<&Admin>()
