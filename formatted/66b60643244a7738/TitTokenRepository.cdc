...
pub contract TitTokenRepository{ 
    ...
    pub resource interface TresorPublic{ 
        pub fun transferAndRemoveTresor(
            buyerVaultRef: &{FungibleToken.Receiver},
            repositoryRef: &TitTokenRepository.Repository{
                TitTokenRepository.RepositoryPublic
            }
        ){} 
    ...
    pub resource interface RepositoryManager{ 
        pub fun purchaseTokens(
            tresorId: UInt64,
            buyer: AuthAccount,
            paymentVault: @FungibleToken.Vault
        ){} 
        
        pub fun createTresor(
            signer: AuthAccount,
            price: UFix64,
            amount: UFix64
        ): UInt64{} 
    ...
    pub resource interface RepositoryPublic{ 
        ...
        pub fun borrowTresor(tresorResourceID: UInt64): &Tresor{
            TresorPublic
        }?{} 
    }
    ...
    pub resource Repository: RepositoryManager, RepositoryPublic{ 
        ...
        pub fun purchaseTokens(tresorId: UInt64, buyer: AuthAccount, paymentVault: @FungibleToken.Vault){ 
            ...
            let receiver = seller.getCapability<&{FungibleToken.Receiver}>(/public/flowTokenReceiver).borrow() ?? panic("Could not borrow receiver reference to the seller's Flow token vault")
            ...
            let buyerReceiver = buyer.getCapability<&{FungibleToken.Receiver}>(TitToken.VaultReceiverPath).borrow() ?? panic("Could not borrow receiver reference to the buyer's TitToken vault")
        ...
        pub fun createTresor(signer: AuthAccount, price: UFix64, amount: UFix64): UInt64{ 
            let vaultRef = signer.borrow<&TitToken.Vault{FungibleToken.Provider, FungibleToken.Balance}>(from: TitToken.VaultStoragePath) ?? panic("Could not borrow reference to the TitToken vault")
