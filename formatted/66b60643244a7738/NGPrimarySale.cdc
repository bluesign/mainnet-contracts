...
pub contract NGPrimarySale{ 
    ...
    pub struct AdminSignedData{ 
        ...
        pub fun toString(): String{ 
            ...
            return self.externalID.concat(":").concat(
                self.primarySaleAddress.toString()
            ).concat(":").concat(self.purchaserAddress.toString()).concat(":")
                .concat(assetIDs).concat(":").concat(self.priceType).concat(":")
    ...
    pub resource interface PrimarySalePublic{ 
        ...
        pub fun claimNFTs(data: AdminSignedData, sig: String): @[
            NonFungibleToken.NFT
        ]{} 
    }
    ...
    pub resource PrimarySale: PrimarySalePublic, PrimarySalePrivate{ 
        priv var externalID: String
        
        pub let nftType: Type
        
        priv var status: PrimarySaleStatus
        
        priv var prices:{ String: UFix64}
        
        priv var availableAssetIDs:{ UInt64: Bool}
        
        // primary sale metadata
        priv var details: PrimarySaleDetails
        
        priv let minterCap: Capability<&{IMinter}>
        
        priv let paymentReceiverCap: Capability<&{FungibleToken.Receiver}>
        
        // pub key used to verify signatures from a specified admin
        priv var adminPublicKey: String
        ...
        pub fun setDetails(name: String, description: String, imageURI: String){ 
            self.details = PrimarySaleDetails(name: name, description: description, imageURI: imageURI)
        }
        ...
        priv fun verifyAdminSignedData(data: AdminSignedData, sig: String): Bool{ 
        ...
        pub fun purchaseNFTs(payment: @FungibleToken.Vault, data: AdminSignedData, sig: String): @[NonFungibleToken.NFT]{ 
            pre{ 
        ...
        pub fun claimNFTs(data: AdminSignedData, sig: String): @[NonFungibleToken.NFT]{ 
            pre{ 
                self.externalID == data.externalID:
                    "externalID mismatch"
                self.status == PrimarySaleStatus.OPEN:
