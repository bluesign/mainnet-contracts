...
pub contract TitTokenMarket{ 
    ...
    pub fun createListing(
        ...
        let vaultRef =
            signer.borrow<
                &TitToken.Vault{FungibleToken.Provider, FungibleToken.Balance}
    ...
    pub fun purchaseTokens(
        ...
        let receiver =
            seller.getCapability<&{FungibleToken.Receiver}>(
                /public/flowTokenReceiver
            ).borrow()
            ?? panic(
                "Could not borrow receiver reference to the seller's Flow token vault"
            )
        ...
        let buyerReceiver =
            buyer.getCapability<&{FungibleToken.Receiver}>(
                TitToken.VaultReceiverPath
            ).borrow()
            ?? panic(
                "Could not borrow receiver reference to the buyer's TitToken vault"
            )
    ...
    pub fun removeListing(signer: AuthAccount, listingId: UInt64){ 
        let listing <-
            ...
            ?? panic("Listing does not exist.")
        assert(
            listing.seller == signer.address,
            message: "Only the seller can remove the listing"
        )
