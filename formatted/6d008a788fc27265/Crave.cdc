...
pub contract Crave: NonFungibleToken{ 
    ...
    pub struct SetData{ 
        ...
        init(setID: UInt32){ 
            ...
            let referencedSet = (&Crave.sets[setID] as &Set?)!
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                case Type<MetadataViews.Display>():
                    ...
                    let metadata = Crave.getCollectibleItemMetaData(collectibleItemID: collectibleItemID)!
                    return MetadataViews.Display(name: metadata["title"] ?? "Crave NFT", description: metadata["description"] ?? "Official Crave NFT", thumbnail: MetadataViews.HTTPFile(url: metadata["posterUrl"] ?? "ipfs://bafybeiedrlfjykj4svmaka7jdxnhr3osigtudyrhitxsf7ska5ljeiwlxa/Crave Critics Banner.jpg"))
                case Type<MetadataViews.Royalties>():
                    let receiver = Crave.account.getCapability<&AnyResource{FungibleToken.Receiver}>(/public/flowTokenReceiver)
                    let royalty = MetadataViews.Royalty(receiver: receiver, cut: 0.05, description: "Crave / Niftory Fees")
                    return MetadataViews.Royalties([royalty])
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: Crave.CollectionStoragePath, publicPath: Crave.CollectionPublicPath, providerPath: /private/CraveCollection001, publicCollection: Type<&Crave.Collection{Crave.CraveCollectionPublic}>(), publicLinkedType: Type<&Crave.Collection{Crave.CraveCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&Crave.Collection{Crave.CraveCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Provider, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
                            return <-Crave.createEmptyCollection()
                        })
            }
            return nil
        }
    ...
    pub resource Collection: CraveCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection{ 
