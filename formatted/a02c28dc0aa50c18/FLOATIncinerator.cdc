...
pub contract FLOATIncinerator{ 
    ...
    pub resource Incinerator: IncineratorPublic{ 
        ...
        pub fun burn(collection: &MusicPeaksMembershipToken.Collection, ids: [UInt64]){ 
            let length = ids.length
            for id in ids{ 
                let float: &MusicPeaksMembershipToken.NFT = collection.borrowFLOAT(id: id) ?? panic("This FLOAT does not exist.")
    ...
    init(){ 
        self
            .IncineratorStoragePath = /storage/MusicPeaksMembershipTokenIncineratorStoragePath
        self
            .IncineratorPublicPath = /public/MusicPeaksMembershipTokenIncineratorPublicPath
