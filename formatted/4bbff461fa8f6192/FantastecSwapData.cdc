...
pub contract FantastecSwapData{ 
    ...
    /** CONTRACT LEVEL PROPERTIES **/    priv var cardCollectionData:{ UInt64: CardCollectionData}
    
    priv var nextCardCollectionId: UInt64
    
    priv var cardData:{ UInt64: CardData}
    
    priv var nextCardId: UInt64
    
    priv var defaultRoyaltyAddress: Address
    
    /** CONTRACT LEVEL RESOURCES */    pub let AdminStoragePath: StoragePath
    ...
    /** CONTRACT LEVEL STRUCTS */    pub struct CardCollectionData{ 
        pub var id: UInt64
    ...
    pub struct CardData{ 
        ...
        access(contract) fun addEditionMintVolume(
            edition: UInt64,
            mintVolume: UInt64
        ){ 
        ...
        access(contract) fun updateEditionMintVolume(
            edition: UInt64,
            mintVolume: UInt64
        ){ 
            if self.editionMintVolume[edition] == nil{ 
        ...
        access(contract) fun bumpEditionTotalSupply(
            _ edition: UInt64,
            quantity: UInt64
        ){ 
        ...
        access(contract) fun bumpEditionNextMintNumber(_ edition: UInt64){ 
            self.editionNextMintNumber[edition] = self.editionNextMintNumber[
                    edition
                ]!
                + 1 as UInt64
        }
        ...
        pub fun hasEditionBeenExhausted(_ edition: UInt64): Bool{ 
            if self.editionTotalSupply[edition]!
            >= self.editionMintVolume[edition]!{ 
                return true
            }
            return false
        }
        ...
        pub fun isEditionCurrent(_ edition: UInt64): Bool{ 
            if self.editionMintVolume.keys[self.editionMintVolume.length - 1]
            == edition{ 
                return true
            }
            return false
    ...
    pub resource Admin{ 
        ...
        pub fun updateCardCollectionById(
            ...
            let cardCollection: CardCollectionData =
                ...
                ?? panic(
                    FantastecSwapData.join(
                        ["No CardCollection found with id: ", id.toString()],
                        ""
                    )
                )
        ...
        pub fun deactivateCardCollectionById(id: UInt64): Bool{ 
            ...
            let cardCollection: CardCollectionData =
                FantastecSwapData.getCardCollectionById(id: id)
                ?? panic(
                    "No CardCollection found with id: ".concat(id.toString())
                )
            if cardCollection.isDeactivated{ 
        ...
        pub fun bumpEditionTotalSupply(
            cardId: UInt64,
            edition: UInt64,
            quantity: UInt64
        ){ 
        ...
        pub fun bumpEditionNextMintNumber(
            cardId: UInt64,
            edition: UInt64
        ): UInt64{ 
            let card: FantastecSwapData.CardData =
    ...
    /** PUBLIC GETTING FUNCTIONS */// CardCollection functions
