...
pub contract FantastecSwapDataV2{ 
    ...
    pub resource Admin{ 
        ...
        // ------------------------
        priv fun getCardCollection(id: UInt64): CardCollectionData{ 
            let cardCollection =
        ...
        pub fun addCardCollection(
            ...
            var newCardCollection =
                CardCollectionData(id, title, description, type, mintVolume)
        ...
        // Card functions
        // --------------
        priv fun getCard(id: UInt64): FantastecSwapDataV2.CardData{ 
            let card =
    ...
    pub resource DataManager{ 
        ...
        access(contract) fun setCardCollectionData(
            _ cardCollection: CardCollectionData
        ){ 
            self.cardCollectionData[cardCollection.id] = cardCollection
        }
        
        access(contract) fun removeCardCollectionData(
            _ cardCollectionId: UInt64
        ){ 
        ...
        init(
            _ cardCollectionData:{ 
                UInt64: CardCollectionData
            },
            _ cardData:{ 
                UInt64: CardData
            }
    ...
    pub fun paginateIds(_ ids: [UInt64], _ offset: Int?, _ pageSize: Int?): [
        UInt64
    ]{ 
        let from = offset ?? 0
        if from >= ids.length{ 
    ...
    access(contract) fun setDataManager(){ 
        let oldDataManager <-
            self.account.load<@DataManager>(from: self.DataStoragePath)
