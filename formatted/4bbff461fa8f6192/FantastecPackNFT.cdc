...
pub contract FantastecPackNFT: NonFungibleToken, IFantastecPackNFT{ 
    ...
    pub resource Pack: IFantastecPackNFT.IFantastecPack{ 
        ...
        pub fun open(recipient: Address){ 
            let receiver = getAccount(recipient).getCapability(FantastecNFT.CollectionPublicPath).borrow<&{NonFungibleToken.CollectionPublic}>() ?? panic("Could not get receiver reference to the NFT Collection - ".concat(recipient.toString()))
    ...
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver{ 
        ...
        destroy(){ 
            ...
            let pack <- FantastecPackNFT.packs.remove(key: self.id) ?? panic("cannot find pack with ID ".concat(self.id.toString()))
            destroy pack
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                case Type<MetadataViews.Display>():
                    return MetadataViews.Display(name: "Fantastec Pack", description: "Reveals Fantstec NFTs when opened", thumbnail: MetadataViews.HTTPFile(self.getThumbnailPath()))
            }
            return nil
        }
    ...
    pub resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, IFantastecPackNFT.IFantastecPackNFTCollectionPublic{ 
