...
pub contract FlowTokenManager{ 
    pub fun TopUpFlowTokens(
        account: PublicAccount,
        flowTokenProvider: &{FungibleToken.Provider}
    ){ 
        if account.storageUsed > account.storageCapacity{ 
            ...
            ((account.getCapability(/public/flowTokenReceiver)!).borrow<&{FungibleToken.Receiver}>()!).deposit(from: <-vault)
        }
    }
}
