...
pub contract StoreManagerV5{ 
    ...
    pub event ProductVolumeForSaleDecremented(
        id: UInt64,
        newVolumeForSale: UInt64
    )
    ...
    pub struct Section{ 
        ...
        pub fun addSectionItem(
            position: UInt64,
            productId: UInt64
        ): SectionItem{ 
            let id = StoreManagerV5.nextSectionItemId
            let sectionItem =
                SectionItem(id: id, position: position, productId: productId)
            self.sectionItems.insert(key: id, sectionItem)
            StoreManagerV5.nextSectionItemId = StoreManagerV5.nextSectionItemId
                + 1
            return sectionItem
        }
        
        pub fun addProductToSectionItem(
            sectionItemId: UInt64,
            product: Product
        ): SectionItem?{ 
    ...
    pub resource DataManager{ 
        ...
        pub fun decrementProductVolumeForSale(productId: UInt64){ 
            let newVolumeForSale =
                self.products[productId]?.decrementProductVolumeForSale()
        ...
        pub fun addSection(
            ...
            let section =
                Section(id: id, position: position, title: title, type: type)
        ...
        pub fun addSectionItemToSection(
            sectionId: UInt64,
            position: UInt64,
            productId: UInt64
        ): UInt64{ 
            let sectionItem =
                self.store[sectionId]?.addSectionItem(
                    position: position,
                    productId: productId
                )
            sectionItem
            ?? panic("no section found with ID ".concat(sectionId.toString()))
            emit SectionItemAdded(
                id: (sectionItem!).id,
                sectionId: sectionId,
                productId: productId
            )
        ...
        pub fun removeSectionItemFromSection(
            sectionId: UInt64,
            sectionItemId: UInt64
        ){ 
            let sectionItem =
                self.store[sectionId]?.removeSectionItem(id: sectionItemId)
            sectionItem
            ?? panic("no section found with ID ".concat(sectionId.toString()))
    ...
    /* Below are Data Migration functions */// pub fun migrateDataProducts(_ page: UInt64, _ size: UInt64) {
    ...
    access(contract) fun setDataManager(){ 
        let oldDataManager <-
            self.account.load<@DataManager>(from: self.StoreManagerDataPath)
        var oldProducts = oldDataManager?.products ??{} 
        var oldStore = oldDataManager?.store ??{} 
        self.account.save<@DataManager>(
    ...
    access(contract) fun getDataManager(): &DataManager{ 
        return self.account.borrow<&DataManager>(
            from: self.StoreManagerDataPath
        )!
    }
