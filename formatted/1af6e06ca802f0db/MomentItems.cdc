...
pub contract MomentItems: NonFungibleToken{ 
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &MomentItems.NFT?{ 
        let collection = (getAccount(from).getCapability(MomentItems.CollectionPublicPath)!).borrow<&MomentItems.Collection{MomentItems.MomentItemsCollectionPublic}>() ?? panic("Couldn't get collection")
