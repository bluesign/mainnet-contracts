...
pub contract MFLPackTemplate{ 
    ...
    priv let packTemplates: @{UInt64: PackTemplate}
    ...
    pub resource PackTemplate{ 
        ...
        init(
            ...
            MFLPackTemplate.nextPackTemplateID = MFLPackTemplate
                    .nextPackTemplateID
                + 1 as UInt64
            self.name = name
            self.description = description
    ...
    access(contract) fun getPackTemplateRef(
        id: UInt64
    ): &MFLPackTemplate.PackTemplate?{ 
        return &self.packTemplates[id] as auth &MFLPackTemplate.PackTemplate?
    }
    
    // Called from MFLPack batchMintPack fct
    access(account) fun increasePackTemplateCurrentSupply(
        id: UInt64,
        nbToMint: UInt32
    ){ 
        self.getPackTemplateRef(id: id)?.increaseCurrentSupply(
            nbToMint: nbToMint
        )
    }
