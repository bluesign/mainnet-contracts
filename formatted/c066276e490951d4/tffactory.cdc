...
pub contract tffactory: FungibleToken, ViewResolver{ 
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, MetadataViews.Resolver{ 
        ...
        pub fun withdraw(amount: UFix64): @FungibleToken.Vault{ 
            ...
            if let owner: Address = self.owner?.address{ 
                tffactory.setBalance(address: owner, balance: self.balance)
            }
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                case Type<FungibleTokenMetadataViews.FTView>():
                    return tffactory.resolveView(view)
        ...
        destroy(){ 
            if self.balance > 0.0{ 
                ...
                tffactory.totalSupply = tffactory.totalSupply - self.balance
            }
        }
    }
    ...
    pub resource Minter: Toucans.Minter{ 
        pub fun mint(amount: UFix64): @Vault{ 
            post{ 
                tffactory.maxSupply == nil || tffactory.totalSupply <= tffactory.maxSupply!:
                    "Exceeded the max supply of tokens allowd."
            }
            tffactory.totalSupply = tffactory.totalSupply + amount
    ...
    pub fun resolveView(_ view: Type): AnyStruct?{ 
        switch view{ 
            ...
            case Type<FungibleTokenMetadataViews.FTDisplay>():
                ...
                return FungibleTokenMetadataViews.FTDisplay(name: "T. F. F", symbol: "TFF", description: "We are a creative organization that provides digital fashion, avatars, and digital fan goods to support the creative activities of musicians and artists.", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
            case Type<FungibleTokenMetadataViews.FTVaultData>():
                return FungibleTokenMetadataViews.FTVaultData(storagePath: tffactory.VaultStoragePath, receiverPath: tffactory.ReceiverPublicPath, metadataPath: tffactory.VaultPublicPath, providerPath: /private/tffactoryVault, receiverLinkedType: Type<&Vault{FungibleToken.Receiver}>(), metadataLinkedType: Type<&Vault{FungibleToken.Balance, MetadataViews.Resolver}>(), providerLinkedType: Type<&Vault{FungibleToken.Provider}>(), createEmptyVaultFunction: fun (): @Vault{ 
                        return <-tffactory.createEmptyVault()
                    })
        }
        return nil
    }
    ...
    init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
        ...
        // Paths
        self.VaultStoragePath = /storage/tffactoryVault
        self.ReceiverPublicPath = /public/tffactoryReceiver
        self.VaultPublicPath = /public/tffactoryMetadata
        self.MinterStoragePath = /storage/tffactoryMinter
        self.AdministratorStoragePath = /storage/tffactoryAdmin
