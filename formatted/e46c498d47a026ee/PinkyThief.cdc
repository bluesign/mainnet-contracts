...
//
pub contract PinkyThief: NonFungibleToken{ 
    ...
    pub resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic{ 
        ...
        pub fun deposit(token: @NonFungibleToken.NFT){ 
            let token <- token as! @PinkyThief.NFT
    ...
    pub resource NFTMinter{ 
        ...
        pub fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, mugenNFTID: UInt64){ 
            if PinkyThief.mintedIds[mugenNFTID] == true{ 
                emit MintFailDuplicateId(id: mugenNFTID)
                panic("Duplicate token id")
            }
            if PinkyThief.totalSupply == PinkyThief.maxSupply{ 
            ...
            recipient.deposit(token: <-create PinkyThief.NFT(initID: mugenNFTID))
            PinkyThief.mintedIds[mugenNFTID] = true
            PinkyThief.totalSupply = PinkyThief.totalSupply + 1 as UInt64
        }
    }
    ...
    init(){ 
        // Mugen contract name
        // Set our named paths
        self.CollectionStoragePath = /storage/PinkyThiefCollection
        self.CollectionPublicPath = /public/PinkyThiefCollection
        self.MinterStoragePath = /storage/PinkyThief
