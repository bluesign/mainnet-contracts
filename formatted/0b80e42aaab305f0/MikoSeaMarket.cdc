...
pub contract MikoSeaMarket{ 
    ...
    pub resource OrderDetail{ 
        ...
        priv fun checkAfterCreate(){ 
            let collection =
                self.holderCap.borrow() ?? panic("COULD_NOT_BORROW_HOLDER")
            self.borrowNFT()
        }
        ...
        access(contract) fun onPaymentSuccess(
            refId: String,
        ...
        priv fun checkBeforePurchase(_ receiverAddress: Address){ 
            ...
            assert(
                self.receiverCap != nil
                && (self.receiverCap!).address == receiverAddress,
        ...
        pub fun purchase(_ receiverAddress: Address){ 
            ...
            log(receiverAddress)
            let receiverRef =
                (self.receiverCap!).borrow() ?? panic("ACCOUNT_NOT_SETUP")
        ...
        destroy(){ 
            ...
            if !self.purchased{ 
                emit OrderCompleted(orderId: self.uuid, purchased: self.purchased, holderAddress: self.holderCap.address, buyerAddress: self.receiverCap?.address, nftID: self.nftID, nftType: self.nftType, price: self.salePrice)
            }
        }
    }
    ...
    pub resource Storefront: StorefrontPublic{ 
        ...
        pub fun createOrder(nftType: Type, nftID: UInt64, holderCap: Capability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>, salePrice: UFix64, royalties: MetadataViews.Royalties, metadata:{ String: String}): UInt64{ 
    ...
    pub resource Admin: AdminPublicCollection{ 
        priv var orders: @{UInt64: OrderDetail}
    ...
    init(){ 
        ...
        self.mikoseaCap = self.account.getCapability<&{FungibleToken.Receiver}>(
                self.tokenPublicPath
            )
        self.account.save(<-create Admin(), to: self.AdminStoragePath)
        self.account.link<&{AdminPublicCollection}>(
            self.AdminPublicPath,
            target: self.AdminStoragePath
        )
        self.adminCap = self.account.getCapability<&{AdminPublicCollection}>(
                self.AdminPublicPath
            )
