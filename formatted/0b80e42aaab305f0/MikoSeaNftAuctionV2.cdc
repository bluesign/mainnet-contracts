...
pub contract MikoSeaNftAuctionV2{ 
    ...
    pub resource AuctionItem{ 
        ...
        priv var minimumBidIncrement: UFix64
        ...
        priv var auctionCompleteTime: UFix64?
        
        //the time the winner receive NFT
        priv var sendNftTime: UFix64?
        ...
        priv var royalties: MetadataViews.Royalties
        ...
        init(
            ...
            MikoSeaNftAuctionV2.totalAuctions = MikoSeaNftAuctionV2
                    .totalAuctions
                + 1
        ...
        access(account) fun isValidPriceCanPay(_ amount: UFix64): Bool{ 
            ...
            || self.paymentWith
            != MikoSeaNftAuctionV2.PaymentWithEnum.creaditCard
        }
        ...
        pub fun getWinnerIndex(): Int?{ 
            ...
            let maxTimeOut =
                Fix64(self.bidList.length) * Fix64(self.timeOutWinner)
        ...
        pub fun getWinner(): BidWinner?{ 
            ...
            return BidWinner(
                bidItem: winner,
                lastTimeCanPay: self.getLastTimeCanPay(winnerIndex!)
            )
        }
        
        pub fun isCanPlaceBid(): Bool{ 
            let isRejected =
                MikoSeaNftAuctionV2.auctionIdRejected.contains(self.auctionId)
            let active =
                !isRejected
                && self.auctionStatus == AuctionStatusEnum.auctioning
        ...
        priv fun isAuctionCompleted(): Bool{ 
            return self.isAuctionExpired()
            || self.auctionStatus == AuctionStatusEnum.compeleted
        }
        
        // is bidder receipted the NFT
        priv fun isAuctionFinished(): Bool{ 
            return self.bidList.length > 0 && self.NFT == nil
        }
        
        priv fun isAuctionStarted(): Bool{ 
            return self.auctionStartTime <= getCurrentBlock().timestamp
        }
        
        priv fun isAuctionCanceled(): Bool{ 
            return self.auctionStatus == AuctionStatusEnum.canceled
        }
        
        priv fun isAuctionRejected(): Bool{ 
            return MikoSeaNftAuctionV2.auctionIdRejected.contains(
                self.auctionId
            )
        }
        ...
        access(account) fun sendNftToWiner(){ 
            ...
            let winerCap =
                getAccount((winer!).bidItem.address).getCapability<
                    &{MIKOSEANFT.MikoSeaCollectionPublic}
        ...
        access(contract) fun sendNFT(
            _ capability: Capability<&{MIKOSEANFT.MikoSeaCollectionPublic}>
        ){ 
        ...
        access(contract) fun SendNFTs(
            _ capability: Capability<&{FungibleToken.Receiver}>
        ){ 
        ...
        access(account) fun payAuction(
            ...
            // cut By Nft
            for royalty in MikoSeaNftAuctionV2.nftRoyalties[nftId]
                ?.getRoyalties()
            ?? []{ 
                let cutValue =
                    self.depositToCut(
                        cutPercentage: royalty.cut,
                        receiverCap: royalty.receiver
                    )
            }
        ...
        access(account) fun placeBid(
            bidPrice: UFix64,
            bidderCollectionCap: Capability<
                &{MIKOSEANFT.MikoSeaCollectionPublic}
            >
        ){ 
            pre{ 
    ...
    pub resource interface AuctionPublic{ 
        ...
        pub fun getPaymentWith(
            _ auctionId: UInt64
        ): MikoSeaNftAuctionV2.PaymentWithEnum{} 
    }
    ...
    pub resource AuctionCollection: AuctionPublic{ 
        ...
        priv fun getAuctionRef(_ id: UInt64): &AuctionItem{ 
            pre{ 
        ...
        pub fun isValidPriceCanPay(id: UInt64, amount: UFix64): Bool{ 
            let itemRef = self.getAuctionRef(                                             
        ...
        pub fun isValidStepBidPrice(auctionId: UInt64                                                     // destroy the empty resources
        ...
        pub fun placeBid(id: UInt64, bidPrice: UFix64, bidderCollectionCap: Capability                                                                                      
            ...
            let itemRef                        
                        ...
                        = self.getAuctionRef(id)
            itemRef.placeBid(bidPrice: bidPrice                                               // Put the Admin in storage
                                               , bidderCollectionCap: bidderCollectionCap)
        }
        
        pub fun payAcution(id: UInt64, winner: Capability<&{MIKOSEANFT.MikoSeaCollectionPublic}>, bidVault: @FungibleToken.Vault){ 
            let itemRef = self.getAuctionRef(id)
    ...
    pub resource Admin{ 
        ...
        pub fun setNftAuctionSaleCuts(
            nftId: UInt64,
            cuts: MetadataViews.Royalties
        ){ 
        ...
        pub fun rejectAuction(auctionId: UInt64){ 
            let indexFound =
                MikoSeaNftAuctionV2.auctionIdRejected.firstIndex(of: auctionId)
            if indexFound == nil{ 
        ...
        pub fun removeUserFromBlacklist(
            auctionId: UInt64,
            addresses: [
                Address
            ]
    ...
    pub fun isUserInBlacList(auctionId: UInt64, address: Address): Bool{ 
        return MikoSeaNftAuctionV2.blackList[auctionId]?.contains(address)
        ?? false
    }
    ...
    init(){ 
        ...
        self
            .CollectionStoragePath = /storage/MikoSeaNftAuctionV2CollectionStoragePath
        self
            .CollectionPublicPath = /public/MikoSeaNftAuctionV2CollectionPublicPath
        self.bidNumber ={} 
