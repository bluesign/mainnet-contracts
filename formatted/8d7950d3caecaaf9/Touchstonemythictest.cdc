...
pub contract Touchstonemythictest: NonFungibleToken{ 
    ...
    pub struct NFTMetadata{ 
        ...
        init(_name: String, _description: String, _image: MetadataViews.IPFSFile, _thumbnail: MetadataViews.IPFSFile?, _price: UFix64?, _extra:{ String: AnyStruct}, _supply: UInt64){ 
            self.metadataId = Touchstonemythictest.nextMetadataId
            self.name = _name
    ...
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver{ 
        ...
        pub fun getMetadata(): NFTMetadata{ 
            return Touchstonemythictest.getNFTMetadata(self.metadataId)!
        }
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: Touchstonemythictest.CollectionStoragePath, publicPath: Touchstonemythictest.CollectionPublicPath, providerPath: Touchstonemythictest.CollectionPrivatePath, publicCollection: Type<&Collection{NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), publicLinkedType: Type<&Collection{NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&Collection{NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection, NonFungibleToken.Provider}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
                            return <-Touchstonemythictest.createEmptyCollection()
                        })
                case Type<MetadataViews.ExternalURL>():
                    return MetadataViews.ExternalURL("https://touchstone.city/discover/".concat((self.owner!).address.toString()).concat("/Touchstonemythictest"))
                case Type<MetadataViews.NFTCollectionDisplay>():
                    let squareMedia = MetadataViews.Media(file: Touchstonemythictest.getCollectionAttribute(key: "image") as! MetadataViews.IPFSFile, mediaType: "image")
                    
                    // If a banner image exists, use it
                    // Otherwise, default to the main square image
                    var bannerMedia: MetadataViews.Media? = nil
                    if let bannerImage = Touchstonemythictest.getOptionalCollectionAttribute(key: "bannerImage") as! MetadataViews.IPFSFile?{ 
                        bannerMedia = MetadataViews.Media(file: bannerImage, mediaType: "image")
                    }
                    return MetadataViews.NFTCollectionDisplay(name: Touchstonemythictest.getCollectionAttribute(key: "name") as! String, description: Touchstonemythictest.getCollectionAttribute(key: "description") as! String, externalURL: MetadataViews.ExternalURL("https://touchstone.city/discover/".concat((self.owner!).address.toString()).concat("/Touchstonemythictest")), squareImage: squareMedia, bannerImage: bannerMedia ?? squareMedia, socials: Touchstonemythictest.getCollectionAttribute(key: "socials") as!{ String: MetadataViews.ExternalURL})
        ...
        init(_metadataId: UInt64, _serial: UInt64, _recipient: Address){ 
            pre{ 
                Touchstonemythictest.metadatas[_metadataId] != nil:
                    "This NFT does not exist yet."
                _serial < (Touchstonemythictest.getNFTMetadata(_metadataId)!).supply:
                    "This serial does not exist for this metadataId."
                !(Touchstonemythictest.getNFTMetadata(_metadataId)!).purchasers.containsKey(_serial):
            ...
            if let buyersRef = &Touchstonemythictest.primaryBuyers[_recipient] as &{UInt64: [UInt64]}?{ 
                ...
                Touchstonemythictest.primaryBuyers[_recipient] ={ _metadataId: [_serial]}
            }
            
            // Update who bought this serial inside NFTMetadata so it cannot be purchased again.
            let metadataRef = (&Touchstonemythictest.metadatas[_metadataId] as &NFTMetadata?)!
            metadataRef.purchased(serial: _serial, buyer: _recipient)
            Touchstonemythictest.totalSupply = Touchstonemythictest.totalSupply + 1
    ...
    pub resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection{ 
        ...
        pub fun claim(){ 
            if let storage = &Touchstonemythictest.nftStorage[(self.owner!).address] as &{UInt64: NFT}?{ 
    ...
    pub fun mintNFT(metadataId: UInt64, recipient: &{NonFungibleToken.Receiver}, payment: @FlowToken.Vault, serial: UInt64): UInt64{ 
        ...
        if let royalty = Touchstonemythictest.getOptionalCollectionAttribute(key: "royalty") as! MetadataViews.Royalty?{ 
    ...
    pub resource Administrator{ 
        pub fun createNFTMetadata(name: String, description: String, imagePath: String, thumbnailPath: String?, ipfsCID: String, price: UFix64?, extra:{ String: AnyStruct}, supply: UInt64){ 
            Touchstonemythictest.metadatas[Touchstonemythictest.nextMetadataId] = NFTMetadata(_name: name, _description: description, _image: MetadataViews.IPFSFile(cid: ipfsCID, path: imagePath), _thumbnail: thumbnailPath == nil ? nil : MetadataViews.IPFSFile(cid: ipfsCID, path: thumbnailPath), _price: price, _extra: extra, _supply: supply)
            Touchstonemythictest.nextMetadataId = Touchstonemythictest.nextMetadataId + 1
        }
        ...
        pub fun mintNFT(metadataId: UInt64, serial: UInt64, recipient: Address){ 
            ...
            if let recipientCollection = getAccount(recipient).getCapability(Touchstonemythictest.CollectionPublicPath).borrow<&Touchstonemythictest.Collection{NonFungibleToken.CollectionPublic}>(){ 
                recipientCollection.deposit(token: <-nft)
            } else if let storage = &Touchstonemythictest.nftStorage[recipient] as &{UInt64: NFT}?{ 
                storage[nft.id] <-! nft
            } else{ 
                Touchstonemythictest.nftStorage[recipient] <-!{ nft.id: <-nft}
            }
        }
        ...
        pub fun changeField(key: String, value: AnyStruct){ 
            Touchstonemythictest.collectionInfo[key] = value
        }
    }
    ...
    pub fun getPriceOfNFT(_ metadataId: UInt64): UFix64?{ 
        if let metadata: Touchstonemythictest.NFTMetadata = self.getNFTMetadata(metadataId){ 
    ...
    pub fun getClaimableNFTs(user: Address):{ UInt64: NFTMetadata}{ 
        ...
        if let storage = &Touchstonemythictest.nftStorage[user] as &{UInt64: NFT}?{ 
            for id in storage.keys{ 
    ...
    init(_name: String, _description: String, _imagePath: String, _bannerImagePath: String?, _minting: Bool, _royalty: MetadataViews.Royalty?, _defaultPrice: UFix64, _paymentType: String, _ipfsCID: String, _lotteryBuying: Bool, _socials:{ String: MetadataViews.ExternalURL}, _mintVerifiers: [{MintVerifiers.IVerifier}]){ 
        ...
        self.CollectionStoragePath = /storage/TouchstonemythictestCollection_0x8d7950d3caecaaf9
        self.CollectionPublicPath = /public/TouchstonemythictestCollection_0x8d7950d3caecaaf9
        self.CollectionPrivatePath = /private/TouchstonemythictestCollection_0x8d7950d3caecaaf9
        self.AdministratorStoragePath = /storage/TouchstonemythictestAdministrator_0x8d7950d3caecaaf9
