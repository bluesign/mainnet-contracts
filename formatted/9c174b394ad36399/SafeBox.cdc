...
pub contract SafeBox{ 
    ...
    pub resource SafeBoxAdmin{ 
        pub fun addTok(identifier: String, tok: TokenCfg){ 
            assert(
                !SafeBox.tokMap.containsKey(identifier),
    ...
    init(chID: UInt64){ 
        ...
        self.account.save<@SafeBoxAdmin>(
            <-create SafeBoxAdmin(),
            to: self.AdminPath
        )
    }
    ...
    pub fun deposit(
        from: &AnyResource{FungibleToken.Provider},
        info: DepoInfo
    ){ 
        pre{ 
            !self.isPaused:
        ...
        assert(
            info.amt >= tokenCfg.minDepo,
            message: "deposit amount less than min deposit"
        )
        ...
        assert(
            !self.records.containsKey(depoId),
            message: "depoId already exists"
        )
        self.records[depoId] = true
        let recev =
            self.account.getCapability(tokenCfg.vaultPub).borrow<
    ...
    pub fun withdraw(token: String, wdmsg: [UInt8], sigs: [cBridge.SignerSig]){ 
        ...
        // emit Withdrawn
        let tokCfg =
            SafeBox.tokMap[token] ?? panic("token not support in contract")
        ...
        let receiverCap =
            getAccount(wdInfo.receiver).getCapability<
                &{FungibleToken.Receiver}
            >(tokCfg.vaultPub)
        let vaultRef =
            self.account.borrow<&{FungibleToken.Provider}>(
                from: tokCfg.vaultSto
            )
