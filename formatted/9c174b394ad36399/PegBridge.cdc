...
pub contract PegBridge{ 
    ...
    pub resource PegBridgeAdmin{ 
        pub fun addTok(identifier: String, tok: TokenCfg){ 
            assert(
                !PegBridge.tokMap.containsKey(identifier),
                message: "this token already exist"
            )
        ...
        pub fun rmTok(identifier: String){ 
            assert(
                PegBridge.tokMap.containsKey(identifier),
                message: "this token do not exist"
            )
    ...
    init(chID: UInt64){ 
        ...
        self.domainPrefix = chID.toBigEndianBytes().concat(
                self.getType().identifier.utf8
            )
        self.isPaused = false
        self.records ={} 
        self.tokMap ={} 
        self.AdminPath = /storage/PegBridgeAdmin
        self.account.save<@PegBridgeAdmin>(
            <-create PegBridgeAdmin(),
            to: self.AdminPath
        )
        ...
        self.account.link<&MinterBurnerMap{IAddMinter}>(
            /public/AddMinter,
            target: self.FTMBMapPath
        )
        self.account.link<&MinterBurnerMap{IAddBurner}>(
            /public/AddBurner,
            target: self.FTMBMapPath
        )
    }
    ...
    pub fun mint(token: String, pbmsg: [UInt8], sigs: [cBridge.SignerSig]){ 
        ...
        let tokCfg =
            PegBridge.tokMap[token] ?? panic("token not support in contract")
        let mintId = String.encodeHex(HashAlgorithm.SHA3_256.hash(pbmsg))
        assert(
            !self.records.containsKey(mintId),
            message: "mintId already exists"
        )
        self.records[mintId] = true
        let receiverCap =
            getAccount(mintInfo.receiver).getCapability<
                &{FungibleToken.Receiver}
            >(tokCfg.vaultPub)
        let minterMap =
            self.account.borrow<&MinterBurnerMap>(from: self.FTMBMapPath)!
        let mintedVault: @FungibleToken.Vault <-
            minterMap.mint(id: token, amt: mintInfo.amount)
    ...
    pub fun burn(from: &AnyResource{FungibleToken.Provider}, info: BurnInfo){ 
        ...
        assert(
            info.amt >= tokenCfg.minBurn,
            message: "burn amount less than min burn"
        )
        ...
        // calculate burnId
        let concatStr =
            user.toString().concat(tokStr).concat(info.amt.toString()).concat(
                info.nonce.toString()
            )
        let burnId =
            String.encodeHex(HashAlgorithm.SHA3_256.hash(concatStr.utf8))
        assert(
            !self.records.containsKey(burnId),
            message: "burnId already exists"
        )
