...
pub contract TouchstoneEscapeScape: NonFungibleToken{ 
    ...
    pub struct NFTMetadata{ 
        ...
        init(_name: String, _description: String, _image: MetadataViews.IPFSFile, _thumbnail: MetadataViews.IPFSFile?, _price: UFix64?, _extra:{ String: AnyStruct}, _supply: UInt64){ 
            self.metadataId = TouchstoneEscapeScape.nextMetadataId
            self.name = _name
    ...
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver{ 
        ...
        pub fun getMetadata(): NFTMetadata{ 
            return TouchstoneEscapeScape.getNFTMetadata(self.metadataId)!
        }
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: TouchstoneEscapeScape.CollectionStoragePath, publicPath: TouchstoneEscapeScape.CollectionPublicPath, providerPath: TouchstoneEscapeScape.CollectionPrivatePath, publicCollection: Type<&Collection{NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), publicLinkedType: Type<&Collection{NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&Collection{NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection, NonFungibleToken.Provider}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
                            return <-TouchstoneEscapeScape.createEmptyCollection()
                        })
                case Type<MetadataViews.ExternalURL>():
                    return MetadataViews.ExternalURL("https://touchstone.city/discover/".concat((self.owner!).address.toString()).concat("/TouchstoneEscapeScape"))
                case Type<MetadataViews.NFTCollectionDisplay>():
                    let squareMedia = MetadataViews.Media(file: TouchstoneEscapeScape.getCollectionAttribute(key: "image") as! MetadataViews.IPFSFile, mediaType: "image")
                    
                    // If a banner image exists, use it
                    // Otherwise, default to the main square image
                    var bannerMedia: MetadataViews.Media? = nil
                    if let bannerImage = TouchstoneEscapeScape.getOptionalCollectionAttribute(key: "bannerImage") as! MetadataViews.IPFSFile?{ 
                        bannerMedia = MetadataViews.Media(file: bannerImage, mediaType: "image")
                    }
                    return MetadataViews.NFTCollectionDisplay(name: TouchstoneEscapeScape.getCollectionAttribute(key: "name") as! String, description: TouchstoneEscapeScape.getCollectionAttribute(key: "description") as! String, externalURL: MetadataViews.ExternalURL("https://touchstone.city/discover/".concat((self.owner!).address.toString()).concat("/TouchstoneEscapeScape")), squareImage: squareMedia, bannerImage: bannerMedia ?? squareMedia, socials: TouchstoneEscapeScape.getCollectionAttribute(key: "socials") as!{ String: MetadataViews.ExternalURL})
        ...
        init(_metadataId: UInt64, _serial: UInt64, _recipient: Address){ 
            pre{ 
                TouchstoneEscapeScape.metadatas[_metadataId] != nil:
                    "This NFT does not exist yet."
                _serial < (TouchstoneEscapeScape.getNFTMetadata(_metadataId)!).supply:
                    "This serial does not exist for this metadataId."
                !(TouchstoneEscapeScape.getNFTMetadata(_metadataId)!).purchasers.containsKey(_serial):
            ...
            if let buyersRef = &TouchstoneEscapeScape.primaryBuyers[_recipient] as &{UInt64: [UInt64]}?{ 
                ...
                TouchstoneEscapeScape.primaryBuyers[_recipient] ={ _metadataId: [_serial]}
            }
            
            // Update who bought this serial inside NFTMetadata so it cannot be purchased again.
            let metadataRef = (&TouchstoneEscapeScape.metadatas[_metadataId] as &NFTMetadata?)!
            metadataRef.purchased(serial: _serial, buyer: _recipient)
            TouchstoneEscapeScape.totalSupply = TouchstoneEscapeScape.totalSupply + 1
    ...
    pub resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection{ 
        ...
        pub fun claim(){ 
            if let storage = &TouchstoneEscapeScape.nftStorage[(self.owner!).address] as &{UInt64: NFT}?{ 
    ...
    pub fun mintNFT(metadataId: UInt64, recipient: &{NonFungibleToken.Receiver}, payment: @FlowToken.Vault, serial: UInt64): UInt64{ 
        ...
        if let royalty = TouchstoneEscapeScape.getOptionalCollectionAttribute(key: "royalty") as! MetadataViews.Royalty?{ 
    ...
    pub resource Administrator{ 
        pub fun createNFTMetadata(name: String, description: String, imagePath: String, thumbnailPath: String?, ipfsCID: String, price: UFix64?, extra:{ String: AnyStruct}, supply: UInt64){ 
            TouchstoneEscapeScape.metadatas[TouchstoneEscapeScape.nextMetadataId] = NFTMetadata(_name: name, _description: description, _image: MetadataViews.IPFSFile(cid: ipfsCID, path: imagePath), _thumbnail: thumbnailPath == nil ? nil : MetadataViews.IPFSFile(cid: ipfsCID, path: thumbnailPath), _price: price, _extra: extra, _supply: supply)
            TouchstoneEscapeScape.nextMetadataId = TouchstoneEscapeScape.nextMetadataId + 1
        }
        ...
        pub fun mintNFT(metadataId: UInt64, serial: UInt64, recipient: Address){ 
            pre{ 
                EmeraldPass.isActive(user: TouchstoneEscapeScape.account.address):
            ...
            if let recipientCollection = getAccount(recipient).getCapability(TouchstoneEscapeScape.CollectionPublicPath).borrow<&TouchstoneEscapeScape.Collection{NonFungibleToken.CollectionPublic}>(){ 
                recipientCollection.deposit(token: <-nft)
            } else if let storage = &TouchstoneEscapeScape.nftStorage[recipient] as &{UInt64: NFT}?{ 
                storage[nft.id] <-! nft
            } else{ 
                TouchstoneEscapeScape.nftStorage[recipient] <-!{ nft.id: <-nft}
            }
        }
        ...
        pub fun changeField(key: String, value: AnyStruct){ 
            TouchstoneEscapeScape.collectionInfo[key] = value
        }
    }
    ...
    pub fun getPriceOfNFT(_ metadataId: UInt64): UFix64?{ 
        if let metadata: TouchstoneEscapeScape.NFTMetadata = self.getNFTMetadata(metadataId){ 
    ...
    pub fun getClaimableNFTs(user: Address):{ UInt64: NFTMetadata}{ 
        ...
        if let storage = &TouchstoneEscapeScape.nftStorage[user] as &{UInt64: NFT}?{ 
            for id in storage.keys{ 
    ...
    init(_name: String, _description: String, _imagePath: String, _bannerImagePath: String?, _minting: Bool, _royalty: MetadataViews.Royalty?, _defaultPrice: UFix64, _paymentType: String, _ipfsCID: String, _lotteryBuying: Bool, _socials:{ String: MetadataViews.ExternalURL}, _mintVerifiers: [{MintVerifiers.IVerifier}]){ 
        ...
        self.CollectionStoragePath = /storage/TouchstoneEscapeScapeCollection_0xaf589e532490afe6
        self.CollectionPublicPath = /public/TouchstoneEscapeScapeCollection_0xaf589e532490afe6
        self.CollectionPrivatePath = /private/TouchstoneEscapeScapeCollection_0xaf589e532490afe6
        self.AdministratorStoragePath = /storage/TouchstoneEscapeScapeAdministrator_0xaf589e532490afe6
