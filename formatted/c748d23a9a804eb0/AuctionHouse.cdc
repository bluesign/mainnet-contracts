...
pub contract AuctionHouse{ 
    ...
    access(contract) var metadataGen:{ 
        UInt64: Capability<&DAAM.MetadataGenerator{DAAM.MetadataGeneratorMint}>
    }
    ...
    /************************************************************************/    pub resource interface AuctionWalletPublic{ 
        ...
        pub fun deposit(
            agent: &DAAM.Admin{DAAM.Agent},
            metadataGenerator: Capability<
                &DAAM.MetadataGenerator{DAAM.MetadataGeneratorMint}
            >,
            mid: UInt64,
            start: UFix64,
            length: UFix64,
    ...
    /************************************************************************/    pub resource AuctionWallet: AuctionWalletPublic{ 
        ...
        pub fun deposit(agent: &DAAM.Admin{DAAM.Agent}, metadataGenerator: Capability<&DAAM.MetadataGenerator{DAAM.MetadataGeneratorMint}>, mid: UInt64, start: UFix64, length: UFix64, isExtended: Bool, extendedTime: UFix64, vault: @FungibleToken.Vault, incrementByPrice: Bool, incrementAmount: UFix64, startingBid: UFix64?, reserve: UFix64, buyNow: UFix64, reprintSeries: UInt64?): UInt64{ 
            pre{ 
        ...
        priv fun createAuctionResource(metadataGenerator: Capability<&DAAM.MetadataGenerator{DAAM.MetadataGeneratorMint}>?, nft: @DAAM.NFT?, id: UInt64, start: UFix64, length: UFix64, isExtended: Bool, extendedTime: UFix64, vault: @FungibleToken.Vault, incrementByPrice: Bool, incrementAmount: UFix64, startingBid: UFix64?, reserve: UFix64, buyNow: UFix64, reprintSeries: UInt64?): @Auction{ 
            pre{ 
        ...
        pub fun getAgentAuctions(): [UInt64]{ 
        ...
        destroy(){ 
            pre{ 
                self.currentAuctions.length == 0 && self.approveAuctions.length == 0
            }
    ...
    /************************************************************************/    pub resource Auction: AuctionPublic{ 
        ...
        priv fun returnFunds(){ 
            ...
            for bidder in self.auctionLog.keys{ 
                ...
                let bidderRef = getAccount(bidder).getCapability<&{FungibleToken.Receiver}>(MetadataViews.getRoyaltyReceiverPublicPath()).borrow()!
        ...
        priv fun payFirstSale(){ 
            ...
            if (self.auctionNFT?.metadata!).creatorInfo.agent == DAAM.company.receiver.address{ 
                ...
                let agent = getAccount(agentAddress).getCapability<&{FungibleToken.Receiver}>(MetadataViews.getRoyaltyReceiverPublicPath()!).borrow()! // get Seller FUSD Wallet Capability
        ...
        destroy(){ // Verify no Funds, NFT are NOT in storage, Auction has ended/closed. 
            pre{ 
                self.auctionNFT == nil:
                    "Illegal Operation: Auction still contains NFT Token ID: ".concat((self.auctionNFT?.metadata!).mid.toString())
                self.auctionMetadata == nil:
                    "Illegal Operation: Auction still contains MetadataID: ".concat((self.auctionMetadata?.mid!).toString())
                self.status == false:
                    "Illegal Operation: Auction is not Finished."
                self.auctionVault.balance == 0.0:
                    "Illegal Operation: Auction Balance is ".concat(self.auctionVault.balance.toString())
            }
    ...
    access(contract) fun notNew(tokenID: UInt64){ 
        let minter =
            self.account.borrow<&DAAM.Minter>(from: DAAM.minterStoragePath)!
    ...
    access(contract) fun mintNFT(metadata: @DAAM.Metadata): @DAAM.NFT{ 
        let minterRef =
            self.account.borrow<&DAAM.Minter>(from: DAAM.minterStoragePath)! // get Minter Reference
    ...
    access(contract) fun updateAllHistory(
        mid: UInt64,
        id: UInt64,
    ...
    pub fun getAgencyFirstSale(mid: UInt64): UFix64{ 
        return self.agencyFirstSale[mid] == nil
            ? 0.15
            : self.agencyFirstSale[mid]!
    }
    
    pub fun addAgencyFirstSale(
        mid: UInt64,
        fee: UFix64,
        permission: &DAAM.Admin
    ){ 
        pre{ 
            DAAM.isAdmin((permission.owner!).address) == true:
                "Permission Denied"
