...
pub contract NFTCollections{ 
    ...
    pub resource interface Provider{ 
        ...
        pub fun withdrawWrapper(
            address: Address,
            withdrawID: UInt64
        ): @NFTWrapper?{} 
        
        pub fun batchWithdraw(
            address: Address,
            batch: [
                UInt64
            ],
            into: &NonFungibleToken.Collection
        ){} 
        
        pub fun batchWithdrawWrappers(
            address: Address,
            batch: [
                UInt64
            ]
    ...
    pub resource NFTCollectionManager{ 
        priv let collections: @{String: Collection}
    ...
    pub resource NFTWrapper: WrappedNFT{ 
        priv let contractName: String
        
        priv let address: Address
        
        priv let collectionPath: PublicPath
    ...
    pub resource Collection: CollectionPublic, Provider, Receiver{ 
        priv var collections: @{Address: ShardedNFTWrapperCollection}
    ...
    pub resource ShardedNFTWrapperCollection{ 
        priv var collections: @{UInt64: NFTWrapperCollection}
        
        priv let numBuckets: UInt64
        ...
        pub fun withdraw(withdrawID: UInt64): @NFTWrapper{ 
            ...
            let wrapper <-
                self.collections[bucket]?.withdraw(withdrawID: withdrawID)!
            return <-wrapper
        }
        
        pub fun batchWithdraw(batch: [UInt64]): @ShardedNFTWrapperCollection{ 
            var batchCollection <-
                NFTCollections.createEmptyShardedNFTWrapperCollection()
            for id in batch{ 
    ...
    pub resource NFTWrapperCollection{ 
        priv var wrappers: @{UInt64: NFTWrapper}
        ...
        pub fun withdraw(withdrawID: UInt64): @NFTWrapper{ 
            let wrapper <-
                ...
                ?? panic(
                    "Cannot withdraw: NFTWrapper does not exist in the collection"
                )
