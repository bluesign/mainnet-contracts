...
pub contract SportsIconBeneficiaries{ 
    ...
    pub event BeneficiaryPaymentFailed(
        ids: [
            UInt64
        ],
        amount: UFix64,
    ...
    pub struct Beneficiary{ 
        ...
        priv let fusdWallet: Capability<&FUSD.Vault{FungibleToken.Receiver}>
        
        priv let flowWallet: Capability<
            &FlowToken.Vault{FungibleToken.Receiver}
        >
        
        init(
            cut: UFix64,
            fusdWallet: Capability<&FUSD.Vault{FungibleToken.Receiver}>,
    ...
    pub struct Beneficiaries{ 
        priv let beneficiaries: [Beneficiary]
    ...
    pub fun getAdminFUSDVault(): Capability<
        &FUSD.Vault{FungibleToken.Receiver}
    >{ 
        return self.account.getCapability<&FUSD.Vault{FungibleToken.Receiver}>(
            /public/fusdReceiver
        )
    }
    
    pub fun getAdminFLOWVault(): Capability<
        &FlowToken.Vault{FungibleToken.Receiver}
    >{ 
    ...
    access(account) fun setMintBeneficiaries(
        setID: UInt64,
    ...
    access(account) fun setMarketBeneficiaries(
        setID: UInt64,
        beneficiaries: Beneficiaries
    ){ 
