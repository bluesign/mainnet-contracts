...
pub contract SportsIconPrimarySalePrices{ 
    pub struct PrimarySaleListing{ 
        ...
        priv let saleCuts: [SportsIconNFTStorefront.SaleCut]
        ...
        init(totalPrice: UFix64, saleCuts: [SportsIconNFTStorefront.SaleCut]){ 
            ...
            assert(
                salePrice >= 0.0,
                message: "Listing must have non-negative price"
            )
            assert(
    ...
    priv let primarySalePrices:{ UInt64:{ String: PrimarySaleListing}}
    
    access(account) fun updateSalePrice(
        setID: UInt64,
        currency: String,
        primarySaleListing: PrimarySaleListing?
    ){ 
