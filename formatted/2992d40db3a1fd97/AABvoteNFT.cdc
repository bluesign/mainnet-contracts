...
pub contract AABvoteNFT: NonFungibleToken{ 
    ...
    pub resource NFTMinter{ 
        pub fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, candidateId: String, name: String, description: String, thumbnail: String, rarity: UInt8, metadata:{ String: String}){ 
    ...
    pub fun getCollection(_ from: Address): &Collection{AABvoteNFTCollectionPublic}{ 
        let collection = (getAccount(from).getCapability(AABvoteNFT.CollectionPublicPath)!).borrow<&AABvoteNFT.Collection{AABvoteNFT.AABvoteNFTCollectionPublic}>() ?? panic("Could not borrow capability from public collection")
        return collection
    }
    ...
    init(){ 
        ...
        self.account.link<&AABvoteNFT.Collection{NonFungibleToken.CollectionPublic, AABvoteNFT.AABvoteNFTCollectionPublic, MetadataViews.ResolverCollection}>(self.CollectionPublicPath, target: self.CollectionStoragePath)
