...
pub contract NFTDojo: FungibleToken, ViewResolver{ 
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, MetadataViews.Resolver{ 
        ...
        pub fun withdraw(amount: UFix64): @FungibleToken.Vault{ 
            ...
            if let owner: Address = self.owner?.address{ 
                NFTDojo.setBalance(address: owner, balance: self.balance)
            }
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                case Type<FungibleTokenMetadataViews.FTView>():
                    return NFTDojo.resolveView(view)
        ...
        destroy(){ 
            if self.balance > 0.0{ 
                ...
                NFTDojo.totalSupply = NFTDojo.totalSupply - self.balance
            }
        }
    }
    ...
    pub resource Minter: Toucans.Minter{ 
        pub fun mint(amount: UFix64): @Vault{ 
            post{ 
                NFTDojo.maxSupply == nil || NFTDojo.totalSupply <= NFTDojo.maxSupply!:
                    "Exceeded the max supply of tokens allowd."
            }
            NFTDojo.totalSupply = NFTDojo.totalSupply + amount
    ...
    pub fun resolveView(_ view: Type): AnyStruct?{ 
        switch view{ 
            ...
            case Type<FungibleTokenMetadataViews.FTDisplay>():
                let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafybeihx2ajgenmkdhuywqfeatnblqiwcqbsdi2wfejnk2eepk3noucw2a"), mediaType: "image")
                let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafybeigwp3m77fzo6dx3ks5wf3e3t2sa3pxcncfkwipu2m6ojfi5lvm6ci"), mediaType: "image")
                let medias = MetadataViews.Medias([media, bannerMedia])
                return FungibleTokenMetadataViews.FTDisplay(name: "NFT Dojo", symbol: "TBR", description: "NFT\u{3084}\u{6697}\u{53f7}\u{901a}\u{8ca8}\u{306b}\u{3064}\u{3044}\u{3066}\u{3001}\u{52c9}\u{5f37}\u{3057}\u{307e}\u{3059}", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
            case Type<FungibleTokenMetadataViews.FTVaultData>():
                return FungibleTokenMetadataViews.FTVaultData(storagePath: NFTDojo.VaultStoragePath, receiverPath: NFTDojo.ReceiverPublicPath, metadataPath: NFTDojo.VaultPublicPath, providerPath: /private/NFTDojoVault, receiverLinkedType: Type<&Vault{FungibleToken.Receiver}>(), metadataLinkedType: Type<&Vault{FungibleToken.Balance, MetadataViews.Resolver}>(), providerLinkedType: Type<&Vault{FungibleToken.Provider}>(), createEmptyVaultFunction: fun (): @Vault{ 
                        return <-NFTDojo.createEmptyVault()
                    })
        }
        return nil
    }
    ...
    init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
        ...
        // Paths
        self.VaultStoragePath = /storage/NFTDojoVault
        self.ReceiverPublicPath = /public/NFTDojoReceiver
        self.VaultPublicPath = /public/NFTDojoMetadata
        self.MinterStoragePath = /storage/NFTDojoMinter
        self.AdministratorStoragePath = /storage/NFTDojoAdmin
