...
pub contract EllaHoldings: FungibleToken, ViewResolver{ 
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, MetadataViews.Resolver{ 
        ...
        pub fun withdraw(amount: UFix64): @FungibleToken.Vault{ 
            ...
            if let owner: Address = self.owner?.address{ 
                EllaHoldings.setBalance(address: owner, balance: self.balance)
            }
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                case Type<FungibleTokenMetadataViews.FTView>():
                    return EllaHoldings.resolveView(view)
        ...
        destroy(){ 
            if self.balance > 0.0{ 
                ...
                EllaHoldings.totalSupply = EllaHoldings.totalSupply - self.balance
            }
        }
    }
    ...
    pub resource Minter: Toucans.Minter{ 
        pub fun mint(amount: UFix64): @Vault{ 
            post{ 
                EllaHoldings.maxSupply == nil || EllaHoldings.totalSupply <= EllaHoldings.maxSupply!:
                    "Exceeded the max supply of tokens allowd."
            }
            EllaHoldings.totalSupply = EllaHoldings.totalSupply + amount
    ...
    pub fun resolveView(_ view: Type): AnyStruct?{ 
        switch view{ 
            ...
            case Type<FungibleTokenMetadataViews.FTDisplay>():
                ...
                return FungibleTokenMetadataViews.FTDisplay(name: "Ella Holdings DAO LLC", symbol: "ELLAH", description: "Investing in a decentralized future, one project at a time.", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
            case Type<FungibleTokenMetadataViews.FTVaultData>():
                return FungibleTokenMetadataViews.FTVaultData(storagePath: EllaHoldings.VaultStoragePath, receiverPath: EllaHoldings.ReceiverPublicPath, metadataPath: EllaHoldings.VaultPublicPath, providerPath: /private/EllaHoldingsVault, receiverLinkedType: Type<&Vault{FungibleToken.Receiver}>(), metadataLinkedType: Type<&Vault{FungibleToken.Balance, MetadataViews.Resolver}>(), providerLinkedType: Type<&Vault{FungibleToken.Provider}>(), createEmptyVaultFunction: fun (): @Vault{ 
                        return <-EllaHoldings.createEmptyVault()
                    })
        }
        return nil
    }
    ...
    init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
        ...
        // Paths
        self.VaultStoragePath = /storage/EllaHoldingsVault
        self.ReceiverPublicPath = /public/EllaHoldingsReceiver
        self.VaultPublicPath = /public/EllaHoldingsMetadata
        self.MinterStoragePath = /storage/EllaHoldingsMinter
        self.AdministratorStoragePath = /storage/EllaHoldingsAdmin
