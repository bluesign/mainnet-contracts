pub contract betaLilaiQuest{ 
    ...
    pub fun updateOwner(tokenId: UInt64, newOwner: Address, caller: Address){ 
        ...
        assert(
            caller == currentOwner,
            message: "Caller is not the owner of the NFT"
        )
    ...
    pub fun createNFTCollection(): @NFTCollection{ 
        ...
        emit LogEvent(
            message: "New LilaiQuest NFTCollection created and saved in storage."
        )
        return <-collection
    }
    ...
    init(){ 
        ...
        self.account.save(
            ...
            to: /storage/LilaiQuestNFTCollection
        )
        self.account.link<&{NFTReceiver}>(
            /public/LilaiQuestNFTReceiver,
            target: /storage/LilaiQuestNFTCollection
        )
        // Save and link NFTMinter
        let minter <- create NFTMinter()
        self.account.save(<-minter, to: /storage/LilaiQuestNFTMinter)
        self.account.link<&betaLilaiQuest.NFTMinter{NFTMinterPublic}>(
            /public/betaLilaiQuestNFTMinter,
            target: /storage/LilaiQuestNFTMinter
        )
    }
}
