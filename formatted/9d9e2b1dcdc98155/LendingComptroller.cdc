...
pub contract LendingComptroller{ 
    ...
    pub event NewOracle(
        _ oldOracleAddress: Address?,
        _ newOracleAddress: Address
    )
    ...
    pub struct Market{ 
        ...
        pub fun setCollateralFactor(newCollateralFactor: UFix64){ 
            ...
            let scaledNewCollateralFactor =
                LendingConfig.UFix64ToScaledUInt256(newCollateralFactor)
        ...
        pub fun setBorrowCap(newBorrowCap: UFix64){ 
            let scaledNewBorrowCap =
                LendingConfig.UFix64ToScaledUInt256(newBorrowCap)
        ...
        pub fun setSupplyCap(newSupplyCap: UFix64){ 
            let scaledNewSupplyCap =
                LendingConfig.UFix64ToScaledUInt256(newSupplyCap)
        ...
        init(
            ...
            self.isOpen = isOpen
            self.isMining = isMining
            self.scaledLiquidationPenalty = LendingConfig.UFix64ToScaledUInt256(
                    liquidationPenalty
                )
            self.scaledCollateralFactor = LendingConfig.UFix64ToScaledUInt256(
                    collateralFactor
                )
            self.scaledBorrowCap = LendingConfig.UFix64ToScaledUInt256(
                    borrowCap
                )
            self.scaledSupplyCap = LendingConfig.UFix64ToScaledUInt256(
                    supplyCap
                )
    ...
    pub resource Comptroller: LendingInterfaces.ComptrollerPublic{ 
        ...
        /// @Return error code
        ///
        pub fun supplyAllowed(poolCertificate: @{LendingInterfaces.IdentityCertificate}, poolAddress: Address, supplierAddress: Address, supplyUnderlyingAmountScaled: UInt256): String?{ 
        ...
        ///
        pub fun redeemAllowed(poolCertificate: @{LendingInterfaces.IdentityCertificate}, poolAddress: Address, redeemerAddress: Address, redeemLpTokenAmountScaled: UInt256): String?{ 
            let err = self.callerAllowed(callerCertificate: <-poolCertificate, callerAddress: poolAddress)
            if err != nil{ 
                return err
            }
        ...
        /// @Return error code
        ///
        pub fun borrowAllowed(poolCertificate: @{LendingInterfaces.IdentityCertificate}, poolAddress: Address, borrowerAddress: Address, borrowUnderlyingAmountScaled: UInt256): String?{ 
            let err = self.callerAllowed(callerCertificate: <-poolCertificate, callerAddress: poolAddress)
            if err != nil{ 
                return err
            }
        ...
        /// @Return error code
        ///
        pub fun repayAllowed(poolCertificate: @{LendingInterfaces.IdentityCertificate}, poolAddress: Address, borrowerAddress: Address, repayUnderlyingAmountScaled: UInt256): String?{ 
            let err = self.callerAllowed(callerCertificate: <-poolCertificate, callerAddress: poolAddress)
            if err != nil{ 
                return err
            }
        ...
        /// @Return error code
        ///
        pub fun liquidateAllowed(poolCertificate: @{LendingInterfaces.IdentityCertificate}, poolBorrowed: Address, poolCollateralized: Address, borrower: Address, repayUnderlyingAmountScaled: UInt256): String?{ 
            pre{ 
        ...
        /// @Return error code
        ///
        pub fun seizeAllowed(poolCertificate: @{LendingInterfaces.IdentityCertificate}, borrowPool: Address, collateralPool: Address, liquidator: Address, borrower: Address, seizeCollateralPoolLpTokenAmountScaled: UInt256): String?{ 
            pre{ 
        ...
        ///
        pub fun calculateCollateralPoolLpTokenToSeize(borrower: Address, borrowPool: Address, collateralPool: Address, actualRepaidBorrowAmountScaled: UInt256): UInt256{ 
        ...
        pub fun callerAllowed(callerCertificate: @{LendingInterfaces.IdentityCertificate}, callerAddress: Address): String?{ 
        ...
        /// Remove pool out of user markets list if necessary
        priv fun removePoolFromAccountMarketsOnCondition(poolAddress: Address, account: Address, scaledRedeemOrRepayAmount: UInt256): Bool{ 
        ...
        ///
        priv fun getHypotheticalAccountLiquidity(account: Address, poolToModify: Address, scaledAmountLPTokenToRedeem: UInt256, scaledAmountUnderlyingToBorrow: UInt256): [UInt256; 3]{ 
    ...
    pub resource Admin{ 
        ...
        pub fun addMarket(
            ...
            let comptrollerRef =
                LendingComptroller.account.borrow<&Comptroller>(
    ...
    init(){ 
        ...
        destroy <-self.account.load<@AnyResource>(
            from: self.ComptrollerStoragePath
        )
        self.account.save(
            <-create Comptroller(),
            to: self.ComptrollerStoragePath
        )
        self.account.unlink(self.ComptrollerPublicPath)
        self.account.link<&{LendingInterfaces.ComptrollerPublic}>(
            self.ComptrollerPublicPath,
            target: self.ComptrollerStoragePath
        )
    }
}
