...
pub contract InceptionMinter{ 
    ...
    priv var whitelistedAccounts:{ Address: UInt64}
    
    priv var publicMintedAccounts:{ Address: UInt64}
    
    priv var tipMintedAccounts:{ Address: UInt64}
    
    priv var usdTipTracker:{ Address: UFix64}
    
    priv var flowTipTracker:{ Address: UFix64}
    
    pub fun tipMintWithDUC(
        buyer: Address,
        setID: UInt64,
        ...
        let admin =
            self.account.borrow<&InceptionAvatar.Admin>(
                from: InceptionAvatar.AdminStoragePath
            )
            ?? panic(
                "Could not borrow a reference to the InceptionAvatar Admin"
            )
        ...
        let merchantDUCReceiverRef =
            getAccount(merchantAccount).getCapability<
        ...
        let NFTReceiver =
            recipient.getCapability(InceptionAvatar.CollectionPublicPath)
        ...
        InceptionMinter.usdTipTracker[buyer] = UFix64(
                InceptionMinter.usdTipTracker[buyer] ?? 0.00
            )
        ...
        if InceptionBlackBox.mintLimit >= InceptionBlackBox.totalSupply{ 
            let InceptionBlackBoxAdmin = self.account.borrow<&InceptionBlackBox.Admin>(from: InceptionBlackBox.AdminStoragePath) ?? panic("Could not borrow a reference to the InceptionBlackBox Admin")
            let InceptionBlackBoxReceiver = recipient.getCapability(InceptionBlackBox.CollectionPublicPath).borrow<&{NonFungibleToken.CollectionPublic}>() ?? panic("Could not get receiver reference to the InceptionBlackBox Collection")
    ...
    pub fun mintInceptionBlackBoxWithInceptionCrystal(
        buyer: Address,
        ...
        let InceptionBlackBoxAdmin =
            self.account.borrow<&InceptionBlackBox.Admin>(
                from: InceptionBlackBox.AdminStoragePath
            )
            ?? panic(
                "Could not borrow a reference to the InceptionBlackBox Admin"
            )
        let recipient = getAccount(buyer)
        let InceptionBlackBoxReceiver =
            recipient.getCapability(InceptionBlackBox.CollectionPublicPath)
                .borrow<&{NonFungibleToken.CollectionPublic}>()
            ?? panic(
                "Could not get receiver reference to the InceptionBlackBox Collection"
            )
        InceptionBlackBoxAdmin.mintInceptionBlackBox(
            recipient: InceptionBlackBoxReceiver
        )
        destroy <-paymentVault
    }
    
    pub fun whitelistFreeMint(
        buyer: Address,
        setID: UInt64,
        numberOfTokens: UInt64
    ){ 
        ...
        let buyerDUCReceiverRef =
            getAccount(buyer).getCapability<&{FungibleToken.Receiver}>(
                /public/dapperUtilityCoinReceiver
            )
        assert(
