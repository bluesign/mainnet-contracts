...
pub contract InceptionExchange{ 
    ...
    // Resets weekly
    priv var BlackBoxTokenIDToRedemptionTimeInSeconds:{ UInt64: UInt64}
    
    priv var InceptionAvatarTokenIDToRedemptionTimeInSeconds:{ UInt64: UInt64}
    
    pub fun getCurrentBlockTimeInSeconds(): UInt64{ 
        return UInt64(getCurrentBlock().timestamp)
    }
    
    pub fun claimInceptionCrystalWithBlackBox(
        signerAuth: AuthAccount,
        tokenID: UInt64
    ){ 
        // Check blackbox collection to ensure ownership
        let tokenIDs =
            getAccount(signerAuth.address).getCapability<
                &InceptionBlackBox.Collection{
                    InceptionBlackBox.InceptionBlackBoxCollectionPublic
                }
            >(InceptionBlackBox.CollectionPublicPath).borrow()?.getIDs()
        ...
        let nextClaimTime =
            self.getNextInceptionBlackBoxRedemptionTimeInSeconds(
                tokenID: tokenID
            )
        ...
        let InceptionCrystalAdmin =
            self.account.borrow<&InceptionCrystal.Admin>(
                from: InceptionCrystal.AdminStoragePath
            )
            ?? panic(
                "Could not borrow a reference to the InceptionCrystal Admin"
            )
        ...
        == nil{ 
            ...
            signerAuth.save(
                <-collection,
                to: InceptionCrystal.CollectionStoragePath
            )
            signerAuth.link<
                &InceptionCrystal.Collection{
                    NonFungibleToken.CollectionPublic,
                    InceptionCrystal.InceptionCrystalCollectionPublic
                }
            >(
        ...
        let InceptionCrystalReceiver =
            recipient.getCapability(InceptionCrystal.CollectionPublicPath)
                .borrow<&{NonFungibleToken.CollectionPublic}>()
            ?? panic(
                "Could not get receiver reference to the InceptionCrystal Collection"
            )
        ...
        InceptionExchange.BlackBoxTokenIDToRedemptionTimeInSeconds[
            tokenID
        ] = UInt64(getCurrentBlock().timestamp)
    }
    
    pub fun exchangeCrystalForFlowToken(
        signerAuth: AuthAccount,
        amount: UInt64
    ){ 
        ...
        let tokenIDs =
            ...
            >(InceptionCrystal.CollectionPublicPath).borrow()?.getIDs()
        ...
        let signerCollectionRef =
            ...
            ?? panic(
                "Could not borrow a reference to the signer's InceptionCrystal collection"
            )
        ...
        let recipientFlowTokenRef =
            recipient.getCapability<&FlowToken.Vault{FungibleToken.Receiver}>(
                /public/flowTokenReceiver
            ).borrow()
            ?? panic("Could not borrow a reference to the recipient's Vault")
        let selfFlowWithdrawVault <-
            (
                self.account.borrow<&FlowToken.Vault>(
    ...
    pub fun getNextInceptionBlackBoxRedemptionTimeInSeconds(
        tokenID: UInt64
    ): UInt64{ 
        let lastClaimTime =
            InceptionExchange.BlackBoxTokenIDToRedemptionTimeInSeconds[tokenID]
            ?? 0
