...
pub contract Authenticator: NonFungibleToken{ 
    ...
    pub event MintLicense(id: UInt64, owner: Address?, contentProviderLabel: String, objectLabel: String?, categoryLabel: String?, expiration: UInt?, transferable: Bool)
    ...
    pub resource interface PublicContentProvider{ 
        ...
        pub fun buyLicenseWithFUSD(vault: @FUSD.Vault, objectLabel: String?, categoryLabel: String?): @NFT{} 
        
        pub fun buyLicenseWithDUC(vault: @DapperUtilityCoin.Vault, objectLabel: String?, categoryLabel: String?): @NFT{} 
    }
    ...
    pub resource Admin{ 
        ...
        pub fun changeStoragePaths(licenseCollectionStoragePath: StoragePath?, cpCollectionStoragePath: StoragePath?, adminContainerStoragePath: StoragePath?){ 
        ...
        pub fun changeNftPrice(nftPrice: UFix64){ 
            Authenticator.nftPrice = nftPrice!
        }
        
        pub fun changeDefaultNftStrings(defaultNftName: String?, defaultNftImage: String?, defaultNftExternalUrl: String?){ 
    ...
    pub resource ContentProvider: PublicContentProvider{ 
        ...
        pub fun createObject(label: String, metadata: String?, hiddenMetadata: String?, codeGenerationUrl: String?, privateInfoUrl: String?): @Object{ 
            pre{ 
        ...
        pub fun publishObject(object: Capability<&Object>, categoryLabels: [String]){ 
            pre{ 
                Authenticator.cpActive[self.label] == true:
                    "Content Provider is banned"
                object.borrow() != nil:
        ...
        pub fun createCategory(label: String, title: String?): @Category{ 
            pre{ 
                Authenticator.cpActive[self.label] == true:
                    "Content Provider is banned"
        ...
        pub fun buyLicenseWithFUSD(vault: @FUSD.Vault, objectLabel: String?, categoryLabel: String?): @NFT{ 
            ...
            let adminFusdReceiver = (getAccount(Authenticator.adminAddress).getCapability(/public/fusdReceiver)!).borrow<&{FungibleToken.Receiver}>() ?? panic("Could not borrow receiver reference to the Admin's FUSD Vault")
        ...
        pub fun buyLicenseWithDUC(vault: @DapperUtilityCoin.Vault, objectLabel: String?, categoryLabel: String?): @NFT{ 
            ...
            let adminDucReceiver = (getAccount(Authenticator.adminDapperWalletAddress).getCapability(/public/dapperUtilityCoinReceiver)!).borrow<&{FungibleToken.Receiver}>() ?? panic("Could not borrow receiver reference to the Admin's DUC Vault")
            adminDucReceiver.deposit(from: <-vault)
            return <-self.createLicenseNFT(transferable: Authenticator.boughtNftIsTransferable, expiration: Authenticator.boughtNftExpiration, objectLabel: objectLabel, categoryLabel: categoryLabel, image: Authenticator.boughtNftImage, name: Authenticator.boughtNftName, url: nil)
        }
        
        pub fun createLicenseNFT(transferable: Bool?, expiration: UInt?, objectLabel: String?, categoryLabel: String?, image: String?, name: String?, url: String?): @NFT{ 
            pre{ 
                Authenticator.cpActive[self.label] == true:
                    "Content Provider is banned"
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: Authenticator.licenseCollectionStoragePath, publicPath: Authenticator.licenseCollectionPubPath, providerPath: /private/nevernevernever, // we dont have this and we dont need it, but its mandatory publicCollection: Type<&Authenticator.Collection{Authenticator.PublicLicenseCollection}>(), publicLinkedType: Type<&Authenticator.Collection{Authenticator.PublicLicenseCollection, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&Authenticator.Collection{Authenticator.PublicLicenseCollection, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
