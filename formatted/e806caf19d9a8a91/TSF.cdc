...
pub contract TSF{ 
    ...
    pub fun mintNFT(recipientAcc: AuthAccount, momentIds: [UInt64]){ 
        if !self.isGameActive{ 
            panic("The Game is not yet active, wait for the admin to start the game")
        }
        if !self.isAddressMintAuthorizedThisWeek(
            address: recipientAcc.address
        ){ 
            panic(
                "The user is not able to mint this week, an entry for this week has already been submitted"
            )
        }
        
        // Check User Owns the TopShot moments at the time of submitting
        let userMoments:{ UInt64: TSF.OwnedPlayerInfo} =
            self.getUserTopShotMoments(
                recipientAcc: recipientAcc,
                momentIdsToMint: momentIds
            )
        let depositRef: &AnyResource{TSF.TopShotCareerCollectionPublic} =
    ...
    pub fun getUserTopShotMoments(
        ...
        let collectionRef =
            (account.getCapability(/public/MomentCollection)!).borrow<
        ...
        let manager =
            recipientAcc.borrow<&HybridCustody.Manager>(
        ...
        for address in (manager!).getChildAddresses(){ 
            ...
            let collectionRef = (account.getCapability(/public/MomentCollection)!).borrow<&{TopShot.MomentCollectionPublic}>()
            if collectionRef != nil{ 
                let collRef = collectionRef!
                for mId in momentIdsToMint{ 
                    let nft = collRef.borrowMoment(id: mId)
    ...
    pub resource Admin{ 
        ...
        pub fun setGameSeason(newSeason: String){ 
            ...
            TSF.currentWeek = 1
            TSF.accountsParticipated.insert(
                key: TSF.currentSeason,{
                TSF.currentWeek:{} }
            )
            TSF.usedMomentIds.insert(
                key: TSF.currentSeason,{
                TSF.currentWeek:{} }
            )
            TSF.userWeeklyTotalPoints.insert(
                key: TSF.currentSeason,{
                TSF.currentWeek:{} }
            )
        ...
        pub fun setGameWeek(newWeek: UInt8){ 
            ...
            (TSF.accountsParticipated[TSF.currentSeason]!).insert(
                key: TSF.currentWeek,{}
            ...
            (TSF.userWeeklyTotalPoints[TSF.currentSeason]!).insert(
                key: TSF.currentWeek,{}
                
            )
        }
        
        pub fun setPlayerWeeklyScore(
            season: String,
            week: UInt8,
            userAddr: Address,
            scores: [{
            ...
            let totalWeeklyScore: Fix64 =
                totalAssistsScore + totalTurnOverScore + totalStealScore
                + totalBlockScore
                + totalPointsScore
                + totalReboundScore
            ...
            let prevSeasonTotal: Fix64? =
                (TSF.userTotalPoints[season]!)[userAddr]
    ...
    init(){ 
        ...
        self.accountsParticipated ={ self.currentSeason:{ self.currentWeek:{} }}
        self.userWeeklyTotalPoints ={ 
                self.currentSeason:{ self.currentWeek:{} }
            }
        ...
        self.account.link<&{TopShotCareerCollectionPublic}>(
