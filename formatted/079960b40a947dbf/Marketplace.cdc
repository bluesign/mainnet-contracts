...
pub contract Marketplace{ 
    ...
    access(account) fun createNewComponentPublic(
        templateId: UInt64,
        mint: UInt64
    ){ 
        ...
        destroy old_res
        let new_res <-
            create ComponentPublic(templateId: templateId, mint: mint)
    ...
    priv fun updateMarketplace(
        userAddress: Address?,
        price: UFix64?,
        FlovatarComponentIDs: [
            UInt64
        ]
    ){ 
    ...
    init(){ 
        ...
        self.account.link<&{FlovatarMarketplace.SalePublic}>(
            FlovatarMarketplace.CollectionPublicPath,
            target: FlovatarMarketplace.CollectionStoragePath
        )
        self.account.save(
            <-create ComponentResource(),
            to: FlovatarComponent.CollectionStoragePath
        )
        self.account.link<&{FlovatarComponent.CollectionPublic}>(
            FlovatarComponent.CollectionPublicPath,
            target: FlovatarComponent.CollectionStoragePath
        )
        self.account.save(<-create Admin(), to: /storage/admin)
    }
}
