...
pub contract Sushi: FungibleToken{ 
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance{ 
        ...
        destroy(){ 
            Sushi.totalSupply = Sushi.totalSupply - self.balance
        }
    }
    ...
    pub resource Minter{ 
        ...
        //
        pub fun mintTokens(amount: UFix64): @Sushi.Vault{ 
            pre{ 
                amount > 0.0:
    ...
    init(){ 
        ...
        self.account.link<&Sushi.Vault{FungibleToken.Receiver}>(self.ReceiverPublicPath, target: self.VaultStoragePath)
        
        // Create a public capability to the stored Vault that only exposes
        // the `balance` field through the `Balance` interface
        self.account.link<&Sushi.Vault{FungibleToken.Balance}>(self.BalancePublicPath, target: self.VaultStoragePath)
