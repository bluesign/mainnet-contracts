...
pub contract BasicBeasts: NonFungibleToken{ 
    ...
    priv var beastTemplates:{ UInt32: BeastTemplate}
    
    priv var retired:{ UInt32: Bool}
    
    priv var numberMintedPerBeastTemplate:{ UInt32: UInt32}
            ...
            switch view{ 
                ...
                case Type<MetadataViews.Royalties>():
                    ...
                    if self.firstOwner != nil{ 
                        royalties.append(MetadataViews.Royalty(recepient: getAccount(self.firstOwner!).getCapability<&FlowToken.Vault{FungibleToken.Receiver}>(/public/flowTokenReceiver), cut: 0.05, // 5% royalty on secondary sales description: "First owner 5% royalty from secondary sales."))
                    }
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: BasicBeasts.CollectionStoragePath, publicPath: BasicBeasts.CollectionPublicPath, providerPath: BasicBeasts.CollectionPrivatePath, publicCollection: Type<&BasicBeasts.Collection{BasicBeasts.BeastCollectionPublic}>(), publicLinkedType: Type<&BasicBeasts.Collection{BasicBeasts.BeastCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&BasicBeasts.Collection{BasicBeasts.BeastCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Provider, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
                            return <-BasicBeasts.createEmptyCollection()
                        })
    ...
    init(){ 
        ...
        self.royalties = [MetadataViews.Royalty(recepient: self.account.getCapability<&FlowToken.Vault{FungibleToken.Receiver}>(/public/flowTokenReceiver), cut: 0.05, // 5% royalty on secondary sales description: "Basic Beasts 5% royalty from secondary sales.")]
        
        // Put Admin in storage
        self.account.save(<-create Admin(), to: self.AdminStoragePath)
        self.account.link<&BasicBeasts.Admin>(self.AdminPrivatePath, target: self.AdminStoragePath) ?? panic("Could not get a capability to the admin")
