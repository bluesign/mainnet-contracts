...
pub contract Poop: FungibleToken{ 
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance{ 
        ...
        destroy(){ 
            Poop.totalSupply = Poop.totalSupply - self.balance
        }
    }
    ...
    pub resource Minter{ 
        ...
        //
        pub fun mintTokens(amount: UFix64): @Poop.Vault{ 
            pre{ 
                amount > 0.0:
    ...
    init(){ 
        ...
        self.account.link<&Poop.Vault{FungibleToken.Receiver}>(self.ReceiverPublicPath, target: self.VaultStoragePath)
        
        // Create a public capability to the stored Vault that only exposes
        // the `balance` field through the `Balance` interface
        self.account.link<&Poop.Vault{FungibleToken.Balance}>(self.BalancePublicPath, target: self.VaultStoragePath)
