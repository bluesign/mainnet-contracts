...
pub contract EmptyPotionBottle: FungibleToken{ 
    ...
    init(){ 
        ...
        self.account.link<&EmptyPotionBottle.Vault{FungibleToken.Receiver}>(self.ReceiverPublicPath, target: self.VaultStoragePath)
        
        // Create a public capability to the stored Vault that only exposes
        // the `balance` field through the `Balance` interface
        self.account.link<&EmptyPotionBottle.Vault{FungibleToken.Balance}>(self.BalancePublicPath, target: self.VaultStoragePath)
