...
pub contract HunterScore{ 
    ...
    priv var hunterScores:{ Address: UInt32}
    
    priv var beastsCollected:{ Address: [UInt64]}
    
    priv var beastTemplatesCollected:{ Address: [UInt32]}
    
    priv var banned: [Address]
    
    // String: skin + " " + starLevel.toString()
    // e.g. "Normal 1"
    // UInt32: pointReward
    //
    priv var pointTable:{ String: UInt32}
    ...
    pub resource Admin{ 
        pub fun deductPoints(wallet: Address, pointsToDeduct: UInt32){ 
            ...
            emit HunterScorePointsDeducted(
                wallet: wallet,
    ...
    access(account) fun increaseHunterScore(
        wallet: Address,
