...
pub contract LufthausNFT: NonFungibleToken{ 
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: LufthausNFT.CollectionStoragePath, publicPath: LufthausNFT.CollectionPublicPath, providerPath: /private/LufthausNFTCollection, publicCollection: Type<&LufthausNFT.Collection{LufthausNFT.LufthausNFTCollectionPublic}>(), publicLinkedType: Type<&LufthausNFT.Collection{LufthausNFT.LufthausNFTCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&LufthausNFT.Collection{LufthausNFT.LufthausNFTCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Provider, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
                            return <-LufthausNFT.createEmptyCollection()
                        })
                case Type<MetadataViews.NFTCollectionDisplay>():
                    let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://swaychain.com/wp-content/uploads/2022/09/LUFTHAUS-LOGO.png"), mediaType: "image/png")
                    return MetadataViews.NFTCollectionDisplay(name: "Lufthaus Store", description: "Lufthaus NFTs are commemorative event tickets.", externalURL: MetadataViews.ExternalURL("http://lufthaus.io/"), squareImage: media, bannerImage: media, socials:{ "twitter": MetadataViews.ExternalURL("https://twitter.com/lufthausmusic"), "instagram": MetadataViews.ExternalURL("https://instagram.com/lufthausmusic")})
            }
            return nil
        }
    }
    ...
    pub resource NFTMinter{ 
        ...
        pub fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, name: String, description: String, thumbnail: String, royalties: [MetadataViews.Royalty], attributes:{ String: String}, metadata:{ String: String}){ 
            ...
            // create a new NFT
            var newNFT <- create NFT(id: LufthausNFT.totalSupply, name: name, description: description, thumbnail: thumbnail, royalties: royalties, attributes: attributes, metadata: metadata)
    ...
    init(){ 
        ...
        self.account.link<&LufthausNFT.Collection{NonFungibleToken.CollectionPublic, LufthausNFT.LufthausNFTCollectionPublic, MetadataViews.ResolverCollection}>(self.CollectionPublicPath, target: self.CollectionStoragePath)
