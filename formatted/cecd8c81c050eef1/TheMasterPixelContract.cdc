...
pub contract TheMasterPixelContract{ 
    ...
    init(){ 
        ...
        if self.account.borrow<&TheMasterPixelMinter>(
            from: self.MinterStoragePath
        )
        == nil
        && self.account.borrow<&TheMasterSectors>(
            from: self.CollectionStoragePath
        )
        == nil{ 
            self.account.save(
                <-create TheMasterPixelMinter(),
                to: self.MinterStoragePath
            )
            self.account.save(
                <-self.createEmptySectors(),
                to: self.CollectionStoragePath
            )
            self.account.link<&{TheMasterSectorsInterface}>(
                self.CollectionPublicPath,
                target: self.CollectionStoragePath
            )
            self.account.link<&TheMasterSectors>(
