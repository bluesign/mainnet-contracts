...
pub contract TheMasterMarketContract{ 
    ...
    init(){ 
        ...
        if self.account.borrow<&TheMasterMarketState>(
            from: self.MarketStateStoragePath
        )
        == nil{ 
            self.account.save(
            ...
            self.account.link<&{TheMasterMarketStateInterface}>(
                self.MarketStatePublicPath,
                target: self.MarketStateStoragePath
            )
        }
        if self.account.borrow<&TheMasterMarket>(
            from: self.CollectionStoragePath
        )
        == nil{ 
            self.account.save(
                <-self.createTheMasterMarket(
                    sectorsRef: self.account.getCapability<
                        &TheMasterPixelContract.TheMasterSectors
                    >(TheMasterPixelContract.CollectionPrivatePath)
                ),
                to: self.CollectionStoragePath
            )
            self.account.link<&{TheMasterMarketInterface}>(
    ...
    access(contract) fun isOpened(ownerAddress: Address): Bool{ 
        let refMarketState =
            getAccount(self.account.address).getCapability<
                &AnyResource{TheMasterMarketStateInterface}
            >(/public/TheMasterMarketState).borrow()!
    ...
    pub resource TheMasterMarketSector{ 
        ...
        access(account) let ownerVault: Capability<
            &AnyResource{FungibleToken.Receiver}
        >
        
        access(account) let creatorVault: Capability<
            &AnyResource{FungibleToken.Receiver}
        >
        
        access(account) let sectorsRef: Capability<
            &TheMasterPixelContract.TheMasterSectors
        >
        ...
        init(
            ...
            creatorVault: Capability<&AnyResource{FungibleToken.Receiver}>,
            sectorsRef: Capability<&TheMasterPixelContract.TheMasterSectors>
        ){ 
        ...
        pub fun purchase(
            ...
            recipient: &AnyResource{
                TheMasterPixelContract.TheMasterSectorsInterface
            },
            vaultRef: &AnyResource{FungibleToken.Provider}
        ){ 
            ...
            TheMasterPieceContract.setWalletSize(
                ...
                size: UInt16(
                    recipientSectorRef.getIds().length + tokenIDs.length
                )
            )
            for tokenID in tokenIDs{ 
