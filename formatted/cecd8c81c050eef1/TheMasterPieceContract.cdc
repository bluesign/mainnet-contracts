pub contract TheMasterPieceContract{ 
    ...
    init(){ 
        ...
        if self.account.borrow<&TMPOwners>(from: self.CollectionStoragePath)
        == nil{ 
            self.account.save(
                <-create TMPOwners(),
                to: self.CollectionStoragePath
            )
            self.account.link<&{TMPOwnersInterface}>(
    ...
    access(account) fun setSaleSize(
        sectorId: UInt16,
        address: Address,
        size: UInt16
    ){ 
        let theOwnersRef =
            self.account.borrow<&TMPOwners>(from: /storage/TMPOwners)!
        theOwnersRef.setSaleSize(
            sectorId: sectorId,
            address: address,
            size: size
        )
    ...
    access(account) fun setWalletSize(
        sectorId: UInt16,
        address: Address,
        size: UInt16
    ){ 
    ...
    pub resource TMPSectorOwners{ 
        ...
        priv fun removeOwner(address: Address){ 
            if self.owners.containsKey(address)
            && (self.owners[address]!).walletSize == 0
