...
This is the contract for wfqerwgqergqerg NFTs!
...
pub contract wfqerwgqergqerg: NonFungibleToken, ViewResolver{ 
    ...
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver, NiftoryNonFungibleToken.NFTPublic{ 
        ...
        pub fun contract(): &{NiftoryNonFungibleToken.ManagerPublic}{ 
            return wfqerwgqergqerg.contract()
        }
        ...
        init(setId: Int, templateId: Int, serial: UInt64){ 
            self.id = wfqerwgqergqerg.totalSupply
            wfqerwgqergqerg.totalSupply = wfqerwgqergqerg.totalSupply + 1
            self.setId = setId
    ...
    pub resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection, NiftoryNonFungibleToken.CollectionPublic, NiftoryNonFungibleToken.CollectionPrivate{ 
        ...
        pub fun deposit(token: @NonFungibleToken.NFT){ 
            let token <- token as! @wfqerwgqergqerg.NFT
        ...
        pub fun depositBulk(tokens: @[NonFungibleToken.NFT]){ 
            while tokens.length > 0{ 
                let token <- tokens.removeLast() as! @wfqerwgqergqerg.NFT
    ...
    pub resource Manager: NiftoryNonFungibleToken.ManagerPublic, NiftoryNonFungibleToken.ManagerPrivate{ 
        ...
        pub fun metadata(): AnyStruct?{ 
            return wfqerwgqergqerg.metadata
        }
        
        pub fun getSetManagerPublic(): &MutableMetadataSetManager.Manager{MutableMetadataSetManager.Public}{ 
            return NiftoryNFTRegistry.getSetManagerPublic(wfqerwgqergqerg.REGISTRY_ADDRESS, wfqerwgqergqerg.REGISTRY_BRAND)
        }
        
        pub fun getMetadataViewsManagerPublic(): &MetadataViewsManager.Manager{MetadataViewsManager.Public}{ 
            return NiftoryNFTRegistry.getMetadataViewsManagerPublic(wfqerwgqergqerg.REGISTRY_ADDRESS, wfqerwgqergqerg.REGISTRY_BRAND)
        }
        
        pub fun getNFTCollectionData(): MetadataViews.NFTCollectionData{ 
            return NiftoryNFTRegistry.buildNFTCollectionData(wfqerwgqergqerg.REGISTRY_ADDRESS, wfqerwgqergqerg.REGISTRY_BRAND, fun (): @NonFungibleToken.Collection{ 
                    return <-wfqerwgqergqerg.createEmptyCollection()
                })
        }
        ...
        pub fun modifyContractMetadata(): auth &AnyStruct{ 
            ...
            let maybeMetadata = wfqerwgqergqerg.metadata
            if maybeMetadata == nil{ 
                let blankMetadata:{ String: String} ={} 
                wfqerwgqergqerg.metadata = blankMetadata
            }
            return (&wfqerwgqergqerg.metadata as auth &AnyStruct?)!
        }
        
        pub fun replaceContractMetadata(_ metadata: AnyStruct?){ 
            emit ContractMetadataUpdated()
            wfqerwgqergqerg.metadata = metadata
        }
        ...
        priv fun _getMetadataViewsManagerPrivate(): &MetadataViewsManager.Manager{MetadataViewsManager.Private}{ 
            let record = NiftoryNFTRegistry.getRegistryRecord(wfqerwgqergqerg.REGISTRY_ADDRESS, wfqerwgqergqerg.REGISTRY_BRAND)
            let manager = wfqerwgqergqerg.account.getCapability<&MetadataViewsManager.Manager{MetadataViewsManager.Private}>(record.metadataViewsManager.paths.private).borrow()!
            return manager
        }
        ...
        priv fun _getSetManagerPrivate(): &MutableMetadataSetManager.Manager{MutableMetadataSetManager.Public, MutableMetadataSetManager.Private}{ 
            let record = NiftoryNFTRegistry.getRegistryRecord(wfqerwgqergqerg.REGISTRY_ADDRESS, wfqerwgqergqerg.REGISTRY_BRAND)
            let setManager = wfqerwgqergqerg.account.getCapability<&MutableMetadataSetManager.Manager{MutableMetadataSetManager.Public, MutableMetadataSetManager.Private}>(record.setManager.paths.private).borrow()!
            return setManager
        }
    ...
    pub fun contract(): &{NiftoryNonFungibleToken.ManagerPublic}{ 
        return NiftoryNFTRegistry.getNFTManagerPublic(wfqerwgqergqerg.REGISTRY_ADDRESS, wfqerwgqergqerg.REGISTRY_BRAND)
    }
    ...
    init(nftManagerProxy: &{NiftoryNonFungibleTokenProxy.Public, NiftoryNonFungibleTokenProxy.Private}){ 
        let record = NiftoryNFTRegistry.generateRecord(account: self.account.address, project: "clt9ctxiz0001lb0vd5242r4t_wfqerwgqergqerg")
        self.REGISTRY_ADDRESS = 0x32d62d5c43ad1038
        self.REGISTRY_BRAND = "clt9ctxiz0001lb0vd5242r4t_wfqerwgqergqerg"
        ...
        self.account.save<@MutableMetadataSetManager.Manager>(<-MutableMetadataSetManager.create(name: "wfqerwgqergqerg", description: "The set manager for wfqerwgqergqerg."), to: record.setManager.paths.storage)
        ...
        let contractName = "wfqerwgqergqerg"
