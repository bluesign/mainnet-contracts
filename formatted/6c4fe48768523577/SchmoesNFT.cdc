...
pub contract SchmoesNFT: NonFungibleToken{ 
    ...
    priv let editionToSchmoeData:{ UInt64: SchmoeData}
    
    priv let editionToProvenance:{ UInt64: String}
    
    priv let editionToNftId:{ UInt64: UInt64}
    
    priv let nftIdToEdition:{ UInt64: UInt64}
    
    // maps assetType to a dict of assetNames to base64 encoded images of assets
    priv let schmoeAssets:{ SchmoeTrait:{ String: String}}
    ...
    access(contract) fun batchMint(_ buyVault: @FungibleToken.Vault, _ mintAmount: UInt64): @NonFungibleToken.Collection{ 
        pre{ 
            buyVault.isInstance((self.account.getCapability<&{FungibleToken.Receiver}>(/public/flowTokenReceiver).borrow()!).getType()):
