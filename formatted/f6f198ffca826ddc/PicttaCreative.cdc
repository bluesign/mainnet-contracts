...
pub contract PicttaCreative: NonFungibleToken{ 
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &PicttaCreative.NFT?{ 
        let collection = (getAccount(from).getCapability(PicttaCreative.CollectionPublicPath)!).borrow<&PicttaCreative.Collection{PicttaCreative.PicttaCreativeCollectionPublic}>() ?? panic("Couldn't get collection")
