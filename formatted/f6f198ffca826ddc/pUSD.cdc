...
pub contract pUSD: FungibleToken{ 
    ...
    // Total supply of pUSD in existence
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance{ 
        ...
        destroy(){ 
            pUSD.totalSupply = pUSD.totalSupply - self.balance
        }
    }
    ...
    //
    pub fun createEmptyVault(): @pUSD.Vault{ 
    ...
    pub resource Minter{ 
        ...
        //
        pub fun mintTokens(amount: UFix64): @pUSD.Vault{ 
            pre{ 
                amount > 0.0:
                    "Amount minted must be greater than zero"
            }
            pUSD.totalSupply = pUSD.totalSupply + amount
    ...
    pub resource Administrator{ 
        ...
        // transactions/pUSD/minter/setup_minter_account.cdc
        // then the admin account running:
        // transactions/pUSD/admin/deposit_minter_capability.cdc
        //
    ...
    init(){ 
        self.AdminStoragePath = /storage/pUSDAdmin
        self.MinterProxyPublicPath = /public/pUSDMinterProxy
        self.MinterProxyStoragePath = /storage/pUSDMinterProxy
        self.VaultStoragePath = /storage/pUSDVault
        self.ReceiverPublicPath = /public/pUSDReceiver
        self.BalancePublicPath = /public/pUSDBalance
        ...
        self.account.save(<-mintedVault, to: self.VaultStoragePath)
        self.account.link<&pUSD.Vault{FungibleToken.Receiver}>(self.ReceiverPublicPath, target: self.VaultStoragePath)
        self.account.link<&pUSD.Vault{FungibleToken.Balance}>(self.BalancePublicPath, target: self.VaultStoragePath)
    }
}
