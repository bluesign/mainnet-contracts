...
pub contract PDS{ 
    ...
    pub resource SharedCapabilities{ 
        priv let withdrawCap: Capability<&{NonFungibleToken.Provider}>
        
        priv let operatorCap: Capability<&{IPackNFT.IOperator}>
        ...
        pub fun mintPackNFT(
            ...
            while i < commitHashes.length{ 
                ...
                mintedNftIDs.append(n.id)
                recvCap.deposit(token: <-n)
            }
            return mintedNftIDs
        }
        
        pub fun revealPackNFT(
            packId: UInt64,
            nfts: [{
        ...
        pub fun openPackNFT(
            ...
            if collectionProviderPath == nil{ 
                ...
                self.releaseEscrow(nftIds: toReleaseNFTs, recvCap: recvCap, collectionProviderPath: collectionProviderPath!)
            }
        }
        
        priv fun releaseEscrow(
            nftIds: [
                UInt64
            ],
            recvCap: &{NonFungibleToken.CollectionPublic},
            ...
            let pdsCollection =
                PDS.account.getCapability(collectionProviderPath).borrow<
                    &{NonFungibleToken.Provider}
                >()
                ?? panic(
                    "Unable to borrow PDS collection provider capability from private path"
                )
            var i = 0
        ...
        priv fun fulfillFromIssuer(
            nftIds: [
                UInt64
            ],
            recvCap: &{NonFungibleToken.CollectionPublic}
        ){ 
            let issuerCollection =
                self.withdrawCap.borrow()
                ?? panic("Unable to borrow withdrawCap")
            var i = 0
            while i < nftIds.length{ 
    ...
    pub resource interface PackIssuerCapReciever{ 
        pub fun setDistCap(
            cap: Capability<&DistributionCreator{IDistCreator}>
        ){} 
    }
    
    pub resource PackIssuer: PackIssuerCapReciever{ 
        priv var cap: Capability<&DistributionCreator{IDistCreator}>?
        ...
        pub fun changeDistributionSharedCap(distIds: [UInt64], withdrawCap: Capability<&{NonFungibleToken.Provider}>, operatorCap: Capability<&{IPackNFT.IOperator}>){ 
            for distId in distIds{ 
                let oldSharedCap <- PDS.DistSharedCap.remove(key: distId)!
                PDS.DistSharedCap[distId] <-! create SharedCapabilities(withdrawCap: withdrawCap, operatorCap: operatorCap)
    ...
    pub resource interface IDistCreator{ 
        pub fun createNewDist(
            sharedCap: @SharedCapabilities,
            title: String,
            metadata:{ 
                String: String
            }
    ...
    pub resource DistributionManager{ 
        pub fun updateDistState(distId: UInt64, state: PDS.DistState){ 
            let d =
                PDS.Distributions.remove(key: distId)
                ?? panic("No such distribution")
        ...
        pub fun withdraw(
            ...
            escrowCollectionPublic: PublicPath
        ){ 
            assert(
                PDS.DistSharedCap.containsKey(distId),
                message: "No such distribution"
            )
            let d <- PDS.DistSharedCap.remove(key: distId)!
            let pdsCollection =
                PDS.getManagerCollectionCap(
                    escrowCollectionPublic: escrowCollectionPublic
                ).borrow()!
            var i = 0
            while i < nftIDs.length{ 
    ...
    access(contract) fun getManagerCollectionCap(
        escrowCollectionPublic: PublicPath
    ): Capability<&{NonFungibleToken.CollectionPublic}>{ 
        let pdsCollection =
            self.account.getCapability<&{NonFungibleToken.CollectionPublic}>(
    ...
    init(
        ...
        self.account.link<&DistributionCreator{PDS.IDistCreator}>(
            self.DistCreatorPrivPath,
            target: self.DistCreatorStoragePath
        )
