...
pub contract MusicBlockMarket{ 
    ...
    pub resource SaleOffer: SaleOfferPublicView{ 
        ...
        priv let sellerItemProvider: Capability<&MusicBlock.Collection{NonFungibleToken.Provider}>
        
        // The Melos vault that will receive that payment if teh sale completes successfully.
        priv let sellerPaymentReceiver: Capability<&Melos.Vault{FungibleToken.Receiver}>
        ...
        //
        pub fun accept(buyerCollection: &MusicBlock.Collection{NonFungibleToken.Receiver}, buyerPayment: @FungibleToken.Vault){ 
            pre{ 
        ...
        //
        // typeID: UInt64,
        init(sellerItemProvider: Capability<&MusicBlock.Collection{NonFungibleToken.Provider, MusicBlock.MusicBlockCollectionPublic}>, itemID: UInt64, sellerPaymentReceiver: Capability<&Melos.Vault{FungibleToken.Receiver}>, price: UFix64){ 
            pre{ 
            ...
            assert(collectionRef.borrowNFT(id: itemID) != nil, message: "Specified NFT is not available in the owner's collection")
