...
pub contract YDYHeartNFT: NonFungibleToken{ 
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: YDYHeartNFT.CollectionStoragePath, publicPath: YDYHeartNFT.CollectionPublicPath, providerPath: /private/ydyNFTCollection, publicCollection: Type<&YDYHeartNFT.Collection{YDYHeartNFT.YDYNFTCollectionPublic}>(), publicLinkedType: Type<&YDYHeartNFT.Collection{YDYHeartNFT.YDYNFTCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&YDYHeartNFT.Collection{YDYHeartNFT.YDYNFTCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Provider, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
                ...
                case Type<MetadataViews.Royalties>():
                    return MetadataViews.Royalties([MetadataViews.Royalty(recepient: getAccount(YDYHeartNFT.account.address).getCapability<&{FungibleToken.Receiver}>(/public/flowTokenReceiver), cut: 0.075, description: "This is the royalty receiver for YDY NFTs")])
            }
            return nil
        }
    }
    ...
    pub fun buy(collectionCapability: Capability<&Collection{YDYHeartNFT.YDYNFTCollectionPublic}>, payment: @FlowToken.Vault, quantity: UInt64){ 
        ...
        let ydyCollection = self.account.getCapability(YDYHeartNFT.CollectionPublicPath).borrow<&Collection{YDYHeartNFT.YDYNFTCollectionPublic, YDYHeartNFT.YDYNFTCollectionPrivate}>() ?? panic("Can't get YDY's collection.")
    ...
    pub resource Admin{ 
        ...
        pub fun transferNFT(collectionCapability: Capability<&Collection{YDYHeartNFT.YDYNFTCollectionPublic}>): UInt64{ 
            let ydyCollection = YDYHeartNFT.account.getCapability(YDYHeartNFT.CollectionPublicPath).borrow<&Collection{YDYHeartNFT.YDYNFTCollectionPublic, YDYHeartNFT.YDYNFTCollectionPrivate}>() ?? panic("Can't get YDY's collection.")
            let availableNFTs = ydyCollection.getIDs()
            if availableNFTs.length > 0{ 
                let receiver = collectionCapability.borrow() ?? panic("Cannot borrow")
                let randomInt = unsafeRandom() % UInt64(availableNFTs.length)
