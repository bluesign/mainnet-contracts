...
import
    ...
    from "../0x28abb9f291cadaf2/BarterYardClubWerewolf.cdc"
...
pub contract AlchemyMetadataWrapperMainnetShard2{ 
    ...
    pub fun getBarterYardClubWerewolf(
        owner: PublicAccount,
        id: UInt64
    ): NFTData?{ 
        let contract =
        ...
        let collection =
            owner.getCapability<&{MetadataViews.ResolverCollection}>(
                BarterYardClubWerewolf.CollectionPublicPath
            ).borrow()
            ?? panic("Could not borrow a reference to the collection")
        let nft = collection.borrowViewResolver(id: id)
        let view =
            nft.resolveView(Type<BarterYardClubWerewolf.CompleteDisplay>())!
        ...
        return NFTData(
            ...
            media: [
                NFTMedia(
                    uri: "https://ipfs.io/ipfs/".concat(display.thumbnail.cid)
                        .concat("/").concat(display.thumbnail.path!),
                    mimetype: "image"
                )
            ],
            metadata:{ 
    ...
    pub fun getNFTIDs(ownerAddress: Address):{ String: [UInt64]}{ 
        ...
        if let col =
            owner.getCapability(BarterYardClubWerewolf.CollectionPublicPath)
                .borrow<
                &BarterYardClubWerewolf.Collection{
                    NonFungibleToken.CollectionPublic
                }
            >(){ 
