...
pub contract PeekToken: NonFungibleToken{ 
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &PeekToken.NFT?{ 
        let collection = (getAccount(from).getCapability(PeekToken.CollectionPublicPath)!).borrow<&PeekToken.Collection{PeekToken.PeekTokenCollectionPublic}>() ?? panic("Couldn't get collection")
