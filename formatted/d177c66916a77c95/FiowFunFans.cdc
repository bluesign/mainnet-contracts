...
pub contract FiowFunFans: FungibleToken, ViewResolver{ 
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, MetadataViews.Resolver{ 
        ...
        pub fun withdraw(amount: UFix64): @FungibleToken.Vault{ 
            ...
            if let owner: Address = self.owner?.address{ 
                FiowFunFans.setBalance(address: owner, balance: self.balance)
            }
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                case Type<FungibleTokenMetadataViews.FTView>():
                    return FiowFunFans.resolveView(view)
        ...
        destroy(){ 
            if self.balance > 0.0{ 
                ...
                FiowFunFans.totalSupply = FiowFunFans.totalSupply - self.balance
            }
        }
    }
    ...
    pub resource Minter: Toucans.Minter{ 
        pub fun mint(amount: UFix64): @Vault{ 
            post{ 
                FiowFunFans.maxSupply == nil || FiowFunFans.totalSupply <= FiowFunFans.maxSupply!:
                    "Exceeded the max supply of tokens allowd."
            }
            FiowFunFans.totalSupply = FiowFunFans.totalSupply + amount
    ...
    pub fun resolveView(_ view: Type): AnyStruct?{ 
        switch view{ 
            ...
            case Type<FungibleTokenMetadataViews.FTDisplay>():
                ...
                return FungibleTokenMetadataViews.FTDisplay(name: "Fiow Fun Fans", symbol: "FFF", description: "Soooooooooooon.", externalURL: MetadataViews.ExternalURL("x.com/freedomrum"), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("freedomrum"), "discord": MetadataViews.ExternalURL("Irin4394")})
            case Type<FungibleTokenMetadataViews.FTVaultData>():
                return FungibleTokenMetadataViews.FTVaultData(storagePath: FiowFunFans.VaultStoragePath, receiverPath: FiowFunFans.ReceiverPublicPath, metadataPath: FiowFunFans.VaultPublicPath, providerPath: /private/FiowFunFansVault, receiverLinkedType: Type<&Vault{FungibleToken.Receiver}>(), metadataLinkedType: Type<&Vault{FungibleToken.Balance, MetadataViews.Resolver}>(), providerLinkedType: Type<&Vault{FungibleToken.Provider}>(), createEmptyVaultFunction: fun (): @Vault{ 
                        return <-FiowFunFans.createEmptyVault()
                    })
        }
        return nil
    }
    ...
    init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
        ...
        // Paths
        self.VaultStoragePath = /storage/FiowFunFansVault
        self.ReceiverPublicPath = /public/FiowFunFansReceiver
        self.VaultPublicPath = /public/FiowFunFansMetadata
        self.MinterStoragePath = /storage/FiowFunFansMinter
        self.AdministratorStoragePath = /storage/FiowFunFansAdmin
