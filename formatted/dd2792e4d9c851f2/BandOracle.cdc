...
pub contract BandOracle{ 
    ...
    pub resource interface OracleAdmin{ 
        pub fun getUpdaterCapabilityPathFromAddress(
            relayer: Address
        ): PrivatePath{} 
    ...
    pub resource BandOracleAdmin: OracleAdmin, DataUpdater{ 
        ...
        ///
        pub fun updateData(symbolsRates:{ String: UInt64}, resolveTime: UInt64, requestID: UInt64, relayerID: UInt64){ 
        ...
        /// @param relayerID: The ID of the relayer carrying the update.
        ///
        pub fun forceUpdateData(symbolsRates:{ String: UInt64}, resolveTime: UInt64, requestID: UInt64, relayerID: UInt64){ 
    ...
    pub resource Relay{ 
        ...
        pub fun relayRates(
            ...
            let updaterRef =
                ...
                ?? panic(
                    "Can't borrow reference to data updater while processing request "
                        .concat(requestID.toString())
                )
    ...
    ///
    access(contract) fun calculateReferenceData(
        baseRefData: RefData,
        quoteRefData: RefData
    ): ReferenceData{ 
        let rate =
            UInt256(
                UInt256(baseRefData.rate) * BandOracle.e18
                / UInt256(quoteRefData.rate)
            )
    ...
    access(contract) fun collectFees(): @FungibleToken.Vault{ 
        ...
        collectedFees.deposit(
            from: <-BandOracle.payments.withdraw(
                amount: BandOracle.payments.balance
            )
        )
    ...
    init(){ 
        ...
        self.account.save(
            <-create BandOracleAdmin(),
            to: self.OracleAdminStoragePath
        )
        self.account.link<&{OracleAdmin}>(
            self.OracleAdminPrivatePath,
            target: self.OracleAdminStoragePath
        )
        self.symbolsRefData ={} 
        ...
        let oracleAdminRef =
            self.account.borrow<&{OracleAdmin}>(
                from: BandOracle.OracleAdminStoragePath
            )
            ?? panic("Can't borrow a reference to the Oracle Admin")
        let dataUpdaterPrivatePath =
            oracleAdminRef.getUpdaterCapabilityPathFromAddress(
                relayer: self.account.address
            )
        self.account.link<&{BandOracle.DataUpdater}>(
            dataUpdaterPrivatePath,
            target: BandOracle.OracleAdminStoragePath
        )
        ?? panic("Data Updater capability for admin creation failed")
        let updaterCapability =
            self.account.getCapability<&{BandOracle.DataUpdater}>(
                dataUpdaterPrivatePath
            )
        let relayer <-
            BandOracle.createRelay(updaterCapability: updaterCapability)
        self.account.save(<-relayer, to: BandOracle.RelayStoragePath)
        self.account.link<&BandOracle.Relay>(
            BandOracle.RelayPrivatePath,
            target: BandOracle.RelayStoragePath
        )
    }
}
