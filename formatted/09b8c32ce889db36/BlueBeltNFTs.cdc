...
pub contract BlueBeltNFTs: NonFungibleToken{ 
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &BlueBeltNFTs.NFT?{ 
        let collection = getAccount(from).getCapability(BlueBeltNFTs.CollectionPublicPath).borrow<&BlueBeltNFTs.Collection{BlueBeltNFTs.BlueBeltNFTsCollectionPublic}>() ?? panic("Couldn't get collection")
