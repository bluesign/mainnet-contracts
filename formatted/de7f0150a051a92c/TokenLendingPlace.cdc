...
pub contract TokenLendingPlace{ 
    ...
    pub fun getmFlowBorrowingTokenPrice(): UFix64{ 
        let delta =
            getCurrentBlock().timestamp - TokenLendingPlace.finalTimestamp
        return TokenLendingPlace.mFlowBorrowingtokenPrice
        + delta * TokenLendingPlace.mFlowBorrowingInterestRate
    ...
    pub fun getmFlowTokenPrice(): UFix64{ 
        ...
        + delta * TokenLendingPlace.mFlowInterestRate
        / (365.0 * 24.0 * 60.0 * 60.0)
    }
    ...
    pub fun getmFiatTokenTokenPrice(): UFix64{ 
        ...
        + delta * TokenLendingPlace.mFiatTokenInterestRate
        / (365.0 * 24.0 * 60.0 * 60.0)
    }
    ...
    pub fun borrowCollection(address: Address): &TokenLendingCollection?{ 
        if self.lendingCollection[address] != nil{ 
            return &self.lendingCollection[address] as                                                       // The method for updating mToken and interest rate in the protocol.
    ...
    access(contract) fun updatePriceAndInterest(){ 
        let delta =
            getCurrentBlock().timestamp - TokenLendingPlace.finalTimestamp
        TokenLendingPlace.mFlowtokenPrice                                          // event
                                          = TokenLendingPlace.mFlowtokenPrice
            // event
            ...
            / (               // event         
               ...
               365.0                     // event         
                     ...
                     *                       // Liquidate the user over the UtilizationRate
                       ...
                       24.0 * 60.0                                   // event
                                   * 60.0)
        TokenLendingPlace.mFiatTokentokenPrice = TokenLendingPlace
                .mFiatTokentokenPrice
            ...
            // event
            + delta // createCollection returns a new collection resource to the caller
...
// getCurrentBlock().height * TokenLendingPlace.mFiatTokenInterestRate
            / (365.0 * 24.0 * 60.0 * 60.0)
        TokenLendingPlace.mFlowBorrowingtokenPrice = TokenLendingPlace
                .mFlowBorrowingtokenPrice
            + delta * TokenLendingPlace.mFlowBorrowingInterestRate
            / (365.0 * 24.0 * 60.0 * 60.0)
        ...
        * TokenLendingPlace.getmFlowBorrowingTokenPrice()
        != 0.0{ 
            ...
            TokenLendingPlace.mFlowInterestRate = TokenLendingPlace
                    .mFlowBorrowingInterestRate
        ...
        * TokenLendingPlace.getmFiatTokenBorrowingTokenPrice()
        != 0.0{ 
            ...
            < TokenLendingPlace.optimalUtilizationRate{ 
                TokenLendingPlace
                    .mFiatTokenBorrowingInterestRate = TokenLendingPlace
    ...
    pub resource TokenLendingCollection{ 
        priv var mFlow: UFix64
        
        priv var mFiatToken: UFix64
        
        priv var myBorrowingmFlow: UFix64
        
        priv var myBorrowingmFiatToken: UFix64
        
        priv var ownerAddress: Address
        ...
        pub fun addLiquidity(
            ...
            _cer: Capability<&UserCertificate>
        ){ 
            assert(
        ...
        pub fun getNetValue(): UFix64{ 
            let NetValue =
                ...
                * TokenLendingPlace.FiatTokenRealPrice
                - self.myBorrowingmFlow
                * TokenLendingPlace.getmFlowBorrowingTokenPrice()
                * TokenLendingPlace.FlowTokenRealPrice
                - self.myBorrowingmFiatToken
                * TokenLendingPlace.getmFiatTokenBorrowingTokenPrice()
                * TokenLendingPlace.FiatTokenRealPrice
            return NetValue
        }
        ...
        pub fun borrowFiatToken(
            ...
            let AmountofmToken =
                _amount / TokenLendingPlace.getmFiatTokenBorrowingTokenPrice()
            TokenLendingPlace.mFiatTokenBorrowingAmountToken = AmountofmToken
                + TokenLendingPlace.mFiatTokenBorrowingAmountToken
            self.myBorrowingmFiatToken = AmountofmToken
                + self.myBorrowingmFiatToken
            let tokenVault <-
                TokenLendingPlace.TokenVaultFiatToken.withdraw(amount: _amount)
            TokenLendingPlace.updatePriceAndInterest()
            self.checkBorrowValid()
            emit Borrow(
                borrower: self.ownerAddress,
        ...
        pub fun repayFiatToken(
            from: @FiatToken.Vault,
            _cer: Capability<&UserCertificate>
        ){ 
            pre{ 
        ...
        priv fun depositemFlow(from: UFix64){ 
            self.mFlow = self.mFlow + from
        }
        
        priv fun depositemFiatToken(from: UFix64){ 
    ...
    pub fun createTokenLendingCollection(_cer: Capability<&UserCertificate>){ 
        ...
        ] <-! create TokenLendingCollection(
                _owner: ((_cer.borrow()!).owner!).address
            )
    }
    ...
    pub resource Setter{ 
        pub fun updatePricefromOracle(
            _FlowPrice: UFix64,
            _FiatTokenPrice: UFix64
        ){ 
            TokenLendingPlace.FlowTokenRealPrice = _FlowPrice
            TokenLendingPlace.FiatTokenRealPrice = _FiatTokenPrice
        }
        
        pub fun updateDepositLimit(
            _FlowLimit: UFix64,
            _FiatTokenLimit: UFix64
        ){ 
    ...
    pub resource SetterProxy: SetterProxyPublic{ 
        priv var SetterCapability: Capability<&Setter>?
