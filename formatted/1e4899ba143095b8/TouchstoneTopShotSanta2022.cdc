...
pub contract TouchstoneTopShotSanta2022: NonFungibleToken{ 
    ...
    pub struct NFTMetadata{ 
        ...
        init(_name: String, _description: String, _image: MetadataViews.IPFSFile, _thumbnail: MetadataViews.IPFSFile?, _price: UFix64?, _extra:{ String: AnyStruct}, _supply: UInt64){ 
            self.metadataId = TouchstoneTopShotSanta2022.nextMetadataId
            self.name = _name
    ...
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver{ 
        ...
        pub fun getMetadata(): NFTMetadata{ 
            return TouchstoneTopShotSanta2022.getNFTMetadata(self.metadataId)!
        }
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: TouchstoneTopShotSanta2022.CollectionStoragePath, publicPath: TouchstoneTopShotSanta2022.CollectionPublicPath, providerPath: TouchstoneTopShotSanta2022.CollectionPrivatePath, publicCollection: Type<&Collection{NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), publicLinkedType: Type<&Collection{NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&Collection{NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection, NonFungibleToken.Provider}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
                            return <-TouchstoneTopShotSanta2022.createEmptyCollection()
                        })
                case Type<MetadataViews.ExternalURL>():
                    return MetadataViews.ExternalURL("https://touchstone.city/discover/".concat((self.owner!).address.toString()).concat("/TouchstoneTopShotSanta2022"))
                case Type<MetadataViews.NFTCollectionDisplay>():
                    let squareMedia = MetadataViews.Media(file: TouchstoneTopShotSanta2022.getCollectionAttribute(key: "image") as! MetadataViews.IPFSFile, mediaType: "image")
                    
                    // If a banner image exists, use it
                    // Otherwise, default to the main square image
                    var bannerMedia: MetadataViews.Media? = nil
                    if let bannerImage = TouchstoneTopShotSanta2022.getOptionalCollectionAttribute(key: "bannerImage") as! MetadataViews.IPFSFile?{ 
                        bannerMedia = MetadataViews.Media(file: bannerImage, mediaType: "image")
                    }
                    return MetadataViews.NFTCollectionDisplay(name: TouchstoneTopShotSanta2022.getCollectionAttribute(key: "name") as! String, description: TouchstoneTopShotSanta2022.getCollectionAttribute(key: "description") as! String, externalURL: MetadataViews.ExternalURL("https://touchstone.city/discover/".concat((self.owner!).address.toString()).concat("/TouchstoneTopShotSanta2022")), squareImage: squareMedia, bannerImage: bannerMedia ?? squareMedia, socials: TouchstoneTopShotSanta2022.getCollectionAttribute(key: "socials") as!{ String: MetadataViews.ExternalURL})
        ...
        init(_metadataId: UInt64, _serial: UInt64, _recipient: Address){ 
            pre{ 
                TouchstoneTopShotSanta2022.metadatas[_metadataId] != nil:
                    "This NFT does not exist yet."
                _serial < (TouchstoneTopShotSanta2022.getNFTMetadata(_metadataId)!).supply:
                    "This serial does not exist for this metadataId."
                !(TouchstoneTopShotSanta2022.getNFTMetadata(_metadataId)!).purchasers.containsKey(_serial):
            ...
            if let buyersRef = &TouchstoneTopShotSanta2022.primaryBuyers[_recipient] as &{UInt64: [UInt64]}?{ 
                ...
                TouchstoneTopShotSanta2022.primaryBuyers[_recipient] ={ _metadataId: [_serial]}
            }
            
            // Update who bought this serial inside NFTMetadata so it cannot be purchased again.
            let metadataRef = (&TouchstoneTopShotSanta2022.metadatas[_metadataId] as &NFTMetadata?)!
            metadataRef.purchased(serial: _serial, buyer: _recipient)
            TouchstoneTopShotSanta2022.totalSupply = TouchstoneTopShotSanta2022.totalSupply + 1
    ...
    pub resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection{ 
        ...
        pub fun claim(){ 
            if let storage = &TouchstoneTopShotSanta2022.nftStorage[(self.owner!).address] as &{UInt64: NFT}?{ 
    ...
    pub fun mintNFT(metadataId: UInt64, recipient: &{NonFungibleToken.Receiver}, payment: @FlowToken.Vault, serial: UInt64): UInt64{ 
        ...
        if let royalty = TouchstoneTopShotSanta2022.getOptionalCollectionAttribute(key: "royalty") as! MetadataViews.Royalty?{ 
    ...
    pub resource Administrator{ 
        pub fun createNFTMetadata(name: String, description: String, imagePath: String, thumbnailPath: String?, ipfsCID: String, price: UFix64?, extra:{ String: AnyStruct}, supply: UInt64){ 
            TouchstoneTopShotSanta2022.metadatas[TouchstoneTopShotSanta2022.nextMetadataId] = NFTMetadata(_name: name, _description: description, _image: MetadataViews.IPFSFile(cid: ipfsCID, path: imagePath), _thumbnail: thumbnailPath == nil ? nil : MetadataViews.IPFSFile(cid: ipfsCID, path: thumbnailPath), _price: price, _extra: extra, _supply: supply)
            TouchstoneTopShotSanta2022.nextMetadataId = TouchstoneTopShotSanta2022.nextMetadataId + 1
        }
        ...
        pub fun mintNFT(metadataId: UInt64, serial: UInt64, recipient: Address){ 
            pre{ 
                EmeraldPass.isActive(user: TouchstoneTopShotSanta2022.account.address):
            ...
            if let recipientCollection = getAccount(recipient).getCapability(TouchstoneTopShotSanta2022.CollectionPublicPath).borrow<&TouchstoneTopShotSanta2022.Collection{NonFungibleToken.CollectionPublic}>(){ 
                recipientCollection.deposit(token: <-nft)
            } else if let storage = &TouchstoneTopShotSanta2022.nftStorage[recipient] as &{UInt64: NFT}?{ 
                storage[nft.id] <-! nft
            } else{ 
                TouchstoneTopShotSanta2022.nftStorage[recipient] <-!{ nft.id: <-nft}
            }
        }
        ...
        pub fun changeField(key: String, value: AnyStruct){ 
            TouchstoneTopShotSanta2022.collectionInfo[key] = value
        }
    }
    ...
    pub fun getPriceOfNFT(_ metadataId: UInt64): UFix64?{ 
        if let metadata: TouchstoneTopShotSanta2022.NFTMetadata = self.getNFTMetadata(metadataId){ 
    ...
    pub fun getClaimableNFTs(user: Address):{ UInt64: NFTMetadata}{ 
        ...
        if let storage = &TouchstoneTopShotSanta2022.nftStorage[user] as &{UInt64: NFT}?{ 
            for id in storage.keys{ 
    ...
    init(_name: String, _description: String, _imagePath: String, _bannerImagePath: String?, _minting: Bool, _royalty: MetadataViews.Royalty?, _defaultPrice: UFix64, _paymentType: String, _ipfsCID: String, _lotteryBuying: Bool, _socials:{ String: MetadataViews.ExternalURL}, _mintVerifiers: [{MintVerifiers.IVerifier}]){ 
        ...
        self.CollectionStoragePath = /storage/TouchstoneTopShotSanta2022Collection_0x1e4899ba143095b8
        self.CollectionPublicPath = /public/TouchstoneTopShotSanta2022Collection_0x1e4899ba143095b8
        self.CollectionPrivatePath = /private/TouchstoneTopShotSanta2022Collection_0x1e4899ba143095b8
        self.AdministratorStoragePath = /storage/TouchstoneTopShotSanta2022Administrator_0x1e4899ba143095b8
