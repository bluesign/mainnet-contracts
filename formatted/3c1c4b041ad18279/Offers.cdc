...
pub contract Offers{ 
    ...
    pub resource Storefront: StorefrontPublic{ 
        priv let offers: @{UInt64: Offer}
        
        pub fun createOffer(offeredAmount: UFix64, paymentTokenType: Type, filterGroup: Filter.FilterGroup, expiry: UInt64, numAcceptable: Int, taker: Address?, paymentProvider: Capability<&{FungibleToken.Provider, FungibleToken.Balance, FungibleToken.Receiver}>, nftReceiver: Capability<&{NonFungibleToken.CollectionPublic}>){ 
            pre{ 
    ...
    pub fun borrowPublicAdmin(): &Admin{AdminPublic}{ 
        return self.account.borrow<&Admin{AdminPublic}>(
            from: Offers.AdminStoragePath
        )!
    }
