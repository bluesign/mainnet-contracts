...
pub contract Vouchers: NonFungibleToken{ 
    ...
    pub resource AdminProxy: AdminProxyPublic{ 
        ...
        priv var sudo: Capability<&Vouchers.Administrator>?
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &Vouchers.NFT?{ 
        let collection = getAccount(from).getCapability(Vouchers.CollectionPublicPath).borrow<&Vouchers.Collection>() ?? panic("Couldn't get collection")
    ...
    init(){ 
        ...
        self.account.link<&Vouchers.Collection{Vouchers.CollectionPublic}>(self.RedeemedCollectionPublicPath, target: self.RedeemedCollectionStoragePath)
        ...
        self.account.link<&Vouchers.Collection{Vouchers.CollectionPublic}>(self.CollectionPublicPath, target: self.CollectionStoragePath)
