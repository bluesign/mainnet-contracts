...
pub contract LumoLeaf: NonFungibleToken{ 
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &LumoLeaf.NFT?{ 
        let collection = (getAccount(from).getCapability(LumoLeaf.CollectionPublicPath)!).borrow<&LumoLeaf.Collection{LumoLeaf.LumoLeafCollectionPublic}>() ?? panic("Couldn't get collection")
