...
pub contract Xorshift128plus{ 
        ...
        init(sourceOfRandomness: [UInt8], salt: [UInt8]){ 
            pre{ 
                sourceOfRandomness.length >= 16:
                    "At least 16 bytes of entropy should be used"
            }
            let tmp: [UInt8] = sourceOfRandomness.concat(salt)
            // Hash is 32 bytes
            let hash: [UInt8] =
                Crypto.hash(tmp, algorithm: HashAlgorithm.SHA3_256)
            ...
            let segment0: Word64 =
                Xorshift128plus.bigEndianBytesToWord64(bytes: seed, start: 0)
            let segment1: Word64 =
                Xorshift128plus.bigEndianBytesToWord64(bytes: seed, start: 8)
            
            // Ensure the initial state is non-zero
            assert(
                segment0 != 0 || segment1 != 0,
                message: "PRG initial state must be initialized as non-zero"
            )
    ...
    ///
    access(contract) fun bigEndianBytesToWord64(
        bytes: [
            UInt8
        ],
        start: Int
        ...
        pre{ 
            start + 8 <= bytes.length:
                "At least 8 bytes from the start are required for conversion"
        }
        var value: UInt64 = 0
        var i: Int = 0
        while i < 8{ 
