...
pub contract SoulMadePack: NonFungibleToken{ 
    ...
    priv var freeClaim:{ Address: String}
    ...
    pub event WithdrawnMain(id: UInt64, mainDetail: SoulMadeMain.MainDetail)
    
    pub event WithdrawnComponent(id: UInt64, componentDetail: SoulMadeComponent.ComponentDetail)
    ...
    pub struct PackDetail{ 
        ...
        init(id: UInt64, scarcity: String, series: String, ipfsHash: String){ 
            self.id = id
    ...
    pub resource NFT: NonFungibleToken.INFT, PackPublic, MetadataViews.Resolver{ 
        ...
        pub fun withdrawMain(mainNftId: UInt64): @SoulMadeMain.NFT?{ 
            let mainWithdraw <- self.mainNft.remove(key: mainNftId)
            let mainWithdrawRef: &SoulMadeMain.NFT? = &mainWithdraw as &SoulMadeMain.NFT?
            emit WithdrawnMain(id: (mainWithdrawRef!).id, mainDetail: (mainWithdrawRef!).mainDetail)
            return <-mainWithdraw
        }
        
        pub fun withdrawComponent(componentNftId: UInt64): @SoulMadeComponent.NFT?{ 
            let componentWithdraw <- self.componentNft.remove(key: componentNftId)
            let componentWithdrawRef: &SoulMadeComponent.NFT? = &componentWithdraw as &SoulMadeComponent.NFT?
            emit WithdrawnComponent(id: (componentWithdrawRef!).id, componentDetail: (componentWithdrawRef!).componentDetail)
            return <-componentWithdraw
        }
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                case Type<MetadataViews.Display>():
                    return MetadataViews.Display(name: "SoulMadepack", description: "SoulMadePack Contains Main and Component NFTs", thumbnail: MetadataViews.IPFSFile(cid: self.packDetail.ipfsHash, path: nil))
                case Type<MetadataViews.Royalties>():
                    return MetadataViews.Royalties([MetadataViews.Royalty(recepient: getAccount(0x9a57dfe5c8ce609c).getCapability<&FlowToken.Vault{FungibleToken.Receiver}>(/public/flowTokenReceiver), cut: 0.00, description: "SoulMade Pack Royalties")])
                case Type<MetadataViews.ExternalURL>():
                    return MetadataViews.ExternalURL("www.soulmade.art")
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: SoulMadePack.CollectionStoragePath, publicPath: SoulMadePack.CollectionPublicPath, providerPath: /private/SoulMadePackCollection, publicCollection: Type<&Collection{CollectionPublic}>(), publicLinkedType: Type<&Collection{CollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&Collection{CollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Provider, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
    ...
    pub resource Collection: CollectionPublic, CollectionFreeClaim, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection{ 
        ...
        pub fun freeClaim(mainNftCollectionRef: &{SoulMadeMain.CollectionPublic}?, componentNftCollectionRef: &{SoulMadeComponent.CollectionPublic}?){ 
            pre{ 
                ((mainNftCollectionRef!).owner!).address == ((componentNftCollectionRef!).owner!).address:
                    "Main and Component NFTs must be owned by the same user"
                SoulMadePack.checkClaimed(address: ((mainNftCollectionRef!).owner!).address) == false:
    ...
    access(account) fun mintPack(scarcity: String, series: String, ipfsHash: String, mainNfts: @[SoulMadeMain.NFT], componentNfts: @[SoulMadeComponent.NFT]): @NFT{ 
        let packDetail = PackDetail(id: SoulMadePack.totalSupply, scarcity: scarcity, series: series, ipfsHash: ipfsHash)
