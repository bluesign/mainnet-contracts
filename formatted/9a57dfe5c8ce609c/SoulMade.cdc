...
pub contract SoulMade{ 
    ...
    pub resource Admin{ 
        pub fun mintComponent(
            ...
            let adminComponentsCollection =
                SoulMade.account.borrow<
                    &SoulMadeComponent.Collection{
        ...
        pub fun moveMainComponentToPack(
            scarcity: String,
            series: String,
            ipfsHash: String,
            mainNftIds: [
            ...
            let adminMainCollection =
                SoulMade.account.borrow<&SoulMadeMain.Collection>(
                    from: SoulMadeMain.CollectionStoragePath
                )!
            let adminComponentCollection =
                SoulMade.account.borrow<&SoulMadeComponent.Collection>(
                    from: SoulMadeComponent.CollectionStoragePath
                )!
            let adminPackCollection =
                SoulMade.account.borrow<
                    &SoulMadePack.Collection{NonFungibleToken.CollectionPublic}
                >(from: adminStoragePath)!
    ...
    pub fun getMainCollectionIds(address: Address): [UInt64]{ 
        let receiverRef =
            getAccount(address).getCapability<&{SoulMadeMain.CollectionPublic}>(
    ...
    pub fun getMainDetail(
        address: Address,
        mainNftId: UInt64
    ): SoulMadeMain.MainDetail{ 
        let receiverRef =
            getAccount(address).getCapability<&{SoulMadeMain.CollectionPublic}>(
                SoulMadeMain.CollectionPublicPath
            ).borrow()
    ...
    pub fun getComponentCollectionIds(address: Address): [UInt64]{ 
        let receiverRef =
            getAccount(address).getCapability<
                &{SoulMadeComponent.CollectionPublic}
            >(SoulMadeComponent.CollectionPublicPath).borrow()
            ?? panic("Could not borrow the receiver reference")
        return receiverRef.getIDs()
    }
    
    pub fun getComponentDetail(
        address: Address,
    ...
    pub fun getPackCollectionIds(address: Address): [UInt64]{ 
        let receiverRef =
            getAccount(address).getCapability<&{SoulMadePack.CollectionPublic}>(
    ...
    pub fun getPackDetail(
        address: Address,
        packNftId: UInt64
    ): SoulMadePack.PackDetail{ 
        let receiverRef =
            getAccount(address).getCapability<&{SoulMadePack.CollectionPublic}>(
                SoulMadePack.CollectionPublicPath
            ).borrow()
