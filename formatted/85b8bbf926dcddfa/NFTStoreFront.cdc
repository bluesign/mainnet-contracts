...
pub contract NFTStoreFront{ 
    ...
    pub resource Listing: ListingPublic{ 
        ...
        pub fun purchase(payment: @FungibleToken.Vault): @NonFungibleToken.NFT{ 
            ...
            emit ListingSold(listingResourceID: self.uuid, storefrontResourceID: self.details.storefrontID, storefrontAddress: self.owner?.address!, nftType: self.details.nftType, nftID: self.details.nftID, timestamp: getCurrentBlock().timestamp)
            return <-nft
        }
        ...
        destroy(){ 
            ...
            if !self.details.purchased{ 
                emit ListingRemoved(listingResourceID: self.uuid, storefrontResourceID: self.details.storefrontID)
            }
        }
    ...
    pub resource Storefront: StorefrontManager, StorefrontPublic{ 
        ...
        pub fun createListing(nftProviderCapability: Capability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>, nftType: Type, nftID: UInt64, salePaymentVaultType: Type, saleCuts: [SaleCut]): UInt64{ 
            ...
            destroy oldListing
            emit ListingAvailable(storefrontAddress: self.owner?.address!, storefrontResourceID: self.uuid, listingResourceID: listingResourceID, nftType: nftType, nftID: nftID, ftVaultType: salePaymentVaultType, price: listingPrice, timestamp: getCurrentBlock().timestamp)
        ...
        pub fun removeListing(listingResourceID: UInt64){ 
            ...
            destroy listing
            emit ListingDestroyed(listingResourceID: listingUUID, storefrontResourceID: self.uuid, storefrontAddress: self.owner?.address!, nftType: listingNftType, nftID: listingNftID, timestamp: getCurrentBlock().timestamp)
        }
        ...
        destroy(){ 
            ...
            emit StorefrontDestroyed(storefrontResourceID: self.uuid, timestamp: getCurrentBlock().timestamp)
        }
        
        init(){ 
            self.listings <-{} 
            emit StorefrontInitialized(storefrontResourceID: self.uuid, timestamp: getCurrentBlock().timestamp)
        }
    }
