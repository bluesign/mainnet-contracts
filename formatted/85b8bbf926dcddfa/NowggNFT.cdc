...
pub contract NowggNFT: NonFungibleToken{ 
    ...
    pub fun borrowNFT(from: Address, itemID: UInt64): &NowggNFT.NFT?{ 
        let collection = (getAccount(from).getCapability(NowggNFT.CollectionPublicPath)!).borrow<&NowggNFT.Collection{NowggNFT.NowggNFTCollectionPublic}>() ?? panic("Couldn't get collection")
    ...
    init(){ 
        ...
        self.account.link<&NowggNFT.Collection{NonFungibleToken.CollectionPublic, NowggNFT.NowggNFTCollectionPublic}>(self.CollectionPublicPath, target: self.CollectionStoragePath)
        ...
        self.account.link<&NowggNFT.NftTypeHelper{NowggNFT.NftTypeHelperPublic}>(self.NFTtypeHelperPublicPath, target: self.NftTypeHelperStoragePath)
