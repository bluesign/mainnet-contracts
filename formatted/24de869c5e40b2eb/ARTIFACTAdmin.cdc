...
pub contract ARTIFACTAdmin: Interfaces{ 
    ...
    pub resource Admin: Interfaces.ARTIFACTAdminOpener{ 
        ...
        pub fun openPack(userPack: &{Interfaces.IPack}, packID: UInt64, owner: Address, royalties: [MetadataViews.Royalty], packOption:{ Interfaces.IPackOption}?): @[NonFungibleToken.NFT]{ 
            pre{ 
                ...
                !ARTIFACTPack.checkPackTemplateLockStatus(packTemplateId: userPack.templateId):
                    "pack template is locked"
            }
        ...
        priv fun getTemplateIdsFromPacksAvailable(packTemplate: &ARTIFACTPack.PackTemplate): [UInt64]{ 
            pre{ 
        ...
        pub fun mintNFT(templateId: UInt64, packID: UInt64, owner: Address): @ARTIFACT.NFT{ 
            pre{ 
                ARTIFACT.templateDatas.containsKey(templateId):
                    "templateId not found"
                (ARTIFACT.templateDatas[templateId]!).maxEditions > ARTIFACT.numberMintedByTemplate[templateId]!:
        ...
        //
        priv fun mintFirstNFT(templateId: UInt64){ 
            let nft: @ARTIFACT.NFT <- self.mintNFT(templateId: templateId, packID: 0, owner: self.owner?.address!)
            let collectionRef = getAccount(self.owner?.address!).getCapability(ARTIFACT.collectionPublicPath).borrow<&{ARTIFACT.CollectionPublic}>() ?? panic("Could not get public collection reference")
    ...
    pub resource AdminTokenReceiver: AdminTokenReceiverPublic{ 
        priv var adminRef: Capability<&Admin>?
        
        priv var adminCapabilityOpener: Capability<&{Interfaces.ARTIFACTAdminOpener}>?
        
        priv var superAdminRef: @[SuperAdmin]
    ...
    init(){ 
        ...
        let adminTokenReceiver = self.account.borrow<&ARTIFACTAdmin.AdminTokenReceiver>(from: self.ARTIFACTAdminTokenReceiverStoragePath) ?? panic("Could not borrow user ARTIFACTAdmin admin token reference")
