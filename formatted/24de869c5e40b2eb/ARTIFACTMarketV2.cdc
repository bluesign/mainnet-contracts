...
pub contract ARTIFACTMarketV2{ 
    ...
    pub event ARTIFACTMarketListed(
        listingID: UInt64,
        price: UFix64,
        saleCuts:{ 
            Address: UFix64
    ...
    pub event ARTIFACTMarketPurchased(
        listingID: UInt64,
        packID: UInt64,
        owner: Address?,
        databaseID: String
    ...
    pub struct Listing{ 
        ...
        init(
            ...
            pre{ 
                salePrice >= 0.0:
                    "Listing must be not negative price"
                sellerCapability.borrow() != nil:
                    "Sellers's Receiver Capability is invalid!"
                ARTIFACTMarketV2.totalSaleCuts(saleCuts: saleCuts) <= 100.0:
                    "SaleCuts can't be greater a 100"
            }
            self.ID = ID
            self.nftID = nftID
    ...
    pub resource interface ManagerPublic{ 
        pub fun purchase(
            ...
            userPackCollection: &ARTIFACTPackV3.Collection{
                ARTIFACTPackV3.CollectionPublic
            },
            userCollection: &ARTIFACTV2.Collection{ARTIFACTV2.CollectionPublic},
            quantity: UInt64
        ){} 
        
        pub fun purchaseOnPreSale(
            listingID: UInt64,
            buyTokens: &FungibleToken.Vault,
            databaseID: String,
            owner: Address,
    ...
    pub resource Manager: ManagerPublic, MarketManager{ 
        ...
        priv var superAdminTokenReceiver: Capability<&ARTIFACTAdminV2.AdminTokenReceiver>
        
        /// A collection of the nfts that the user has for sale 
        priv var ownerCollection: Capability<&ARTIFACTV2.Collection>
        ...
        pub fun createListing(nftID: UInt64?, packTemplateID: UInt64?, salePrice: UFix64, saleCuts: [SaleCut], salePaymentVaultType: Type, sellerCapability: Capability<&{FungibleToken.Receiver}>, databaseID: String, whitelist: [Address], isPreSale: Bool, preSaleBuyerMaxLimit:{ Address: UInt64}, preSalePrice: UFix64, nftSaleCuts: [SaleCut]){ 
            pre{ 
                salePrice >= 0.0:
                    "must not be negative price"
                preSalePrice >= 0.0:
                    "must have a not negative preSalePrice "
                nftID != nil || packTemplateID != nil:
                    "must pass one template ID"
                self.getNumberValue(value: nftID != nil) + self.getNumberValue(value: packTemplateID != nil) == 1:
                    "must pass only one template ID"
            }
            let listingID = ARTIFACTMarketV2.nextListingID
            ARTIFACTMarketV2.nextListingID = listingID + 1
            let listing = Listing(ID: listingID, nftID: nftID, packTemplateID: packTemplateID, salePrice: salePrice, saleCuts: saleCuts, salePaymentVaultType: salePaymentVaultType, sellerCapability: sellerCapability, whitelist: whitelist, isPreSale: isPreSale, preSaleBuyerMaxLimit: preSaleBuyerMaxLimit, preSalePrice: preSalePrice, nftSaleCuts: nftSaleCuts)
        ...
        priv fun purchaseListing(listingID: UInt64, buyTokens: &FungibleToken.Vault, databaseID: String, owner: Address, userPackCollection: &ARTIFACTPackV3.Collection{ARTIFACTPackV3.CollectionPublic}, userCollection: &ARTIFACTV2.Collection{ARTIFACTV2.CollectionPublic}, quantity: UInt64){ 
            pre{ 
                quantity <= 200:
                    "Max quantity is 200"
        ...
        priv fun getTemplateInformation(listing: Listing): AnyStruct{ 
            if listing.packTemplateID != nil{ 
        ...
        priv fun checkBuyerMaxLimit(address: Address, listingID: UInt64, quantity: UInt64): Bool{ 
            let listing = self.listings[listingID]!
        ...
        priv fun getNFT(nft: AnyStruct, owner: Address, listing: Listing, quantity: UInt64): @NonFungibleToken.NFT{ 
            if listing.packTemplateID != nil{ 
        ...
        // Parameters: listing: The listing struct
        //
        priv fun removeListingByGenericNFT(nft: AnyStruct, listing: Listing){ 
            if listing.packTemplateID != nil{ 
                let packTemplate = nft as! ARTIFACTPackV3.PackTemplate
        ...
        pub fun updateWhitelist(listingResourceID: UInt64, whitelist: [Address], preSaleBuyerMaxLimit:{ Address: UInt64}){ 
            ...
            let listing = (&self.listings[listingResourceID] as                                                                
                                                                // -----------------------------------------------------------------------
    ...
    pub fun createManager(
        superAdminTokenReceiver: Capability<
            &ARTIFACTAdminV2.AdminTokenReceiver
        >,
    ...
    init(){ 
        ...
        if self.account.borrow<&{ARTIFACTPackV3.CollectionPublic}>(
            from: ARTIFACTPackV3.collectionStoragePath
        )
        == nil{ 
            let collection <-
                ARTIFACTPackV3.createEmptyCollection()
                as!
                @ARTIFACTPackV3.Collection
            self.account.save<@ARTIFACTPackV3.Collection>(
                <-collection,
                to: ARTIFACTPackV3.collectionStoragePath
            )
            self.account.link<&{ARTIFACTPackV3.CollectionPublic}>(
                ARTIFACTPackV3.collectionPublicPath,
                target: ARTIFACTPackV3.collectionStoragePath
            )
        }
        if self.account.borrow<&{ARTIFACTMarketV2.ManagerPublic}>(
            from: ARTIFACTMarketV2.marketStoragePath
        )
        == nil{ 
            let ownerCollection =
                self.account.link<&ARTIFACTV2.Collection>(
                    ARTIFACTV2.collectionPrivatePath,
                    target: ARTIFACTV2.collectionStoragePath
                )!
            let superAdminTokenReceiverCapability =
                self.account.link<&ARTIFACTAdminV2.AdminTokenReceiver>(
                    ARTIFACTAdminV2.ARTIFACTAdminTokenReceiverPrivatePath,
                    target: ARTIFACTAdminV2
                        .ARTIFACTAdminTokenReceiverStoragePath
                )!
            self.account.save(
                <-create ARTIFACTMarketV2.Manager(
                    superAdminTokenReceiver: superAdminTokenReceiverCapability,
                    ownerCollection: ownerCollection
                ),
                to: ARTIFACTMarketV2.marketStoragePath
            )
            self.account.link<
                &ARTIFACTMarketV2.Manager{ARTIFACTMarketV2.ManagerPublic}
            >(
                ARTIFACTMarketV2.marketPublicPath,
                target: ARTIFACTMarketV2.marketStoragePath
            )
        }
    }
}
