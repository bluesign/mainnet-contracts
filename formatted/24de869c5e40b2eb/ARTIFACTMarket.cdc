...
pub contract ARTIFACTMarket{ 
    ...
    pub resource interface ManagerPublic{ 
        pub fun purchase(
            ...
            userCollection: &ARTIFACT.Collection{ARTIFACT.CollectionPublic},
            quantity: UInt64
        ){} 
    ...
    pub resource Manager: ManagerPublic, MarketManager{ 
        ...
        priv var superAdminTokenReceiver: Capability<&ARTIFACTAdmin.AdminTokenReceiver>
        
        /// A collection of the nfts that the user has for sale 
        priv var ownerCollection: Capability<&ARTIFACT.Collection>
        ...
        priv fun purchaseListing(listingID: UInt64, buyTokens: &FungibleToken.Vault, databaseID: String, owner: Address, userPackCollection: &ARTIFACTPack.Collection{ARTIFACTPack.CollectionPublic}, userCollection: &ARTIFACT.Collection{ARTIFACT.CollectionPublic}, quantity: UInt64){ 
            pre{ 
                quantity <= 6:
                    "Max quantity is 6"
    ...
    init(){ 
        ...
        if self.account.borrow<&{ARTIFACTPack.CollectionPublic}>(
            from: ARTIFACTPack.collectionStoragePath
        )
        == nil{ 
            let collection <-
                ARTIFACTPack.createEmptyCollection()
                as!
                @ARTIFACTPack.Collection
            self.account.save<@ARTIFACTPack.Collection>(
                <-collection,
                to: ARTIFACTPack.collectionStoragePath
            )
            self.account.link<&{ARTIFACTPack.CollectionPublic}>(
                ARTIFACTPack.collectionPublicPath,
                target: ARTIFACTPack.collectionStoragePath
            )
        }
        if self.account.borrow<&{ARTIFACTMarket.ManagerPublic}>(
            from: ARTIFACTMarket.marketStoragePath
        )
        == nil{ 
            let ownerCollection =
                self.account.link<&ARTIFACT.Collection>(
                    ARTIFACT.collectionPrivatePath,
                    target: ARTIFACT.collectionStoragePath
                )!
            let superAdminTokenReceiverCapability =
                self.account.link<&ARTIFACTAdmin.AdminTokenReceiver>(
                    ARTIFACTAdmin.ARTIFACTAdminTokenReceiverPrivatePath,
                    target: ARTIFACTAdmin.ARTIFACTAdminTokenReceiverStoragePath
                )!
            self.account.save(
            ...
            self.account.link<
                &ARTIFACTMarket.Manager{ARTIFACTMarket.ManagerPublic}
            >(
                ARTIFACTMarket.marketPublicPath,
                target: ARTIFACTMarket.marketStoragePath
            )
        }
    }
}
