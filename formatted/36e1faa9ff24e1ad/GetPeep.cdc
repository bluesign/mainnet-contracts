...
pub contract GetPeep: NonFungibleToken{ 
    ...
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver, NiftoryNonFungibleToken.NFTPublic{ 
        ...
        init(setId: Int, templateId: Int, serial: UInt64){ 
            self.id = GetPeep.totalSupply
            GetPeep.totalSupply = GetPeep.totalSupply + 1
            self.setId = setId
    ...
    pub resource Manager: NiftoryNonFungibleToken.ManagerPublic, NiftoryNonFungibleToken.ManagerPrivate{ 
        ...
        pub fun getNFTCollectionData(): MetadataViews.NFTCollectionData{ 
            return NiftoryNFTRegistry.buildNFTCollectionData(GetPeep.REGISTRY_ADDRESS, GetPeep.REGISTRY_BRAND, fun (): @NonFungibleToken.Collection{ 
                    return <-GetPeep.createEmptyCollection()
                })
        }
        ...
        pub fun modifyContractMetadata(): auth &AnyStruct{ 
            return &GetPeep.metadata as auth &AnyStruct
        }
        ...
        priv fun _getMetadataViewsManagerPrivate(): &MetadataViewsManager.Manager{MetadataViewsManager.Private}{ 
            let record = NiftoryNFTRegistry.getRegistryRecord(GetPeep.REGISTRY_ADDRESS, GetPeep.REGISTRY_BRAND)
            let manager = GetPeep.account.getCapability<&MetadataViewsManager.Manager{MetadataViewsManager.Private}>(record.metadataViewsManager.paths.private).borrow()!
            return manager
        }
        ...
        priv fun _getSetManagerPrivate(): &MutableMetadataSetManager.Manager{MutableMetadataSetManager.Public, MutableMetadataSetManager.Private}{ 
            let record = NiftoryNFTRegistry.getRegistryRecord(GetPeep.REGISTRY_ADDRESS, GetPeep.REGISTRY_BRAND)
            let setManager = GetPeep.account.getCapability<&MutableMetadataSetManager.Manager{MutableMetadataSetManager.Public, MutableMetadataSetManager.Private}>(record.setManager.paths.private).borrow()!
            return setManager
        }
    ...
    init(){ 
        let record = NiftoryNFTRegistry.generateRecord(account: self.account.address, project: "cl9gyoa8300010hl8rxl1751p_GetPeep")
        self.REGISTRY_ADDRESS = 0x32d62d5c43ad1038
        self.REGISTRY_BRAND = "cl9gyoa8300010hl8rxl1751p_GetPeep"
        ...
        self.account.save<@MutableMetadataSetManager.Manager>(<-MutableMetadataSetManager.create(name: "GetPeep", description: "The set manager for GetPeep."), to: record.setManager.paths.storage)
