...
import SwapError from "../0x5f4da03554851654/SwapError.cdc"
...
pub contract SwapPair: FungibleToken{ 
    ...
    priv let token0Vault: @FungibleToken.Vault
    
    priv let token1Vault: @FungibleToken.Vault
    ...
    /// Burn lpTokens
    priv fun burnLpToken(from: @SwapPair.Vault){ 
    ...
    pub fun addLiquidity(tokenAVault: @FungibleToken.Vault, tokenBVault: @FungibleToken.Vault): @FungibleToken.Vault{ 
        ...
        if self.totalSupply == 0.0{ 
            ...
            let initialLpAmount = SwapConfig.ScaledUInt256ToUFix64(SwapConfig.sqrt(SwapConfig.UFix64ToScaledUInt256(self.token0Vault.balance) * SwapConfig.UFix64ToScaledUInt256(self.token1Vault.balance) / SwapConfig.scaleFactor))
        ...
        if feeOn{ 
            self.rootKLast = SwapConfig.ScaledUInt256ToUFix64(SwapConfig.sqrt(SwapConfig.UFix64ToScaledUInt256(self.token0Vault.balance) * SwapConfig.UFix64ToScaledUInt256(self.token1Vault.balance) / SwapConfig.scaleFactor))
        }
        self.lock = false
        return <-lpTokenVault
    }
    ...
    ///
    priv fun _update(reserve0LastScaled: UInt256, reserve1LastScaled: UInt256){ 
        ...
        if timeElapsed > 0.0 && reserve0LastScaled != 0 && reserve1LastScaled != 0{ 
            ...
            self.price0CumulativeLastScaled = self.price0CumulativeLastScaled + reserve1LastScaled * timeElapsedScaled / reserve0LastScaled
            self.price1CumulativeLastScaled = self.price1CumulativeLastScaled + reserve0LastScaled * timeElapsedScaled / reserve1LastScaled
        }
    ...
    ///
    priv fun _mintFee(reserve0: UFix64, reserve1: UFix64): Bool{ 
        ...
        if rootKLast > 0.0{ 
            let rootK = SwapConfig.ScaledUInt256ToUFix64(SwapConfig.sqrt(SwapConfig.UFix64ToScaledUInt256(reserve0) * SwapConfig.UFix64ToScaledUInt256(reserve1) / SwapConfig.scaleFactor))
            if rootK > rootKLast{ 
