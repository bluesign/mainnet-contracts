...
pub contract LockedMoxyToken{ 
    ...
    pub resource ConversionRequest{ 
        ...
        access(account) fun withdraw(): @MoxyToken.Vault{ 
            let vault <-
                self.vault.withdraw(amount: self.vault.balance)
                as!
                @MoxyToken.Vault
            LockedMoxyToken.totalSupply = LockedMoxyToken.totalSupply
                - vault.balance
            return <-vault
        }
    ...
    pub resource interface Receiver{ 
        ...
        pub fun depositFromSchedule(
            from: @MoxyToken.Vault,
            schedule:{ 
                UFix64: UFix64
            }
        ){} 
        
        pub fun depositFromFixedSchedule(
            from: @MoxyToken.Vault,
