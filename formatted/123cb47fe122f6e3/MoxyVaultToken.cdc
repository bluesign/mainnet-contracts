...
pub contract MoxyVaultToken: FungibleToken{ 
    ...
    init(){ 
        ...
        self.account.link<&MoxyVaultToken.Vault>(self.moxyVaultTokenVaultPrivate, target: self.moxyVaultTokenVaultStorage)
        
        // Create a public capability to the stored Vault that only exposes
        // the `deposit` method through the `Receiver` interface
        //
        self.account.link<&{FungibleToken.Receiver}>(self.moxyVaultTokenReceiverPath, target: self.moxyVaultTokenVaultStorage)
        // Link to receive tokens in a specific timestamp
        self.account.link<&{MoxyVaultToken.ReceiverInterface}>(self.moxyVaultTokenReceiverTimestampPath, target: self.moxyVaultTokenVaultStorage)
        
        // Create a public capability to the stored Vault that only exposes
        // the `balance` field through the `Balance` interface
        //
        self.account.link<&MoxyVaultToken.Vault{FungibleToken.Balance}>(self.moxyVaultTokenBalancePath, target: self.moxyVaultTokenVaultStorage)
        self.account.link<&MoxyVaultToken.Vault{DailyBalancesInterface}>(self.moxyVaultTokenDailyBalancePath, target: self.moxyVaultTokenVaultStorage)
