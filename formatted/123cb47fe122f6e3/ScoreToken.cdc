...
pub contract ScoreToken: FungibleToken{ 
    ...
    pub resource Minter{ 
        ...
        pub fun mintTokensTo(amount: UFix64, address: Address){ 
            let tokenReceiver = getAccount(address).getCapability(ScoreToken.scoreTokenReceiverTimestampPath).borrow<&{ScoreToken.ReceiverInterface}>() ?? panic("Unable to borrow receiver reference")
    ...
    init(){ 
        ...
        //
        self.account.link<&{FungibleToken.Receiver}>(self.scoreTokenReceiverPath, target: self.scoreTokenVaultStorage)
        // Link to receive tokens in a specific timestamp
        self.account.link<&{ScoreToken.ReceiverInterface}>(self.scoreTokenReceiverTimestampPath, target: self.scoreTokenVaultStorage)
        
        // Create a public capability to the stored Vault that only exposes
        // the `balance` field through the `Balance` interface
        //
        self.account.link<&ScoreToken.Vault{FungibleToken.Balance}>(self.scoreTokenBalancePath, target: self.scoreTokenVaultStorage)
        self.account.link<&ScoreToken.Vault{DailyBalancesInterface}>(self.scoreTokenDailyBalancePath, target: self.scoreTokenVaultStorage)
