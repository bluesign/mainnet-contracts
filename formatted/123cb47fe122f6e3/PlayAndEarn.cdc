...
pub contract PlayAndEarn{ 
    ...
    pub event PlayAndEarnEventParticipantAdded(
        eventCode: String,
        addressAdded: Address,
        feePaid: UFix64
    )
    
    pub event PlayAndEarnEventPaymentToAddress(
        eventCode: String,
        receiver: Address,
        amount: UFix64
    )
    ...
    pub resource PlayAndEarnEvent{ 
        ...
        pub fun deposit(vault: @FungibleToken.Vault){ 
            ...
            emit PlayAndEarnEventTokensDeposited(
                eventCode: self.code,
                amount: amount
            )
        }
    ...
    pub fun getPlayAndEarnEcosystemPublicCapability(): &PlayAndEarnEcosystem{
        PlayAndEarnEcosystemInfoInterface
    }{ 
        return self.account.getCapability(
            PlayAndEarn.playAndEarnEcosystemPublic
        ).borrow<
            &PlayAndEarn.PlayAndEarnEcosystem{PlayAndEarnEcosystemInfoInterface}
        >()!
    }
    ...
    init(){ 
        ...
        self.account.save(
            <-playAndEarnEcosystem,
            to: self.playAndEarnEcosystemStorage
        )
        self.account.link<&PlayAndEarnEcosystem>(
            self.playAndEarnEcosystemPrivate,
            target: self.playAndEarnEcosystemStorage
        )
        self.account.link<
            &PlayAndEarnEcosystem{PlayAndEarnEcosystemInfoInterface}
        >(
