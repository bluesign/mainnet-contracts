...
pub contract MoxyToken: FungibleToken{ 
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance{ 
        ...
        access(account) fun depositLocked(from: @FungibleToken.Vault){ 
    ...
    init(){ 
        ...
        //
        self.account.link<&{FungibleToken.Receiver}>(self.moxyTokenReceiverPath, target: self.moxyTokenVaultStorage)
        
        // Create a public capability to the stored Vault that only exposes
        // the `balance` field through the `Balance` interface
        //
        self.account.link<&MoxyToken.Vault{FungibleToken.Balance}>(self.moxyTokenBalancePath, target: self.moxyTokenVaultStorage)
        ...
        self.account.link<&FungibleToken.Vault>(self.moxyTokenPlayAndEarnVaultPrivate, target: self.moxyTokenPlayAndEarnVaultStorage)
        self.account.link<&FungibleToken.Vault{FungibleToken.Balance}>(self.moxyTokenPlayAndEarnBalancePath, target: self.moxyTokenPlayAndEarnVaultStorage)
