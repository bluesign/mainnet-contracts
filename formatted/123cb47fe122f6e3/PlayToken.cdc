...
pub contract PlayToken: FungibleToken{ 
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, DailyBalancesInterface, ReceiverInterface{ 
        ...
        pub fun convertedFromMOXY(from: @FungibleToken.Vault){ 
    ...
    pub let playTokenReceiverInterfacePath: PublicPath
    ...
    init(){ 
        ...
        //
        self.account.link<&{FungibleToken.Receiver}>(self.playTokenReceiverPath, target: self.playTokenVaultStorage)
        self.account.link<&{PlayToken.ReceiverInterface}>(self.playTokenReceiverInterfacePath, target: self.playTokenVaultStorage)
        
        // Create a public capability to the stored Vault that only exposes
        // the `balance` field through the `Balance` interface
        //
        self.account.link<&PlayToken.Vault{FungibleToken.Balance}>(self.playTokenBalancePath, target: self.playTokenVaultStorage)
        self.account.link<&PlayToken.Vault{FungibleToken.Balance}>(self.playTokenDailyBalancePath, target: self.playTokenVaultStorage)
