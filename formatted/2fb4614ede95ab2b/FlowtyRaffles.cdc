...
pub contract FlowtyRaffles{ 
    ...
    pub event RaffleReceiptCommitted(
        address: Address?,
        raffleID: UInt64,
        receiptID: UInt64,
        commitBlock: UInt64
    ...
    pub event RaffleReceiptRevealed(
        address: Address?,
        raffleID: UInt64,
        receiptID: UInt64,
        commitBlock: UInt64,
    ...
    pub resource Receipt{ 
        ...
        access(contract) fun reveal(
            _ source: &{RaffleSourcePublic, RaffleSourcePrivate}
        ): DrawingResult{ 
            pre{ 
            ...
            let rand =
                FlowtyRaffles.randUInt64(
                    atBlockHeight: self.commitBlock,
                    salt: self.uuid
                )
        ...
        init(
            commitBlock: UInt64,
            source: &{RaffleSourcePublic, RaffleSourcePrivate}
        ){ 
    ...
    pub resource interface ManagerPublic{ 
        ...
        pub fun revealDrawing(
            manager: &Manager{ManagerPublic},
            raffleID: UInt64,
            receiptID: UInt64
        ): DrawingResult{} 
        
        access(contract) fun _revealDrawing(
            raffleID: UInt64,
            receiptID: UInt64,
            drawer: &Manager
        ): DrawingResult{} 
    ...
    init(){ 
        let identifier =
            "FlowtyRaffles_".concat(self.account.address.toString())
