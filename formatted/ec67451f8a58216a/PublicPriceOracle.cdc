...
pub contract PublicPriceOracle{ 
    ...
    priv let oracleAddrToPriceIdentifier:{ Address: String}
    ...
    pub fun getLatestPrice(oracleAddr: Address): UFix64{ 
        ...
        let priceReaderRef =
            PublicPriceOracle.account.borrow<&OracleInterface.PriceReader>(
    ...
    pub resource Admin{ 
        pub fun addOracle(oracleAddr: Address){ 
            if !PublicPriceOracle.oracleAddrToPriceIdentifier.containsKey(
                oracleAddr
            ){ 
                /// Mint oracle reader
                let oraclePublicInterface_ReaderRef =
                ...
                let priceReaderSuggestedPath =
                    oraclePublicInterface_ReaderRef.getPriceReaderStoragePath()
                if PublicPriceOracle.account.borrow<
                    &OracleInterface.PriceReader
                >(from: priceReaderSuggestedPath)
                == nil{ 
                    let priceReader <-
                        oraclePublicInterface_ReaderRef.mintPriceReader()
                    destroy <-PublicPriceOracle.account.load<@AnyResource>(
                        from: priceReaderSuggestedPath
                    )
                    PublicPriceOracle.oracleAddrToPriceIdentifier[
                        oracleAddr
                    ] = priceReader.getPriceIdentifier()
                    PublicPriceOracle.account.save(
                        <-priceReader,
                        to: priceReaderSuggestedPath
                    )
                }
        ...
        pub fun removeOracle(oracleAddr: Address){ 
            PublicPriceOracle.oracleAddrToPriceIdentifier.remove(
                key: oracleAddr
            )
