...
import
    ...
    from "../0x8d0e87b65159ae63/FlowStakingCollection.cdc"
...
pub contract sFlowStakingManager{ 
    ...
    pub fun getCurrentPoolAmount(): UFix64{ 
        let vaultRef =
            self.account.getCapability(/public/flowTokenBalance).borrow<
    ...
    pub fun getCurrentPrice(): UFix64{ 
        ...
        if amountInPool + amountInStaking == 0.0
    ...
    pub fun stake(from: @FungibleToken.Vault): @sFlowToken.Vault{ 
        ...
        let managerFlowVault =
            self.account.borrow<&FlowToken.Vault>(from: /storage/flowTokenVault)
            ?? panic("Could not borrow Manager's Flow Vault")
        managerFlowVault.deposit(from: <-vault)
        let managerMinterVault =
            self.account.borrow<&sFlowToken.Minter>(
                from: /storage/sFlowTokenMinter
            )
    ...
    pub fun unstake(accountAddress: Address, from: @FungibleToken.Vault){ 
        self.unstakeList.append({
            "address": accountAddress, "amount": from.balance}
        )
        let managersFlowTokenVault =
            self.account.borrow<&sFlowToken.Vault>(
                from: /storage/sFlowTokenVault
            )
            ?? panic("Could not borrow Manager's Minter Vault")
    ...
    pub resource Instance: InstanceInterface{ 
        priv var managerCapability: Capability<&Manager>?
    ...
    pub resource Manager{ 
        ...
        pub fun setNewDelegator(nodeID: String, delegatorID: UInt32){ 
            ...
            sFlowStakingManager.prevDelegatorID = sFlowStakingManager
                    .delegatorID
        ...
        pub fun registerNewDelegator(id: String, amount: UFix64){ 
            let stakingCollectionRef: &FlowStakingCollection.StakingCollection =
                sFlowStakingManager.account.borrow<
        ...
        pub fun unstakeAll(nodeId: String, delegatorId: UInt32){ 
            ...
            for info in delegatingInfo{ 
                if info.nodeID == nodeId && info.id == delegatorId{ 
                    let stakingCollectionRef: &FlowStakingCollection.StakingCollection = sFlowStakingManager.account.borrow<&FlowStakingCollection.StakingCollection>(from: FlowStakingCollection.StakingCollectionStoragePath) ?? panic("Could not borrow ref to StakingCollection")
    ...
    pub fun manageCollection(){ 
        ...
        for unstakeTicket in sFlowStakingManager.unstakeList{ 
            ...
            if sFlowStakingManager.getCurrentPoolAmount() > requiredFlow + sFlowStakingManager.minimumPoolTaking{ 
                let providerRef = sFlowStakingManager.account.borrow<&FlowToken.Vault>(from: /storage/flowTokenVault) ?? panic("Could not borrow provider reference to the provider's Vault")
                ...
                let receiverRef = accountStaker.getCapability(/public/flowTokenReceiver).borrow<&{FungibleToken.Receiver}>() ?? panic("Could not borrow receiver reference to the recipient's Vault")
                receiverRef.deposit(from: <-sentVault)
                let managersFlowTokenVault = sFlowStakingManager.account.borrow<&sFlowToken.Vault>(from: /storage/sFlowTokenVault) ?? panic("Could not borrow provider reference to the provider's Vault")
                
                // Deposit the withdrawn tokens in the provider's receiver
                let burningVault: @FungibleToken.Vault <- managersFlowTokenVault.withdraw(amount: amount)
                let managersFlowTokenBurnerVault = sFlowStakingManager.account.borrow<&sFlowToken.Burner>(from: /storage/sFlowTokenBurner) ?? panic("Could not borrow provider reference to the provider's Vault")
        ...
        log("required stake amount: ")
        log(requiredStakedAmount)
        if requiredStakedAmount == 0.0 && bStakeNew
        && FlowIDTableStaking.stakingEnabled(){ 
            ...
            let stakingCollectionRef: &FlowStakingCollection.StakingCollection =
                ...
                >(from: FlowStakingCollection.StakingCollectionStoragePath)
                ?? panic("Could not borrow ref to StakingCollection")
            stakingCollectionRef.stakeUnstakedTokens(
                nodeID: sFlowStakingManager.nodeID,
                delegatorID: sFlowStakingManager.delegatorID,
                amount: delegatingInfo.tokensUnstaked
            )
            ...
            if sFlowStakingManager.getCurrentPoolAmount()
            > sFlowStakingManager.minimumPoolTaking{ 
                stakingCollectionRef.stakeNewTokens(
                    nodeID: sFlowStakingManager.nodeID,
                    delegatorID: sFlowStakingManager.delegatorID,
                    amount: sFlowStakingManager.getCurrentPoolAmount()
                    - sFlowStakingManager.minimumPoolTaking
                    - accountStorageAmount
                )
            }
        }
        ...
        && FlowIDTableStaking.stakingEnabled(){ 
            ...
            || delegatingInfo.tokensStaked > 0.0{ 
                stakingCollectionRef.requestUnstaking(
                    nodeID: sFlowStakingManager.prevNodeID,
                    delegatorID: sFlowStakingManager.prevDelegatorID,
                    amount: delegatingInfo.tokensCommitted
