...
pub contract sFlowStakingManagerV2{ 
    ...
    pub fun getsFlowPrice(): UFix64{ 
        ...
        return (undelegatedFlowBalance + delegatedFlowBalance)
        / sFlowToken.totalSupply
    }
    ...
    access(contract) fun borrowStakingCollection(): &FlowStakingCollection.StakingCollection{ 
        let stakingCollection =
            self.account.borrow<&FlowStakingCollection.StakingCollection>(
    ...
    access(contract) fun borrowsFlowBurner(): &sFlowToken.Burner{ 
        let sFlowBurner =
            self.account.borrow<&sFlowToken.Burner>(
                from: /storage/sFlowTokenBurner
            )
            ?? panic(
                "Could not borrow provider reference to the provider's Burner"
            )
        return sFlowBurner
    }
    ...
    pub resource Manager{ 
        ...
        pub fun setNewDelegator(nodeID: String, delegatorID: UInt32){ 
            ...
            sFlowStakingManagerV2.prevDelegatorID = sFlowStakingManagerV2
                    .delegatorID
        ...
        pub fun registerNewDelegator(id: String, amount: UFix64){ 
            let stakingCollectionRef: &FlowStakingCollection.StakingCollection =
                sFlowStakingManagerV2.account.borrow<
        ...
        pub fun unstakeAll(nodeId: String, delegatorId: UInt32){ 
            let delegatingInfo =
                FlowStakingCollection.getAllDelegatorInfo(
                    address: sFlowStakingManagerV2.account.address
                )
            ...
            for info in delegatingInfo{ 
                if info.nodeID == nodeId && info.id == delegatorId{ 
                    let stakingCollectionRef: &FlowStakingCollection.StakingCollection = sFlowStakingManagerV2.account.borrow<&FlowStakingCollection.StakingCollection>(from: FlowStakingCollection.StakingCollectionStoragePath) ?? panic("Could not borrow ref to StakingCollection")
    ...
    init(nodeID: String, delegatorID: UInt32){ 
        ...
        self.account.save(
            <-create Manager(),
            to: /storage/sFlowStakingManagerV2
        )
