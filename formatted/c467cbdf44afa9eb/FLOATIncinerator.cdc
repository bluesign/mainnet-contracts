...
pub contract FLOATIncinerator{ 
    ...
    pub resource Incinerator: IncineratorPublic{ 
        ...
        pub fun burn(collection: &MusicPeaksAttendanceToken.Collection, ids: [UInt64]){ 
            let length = ids.length
            for id in ids{ 
                let float: &MusicPeaksAttendanceToken.NFT = collection.borrowFLOAT(id: id) ?? panic("This FLOAT does not exist.")
    ...
    init(){ 
        self
            .IncineratorStoragePath = /storage/MusicPeaksAttendanceTokenIncineratorStoragePath
        self
            .IncineratorPublicPath = /public/MusicPeaksAttendanceTokenIncineratorPublicPath
