...
pub contract EverSinceNFT: NonFungibleToken{ 
            ...
            switch view{ 
                case Type<MetadataViews.Display>():
                    if self.metadata["bonus"] != "0"{ 
                        return MetadataViews.Display(name: name.concat(" #").concat(self.id.toString()), description: description, thumbnail: MetadataViews.HTTPFile(url: self.metadata["uri"]!))
                    } else{ 
                        return MetadataViews.Display(name: name.concat(" #").concat(self.id.toString()), description: description, thumbnail: MetadataViews.HTTPFile(url: self.metadata["usedUri"]!))
                    }
            }
            return nil
        }
    ...
    pub resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection, EverSinceNFTCollectionPublic{ 
    ...
    init(){ 
        ...
        self.account.link<&EverSinceNFT.Collection{NonFungibleToken.CollectionPublic, EverSinceNFT.EverSinceNFTCollectionPublic, MetadataViews.ResolverCollection}>(self.CollectionPublicPath, target: self.CollectionStoragePath)
        
        // store a minter resource in account storage
        self.account.save(<-create NFTMinter(), to: self.MinterStoragePath)
        self.account.link<&EverSinceNFT.NFTMinter{EverSinceNFT.EverSinceNFTMinterPublic}>(self.MinterPublicPath, target: self.MinterStoragePath)
