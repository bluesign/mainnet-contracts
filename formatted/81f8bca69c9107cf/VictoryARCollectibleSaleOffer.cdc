...
/*
    ...
    in order to list and sell VictoryARCollectible.
...
pub contract VictoryARCollectibleSaleOffer{ 
    // SaleOffer events.
    //
    ...
    pub event CollectionPriceRaised(
        owner: Address,
        bundleID: UInt64,
        price: UFix64,
        bidder: Address
    ...
    // SaleOffer
    // A bundle of VictoryARCollectible NFTs being offered to sale for a set fee paid in Flow.
    //
    pub resource SaleOffer: SaleOfferPublicView{ 
        ...
        // The VictoryARCollectible NFT IDs for sale.
        priv let itemIDs: [UInt64]
        
        // The collection containing the IDs.
        priv let sellerItemProvider: Capability<&VictoryARCollectible.Collection{NonFungibleToken.Provider}>
        
        // The Flow vault that will receive the payment if the sale completes successfully.
        priv let sellerPaymentReceiver: Capability<&{FungibleToken.Receiver}>
        
        // The Flow vault that will receive royalty from the payment if the sale completes successfully.
        priv let royaltyPaymentReceiver: Capability<&{FungibleToken.Receiver}>
        
        // Called by a purchaser to accept the sale offer.
        // If they send the correct payment, and if the item is still available,
        // the VictoryARCollectible NFT will be placed in their VictoryARCollectible.Collection.
        //
        ...
        //
        pub fun accept(buyerCollection: &VictoryARCollectible.Collection{NonFungibleToken.Receiver}, buyerPayment: @FungibleToken.Vault, ownerPaymentReceiver: Capability<&{FungibleToken.Receiver}>){ 
            pre{ 
        ...
        //
        pub fun raisePrice(newPrice: UFix64, bidder: Address, bidderReceiver: Capability<&{FungibleToken.Receiver}>, bidVault: @FungibleToken.Vault){ 
            pre{ 
                self.saleCompleted == false:
                    "the sale offer has already been accepted"
        ...
        // to transfer the VictoryARCollectible NFT and the capability to receive Flow in payment.
        //
        init(sellerItemProvider: Capability<&VictoryARCollectible.Collection{NonFungibleToken.Provider, VictoryARCollectible.VictoryARCollectibleCollectionPublic}>, bundleID: UInt64, sellerPaymentReceiver: Capability<&{FungibleToken.Receiver}>, price: UFix64, saleType: UInt8, startTime: UInt32, endTime: UInt32, targetPrice: UFix64, royaltyPaymentReceiver: Capability<&{FungibleToken.Receiver}>, royaltyFactor: UFix64){ 
            pre{ 
    ...
    pub resource interface CollectionManager{ 
        pub fun createSaleOffer(
            sellerItemProvider: Capability<
                &VictoryARCollectible.Collection{
                    NonFungibleToken.Provider,
                    VictoryARCollectible.VictoryARCollectibleCollectionPublic
                }
            >,
            bundleID: UInt64,
        ...
        pub fun insert(offer: @VictoryARCollectibleSaleOffer.SaleOffer){} 
    ...
    pub resource interface CollectionPurchaser{ 
        pub fun purchase(
            bundleID: UInt64,
            buyerCollection: &VictoryARCollectible.Collection{
    ...
    pub resource interface CollectionPublic{ 
        ...
        pub fun borrowSaleItem(bundleID: UInt64): &SaleOffer{
            SaleOfferPublicView
        }?{} 
    ...
    pub resource Collection:
        ...
        priv var saleOffers: @{UInt64: SaleOffer}
        ...
        pub fun remove(bundleID: UInt64): @SaleOffer{ 
            ...
            return <-(
                self.saleOffers.remove(key: bundleID)
        ...
        //   2. VictoryARCollectible.Withdraw
        //   3. VictoryARCollectible.Deposit
        ...
        pub fun purchase(
            ...
            offer.accept(
                ...
                ownerPaymentReceiver: ownerPaymentReceiver
            )
            destroy offer
        }
    ...
    init(){ 
        self
            .CollectionStoragePath = /storage/VictoryARCollectibleSaleOfferCollection
        self
            .CollectionPublicPath = /public/VictoryARCollectibleSaleOfferCollection
    }
}
