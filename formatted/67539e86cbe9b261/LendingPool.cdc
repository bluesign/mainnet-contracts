...
pub contract LendingPool{ 
    ...
    priv let accountLpTokens:{ Address: UInt256}
    ...
    priv let accountBorrows:{ Address: BorrowSnapshot}
    
    /// Model used to calculate underlying asset's borrow interest rate
    pub var interestRateModelAddress: Address?
    
    pub var interestRateModelCap: Capability<
        &{LendingInterfaces.InterestRateModelPublic}
    >?
    ...
    priv let underlyingVault: @FungibleToken.Vault
    
    /// Underlying type
    priv let underlyingAssetType: Type
    ...
    pub event ReservesReduced(
        scaledReduceAmount: UInt256,
        scaledNewTotalReserves: UInt256
    )
    ...
    pub event NewInterestRateModel(
    ...
    pub event NewReserveFactor(
        _ oldReserveFactor: UFix64,
        _ newReserveFactor: UFix64
    )
    
    /// Event emitted when the poolSeizeShare is changed
    pub event NewPoolSeizeShare(
        _ oldPoolSeizeShare: UFix64,
        _ newPoolSeizeShare: UFix64
    )
    
    /// Event emitted when the comptroller is changed
    pub event NewComptroller(
        _ oldComptrollerAddress: Address?,
        _ newComptrollerAddress: Address
    )
    ...
    pub fun accrueInterestReadonly(): [UInt256; 4]{ 
        ...
        let scaledInterestAccumulated =
            scaledInterestFactor * scaledBorrowPrior / scaleFactor
        let scaledTotalBorrowsNew =
            scaledInterestAccumulated + scaledBorrowPrior
        let scaledTotalReservesNew =
            self.scaledReserveFactor * scaledInterestAccumulated / scaleFactor
            + scaledReservesPrior
        let scaledBorrowIndexNew =
            scaledInterestFactor * scaledBorrowIndexPrior / scaleFactor
            + scaledBorrowIndexPrior
        return [
            currentBlockNumber,
    ...
    pub fun borrowBalanceSnapshotScaled(borrowerAddress: Address): UInt256{ 
        ...
        return borrower.scaledPrincipal * self.scaledBorrowIndex
        / borrower.scaledInterestIndex
    }
    ...
    pub fun supply(
        ...
        inUnderlyingVault: @FungibleToken.Vault
    ){ 
        pre{ 
        ...
        let scaledAmount =
            LendingConfig.UFix64ToScaledUInt256(inUnderlyingVault.balance)
        let err =
        ...
        let underlyingToken2LpTokenRateScaled =
            self.underlyingToLpTokenRateSnapshotScaled()
        let scaledMintVirtualAmount =
            scaledAmount * LendingConfig.scaleFactor
            / underlyingToken2LpTokenRateScaled
        // mint pool tokens for supply certificate
        self.accountLpTokens[supplierAddr] = scaledMintVirtualAmount
            + (self.accountLpTokens[supplierAddr] ?? 0)
        self.scaledTotalSupply = self.scaledTotalSupply
            + scaledMintVirtualAmount
    ...
    priv fun redeemInternal(
        ...
        let amountUnderlyingToRedeem =
            LendingConfig.ScaledUInt256ToUFix64(scaledUnderlyingToRedeem)
    ...
    pub fun borrow(
        ...
        let scaledBorrowAmount =
            LendingConfig.UFix64ToScaledUInt256(borrowAmount)
        assert(
        ...
        let scaledBorrowBalanceNew =
            scaledBorrowAmount
            + self.borrowBalanceSnapshotScaled(borrowerAddress: borrower)
    ...
    priv fun repayBorrowInternal(
        ...
        let scaledAccountTotalBorrowsNew =
            scaledAccountTotalBorrows > scaledRepayAmount
                ? scaledAccountTotalBorrows - scaledRepayAmount
                : 0 as UInt256
        self.scaledTotalBorrows = self.scaledTotalBorrows
            - scaledActualRepayAmount
        emit Repay(
            borrower: borrower,
    ...
    pub fun liquidate(
        ...
        let scaledActualRepayAmount =
            scaledUnderlyingAmountToRepay - scaledRemainingAmount
        // Calculate collateralLpTokenSeizedAmount based on actualRepayAmount
        let scaledCollateralLpTokenSeizedAmount =
            ((self.comptrollerCap!).borrow()!)
                .calculateCollateralPoolLpTokenToSeize(
                borrower: borrower,
        ...
        if poolCollateralizedToSeize == self.poolAddress{ 
            ...
            // Seize external
            let externalPoolPublicRef = getAccount(poolCollateralizedToSeize).getCapability<&{LendingInterfaces.PoolPublic}>(LendingConfig.PoolPublicPublicPath).borrow() ?? panic(LendingError.ErrorEncode(msg: "Cannot borrow reference to external PoolPublic resource", err: LendingError.ErrorCode.CANNOT_ACCESS_POOL_PUBLIC_CAPABILITY))
    ...
    priv fun seizeInternal(
        ...
        let scaledProtocolSeizedLpTokens =
            scaledBorrowerLpTokenToSeize * self.scaledPoolSeizeShare
            / scaleFactor
        let scaledLiquidatorSeizedLpTokens =
            scaledBorrowerLpTokenToSeize - scaledProtocolSeizedLpTokens
        let scaledUnderlyingToLpTokenRate =
            self.underlyingToLpTokenRateSnapshotScaled()
        let scaledAddedUnderlyingReserves =
            scaledUnderlyingToLpTokenRate * scaledProtocolSeizedLpTokens
            / scaleFactor
        self.scaledTotalReserves = self.scaledTotalReserves
            + scaledAddedUnderlyingReserves
        self.scaledTotalSupply = self.scaledTotalSupply
            - scaledProtocolSeizedLpTokens
        ...
        self.accountLpTokens[liquidator] = scaledLiquidatorSeizedLpTokens
            + (self.accountLpTokens[liquidator] ?? 0)
        emit ReservesAdded(
    ...
    pub fun flashloan(
        ...
        pre{ 
            self.isFlashloanOpen():
                LendingError.ErrorEncode(msg: "LendingError: flashloan is not open", err: LendingError.ErrorCode.MARKET_NOT_OPEN)
            requestedAmount > 0.0 && requestedAmount < self.underlyingVault.balance:
                LendingError.ErrorEncode(msg: "LendingError: flashloan invalid requested amount", err: LendingError.ErrorCode.INSUFFICIENT_POOL_LIQUIDITY)
            executorCap.check():
                LendingError.ErrorEncode(msg: "LendingError: flashloan executor resource not properly setup", err: LendingError.ErrorCode.FLASHLOAN_EXECUTOR_SETUP)
        }
        // Accrues interests and checkpoints latest states
        self.accrueInterest()
        let requestedAmountScaled =
            LendingConfig.UFix64ToScaledUInt256(requestedAmount)
        let tokenOut <- self.underlyingVault.withdraw(amount: requestedAmount)
        self.scaledTotalBorrows = self.scaledTotalBorrows
            + requestedAmountScaled
        let tokenIn <-
            (executorCap.borrow()!).executeAndRepay(
                loanedToken: <-tokenOut,
                params: params
            )
        assert(
            tokenIn.isInstance(self.underlyingAssetType),
            message: LendingError.ErrorEncode(
        ...
        assert(
            tokenIn.balance
            >= requestedAmount
            * (1.0 + UFix64(self.getFlashloanRateBps()) / 10000.0),
            message: LendingError.ErrorEncode(
        ...
        self.scaledTotalBorrows = self.scaledTotalBorrows
            - requestedAmountScaled
        emit Flashloan(
            executor: ((executorCap.borrow()!).owner!).address,
            executorType: (executorCap.borrow()!).getType(),
    ...
    ///
    priv fun checkUserCertificateType(
    ...
    pub resource PoolPublic: LendingInterfaces.PoolPublic{ 
        ...
        pub fun seize(seizerPoolCertificate: @{LendingInterfaces.IdentityCertificate}, seizerPool: Address, liquidator: Address, borrower: Address, scaledBorrowerCollateralLpTokenToSeize: UInt256){ 
    ...
    pub resource PoolAdmin: LendingInterfaces.PoolAdminPublic{ 
        ...
        pub fun setInterestRateModel(newInterestRateModelAddress: Address){ 
            ...
            if newInterestRateModelAddress != LendingPool.interestRateModelAddress{ 
                ...
                LendingPool.interestRateModelCap = getAccount(newInterestRateModelAddress).getCapability<&{LendingInterfaces.InterestRateModelPublic}>(LendingConfig.InterestRateModelPublicPath)
        ...
        pub fun setComptroller(newComptrollerAddress: Address){ 
            ...
            if newComptrollerAddress != LendingPool.comptrollerAddress{ 
                ...
                LendingPool.comptrollerCap = getAccount(newComptrollerAddress).getCapability<&{LendingInterfaces.ComptrollerPublic}>(LendingConfig.ComptrollerPublicPath)
        ...
        pub fun initializePool(reserveFactor: UFix64, poolSeizeShare: UFix64, interestRateModelAddress: Address){ 
            ...
            LendingPool.interestRateModelCap = getAccount(interestRateModelAddress).getCapability<&{LendingInterfaces.InterestRateModelPublic}>(LendingConfig.InterestRateModelPublicPath)
        }
        ...
        pub fun withdrawReserves(reduceAmount: UFix64): @FungibleToken.Vault{ 
            ...
            assert(reduceAmountScaled <= LendingPool.scaledTotalReserves, message: LendingError.ErrorEncode(msg: "exceeded pool total reserve", err: LendingError.ErrorCode.EXCEED_TOTAL_RESERVES))
            assert(reduceAmountScaled <= LendingPool.getPoolCash(), message: LendingError.ErrorEncode(msg: "insufficient pool liquidity to withdraw reserve", err: LendingError.ErrorCode.INSUFFICIENT_POOL_LIQUIDITY))
        ...
        pub fun setFlashloanRateBps(rateBps: UInt64){ 
            pre{ 
                rateBps >= 0 && rateBps <= 10000:
                    LendingError.ErrorEncode(msg: "LendingPool: flashloan rateBps should be in [0, 10000]", err: LendingError.ErrorCode.INVALID_PARAMETERS)
            }
    ...
    init(){ 
        ...
        self.underlyingVault <- self.account.load<@FungibleToken.Vault>(
                from: self.UnderlyingAssetVaultStoragePath
            )
            ?? panic(
                "Deployer should own zero-balanced underlying asset vault first"
            )
        ...
        // save pool admin
        destroy <-self.account.load<@AnyResource>(
            from: self.PoolAdminStoragePath
        )
        ...
        destroy <-self.account.load<@AnyResource>(
            from: self.PoolPublicStoragePath
        )
        self.account.save(<-create PoolPublic(), to: self.PoolPublicStoragePath)
        self.account.unlink(self.PoolPublicPublicPath)
        self.account.link<&{LendingInterfaces.PoolPublic}>(
            LendingConfig.PoolPublicPublicPath,
            target: self.PoolPublicStoragePath
        )
    }
}
