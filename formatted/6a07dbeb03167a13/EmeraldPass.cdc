...
pub contract EmeraldPass{ 
    priv var treasury: ECTreasury
    
    // Maps the type of a token to its pricing
    priv var pricing:{ Type: Pricing}
    ...
    pub resource Admin{ 
        ...
        pub fun giveUserTime(user: Address, time: UFix64){ 
            let userVault =
                getAccount(user).getCapability(EmeraldPass.VaultPublicPath)
                    .borrow<&Vault{VaultPublic}>()
                ?? panic(
                    "This receiver has not set up a Vault for Emerald Pass yet."
                )
    ...
    pub fun depositToECTreasury(vault: @FungibleToken.Vault){ 
        ...
        ((self.getTreasury()[vault.getType()]!).borrow()!).deposit(
            from: <-vault
        )
    }
    ...
    pub fun donate(
        ...
        let userVault =
            getAccount(to).getCapability(EmeraldPass.VaultPublicPath).borrow<
                &Vault{VaultPublic}
            >()
            ?? panic(
                "This receiver has not set up a Vault for Emerald Pass yet."
    ...
    init(){ 
        ...
        self.pricing ={ 
                Type<@FUSD.Vault>():
                Pricing({100.0: 2629743.0, // 1 month 1000.0: 31556926.0 // 1 year})
            }
        self.purchased = 0
