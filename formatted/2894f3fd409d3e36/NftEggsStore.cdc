...
import NftEggsNFT from "./NftEggsNFT.cdc"

//import NftEggsNFT from "../0xf8d6e0586b0a20c7/NftEggsNFT.cdc"
...
pub contract NftEggsStore{ 
    ...
    pub resource Listing: ListingPublic{ 
        ...
        //当前投标用户钱包引用
        priv var bidVault: @FungibleToken.Vault
        ...
        pub fun purchase(payment: @FungibleToken.Vault, buyerAddress: Address){ 
            pre{ 
                ...
                self.details.saleType == 1:
                    "listing's saleType is bid"
            ...
            // 对收件人的NFT集合的引用
            let depositRef = getAccount(buyerAddress).getCapability(NftEggsNFT.CollectionPublicPath).borrow<&{NftEggsNFT.CollectionPublic}>() ?? panic("Could not borrow a reference to the receiver's collection")
            // 存入收件人NFT集合
        ...
        pub fun placeBid(bidTokens: @FungibleToken.Vault, address: Address){ 
            pre{ 
                self.details.purchased == false:
                    "listing has already been purchased"
                self.details.saleType == 2:
                    "listing's saleType is not bid"
                bidTokens.isInstance(self.details.salePaymentVaultType):
                    "payment vault is not requested fungible token"
            }
            ...
            self.details.setBid(endTime: endTime, bidPrice: self.bidVault.balance, bidAddress: address)
            emit Bid(storefrontAddress: self.owner?.address!, listingResourceID: self.uuid, storefrontResourceID: self.details.storefrontID, bidAddress: self.details.bidAddress!, bidPrice: self.details.bidPrice, nftID: self.details.nftID, endTime: self.details.endTime)
        }
        ...
        pub fun endSale(owner: Bool){ 
            ...
            if self.details.saleType != 1 && self.details.bidPrice > 0.0{ 
                ...
                assert(nft.id == self.details.nftID, message: "withdrawn NFT does not have specified ID")
                // 对收件人的NFT集合的引用
                let depositRef = getAccount(self.details.bidAddress!).getCapability(NftEggsNFT.CollectionPublicPath).borrow<&{NftEggsNFT.CollectionPublic}>() ?? panic("Could not borrow a reference to the receiver's collection")
                // 存入收件人NFT集合
                depositRef.deposit(token: <-nft)
        ...
        destroy(){ 
            if !self.details.purchased{ 
                emit ListingCompleted(storefrontAddress: self.owner?.address, listingResourceID: self.uuid, storefrontResourceID: self.details.storefrontID, purchased: self.details.purchased, buyerAddress: nil, dealPrice: nil, nftID: self.details.nftID)
            }
        ...
        // initializer
        //
        init(nftProviderCapability: Capability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>, nftType: Type, nftID: UInt64, salePaymentVaultType: Type, saleItemPrice: UFix64, saleCuts:{ String: NftEggsNFT.Royalty}, saleType: UInt64, startTime: UFix64, endTime: UFix64, minimumBidIncrement: UFix64, storefrontID: UInt64){ 
            if saleType == 2 && !(minimumBidIncrement > 0.0){ 
                panic("when saleType is 2, minimumBidIncrement must greater then 0")
            }
            
            // 存储销售信息
            self.details = ListingDetails(nftType: nftType, nftID: nftID, salePaymentVaultType: salePaymentVaultType, salePrice: saleItemPrice, saleCuts: saleCuts, saleType: saleType, startTime: startTime, endTime: endTime, minimumBidIncrement: minimumBidIncrement, storefrontID: storefrontID)
    ...
    pub resource Storefront: StorefrontManager, StorefrontPublic{ 
        ...
        // 创建销售列表项
        // 允许店面拥有者创建并插入信息到销售列表
        pub fun createListing(nftProviderCapability: Capability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>, nftType: Type, nftID: UInt64, salePaymentVaultType: Type, saleItemPrice: UFix64, saleType: UInt64, startTime: UFix64, endTime: UFix64, minimumBidIncrement: UFix64): UInt64{ 
            ...
            let collectionRef = getAccount(nftProviderCapability.address).getCapability(NftEggsNFT.CollectionPublicPath).borrow<&{NftEggsNFT.CollectionPublic}>() ?? panic("Could not borrow capability from public collection")
            ...
            destroy oldListing
            emit ListingAvailable(storefrontAddress: self.owner?.address!, listingResourceID: listingResourceID, nftType: nftType, nftID: nftID, ftVaultType: salePaymentVaultType, price: listingPrice, saleType: saleType, minimumBidIncrement: minimumBidIncrement, startTime: startTime, endTime: endTime)
