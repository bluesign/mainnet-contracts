...
pub contract OneFootballCollectible: NonFungibleToken{ 
    ...
    priv let templates:{ UInt64: Template}
    ...
    pub fun check(_ address: Address): Bool{ 
        return getAccount(address).getCapability<&{NonFungibleToken.CollectionPublic}>(OneFootballCollectible.CollectionPublicPath).check()
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &OneFootballCollectible.NFT?{ 
        let collection = getAccount(from).getCapability(OneFootballCollectible.CollectionPublicPath).borrow<&{OneFootballCollectible.OneFootballCollectibleCollectionPublic}>() ?? panic("Couldn't get collection")
    ...
    init(){ 
        ...
        self.account.link<&OneFootballCollectible.Collection{NonFungibleToken.CollectionPublic, OneFootballCollectible.OneFootballCollectibleCollectionPublic}>(OneFootballCollectible.CollectionPublicPath, target: OneFootballCollectible.CollectionStoragePath)
