...
pub contract NFTAirDrop{ 
    pub event Claimed(nftType: Type, nftID: UInt64, recipient: Address)
    
    pub let DropStoragePath: StoragePath
    
    pub let DropPublicPath: PublicPath
    
    /* Custom public interface for our drop capability. */    pub resource interface DropPublic{ 
        pub fun claim(
            id: UInt64,
            signature: [
                UInt8
    ...
    pub resource Drop: DropPublic{ 
        ...
        priv let collection: Capability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
        
        // map of nft ids to decoded public claim keys
        priv let claims:{ UInt64: [UInt8]}
        ...
        pub fun claim(id: UInt64, signature: [UInt8], receiver: &{NonFungibleToken.CollectionPublic}){ 
            let collection = self.collection.borrow()!
