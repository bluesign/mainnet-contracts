...
pub contract MetaverseMarket: NonFungibleToken{ 
    ...
    priv var categoriesList:{ UInt64: String}
    
    // {CategoryName: [NFT To Sell ID]}
    priv var categoriesNFTsToSell:{ UInt64: [UInt64]}
    
    // Dictionary with NFT List Data
    priv var nftsToSell:{ UInt64: OzoneListToSellMetadata}
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: MetaverseMarket.CollectionStoragePath, publicPath: MetaverseMarket.CollectionPublicPath, providerPath: /private/ProvenancedCollectionsV9, publicCollection: Type<&MetaverseMarket.Collection{MetaverseMarket.MetaverseMarketCollectionPublic, NonFungibleToken.CollectionPublic}>(), publicLinkedType: Type<&MetaverseMarket.Collection{MetaverseMarket.MetaverseMarketCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&MetaverseMarket.Collection{MetaverseMarket.MetaverseMarketCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
    ...
    pub resource Collection: MetaverseMarketCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection{ 
        ...
        pub fun borrowNFTSafe(id: UInt64): &NFT?{ 
            ...
            return self.ownedNFTs[id] != nil ? (&self.ownedNFTs[id] as auth &NonFungibleToken.NFT?)! as! &MetaverseMarket.NFT : nil
        }
    ...
    pub resource Admin{ 
        ...
        pub fun createList(listId: UInt64, name: String, description: String, categoryId: UInt64, creator: Address?, creatorDapperAddress: Address?, fileName: String, previewImage: String, format: String, fileIPFS: String, price: UFix64, maxSupply: UInt64){ 
            var max = 0 as UInt64
        ...
        pub fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, payment: @FungibleToken.Vault, listedNftId: UInt64){ 
            ...
            switch payment.getType(){ 
                case Type<@FlowToken.Vault>():
                    ...
                    let receiverRef = getAccount(list.creator!).getCapability(/public/flowTokenReceiver).borrow<&{FungibleToken.Receiver}>() ?? panic("Could not borrow receiver reference to the recipient's Vault")
                    
                    // Get a reference to the recipient's Receiver
                    let royaltyReceiver = getAccount(MetaverseMarket.account.address).getCapability(/public/flowTokenReceiver).borrow<&{FungibleToken.Receiver}>() ?? panic("Could not borrow receiver reference to the recipient's Vault")
                    royaltyReceiver.deposit(from: <-royalty)
                    receiverRef.deposit(from: <-payment)
                    
                    // deposit it in the recipient's account using their reference
                    recipient.deposit(token: <-create MetaverseMarket.NFT(initID: MetaverseMarket.totalSupply, uniqueListId: list.minted, listId: list.listId, name: list.name, categoryId: list.categoryId, description: list.description, previewImage: list.previewImage, creator: list.creator, creatorDapperAddress: list.creatorDapperAddress, fileName: list.fileName, format: list.format, fileIPFS: list.fileIPFS))
                case Type<@FlowUtilityToken.Vault>():
                    // Get a reference to the recipient's Receiver
                    let receiverRef = getAccount(list.creatorDapperAddress!).getCapability(/public/flowUtilityTokenReceiver).borrow<&{FungibleToken.Receiver}>() ?? panic("Could not borrow receiver reference to the creator Dapper Address Vault")
                    
                    // Get a reference to the recipient's Receiver
                    let royaltyReceiver = getAccount(0x43fbb5fb34ba8ef0).getCapability(/public/flowUtilityTokenReceiver).borrow<&{FungibleToken.Receiver}>() ?? panic("Could not borrow receiver reference to the recipient's Vault")
                    royaltyReceiver.deposit(from: <-royalty)
                    receiverRef.deposit(from: <-payment)
                    
                    // deposit it in the recipient's account using their reference
                    recipient.deposit(token: <-create MetaverseMarket.NFT(initID: MetaverseMarket.totalSupply, uniqueListId: list.minted, listId: list.listId, name: list.name, categoryId: list.categoryId, description: list.description, previewImage: list.previewImage, creator: list.creatorDapperAddress, creatorDapperAddress: list.creatorDapperAddress, fileName: list.fileName, format: list.format, fileIPFS: list.fileIPFS))
                default:
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &MetaverseMarket.NFT?{ 
        let collection = getAccount(from).getCapability(MetaverseMarket.CollectionPublicPath).borrow<&MetaverseMarket.Collection{MetaverseMarket.MetaverseMarketCollectionPublic}>() ?? panic("Couldn't get collection")
