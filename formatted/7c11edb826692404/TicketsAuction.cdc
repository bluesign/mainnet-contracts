...
pub contract TicketsAuction{ 
    ...
    pub resource Auction: AuctionPublic{ 
        ...
        pub fun placeBid(refund: Capability<&{FungibleToken.Receiver}>, recipient: Capability<&{NonFungibleToken.CollectionPublic}>, vault: @FungibleToken.Vault, ref: String?){ 
            pre{ 
        ...
        priv fun increaseBid(vault: @FungibleToken.Vault, ref: String?){ 
            pre{ 
        ...
        priv fun createBid(refund: Capability<&{FungibleToken.Receiver}>, recipient: Capability<&{NonFungibleToken.CollectionPublic}>, vault: @FungibleToken.Vault, ref: String?){ 
            pre{ 
                TicketsAuction.isOpen():
                    "Auction not open"
    ...
    pub resource Admin{ 
        ...
        pub fun completeAuction(auctionID: UInt64){ 
            let auction <-
                TicketsAuction.auctions.remove(key: auctionID)
    ...
    pub fun borrow(auctionID: UInt64): &Auction{AuctionPublic}?{ 
        if self.auctions[auctionID] != nil{ 
            return (&self.auctions[auctionID] as                                                 // Thu Apr 14 2022 12:00:00 GMT+0000
