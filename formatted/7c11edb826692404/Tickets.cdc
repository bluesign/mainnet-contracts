...
pub contract Tickets{ 
    ...
    priv let candidateDept:{ UInt64: UFix64}
    
    priv var minted:{ UInt64:{ Ticket.Level: UInt64}}
    
    priv let candidateFund: @FungibleToken.Vault
    ...
    pub struct SaleCut{ 
        ...
        init(
            recipient: Capability<&FlowToken.Vault{FungibleToken.Receiver}>,
            rate: UFix64
        ){ 
            self.recipient = recipient
            self.rate = rate
    ...
    pub resource Admin{ 
        ...
        pub fun setPaymentCut(cuts: [PaymentCut]){ 
            ...
            for cut in cuts{ 
                ...
                let cap = getAccount(cut.recipient).getCapability<&FlowToken.Vault{FungibleToken.Receiver}>(Tickets.FlowReceiverPath)
    ...
    priv fun payout(payment: @FungibleToken.Vault){ 
        let total = payment.balance
        let payouts: [PaymentCut] = []
        self.candidateFund.deposit(
            from: <-payment.withdraw(amount: self.candidateFundRate * total)
        )
        payouts.append(
            PaymentCut(
    ...
    priv fun mintTicket(
        ...
        var i = 0 as UInt64
        let minter =
            self.account.borrow<&Ticket.NFTMinter>(
                from: Ticket.MinterStoragePath
            )!
    ...
    priv fun buy(
        ...
        let price =
            self.ticketPrices[level]! - self.ticketPrices[level]! * discountRate
        assert(
            price * UFix64(qty) == payment.balance,
    ...
    priv fun mapToVnMissTicketLevel(level: UInt8): VnMiss.Level{ 
        switch level{ 
    ...
    priv fun fundForCandidate(
        c: VnMissCandidate.Candidate,
        issuePrice: UFix64
    ){ 
        let recipient =
            getAccount(c.fundAdress).getCapability<
                &FlowToken.Vault{FungibleToken.Receiver}
            >(Tickets.FlowReceiverPath).borrow()
    ...
    priv fun canMint(candidateID: UInt64, level: Ticket.Level): Bool{ 
    ...
    pub fun buyTickets(
        ...
        assert(
            Tickets.ticketStartAt <= now && Tickets.ticketEndAt >= now,
            message: "Not open"
        )
        Tickets.buy(
            recipient: recipient,
            level: Ticket.Level(level)!,
            qty: qty,
            payment: <-payment,
            discountRate: 0.0,
            ref: ref
    ...
    pub fun buyWhitelist(
        ...
        assert(
            Whitelist.hasBought(address: buyer) == false,
            message: "Only buy 1 ticket discount"
        )
    ...
    pub fun swapForNFT(
        ...
        let now = getCurrentBlock().timestamp
        assert(
            Tickets.swapStartAt <= now && Tickets.swapEndAt >= now,
            message: "Not open"
        )
        let c =
            VnMissCandidate.getCandidate(id: candidateID)
            ?? panic("Candidate not exist")
        self.minted[candidateID] = self.minted[candidateID] ??{} 
        let level =
            Ticket.Level(rawValue: ticket.level) ?? panic("Level invalid")
