...
pub contract Relics: NonFungibleToken{ 
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: Relics.CollectionStoragePath, publicPath: Relics.CollectionPublicPath, providerPath: /private/RelicsCollection, publicCollection: Type<&Relics.Collection{NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection}>(), publicLinkedType: Type<&Relics.Collection{NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&Relics.Collection{NonFungibleToken.CollectionPublic, NonFungibleToken.Provider, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
    ...
    pub resource RelicMinter{ 
        ...
        pub fun updateMediaURL(_from: Address, _id: UInt64, _whichURL: String, _newURL: String){ 
            let collection = getAccount(_from).getCapability(Relics.CollectionPublicPath).borrow<&Relics.Collection{Relics.RelicCollectionPublic}>() ?? panic("Couldn't get collection")
        ...
        pub fun updateMediaURLs(from: Address, id: UInt64, _newAssetVideoURL: String, _newAssetImageURL: String, _newMusicURL: String, _newArtworkURL: String, _newMarketDisplay: String){ 
            let collection = getAccount(from).getCapability(Relics.CollectionPublicPath).borrow<&Relics.Collection{Relics.RelicCollectionPublic}>() ?? panic("Couldn't get collection")
    ...
    init(){ 
        ...
        self.account.link<&Relics.Collection{NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection, RelicCollectionPublic}>(self.CollectionPublicPath, target: self.CollectionStoragePath)
