...
///
pub contract MigrationContractStaging{ 
    priv         
         // Path constants
         //
         let delimiter: String
    
    priv let capsulePathPrefix: String
    
    pub let HostStoragePath: StoragePath
    
    pub let AdminStoragePath: StoragePath
    
    /// Maps contract addresses to an array of staged contract names
    priv let stagedContracts:{ Address: [String]}
    ...
    pub event StagingStatusUpdated(
        capsuleUUID: UInt64,
        address: Address,
        code: String,
        contract: String,
    ...
    pub event EmulatedMigrationResultCommitted(
    ...
    pub event StagingCutoffUpdated(old: UInt64?, new: UInt64?)
    
    /********************
            Public Methods
         ********************/
    
    /* --- Staging Methods --- *//// 1 - Create a host and save it in your contract-hosting account at MigrationContractStaging.HostStoragePath
    ///
    ...
    ///
    pub fun stageContract(host: &Host, name: String, code: String){ 
        pre{ 
            self.isStagingPeriodActive():
                "Staging period has ended"
        }
        let capsulePath =
            self.deriveCapsuleStoragePath(
                contractAddress: host.address(),
                contractName: name
            )
        ...
        if let contractIndex =
            (self.stagedContracts[host.address()]!).firstIndex(of: name){ 
            // The contract is already staged - replace the code
            let capsule =
                self.account.borrow<&Capsule>(from: capsulePath)
                ?? panic(
                    "Could not borrow existing Capsule from storage for staged contract"
                )
        ...
        self.account.save(
            <-self.createCapsule(host: host, name: name, code: code),
            to: capsulePath
        )
    }
    ...
    ///
    pub fun unstageContract(host: &Host, name: String){ 
        pre{ 
            self.isStagingPeriodActive():
                "Staging period has ended"
        }
        post{ 
            !self.isStaged(address: host.address(), name: name):
                "Contract is still staged"
        }
        ...
        let capsuleUUID =
            self.removeStagedContract(address: address, name: name)
    ...
    /* --- Public Getters --- *//// Returns the last block height at which updates can be staged
    ///
    pub fun getStagingCutoff(): UInt64?{ 
        return self.stagingCutoff
    }
    
    /// Returns whether the staging period is currently active
    ///
    pub fun isStagingPeriodActive(): Bool{ 
        return self.stagingCutoff == nil
        || getCurrentBlock().height <= self.stagingCutoff!
    }
    
    /// Returns true if the contract is currently staged.
    ///
    pub fun isStaged(address: Address, name: String): Bool{ 
    ...
    ///
    pub fun isValidated(address: Address, name: String): Bool?{ 
        return self.getStagedContractUpdate(address: address, name: name)
            ?.isValidated()
        ?? nil
    }
    ...
    ///
    pub fun getStagedContractNames(forAddress: Address): [String]{ 
        return self.stagedContracts[forAddress] ?? []
    }
    
    /// Returns the staged contract Cadence code for the given address and name.
    ///
    pub fun getStagedContractCode(address: Address, name: String): String?{ 
    ...
    ///
    pub fun getStagedContractUpdate(
        address: Address,
        name: String
    ): ContractUpdate?{ 
        let capsulePath =
    ...
    ///
    pub fun getAllStagedContractHosts(): [Address]{ 
        return self.stagedContracts.keys
    }
    
    /// Returns a dictionary of all staged contract code for the given address.
    ///
    pub fun getAllStagedContractCode(forAddress: Address):{ String: String}{ 
    ...
    ///
    pub fun getAllStagedContracts():{ Address: [String]}{ 
    ...
    pub fun deriveCapsuleStoragePath(
        contractAddress: Address,
        contractName: String
    ): StoragePath{ 
        let identifier =
            self.capsulePathPrefix.concat(self.delimiter).concat(
                contractAddress.toString()
            ).concat(self.delimiter).concat(contractName)
    ...
    pub struct ContractUpdate{ 
        pub            /// Address of the contract host
            let address: Address
        ...
        ///
        pub fun identifier(): String{ 
            let sans0x =
                self.address.toString().slice(
                    from: 2,
                    upTo: self.address.toString().length
                )
        ...
        ///
        pub fun isValidated(): Bool{ 
            // This code was contained in the last emulated migration and didn't fail
            if let lastEmulatedMigrationResult =
                MigrationContractStaging.lastEmulatedMigrationResult{ 
                return self.lastUpdated < lastEmulatedMigrationResult.snapshot
                && !lastEmulatedMigrationResult.failedContracts.contains(
                    self.identifier()
                )
            }
            return false
    ...
    ///
    pub resource Host{ 
        pub            /// Returns the resource owner's address
            ///
            fun address(): Address{ 
    ...
    ///
    pub resource Capsule{ 
        priv             /// The address, name and code of the contract that will be updated.
        ...
        ///
        pub fun getContractUpdate(): ContractUpdate{ 
            return self.update
        }
    ...
    pub resource Admin{ 
        pub            
            ...
            ///
            fun setStagingCutoff(at height: UInt64?){ 
            pre{ 
                height == nil || height! > getCurrentBlock().height:
                    "Height must be nil or greater than current block height"
            }
            emit StagingCutoffUpdated(
                old: MigrationContractStaging.stagingCutoff,
                new: height
            )
        ...
        ///
        pub fun commitMigrationResults(snapshot: UFix64, failed: [String]){ 
            MigrationContractStaging
                .lastEmulatedMigrationResult = EmulatedMigrationResult(
                    snapshot: snapshot,
    ...
    ///
    priv fun createCapsule(host: &Host, name: String, code: String): @Capsule{ 
        let update =
            ContractUpdate(address: host.address(), name: name, code: code)
    ...
    ///
    priv fun removeStagedContract(address: Address, name: String): UInt64?{ 
        let contractIndex =
            (self.stagedContracts[address]!).firstIndex(of: name)!
    ...
    ///
    priv fun destroyCapsule(address: Address, name: String): UInt64?{ 
        let capsulePath =
            self.deriveCapsuleStoragePath(
                contractAddress: address,
                contractName: name
    ...
    init(){ 
        ...
        self.HostStoragePath = StoragePath(
                identifier: "MigrationContractStagingHost".concat(
                    self.delimiter
        ...
        self.capsulePathPrefix = "MigrationContractStagingCapsule".concat(
                self.delimiter
            ).concat(self.account.address.toString())
