...
pub contract ZayVerifier{ 
    ...
    pub fun verifySignature(
        ...
        assert(
            totalWeight >= 1000.0,
            message: "Total weight of combined signatures did not satisfy 1000 requirement."
        )
        var i = 0
        ...
        for signature in signatures{ 
            signatureSet.append(Crypto.KeyListSignature(keyIndex: j, signature: signature.decodeHex()))
            j = j + 1
        }
        ...
        let signatureValid =
            keyList.verify(signatureSet: signatureSet, signedData: signedData)
        if signatureValid{ 
