...
import SchmoesNFT from "../0x6c4fe48768523577/SchmoesNFT.cdc"

import ZayVerifier from "./ZayVerifier.cdc"
...
pub contract ZayTrader{ 
    ...
    pub struct TradeOfferDetails{ 
        ...
        access(contract) let offerFeePayer: Capability<
    ...
    pub resource TradeBundle{ 
        ...
        destroy(){ 
            assert(
                self.fungibleAssets.length == 0,
                message: "Can not destroy trade bundle with ft resources within"
            )
            assert(
                self.nftAssets.length == 0,
                message: "Can not destroy trade bundle with nft resources within"
            )
    ...
    pub resource TradeOffer: TradeOfferPublic{ 
        ...
        priv let details: TradeOfferDetails
        ...
        //
        priv fun executeTrade(tradeBundle: @TradeBundle): @TradeBundle{ 
            pre{ 
        ...
        pub fun acceptTrade(tradeBundle: @TradeBundle, fee: @FungibleToken.Vault, signingAddress: Address?, signedMessage: String?, keyIds: [Int], signatures: [String], signatureBlock: UInt64?): @TradeBundle{ 
            let signatureRequired = self.details.requestedAddress != nil
            assert(self.details.requestedAddress == nil || signingAddress! == self.details.requestedAddress!, message: "Requested trade account does not match provided account or expected signature not provided")
            
            // Check provided signature is valid for the 
            // signing account if it was provided
            var validSignature = false
            if signingAddress != nil && signedMessage != nil{ 
                let signatureTimestamp = ZayVerifier.verifySignature(acctAddress: signingAddress!, message: signedMessage!, keyIds: keyIds, signatures: signatures, signatureBlock: signatureBlock!)
            ...
            if validSignature{ 
                accountHasSchmoe = ZayVerifier.checkOwnership(address: signingAddress!, collectionPath: SchmoesNFT.CollectionPublicPath, nftType: Type<@SchmoesNFT.NFT>())
            }
            if accountHasSchmoe{ 
        ...
        destroy(){ 
            ...
            if !self.details.executed{ 
                emit TradeCancelled(tradeOfferResourceID: self.uuid, tradeCollectionResourceID: self.details.tradeCollectionID)
            }
        }
        
        // initializer
        //
        init(tradeCollectionID: UInt64, offeredNfts: [OfferedNftTradeAsset], offeredFts: [OfferedFungibleTradeAsset], offerFeePayer: Capability<&FlowToken.Vault{FungibleToken.Provider}>, offerFee: UFix64, requestedNfts: [RequestedNftTradeAsset], requestedFts: [RequestedFungibleTradeAsset], creatorAddress: Address, requestedAddress: Address?, expiration: UFix64){ 
    ...
    pub resource interface TradeCollectionPublic{ 
        ...
        pub fun borrowTradeOffer(tradeOfferResourceID: UInt64): &TradeOffer{
            TradeOfferPublic
        }?{} 
    ...
    //
    pub resource TradeCollection:
        TradeCollectionManager,
        TradeCollectionPublic{
    
        // The dictionary of Listing uuids to Listing resources.
        priv var tradeOffers: @{UInt64: TradeOffer}
        ...
        //
        pub fun removeTradeOffer(tradeOfferResourceID: UInt64){ 
            let tradeOfferResource <-
                self.tradeOffers.remove(key: tradeOfferResourceID)
        ...
        pub fun borrowTradeOffer(tradeOfferResourceID: UInt64): &TradeOffer{
            ...
            if self.tradeOffers[tradeOfferResourceID] != nil{ 
                return &self.tradeOffers[tradeOfferResourceID] as                                                                  
                                                                  // cleanup
        ...
        //
        pub fun cleanup(tradeOfferResourceID                                            
                                            // createTradeBundle
                                            // Make a TradeBundle for transferring many trade assets
                                            //
                                            : UInt64){ 
            pre{ 
                self.tradeOffers[tradeOfferResourceID]                                                       
            ...
            let tradeOffer <-
                self.tradeOffers.remove(key: tradeOfferResourceID)!
            assert(
                tradeOffer.getDetails().executed == true,
                message: "trade offer has not been executed, only owner can cancel it"
            )
            destroy tradeOffer
        }
        ...
        init(){ 
            self.tradeOffers <-{} 
            emit TradeCollectionInitialized(
                tradeCollectionResourceID: self.uuid
            )
        }
    }
    
    pub resource Admin{ 
        pub fun updateFeeReceiver(
            cap: Capability<&FlowToken.Vault{FungibleToken.Receiver}>
        ){ 
    ...
    init(){ 
        ...
        self.feeReceiver = self.account.getCapability<
