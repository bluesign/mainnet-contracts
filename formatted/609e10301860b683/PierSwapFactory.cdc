...
pub contract PierSwapFactory{ 
    ...
    pub fun getPoolById(poolId: UInt64): &PierPair.Pool{IPierPair.IPool}{ 
        ...
        >(self.SwapPoolPublicPath).borrow()
        ?? panic(
            "Metapier PierSwapFactory: Couldn't borrow swap pool from the account"
        )
    }
    ...
    pub fun getPoolByTypes(tokenAType: Type, tokenBType: Type): &PierPair.Pool{
        IPierPair.IPool
    }?{ 
        let pairHash =
            self.getPairHash(
    ...
    // @return The resource reference of the requested liquidity pool, or nil
    //  if there's no liquidity pool for the token pair
    pub fun getPoolByTypeIdentifiers(
        tokenATypeIdentifier: String,
        tokenBTypeIdentifier: String
    ): &PierPair.Pool{IPierPair.IPool}?{ 
        let pairHash =
    ...
    pub fun getPoolIdByTypeIdentifiers(
        tokenATypeIdentifier: String,
        tokenBTypeIdentifier: String
    ): UInt64?{ 
        let pairHash =
    ...
    pub fun createPoolForPair(
        ...
        newAccount.link<&PierPair.Pool{IPierPair.IPool}>(
            self.SwapPoolPublicPath,
            target: self.SwapPoolStoragePath
        )
