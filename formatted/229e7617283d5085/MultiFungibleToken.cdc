...
pub contract MultiFungibleToken{ 
    ...
    // Resources ---------------------------------------------------------------------------------
    pub resource MultiFungibleTokenManager:
        FungibleToken.Receiver,
        MultiFungibleTokenBalance{
        ...
        pub fun deposit(from: @FungibleToken.Vault){ // deposit takes a Vault and deposits it into the implementing resource type 
            ...
            let ref =
                ((self.owner!).getCapability((ftInfo!).publicPath!)!).borrow<
                    &{FungibleToken.Receiver}
        ...
        access(contract) fun removeDeposit(
            _ identifier: String
        ): @FungibleToken.Vault{ 
            pre{ 
    ...
    pub fun createMissingWalletsAndDeposit(
        _ owner: AuthAccount,
        _ mft: &MultiFungibleTokenManager
    ){ 
    ...
    access(contract) fun getFungibleTokenInfo(
        _ type: Type
        ...
        switch identifier{ 
            /* FUSD   */            case "A.3c5959b568896393.FUSD.Vault":
