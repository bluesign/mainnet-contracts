...
import
    ...
    from "../0xf087790fe77461e4/OlympicPinShardedCollection.cdc"
...
pub contract OlympicPinAdminReceiver{ 
    ...
    init(){ 
        ...
        if self.account.borrow<&OlympicPinShardedCollection.ShardedCollection>(
            from: OlympicPinShardedCollection.ShardedPieceCollectionPath
        )
        == nil{ 
            let collection <-
                OlympicPinShardedCollection.createEmptyCollection(
                    numBuckets: 32
                )
            ...
            self.account.link<&{OlympicPin.PieceCollectionPublic}>(
                OlympicPin.CollectionPublicPath,
                target: OlympicPinShardedCollection.ShardedPieceCollectionPath
            )
        }
    }
}
