...
pub contract ArtistRegistery{ 
    ...
    priv var artists: @{UInt64: Artist}
    
    priv var artistTempVaults: @{UInt64: FUSD.Vault} // temporary vaults when the artists' ones are not available
    ...
    pub resource Artist{ 
        ...
        priv var vault: Capability<&FUSD.Vault{FungibleToken.Receiver}>?
        
        init(name: String){ 
            ArtistRegistery.numberOfArtists = ArtistRegistery.numberOfArtists
                + 1 as UInt64
        ...
        access(contract) fun updateVault(
            vault: Capability<&FUSD.Vault{FungibleToken.Receiver}>
        ){ 
            self.vault = vault
        ...
        pub fun unlockArtistShare(){ 
            let artistTempVault =
                &ArtistRegistery.artistTempVaults[self.id] as &FUSD.Vault
            if self.vault != nil && (self.vault!).check(){ 
                let vaultRef = (self.vault!).borrow()!
    ...
    pub resource interface ManagerClient{ 
        pub fun setArtist(
            artistID: UInt64,
            server: Capability<&Admin{ArtistModifier}>
        ){} 
    }
    ...
    pub resource Manager: ManagerClient{ 
        priv var artistID: UInt64?
        
        priv var server: Capability<&Admin{ArtistModifier}>?
    ...
    init(){ 
        ...
        self.account.link<&Admin{ArtistModifier}>(
