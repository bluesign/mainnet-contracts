...
pub contract BaguetteSale{ 
    ...
    pub resource Collection: CollectionPublic{ 
        ...
        priv var ownerCollection: Capability<&Record.Collection{Record.CollectionPublic, NonFungibleToken.Provider}>
        
        priv var ownerCapability: Capability<&FUSD.Vault{FungibleToken.Receiver}>
        
        priv var prices:{ UInt64: UFix64}
        
        init(parameters: SaleParameters, fVault: Capability<&FUSD.Vault{FungibleToken.Receiver}>, rCollection: Capability<&Record.Collection{Record.CollectionPublic, NonFungibleToken.Provider}>){ 
            pre{ 
                fVault.check():
    ...
    pub resource Admin{ 
        ...
        pub fun setMarketVault(
            marketVault: Capability<&FUSD.Vault{FungibleToken.Receiver}>
        ){ 
            pre{ 
                marketVault.check():
        ...
        pub fun createCustomSaleCollection(
            ...
            ownerNFVault: Capability<
                &Record.Collection{
                    Record.CollectionPublic,
