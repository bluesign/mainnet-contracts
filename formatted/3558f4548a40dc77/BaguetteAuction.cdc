...
pub contract BaguetteAuction{ 
    ...
    priv var marketVault: Capability<&FUSD.Vault{FungibleToken.Receiver}>?
    
    priv var lostFVault: Capability<&FUSD.Vault{FungibleToken.Receiver}>?
    
    priv var lostRCollection: Capability<
        &Record.Collection{Record.CollectionPublic}
    >?
    ...
    pub event FUSDLostAndFound(
        auctionID: UInt64,
        amount: UFix64,
        address: Address
    )
    
    pub event RecordLostAndFound(
        auctionID: UInt64,
        recordID: UInt64,
        address: Address
    )
    ...
    pub resource Auction{ 
        ...
        priv var bidderRCollection: Capability<
            &Record.Collection{Record.CollectionPublic}
        >?
        
        priv let ownerFVault: Capability<&FUSD.Vault{FungibleToken.Receiver}>
        
        priv let ownerRCollection: Capability<
            &Record.Collection{Record.CollectionPublic}
        >
        ...
        init(
            ...
            BaguetteAuction.totalAuctions = BaguetteAuction.totalAuctions
                + 1 as UInt64
        ...
        priv fun sendBidTokens(
            _ capability: Capability<&FUSD.Vault{FungibleToken.Receiver}>
        ){ 
        ...
        priv fun retToOwner(){ 
            self                // deposit the NFT into the owner's collection
                .sendNFT(self.ownerRCollection)
        }
        
        // Extend the auction by the amount of seconds
        priv fun extendWith(_ amount: UFix64){ 
        ...
        pub fun settleAuction(){ 
            ...
            let marketVault =
                (BaguetteAuction.marketVault!).borrow()
                ?? panic("The market vault link is broken.")
    ...
    pub resource Collection: CollectionPublic, AuctionCreator, Bidder{ 
        ...
        priv var auctionItems: @{UInt64: Auction}
        ...
        pub fun createAuction(record: @Record.NFT, startPrice: UFix64, ownerFVault: Capability<&FUSD.Vault{FungibleToken.Receiver}>, ownerRCollection: Capability<&Record.Collection{Record.CollectionPublic}>){ 
            let id = record.id
        ...
        pub fun placeBid(recordID: UInt64, bidTokens: @FungibleToken.Vault, fVault: Capability<&FUSD.Vault{FungibleToken.Receiver}>, rCollection: Capability<&Record.Collection{Record.CollectionPublic}>){ 
            pre{ 
                self.auctionItems[recordID] != nil:
    ...
    pub resource Admin: CollectionCreator{ 
        ...
        pub fun setLostAndFoundVaults(fVault: Capability<&FUSD.Vault{FungibleToken.Receiver}>, rCollection: Capability<&Record.Collection{Record.CollectionPublic}>){ 
            pre{ 
                fVault.check():
    ...
    pub resource Manager: ManagerClient, CollectionCreator{ 
        priv var server: Capability<&Admin{CollectionCreator}>?
    ...
    init(){ 
        ...
        self.account.link<&Admin{CollectionCreator}>(
