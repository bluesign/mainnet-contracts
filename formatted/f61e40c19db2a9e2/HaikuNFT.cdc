...
pub contract HaikuNFT: NonFungibleToken{ 
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            ...
            switch view{ 
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: HaikuNFT.HaikuCollectionStoragePath, publicPath: HaikuNFT.HaikuCollectionPublicPath, providerPath: /private/BitkuCollection, publicCollection: Type<&HaikuNFT.Collection{HaikuNFT.HaikuCollectionPublic}>(), publicLinkedType: Type<&HaikuNFT.Collection{HaikuNFT.HaikuCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&HaikuNFT.Collection{HaikuNFT.HaikuCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Provider, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
    ...
    pub fun mintHaiku(recipient: &NonFungibleToken.Collection, vault: @FungibleToken.Vault, id: UInt64, flowReceiverRef: &FlowToken.Vault{FungibleToken.Receiver}){ 
        ...
        let contractReceiverRef = self.account.getCapability(/public/fusdReceiver).borrow<&{FungibleToken.Receiver}>() ?? panic("Could not borrow reference to contract's fusd receiver")
        ...
        let flowVaultRef = self.account.borrow<&FlowToken.Vault>(from: /storage/flowTokenVault) ?? panic("Could not borrow reference to the contract's FLOW Vault!")
    ...
    pub fun preMintHaikus(num: UInt64): UInt64{ 
        ...
        let collection = self.account.borrow<&NonFungibleToken.Collection>(from: HaikuNFT.HaikuCollectionStoragePath) ?? panic("Could not borrow reference to NFT Collection!")
    ...
    init(){ 
        ...
        self.account.link<&HaikuNFT.Collection{NonFungibleToken.CollectionPublic, HaikuNFT.HaikuCollectionPublic, MetadataViews.ResolverCollection}>(self.HaikuCollectionPublicPath, target: self.HaikuCollectionStoragePath)
