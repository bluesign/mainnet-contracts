...
pub contract PonsUtils{ 
    /* Flow Units struct */    pub struct FlowUnits{ 
        /* Represents the amount of Flow tokens */        pub let flowAmount: UFix64
        
        init(flowAmount: UFix64){ 
            self.flowAmount = flowAmount
        }
        
        /* Check whether the amount is at least the amount of another FlowUnits */        pub fun isAtLeast(_ flowUnits: FlowUnits): Bool{ 
            return self.flowAmount >= flowUnits.flowAmount
        }
        
        /* Make another FlowUnits equivalent to the amount being scaled by a ratio */        pub fun scale(ratio: Ratio): FlowUnits{ 
            return FlowUnits(flowAmount: self.flowAmount * ratio.amount)
        }
        
        /* Make another FlowUnits equivalent to the amount being subtracted by another amount of FlowUnits */        pub fun cut(_ flowUnits: FlowUnits): FlowUnits{ 
            return FlowUnits(flowAmount: self.flowAmount - flowUnits.flowAmount)
        }
        
        /* Produce a string representation in a format like "1234.56 FLOW" */        pub fun toString(): String{ 
            return self.flowAmount.toString().concat(" FLOW")
        }
    }
    
    /* Ratio struct */    pub struct Ratio{ 
        /* Represents the numerical ratio, so that for example 0.1 represents 10%, and 1.0 represents 100% */        pub let amount: UFix64
    ...
    /* Produce a FlowUnits equivalent to the sum of the two separate amounts of FlowUnits */    pub fun sumFlowUnits(
        _ flowUnits1: FlowUnits,
        _ flowUnits2: FlowUnits
    ): FlowUnits{ 
