...
pub contract FlowEpoch{ 
    ...
    pub struct EpochMetadata{ 
        ...
        access(account) fun setRewardAmounts(
            _ rewardBreakdown: [
                FlowIDTableStaking.RewardsBreakdown
            ]
        ){ 
    ...
    pub struct EpochTimingConfig{ 
        ...
        pub fun getTargetEndTimeForEpoch(_ targetEpochCounter: UInt64): UInt64{ 
            return self.refTimestamp
            + self.duration * (targetEpochCounter - self.refCounter)
        }
    ...
    pub fun getEpochMetadata(_ epochCounter: UInt64): EpochMetadata?{ 
        if let metadataDictionary =
            self.account.borrow<&{UInt64: EpochMetadata}>(
                from: self.metadataStoragePath
            ){ 
    ...
    pub resource Admin{ 
        ...
        pub fun updateAuctionViews(_ newAuctionViews: UInt64){ 
            ...
            FlowEpoch.configurableMetadata
                .numViewsInStakingAuction = newAuctionViews
        }
        ...
        pub fun updateEpochTimingConfig(_ newConfig: EpochTimingConfig){ 
            ...
            FlowEpoch.account.load<EpochTimingConfig>(
                from: /storage/flowEpochTimingConfig
            )
            FlowEpoch.account.save(
                newConfig,
                to: /storage/flowEpochTimingConfig
            )
        }
        ...
        pub fun updateFLOWSupplyIncreasePercentage(_ newPercentage: UFix64){ 
            ...
            FlowEpoch.configurableMetadata
                .FLOWsupplyIncreasePercentage = newPercentage
        }
        
        // Enable or disable automatic rewards calculations and payments
        pub fun updateAutomaticRewardsEnabled(_ enabled: Bool){ 
            FlowEpoch.account.load<Bool>(
                from: /storage/flowAutomaticRewardsEnabled
            )
            FlowEpoch.account.save(
                enabled,
                to: /storage/flowAutomaticRewardsEnabled
            )
        }
    ...
    pub resource Heartbeat{ 
        ...
        pub fun advanceBlock(){ 
            switch FlowEpoch.currentEpochPhase{ 
                case EpochPhase.STAKINGAUCTION:
                    ...
                    if currentBlock.view
                    >= currentEpochMetadata.stakingEndView{ 
                        self.endStakingAuction()
                    }
                case EpochPhase.EPOCHSETUP:
                    if FlowClusterQC.votingCompleted()
                    && FlowDKG.dkgCompleted() != nil{ 
        ...
        pub fun endStakingAuction(){ 
            ...
            var randomSource =
                String.encodeHex(randomHigh).concat(String.encodeHex(randomLow))
            assert(
                randomSource.length == 32,
                message: "Random source must be a hex string of 32 characters"
            )
            FlowEpoch.startEpochSetup(
    ...
    access(account) fun startNewEpoch(){ 
        ...
        self.borrowStakingAdmin().moveTokens(
            newEpochCounter: self.proposedEpochCounter()
        )
        ...
        let previousEpochMetadata =
            self.getEpochMetadata(self.currentEpochCounter - 1)!
        ...
        emit EpochStart(
            ...
            totalFlowSupply: FlowToken.totalSupply
            + previousEpochMetadata.totalRewards,
    ...
    access(account) fun startEpochSetup(
        ...
        let collectorClusters =
            self.createCollectorClusters(nodeIDs: collectorNodeIDs)
        ...
        let proposedEpochMetadata =
            EpochMetadata(
                ...
                endView: currentEpochMetadata.endView
                + self.configurableMetadata.numViewsInEpoch,
    ...
    access(account) fun startEpochCommit(){ 
        ...
        // and stop QC voting
        let proposedEpochMetadata =
            self.getEpochMetadata(self.proposedEpochCounter())!
    ...
    access(contract) fun borrowStakingAdmin(): &FlowIDTableStaking.Admin{
        FlowIDTableStaking.EpochOperations
    }{ 
        let adminCapability =
            self.account.copy<Capability>(
                from: /storage/flowStakingAdminEpochOperations
            )
            ?? panic("Could not get capability from account storage")
        
        // borrow a reference to the staking admin object
        let adminRef =
            adminCapability.borrow<
                &FlowIDTableStaking.Admin{FlowIDTableStaking.EpochOperations}
            >()
    ...
    access(contract) fun borrowClusterQCAdmin(): &FlowClusterQC.Admin{
        FlowClusterQC.EpochOperations
    }{ 
        let adminCapability =
            self.account.copy<Capability>(
                from: /storage/flowQCAdminEpochOperations
            )
            ?? panic("Could not get capability from account storage")
        ...
        let adminRef =
            adminCapability.borrow<
                &FlowClusterQC.Admin{FlowClusterQC.EpochOperations}
            >()
    ...
    access(contract) fun borrowDKGAdmin(): &FlowDKG.Admin{
        FlowDKG.EpochOperations
    }{ 
        let adminCapability =
            self.account.copy<Capability>(
                from: /storage/flowDKGAdminEpochOperations
            )
            ?? panic("Could not get capability from account storage")
        
        // borrow a reference to the dkg admin object
        let adminRef =
            adminCapability.borrow<&FlowDKG.Admin{FlowDKG.EpochOperations}>()
    ...
    pub fun isValidPhaseConfiguration(
        _ auctionLen: UInt64,
    ...
    pub fun createCollectorClusters(nodeIDs: [String]): [
        FlowClusterQC.Cluster
    ]{ 
        pre{ 
    ...
    pub fun getClusterQCVoter(
        nodeStaker: &FlowIDTableStaking.NodeStaker
    ): @FlowClusterQC.Voter{ 
        ...
        return <-clusterQCAdmin.createVoter(
    ...
    pub fun getEpochTimingConfig(): EpochTimingConfig{ 
        return self.account.copy<EpochTimingConfig>(
            from: /storage/flowEpochTimingConfig
        )!
    }
    ...
    init(
        ...
        self.account.save<
            Capability<
                &FlowIDTableStaking.Admin{FlowIDTableStaking.EpochOperations}
            >
        >(stakingAdminCapability, to: /storage/flowStakingAdminEpochOperations)
        
        // Create a private capability to the qc admin
        // and store it in a different path
        let qcAdminCapability =
            self.account.link<
                &FlowClusterQC.Admin{FlowClusterQC.EpochOperations}
            >(
        ...
        self.account.save<
            Capability<&FlowClusterQC.Admin{FlowClusterQC.EpochOperations}>
        >(qcAdminCapability, to: /storage/flowQCAdminEpochOperations)
        
        // Create a private capability to the dkg admin
        // and store it in a different path
        let dkgAdminCapability =
            self.account.link<&FlowDKG.Admin{FlowDKG.EpochOperations}>(
                /private/flowDKGAdminEpochOperations,
                target: FlowDKG.AdminStoragePath
            )
            ?? panic("Could not link Flow DKG admin capability")
        self.account.save<Capability<&FlowDKG.Admin{FlowDKG.EpochOperations}>>(
            dkgAdminCapability,
