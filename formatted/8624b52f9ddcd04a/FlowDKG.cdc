...
pub contract FlowDKG{ 
    ...
    pub resource Participant{ 
        ...
        pub fun sendFinalSubmission(_ submission: [String?]){ 
            ...
            while finalSubmissionIndex
            <= FlowDKG.uniqueFinalSubmissions.length{ 
                // If no matches were found, add this submission as a new unique one
                // and emit an event
                if finalSubmissionIndex
                == FlowDKG.uniqueFinalSubmissions.length{ 
                ...
                let existingSubmission =
                    FlowDKG.uniqueFinalSubmissions[finalSubmissionIndex]
    ...
    access(account) fun submissionsEqual(
        _ existingSubmission: [
            String?
        ],
        _ submission: [
            String?
    ...
    pub fun getSafeThresholdPercentage(): UFix64?{ 
        let safetyRate =
            self.account.copy<UFix64>(from: /storage/flowDKGSafeThreshold)
        return safetyRate
    }
