...
pub contract FlowIDTableStaking{ 
    /****** ID Table and Staking Events ******/    pub event NewEpoch(
        totalStaked: UFix64,
        totalRewardPayout: UFix64,
        newEpochCounter: UInt64
    )
    
    pub event EpochTotalRewardsPaid(
        total: UFix64,
        fromFees: UFix64,
        minted: UFix64,
        feesBurned: UFix64,
    ...
    pub event DelegatorTokensCommitted(
        nodeID: String,
        delegatorID: UInt32,
        amount: UFix64
    )
    
    pub event DelegatorTokensStaked(
        nodeID: String,
        delegatorID: UInt32,
        amount: UFix64
    )
    
    pub event DelegatorTokensRequestedToUnstake(
        nodeID: String,
        delegatorID: UInt32,
        amount: UFix64
    )
    
    pub event DelegatorTokensUnstaking(
        nodeID: String,
        delegatorID: UInt32,
        amount: UFix64
    )
    
    pub event DelegatorTokensUnstaked(
        nodeID: String,
        delegatorID: UInt32,
        amount: UFix64
    )
    ...
    pub resource NodeRecord{ 
        ...
        destroy(){ 
            ...
            FlowIDTableStaking.totalTokensStakedByNodeType[
                self.role
            ] = FlowIDTableStaking.totalTokensStakedByNodeType[self.role]!
                - self.tokensStaked.balance
        ...
        access(account) fun borrowDelegatorRecord(
        ...
        access(account) fun setDelegator(
            delegatorID: UInt32,
            delegator: @DelegatorRecord
        ){ 
    ...
    pub struct NodeInfo{ 
        ...
        pub fun totalTokensInRecord(): UFix64{ 
            return self.tokensStaked + self.tokensCommitted
    ...
    pub resource NodeStaker: NodeStakerPublic{ 
        ...
        priv fun isEligibleForCandidateNodeStatus(_ nodeRecord: &FlowIDTableStaking.NodeRecord): Bool{ 
    ...
    pub struct RewardsBreakdown{ 
        ...
        pub fun scaleDelegatorRewards(
            delegatorID: UInt32,
            scalingFactor: UFix64
        ){ 
    ...
    pub resource interface EpochOperations{ 
        ...
        pub fun payRewards(
            forEpochCounter: UInt64,
            rewardsSummary: EpochRewardsSummary
        ){} 
    ...
    pub resource Admin: EpochOperations{ 
        ...
        priv fun unsafeSetApprovedList(_ newApproveList:{ String: Bool}){ 
            let currentApproveList = FlowIDTableStaking.account.load<{String: Bool}>(from: /storage/idTableApproveList) ?? panic("Could not load the current approve list from storage")
        ...
        priv fun unsafeRemoveAndRefundNodeRecord(_ nodeID: String){ 
            let nodeRecord = FlowIDTableStaking.borrowNodeRecord(nodeID)
            ...
            var movesPendingList = FlowIDTableStaking.account.borrow<&{String:{ UInt32: Bool}}>(from: /storage/idTableMovesPendingList) ?? panic("No moves pending list in account storage")
        ...
        pub fun payRewards(forEpochCounter: UInt64, rewardsSummary: EpochRewardsSummary){ 
            ...
            if mintedRewards > 0.0{ 
                let flowTokenMinter = FlowIDTableStaking.account.borrow<&FlowToken.Minter>(from: /storage/flowTokenMinter) ?? panic("Could not borrow minter reference")
        ...
        pub fun moveTokens(newEpochCounter: UInt64){ 
            ...
            let approvedNodeIDs = FlowIDTableStaking.getApprovedList() ?? panic("Could not read the approve list from storage")
            let movesPendingNodeIDs = FlowIDTableStaking.account.load<{String:{ UInt32: Bool}}>(from: /storage/idTableMovesPendingList) ?? panic("No moves pending list in account storage")
            ...
            let newMovesPendingList = FlowIDTableStaking.account.borrow<&{String:{ UInt32: Bool}}>(from: /storage/idTableMovesPendingList) ?? panic("No moves pending list in account storage")
            let stakedNodeIDs:{ String: Bool} = FlowIDTableStaking.getParticipantNodeList()!
    ...
    pub fun addNodeRecord(
        ...
        assert(
            self.isGreaterThanMinimumForRole(
                numTokens: tokensCommitted.balance,
                role: role
            ),
            message: "Tokens committed for registration is not above the minimum ("
                .concat(minimum.toString()).concat(
                ") for the chosen node role (".concat(role.toString()).concat(
                    ")"
                )
            )
        )
    ...
    access(account) fun borrowFeesAdmin(): &FlowFees.Administrator{ 
        let feesAdmin =
            self.account.borrow<&FlowFees.Administrator>(
                from: /storage/flowFeesAdmin
            )
            ?? panic(
                "Could not borrow a reference to the FlowFees Admin object"
            )
        return feesAdmin
    }
    ...
    access(account) fun updateClaimed(
        path: StoragePath,
        _ key: String,
        claimed: Bool
    ){ 
    ...
    access(contract) fun setParticipantNodeList(_ nodeIDs:{ String: Bool}){ 
        let list =
            self.account.load<{String: Bool}>(from: /storage/idTableCurrentList)
        self.account.save<{String: Bool}>(
            nodeIDs,
            to: /storage/idTableCurrentList
        )
    }
    ...
    pub fun getParticipantNodeList():{ String: Bool}?{ 
        return self.account.copy<{String: Bool}>(
            from: /storage/idTableCurrentList
        )
    }
    ...
    access(contract) fun modifyNewMovesPending(
        nodeID: String,
        delegatorID: UInt32?,
        existingList: &{
            String:{ 
        ...
        let movesPendingList =
            existingList
            ?? self.account.borrow<&{String:{ UInt32: Bool}}>(
    ...
    pub fun getMovesPendingList():{ String:{ UInt32: Bool}}?{ 
        return self.account.copy<{String:{ UInt32: Bool}}>(
            from: /storage/idTableMovesPendingList
        )
    }
    ...
    pub fun getCandidateNodeLimits():{ UInt8: UInt64}?{ 
        return self.account.copy<{UInt8: UInt64}>(
            from: /storage/idTableCandidateNodeLimits
        )
    }
    
    /// Adds the provided node ID to the candidate node list
    access(contract) fun addToCandidateNodeList(
        nodeID: String,
        roleToAdd: UInt8
    ){ 
        pre{ 
        ...
        var candidateNodes =
            FlowIDTableStaking.account.borrow<&{UInt8:{ String: Bool}}>(
        ...
        >= (self.getCandidateNodeLimits()!)[roleToAdd]!{ 
            panic(
                "Candidate node limit exceeded for node role ".concat(
                    roleToAdd.toString()
                )
            )
        }
    ...
    access(contract) fun removeFromCandidateNodeList(
        nodeID: String,
        role: UInt8
    ){ 
        pre{ 
    ...
    pub fun getCandidateNodeList():{ UInt8:{ String: Bool}}{ 
        return FlowIDTableStaking.account.copy<{UInt8:{ String: Bool}}>(
            from: /storage/idTableCandidateNodes
        )
        ??{ 1:{} , 2:{} , 3:{} , 4:{} , 5:{} }
    }
    ...
    pub fun getCurrentRoleNodeCounts():{ UInt8: UInt16}{ 
        if let currentCounts =
            FlowIDTableStaking.account.copy<{UInt8: UInt16}>(
    ...
    pub fun isGreaterThanMinimumForRole(numTokens: UFix64, role: UInt8): Bool{ 
        let minimumStake =
            ...
            ?? panic(
                "Incorrect role provided for minimum stake. Must be 1, 2, 3, 4, or 5"
            )
    ...
    pub fun getApprovedList():{ String: Bool}?{ 
        return self.account.copy<{String: Bool}>(
            from: /storage/idTableApproveList
        )
    }
    ...
    pub fun getNonOperationalNodesList():{ String: UFix64}{ 
        return self.account.copy<{String: UFix64}>(
    ...
    init(
        _ epochTokenPayout: UFix64,
        _ rewardCut: UFix64,
        _ candidateNodeLimits:{ 
            UInt8: UInt64
        }
    ){ 
        ...
        self.account.save(
            claimedDictionary,
            to: /storage/networkingAddressesClaimed
        )
