...
pub contract GeniacePacks{ 
    ...
    pub fun purchase(
        ...
        (         
         ...
         residualReceiver!).deposit(
            from: <-paymentVault.withdraw(amount: paymentVault.balance)
        )
    ...
    pub resource interface collectionCapabilityPublic{ 
        pub fun setCollectionCapability(
    ...
    pub resource interface collectionCapabilityManager{ 
        pub fun getCollectionCapability(collectionOwner: Address): &{
    ...
    pub resource collectionCapabilityHolder:
        collectionCapabilityPublic,
        collectionCapabilityManager{
    
        // This dictionary variable will hold the address-capabilty pair
        priv var collectionCapabilityList:{ 
            Address: Capability<
                &{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}
            >
        }
        ...
        pub fun getCollectionCapability(collectionOwner: Address): &{
            ...
            let ref =
                (self.collectionCapabilityList[collectionOwner]!).borrow()!
            return ref
        }
