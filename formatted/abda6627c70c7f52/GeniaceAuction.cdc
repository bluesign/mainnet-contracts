...
pub contract GeniaceAuction{ 
    ...
    pub resource AuctionItem{ 
        ...
        priv var recipientCollectionCap: Capability<
            &GeniaceNFT.Collection{NonFungibleToken.Receiver}
        >?
        ...
        init(
            ...
            GeniaceAuction.totalAuctions = GeniaceAuction.totalAuctions
                + 1 as UInt64
            self.NFT <- NFT
        ...
        pub fun settleAuction(){ 
            ...
            assert(
                ...
                message: "No valid payment receivers"
            )
            (             
    ...
    pub resource AuctionCollection: AuctionPublic{ 
        ...
        pub fun createAuction(token: @GeniaceNFT.NFT, currencyType: Type, minimumBidIncrement: UFix64                                                                                                     
                                                                                                     // placeBid sends the bidder's tokens to the bid vault and updates the
                                                                                                     // currentPrice of the current auction item
                                                                                                     , auctionLength: UFix64, auctionStartTime: UFix64, startPrice: UFix64                                                                                                                                                                          
                                                                                                                                                                          // Get the auction item resources
                                                                                                                                                                          , buyNowPrice: UFix64, reservePrice: UFix64                                                                                                                                                                                                                     // destroy the empty resources
                                                                                                                                                                                                                     ...
                                                                                                                                                                                                                     , collectionCap: Capability<&GeniaceNFT.Collection{NonFungibleToken.Receiver}>, saleCuts: [SaleCut]){ 
            let                
                // create a new auction items resource container
                item <- GeniaceAuction                                      // createAuctionCollection returns a new AuctionCollection resource to the caller
                                      .createStandaloneAuction(token: <-token, currencyType: currencyType, minimumBidIncrement: minimumBidIncrement, auctionLength: auctionLength, auctionStartTime: auctionStartTime, startPrice: startPrice, buyNowPrice: buyNowPrice, reservePrice: reservePrice, collectionCap: collectionCap, saleCuts: saleCuts)
        ...
        pub fun buyNow(id: UInt64, tokens: @FungibleToken.Vault, collectionCap: Capability<&GeniaceNFT.Collection{NonFungibleToken.Receiver}>){ 
            let itemRef = (&self.auctionItems[id] as &AuctionItem?)!
        ...
        pub fun placeBid(id: UInt64, bidTokens: @FungibleToken.Vault, vaultCap: Capability<&{FungibleToken.Receiver}>, collectionCap: Capability<&GeniaceNFT.Collection{NonFungibleToken.Receiver}>){ 
            pre{ 
                self.auctionItems[id] != nil:
