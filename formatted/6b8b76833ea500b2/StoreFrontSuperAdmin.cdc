...
pub contract StoreFrontSuperAdmin{ 
    ...
    pub resource interface ISuperAdminStoreFrontPublic{ 
        pub fun getStoreFrontPublic(): &NFTStorefront.Storefront{
    ...
    pub resource interface AdminTokenReceiverPublic{ 
        ...
        pub fun getSuperAdminRefPublic(): &SuperAdmin{
            ISuperAdminStoreFrontPublic
        }?{} 
    }
    
    pub resource AdminTokenReceiver: AdminTokenReceiverPublic{ 
        priv var adminRef: Capability<&StoreFront.Admin>?
        
        priv var superAdminRef: Capability<&SuperAdmin>?
    ...
    pub fun createSuperAdmin(
        ...
        let capabilitySuperAdmin =
            self.account.link<&SuperAdmin>(
                storeFrontSuperAdminPrivatePath,
                target: self.storeFrontSuperAdminStoragePath
            )!
        let capabilityAdmin =
            self.account.link<&StoreFront.Admin>(
                storeFrontAdminPrivatePath,
                target: self.storeFrontAdminStoragePath
            )!
        (adminTokenReceiver.borrow()!).receiveSuperAdmin(
            superAdminRef: capabilitySuperAdmin
        )
    ...
    init(requestedAdress: Address){ 
        // Paths
        self
            .storeFrontAdminReceiverPublicPath = /public/AdminTokenReceiver0x6b8b76833ea500b2
        self
            .storeFrontAdminReceiverStoragePath = /storage/AdminTokenReceiver0x6b8b76833ea500b2
        ...
        self.account.save<@SuperAdmin>(
            <-superAdmin,
            to: self.storeFrontSuperAdminStoragePath
        )
        self.account.save<@StoreFront.Admin>(
            <-admin,
            to: self.storeFrontAdminStoragePath
        )
