...
pub contract TokenManager{ 
    ...
    pub resource LockedVault: Public{ 
        ...
        access(account) fun distribute(to: Address, amount: UFix64){ 
            let recipientVault = getAccount(to).getCapability(TokenManager.VaultPublicPath).borrow<&LockedVault{Public}>() ?? panic("This user does not have a vault set up.")
            let tokens <- self.tokens.withdraw(amount: amount) as! @Rumble.Vault
    ...
    pub fun checkUserDepositStatusIsValid(user: Address, amount: UFix64): Bool{ 
        let userVault =
            getAccount(user).getCapability(TokenManager.VaultPublicPath).borrow<
                &LockedVault{Public}
            >()
