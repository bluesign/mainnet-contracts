...
pub contract ArtworkNFTsMarket{ 
    ...
    pub resource SaleOffer: SaleOfferPublicView{ 
        ...
        priv let sellerItemProvider: Capability<&ArtworkNFTs.Collection{NonFungibleToken.Provider}>
        
        // The PigCoin vault that will receive that payment if teh sale completes successfully.
        priv let sellerPaymentReceiver: Capability<&PigCoin.Vault{FungibleToken.Receiver}>
        ...
        //
        pub fun accept(buyerCollection: &ArtworkNFTs.Collection{NonFungibleToken.Receiver}, buyerPayment: @FungibleToken.Vault){ 
            pre{ 
        ...
        //
        init(sellerItemProvider: Capability<&ArtworkNFTs.Collection{NonFungibleToken.Provider}>, itemID: UInt64, typeID: UInt64, hashCode: String, sellerPaymentReceiver: Capability<&PigCoin.Vault{FungibleToken.Receiver}>, price: UFix64){ 
            pre{ 
    ...
    pub fun createSaleOffer(
        sellerItemProvider: Capability<
            &ArtworkNFTs.Collection{NonFungibleToken.Provider}
        >,
        itemID: UInt64,
        typeID: UInt64,
        hashCode: String,
        sellerPaymentReceiver: Capability<
            &PigCoin.Vault{FungibleToken.Receiver}
        >,
        price: UFix64
    ): @SaleOffer{ 
