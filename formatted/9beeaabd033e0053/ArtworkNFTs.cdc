...
pub contract ArtworkNFTs: NonFungibleToken{ 
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &ArtworkNFTs.NFT?{ 
        let collection = (getAccount(from).getCapability(ArtworkNFTs.CollectionPublicPath)!).borrow<&ArtworkNFTs.Collection{ArtworkNFTs.ArtworkNFTsCollectionPublic}>() ?? panic("Couldn't get collection")
