...
pub contract BSMarket{ 
    ...
    pub resource SaleOffer: SaleOfferPublicView{ 
        ...
        priv let sellerItemProvider: Capability<&BSListings.Collection{BSListings.BSListingsCollectionPublic, NonFungibleToken.Provider}>
        
        // The sale payment price.
        pub let salePrice: UFix64
        
        // The DevryCoin vault that will receive that payment if the sale completes successfully.
        priv let sellerPaymentReceiver: Capability<&DevryCoin.Vault{FungibleToken.Receiver}>
        
        priv let beneficiaries:{ Address: UFix64}
        ...
        //
        pub fun accept(buyerCollection: &BSListings.Collection{NonFungibleToken.Receiver}, buyerPayment: @FungibleToken.Vault){ 
            pre{ 
            ...
            for beneficiary in self.beneficiaries.keys{ 
                ...
                let beneficiaryRef = getAccount(beneficiary).getCapability(DevryCoin.ReceiverPublicPath).borrow<&{FungibleToken.Receiver}>()
        ...
        //
        init(sellerItemProvider: Capability<&BSListings.Collection{BSListings.BSListingsCollectionPublic, NonFungibleToken.Provider}>, saleNFTID: UInt64, sellerPaymentReceiver: Capability<&DevryCoin.Vault{FungibleToken.Receiver}>, salePrice: UFix64, beneficiaries:{ Address: UFix64}){ 
            pre{ 
    ...
    pub fun createSaleOffer(
        sellerItemProvider: Capability<
            &BSListings.Collection{
                BSListings.BSListingsCollectionPublic,
                NonFungibleToken.Provider
            }
        >,
        saleNFTID: UInt64,
        sellerPaymentReceiver: Capability<
            &DevryCoin.Vault{FungibleToken.Receiver}
        >,
        salePrice: UFix64,
        beneficiaries:{ 
            Address: UFix64
    ...
    pub resource interface CollectionPublic{ 
        ...
        pub fun borrowSaleItem(saleNFTID: UInt64): &SaleOffer{
            SaleOfferPublicView
        }?{} 
    ...
    pub resource Collection:
        ...
        pub fun remove(saleNFTID: UInt64): @SaleOffer{ 
            ...
            return <-(
                self.saleOffers.remove(key: saleNFTID)
