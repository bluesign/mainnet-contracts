...
pub contract BSListings: NonFungibleToken{ 
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &BSListings.NFT?{ 
        let collection = getAccount(from).getCapability(BSListings.CollectionPublicPath).borrow<&BSListings.Collection{BSListings.BSListingsCollectionPublic}>() ?? panic("Couldn't get collection")
