...
pub contract Piece: NonFungibleToken, ViewResolver{ 
    ...
    pub struct NFTMetadata{ 
        ...
        access(account) fun createUsername(): String{ 
            var username = ""
            return ""
        }
        
        init(_creatorID: UInt64, _creatorUsername: String, _creatorAddress: Address, _description: String, _image: MetadataViews.HTTPFile, _extra:{ String: AnyStruct}, _currentTime: UFix64, _embededHTML: String){ 
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            ...
            switch view{ 
                case Type<MetadataViews.Display>():
                    return MetadataViews.Display(creatorID: metadata.creatorID.toString(), description: metadata.description, thumbnail: metadata.image)
    ...
    pub resource Administrator{ 
        ...
        pub fun createNFTMetadata(channel: String, creatorID: UInt64, creatorUsername: String, creatorAddress: Address, sourceURL: String, description: String, pieceCreationDate: String, contentCreationDate: String, imgUrl: String, embededHTML: String){ 
            let metadatas <- Piece.account.load<@Piece.MetadataStorage>(from: Piece.MetadataStoragePath)!
            // Check if Metadata already exist
            if metadatas.metadataIsNew(creatorID, description){ 
                metadatas.addMetadata(creatorID, NFTMetadata(_creatorID: creatorID, _creatorUsername: creatorUsername, _creatorAddress: creatorAddress, _description: description, _image: MetadataViews.HTTPFile(url: imgUrl), _extra:{ "Channel": channel, "CreatorID": creatorID, "Source": sourceURL, "Text content": description, "Piece creation date": pieceCreationDate, "Content creation date": contentCreationDate}, _currentTime: getCurrentBlock().timestamp, _embededHTML: embededHTML))
        ...
        pub fun mintNFT(creatorId: UInt64, description: String, recipient: Address){ 
            ...
            } else if let storage = &Piece.nftStorage[recipient] as &{UInt64: NFT}?{ 
                storage[nft.id] <-! nft
            } else{ 
    ...
    pub fun resolveView(_ view: Type): AnyStruct?{ 
        switch view{ 
            case Type<MetadataViews.NFTCollectionData>():
                return MetadataViews.NFTCollectionData(storagePath: Piece.CollectionStoragePath, publicPath: Piece.CollectionPublicPath, providerPath: Piece.CollectionPrivatePath, publicCollection: Type<&Piece.Collection{Piece.PieceCollectionPublic}>(), publicLinkedType: Type<&Piece.Collection{Piece.PieceCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&Piece.Collection{Piece.PieceCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Provider, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
