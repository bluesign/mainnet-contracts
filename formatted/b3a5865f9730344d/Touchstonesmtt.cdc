...
pub contract Touchstonesmtt: NonFungibleToken{ 
    ...
    pub struct NFTMetadata{ 
        ...
        init(_name: String, _description: String, _image: MetadataViews.IPFSFile, _thumbnail: MetadataViews.IPFSFile?, _price: UFix64?, _extra:{ String: AnyStruct}){ 
            self.metadataId = Touchstonesmtt.nextMetadataId
            self.name = _name
            ...
            self.price = _price
            self.extra = _extra
            Touchstonesmtt.nextMetadataId = Touchstonesmtt.nextMetadataId + 1
        }
    }
    ...
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver{ 
        ...
        pub fun getMetadata(): NFTMetadata{ 
            return Touchstonesmtt.getNFTMetadata(self.metadataId)!
        }
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: Touchstonesmtt.CollectionStoragePath, publicPath: Touchstonesmtt.CollectionPublicPath, providerPath: Touchstonesmtt.CollectionPrivatePath, publicCollection: Type<&Collection{NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), publicLinkedType: Type<&Collection{NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&Collection{NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection, NonFungibleToken.Provider}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
                            return <-Touchstonesmtt.createEmptyCollection()
                        })
                case Type<MetadataViews.ExternalURL>():
                    return MetadataViews.ExternalURL("https://touchstone.city/discover/".concat((self.owner!).address.toString()).concat("/Touchstonesmtt"))
                case Type<MetadataViews.NFTCollectionDisplay>():
                    let squareMedia = MetadataViews.Media(file: Touchstonesmtt.getCollectionAttribute(key: "image") as! MetadataViews.IPFSFile, mediaType: "image")
                    
                    // If a banner image exists, use it
                    // Otherwise, default to the main square image
                    var bannerMedia: MetadataViews.Media? = nil
                    if let bannerImage = Touchstonesmtt.getOptionalCollectionAttribute(key: "bannerImage") as! MetadataViews.IPFSFile?{ 
                        bannerMedia = MetadataViews.Media(file: bannerImage, mediaType: "image")
                    }
                    return MetadataViews.NFTCollectionDisplay(name: Touchstonesmtt.getCollectionAttribute(key: "name") as! String, description: Touchstonesmtt.getCollectionAttribute(key: "description") as! String, externalURL: MetadataViews.ExternalURL("https://touchstone.city/discover/".concat((self.owner!).address.toString()).concat("/Touchstonesmtt")), squareImage: squareMedia, bannerImage: bannerMedia ?? squareMedia, socials: Touchstonesmtt.getCollectionAttribute(key: "socials") as!{ String: MetadataViews.ExternalURL})
        ...
        init(_metadataId: UInt64, _recipient: Address){ 
            pre{ 
                Touchstonesmtt.metadatas[_metadataId] != nil:
                    "This NFT does not exist yet."
                !Touchstonesmtt.primaryBuyers.containsKey(_metadataId):
                    "This NFT has already been minted."
            }
            self.id = self.uuid
            self.metadataId = _metadataId
            Touchstonesmtt.primaryBuyers[_metadataId] = _recipient
            Touchstonesmtt.totalSupply = Touchstonesmtt.totalSupply + 1
    ...
    pub resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection{ 
        ...
        pub fun claim(){ 
            if let storage = &Touchstonesmtt.nftStorage[(self.owner!).address] as &{UInt64: NFT}?{ 
    ...
    pub fun mintNFT(metadataId: UInt64, recipient: &{NonFungibleToken.Receiver}, payment: @FlowToken.Vault): UInt64{ 
        ...
        if let royalty = Touchstonesmtt.getOptionalCollectionAttribute(key: "royalty") as! MetadataViews.Royalty?{ 
    ...
    pub resource Administrator{ 
        pub fun createNFTMetadata(name: String, description: String, imagePath: String, thumbnailPath: String?, ipfsCID: String, price: UFix64?, extra:{ String: AnyStruct}){ 
            Touchstonesmtt.metadatas[Touchstonesmtt.nextMetadataId] = NFTMetadata(_name: name, _description: description, _image: MetadataViews.IPFSFile(cid: ipfsCID, path: imagePath), _thumbnail: thumbnailPath == nil ? nil : MetadataViews.IPFSFile(cid: ipfsCID, path: thumbnailPath), _price: price, _extra: extra)
        }
        ...
        pub fun mintNFT(metadataId: UInt64, recipient: Address){ 
            pre{ 
                EmeraldPass.isActive(user: Touchstonesmtt.account.address):
            ...
            if let recipientCollection = getAccount(recipient).getCapability(Touchstonesmtt.CollectionPublicPath).borrow<&Touchstonesmtt.Collection{NonFungibleToken.CollectionPublic}>(){ 
                recipientCollection.deposit(token: <-nft)
            } else if let storage = &Touchstonesmtt.nftStorage[recipient] as &{UInt64: NFT}?{ 
                storage[nft.id] <-! nft
            } else{ 
                Touchstonesmtt.nftStorage[recipient] <-!{ nft.id: <-nft}
            }
        }
        ...
        pub fun changeField(key: String, value: AnyStruct){ 
            Touchstonesmtt.collectionInfo[key] = value
        }
    }
    ...
    pub fun getPriceOfNFT(_ metadataId: UInt64): UFix64?{ 
        if let metadata: Touchstonesmtt.NFTMetadata = self.getNFTMetadata(metadataId){ 
    ...
    pub fun getClaimableNFTs(user: Address):{ UInt64: NFTMetadata}{ 
        ...
        if let storage = &Touchstonesmtt.nftStorage[user] as &{UInt64: NFT}?{ 
            for id in storage.keys{ 
    ...
    init(_name: String, _description: String, _imagePath: String, _bannerImagePath: String?, _minting: Bool, _royalty: MetadataViews.Royalty?, _defaultPrice: UFix64, _paymentType: String, _ipfsCID: String, _lotteryBuying: Bool, _socials:{ String: MetadataViews.ExternalURL}, _mintVerifiers: [{MintVerifiers.IVerifier}]){ 
        ...
        self.CollectionStoragePath = /storage/TouchstonesmttCollection_0xb3a5865f9730344d
        self.CollectionPublicPath = /public/TouchstonesmttCollection_0xb3a5865f9730344d
        self.CollectionPrivatePath = /private/TouchstonesmttCollection_0xb3a5865f9730344d
        self.AdministratorStoragePath = /storage/TouchstonesmttAdministrator_0xb3a5865f9730344d
