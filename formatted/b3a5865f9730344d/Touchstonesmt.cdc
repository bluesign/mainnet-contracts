...
pub contract Touchstonesmt: NonFungibleToken{ 
    ...
    pub struct NFTMetadata{ 
        ...
        init(_name: String, _description: String, _image: MetadataViews.IPFSFile, _thumbnail: MetadataViews.IPFSFile?, _price: UFix64?, _extra:{ String: AnyStruct}){ 
            self.metadataId = Touchstonesmt.nextMetadataId
            self.name = _name
            ...
            self.price = _price
            self.extra = _extra
            Touchstonesmt.nextMetadataId = Touchstonesmt.nextMetadataId + 1
        }
    }
    ...
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver{ 
        ...
        pub fun getMetadata(): NFTMetadata{ 
            return Touchstonesmt.getNFTMetadata(self.metadataId)!
        }
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: Touchstonesmt.CollectionStoragePath, publicPath: Touchstonesmt.CollectionPublicPath, providerPath: Touchstonesmt.CollectionPrivatePath, publicCollection: Type<&Collection{NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), publicLinkedType: Type<&Collection{NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&Collection{NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection, NonFungibleToken.Provider}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
                            return <-Touchstonesmt.createEmptyCollection()
                        })
                case Type<MetadataViews.ExternalURL>():
                    return MetadataViews.ExternalURL("https://touchstone.city/discover/".concat((self.owner!).address.toString()).concat("/Touchstonesmt"))
                case Type<MetadataViews.NFTCollectionDisplay>():
                    let squareMedia = MetadataViews.Media(file: Touchstonesmt.getCollectionAttribute(key: "image") as! MetadataViews.IPFSFile, mediaType: "image")
                    
                    // If a banner image exists, use it
                    // Otherwise, default to the main square image
                    var bannerMedia: MetadataViews.Media? = nil
                    if let bannerImage = Touchstonesmt.getOptionalCollectionAttribute(key: "bannerImage") as! MetadataViews.IPFSFile?{ 
                        bannerMedia = MetadataViews.Media(file: bannerImage, mediaType: "image")
                    }
                    return MetadataViews.NFTCollectionDisplay(name: Touchstonesmt.getCollectionAttribute(key: "name") as! String, description: Touchstonesmt.getCollectionAttribute(key: "description") as! String, externalURL: MetadataViews.ExternalURL("https://touchstone.city/discover/".concat((self.owner!).address.toString()).concat("/Touchstonesmt")), squareImage: squareMedia, bannerImage: bannerMedia ?? squareMedia, socials: Touchstonesmt.getCollectionAttribute(key: "socials") as!{ String: MetadataViews.ExternalURL})
        ...
        init(_metadataId: UInt64, _recipient: Address){ 
            pre{ 
                Touchstonesmt.metadatas[_metadataId] != nil:
                    "This NFT does not exist yet."
                !Touchstonesmt.primaryBuyers.containsKey(_metadataId):
                    "This NFT has already been minted."
            }
            self.id = self.uuid
            self.metadataId = _metadataId
            Touchstonesmt.primaryBuyers[_metadataId] = _recipient
            Touchstonesmt.totalSupply = Touchstonesmt.totalSupply + 1
    ...
    pub resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection{ 
        ...
        pub fun claim(){ 
            if let storage = &Touchstonesmt.nftStorage[(self.owner!).address] as &{UInt64: NFT}?{ 
    ...
    pub fun mintNFT(metadataId: UInt64, recipient: &{NonFungibleToken.Receiver}, payment: @FlowToken.Vault): UInt64{ 
        ...
        if let royalty = Touchstonesmt.getOptionalCollectionAttribute(key: "royalty") as! MetadataViews.Royalty?{ 
    ...
    pub resource Administrator{ 
        pub fun createNFTMetadata(name: String, description: String, imagePath: String, thumbnailPath: String?, ipfsCID: String, price: UFix64?, extra:{ String: AnyStruct}){ 
            Touchstonesmt.metadatas[Touchstonesmt.nextMetadataId] = NFTMetadata(_name: name, _description: description, _image: MetadataViews.IPFSFile(cid: ipfsCID, path: imagePath), _thumbnail: thumbnailPath == nil ? nil : MetadataViews.IPFSFile(cid: ipfsCID, path: thumbnailPath), _price: price, _extra: extra)
        }
        ...
        pub fun mintNFT(metadataId: UInt64, recipient: Address){ 
            pre{ 
                EmeraldPass.isActive(user: Touchstonesmt.account.address):
            ...
            if let recipientCollection = getAccount(recipient).getCapability(Touchstonesmt.CollectionPublicPath).borrow<&Touchstonesmt.Collection{NonFungibleToken.CollectionPublic}>(){ 
                recipientCollection.deposit(token: <-nft)
            } else if let storage = &Touchstonesmt.nftStorage[recipient] as &{UInt64: NFT}?{ 
                storage[nft.id] <-! nft
            } else{ 
                Touchstonesmt.nftStorage[recipient] <-!{ nft.id: <-nft}
            }
        }
        ...
        pub fun changeField(key: String, value: AnyStruct){ 
            Touchstonesmt.collectionInfo[key] = value
        }
    }
    ...
    pub fun getPriceOfNFT(_ metadataId: UInt64): UFix64?{ 
        if let metadata: Touchstonesmt.NFTMetadata = self.getNFTMetadata(metadataId){ 
    ...
    pub fun getClaimableNFTs(user: Address):{ UInt64: NFTMetadata}{ 
        ...
        if let storage = &Touchstonesmt.nftStorage[user] as &{UInt64: NFT}?{ 
            for id in storage.keys{ 
    ...
    init(_name: String, _description: String, _imagePath: String, _bannerImagePath: String?, _minting: Bool, _royalty: MetadataViews.Royalty?, _defaultPrice: UFix64, _paymentType: String, _ipfsCID: String, _lotteryBuying: Bool, _socials:{ String: MetadataViews.ExternalURL}, _mintVerifiers: [{MintVerifiers.IVerifier}]){ 
        ...
        self.CollectionStoragePath = /storage/TouchstonesmtCollection_0xb3a5865f9730344d
        self.CollectionPublicPath = /public/TouchstonesmtCollection_0xb3a5865f9730344d
        self.CollectionPrivatePath = /private/TouchstonesmtCollection_0xb3a5865f9730344d
        self.AdministratorStoragePath = /storage/TouchstonesmtAdministrator_0xb3a5865f9730344d
