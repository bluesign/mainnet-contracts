...
import TheFabricantS1ItemNFT from "./TheFabricantS1ItemNFT.cdc"

import
    TheFabricantS2GarmentNFT
    from "../0x7752ea736384322f/TheFabricantS2GarmentNFT.cdc"

import
    TheFabricantS2MaterialNFT
    from "../0x7752ea736384322f/TheFabricantS2MaterialNFT.cdc"

import
    TheFabricantS2ItemNFT
    from "../0x7752ea736384322f/TheFabricantS2ItemNFT.cdc"

import TheFabricantMarketplace from "./TheFabricantMarketplace.cdc"

import MetadataViews from "../0x1d7e57aa55817448/MetadataViews.cdc"

import
    TheFabricantAccessPass
    from "../0x7752ea736384322f/TheFabricantAccessPass.cdc"

import TheFabricantXXories from "../0x7752ea736384322f/TheFabricantXXories.cdc"

import
    TheFabricantNFTStandard
    from "../0x7752ea736384322f/TheFabricantNFTStandard.cdc"

import
    TheFabricantNFTStandardV2
    from "../0x7752ea736384322f/TheFabricantNFTStandardV2.cdc"

import
    CAT_EnterTheEvolution
    from "../0x7752ea736384322f/CAT_EnterTheEvolution.cdc"

import TheFabricantKapers from "../0x7752ea736384322f/TheFabricantKapers.cdc"
...
pub contract TheFabricantMarketplaceHelper{ 
    ...
    priv var saleCuts:{ String: SaleCutValues}
    ...
    pub fun s0ListItem(
        ...
        let itemCap =
            getAccount(itemRef.royaltyVault.address).getCapability<
        ...
        let garmentCap =
            getAccount((itemRef.borrowGarment()!).royaltyVault.address)
                .getCapability<&FlowToken.Vault{FungibleToken.Receiver}>(
                /public/flowTokenReceiver
            )
        let materialCap =
            getAccount((itemRef.borrowMaterial()!).royaltyVault.address)
                .getCapability<&FlowToken.Vault{FungibleToken.Receiver}>(
                /public/flowTokenReceiver
            )
        ...
        let saleCutArray: [TheFabricantMarketplace.SaleCut] =
            [
                TheFabricantMarketplace.SaleCut(
                    name: "Season 0 Item Creator",
                    receiver: itemCap,
                    initialAmount: (
                        TheFabricantMarketplaceHelper.saleCuts["item"]!
                    ).initialAmount,
                    amount: (TheFabricantMarketplaceHelper.saleCuts["item"]!)
                        .amount
                ),
                TheFabricantMarketplace.SaleCut(
                    name: "Season 0 Garment Creator",
                    receiver: garmentCap,
                    initialAmount: (
                        TheFabricantMarketplaceHelper.saleCuts["garment"]!
                    ).initialAmount,
                    amount: (TheFabricantMarketplaceHelper.saleCuts["garment"]!)
                        .amount
                ),
                TheFabricantMarketplace.SaleCut(
                    name: "Season 0 Material Creator",
                    receiver: materialCap,
                    initialAmount: (
                        TheFabricantMarketplaceHelper.saleCuts["material"]!
                    ).initialAmount,
                    amount: (
                        TheFabricantMarketplaceHelper.saleCuts["material"]!
                    ).amount
                ...
                TheFabricantMarketplace.SaleCut(
                    ...
                    initialAmount: (
                        TheFabricantMarketplaceHelper.saleCuts["channelFee"]!
                    ).initialAmount,
                    amount: (
                        TheFabricantMarketplaceHelper.saleCuts["channelFee"]!
                    ).amount
    ...
    pub fun s0ItemMakeOffer(
        ...
        ftProviderCapability: Capability<
        ...
        emit S0ItemOfferMade(
            offerID: offerID,
            nftType: nftType,
            nftID: nftID,
            ftVaultType: offerPaymentVaultType,
            price: price,
            offerer: offerRef.owner?.address,
            initialNFTOwner: initialNFTOwner,
            season: "0"
        )
    }
    ...
    pub fun s1ListItem(
        ...
        // get the flowToken capabilities for each component of the item (garment, item, material)
        let itemCap =
            getAccount(itemRef.royaltyVault.wallet.address).getCapability<
                &FlowToken.Vault{FungibleToken.Receiver}
            >(/public/flowTokenReceiver)
        let itemDataID = itemRef.item.itemDataID
        ...
        let garmentRoyalties =
            TheFabricantS1GarmentNFT.getGarmentData(id: garmentData)
                .getRoyalty()
        let materialData = (itemRef.borrowMaterial()!).material.materialDataID
        let materialRoyalties =
            TheFabricantS1MaterialNFT.getMaterialData(id: materialData)
                .getRoyalty()
        ...
        for key in garmentRoyalties.keys{ 
            saleCutArray.append(TheFabricantMarketplace.SaleCut(name: "Season 1 Garment Creator", receiver: (garmentRoyalties[key]!).wallet,                                                                                                                                             //receiver: getAccount(garmentRoyalties[key]!.wallet.address).getCapability<&FlowToken.Vault{FungibleToken.Receiver}>(/public/flowTokenReceiver),
                                                                                                                                             initialAmount: (garmentRoyalties[key]!).initialCut, amount: (garmentRoyalties[key]!).cut))
        }
        // add all flowToken capabilities for material creators
        for key in materialRoyalties.keys{ 
            saleCutArray.append(TheFabricantMarketplace.SaleCut(name: "Season 1 Material Creator", receiver: (materialRoyalties[key]!).wallet,                                                                                                                                               //receiver: getAccount(materialRoyalties[key]!.wallet.address).getCapability<&FlowToken.Vault{FungibleToken.Receiver}>(/public/flowTokenReceiver),
                                                                                                                                               initialAmount: (materialRoyalties[key]!).initialCut, amount: (materialRoyalties[key]!).cut))
        }
        
        // add the flowToken capabilities for item creator and channel fee
        saleCutArray.append(
            TheFabricantMarketplace.SaleCut(
                name: "Season 1 Item Creator",
                receiver: itemCap,
        ...
        saleCutArray.append(
            TheFabricantMarketplace.SaleCut(
                ...
                amount: (TheFabricantMarketplaceHelper.saleCuts["channelFee"]!)
                    .amount
            )
        )
        let listingID =
    ...
    pub fun s2ListItem(
        ...
        images = images.concat([itemImage2, itemImage3, itemImage4])
        let edition =
            itemMetadata["edition"] != nil
                ? (itemMetadata["edition"]!).metadataValue
                : nil
        let garmentData = (itemRef.borrowGarment()!).garment.garmentDataID
        let garmentRoyalties =
            TheFabricantS2GarmentNFT.getGarmentData(id: garmentData)
                .getRoyalty()
        let materialData = (itemRef.borrowMaterial()!).material.materialDataID
        let materialRoyalties =
            TheFabricantS2MaterialNFT.getMaterialData(id: materialData)
                .getRoyalty()
        var saleCutArray: [TheFabricantMarketplace.SaleCut] = []
        // initialize sale cuts for item, garment, material and contract
        // add all flowToken capabilities for garment creators
        for key in garmentRoyalties.keys{ 
            saleCutArray.append(TheFabricantMarketplace.SaleCut(name: "Season 2 Garment Creator", receiver: (garmentRoyalties[key]!).wallet,                                                                                                                                             //receiver: getAccount(garmentRoyalties[key]!.wallet.address).getCapability<&FlowToken.Vault{FungibleToken.Receiver}>(/public/flowTokenReceiver),
                                                                                                                                             initialAmount: (garmentRoyalties[key]!).initialCut, amount: (garmentRoyalties[key]!).cut))
        }
        // add all flowToken capabilities for material creators
        for key in materialRoyalties.keys{ 
            saleCutArray.append(TheFabricantMarketplace.SaleCut(name: "Season 2 Material Creator", receiver: (materialRoyalties[key]!).wallet,                                                                                                                                               //receiver: getAccount(materialRoyalties[key]!.wallet.address).getCapability<&FlowToken.Vault{FungibleToken.Receiver}>(/public/flowTokenReceiver),
                                                                                                                                               initialAmount: (materialRoyalties[key]!).initialCut, amount: (materialRoyalties[key]!).cut))
        }
    ...
    pub fun xxoryListItem(
        ...
        let TFSaleCut =
            TheFabricantMarketplace.SaleCut(
                ...
                receiver: TheFabricantMarketplace.getChannelFeeCap()!,
                initialAmount: (
                    TheFabricantMarketplaceHelper.saleCuts[
                        "TheFabricantXXories"
                    ]!
                ).initialAmount,
                amount: (
    ...
    pub fun catListItem(
        ...
        while i < tfRoyalties.length{ 
            ...
            let TFSaleCut = TheFabricantMarketplace.SaleCut(name: cutInfo.description, receiver: cutInfo.receiver, initialAmount: cutInfo.initialCut, amount: cutInfo.cut)
            saleCutArray.append(TFSaleCut)
            i = i + 1
        }
        let listingID =
    ...
    pub resource Admin{ 
        ...
        pub fun addSaleCutValues(
            ...
            TheFabricantMarketplaceHelper.saleCuts[
                royaltyName
            ] = TheFabricantMarketplaceHelper.SaleCutValues(
    ...
    pub init(){ 
        self
            .AdminStoragePath = /storage/fabricantTheFabricantMarketplaceHelperAdmin0021
        self.saleCuts ={} 
