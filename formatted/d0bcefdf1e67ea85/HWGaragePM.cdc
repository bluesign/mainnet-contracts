...
pub contract HWGaragePM{ 
    ...
    pub event UpdateTokenEditionMetadata(
        id: UInt64,
        metadata:{ 
            String: String
        },
    ...
    priv let HWGarageCardsMintedEditions:{ UInt64: Bool}
    
    priv var HWGarageCardsSequentialMintMin: UInt64
    ...
    priv let packsMintedEditions:{ UInt64: Bool}
    
    priv let packsByPackIdMintedEditions:{ UInt64:{ UInt64: Bool}}
    
    priv var packsSequentialMintMin: UInt64
    
    priv var packsByPackIdSequentialMintMin:{ UInt64: UInt64}
    ...
    pub resource Manager{ 
        ...
        pub fun updateHWGarageCardEditionMetadata(
            ...
            HWGarageCard.setEditionMetadata(
        ...
        pub fun updateHWGarageCardCollectionMetadata(
            metadata:{ 
                String: String
            }
        ){ 
        ...
        pub fun mintHWGarageCardAtEdition(
            edition: UInt64,
            packID: UInt64
        ): @NonFungibleToken.NFT{ 
            emit AdminMintHWGarageCard(id: edition)
            return <-HWGaragePM.mintHWGarageCard(
                edition: edition,
                packID: packID
            )
        }
        
        pub fun mintSequentialHWGarageCard(
            packID: UInt64
        ): @NonFungibleToken.NFT{ 
            let HWGarageCard <-
                HWGaragePM.mintSequentialHWGarageCard(packID: packID)
        ...
        pub fun updatePackEditionMetadata(
            editionNumber: UInt64,
            metadata:{ 
                String: String
            }
        ){ 
            HWGaragePack.setEditionMetadata(
                editionNumber: editionNumber,
                metadata: metadata
            )
            emit UpdatePackEditionMetadata(
                id: editionNumber,
                metadata: metadata
            )
        }
        ...
        pub fun mintSequentialHWGaragePack(
            packID: UInt64,
            address: Address,
            packHash: String
        ): @NonFungibleToken.NFT{ 
            let HWGarageCard <-
                HWGaragePM.mintSequentialPack(
                    packID: packID,
                    packHash: packHash
                )
        ...
        pub fun updateHWGaragePackRedeemStartTime(_ redeemStartTime: UFix64){ 
            ...
            emit UpdateHWGaragePackRedeemInfo(
                redeemStartTime: HWGaragePM.packRedeemStartTime
            )
        }
        ...
        pub fun airdropRedeemable(
            ...
            let HWGarageAirdrop <-
                HWGaragePM.mintSequentialAirdrop(airdropID: airdropSeriesID)
    ...
    access(contract) fun mintHWGarageCard(
        edition: UInt64,
        packID: UInt64
    ): @NonFungibleToken.NFT{ 
        pre{ 
    ...
    priv fun mintSequentialHWGarageCard(packID: UInt64): @NonFungibleToken.NFT{ 
        ...
        let hwGarageCard <-
            self.mintHWGarageCard(edition: currentEditionNumber, packID: packID)
        return <-hwGarageCard
    }
    ...
    access(contract) fun mintPackAtEdition(
        edition: UInt64,
        packID: UInt64,
        packEditionID: UInt64,
        packHash: String
    ...
    access(contract) fun mintSequentialAirdrop(
        airdropID: UInt64
    ): @NonFungibleToken.NFT{ 
