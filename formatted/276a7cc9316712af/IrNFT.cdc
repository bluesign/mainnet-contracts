...
pub contract IrNFT: NonFungibleToken{ 
    ...
    // Brand Events
    //
    pub event BrandCreated(id: UInt32, name: String)
    
    // Collection Events
    //
    pub event CollectionCreated(id: UInt32, brandIDs: [UInt32], name: String)
    
    pub event CollectionItemAdded(id: UInt32, itemID: UInt32)
    
    pub event CollectionDropAdded(id: UInt32, dropID: UInt32)
    
    pub event CollectionClosed(id: UInt32)
    
    // Item Events
    //
    pub event ItemCreated(id: UInt32, collectionID: UInt32, name: String)
    
    pub event ItemRetired(id: UInt32, collectionID: UInt32, name: String)
    
    // Voucher Events
    //
    pub event VoucherPurchased(id: UInt64, collectionID: UInt32, dropID: UInt32, by: Address)
    
    pub event VoucherGifted(id: UInt64, collectionID: UInt32, dropID: UInt32, by: Address)
    
    pub event VoucherRedeemed(id: UInt64, collectionID: UInt32, dropID: UInt32, nftID: UInt64, by: Address)
    
    // NFT Events
    //
    pub event NFTMinted(id: UInt64, collectionID: UInt32, itemID: UInt32, serial: UInt32)
    ...
    priv var rarityMaxSupply:{ IrRarity: UInt64}
    
    priv var rarityDefault: IrRarity
    ...
    //
    priv var brands: @{UInt32: IrBrand}
    
    priv var brandIDsByName:{ String: UInt32}
    
    priv var collections: @{UInt32: IrCollection}
    
    priv var collectionIDsByName:{ String: UInt32}
    
    priv var items: @{UInt32: IrItem}
    
    priv var drops: @{UInt32: IrDrop}
    
    priv var activeDrops: [UInt32]
    ...
    pub struct IrDropData{ 
        ...
        pub fun purchaseVoucher(recipient: &{NonFungibleToken.CollectionPublic}, paymentVault: @FungibleToken.Vault){ 
            let drop = (&IrNFT.drops[self.id] as &IrNFT.IrDrop?)!
            drop.purchaseVoucher(recipient: recipient, paymentVault: <-paymentVault)
        }
        
        pub fun redeemVoucher(recipient: &{NonFungibleToken.CollectionPublic}, token: @NonFungibleToken.NFT){ 
            let drop = (&IrNFT.drops[self.id] as &IrNFT.IrDrop?)!
    ...
    pub resource IrDrop{ 
        ...
        pub fun purchaseVoucher(recipient: &{NonFungibleToken.CollectionPublic}, paymentVault: @FungibleToken.Vault){ 
            ...
            let paymentTargetVault = IrNFT.account.getCapability<&FUSD.Vault{FungibleToken.Receiver}>(/public/fusdReceiver).borrow() ?? panic("Could not borrow reference to target token vault")
    ...
    // mintDropNFT
    //
    access(account) fun mintDropNFT(collectionID: UInt32, itemID: UInt32, dropID: UInt32): @NFT{ 
    ...
    pub resource Admin{ 
        ...
        //
        pub fun createBrand(publicID: String, name: String): UInt32{ 
            pre{ 
        ...
        //
        pub fun createCollection(publicID: String, name: String, brandIDs: [UInt32], description: String?, metadata:{ String: String}?): UInt32{ 
        ...
        //
        pub fun createItem(collectionID: UInt32, publicID: String, name: String, supply: UInt64, utilities: [String], assets: [IrItemAsset], metadata:{ String: String}?): UInt32{ 
            pre{ 
                IrNFT.collections.containsKey(collectionID):
                    "No such collectionID"
            }
        ...
        //
        pub fun createDrop(collectionID: UInt32, publicID: String, name: String, description: String?, price: UFix64, start: UFix64, end: UFix64, items: [UInt32], supplyPerItem:{ UInt32: UInt64}, metadata:{ String: String}): UInt32{ 
            pre{ 
                IrNFT.collections.containsKey(collectionID):
                    "No such collectionID"
            }
        ...
        //
        pub fun giveawayVoucher(dropID: UInt32, recipient: &{NonFungibleToken.CollectionPublic}){ 
            pre{ 
                IrNFT.drops.containsKey(dropID):
                    "No such dropID"
        ...
        //
        pub fun mintItemNFT(collectionID: UInt32, itemID: UInt32): @IrNFT.NFT{ 
            pre{ 
                IrNFT.collections.containsKey(collectionID):
                    "No such collectionID"
                IrNFT.items.containsKey(itemID):
        ...
        //
        pub fun mintDropNFT(collectionID: UInt32, itemID: UInt32, dropID: UInt32): @IrNFT.NFT{ 
            pre{ 
                IrNFT.collections.containsKey(collectionID):
                    "No such collectionID"
                IrNFT.items.containsKey(itemID):
    ...
    init(){ 
        ...
        self.account.link<&IrNFT.Collection{NonFungibleToken.CollectionPublic, IrNFT.CollectionPublic}>(self.CollectionPublicPath, target: self.CollectionStoragePath)
        ...
        self.account.save(<-newAdmin, to: self.AdminStoragePath)
