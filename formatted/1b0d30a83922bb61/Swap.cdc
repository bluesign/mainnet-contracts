...
pub contract Swap{ 
    ...
    pub struct OrderDetails{ 
        ...
        init(
            ...
            assert(
                totalFeeAmount
                / (priceBeforeFees + totalFeeAmount + totalRoyalties)
                >= Swap.Fee(),
    ...
    pub resource interface OrderPublic{ 
        ...
        pub fun fill(payment: @FungibleToken.Vault): @NonFungibleToken.NFT{} 
    ...
    pub resource Order: OrderPublic{ 
        ...
        destroy(){ 
            if !self.details.filled{ 
                emit OrderCanceled(orderID: self.uuid, nftType: self.details.nftType, nftUUID: self.details.nftUUID, nftID: self.details.nftID, currency: self.details.currency, price: self.details.price, expiry: self.details.expiry)
            }
        }
    ...
    pub resource Portfolio: PortfolioManager, PortfolioPublic{ 
        ...
        pub fun createSellOrder(nftProviderCapability: Capability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>, nftType: Type, nftID: UInt64, currency: Type, payments: [Payment], royalties: [Payment], fees: [Payment], expiry: UInt64): UInt64{ 
