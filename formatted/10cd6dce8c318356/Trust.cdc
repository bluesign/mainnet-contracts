...
This is the contract for Trust NFTs!
...
pub contract Trust: NonFungibleToken, ViewResolver{ 
    ...
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver, NiftoryNonFungibleToken.NFTPublic{ 
        ...
        init(setId: Int, templateId: Int, serial: UInt64){ 
            self.id = Trust.totalSupply
            Trust.totalSupply = Trust.totalSupply + 1
            self.setId = setId
    ...
    pub resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection, NiftoryNonFungibleToken.CollectionPublic, NiftoryNonFungibleToken.CollectionPrivate{ 
        ...
        pub fun depositBulk(tokens: @[NonFungibleToken.NFT]){ 
            while tokens.length > 0{ 
                let token <- tokens.removeLast() as! @Trust.NFT
    ...
    pub resource Manager: NiftoryNonFungibleToken.ManagerPublic, NiftoryNonFungibleToken.ManagerPrivate{ 
        ...
        pub fun getSetManagerPublic(): &MutableMetadataSetManager.Manager{MutableMetadataSetManager.Public}{ 
            return NiftoryNFTRegistry.getSetManagerPublic(Trust.REGISTRY_ADDRESS, Trust.REGISTRY_BRAND)
        }
        
        pub fun getMetadataViewsManagerPublic(): &MetadataViewsManager.Manager{MetadataViewsManager.Public}{ 
            return NiftoryNFTRegistry.getMetadataViewsManagerPublic(Trust.REGISTRY_ADDRESS, Trust.REGISTRY_BRAND)
        }
        
        pub fun getNFTCollectionData(): MetadataViews.NFTCollectionData{ 
            return NiftoryNFTRegistry.buildNFTCollectionData(Trust.REGISTRY_ADDRESS, Trust.REGISTRY_BRAND, fun (): @NonFungibleToken.Collection{ 
                    return <-Trust.createEmptyCollection()
                })
        }
        ...
        pub fun modifyContractMetadata(): auth &AnyStruct{ 
            ...
            let maybeMetadata = Trust.metadata
            if maybeMetadata == nil{ 
                let blankMetadata:{ String: String} ={} 
                Trust.metadata = blankMetadata
            }
            return (&Trust.metadata as auth &AnyStruct?)!
        }
        
        pub fun replaceContractMetadata(_ metadata: AnyStruct?){ 
            emit ContractMetadataUpdated()
            Trust.metadata = metadata
        }
        ...
        priv fun _getMetadataViewsManagerPrivate(): &MetadataViewsManager.Manager{MetadataViewsManager.Private}{ 
            let record = NiftoryNFTRegistry.getRegistryRecord(Trust.REGISTRY_ADDRESS, Trust.REGISTRY_BRAND)
            let manager = Trust.account.getCapability<&MetadataViewsManager.Manager{MetadataViewsManager.Private}>(record.metadataViewsManager.paths.private).borrow()!
            return manager
        }
        ...
        priv fun _getSetManagerPrivate(): &MutableMetadataSetManager.Manager{MutableMetadataSetManager.Public, MutableMetadataSetManager.Private}{ 
            let record = NiftoryNFTRegistry.getRegistryRecord(Trust.REGISTRY_ADDRESS, Trust.REGISTRY_BRAND)
            let setManager = Trust.account.getCapability<&MutableMetadataSetManager.Manager{MutableMetadataSetManager.Public, MutableMetadataSetManager.Private}>(record.setManager.paths.private).borrow()!
            return setManager
        }
    ...
    pub fun contract(): &{NiftoryNonFungibleToken.ManagerPublic}{ 
        return NiftoryNFTRegistry.getNFTManagerPublic(Trust.REGISTRY_ADDRESS, Trust.REGISTRY_BRAND)
    }
    ...
    init(nftManagerProxy: &{NiftoryNonFungibleTokenProxy.Public, NiftoryNonFungibleTokenProxy.Private}){ 
        let record = NiftoryNFTRegistry.generateRecord(account: self.account.address, project: "clf8jsk7d0000mo0ub65oplmk_Trust")
        self.REGISTRY_ADDRESS = 0x32d62d5c43ad1038
        self.REGISTRY_BRAND = "clf8jsk7d0000mo0ub65oplmk_Trust"
        ...
        self.account.save<@MutableMetadataSetManager.Manager>(<-MutableMetadataSetManager.create(name: "Trust", description: "The set manager for Trust."), to: record.setManager.paths.storage)
