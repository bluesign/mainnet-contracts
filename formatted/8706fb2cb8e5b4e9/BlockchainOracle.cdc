...
This is the contract for BlockchainOracle NFTs!
...
pub contract BlockchainOracle: NonFungibleToken, ViewResolver{ 
    ...
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver, NiftoryNonFungibleToken.NFTPublic{ 
        ...
        pub fun contract(): &{NiftoryNonFungibleToken.ManagerPublic}{ 
            return BlockchainOracle.contract()
        }
        ...
        init(setId: Int, templateId: Int, serial: UInt64){ 
            self.id = BlockchainOracle.totalSupply
            BlockchainOracle.totalSupply = BlockchainOracle.totalSupply + 1
            self.setId = setId
    ...
    pub resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection, NiftoryNonFungibleToken.CollectionPublic, NiftoryNonFungibleToken.CollectionPrivate{ 
        ...
        pub fun deposit(token: @NonFungibleToken.NFT){ 
            let token <- token as! @BlockchainOracle.NFT
        ...
        pub fun depositBulk(tokens: @[NonFungibleToken.NFT]){ 
            while tokens.length > 0{ 
                let token <- tokens.removeLast() as! @BlockchainOracle.NFT
    ...
    pub resource Manager: NiftoryNonFungibleToken.ManagerPublic, NiftoryNonFungibleToken.ManagerPrivate{ 
        ...
        pub fun metadata(): AnyStruct?{ 
            return BlockchainOracle.metadata
        }
        
        pub fun getSetManagerPublic(): &MutableMetadataSetManager.Manager{MutableMetadataSetManager.Public}{ 
            return NiftoryNFTRegistry.getSetManagerPublic(BlockchainOracle.REGISTRY_ADDRESS, BlockchainOracle.REGISTRY_BRAND)
        }
        
        pub fun getMetadataViewsManagerPublic(): &MetadataViewsManager.Manager{MetadataViewsManager.Public}{ 
            return NiftoryNFTRegistry.getMetadataViewsManagerPublic(BlockchainOracle.REGISTRY_ADDRESS, BlockchainOracle.REGISTRY_BRAND)
        }
        
        pub fun getNFTCollectionData(): MetadataViews.NFTCollectionData{ 
            return NiftoryNFTRegistry.buildNFTCollectionData(BlockchainOracle.REGISTRY_ADDRESS, BlockchainOracle.REGISTRY_BRAND, fun (): @NonFungibleToken.Collection{ 
                    return <-BlockchainOracle.createEmptyCollection()
                })
        }
        ...
        pub fun modifyContractMetadata(): auth &AnyStruct{ 
            ...
            let maybeMetadata = BlockchainOracle.metadata
            if maybeMetadata == nil{ 
                let blankMetadata:{ String: String} ={} 
                BlockchainOracle.metadata = blankMetadata
            }
            return (&BlockchainOracle.metadata as auth &AnyStruct?)!
        }
        
        pub fun replaceContractMetadata(_ metadata: AnyStruct?){ 
            emit ContractMetadataUpdated()
            BlockchainOracle.metadata = metadata
        }
        ...
        priv fun _getMetadataViewsManagerPrivate(): &MetadataViewsManager.Manager{MetadataViewsManager.Private}{ 
            let record = NiftoryNFTRegistry.getRegistryRecord(BlockchainOracle.REGISTRY_ADDRESS, BlockchainOracle.REGISTRY_BRAND)
            let manager = BlockchainOracle.account.getCapability<&MetadataViewsManager.Manager{MetadataViewsManager.Private}>(record.metadataViewsManager.paths.private).borrow()!
            return manager
        }
        ...
        priv fun _getSetManagerPrivate(): &MutableMetadataSetManager.Manager{MutableMetadataSetManager.Public, MutableMetadataSetManager.Private}{ 
            let record = NiftoryNFTRegistry.getRegistryRecord(BlockchainOracle.REGISTRY_ADDRESS, BlockchainOracle.REGISTRY_BRAND)
            let setManager = BlockchainOracle.account.getCapability<&MutableMetadataSetManager.Manager{MutableMetadataSetManager.Public, MutableMetadataSetManager.Private}>(record.setManager.paths.private).borrow()!
            return setManager
        }
    ...
    pub fun contract(): &{NiftoryNonFungibleToken.ManagerPublic}{ 
        return NiftoryNFTRegistry.getNFTManagerPublic(BlockchainOracle.REGISTRY_ADDRESS, BlockchainOracle.REGISTRY_BRAND)
    }
    ...
    init(nftManagerProxy: &{NiftoryNonFungibleTokenProxy.Public, NiftoryNonFungibleTokenProxy.Private}){ 
        let record = NiftoryNFTRegistry.generateRecord(account: self.account.address, project: "clhddm2750000l70u33sxlpmm_BlockchainOracle")
        self.REGISTRY_ADDRESS = 0x32d62d5c43ad1038
        self.REGISTRY_BRAND = "clhddm2750000l70u33sxlpmm_BlockchainOracle"
        ...
        self.account.save<@MutableMetadataSetManager.Manager>(<-MutableMetadataSetManager.create(name: "BlockchainOracle", description: "The set manager for BlockchainOracle."), to: record.setManager.paths.storage)
        ...
        let contractName = "BlockchainOracle"
