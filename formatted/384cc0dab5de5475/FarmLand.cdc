...
pub contract FarmLand: FungibleToken, ViewResolver{ 
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, MetadataViews.Resolver{ 
        ...
        pub fun withdraw(amount: UFix64): @FungibleToken.Vault{ 
            ...
            if let owner: Address = self.owner?.address{ 
                FarmLand.setBalance(address: owner, balance: self.balance)
            }
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                case Type<FungibleTokenMetadataViews.FTView>():
                    return FarmLand.resolveView(view)
        ...
        destroy(){ 
            if self.balance > 0.0{ 
                ...
                FarmLand.totalSupply = FarmLand.totalSupply - self.balance
            }
        }
    }
    ...
    pub resource Minter: Toucans.Minter{ 
        pub fun mint(amount: UFix64): @Vault{ 
            post{ 
                FarmLand.maxSupply == nil || FarmLand.totalSupply <= FarmLand.maxSupply!:
                    "Exceeded the max supply of tokens allowd."
            }
            FarmLand.totalSupply = FarmLand.totalSupply + amount
    ...
    pub fun resolveView(_ view: Type): AnyStruct?{ 
        switch view{ 
            ...
            case Type<FungibleTokenMetadataViews.FTDisplay>():
                let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafybeieufzru32cxku727m7daevlyxj5ry33jgijjzrygj7pcxlqmxtvdu"), mediaType: "image")
                let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafybeiavezct2525f3khhmqj4g6yjl7gou3eii3r4vtmphvdnz6fxjwi2i"), mediaType: "image")
                let medias = MetadataViews.Medias([media, bannerMedia])
                return FungibleTokenMetadataViews.FTDisplay(name: "Farm Land", symbol: "CORN", description: "Farms feed cities. ", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
            case Type<FungibleTokenMetadataViews.FTVaultData>():
                return FungibleTokenMetadataViews.FTVaultData(storagePath: FarmLand.VaultStoragePath, receiverPath: FarmLand.ReceiverPublicPath, metadataPath: FarmLand.VaultPublicPath, providerPath: /private/FarmLandVault, receiverLinkedType: Type<&Vault{FungibleToken.Receiver}>(), metadataLinkedType: Type<&Vault{FungibleToken.Balance, MetadataViews.Resolver}>(), providerLinkedType: Type<&Vault{FungibleToken.Provider}>(), createEmptyVaultFunction: fun (): @Vault{ 
                        return <-FarmLand.createEmptyVault()
                    })
        }
        return nil
    }
    ...
    init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
        ...
        // Paths
        self.VaultStoragePath = /storage/FarmLandVault
        self.ReceiverPublicPath = /public/FarmLandReceiver
        self.VaultPublicPath = /public/FarmLandMetadata
        self.MinterStoragePath = /storage/FarmLandMinter
        self.AdministratorStoragePath = /storage/FarmLandAdmin
