...
pub contract SprtNFTStorefront{ 
    ...
    priv let addressMap:{ UInt64: Address}
    ...
    pub resource Listing: ListingPublic{ 
        ...
        pub fun purchase(payment: @FungibleToken.Vault): @NonFungibleToken.NFT{ 
            ...
            emit ListingCompleted(listingResourceID: self.uuid, storefrontAddress: SprtNFTStorefront.addressMap[self.details.storefrontID], storefrontResourceID: self.details.storefrontID, purchased: self.details.purchased, nftType: self.details.nftType, nftID: self.details.nftID, price: self.details.salePrice)
            return <-nft
        }
        ...
        destroy(){ 
            ...
            if !self.details.purchased{ 
                emit ListingCompleted(listingResourceID: self.uuid, storefrontAddress: SprtNFTStorefront.addressMap[self.details.storefrontID], storefrontResourceID: self.details.storefrontID, purchased: self.details.purchased, nftType: self.details.nftType, nftID: self.details.nftID, price: nil)
            }
        }
    ...
    pub resource Storefront: StorefrontManager, StorefrontPublic{ 
        ...
        //
        pub fun createListing(nftProviderCapability: Capability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>, nftType: Type, nftID: UInt64, salePaymentVaultType: Type, saleCuts: [SaleCut]): UInt64{ 
            ...
            for saleCut in saleCuts{ 
                ...
                newSaleCuts.appendAll([SaleCut(receiver: saleCut.receiver, amount: saleCut.amount - feeAmount), SaleCut(receiver: ElvnFeeTreasury.getReceiver(), amount: feeAmount)])
            }
            let listing <- create Listing(nftProviderCapability: nftProviderCapability, nftType: nftType, nftID: nftID, salePaymentVaultType: salePaymentVaultType, saleCuts: newSaleCuts, storefrontID: self.uuid)
            ...
            destroy oldListing
            emit ListingAvailable(storefrontResourceID: self.uuid, storefrontAddress: self.owner?.address!, listingResourceID: listingResourceID, nftType: nftType, nftID: nftID, ftVaultType: salePaymentVaultType, price: listingPrice)
    ...
    pub fun getFeeRate(amount: UFix64): UFix64{ 
        ...
        return self.feeInfo.baseFeeRate
        - self.feeInfo.feeRateTickSize * UFix64(index)
    }
