...
pub contract SwapRouter{ 
    ...
    pub fun getAmountsOut(amountIn: UFix64, tokenKeyPath: [String]): [UFix64]{ 
        pre{ 
            tokenKeyPath.length >= 2:
                SwapError.ErrorEncode(msg: "SwapRouter: Invalid path", err: SwapError.ErrorCode.INVALID_PARAMETERS)
        }
        var amounts: [UFix64] = []
        ...
        while i < tokenKeyPath.length - 1{ 
            ...
            let pairPublicRef = getAccount(pairAddr).getCapability<&{SwapInterfaces.PairPublic}>(SwapConfig.PairPublicPath).borrow() ?? panic(SwapError.ErrorEncode(msg: "SwapRouter: Lost SwapPair public capability", err: SwapError.ErrorCode.LOST_PUBLIC_CAPABILITY))
    ...
    pub fun swapExactTokensForTokens(
        ...
        return <-self.swapWithPath(
            vaultIn: <-exactVaultIn,
            tokenKeyPath: tokenKeyPath,
            exactAmounts: nil
        )
    }
    ...
    pub fun swapWithPath(
        ...
        pre{ 
            tokenKeyPath.length >= 2:
                SwapError.ErrorEncode(msg: "Invalid path.", err: SwapError.ErrorCode.INVALID_PARAMETERS)
        }
        /// To reduce the gas cost, handle the first five swap out of the loop
        var exactAmountOut1: UFix64? = nil
        if exactAmounts != nil{ 
            exactAmountOut1 = (exactAmounts!)[1]
        }
        let vaultOut1 <-
            self.swapWithPair(
                vaultIn: <-vaultIn,
                exactAmountOut: exactAmountOut1,
                token0Key: tokenKeyPath[0],
                token1Key: tokenKeyPath[1]
            )
        ...
        if exactAmounts != nil{ 
            exactAmountOut2 = (exactAmounts!)[2]
        }
        let vaultOut2 <-
            self.swapWithPair(
                vaultIn: <-vaultOut1,
                exactAmountOut: exactAmountOut2,
                token0Key: tokenKeyPath[1],
                token1Key: tokenKeyPath[2]
            )
        ...
        if exactAmounts != nil{ 
            exactAmountOut3 = (exactAmounts!)[3]
        }
        let vaultOut3 <-
            self.swapWithPair(
                vaultIn: <-vaultOut2,
                exactAmountOut: exactAmountOut3,
                token0Key: tokenKeyPath[2],
                token1Key: tokenKeyPath[3]
            )
        ...
        if exactAmounts != nil{ 
            exactAmountOut4 = (exactAmounts!)[4]
        }
        let vaultOut4 <-
            self.swapWithPair(
                vaultIn: <-vaultOut3,
                exactAmountOut: exactAmountOut4,
                token0Key: tokenKeyPath[3],
                token1Key: tokenKeyPath[4]
            )
        ...
        while index < tokenKeyPath.length - 1{ 
            ...
            if exactAmounts != nil{ 
                exactAmountOut = (exactAmounts!)[index + 1]
            }
