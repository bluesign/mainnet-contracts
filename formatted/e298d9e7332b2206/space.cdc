...
pub contract space: FungibleToken, ViewResolver{ 
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, MetadataViews.Resolver{ 
        ...
        destroy(){ 
            if self.balance > 0.0{ 
                ...
                space.totalSupply = space.totalSupply - self.balance
            }
        }
    }
    ...
    pub fun resolveView(_ view: Type): AnyStruct?{ 
        switch view{ 
            ...
            case Type<FungibleTokenMetadataViews.FTVaultData>():
                return FungibleTokenMetadataViews.FTVaultData(storagePath: space.VaultStoragePath, receiverPath: space.ReceiverPublicPath, metadataPath: space.VaultPublicPath, providerPath: /private/spaceVault, receiverLinkedType: Type<&Vault{FungibleToken.Receiver}>(), metadataLinkedType: Type<&Vault{FungibleToken.Balance, MetadataViews.Resolver}>(), providerLinkedType: Type<&Vault{FungibleToken.Provider}>(), createEmptyVaultFunction: fun (): @Vault{ 
                        return <-space.createEmptyVault()
                    })
        }
        return nil
    }
