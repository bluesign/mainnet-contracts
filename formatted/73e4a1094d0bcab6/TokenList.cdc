pub contract TokenList{ 
    priv var FT:{ String: TokenInfo}
    
    priv var NFT:{ String: TokenInfo}
    ...
    pub fun updateNFTTokenImage(_ acc: AuthAccount, _ t: String){ 
        pre{ 
            acc.address == self.account.address
        }
    }
    
    pub fun addNFTTokenInfo(_ acc: AuthAccount, _ t: String, _ i: TokenInfo){ 
        pre{ 
            acc.address == self.account.address
        }
        self.NFT[t] = i
    ...
    pub fun getFTInfo(_ s: String): TokenInfo?{ 
        return self.FT[s]
    }
