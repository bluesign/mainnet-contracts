...
pub contract FDNZ{ 
    pub fun getAccountLinks(_ account: AuthAccount, domain: String): [{
    ...
    pub fun getAccountStorageNFT(
        _ account: AuthAccount,
        path: String,
        uuid: UInt64
    ): AnyStruct{ 
        var obj =
            account.borrow<auth &AnyResource>(
    ...
    pub fun getAccountStorage(_ account: AuthAccount, path: String): AnyStruct{ 
        var obj =
            account.borrow<auth &AnyResource>(
                from: StoragePath(identifier: path)!
            )!
    ...
    pub fun getAccountData(_ account: AuthAccount):{ String: AnyStruct}{ 
        var paths: [Path] = []
        ...
        //find profile
        var findProfile =
            account.borrow<&AnyResource>(from: /storage/findProfile)
