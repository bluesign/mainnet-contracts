...
pub contract SPORTCASTER: NonFungibleToken{ 
    ...
    /* withdraw event */    pub event Withdraw(id: UInt64, from: Address?)
    
    /* Event that is issued when an NFT is deposited */    pub event Deposit(id: UInt64, to: Address?)
    
    /* event that is emitted when a new collection is created */    pub event NewCollection(collectionName: String, collectionID: UInt64)
    
    /* Event that is emitted when new NFT is created*/    pub event NewNFTminted(name: String, id: UInt64)
    
    /* Event that returns how many IDs a collection has */    pub event TotalsIDs(ids: [UInt64])
    ...
    pub resource Collection: CollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic{ 
        ...
        /* Function to remove the NFt from the Collection */        pub fun withdraw(withdrawID: UInt64): @NonFungibleToken.NFT{ 
            ...
            /* Emit event when a common user withdraws an NFT*/            emit Withdraw(id: withdrawID, from: self.owner?.address)
            return <-token
        }
        
        /*Function to deposit a  NFT in the collection*/        pub fun deposit(token: @NonFungibleToken.NFT){ 
        ...
        /*Function get Ref NFT*/        pub fun getRefNFT(id: UInt64): &NonFungibleToken.NFT{ 
            return (&self.ownedNFTs[id] as &NonFungibleToken.NFT?)!
        }
        
        /*Function borrow NFT*/        pub fun borrowNFT(id: UInt64): &NonFungibleToken.NFT{ 
            return (&self.ownedNFTs[id] as &NonFungibleToken.NFT?)!
        }
        
        /*Function borrow SPORTCASTER View NFT*/        pub fun borrowSPORTCASTERNFT(id: UInt64): &SPORTCASTER.NFT?{ 
    ...
    pub resource NFTTemplate: NFTCollectionReceiver{ 
        ...
        init(name: String, metadata:{ String: AnyStruct}, thumbnail: String, description: String, collection: Capability<&SPORTCASTER.Collection{NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic}>){ 
            self.metadata = metadata
            self.name = name
            self.thumbnail = thumbnail
            self.description = description
    ...
    pub resource NFTMinter{ 
        pub fun createNFTTemplate(name: String, metadata:{ String: AnyStruct}, thumbnail: String, description: String, collection: Capability<&SPORTCASTER.Collection{NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic}>): @NFTTemplate{ 
