...
//
pub contract MotoGPPackMetadata: ContractVersion{ 
    ...
    access(contract) fun resolveDisplayView(packID: UInt64): MetadataViews.Display{ 
    ...
    access(contract) fun resolveMediasView(packID: UInt64): MetadataViews.Medias{ 
        let metadata = self.metadatas[packID]!
    ...
    access(contract) fun resolveTraitsView(packID: UInt64): MetadataViews.Traits{ 
        let metadata = self.metadatas[packID]!
    ...
    pub fun resolveView(view: Type, id: UInt64, packID: UInt64, serial: UInt64, publicCollectionType: Type, publicLinkedType: Type, providerLinkedType: Type, createEmptyCollectionFunction: ((): @NonFungibleToken.Collection)): AnyStruct?{ 
        switch view{ 
            ...
            case Type<MetadataViews.NFTCollectionData>():
                return MetadataViews.NFTCollectionData(storagePath: StoragePath(identifier: "motogpPackCollection")!, publicPath: PublicPath(identifier: "motogpPackCollection")!, providerPath: PrivatePath(identifier: "motogpPackCollection")!, publicCollection: publicCollectionType, publicLinkedType: publicLinkedType, providerLinkedType: providerLinkedType, createEmptyCollectionFunction: createEmptyCollectionFunction)
        }
        return nil
    }
    ...
    //
    priv let metadatas:{ UInt64: MotoGPPackMetadata.Metadata}
    
    // Get all metadatas
    //
    pub fun getMetadatas():{ UInt64: MotoGPPackMetadata.Metadata}{ 
    ...
    //
    pub fun getMetadataForPackID(packID: UInt64): MotoGPPackMetadata.Metadata?{ 
        return self.metadatas[packID]
    }
    
    //Access to set metadata is controlled using an Admin reference as argument
    //
    pub fun setMetadata(adminRef: &MotoGPAdmin.Admin, packID: UInt64, name: String, description: String, imageUrl: String, data:{ String: String}){ 
        pre{ 
            adminRef != nil:
                "adminRef is nil"
        }
    ...
    //
    pub fun removeMetadata(adminRef: &MotoGPAdmin.Admin, packID: UInt64){ 
        pre{ 
            adminRef != nil:
                "adminRef is nil"
        }
