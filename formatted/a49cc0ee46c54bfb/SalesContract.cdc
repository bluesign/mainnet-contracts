...
pub contract SalesContract{ 
    ...
    pub fun buyPack(
        ...
        recipientCollectionRef: &MotoGPPack.Collection{
            MotoGPPack.IPackCollectionPublic
        }
    ){ 
        pre{ 
        ...
        let message =
            skuName.concat(recipientStr).concat(nonce.toString()).concat(
                packType.toString()
            )
        let isValid =
            self.isValidSignature(signature: signature, message: message)
        if isValid == false{ 
        ...
        let payoutReceiver =
            payoutRecipient.getCapability(/public/flowTokenReceiver).borrow<
        ...
        // Mint a pack
        let nft <-
            MotoGPPack.createPack(packNumber: packNumber, packType: packType)
    ...
    pub fun setSerialStatusInPackTypeMap(
        adminRef: &Admin,
        packType: UInt64,
        serial: UInt64,
        value: Bool
    ...
    pub fun setPayoutAddressForSKU(
        adminRef: &Admin,
        name: String,
        payoutAddress: Address
    ){ 
