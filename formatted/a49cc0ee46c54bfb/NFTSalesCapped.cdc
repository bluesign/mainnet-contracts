...
pub contract NFTSalesCapped: ContractVersion{ 
    ...
    pub resource Sales{ 
        ...
        priv var shrdPerGrade:{ String: UFix64} // grade (rarity, e.g. Legendary) => amount of SHRD
        ...
        pub var sold: UInt64 // the number of cards minted from this Sales resource
        
        priv var cardTypeWeights: [[UInt32]] // A list of bucket card probability weights. Each inner array holds the weights for one bucket
        
        priv var cardTypes: [[UInt64]] // A list of bucket card types. Each inner array holds the types for one bucket. Needs to be ordered to match cardTypeWeights's order
        
        pub let pausableCap: Capability<&{Pausable.PausableExternal}>
        
        init(name: String, totalSupply: UInt64, maxSHRD: UFix64, shrdPerGrade:{ String: UFix64}, cardsPerPack: UInt64, price: UFix64, startTime: UFix64, endTime: UFix64, maxPerWallet: UInt64, cardTypeWeights: [[UInt32]], // If empty, should be [], not [[],[],[]] cardTypes: [[UInt64]], // If empty, should be [], not [[],[],[]] pausableCap: Capability<&{Pausable.PausableExternal}>){ 
            pre{ 
                name.length > 0:
        ...
        access(contract) fun setTotalSupply(totalSupply: UInt64){ 
            self.totalSupply = totalSupply
        }
        ...
        pub fun openPacksWithCreditCardPayment(signature: String, orderId: String, address: Address, quantity: UInt64, hashAlgorithm: HashAlgorithm): @OpenedPack{ 
            pre{ 
        ...
        pub fun openPacksWithVaultPayment(signature: String, revvVault: @REVV.Vault, address: Address, quantity: UInt64, hashAlgorithm: HashAlgorithm): @OpenedPack{ 
            pre{ 
                !(self.pausableCap.borrow()!).isPaused():
                    "Invalid: Sales is paused"
                self.isOpen():
        ...
        access(contract) fun openPack( // TODO: change to access(self)signature: String, address: Address, quantity: UInt64, message: String): @OpenedPack{ 
    ...
    pub resource SalesCollection: SalesCollectionPublic{ 
        ...
        pub fun setSalesOpenOverride(salesID: UInt64, state: NFTSalesCapped.SalesOpenOverride){ 
            self.borrowSales(salesID: salesID).setSalesOpenOverride(state: state)
        }
        
        pub fun setTotalSupply(salesID: UInt64, totalSupply: UInt64){ 
    ...
    pub fun createSales(adminRef: &MotoGPAdmin.Admin, name: String, totalSupply: UInt64, maxSHRD: UFix64, shrdPerGrade:{ String: UFix64}, cardsPerPack: UInt64, price: UFix64, startTime: UFix64, endTime: UFix64, maxPerWallet: UInt64, cardTypeWeights: [[UInt32]], cardTypes: [[UInt64]], pausableCap: Capability<&{Pausable.PausableExternal}>): @Sales{ 
        pre{ 
            adminRef != nil:
                "adminRef is nil"
        }
    ...
    access(contract) fun isValidSignature(publicKey: String, signatureAlgorithm: SignatureAlgorithm, hashAlgorithm: HashAlgorithm, signature: String, message: String): Bool{ 
