...
pub contract MotoGPNFTStorefront{ 
    ...
    access(contract) var commissionReceiver: Capability<
        &{FungibleToken.Receiver}
    >?
    
    access(contract) var commissionReceiverMap:{ 
    ...
    pub struct SaleOfferDetails{ 
        ...
        init(
            ...
            assert(
                saleCuts.length > 0,
                message: "SaleOffer must have at least one payment cut recipient"
            )
            ...
            assert(
                salePrice > 0.0,
                message: "SaleOffer must have non-zero price"
            )
    ...
    pub resource SaleOffer: SaleOfferPublic{ 
        ...
        pub fun accept(payment: @FungibleToken.Vault): @NonFungibleToken.NFT{ 
            ...
            emit SaleOfferCompleted(saleOfferResourceID: self.uuid, storefrontResourceID: self.details.storefrontID, accepted: self.details.accepted, price: self.details.salePrice, ftVaultType: self.details.salePaymentVaultType, nftType: self.details.nftType, nftID: self.details.nftID)
            return <-nft
        }
        ...
        // initializer
        //
        init(nftProviderCapability: Capability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>, nftType: Type, nftID: UInt64, salePaymentVaultType: Type, price: UFix64, // GK: pass in price UFix64, sellerReceiver: Capability<&{FungibleToken.Receiver}>, //for the seller to receive her payout storefrontID: UInt64){ 
            ...
            self.details = SaleOfferDetails(nftType: nftType, nftID: nftID, salePaymentVaultType: salePaymentVaultType, saleCuts: saleCuts, storefrontID: storefrontID)
    ...
    pub resource interface StorefrontPublic{ 
        ...
        pub fun borrowSaleOffer(saleOfferResourceID: UInt64): &SaleOffer{
            SaleOfferPublic
        }?{} 
    ...
    pub resource Storefront: StorefrontManager, StorefrontPublic{ 
        ...
        //
        pub fun createSaleOffer(nftProviderCapability: Capability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>, nftType: Type, nftID: UInt64, salePaymentVaultType: Type, price: UFix64, sellerReceiver: Capability<&{FungibleToken.Receiver}>): UInt64{ 
            let saleOffer <- create SaleOffer(nftProviderCapability: nftProviderCapability, nftType: nftType, nftID: nftID, salePaymentVaultType: salePaymentVaultType, price: price, sellerReceiver: sellerReceiver, // GK: use internal storefrontID: self.uuid)
            ...
            destroy oldOffer
            emit SaleOfferAvailable(storefrontAddress: self.owner?.address!, saleOfferResourceID: saleOfferResourceID, nftType: nftType, nftID: nftID, ftVaultType: salePaymentVaultType, price: saleOfferPrice)
    ...
    pub fun setCommissionReceiver(
        ...
        pre{ 
            adminRef != nil:
                "adminRef is nil"
            commissionReceiver.borrow() != nil:
                "commissionReceiver is nil"
        }
