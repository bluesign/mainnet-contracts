...
pub contract VroomTokenRepository{ 
    ...
    pub resource interface TresorPublic{ 
        pub fun transferAndRemoveTresor(
            ...
            repositoryRef: &VroomTokenRepository.Repository{
                VroomTokenRepository.RepositoryPublic
            }
        ){} 
    ...
    pub resource Repository: RepositoryManager, RepositoryPublic{ 
        ...
        pub fun createTresor(signer: AuthAccount, price: UFix64, amount: UFix64): UInt64{ 
            let vaultRef = signer.borrow<&VroomToken.Vault{FungibleToken.Provider, FungibleToken.Balance}>(from: VroomToken.VaultStoragePath) ?? panic("Could not borrow reference to the VroomToken vault")
