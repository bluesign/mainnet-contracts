...
pub contract TopTCollection2: NonFungibleToken{ 
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: TopTCollection2.CollectionStoragePath, publicPath: TopTCollection2.CollectionPublicPath, providerPath: /private/topTNFTCollection, publicCollection: Type<&TopTCollection2.Collection{TopTCollection2.TopTCollectionPublic}>(), publicLinkedType: Type<&TopTCollection2.Collection{TopTCollection2.TopTCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&TopTCollection2.Collection{TopTCollection2.TopTCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Provider, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
    ...
    pub resource NFTMinter{ 
        ...
        //
        pub fun mintBADGE(recipient: &{TopTCollection2.TopTCollectionPublic}, kind: Kind, marketRoyalty: MetadataViews.Royalty){ 
            pre{ 
            ...
            recipient.depositBadge(token: <-create TopTCollection2.BADGE(id: TopTCollection2.totalSupplyBadges, kind: kind, marketRoyalty: MetadataViews.Royalty(marketWalletCap, 0.1, "Market")))
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &TopTCollection2.NFT?{ 
        let collection = getAccount(from).getCapability(TopTCollection2.CollectionPublicPath).borrow<&TopTCollection2.Collection{TopTCollection2.TopTCollectionPublic}>() ?? panic("Couldn't get collection")
    ...
    pub fun mintNFT(name: String, description: String, caption: String, storagePath: String, artistAddress: Address, royalties: [MetadataViews.Royalty], thumbnail: String): @TopTCollection2.NFT{ 
        let marketWalletCap = getAccount(0xf8d6e0586b0a20c7).getCapability<&{FungibleToken.Receiver}>(/public/flowTokenReceiver)
        var newNFT <- create NFT(initID: TopTCollection2.totalSupply, metadata: Metadata(artistAddress: artistAddress, storagePath: storagePath, caption: caption), name: name, description: description, thumbnail: thumbnail, royalties: royalties.concat([MetadataViews.Royalty(marketWalletCap, 0.1, "Market")]))
