...
pub contract AtlantaHawks_NFT: NonFungibleToken{ 
    
    // AtlantaHawks_NFT Events
    //
    // Emitted when the AtlantaHawks_NFT contract is created
    ...
    // totalSupply
    // The total number of AtlantaHawks_NFT that have been minted
    //
    ...
    // A Series is special resource type that contains functions to mint AtlantaHawks_NFT NFTs, 
    ...
    pub resource Series{ 
        ...
        init(seriesId: UInt32, metadata:{ String: String}){ 
            ...
            self.setIds = []
            self.setSealedState ={} 
            AtlantaHawks_NFT.seriesData[seriesId] = SeriesData(seriesId: seriesId, metadata: metadata)
        ...
        pub fun addNftSet(setId: UInt32, maxEditions: UInt32, ipfsMetadataHashes:{ UInt32: String}, metadata:{ String: String}){ 
            ...
            AtlantaHawks_NFT.setData[setId] = newNFTSet
        ...
        pub fun updateSeriesMetadata(metadata:{ String: String}){ 
            ...
            AtlantaHawks_NFT.seriesData[self.seriesId] = newSeriesMetadata
        ...
        pub fun updateSetMetadata(setId: UInt32, maxEditions: UInt32, ipfsMetadataHashes:{ UInt32: String}, metadata:{ String: String}){ 
            ...
            AtlantaHawks_NFT.setData[setId] = newSetMetadata
            emit SetMetadataUpdated(seriesId: self.seriesId, setId: setId)
        }
        
        // mintAtlantaHawks_NFT
        // Mints a new NFT with a new ID
        // and deposits it in the recipients collection using their collection reference
        //
        pub fun mintAtlantaHawks_NFT(recipient: &{NonFungibleToken.CollectionPublic}, tokenId: UInt64, setId: UInt32){ 
            pre{ 
                self.numberEditionsMintedPerSet[setId] != nil:
                    "The Set does not exist."
                self.numberEditionsMintedPerSet[setId]! <= AtlantaHawks_NFT.getSetMaxEditions(setId: setId)!:
            ...
            recipient.deposit(token: <-create AtlantaHawks_NFT.NFT(tokenId: tokenId, setId: setId, editionNum: editionNum))
            
            // Increment the count of global NFTs 
            AtlantaHawks_NFT.totalSupply = AtlantaHawks_NFT.totalSupply + 1 as UInt64
            
            // Update the count of Editions minted in the set
            self.numberEditionsMintedPerSet[setId] = editionNum
        }
        
        // mintEditionAtlantaHawks_NFT
        // Mints a new NFT with a new ID and specific edition Num (random open edition)
        // and deposits it in the recipients collection using their collection reference
        //
        pub fun mintEditionAtlantaHawks_NFT(recipient: &{NonFungibleToken.CollectionPublic}, tokenId: UInt64, setId: UInt32, edition: UInt32){ 
            pre{ 
                self.numberEditionsMintedPerSet[setId] != nil:
                    "The Set does not exist."
                self.numberEditionsMintedPerSet[setId]! <= AtlantaHawks_NFT.getSetMaxEditions(setId: setId)!:
            ...
            // deposit it in the recipient's account using their reference
            recipient.deposit(token: <-create AtlantaHawks_NFT.NFT(tokenId: tokenId, setId: setId, editionNum: edition))
        ...
        // batchMintAtlantaHawks_NFT
        // Mints multiple new NFTs given and deposits the NFTs
        // into the recipients collection using their collection reference
        pub fun batchMintAtlantaHawks_NFT(recipient: &{NonFungibleToken.CollectionPublic}, setId: UInt32, tokenIds: [UInt64]){ 
            pre{ 
                tokenIds.length > 0:
                    "Number of token Ids must be > 0"
            }
            for tokenId in tokenIds{ 
                self.mintAtlantaHawks_NFT(recipient: recipient, tokenId: tokenId, setId: setId)
            }
        }
    ...
    // A resource that represents the AtlantaHawks_NFT NFT
    //
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver{ 
        ...
        init(tokenId: UInt64, setId: UInt32, editionNum: UInt32){ 
            ...
            let seriesId = AtlantaHawks_NFT.getSetSeriesId(setId: setId)!
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                case Type<MetadataViews.Display>():
                    return MetadataViews.Display(name: AtlantaHawks_NFT.getSetMetadataByField(setId: self.setId, field: "name")!, description: AtlantaHawks_NFT.getSetMetadataByField(setId: self.setId, field: "description")!, thumbnail: MetadataViews.HTTPFile(url: AtlantaHawks_NFT.getSetMetadataByField(setId: self.setId, field: "preview")!))
                case Type<MetadataViews.Serial>():
                    return MetadataViews.Serial(self.id)
                case Type<MetadataViews.Editions>():
                    let maxEditions = AtlantaHawks_NFT.setData[self.setId]?.maxEditions ?? 0
                    let editionName = AtlantaHawks_NFT.getSetMetadataByField(setId: self.setId, field: "name")!
                ...
                case Type<MetadataViews.ExternalURL>():
                    if let externalBaseURL = AtlantaHawks_NFT.getSetMetadataByField(setId: self.setId, field: "external_token_base_url"){ 
                ...
                case Type<MetadataViews.Royalties>():
                    ...
                    for metadataKey in (AtlantaHawks_NFT.getSetMetadata(setId: self.setId)!).keys{ 
                        ...
                        if metadataKey.length >= 13{ 
                            if metadataKey.slice(from: 0, upTo: 13) == "royalty_addr_"{ 
                                ...
                                let royaltyAddress = AtlantaHawks_NFT.convertStringToAddress(AtlantaHawks_NFT.getSetMetadataByField(setId: self.setId, field: "royalty_addr_".concat(royaltyName))!)!
                                let royaltyReceiver: PublicPath = PublicPath(identifier: AtlantaHawks_NFT.getSetMetadataByField(setId: self.setId, field: "royalty_rcv_".concat(royaltyName))!)!
                                let royaltyCut = AtlantaHawks_NFT.getSetMetadataByField(setId: self.setId, field: "royalty_cut_".concat(royaltyName))!
                                let cutValue: UFix64 = AtlantaHawks_NFT.royaltyCutStringToUFix64(royaltyCut)
                                if cutValue != 0.0{ 
                                    royalties.append(MetadataViews.Royalty(receiver: getAccount(royaltyAddress).getCapability<&FungibleToken.Vault{FungibleToken.Receiver}>(royaltyReceiver), cut: cutValue, description: AtlantaHawks_NFT.getSetMetadataByField(setId: self.setId, field: "royalty_desc_".concat(royaltyName))!))
                                }
                            }
                        }
                    }
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: AtlantaHawks_NFT.CollectionStoragePath, publicPath: AtlantaHawks_NFT.CollectionPublicPath, providerPath: /private/AtlantaHawks_NFT, publicCollection: Type<&AtlantaHawks_NFT.Collection{AtlantaHawks_NFT.AtlantaHawks_NFTCollectionPublic, NonFungibleToken.CollectionPublic}>(), publicLinkedType: Type<&AtlantaHawks_NFT.Collection{AtlantaHawks_NFT.AtlantaHawks_NFTCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&AtlantaHawks_NFT.Collection{AtlantaHawks_NFT.AtlantaHawks_NFTCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
                            return <-AtlantaHawks_NFT.createEmptyCollection()
                        })
                case Type<MetadataViews.NFTCollectionDisplay>():
                    let squareImage = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://media.gigantik.io/atlantahawks/square.png"), mediaType: "image/png")
                    let bannerImage = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://media.gigantik.io/atlantahawks/banner.png"), mediaType: "image/png")
                    ...
                    for metadataKey in (AtlantaHawks_NFT.getSetMetadata(setId: self.setId)!).keys{ 
                        ...
                        if metadataKey.length >= 18{ 
                            if metadataKey.slice(from: 0, upTo: 18) == "collection_social_"{ 
                                ...
                                socials.insert(key: metadataKey.slice(from: 18, upTo: metadataKey.length), MetadataViews.ExternalURL(AtlantaHawks_NFT.getSetMetadataByField(setId: self.setId, field: metadataKey)!))
                            }
                        }
                    }
                    return MetadataViews.NFTCollectionDisplay(name: AtlantaHawks_NFT.getSetMetadataByField(setId: self.setId, field: "collection_name") ?? "", description: AtlantaHawks_NFT.getSetMetadataByField(setId: self.setId, field: "collection_description") ?? "", externalURL: MetadataViews.ExternalURL(AtlantaHawks_NFT.getSetMetadataByField(setId: self.setId, field: "external_url") ?? ""), squareImage: squareImage, bannerImage: bannerImage, socials: socials)
                    ...
                    for metadataKey in (AtlantaHawks_NFT.getSetMetadata(setId: self.setId)!).keys{ 
                        ...
                        if metadataKey.length >= 6{ 
                            if metadataKey.slice(from: 0, upTo: 6) == "trait_"{ 
                                ...
                                traitDictionary.insert(key: metadataKey.slice(from: 6, upTo: metadataKey.length), AtlantaHawks_NFT.getSetMetadataByField(setId: self.setId, field: metadataKey)!)
                            }
                        }
                    }
                    return MetadataViews.dictToTraits(dict: traitDictionary, excludedNames: [])
                case Type<MetadataViews.Medias>():
                    return MetadataViews.Medias(items: [MetadataViews.Media(file: MetadataViews.HTTPFile(url: AtlantaHawks_NFT.getSetMetadataByField(setId: self.setId, field: "image")!), mediaType: self.getMimeType())])
            }
            return nil
        }
        
        pub fun getMimeType(): String{ 
            var metadataFileType = (AtlantaHawks_NFT.getSetMetadataByField(setId: self.setId, field: "image_file_type")!).toLower()
    ...
    pub resource Admin{ 
        pub fun addSeries(seriesId: UInt32, metadata:{ String: String}){ 
            pre{ 
                AtlantaHawks_NFT.series[seriesId] == nil:
            ...
            AtlantaHawks_NFT.series[seriesId] <-! newSeries
        }
        ...
        pub fun borrowSeries(seriesId: UInt32): &Series{ 
            ...
            return (&AtlantaHawks_NFT.series[seriesId] as &Series?)!
        }
    ...
    // to allow others to deposit AtlantaHawks_NFT into their Collection. It also allows for reading
    // the details of AtlantaHawks_NFT in the Collection.
    pub resource interface AtlantaHawks_NFTCollectionPublic{ 
        ...
        pub fun borrowAtlantaHawks_NFT(id: UInt64): &AtlantaHawks_NFT.NFT?{ 
            // If the result isn't nil, the id of the returned reference
            // should be the same as the argument to the function
            post{ 
                result == nil || result?.id == id:
                    "Cannot borrow AtlantaHawks_NFT reference: The ID of the returned reference is incorrect"
            }
        }
    }
    
    // Collection
    // A collection of AtlantaHawks_NFT NFTs owned by an account
    //
    pub resource Collection: AtlantaHawks_NFTCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection{ 
        ...
        //
        pub fun deposit(token: @NonFungibleToken.NFT){ 
            let token <- token as! @AtlantaHawks_NFT.NFT
        ...
        // borrowAtlantaHawks_NFT
        // Gets a reference to an NFT in the collection as a AtlantaHawks_NFT,
        // exposing all of its fields.
        // This is safe as there are no functions that can be called on the AtlantaHawks_NFT.
        //
        pub fun borrowAtlantaHawks_NFT(id: UInt64): &AtlantaHawks_NFT.NFT?{ 
            let ref = &self.ownedNFTs[id] as auth &NonFungibleToken.NFT?
            return ref as! &AtlantaHawks_NFT.NFT?
        }
        ...
        pub fun borrowViewResolver(id: UInt64): &AnyResource{MetadataViews.Resolver}{ 
            ...
            let AtlantaHawks_NFTNft = nft as! &AtlantaHawks_NFT.NFT
            return AtlantaHawks_NFTNft as &AnyResource{MetadataViews.Resolver}
        }
    ...
    // fetch
    // Get a reference to a AtlantaHawks_NFT from an account's Collection, if available.
    // If an account does not have a AtlantaHawks_NFT.Collection, panic.
    ...
    //
    pub fun fetch(_ from: Address, id: UInt64): &AtlantaHawks_NFT.NFT?{ 
        let collection = getAccount(from).getCapability(AtlantaHawks_NFT.CollectionPublicPath).borrow<&AtlantaHawks_NFT.Collection{AtlantaHawks_NFT.AtlantaHawks_NFTCollectionPublic}>() ?? panic("Couldn't get collection")
        // We trust AtlantaHawks_NFT.Collection.borrowAtlantaHawks_NFT to get the correct id
        // (it checks it before returning it).
        return collection.borrowAtlantaHawks_NFT(id: id)
    }
    
    // getAllSeries returns all the sets
    //
    // Returns: An array of all the series that have been created
    pub fun getAllSeries(): [AtlantaHawks_NFT.SeriesData]{ 
        return AtlantaHawks_NFT.seriesData.values
    }
    
    // getAllSets returns all the sets
    //
    // Returns: An array of all the sets that have been created
    pub fun getAllSets(): [AtlantaHawks_NFT.NFTSetData]{ 
        return AtlantaHawks_NFT.setData.values
    }
    ...
    pub fun getSeriesMetadata(seriesId: UInt32):{ String: String}?{ 
        return AtlantaHawks_NFT.seriesData[seriesId]?.getMetadata()
    }
    ...
    pub fun getSetMaxEditions(setId: UInt32): UInt32?{ 
        return AtlantaHawks_NFT.setData[setId]?.maxEditions
    }
    ...
    pub fun getSetMetadata(setId: UInt32):{ String: String}?{ 
        return AtlantaHawks_NFT.setData[setId]?.getMetadata()
    }
    ...
    pub fun getSetSeriesId(setId: UInt32): UInt32?{ 
        return AtlantaHawks_NFT.setData[setId]?.seriesId
    }
    ...
    pub fun getIpfsMetadataHashByNftEdition(setId: UInt32, editionNum: UInt32): String?{ 
        ...
        if let set = AtlantaHawks_NFT.setData[setId]{ 
    ...
    init(){ 
        // Set named paths
        self.CollectionStoragePath = /storage/AtlantaHawks_NFTCollection
        self.CollectionPublicPath = /public/AtlantaHawks_NFTCollection
        self.AdminStoragePath = /storage/AtlantaHawks_NFTAdmin
        self.AdminPrivatePath = /private/AtlantaHawks_NFTAdminUpgrade
        ...
        self.account.link<&AtlantaHawks_NFT.Admin>(self.AdminPrivatePath, target: self.AdminStoragePath) ?? panic("Could not get a capability to the admin")
