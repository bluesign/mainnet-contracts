...
pub contract BulkPurchase{ 
    pub event BulkPurchaseCompleted(completedOrders: [CompletedPurchaseOrder])
    ...
    pub fun getStorefrontFromIdentifier(_ identifier: String): Storefront{ 
        switch identifier{ 
    ...
    pub fun getStorefrontIdentifier(_ storefront: Storefront): String{ 
        switch storefront{ 
    ...
    pub struct PurchaseOrder{ 
        ...
        init(listingID: UInt64, ownerAddress: Address, storefront: Storefront){ 
    ...
    pub struct CompletedPurchaseOrder{ 
        pub let listingID: UInt64
        ...
        init(_ order: PurchaseOrder){ 
            self.listingID = order.listingID
            self.ownerAddress = order.ownerAddress.toString()
            self.storefront = BulkPurchase.getStorefrontIdentifier(
                    order.storefront
                )
    ...
    // Helper functions
    access(contract) fun getStorefrontV1Ref(
        address: Address
    ): &NFTStorefront.Storefront{NFTStorefront.StorefrontPublic}?{ 
        return getAccount(address).getCapability<
            &NFTStorefront.Storefront{NFTStorefront.StorefrontPublic}
        >(NFTStorefront.StorefrontPublicPath).borrow()
    }
    
    access(contract) fun getStorefrontV2Ref(
        address: Address
    ...
    access(contract) fun getTopshotV1MarketRef(
        address: Address
    ): &Market.SaleCollection{Market.SalePublic}?{ 
        return getAccount(address).getCapability<
            &Market.SaleCollection{Market.SalePublic}
        >(/public/topshotSaleCollection).borrow()
    }
    
    access(contract) fun getTopshotV3MarketRef(
        address: Address
    ): &TopShotMarketV3.SaleCollection{Market.SalePublic}?{ 
        return getAccount(address).getCapability<
            &TopShotMarketV3.SaleCollection{Market.SalePublic}
        >(TopShotMarketV3.marketPublicPath).borrow()
    }
    
    access(contract) fun getFlovatarMarketRef(
        address: Address
    ): &FlovatarMarketplace.SaleCollection{FlovatarMarketplace.SalePublic}?{ 
        return (
            getAccount(address).getCapability<
                &FlovatarMarketplace.SaleCollection{
                    FlovatarMarketplace.SalePublic
                }
    ...
    access(contract) fun processStorefrontV1Order(
        ...
        let listing =
            storefront.borrowListing(listingResourceID: order.listingID)
        if listing == nil{ 
            if unsafeMode{ 
        ...
        let paymentVaultRef =
            paymentVaultRefs[listingDetails.salePaymentVaultType.identifier]!
        assert(
            listingDetails.salePaymentVaultType == paymentVaultRef.getType(),
            message: "payment vault type does not match, listing id: ".concat(
        ...
        let receiverCapability =
            nftReceiverCapabilities[listingDetails.nftType.identifier]
            ?? panic(
                "failed to get nft receiver capability for Storefront V1, NFT type: "
                    .concat(listingDetails.nftType.identifier)
            )
        let receiver =
            receiverCapability.borrow()
            ?? panic(
                "failed to borrow receiver for Storefront V1, NFT type: "
                    .concat(listingDetails.nftType.identifier)
            )
        let payment <-
            paymentVaultRef.withdraw(amount: listingDetails.salePrice)
    ...
    access(contract) fun processStorefrontV2Order(
        ...
        commissionReceiverCapabilities:{ 
        ...
        let receiverCapability =
            nftReceiverCapabilities[listingDetails.nftType.identifier]
            ?? panic(
                "failed to get nft receiver capability for Storefront V2, NFT type: "
                    .concat(listingDetails.nftType.identifier)
            )
        let receiver =
            receiverCapability.borrow()
            ?? panic(
                "failed to borrow receiver for Storefront V2, NFT type: "
                    .concat(listingDetails.nftType.identifier)
            )
        ...
        if listingDetails.commissionAmount > 0.0{ 
            ...
            if let allowedCommissionReceivers = (listing!).getAllowedCommissionReceivers(){ 
                ...
                for allowedReceiver in allowedCommissionReceivers{ 
                    ...
                    if commissionRecipient != nil && (commissionRecipient!).address == allowedReceiver.address && (commissionRecipient!).getType() == allowedReceiver.getType(){ 
                        found = true
                        break
                    }
                }
    ...
    access(contract) fun processTopshotOrder(
        ...
        let paymentVaultRef =
            paymentVaultRefs[Type<@DapperUtilityCoin.Vault>().identifier]!
        assert(
            paymentVaultRef.getType() == Type<@DapperUtilityCoin.Vault>(),
            message: "payment vault type does not match, listing id: ".concat(
                order.listingID.toString()
            )
        )
        ...
        let receiverCapability =
            nftReceiverCapabilities[Type<@TopShot.NFT>().identifier]
            ?? panic("failed to get nft receiver capability for topshot")
        let receiver =
            receiverCapability.borrow()
            ?? panic("failed to borrow receiver for TopShot")
        let buyTokens <-
            paymentVaultRef.withdraw(amount: salePrice)
            as!
            @DapperUtilityCoin.Vault
        let purchasedItem <-
            (topShotMarket!).purchase(
                tokenID: order.listingID,
                buyTokens: <-buyTokens
            )
    ...
    pub fun purchase(
        ...
        pre{ 
            orders.length <= 30:
                "maximum 30 orders per transaction"
        }
        
        // storefront references
        let storefrontV1Refs:{ 
            Address: &NFTStorefront.Storefront{NFTStorefront.StorefrontPublic}
        } ={} 
        let storefrontV2Refs:{ 
            Address: &NFTStorefrontV2.Storefront{
                NFTStorefrontV2.StorefrontPublic
            }
        } ={} 
        let topShotV1MarketRefs:{ 
            Address: &Market.SaleCollection{Market.SalePublic}
        } ={} 
        let topShotV3MarketRefs:{ 
            Address: &TopShotMarketV3.SaleCollection{Market.SalePublic}
        } ={} 
        let flovatarMarketRefs:{ 
            Address: &FlovatarMarketplace.SaleCollection{
                FlovatarMarketplace.SalePublic
            }
        } ={} 
        ...
        for order in orders{ 
            switch order.storefront{ 
                case Storefront.StorefrontV1:
                    ...
                    let processedOrder = self.processStorefrontV1Order(order: order, paymentVaultRefs: paymentVaultRefs, nftReceiverCapabilities: nftReceiverCapabilities, storefront: storefront, unsafeMode: unsafeMode)
                ...
                case Storefront.StorefrontV2:
                    ...
                    let processedOrder = self.processStorefrontV2Order(order: order, paymentVaultRefs: paymentVaultRefs, nftReceiverCapabilities: nftReceiverCapabilities, storefront: storefrontV2Refs[order.ownerAddress]!, commissionReceiverCapabilities: commissionReceiverCapabilities, unsafeMode: unsafeMode)
                    completedOrders.append(CompletedPurchaseOrder(processedOrder))
                ...
                case Storefront.TopShotV1:
                    ...
                    let processedOrder = self.processTopshotOrder(order: order, paymentVaultRefs: paymentVaultRefs, nftReceiverCapabilities: nftReceiverCapabilities, topShotMarket: topShotV1MarketRefs[order.ownerAddress]!, unsafeMode: unsafeMode)
                    completedOrders.append(CompletedPurchaseOrder(processedOrder))
                case Storefront.TopShotV3:
                    ...
                    let processedOrder = self.processTopshotOrder(order: order, paymentVaultRefs: paymentVaultRefs, nftReceiverCapabilities: nftReceiverCapabilities, topShotMarket: topShotV3MarketRefs[order.ownerAddress]!, unsafeMode: unsafeMode)
                    completedOrders.append(CompletedPurchaseOrder(processedOrder))
                case Storefront.Flovatar:
