...
pub contract OrdinalVendor{ 
    ...
    pub event OrdinalRestricted(id: UInt64)
    ...
    pub struct PurchaseData{ 
        ...
        pub let inscriptionData: String
    ...
    pub struct AdminSignedPayload{ 
        ...
        pub fun toString(): String{ 
            return self.type.concat("-").concat(self.creator.toString()).concat(
                "-"
            ).concat(self.expiration.toString())
        }
    }
    
    pub resource interface VendorPublic{ 
        pub fun purchaseDomain(
    ...
    pub resource Vendor: VendorPublic{ 
        priv let minterCap: Capability<&{IMinter}>
        
        priv var prices:{ String: PriceData}
        
        priv var paymentReceiverCaps:{ String: Capability<&{FungibleToken.Receiver}>}
        
        init(minterCap: Capability<&{IMinter}>, prices:{ String: PriceData}, paymentReceiverCaps:{ String: Capability<&AnyResource{FungibleToken.Receiver}>}){ 
        ...
        pub fun setPaymentReceiver(salePaymentVaultType: String, paymentReceiverCap: Capability<&{FungibleToken.Receiver}>){ 
            pre{ 
        ...
        priv fun verifyAdminSignedPayload(signedPayloadData: AdminSignedPayload, signature: String): Bool{ 
        ...
        priv fun validateDomain(domain: String, ownerAddress: Address){ 
            var i = 0
            var index = -1
            ...
            if domainExtension == "flowverse"{ 
                let mysteryPassCollectionRef = getAccount(ownerAddress).getCapability(FlowversePass.CollectionPublicPath).borrow<&{FlowversePass.CollectionPublic}>() ?? panic("FlowversePass Collection reference not found")
            ...
            if domainExtension == "socks"{ 
                let socksCollectionRef = getAccount(ownerAddress).getCapability(FlowverseSocks.CollectionPublicPath).borrow<&{FlowverseSocks.FlowverseSocksCollectionPublic}>() ?? panic("FlowverseSocks Collection reference not found")
            ...
            if domainExtension == "shirt"{ 
                let shirtCollectionRef = getAccount(ownerAddress).getCapability(FlowverseShirt.CollectionPublicPath).borrow<&{FlowverseShirt.CollectionPublic}>() ?? panic("FlowverseShirt Collection reference not found")
                assert(shirtCollectionRef.getIDs().length > 0, message: "ineligible for .shirt domain as user does not own a Flowverse Shirt")
            }
        }
        
        pub fun purchaseDomain(payment: @FungibleToken.Vault, data: PurchaseData, adminSignedPayload: AdminSignedPayload, signature: String){ 
            pre{ 
                data.type == "domain":
        ...
        pub fun purchaseText(payment: @FungibleToken.Vault, data: PurchaseData, adminSignedPayload: AdminSignedPayload, signature: String){ 
            pre{ 
                data.type == "text":
        ...
        pub fun purchaseImage(payment: @FungibleToken.Vault, data: PurchaseData, adminSignedPayload: AdminSignedPayload, signature: String){ 
            pre{ 
                data.type == "image":
        ...
        priv fun handlePurchase(payment: @FungibleToken.Vault, data: PurchaseData){ 
            pre{ 
    ...
    pub resource Admin{ 
        pub fun initialise(
            ...
            paymentReceiverCaps:{ 
                String: Capability<&AnyResource{FungibleToken.Receiver}>
            }
        ){ 
            pre{ 
        ...
        pub fun addRestrictedID(id: UInt64){ 
            pre{ 
    ...
    pub fun getPrice(
        ...
        let price =
            priceData.price[salePaymentVaultType]
            ?? panic("payment type not supported")
    ...
    pub fun getDomainSize(domain: String): UInt64{ 
        var i = 0
        for c in domain{ 
            if c == "."{ 
                break
    ...
    pub fun checkDomainAvailability(domain: String): Bool{ 
        return OrdinalVendor.domains[domain] == nil
    }
    
    pub fun checkOrdinalRestricted(id: UInt64): Bool{ 
