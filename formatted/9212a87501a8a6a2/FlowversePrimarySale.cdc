...
pub contract FlowversePrimarySale{ 
    ...
    pub event PrimarySaleDateUpdated(
        primarySaleID: UInt64,
        date: String,
        isLaunch: Bool
    )
    ...
    pub resource PrimarySale: PrimarySalePublic{ 
        ...
        priv var pooled: Bool
        
        priv let minterCap: Capability<&{IMinter}>
        
        priv var paymentReceiverCap: Capability<&{FungibleToken.Receiver}>
        ...
        pub fun purchaseRandomNFTs(payment: @FungibleToken.Vault, data: PurchaseDataRandom, adminSignedPayload: AdminSignedPayload, signature: String){ 
            pre{ 
        ...
        pub fun purchaseNFTs(payment: @FungibleToken.Vault, data: PurchaseData, adminSignedPayload: AdminSignedPayload, signature: String){ 
            pre{ 
                self.primarySaleID == data.primarySaleID:
                    "primarySaleID mismatch"
                self.status == PrimarySaleStatus.OPEN:
        ...
        pub fun claimTreasures(primarySaleID: UInt64, pool: String?, claimerAddress: Address, claimerCollectionRef: &{NonFungibleToken.Receiver}, adminSignedPayload: AdminSignedPayload, signature: String){ 
            pre{ 
                self.primarySaleID == primarySaleID:
                    "primarySaleID mismatch"
                self.status == PrimarySaleStatus.OPEN:
            ...
            let mysteryPassCollectionRef = getAccount(claimerAddress).getCapability(FlowversePass.CollectionPublicPath).borrow<&{FlowversePass.CollectionPublic}>() ?? panic("FlowversePass Collection reference not found")
            ...
            var numSocksOwned = 0
            if pool == "sockholders"{ 
                let socksCollectionRef = getAccount(claimerAddress).getCapability(FlowverseSocks.CollectionPublicPath).borrow<&{FlowverseSocks.FlowverseSocksCollectionPublic}>() ?? panic("FlowverseSocks Collection reference not found")
    ...
    pub resource Admin{ 
        pub fun createPrimarySale(
            ...
            let key =
                contractName.concat(
                    contractAddress.toString().concat(setID.toString())
                )
            assert(
            ...
            FlowversePrimarySale.nextPrimarySaleID = FlowversePrimarySale
                    .nextPrimarySaleID
                + UInt64(1)
    ...
    pub fun getPrimarySaleData(primarySaleID: UInt64): PrimarySaleData{ 
        ...
        let primarySale =
            (
                &FlowversePrimarySale.primarySales[primarySaleID]
                as
                &PrimarySale?
            )!
    ...
    pub fun purchaseRandomNFTs(
        ...
        let primarySale =
            (
                &FlowversePrimarySale.primarySales[data.primarySaleID]
                as
                &PrimarySale?
            )!
