...
pub contract FlowversePrimarySaleV2{ 
    ...
    pub resource PrimarySale: PrimarySalePublic{ 
        priv var primarySaleID: UInt64
        
        priv var contractName: String
        
        priv var contractAddress: Address
        
        priv var status: PrimarySaleStatus
        
        priv var prices:{ String: PriceData}
        
        priv var pooledEntities:{ String:{ UInt64: UInt64}}
        
        priv var numMintedPerUser:{ String:{ Address: UInt64}}
        
        priv var launchDate: String
        
        priv var endDate: String
        
        priv let minterCap: Capability<&{IMinter}>
        
        priv var paymentReceiverCaps:{ String: Capability<&{FungibleToken.Receiver}>}
        
        init(contractName: String, contractAddress: Address, prices:{ String: PriceData}, minterCap: Capability<&{IMinter}>, paymentReceiverCaps:{ String: Capability<&AnyResource{FungibleToken.Receiver}>}, launchDate: String, endDate: String){ 
        ...
        pub fun purchaseHeroesBox(payment: @FungibleToken.Vault, data: PurchaseDataSequential, adminSignedPayload: AdminSignedPayload, signature: String){ 
            pre{ 
                ...
                self.contractName == "HeroesOfTheFlow":
                    "only supports heroes of the flow contract"
            }
            let pool = data.pool
            ...
            let purchaseData = PurchaseData(primarySaleID: data.primarySaleID, purchaserAddress: data.purchaserAddress, purchaserCollectionRef: data.purchaserCollectionRef, orders: orders, pool: data.pool)
            self.purchase(payment: <-payment, data: purchaseData)
        }
        
        pub fun purchaseSequentialNFTs(payment: @FungibleToken.Vault, data: PurchaseDataSequential, adminSignedPayload: AdminSignedPayload, signature: String){ 
            pre{ 
                self.primarySaleID == data.primarySaleID:
                    "primarySaleID mismatch"
                self.status == PrimarySaleStatus.OPEN:
                ...
                self.verifyAdminSignedPayload(signedPayloadData: adminSignedPayload, signature: signature):
                    "failed to validate signature for the primary sale purchase"
                self.contractName != "HeroesOfTheFlow":
                    "HeroesOfTheFlow does not support sequential purchase"
            }
            let pool = data.pool
            let supply = self.getSupply(pool: pool)
        ...
        priv fun purchase(payment: @FungibleToken.Vault, data: PurchaseData){ 
            pre{ 
                self.primarySaleID == data.primarySaleID:
                    "primarySaleID mismatch"
                self.status == PrimarySaleStatus.OPEN:
                ...
                self.minterCap.borrow() != nil:
                    "cannot borrow minter"
                self.pooledEntities.containsKey(data.pool):
                    "Pool does not exist"
            }
    ...
    pub resource Admin{ 
        pub fun createPrimarySale(
            ...
            let key = contractName.concat(contractAddress.toString())
            assert(
                !FlowversePrimarySaleV2.primarySaleIDs.containsKey(key),
                message: "Primary sale with contractName, contractAddress already exists"
            )
            var primarySale <-
                create PrimarySale(
            ...
            FlowversePrimarySaleV2.nextPrimarySaleID = FlowversePrimarySaleV2
                    .nextPrimarySaleID
                + UInt64(1)
    ...
    pub struct PrimarySaleData{ 
        ...
        init(
            ...
            supply: UInt64,
            prices:{ 
                String: FlowversePrimarySaleV2.PriceData
            },
            status: String,
            pooledEntities:{ 
                String:{ 
    ...
    pub fun getPrimarySaleData(primarySaleID: UInt64): PrimarySaleData{ 
        pre{ 
            FlowversePrimarySaleV2.primarySales.containsKey(primarySaleID):
                "Primary sale does not exist"
        }
        let primarySale =
            (
                &FlowversePrimarySaleV2.primarySales[primarySaleID]
                as
                &PrimarySale?
            )!
    ...
    pub fun getPrice(
        ...
        assert(
            (prices[pool]!).price.containsKey(salePaymentVaultType),
            message: "salePaymentVaultType not supported"
        )
    ...
    pub fun purchaseHeroesBox(
        ...
        pre{ 
            FlowversePrimarySaleV2.primarySales.containsKey(data.primarySaleID):
                "Primary sale does not exist"
        }
        let primarySale =
            (
                &FlowversePrimarySaleV2.primarySales[data.primarySaleID]
                as
                &PrimarySale?
            )!
        primarySale.purchaseHeroesBox(
            payment: <-payment,
            data: data,
            adminSignedPayload: adminSignedPayload,
            signature: signature
    ...
    pub fun purchaseSequentialNFTs(
        ...
        primarySale.purchaseSequentialNFTs(
            payment: <-payment,
            data: data,
            adminSignedPayload: adminSignedPayload,
            signature: signature
