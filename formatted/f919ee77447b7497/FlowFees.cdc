...
pub contract FlowFees{ 
    ...
    pub event FeeParametersChanged(
        surgeFactor: UFix64,
    ...
    priv var vault: @FlowToken.Vault
    ...
    pub resource Administrator{ 
        ...
        pub fun withdrawTokensFromFeeVault(
            amount: UFix64
        ): @FungibleToken.Vault{ 
        ...
        pub fun setFeeSurgeFactor(_ surgeFactor: UFix64){ 
            ...
            let newParameters =
                FeeParameters(
                    surgeFactor: surgeFactor,
                    inclusionEffortCost: oldParameters.inclusionEffortCost,
                    executionEffortCost: oldParameters.executionEffortCost
                )
            FlowFees.setFeeParameters(newParameters)
        }
    }
    ...
    pub struct VerifyPayerBalanceResult{ 
        ...
        init(
            canExecuteTransaction: Bool,
            requiredBalance: UFix64,
            maximumTransactionFees: UFix64
        ){ 
    ...
    pub fun verifyPayersBalanceForTransactionExecution(
        ...
        var maxTransactionFee =
            self.computeFees(
                inclusionEffort: inclusionEffort,
                executionEffort: maxExecutionEffort
            )
        
        // Get the minimum required payer's balance for the transaction.
        let minimumRequiredBalance =
            FlowStorageFees.defaultTokenReservedBalance(payerAcct.address)
        if minimumRequiredBalance == UFix64(0){ 
            // If the required balance is zero exit early.
            return VerifyPayerBalanceResult(canExecuteTransaction: true, requiredBalance: minimumRequiredBalance, maximumTransactionFees: maxTransactionFee)
        }
        ...
        var balance = 0.0
        if let tokenVault =
            payerAcct.borrow<&FlowToken.Vault>(from: /storage/flowTokenVault){ 
    ...
    pub fun deductTransactionFee(
        ...
        let tokenVault =
            acct.borrow<&FlowToken.Vault>(from: /storage/flowTokenVault)
    ...
    pub fun getFeeParameters(): FeeParameters{ 
        return self.account.copy<FeeParameters>(
            from: /storage/FlowTxFeeParameters
        )
        ?? panic(
            "Error getting tx fee parameters. They need to be initialized first!"
        )
    }
    
    priv fun setFeeParameters(_ feeParameters: FeeParameters){ 
        ...
        emit FeeParametersChanged(
            surgeFactor: feeParameters.surgeFactor,
            inclusionEffortCost: feeParameters.inclusionEffortCost,
            executionEffortCost: feeParameters.executionEffortCost
        )
    }
    ...
    pub fun computeFees(
        ...
        let totalFees =
            params.surgeFactor
            * (
                inclusionEffort * params.inclusionEffortCost
                + executionEffort * params.executionEffortCost
            )
        return totalFees
    }
