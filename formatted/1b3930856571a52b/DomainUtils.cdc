...
pub contract DomainUtils{ 
    pub fun getAddressesOfDomains(names: [String], roots: [String]):{ 
        String: Address
    }{ 
        assert(
            names.length == roots.length,
            message: "names and roots should have the same length"
        )
    ...
    pub fun getDefaultDomainsOfAddresses(_ addresses: [Address]):{ 
        Address:{ 
            String: String
        }
    }{ 
