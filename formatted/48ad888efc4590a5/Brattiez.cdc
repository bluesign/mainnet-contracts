...
pub contract Brattiez: NonFungibleToken{ 
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &Brattiez.NFT?{ 
        let collection = (getAccount(from).getCapability(Brattiez.CollectionPublicPath)!).borrow<&Brattiez.Collection{Brattiez.BrattiezCollectionPublic}>() ?? panic("Couldn't get collection")
