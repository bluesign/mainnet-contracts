...
pub contract MetadataViews{ 
    ...
    // NFTView 将所有核心视图包装为“id”和“uuid”字段，并用于提供 NFT 的完整图片。大多数 NFT 应该实现此视图。
    pub struct NFTView{ 
        pub let id: UInt64
    ...
    // 获取 NFT VIEW 的函数
    /// @param id:  NFT id
    /// @param viewResolver: 对解析程序资源的引用
    /// @return 返回一个 NFTView 结构
    ...
    pub struct Royalty{ 
        ...
        init(
            ...
            pre{ 
                cut >= 0.0 && cut <= 1.0:
                    "Cut value should be in valid range i.e [0,1]"
            }
    ...
    ///
    pub fun getNFTCollectionData(
        _ viewResolver: &{Resolver}
    ): NFTCollectionData?{ 
