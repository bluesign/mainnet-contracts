...
pub contract Fuchibola_NFT: NonFungibleToken{ 
    ...
    pub resource Series{ 
        ...
        init(seriesId: UInt32, metadata:{ String: String}){ 
            ...
            self.setIds = []
            self.setSealedState ={} 
            Fuchibola_NFT.seriesData[seriesId] = SeriesData(seriesId: seriesId, metadata: metadata)
        ...
        //
        pub fun mintFuchibola_NFT(recipient: &{NonFungibleToken.CollectionPublic}, tokenId: UInt64, setId: UInt32){ 
            pre{ 
            ...
            recipient.deposit(token: <-create Fuchibola_NFT.NFT(tokenId: tokenId, setId: setId, editionNum: editionNum))
        ...
        // mintEditionFuchibola_NFT
        // Mints a new NFT with a new ID and specific edition Num (random open edition)
        // and deposits it in the recipients collection using their collection reference
        //
        pub fun mintEditionFuchibola_NFT(recipient: &{NonFungibleToken.CollectionPublic}, tokenId: UInt64, setId: UInt32, edition: UInt32){ 
            pre{ 
                self.numberEditionsMintedPerSet[setId] != nil:
                    "The Set does not exist."
                self.numberEditionsMintedPerSet[setId]! <= Fuchibola_NFT.getSetMaxEditions(setId: setId)!:
            ...
            // deposit it in the recipient's account using their reference
            recipient.deposit(token: <-create Fuchibola_NFT.NFT(tokenId: tokenId, setId: setId, editionNum: edition))
        ...
        pub fun batchMintFuchibola_NFT(recipient: &{NonFungibleToken.CollectionPublic}, setId: UInt32, tokenIds: [UInt64]){ 
            pre{ 
                tokenIds.length > 0:
                    "Number of token Ids must be > 0"
            }
            for tokenId in tokenIds{ 
                self.mintFuchibola_NFT(recipient: recipient, tokenId: tokenId, setId: setId)
            }
        }
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                case Type<MetadataViews.Display>():
                    return MetadataViews.Display(name: Fuchibola_NFT.getSetMetadataByField(setId: self.setId, field: "name")!, description: Fuchibola_NFT.getSetMetadataByField(setId: self.setId, field: "description")!, thumbnail: MetadataViews.HTTPFile(url: Fuchibola_NFT.getSetMetadataByField(setId: self.setId, field: "preview")!))
                case Type<MetadataViews.Serial>():
                    return MetadataViews.Serial(self.id)
                case Type<MetadataViews.Editions>():
                    let maxEditions = Fuchibola_NFT.setData[self.setId]?.maxEditions ?? 0
                    let editionName = Fuchibola_NFT.getSetMetadataByField(setId: self.setId, field: "name")!
                ...
                case Type<MetadataViews.ExternalURL>():
                    if let externalBaseURL = Fuchibola_NFT.getSetMetadataByField(setId: self.setId, field: "external_token_base_url"){ 
                    ...
                    for metadataKey in (Fuchibola_NFT.getSetMetadata(setId: self.setId)!).keys{ 
                        ...
                        if metadataKey.length >= 13{ 
                            if metadataKey.slice(from: 0, upTo: 13) == "royalty_addr_"{ 
                                ...
                                if cutValue != 0.0{ 
                                    royalties.append(MetadataViews.Royalty(receiver: getAccount(royaltyAddress).getCapability<&FungibleToken.Vault{FungibleToken.Receiver}>(royaltyReceiver), cut: cutValue, description: Fuchibola_NFT.getSetMetadataByField(setId: self.setId, field: "royalty_desc_".concat(royaltyName))!))
                                }
                            }
                        }
                    }
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: Fuchibola_NFT.CollectionStoragePath, publicPath: Fuchibola_NFT.CollectionPublicPath, providerPath: /private/Fuchibola_NFT, publicCollection: Type<&Fuchibola_NFT.Collection{Fuchibola_NFT.Fuchibola_NFTCollectionPublic, NonFungibleToken.CollectionPublic}>(), publicLinkedType: Type<&Fuchibola_NFT.Collection{Fuchibola_NFT.Fuchibola_NFTCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&Fuchibola_NFT.Collection{Fuchibola_NFT.Fuchibola_NFTCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
                            return <-Fuchibola_NFT.createEmptyCollection()
                        })
                case Type<MetadataViews.NFTCollectionDisplay>():
                    let squareImage = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://media.gigantik.io/fuchibola/square.png"), mediaType: "image/png")
                    let bannerImage = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://media.gigantik.io/fuchibola/banner.png"), mediaType: "image/png")
                    ...
                    for metadataKey in (Fuchibola_NFT.getSetMetadata(setId: self.setId)!).keys{ 
                        ...
                        if metadataKey.length >= 18{ 
                            if metadataKey.slice(from: 0, upTo: 18) == "collection_social_"{ 
                                ...
                                socials.insert(key: metadataKey.slice(from: 18, upTo: metadataKey.length), MetadataViews.ExternalURL(Fuchibola_NFT.getSetMetadataByField(setId: self.setId, field: metadataKey)!))
                            }
                        }
                    }
                    return MetadataViews.NFTCollectionDisplay(name: Fuchibola_NFT.getSetMetadataByField(setId: self.setId, field: "collection_name") ?? "", description: Fuchibola_NFT.getSetMetadataByField(setId: self.setId, field: "collection_description") ?? "", externalURL: MetadataViews.ExternalURL(Fuchibola_NFT.getSetMetadataByField(setId: self.setId, field: "external_url") ?? ""), squareImage: squareImage, bannerImage: bannerImage, socials: socials)
                ...
                case Type<MetadataViews.Medias>():
                    return MetadataViews.Medias(items: [MetadataViews.Media(file: MetadataViews.HTTPFile(url: Fuchibola_NFT.getSetMetadataByField(setId: self.setId, field: "image")!), mediaType: self.getMimeType())])
            }
            return nil
        }
        
        pub fun getMimeType(): String{ 
            var metadataFileType = (Fuchibola_NFT.getSetMetadataByField(setId: self.setId, field: "image_file_type")!).toLower()
    ...
    pub fun fetch(_ from: Address, id: UInt64): &Fuchibola_NFT.NFT?{ 
        let collection = getAccount(from).getCapability(Fuchibola_NFT.CollectionPublicPath).borrow<&Fuchibola_NFT.Collection{Fuchibola_NFT.Fuchibola_NFTCollectionPublic}>() ?? panic("Couldn't get collection")
    ...
    init(){ 
        ...
        self.account.link<&Fuchibola_NFT.Admin>(self.AdminPrivatePath, target: self.AdminStoragePath) ?? panic("Could not get a capability to the admin")
