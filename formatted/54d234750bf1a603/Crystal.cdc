...
pub contract Crystal: NonFungibleToken{ 
    ...
    pub event CrystalMinted(id: UInt64, shardIDs: [UInt64], clipIDs: [UInt32], momentIDs: [UInt32], purity: Int)
    ...
    pub fun getShardsByIDs(owner: Address, ids: [UInt64]): [&Shard.NFT]{ 
        ...
        let collection = getAccount(owner).getCapability(/public/EternalShardCollection).borrow<&{Shard.ShardCollectionPublic}>() ?? panic("Could not get receiver reference to the Shard Collection")
    ...
    init(){ 
        ...
        self.account.link<&{Crystal.CrystalCollectionPublic}>(/public/EternalCrystalCollection, target: /storage/EternalCrystalCollection)
