...
pub contract PopsyclPackMarketplace{ 
    ...
    pub resource PackSaleCollection: SalePublic{ 
        ...
        priv let packForSale: @{UInt64: PopsyclPack.NFT}
        
        // Dictionary of the NFTs that the user is putting up for sale  
        priv let forSale: @{UInt64: Popsycl.NFT}
        
        // Dictionary of the pack prices for each NFT by ID
        priv let packPrices:{ UInt64: UFix64}
        
        // Dictionary of the pack prices for each NFT by ID
        priv let packCreators:{ UInt64: Address?}
        
        // Dictionary of the pack prices for each NFT by ID
        priv let packInfluencer:{ UInt64: Address?}
        
        // Dictionary of pack sale type (either sale/resale)
        priv let packResale:{ UInt64: Bool}
        
        // Dictionary of pack royalty for each NFT by ID
        priv let packRoylty:{ UInt64: UFix64}
        
        // Dictionary of pack token owner
        priv let packSeller:{ UInt64: Address?}
        ...
        pub fun packPurchase(packId: UInt64, tokens: [UInt64], recipient: &{Popsycl.PopsyclCollectionPublic}, payment: @FungibleToken.Vault){ 
            ...
            // Popvalut reference
            let PopsyclvaultRef = getAccount(PopsyclMarketplace.marketAddress).getCapability(/public/flowTokenReceiver).borrow<&{FungibleToken.Receiver}>() ?? panic("failed to borrow reference to PopsyclPackMarketplace vault")
            // influencer vault reference
            let influencerVaultRef = getAccount(owner).getCapability(/public/flowTokenReceiver).borrow<&{FungibleToken.Receiver}>() ?? panic("failed to borrow reference to owner vault")
        ...
        pub fun saleWithdrawn(packId: UInt64){ 
            ...
            let receiver = getAccount(owner).getCapability(PopsyclPack.PopsyclPackPublicPath).borrow<&{PopsyclPack.PopsyclPackCollectionPublic}>() ?? panic("Could not get receiver reference to the NFT Collection")
    ...
    pub fun createPackSaleCollection(
