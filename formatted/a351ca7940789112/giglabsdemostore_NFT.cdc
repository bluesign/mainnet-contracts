...
pub contract giglabsdemostore_NFT: NonFungibleToken{ 
    
    // giglabsdemostore_NFT Events
    //
    // Emitted when the giglabsdemostore_NFT contract is created
    ...
    // totalSupply
    // The total number of giglabsdemostore_NFT that have been minted
    //
    ...
    // A Series is special resource type that contains functions to mint giglabsdemostore_NFT NFTs, 
    ...
    pub resource Series{ 
        ...
        init(seriesId: UInt32, metadata:{ String: String}){ 
            ...
            self.setIds = []
            self.setSealedState ={} 
            giglabsdemostore_NFT.seriesData[seriesId] = SeriesData(seriesId: seriesId, metadata: metadata)
        ...
        pub fun addNftSet(setId: UInt32, maxEditions: UInt32, ipfsMetadataHashes:{ UInt32: String}, metadata:{ String: String}){ 
            ...
            giglabsdemostore_NFT.setData[setId] = newNFTSet
        ...
        pub fun updateSeriesMetadata(metadata:{ String: String}){ 
            ...
            giglabsdemostore_NFT.seriesData[self.seriesId] = newSeriesMetadata
        ...
        pub fun updateSetMetadata(setId: UInt32, maxEditions: UInt32, ipfsMetadataHashes:{ UInt32: String}, metadata:{ String: String}){ 
            ...
            giglabsdemostore_NFT.setData[setId] = newSetMetadata
            emit SetMetadataUpdated(seriesId: self.seriesId, setId: setId)
        }
        
        // mintgiglabsdemostore_NFT
        // Mints a new NFT with a new ID
        // and deposits it in the recipients collection using their collection reference
        //
        pub fun mintgiglabsdemostore_NFT(recipient: &{NonFungibleToken.CollectionPublic}, tokenId: UInt64, setId: UInt32){ 
            pre{ 
                self.numberEditionsMintedPerSet[setId] != nil:
                    "The Set does not exist."
                self.numberEditionsMintedPerSet[setId]! <= giglabsdemostore_NFT.getSetMaxEditions(setId: setId)!:
            ...
            recipient.deposit(token: <-create giglabsdemostore_NFT.NFT(tokenId: tokenId, setId: setId, editionNum: editionNum))
            
            // Increment the count of global NFTs 
            giglabsdemostore_NFT.totalSupply = giglabsdemostore_NFT.totalSupply + 1 as UInt64
            
            // Update the count of Editions minted in the set
            self.numberEditionsMintedPerSet[setId] = editionNum
        }
        
        // mintEditiongiglabsdemostore_NFT
        // Mints a new NFT with a new ID and specific edition Num (random open edition)
        // and deposits it in the recipients collection using their collection reference
        //
        pub fun mintEditiongiglabsdemostore_NFT(recipient: &{NonFungibleToken.CollectionPublic}, tokenId: UInt64, setId: UInt32, edition: UInt32){ 
            pre{ 
                self.numberEditionsMintedPerSet[setId] != nil:
                    "The Set does not exist."
                self.numberEditionsMintedPerSet[setId]! <= giglabsdemostore_NFT.getSetMaxEditions(setId: setId)!:
            ...
            // deposit it in the recipient's account using their reference
            recipient.deposit(token: <-create giglabsdemostore_NFT.NFT(tokenId: tokenId, setId: setId, editionNum: edition))
        ...
        pub fun batchMintgiglabsdemostore_NFT(recipient: &{NonFungibleToken.CollectionPublic}, setId: UInt32, tokenIds: [UInt64]){ 
            pre{ 
                tokenIds.length > 0:
                    "Number of token Ids must be > 0"
            }
            for tokenId in tokenIds{ 
                self.mintgiglabsdemostore_NFT(recipient: recipient, tokenId: tokenId, setId: setId)
            }
        }
    ...
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver{ 
        ...
        init(tokenId: UInt64, setId: UInt32, editionNum: UInt32){ 
            ...
            let seriesId = giglabsdemostore_NFT.getSetSeriesId(setId: setId)!
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                case Type<MetadataViews.Display>():
                    return MetadataViews.Display(name: giglabsdemostore_NFT.getSetMetadataByField(setId: self.setId, field: "name")!, description: giglabsdemostore_NFT.getSetMetadataByField(setId: self.setId, field: "description")!, thumbnail: MetadataViews.HTTPFile(url: giglabsdemostore_NFT.getSetMetadataByField(setId: self.setId, field: "preview")!))
                case Type<MetadataViews.Serial>():
                    return MetadataViews.Serial(self.id)
                case Type<MetadataViews.Editions>():
                    let maxEditions = giglabsdemostore_NFT.setData[self.setId]?.maxEditions ?? 0
                ...
                case Type<MetadataViews.ExternalURL>():
                    if let externalBaseURL = giglabsdemostore_NFT.getSetMetadataByField(setId: self.setId, field: "external_token_base_url"){ 
                ...
                case Type<MetadataViews.Royalties>():
                    ...
                    for metadataKey in (giglabsdemostore_NFT.getSetMetadata(setId: self.setId)!).keys{ 
                        ...
                        if metadataKey.length >= 13{ 
                            if metadataKey.slice(from: 0, upTo: 13) == "royalty_addr_"{ 
                                ...
                                let royaltyAddress = giglabsdemostore_NFT.convertStringToAddress(giglabsdemostore_NFT.getSetMetadataByField(setId: self.setId, field: "royalty_addr_".concat(royaltyName))!)!
                                let royaltyReceiver: PublicPath = PublicPath(identifier: giglabsdemostore_NFT.getSetMetadataByField(setId: self.setId, field: "royalty_rcv_".concat(royaltyName))!)!
                                let royaltyCut = giglabsdemostore_NFT.getSetMetadataByField(setId: self.setId, field: "royalty_cut_".concat(royaltyName))!
                                let cutValue: UFix64 = giglabsdemostore_NFT.royaltyCutStringToUFix64(royaltyCut)
                                if cutValue != 0.0{ 
                                    royalties.append(MetadataViews.Royalty(receiver: getAccount(royaltyAddress).getCapability<&FungibleToken.Vault{FungibleToken.Receiver}>(royaltyReceiver), cut: cutValue, description: giglabsdemostore_NFT.getSetMetadataByField(setId: self.setId, field: "royalty_desc_".concat(royaltyName))!))
                                }
                            }
                        }
                    }
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: giglabsdemostore_NFT.CollectionStoragePath, publicPath: giglabsdemostore_NFT.CollectionPublicPath, providerPath: /private/giglabsdemostore_NFT, publicCollection: Type<&giglabsdemostore_NFT.Collection{giglabsdemostore_NFT.giglabsdemostore_NFTCollectionPublic, NonFungibleToken.CollectionPublic}>(), publicLinkedType: Type<&giglabsdemostore_NFT.Collection{giglabsdemostore_NFT.giglabsdemostore_NFTCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&giglabsdemostore_NFT.Collection{giglabsdemostore_NFT.giglabsdemostore_NFTCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
                            return <-giglabsdemostore_NFT.createEmptyCollection()
                        })
                case Type<MetadataViews.NFTCollectionDisplay>():
                    let squareImage = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://media.gigantik.io/giglabsdemostore/square.png"), mediaType: "image/png")
                    let bannerImage = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://media.gigantik.io/giglabsdemostore/banner.png"), mediaType: "image/png")
                    ...
                    for metadataKey in (giglabsdemostore_NFT.getSetMetadata(setId: self.setId)!).keys{ 
                        ...
                        if metadataKey.length >= 18{ 
                            if metadataKey.slice(from: 0, upTo: 18) == "collection_social_"{ 
                                ...
                                socials.insert(key: metadataKey.slice(from: 18, upTo: metadataKey.length), MetadataViews.ExternalURL(giglabsdemostore_NFT.getSetMetadataByField(setId: self.setId, field: metadataKey)!))
                            }
                        }
                    }
                    return MetadataViews.NFTCollectionDisplay(name: giglabsdemostore_NFT.getSetMetadataByField(setId: self.setId, field: "collection_name") ?? "", description: giglabsdemostore_NFT.getSetMetadataByField(setId: self.setId, field: "collection_description") ?? "", externalURL: MetadataViews.ExternalURL(giglabsdemostore_NFT.getSetMetadataByField(setId: self.setId, field: "external_url") ?? ""), squareImage: squareImage, bannerImage: bannerImage, socials: socials)
                    ...
                    for metadataKey in (giglabsdemostore_NFT.getSetMetadata(setId: self.setId)!).keys{ 
                        ...
                        if metadataKey.length >= 6{ 
                            if metadataKey.slice(from: 0, upTo: 6) == "trait_"{ 
                                ...
                                traitDictionary.insert(key: metadataKey.slice(from: 6, upTo: metadataKey.length), giglabsdemostore_NFT.getSetMetadataByField(setId: self.setId, field: metadataKey)!)
                            }
                        }
                    }
    ...
    // to allow others to deposit giglabsdemostore_NFT into their Collection. It also allows for reading
    // the details of giglabsdemostore_NFT in the Collection.
    pub resource interface giglabsdemostore_NFTCollectionPublic{ 
        ...
        pub fun borrowgiglabsdemostore_NFT(id: UInt64): &giglabsdemostore_NFT.NFT?{ 
            // If the result isn't nil, the id of the returned reference
            // should be the same as the argument to the function
            post{ 
                result == nil || result?.id == id:
                    "Cannot borrow giglabsdemostore_NFT reference: The ID of the returned reference is incorrect"
            }
        }
    }
    
    // Collection
    // A collection of giglabsdemostore_NFT NFTs owned by an account
    //
    pub resource Collection: giglabsdemostore_NFTCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection{ 
        ...
        // borrowgiglabsdemostore_NFT
        // Gets a reference to an NFT in the collection as a giglabsdemostore_NFT,
        ...
        pub fun borrowViewResolver(id: UInt64): &AnyResource{MetadataViews.Resolver}{ 
            ...
            let giglabsdemostore_NFTNft = nft as! &giglabsdemostore_NFT.NFT
            return giglabsdemostore_NFTNft as &AnyResource{MetadataViews.Resolver}
        }
    ...
    // fetch
    // Get a reference to a giglabsdemostore_NFT from an account's Collection, if available.
    // If an account does not have a giglabsdemostore_NFT.Collection, panic.
    ...
    //
    pub fun fetch(_ from: Address, id: UInt64): &giglabsdemostore_NFT.NFT?{ 
        let collection = getAccount(from).getCapability(giglabsdemostore_NFT.CollectionPublicPath).borrow<&giglabsdemostore_NFT.Collection{giglabsdemostore_NFT.giglabsdemostore_NFTCollectionPublic}>() ?? panic("Couldn't get collection")
    ...
    pub fun getAllSeries(): [giglabsdemostore_NFT.SeriesData]{ 
    ...
    pub fun getAllSets(): [giglabsdemostore_NFT.NFTSetData]{ 
    ...
    init(){ 
        // Set named paths
        self.CollectionStoragePath = /storage/giglabsdemostore_NFTCollection
        self.CollectionPublicPath = /public/giglabsdemostore_NFTCollection
        self.AdminStoragePath = /storage/giglabsdemostore_NFTAdmin
        self.AdminPrivatePath = /private/giglabsdemostore_NFTAdminUpgrade
        ...
        self.account.link<&giglabsdemostore_NFT.Admin>(self.AdminPrivatePath, target: self.AdminStoragePath) ?? panic("Could not get a capability to the admin")
