...
pub contract LPStaking{ 
    ...
    priv let pools: [Address]
    
    priv let poolMap:{ Int: Address} // vaultId -> pool
    
    priv let pairMap:{ Address: Address} // pair -> pool
    ...
    pub event PoolTemplateAddressChanged(
        oldTemplate: Address,
        newTemplate: Address
    )
    
    pub event PoolAccountPublicKeyChanged(
        oldPublicKey: String?,
        newPublicKey: String?
    )
    ...
    pub fun createPool(
        ...
        let vaultRef =
            getAccount(vaultAddress).getCapability<
                &AnyResource{StarVaultInterfaces.VaultPublic}
            >(StarVaultConfig.VaultPublicPath).borrow()
        ...
        let pairAddr =
            SwapFactory.getPairAddress(
                token0Key: token0Key,
                token1Key: token1Key
            )
            ?? panic(
                "createPool: nonexistent pair ".concat(token0Key).concat(
                    " <-> "
                ).concat(token1Key).concat(", create pair first")
            )
        assert(
            !self.pairMap.containsKey(pairAddr),
            message: "LPStaking: pairAddr already exists"
        )
        (
        ...
        let poolTemplateContract =
            getAccount(self.poolTemplate).contracts.get(name: "RewardPool")!
    ...
    pub fun distributeFees(vaultId: Int, vault: @FungibleToken.Vault){ 
        ...
        let poolRef =
            getAccount(pool).getCapability<&{StarVaultInterfaces.PoolPublic}>(
                StarVaultConfig.PoolPublicPath
            ).borrow()!
        poolRef.queueNewRewards(vault: <-vault)
    }
    
    pub resource LPStakingCollection:
        StarVaultInterfaces.LPStakingCollectionPublic{
    
        priv var tokenVaults: @{Address: FungibleToken.Vault}
        ...
        pub fun deposit(
            ...
            let pairRef =
                getAccount(tokenAddress).getCapability<
                    &{SwapInterfaces.PairPublic}
                >(SwapConfig.PairPublicPath).borrow()!
            assert(
                tokenVault.getType() == pairRef.getLpTokenVaultType(),
                message: "LPStakingCollection: input token vault type mismatch with token vault"
            )
        ...
        pub fun withdraw(
            ...
            let vaultRef =
                (&self.tokenVaults[tokenAddress] as &FungibleToken.Vault?)!
        ...
        priv fun updateReward(tokenAddress: Address){ 
            let pool = LPStaking.pairMap[tokenAddress]!
            let poolRef =
                getAccount(pool).getCapability<
                    &{StarVaultInterfaces.PoolPublic}
                >(StarVaultConfig.PoolPublicPath).borrow()!
    ...
    init(poolTemplate: Address, poolAccountPublicKey: String){ 
        ...
        self.account.save(
            <-create Admin(),
            to: StarVaultConfig.LPStakingAdminStoragePath
        )
    }
}
