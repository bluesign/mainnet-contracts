...
pub contract PriceOracleStFlow: OracleInterface{ 
    ...
    priv let _ReaderWhiteList:{ Address: Bool}
    ...
    pub resource PriceFeeder: OracleInterface.PriceFeederPublic{ 
        ...
        /* Do nothing */        pub fun setExpiredDuration(blockheightDuration: UInt64){} 
        
        /* Do nothing */        pub fun fetchPrice(certificate: &OracleInterface.OracleCertificate): UFix64{ 
    ...
    init(flowPriceOracle: Address){ 
        ...
        let flowPriceOraclePublicInterfaceReaderRef = getAccount(flowPriceOracle).getCapability<&{OracleInterface.OraclePublicInterface_Reader}>(OracleConfig.OraclePublicInterface_ReaderPath).borrow() ?? panic("cannot borrow reference to Flow/USD PriceOracle Reader")
