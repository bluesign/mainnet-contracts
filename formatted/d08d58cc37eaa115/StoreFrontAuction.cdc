...
pub contract StoreFrontAuction{ 
    ...
    pub resource AuctionItem{ 
        ...
        init(
            ...
            self.meta = meta
            StoreFrontAuction.totalAuctions = StoreFrontAuction.totalAuctions
                + UInt64(1)
        }
        ...
        access(contract) fun updateRecipientVaultCap(
    ...
    pub struct ItemMeta{ 
        ...
        pub(set) var recipientCollectionCap: Capability<
        ...
        pub let ownerCollectionCap: Capability<
            &{NonFungibleToken.CollectionPublic}
        >
    ...
    pub resource AuctionCollection: AuctionPublic{ 
        ...
        pub fun getAuctionPrices():{ UInt64: UFix64}{ 
            ...
            for id in self.auctionItems.keys{ 
                let itemRef = (&self.auctionItems[id] as                                                         // settleAuction sends the auction item to the highest bidder
                                                         // and deposits the FungibleTokens into the auction owner's account
                                                         &AuctionItem?)!
                if itemRef.meta.auctionCompleted == false{ 
                    priceList[                              // return if there are no bids to settle
                              id] = itemRef.meta.currentPrice
                }
            }
        ...
        pub fun settleAuction(_ id                                  // placeBid sends the bidder's tokens to the bid vault and updates the
                                  // currentPrice of the current auction item
                                  : UInt64){ 
            let itemRef                        // Get the auction item resources
                        = (&self.auctionItems[id] as &AuctionItem?)!
            let itemMeta = itemRef.meta
            if itemMeta.auctionCompleted{ 
                panic("This auction is already settled")
            }
            if itemRef.NFT                           // Update the auction item
    ...
    init(){ 
        ...
        self
            .collectionPublicPath = /public/StoreFrontAuctionCollection0xd08d58cc37eaa115
        self
            .collectionStoragePath = /storage/StoreFrontAuctionCollection0xd08d58cc37eaa115
    }
}
