...
pub contract VeraTicket: NonFungibleToken{ 
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &VeraTicket.NFT?{ 
        let collection = getAccount(from).getCapability(VeraTicket.VeraTicketPubStorage).borrow<&VeraTicket.Collection{VeraTicket.TicketsCollectionPublic}>() ?? panic("Couldn't get collection")
