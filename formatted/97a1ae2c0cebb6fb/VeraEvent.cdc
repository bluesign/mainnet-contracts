...
pub contract VeraEvent{ 
    ...
    pub resource EventCollection{ 
        ...
        pub fun addEvent(
            veraevent: VeraEvent.EventStruct,
            metadata:{ 
                String: String
            }
        ){ 
        ...
        pub fun getEvent(eventId: UInt64): VeraEvent.EventStruct{ 
            let veraevent =
                self.eventsCollection[eventId] ?? panic("missing Event")
            return veraevent
        }
        
        pub fun getMetadata(eventId: UInt64):{ String: String}{ 
            let metadata =
                self.metadataObjs[eventId] ?? panic("missing Metadata")
            return metadata
        }
        ...
        pub fun incrementTicketMinted(
            ...
            let veraevent =
                self.eventsCollection[eventId] ?? panic("missing Event")
            (self.eventsCollection[eventId]!).incrementTicketMinted(
                tier: tier,
                subtier: subtier
            )
            if let eventTier: VeraEvent.Tier =
                (self.eventsCollection[eventId]!).tier[tier]{ 
                ((self.eventsCollection[eventId]!).tier[tier]!)
                    .incrementTicketMinted()
            }
            if let eventSubtier: VeraEvent.SubTier =
                ((self.eventsCollection[eventId]!).tier[tier]!).subtier[
                    subtier
                ]{ 
                (
                    ((self.eventsCollection[eventId]!).tier[tier]!).subtier[
        ...
        pub fun decrementTicketMinted(
            ...
            let veraevent =
                self.eventsCollection[eventId] ?? panic("missing Event")
            (self.eventsCollection[eventId]!).decrementTicketMinted(
                tier: tier,
                subtier: subtier
            )
            if let eventTier: VeraEvent.Tier =
    ...
    pub resource EventAdmin{ 
        ...
        pub fun createEvent(
            ...
            let collection =
                VeraEvent.account.borrow<&VeraEvent.EventCollection>(
