...
pub contract PublishedNFTStorefront{ 
    ...
    priv var nftIDbyListing:{ UInt64: UInt64}
    
    // Variable size dictionary of minted templates structs
    priv var numberPurchasedByType:{ UInt64: UInt64}
    ...
    init(){ 
        ...
        self.account.link<
            &PublishedNFTStorefront.Storefront{
                PublishedNFTStorefront.StorefrontPublic
            }
        >(self.StorefrontPublicPath, target: self.StorefrontStoragePath)
        if self.account.borrow<&FUSD.Vault>(from: /storage/fusdVault) == nil{ 
            ...
            self.account.link<&FUSD.Vault{FungibleToken.Provider}>(/public/fusdProvider, target: /storage/fusdProvider)
