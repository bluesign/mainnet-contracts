...
pub contract AvatarArtTransactionInfo{ 
    priv var acceptCurrencies: [Type]
    
    priv var nftInfo:{ UInt64: NFTInfo}
    ...
    pub event TransactionAddressUpdated(
        tokenId: UInt64,
        storing: Address?,
        insurance: Address?,
        contractor: Address?,
    ...
    pub event DefaultAddressChanged(
        storing: Address?,
        insurance: Address?,
        contractor: Address?,
        platform: Address?
    ...
    pub resource FeeInfo: PublicFeeInfo{ 
        ...
        // 1: Physical 1st
        // 2: Physical 2st
        pub var defaultFees:{ UInt: FeeInfoItem}
    ...
    pub resource TransactionAddress: PublicTransactionAddress{ 
        ...
        pub fun setAddress(tokenId: UInt64, payType: Type, storing: Capability<&{FungibleToken.Receiver}>?, insurance: Capability<&{FungibleToken.Receiver}>?, contractor: Capability<&{FungibleToken.Receiver}>?, platform: Capability<&{FungibleToken.Receiver}>?){ 
            pre{ 
                tokenId > 0:
                    "tokenId parameter is zero"
            }
        ...
        pub fun setDefaultAddress(payType: Type, storing: Capability<&{FungibleToken.Receiver}>?, insurance: Capability<&{FungibleToken.Receiver}>?, contractor: Capability<&{FungibleToken.Receiver}>?, platform: Capability<&{FungibleToken.Receiver}>?){ 
    ...
    init(){ 
        ...
        self
            .TransactionAddressStoragePath = /storage/avatarArtTransactionInfoRecepientAddress
        self
            .TransactionAddressPublicPath = /public/avatarArtTransactionInfoRecepientAddress
        ...
        self.account.save(
            <-transactionAddress,
            to: self.TransactionAddressStoragePath
        )
        self.account.link<
