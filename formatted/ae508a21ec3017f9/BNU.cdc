...
pub contract BNU: FungibleToken{ 
    ...
    init(){ 
        ...
        self.account.link<&BNU.Vault{FungibleToken.Receiver}>(self.ReceiverPath, target: self.StorageVaultPath)
        self.account.link<&BNU.Vault{FungibleToken.Balance}>(self.BalancePublicPath, target: self.StorageVaultPath)
    }
}
