...
pub contract ByteNextStaking{ 
    ...
    priv let lpVault: @FungibleToken.Vault
    
    priv let rewardVault: @FungibleToken.Vault
    
    priv let userInfo:{ Address: UserInfo}
    ...
    pub resource StakingProxy{ 
        pub fun deposit(vault: @BNU.Vault): @FungibleToken.Vault?{ 
            ...
            return <-ByteNextStaking.deposit(
        ...
        pub fun withdraw(amount: UFix64): @FungibleToken.Vault{ 
            ...
            return <-ByteNextStaking.withdraw(
                user: (self.owner!).address,
                amount: amount
            )
        }
        ...
        pub fun emergencyWithdraw(user: Address): @FungibleToken.Vault{ 
            ...
            return <-ByteNextStaking.emergencyWithdraw(
                user: (self.owner!).address
            )
        }
    }
    ...
    priv fun deposit(
        ...
        let userInfo =
            self.userInfo[user] ?? UserInfo(amount: 0.0, rewardDebt: 0.0)
        self.updatePool()
        ...
        userInfo.setRewardDebt(
            rewardDebt: userInfo.amount * self.accTokenPerShare
        )
    ...
    priv fun withdraw(user: Address, amount: UFix64): @FungibleToken.Vault{ 
        pre{ 
    ...
    priv fun updatePool(){ 
        ...
        let multiplier =
            self.getMultiplier(_from: self.lastRewardBlock, _to: currentBlock)
        let reward = UFix64(multiplier) * self.rewardPerBlock
    ...
    priv fun getMultiplier(_from: UInt64, _to: UInt64): UInt64{ 
    ...
    priv fun emergencyWithdraw(user: Address): @FungibleToken.Vault{ 
        let info = self.userInfo[user]!
