...
pub contract AvatarArtNFT: NonFungibleToken{ 
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &AvatarArtNFT.NFT?{ 
        let collection = getAccount(from).getCapability(AvatarArtNFT.CollectionPublicPath).borrow<&AvatarArtNFT.Collection{AvatarArtNFT.CollectionPublic}>() ?? panic("Couldn't get collection")
