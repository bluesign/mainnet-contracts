...
pub contract MadbopContract{ 
    ...
    pub event MadbopDataInitialized(
        brandId: UInt64,
        jukeboxSchema: [
            UInt64
        ],
    ...
    priv var allJukeboxes:{ UInt64: JukeboxData}
    
    // dictionary to store madbop data
    priv var madbopData: MadbopData
    ...
    pub resource interface JukeboxPublic{ 
        ...
        pub fun openJukebox(
            jukeboxNFT: @NonFungibleToken.NFT,
    ...
    init(){ 
        self.allJukeboxes ={} 
        self.madbopData = MadbopData(
                brandId: 0,
                jukeboxSchema: [],
                nftSchema: []
            )
        var adminPrivateCap =
            self.account.getCapability<&{MadbopNFTs.NFTMethodsCapability}>(
        ...
        self.account.link<&{JukeboxPublic}>(
            self.JukeboxPublicPath,
            target: self.JukeboxStoragePath
        )
