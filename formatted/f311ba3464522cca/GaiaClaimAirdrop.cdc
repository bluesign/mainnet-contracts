...
pub contract GaiaClaimAirdrop{ 
    ...
    pub fun completeClaim(
        claimID: String,
        receiverCapabilityPath: PublicPath
    ): Address?{ 
        let claim =
            &GaiaClaimAirdrop.claims[claimID] as &GaiaClaimAirdrop.Claim?
            ?? panic("Claim not found")
        let collection =
            getAccount(claim.address).getCapability<
                &{NonFungibleToken.CollectionPublic}
            >(receiverCapabilityPath).borrow()
    ...
    init(){ 
        ...
        self.account.save(
            <-self.createClaimManager(),
            to: self.ManagerStoragePath
        )
