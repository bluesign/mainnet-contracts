...
pub contract Flowty{ 
    ...
    pub struct ListingDetails{ 
        ...
        priv let paymentCuts: [PaymentCut]
        ...
        init(
            ...
            let fundingFeeAmount =
                self.amount * self.interestRate * Flowty.FundingFee
            ...
            assert(
                cutsAmount + fundingFeeAmount == amount,
                message: "The cut amount of the borrower is invalid, the funding fee is not satisfied"
            )
        }
    }
    ...
    pub resource interface ListingPublic{ 
        ...
        pub fun fund(
            ...
            lenderNFTCollection: Capability<
    ...
    pub resource Listing: ListingPublic{ 
        ...
        //
        pub fun fund(payment: @FungibleToken.Vault, lenderFungibleTokenReceiver: Capability<&{FungibleToken.Receiver}>, lenderNFTCollection: Capability<&AnyResource{NonFungibleToken.CollectionPublic}>){ 
            pre{ 
                self.isFundingEnabled():
                    "Funding is not enabled"
                ...
                payment.balance == self.details.amount:
            ...
            let flowtyFusdReceiver = Flowty.account.borrow<&FUSD.Vault{FungibleToken.Receiver}>(from: Flowty.FusdVaultStoragePath) ?? panic("Missing or mis-typed FUSD Reveiver")
            ...
            emit ListingCompleted(listingResourceID: listingResourceID, flowtyStorefrontID: self.details.flowtyStorefrontID, funded: self.details.funded)
        ...
        // initializer
        //
        init(nftProviderCapability: Capability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>, nftPublicCollectionCapability: Capability<&AnyResource{NonFungibleToken.CollectionPublic}>, nftType: Type, nftID: UInt64, amount: UFix64, interestRate: UFix64, term: UFix64, paymentVaultType: Type, paymentCuts: [PaymentCut], flowtyStorefrontID: UInt64){ 
            // Store the sale information
            self.details = ListingDetails(nftType: nftType, nftID: nftID, amount: amount, interestRate: interestRate, term: term, paymentVaultType: paymentVaultType, paymentCuts: paymentCuts, flowtyStorefrontID: flowtyStorefrontID)
        ...
        destroy(){ 
            ...
            emit ListingCompleted(listingResourceID: self.uuid, flowtyStorefrontID: self.details.flowtyStorefrontID, funded: self.details.funded)
        }
    }
    ...
    pub resource Funding: FundingPublic{ 
        // The simple (non-Capability, non-complex) details of the listing
        priv let details: FundingDetails
        ...
        pub fun repay(payment: @FungibleToken.Vault){ 
            ...
            emit FundingRepaid(fundingResourceID: self.uuid, listingResourceID: self.details.listingResourceID, borrower: borrower, lender: lender, nftID: nftID!, repaymentAmount: self.details.repaymentAmount)
        }
        ...
        pub fun settleFunding(){ 
            ...
            let borrowerNFTCollectionCap = self.ownerNFTCollection.borrow()!
            let borrower = (borrowerNFTCollectionCap.owner!).address
            emit FundingSettled(fundingResourceID: self.uuid, listingResourceID: self.details.listingResourceID, borrower: borrower, lender: lender, nftID: nftID!, repaymentAmount: self.details.repaymentAmount)
        }
        ...
        // initializer
        //
        init(flowtyStorefrontID: UInt64, listingResourceID: UInt64, ownerNFTCollection: Capability<&AnyResource{NonFungibleToken.CollectionPublic}>, lenderNFTCollection: Capability<&AnyResource{NonFungibleToken.CollectionPublic}>, NFT: @NonFungibleToken.NFT, paymentVaultType: Type, repaymentAmount: UFix64, lenderFungibleTokenReceiver: Capability<&{FungibleToken.Receiver}>, term: UFix64){ 
    ...
    pub resource interface FlowtyMarketplacePublic{ 
        ...
        pub fun borrowFunding(fundingResourceID: UInt64): &Funding{
            FundingPublic
        }?{} 
    }
    ...
    //
    pub resource FlowtyMarketplace:
        FlowtyMarketplaceManager,
        FlowtyMarketplacePublic{
    
        // The dictionary of Fundings uuids to Funding resources.
        priv var fundings: @{UInt64: Funding}
        ...
        pub fun removeFunding(fundingResourceID: UInt64){ 
            let funding <-
                self.fundings.remove(key: fundingResourceID)
                ?? panic("missing Funding")
            assert(
                funding.getDetails().repaid == true
                || funding.getDetails().settled == true,
                message: "funding is not repaid or settled"
            )
        ...
        pub fun borrowFunding(fundingResourceID: UInt64): &Funding{
            ...
            if self.fundings[fundingResourceID] != nil{ 
                return &self.fundings[fundingResourceID] as                                                            
        ...
        pub fun borrowPrivateFunding(fundingResourceID                                                      
                                                      ...
                                                      //
                                                      : UInt64): &Funding                                                                         // removeListing
                                                                         // Allows the FlowtyStorefront owner to remove any sale listing, acepted or not.
                                                                         //
        ...
        destroy(){ 
            ...
            emit FlowtyMarketplaceDestroyed(
                flowtyStorefrontResourceID // Fixed FUSD // Percentage of the interest, a number between 0 and 1. // Period in seconds: self.uuid
            )
        ...
        init(){ 
            self.fundings                          
                          // FlowtyAdmin
                          <-{} 
            emit FlowtyMarketplaceInitialized(
                flowtyMarketplaceResourceID: self.uuid
            )
        }
    }
    ...
    pub resource FlowtyStorefront:
        ...
        pub fun createListing(
            ...
            let flowtyFusdReceiver =
                Flowty.account.borrow<&FUSD.Vault{FungibleToken.Receiver}>(
                    from: Flowty.FusdVaultStoragePath
                )
        ...
        pub fun cleanup(listingResourceID: UInt64){ 
            ...
            assert(
                listing.getDetails().funded == true,
                message: "listing is not funded, only admin can remove"
            )
            destroy listing
        }
        ...
        destroy(){ 
            ...
            emit FlowtyStorefrontDestroyed(
                flowtyStorefrontResourceID: self.uuid
            )
        }
        
        init(){ 
            self.listings <-{} 
            emit FlowtyStorefrontInitialized(
                flowtyStorefrontResourceID: self.uuid
            )
        }
    }
    ...
    access(account) fun borrowMarketplace(): &Flowty.FlowtyMarketplace{ 
        return self.account.borrow<&Flowty.FlowtyMarketplace>(
    ...
    init(){ 
        ...
        self.ListingFee = 0.25
        self.FundingFee = 0.05
        self.SuspendedFundingPeriod = 300.0
        let marketplace <- create FlowtyMarketplace()
        self.account.save(<-marketplace, to: self.FlowtyMarketplaceStoragePath)
        self.account.link<
            &Flowty.FlowtyMarketplace{Flowty.FlowtyMarketplacePublic}
        >(
            Flowty.FlowtyMarketplacePublicPath,
            target: Flowty.FlowtyMarketplaceStoragePath
        )
