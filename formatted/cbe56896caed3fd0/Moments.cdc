...
pub contract Moments: NonFungibleToken{ 
    ...
    pub fun fetch(_ from: Address, itemId: UInt64): &Moments.NFT?{ 
        let collection = getAccount(from).getCapability(Moments.CollectionPublicPath).borrow<&Moments.Collection{Moments.MomentsCollectionPublic}>() ?? panic("Couldn't get collection")
