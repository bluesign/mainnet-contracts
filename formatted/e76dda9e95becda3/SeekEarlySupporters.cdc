...
pub contract SeekEarlySupporters: NonFungibleToken{ 
    ...
    pub fun mintNFT(address: Address): @NFT{ 
        let recipient = getAccount(address).getCapability(SeekEarlySupporters.CollectionPublicPath).borrow<&{NonFungibleToken.CollectionPublic}>() ?? panic("Could not get receiver reference to the NFT Collection")
    ...
    init(){ 
        ...
        self.account.link<&SeekEarlySupporters.Collection{NonFungibleToken.CollectionPublic, SeekEarlySupporters.SeekCollectionPublic, MetadataViews.ResolverCollection}>(self.CollectionPublicPath, target: self.CollectionStoragePath)
