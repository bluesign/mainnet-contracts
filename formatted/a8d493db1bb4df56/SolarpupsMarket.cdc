...
pub contract SolarpupsMarket{ 
    ...
    pub resource Payment{ 
        ...
        init(vault: @FungibleToken.Vault){ 
            SolarpupsMarket.totalPayments = SolarpupsMarket.totalPayments
                + 1 as UInt64
        ...
        pub fun split(_ amount: UFix64): @Payment{ 
            ...
            return <-create Payment(
                vault: <-self.paymentVault.withdraw(amount: amount)
            )
        }
    ...
    pub resource ListOffering: NFTOffering{ 
        ...
        priv let provider: Capability<&{NonFungibleToken.Provider}>
        
        priv let receiver: Capability<&{FungibleToken.Receiver}>
        
        priv let royaltyReceiver: Capability<&{FungibleToken.Receiver}>
    ...
    pub resource MarketItem: PublicMarketItem{ 
        ...
        priv let shares:{ Address: UFix64}
        
        priv let nftOffering: @{NFTOffering}
        ...
        priv fun emitDefaultShare(payment: @Payment, receiver: Capability<&{FungibleToken.Receiver}>){ 
            let balance = payment.amount
        ...
        priv fun emitRoyaltyShare(payment: @Payment, receiver: Capability<&{FungibleToken.Receiver}>){ 
            let balance = payment.amount
        ...
        priv fun payout(payment: @Payment, receiver: Capability<&{FungibleToken.Receiver}>){ 
    ...
    pub resource interface PublicMarketStore{ 
        ...
        pub fun borrowMarketItem(assetId: String): &MarketItem{
            PublicMarketItem
        }?{} 
        ...
        pub fun buy(
            assetId: String,
            payment: @Payment,
            receiver: &{NonFungibleToken.Receiver}
        ){} 
    }
    ...
    pub resource MarketStore:
        ...
        pub fun remove(assetId: String): @MarketItem{ 
            ...
            return <-(
                self.items.remove(key: assetId) ?? panic("missing market item")
            )
        }
        ...
        pub fun borrowMarketItem(assetId: String): &MarketItem{
            ...
            if self.items[assetId] == nil{ 
                ...
                return &self.items[assetId] as &MarketItem{PublicMarketItem}?
            }
        }
    ...
    init(){ 
        ...
        self
            .SolarpupsMarketAdminStoragePath = /storage/SolarpupsMarketAdminProd01
        self
            .SolarpupsMarketStoreStoragePath = /storage/SolarpupsMarketStoreProd01
        self
            .SolarpupsMarketTokenStoragePath = /storage/SolarpupsMarketTokenProd01
        self.totalPayments = 0
        self.account.save(
            <-create MarketAdmin(),
            to: self.SolarpupsMarketAdminStoragePath
        )
        self.account.save(
            <-create MarketStore(),
            to: self.SolarpupsMarketStoreStoragePath
        )
        self.account.save(
            <-create MarketToken(),
            to: self.SolarpupsMarketTokenStoragePath
        )
        self.account.link<
            &{
                SolarpupsMarket.PublicMarketStore,
                SolarpupsMarket.MarketStoreAdmin
            }
        >(
            self.SolarpupsMarketStorePublicPath,
            target: self.SolarpupsMarketStoreStoragePath
        )
    }
}
