...
pub contract AnalogMinter{ 
    ...
    pub struct MintableSet{ 
        ...
        access(account) fun addWhiteListAddress(
            address: Address,
            amount: UInt64
        ){ 
            pre{ 
        ...
        access(account) fun updateWhiteListAddressAmount(
            address: Address,
            amount: UInt64
        ){ 
            pre{ 
    ...
    pub fun mintPrivateNFTWithFUT(
        ...
        let admin =
            self.account.borrow<&Analogs.Admin>(from: Analogs.AdminStoragePath)
        ...
        let merchantFUTReceiverRef =
            getAccount(set.analogRoyaltyAddress).getCapability<
        ...
        let NFTReceiver =
            recipient.getCapability(Analogs.CollectionPublicPath).borrow<
        ...
        if (AnalogMinter.mintableSets[setID]!).whitelistedAccounts[buyer]!
        - numberOfTokens
        == 0{ 
            (AnalogMinter.mintableSets[setID]!).removeWhiteListAddress(
                address: buyer
            )
        } else{ 
    ...
    pub fun mintFreeNFT(buyer: Address, numberOfTokens: UInt64){ 
        ...
        let setID: UInt64 = 1
        var mintedCount: UInt64? =
            (AnalogMinter.mintableSets[setID]!).whitelistedAccounts[buyer]
        if mintedCount == nil{ 
            mintedCount = 0
        ...
        assert(
            mintedCount! + numberOfTokens <= 5,
            message: "You exceed the minting limit"
        )
        let admin =
            self.account.borrow<&Analogs.Admin>(from: Analogs.AdminStoragePath)
            ?? panic("Could not borrow a reference to the Analogs Admin")
    ...
    pub resource Admin{ 
        pub fun createMintableSet(
            setID: UInt64,
            privateSalePrice: UFix64,
            publicSalePrice: UFix64
        ){ 
            pre{ 
        ...
        pub fun updateWhiteListAddressAmount(
            setID: UInt64,
            address: Address,
            amount: UInt64
        ){ 
        ...
        pub fun updatePrivateSalePrice(setID: UInt64, price: UFix64){ 
            (AnalogMinter.mintableSets[setID]!).updatePrivateSalePrice(
                price: price
            )
        }
        
        pub fun updatePublicSalePrice(setID: UInt64, price: UFix64){ 
            (AnalogMinter.mintableSets[setID]!).updatePublicSalePrice(
                price: price
            )
        }
    }
