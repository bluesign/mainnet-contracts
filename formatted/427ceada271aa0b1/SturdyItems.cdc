...
pub contract SturdyItems: NonFungibleToken{ 
    ...
    pub event Minted(id: UInt64, typeID: UInt64, tokenURI: String, tokenTitle: String, tokenDescription: String, artist: String, secondaryRoyalty: String, platformMintedOn: String)
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            ...
            switch view{ 
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: SturdyItems.CollectionStoragePath, publicPath: SturdyItems.CollectionPublicPath, providerPath: /private/SturdyItemsCollection, publicCollection: Type<&SturdyItems.Collection{SturdyItems.SturdyItemsCollectionPublic}>(), publicLinkedType: Type<&SturdyItems.Collection{SturdyItems.SturdyItemsCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&SturdyItems.Collection{SturdyItems.SturdyItemsCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Provider, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
                ...
                case Type<MetadataViews.Royalties>():
                    return MetadataViews.Royalties([MetadataViews.Royalty(receiver: getAccount(HoodlumsMetadata.sturdyRoyaltyAddress).getCapability<&{FungibleToken.Receiver}>(/public/dapperUtilityCoinReceiver), cut: HoodlumsMetadata.sturdyRoyaltyCut, description: "Sturdy Royalty"), MetadataViews.Royalty(receiver: getAccount(HoodlumsMetadata.artistRoyaltyAddress).getCapability<&{FungibleToken.Receiver}>(/public/dapperUtilityCoinReceiver), cut: HoodlumsMetadata.artistRoyaltyCut, description: "Artist Royalty")])
            }
            return nil
        }
    ...
    pub resource NFTMinter{ 
        ...
        // price: price
        // initPrice: price
        pub fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, typeID: UInt64, tokenURI: String, tokenTitle: String, tokenDescription: String, artist: String, secondaryRoyalty: String, platformMintedOn: String){ 
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &SturdyItems.NFT?{ 
        let collection = (getAccount(from).getCapability(SturdyItems.CollectionPublicPath)!).borrow<&SturdyItems.Collection{SturdyItems.SturdyItemsCollectionPublic}>() ?? panic("Couldn't get collection")
