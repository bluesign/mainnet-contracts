...
pub contract SturdyMinter{ 
    ...
    pub fun mintPrivateNFTWithDUC(
        ...
        let admin =
            self.account.borrow<&SturdyTokens.Admin>(
                from: SturdyTokens.AdminStoragePath
            )
        ...
        let NFTReceiver =
            recipient.getCapability(SturdyTokens.CollectionPublicPath).borrow<
        ...
        if (SturdyMinter.mintableSets[setID]!).whitelistedAccounts[buyer]!
        - numberOfTokens
        == 0{ 
            (SturdyMinter.mintableSets[setID]!).removeWhiteListAddress(
                address: buyer
            )
        } else{ 
    ...
    pub resource Admin{ 
        ...
        pub fun updatePrivateSalePrice(setID: UInt64, price: UFix64){ 
            (SturdyMinter.mintableSets[setID]!).updatePrivateSalePrice(
                price: price
            )
        }
        
        pub fun updatePublicSalePrice(setID: UInt64, price: UFix64){ 
            (SturdyMinter.mintableSets[setID]!).updatePublicSalePrice(
                price: price
            )
        }
    }
