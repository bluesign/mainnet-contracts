/* SPDX-License-Identifier: UNLICENSED */import DimeCollectibleV3 from "../0xf5cdaace879e5a79/DimeCollectibleV3.cdc"
...
pub contract DimeRoyalties{ 
    ...
    priv var nextReleaseId: UInt64
    
    pub struct SaleShares{ 
        priv let allotments:{ Address: UFix64}
        ...
        init(allotments:{ Address: UFix64}){ 
            ...
            assert(
                total == 1.0,
                message: "Total sale shares must equal exactly 1"
            )
    ...
    pub resource Release: ReleasePublic{ 
        ...
        // the royalty NFTs
        priv let royaltyNFTs:{ UInt64: Address}
        ...
        pub fun updateOwner(id: UInt64, newOwner: Address){ 
            let collection = getAccount(newOwner).getCapability<&DimeCollectibleV3.Collection{DimeCollectibleV3.DimeCollectionPublic}>(DimeCollectibleV3.CollectionPublicPath).borrow() ?? panic("Couldn't borrow a capability to the new owner's collection")
        ...
        priv var artistShares: SaleShares
        
        priv var managerShares: SaleShares
        ...
        pub init(id: UInt64, royaltiesPerShare: UFix64, numRoyaltyNFTs: UInt64, managerFees: UFix64, artistShares: SaleShares, managerShares: SaleShares, secondarySaleRoyalties: DimeCollectibleV3.Royalties){ 
            self.id = id
    ...
    pub resource ReleaseCollection: ReleaseCollectionPublic{ 
        ...
        pub fun createRelease(collection: &DimeCollectibleV3.Collection{NonFungibleToken.CollectionPublic}, totalRoyalties: UFix64, numRoyaltyNFTs: UInt64, tradeable: Bool, managerFees: UFix64, artistShares: SaleShares, managerShares: SaleShares, secondarySaleRoyalties: DimeCollectibleV3.Royalties){ 
            let minterAddress: Address = 0xf5cdaace879e5a79
            let minterRef = getAccount(minterAddress).getCapability<&DimeCollectibleV3.NFTMinter>(DimeCollectibleV3.MinterPublicPath).borrow()!
            let release <- create Release(id: DimeRoyalties.nextReleaseId, royaltiesPerShare: totalRoyalties / UFix64(numRoyaltyNFTs), numRoyaltyNFTs: numRoyaltyNFTs, managerFees: managerFees, artistShares: artistShares, managerShares: managerShares, secondarySaleRoyalties: secondarySaleRoyalties)
        ...
        pub fun deleteRelease(releaseId: UInt64, collection: Capability<&DimeCollectibleV3.Collection>){ 
