...
pub contract FlowtyOffersResolver{ 
    ...
    pub resource OfferResolver: Resolver.ResolverPublic{ 
        pub fun checkOfferResolver(item: &AnyResource{NonFungibleToken.INFT, MetadataViews.Resolver}, offerParamsString:{ String: String}, offerParamsUInt64:{ String: UInt64}, offerParamsUFix64:{ String: UFix64}): Bool{ 
    ...
    pub fun getResolverCap(): Capability<
        &FlowtyOffersResolver.OfferResolver{Resolver.ResolverPublic}
    >{ 
    ...
    init(){ 
        ...
        self.account.link<
            &FlowtyOffersResolver.OfferResolver{Resolver.ResolverPublic}
        >(self.PublicPath, target: self.StoragePath)
    }
}
