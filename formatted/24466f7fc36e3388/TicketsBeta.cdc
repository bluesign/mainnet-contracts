...
pub contract TicketsBeta{ 
    ...
    pub event TicketRequested(
        dispenser_id: UInt32,
        user_id: UInt32,
        address: Address
    )
    
    pub event TicketUsed(
        dispenser_id: UInt32,
        user_id: UInt32,
        token_id: UInt64,
        address: Address,
    ...
    pub let DispenserFlowTokenVault:{ 
        UInt32: Capability<&FlowToken.Vault{FungibleToken.Receiver}>
    }
    
    pub let UserFlowTokenVault:{ 
        UInt32: Capability<&FlowToken.Vault{FungibleToken.Receiver}>
    }
    ...
    pub resource AdminPublic{ 
        ...
        // [public access]
        pub fun getTicketRequesters(dispenser_id: UInt32):{ 
            UInt32: RequestStruct
        }?{ 
    ...
    pub fun getTicketRequestStatus(
        dispenser_id: UInt32,
        user_id: UInt32
    ): RequestStruct?{ 
    ...
    init(){ 
        ...
        self
            .CapabilityReceiverVaultPublicPath = /public/TicketsBetaCapabilityReceiverVault
        ...
        self.account.save<@TicketsBeta.Admin>(
            <-create Admin(),
            to: self.AdminPrivateStoragePath
        )
        self.account.link<&TicketsBeta.Admin>(
            self.AdminPrivatePath,
            target: self.AdminPrivateStoragePath
        )
        self.account.save<@TicketsBeta.AdminPublic>(
            <-create AdminPublic(),
            to: self.AdminPublicStoragePath
        )
        self.account.link<&TicketsBeta.AdminPublic>(
            self.AdminPublicPath,
            target: self.AdminPublicStoragePath
        )
    }
}
