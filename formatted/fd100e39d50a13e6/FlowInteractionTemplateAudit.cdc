...
pub contract FlowInteractionTemplateAudit{ 
    ...
    // AuditManager.
    // 
    pub event AuditAdded(
        templateId: String,
        auditor: Address,
        auditManagerID: UInt64
    )
    
    // The event that is emitted when an audit is revoked from an 
    // AuditManager.
    // 
    pub event AuditRevoked(
        templateId: String,
        auditor: Address,
        auditManagerID: UInt64
    )
    ...
    /****** Storage Paths ******/    pub let AuditManagerStoragePath: StoragePath
    ...
    //
    pub fun getHasTemplateBeenAuditedByAuditors(
        templateId: String,
        auditors: [
            Address
        ]
        ...
        for auditor in auditors{ 
            let auditManagerRef = getAccount(auditor).getCapability(FlowInteractionTemplateAudit.AuditManagerPublicPath).borrow<&FlowInteractionTemplateAudit.AuditManager{FlowInteractionTemplateAudit.AuditManagerPublic}>() ?? panic("Could not borrow Audit Manager public reference")
    ...
    init(){ 
        self
            .AuditManagerStoragePath = /storage/FlowInteractionTemplateAuditManagerStoragePath
        self
            .AuditManagerPublicPath = /public/FlowInteractionTemplateAuditManagerPublicPath
        self
            .AuditManagerPrivatePath = /private/FlowInteractionTemplateAuditManagerPrivatePath
    }
}
