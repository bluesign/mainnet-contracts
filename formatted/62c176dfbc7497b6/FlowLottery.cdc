...
pub contract FlowLottery{ 
    ...
    pub fun buyTickets(address: Address, payment: @FlowToken.Vault){ 
        ...
        let currentLottery: &LotteryDetails =
            self.borrowCurrentLotteryDetails()
            ?? panic("There is no lottery happening right now.")
    ...
    pub resource LotteryDetails{ 
        ...
        access(contract) fun addWinner(
            address: Address,
            amount: UFix64,
            removeAtIndex: UInt64
        ){ 
            self.winners.append(
                Winner(
                    address: address,
    ...
    pub resource Admin{ 
        ...
        pub fun resolveWinner(): Winner{ 
            let currentLotteryDetails: &LotteryDetails =
                self.borrowCurrentLotteryDetails()!
            assert(
            ...
            let percentAmounts:{ Int: UFix64} ={
                0: 0.01, 1: 0.02, 2: 0.07, 3: 0.20, 4: 1.00}
            
            // get amount
            let amount: UFix64 =
                FlowLottery.pot.balance * percentAmounts[numDone]!
            let hitJackpot: Bool = numDone == 4
            
            // get the winner
            let randomWinnerIndex: UInt64 =
                FlowLottery.getRandom(
                    min: 0,
                    max: UInt64(currentLotteryDetails.entries.length) - 1
                )
            let winner: Address =
                currentLotteryDetails.entries[randomWinnerIndex]
            
            // remove rake
            // 2.5% total:
            // 0.5% is for the platform (development + marketing)
            // 2% rollover gets immediately deposited back into the pot for future drawings
            let winnings: @FungibleToken.Vault <-
                FlowLottery.pot.withdraw(amount: amount)
            let rollover: @FungibleToken.Vault <-
                winnings.withdraw(amount: amount * 0.02)
            let platformTax: @FungibleToken.Vault <-
                winnings.withdraw(amount: amount * 0.005)
            let platformVault: &FlowToken.Vault{FungibleToken.Receiver} =
                FlowLottery.account.getCapability(/public/flowTokenReceiver)
                    .borrow<&FlowToken.Vault{FungibleToken.Receiver}>()!
            platformVault.deposit(from: <-platformTax)
            
            // deposit winnings
            let winnerFlowVault: &FlowToken.Vault{FungibleToken.Receiver} =
                getAccount(winner).getCapability(/public/flowTokenReceiver)
                    .borrow<&FlowToken.Vault{FungibleToken.Receiver}>()
        ...
        pub fun borrowCurrentLotteryDetails(): &LotteryDetails?{ 
            return &self.lotteryHistory[FlowLottery.totalLotteries - 1]
            as
            &LotteryDetails?
        }
    ...
    access(contract) fun borrowAdmin(): &Admin{ 
        return self.account.borrow<&FlowLottery.Admin>(
