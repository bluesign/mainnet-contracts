...
pub contract BeerRun: NonFungibleToken, ViewResolver{ 
    ...
    pub resource Manager: NiftoryNonFungibleToken.ManagerPublic, NiftoryNonFungibleToken.ManagerPrivate{ 
        ...
        pub fun getNFTCollectionData(): MetadataViews.NFTCollectionData{ 
            return NiftoryNFTRegistry.buildNFTCollectionData(BeerRun.REGISTRY_ADDRESS, BeerRun.REGISTRY_BRAND, fun (): @NonFungibleToken.Collection{ 
                    return <-BeerRun.createEmptyCollection()
                })
        }
        ...
        pub fun modifyContractMetadata(): auth &AnyStruct{ 
            ...
            let maybeMetadata = BeerRun.metadata
            if maybeMetadata == nil{ 
                let blankMetadata:{ String: String} ={} 
                BeerRun.metadata = blankMetadata
            }
            return (&BeerRun.metadata as auth &AnyStruct?)!
        }
        ...
        priv fun _getMetadataViewsManagerPrivate(): &MetadataViewsManager.Manager{MetadataViewsManager.Private}{ 
            ...
            let manager = BeerRun.account.getCapability<&MetadataViewsManager.Manager{MetadataViewsManager.Private}>(record.metadataViewsManager.paths.private).borrow()!
            return manager
        }
        ...
        priv fun _getSetManagerPrivate(): &MutableMetadataSetManager.Manager{MutableMetadataSetManager.Public, MutableMetadataSetManager.Private}{ 
            let record = NiftoryNFTRegistry.getRegistryRecord(BeerRun.REGISTRY_ADDRESS, BeerRun.REGISTRY_BRAND)
            let setManager = BeerRun.account.getCapability<&MutableMetadataSetManager.Manager{MutableMetadataSetManager.Public, MutableMetadataSetManager.Private}>(record.setManager.paths.private).borrow()!
            return setManager
        }
    ...
    init(nftManagerProxy: &{NiftoryNonFungibleTokenProxy.Public, NiftoryNonFungibleTokenProxy.Private}){ 
        ...
        self.account.save<@MutableMetadataSetManager.Manager>(<-MutableMetadataSetManager.create(name: "BeerRun", description: "The set manager for BeerRun."), to: record.setManager.paths.storage)
