...
pub contract TuneGOMarket{ 
    ...
    pub resource SaleOffer: SaleOfferPublic{ 
        ...
        priv let paymentReceiver: Capability<&FlowToken.Vault{FungibleToken.Receiver}>
        
        priv let royalties: [Royalty]
        
        priv let saleItemProviderCapability: Capability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
        
        priv let tunegoPaymentFeeReceiver: Capability<&{FungibleToken.Receiver}>
        
        priv let tunegoPaymentFeePercentage: UFix64
        
        pub fun purchase(payment: @FungibleToken.Vault, buyerCollection: Capability<&{NonFungibleToken.Receiver}>): @NonFungibleToken.NFT{ 
            pre{ 
        ...
        init(saleItemProviderCapability: Capability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>, collectibleType: Type, collectibleId: UInt64, royalties: [Royalty], price: UFix64, paymentReceiver: Capability<&FlowToken.Vault{FungibleToken.Receiver}>){ 
    ...
    pub resource interface CollectionPublic{ 
        ...
        pub fun borrowSaleOffer(saleOfferId: UInt64): &SaleOffer{
            SaleOfferPublic
        }?{} 
    }
    ...
    pub resource Collection: CollectionManager, CollectionPublic{ 
        ...
        pub fun createSaleOffer(saleItemProviderCapability: Capability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>, collectibleType: Type, collectibleId: UInt64, royalties: [Royalty], price: UFix64, paymentReceiver: Capability<&FlowToken.Vault{FungibleToken.Receiver}>): UInt64{ 
            assert(false, message: "This method has been disabled - the contract is no longer supported")
    ...
    pub resource Admin{ 
        pub fun setTuneGOFee(tunegoFee: MarketFee){ 
            ...
            emit TuneGOMarketFeeSet(
                receiver: tunegoFee.receiver,
                percentage: tunegoFee.percentage
            )
        }
    ...
    init(){ 
        ...
        self.account.save<@Admin>(
            <-create Admin(),
            to: TuneGOMarket.AdminStoragePath
        )
        self.SupportedNFTTypes = [
                Type<@TuneGO.NFT>(),
                Type<@TicalUniverse.NFT>()
            ]
        let initialMarketFee =
            MarketFee(receiver: self.account.address, percentage: UFix64(2.5))
