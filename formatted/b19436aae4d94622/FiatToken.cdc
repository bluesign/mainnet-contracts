...
import OnChainMultiSig from "../0x220c1b4155f86f2f/OnChainMultiSig.cdc"
...
pub contract FiatToken: FungibleToken{ 
    ...
    access(contract) fun linkAdminExec(_ newPrivPath: PrivatePath): Capability<&AdminExecutor>{ 
        return self.account.link<&AdminExecutor>(newPrivPath, target: FiatToken.AdminExecutorStoragePath) ?? panic("could not create new AdminExecutor capability link")
    }
    
    access(contract) fun linkOwnerExec(_ newPrivPath: PrivatePath): Capability<&OwnerExecutor>{ 
        return self.account.link<&OwnerExecutor>(newPrivPath, target: FiatToken.OwnerExecutorStoragePath) ?? panic("could not create new OwnerExecutor capability link")
    }
    
    access(contract) fun linkMasterMinterExec(_ newPrivPath: PrivatePath): Capability<&MasterMinterExecutor>{ 
        return self.account.link<&MasterMinterExecutor>(newPrivPath, target: FiatToken.MasterMinterExecutorStoragePath) ?? panic("could not create new MasterMinterExecutor capability link")
    }
    
    access(contract) fun linkBlocklistExec(_ newPrivPath: PrivatePath): Capability<&FiatToken.BlocklistExecutor>{ 
        return self.account.link<&BlocklistExecutor>(newPrivPath, target: FiatToken.BlocklistExecutorStoragePath) ?? panic("could not create new BlocklistExecutor capability link")
    }
    
    access(contract) fun linkPauserExec(_ newPrivPath: PrivatePath): Capability<&FiatToken.PauseExecutor>{ 
        return self.account.link<&FiatToken.PauseExecutor>(newPrivPath, target: FiatToken.PauseExecutorStoragePath) ?? panic("could not create new PauseExecutor capability link")
    }
    ...
    pub resource Vault: ResourceId, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance{ 
        ...
        access(contract) fun burn(){ 
            pre{ 
                self.balance > 0.0:
                    "Cannot burn USDC Vault with zero balance"
            }
        ...
        destroy(){ 
            pre{ 
                self.balance == 0.0:
                    "Cannot destroy USDC Vault with non-zero balance"
            }
    ...
    pub resource AdminExecutor{ 
        priv var currentCapPath: PrivatePath?
        ...
        pub fun changeAdmin(to: Address, newPath: PrivatePath){ 
            ...
            let receiver = getAccount(to).getCapability<&Admin{AdminCapReceiver}>(FiatToken.AdminCapReceiverPubPath).borrow() ?? panic("could not borrow AdminCapReceiver capability")
            let idRef = getAccount(to).getCapability<&Admin{ResourceId}>(FiatToken.AdminUUIDPubPath).borrow() ?? panic("could not borrow Admin ResourceId capability")
    ...
    pub resource Admin: OnChainMultiSig.PublicSigner, ResourceId, AdminCapReceiver{ 
        priv let multiSigManager: @OnChainMultiSig.Manager
        
        priv var adminExecutorCapability: Capability<&AdminExecutor>?
    ...
    pub resource OwnerExecutor{ 
        priv var ownerCapPath: PrivatePath?
        
        priv var masterMinterCapPath: PrivatePath?
        
        priv var blocklisterCapPath: PrivatePath?
        
        priv var pauserCapPath: PrivatePath?
        
        pub fun reassignOwner(to: Address, newPath: PrivatePath){ 
            let newCap = FiatToken.linkOwnerExec(newPath)
            let receiver = getAccount(to).getCapability<&Owner{OwnerCapReceiver}>(FiatToken.OwnerCapReceiverPubPath).borrow() ?? panic("could not borrow the OwnerCapReceiver capability")
            let idRef = getAccount(to).getCapability<&Owner{ResourceId}>(FiatToken.OwnerUUIDPubPath).borrow() ?? panic("could not borrow the Owner ResourceId capability")
        ...
        pub fun reassignMasterMinter(to: Address, newPath: PrivatePath){ 
            ...
            let receiver = getAccount(to).getCapability<&MasterMinter{MasterMinterCapReceiver}>(FiatToken.MasterMinterCapReceiverPubPath).borrow() ?? panic("could not borrow the MasterMinterCapReceiver capability")
            let idRef = getAccount(to).getCapability<&MasterMinter{ResourceId}>(FiatToken.MasterMinterUUIDPubPath).borrow() ?? panic("could not borrow the MasterMinter ResourceId capability")
        ...
        pub fun reassignBlocklister(to: Address, newPath: PrivatePath){ 
            ...
            let receiver = getAccount(to).getCapability<&Blocklister{BlocklisterCapReceiver}>(FiatToken.BlocklisterCapReceiverPubPath).borrow() ?? panic("could not borrow the BlocklisterCapReceiver capability ")
            let idRef = getAccount(to).getCapability<&Blocklister{ResourceId}>(FiatToken.BlocklisterUUIDPubPath).borrow() ?? panic("could not borrow the Blocklister ResourceId capability")
        ...
        pub fun reassignPauser(to: Address, newPath: PrivatePath){ 
            ...
            let receiver = getAccount(to).getCapability<&Pauser{PauseCapReceiver}>(FiatToken.PauserCapReceiverPubPath).borrow() ?? panic("could not borrow the PauseCapReceiver capability")
            let idRef = getAccount(to).getCapability<&Pauser{ResourceId}>(FiatToken.PauserUUIDPubPath).borrow() ?? panic("could not borrow the Pauser ResourceId capability")
    ...
    pub resource Owner: OnChainMultiSig.PublicSigner, ResourceId, OwnerCapReceiver{ 
        ...
        priv var ownerExecutorCapability: Capability<&OwnerExecutor>?
    ...
    pub resource MasterMinter: ResourceId, OnChainMultiSig.PublicSigner, MasterMinterCapReceiver{ 
        ...
        priv var masterMinterExecutorCapability: Capability<&MasterMinterExecutor>?
    ...
    pub resource Blocklister: ResourceId, BlocklisterCapReceiver, OnChainMultiSig.PublicSigner{ 
        priv var blocklistCap: Capability<&BlocklistExecutor>?
    ...
    pub resource Pauser: ResourceId, PauseCapReceiver, OnChainMultiSig.PublicSigner{ 
        priv var pauseCap: Capability<&PauseExecutor>?
    ...
    init(contractAccount: AuthAccount, VaultStoragePath: StoragePath, VaultBalancePubPath: PublicPath, VaultUUIDPubPath: PublicPath, VaultReceiverPubPath: PublicPath, BlocklistExecutorStoragePath: StoragePath, BlocklisterStoragePath: StoragePath, BlocklisterCapReceiverPubPath: PublicPath, BlocklisterUUIDPubPath: PublicPath, BlocklisterPubSigner: PublicPath, PauseExecutorStoragePath: StoragePath, PauserStoragePath: StoragePath, PauserCapReceiverPubPath: PublicPath, PauserUUIDPubPath: PublicPath, PauserPubSigner: PublicPath, AdminExecutorStoragePath: StoragePath, AdminStoragePath: StoragePath, AdminCapReceiverPubPath: PublicPath, AdminUUIDPubPath: PublicPath, AdminPubSigner: PublicPath, OwnerExecutorStoragePath: StoragePath, OwnerStoragePath: StoragePath, OwnerCapReceiverPubPath: PublicPath, OwnerUUIDPubPath: PublicPath, OwnerPubSigner: PublicPath, MasterMinterExecutorStoragePath: StoragePath, MasterMinterStoragePath: StoragePath, MasterMinterCapReceiverPubPath: PublicPath, MasterMinterPubSigner: PublicPath, MasterMinterUUIDPubPath: PublicPath, MinterControllerStoragePath: StoragePath, MinterControllerUUIDPubPath: PublicPath, MinterControllerPubSigner: PublicPath, MinterStoragePath: StoragePath, MinterUUIDPubPath: PublicPath, initialAdminCapabilityPrivPath: PrivatePath, initialOwnerCapabilityPrivPath: PrivatePath, initialMasterMinterCapabilityPrivPath: PrivatePath, initialPauserCapabilityPrivPath: PrivatePath, initialBlocklisterCapabilityPrivPath: PrivatePath, tokenName: String, version: String, initTotalSupply: UFix64, initPaused: Bool, adminPubKeys: [String], adminPubKeysWeights: [UFix64], adminPubKeysAlgos: [UInt8], ownerPubKeys: [String], ownerPubKeysWeights: [UFix64], ownerPubKeysAlgos: [UInt8], masterMinterPubKeys: [String], masterMinterPubKeysWeights: [UFix64], masterMinterPubKeysAlgos: [UInt8], blocklisterPubKeys: [String], blocklisterPubKeysWeights: [UFix64], blocklisterPubKeysAlgos: [UInt8], pauserPubKeys: [String], pauserPubKeysWeights: [UFix64], pauserPubKeysAlgos: [UInt8]){ 
        ...
        let adminExecutorRef = contractAccount.borrow<&FiatToken.AdminExecutor>(from: self.AdminExecutorStoragePath) ?? panic("cannot borrow AdminExecutor from storage")
        let ownerExecutorRef = contractAccount.borrow<&FiatToken.OwnerExecutor>(from: self.OwnerExecutorStoragePath) ?? panic("cannot borrow OwnerExecutor from storage")
