...
import
    CollecticoStandardNFT
    from "../0x11cbef9729b236f3/CollecticoStandardNFT.cdc"

import
    CollecticoStandardViews
    from "../0x11cbef9729b236f3/CollecticoStandardViews.cdc"
...
pub contract StartupyWGrze:
    ...
    priv var items: @{UInt64: Item}
    
    priv var nextItemId: UInt64
    
    priv var metadata:{ String: AnyStruct}
    
    priv var claims:{ String: Bool}
    
    priv var defaultRoyalties: [MetadataViews.Royalty]
    
    // for the future use
    priv var nftViewResolvers: @{
        String: AnyResource{CollecticoStandardViews.NFTViewResolver}
    }
    
    priv var itemViewResolvers: @{
        String: AnyResource{CollecticoStandardViews.ItemViewResolver}
    }
        ...
        switch view{ 
            case Type<MetadataViews.Display>():
                return MetadataViews.Display(
                    ...
                    thumbnail: (
                        self.metadata["squareImage"]! as! MetadataViews.Media
                    ).file
                )
            ...
            case Type<MetadataViews.Royalties>():
                return MetadataViews.Royalties(
                    self.defaultRoyalties.concat(
                        CollecticoRoyalties.getIssuerRoyalties()
                    )
                )
            case Type<MetadataViews.License>():
                let licenseId: String? = self.metadata["_licenseId"] as! String?
                return licenseId != nil
                    ? MetadataViews.License(licenseId!)
                    : nil
    ...
    pub resource Item: CollecticoStandardNFT.IItem, MetadataViews.Resolver{ 
        ...
        init(id: UInt64, name: String, description: String, thumbnail: AnyStruct{MetadataViews.File}, metadata:{ String: AnyStruct}?, maxSupply: UInt64?, isTransferable: Bool, royalties: [MetadataViews.Royalty]?){ 
            self.id = id
            self.name = name
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            ...
            switch view{ 
                case Type<CollecticoStandardViews.NFTView>():
                    return CollecticoStandardViews.NFTView(id: self.id, itemId: self.itemId, itemName: item.name.concat(" #").concat(self.serialNumber.toString()), itemDescription: item.description, itemThumbnail: item.thumbnail, itemMetadata: item.metadata, serialNumber: self.serialNumber, metadata: self.metadata, itemTotalSupply: item.getTotalSupply(), itemMaxSupply: item.maxSupply, isTransferable: self.isTransferable, contractInfo: StartupyWGrze.getContractInfo(), collectionDisplay: StartupyWGrze.getCollectionDisplay(), royalties: MetadataViews.getRoyalties(&self as &StartupyWGrze.NFT{MetadataViews.Resolver}), display: MetadataViews.getDisplay(&self as &StartupyWGrze.NFT{MetadataViews.Resolver}), traits: MetadataViews.getTraits(&self as &StartupyWGrze.NFT{MetadataViews.Resolver}), editions: MetadataViews.getEditions(&self as &StartupyWGrze.NFT{MetadataViews.Resolver}), medias: MetadataViews.getMedias(&self as &StartupyWGrze.NFT{MetadataViews.Resolver}), license: MetadataViews.getLicense(&self as &StartupyWGrze.NFT{MetadataViews.Resolver}))
                case Type<CollecticoStandardViews.ContractInfo>():
                    return StartupyWGrze.getContractInfo()
                case Type<MetadataViews.Display>():
    ...
    pub resource Collection:
        ...
        pub fun withdraw(withdrawID: UInt64): @NonFungibleToken.NFT{ 
            ...
            let nft =
                self.borrowCollecticoNFT(id: withdrawID)
                ?? panic("Requested NFT does not exist in the collection")
    ...
    pub fun getItem(itemId: UInt64, view: Type): AnyStruct?{ 
        ...
        let item: &Item{MetadataViews.Resolver} =
            self.getItemRef(itemId: itemId)
    ...
    pub fun getCollectionData(): MetadataViews.NFTCollectionData{ 
        return MetadataViews.NFTCollectionData(
            ...
            publicCollection: Type<
                &StartupyWGrze.Collection{StartupyWGrze.CollectionPublic}
            >(),
            publicLinkedType: Type<
                &StartupyWGrze.Collection{
                    StartupyWGrze.CollectionPublic,
                    NonFungibleToken.CollectionPublic,
                    NonFungibleToken.Receiver,
                    MetadataViews.ResolverCollection
                }
    ...
    pub fun getDictValue(
        ...
        || !((dict!)[key]!).isInstance(type){ 
            return nil
        }
        return (dict!)[key]!
    }
    
    pub fun dictToTraits(
        dict:{ 
            String: AnyStruct
        }?,
        excludedNames: [
        ...
        let traits =
            self.dictToTraitArray(dict: dict, excludedNames: excludedNames)
        return traits.length != 0 ? MetadataViews.Traits(traits) : nil
    }
    
    pub fun dictToTraitArray(
        dict:{ 
            String: AnyStruct
        }?,
        excludedNames: [
    ...
    pub fun dictToMedias(
        ...
        let medias =
            self.dictToMediaArray(dict: dict, excludedNames: excludedNames)
        return medias.length != 0 ? MetadataViews.Medias(medias) : nil
    }
    
    pub fun dictToMediaArray(
        dict:{ 
            String: AnyStruct
        }?,
        excludedNames: [
    ...
    pub resource Admin{ 
        ...
        pub fun createItem(
            ...
            StartupyWGrze.items[newItemId] <-! create Item(
                    ...
                    maxSupply: maxSupply,
                    isTransferable: isTransferable != nil
                        ? isTransferable!
                        : true,
                    royalties: royalties
                )
