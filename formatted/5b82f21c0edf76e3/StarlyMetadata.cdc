...
pub contract StarlyMetadata{ 
    ...
    pub fun getDisplay(starlyID: String): MetadataViews.Display?{ 
        if let cardEdition = self.getCardEdition(starlyID: starlyID){ 
            ...
            return MetadataViews.Display(name: title.concat(" #").concat(edition).concat("/").concat(editions).concat(" by ").concat(creatorName), description: cardEdition.card.description, thumbnail: MetadataViews.HTTPFile(thumbnail!))
        }
        return nil
    }
    ...
    pub fun getRoyalties(starlyID: String): MetadataViews.Royalties?{ 
        if let cardEdition = self.getCardEdition(starlyID: starlyID){ 
            ...
            let royalties: [MetadataViews.Royalty] = [MetadataViews.Royalty(receiver: getAccount(0x12c122ca9266c278).getCapability<&{FungibleToken.Receiver}>(/public/flowTokenReceiver)!, cut: 0.05, description: "Starly royalty (5%)"), MetadataViews.Royalty(receiver: getAccount(creator.address!).getCapability<&{FungibleToken.Receiver}>(/public/flowTokenReceiver)!, cut: 0.05, description: "Creator royalty (%5) for ".concat(creator.username))]
    ...
    pub fun getExternalURL(starlyID: String): MetadataViews.ExternalURL?{ 
        if let cardEdition = self.getCardEdition(starlyID: starlyID){ 
            return MetadataViews.ExternalURL(url: cardEdition.url)
        }
        return nil
    }
    ...
    pub fun getNFTCollectionDisplay(): MetadataViews.NFTCollectionDisplay{ 
        return MetadataViews.NFTCollectionDisplay(
            ...
            socials:{ 
                "twitter":
                MetadataViews.ExternalURL("https://twitter.com/StarlyNFT"),
                "discord":
                MetadataViews.ExternalURL("https://discord.gg/starly"),
                "medium":
                MetadataViews.ExternalURL("https://medium.com/@StarlyNFT")
            }
        )
    }
    ...
    pub resource EditorProxy: IEditor, EditorProxyPublic{ 
        priv var editorCapability: Capability<&Editor>?
