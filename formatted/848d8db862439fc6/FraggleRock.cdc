...
pub contract FraggleRock:
    ...
    pub resource NFT:
        ...
        pub fun metadata():{ String: AnyStruct}?{ 
            let content =
                self.contentCapability.borrow<&{TiblesProducer.IContent}>()
                ?? panic("Failed to borrow content provider")
        ...
        pub fun displayData():{ String: String}{ 
            ...
            if metadata.containsKey("pack"){ 
                return{ "name": "FraggleRock pack", "description": "A FraggleRock pack", "imageUrl": "https://i.tibles.com/m/fragglerock-flow-icon.png"}
            }
            let set = metadata["set"]! as! &FraggleRock.Set
            let item = metadata["item"]! as! &FraggleRock.Item
            let variant = metadata["variant"]! as! &FraggleRock.Variant
            var edition: String = ""
        ...
        pub fun editions(): MetadataViews.Editions{ 
            ...
            let metadata = self.metadata() ?? panic("Missing NFT metadata")
            if metadata.containsKey("pack"){ 
                return MetadataViews.Editions([MetadataViews.Edition(name: "FraggleRock pack", number: UInt64(self.mintNumber), max: nil)])
            }
            let variant = metadata["variant"]! as! &FraggleRock.Variant
        ...
        pub fun externalURL(): MetadataViews.ExternalURL{ 
            return MetadataViews.ExternalURL(
                "https://fragglerock.tibles.com/collection".concat(
                    self.id.toString()
                )
            )
        }
        ...
        pub fun nftCollectionData(): MetadataViews.NFTCollectionData{ 
            return MetadataViews.NFTCollectionData(
                storagePath: FraggleRock.CollectionStoragePath,
                publicPath: FraggleRock.PublicCollectionPath,
                providerPath: /private/FraggleRockCollection,
                publicCollection: Type<
                    &FraggleRock.Collection{TiblesNFT.CollectionPublic}
                >(),
                publicLinkedType: Type<
                    &FraggleRock.Collection{
                ...
                createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
                    return <-FraggleRock.createEmptyCollection()
                }
            )
        }
        ...
        pub fun nftCollectionDisplay(): MetadataViews.NFTCollectionDisplay{ 
            let squareMedia =
                MetadataViews.Media(
                    file: MetadataViews.HTTPFile(
                        url: "https://i.tibles.com/m/fragglerock-flow-icon.png"
                    ),
            ...
            let bannerMedia =
                MetadataViews.Media(
                    file: MetadataViews.HTTPFile(
                        url: "https://i.tibles.com/m/fragglerock-flow-collection-banner.png"
                    ),
                    mediaType: "image/png"
                )
            let socialsData:{ String: String} ={
                "twitter": "https://twitter.com/tibleshq"}
            ...
            return MetadataViews.NFTCollectionDisplay(
                name: "Fraggle Rock Collection by Tibles",
                description: "Tibles Fraggle Rock is a digital trading card collecting experience by Tibles, made just for Fraggle Rock fans, backed by the FLOW blockchain.",
                externalURL: MetadataViews.ExternalURL(
                    "https://fragglerock.tibles.com"
                ),
    ...
    pub resource Collection:
        ...
        pub fun deposit(token: @NonFungibleToken.NFT){ 
            let tible <- token as! @FraggleRock.NFT
        ...
        pub fun depositTible(tible: @AnyResource{TiblesNFT.INFT}){ 
            ...
            let token <- tible as! @FraggleRock.NFT
            let id = token.id
        ...
        pub fun withdraw(withdrawID: UInt64): @NonFungibleToken.NFT{ 
            let token <-
                ...
                ?? panic(
                    "Cannot withdraw: tible does not exist in the collection"
                )
        ...
        pub fun destroyTible(id: UInt64){ 
            let token <-
                self.ownedNFTs.remove(key: id) ?? panic("NFT not found")
            destroy token
        }
    ...
    pub struct Variant{ 
        ...
        pub fun update(title: String, batchSize: UInt32?, maxCount: UInt32?){ 
            assert(
                batchSize == nil != (maxCount == nil),
                message: "batch size or max count can be used, not both"
            )
            let metadata:{ String: AnyStruct} ={ "title": title}
            let previousBatchSize =
                (self.metadata ??{} )["batchSize"] as! UInt32?
    ...
    pub resource Minter: TiblesProducer.IMinter{ 
        ...
        pub fun mintNext(){ 
            ...
            let id = FraggleRock.totalSupply + 1
    ...
    pub fun createNewPackMinter(
        id: String,
        contentCapability: Capability
    ): @PackMinter{ 
        assert(
            self.account.address == 0x848d8db862439fc6,
    ...
    init(){ 
        self.totalSupply = 0
        self.appId = "com.tibles.fragglerock"
        self.title = "Tibles Fraggle Rock"
        self
            .description = "Fraggle Rock officially licensed digital collectibles"
        self.ProducerStoragePath = /storage/TiblesFraggleRockProducer
        self.ProducerPath = /private/TiblesFraggleRockProducer
        self.ContentPath = /public/TiblesFraggleRockContent
        self.CollectionStoragePath = /storage/TiblesFraggleRockCollection
        self.PublicCollectionPath = /public/TiblesFraggleRockCollection
        let producer <- create Producer()
        self.account.save<@Producer>(<-producer, to: self.ProducerStoragePath)
        self.account.link<&Producer>(
            self.ProducerPath,
            target: self.ProducerStoragePath
        )
        self.account.link<&{TiblesProducer.IContent}>(
            self.ContentPath,
            target: self.ProducerStoragePath
        )
