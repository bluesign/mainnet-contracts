...
pub contract ColdStorage{ 
    ...
    pub resource PendingWithdrawal{ 
        priv var pendingVault: @FungibleToken.Vault
        
        priv var request: WithdrawRequest
        ...
        pub fun execute(fungibleTokenReceiverPath: PublicPath){ 
            ...
            let receiver =
                (recipient.getCapability(fungibleTokenReceiverPath)!).borrow<
    ...
    pub resource Vault: FungibleToken.Receiver, PublicVault{ 
        ...
        priv var contents: @FungibleToken.Vault
        
        priv var seqNo: UInt64
        ...
        priv fun incrementSequenceNumber(){ 
            self.seqNo = self.seqNo + UInt64(1)
        }
        
        priv fun isValidSignature(request:{ ColdStorage.ColdStorageRequest}): Bool{ 
            pre{ 
    ...
    pub fun validateSignature(
        ...
        let hashAlgorithm =
            HashAlgorithm(rawValue: key.hashAlgorithm)
