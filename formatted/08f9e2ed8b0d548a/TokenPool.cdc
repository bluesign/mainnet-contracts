...
pub contract TokenPool{ 
    ...
    pub resource Admin{ 
        ...
        pub fun removeLiquidity(
            amountToken1: UFix64,
            amountToken2: UFix64
        ): @FlowSwapPair.TokenBundle{ 
            let token1Vault <-
                TokenPool.token1Vault.withdraw(amount: amountToken1)
                as!
                @FlowToken.Vault
            let token2Vault <-
            ...
            let tokenBundle <-
                FlowSwapPair.createTokenBundle(
        ...
        pub fun withdrawFspLpTokens(amount: UFix64): @FlowSwapPair.Vault{ 
            let tokenVault <-
                TokenPool.fspLpTokenVault.withdraw(amount: amount)
                as!
                @FlowSwapPair.Vault
            return <-tokenVault
        }
        
        pub fun updateVirtualAmounts(
            amountToken1: UFix64,
            amountToken2: UFix64
        ){ 
    ...
    pub fun quoteSwapExactToken1ForToken2(amount: UFix64): UFix64{ 
        ...
        assert(
            self.token2Vault.balance > quote,
            message: "Not enough Token2 in the pool"
        )
        return quote
    }
    ...
    pub fun quoteSwapToken1ForExactToken2(amount: UFix64): UFix64{ 
        ...
        return self.preciseDiv(
            numerator: amount,
            denominator: self.buyBackPrice
        )
    }
    ...
    pub fun quoteSwapExactToken2ForToken1(amount: UFix64): UFix64{ 
        ...
        return self.token1Vault.balance > quote
            ? quote
            : self.token1Vault.balance
    }
    ...
    pub fun swapToken1ForToken2(
        ...
        let token2Amount =
            self.quoteSwapExactToken1ForToken2(amount: token1Amount)
        ...
        return <-(
            self.token2Vault.withdraw(amount: token2Amount)
            as!
            @TeleportedTetherToken.Vault
        )
    }
    ...
    pub fun swapToken2ForToken1(
        from: @TeleportedTetherToken.Vault
    ): @FlowToken.Vault{ 
        pre{ 
            !TokenPool.isFrozen:
                "TokenPool is frozen"
        ...
        let token1Amount =
            self.quoteSwapExactToken2ForToken1(amount: token2Amount)
        ...
        return <-(
            self.token1Vault.withdraw(amount: token1Amount) as! @FlowToken.Vault
        )
    }
