...
pub contract WLD: FungibleToken{ 
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance{ 
        ...
        destroy(){ 
            WLD.totalSupply = WLD.totalSupply - self.balance
        }
    }
    ...
    pub resource Minter{ 
        ...
        //
        pub fun mintTokens(amount: UFix64): @WLD.Vault{ 
            pre{ 
                amount > UFix64(0):
    ...
    init(){ 
        ...
        self.account.save(<-admin, to: /storage/wldAdmin)
        
        // mint tokens
        let tokenVault <- minter.mintTokens(amount: self.totalSupply)
        self.account.save(<-tokenVault, to: /storage/wldVault)
        destroy minter
        
        // Create a public capability to the stored Vault that only exposes
        // the `balance` field through the `Balance` interface
        self.account.link<&WLD.Vault{FungibleToken.Balance}>(/public/wldBalance, target: /storage/wldVault)
        
        // Create a public capability to the stored Vault that only exposes
        // the `deposit` method through the `Receiver` interface
        self.account.link<&{FungibleToken.Receiver}>(/public/wldReceiver, target: /storage/wldVault)
