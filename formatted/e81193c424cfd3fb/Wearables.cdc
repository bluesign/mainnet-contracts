...
pub contract Wearables: NonFungibleToken{ 
    ...
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver{ 
        ...
        pub fun getAllTraitsMetadata(): [MetadataViews.Trait]{ 
            ...
            traits.append(MetadataViews.Trait(name: "template", value: template.name, displayType: "string", rarity: rarity))
            traits.append(MetadataViews.Trait(name: "template_id", value: self.template.id, displayType: "number", rarity: rarity))
            traits.append(MetadataViews.Trait(name: "position", value: template.getPosition().name, displayType: "string", rarity: rarity))
            let layer = self.getTemplate().getLayer()
            if layer != ""{ 
                traits.append(MetadataViews.Trait(name: "layer", value: layer, displayType: "string", rarity: rarity))
            }
            traits.append(MetadataViews.Trait(name: "set", value: template.getSetName(), displayType: "string", rarity: rarity))
            let setType = template.getSet().getSetType()
            if setType != ""{ 
                traits.append(MetadataViews.Trait(name: "set_type", value: setType, displayType: "string", rarity: rarity))
            }
            traits.append(MetadataViews.Trait(name: "set_creator", value: template.getSet().getCreator(), displayType: "string", rarity: rarity))
            let edition = self.editions[0]
            let active = self.getActive(edition.name)
            ...
            if termsOfService != nil{ 
                traits.append(MetadataViews.Trait(name: "Terms of Service", value: termsOfService!, displayType: "string", rarity: nil))
            }
            return traits
        }
    }
    ...
    access(account) fun mintNFT(recipient: &{NonFungibleToken.Receiver}, template: UInt64, context:{ String: String}){ 
        pre{ 
            recipient.owner != nil:
                "Recipients NFT collection is not owned"
        }
        let newNFT <- Wearables.mintNFTDirect(recipientAddress: (recipient.owner!).address, template: template, context: context)
    ...
    access(account) fun mintNFTDirect(recipientAddress: Address, template: UInt64, context:{ String: String}): @Wearables.NFT{ 
        pre{ 
    ...
    // and deposit it in the recipients collection using their collection reference
    access(account) fun mintEditionNFT(recipient: &{NonFungibleToken.Receiver}, template: UInt64, setEdition: UInt64, positionEdition: UInt64, templateEdition: UInt64, taggedTemplateEdition: UInt64, tagEditions: [UInt64], context:{ String: String}){ 
        pre{ 
            recipient.owner != nil:
                "Recipients NFT collection is not owned"
            self.templates.containsKey(template):
    ...
    access(account) fun updateTemplateDescription(templateId: UInt64, description: String){ 
        pre{ 
            self.templates.containsKey(templateId):
    ...
    init(){ 
        ...
        self.account.link<&Wearables.Collection{NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(Wearables.CollectionPublicPath, target: Wearables.CollectionStoragePath)
        self.account.link<&Wearables.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(Wearables.CollectionPrivatePath, target: Wearables.CollectionStoragePath)
