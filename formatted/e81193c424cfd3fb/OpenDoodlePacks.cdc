...
pub contract OpenDoodlePacks: NonFungibleToken{ 
    ...
    access(contract) fun revealRemainingDistributionsMinAmount(receiverAddress: Address, randomNumbers: [UFix64], packId: UInt64, packType: DoodlePackTypes.PackType, remainingDistributions: [OpenDoodlePacks.MintableTemplateDistribution]): [OpenDoodlePacks.MintableTemplateDistribution]{ 
    ...
    access(contract) fun revealRemainingDistributions(receiverAddress: Address, packId: UInt64, packType: DoodlePackTypes.PackType, randomNumber: UFix64, remainingDistributions: [OpenDoodlePacks.MintableTemplateDistribution]): [OpenDoodlePacks.MintableTemplateDistribution]{ 
    ...
    priv fun mintNFTFromPack(collection: DoodlePackTypes.Collection, receiverAddress: Address, templateId: UInt64, packId: UInt64, packType: DoodlePackTypes.PackType){ 
        switch collection{ 
            case DoodlePackTypes.Collection.Wearables:
                let recipient = getAccount(receiverAddress).getCapability<&{NonFungibleToken.Receiver}>(Wearables.CollectionPublicPath).borrow() ?? panic("Could not borrow wearables receiver capability")
            ...
            case DoodlePackTypes.Collection.Redeemables:
                let recipient = getAccount(receiverAddress).getCapability<&{NonFungibleToken.Receiver}>(Redeemables.CollectionPublicPath).borrow() ?? panic("Could not borrow redeemables receiver capability")
                Redeemables.mintNFT(recipient: recipient, templateId: templateId)
        }
    }
    
    priv fun getRandomNumbersFromPack(pack: &OpenDoodlePacks.NFT, amount: UInt8): [UFix64]{ 
    ...
    init(){ 
        ...
        self.account.link<&OpenDoodlePacks.Collection{OpenDoodlePacks.CollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(OpenDoodlePacks.CollectionPublicPath, target: OpenDoodlePacks.CollectionStoragePath)
        self.account.link<&OpenDoodlePacks.Collection{OpenDoodlePacks.CollectionPublic, NonFungibleToken.Provider, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(OpenDoodlePacks.CollectionPrivatePath, target: OpenDoodlePacks.CollectionStoragePath)
