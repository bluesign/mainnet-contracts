...
pub contract Teleport{ 
    ...
    pub struct AllowedStatus{ 
        ...
        init(remoteId: UInt64, teleporterId: UInt64){ 
            ...
            let ids =
                "DoodleID : ".concat(remoteId.toString()).concat(
                    " DooplicatorID : "
                ).concat(teleporterId.toString())
    ...
    pub fun isGenesisBoxAllowed(
        _ genesisBoxId: UInt64
    ): GenesisBoxRegistry.AllowedStatus{ 
    ...
    init(){ 
        ...
        self.account.link<&Server>(
            self.TeleportServerPrivatePath,
            target: self.TeleportServerStoragePath
        )
    }
}
