...
pub contract DoodleNames: NonFungibleToken{ 
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: DoodleNames.CollectionStoragePath, publicPath: DoodleNames.CollectionPublicPath, providerPath: DoodleNames.CollectionPrivatePath, publicCollection: Type<&Collection{NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), publicLinkedType: Type<&Collection{NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
                            return <-DoodleNames.createEmptyCollection()
                        })
            }
            return nil
        }
    ...
    access(account) fun mintNFT(recipient: &{NonFungibleToken.Receiver}, name: String, context:{ String: String}){ 
        pre{ 
            recipient.owner != nil:
    ...
    access(account) fun mintName(name: String, context:{ String: String}, address: Address): @NFT{ 
        pre{ 
            !self.registry.containsKey(name):
                "Name already exist. Name : ".concat(name)
            FIND.validateFindName(name):
    ...
    init(){ 
        ...
        self.account.link<&DoodleNames.Collection{NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(DoodleNames.CollectionPublicPath, target: DoodleNames.CollectionStoragePath)
        self.account.link<&DoodleNames.Collection{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(DoodleNames.CollectionPrivatePath, target: DoodleNames.CollectionStoragePath)
