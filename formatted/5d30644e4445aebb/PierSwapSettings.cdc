...
pub contract PierSwapSettings{ 
    ...
    pub resource Admin{ 
        ...
        pub fun setFees(newTotalFee: UFix64, newProtocolFee: UFix64){ 
            ...
            emit SwapFeesUpdated(
                poolTotalFee: newTotalFee,
                poolProtocolFee: newProtocolFee
            )
        }
        ...
        pub fun setProtocolFeeRecipient(newAddress: Address){ 
            pre{ 
                getAccount(newAddress).getCapability<&PierLPToken.Collection{MultiFungibleToken.Receiver}>(PierLPToken.CollectionPublicPath).check():
        ...
        pub fun enableObservation(){ 
            ...
            emit ObservationSwitchUpdated(
                enabled: PierSwapSettings.observationEnabled
            )
        }
    ...
    pub fun depositProtocolFee(vault: @MultiFungibleToken.Vault){ 
        let feeCollectionRef =
            getAccount(self.protocolFeeRecipient).getCapability<
                &PierLPToken.Collection{MultiFungibleToken.Receiver}
            >(PierLPToken.CollectionPublicPath).borrow()
            ?? panic(
                "Metapier PierSwapSettings: Protocol fee receiver not found"
            )
