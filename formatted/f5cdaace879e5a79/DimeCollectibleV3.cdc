...
pub contract DimeCollectibleV3: NonFungibleToken{ 
    ...
    pub resource NFTMinter{ 
        ...
        pub fun mintNFTs(collection: &{NonFungibleToken.CollectionPublic}, blueprintId: UInt64, numCopies: UInt64, creators: [Address], content: String, hiddenContent: String?, tradeable: Bool, previousHistory: [Transaction]?, royalties: Royalties, initialSale: Transaction?){ 
        ...
        pub fun mintRoyaltyNFTs(collection: &{NonFungibleToken.CollectionPublic}, blueprintId: UInt64, numCopies: UInt64, creators: [Address], content: String, tradeable: Bool, royalties: Royalties, initialSale: Transaction?): [UInt64]{ 
            let history: [Transaction] = initialSale != nil ? [initialSale!] : []
            var counter = 1 as UInt64
            let idsUsed: [UInt64] = []
            while counter <= numCopies{ 
        ...
        pub fun mintReleaseNFTs(collection: &{NonFungibleToken.CollectionPublic}, blueprintId: UInt64, numCopies: UInt64, creators: [Address], content: String, hiddenContent: String?, tradeable: Bool, previousHistory: [Transaction]?, royalties: Royalties, initialSale: Transaction?){ 
            let history: [Transaction] = initialSale != nil ? [initialSale!] : []
            var counter = 1 as UInt64
            while counter <= numCopies{ 
                let tokenId = DimeCollectibleV3.totalSupply + counter
