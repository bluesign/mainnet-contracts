...
pub contract TuneGO: NonFungibleToken{ 
    ...
    // -----------------------------------------------------------------------
    // TuneGO contract Events
    // -----------------------------------------------------------------------
    
    // Emitted when the TuneGO contract is created
    ...
    priv var itemDatas:{ UInt32: Item}
    ...
    // -----------------------------------------------------------------------
    // TuneGO contract-level Composite Type definitions
    // -----------------------------------------------------------------------
    ...
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver{ 
        ...
        pub fun getViews(): [Type]{ 
            return [Type<MetadataViews.Royalties>(), Type<MetadataViews.Display>(), Type<MetadataViews.Editions>(), Type<MetadataViews.ExternalURL>(), Type<MetadataViews.NFTCollectionData>(), Type<MetadataViews.NFTCollectionDisplay>(), Type<MetadataViews.Serial>(), Type<MetadataViews.Traits>()]
        }
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                ...
                case Type<MetadataViews.Display>():
                    ...
                    return MetadataViews.Display(name: metadata["Title"]!, description: metadata["Description"]!, thumbnail: MetadataViews.HTTPFile(url: assetUrl.slice(from: 0, upTo: assetUrl.length - 3).concat("gif")))
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: TuneGO.CollectionStoragePath, publicPath: TuneGO.CollectionPublicPath, providerPath: TuneGO.CollectionPrivatePath, publicCollection: Type<&TuneGO.Collection{TuneGO.TuneGOCollectionPublic}>(), publicLinkedType: Type<&TuneGO.Collection{TuneGO.TuneGOCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&TuneGO.Collection{TuneGO.TuneGOCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Provider, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
                ...
                case Type<MetadataViews.NFTCollectionDisplay>():
                    ...
                    let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://tunegonft.com/assets/images/collections-page/tunekitties.png"), mediaType: "image/png")
                    let socials:{ String: MetadataViews.ExternalURL} ={} 
                    return MetadataViews.NFTCollectionDisplay(name: "TuneKitties", description: "Deep in the clubs and streets of the metaverse TuneKitties prowl with the purrfect mix of sound and style. Limited edition NFTs by TuneGO.", externalURL: MetadataViews.ExternalURL("https://www.tunegonft.com/collection-details/98434c03-17e3-4e9a-9f6b-3b786a4fad6e"), squareImage: media, bannerImage: media, socials: socials)
    ...
    // Interface that users can cast their TuneGO Collection as
    // to allow others to deposit TuneGO Collectibles into their Collection.
    pub resource interface TuneGOCollectionPublic{ 
    ...
    // -----------------------------------------------------------------------
    // TuneGO contract-level function definitions
    // -----------------------------------------------------------------------
    ...
    pub fun getAllItems(): [TuneGO.Item]{ 
    ...
    // -----------------------------------------------------------------------
    // TuneGO initialization function
    // -----------------------------------------------------------------------
    
    init(){ 
        // Paths
        self.CollectionPublicPath = /public/TuneGOCollection
        self.CollectionPrivatePath = /private/TuneGOCollection
        self.CollectionStoragePath = /storage/TuneGOCollection
        self.AdminPublicPath = /public/TuneGOAdmin
        self.AdminStoragePath = /storage/TuneGOAdmin
        ...
        self.account.save<@Collection>(<-create Collection(), to: TuneGO.CollectionStoragePath)
        
        // Create a public capability for the Collection
        self.account.link<&{TuneGOCollectionPublic}>(TuneGO.CollectionPublicPath, target: TuneGO.CollectionStoragePath)
        
        // Put the Admin in storage
        self.account.save<@Admin>(<-create Admin(), to: TuneGO.AdminStoragePath)
