...
pub contract FreshmintClaimSale{ 
    ...
    pub resource Sale: SalePublic{ 
        ...
        ///
        priv let collection: Capability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection}>
        ...
        ///
        priv let allowlist: Capability<&Allowlist>?
        
        init(id: String, collection: Capability<&AnyResource{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection}>, receiverPath: PublicPath, paymentReceiver: Capability<&{FungibleToken.Receiver}>, price: UFix64, allowlist: Capability<&Allowlist>?){ 
            self.id = id
            self.price = price
        ...
        ///
        priv fun checkAllowlist(address: Address){ 
        ...
        pub fun claim(payment: @FungibleToken.Vault, address: Address){ 
            ...
            let nftReceiver = getAccount(address).getCapability(self.receiverPath).borrow<&{NonFungibleToken.CollectionPublic}>()!
    ...
    pub resource Allowlist{ 
        ...
        ///
        priv let claimsByAddress:{ Address: UInt}
