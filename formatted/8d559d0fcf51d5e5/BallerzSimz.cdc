...
pub contract BallerzSimz{ 
    ...
    pub event SimStarted(
        simId: UInt64,
        teams: [
            BallerzTeamSimEventInfo
        ],
        bonuses:{ 
            String: String
        }
    ...
    pub resource BallerzTeam{ 
        ...
        priv let teamSize: UInt8
        
        priv let ballerIdz: [UInt64]
        
        // {ballerId: ownerAddress}
        priv let ballerOwnerz:{ UInt64: Address}
        
        // Capabilities to let us access the baller NFTs (to access the metadata)
        access(contract) let ownerCapabilities:{ 
            UInt64: Capability<&AnyResource{Gaia.CollectionPublic}>
        }
        
        // Capabilities to payout winners
        access(contract) let ownerReceivers:{ 
            UInt64: Capability<&{FungibleToken.Receiver}>
        }
        ...
        init(
            ...
            self.teamName = teamName
            self.feePercentage = BallerzSimz.getEntryFeeToFeePercentageMap()[
                    entryTokens.balance
                ]!
            self.ballerIdz = []
        ...
        access(contract) fun joinTeam(
            ...
            let ownerGaiaCollectionCapability =
                ballerOwner.getCapability<&{Gaia.CollectionPublic}>(
            ...
            self.ownerCapabilities.insert(
                key: baller.id,
                ownerGaiaCollectionCapability
            )
            // get capability to deposit winnings into owners account
            self.ownerReceivers.insert(
                key: baller.id,
                ballerOwner.getCapability<&{FungibleToken.Receiver}>(
        ...
        access(contract) fun leaveTeam(baller: &Gaia.NFT): Int{ 
            ...
            (ownerReceiver.borrow()!).deposit(
                from: <-self.entryTokens.remove(key: baller.id)!
            )
        ...
        access(contract) fun transferAllEntryTokens(
        ...
        destroy(){ 
            if !self.startedSim{ 
                ...
                for ballerId in self.ballerIdz{ 
                    self.leaveTeam(baller: ((self.ownerCapabilities[ballerId]!).borrow()!).borrowGaiaNFT(id: ballerId)!)
                }
    ...
    pub resource BallerzSimzResource: BallerzSimzAdmin, BallerzSimzPublic{ 
        ...
        priv let ownerVaultProvider: Capability<&{FungibleToken.Provider}>
        
        priv let addressOwner: Capability<&{FungibleToken.Balance}>
        ...
        pub fun updateFeeOwner(newAddress: Address){ 
            ...
            self.feeOwnerCapability = newAddressAccount.getCapability<&{FungibleToken.Receiver}>(BounceToken.bounceTokenReceiverPath)
        }
        ...
        pub fun startSim(team1Id: UInt64, team2Id: UInt64, entryAmount: UFix64, feePercentage: UFix64, bonuses:{ String: String}): UInt64{ 
            pre{ 
        ...
        pub fun completeSim(simId: UInt64, winnerId: UInt64, loserId: UInt64, winnerScores: [UInt16], loserScores: [UInt16]){ 
            pre{ 
    ...
    pub resource BallerzSimzConfig: BallerzSimzConfigAdmin{ 
        priv var teamSizes: [UInt8]
        
        // {entryFee : feePercentage}
        priv var entryFeeToFeePercentageMap:{ UFix64: UFix64}
        
        priv var currentEntryFees: [UFix64]
        ...
        priv var contractPaused: Bool
        
        priv var acceptedBallerzSetIdz: [UInt64]
    ...
    pub fun getCurrentEntryFees(): [UFix64]{ 
        let configAdmin =
            self.account.getCapability<&{BallerzSimzConfigAdmin}>(
    ...
    init(){ 
        ...
        self.account.link<&{BallerzSimzAdmin}>(
            self.BallerzSimzResourceAdminPath,
            target: self.BallerzSimzResourceStoragePath
        )
        // Publish public capability to public
        self.account.link<&{BallerzSimzPublic}>(
            self.BallerzSimzResourcePublicPath,
            target: self.BallerzSimzResourceStoragePath
        )
        ...
        self.account.link<&{BallerzSimzConfigAdmin}>(
            self.BallerzSimzConfigAdminPath,
            target: self.BallerzSimzConfigStoragePath
        )
    }
}
