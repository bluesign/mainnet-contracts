    ...
    pub resource CollectionQueue:
        ...
        pub fun deposit(token: @NonFungibleToken.NFT){ 
            let collection =
                ...
                ?? panic(
                    "CollectionQueue.deposit: failed to borrow collection capability"
                )
        ...
        pub fun borrowNFT(id: UInt64): &NonFungibleToken.NFT{ 
            let collection =
                self.collection.borrow()
                ?? panic(
                    "CollectionQueue.borrowNFT: failed to borrow collection capability"
                )
        ...
        pub fun getNextNFT(): @NonFungibleToken.NFT?{ 
            ...
            let collection =
                self.collection.borrow()
                ?? panic(
                    "CollectionQueue.getNextNFT: failed to borrow collection capability"
                )
