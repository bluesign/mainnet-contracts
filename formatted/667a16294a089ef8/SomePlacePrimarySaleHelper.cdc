...
pub contract SomePlacePrimarySaleHelper{ 
    priv let premintedNFTCap: Capability<
    ...
    access(account) fun retrieveAvailableNFT(): @SomePlaceCollectible.NFT{ 
        ...
        return <-(
            nftCollection.withdraw(
                withdrawID: nftCollection.getIDs()[randomIndex]
            )
            as!
    ...
    init(){ 
        self.account.link<&SomePlaceCollectible.Collection>(
            /private/SomePlacePrimarySaleHelperAccess,
            target: SomePlaceCollectible.CollectionStoragePath
        )
        self.premintedNFTCap = self.account.getCapability<
                &{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}
            >(/private/SomePlacePrimarySaleHelperAccess)
    }
}
