...
pub contract ListenAuction{ 
    ...
    pub event AuctionSettled(
        id: UInt64,
        winnersAddress: Address,
        finalSalePrice: UFix64
    )
    ...
    pub resource Auction{ 
        ...
        init(
            ...
            self.bid <- create Bid(
                    funds: <-ListenUSD.createEmptyVault(),
                    ftReceiverCap: nil,
                    nftReceiverCap: nil
                )
            self.history = []
        ...
        destroy(){ 
            if self.nftCollection.getIDs().length > 0{ 
                let depositRef = ListenAuction.account.getCapability(ListenNFT.CollectionPublicPath).borrow<&{NonFungibleToken.CollectionPublic}>() ?? panic("Could not borrow a reference to accounts's collection") // possible safer alternative would be to store in contract 
    ...
    pub resource Bid{ 
        ...
        access(contract) fun returnBidToOwner(){ 
            ...
            var ownersVaultRef =
                ftReceiverCap.borrow<&{FungibleToken.Receiver}>()!
    ...
    pub resource Admin{ 
        pub fun createAuction(
            ...
            let temp <-
                ListenAuction.auctions.insert(
                    key: ListenAuction.nextID,
                    <-auction
                )
            destroy temp
        ...
        pub fun removeAuction(auctionID: UInt64){ 
            let auctionRef =
                ListenAuction.borrowAuction(id: auctionID)
                ?? panic("Auction ID does not exist")
            let bidRef = &auctionRef.bid as &Bid
            assert(
                bidRef.vault.balance == 0.0,
                message: "Auction still has a bid, can't remove"
            )
        ...
        pub fun settleAuction(auctionID: UInt64){ 
            ...
            assert(
                bidRef.vault.balance > 0.0,
                message: "Auction must have a bid"
            )
            let winnerNFTcap = &bidRef.nftReceiverCap! as &Capability
            let winnersReceiverRef =
                winnerNFTcap.borrow<&{NonFungibleToken.CollectionPublic}>()!
            for id in auctionRef.nftCollection.getIDs(){ 
                let nft <- auctionRef.nftCollection.withdraw(withdrawID: id)
            ...
            let ftReceiverCap =
                ListenAuction.account.getCapability(
    ...
    pub fun getAuctionMeta(auctionID: UInt64): AuctionMeta{ 
        let auctionRef =
            ListenAuction.borrowAuction(id: auctionID)
            ?? panic("No Auction with that ID exists")
        let bidRef = &auctionRef.bid as &Bid
        let vaultRef = &bidRef.vault as &FungibleToken.Vault
        let auctionState =
            ListenAuction.stateToString(auctionRef.getAuctionState())
    ...
    pub fun placeBid(
        ...
        let auctionRef =
            ...
            ?? panic("Auction ID does not exist")
        assert(
            funds.balance >= auctionRef.startingPrice,
            message: "Bid must be above starting bid"
        )
        assert(
            ListenAuction.now() > auctionRef.startTime,
            message: "Auction hasn't started"
        )
        assert(
            ListenAuction.now() < auctionRef.endTime,
        ...
        if ListenAuction.now()
        > auctionRef.endTime - ListenAuction.EXTENSION_TIME{ 
    ...
    init(){ 
        ...
        self.account.link<&ListenAuction.Admin>(
            ListenAuction.AdminCapabilityPath,
            target: ListenAuction.AdminStoragePath
        )
