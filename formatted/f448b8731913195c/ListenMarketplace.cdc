...
pub contract ListenMarketplace{ 
    ...
    pub resource ListenStorefront:
        ...
        pub fun createListing(
         ...
         : UInt64{ 
            let royaltyReceiverCap =
                getAccount(ListenMarketplace.listenRoyaltyPaymentAddress)
                    .getCapability<&{FungibleToken.Receiver}>(
                    ListenUSD.ReceiverPublicPath
                )
            assert(
                royaltyReceiverCap.borrow() != nil,
                message: "Missing or mis-typed ListenUSD receiver (royalty account)"
            )
            
            // 5% royalty fee 
            let royalty =
                ListenMarketplace.SaleCut(
            ...
            let saleCut =
                ListenMarketplace.SaleCut(
                    ...
                    amount: salePrice - royalty.amount
                ) // -royalty.amount = levy on seller  omitted equivalent to levy on buyer
            let listing <-
                create Listing(
        ...
        pub fun cleanup(listingResourceID: UInt64){ 
            ...
            assert(
                listing.getDetails().purchased == true,
                message: "listing is not purchased, only admin can remove"
            )
            destroy listing
        }
    ...
    init(){ 
        ...
        let listenUSD_Vault =
            self.account.borrow<&ListenUSD.Vault>(
                from: ListenUSD.VaultStoragePath
            )
