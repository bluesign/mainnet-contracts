...
pub contract Flowns{ 
    ...
    pub resource RootDomainCollection:
        ...
        access(account) fun changeRootDomainVault(
            ...
            pre{ 
                self.domains[domainId]                                       // Server is the root collection for manager to create and store root domain
        ...
        access(account) fun mintDomain(
            ...
            root                
                ...
                .mintDomain(name: name, duration: duration                                                          
                                                          // calc hash with node and lable
                                                          , receiver: receiver)
        }
        
        pub fun getAllDomains()                               
                               // Query root domain
                               :{ UInt64: RootDomainInfo}{ 
            var domainInfos:{ UInt64                                    // Query all root domain
                                    : RootDomainInfo} ={} 
            for id in self.domains.keys{ 
                let itemRef =                              
                              // Check domain available 
                              &self.domains[id] as &RootDomain
