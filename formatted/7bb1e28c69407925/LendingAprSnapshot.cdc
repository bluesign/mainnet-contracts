...
pub contract LendingAprSnapshot{ 
    ...
    priv let _markets:{ Address: AprSnapshot}
    
    /// Reserved parameter fields: {ParamName: Value}
    priv let _reservedFields:{ String: AnyStruct}
    
    pub event MarketDataTracked(
        market: Address,
        marketType: String,
        startTrackingFrom: UFix64
    )
    
    pub event MarketDataErased(market: Address, erasedFrom: UFix64)
    
    pub event AprSampled(
        market: Address,
    ...
    pub struct AprSnapshot{ 
        ...
        priv let aprObservations: [Observation]
        ...
        pub fun queryHistoricalAprData(scale: UInt8, plotPoints: UInt64): [
            Observation
        ]{ 
            let now = getCurrentBlock().timestamp
            ...
            let numSampledPoints =
                idxPrev < idxNow
                    ? idxNow - idxPrev + 1
                    : self.numSamples + idxNow - idxPrev + 1
            assert(
    ...
    pub fun queryHistoricalAprData(
        poolAddr: Address,
        scale: UInt8,
        plotPoints: UInt64
    ): [
    ...
    access(contract) fun trackMarketData(poolAddr: Address){ 
        ...
        let poolPublicCap =
            getAccount(poolAddr).getCapability<&{LendingInterfaces.PoolPublic}>(
