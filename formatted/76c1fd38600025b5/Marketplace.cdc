...
pub contract Marketplace{ 
    ...
    pub event ArtistRoyaltyDeposited(
        itemID: UInt64,
        amount: UFix64,
        to: Address?
    )
    
    pub event MaterialRoyaltyDeposited(
        itemID: UInt64,
        amount: UFix64,
        to: Address?
    )
    
    pub event ContractRoyaltyDeposited(
        itemID: UInt64,
        amount: UFix64,
        to: Address?
    )
    ...
    priv var itemSaleCount:{ UInt64: UInt32}
    
    // the FlowToken capability that channel fee royalties will be transferred to 
    priv var contractCap: Capability<&FlowToken.Vault{FungibleToken.Receiver}>
    ...
    pub resource interface ListingsPublic{ 
        ...
        pub fun getItemListingDetail(
            itemID: UInt64
        ): Marketplace.ListingDetails{} 
    ...
    pub resource Listings: ListingsPublic{ 
        ...
        priv var listings:{ UInt64: ListingDetails}
        ...
        // to this Vault
        priv var ownerFlowTokenCapability: Capability<&FlowToken.Vault{FungibleToken.Receiver}>
        
        init(itemProviderCapability: Capability<&ItemNFT.Collection{ItemNFT.ItemCollectionPublic, NonFungibleToken.Provider}>, ownerFlowTokenCapability: Capability<&FlowToken.Vault{FungibleToken.Receiver}>){ 
            pre{ 
    ...
    pub fun createListings(
        itemProviderCapability: Capability<
            &ItemNFT.Collection{
                ItemNFT.ItemCollectionPublic,
                NonFungibleToken.Provider
            }
        >,
        ownerFlowTokenCapability: Capability<
    ...
    pub resource Offers: OffersPublic{ 
        ...
        priv var offers:{ UInt64: OfferDetails}
        ...
        // to this Vault
        priv var itemVaultCapability: Capability<&{ItemNFT.ItemCollectionPublic}>
        
        init(itemVaultCapability: Capability<&{ItemNFT.ItemCollectionPublic}>, flowTokenProviderCapability: Capability<&FlowToken.Vault{FungibleToken.Provider, FungibleToken.Balance}>){ 
            pre{ 
        ...
        pub fun makeOffer(itemAddress: Address, itemID: UInt64, price: UFix64){ 
            ...
            let collectionRef = getAccount(itemAddress).getCapability(ItemNFT.CollectionPublicPath).borrow<&{ItemNFT.ItemCollectionPublic}>()!
    ...
    pub fun createOffers(
        ...
        flowTokenProviderCapability: Capability<
            &FlowToken.Vault{FungibleToken.Provider, FungibleToken.Balance}
        >
    ): @Offers{ 
    ...
    pub resource Admin{ 
        ...
        pub fun setContractRoyaltyCap(
            contractCap: Capability<&FlowToken.Vault{FungibleToken.Receiver}>
        ){ 
            pre{ 
    ...
    pub init(){ 
        ...
        self
            .ListingsStoragePath = /storage/fabricantStorageMarketplaceListings0001
        ...
        self.contractCap = self.account.getCapability<
                &FlowToken.Vault{FungibleToken.Receiver}
            >(/public/flowTokenReceiver)
