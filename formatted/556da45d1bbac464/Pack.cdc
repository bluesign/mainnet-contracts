...
pub contract Pack{ 
    priv let vault: @Elvn.Vault
    
    priv let momentsListCandidate: @{UInt64: [[Moments.NFT]]}
    
    priv let salePacks: @{UInt64: [Pack.Token]}
    ...
    pub resource Token{ 
        ...
        pub fun openPacks(): @[Moments.NFT]{ 
            ...
            let momentsListLength =
                Pack.getMomentsListRemainingCount(releaseId: self.releaseId)
            let randomIndex = unsafeRandom() % UInt64(momentsListLength)
            let momentsListCandidateRef =
                (
                    &Pack.momentsListCandidate[self.releaseId]
                    as
                    &[
                        [
                            Moments.NFT
    ...
    pub fun getMomentsListRemainingCount(releaseId: UInt64): Int{ 
        ...
        let momentsListCandidateRef =
            (&self.momentsListCandidate[releaseId] as &[[Moments.NFT]]?)!
    ...
    init(){ 
        ...
        self.account.save(
            ...
            to: /storage/sportiumPackAdministrator
        )
    }
}
