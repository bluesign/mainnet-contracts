...
import StarVaultConfig from "../0xb6cb9ef4b1692616/StarVaultConfig.cdc"

import StarVaultInterfaces from "../0xb6cb9ef4b1692616/StarVaultInterfaces.cdc"

import LPStaking from "../0xb6cb9ef4b1692616/LPStaking.cdc"
...
pub contract StarVault: FungibleToken{ 
    ...
    priv fun mintToken(amount: UFix64): @StarVault.Vault{ 
        self.totalSupply = self.totalSupply + amount
        emit TokensMinted(amount: amount)
        return <-create Vault(balance: amount)
    }
    
    priv fun burnToken(from: @FungibleToken.Vault){ 
        let amount = from.balance
        destroy from
        emit TokensBurned(amount: amount)
    }
    
    priv fun distributeFees(fee: @FungibleToken.Vault){ 
    ...
    pub fun redeem(amount: Int, vault: @FungibleToken.Vault, specificIds: [UInt64], feeVault: @FungibleToken.Vault): @[AnyResource]{ 
        pre{ 
    ...
    pub fun swap(nfts: @[NonFungibleToken.NFT], specificIds: [UInt64], feeVault: @FungibleToken.Vault): @[AnyResource]{ 
        pre{ 
    ...
    priv fun receiveNFTs(nfts: @[NonFungibleToken.NFT]): Int{ 
        let collection = self.account.borrow<&NonFungibleToken.Collection>(from: StarVaultConfig.VaultNFTCollectionStoragePath)!
        let count = nfts.length
        var i = 0
        while i < count{ 
    ...
    priv fun withdrawNFTs(count: Int, specificIds: [UInt64]): @[NonFungibleToken.NFT]{ 
        let collection = self.account.borrow<&NonFungibleToken.Collection>(from: StarVaultConfig.VaultNFTCollectionStoragePath)!
    ...
    priv fun getRandomNFTFromCollection(): @NonFungibleToken.NFT{ 
        let collection = self.account.borrow<&NonFungibleToken.Collection>(from: StarVaultConfig.VaultNFTCollectionStoragePath)!
    ...
    priv fun setVaultFeatures(enableMint: Bool, enableRandomRedeem: Bool, enableTargetRedeem: Bool, enableRandomSwap: Bool, enableTargetSwap: Bool){ 
    ...
    priv fun setVaultName(vaultName: String){ 
    ...
    pub resource Admin: StarVaultInterfaces.VaultAdmin{ 
        pub fun setVaultFeatures(enableMint: Bool, enableRandomRedeem: Bool, enableTargetRedeem: Bool, enableRandomSwap: Bool, enableTargetSwap: Bool){ 
