...
pub contract NFTStorefront{ 
    ...
    pub resource Listing: ListingPublic{ 
        ...
        pub fun purchase(payment: @FungibleToken.Vault): @NonFungibleToken.NFT{ 
            ...
            for cut in self.details.saleCuts{ 
                if !cut.receiver.check(){ 
                    panic("Given recipient has not authorised to receive the payment")
                }
            ...
            destroy payment
            emit ListingCompleted(id: self.details.id, nftID: self.details.nftID, nftType: self.details.nftType, paymentVaultType: self.details.paymentVaultType, price: self.details.price, expiry: self.details.expiry, purchased: self.details.purchased)
            // Tranfer token
            return <-nft
        }
        ...
        /// initializer
        ///
        init(id: UInt64, nftProviderCapability: Capability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>, nftID: UInt64, nftType: Type, paymentVaultType: Type, saleCuts: [SaleCut], expiry: UInt64){ 
            ...
            if let capability = self.nftProviderCapability as? Capability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection}>{ 
                ...
                for trait in traitsView.traits{ 
                    if trait.name == "minter"{ 
                        totalPrice.append(SaleCut(receiver: getAccount(trait.value as! Address).getCapability<&{FungibleToken.Receiver}>(MemeToken.ReceiverPublicPath)!, amount: currentPrice * MarketplaceFees.getFeeParameters().rate, description: "minter"))
                    }
                }
            } else{ 
            ...
            // Add marketplace fee
            totalPrice.append(SaleCut(receiver: MarketplaceFees.getFeeParameters().receiverCapability, amount: currentPrice * MarketplaceFees.getFeeParameters().rate, description: "marketplace"))
            
            // Add seller asking price.
            for cut in saleCuts{ 
                totalPrice.append(NFTStorefront.SaleCut(receiver: cut.receiver, amount: cut.amount * (UFix64(1.0) - MarketplaceFees.getFeeParameters().rate - MarketplaceFees.getFeeParameters().rate), description: cut.description))
            }
            ...
            self.details = ListingDetails(id: id, nftID: nftID, nftType: nftType, paymentVaultType: paymentVaultType, saleCuts: totalPrice, expiry: expiry)
        }
    }
    ...
    pub resource Storefront: StorefrontManager, StorefrontPublic{ 
        ...
        ///
        pub fun createListing(nftProviderCapability: Capability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>, nftID: UInt64, nftType: Type, paymentVaultType: Type, saleCuts: [SaleCut], expiry: UInt64): UInt64{ 
            ...
            var listing <- create Listing(id: currentIndex, nftProviderCapability: nftProviderCapability, nftID: nftID, nftType: nftType, paymentVaultType: paymentVaultType, saleCuts: saleCuts, expiry: expiry)
            emit ListingAvailable(id: currentIndex, address: self.owner?.address!, nftID: nftID, nftType: nftType, paymentVaultType: paymentVaultType, price: listing.getDetails().price, expiry: expiry)
        ...
        ///
        pub fun removeListing(id: UInt64){ 
            let listing <- self.listings.remove(key: id) ?? panic("missing Listing")
        ...
        pub fun cleanupPurchasedListings(id: UInt64){ 
            pre{ 
                ...
                (self.borrowListing(id: id)!).getDetails().purchased == true:
