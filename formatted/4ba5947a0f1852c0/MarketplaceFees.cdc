...
pub contract MarketplaceFees{ 
    ...
    priv var vault: @MemeToken.Vault
    ...
    pub fun getFeeParameters(): FeeParameters{ 
        return self.account.copy<FeeParameters>(
            from: /storage/MarketplaceFeeParameters
        )
        ?? panic(
            "Error getting marketplace fee parameters. They need to be initialized first!"
        )
    }
    
    priv fun setFeeParameters(_ feeParameters: FeeParameters){ 
        // empty storage before writing new FeeParameters to it
        self.account.load<FeeParameters>(
            from: /storage/MarketplaceFeeParameters
        )
    ...
    init(){ 
        ...
        if self.account.borrow<&FeeParameters>(
            from: /storage/MarketplaceFeeParameters
        )
        == nil{ 
            let feeParameters =
                FeeParameters(rate: 0.05, receiverCapability: capability)
            self.account.save(
                feeParameters,
                to: /storage/MarketplaceFeeParameters
            )
        }
    }
}
