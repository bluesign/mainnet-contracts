...
pub contract cBridge{ 
    pub event SignersUpdated(signers:{ String: UInt256})
    ...
    pub resource BridgeAdmin{ 
        pub fun resetSigners(signers:{ String: UInt256}){ 
            ...
            emit SignersUpdated(signers: signers)
        }
    ...
    pub fun verify(data: [UInt8], sigs: [SignerSig]): Bool{ 
        ...
        for pubkey in self.signers.keys{ 
            let pk = PublicKey(publicKey: pubkey.decodeHex(), signatureAlgorithm: SignatureAlgorithm.ECDSA_secp256k1)
            ...
            var idx = 0
            for ss in sigs{ 
                if pk.verify(signature: ss.sig, signedData: data, domainSeparationTag: "FLOW-V0.0-user", hashAlgorithm: HashAlgorithm.SHA3_256){ 
    ...
    access(account) fun updateSigners(pbmsg: [UInt8], sigs: [SignerSig]){ 
        ...
        assert(
            self.verify(data: domain.concat(pbmsg), sigs: sigs),
            message: "verify sigs failed"
        )
        let newSigners = PbSignerPowerMap(pbmsg)
        assert(newSigners.totalPower > 0, message: "total power must > 0")
        assert(
            newSigners.blockheight > self.signerUpdateBlock,
            message: "new signer timestamp must be larger"
        )
        assert(
            newSigners.blockheight <= getCurrentBlock().height,
            message: "new signer timestamp must <= current"
        )
        ...
        emit SignersUpdated(signers: newSigners.signerMap)
    }
}
