...
pub contract Planarias{ 
    ...
    pub resource Planaria{ 
        ...
        pub fun reproduceSexually(): @Planaria{ 
            ...
            let fatherName =
                fatherNames[Int(unsafeRandom() % UInt64(fatherNames.length))]!
    ...
    pub resource Habitat{ 
        ...
        pub fun out(name: UInt64): @Planaria{ 
            let planaria <-
                self.planarias.remove(key: name) ?? panic("Missing Planaria")
    ...
    init(){ 
        ...
        self.account.link<&Planarias.Habitat>(
            /public/PlanariasHabitat,
            target: /storage/PlanariasHabitat
        )
    }
}
