...
pub contract GooberXContract: NonFungibleToken{ 
    ...
    priv var gooberRegister: [UInt64]
    
    // Contains all minted goobers including the payload
    priv var mintedGoobers: [GooberStruct]
    
    // Index containing the reference between GooberID and the GooberStruct
    priv var mintedGoobersIndex:{ UInt64: Int}
    
    // Pool of mintable goobers
    priv var gooberPool: [GooberStruct]
    
    // Giveaways
    priv var giveaways:{ String: GooberStruct}
    ...
    pub struct GooberStruct{ 
        ...
        //
        init(gooberID: UInt64, uri: String, metadata:{ String: AnyStruct}, price: UFix64){ 
    ...
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver{ 
        ...
        init(goober: GooberStruct){ 
            ...
            self.data = GooberStruct(gooberID: gooberID, uri: goober.uri, metadata: goober.metadata, price: goober.price)
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                case Type<MetadataViews.Display>():
                    ...
                    return MetadataViews.Display(name: name, description: "Party Goob #".concat(self.id.toString()), thumbnail: MetadataViews.IPFSFile(cid: self.data.uri.slice(from: "ipfs://".length, upTo: self.data.uri.length), path: nil))
                ...
                case Type<MetadataViews.Royalties>():
                    ...
                    return MetadataViews.Royalties([MetadataViews.Royalty(receiver: cap, cut: 0.06, description: "Party Mansion")])
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: GooberXContract.CollectionStoragePath, publicPath: GooberXContract.CollectionPublicPath, providerPath: /private/GooberzPartyFolksCollection, publicCollection: Type<&GooberXContract.Collection{GooberXContract.GooberCollectionPublic, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection}>(), publicLinkedType: Type<&GooberXContract.Collection{GooberXContract.GooberCollectionPublic, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&GooberXContract.Collection{GooberXContract.GooberCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
                            return <-GooberXContract.createEmptyCollection()
                        })
                case Type<MetadataViews.NFTCollectionDisplay>():
                    return MetadataViews.NFTCollectionDisplay(name: "Party Mansion Gooberz", description: "The Party Gooberz is a fun and comical art collection of 3550 collectibles living on the Flow Blockchain. As one of the first PFP collectibles on Flow, we enjoy bringing the party and hanging with friends. So grab a drink, pump up the music, and get ready to party because The Party Goobz are ready to go within Party Mansion!", externalURL: MetadataViews.ExternalURL(url: "https://partymansion.io/"), squareImage: MetadataViews.Media(file: MetadataViews.IPFSFile(cid: "QmeiwpEXCidsPae3ZPpSJTKVit1R2LHiF4cw5pvmMPRC4x", path: nil), mediaType: "image/jpg"), bannerImage: MetadataViews.Media(file: MetadataViews.IPFSFile(cid: "QmdU1j5nqeQBmVWZZDhz23z6mMPwMp5i2Ka2sBpMhYggPT", path: nil), mediaType: "image/jpg"), socials:{ "twitter": MetadataViews.ExternalURL(url: "https://mobile.twitter.com/the_goobz_nft"), "discord": MetadataViews.ExternalURL(url: "http://discord.gg/zJRNqKuDQH")})
    ...
    pub resource Admin{ 
        ...
        //
        pub fun addGooberToPool(uri: String, metadata:{ String: AnyStruct}, address: Address, price: UFix64){ 
            pre{ 
                uri.length > 0:
                    "Could not create Goober: uri is required."
            }
            GooberXContract.gooberPool.append(GooberStruct(gooberID: 0 as UInt64, uri: uri, metadata: metadata, price: price))
        }
        
        // Replaces Goobers in global pool
        // This is used to exchange existing Goober Pool 
        //
        pub fun replaceGooberInPool(uri: String, metadata:{ String: AnyStruct}, address: Address, price: UFix64, index: Int){ 
            pre{ 
                uri.length > 0:
                    "Could not create Goober: uri is required."
                index >= 0:
                    "Index is out of bounds"
                GooberXContract.gooberPool.length > index:
                    "Index is out of bounds."
            }
            GooberXContract.gooberPool[index] = GooberStruct(gooberID: 0 as UInt64, uri: uri, metadata: metadata, price: price)
        }
        ...
        pub fun removeGooberFromPool(index: UInt64){ 
            pre{ 
                GooberXContract.gooberPool[index] != nil:
                    "Could not delete goober from pool: goober does not exist."
            }
        ...
        pub fun adminMintNFT(recipient: &{NonFungibleToken.CollectionPublic}){ 
            pre{ 
                GooberXContract.gooberPool.length > 0:
                    "GooberPool doesnt contain any goober to mint"
            }
        ...
        //
        pub fun adminMintAirDropNFT(uri: String, metadata:{ String: AnyStruct}, address: Address, price: UFix64, recipient: &{NonFungibleToken.CollectionPublic}){ 
            pre{ 
                uri.length > 0:
                    "Could not create Goober: uri is required."
            }
            // GooberStruct initializing
            let goober: GooberStruct = GooberStruct(gooberID: 0 as UInt64, uri: uri, metadata: metadata, price: price)
        ...
        pub fun createGiveaway(giveawayKey: String){ 
            pre{ 
                GooberXContract.gooberPool.length > 0:
                    "GooberPool doesnt contain any goober to add as giveaway"
            }
    ...
    pub resource Collection: GooberCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection{ 
        ...
        pub fun deposit(token: @NonFungibleToken.NFT){ 
            ...
            GooberXContract.mintedGoobers[index!] = GooberStruct(gooberID: tmpGoober.gooberID, uri: tmpGoober.uri, metadata: tmpGoober.metadata, price: tmpGoober.price)
        }
    ...
    pub fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, address: Address, paymentVault: @FungibleToken.Vault){ 
        pre{ 
            GooberXContract.gooberPool.length > 0:
                "GooberPool doesnt contain any goober to mint"
            paymentVault.balance >= GooberXContract.gooberPool[GooberXContract.firstGooberInPoolIndex].price:
                "Could not mint goober: payment balance insufficient."
    ...
    pub fun nameYourGoober(newName: String, gooberID: UInt64, collection: &GooberXContract.Collection, paymentVault: @FungibleToken.Vault){ 
        pre{ 
            ...
            paymentVault.isInstance(Type<@FUSD.Vault>()):
                "payment vault is not requested fungible token"
            newName.length <= 20:
                "Cannot give the Goober a name which is longer than 20 characters"
        }
