...
pub contract Bakesale_Beer_NFT: NonFungibleToken{ 
    ...
    pub resource Admin{ 
        ...
        pub fun addNftSet(setId: UInt32, maxEditions: UInt32, ipfsMetadataHash: String, ipfsMediaHash: String){ 
            pre{ 
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &Bakesale_Beer_NFT.NFT?{ 
        let collection = getAccount(from).getCapability(Bakesale_Beer_NFT.CollectionPublicPath).borrow<&{Bakesale_Beer_NFT.Bakesale_Beer_NFTCollectionPublic}>() ?? panic("Couldn't get collection")
