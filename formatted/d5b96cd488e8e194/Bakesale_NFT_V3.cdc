...
pub contract Bakesale_NFT_V3: NonFungibleToken{ 
    ...
    pub resource Admin{ 
        ...
        pub fun batchMintNFT(recipient: &{NonFungibleToken.CollectionPublic}, setId: UInt32, quantity: UInt64){ 
            pre{ 
                quantity > 0:
                    "Quantity must be > 0"
            }
        ...
        pub fun addNftSet(setId: UInt32, maxEditions: UInt32, ipfsMetadataHash: String, ipfsMediaHash: String, quantityClaimLimit: UInt32, timeWaitedClaimLimit: UFix64){ 
            pre{ 
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &Bakesale_NFT_V3.NFT?{ 
        let collection = getAccount(from).getCapability(Bakesale_NFT_V3.CollectionPublicPath).borrow<&{Bakesale_NFT_V3.Bakesale_NFT_V3CollectionPublic}>() ?? panic("Couldn't get collection")
