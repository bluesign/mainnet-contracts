...
pub contract TrmAssetV2_1: NonFungibleToken{ 
    ...
    pub resource Admin{ 
        pub fun withdrawAsset(assetCollectionAddress: Address, id: UInt64): @NonFungibleToken.NFT{ 
            let assetCollectionCapability = getAccount(assetCollectionAddress).getCapability<&TrmAssetV2_1.Collection{TrmAssetV2_1.CollectionPublic}>(TrmAssetV2_1.collectionPublicPath).borrow() ?? panic("Could not borrow asset collection capability from provided asset collection address")
