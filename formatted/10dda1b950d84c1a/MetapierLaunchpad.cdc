...
pub contract MetapierLaunchpad{ 
    ...
    pub event UserClaimedLaunchToken(
        poolId: String,
        address: Address,
        amount: UFix64
    )
    ...
    pub resource interface PublicLaunchPool{ 
        ...
        pub fun depositFunds(
            privatePass: &MetapierLaunchpadPass.NFT,
            amount: UFix64
        ){ 
            pre{ 
                amount > 0.0:
    ...
    pub resource LaunchPool: PublicLaunchPool{ 
        ...
        // ParticipantInfo
        priv var participations:{ Address: ParticipantInfo}
        ...
        priv fun getPublicPassCollection(address: Address): &MetapierLaunchpadPass.Collection{MetapierLaunchpadPass.CollectionPublic}{ 
            return getAccount(address).getCapability<&MetapierLaunchpadPass.Collection{MetapierLaunchpadPass.CollectionPublic}>(MetapierLaunchpadPass.CollectionPublicPath).borrow()!
        }
        ...
        pub fun ownerWithdrawFunds(ownerPass: &MetapierLaunchpadOwnerPass.NFT, amount: UFix64): @FungibleToken.Vault{ 
    ...
    pub fun getPublicLaunchPoolById(poolId: String): &{
        MetapierLaunchpad.PublicLaunchPool
    }?{ 
    ...
    pub resource Admin{ 
        ...
        pub fun updateTimeline(
            ...
            let poolRef =
                &MetapierLaunchpad.pools[poolId]
                as
                &MetapierLaunchpad.LaunchPool
            poolRef.fundingStartTime = fundingStartTime
                ?? poolRef.fundingStartTime
            poolRef.fundingEndTime = fundingEndTime ?? poolRef.fundingEndTime
            poolRef.claimingStartTime = claimingStartTime
                ?? poolRef.claimingStartTime
