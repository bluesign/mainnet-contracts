...
pub contract FGameLottery{ 
    ...
    pub event TicketPowerupChanged(
        poolAddr: Address,
        lotteryId: UInt64,
        address: Address,
        ticketId: UInt64,
    ...
    pub event TicketPrizeDisbursed(
        poolAddr: Address,
        lotteryId: UInt64,
        address: Address,
        ticketId: UInt64,
    ...
    pub event LotteryParticipantsVerified(
        poolAddr: Address,
        lotteryId: UInt64,
        participants: [
            Address
    ...
    pub event LotteryJackpotWinnerUpdated(
        poolAddr: Address,
        lotteryId: UInt64,
        winner: Address
    )
    ...
    /// Event emitted when a lottery is verified with jackpot updated
    pub event LotteryJackpotFinialized(
        poolAddr: Address,
        lotteryId: UInt64,
        jackpotAmount: UFix64,
        nonJackpotTotal: UFix64,
    ...
    pub event LotteryPrizesDisbursed(poolAddr: Address, lotteryId: UInt64)
    
    /// Event emitted when a lottery jackpot is donated
    pub event LotteryJackpotDonated(poolAddr: Address, donationAmount: UFix64)
    
    /* --- Variable, Enums and Structs --- */    pub let userCollectionStoragePath: StoragePath
    
    pub let userCollectionPublicPath: PublicPath
    
    pub let lotteryPoolStoragePath: StoragePath
    
    pub let lotteryPoolPublicPath: PublicPath
    
    pub var MAX_WHITE_NUMBER: UInt8
    
    pub var MAX_RED_NUMBER: UInt8
    
    /* --- Interfaces & Resources --- *//// Struct for the ticket number
    ...
    ///
    access(contract) fun createRandomTicketNumber(): TicketNumber{ 
    ...
    pub enum TicketStatus: UInt8{ 
        ...
        pub case WIN_DISBURSED
    }
    
    /// Enum for the prize rank
    ///
    pub enum PrizeRank: UInt8{ 
        pub case JACKPOT
        
        pub // 100% Jackpot Pool + Math.min(50%, Remaining) of Current Pool case SECOND
        
        pub case // 50000x Ticket Price THIRD
        
        pub case FOURTH // 5000x Ticket Price
        
        pub case FIFTH
        
        pub // 25x Ticket Price case SIXTH // 4x Ticket Price // 2x Ticket Price
    }
    
    /// Ticket entry resource interface
    ///
    pub resource interface TicketEntryPublic{ 
        pub            // variables
            let pool: Address
        
        pub let lotteryId: UInt64
        
        pub let numbers: TicketNumber
        
        pub let boughtAt: UFix64
        
        // view functions
        pub fun getStatus(): TicketStatus{} 
        
        pub fun getTicketId(): UInt64{} 
        
        pub fun getTicketOwner(): Address{} 
        
        pub fun getNumbers(): [UInt8; 6]{} 
        
        pub fun getPowerup(): UFix64{} 
        
        pub fun getWinPrizeRank(): PrizeRank?{} 
        
        pub fun getEstimatedPrizeAmount(): UFix64?{} 
        
        // borrow methods
        pub fun borrowLottery(): &Lottery{LotteryPublic}{} 
    ...
    ///
    pub resource TicketEntry: TicketEntryPublic{ 
        /// Lottery Pool Address for the ticket
        pub let pool: Address
        ...
        priv var powerup: UFix64
        
        /// Ticket status
        priv var status: TicketStatus
        
        /// Winner prize rank
        priv var winPrizeRank: PrizeRank?
        ...
        /** Update Ticket Data */        access(contract) fun setPowerup(powerup: UFix64){ 
            pre{ 
                powerup >= 1.0:
                    "Powerup must be greater than 0"
                powerup <= 10.0:
        ...
        /** --- Internal Methods --- */        priv fun _borrowLotteryPool(): &LotteryPool{LotteryPoolPublic, FixesHeartbeat.IHeartbeatHook}{ 
            return FGameLottery.borrowLotteryPool(self.pool) ?? panic("Lottery pool not found")
        }
        
        priv fun _setStatus(toStatus: TicketStatus){ 
    ...
    pub resource interface TicketCollectionPublic{ 
        ...
        pub fun getTicketAmount(): Int{} 
        
        pub fun borrowTicket(ticketId: UInt64): &TicketEntry{
            TicketEntryPublic
        }?{} 
        
        // --- write methods ---
        access(contract) fun addTicket(_ ticket: @TicketEntry){} 
    }
    
    /// User's ticket collection
    ///
    pub resource TicketCollection: TicketCollectionPublic{ 
        priv let tickets: @{UInt64: TicketEntry}
        
        priv let dscSortedIDs: [UInt64]
        ...
        /** ---- Public Methods ---- *//// Get the ticket IDs
        ///
        ...
        ///
        pub fun borrowTicket(ticketId: UInt64): &TicketEntry{TicketEntryPublic}?{ 
            return &self.tickets[ticketId] as &TicketEntry{TicketEntryPublic}?
        }
        
        /** ---- Private Methods ---- *//// Add a new ticket to the collection
        ///
        access(contract) fun addTicket(_ ticket: @TicketEntry){ 
            pre{ 
                self.owner != nil:
        ...
        /** --- Internal Methods --- *//// Borrow a ticket reference
        ///
        priv fun borrowTicketRef(ticketId: UInt64): &TicketEntry{ 
    ...
    pub struct TicketIdentifier{ 
        ...
        ///
        pub fun borrowTicket(): &TicketEntry{TicketEntryPublic}?{ 
    ...
    ///
    pub enum LotteryStatus: UInt8{ 
        pub case ACTIVE
        
        pub case READY_TO_DRAW
        
        pub case DRAWN
        
        pub case DRAWN_AND_VERIFIED
    }
    ...
    pub struct LotteryResult{ 
        ...
        pub var verifyingProgress: UFix64
        
        pub var disbursingProgress: UFix64
        
        pub let winners:{ Address: UInt64}
        
        pub var nonJackpotTotal: UFix64
        
        pub var nonJackpotDowngradeRatio: UFix64
        
        pub let nonJackpotWinners:{ UInt8: UInt64}
        
        pub var jackpotAmount: UFix64
        
        pub var jackpotWinners: [Address]?
        ...
        /// Add a winner
        ///
        access(contract) fun addWinner(_ address: Address){ 
            self.winners[address] = (self.winners[address] ?? 0) + 1
        }
        
        /// Increment the non-jackpot total
        ///
        access(contract) fun incrementNonJackpotTotal(
            _ rank: PrizeRank,
            _ amount: UFix64
        ){ 
            self.nonJackpotWinners[rank.rawValue] = (
                    self.nonJackpotWinners[rank.rawValue] ?? 0
                )
                + 1
        ...
        ///
        access(contract) fun setNonJackpotDowngradeRatio(_ ratio: UFix64){ 
        ...
        ///
        access(contract) fun updateJackpot(_ amount: UFix64){ 
            self.jackpotAmount = amount
        }
        
        /// Add a jackpot winner
        ///
        access(contract) fun addJackpotWinner(_ address: Address){ 
        ...
        ///
        access(contract) fun setVerifyingProgress(_ progress: UFix64){ 
            if progress >= 0.0 && progress <= 1.0{ 
                self.verifyingProgress = progress
            }
        }
        
        access(contract) fun setDisbursingProgress(_ progress: UFix64){ 
            if progress >= 0.0 && progress <= 1.0{ 
    ...
    ///
    pub struct LotteryBasicInfo{ 
        pub let epochIndex: UInt64
        
        pub let epochStartAt: UFix64
        
        pub let currentPool: UFix64
        
        pub let participantsAmount: UInt64
        
        pub let status: LotteryStatus
        
        pub let disbursing: Bool
    ...
    pub resource interface LotteryPublic{ 
        pub            /// Lottery info - public view
            fun getInfo(): LotteryBasicInfo{} 
        
        /// Lottery status
        pub fun getStatus(): LotteryStatus{} 
        
        /// Lotter result
        pub fun getResult(): LotteryResult?{} 
        
        /// Return the participant addresses
        pub fun getParticipants(): [Address]{} 
        
        /// Return the participant amount
        pub fun getParticipantAmount(): UInt64{} 
        
        /// Get the total bought balance
        pub fun getCurrentLotteryBalance(): UFix64{} 
        
        /// Check if the lottery is disbursing prizes
        pub fun isDisbursing(): Bool{} 
    }
    ...
    pub resource Lottery: LotteryPublic{ 
        ...
        priv let current: @FRC20FTShared.Change
        
        /// Participants tickets: [Address: [TicketID]]
        priv let participants:{ Address: [UInt64]}
        
        /// Lottery final status
        priv var drawnResult: LotteryResult?
        
        /// Lottery draw checker queue
        priv var checkingQueue: [Address]?
        
        /// Lottery winners
        priv let disbursingQueque: [TicketIdentifier]
        ...
        /** ---- Public Methods ---- *//// Lottery info - public view
        ///
        pub fun getInfo(): LotteryBasicInfo{ 
        ...
        /** ---- Contract level Methods ----- *//// Create a new ticket and add it to user's collection
        ///
        access(contract) fun buyNewTicket(payment: @FRC20FTShared.Change, recipient: Capability<&TicketCollection{TicketCollectionPublic}>, powerup: UFix64?) // default is 1.0, you can increase the powerup to increase the winning amount: UInt64{ 
            pre{ 
        ...
        ///
        access(contract) fun drawLottery(){ 
        ...
        ///
        access(contract) fun verifyParticipantsTickets(_ maxEntries: Int){ 
        ...
        ///
        access(contract) fun disbursePrizes(_ maxEntries: Int){ 
        ...
        /** ---- Internal Methods ----- *//// Disburse the prize to the winners
        ///
        priv fun _disbursePrize(ticket: &TicketEntry{TicketEntryPublic}){ 
        ...
        priv fun borrowCurrentLotteryChange(): &FRC20FTShared.Change{ 
            return &self.current as &FRC20FTShared.Change
        }
        
        priv fun borrowLotteryPool(): &LotteryPool{ 
    ...
    pub resource interface LotteryPoolPublic{ 
        pub            // --- read methods ---
            fun getName(): String{} 
        
        pub fun getAddress(): Address{} 
        
        pub fun getCurrentEpochIndex(): UInt64{} 
        
        pub fun getEpochInterval(): UFix64{} 
        
        pub fun getLotteryToken(): String{} 
        
        pub fun getTicketPrice(): UFix64{} 
        
        pub fun getJackpotPoolBalance(): UFix64{} 
        
        pub fun getServiceFee(): UFix64{} 
        
        pub fun isEpochAutoStart(): Bool{} 
        
        pub fun getWinnerPrizeByRank(_ rank: PrizeRank): UFix64{} 
        
        // --- read methods: default implement ---
        
        /// Check if the current lottery is active
        pub fun isCurrentLotteryActive(): Bool{ 
        ...
        pub fun isCurrentLotteryReadyToDraw(): Bool{ 
            let currentLotteryRef = self.borrowCurrentLottery()
            let status = currentLotteryRef?.getStatus()
            return currentLotteryRef != nil
            && status == LotteryStatus.READY_TO_DRAW
        }
        
        /// Check if the current lottery is finished
        pub fun isCurrentLotteryFinished(): Bool{ 
            let currentLotteryRef = self.borrowCurrentLottery()
            let status = currentLotteryRef?.getStatus()
        ...
        pub fun donateToJackpot(payment: @FRC20FTShared.Change){} 
        
        // --- borrow methods ---
        
        pub fun borrowLottery(_ epochIndex: UInt64): &Lottery{LotteryPublic}?{} 
        
        pub fun borrowCurrentLottery(): &Lottery{LotteryPublic}?{} 
        
        // Internal usage
        access(contract) fun borrowSelf(): &LotteryPool{} 
    }
    
    pub resource interface LotteryPoolAdmin{ 
        pub            // --- write methods ---
            fun startNewEpoch(){} 
    }
    ...
    pub resource LotteryPool:
        ...
        priv let initEpochInterval: UFix64
        
        priv let initTicketPrice: UFix64
        
        // Lottery pool variables
        priv let jackpotPool: @FRC20FTShared.Change
        
        priv let lotteries: @{UInt64: Lottery}
        
        priv var currentEpochIndex: UInt64
        
        priv var finishedEpoches: [UInt64]
        
        priv var lastSealedEpochIndex: UInt64?
        ...
        /** ---- Public Methods ---- */        pub fun getName(): String{ 
            return self.name
        }
        ...
        pub fun getEpochInterval(): UFix64{ 
            ...
            let interval =
                store.getByEnum(
                    FRC20FTShared.ConfigType.GameLotteryEpochInterval
                )
                as!
                UFix64?
        ...
        pub fun getTicketPrice(): UFix64{ 
            let store = self.borrowConfigStore()
            let price =
                store.getByEnum(FRC20FTShared.ConfigType.GameLotteryTicketPrice)
                as!
                UFix64?
        ...
        pub fun getServiceFee(): UFix64{ 
            let store = self.borrowConfigStore()
            let fee =
                store.getByEnum(FRC20FTShared.ConfigType.GameLotteryServiceFee)
                as!
                UFix64?
            return fee ?? 0.16
        }
        ...
        pub fun isEpochAutoStart(): Bool{ 
            let store = self.borrowConfigStore()
            let autoStart =
                store.getByEnum(FRC20FTShared.ConfigType.GameLotteryAutoStart)
                as!
                Bool?
        ...
        /** ---- Public Methods: write ----- *//// Buy lottery tickets
        pub fun buyTickets(
            payment: @FRC20FTShared.Change,
            amount: UInt64,
            powerup: UFix64?,
            ...
            let lotteryRef =
                self.borrowLotteryRef(self.currentEpochIndex)
        ...
        /** ---- Admin Methods ----- *//// Start a new epoch
        ///
        ...
        /** ---- Heartbeat Implementation Methods ----- *//// The methods that is invoked when the heartbeat is executed
        ...
        access(contract) fun borrowJackpotPool(): &FRC20FTShared.Change{ 
            return &self.jackpotPool as &FRC20FTShared.Change
        }
        
        priv fun borrowConfigStore(): &FRC20FTShared.SharedStore{
            FRC20FTShared.SharedStorePublic
        }{ 
            return FRC20FTShared.borrowStoreRef((self.owner!).address)
            ?? panic("Config store not found")
        }
        
        priv fun borrowLotteryRef(_ epochIndex: UInt64): &Lottery?{ 
            return &self.lotteries[epochIndex] as &Lottery?
        }
    }
    
    /* --- Account Access Methods --- *//// Create a new lottery pool
    ///
    access(account) fun createLotteryPool(
        name: String,
        rewardTick: String,
        ticketPrice: UFix64,
        epochInterval: UFix64
    ...
    /* --- Public methods  --- *//// Create a ticket collection
    ///
    pub fun createTicketCollection(): @TicketCollection{ 
        return <-create TicketCollection()
    }
    
    /// Get the user's ticket collection capability
    ///
    pub fun getUserTicketCollection(_ addr: Address): Capability<
        &TicketCollection{TicketCollectionPublic}
    >{ 
        return getAccount(addr).getCapability<
            &TicketCollection{TicketCollectionPublic}
        >(self.userCollectionPublicPath)
    }
    
    /// Borrow Lottery Pool
    ///
    pub fun borrowLotteryPool(_ addr: Address): &LotteryPool{
        LotteryPoolPublic,
        FixesHeartbeat.IHeartbeatHook
    }?{ 
        return getAccount(addr).getCapability<
            &LotteryPool{LotteryPoolPublic, FixesHeartbeat.IHeartbeatHook}
    ...
    init(){ 
        ...
        self.lotteryPoolStoragePath = StoragePath(
                identifier: identifier.concat("_LotteryPool")
            )!
        self.lotteryPoolPublicPath = PublicPath(
                identifier: identifier.concat("_LotteryPool")
            )!
