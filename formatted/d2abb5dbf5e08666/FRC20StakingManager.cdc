...
pub contract FRC20StakingManager{ 
    ...
    pub event StakingPoolRewardStrategyRegistered(
        tick: String,
        rewardTick: String,
        by: Address
    )
    ...
    pub event StakingPoolDonatedAsVesting(
        tick: String,
        rewardTick: String,
        amount: UFix64,
        by: Address,
    ...
    /* --- Variable, Enums and Structs --- */    pub let StakingAdminStoragePath: StoragePath
    
    pub let StakingAdminPublicPath: PublicPath
    
    pub let StakingControllerStoragePath: StoragePath
    
    /* --- Interfaces & Resources --- *//// Staking Admin Public Resource interface
    ///
    pub resource interface StakingAdminPublic{ 
        pub fun isWhitelisted(address: Address): Bool{} 
    }
    
    /// Staking Admin Resource, represents a staking admin and store in admin's account
    ///
    pub resource StakingAdmin: StakingAdminPublic{ 
    ...
    ///
    pub resource StakingController{ 
        pub            
        ...
        ///
        pub fun enableAndCreateFRC20Staking(
            tick: String,
        ...
        ///
        pub fun ensureStakingResourcesAvailable(tick: String){ 
            pre{ 
                FRC20StakingManager.isWhitelisted(self.getControllerAddress()):
                    "The controller is not whitelisted"
            }
            let isUpdated =
                FRC20StakingManager._ensureStakingResourcesAvailable(tick: tick)
            if isUpdated{ 
                // singleton resources
                let acctsPool = FRC20AccountsPool.borrowAccountsPool()
        ...
        ///
        pub fun donateAllSharedPoolFlowTokenToVesting(
            tick: String,
            ...
            let flowVaultRef =
                childAcctRef.borrow<&FlowToken.Vault>(
                    from: /storage/flowTokenVault
                )
                ?? panic("The flow vault is not found")
            let storageUsageBalance =
                UFix64(childAcctRef.storageUsed)
                / UFix64(childAcctRef.storageCapacity)
                * flowVaultRef.balance
            // Keep the 2x of the storage usage balance
            let availbleBalance =
                flowVaultRef.balance - storageUsageBalance * 2.0 - 0.01
        ...
        ///
        pub fun donateFlowTokenToVesting(
            tick: String,
            amount: UFix64,
            vestingBatchAmount: UInt32,
            vestingInterval: UFix64
        ...
        /// Donate FLOW in the child account to the vesting pool
        ///
        priv fun _donateTokenToVesting(
        ...
        ///
        pub fun registerRewardStrategy(stakeTick: String, rewardTick: String){ 
            // singleton resources
    ...
    /* --- Contract access methods  --- */    access(contract) fun _ensureStakingResourcesAvailable(tick: String): Bool{ 
        let acctsPool = FRC20AccountsPool.borrowAccountsPool()
        ...
        if let pool =
            childAcctRef.borrow<&FRC20Staking.Pool>(
                from: FRC20Staking.StakingPoolStoragePath
            ){ 
            assert(
                pool.tick == tick,
                message: "The staking pool tick is not the same as the requested"
        ...
        == nil{ 
            ...
            childAcctRef.save(
                <-hooks,
                to: FRC20FTShared.TransactionHookStoragePath
            )
            isUpdated = true || isUpdated
        }
        ...
        >(FRC20FTShared.TransactionHookPublicPath).borrow()
        == nil{ 
            // link the hooks to the public path
        ...
        if childAcctRef.borrow<&AnyResource>(
            from: FRC20StakingVesting.storagePath
        )
        == nil{ 
        ...
        if childAcctRef.getCapability<
            &FRC20StakingVesting.Vault{
                FRC20StakingVesting.VestingVaultPublic,
                FRC20FTShared.TransactionHook,
                FixesHeartbeat.IHeartbeatHook
            }
        >(FRC20StakingVesting.publicPath).borrow()
        == nil{ 
        ...
        == nil{ 
            let forwarder <-
                FRC20StakingForwarder.createNewForwarder(childAcctRef.address)
            childAcctRef.save(
                <-forwarder,
                to: FRC20StakingForwarder.StakingForwarderStoragePath
            )
            
            // link public interface
            childAcctRef.link<
                &FRC20StakingForwarder.Forwarder{
                    FRC20StakingForwarder.ForwarderPublic
                }
            >(
        ...
        if childAcctRef.getCapability(flowReceiverPath).check<
            &{FungibleToken.Receiver}
        >(){ 
            // link the forwarder to the public path
            childAcctRef.unlink(flowReceiverPath)
            // Link the new forwarding receiver capability
            childAcctRef.link<&{FungibleToken.Receiver}>(
                flowReceiverPath,
                target: FRC20StakingForwarder.StakingForwarderStoragePath
            )
            // link the FlowToken to the forwarder fallback path
            let fallbackPath =
                FRC20StakingForwarder.getFallbackFlowTokenPublicPath()
            childAcctRef.unlink(fallbackPath)
            childAcctRef.link<
                &FlowToken.Vault{FungibleToken.Receiver, FungibleToken.Balance}
            >(fallbackPath, target: /storage/flowTokenVault)
            isUpdated = true || isUpdated
        }
        return isUpdated
    }
    
    /** ---- Public Methods - Controller ---- *//// Check if the given address is whitelisted
    ///
    pub fun isWhitelisted(_ address: Address): Bool{ 
        ...
        let admin =
            self.account.getCapability<&StakingAdmin{StakingAdminPublic}>(
                self.StakingAdminPublicPath
            ).borrow()
    ...
    ///
    pub fun isEligibleForRegistering(stakeTick: String, addr: Address): Bool{ 
        // singleton resources
        let acctsPool = FRC20AccountsPool.borrowAccountsPool()
        ...
        let pool =
            FRC20Staking.borrowPool(poolAddress)
            ?? panic("The staking pool is not found")
        // Check if the controller is whitelisted or staked enough tokens
    ...
    ///
    pub fun createController(): @StakingController{ 
        return <-create StakingController()
    }
    
    /** ---- Public Methods - User ---- *//// Get the staking ticker name.
    ///
    pub fun getPlatformStakingTickerName(): String{ 
        let globalSharedStore = FRC20FTShared.borrowGlobalStoreRef()
        let stakingToken =
            globalSharedStore.getByEnum(
                FRC20FTShared.ConfigType.PlatofrmMarketplaceStakingToken
            )
            as!
            String?
    ...
    ///
    pub fun borrowPlatformStakingPool(): &FRC20Staking.Pool{
        FRC20Staking.PoolPublic
    }{ 
        // singleton resources
        let acctsPool = FRC20AccountsPool.borrowAccountsPool()
        // Get the staking pool address
        ...
        let poolAddress =
            ...
            ?? panic("The staking pool is not enabled")
        // borrow the staking pool
        return FRC20Staking.borrowPool(poolAddress)
        ?? panic("The staking pool is not found")
    }
    
    /// Stake tokens
    ///
    pub fun stake(ins: &Fixes.Inscription){ 
        // singleton resources
        let frc20Indexer = FRC20Indexer.getIndexer()
        let acctsPool = FRC20AccountsPool.borrowAccountsPool()
        ...
        // inscription data
        let meta =
            frc20Indexer.parseMetadata(&ins.getData() as &Fixes.InscriptionData)
        let op =
            meta["op"]?.toLower() ?? panic("The token operation is not found")
        assert(
            op == "withdraw",
            message: "The token operation should be withdraw."
        )
        let usage =
            meta["usage"]?.toLower() ?? panic("The token usage is not found")
        assert(
            usage == "staking",
            message: "The token usage should be staking."
        )
        let fromAddr =
            ins.owner?.address ?? panic("The inscription owner is not found")
        assert(
            FRC20Staking.borrowDelegator(fromAddr) != nil,
            message: "The inscription owner is not a delegator"
        )
        let tick =
            meta["tick"]?.toLower() ?? panic("The token tick is not found")
        
        /// Check if the staking is already enabled
        let stakingAddress =
            acctsPool.getFRC20StakingAddress(tick: tick)
            ?? panic("The staking pool is not enabled")
        let stakingPool =
            FRC20Staking.borrowPool(stakingAddress)
            ?? panic("The staking pool is not found")
    ...
    pub fun unstake(
        ...
        let semiNFTCol =
            semiNFTColCap.borrow()
            ?? panic("Could not borrow the semiNFT collection")
        let nft =
            semiNFTCol.borrowFRC20SemiNFT(id: nftId)
    ...
    ///
    pub fun claimUnlockedUnstakingChange(ins: &Fixes.Inscription){ 
        pre{ 
            ins.isExtractable():
        ...
        let stakingPool =
            FRC20Staking.borrowPool(poolAddress)
            ?? panic("The staking pool is not found")
        if let unstakedChange <-
            stakingPool.claimUnlockedUnstakingChange(delegator: fromAddr){ 
    ...
    ///
    pub fun donateToStakingPool(tick: String, ins: &Fixes.Inscription){ 
        // singleton resources
        let acctsPool = FRC20AccountsPool.borrowAccountsPool()
        ...
        emit StakingPoolDonated(
            ...
            ?? panic("The inscription owner is not found")
        )
    }
    
    /// Donate FRC20 to the vesting reward pool
    ///
    pub fun donateToVesting(
        tick: String,
    ...
    /// (Internal Method)
    ///
    access(account) fun donateToVestingFromChange(
    ...
    /// (Internal Method)
    ///
    access(contract) fun _withdrawDonateChange(
        tick: String,
        ins: &Fixes.Inscription
    ...
    init(){ 
        ...
        self.StakingAdminStoragePath = StoragePath(
                identifier: identifier.concat("_admin")
            )!
        self.StakingAdminPublicPath = PublicPath(
                identifier: identifier.concat("_admin")
            )!
        ...
        self.account.link<&StakingAdmin{StakingAdminPublic}>(
            self.StakingAdminPublicPath,
            target: self.StakingAdminStoragePath
        )
