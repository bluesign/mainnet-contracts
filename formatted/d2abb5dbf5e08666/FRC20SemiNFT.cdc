...
pub contract FRC20SemiNFT: NonFungibleToken, ViewResolver{ 
    /* --- Events --- */
    ...
    pub event Wrapped(id: UInt64, pool: Address, tick: String, balance: UFix64)
    
    /// The event that is emitted when an NFT is unwrapped
    pub event Unwrapped(id: UInt64, pool: Address, tick: String, balance: UFix64)
    
    /// The event that is emitted when an NFT is merged
    pub event Merged(id: UInt64, mergedId: UInt64, pool: Address, tick: String, mergedBalance: UFix64)
    
    /// The event that is emitted when an NFT is splitted
    pub event Split(id: UInt64, splittedId: UInt64, pool: Address, tick: String, splitBalance: UFix64)
    
    /// The event that is emitted when the claiming record is updated
    pub event ClaimingRecordUpdated(id: UInt64, tick: String, pool: Address, strategy: String, time: UInt64, globalYieldRate: UFix64, totalClaimedAmount: UFix64)
    
    /* --- Variable, Enums and Structs --- *//// Storage and Public Paths
    ...
    /* --- Interfaces & Resources --- *//// Reward Claiming Record Struct, stored in SemiNFT
    ///
    pub struct RewardClaimRecord{ 
        pub            // The pool address
        ...
        pub var lastClaimedTime: UInt64
        
        // The last global yield rate
        pub var lastGlobalYieldRate: UFix64
        
        // The total claimed amount by this record
        pub var totalClaimedAmount: UFix64
    ...
    pub resource interface IFRC20SemiNFT{ 
        ...
        pub fun getFromAddress(): Address{} 
        
        pub fun getBalance(): UFix64{} 
        
        pub fun getRewardStrategies(): [String]{} 
        
        pub fun getClaimingRecord(_ uniqueName: String): RewardClaimRecord?{} 
        
        pub fun buildUniqueName(_ addr: Address, _ strategy: String): String{} 
    }
    ...
    ///
    pub resource NFT: IFRC20SemiNFT, NonFungibleToken.INFT, MetadataViews.Resolver{ 
        /// The unique ID that each NFT has
        pub let id: UInt64
        
        /// Wrapped FRC20FTShared.Change
        priv var wrappedChange: @FRC20FTShared.Change
        
        /// Claiming Records for staked FRC20FTShared.Change
        /// Unique Name => Reward Claim Record
        priv let claimingRecords:{ String: RewardClaimRecord}
        ...
        /** ----- MetadataViews.Resolver ----- *//// Function that returns all the Metadata Views implemented by a Non Fungible Token
        ///
            ...
            if colViews.contains(view){ 
                ...
                switch view{ 
                    ...
                    case Type<MetadataViews.Royalties>():
                        ...
                        let flowCap = getAccount(deployerAddr).getCapability<&{FungibleToken.Receiver}>(/public/flowTokenReceiver)
        ...
        /** ----- Semi-NFT Methods ----- */        pub fun getOriginalTick(): String{ 
        ...
        // Merge the NFT
        //
        pub fun merge(_ other: @FRC20SemiNFT.NFT){ 
            pre{ 
        ...
        // Split the NFT
        pub fun split(_ percent: UFix64): @FRC20SemiNFT.NFT{ 
            pre{ 
                percent > 0.0:
        ...
        /** ---- Account level methods ---- *//// Get the unique name of the reward strategy
        ///
        pub fun buildUniqueName(_ addr: Address, _ strategy: String): String{ 
        ...
        ///
        access(account) fun onClaimingReward(poolAddress: Address, rewardTick: String, amount: UFix64, currentGlobalYieldRate: UFix64){ 
        ...
        /** Internal Method *//// Update the claiming record
        ///
        access(contract) fun _updateClaimingRecord(poolAddress: Address, rewardTick: String, currentGlobalYieldRate: UFix64, currentTime: UInt64?, amount: UFix64, isSubtract: Bool){ 
            pre{ 
                self.isStakedTick():
                    "The tick must be a staked \u{1d509}rc20 token"
            }
        ...
        ///
        access(contract) fun borrowChange(): &FRC20FTShared.Change{ 
        ...
        ///
        priv fun _borrowOrCreateClaimingRecord(poolAddress: Address, rewardTick: String): &RewardClaimRecord{ 
        ...
        ///
        priv fun _borrowClaimingRecord(_ uniqueName: String): &RewardClaimRecord?{ 
    ...
    ///
    pub resource interface FRC20SemiNFTCollectionPublic{ 
        ...
        /** ----- Specific Methods For SemiNFT ----- */        pub fun getIDsByTick(tick: String): [UInt64]{} 
        ...
        pub fun borrowFRC20SemiNFTPublic(id: UInt64): &FRC20SemiNFT.NFT{IFRC20SemiNFT, NonFungibleToken.INFT, MetadataViews.Resolver}?{ 
            post{ 
    ...
    ///
    pub resource interface FRC20SemiNFTBorrowable{ 
        pub            /** ----- Specific Methods For SemiNFT ----- */            fun getIDsByTick(tick: String): [UInt64]{} 
        
        pub fun borrowFRC20SemiNFT(id: UInt64): &FRC20SemiNFT.NFT?{ 
            post{ 
                result == nil || result?.id == id:
                    "Cannot borrow FRC20SemiNFT reference: the ID of the returned reference is incorrect"
            }
    ...
    ///
    pub resource Collection: FRC20SemiNFTCollectionPublic, FRC20SemiNFTBorrowable, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection{ 
        // dictionary of NFT conforming tokens
        // NFT is a resource type with an `UInt64` ID field
        pub var ownedNFTs: @{UInt64: NonFungibleToken.NFT}
        
        // Tick => NFT ID Array
        priv let tickIDsMapping:{ String: [UInt64]}
        ...
        /** ----- ViewResolver ----- *//// Gets a reference to the NFT only conforming to the `{MetadataViews.Resolver}`
        ...
        /** ------ Internal Methods ------ *//// Borrow the tick IDs mapping
        ///
        priv fun _borrowTickIDs(_ tick: String): &[UInt64]?{ 
            return &self.tickIDsMapping[tick] as &[UInt64]?
        }
        
        /// Borrow or create the tick IDs mapping
        ///
        priv fun _borrowOrCreateTickIDs(_ tick: String): &[UInt64]{ 
    ...
    ///
    access(account) fun wrap(recipient: &FRC20SemiNFT.Collection{NonFungibleToken.CollectionPublic}, change: @FRC20FTShared.Change, initialYieldRates:{ String: UFix64}): UInt64{ 
        pre{ 
            change.isBackedByVault() == false:
                "Cannot wrap a vault backed FRC20 change"
        }
    ...
    ///
    access(account) fun unwrapFRC20(nftToUnwrap: @FRC20SemiNFT.NFT): @FRC20FTShared.Change{ 
        pre{ 
    ...
    ///
    access(account) fun unwrapStakedFRC20(nftToUnwrap: @FRC20SemiNFT.NFT): @FRC20FTShared.Change{ 
        pre{ 
    ...
    ///
    access(contract) fun _unwrap(_ nftToUnwrap: @FRC20SemiNFT.NFT): @FRC20FTShared.Change{ 
    ...
    pub fun resolveView(_ view: Type): AnyStruct?{ 
        switch view{ 
            ...
            case Type<MetadataViews.NFTCollectionData>():
                return MetadataViews.NFTCollectionData(storagePath: FRC20SemiNFT.CollectionStoragePath, publicPath: FRC20SemiNFT.CollectionPublicPath, providerPath: FRC20SemiNFT.CollectionPrivatePath, publicCollection: Type<&FRC20SemiNFT.Collection{FRC20SemiNFT.FRC20SemiNFTCollectionPublic}>(), publicLinkedType: Type<&FRC20SemiNFT.Collection{FRC20SemiNFT.FRC20SemiNFTCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&FRC20SemiNFT.Collection{FRC20SemiNFT.FRC20SemiNFTCollectionPublic, FRC20SemiNFT.FRC20SemiNFTBorrowable, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
    ...
    init(){ 
        ...
        self.account.link<&FRC20SemiNFT.Collection{NonFungibleToken.CollectionPublic, FRC20SemiNFT.FRC20SemiNFTCollectionPublic, MetadataViews.ResolverCollection}>(self.CollectionPublicPath, target: self.CollectionStoragePath)
