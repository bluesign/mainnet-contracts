...
pub contract FGameLotteryFactory{ 
    pub        /* --- Public Methods - Controller --- */        fun getFIXESLotteryPoolName(): String{ 
        return "FIXES_BASIS_LOTTERY_POOL"
    }
    
    pub fun getFIXESMintingLotteryPoolName(): String{ 
    ...
    ///
    pub fun initializeFIXESLotteryPool(
        ...
        self._initializeLotteryPool(
            ...
            rewardTick: "fixes",
            ticketPrice: FixesInscriptionFactory
                .estimateLotteryFIXESTicketsCost(1, nil),
    ...
    ///
    pub fun initializeFIXESMintingLotteryPool(
        _ controller: &FGameLotteryRegistry.RegistryController,
        newAccount: Capability<&AuthAccount>
    ){ 
        // initialize with 3 days
        ...
        let fixesMintingStr =
            FixesInscriptionFactory.buildMintFRC20(tick: "fixes", amt: 1000.0)
        var estimateMintingCost =
            FixesInscriptionFactory.estimateFrc20InsribeCost(fixesMintingStr)
    ...
    ///
    access(contract) fun _initializeLotteryPool(
        _ controller: &FGameLotteryRegistry.RegistryController,
        name: String,
        rewardTick: String,
        ticketPrice: UFix64,
    ...
    /* --- Public methods - User --- *//// PowerUp Types
    ///
    pub enum PowerUpType: UInt8{ 
        pub case x1
    ...
    ///
    pub fun getPowerUpValue(_ type: PowerUpType): UFix64{ 
        switch type{ 
            case PowerUpType.x2:
                return 2.0
            case PowerUpType.x3:
    ...
    ///
    pub fun isFIXESMintingAvailable(): Bool{ 
        // Singleton Resource
    ...
    ///
    pub fun getFIXESMintingLotteryFlowCost(
        ...
        let lotteryPoolAddr =
            registry.getLotteryPoolAddress(poolName)
            ?? panic("Lottery pool not found")
        let lotteryPoolRef =
            FGameLottery.borrowLotteryPool(lotteryPoolAddr)
            ?? panic("Lottery pool not found")
    ...
    ///
    pub fun buyFIXESMintingLottery(
        flowVault: @FlowToken.Vault,
        ticketAmount: UInt64,
        powerup: PowerUpType,
        withMinting: Bool,
        recipient: Capability<
            &FGameLottery.TicketCollection{FGameLottery.TicketCollectionPublic}
        >,
    ...
    ///
    pub fun getFIXESLotteryFlowCost(
        _ ticketAmount: UInt64,
        _ powerup: PowerUpType,
        _ recipient: Address
    ): UFix64{ 
        // check if the FLOW balance is sufficient
        let ticketPrice =
            FixesInscriptionFactory.estimateLotteryFIXESTicketsCost(1, nil)
        let powerupValue: UFix64 = self.getPowerUpValue(powerup)
        let ticketsPayment: UFix64 =
            ticketPrice * UFix64(ticketAmount) * powerupValue
        var requiredFlow: UFix64 =
            FixesInscriptionFactory.estimateFrc20InsribeCost(
                FixesInscriptionFactory.buildLotteryBuyFIXESTickets(
                    ticketAmount,
                    powerupValue
                )
            )
        ...
        // Mint $FIXES information
        let fixesMeta =
            frc20Indexer.getTokenMeta(tick: "fixes")
            ?? panic("FIXES Token meta not found")
        let fixesMintingStr =
            FixesInscriptionFactory.buildMintFRC20(
                tick: "fixes",
                amt: fixesMeta.limit
            )
    ...
    pub fun buyFIXESLottery(
        ...
        // check balance of the $FIXES token if it is sufficient to buy the tickets
        var balance =
            frc20Indexer.getBalance(tick: "fixes", addr: recipientAddr)
        // ensure the balance is sufficient
        if balance < ticketsPayment{ 
            // mint enough $FIXES to buy the tickets
            let totalMintAmount = UInt64((ticketsPayment - balance) / fixesMeta.limit) + 1
        ...
        let costReserve <-
            flowVault.withdraw(amount: estimatedBuyTicketsInsCost)
