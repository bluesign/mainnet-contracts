...
pub contract FRC20MarketManager{ 
    ...
    pub event              
              ...
              NewMarketEnabled(tick: String, address: Address, by: Address)
    
    /* --- Variable, Enums and Structs --- */    pub let FRC20MarketManagerStoragePath: StoragePath
    
    /* --- Interfaces & Resources --- *//// The resource manager for the FRC20MarketManager
    ///
    pub resource Manager: FRC20Marketplace.MarketManager{ 
        /// Update the admin whitelist
        ///
        pub fun updateAdminWhitelist(tick: String, address: Address, isWhitelisted: Bool){ 
        ...
        ///
        pub fun updateMarketplaceProperties(tick: String, _ props:{ FRC20FTShared.ConfigType: String}){ 
            let market = FRC20MarketManager.borrowMarket(tick) ?? panic("The market is not enabled")
            market.updateMarketplaceProperties(mananger: &self as &{FRC20Marketplace.MarketManager}, props)
        }
    }
    
    /* --- Contract access methods  --- */    access(contract) fun _ensureMarketResourcesAvailable(tick: String){ 
        ...
        if childAcctRef.borrow<&AnyResource>(
            from: FRC20TradingRecord.TradingRecordsStoragePath
        )
        == nil{ 
            let tradingRecords <- FRC20TradingRecord.createTradingRecords(tick)
            childAcctRef.save(
                <-tradingRecords,
                to: FRC20TradingRecord.TradingRecordsStoragePath
            )
            // link the trading records to the public path
            childAcctRef.unlink(FRC20TradingRecord.TradingRecordsPublicPath)
            childAcctRef.link<
                &FRC20TradingRecord.TradingRecords{
                    FRC20TradingRecord.TradingRecordsPublic,
                    FRC20TradingRecord.TradingStatusViewer,
                    FRC20FTShared.TransactionHook
                }
            >(
        ...
        let tradingRecordsCap =
            ...
            >(FRC20TradingRecord.TradingRecordsPublicPath)
        assert(
            tradingRecordsCap.check(),
            message: "The trading record hook is not valid"
        )
        // get the reference of the trading record hook
        let recordsRef =
            tradingRecordsCap.borrow()
    ...
    ///
    pub fun enableAndCreateFRC20Market(
        ...
        let tokenMeta =
            frc20Indexer.getTokenMeta(tick: tick)
    ...
    ///
    pub fun borrowMarket(_ tick: String): &FRC20Marketplace.Market{
        FRC20Marketplace.MarketPublic
    }?{ 
        let acctsPool = FRC20AccountsPool.borrowAccountsPool()
    ...
    init(){ 
        let identifier =
            "FRC20MarketManager_".concat(self.account.address.toString())
        self.FRC20MarketManagerStoragePath = StoragePath(
