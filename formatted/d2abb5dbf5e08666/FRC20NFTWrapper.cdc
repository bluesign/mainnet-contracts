...
pub contract FRC20NFTWrapper{ 
    ...
    pub event WrapperIndexerUpdatedNFTCollectionDisplay(
        nftType: Type,
        name: String,
        description: String
    )
    
    /* --- Variable, Enums and Structs --- */    pub let FRC20NFTWrapperStoragePath: StoragePath
    
    pub let FRC20NFTWrapperPublicPath: PublicPath
    
    pub let FRC20NFTWrapperIndexerStoragePath: StoragePath
    
    pub let FRC20NFTWrapperIndexerPublicPath: PublicPath
    
    /* --- Interfaces & Resources --- */    pub struct FRC20Strategy{ 
        pub let tick: String
        ...
        pub fun isUsedUp(): Bool{ 
            return self.usedAmt >= self.copies
        }
        
        access(contract) fun use(){ 
            pre{ 
    ...
    pub resource interface WrapperPublic{ 
        ...
        pub fun isFRC20NFTWrappered(nft: &NonFungibleToken.NFT): Bool{} 
        
        pub fun hasFRC20Strategy(_ collectionType: Type): Bool{} 
        
        pub fun getStrategiesAmount(all: Bool): UInt64{} 
        
        pub fun getStrategies(all: Bool): [FRC20Strategy]{} 
        
        pub fun isAuthorizedToRegister(addr: Address): Bool{} 
        
        pub fun getWhitelistedAddresses(): [Address]{} 
        
        pub fun getOption(key: String): AnyStruct?{} 
        
        pub fun getOptions():{ String: AnyStruct}{} 
        
        // write methods ----
        
        /// Donate to the internal flow vault
        pub fun donate(value: @FlowToken.Vault): Void{} 
        
        /// Register a new FRC20 strategy
        pub fun registerFRC20Strategy(
            type: Type,
            alloc: UFix64,
            copies: UInt64,
            cond: String?,
        ...
        pub fun wrap(
            recipient: &FixesWrappedNFT.Collection{
                FixesWrappedNFT.FixesWrappedNFTCollectionPublic,
    ...
    pub resource Wrapper: WrapperPublic{ 
        priv let strategies:{ Type: FRC20Strategy}
        
        priv let histories:{ Type:{ UInt64: Bool}}
        
        priv let internalFlowVault: @FlowToken.Vault
        
        priv let whitelist:{ Address: Bool}
        
        priv let options:{ String: AnyStruct}
        ...
        pub fun getInternalFlowBalance(): UFix64{ 
        ...
        /// Register a new FRC20 strategy
        pub fun registerFRC20Strategy(type: Type, alloc: UFix64, copies: UInt64, cond: String?, ins: &Fixes.Inscription){ 
            pre{ 
                ins.isExtractable():
        ...
        ///
        pub fun wrap(recipient: &FixesWrappedNFT.Collection{FixesWrappedNFT.FixesWrappedNFTCollectionPublic, NonFungibleToken.CollectionPublic}, nftToWrap: @NonFungibleToken.NFT): UInt64{ 
        ...
        pub fun updateOptions(key: String, value: AnyStruct){ 
        ...
        ///
        pub fun updateWhitelist(addr: Address, isAuthorized: Bool): Void{ 
    ...
    pub resource interface WrapperIndexerPublic{ 
        ...
        ///
        pub fun borrowWrapperPublic(addr: Address): &Wrapper{WrapperPublic}?{ 
            return FRC20NFTWrapper.borrowWrapperPublic(addr: addr)
        }
        
        /// Get the NFT collection display
        ///
        pub fun getNFTCollectionDisplay(
            nftType: Type
        ...
        pub fun registerWrapper(wrapper: &Wrapper){} 
    }
    ...
    pub resource WrapperIndexer: WrapperIndexerPublic{ 
        /// The event that is emitted when the contract is created
        priv let wrappers:{ Address: Bool}
        
        priv let displayHelper:{ Type: MetadataViews.NFTCollectionDisplay}
        ...
        /// Check if the wrapper is registered
        ///
        pub fun hasRegisteredWrapper(addr: Address): Bool{ 
            return self.wrappers[addr] != nil
        }
        
        /// Get all the wrappers
        ///
        pub fun getAllWrappers(_ includeNoStrategy: Bool, _ includeFinished: Bool): [Address]{ 
        ...
        ///
        pub fun getNFTCollectionDisplay(nftType: Type): MetadataViews.NFTCollectionDisplay{ 
        ...
        pub fun updateExtraNFTCollectionDisplay(nftType: Type, display: MetadataViews.NFTCollectionDisplay): Void{ 
            let collectionType = FRC20NFTWrapper.asCollectionType(nftType.identifier)
    ...
    /// Donate to the internal flow vault
    ///
    pub fun donate(addr: Address, _ value: @FlowToken.Vault): Void{ 
        let ref =
            self.borrowWrapperPublic(addr: addr)
    ...
    ///
    pub fun createNewWrapper(): @Wrapper{ 
        return <-create Wrapper()
    }
    
    /// Make a new NFT type
    ///
    pub fun asNFTType(_ identifier: String): Type{ 
    ...
    ///
    pub fun asCollectionType(_ identifier: String): Type{ 
        let ids = StringUtils.split(identifier, ".")
        assert(
            ids.length == 4,
            message: "Invalid NFT Collection type identifier!"
        )
    ...
    ///
    pub fun borrowWrapperPublic(addr: Address): &FRC20NFTWrapper.Wrapper{
        WrapperPublic
    }?{ 
        return getAccount(addr).getCapability<
            &FRC20NFTWrapper.Wrapper{WrapperPublic}
        >(self.FRC20NFTWrapperPublicPath).borrow()
    }
    
    /// Borrow the public interface to the Wrapper Indexer resource
    ///
    pub fun borrowWrapperIndexerPublic(): &FRC20NFTWrapper.WrapperIndexer{
        WrapperIndexerPublic
    }{ 
        return getAccount(self.account.address).getCapability<
            &FRC20NFTWrapper.WrapperIndexer{WrapperIndexerPublic}
        >(self.FRC20NFTWrapperIndexerPublicPath).borrow()
    ...
    pub fun getNFTCollectionDisplay(
        ...
        return self.borrowWrapperIndexerPublic().getNFTCollectionDisplay(
            nftType: nftType
        )
    }
    ...
    init(){ 
        let identifier =
            "FixesFRC20NFTWrapper_".concat(self.account.address.toString())
        ...
        self.account.link<
            &FRC20NFTWrapper.Wrapper{FRC20NFTWrapper.WrapperPublic}
        >(
        ...
        self.account.link<
            &FRC20NFTWrapper.WrapperIndexer{
                FRC20NFTWrapper.WrapperIndexerPublic
            }
        >(
        ...
        let indexerRef =
            self.account.borrow<&FRC20NFTWrapper.WrapperIndexer>(
        ...
        let wrapper =
            self.account.borrow<&Wrapper>(
                from: FRC20NFTWrapper.FRC20NFTWrapperStoragePath
            )
