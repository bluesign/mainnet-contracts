...
pub contract FRC20Marketplace{ 
    ...
    pub event              
              ...
              MarketCreated(tick: String, uuid: UInt64)
    
    /// Event emitted when a new listing is added
    pub event ListingAdded(
        tick: String,
        storefront: Address,
        listingId: UInt64,
        type: UInt8
    ...
    pub event MarketAdminWhitelistUpdated(
        tick: String,
        addr: Address,
        isWhitelisted: Bool
    )
    ...
    pub event MarketAdminPropertiesUpdated(
        tick: String,
        key: UInt8,
        value: String
    )
    
    /* --- Variable, Enums and Structs --- */    pub let FRC20MarketStoragePath: StoragePath
    
    pub let FRC20MarketPublicPath: PublicPath
    
    /* --- Interfaces & Resources --- *//// The Listing item information
    pub struct ListedItem{ 
        pub            // The combined uid for querying in the market
            let rankedId: String
        ...
        init(address: Address, listingID: UInt64){ 
            let storefront =
                ...
                ?? panic(
                    "no storefront found in address:".concat(address.toString())
                )
            ...
            let listingRef =
                ...
                ?? panic(
                    "no listing id found in storefront:".concat(
                        address.toString()
                    )
                )
        ...
        pub fun getDetails(): FRC20Storefront.ListingDetails?{ 
            let listingRef = self.borrowListing()
            return listingRef?.getDetails()
        }
        
        /// Borrow the listing resource
        pub fun borrowListing(): &FRC20Storefront.Listing{
            FRC20Storefront.ListingPublic
        }?{ 
        ...
        pub fun borrowStorefront(): &FRC20Storefront.Storefront{
            FRC20Storefront.StorefrontPublic
        }?{ 
    ...
    pub struct ItemIdentifier{ 
        pub let type: FRC20Storefront.ListingType
    ...
    pub resource interface ListingCollectionPublic{ 
        pub fun getListedIds(): [UInt64]{} 
        
        pub fun getListedItem(_ id: UInt64): ListedItem?{} 
    }
    ...
    pub resource ListingCollection: ListingCollectionPublic{ 
        ...
        access(contract) let listingIDItems:{ UInt64: ListedItem}
        ...
        access(contract) fun borrowListedItem(_ id: UInt64): &ListedItem?{ 
            return &self.listingIDItems[id] as &ListedItem?
        }
        
        access(contract) fun addListedItem(_ listedItem: ListedItem){ 
            self.listingIDItems[listedItem.id] = listedItem
        }
        
        access(contract) fun removeListedItem(_ id: UInt64){ 
    ...
    pub resource interface MarketPublic{ 
        pub            // ---- Public read methods ----
            fun getTickerName(): String{} 
        
        pub fun getSuperAdmin(): Address{} 
        
        pub fun getPriceRanks(type: FRC20Storefront.ListingType): [UInt64]{} 
        
        pub fun getListedIds(type: FRC20Storefront.ListingType, rank: UInt64): [
            UInt64
        ]{} 
        
        pub fun getListedItem(
            type: FRC20Storefront.ListingType,
            rank: UInt64,
            id: UInt64
        ): ListedItem?{} 
        ...
        access(account) fun updateMarketplaceProperties(
            mananger: &AnyResource{MarketManager},
            _ props:{ 
    ...
    pub resource Market: MarketPublic{ 
        ...
        priv let collections: @{FRC20Storefront.ListingType:{ UInt64: ListingCollection}}
        
        priv let sortedPriceRanks:{ FRC20Storefront.ListingType: [UInt64]}
        
        priv let adminWhitelist:{ Address: Bool}
        
        priv let accessWhitelist:{ Address: Bool}
        
        priv var listedItemAmount: UInt64
        ...
        /** ---- Public Methods ---- *//// The ticker name of the FRC20 market
        ///
        ...
        /// Get the listed ids
        ///
        pub fun getListedIds(type: FRC20Storefront.ListingType, rank: UInt64): [UInt64]{ 
        ...
        /// Get the listing item
        ///
        pub fun getListedItem(type: FRC20Storefront.ListingType, rank: UInt64, id: UInt64): ListedItem?{ 
        ...
        ///
        access(account) fun updateAdminWhitelist(mananger: &{MarketManager}, address: Address, isWhitelisted: Bool){ 
            pre{ 
        ...
        /// The method is called by the manager resource
        ///
        access(account) fun updateMarketplaceProperties(mananger: &{MarketManager}, _ props:{ FRC20FTShared.ConfigType: String}){ 
            pre{ 
                self.isInAdminWhitelist(mananger.getOwnerAddress()):
                    "The manager is not in the admin whitelist"
            }
        ...
        /** ---- Internal Methods ---- *//// Borrow the shared store
        ///
        priv fun borrowSharedStore(): &FRC20FTShared.SharedStore{FRC20FTShared.SharedStorePublic}?{ 
            return FRC20FTShared.borrowStoreRef((self.owner!).address)
        }
        
        /// Parse the ranked id
        ///
        priv fun parseRankedId(rankedId: String): ItemIdentifier{ 
        ...
        ///
        priv fun borrowOrCreateCollection(_ type: FRC20Storefront.ListingType, _ rank: UInt64): &ListingCollection{ 
        ...
        priv fun borrowCollection(_ type: FRC20Storefront.ListingType, _ rank: UInt64): &ListingCollection?{ 
        ...
        priv fun _borrowCollectionDict(_ type: FRC20Storefront.ListingType): &{UInt64: ListingCollection}?{ 
            return &self.collections[type] as &{UInt64: ListingCollection}?
        }
    }
    
    /** ---– Account Access methods ---- *//// Create a new market
    ///
    access(account) fun createMarket(_ tick: String): @Market{ 
    ...
    ///
    pub fun borrowMarket(_ addr: Address): &Market{MarketPublic}?{ 
        return getAccount(addr).getCapability<&Market{MarketPublic}>(
            self.FRC20MarketPublicPath
        ).borrow()
    }
