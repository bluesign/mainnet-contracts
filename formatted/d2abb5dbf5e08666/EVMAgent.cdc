...
pub contract EVMAgent{ 
    ...
    pub event              
              ...
              NewAgencySetup(agency: Address)
    
    /// Event emitted when a new agency manager is created
    pub event NewAgencyManagerCreated(forAgency: Address)
    
    /// Event emitted when a new entrusted account is created.
    pub event NewEntrustedAccountCreated(
        evmAddress: String,
    ...
    pub event EntrustedAccountVerified(
        evmAddress: String,
        entrustedAccount: Address,
        byAgency: Address,
        message: String,
    ...
    /* --- Variable, Enums and Structs --- */    pub let entrustedStatusStoragePath: StoragePath
    
    pub let entrustedStatusPublicPath: PublicPath
    
    pub let evmAgencyManagerStoragePath: StoragePath
    
    pub let evmAgencyStoragePath: StoragePath
    
    pub let evmAgencyPublicPath: PublicPath
    
    pub let evmAgencyCenterStoragePath: StoragePath
    
    pub let evmAgencyCenterPublicPath: PublicPath
    
    /* --- Interfaces & Resources --- */    pub resource interface IEntrustedStatus{ 
        pub let key: String
        
        /// Borrow the agency capability
        pub fun borrowAgency(): &Agency{AgencyPublic}{} 
        
        /// Get the flow spent by the entrusted account
        pub fun getFeeSpent(): UFix64{} 
    ...
    ///
    pub resource EntrustedStatus: IEntrustedStatus{ 
        pub let key: String
        
        // Capability to the agency
        priv let agency: Capability<&Agency{AgencyPublic}>
        
        // record the flow spent by the entrusted account
        priv var feeSpent: UFix64
        
        init(key: String, _ agency: Capability<&Agency{AgencyPublic}>){ 
            self.key = key
            self.agency = agency
            self.feeSpent = 0.0
        }
    ...
    ///
    pub resource AgencyManager{ 
        priv             // Capability to the agency
             let agency: Capability<&Agency{AgencyPublic, AgencyPrivate}>
        
        init(_ agency: Capability<&Agency{AgencyPublic, AgencyPrivate}>){ 
            self.agency = agency
        }
        
        /// Borrow the agency capability
        pub fun borrowAgency(): &Agency{AgencyPublic, AgencyPrivate}{ 
            return self.agency.borrow() ?? panic("Agency not found")
        }
        
        /// Withdraw the flow from the agency
        ///
        pub fun withdraw(amt: UFix64): @FlowToken.Vault{ 
    ...
    pub struct AgencyStatus{ 
        ...
        pub var managingEntrustedAccounts: UInt64
        
        pub var spentFlowAmount: UFix64
        
        pub var earnedFlowAmount: UFix64
        ...
        access(contract) fun addSpentFlowAmount(_ amount: UFix64){ 
            self.spentFlowAmount = self.spentFlowAmount + amount
        }
        
        access(contract) fun addEarnedFlowAmount(_ amount: UFix64){ 
            self.earnedFlowAmount = self.earnedFlowAmount + amount
        }
        
        access(contract) fun addManagingEntrustedAccounts(_ count: UInt64){ 
            self.managingEntrustedAccounts = self.managingEntrustedAccounts
                + count
        }
        
        access(contract) fun updateExtra(_ key: String, _ value: AnyStruct){ 
    ...
    ///
    pub resource interface AgencyPublic{ 
        pub            /// Get the owner address
        ...
        ///
        pub fun verifyAndBorrowEntrustedAccount(
    ...
    ///
    pub resource interface AgencyPrivate{ 
        pub            /// Create a new agency manager
    ...
    /// Agency resource
    ///
    pub resource Agency: AgencyPublic, AgencyPrivate{ 
        pub let creator: Address
        
        /// Current status of the agency
        priv let status: AgencyStatus
        
        /// EVMAddress String => Address
        priv let managedEntrustedAccounts:{ String: Address}
        
        init(_ creatingIns: &Fixes.Inscription){ 
        ...
        /// Setup the agency
        pub fun setup(_ acctCap: Capability<&AuthAccount>){ 
            pre{ 
                self.getOwnerAddress() == acctCap.address:
                    "Only the owner can setup the agency"
            }
            let acctsPool = FRC20AccountsPool.borrowAccountsPool()
            let creator = self.creator.toString()
            assert(acctsPool.getEVMAgencyAddress(creator) == nil, message: "Agency already registered")
            acctsPool.setupNewChildForEVMAgency(owner: creator, acctCap)
            
            // get the agency account
            let authAcct = acctsPool.borrowChildAccount(type: FRC20AccountsPool.ChildAccountType.EVMAgency, creator) ?? panic("Agency account not found")
        ...
        /** ---- Private method ---- */        pub fun createAgencyManager(): @AgencyManager{ 
        ...
        pub fun withdraw(amt: UFix64): @FlowToken.Vault{ 
            ...
            let flowVaultRef = acct.borrow<&FlowToken.Vault>(from: /storage/flowTokenVault) ?? panic("The flow vault is not found")
            assert(flowVaultRef.balance >= amt, message: "Insufficient balance")
            let vault <- flowVaultRef.withdraw(amount: amt)
            return <-(vault as! @FlowToken.Vault)
        }
        
        /* --- Public methods  --- */// Check if the EVM address is managed by the agency
        pub fun isEVMAccountManaged(_ evmAddress: String): Bool{ 
        ...
        /// Get the balance of the flow for the agency
        pub fun getFlowBalance(): UFix64{ 
            if let ref = getAccount(self.getOwnerAddress()).getCapability(/public/flowTokenBalance).borrow<&FlowToken.Vault{FungibleToken.Balance}>(){ 
                return ref.balance
            }
            return 0.0
        }
        ...
        ///
        pub fun createEntrustedAccount(hexPublicKey: String, hexSignature: String, timestamp: UInt64, _ acctCap: Capability<&AuthAccount>): @FlowToken.Vault{ 
            pre{ 
                acctCap.check():
                    "Invalid account capability"
            }
            let evmAddress = ETHUtils.getETHAddressFromPublicKey(hexPublicKey: hexPublicKey)
            assert(self.managedEntrustedAccounts[evmAddress] == nil, message: "EVM address already registered for an agent account")
            let acctsPool = FRC20AccountsPool.borrowAccountsPool()
            // Ensure the evmAddress is not already registered
            let existingAddr = acctsPool.getEVMEntrustedAccountAddress(evmAddress)
            assert(existingAddr == nil, message: "EVM address already registered for an agent account")
            let message = "op=create-entrusted-account(),params=".concat(",address=").concat(evmAddress).concat(",timestamp=").concat(timestamp.toString())
            ...
            let entrustedAcctFlowBalanceRef = entrustedAcct.getCapability(/public/flowTokenBalance).borrow<&FlowToken.Vault{FungibleToken.Balance}>() ?? panic("Could not borrow Balance reference to the Vault")
            // Reference to the recipient's receiver
            let entrustedAcctFlowRecipientRef = entrustedAcct.getCapability(/public/flowTokenReceiver).borrow<&FlowToken.Vault{FungibleToken.Receiver}>() ?? panic("Could not borrow receiver reference to the recipient's Vault")
            
            // Get the flow vault from the agency account
            let flowVaultRef = agencyAcct.borrow<&FlowToken.Vault>(from: /storage/flowTokenVault) ?? panic("The flow vault is not found")
            ...
            // emit event
            emit NewEntrustedAccountCreated(evmAddress: evmAddress, entrustedAccount: entrustedAddress, byAgency: agencyAcct.address, initialFunding: spentFlowAmt)
        ...
        ///
        pub fun verifyAndBorrowEntrustedAccount(methodFingerprint: String, params: [String], hexPublicKey: String, hexSignature: String, timestamp: UInt64): &AuthAccount{ 
            let message = "op=".concat(methodFingerprint).concat(",params=").concat(StringUtils.join(params, "|"))
            return self._verifyAndBorrowEntrustedAccount(message: message, hexPublicKey: hexPublicKey, hexSignature: hexSignature, timestamp: timestamp)
        }
        
        /// Verify the evm signature, if valid, borrow the reference of the entrusted account
        ///
        priv fun _verifyAndBorrowEntrustedAccount(message: String, hexPublicKey: String, hexSignature: String, timestamp: UInt64): &AuthAccount{ 
            let evmAddress = ETHUtils.getETHAddressFromPublicKey(hexPublicKey: hexPublicKey)
            assert(self.managedEntrustedAccounts[evmAddress] != nil, message: "EVM address not registered for an agent account")
            let acctsPool = FRC20AccountsPool.borrowAccountsPool()
            // Ensure the evmAddress is not already registered
            let entrustedAddr = acctsPool.getEVMEntrustedAccountAddress(evmAddress)
            assert(entrustedAddr != nil, message: "EVM address not registered for an agent account")
            let msgToVerify = message.concat(",address=").concat(evmAddress).concat(",timestamp=").concat(timestamp.toString())
            let isValid = ETHUtils.verifySignature(hexPublicKey: hexPublicKey, hexSignature: hexSignature, message: msgToVerify)
            assert(isValid, message: "Invalid signature")
            
            // Since the signature is valid, we think the transaction is valid
            // and we can borrow the reference to the entrusted account
            let entrustedAcct = acctsPool.borrowChildAccount(type: FRC20AccountsPool.ChildAccountType.EVMEntrustedAccount, evmAddress) ?? panic("The staking account was not created")
            
            // The entrusted account need to pay a fee to the agency
            let entrustedAcctFlowVauleRef = entrustedAcct.borrow<&FlowToken.Vault>(from: /storage/flowTokenVault) ?? panic("The flow vault is not found")
            let agencyFlowReceiptRef = self._borrowAgencyAccount().getCapability(/public/flowTokenReceiver).borrow<&{FungibleToken.Receiver}>() ?? panic("Could not borrow receiver reference to the recipient's Vault")
            ...
            if let entrustedStatus = entrustedAcct.borrow<&EntrustedStatus{IEntrustedStatus}>(from: EVMAgent.entrustedStatusStoragePath){ 
                entrustedStatus.addSpentFlowFee(fee)
            }
            
            // emit event
            emit EntrustedAccountVerified(evmAddress: evmAddress, entrustedAccount: entrustedAcct.address, byAgency: self.getOwnerAddress(), message: message, fee: fee)
            return entrustedAcct
        }
        
        /* --- Contract access methods  --- *//// Ensure the resources are initialized to the entrusted account
        ///
        access(contract) fun _ensureEntrustedAcctResources(_ key: String): Bool{ 
            let acctsPool = FRC20AccountsPool.borrowAccountsPool()
            // try to borrow the account to check if it was created
            let childAcctRef = acctsPool.borrowChildAccount(type: FRC20AccountsPool.ChildAccountType.EVMEntrustedAccount, key) ?? panic("The staking account was not created")
            var isUpdated = false
        ...
        /* --- Internal access methods  --- *//// Get the private capability of the agency
        ///
        priv fun _getSelfPrivCap(): Capability<&Agency{AgencyPublic, AgencyPrivate}>{ 
            // get the agency account
        ...
        priv fun _borrowAgencyAccount(): &AuthAccount{ 
            let acctsPool = FRC20AccountsPool.borrowAccountsPool()
            return acctsPool.borrowChildAccount(type: FRC20AccountsPool.ChildAccountType.EVMAgency, self.creator.toString()) ?? panic("Agency account not found")
        }
    }
    
    /// Agency center public interface
    ///
    pub resource interface AgencyCenterPublic{ 
        pub            /// Get the agencies
            fun getAgencies(): [Address]{} 
        
        /// Create a new agency
        pub fun createAgency(
            ins: &Fixes.Inscription,
            _ acctCap: Capability<&AuthAccount>
        ): @AgencyManager{} 
        
        /// Get the agency by evm address
        pub fun borrowAgencyByEVMAddress(_ evmAddress: String): &Agency{
            AgencyPublic
        }?{} 
        
        /// Get the agency by address
        pub fun pickValidAgency(): &Agency{AgencyPublic}?{} 
    }
    
    /// Agency center resource
    ///
    pub resource AgencyCenter: AgencyCenterPublic{ 
        priv let agencies:{ Address: Bool}
        ...
        /// Get the agencies
        ///
        pub fun getAgencies(): [Address]{ 
            return self.agencies.keys
        }
        
        /// Create a new agency
        ///
        pub fun createAgency(ins: &Fixes.Inscription, _ acctCap: Capability<&AuthAccount>): @AgencyManager{ 
            // singleton resources
            ...
            assert(op == "create-evm-agency", message: "Invalid operation")
            ...
            let delegator = FRC20Staking.borrowDelegator(fromAddr) ?? panic("The delegator is not found")
            ...
            assert(stakedBalance >= 10000.0, message: "The delegator should have staked enough balance")
            ...
            let flowReceiverRef = FRC20Indexer.borrowFlowTokenReceiver(addr) ?? panic("Could not borrow receiver reference to the recipient's Vault")
            flowReceiverRef.deposit(from: <-ins.extract())
            
            // save the agency
            acct.save(<-agency, to: EVMAgent.evmAgencyStoragePath)
            let agencyRef = acct.borrow<&Agency>(from: EVMAgent.evmAgencyStoragePath) ?? panic("Agency not found")
        ...
        /// Get the agency by evm address
        ///
        pub fun borrowAgencyByEVMAddress(_ evmAddress: String): &Agency{AgencyPublic}?{ 
            let acctsPool = FRC20AccountsPool.borrowAccountsPool()
        ...
        pub fun pickValidAgency(): &Agency{AgencyPublic}?{ 
            ...
            for addr in keys{ 
                ...
                if let flowVaultRef = getAccount(addr).getCapability(/public/flowTokenBalance).borrow<&FlowToken.Vault{FungibleToken.Balance}>(){ 
    ...
    /* --- Public methods  --- */    pub fun getIdentifierPrefix(): String{ 
        return "EVMAgency_".concat(self.account.address.toString())
    }
    
    /// Get the fee for any operation by the agency
    ///
    pub fun getAgencyFlowFee(): UFix64{ 
        return 0.01
    }
    
    /// Get the capability to the entrusted status
    ///
    pub fun borrowEntrustStatus(_ addr: Address): &EntrustedStatus{
        IEntrustedStatus
    }?{ 
        return getAccount(addr).getCapability<
            &EntrustedStatus{IEntrustedStatus}
        >(self.entrustedStatusPublicPath).borrow()
    }
    
    /// Get the capability to the agency
    ///
    pub fun getAgencyPublicCap(_ addr: Address): Capability<
        &Agency{AgencyPublic}
    >{ 
        return getAccount(addr).getCapability<&Agency{AgencyPublic}>(
            self.evmAgencyPublicPath
        )
    }
    
    /// Borrow the reference to agency public
    ///
    pub fun borrowAgency(_ addr: Address): &Agency{AgencyPublic}?{ 
        return self.getAgencyPublicCap(addr).borrow()
    }
    
    /// Borrow the reference to agency public
    ///
    pub fun borrowAgencyByEVMPublicKey(_ hexPubKey: String): &Agency{
        AgencyPublic
    }?{ 
        let center = self.borrowAgencyCenter()
        let evmAddr =
            ETHUtils.getETHAddressFromPublicKey(hexPublicKey: hexPubKey)
        return center.borrowAgencyByEVMAddress(evmAddr)
    }
    
    /// Borrow the reference to agency center
    ///
    pub fun borrowAgencyCenter(): &AgencyCenter{AgencyCenterPublic}{ 
        return getAccount(self.account.address).getCapability<
            &AgencyCenter{AgencyCenterPublic}
        >(self.evmAgencyCenterPublicPath).borrow()
        ?? panic("Agency center not found")
    }
    
    init(){ 
        let prefix = EVMAgent.getIdentifierPrefix()
        self.entrustedStatusStoragePath = StoragePath(
                identifier: prefix.concat("_entrusted_status")
            )!
        self.entrustedStatusPublicPath = PublicPath(
                identifier: prefix.concat("_entrusted_status")
            )!
        self.evmAgencyManagerStoragePath = StoragePath(
                identifier: prefix.concat("_agency_manager")
            )!
        self.evmAgencyStoragePath = StoragePath(
                identifier: prefix.concat("_agency")
            )!
        self.evmAgencyPublicPath = PublicPath(
                identifier: prefix.concat("_agency")
            )!
        self.evmAgencyCenterStoragePath = StoragePath(
                identifier: prefix.concat("_center")
            )!
        self.evmAgencyCenterPublicPath = PublicPath(
                identifier: prefix.concat("_center")
            )!
        ...
        self.account.link<&AgencyCenter{AgencyCenterPublic}>(
            self.evmAgencyCenterPublicPath,
            target: self.evmAgencyCenterStoragePath
        )
