...
pub contract FRC20Storefront{ 
    pub        /* --- Events --- */        event StorefrontInitialized(uuid: UInt64)
    ...
    /* --- Variable, Enums and Structs --- */    pub let StorefrontStoragePath: StoragePath
    
    pub let StorefrontPublicPath: PublicPath
    
    /* --- Interfaces & Resources --- */    pub enum ListingStatus: UInt8{ 
        pub case Available
    ...
    pub struct ListingDetails{ 
        pub            // constants data values
            let storefrontId: UInt64
        
        pub let inscriptionId: UInt64
        
        pub let type: ListingType
        ...
        /// Sale cuts
        pub let saleCuts: [FRC20FTShared.SaleCut]
        
        // Calculated values
        pub let totalPrice: UFix64
        
        pub let priceValuePerMint: UFix64
        ...
        pub var transactedAmount: UFix64
        ...
        init(
            ...
            pre{ 
                ...
                saleCuts.length > 0:
                    "Listing must have at least one payment cut recipient"
                totalPrice > 0.0:
                    "Listing must have non-zero price"
                amount > 0.0:
                    "Listing must have non-zero amount"
            }
            let tokenMeta =
                FRC20Indexer.getIndexer().getTokenMeta(tick: tick)
        ...
        ///
        pub fun pricePerToken(): UFix64{ 
            return self.totalPrice / self.amount
        }
        
        /// Get the price rank
        pub fun priceRank(): UInt64{ 
        ...
        ///
        pub fun isFullyTransacted(): Bool{ 
            return self.transactedAmount == self.amount
        }
        
        pub fun getPriceByTransactedAmount(_ transactedAmount: UFix64): UFix64{ 
            pre{ 
                transactedAmount <= self.amount:
                    "Transacted amount should not exceed the total amount"
            }
            return self.totalPrice.saturatingMultiply(
                transactedAmount / self.amount
            )
        }
        ...
        access(contract) fun setToCompleted(){ 
            pre{ 
                ...
                self.isFullyTransacted():
                    "Listing must be fully transacted"
            }
        ...
        ///
        access(contract) fun transact(amount: UFix64){ 
            pre{ 
                self.transactedAmount.saturatingAdd(amount) <= self.amount:
                    "Transacted amount should not exceed the total amount"
            }
    ...
    pub resource interface ListingPublic{ 
        ...
        /// Fetches the status
        pub fun getStatus(): ListingStatus{} 
        ...
        pub fun getAllowedCommissionReceivers(): [
            Capability<&FlowToken.Vault{FungibleToken.Receiver}>
        ]?{} 
        ...
        pub fun takeBuyNow(
            ...
            commissionRecipient: Capability<
        ...
        /** ---- Internal Methods ---- *//// borrow the inscription reference
        access(contract) fun borrowInspection(): &Fixes.Inscription{} 
    }
    ...
    pub resource Listing: ListingPublic{ 
        ...
        access(contract) let commissionRecipientCaps: [Capability<&FlowToken.Vault{FungibleToken.Receiver}>]?
        
        /// The frozen change for this listing.
        access(contract) var frozenChange: @FRC20FTShared.Change?
        
        /// initializer
        ///
        init(storefrontId: UInt64, listIns: &Fixes.Inscription, commissionRecipientCaps: [Capability<&FlowToken.Vault{FungibleToken.Receiver}>]?, customID: String?){ 
            ...
            switch op{ 
                case "list-buynow":
                    ...
                    assert(order?.tick == order?.change?.tick && order?.amount == order?.change?.getBalance(), message: "Tick and amount in the inscription and the change should be the same")
            ...
            self.details = ListingDetails(storefrontId: storefrontId, inscriptionId: listIns.getId(), type: listType, tick: order?.tick ?? panic("Unable to fetch the tick"), amount: order?.amount ?? panic("Unable to fetch the amount"), totalPrice: order?.totalPrice ?? panic("Unable to fetch the total price"), saleCuts: order?.cuts ?? panic("Unable to fetch the cuts"), customID: customID)
        ...
        destroy(){ 
            pre{ 
                ...
                self.frozenChange == nil:
                    "Frozen change must be nil"
            }
        ...
        pub fun getTickMeta(): FRC20Indexer.FRC20Meta{ 
            let indexer = FRC20Indexer.getIndexer()
            return indexer.getTokenMeta(tick: self.details.tick) ?? panic("Unable to fetch the token meta")
        }
        ...
        /// Fetches the allowed marketplaces capabilities or commission receivers.
        /// If it returns `nil` then commission is up to grab by anyone.
        pub fun getAllowedCommissionReceivers(): [Capability<&FlowToken.Vault{FungibleToken.Receiver}>]?{ 
        ...
        /// This pays the beneficiaries and returns the token to the buyer.
        ///
        pub fun takeBuyNow(ins: &Fixes.Inscription, commissionRecipient: Capability<&FlowToken.Vault{FungibleToken.Receiver}>?){ 
            pre{ 
                self.details.type == ListingType.FixedPriceBuyNow:
                    "Listing must be a buy now listing"
                self.details.status == ListingStatus.Available:
                    "Listing must be available"
                self.owner != nil:
            ...
            assert(FRC20Storefront.isListFRC20Inscription(ins: ins), message: "Given inscription is not a valid FRC20 listing inscription")
            ...
            assert(frc20TokenChange != nil && frc20TokenChange?.isBackedByVault() == false, message: "Frozen change should be backed by a non-vault change")
            ...
            assert(seller != buyer && seller == storefrontAddress, message: "Seller should be the storefront address and buyer should not be the storefront address")
            
            // The change to use
            let extractedFlowChange <- frc20Indexer.extractFlowVaultChangeFromInscription(ins, amount: ins.getInscriptionValue() - ins.getMinCost())
            assert(extractedFlowChange.from == buyer && extractedFlowChange.tick == "" && extractedFlowChange.getVaultType() == Type<@FlowToken.Vault>(), message: "Extracted change should be backed by the inscription owner")
            
            // apply the change and both inscriptions in frc20 indexer
            var restChange <- frc20Indexer.applyBuyNowOrder(makerIns: sellerIns, takerIns: ins, maxAmount: maxAmount, change: <-(frc20TokenChange ?? panic("Unable to extract the change")))
            let transactedAmt = currentAmount.saturatingSubtract(restChange.getBalance())
            // update the transacted amount
            self.details.transact(amount: transactedAmt)
            assert(self.details.amount - self.details.transactedAmount == restChange.getBalance(), message: "Un-transacted amount should be equal to the change balance")
            ...
            assert(extractedFlowChange.getBalance() >= transactedPrice, message: "Insufficient payment value")
            ...
            let commissionAmount = self._payToSaleCuts(payment: <-(flowVault as! @FlowToken.Vault), commissionRecipient: commissionRecipient, paymentRecipient: nil)
            ...
            if extractedFlowChange.getBalance() > 0.0{ 
                ...
                let buyerVault = FRC20Indexer.borrowFlowTokenReceiver(buyer) ?? panic("Unable to fetch the buyer vault")
            ...
            self._onTransactionDeal(storefrontAddress: storefrontAddress, seller: seller, buyer: buyer, transactedAmt: transactedAmt, transactedPrice: transactedPrice, commissionAmount: commissionAmount, commissionRecipientAddress: commissionRecipient?.address)
        }
        
        /// Purchase the listing, selling the token.
        /// This pays the beneficiaries and returns the token to the buyer.
        pub fun takeSellNow(ins: &Fixes.Inscription, commissionRecipient: Capability<&FlowToken.Vault{FungibleToken.Receiver}>?){ 
            pre{ 
                self.details.type == ListingType.FixedPriceSellNow:
                    "Listing must be a buy now listing"
                self.details.status == ListingStatus.Available:
                    "Listing must be available"
                self.owner != nil:
            ...
            assert(flowTokenChange != nil && flowTokenChange?.isBackedByFlowTokenVault() == true, message: "Frozen change should be backed by a vault change")
            let maxAmount = self.details.amount - self.details.transactedAmount
            ...
            let storefrontAddress = self.owner?.address ?? panic("Unable to fetch the storefront address")
            assert(seller != buyer && buyer == storefrontAddress, message: "Buyer should be the storefront address and seller should not be the storefront address")
            let detailRef = &self.details as &ListingDetails
            var payment: @FlowToken.Vault? <- nil
            var transactedAmt: UFix64? = nil
            let restChange <- frc20Indexer.applySellNowOrder(makerIns: buyerIns, takerIns: ins, maxAmount: maxAmount, change: <-(flowTokenChange ?? panic("Unable to get the flow token change")), fun (_ realTransactedAmt: UFix64, flowToPay: @FlowToken.Vault): Bool{ 
                        pre{ 
                            realTransactedAmt > 0.0:
                                "Transacted amount should be greater than zero"
                            flowToPay.balance > 0.0:
            ...
            let paymentRecipient = FRC20Indexer.borrowFlowTokenReceiver(seller) ?? panic("Unable to fetch the payment recipient")
            
            // Pay the sale cuts to the recipients.
            let commissionAmount = self._payToSaleCuts(payment: <-(payment ?? panic("Payment is nil")), commissionRecipient: commissionRecipient, paymentRecipient: paymentRecipient)
        ...
        ///
        priv fun _onTransactionDeal(storefrontAddress: Address, seller: Address, buyer: Address, transactedAmt: UFix64, transactedPrice: UFix64, commissionAmount: UFix64, commissionRecipientAddress: Address?){ 
            // Some singleton resources
            let frc20Indexer = FRC20Indexer.getIndexer()
            ...
            if let marketTransactionHook = FRC20FTShared.borrowTransactionHook(marketAddress){ 
                marketTransactionHook.onDeal(storefront: storefrontAddress, listingId: self.details.storefrontId, seller: seller, buyer: buyer, tick: tickName, dealAmount: transactedAmt, dealPrice: transactedPrice, totalAmountInListing: self.details.amount)
            }
            // for seller hook
            if let sellerTransactionHook = FRC20FTShared.borrowTransactionHook(seller){ 
                sellerTransactionHook.onDeal(storefront: storefrontAddress, listingId: self.details.storefrontId, seller: seller, buyer: buyer, tick: tickName, dealAmount: transactedAmt, dealPrice: transactedPrice, totalAmountInListing: self.details.amount)
            }
            // for buyer hook
            if let buyerTransactionHook = FRC20FTShared.borrowTransactionHook(buyer){ 
                buyerTransactionHook.onDeal(storefront: storefrontAddress, listingId: self.details.storefrontId, seller: seller, buyer: buyer, tick: tickName, dealAmount: transactedAmt, dealPrice: transactedPrice, totalAmountInListing: self.details.amount)
            }
            // ------- end ---------------------------------
            
            
            // emit ListingPartiallyDeal event
            emit ListingPartiallyDeal(storefrontAddress: storefrontAddress, storefrontId: self.details.storefrontId, listingResourceID: self.uuid, inscriptionId: self.details.inscriptionId, type: self.details.type.rawValue, tick: tickName, totalAmount: self.details.amount, transactedAmount: transactedAmt, transactedPrice: transactedPrice, customID: self.details.customID, commissionAmount: commissionAmount, commissionReceiver: commissionAmount != 0.0 ? commissionRecipientAddress : nil)
            ...
            if self.details.isFullyTransacted(){ 
                ...
                if nilChange?.getBalance() != 0.0{ 
                    frc20Indexer.returnChange(change: <-(nilChange ?? panic("Unable to extract the change")))
                } else{ 
                    destroy nilChange
                }
                
                // emit ListingCompleted event
                emit ListingCompleted(storefrontAddress: storefrontAddress, storefrontId: self.details.storefrontId, listingResourceID: self.uuid, inscriptionId: self.details.inscriptionId, type: self.details.type.rawValue, tick: self.details.tick, amount: self.details.amount, totalPrice: self.details.totalPrice, customID: self.details.customID, commissionAmount: commissionAmount, commissionReceiver: commissionAmount != 0.0 ? commissionRecipientAddress : nil)
            }
        }
        
        /** ---- Account or contract methods ---- */        access(contract) fun cancel(){ 
            pre{ 
                self.details.status == ListingStatus.Available:
                    "Listing must be available"
                self.owner != nil:
            ...
            assert(changeToReturn != nil, message: "Frozen change should not be nil")
            
            // apply the change and rollback inscriptions in frc20 indexer
            frc20Indexer.cancelListing(listedIns: self.borrowInspection(), change: <-(changeToReturn ?? panic("Unable to extract the change")))
            emit ListingCancelled(storefrontAddress: self.owner?.address ?? panic("Unable to fetch the storefront address"), storefrontId: self.details.storefrontId, listingResourceID: self.uuid, inscriptionId: self.details.inscriptionId, type: self.details.type.rawValue, tick: self.details.tick, amount: self.details.amount, price: self.details.totalPrice, customID: self.details.customID)
        }
        ...
        /* ---- Internal methods ---- *//// Pay the sale cuts to the recipients.
        /// Returns the amount of commission paid.
        ///
        priv fun _payToSaleCuts(payment: @FlowToken.Vault, commissionRecipient: Capability<&FlowToken.Vault{FungibleToken.Receiver}>?, paymentRecipient: &{FungibleToken.Receiver}?): UFix64{ 
            // Some singleton resources
            let frc20Indexer = FRC20Indexer.getIndexer()
            let acctsPool = FRC20AccountsPool.borrowAccountsPool()
            ...
            let listingTokenMeta = frc20Indexer.getTokenMeta(tick: listingTick) ?? panic("Unable to fetch the token meta")
        ...
        ///
        priv fun _borrowStorefront(): &Storefront{StorefrontPublic}{ 
            return FRC20Storefront.borrowStorefront(address: (self.owner!).address) ?? panic("Storefront not found")
        }
    }
    ...
    pub resource interface StorefrontManager{ 
        ...
        ///
        pub fun removeListing(listingResourceID: UInt64): @Fixes.Inscription{} 
    }
    ...
    pub resource interface StorefrontPublic{ 
        pub            /** ---- Public Methods ---- *//// get all listingIDs
            fun getListingIDs(): [UInt64]{} 
        
        // Borrow the listing reference
        pub fun borrowListing(_ listingResourceID: UInt64): &Listing{
            ListingPublic
        }?{} 
        
        // Cleanup methods
        pub fun tryCleanupFinishedListing(_ listingResourceID: UInt64){} 
        
        /** ---- Contract Methods ---- *//// borrow the inscription reference
        access(contract) fun borrowInspection(
            _ id: UInt64
        ): &Fixes.Inscription{} 
    }
    ...
    pub resource Storefront: StorefrontManager, StorefrontPublic{ 
        ...
        access(contract) var inscriptions: @{UInt64: Fixes.Inscription}
        ...
        access(contract) var listedTicks:{ String: [UInt64]}
        ...
        ///
        pub fun borrowListing(_ listingResourceID: UInt64): &Listing{ListingPublic}?{ 
        ...
        ///
        pub fun createListing(ins: @Fixes.Inscription, marginVault: @FlowToken.Vault?, commissionRecipientCaps: [Capability<&FlowToken.Vault{FungibleToken.Receiver}>]?, customID: String?): UInt64{ 
            pre{ 
                self.owner != nil:
                    "Resource doesn't have the assigned owner"
            }
            var insRef = &ins as &Fixes.Inscription
            assert(FRC20Storefront.isListFRC20Inscription(ins: insRef), message: "Given inscription is not a valid FRC20 listing inscription")
            ...
            let listing <- create Listing(storefrontId: storefrontId, listIns: insRef, commissionRecipientCaps: commissionRecipientCaps, customID: customID)
            ...
            emit ListingAvailable(storefrontAddress: self.owner?.address ?? panic("Storefront owner is not set"), storefrontId: storefrontId, listingResourceID: listingResourceID, inscriptionId: details.inscriptionId, type: details.type.rawValue, tick: details.tick, amount: details.amount, totalPrice: details.totalPrice, customID: customID, commissionReceivers: allowedCommissionReceivers)
        ...
        pub fun removeListing(listingResourceID: UInt64): @Fixes.Inscription{ 
            ...
            if currentStatus == ListingStatus.Available{ 
                listingRef.cancel()
            } else if currentStatus == ListingStatus.Cancelled{}                                                                  // If the listing is already cancelled, then we don't need to do anything.
                                                                  else if currentStatus == ListingStatus.Completed{} 
            // If the listing is already completed, then we don't need to do anything.
            
            // ensure the change in listing is nil
            assert(listingRef.frozenChange == nil, message: "Listing change should be nil")
            let listing <- self.listings.remove(key: listingResourceID) ?? panic("missing Listing")
            let details = listing.getDetails()
            emit ListingRemoved(storefrontAddress: self.owner?.address ?? panic("Storefront owner is not set"), storefrontId: self.uuid, listingResourceID: listingResourceID, inscriptionId: details.inscriptionId, customID: details.customID, withStatus: currentStatus.rawValue)
            destroy listing
        ...
        ///
        pub fun tryCleanupFinishedListing(_ listingResourceID: UInt64){ 
            let listingRef = self.borrowListing(listingResourceID) ?? panic("Could not find listing with given id")
            ...
            emit ListingRemoved(storefrontAddress: self.owner?.address ?? panic("Storefront owner is not set"), storefrontId: self.uuid, listingResourceID: listingResourceID, inscriptionId: details.inscriptionId, customID: details.customID, withStatus: details.status.rawValue)
            // destroy listing
            destroy listing
        ...
        /** ---- Internal Method ---- *//// borrow the inscription reference
        ///
        access(contract) fun borrowInspection(_ id: UInt64): &Fixes.Inscription{ 
            return &self.inscriptions[id] as &Fixes.Inscription? ?? panic("Inscription not found")
        }
        
        /// borrow the listing reference
        ///
        access(contract) fun borrowListingPrivate(_ id: UInt64): &Listing{ 
            return &self.listings[id] as &Listing? ?? panic("Listing not found")
        }
    }
    
    /* --- Public Resource Interfaces --- *//// Check if the given inscription is a valid FRC20 listing inscription.
    ///
    pub fun isListFRC20Inscription(ins: &Fixes.Inscription): Bool{ 
        let indexer = FRC20Indexer.getIndexer()
        if !indexer.isValidFRC20Inscription(ins: ins){ 
            return false
        }
        let meta =
            indexer.parseMetadata(&ins.getData() as &Fixes.InscriptionData)
    ...
    ///
    pub fun borrowStorefront(address: Address): &Storefront{StorefrontPublic}?{ 
        return getAccount(address).getCapability<&Storefront{StorefrontPublic}>(
            self.StorefrontPublicPath
        ).borrow()
    }
    
    init(){ 
        let identifier =
            "FRC20Storefront_".concat(self.account.address.toString())
