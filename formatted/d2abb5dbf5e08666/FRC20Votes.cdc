...
pub contract FRC20Votes{ 
    ...
    pub event ProposalStatusChanged(
        proposalId: UInt64,
        tick: String,
        prevStatus: UInt8?,
        newStatus: UInt8,
    ...
    pub event ProposalCancelled(proposalId: UInt64, tick: String)
    ...
    pub event VotesManagerWhitelistUpdated(voter: Address, isWhitelisted: Bool)
    
    /* --- Variable, Enums and Structs --- */    pub let VoterStoragePath: StoragePath
    
    pub let VoterPublicPath: PublicPath
    
    pub let FRC20VotesManagerStoragePath: StoragePath
    
    pub let FRC20VotesManagerPublicPath: PublicPath
    
    /* --- Interfaces & Resources --- *//// The Proposal status.
    ///
    ...
    pub resource interface VoterPublic{ 
        pub fun getVoterAddress(): Address{} 
        
        /// Get the voting power.
        pub fun getVotingPower(): UFix64{} 
        
        /// Check whether the proposal is voted.
        pub fun hasVoted(_ proposalId: UInt64): Bool{} 
        
        /// Get the voted points.
        pub fun getVotedPoints(_ proposalId: UInt64): UFix64?{} 
        
        /// Get the voted choice.
        pub fun getVotedChoice(_ proposalId: UInt64): Int?{} 
        
        /// Get the voted proposals.
        pub fun getVotedProposals(tick: String): [UInt64]{} 
        
        /// ---- Write: contract level ----
        access(contract) fun onVote(
            choice: Int,
            proposal: &Proposal{ProposalPublic}
        ){} 
        
        access(contract) fun onProposalFinalized(
            proposal: &Proposal{ProposalPublic}
        ){} 
    }
    ...
    pub resource VoterIdentity:
        ...
        priv let semiNFTColCap: Capability<
        ...
        priv let lockedSemiNFTCollection: @FRC20SemiNFT.Collection
        
        // ----- Voting Info ----
        /// Voted proposal id -> Points
        priv let voted:{ UInt64: UFix64}
        
        /// ProposalID -> Choice ID
        priv let votedChoices:{ UInt64: Int}
        
        /// Tick -> ProposalId[]
        priv let votedTicksMapping:{ String: [UInt64]}
        
        /// ProposalID -> EndedAt
        priv let activeProposals:{ UInt64: UFix64}
        ...
        /** ----- Implement NFT Standard ----- */        pub fun deposit(token: @NonFungibleToken.NFT){ 
            self.lockedSemiNFTCollection.deposit(token: <-token)
        }
        
        pub fun withdraw(withdrawID: UInt64): @NonFungibleToken.NFT{ 
            return <-self.lockedSemiNFTCollection.withdraw(
        ...
        pub fun getIDsByTick(tick: String): [UInt64]{ 
        ...
        pub fun borrowFRC20SemiNFTPublic(id: UInt64): &FRC20SemiNFT.NFT{
            FRC20SemiNFT.IFRC20SemiNFT,
        ...
        /** ----- Write ----- */        access(contract) fun onVote(
            choice: Int,
            proposal: &Proposal{ProposalPublic}
        ){ 
            pre{ 
            ...
            let stakeTick = FRC20StakingManager.getPlatformStakingTickerName()
            let stakedNFTColRef =
                self.semiNFTColCap.borrow()
                ?? panic("The staked NFT collection is not found")
            let unlockingStakedBalance =
                stakedNFTColRef.getStakedBalance(tick: stakeTick)
            ...
            let lockedNFTIds =
                self.lockedSemiNFTCollection.getIDsByTick(tick: stakeTick)
    ...
    ///
    pub struct ChoiceSlotDetails{ 
        pub let message: String
        
        pub let command:{ FRC20VoteCommands.IVoteCommand}
    ...
    pub struct ProposalDetails{ 
        ...
        pub let slots: [ChoiceSlotDetails]
        
        pub let beginningTime: UFix64
        
        pub let endingTime: UFix64
        
        pub let executableThreshold: UFix64
        ...
        pub var discussionLink: String?
        
        pub var isCancelled: Bool
        ...
        /** ----- Write ----- */        access(contract) fun updateProposal(
            title: String?,
            description: String?,
            discussionLink: String?
        ){ 
        ...
        access(contract) fun cancelProposal(){ 
            pre{ 
                self.isCancelled == false:
    ...
    pub resource interface ProposalPublic{ 
        ...
        pub fun isEditable(): Bool{} 
        
        pub fun isFinalized(): Bool{} 
        
        pub fun getStatus(): ProposalStatus{} 
        
        pub fun getDetails(): ProposalDetails{} 
        
        pub fun getLogs(): [StatusLog]{} 
        
        pub fun getVotersAmount(): Int{} 
        
        pub fun getVoters(): [Address]{} 
        
        pub fun getTotalVotedPoints(): UFix64{} 
        
        pub fun getVotingChoices():{ Int: UFix64}{} 
        
        pub fun getWinningChoice(): Int?{} 
        
        pub fun isValidateForThreshold(): Bool{} 
        
        pub fun isVotingAllowed(): Bool{} 
        
        pub fun isVoteCommandsExecutable(): Bool{} 
        
        pub fun isWinningInscriptionsExecuted(): Bool{} 
        
        pub fun isChoiceInscriptionsExtracted(choice: Int): Bool{} 
        
        pub fun isVoted(
            _ semiNFT: &FRC20SemiNFT.NFT{FRC20SemiNFT.IFRC20SemiNFT}
        ): Bool{} 
        
        // --- Write Methods ---
        access(contract) fun vote(
            choice: Int,
            semiNFT: &FRC20SemiNFT.NFT{FRC20SemiNFT.IFRC20SemiNFT}
        ){} 
    }
    
    /// The struct of the FixesVotes proposal.
    ///
    pub resource Proposal: ProposalPublic, FixesHeartbeat.IHeartbeatHook{ 
        priv let proposer: Address
        
        priv let statusLog: [StatusLog]
        
        priv let details: ProposalDetails
        
        // ----- Voting status -----
        /// Address -> Points
        priv var votedAccounts:{ Address: UFix64}
        
        /// NFTId -> Bool
        priv var votedNFTs:{ UInt64: Bool}
        
        /// Vote choice -> Points
        priv var votes:{ Int: UFix64}
        ...
        /** ------ Public Methods ------ */        pub fun getProposer(): Address{ 
        ...
        ///
        pub fun isVoted(_ semiNFT: &FRC20SemiNFT.NFT{FRC20SemiNFT.IFRC20SemiNFT}): Bool{ 
            return self.votedNFTs[semiNFT.id] != nil
        }
        
        /** ------ Write Methods: Contract ------ *//// Vote on a proposal.
        ///
        access(contract) fun vote(choice: Int, semiNFT: &FRC20SemiNFT.NFT{FRC20SemiNFT.IFRC20SemiNFT}){ 
            pre{ 
        ...
        ///
        access(contract) fun executeSafe(): Bool{ 
            if self.getStatus() != ProposalStatus.Successed{ 
                return false
            }
            if let winningChoice = self.getWinningChoice(){ 
        ...
        ///
        access(contract) fun refundInscriptionCostSafe(): Bool{ 
            let status = self.getStatus()
        ...
        ///
        access(contract) fun refundInscriptionCost(){ 
            let result = self.refundInscriptionCostSafe()
            assert(result, message: "The proposal cannot be not refunded")
        }
        
        /** ------ Internal Methods: Implement Heartbeat ------ *//// Implement the heartbeat hook.
        ///
        ...
        access(contract) fun borrowDetails(): &ProposalDetails{ 
    ...
    pub resource interface VotesManagerPublic{ 
        pub            /// Check whether the proposer is valid.
            fun isValidProposer(_ voterAddr: Address): Bool{} 
        
        /** ------ Proposal Getter ------ */        pub fun getProposalLength(): Int{} 
        
        pub fun getProposalIds(): [UInt64]{} 
        
        pub fun getActiveProposalIds(): [UInt64]{} 
        
        pub fun getProposalIdsByTick(tick: String): [UInt64]{} 
        
        /// Borrow the proposal.
        pub fun borrowProposal(_ proposalId: UInt64): &Proposal{
            ProposalPublic
        }?{} 
        
        /** ------ Write Methods ------ *//// Create a new proposal.
        ...
        /// Vote on a proposal.
        pub fun vote(voter: &VoterIdentity, proposalId: UInt64, choice: Int){} 
        ...
        pub fun cancelProposal(voter: &VoterIdentity, proposalId: UInt64){} 
        
        pub fun executeProposal(voter: &VoterIdentity, proposalId: UInt64){} 
    }
    
    /// The resource of the FixesVotes manager.
    ///
    pub resource VotesManager:
        VotesManagerPublic,
        FixesHeartbeat.IHeartbeatHook{
    
        /// Voter -> Bool
        priv let whitelisted:{ Address: Bool}
        
        /// ProposalId -> Proposal
        priv let proposals: @{UInt64: Proposal}
        
        /// active proposal ids
        priv let activeProposalIds: [UInt64]
        
        /// Ticker -> ProposalId[]
        priv let proposalIdsByTick:{ String: [UInt64]}
        ...
        /** ----- Read ----- *//// Check whether the proposer is valid.
        ///
        pub fun isValidProposer(_ voterAddr: Address): Bool{ 
        ...
        pub fun createProposal(
            ...
            let voterAddr =
                voter.owner?.address ?? panic("Voter's owner is not found")
            assert(
        ...
        pub fun vote(voter: &VoterIdentity, proposalId: UInt64, choice: Int){ 
            let proposalRef =
                self.borrowProposal(proposalId)
        ...
        /** ------ Write Methods: Proposer ------- *//// Update the proposal.
        ///
        pub fun updateProposal(
            voter: &VoterIdentity,
            proposalId: UInt64,
            ...
            let proposalRef =
                self.borrowProposalRef(proposalId)
                ?? panic("The proposal is not found")
        ...
        /** ----- Write: Private ----- */        pub fun updateWhitelist(voter: Address, isWhitelisted: Bool){ 
            self.whitelisted[voter] = isWhitelisted
            emit VotesManagerWhitelistUpdated(
                voter: voter,
                isWhitelisted: isWhitelisted
            )
        }
        
        pub fun forceHeartbeat(){ 
        ...
        /** ------ Internal Methods ------ */        access(account) fun onHeartbeat(_ deltaTime: UFix64){ 
        ...
        /** ----- Internal ----- */        priv fun borrowProposalRef(_ proposalId: UInt64): &Proposal?{ 
            return &self.proposals[proposalId] as &Proposal?
        }
    }
    
    /* --- Public Functions --- */    pub fun createVoter(
        _ cap: Capability<
            &FRC20SemiNFT.Collection{
                FRC20SemiNFT.FRC20SemiNFTCollectionPublic,
                FRC20SemiNFT.FRC20SemiNFTBorrowable,
    ...
    ///
    pub fun getProposerStakingThreshold(): UFix64{ 
        return 0.15
    }
    
    /// Get the total staked amount.
    ///
    pub fun getTotalStakedAmount(): UFix64{ 
    ...
    ///
    pub fun borrowSystemInscriptionsStore(): &Fixes.InscriptionsStore{
        Fixes.InscriptionsStorePublic,
        Fixes.InscriptionsPublic
    }{ 
        let storePubPath = Fixes.getFixesStorePublicPath()
        return self.account.getCapability<
            &Fixes.InscriptionsStore{
                Fixes.InscriptionsStorePublic,
                Fixes.InscriptionsPublic
            }
        >(storePubPath).borrow()
    ...
    ///
    pub fun borrowVoterPublic(_ addr: Address): &VoterIdentity{
        VoterPublic,
        FRC20SemiNFT.FRC20SemiNFTCollectionPublic,
        NonFungibleToken.Receiver,
        NonFungibleToken.CollectionPublic
        ...
        >(self.VoterPublicPath).borrow()
    }
    
    /// Borrow the VotesManager resource.
    ///
    pub fun borrowVotesManager(): &VotesManager{VotesManagerPublic}{ 
        return self.account.getCapability<&VotesManager{VotesManagerPublic}>(
    ...
    init(){ 
        let votesIdentifier =
            "FRC20VotesManager_".concat(self.account.address.toString())
        self.FRC20VotesManagerStoragePath = StoragePath(
                identifier: votesIdentifier
            )!
        self.FRC20VotesManagerPublicPath = PublicPath(
                identifier: votesIdentifier
            )!
        ...
        self.account.save(
            <-create VotesManager(),
            to: self.FRC20VotesManagerStoragePath
        )
        self.account.link<
            &VotesManager{VotesManagerPublic, FixesHeartbeat.IHeartbeatHook}
        >(
            self.FRC20VotesManagerPublicPath,
        ...
        // @deprecated after Cadence 1.0
        if !self.account.getCapability<
            &Fixes.InscriptionsStore{
                Fixes.InscriptionsStorePublic,
                Fixes.InscriptionsPublic
            }
        >(insStorePubPath).check(){ 
            ...
            >(insStorePubPath, target: insStoreStoragePath)
        }
        let voterIdentifier =
            "FRC20Voter_".concat(self.account.address.toString())
