...
pub contract SHRDMintAccess{ 
    ...
    pub resource interface MintProxyPublic{ 
        pub fun setCapability(
            mintCapability: Capability<&MintGuard{MintGuardPrivate}>
        ){} 
    ...
    pub resource MintGuard: MintGuardPrivate, MintGuardPublic{ 
        ...
        priv let mintCapability: Capability<&SHRD.SHRDMinter{SHRD.SHRDMinterPrivate}>
    ...
    pub fun createMintGuard(
        adminRef: &Admin,
        privateMintCapability: Capability<
            &SHRD.SHRDMinter{SHRD.SHRDMinterPrivate}
        >,
        ...
        let mintGuard <-
            create MintGuard(
                privateMintCapability: privateMintCapability,
                max: max
            )
        let storagePath =
            self.getStoragePath(
        ...
        self.account.link<&MintGuard{MintGuardPrivate}>(
            privatePath,
            target: storagePath
        )
        self.account.link<&MintGuard{MintGuardPublic}>(
            publicPath,
            target: storagePath
        )
    ...
    pub fun createMintProxy(authAccount: AuthAccount){ 
        ...
        let storagePath =
            self.getStoragePath(
                address: address,
                objectType: MintObjectType.MintProxy
            )
        let privatePath =
            self.getPrivatePath(
                address: address,
        ...
        authAccount.link<&MintProxy{MintProxyPrivate}>(
            privatePath,
            target: storagePath
        )
        authAccount.link<&MintProxy{MintProxyPublic}>(
            publicPath,
            target: storagePath
        )
    }
    ...
    //
    pub fun getStoragePath(
        address: Address,
        objectType: MintObjectType
    ): StoragePath{ 
    ...
    pub fun getPublicProxyPath(address: Address): PublicPath{ 
        let index = self.addressToPathIndexMap[address]!
        return PublicPath(
            identifier: self.mintProxyPathPrefix.concat(index.toString())
        )!
    }
    
    pub fun getPublicPath(
        address: Address,
        objectType: MintObjectType
    ): PublicPath{ 
        let index = self.addressToPathIndexMap[address]!
