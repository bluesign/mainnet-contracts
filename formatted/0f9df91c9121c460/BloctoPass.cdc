...
pub contract BloctoPass: NonFungibleToken{ 
    ...
    pub resource NFT: NonFungibleToken.INFT, FungibleToken.Provider, FungibleToken.Receiver, BloctoPassPrivate, BloctoPassPublic{ 
        // BLT holder vault
        priv let vault: @BloctoToken.Vault
        
        // BLT staker handle
        priv let staker: @BloctoTokenStaking.Staker
        ...
        // key: timestamp
        // value: percentage of BLT that must remain in the BloctoPass at this timestamp
        priv let lockupSchedule:{ UFix64: UFix64}?
        ...
        init(initID: UInt64, originalOwner: Address?, metadata:{ String: String}, vault: @FungibleToken.Vault, lockupScheduleId: Int?, lockupSchedule:{ UFix64: UFix64}?){ 
            let stakingAdmin = BloctoPass.account.borrow<&BloctoTokenStaking.Admin>(from: BloctoTokenStaking.StakingAdminStoragePath) ?? panic("Could not borrow admin reference")
            self.id = initID
    ...
    pub resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, CollectionPublic, CollectionPrivate{ 
    ...
    pub resource NFTMinter: MinterPublic{ 
        ...
        pub fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, metadata:{ String: String}){ 
            self.mintNFTWithCustomLockup(recipient: recipient, metadata: metadata, vault: <-BloctoToken.createEmptyVault(), lockupSchedule:{ 0.0: 0.0})
        }
        
        pub fun mintNFTWithPredefinedLockup(recipient: &{NonFungibleToken.CollectionPublic}, metadata:{ String: String}, vault: @FungibleToken.Vault, lockupScheduleId: Int?){ 
        ...
        pub fun mintNFTWithCustomLockup(recipient: &{NonFungibleToken.CollectionPublic}, metadata:{ String: String}, vault: @FungibleToken.Vault, lockupSchedule:{ UFix64: UFix64}){ 
    ...
    init(){ 
        ...
        self.account.link<&{NonFungibleToken.CollectionPublic, BloctoPass.CollectionPublic}>(self.CollectionPublicPath, target: self.CollectionStoragePath)
