...
pub contract BloctoTokenStaking{ 
    /****** Staking Events ******/    pub event NewEpoch(
        epoch: UInt64,
        totalStaked: UFix64,
    ...
    /*********** Staking Composite Type Definitions *************//// Contains information that is specific to a staker
    pub resource StakerRecord{ 
        ...
        init(id: UInt64){ 
            pre{ 
                BloctoTokenStaking.stakers[id] == nil:
                    "The ID cannot already exist in the record"
            }
            self.id = id
            self.tokensCommitted <- BloctoToken.createEmptyVault()
                as!
                @BloctoToken.Vault
            self.tokensStaked <- BloctoToken.createEmptyVault()
                as!
                @BloctoToken.Vault
            self.tokensUnstaked <- BloctoToken.createEmptyVault()
                as!
                @BloctoToken.Vault
            self.tokensRewarded <- BloctoToken.createEmptyVault()
                as!
                @BloctoToken.Vault
            self.tokensRequestedToUnstake = 0.0
            emit NewStakerCreated(
                stakerID: self.id,
        ...
        destroy(){ 
            let bloctoTokenRef =
                BloctoTokenStaking.account.borrow<&BloctoToken.Vault>(
                    from: BloctoToken.TokenStoragePath
                )!
            BloctoTokenStaking.totalTokensStaked = BloctoTokenStaking
                    .totalTokensStaked
    ...
    pub struct StakerInfo{ 
        ...
        init(stakerID: UInt64){ 
            ...
            self.tokensRequestedToUnstake = stakerRecord
    ...
    pub resource Staker: StakerPublic{ 
        ...
        pub fun requestUnstaking(amount: UFix64){ 
            ...
            assert(stakerRecord.tokensStaked.balance + stakerRecord.tokensCommitted.balance >= amount + stakerRecord.tokensRequestedToUnstake, message: "Not enough tokens to unstake!")
    ...
    pub resource Admin{ 
        ...
        pub fun startNewEpoch(){ 
            ...
            BloctoTokenStaking.setEpoch(
                BloctoTokenStaking.getEpoch() + 1 as UInt64
            )
            emit NewEpoch(
                epoch: BloctoTokenStaking.getEpoch(),
                totalStaked: BloctoTokenStaking.getTotalStaked(),
                totalRewardPayout: BloctoTokenStaking.epochTokenPayout
            )
        }
        ...
        pub fun payRewards(_ stakerIDs: [UInt64]){ 
            pre{ 
                !BloctoTokenStaking.stakingEnabled:
                    "Cannot pay rewards if the staking auction is still in progress"
            }
            let BloctoTokenMinter =
                BloctoTokenStaking.account.borrow<&BloctoToken.Minter>(
                    from: /storage/bloctoTokenStakingMinter
                )
            ...
            var totalRewardScale =
                BloctoTokenStaking.epochTokenPayout / totalStaked
            let epoch = BloctoTokenStaking.getEpoch()
            let epochPath =
                BloctoTokenStaking.getStakingRewardPath(epoch: epoch)
            var stakingRewardRecordsRefOpt =
                BloctoTokenStaking.account.borrow<&{String: Bool}>(
                    from: epochPath
                )
    ...
    access(account) fun borrowStakerRecord(_ stakerID: UInt64): &StakerRecord{ 
        ...
        return (&BloctoTokenStaking.stakers[stakerID] as                                                         
    ...
    pub fun getEpoch(): UInt64{ 
        return self.account.copy<UInt64>(from: /storage/bloctoTokenStakingEpoch)
        ?? 0 as UInt64
    }
    ...
    access(contract) fun getStakingRewardKey(
        epoch: UInt64,
        stakerID: UInt64
    ): String{ 
    ...
    access(contract) fun getStakingRewardPath(epoch: UInt64): StoragePath{ 
        return StoragePath(
            identifier: "bloctoTokenStakingStakingRewardRecords".concat("_")
    ...
    pub fun hasSentStakingReward(epoch: UInt64, stakerID: UInt64): Bool{ 
        let stakingRewardRecordsRef =
            self.account.borrow<&{String: Bool}>(
                from: BloctoTokenStaking.getStakingRewardPath(epoch: epoch)
            )
        if stakingRewardRecordsRef == nil{ 
            return false
        }
        let key =
            BloctoTokenStaking.getStakingRewardKey(
                epoch: epoch,
                stakerID: stakerID
            )
