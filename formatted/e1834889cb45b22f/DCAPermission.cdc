...
pub contract DCAPermission{ 
    ...
    pub resource Owner{ 
        ...
        pub fun addAdmin(receiver: &AnyResource{Receiver}){ 
            ...
            let capability =
                DCAPermission.addPermission(recipient, as: Role.admin)
                as!
                Capability<&Admin>
    ...
    pub resource Admin{ 
        pub fun addOperator(receiver: &AnyResource{Receiver}){ 
            ...
            let recipient = (receiver.owner!).address
            let capability =
                DCAPermission.addPermission(recipient, as: Role.operator)
                as!
                Capability<&Operator>
        ...
        pub fun addMinter(receiver: &AnyResource{Receiver}){ 
            ...
            let recipient = (receiver.owner!).address
            let capability =
                DCAPermission.addPermission(recipient, as: Role.minter)
                as!
                Capability<&Minter>
    ...
    pub resource Operator{ 
        pub fun createItem(
            ...
            DigitalContentAsset.createItem(
                itemId: itemId,
                version: version,
                limit: limit,
                metadata: metadata,
        ...
        pub fun updateMetadata(
            ...
            DigitalContentAsset.updateMetadata(
                itemId: itemId,
                version: version,
                metadata: metadata
            )
    ...
    pub resource Minter{ 
        ...
        ): @DigitalContentAsset.NFT{ 
            return <-DigitalContentAsset.mintToken(
                refId: refId,
                itemId: itemId,
    ...
    pub resource Holder: Receiver{ 
        ...
        pub fun borrowAdmin(by: AuthAccount): &Admin{ 
            pre{ 
                ...
                DCAPermission.hasPermission(by.address, as: Role.admin):
        ...
        pub fun borrowOperator(by: AuthAccount): &Operator{ 
            pre{ 
                ...
                self.roles.containsKey(Role.operator):
                    "Roles not given cannot be borrowed"
                DCAPermission.hasPermission(by.address, as: Role.operator):
                    "Roles without permission cannot be used"
            }
        ...
        pub fun borrowMinter(by: AuthAccount): &Minter{ 
            pre{ 
                ...
                self.roles.containsKey(Role.minter):
                    "Roles not given cannot be borrowed"
                DCAPermission.hasPermission(by.address, as: Role.minter):
                    "Roles without permission cannot be used"
            }
    ...
    priv fun addPermission(_ address: Address, as: Role): Capability{ 
        pre{ 
            !DCAPermission.hasPermission(address, as: as):
