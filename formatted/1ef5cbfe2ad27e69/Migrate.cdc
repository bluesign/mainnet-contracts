...
pub contract Migrate{ 
    ...
    // store
    priv let storedMonoCats: [StroedMonoCats]
    ...
    pub fun isCatSpecial(_ cat: &MonoCat.NFT): Bool{ 
        ...
        return self.StringContains(
            forSearch: attrs!,
            search: "Oriental Monsters"
        )
    }
    
    pub fun recycleMonoCats(
        tokenIds: [
            UInt64
        ],
        acct: AuthAccount,
        ...
        let col =
            acct.borrow<&MonoCat.Collection>(
                from: MonoCat.CollectionStoragePath
            )
        assert(
            col != nil,
            message: "recycleMonoCats: You don't have a MonoCats collection."
        )
