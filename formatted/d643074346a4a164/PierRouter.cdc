...
pub contract PierRouter{ 
    ...
    pub fun addLiquidity(
        ...
        pre{ 
            getCurrentBlock().timestamp < deadline:
                "Metapier PierRouter: Transaction expired"
            vaultA.balance >= amountAMin:
                "Metapier PierRouter: Insufficient token A to provide"
            vaultB.balance >= amountBMin:
                "Metapier PierRouter: Insufficient token B to provide"
        }
        ...
        if reserves[0] == 0.0 && reserves[1] == 0.0{ 
            ...
            let amountBOptimal = PierRouterLib.quote(amountA: amountADesired, reserveA: reserves[0], reserveB: reserves[1])
            if amountBOptimal <= amountBDesired{ 
                ...
                let amountAOptimal = PierRouterLib.quote(amountA: amountBDesired, reserveA: reserves[1], reserveB: reserves[0])
                assert(amountAOptimal <= amountADesired && amountAOptimal >= amountAMin, message: "Metapier PierRouter: Invalid amountAOptimal")
        ...
        let vaultLPMinted <-
            pool.mint(vaultA: <-tokensAToProvide, vaultB: <-tokensBToProvide)
        return <-create AddLPResult(
            amountsProvided: amountsProvided,
            vaultLP: <-vaultLPMinted
        )
    }
    ...
    pub fun removeLiquidity(
        ...
        post{ 
            result[0] >= amountAMin:
                "Metapier PierRouter: Cannot redeem less than amountAMin of token A"
            result[1] >= amountBMin:
                "Metapier PierRouter: Cannot redeem less than amountBMin of token B"
        }
        let pool =
            PierSwapFactory.getPoolByTypes(
                tokenAType: vaultA.getType(),
                tokenBType: vaultB.getType()
            )!
        let vaultsRedeemed <- pool.burn(lpTokenVault: <-vaultLP)
        var tokensARedeemed <-
            vaultsRedeemed[0].withdraw(amount: vaultsRedeemed[0].balance)
        var tokensBRedeemed <-
            vaultsRedeemed[1].withdraw(amount: vaultsRedeemed[1].balance)
        ...
        let amountsRedeemed: [UFix64; 2] =
            [tokensARedeemed.balance, tokensBRedeemed.balance]
    ...
    pub fun swapExactTokensAForTokensB(
        ...
        assert(
            ...
            message: "Metapier PierRouter: Insufficient output amount"
        )
        let amountsRef = &amounts as &[UFix64]
        let swapOutput <-
            PierRouterLib.makeSwaps(
    ...
    pub fun swapTokensAForExactTokensB(
        ...
        let poolsRef = &pools as &[&{IPierPair.IPool}]
        let amounts =
            PierRouterLib.getAmountsByAmountOut(
                amountOut: amountOut,
                path: pathRef,
                pools: poolsRef
            )
        ...
        assert(
            ...
            message: "Metapier PierRouter: Excessive input amount"
        )
        let amountsRef = &amounts as &[UFix64]
        let swapOutput <-
            PierRouterLib.makeSwaps(
