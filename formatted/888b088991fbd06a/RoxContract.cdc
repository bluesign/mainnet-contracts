...
pub contract RoxContract: NonFungibleToken{ 
    ...
    priv var boxes: @{UInt32: Box}
    
    // The number of Rox NFTs each box contains
    priv var mintedNumberPerBox:{ UInt32: UInt32}
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &RoxContract.NFT?{ 
        let collection = getAccount(from).getCapability(RoxContract.CollectionPublicPath).borrow<&RoxContract.Collection{RoxContract.RoxCollectionPublic}>() ?? panic("Couldn't get collection")
