...
pub contract Geeft: NonFungibleToken{ 
    ...
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver{ 
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                case Type<MetadataViews.Display>():
                    return MetadataViews.Display(name: "Geeft #".concat(self.id.toString()), description: self.message ?? (self.from == nil ? "This is a Geeft." : "This is a Geeft from ".concat((self.from!).toString()).concat(".")), thumbnail: MetadataViews.HTTPFile(url: "https://i.imgur.com/dZxbOEa.png"))
            }
            return nil
        }
        ...
        destroy(){ 
            pre{ 
                self.storedNFTs.keys.length == 0:
                    "There are still NFTs left in this Geeft."
                self.storedTokens.length == 0:
                    "There are still tokens left in this Geeft."
            }
    ...
    pub fun sendGeeft(from: Address?, message: String?, nfts: @{String: [NonFungibleToken.NFT]}, tokens: @{String: FungibleToken.Vault}, extra:{ String: AnyStruct}, recipient: Address){ 
        let geeft <- create NFT(from: from, message: message, nfts: <-nfts, tokens: <-tokens, extra: extra)
        let collection = getAccount(recipient).getCapability(Geeft.CollectionPublicPath).borrow<&Collection{NonFungibleToken.Receiver}>() ?? panic("The recipient does not have a Geeft Collection")
