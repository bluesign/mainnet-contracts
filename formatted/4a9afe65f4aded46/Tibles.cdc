...
pub contract Tibles: NonFungibleToken, TiblesApp, TiblesNFT, TiblesProducer{ 
    ...
    pub resource NFT: NonFungibleToken.INFT, TiblesNFT.INFT, MetadataViews.Resolver{ 
        ...
        pub fun displayData():{ String: String}{ 
            ...
            if metadata.containsKey("pack"){ 
                return{ "name": "Tibles pack", "description": "A Tibles pack", "imageUrl": "https://i.tibles.com/H65uAeuNyq.png"}
            }
            let set = metadata["set"]! as! &Tibles.Set
            let item = metadata["item"]! as! &Tibles.Item
            let variant = metadata["variant"]! as! &Tibles.Variant
            var edition: String = ""
        ...
        pub fun editions(): MetadataViews.Editions{ 
            ...
            let metadata = self.metadata() ?? panic("Missing NFT metadata")
            if metadata.containsKey("pack"){ 
                return MetadataViews.Editions([MetadataViews.Edition(name: "Tibles pack", number: UInt64(self.mintNumber), max: nil)])
            }
            let variant = metadata["variant"]! as! &Tibles.Variant
        ...
        pub fun externalURL(): MetadataViews.ExternalURL{ 
            return MetadataViews.ExternalURL("https://tibles.com/collection".concat(self.id.toString()))
        }
        
        pub fun nftCollectionData(): MetadataViews.NFTCollectionData{ 
            return MetadataViews.NFTCollectionData(storagePath: Tibles.CollectionStoragePath, publicPath: Tibles.PublicCollectionPath, providerPath: /private/TiblesCollection, publicCollection: Type<&Tibles.Collection{TiblesNFT.CollectionPublic}>(), publicLinkedType: Type<&Tibles.Collection{TiblesNFT.CollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&Tibles.Collection{TiblesNFT.CollectionPublic, NonFungibleToken.Provider, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
                    return <-Tibles.createEmptyCollection()
                })
        }
        
        pub fun nftCollectionDisplay(): MetadataViews.NFTCollectionDisplay{ 
            let squareMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://i.tibles.com/m/tibles-flow-icon.png"), mediaType: "image/svg+xml")
            let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://i.tibles.com/m/tibles-flow-collection-banner.png"), mediaType: "image/png")
            ...
            return MetadataViews.NFTCollectionDisplay(name: "Tibles Collection", description: "Tibles collectibles", externalURL: MetadataViews.ExternalURL("https://app.tibles.com"), squareImage: squareMedia, bannerImage: bannerMedia, socials: socials)
        }
    ...
    pub fun createNewPackMinter(id: String, contentCapability: Capability): @PackMinter{ 
        assert(self.account.address == 0x4a9afe65f4aded46, message: "wrong address")
        return <-create PackMinter(id: id, contentCapability: contentCapability)
    }
    
    init(){ 
        self.totalSupply = 0
        self.appId = "com.tibles.tibles"
        self.title = "Tibles"
        self.description = "Tibles Collectibles"
        self.ProducerStoragePath = /storage/TiblesTiblesProducer
        self.ProducerPath = /private/TiblesTiblesProducer
        self.ContentPath = /public/TiblesTiblesContent
        self.CollectionStoragePath = /storage/TiblesTiblesCollection
        self.PublicCollectionPath = /public/TiblesTiblesCollection
