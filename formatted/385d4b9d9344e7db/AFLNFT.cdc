...
pub contract AFLNFT: NonFungibleToken{ 
    ...
    access(account) fun mintNFT(templateId: UInt64, account: Address){ 
        ...
        let recipientCollection = receiptAccount.getCapability(AFLNFT.CollectionPublicPath).borrow<&{AFLNFT.AFLNFTCollectionPublic}>() ?? panic("Could not get receiver reference to the NFT Collection")
