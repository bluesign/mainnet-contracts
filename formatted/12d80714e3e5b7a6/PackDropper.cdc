...
pub contract PackDropper{ 
    ...
    pub resource PacksHandler: PackPurchaser, PacksInfo{ 
        priv var packStats:{ UInt32: PackData}
        ...
        pub fun buyPack(packId: UInt32, buyerPayment: @FungibleToken.Vault, buyerAddress: Address){ 
            pre{ 
                self.packStats[packId] != nil:
                ...
                (self.owner!).getCapability<&FUSD.Vault{FungibleToken.Receiver}>(/public/fusdReceiver).borrow() != nil:
    ...
    init(){ 
        ...
        self.account.link<&PackDropper.PacksHandler{PackDropper.PackPurchaser}>(
            self.PacksHandlerPublicPath,
            target: self.PacksHandlerStoragePath
        )
        self.account.link<&PackDropper.PacksHandler{PackDropper.PacksInfo}>(
            self.PacksInfoPublicPath,
            target: self.PacksHandlerStoragePath
        )
