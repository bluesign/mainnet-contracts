...
pub contract OlympicPinMarket{ 
    ...
    pub resource interface SalePublic{ 
        ...
        pub fun purchase(
            pieceId: UInt64,
            buyTokens: @NWayUtilityCoin.Vault
        ): @OlympicPin.NFT{ 
            post{ 
                result.id == pieceId:
                    "The ID of the withdrawn piece must be the same as the requested pieceId"
            }
        }
    ...
    pub resource SaleCollection: SalePublic{ 
        ...
        priv var ownerCollection: Capability<&OlympicPin.Collection>
        ...
        init(ownerCollection: Capability<&OlympicPin.Collection>, ownerCapability: Capability<&{FungibleToken.Receiver}>, beneficiaryCapability: Capability<&{FungibleToken.Receiver}>, cutPercentage: UFix64){ 
            pre{ 
        ...
        pub fun changePercentage(_ newPercent: UFix64){ 
            pre{ 
                newPercent <= 1.0:
                    "Cannot set cut percentage to more than 100%"
            }
    ...
    pub fun createSaleCollection(
        ownerCollection: Capability<&OlympicPin.Collection>,
