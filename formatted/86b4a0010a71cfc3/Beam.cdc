...
pub contract Beam: NonFungibleToken{ 
    ...
    pub struct SetData{ 
        ...
        init(setID: UInt32){ 
            ...
            let referencedSet = (&Beam.sets[setID] as &Set?)!
    ...
    pub resource Set{ 
        ...
        pub fun addCollectibleItem(collectibleItemID: UInt32){ 
            pre{ 
                Beam.collectibleItemDatas[collectibleItemID] != nil:
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                case Type<MetadataViews.Display>():
                    ...
                    // assume `ipfs://`
                    var url = metadata["posterUrl"] ?? "ipfs://bafybeichtxzrocxo7ec5qybfxxlyod5bbymblitjwb2aalv2iyhe42pk4e/Frightclub.jpg"
                    let prefixes = ["ipfs://", "http://", "https:/"]
                    if !prefixes.contains(url.slice(from: 0, upTo: 7)){ 
                        url = prefixes[0].concat(url)
                    }
                    return MetadataViews.Display(name: metadata["title"] ?? "Frightclub NFT", description: metadata["description"] ?? "Official Frightclub NFT", thumbnail: MetadataViews.HTTPFile(url: url))
                case Type<MetadataViews.Royalties>():
                    let receiver = Beam.account.getCapability<&AnyResource{FungibleToken.Receiver}>(/public/flowTokenReceiver)
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: Beam.CollectionStoragePath, publicPath: Beam.CollectionPublicPath, providerPath: /private/BeamCollection001, publicCollection: Type<&Beam.Collection{Beam.BeamCollectionPublic}>(), publicLinkedType: Type<&Beam.Collection{Beam.BeamCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&Beam.Collection{Beam.BeamCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Provider, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
                            return <-Beam.createEmptyCollection()
                        })
                case Type<MetadataViews.NFTCollectionDisplay>():
                    return MetadataViews.NFTCollectionDisplay(name: "Frightclub", description: "Frightclub collection", externalURL: MetadataViews.ExternalURL(url: "https://frightclub.io"), squareImage: MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://frightclub.niftory.com/fc_black_logo_round.svg"), mediaType: "image/svg+xml"), bannerImage: MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://frightclub.niftory.com/fc_black_logo_round.svg"), mediaType: "image/svg+xml"), socials:{} )
                case Type<MetadataViews.ExternalURL>():
                    return MetadataViews.ExternalURL(url: "https://frightclub.io")
            }
            return nil
        }
    ...
    pub resource Collection: BeamCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection{ 
