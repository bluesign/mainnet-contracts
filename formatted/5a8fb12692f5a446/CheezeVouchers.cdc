...
pub contract CheezeVouchers{ 
    ...
    priv var cutPercent: UFix64
    
    priv var cutReceiver: Capability<&FUSD.Vault{FungibleToken.Receiver}>
    
    priv var listings:{ UInt64: ListingDetails}
    
    priv var storedNFTs: @{UInt64: [CheezeNFT.NFT]}
    
    priv var vouchersAvailable:{ UInt64: UInt64}
    
    priv var voucherUsed:{ UInt64: Bool}
    ...
    pub resource Administrator{ 
        pub fun fulfillVoucher(
            address: Address,
            voucherId: UInt64,
            random: UInt64
        ){ 
            pre{ 
                (getAccount(address).getCapability<&CheezeVouchers.VoucherCollection>(/public/VoucherCollection).borrow()!).getIDs().contains(voucherId)
            ...
            let voucherCollection =
                getAccount(address).getCapability<
                    &CheezeVouchers.VoucherCollection
                >(/public/VoucherCollection).borrow()!
            let listingId =
                voucherCollection.listingIdForVoucherWithId(
                    voucherId: voucherId
                )
            ...
            let receiver =
                getAccount(address).getCapability<
                    &CheezeNFT.Collection{NonFungibleToken.Receiver}
                >(/public/CheezeNFTReceiver).borrow()!
    ...
    pub resource ListingCreator{ 
        ...
        pub fun putOnSale(tokens: @[CheezeNFT.NFT]){ 
            var i = 0 as UInt64
    ...
    pub struct ListingDetails{ 
        ...
        pub var sellerPaymentReceiver: Capability<
            &FUSD.Vault{FungibleToken.Receiver}
        >
    ...
    init(){ 
        ...
        self.cutReceiver = self.account.getCapability<
                &FUSD.Vault{FungibleToken.Receiver}
            >(/public/fusdReceiver)!
