...
pub contract CheezeMarket{ 
    ...
    priv var firstSaleCutPercent: UFix64
    
    priv var royaltyCutPercent: UFix64
    
    priv var maxRoyaltyPercent: UFix64
    ...
    pub struct ListingDetails{ 
        ...
        pub var sellerNftReturnReceiver: Capability<
            &CheezeNFT.Collection{NonFungibleToken.Receiver}
        >
        
        pub var royaltyReceiver: Capability<
            &FUSD.Vault{FungibleToken.Receiver}
        >?
        ...
        init(
            ...
            sellerPaymentReceiver: Capability<
                &FUSD.Vault{FungibleToken.Receiver}
            >,
            sellerNftReturnReceiver: Capability<
                &CheezeNFT.Collection{NonFungibleToken.Receiver}
            >,
    ...
    access(contract) fun revokeSale(tokenID: UInt64){ 
        ...
        let token <- self.storedNFTs.remove(key: tokenID)!
        ((self.listings[tokenID]!).sellerNftReturnReceiver.borrow()!).deposit(
            token: <-token
        )
        self.listings[tokenID] = nil
