...
pub contract TheFabricantFBRCAirdrop{ 
    pub event AddressClaimAdded(
        address: Address,
        claimName: String,
        claimAmount: UFix64
    )
    
    pub event AddressClaimed(
        address: Address,
        claimName: String,
        claimAmount: UFix64
    )
    ...
    priv var addressClaim:{ Address:{ String: ClaimStruct}}
    ...
    pub resource Admin{ 
        ...
        pub fun setFBRCMinterCap(
            fbrcMinterCap: Capability<&FBRC.Administrator>
        ){ 
            pre{ 
    ...
    pub resource Claimer{ 
        ...
        pub fun claimFBRC(
            ...
            let claimAmount =
                (
                    (TheFabricantFBRCAirdrop.addressClaim[claimAddress]!)[
                        claimName
                    ]!
                ).claimAmount
            let claimStruct =
            ...
            let fbrcAdmin =
                (TheFabricantFBRCAirdrop.fbrcMinterCapability!).borrow()!
            let fbrcMinter <-
                fbrcAdmin.createNewMinter(allowedAmount: claimAmount)
            let mintedVault <- fbrcMinter.mintTokens(amount: claimAmount)
            let recipientFBRCVault =
                fbrcCap.borrow() ?? panic("FBRC Vault Capability invalid")
