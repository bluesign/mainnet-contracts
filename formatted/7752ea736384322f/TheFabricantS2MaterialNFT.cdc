...
pub contract TheFabricantS2MaterialNFT: NonFungibleToken{ 
    ...
    pub struct MaterialData{ 
        ...
        priv let royalty:{ String: Royalty}
    ...
    //
    pub resource Admin{ 
        pub fun createMaterialData(designerAddress: Address, metadata:{ String: String}, royalty:{ String: Royalty}): UInt32{ 
        ...
        pub fun updateMaterialMetadata(id: UInt32, key: String, value: String){ 
            assert(TheFabricantS2MaterialNFT.materialDatas[id] != nil, message: "garment data does not exist")
        ...
        pub fun mintNFT(materialDataID: UInt32): @NFT{ 
            let numInMaterial = TheFabricantS2MaterialNFT.numberMintedPerMaterial[materialDataID] ?? panic("no materialDataID found")
        ...
        pub fun retireMaterialData(materialDataID: UInt32){ 
            pre{ 
                TheFabricantS2MaterialNFT.isMaterialDataRetired[materialDataID] != nil:
    ...
    pub fun getMaterialNumberMinted(id: UInt32): UInt32{ 
        let numberMinted = TheFabricantS2MaterialNFT.numberMintedPerMaterial[id] ?? panic("materialDataID not found")
        return numberMinted
    }
    
    // get the materialData of a specific id
    pub fun getMaterialData(id: UInt32): MaterialData{ 
        let materialData = TheFabricantS2MaterialNFT.materialDatas[id] ?? panic("materialDataID not found")
        return materialData
    }
    ...
    pub fun getMaterialDataRetired(materialDataID: UInt32): Bool{ 
        let isMaterialDataRetired = TheFabricantS2MaterialNFT.isMaterialDataRetired[materialDataID] ?? panic("materialDataID not found")
    ...
    init(){ 
        ...
        self.totalSupply = 0
        self.CollectionPublicPath = /public/S2MaterialCollection0028
        self.CollectionStoragePath = /storage/S2MaterialCollection0028
        self.AdminStoragePath = /storage/S2MaterialAdmin0028
