pub contract PrimalRaveVariantMintLimits{ 
    ...
    pub resource Admin{ 
        ...
        pub fun setVariantMintsForAddress(
            address: Address,
            variantId: UInt64,
            ...
            if let variantMintsByAddress:{ UInt64: UInt64} =
                ...
                PrimalRaveVariantMintLimits.addressVariantMints[
                    address
                ] = variantMintsByAddress
            } else{ 
    ...
    pub fun checkAddressCanMintVariant(
        address: Address,
        variantId: UInt64
    ): Bool{ 
        pre{ 
        ...
        let variantMintsByAddress:{ UInt64: UInt64}? =
            self.addressVariantMints[address]
    ...
    pub fun incrementVariantMintsForAddress(
        address: Address,
        variantId: UInt64
    ){ 
        if let variantMintsByAddress:{ UInt64: UInt64} =
    ...
    pub fun getVariantMintsForAddress(address: Address, variantId: UInt64):{ 
        UInt64: UInt64
    }?{ 
        if let variantMintsByAddress:{ UInt64: UInt64} =
            self.addressVariantMints[address]{ 
    ...
    pub fun getVariantMintsAndLimitsForAddress(address: Address):{ 
        UInt64:{ 
            String: UInt64?
        }
    }{ 
