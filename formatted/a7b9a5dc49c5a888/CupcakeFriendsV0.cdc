pub contract CupcakeFriendsV0{ 
    ...
    pub resource Cupcake: FriendlyCupcake, NotYetKnownCupcake, MyCupcake{ 
        ...
        pub fun getFriendRequests(): [CupcakeState]{ 
            ...
            for key in self.openFriendshipRequests.keys{ 
                ...
                requestStates.append(CupcakeState(id: otherRequest.getId(), accessoryId: otherRequest.getAccessory()))
            }
    ...
    pub resource interface MyCupcake{ 
        pub fun createFriendshipRequest(
            myCupcake: Capability<&AnyResource{NotYetKnownCupcake}>,
            otherCupcakeId: Int,
            time: UInt64
        ): @CupcakeFriendshipRequest{} 
        
        pub fun acceptBothSides(
            myCupcake: Capability<&AnyResource{NotYetKnownCupcake}>,
            friendCupcake: Int,
            time: UInt64
        ){} 
        
        pub fun denyBothSides(
            myCupcake: Capability<&AnyResource{NotYetKnownCupcake}>,
            friendCupcake: Int,
            time: UInt64
        ){} 
    ...
    pub resource interface NotYetKnownCupcake{ 
        pub fun depositFriendshipRequest(
            request: @CupcakeFriendshipRequestHolder
        ){} 
    ...
    pub resource CupcakeFriendshipRequestHolder{ 
        ...
        init(
            request: @CupcakeFriendshipRequest,
            acceptor: @CupcakeFriendshipAcceptor
        ){ 
    ...
    pub resource CupcakeFriendShip{ 
        ...
        init(
            ...
            acceptorId: Int,
            creationTime: UInt64,
            requestorCupcake: Capability<&AnyResource{NotYetKnownCupcake}>
        ){ 
        ...
        pub fun getRequestorCupcake(): Capability<
            &AnyResource{NotYetKnownCupcake}
        >{ 
    ...
    pub resource CupcakeFriendshipAcceptor{ 
        ...
        init(
            requestorId: Int,
            cupcake: Capability<&AnyResource{FriendlyCupcake}>
        ){ 
            self.requestorId = requestorId
        ...
        pub fun accept(request: @CupcakeFriendshipRequest, time: UInt64){ 
            ...
            let cupcake =
                self.cupcake.borrow() ?? panic("Cannot borrow Cupcake")
    ...
    pub resource CupcakeCollection:
        CupcakeReceiver,
        FriendlyCupcake,
        NotYetKnownCupcake,
        MyCupcake{
        ...
        pub fun removeRequestedFriendship(otherCupcakeId: Int){ 
            self.getCupcake().removeRequestedFriendship(
                otherCupcakeId: otherCupcakeId
            )
        }
        
        pub fun storeRequestedFriendship(otherCupcakeId: Int){ 
            self.getCupcake().storeRequestedFriendship(
                otherCupcakeId: otherCupcakeId
            )
        }
        ...
        pub fun createFriendshipRequest(
            ...
            return <-self.getCupcake().createFriendshipRequest(
                myCupcake: myCupcake,
                otherCupcakeId: otherCupcakeId,
                time: time
            )
        ...
        pub fun acceptBothSides(
            ...
            self.getCupcake().acceptBothSides(
                myCupcake: myCupcake,
                friendCupcake: friendCupcake,
                time: time
            )
        }
    ...
    pub fun requestFriendship(
        myCupcakeRef: Capability<&AnyResource{MyCupcake}>,
        notYetKnownMyCupcakeRef: Capability<&AnyResource{NotYetKnownCupcake}>,
        friendlyCupcakeRef: Capability<&AnyResource{FriendlyCupcake}>,
        otherCupcakeRef: Capability<&AnyResource{NotYetKnownCupcake}>,
        time: UInt64
    ){ 
        let myCupcake =
            myCupcakeRef.borrow() ?? panic("Cannot borrow my Cupcake")
        let otherCupcake =
            otherCupcakeRef.borrow() ?? panic("Cannot borrow other Cupcake")
        let request <-
        ...
        let holder <-
            create CupcakeFriendshipRequestHolder(
                request: <-request,
                acceptor: <-acceptor
            )
    ...
    init(){ 
        self.CollectionStoragePath = /storage/barcampCupcakeCollectionV0
        self.CollectionPrivatePath = /private/barcampCupcakeCollectionV0
        self.CollectionPublicPath = /public/barcampCupcakeCollectionV0
        self.MinterStoragePath = /storage/barcampCupcakeMinterV0
