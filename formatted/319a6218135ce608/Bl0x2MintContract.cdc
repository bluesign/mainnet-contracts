...
pub contract Bl0x2MintContract{ 
    ...
    pub fun paymentMint(_ signer: AuthAccount, count: UInt64){ 
        ...
        var paymentMintStartTime =
            self.extraFields["paymentMintStartTime"] as! UFix64?
        var paymentMintEndTime =
            self.extraFields["paymentMintEndTime"] as! UFix64?
        var currentTokenId =
            UInt64(self.extraFields["currentTokenId"] as! Number? ?? 0)
        var maxTokenId =
            UInt64(self.extraFields["maxTokenId"] as! Number? ?? 2221)
        ...
        let receiver1 =
            getAccount(receiverAddr1).getCapability<&{FungibleToken.Receiver}>(
                tokenReceiverPath
            ).borrow()
            ?? panic("Cannot borrow FungibleToken receiver")
        let receiver2 =
            getAccount(receiverAddr2).getCapability<&{FungibleToken.Receiver}>(
                tokenReceiverPath
            ).borrow()
            ?? panic("Cannot borrow FungibleToken receiver")
        ...
        // start minting
        let minter =
            self.account.borrow<&Bl0x2.NFTMinter>(
                from: Bl0x2.MinterStoragePath
            )!
    ...
    pub fun freeMint(_ signer: AuthAccount){ 
        var opened = self.extraFields["opened"] as! Bool? ?? false
        if !opened{ 
            panic("sale closed")
        }
