pub contract Components{ 
    ...
    pub struct Accessories: Component{ 
        ...
        pub fun build(components:{ String:{ Component}}, colors: Colors): String{ 
            let content = Components.account.borrow<&[String]>(from: StoragePath(identifier: "accessories_".concat(self.name))!) ?? panic("accessory not found")
            switch self.name{ 
                case "eyepatch":
                    return content[0]
                default:
    ...
    pub struct Clothing: Component{ 
        ...
        pub fun build(components:{ String:{ Component}}, colors: Colors): String{ 
            let content = Components.account.borrow<&[String]>(from: StoragePath(identifier: "clothing_".concat(self.name))!) ?? panic("clothing not found")
            switch self.name{ 
                case "graphicShirt":
    ...
    pub struct ClothingGraphic: Component{ 
        ...
        pub fun build(components:{ String:{ Component}}, colors: Colors): String{ 
            let content = Components.account.borrow<&[String]>(from: StoragePath(identifier: "clothingGraphic_".concat(self.name))!) ?? panic("clothing not found")
            return content[0]
        }
    ...
    pub struct Eyebrows: Component{ 
        ...
        pub fun build(components:{ String:{ Component}}, colors: Colors): String{ 
            let content = Components.account.borrow<&[String]>(from: StoragePath(identifier: "eyebrows_".concat(self.name))!) ?? panic("eyebrows not found")
            return content[0]
        }
    ...
    pub struct Eyes: Component{ 
        ...
        pub fun build(components:{ String:{ Component}}, colors: Colors): String{ 
            let content = Components.account.borrow<&[String]>(from: StoragePath(identifier: "eyes_".concat(self.name))!) ?? panic("eyes not found")
            return content[0]
        }
    ...
    pub struct FacialHair: Component{ 
        ...
        pub fun build(components:{ String:{ Component}}, colors: Colors): String{ 
            let content = Components.account.borrow<&[String]>(from: StoragePath(identifier: "facialHair_".concat(self.name))!) ?? panic("facialHair not found")
    ...
    pub struct Mouth: Component{ 
        ...
        pub fun build(components:{ String:{ Component}}, colors: Colors): String{ 
            let content = Components.account.borrow<&[String]>(from: StoragePath(identifier: "mouth_".concat(self.name))!) ?? panic("mouth not found")
            return content[0]
        }
    ...
    pub struct Nose: Component{ 
        ...
        pub fun build(components:{ String:{ Component}}, colors: Colors): String{ 
            let content = Components.account.borrow<&[String]>(from: StoragePath(identifier: "nose_".concat(self.name))!) ?? panic("nose not found")
            return content[0]
        }
    ...
    pub struct Style: Component{ 
        ...
        pub fun build(components:{ String:{ Component}}, colors: Colors): String{ 
            let content = Components.account.borrow<&[String]>(from: StoragePath(identifier: "style_".concat(self.name))!) ?? panic("style not found")
    ...
    pub struct Top: Component{ 
        ...
        pub fun build(components:{ String:{ Component}}, colors: Colors): String{ 
            let content = Components.account.borrow<&[String]>(from: StoragePath(identifier: "top_".concat(self.name))!) ?? panic("top not found")
            switch self.name{ 
                case "hat":
    ...
    pub struct Renderer{ 
        ...
        pub fun build(): String{ 
            let content =
                Components.account.borrow<&[String]>(
            ...
            let tmp =
                content[0].concat(self.colors.skin).concat(content[1]).concat(
                    ...
                    self.components["nose"]?.build(
                        components: self.components,
                        colors: self.colors
                    )
                    ?? ""
                ).concat(content[4]).concat(
                    self.components["eyes"]?.build(
                        components: self.components,
                        colors: self.colors
                    )
                    ?? ""
    ...
    pub resource Admin{ 
        ...
        pub fun createRandom(): Renderer{ 
            ...
            components["eyebrows"] = Eyebrows(
                    self.rollOption(segment: "eyebrows")
                )
            components["top"] = Top(self.rollOption(segment: "top"))
            components["facialHair"] = FacialHair(
                    self.rollOption(segment: "facialHair")
                )
            components["accessories"] = Accessories(
                    self.rollOption(segment: "accessories")
                )
        ...
        pub fun registerContent(
            ...
            let storagePath =
                StoragePath(identifier: component.concat("_").concat(name))!
