...
//
pub contract KeeprNFTStorefront{ 
    // KeeprNFTStorefrontInitialized
    // This contract has been deployed.
    // Event consumers can now expect events from this contract.
    //
    pub event KeeprNFTStorefrontInitialized()
    ...
    // KeeprNFTStorefront workflow, so be careful to check when using them.
    //
    ...
    pub struct ListingDetails{ 
        ...
        pub var saleCuts: [SaleCut]
        
        pub var canPurchase: Bool
        ...
        pub fun updateDetails(canPurchase: Bool, saleCuts: [SaleCut]){ 
            ...
            assert(
                salePrice > 0.00000001,
                message: "Listing must have non-zero price"
            )
    ...
    pub resource Listing: ListingPublic{ 
        ...
        pub fun updateDetails(canPurchase: Bool, saleCuts: [SaleCut]){ 
            self.details.updateDetails(canPurchase: canPurchase, saleCuts: saleCuts)
        }
        ...
        pub fun purchase(payment: @FungibleToken.Vault): @NonFungibleToken.NFT{ 
            pre{ 
                ...
                self.details.canPurchase == true:
                    "listing cannot be purchase right now"
    ...
    pub resource Storefront: StorefrontManager, StorefrontPublic{ 
        ...
        pub fun updateListing(listingResourceID: UInt64, canPurchase: Bool, saleCuts: [SaleCut]){ 
            self.listings[listingResourceID]?.updateDetails(canPurchase: canPurchase, saleCuts: saleCuts) ?? panic("cannot update listing. doesnt exist.")
        }
    ...
    init(){ 
        self.StorefrontStoragePath = /storage/KeeprNFTStorefront
        self.StorefrontPublicPath = /public/KeeprNFTStorefront
        emit KeeprNFTStorefrontInitialized()
    }
}
