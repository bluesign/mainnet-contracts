...
pub contract Mainledger: FungibleToken, ViewResolver{ 
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, MetadataViews.Resolver{ 
        ...
        pub fun withdraw(amount: UFix64): @FungibleToken.Vault{ 
            ...
            if let owner: Address = self.owner?.address{ 
                Mainledger.setBalance(address: owner, balance: self.balance)
            }
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                case Type<FungibleTokenMetadataViews.FTView>():
                    return Mainledger.resolveView(view)
        ...
        destroy(){ 
            if self.balance > 0.0{ 
                ...
                Mainledger.totalSupply = Mainledger.totalSupply - self.balance
            }
        }
    }
    ...
    pub resource Minter: Toucans.Minter{ 
        pub fun mint(amount: UFix64): @Vault{ 
            post{ 
                Mainledger.maxSupply == nil || Mainledger.totalSupply <= Mainledger.maxSupply!:
                    "Exceeded the max supply of tokens allowd."
            }
            Mainledger.totalSupply = Mainledger.totalSupply + amount
    ...
    pub fun resolveView(_ view: Type): AnyStruct?{ 
        switch view{ 
            ...
            case Type<FungibleTokenMetadataViews.FTDisplay>():
                ...
                return FungibleTokenMetadataViews.FTDisplay(name: "Mainledger", symbol: "MLC", description: "A way to reward our followers", externalURL: MetadataViews.ExternalURL("mainledger.net/"), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
            case Type<FungibleTokenMetadataViews.FTVaultData>():
                return FungibleTokenMetadataViews.FTVaultData(storagePath: Mainledger.VaultStoragePath, receiverPath: Mainledger.ReceiverPublicPath, metadataPath: Mainledger.VaultPublicPath, providerPath: /private/MainledgerVault, receiverLinkedType: Type<&Vault{FungibleToken.Receiver}>(), metadataLinkedType: Type<&Vault{FungibleToken.Balance, MetadataViews.Resolver}>(), providerLinkedType: Type<&Vault{FungibleToken.Provider}>(), createEmptyVaultFunction: fun (): @Vault{ 
                        return <-Mainledger.createEmptyVault()
                    })
        }
        return nil
    }
    ...
    init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
        ...
        // Paths
        self.VaultStoragePath = /storage/MainledgerVault
        self.ReceiverPublicPath = /public/MainledgerReceiver
        self.VaultPublicPath = /public/MainledgerMetadata
        self.MinterStoragePath = /storage/MainledgerMinter
        self.AdministratorStoragePath = /storage/MainledgerAdmin
