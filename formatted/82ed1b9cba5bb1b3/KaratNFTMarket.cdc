...
pub contract KaratNFTMarket{ 
    ...
    pub resource SaleOffer: SaleOfferPublicView{ 
        ...
        priv let sellerItemProvider: Capability<&KaratNFT.Collection{NonFungibleToken.Provider}>
        
        // The FungibleToken vault that will receive that payment if teh sale completes successfully.
        priv let sellerPaymentReceiver: Capability<&AnyResource{FungibleToken.Receiver}>
        ...
        //
        pub fun accept(buyerCollection: &KaratNFT.Collection{NonFungibleToken.Receiver}, buyerPayment: @FungibleToken.Vault){ 
            pre{ 
        ...
        //
        init(sellerItemProvider: Capability<&KaratNFT.Collection{NonFungibleToken.Provider}>, itemID: UInt64, sellerPaymentReceiver: Capability<&AnyResource{FungibleToken.Receiver}>, price: UFix64, receiverPublicPath: PublicPath){ 
            pre{ 
    ...
    pub fun createSaleOffer(
        sellerItemProvider: Capability<
            &KaratNFT.Collection{NonFungibleToken.Provider}
        >,
        itemID: UInt64,
    ...
    pub resource Collection: CollectionManager, CollectionPublic{ 
        ...
        //
        pub fun purchase(itemID: UInt64, buyerCollection: &KaratNFT.Collection{NonFungibleToken.Receiver}, buyerPayment: @FungibleToken.Vault){ 
            pre{ 
