...
pub contract KaratItems: NonFungibleToken{ 
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &KaratItems.NFT?{ 
        let collection = (getAccount(from).getCapability(KaratItems.CollectionPublicPath)!).borrow<&KaratItems.Collection{KaratItems.KaratItemsCollectionPublic}>() ?? panic("Couldn't get collection")
