...
pub contract StanzClub: NonFungibleToken{ 
    ...
    /** Resource interface to expose account access-only NFT modifiers. **/    pub resource interface NFTModifier{ 
    ...
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver, NFTModifier{ 
        ...
        /* StanzClub.@NFT resource constructor */        init(initID: UInt64, initlink: String, initbatch: UInt32, initsequence: UInt16, initlimit: UInt16, name: String, description: String, thumbnail: String, royalties: [MetadataViews.Royalty], editionNumber: UInt64, metadata:{ String: String}){ 
            self.id = initID
            self.link = initlink
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                ...
                case Type<MetadataViews.ExternalURL>():
                    return MetadataViews.ExternalURL(url: self.link.concat(self.batch.toString()).concat("/").concat(self.sequence.toString()))
                // This view provides information for where to set up a NFT 
                // collection in storage and to create the empty collection.
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: StanzClub.CollectionStoragePath, publicPath: StanzClub.CollectionPublicPath, providerPath: /private/StanzClubCollection, publicCollection: Type<&StanzClub.Collection{StanzClub.StanzClubCollectionPublic}>(), publicLinkedType: Type<&StanzClub.Collection{StanzClub.StanzClubCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&StanzClub.Collection{StanzClub.StanzClubCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Provider, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
                ...
                case Type<MetadataViews.NFTCollectionDisplay>():
                    ...
                    return MetadataViews.NFTCollectionDisplay(name: "StanzClub Collection", description: "StanzClub Collection", externalURL: MetadataViews.ExternalURL(url: self.link), squareImage: squareImage, bannerImage: bannerImage, socials:{} )
    ...
    /** Resource interface to expose public collection functions. **/    pub resource interface StanzClubCollectionPublic{ 
    ...
    pub resource Collection: StanzClubCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection{ 
        ...
        /* StanzClub.@Collection resource constructor */        init(){ 
            self.ownedNFTs <-{} 
        }
        ...
        /* Resource destructor override function */        destroy(){ 
    ...
    /* Public function to initialize an empty collection */    pub fun createEmptyCollection(): @StanzClub.Collection{ 
        return <-create Collection()
    }
    
    /* Resource defining the minting of a @StanzClub.NFT */    pub resource NFTMinter{ 
        pub var minterID: UInt64
        
        /* StanzClub.@NFTMinter resource constructor */        init(){ 
            self.minterID = 0
        }
        ...
        pub fun mintNFT(glink: String, gbatch: UInt32, glimit: UInt16, gsequence: UInt16, name: String, description: String, thumbnail: String, royalties: [MetadataViews.Royalty], editionNumber: UInt64, metadata:{ String: String}): @NFT{ 
    ...
    /* Public resource storing the modifier functions for the account. */    pub resource Modifier{ 
        ...
        /* StanzClub.@Modifier resource constructor */        init(){ 
            self.ModifierID = 0
        }
    }
    ...
    init(){ 
        ...
        self.account.link<&{NonFungibleToken.CollectionPublic, StanzClub.StanzClubCollectionPublic, MetadataViews.ResolverCollection}>(self.CollectionPublicPath, target: self.CollectionStoragePath)
