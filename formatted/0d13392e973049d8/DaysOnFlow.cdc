...
pub contract DaysOnFlow: NonFungibleToken{ 
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                case Type<MetadataViews.Display>():
                    return MetadataViews.Display(name: self.seriesName, description: self.seriesDescription, file: MetadataViews.IPFSFile(cid: self.seriesImage, path: nil))
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: DaysOnFlow.CollectionStoragePath, publicPath: DaysOnFlow.CollectionPublicPath, providerPath: /private/DOFCollectionProviderPath, publicCollection: Type<&DaysOnFlow.Collection{DaysOnFlow.CollectionPublic}>(), publicLinkedType: Type<&DaysOnFlow.Collection{DaysOnFlow.CollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&DaysOnFlow.Collection{DaysOnFlow.CollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Provider, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
    ...
    pub resource DOFSeries: MetadataViews.Resolver{ 
        ...
        priv let dayNFTwlClaimed:{ UInt64: Bool}
        
        // Keeps track of the WL addresses that have already claimed
        priv let wlClaimed:{ Address: Bool}
        ...
        pub fun nbDayNFTwlToMint(address: Address): Int{ 
            // DayNFT collection
            let holder = getAccount(address).getCapability(DayNFT.CollectionPublicPath).borrow<&DayNFT.Collection{DayNFT.CollectionPublic}>()
        ...
        pub fun mintDayNFTwl(address: Address){ 
            ...
            // DayNFT collection
            let holder = getAccount(address).getCapability(DayNFT.CollectionPublicPath).borrow<&DayNFT.Collection{DayNFT.CollectionPublic}>() ?? panic("Could not get receiver reference to the DayNFT Collection")
            for id in holder.getIDs(){ 
                if self.dayNFTwlClaimed[id] != nil && !self.dayNFTwlClaimed[id]!{ 
        ...
        pub fun mintWl(address: Address, vault: @FlowToken.Vault){ 
            ...
            let rec = DaysOnFlow.account.getCapability(/public/flowTokenReceiver).borrow<&FlowToken.Vault{FungibleToken.Receiver}>() ?? panic("Could not borrow a reference to the Flow receiver")
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                case Type<MetadataViews.Display>():
                    return MetadataViews.Display(name: self.name, description: self.description, file: MetadataViews.IPFSFile(cid: self.image, path: nil))
            }
            return nil
        }
        ...
        access(contract) fun mint(address: Address, saleType: String): UInt64{ 
            // DOF collection
            let receiver = getAccount(address).getCapability(DaysOnFlow.CollectionPublicPath).borrow<&{DaysOnFlow.CollectionPublic}>() ?? panic("Could not get receiver reference to the NFT Collection")
        ...
        init(_wlClaimable: Bool, _publicClaimable: Bool, _description: String, _image: String, _name: String, _dayNFTwl: [UInt64], // List of DayNFT ids giving right to an item _wl: [Address], // List of WL addresses _wlPrice: UFix64, _publicSupply: UInt64, _publicPrice: UFix64){ 
    ...
    pub resource SeriesMinter{ 
        ...
        // Create a new series
        pub fun createSeries(_wlClaimable: Bool, _publicClaimable: Bool, _description: String, _image: String, _name: String, _dayNFTwl: [UInt64], _wl: [Address], _wlPrice: UFix64, _publicSupply: UInt64, _publicPrice: UFix64){ 
