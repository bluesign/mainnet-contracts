/*
    Description: Central Smart Contract for ethosAIFIIInvestor
    
    This smart contract contains the core functionality for 
    ethosAIFIIInvestor, created by ethos Multiverse Inc.
    ...
    every ethosAIFIIInvestor NFT owner will store in their account
    to manage their NFT collection.
    
    The main ethosAIFIIInvestor account operated by ethos Multiverse Inc. 
    will also have its own ethosAIFIIInvestor collection it can use to hold its 
...
pub contract ethosAIFIIInvestor: NonFungibleToken{ 
    ...
    // -----------------------------------------------------------------------
    // ethosAIFIIInvestor contract Events
    // -----------------------------------------------------------------------
    
    
    // Emited when the ethosAIFIIInvestor contract is created
    pub event ContractInitialized()
    
    // Emmited when a user transfers a ethosAIFIIInvestor NFT out of their collection
    pub event Withdraw(id: UInt64, from: Address?)
    
    // Emmited when a user recieves a ethosAIFIIInvestor NFT into their collection
    pub event Deposit(id: UInt64, to: Address?)
    
    // Emmited when a ethosAIFIIInvestor NFT is minted
    pub event Minted(id: UInt64)
    
    // Emmited when a batch of ethosAIFIIInvestor NFTs are minted
    pub event BatchMint(metadatas: [{String: String}])
    
    // -----------------------------------------------------------------------
    // ethosAIFIIInvestor Named Paths
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    // ethosAIFIIInvestor contract-level fields.
    ...
    // Array of all existing ethosAIFIIInvestor NFTs
    priv var metadatas: [{String: String}]
    
    // The total number of ethosAIFIIInvestor NFTs that have been created
    ...
    // -----------------------------------------------------------------------
    // ethosAIFIIInvestor contract-level Composite Type definitions
    // -----------------------------------------------------------------------
    
    // The resource that represents the ethosAIFIIInvestor NFTs
    //
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver{ 
        ...
        init(_metadata:{ String: String}){ 
            self.id = ethosAIFIIInvestor.totalSupply
            self.metadata = _metadata
            
            // Total Supply
            ethosAIFIIInvestor.totalSupply = ethosAIFIIInvestor.totalSupply + 1
            
            // Add the metadata to the metadatas array
            ethosAIFIIInvestor.metadatas.append(_metadata)
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                ...
                case Type<MetadataViews.ExternalURL>():
                    return MetadataViews.ExternalURL("https://jade.ethosnft.com/collections/".concat((self.owner!).address.toString()).concat("/ethosAIFIIInvestor"))
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: ethosAIFIIInvestor.CollectionStoragePath, publicPath: ethosAIFIIInvestor.CollectionPublicPath, providerPath: ethosAIFIIInvestor.CollectionPrivatePath, publicCollection: Type<&Collection{NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), publicLinkedType: Type<&Collection{NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&Collection{NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection, NonFungibleToken.Provider}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
                            return <-ethosAIFIIInvestor.createEmptyCollection()
                        })
                case Type<MetadataViews.NFTCollectionDisplay>():
                    let squareMedia: MetadataViews.Media = MetadataViews.Media(file: ethosAIFIIInvestor.getCollectionAttribute(key: "image") as! MetadataViews.HTTPFile, mediaType: "image")
                    
                    // Check if banner image exists
                    var bannerMedia: MetadataViews.Media? = nil
                    if let bannerImage: MetadataViews.IPFSFile = ethosAIFIIInvestor.getCollectionAttribute(key: "bannerImage") as! MetadataViews.IPFSFile?{ 
                        bannerMedia = MetadataViews.Media(file: bannerImage, mediaType: "image")
                    }
                    return MetadataViews.NFTCollectionDisplay(name: ethosAIFIIInvestor.getCollectionAttribute(key: "name") as! String, description: ethosAIFIIInvestor.getCollectionAttribute(key: "description") as! String, externalURL: MetadataViews.ExternalURL("https://jade.ethosnft.com/collections/".concat((self.owner!).address.toString()).concat("/ethosAIFIIInvestor")), squareImage: squareMedia, bannerImage: bannerMedia ?? squareMedia, socials: ethosAIFIIInvestor.getCollectionAttribute(key: "socials") as!{ String: MetadataViews.ExternalURL})
                case Type<MetadataViews.Royalties>():
                    return MetadataViews.Royalties([                                                    // For ethos Multiverse Inc. in favor of producing Jade, a tool for deploying NFT contracts and minting/managing collections.
                                                    MetadataViews.Royalty(receiver: getAccount(0xeaf1bb3f70a73336).getCapability<&FlowToken.Vault{FungibleToken.Receiver}>(/public/flowTokenReceiver), cut: 0.025, // 2.5% on secondary sales description: "ethos Multiverse Inc. receives a 2.5% royalty from secondary sales because this collection was created using Jade (https://jade.ethosnft.com), a tool for deploying NFT contracts and minting/managing collections, created by ethos Multiverse Inc.")])
    ...
    // The interface that users can cast their ethosAIFIIInvestor Collection as
    // to allow others to deposit ethosAIFIIInvestor into thier Collection. It also
    // allows for the reading of the details of ethosAIFIIInvestor
    pub resource interface CollectionPublic{ 
        ...
        pub fun borrowEntireNFT(id: UInt64): &ethosAIFIIInvestor.NFT?{} 
    }
    ...
    pub resource Collection: NonFungibleToken.Receiver, NonFungibleToken.Provider, NonFungibleToken.CollectionPublic, CollectionPublic, MetadataViews.ResolverCollection{ 
        ...
        //
        pub fun deposit(token: @NonFungibleToken.NFT){ 
            let myToken <- token as! @ethosAIFIIInvestor.NFT
        ...
        // borrowNFT Returns a borrowed reference to a ethosAIFIIInvestor NFT in the Collection
        ...
        // borrowEntireNFT returns a borrowed reference to a ethosAIFIIInvestor 
        ...
        pub fun borrowEntireNFT(id: UInt64): &ethosAIFIIInvestor.NFT?{ 
            if self.ownedNFTs[id] != nil{ 
                ...
                return reference as! &ethosAIFIIInvestor.NFT
            } else{ 
                return nil
            }
        }
    ...
    pub resource Admin{ 
        ...
        // batchMint
        // Batch mints ethosAIFIIInvestor NFTs
        ...
        pub fun updateCollectionInfo(key: String, value: AnyStruct){ 
            ethosAIFIIInvestor.collectionInfo[key] = value
        }
    ...
    // -----------------------------------------------------------------------
    // ethosAIFIIInvestor contract-level function definitions
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    // ethosAIFIIInvestor initialization function
    // -----------------------------------------------------------------------
    ...
    init(){ 
        ...
        self.collectionInfo ={} 
        self.collectionInfo["name"] = "ethosAIFIIInvestor"
        self.collectionInfo["description"] = "Collection of ethos AIF II Investor NFTs"
        ...
        // Set named paths
        self.CollectionStoragePath = /storage/ethosAIFIIInvestorCollection
        self.CollectionPublicPath = /public/ethosAIFIIInvestorCollection
        self.CollectionPrivatePath = /private/ethosAIFIIInvestorCollection
        self.AdminStoragePath = /storage/ethosAIFIIInvestorAdmin
        self.AdminPrivatePath = /private/ethosAIFIIInvestorAdminUpgrade
        ...
        self.account.link<&ethosAIFIIInvestor.Collection{NonFungibleToken.CollectionPublic, ethosAIFIIInvestor.CollectionPublic, MetadataViews.ResolverCollection}>(self.CollectionPublicPath, target: self.CollectionStoragePath)
        
        // Create a private capability for the admin resource
        self.account.link<&ethosAIFIIInvestor.Admin>(self.AdminPrivatePath, target: self.AdminStoragePath) ?? panic("Could not get Admin capability")
