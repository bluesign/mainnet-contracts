...
pub contract HeavengodFlow: FungibleToken, ViewResolver{ 
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, MetadataViews.Resolver{ 
        ...
        pub fun withdraw(amount: UFix64): @FungibleToken.Vault{ 
            ...
            if let owner: Address = self.owner?.address{ 
                HeavengodFlow.setBalance(address: owner, balance: self.balance)
            }
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                case Type<FungibleTokenMetadataViews.FTView>():
                    return HeavengodFlow.resolveView(view)
        ...
        destroy(){ 
            if self.balance > 0.0{ 
                ...
                HeavengodFlow.totalSupply = HeavengodFlow.totalSupply - self.balance
            }
        }
    }
    ...
    pub resource Minter: Toucans.Minter{ 
        pub fun mint(amount: UFix64): @Vault{ 
            post{ 
                HeavengodFlow.maxSupply == nil || HeavengodFlow.totalSupply <= HeavengodFlow.maxSupply!:
                    "Exceeded the max supply of tokens allowd."
            }
            HeavengodFlow.totalSupply = HeavengodFlow.totalSupply + amount
    ...
    pub fun resolveView(_ view: Type): AnyStruct?{ 
        switch view{ 
            ...
            case Type<FungibleTokenMetadataViews.FTDisplay>():
                ...
                return FungibleTokenMetadataViews.FTDisplay(name: "Heavengod", symbol: "HVNGD", description: "I've created a DAO token for friends who enjoy following my ventures, enabling me to delve deeper into the possibilities offered by the Flow blockchain.", externalURL: MetadataViews.ExternalURL("linktr.ee/heavengod"), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("arka888"), "discord": MetadataViews.ExternalURL("arka888#0620")})
            case Type<FungibleTokenMetadataViews.FTVaultData>():
                return FungibleTokenMetadataViews.FTVaultData(storagePath: HeavengodFlow.VaultStoragePath, receiverPath: HeavengodFlow.ReceiverPublicPath, metadataPath: HeavengodFlow.VaultPublicPath, providerPath: /private/HeavengodFlowVault, receiverLinkedType: Type<&Vault{FungibleToken.Receiver}>(), metadataLinkedType: Type<&Vault{FungibleToken.Balance, MetadataViews.Resolver}>(), providerLinkedType: Type<&Vault{FungibleToken.Provider}>(), createEmptyVaultFunction: fun (): @Vault{ 
                        return <-HeavengodFlow.createEmptyVault()
                    })
        }
        return nil
    }
    ...
    init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
        ...
        // Paths
        self.VaultStoragePath = /storage/HeavengodFlowVault
        self.ReceiverPublicPath = /public/HeavengodFlowReceiver
        self.VaultPublicPath = /public/HeavengodFlowMetadata
        self.MinterStoragePath = /storage/HeavengodFlowMinter
        self.AdministratorStoragePath = /storage/HeavengodFlowAdmin
