...
pub contract MindtrixViews{ 
    ...
    pub struct Serials{ 
        ...
        init(data:{ String: String}){ 
            ...
            let str =
                (data["essenceRealmSerial"] ?? "0").concat(
                    data["essenceTypeSerial"] ?? "0"
                ).concat(data["showSerial"] ?? "0").concat(
                    data["episodeSerial"] ?? "0"
                ).concat(data["audioEssenceSerial"] ?? "0").concat(
                    data["nftEditionSerial"] ?? "0"
                )
            self.dic = data
            self.arr = arr
    ...
    pub struct interface IVerifier{ 
        pub fun verify(_ params:{ String: AnyStruct}, _ isAssert: Bool):{ 
            String: Bool
        }{} 
    }
    ...
    pub resource interface IHashProvider{ 
        pub fun borrowHashVerifier(setID: UInt64, packID: UInt64): &{
            IHashVerifier
        }{} 
    }
}
