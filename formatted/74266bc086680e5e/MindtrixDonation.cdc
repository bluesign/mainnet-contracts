...
pub contract MindtrixDonation{ 
    ...
    pub event UpdateDonationRoyalties(
        showGuid: String,
    ...
    priv var episodeGuidToDonations:{ 
        String:{ 
            Address: [
    ...
    //  }
    //}
    priv var showGuidToRoyalties:{ String:{ String: [MetadataViews.Royalty]}}
    ...
    access(account) fun updateDonorDic(
        donorAddress: Address,
        episodeGuid: String,
    ...
    access(account) fun getNFTEditionFromDonationDicByEpisodeGuid(
        episodeGuid: String
    ): UInt64{ 
        if self.episodeGuidToDonations[episodeGuid] == nil{ 
            return 0
    ...
    access(account) fun replaceShowGuidToRoyalties(
        showGuid: String,
        creatorAddress: Address,
        primaryRoyalties: [
        ...
        (showGuidToRoyaltiesTmp[showGuid]!).insert(
            key: "secondary",
            secondaryRoyalties
        )
    ...
    pub fun getShowGuidRoyalties(showGuid: String):{ 
        String: [
            MetadataViews.Royalty
        ]
    }?{ 
    ...
    pub fun mintNFTFromDonation(
        ...
        let essenceStruct =
            MindtrixEssence.getOneEssenceStruct(essenceId: essenceId)!
        ...
        assert(
            claimable == true,
            message: "Cannot donate to an unclaimable episode."
        )
        assert(
            mintPrices != nil,
            message: "The donation price should not be nil."
        )
        assert(
            (mintPrices!).containsKey(flowTokenVaultIdentifier),
            message: "The the token address from the price is incorrect."
        )
        assert(
            ((mintPrices!)[flowTokenVaultIdentifier]!).price > UFix64(0),
            message: "The donation price should not be free."
        )
        
        // Verify minting conditions and abort if an error occurs.
        essenceStruct.verifyMintingConditions(
        ...
        let royaltiesFromShowGuid =
            MindtrixDonation.showGuidToRoyalties[showGuid]!
            as{
        ...
        for primaryRoyalty in royaltiesFromShowGuid["primary"]!{ 
            ...
            if address == mindtrixTreasuryAddress{ 
                ...
                MindtrixVault = primaryRoyalty.receiver.borrow() ?? panic("Could not borrow the &{FungibleToken.Receiver} from Mindtrix's Vault.")
        ...
        CreatorVault = getAccount(0xf8d6e0586b0a20c7).getCapability<
                &FlowToken.Vault{FungibleToken.Receiver}
            >(/public/flowTokenReceiver).borrow()!
        let mindtrixFlow <-
            payment.withdraw(amount: payment.balance * mindtrixPrimaryCut)
        ...
        let mintedEdition =
            self.getNFTEditionFromDonationDicByEpisodeGuid(
        ...
        var orgRoyalties =
            essenceStruct.getRoyalties() as! [MetadataViews.Royalty]
