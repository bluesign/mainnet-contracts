...
pub contract MindtrixEssence{ 
    ...
    pub event EssenceClaimableUpdated(
        essenceId: UInt64,
        essenceName: String,
        claimable: Bool,
        showGuid: String,
    ...
    priv var showGuidToEssenceIds:{ String:{ UInt64: Bool}}
    
    priv var episodeGuidToEssenceIds:{ String:{ UInt64: Bool}}
    
    priv var essenceDic: @{UInt64: EssenceRes}
    
    priv var essenceIdsToCreationIds:{ UInt64:{ UInt64: Bool}}
    
    // ========================================================
    //                      IMMUTABLE STATE
    // ========================================================
    pub let MindtrixDaoTreasuryRef: Capability<
    ...
    pub struct EssenceStruct{ 
        ...
        pub fun verifyMintingConditions(
            ...
            let identifiers =
                self.getMintingRecordsByAddress(address: minterAddress)
        ...
        access(account) fun updatePreviewURL(
        ...
        access(account) fun clearMinters(){ 
            self.minters ={} 
        }
        ...
        init(
            ...
            if metadata["essenceTypeSerial"] == "1"
            || metadata["essenceTypeSerial"] == "2"{ 
                royalties.append(
                    MetadataViews.Royalty(
    ...
    pub resource EssenceRes: EssencePublic, MetadataViews.Resolver{ 
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                ...
                case Type<MindtrixViews.EssenceIdentifier>():
                    return MindtrixViews.EssenceIdentifier(uuid: self.uuid, serials: MindtrixViews.Serials(self.getSerialDic()).arr, holder: MindtrixEssence.account.address, showGuid: self.data.metadata["showGuid"] ?? "", episodeGuid: self.data.metadata["episodeGuid"] ?? "", createdTime: self.data.createdTime)
        ...
        pub fun getSerialDic():{ String: String}{ 
            return{ "essenceRealmSerial": self.data.metadata["essenceRealmSerial"] ?? "0", "essenceTypeSerial": self.data.metadata["essenceTypeSerial"] ?? "0", "showSerial": self.data.metadata["showSerial"] ?? "0", "episodeSerial": self.data.metadata["episodeSerial"] ?? "0", // the index from the minted episode "audioEssenceSerial": self.data.metadata["audioEssenceSerial"] ?? "0", "nftEditionSerial": self.data.currentEdition.toString()}
        }
    ...
    pub resource EssenceCollection{ 
        ...
        pub fun clearEssenceMinters(essenceId: UInt64){ 
        ...
        pub fun createEssence(
            ...
            let audioEssence =
                metadata["audioEssence"] as? MindtrixViews.AudioEssence
    ...
    pub fun getOneEssenceRes(essenceId: UInt64): &EssenceRes{EssencePublic}?{ 
        return &MindtrixEssence.essenceDic[essenceId]
        as
        &EssenceRes{EssencePublic}?
    }
    
    pub fun getOneEssenceStruct(essenceId: UInt64): EssenceStruct?{ 
        let essence =
            &MindtrixEssence.essenceDic[essenceId]
            as
            &EssenceRes{EssencePublic}?
        return essence?.data ?? nil
    }
    
    pub fun borrowEssenceViewResolver(id: UInt64): &AnyResource{
        MetadataViews.Resolver
    }{ 
        let essence =
            (&MindtrixEssence.essenceDic[id] as &MindtrixEssence.EssenceRes?)!
        return essence as &AnyResource{MetadataViews.Resolver}
    }
    
    access(account) fun updateEsenceIdsToCreationIds(
        essenceId: UInt64,
        nftId: UInt64
    ){ 
    ...
    init(){ 
        ...
        self.MindtrixDaoTreasuryRef = self.account.getCapability<
                &{FungibleToken.Receiver}
            >(royaltyReceiverPublicPath)
        self
            .MindtrixEssenceV2CollectionStoragePath = /storage/MindtrixEssenceV2CollectionStoragePath
        self
            .MindtrixEssenceV2CollectionPublicPath = /public/MindtrixEssenceV2CollectionPublicPath
    }
}
