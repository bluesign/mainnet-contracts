...
This is the contract for psk NFTs!
...
pub contract psk: NonFungibleToken, ViewResolver{ 
    ...
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver, NiftoryNonFungibleToken.NFTPublic{ 
        ...
        init(setId: Int, templateId: Int, serial: UInt64){ 
            self.id = psk.totalSupply
            psk.totalSupply = psk.totalSupply + 1
            self.setId = setId
    ...
    pub resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection, NiftoryNonFungibleToken.CollectionPublic, NiftoryNonFungibleToken.CollectionPrivate{ 
        ...
        pub fun depositBulk(tokens: @[NonFungibleToken.NFT]){ 
            while tokens.length > 0{ 
                let token <- tokens.removeLast() as! @psk.NFT
    ...
    pub resource Manager: NiftoryNonFungibleToken.ManagerPublic, NiftoryNonFungibleToken.ManagerPrivate{ 
        ...
        pub fun getSetManagerPublic(): &MutableMetadataSetManager.Manager{MutableMetadataSetManager.Public}{ 
            return NiftoryNFTRegistry.getSetManagerPublic(psk.REGISTRY_ADDRESS, psk.REGISTRY_BRAND)
        }
        
        pub fun getMetadataViewsManagerPublic(): &MetadataViewsManager.Manager{MetadataViewsManager.Public}{ 
            return NiftoryNFTRegistry.getMetadataViewsManagerPublic(psk.REGISTRY_ADDRESS, psk.REGISTRY_BRAND)
        }
        
        pub fun getNFTCollectionData(): MetadataViews.NFTCollectionData{ 
            return NiftoryNFTRegistry.buildNFTCollectionData(psk.REGISTRY_ADDRESS, psk.REGISTRY_BRAND, fun (): @NonFungibleToken.Collection{ 
                    return <-psk.createEmptyCollection()
                })
        }
        ...
        pub fun modifyContractMetadata(): auth &AnyStruct{ 
            ...
            let maybeMetadata = psk.metadata
            if maybeMetadata == nil{ 
                let blankMetadata:{ String: String} ={} 
                psk.metadata = blankMetadata
            }
            return (&psk.metadata as auth &AnyStruct?)!
        }
        
        pub fun replaceContractMetadata(_ metadata: AnyStruct?){ 
            emit ContractMetadataUpdated()
            psk.metadata = metadata
        }
        ...
        priv fun _getMetadataViewsManagerPrivate(): &MetadataViewsManager.Manager{MetadataViewsManager.Private}{ 
            let record = NiftoryNFTRegistry.getRegistryRecord(psk.REGISTRY_ADDRESS, psk.REGISTRY_BRAND)
            let manager = psk.account.getCapability<&MetadataViewsManager.Manager{MetadataViewsManager.Private}>(record.metadataViewsManager.paths.private).borrow()!
            return manager
        }
        ...
        priv fun _getSetManagerPrivate(): &MutableMetadataSetManager.Manager{MutableMetadataSetManager.Public, MutableMetadataSetManager.Private}{ 
            let record = NiftoryNFTRegistry.getRegistryRecord(psk.REGISTRY_ADDRESS, psk.REGISTRY_BRAND)
            let setManager = psk.account.getCapability<&MutableMetadataSetManager.Manager{MutableMetadataSetManager.Public, MutableMetadataSetManager.Private}>(record.setManager.paths.private).borrow()!
            return setManager
        }
    ...
    pub fun contract(): &{NiftoryNonFungibleToken.ManagerPublic}{ 
        return NiftoryNFTRegistry.getNFTManagerPublic(psk.REGISTRY_ADDRESS, psk.REGISTRY_BRAND)
    }
    ...
    init(nftManagerProxy: &{NiftoryNonFungibleTokenProxy.Public, NiftoryNonFungibleTokenProxy.Private}){ 
        let record = NiftoryNFTRegistry.generateRecord(account: self.account.address, project: "clejpvqj90000l90u6og1o70a_psk")
        self.REGISTRY_ADDRESS = 0x32d62d5c43ad1038
        self.REGISTRY_BRAND = "clejpvqj90000l90u6og1o70a_psk"
        ...
        self.account.save<@MutableMetadataSetManager.Manager>(<-MutableMetadataSetManager.create(name: "psk", description: "The set manager for psk."), to: record.setManager.paths.storage)
