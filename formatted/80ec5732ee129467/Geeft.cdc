...
pub contract Geeft: NonFungibleToken{ 
    ...
    pub resource CollectionContainer{ 
        ...
        init(publicPath: PublicPath, storagePath: StoragePath, assets: @[{MetadataViews.Resolver}], to: Address){ 
        ...
        priv fun getCap(to: Address): Capability<&{NonFungibleToken.Receiver}>{ 
    ...
    pub resource VaultContainer{ 
        ...
        priv fun getCap(to: Address): Capability<&{FungibleToken.Receiver}>{ 
            if to == self.originalReceiverCap.address{ 
                return self.originalReceiverCap
            } else{ 
    ...
    pub fun sendGeeft(createdBy: Address?, message: String?, collections: @{String: CollectionContainer}, vaults: @{String: VaultContainer}, extra:{ String: AnyStruct}, recipient: Address){ 
