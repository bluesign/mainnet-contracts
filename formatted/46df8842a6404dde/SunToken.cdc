...
pub contract SunToken: FungibleToken{ 
    ...
    init(){ 
        ...
        //
        self.account.link<&{FungibleToken.Receiver}>(self.VestedReceiverPublicPath, target: self.VaultStoragePath)
        // Create a public capability to the stored Vault that only exposes
        // the VestedReceiver methods through the `Receiver` interface
        //
        self.account.link<&{SunToken.VestedReceiver}>(self.VestedReceiverPublicPath, target: self.VaultStoragePath)
        // Create a public capability to the stored Vault that only exposes
        // the `balance` field through the `Balance` interface
        //
        self.account.link<&SunToken.Vault{FungibleToken.Balance}>(self.BalanceReceiverPublicPath, target: self.VaultStoragePath)
