...
pub contract SwapPair{ 
    priv fun bloctoScaledReserves(): [UInt256]{ 
        return [
            SwapConfig.UFix64ToScaledUInt256(
    ...
    priv fun bloctoUsdScaledReserves(): [UInt256]{ 
        return [
            SwapConfig.UFix64ToScaledUInt256(
                UsdcUsdtSwapPair.getPoolAmounts().token1Amount
            ),
            SwapConfig.UFix64ToScaledUInt256(
                UsdcUsdtSwapPair.getPoolAmounts().token2Amount
            )
        ]
    }
    
    priv fun incrementScaledReserves(): [UInt256]{ 
        let pairInfo =
            (
                getAccount(0xfa82796435e15832).getCapability<
    ...
    priv fun metaScaledReserves(): [UInt256]{ 
        let mpool =
            getAccount(0x18187a9d276c0329).getCapability<
                &PierPair.Pool{IPierPair.IPool}
            >(/public/metapierSwapPoolPublic).borrow()!
    ...
    priv fun aggregateReserves(): [UFix64]{ 
        var fr: UInt256 = 0
        var ur: UInt256 = 0
    ...
    priv fun getTokenOutVault(_ vaultInType: Type): @FungibleToken.Vault{ 
    ...
    priv fun checkBloctoAmounts(_ amountIn: UFix64, _ vaultInType: Type): Bool{ 
    ...
    priv fun check(_ amount: UFix64): Bool{ 
    ...
    priv fun prune(_ height: UInt64){ 
    ...
    pub fun removeLiquidity(lpTokenVault: @FungibleToken.Vault): @[
        FungibleToken.Vault
    ]{ 
        pre{ 
            false:
    ...
    priv let token0VaultType: Type
    
    priv let token1VaultType: Type
    
    priv let token0Key: String
    
    priv let token1Key: String
    
    priv var keys:{ UFix64: UInt64}
    ...
    init(){ 
        ...
        self.account.link<&{SwapInterfaces.PairPublic}>(
            SwapConfig.PairPublicPath,
            target: /storage/pair_public
        )
    }
}
