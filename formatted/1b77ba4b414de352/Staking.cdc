...
pub contract Staking{ 
    ...
    priv let userStakingIds:{ Address:{ UInt64: Bool}}
    ...
    init(){ 
        ...
        self
            .UserCertificateStoragePath = /storage/increment_stakingUserCertificate
        self
            .UserCertificatePrivatePath = /private/increment_stakingUserCertificate
        self.poolCount = 0
        self.account.save(<-create Admin(), to: self.StakingAdminStoragePath)
        self.account.save(
            <-create StakingPoolCollection(),
            to: self.CollectionStoragePath
        )
        self.account.link<&{PoolCollectionPublic}>(
    ...
    pub struct RewardInfo{ 
        ...
        access(contract) fun updateRewardInfo(
            ...
            let toBeDistributeReward =
                self.rewardPerSession * UFix64(currentRound - lastRound)
            let toBeDistributeRewardScaled =
                SwapConfig.UFix64ToScaledUInt256(toBeDistributeReward)
            let stakingBalanceScaled =
                SwapConfig.UFix64ToScaledUInt256(stakingBalance)
            // update pool's reward per seed index
            self.rewardPerSeed = self.rewardPerSeed
                + SwapConfig.ScaledUInt256ToUFix64(
                    toBeDistributeRewardScaled * SwapConfig.scaleFactor
                    / stakingBalanceScaled
                )
            emit RPSUpdated(
        ...
        access(contract) fun appendRewardPerSession(addRewardAmount: UFix64){ 
            ...
            self.rewardPerSession = self.rewardPerSession
                + addRewardAmount / UFix64(leftRound)
        }
    }
    ...
    pub struct UserInfo{ 
        ...
        access(contract) fun updateRewardPerSeed(
            tokenKey: String,
            rps: UFix64
        ){ 
        ...
        access(contract) fun addClaimedReward(
            tokenKey: String,
            amount: UFix64
        ){ 
        ...
        access(contract) fun updateUnclaimedReward(
            tokenKey: String,
            newValue: UFix64
        ){ 
    ...
    pub resource interface PoolPublic{ 
        ...
        pub fun boostReward(
            rewardPerSessionToAdd: UFix64,
            rewardToken: @FungibleToken.Vault
        ): @FungibleToken.Vault{} 
        
        pub fun stake(staker: Address, stakingToken: @FungibleToken.Vault){} 
        
        pub fun unstake(
            userCertificateCap: Capability<&{IdentityCertificate}>,
            amount: UFix64
        ): @FungibleToken.Vault{} 
        ...
        pub fun setClear(adminRef: &Admin?, poolAdminRef: &PoolAdmin): @{
            String: FungibleToken.Vault
        }{} 
        
        pub fun setUserBlockedStatus(
            adminRef: &Admin?,
            poolAdminRef: &PoolAdmin,
            address: Address,
            flag: Bool
    ...
    pub event PermissionlessStateChanged(
        permissionless: Bool,
        operator: Address
    )
    
    // Pool admin events
    pub event UserBlockedStateChanged(
        pid: UInt64,
        address: Address,
        blockedFlag: Bool,
        operator: Address
    ...
    pub resource Admin{ 
        ...
        pub fun setIsPermissionless(_ flag: Bool){ 
            ...
            Staking.isPermissionless = flag
            emit PermissionlessStateChanged(
                permissionless: flag,
                operator: (self.owner!).address
            )
        }
    ...
    pub resource Pool: PoolPublic{ 
        ...
        priv let stakingTokenVault: @FungibleToken.Vault
        
        // Vaults for reward tokens
        priv let rewardVaults: @{String: FungibleToken.Vault}
        
        // maps for userInfo
        priv let usersInfo:{ Address: UserInfo}
        ...
        priv fun harvest(harvester: Address, harvestMode: Bool): @{String: FungibleToken.Vault}{ 
            pre{ 
    ...
    pub fun updatePool(pid: UInt64){ 
        let collectionCap =
            Staking.account.getCapability<&{Staking.PoolCollectionPublic}>(
