...
pub contract PlayTodayItems: NonFungibleToken{ 
    ...
    priv var images:{ Kind:{ Rarity: String}}
    
    // Mapping from rarity -> price
    //
    priv var itemRarityPriceMap:{ Rarity: UFix64}
    ...
    pub resource NFTMinter{ 
        ...
        //
        pub fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, kind: Kind, rarity: Rarity){ 
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &PlayTodayItems.NFT?{ 
        let collection = (getAccount(from).getCapability(PlayTodayItems.CollectionPublicPath)!).borrow<&PlayTodayItems.Collection{PlayTodayItems.PlayTodayItemsCollectionPublic}>() ?? panic("Couldn't get collection")
