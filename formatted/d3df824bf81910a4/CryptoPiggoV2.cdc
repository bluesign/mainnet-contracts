...
pub contract CryptoPiggoV2: NonFungibleToken{ 
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                case Type<MetadataViews.Display>():
                    return MetadataViews.Display(name: "Crypto Piggo V2 NFT", description: "Crypto Piggo V2 NFT #".concat(self.id.toString()), thumbnail: MetadataViews.HTTPFile(url: "https://ipfs.tenzingai.com/ipfs/".concat(self.file.cid)))
                case Type<MetadataViews.Editions>():
                    // There is no max number of NFTs that can be minted from this contract
                    // so the max edition field value is set to nil
                    return MetadataViews.Editions([MetadataViews.Edition(name: "Crypto Piggo V2 NFT Edition", number: self.id, max: nil)])
                ...
                case Type<MetadataViews.Royalties>():
                    ...
                    for address in receivers.keys{ 
                        let receiver = getAccount(address).getCapability<&{FungibleToken.Receiver}>(MetadataViews.getRoyaltyReceiverPublicPath())
                        if receiver.check(){ 
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: CryptoPiggoV2.CollectionStoragePath, publicPath: CryptoPiggoV2.CollectionPublicPath, providerPath: /private/CryptoPiggoV2Collection, publicCollection: Type<&CryptoPiggoV2.Collection{NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection}>(), publicLinkedType: Type<&CryptoPiggoV2.Collection{NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&CryptoPiggoV2.Collection{NonFungibleToken.CollectionPublic, NonFungibleToken.Provider, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
                ...
                case Type<MetadataViews.NFTCollectionDisplay>():
                    ...
                    return MetadataViews.NFTCollectionDisplay(name: "The Crypto Piggo V2 NFT Collection", description: "", externalURL: MetadataViews.ExternalURL("https://www.rareworx.com/"), squareImage: media, bannerImage: media, socials:{} )
    ...
    pub resource NFTMinter{ 
        ...
        priv fun transformMetadata(_ metadata:{ String: String}):{ String: String}{ 
            pre{ 
                metadata.containsKey("name") && metadata.containsKey("hash") && metadata.containsKey("tag") && metadata.containsKey("body") && metadata.containsKey("background") && metadata.containsKey("outfit") && metadata.containsKey("head") && metadata.containsKey("face") && metadata.containsKey("unique") && metadata.containsKey("rarity") && metadata.containsKey("rarityOrder") && metadata.containsKey("rarityType"):
                    "Metadata does not conform to schema"
            }
            return{ "name": metadata["name"]!, "hash": metadata["hash"]!, "tag": metadata["tag"]!, "body": metadata["body"]!, "background": metadata["background"]!, "outfit": metadata["outfit"]!, "head": metadata["head"]!, "face": metadata["face"]!, "unique": metadata["unique"]!, "rarity": metadata["rarity"]!, "rarityOrder": metadata["rarityOrder"]!, "rarityType": metadata["rarityType"]!}
        }
        
        // mintNFT mints a new NFT with a new ID
        // and deposit it in the recipients collection using their collection reference
        pub fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, file: MetadataViews.IPFSFile, royalties: [MetadataViews.Royalty], metadata:{ String: String}){ 
            // create a new NFT
    ...
    init(){ 
        ...
        self.account.link<&CryptoPiggoV2.Collection{NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection}>(self.CollectionPublicPath, target: self.CollectionStoragePath)
