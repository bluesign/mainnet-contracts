...
pub contract Fungables: NonFungibleToken{ 
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &Fungables.NFT?{ 
        let collection = (getAccount(from).getCapability(Fungables.CollectionPublicPath)!).borrow<&Fungables.Collection{Fungables.FungablesCollectionPublic}>() ?? panic("Couldn't get collection")
