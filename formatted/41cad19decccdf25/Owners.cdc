...
pub contract Owners: NonFungibleToken{ 
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &Owners.NFT?{ 
        let collection = (getAccount(from).getCapability(Owners.CollectionPublicPath)!).borrow<&Owners.Collection{Owners.OwnersCollectionPublic}>() ?? panic("Couldn't get collection")
