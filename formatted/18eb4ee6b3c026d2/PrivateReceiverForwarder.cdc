...
pub contract PrivateReceiverForwarder{ 
    ...
    pub resource Forwarder{ 
        ...
        //
        priv var recipient: Capability<&{FungibleToken.Receiver}>
        ...
        init(recipient: Capability<&{FungibleToken.Receiver}>){ 
            pre{ 
                recipient.borrow() != nil:
                    "Could not borrow Receiver reference from the Capability"
            }
    ...
    pub resource Sender{ 
        pub fun sendPrivateTokens(
            _ address: Address,
            tokens: @FungibleToken.Vault
        ){ 
            let account = getAccount(address)
            let privateReceiver =
                account.getCapability<&PrivateReceiverForwarder.Forwarder>(
