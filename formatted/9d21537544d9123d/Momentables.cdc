...
pub contract Momentables: NonFungibleToken{ 
    ...
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver{ 
        ...
        priv let traits:{ String:{ String: String}}
        
        priv let creator: Creator
        
        priv let collaborators: [Collaborator]
        
        priv let momentableCollectionDetails:{ String: String}
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                ...
                case Type<MetadataViews.Royalties>():
                    ...
                    momentableRoyalties.append(MetadataViews.Royalty(reciever: self.creator.creatorWallet, cut: self.creator.creatorRoyalty / 100.0, description: "Sale cut for : ".concat(self.creator.creatorName)))
                    
                    // Add the collaborator's royalty
                    for collaborator in self.collaborators{ 
                        momentableRoyalties.append(MetadataViews.Royalty(reciever: collaborator.collaboratorWallet, cut: collaborator.collaboratorRoyalty / 100.0, description: "Sale cut for : ".concat(collaborator.collaboratorName)))
                    }
                    return MetadataViews.Royalties(cutInfos: momentableRoyalties)
                case Type<MetadataViews.ExternalURL>():
                    return MetadataViews.ExternalURL("https://www.momentable.ai/".concat(self.id.toString()))
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: Momentables.CollectionStoragePath, publicPath: Momentables.CollectionPublicPath, providerPath: /private/MomentablesCollectionProvider, publicCollection: Type<&Momentables.Collection{Momentables.MomentablesCollectionPublic}>(), publicLinkedType: Type<&Momentables.Collection{Momentables.MomentablesCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&Momentables.Collection{Momentables.MomentablesCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Provider, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &Momentables.NFT?{ 
        let collection = getAccount(from).getCapability(Momentables.CollectionPublicPath).borrow<&Momentables.Collection{Momentables.MomentablesCollectionPublic}>() ?? panic("Couldn't get collection")
