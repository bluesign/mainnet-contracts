...
pub contract WineMinter{ 
    ...
    priv var saleAccounts:{ String:{ UInt64:{ Address: UInt64}}}
    
    pub var merchantAccounts:{ String: Address}
    
    priv fun initSaleAccounts(){ 
        if self.saleAccounts["PUBLIC"] == nil{ 
            self.saleAccounts["PUBLIC"] ={} 
        }
    }
    
    priv fun initPublicSaleAccountsForBuyer(
        templateID: UInt64,
        buyer: Address
    ){ 
        if self.saleAccounts["PUBLIC"] == nil{ 
    ...
    priv fun updateSaleAccountsForBuyer(
        templateID: UInt64,
        buyer: Address,
        newValue: UInt64
    ){ 
    ...
    pub fun mintNFTWithDUC(
        ...
        self.initPublicSaleAccountsForBuyer(
        ...
        let admin =
            self.account.borrow<&Wine.Admin>(from: Wine.AdminStoragePath)
        ...
        let merchantDUCReceiverRef =
            getAccount(self.merchantAccounts["DUC"]!).getCapability<
        ...
        let NFTReceiver =
            recipient.getCapability(Wine.CollectionPublicPath).borrow<
    ...
    pub fun mintNFTWithFUT(
        ...
        let merchantFUTReceiverRef =
            getAccount(self.merchantAccounts["FUT"]!).getCapability<
                &{FungibleToken.Receiver}
