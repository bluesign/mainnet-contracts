...
pub contract ZeedzINO: NonFungibleToken{ 
    ...
    pub fun fetch(_ from: Address, zeedleID: UInt64): &ZeedzINO.NFT?{ 
        let collection = (getAccount(from).getCapability(ZeedzINO.CollectionPublicPath)!).borrow<&ZeedzINO.Collection{ZeedzINO.ZeedzCollectionPublic}>() ?? panic("Couldn't get collection")
