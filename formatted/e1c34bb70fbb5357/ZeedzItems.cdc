...
pub contract ZeedzItems: NonFungibleToken{ 
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &ZeedzItems.NFT?{ 
        let collection = (getAccount(from).getCapability(ZeedzItems.CollectionPublicPath)!).borrow<&ZeedzItems.Collection{ZeedzItems.ZeedzItemsCollectionPublic}>() ?? panic("Couldn't get collection")
