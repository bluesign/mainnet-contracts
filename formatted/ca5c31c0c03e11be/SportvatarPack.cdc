...
pub contract SportvatarPack{ 
    ...
    pub fun getPacks(address: Address): [UInt64]?{ 
        ...
        if let packCollection =
            account.getCapability(self.CollectionPublicPath).borrow<
                &{SportvatarPack.CollectionPublic}
            >(){ 
    ...
    access(account) fun createPack(
        ...
        emit Created(
            id: newPack.id,
            prefix: randomString.slice(from: 0, upTo: 5)
        )
        return <-newPack
    }
    ...
    init(){ 
        let wallet =
            getAccount(0x8a86f18e0e05bd9f).getCapability<
        ...
        self.totalSupply = 0
        self.account.save<@SportvatarPack.Collection>(
            <-SportvatarPack.createEmptyCollection(),
            to: SportvatarPack.CollectionStoragePath
        )
        self.account.link<
            &SportvatarPack.Collection{SportvatarPack.CollectionPublic}
        >(
            SportvatarPack.CollectionPublicPath,
            target: SportvatarPack.CollectionStoragePath
        )
