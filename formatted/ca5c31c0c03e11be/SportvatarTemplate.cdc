...
pub contract SportvatarTemplate{ 
    ...
    pub struct SeriesData{ 
        ...
        init(
            ...
            self.metadata = metadata
            self.maxMintable = maxMintable
            self.totalMintedCollectibles = SportvatarTemplate
    ...
    pub struct TemplateData{ 
        ...
        init(
            ...
            self.svg = svg
            self.maxMintableComponents = maxMintableComponents
            self.totalMintedComponents = SportvatarTemplate
                    .getTotalMintedComponents(id: id)!
            self.lastComponentMintedAt = SportvatarTemplate
    ...
    pub fun getTemplates(): [TemplateData]{ 
        ...
        if let templateCollection =
            self.account.getCapability(self.CollectionPublicPath).borrow<
                &{SportvatarTemplate.CollectionPublic}
            >(){ 
    ...
    access(account) fun increaseTotalMintedComponents(id: UInt64){ 
        let totMintedComponents: UInt64? =
            SportvatarTemplate.totalMintedComponents[id]
    ...
    access(account) fun increaseTotalMintedCollectibles(series: UInt64){ 
        let totMintedCollectibles: UInt64? =
            SportvatarTemplate.totalMintedCollectibles[series]
    ...
    access(account) fun createTemplate(
        ...
        SportvatarTemplate.setTotalMintedComponents(
            id: newTemplate.id,
            value: UInt64(0)
        )
        SportvatarTemplate.setLastComponentMintedAt(
            id: newTemplate.id,
            value: UFix64(0)
        )
        return <-newTemplate
    ...
    access(account) fun createSeries(
        ...
        SportvatarTemplate.setTotalMintedCollectibles(
            series: newSeries.id,
            value: UInt64(0)
        )
        return <-newSeries
    ...
    init(){ 
        ...
        self.account.save<@SportvatarTemplate.Collection>(
            <-SportvatarTemplate.createEmptyCollection(),
            to: SportvatarTemplate.CollectionStoragePath
        )
        self.account.link<
            &SportvatarTemplate.Collection{SportvatarTemplate.CollectionPublic}
        >(
            SportvatarTemplate.CollectionPublicPath,
            target: SportvatarTemplate.CollectionStoragePath
        )
