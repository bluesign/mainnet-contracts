...
This is the contract for Laser NFTs!
...
pub contract Laser: NonFungibleToken, ViewResolver{ 
    ...
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver, NiftoryNonFungibleToken.NFTPublic{ 
        ...
        init(setId: Int, templateId: Int, serial: UInt64){ 
            self.id = Laser.totalSupply
            Laser.totalSupply = Laser.totalSupply + 1
            self.setId = setId
    ...
    pub resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection, NiftoryNonFungibleToken.CollectionPublic, NiftoryNonFungibleToken.CollectionPrivate{ 
        ...
        pub fun depositBulk(tokens: @[NonFungibleToken.NFT]){ 
            while tokens.length > 0{ 
                let token <- tokens.removeLast() as! @Laser.NFT
    ...
    pub resource Manager: NiftoryNonFungibleToken.ManagerPublic, NiftoryNonFungibleToken.ManagerPrivate{ 
        ...
        pub fun metadata(): AnyStruct?{ 
            return Laser.metadata
        }
        
        pub fun getSetManagerPublic(): &MutableMetadataSetManager.Manager{MutableMetadataSetManager.Public}{ 
            return NiftoryNFTRegistry.getSetManagerPublic(Laser.REGISTRY_ADDRESS, Laser.REGISTRY_BRAND)
        }
        
        pub fun getMetadataViewsManagerPublic(): &MetadataViewsManager.Manager{MetadataViewsManager.Public}{ 
            return NiftoryNFTRegistry.getMetadataViewsManagerPublic(Laser.REGISTRY_ADDRESS, Laser.REGISTRY_BRAND)
        }
        
        pub fun getNFTCollectionData(): MetadataViews.NFTCollectionData{ 
            return NiftoryNFTRegistry.buildNFTCollectionData(Laser.REGISTRY_ADDRESS, Laser.REGISTRY_BRAND, fun (): @NonFungibleToken.Collection{ 
                    return <-Laser.createEmptyCollection()
                })
        }
        ...
        pub fun modifyContractMetadata(): auth &AnyStruct{ 
            ...
            let maybeMetadata = Laser.metadata
            if maybeMetadata == nil{ 
                let blankMetadata:{ String: String} ={} 
                Laser.metadata = blankMetadata
            }
            return (&Laser.metadata as auth &AnyStruct?)!
        }
        
        pub fun replaceContractMetadata(_ metadata: AnyStruct?){ 
            emit ContractMetadataUpdated()
            Laser.metadata = metadata
        }
        ...
        priv fun _getMetadataViewsManagerPrivate(): &MetadataViewsManager.Manager{MetadataViewsManager.Private}{ 
            let record = NiftoryNFTRegistry.getRegistryRecord(Laser.REGISTRY_ADDRESS, Laser.REGISTRY_BRAND)
            let manager = Laser.account.getCapability<&MetadataViewsManager.Manager{MetadataViewsManager.Private}>(record.metadataViewsManager.paths.private).borrow()!
            return manager
        }
        ...
        priv fun _getSetManagerPrivate(): &MutableMetadataSetManager.Manager{MutableMetadataSetManager.Public, MutableMetadataSetManager.Private}{ 
            let record = NiftoryNFTRegistry.getRegistryRecord(Laser.REGISTRY_ADDRESS, Laser.REGISTRY_BRAND)
            let setManager = Laser.account.getCapability<&MutableMetadataSetManager.Manager{MutableMetadataSetManager.Public, MutableMetadataSetManager.Private}>(record.setManager.paths.private).borrow()!
            return setManager
        }
    ...
    pub fun contract(): &{NiftoryNonFungibleToken.ManagerPublic}{ 
        return NiftoryNFTRegistry.getNFTManagerPublic(Laser.REGISTRY_ADDRESS, Laser.REGISTRY_BRAND)
    }
    ...
    init(nftManagerProxy: &{NiftoryNonFungibleTokenProxy.Public, NiftoryNonFungibleTokenProxy.Private}){ 
        let record = NiftoryNFTRegistry.generateRecord(account: self.account.address, project: "cla5ao46p00010gl34xs81k1l_Laser")
        self.REGISTRY_ADDRESS = 0x32d62d5c43ad1038
        self.REGISTRY_BRAND = "cla5ao46p00010gl34xs81k1l_Laser"
        ...
        self.account.save<@MutableMetadataSetManager.Manager>(<-MutableMetadataSetManager.create(name: "Laser", description: "The set manager for Laser."), to: record.setManager.paths.storage)
