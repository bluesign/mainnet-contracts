...
pub contract UserProfile{ 
    ...
    pub event ProfileUpsertIdentity(
        profile: Address,
        platform: String,
        uid: String,
        name: String,
    ...
    pub event ProfileSeasonAddPoints(
        profile: Address,
        seasonId: UInt64,
        points: UInt64
    )
    ...
    pub event MissionRecordUpdateParams(
        profile: Address,
        missionKey: String,
        step: Int,
        keys: [
    ...
    pub event MissionRecordUpdateResult(
        profile: Address,
        missionKey: String,
        step: Int,
        result: Bool,
    ...
    pub struct VerificationStep{ 
        ...
        access(contract) fun updateParams(
            idx: Int,
            params:{ 
                String: AnyStruct
            }
    ...
    pub struct MissionRecord{ 
        ...
        access(contract) fun updateVerifactionParams(
            step: Int,
            params:{ 
                String: AnyStruct
            }
        ){ 
            self.steps[step].updateParams(
                idx: self.getLatestIndex(),
                params: params
            )
        }
    ...
    pub resource Profile: Interfaces.ProfilePublic, ProfilePrivate{ 
        priv let campetitionServiceCap: Capability<&{Interfaces.CompetitionServicePublic}>
        
        priv var linkedIdentities:{ String: Interfaces.LinkedIdentity}
        
        priv var seasonScores:{ UInt64: SeasonRecord}
        
        priv let referredFromAddress: Address?
        
        priv var referralCode: String?
        ...
        priv fun fetchOrCreateMissionRecordRef(missionKey: String): &MissionRecord{ 
        ...
        priv fun borrowSeasonRecordRef(_ seasonId: UInt64): &SeasonRecord?{ 
    ...
    pub fun borrowUserProfilePublic(_ acct: Address): &Profile{
        Interfaces.ProfilePublic
    }{ 
        return getAccount(acct).getCapability<
            &Profile{Interfaces.ProfilePublic}
        >(UserProfile.ProfilePublicPath).borrow()
