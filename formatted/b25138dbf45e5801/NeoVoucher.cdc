...
pub contract NeoVoucher: NonFungibleToken{ 
    ...
    access(account) fun consume(voucherID: UInt64, rewardID: UInt64){ 
        ...
        let receiver = recipient.getCapability(NeoMember.CollectionPublicPath).borrow<&NeoMember.Collection{NonFungibleToken.Receiver}>() ?? panic("Could not borrow a reference to the recipient's collection")
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &NeoVoucher.NFT?{ 
        let collection = getAccount(from).getCapability(NeoVoucher.CollectionPublicPath).borrow<&NeoVoucher.Collection>() ?? panic("Couldn't get collection")
