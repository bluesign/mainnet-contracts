...
import StarVaultFactory from "../0xb6cb9ef4b1692616/StarVaultFactory.cdc"
...
pub contract RewardPool{ 
    ...
    priv var balances:{ Address: UFix64}
    
    pub fun getBalance(account: Address): UFix64{ 
        let collectionRef =
            getAccount(account).getCapability<
                &LPStaking.LPStakingCollection{
                    StarVaultInterfaces.LPStakingCollectionPublic
                }
            >(StarVaultConfig.LPStakingCollectionPublicPath).borrow()
    ...
    pub fun rewardPerToken(): UFix64{ 
        ...
        return self.rewardPerTokenStored
        + (self.lastTimeRewardApplicable() - self.lastUpdateTime)
        * self.rewardRate
        / self.totalSupply
    }
    ...
    pub fun earned(account: Address): UFix64{ 
        ...
        return balance * (self.rewardPerToken() - userRewardPerTokenPaid)
        + rewards
    }
    ...
    pub fun queueNewRewards(vault: @FungibleToken.Vault){ 
        ...
        let receiver =
            self.account.getCapability<&AnyResource{FungibleToken.Receiver}>(
    ...
    priv fun notifyRewardAmount(rewards: UFix64){ 
    ...
    init(pid: Int, stakeToken: Address){ 
        ...
        self.account.link<&{StarVaultInterfaces.PoolPublic}>(
            StarVaultConfig.PoolPublicPath,
            target: poolStoragePath
        )
        let collectionStoragePath =
            StarVaultConfig.LPStakingCollectionStoragePath
        ...
        self.account.link<&{StarVaultInterfaces.LPStakingCollectionPublic}>(
