pub contract DataObject{ 
    ...
    pub resource Collection{ 
        ...
        pub fun removeObject(objectId: String){ 
            let item <-
                self.objects.remove(key: objectId) ?? panic("Cannot remove")
            destroy item
        }
    ...
    pub fun check(objectID: String, address: Address): Bool{ 
        return getAccount(address).getCapability<&Collection>(self.publicPath)
            .check()
    }
    ...
    init(){ 
        ...
        self.account.save(
            <-self.createEmptyObjectCollection(),
            to: self.privatePath
        )
