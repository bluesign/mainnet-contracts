...
pub contract CaaPass: NonFungibleToken{ 
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            ...
            switch view{ 
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: CaaPass.CollectionStoragePath, publicPath: CaaPass.CollectionPublicPath, providerPath: /private/caaPassCollection, // not exists for now publicCollection: Type<&CaaPass.Collection{CaaPass.CollectionPublic}>(), publicLinkedType: Type<&CaaPass.Collection{CaaPass.CollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&CaaPass.Collection{CaaPass.CollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Provider, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &CaaPass.NFT?{ 
        let collection = (getAccount(from).getCapability(CaaPass.CollectionPublicPath)!).borrow<&CaaPass.Collection>() ?? panic("Couldn't get collection")
