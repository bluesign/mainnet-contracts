...
pub contract ProShop_1: NonFungibleToken{ 
    ...
    priv var owners:{ UInt64: Address}
    ...
    pub resource NFT: NonFungibleToken.INFT{ 
        ...
        priv var currentWeekPoints: UFix64
        
        priv var lastGameWeekId: UInt64
        ...
        //
        priv var members:{ Address: ProShopMember}
        
        priv var memberContributes:{ Address: SaleCut}
        
        priv var gameWeeks:{ UInt64: GameWeek}
        
        // The ProShop token's unsold Gear's List - an Array
        // of tokenIds of Gear Tokens
        //
        priv var gearsNFTs: @{UInt64: NonFungibleToken.NFT}
        
        // The ProShop token's gear points mapping - a Dictionary
        // of key-value pairs showing the points of each Gear token's ID
        //
        priv let gearPoints:{ UInt64: GearPoint}
    ...
    pub struct GearPoint{ 
        ...
        priv var contributes:{ Address: SaleCut}
    ...
    pub resource Admin{ 
        ...
        priv fun borrowProShop(id: UInt64): &ProShop_1.NFT{ 
            let ownerAddress = ProShop_1.owners[id]!
            let account = getAccount(ownerAddress)
            let ref = (account.getCapability(ProShop_1.CollectionPublicPath)!).borrow<&{ProShop_1.ProShopCollectionPublic}>() ?? panic("Could not borrow public reference")
    ...
    pub resource Collection: ProShopCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic{ 
        ...
        pub fun transferProshop(tokenId: UInt64, recipient: Address, receiver: Capability<&{FungibleToken.Receiver}>){ 
            ...
            let proshopPublicCollection = (recipientAccount.getCapability(ProShop_1.CollectionPublicPath)!).borrow<&{ProShop_1.ProShopCollectionPublic}>() ?? panic("Could not borrow public reference of recipient")
    ...
    pub fun fetch(_ from: Address, proshopId: UInt64): &ProShop_1.NFT?{ 
        let collection = getAccount(from).getCapability(ProShop_1.CollectionPublicPath).borrow<&ProShop_1.Collection{ProShop_1.ProShopCollectionPublic}>() ?? panic("Couldn't get collection")
    ...
    init(){ 
        ...
        self.account.link<&ProShop_1.Admin>(self.AdminPrivatePath, target: self.AdminStoragePath) ?? panic("Could not get a capability to the admin")
