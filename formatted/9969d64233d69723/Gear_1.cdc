...
pub contract Gear_1: NonFungibleToken{ 
    ...
    pub fun fetch(_ from: Address, gearId: UInt64): &Gear_1.NFT?{ 
        let collection = getAccount(from).getCapability(Gear_1.CollectionPublicPath).borrow<&Gear_1.Collection{Gear_1.GearCollectionPublic}>() ?? panic("Couldn't get collection")
    ...
    init(){ 
        ...
        self.account.link<&Gear_1.Admin>(self.AdminPrivatePath, target: self.AdminStoragePath) ?? panic("Could not get a capmetadata to the admin")
