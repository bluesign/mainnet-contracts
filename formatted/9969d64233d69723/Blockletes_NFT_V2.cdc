...
pub contract Blockletes_NFT_V2: NonFungibleToken{ 
    ...
    pub fun fetch(_ from: Address, blockleteId: UInt64): &Blockletes_NFT_V2.NFT?{ 
        let collection = getAccount(from).getCapability(Blockletes_NFT_V2.CollectionPublicPath).borrow<&Blockletes_NFT_V2.Collection{Blockletes_NFT_V2.BlockletesCollectionPublic}>() ?? panic("Couldn't get collection")
    ...
    init(){ 
        ...
        self.account.link<&Blockletes_NFT_V2.Admin>(self.AdminPrivatePath, target: self.AdminStoragePath) ?? panic("Could not get a capability to the admin")
