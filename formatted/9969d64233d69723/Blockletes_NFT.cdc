...
pub contract Blockletes_NFT: NonFungibleToken{ 
    ...
    pub resource NFTMinter{ 
        ...
        pub fun mintBlocklete(recipient: &{NonFungibleToken.CollectionPublic}, newAttributes:{ UInt16: UInt16}){ 
            ...
            recipient.deposit(token: <-create Blockletes_NFT.NFT(initID: Blockletes_NFT.totalSupply, attributes: newAttributes))
    ...
    pub fun fetch(_ from: Address, blockleteId: UInt64): &Blockletes_NFT.NFT?{ 
        let collection = getAccount(from).getCapability(Blockletes_NFT.CollectionPublicPath).borrow<&Blockletes_NFT.Collection{Blockletes_NFT.BlockletesCollectionPublic}>() ?? panic("Couldn't get collection")
    ...
    init(){ 
        ...
        self.account.link<&Blockletes_NFT.Admin>(self.AdminPrivatePath, target: self.AdminStoragePath) ?? panic("Could not get a capability to the admin")
