...
import
    ...
    from "../0x3cdbb3d569211ff3/FlowtyListingCallback.cdc"
...
pub contract FlowtyRentals{ 
    ...
    // that is on them.
    //
    pub event FlowtyRentalsStorefrontInitialized(
        flowtyRentalsStorefrontResourceID: UInt64
    )
    ...
    //
    pub event FlowtyRentalsStorefrontDestroyed(
        flowtyRentalsStorefrontResourceID: UInt64
    )
    ...
    pub event FlowtyRentalsMarketplaceInitialized(
        flowtyRentalsMarketplaceResourceID: UInt64
    )
    ...
    // Note that we do not specify an address.
    //
    pub event FlowtyRentalsMarketplaceDestroyed(
        flowtyRentalsMarketplaceResourceID: UInt64
    )
    ...
    pub struct ListingDetails{ 
        ...
        priv let paymentCut: Flowty.PaymentCut
        ...
        init(
            ...
            assert(
                paymentCut.amount > 0.0,
                message: "Listing must have non-zero requested amount"
            )
    ...
    pub resource interface ListingPublic{ 
        ...
        pub fun rent(
            ...
            renterNFTCollection: Capability<
                &AnyResource{NonFungibleToken.CollectionPublic}
            >,
    ...
    pub resource Listing: ListingPublic, FlowtyListingCallback.Listing{ 
        ...
        // if renterNFTProvider is provided, giving us a way obtain the the rented nft automatically
        // to be returned.
        pub fun rent(payment: @FungibleToken.Vault, renterFungibleTokenReceiver: Capability<&{FungibleToken.Receiver}>, renterNFTCollection: Capability<&AnyResource{NonFungibleToken.CollectionPublic}>, renterNFTProvider: Capability<&AnyResource{NonFungibleToken.CollectionPublic, NonFungibleToken.Provider}>?){ 
            pre{ 
            ...
            let depositor = FlowtyRentals.account.borrow<&LostAndFound.Depositor>(from: LostAndFound.DepositorStoragePath)!
            if royalties != nil{ 
        ...
        init(nftProviderCapability: Capability<&AnyResource{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>, nftPublicCollectionCapability: Capability<&AnyResource{NonFungibleToken.CollectionPublic}>, ownerFungibleTokenReceiver: Capability<&AnyResource{FungibleToken.Receiver}>, nftType: Type, nftID: UInt64, amount: UFix64, deposit: UFix64, term: UFix64, paymentVaultType: Type, paymentCut: Flowty.PaymentCut, storefrontID: UInt64, expiresAfter: UFix64, renter: Address?){ 
            pre{ 
        ...
        destroy(){ 
            ...
            emit ListingDestroyed(flowtyStorefrontAddress: self.ownerFungibleTokenReceiver.address, flowtyStorefrontID: self.details.flowtyStorefrontID, listingResourceID: self.uuid, nftID: self.details.nftID, nftType: self.details.nftType.identifier)
        }
    }
    ...
    pub resource Rental: RentalPublic, FlowtyListingCallback.Listing{ 
        // The non-resource data of a Rental
        priv let details: RentalDetails
        
        // The details of the listing that was rented
        priv let listingDetails: ListingDetails
        ...
        destroy(){ 
            pre{ 
                self.details.settled || self.details.returned:
                    "rental must be returned or settled to be destroyed"
                self.depositedFungibleTokens == nil || self.depositedFungibleTokens?.balance == 0.0:
                    "deposit balance is not 0"
            }
            if self.depositedFungibleTokens != nil{ 
                let deposit <- self.depositedFungibleTokens <- nil
                destroy deposit
    ...
    pub resource interface FlowtyRentalsMarketplaceManager{ 
        ...
        access(contract) fun createRental(
            ...
            ownerNFTCollectionPublic: Capability<
                &AnyResource{NonFungibleToken.CollectionPublic}
            >,
    ...
    pub resource interface FlowtyRentalsMarketplacePublic{ 
        ...
        pub fun borrowRental(rentalResourceID: UInt64): &Rental{
            RentalPublic
        }?{} 
    }
    
    pub resource FlowtyRentalsMarketplace:
        FlowtyRentalsMarketplaceManager,
        FlowtyRentalsMarketplacePublic{
    
        priv var rentals: @{UInt64: Rental}
        ...
        pub fun removeRental(rentalResourceID: UInt64){ 
            let rental <-
                self.rentals.remove(key: rentalResourceID)
            ...
            assert(
                rental.getDetails().returned == true
                || rental.getDetails().settled == true,
        ...
        destroy(){ 
            ...
            emit FlowtyRentalsMarketplaceDestroyed(
                flowtyRentalsMarketplaceResourceID: self.uuid
            )
        }
        ...
        init(){ 
            ...
            emit FlowtyRentalsMarketplaceInitialized(
                flowtyRentalsMarketplaceResourceID: self.uuid
            )
        }
    }
    ...
    pub resource interface FlowtyRentalsStorefrontManager{ 
        pub fun createListing(
            ...
            >,
            nftPublicCollectionCapability: Capability<
                &AnyResource{NonFungibleToken.CollectionPublic}
            >,
            ownerFungibleTokenReceiver: Capability<
    ...
    pub resource FlowtyRentalsStorefront:
        ...
        pub fun createListing(
            ...
            assert(
                FlowtyUtils.isSupported(ref),
                message: "nft type is not supported"
            )
            let royalties =
                ref.resolveView(Type<MetadataViews.Royalties>())
                as!
        ...
        pub fun removeListing(listingResourceID: UInt64){ 
            let listing <-
                self.listings.remove(key: listingResourceID)
        ...
        destroy(){ 
            ...
            emit FlowtyRentalsStorefrontDestroyed(
                flowtyRentalsStorefrontResourceID: self.uuid
            )
        }
        ...
        init(){ 
            ...
            // Let event consumers know that this storefront exists
            emit FlowtyRentalsStorefrontInitialized(
                flowtyRentalsStorefrontResourceID: self.uuid
            )
        }
    }
    ...
    access(account) fun borrowMarketplace(): &FlowtyRentals.FlowtyRentalsMarketplace{ 
        return self.account.borrow<&FlowtyRentals.FlowtyRentalsMarketplace>(
            from: FlowtyRentals.FlowtyRentalsMarketplaceStoragePath
        )!
    }
    
    pub fun borrowMarketplacePublic(): &FlowtyRentalsMarketplace{
        FlowtyRentalsMarketplacePublic
    }{ 
        let mp =
            self.account.borrow<
                &FlowtyRentalsMarketplace{FlowtyRentalsMarketplacePublic}
            >(from: FlowtyRentals.FlowtyRentalsMarketplaceStoragePath)!
        return mp
    }
    
    pub fun borrowStorefrontPublic(addr: Address): &FlowtyRentalsStorefront{
        FlowtyRentalsStorefrontPublic
    }?{ 
        return getAccount(addr).getCapability<
            &FlowtyRentalsStorefront{FlowtyRentalsStorefrontPublic}
        >(FlowtyRentals.FlowtyRentalsStorefrontPublicPath).borrow()
    }
    
    pub fun settleRental(rentalResourceID: UInt64){ 
        let marketplace = FlowtyRentals.borrowMarketplace()
        let rental =
            marketplace.borrowRental(rentalResourceID: rentalResourceID)
        (rental!).settleRental()
    }
    ...
    access(contract) fun borrowCallbackContainer(): &FlowtyListingCallback.Container?{ 
        return self.account.borrow<&FlowtyListingCallback.Container>(
    ...
    init(){ 
        self
            .FlowtyRentalsStorefrontStoragePath = /storage/FlowtyRentalsStorefront
        self.FlowtyRentalsStorefrontPublicPath = /public/FlowtyRentalsStorefront
        self
            .FlowtyRentalsMarketplaceStoragePath = /storage/FlowtyRentalsMarketplace
        self
            .FlowtyRentalsMarketplacePublicPath = /public/FlowtyRentalsMarketplace
        ...
        self.account.save(
            <-marketplace,
            to: self.FlowtyRentalsMarketplaceStoragePath
        )
        self.account.link<
            &FlowtyRentals.FlowtyRentalsMarketplace{
                FlowtyRentals.FlowtyRentalsMarketplacePublic
            }
        >(
            FlowtyRentals.FlowtyRentalsMarketplacePublicPath,
            target: FlowtyRentals.FlowtyRentalsMarketplaceStoragePath
        )
