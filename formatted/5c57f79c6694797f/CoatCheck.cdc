...
pub contract CoatCheck{ 
    ...
    pub event TicketCreated(
        ticketResourceID: UInt64,
        redeemer: Address,
        fungibleTokenTypes: [
            Type
        ],
    ...
    pub struct TicketDetails{ 
        ...
        init(redeemer: Address, nftTypes: [Type], vaultTypes: [Type]){ 
    ...
    pub resource Ticket: TicketPublic{ 
        ...
        priv var fungibleTokenVaults: @[FungibleToken.Vault]?
        
        priv var nonFungibleTokens: @[NonFungibleToken.NFT]?
        
        pub let details: TicketDetails
        
        // The following variables are maintained by the CoatCheck contract
        priv var redeemed: Bool // a ticket can only be redeemed once. It also cannot be destroyed unless redeemed is set to true
        
        priv var storageFee: UFix64 // the storage fee taken to hold this ticket in storage. It is returned when the ticket is redeemed.
        
        init(fungibleTokenVaults: @[FungibleToken.Vault]?, nonFungibleTokens: @[NonFungibleToken.NFT]?, redeemer: Address){ 
            assert(fungibleTokenVaults != nil || nonFungibleTokens != nil, message: "must provide either FungibleToken vaults or NonFungibleToken NFTs")
            let ftTypes: [Type] = []
            let nftTypes: [Type] = []
        ...
        pub fun redeem(fungibleTokenReceiver: &{FungibleToken.Receiver}?, nonFungibleTokenReceiver: &{NonFungibleToken.CollectionPublic}?){ 
            pre{ 
                fungibleTokenReceiver == nil || ((fungibleTokenReceiver!).owner!).address == self.details.redeemer:
                    "incorrect owner"
                nonFungibleTokenReceiver == nil || ((nonFungibleTokenReceiver!).owner!).address == self.details.redeemer:
                    "incorrect owner"
                self.fungibleTokenVaults == nil || fungibleTokenReceiver != nil:
                    "must provide fungibleTokenReceiver when there is a vault to claim"
                self.nonFungibleTokens == nil || nonFungibleTokenReceiver != nil:
                    "must provide nonFungibleTokenReceiver when there is a vault to claim"
            }
            self.redeemed = true
    ...
    pub resource Valet: ValetPublic{ 
        ...
        access(account) fun createTicket(redeemer: Address, vaults: @[FungibleToken.Vault]?, tokens: @[NonFungibleToken.NFT]?){ 
            let ticket <- create Ticket(fungibleTokenVaults: <-vaults, nonFungibleTokens: <-tokens, redeemer: redeemer)
        ...
        pub fun borrowTicket(ticketID: UInt64): &Ticket{TicketPublic}?{ 
            if self.tickets[ticketID] != nil{ 
                return &self.tickets[ticketID] as                                                  
        ...
        pub fun redeemTicket(ticketID: UInt64, fungibleTokenReceiver: &{FungibleToken.Receiver}?, nonFungibleTokenReceiver: &{NonFungibleToken.CollectionPublic}?){ 
            pre{ 
