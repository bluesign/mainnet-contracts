...
pub contract ReddieCoin: FungibleToken, ViewResolver{ 
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, MetadataViews.Resolver{ 
        ...
        pub fun withdraw(amount: UFix64): @FungibleToken.Vault{ 
            ...
            if let owner: Address = self.owner?.address{ 
                ReddieCoin.setBalance(address: owner, balance: self.balance)
            }
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                case Type<FungibleTokenMetadataViews.FTView>():
                    return ReddieCoin.resolveView(view)
        ...
        destroy(){ 
            if self.balance > 0.0{ 
                ...
                ReddieCoin.totalSupply = ReddieCoin.totalSupply - self.balance
            }
        }
    }
    ...
    pub resource Minter: Toucans.Minter{ 
        pub fun mint(amount: UFix64): @Vault{ 
            post{ 
                ReddieCoin.maxSupply == nil || ReddieCoin.totalSupply <= ReddieCoin.maxSupply!:
                    "Exceeded the max supply of tokens allowd."
            }
            ReddieCoin.totalSupply = ReddieCoin.totalSupply + amount
    ...
    pub fun resolveView(_ view: Type): AnyStruct?{ 
        switch view{ 
            ...
            case Type<FungibleTokenMetadataViews.FTDisplay>():
                ...
                return FungibleTokenMetadataViews.FTDisplay(name: "Reddie Coin", symbol: "RED", description: "I made Red Coin because my family spends all my other tangible coins. ", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("reddiemiller31"), "discord": MetadataViews.ExternalURL("")})
            case Type<FungibleTokenMetadataViews.FTVaultData>():
                return FungibleTokenMetadataViews.FTVaultData(storagePath: ReddieCoin.VaultStoragePath, receiverPath: ReddieCoin.ReceiverPublicPath, metadataPath: ReddieCoin.VaultPublicPath, providerPath: /private/ReddieCoinVault, receiverLinkedType: Type<&Vault{FungibleToken.Receiver}>(), metadataLinkedType: Type<&Vault{FungibleToken.Balance, MetadataViews.Resolver}>(), providerLinkedType: Type<&Vault{FungibleToken.Provider}>(), createEmptyVaultFunction: fun (): @Vault{ 
                        return <-ReddieCoin.createEmptyVault()
                    })
        }
        return nil
    }
    ...
    init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
        ...
        // Paths
        self.VaultStoragePath = /storage/ReddieCoinVault
        self.ReceiverPublicPath = /public/ReddieCoinReceiver
        self.VaultPublicPath = /public/ReddieCoinMetadata
        self.MinterStoragePath = /storage/ReddieCoinMinter
        self.AdministratorStoragePath = /storage/ReddieCoinAdmin
