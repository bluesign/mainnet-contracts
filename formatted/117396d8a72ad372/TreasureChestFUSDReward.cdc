...
pub contract TreasureChestFUSDReward{ 
    ...
    pub resource interface Public{ 
        ...
        pub fun claimReward(
            recipient: &FUSD.Vault{FungibleToken.Receiver},
    ...
    pub resource CentralizedInbox: Public{ 
        ...
        priv var claimed:{ UInt64: Address}
        
        // The chest and the vault with the reward amount
        priv var rewards: @{UInt64: FUSD.Vault}
        
        // List of addresses and their bonus rewards
        priv var bonusRewards:{ Address: String}
        ...
        destroy(){ 
            pre{ 
                self.rewards.length == 0:
                    "Can't destroy: rewards are left in the inbox"
            }
    ...
    init(){ 
        // Set named paths
        self
            .CentralizedInboxStoragePath = /storage/BasicBeastsTreasureChestFUSDReward
        self
            .CentralizedInboxPrivatePath = /private/BasicBeastsTreasureChestFUSDRewardUpgrade
        self
            .CentralizedInboxPublicPath = /public/BasicBeastsTreasureChestFUSDReward
        
        // Put CentralizedInbox in storage
        self.account.save(
            <-create CentralizedInbox(),
            to: self.CentralizedInboxStoragePath
        )
        self.account.link<&TreasureChestFUSDReward.CentralizedInbox>(
            self.CentralizedInboxPrivatePath,
            target: self.CentralizedInboxStoragePath
        )
        ?? panic("Could not get a capability to the Centralized Inbox")
        self.account.link<&TreasureChestFUSDReward.CentralizedInbox{Public}>(
            self.CentralizedInboxPublicPath,
            target: self.CentralizedInboxStoragePath
        )
        ?? panic("Could not get a capability to the Centralized Inbox")
    }
