...
pub contract Migrator{ 
    ...
    priv let balanceMap:{ Address: UFix64}
    ...
    pub fun migrate(from: @FungibleToken.Vault): @FungibleToken.Vault{ 
        ...
        destroy from
        let collectionRef =
            self.account.borrow<&StarVaultFactory.VaultTokenCollection>(
                from: StarVaultConfig.VaultTokenCollectionStoragePath
            )!
        return <-collectionRef.withdraw(
            vault: self.vaultAddress,
            amount: balance
        )
    }
    ...
    pub fun withdraw(to: Address){ 
        ...
        let vault <-
            collectionRef.withdraw(vault: self.vaultAddress, amount: balance)
        self.balanceMap[to] = 0.0
        var toCollectionRef =
            getAccount(to).getCapability<
                &{StarVaultInterfaces.VaultTokenCollectionPublic}
            >(StarVaultConfig.VaultTokenCollectionPublicPath).borrow()!
    ...
    init(fromTokenKey: String, vaultAddress: Address){ 
        ...
        self.account.link<
            &StarVaultFactory.VaultTokenCollection{
                StarVaultInterfaces.VaultTokenCollectionPublic
            }
        >(StarVaultConfig.VaultTokenCollectionPublicPath, target: storagePath)
    }
}
