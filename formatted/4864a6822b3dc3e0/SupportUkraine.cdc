...
pub contract SupportUkraine: NonFungibleToken{ 
    ...
    pub resource NFTMinter{ 
        ...
        //
        pub fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, description: String, image: String, name: String, series: String){ 
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &SupportUkraine.NFT?{ 
        let collection = (getAccount(from).getCapability(SupportUkraine.CollectionPublicPath)!).borrow<&SupportUkraine.Collection{SupportUkraine.SupportUkraineCollectionPublic}>() ?? panic("Couldn't get collection")
