...
pub contract DateUtils{ 
    ...
    pub struct Date{ 
        ...
        pub fun toTwoDigitString(_ num: Int): String{ 
            ...
            let formattedNumber =
                raw.slice(from: raw.length - 2, upTo: raw.length)
            return formattedNumber
        }
        
        pub fun toString(): String{ 
            return self.toTwoDigitString(self.day).concat("-").concat(
                self.toTwoDigitString(self.month).concat("-").concat(
                    self.year.toString()
                )
            )
        }
        ...
        pub fun equals(_ other: Date): Bool{ 
            return self.day == other.day && self.month == other.month
    ...
    // Auxiliary functions
    priv fun isLeapYear(_ year: Int): Bool{ 
        return year % 400 == 0 || year % 4 == 0 && year % 100 != 0
    }
    
    priv fun daysForYear(_ year: Int): Int{ 
        return self.isLeapYear(year) ? 366 : 365
    }
    
    priv fun daysPerMonth(_ year: Int): [Int]{ 
        return [
            0,
            31,
