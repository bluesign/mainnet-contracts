...
pub contract PonsNftContract_v1: PonsNftContractInterface, NonFungibleToken{ 
    /* Storage path at which the Minter will be stored */    access(account) let MinterStoragePath: StoragePath
    
    /* Capability to the Pons NFT Minter, for convenience of usage */    access(account) let MinterCapability: Capability<&NftMinter_v1>
    
    /* PonsNftContractInit_v1 is emitted on initialisation of this contract */    pub event PonsNftContractInit_v1()
    
    /* ContractInitialized is emitted on initilisation of this contract likewise, this variant satisfies requirements from the NonFungibleToken interface */    pub event ContractInitialized()
    
    /* Withdraw is emitted on withdrawals of Pons NFTs from a PonsCollection, this event satisfying requirements from the NonFungibleToken interface */    pub event Withdraw(id: UInt64, from: Address?)
    
    /* Withdraw is emitted on deposits of Pons NFTs to a PonsCollection, this event satisfying requirements from the NonFungibleToken interface */    pub event Deposit(id: UInt64, to: Address?)
    
    /* Represents the total number of Pons NFTs (v1) created, satisfying requirements from the NonFungibleToken interface */    pub var totalSupply: UInt64
    
    /* Map from nftId to serialNumber */    access(account) var ponsNftSerialNumbers:{ String: UInt64}
    
    /* Map from serialNumber to nftId */    access(account) var ponsNftIds:{ UInt64: String}
    
    /* Map from nftId to ponsArtistId */    access(account) var ponsNftArtistIds:{ String: String}
    
    /* Map from nftId to royalties ratio */    access(account) var ponsNftRoyalties:{ String: PonsUtils.Ratio}
    
    /* Map from nftId to edition label */    access(account) var ponsNftEditionLabels:{ String: String}
    
    /* Map from nftId to metadata */    access(account) var ponsNftMetadatas:{ String:{ String: String}}
    
    /* Inserts or updates the royalty for a NFT */    access(account) fun insertRoyalty(nftId: String, royalty: PonsUtils.Ratio): PonsUtils.Ratio?{ 
        return self.ponsNftRoyalties.insert(key: nftId, royalty)
    }
    
    /* Inserts or updates the edition label for a NFT */    access(account) fun insertEditionLabel(nftId: String, editionLabel: String): String?{ 
        return self.ponsNftEditionLabels.insert(key: nftId, editionLabel)
    }
    
    /* Inserts or updates the metadata for a NFT */    access(account) fun insertMetadata(nftId: String, metadata:{ String: String}):{ String: String}?{ 
        return self.ponsNftMetadatas.insert(key: nftId, metadata)
    }
    
    /* Removes the royalty for a NFT */    access(account) fun removeRoyalty(nftId: String): PonsUtils.Ratio?{ 
        return self.ponsNftRoyalties.remove(key: nftId)
    }
    
    /* Removes the edition label for a NFT */    access(account) fun removeEditionLabel(nftId: String): String?{ 
        return self.ponsNftEditionLabels.remove(key: nftId)
    }
    
    /* Removes the metadata for a NFT */    access(account) fun removeMetadata(nftId: String):{ String: String}?{ 
        return self.ponsNftMetadatas.remove(key: nftId)
    }
    
    /* The concrete Pons NFT resource. Striaghtforward implementation of the PonsNft and INFT interfaces */    pub resource NFT: PonsNftContractInterface.PonsNft, NonFungibleToken.INFT{ 
        /* Ensures the authenticity of this PonsNft; requirement by PonsNft */        pub let ponsCertification: @PonsCertificationContract.PonsCertification
        
        /* Unique identifier for the NFT; requirement by PonsNft */        pub let nftId: String
        
        /* Unique identifier for the NFT; requirement by INFT */        pub let id: UInt64
        ...
        init(nftId: String, serialNumber: UInt64){ 
            ...
            /* Proof of creation by Pons */            self.ponsCertification <- PonsCertificationContract.makePonsCertification()
            self.nftId = nftId
            self.id = serialNumber
            /* For the sake of consistency, the link between nftId and serialNumber should be established once it is known */            PonsNftContract_v1.ponsNftSerialNumbers.insert(key: nftId, serialNumber)
    ...
    /* The concrete Pons Collection resource. Striaghtforward implementation of the PonsCollection, Provider, Receiver, and Collection interfaces */    pub resource Collection: PonsNftContractInterface.PonsCollection, PonsNftContractInterface.PonsNftReceiver, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic{ 
        /* Ensures the authenticity of this PonsCollection; requirement from PonsCollection */        pub let ponsCertification: @PonsCertificationContract.PonsCertification
        
        /* Stores the owned NFT resources; requirement from NonFungibleToken.Collection */        pub var ownedNFTs: @{UInt64: NonFungibleToken.NFT}
        
        /* Withdraw an NFT from the PonsCollection, given its nftId */        pub fun withdrawNft(nftId: String): @PonsNftContractInterface.NFT{ 
            pre{ 
        ...
        /* Deposit a NFT from the PonsCollection, given its nftId */        pub fun depositNft(_ ponsNft: @PonsNftContractInterface.NFT): Void{ 
            pre{ 
        ...
        /* Get a list of nftIds stored in the PonsCollection */        pub fun getNftIds(): [String]{ 
        ...
        /* Borrow a reference to a NFT in the PonsCollection, given its nftId */        pub fun borrowNft(nftId: String): &PonsNftContractInterface.NFT{ 
            pre{ 
                self.ownedNFTs.containsKey(PonsNftContract_v1.ponsNftSerialNumbers[nftId]!):
                    "Pons NFT with this nftId not found"
            }
        ...
        /* Withdraw an NFT from the PonsCollection, given its serialNumber */        pub fun withdraw(withdrawID: UInt64): @NonFungibleToken.NFT{ 
            let nftId = PonsNftContract_v1.ponsNftIds[withdrawID]!
            var nft <- self.withdrawNft(nftId: nftId) as! @NonFungibleToken.NFT
            return <-nft
        }
        
        /* Deposit an NFT to the PonsCollection */        pub fun deposit(token: @NonFungibleToken.NFT): Void{ 
            var nft <- token as! @PonsNftContractInterface.NFT
            self.depositNft(<-nft)
        }
        
        /* Get a list of serialNumbers stored in the PonsCollection */        pub fun getIDs(): [UInt64]{ 
            return self.ownedNFTs.keys
        }
        
        /* Borrow a reference to a NFT in the PonsCollection, given its serialNumber */        pub fun borrowNFT(id: UInt64): &NonFungibleToken.NFT{ 
            pre{ 
    ...
    /* Create a empty Pons Collection */    pub fun createEmptyCollection(): @Collection{ 
    ...
    */
    ...
    pub resource NftMinter_v1{ 
        /* Stores available nftIds for new new NFTs */        access(account) var nftIds: [String]
        
        /* Adds nftIds to the pool of available nftIds */        pub fun refillMintIds(mintIds: [String]){ 
            var idIndex = 0
        ...
        /* Mints a new Pons NFT v1 */        pub fun mintNft(_ artistCertificate: &PonsNftContract.PonsArtistCertificate, royalty: PonsUtils.Ratio, editionLabel: String, metadata:{ String: String}): @PonsNftContractInterface.NFT{ 
            pre{ 
    ...
    /* A straightforward instance of PonsNftContractImplementation which utilises PonsNftContract_v1 for all its functionality, used on initialization of the PonsNftContract_v1 contract. */    pub resource PonsNftContractImplementation_v1: PonsNftContract.PonsNftContractImplementation{ 
