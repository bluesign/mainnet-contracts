...
pub contract PonsNftMarketContract_v1{ 
    /* PonsNftMarketContractInit_v1 is emitted on initialisation of this contract */    pub event PonsNftMarketContractInit_v1()
    
    /* FundsHeldOnBehalfOfArtist is emitted when an NFT is purchased, but the artist's Capability to receive payment is invalid, and the market holds the funds on behalf of the artist */    pub event FundsHeldOnBehalfOfArtist(
        ponsArtistId: String,
        nftId: String,
        flowUnits: PonsUtils.FlowUnits
    )
    
    /* The concrete Pons NFT Market resource. Striaghtforward implementation of the PonsNftMarket interface */    pub resource PonsNftMarket_v1: PonsNftMarketContract.PonsNftMarket{ 
        /* Pons v1 collection to store the NFTs on sale */        access(account) let collection: @PonsNftContract_v1.Collection
        
        /* Capability for the market to receive payment */        access(account) var marketReceivePaymentCap: Capability<&{FungibleToken.Receiver}>
        
        /* Sales prices for each nftId */        access(account) var salePrices:{ String: PonsUtils.FlowUnits}
        
        /* Seller capabilities to receive payment for each nftId */        access(account) var saleReceivePaymentCaps:{ String: Capability<&{FungibleToken.Receiver}>}
        
        /* Stores the number of times each nftId has been listed on the market *//* 0 means freshly minted listing *//* 1 or above means resell listing *//* This helps the marketplace evaluate the validity of different listing certificate */        access(account) var listingCounts:{ String: Int}
        
        /* Minimum minting price */        access(account) var minimumMintingPrice: PonsUtils.FlowUnits
        
        /* Commission ratio of the market on freshly minted NFTs */        access(account) var primaryCommissionRatio: PonsUtils.Ratio
        
        /* Commission ratio of the market on resold NFTs */        access(account) var secondaryCommissionRatio: PonsUtils.Ratio
        
        /* Inserts or updates a sale price */        access(account) fun insertSalePrice(nftId: String, price: PonsUtils.FlowUnits): PonsUtils.FlowUnits?{ 
            return self.salePrices.insert(key: nftId, price)
        }
        
        /* Removes a sale price */        access(account) fun removeSalePrice(nftId: String): PonsUtils.FlowUnits?{ 
            return self.salePrices.remove(key: nftId)
        }
        
        /* Get the nftIds of all NFTs for sale */        pub fun getForSaleIds(): [String]{ 
            return self.salePrices.keys
        }
        
        /* Get the price of an NFT */        pub fun getPrice(nftId: String): PonsUtils.FlowUnits?{ 
            return self.salePrices[nftId]
        }
        
        /* Borrow an NFT from the marketplace, to browse its details */        pub fun borrowNft(nftId: String): &PonsNftContractInterface.NFT?{ 
            return self.collection.borrowNft(nftId: nftId)
        }
        
        /* Given a Pons artist certificate, mint new Pons NFTs on behalf of the artist and list it on the marketplace for sale *//* The price of the first edition of the NFT minted is determined by the basePrice, which must be at least the minimumMintingPrice *//* When only one edition is minted, the incrementalPrice is inconsequential *//* When the Pons marketplace mints multiple editions of NFTs, the market price of each succeesive NFT is incremented by the incrementalPrice */        pub fun mintForSale(_ artistCertificate: &PonsNftContract.PonsArtistCertificate, metadata:{ String: String}, quantity: Int, basePrice: PonsUtils.FlowUnits, incrementalPrice: PonsUtils.FlowUnits, _ royaltyRatio: PonsUtils.Ratio, _ receivePaymentCap: Capability<&{FungibleToken.Receiver}>): @[{PonsNftMarketContract.PonsListingCertificate}]{ 
        ...
        /* List a Pons NFT on the marketplace for sale */        pub fun listForSale(_ nft: @PonsNftContractInterface.NFT, _ salePrice: PonsUtils.FlowUnits, _ receivePaymentCap: Capability<&{FungibleToken.Receiver}>): @{PonsNftMarketContract.PonsListingCertificate}{ 
        ...
        /* Purchase a Pons NFT from the marketplace */        pub fun purchase(nftId: String, _ purchaseVault: @FungibleToken.Vault): @PonsNftContractInterface.NFT{ 
            ...
            let sellerPrice = purchasePrice.cut(commissionPrice).cut(royalties ?? PonsUtils.FlowUnits(0.0))
        ...
        /* Unlist a Pons NFT from the marketplace */        pub fun unlist(_ ponsListingCertificate: @{PonsNftMarketContract.PonsListingCertificate}): @PonsNftContractInterface.NFT{ 
    ...
    /* The concrete Pons Listing Certificate resource. Striaghtforward implementation of the PonsNftMarket interface, and also record the number of times the NFT has previously been listed */    pub resource PonsListingCertificate_v1:
    ...
    init(){ 
        ...
        if account.borrow<&FlowToken.Vault>(from: /storage/flowTokenVault)
        == nil{ 
            account.save(
        ...
        if !account.getCapability<&FlowToken.Vault{FungibleToken.Receiver}>(
        ...
        if !account.getCapability<&FlowToken.Vault{FungibleToken.Balance}>(
        ...
        let marketReceivePaymentCap =
            account.getCapability<&{FungibleToken.Receiver}>(
                /public/flowTokenReceiver
            )
        var ponsMarketV1 <-
