...
pub contract PonsNftMarketAdminContract_v1{ 
    /* Storage path at which the NFT Admin resource will be stored */    access(account) let AdminStoragePath: StoragePath
    
    /* Capability to the NFT Admin, for convenience of usage */    access(account) let AdminCapability: Capability<&NftMarketAdmin_v1>
    
    /* PonsNftMarketAdminContractInit_v1 is emitted on initialisation of this contract */    pub event PonsNftMarketAdminContractInit_v1()
    ...
    */
    ...
    pub resource NftMarketAdmin_v1{ 
        /* Updates the royalty of the Pons NFT */        pub fun updatePonsNftRoyalty(
            nftId: String,
        ...
        /* Updates the edition label of the Pons NFT */        pub fun updatePonsEditionLabel(
            nftId: String,
            editionLabel: String
        ): Void{ 
            PonsNftContract_v1.insertEditionLabel(
                nftId: nftId,
                editionLabel: editionLabel
            )
        }
        
        /* Updates the metadata of the Pons NFT */        pub fun updatePonsNftMetadata(
            nftId: String,
            metadata:{ 
                String: String
            }
        ...
        /* Updates the price of the Pons NFT on the marketplace */        pub fun updateSalePrice(
            nftId: String,
            price: PonsUtils.FlowUnits
        ): Void{ 
            let ponsNftMarketRef =
                &PonsNftMarketContract.ponsMarket
                as
                auth &{PonsNftMarketContract.PonsNftMarket}
            let ponsNftMarketV1Ref =
                ponsNftMarketRef as! &PonsNftMarketContract_v1.PonsNftMarket_v1
            ponsNftMarketV1Ref.insertSalePrice(nftId: nftId, price: price)
        }
        
        /* Borrows the NFT collection of the marketplace */        pub fun borrowCollection(): &PonsNftContract_v1.Collection{ 
            let ponsNftMarketRef =
                &PonsNftMarketContract.ponsMarket
                as
                auth &{PonsNftMarketContract.PonsNftMarket}
            let ponsNftMarketV1Ref =
                ponsNftMarketRef as! &PonsNftMarketContract_v1.PonsNftMarket_v1
            return &ponsNftMarketV1Ref.collection
            as
            &PonsNftContract_v1.Collection
        }
    }
    ...
    init(){ 
        ...
        self.account.save(
            <-create NftMarketAdmin_v1(),
            to: self.AdminStoragePath
        )
        
        // Create and save a capability to the admin for convenience
        self.AdminCapability = self.account.link<&NftMarketAdmin_v1>(
                /private/ponsMarketAdmin_v1,
