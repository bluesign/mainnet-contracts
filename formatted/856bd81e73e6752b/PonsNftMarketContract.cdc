...
pub contract PonsNftMarketContract{ 
    /* The storage path for the PonsNftMarket */    pub let PonsNftMarketAddress: Address
    
    /* Standardised storage path for PonsListingCertificateCollection */    pub let PonsListingCertificateCollectionStoragePath: StoragePath
    
    /* PonsMarketContractInit is emitted on initialisation of this contract */    pub event PonsMarketContractInit()
    
    /* PonsNFTListed is emitted on the listing of Pons NFTs on the marketplace */    pub event PonsNFTListed(
        nftId: String,
        serialNumber: UInt64,
        editionLabel: String,
    ...
    /* PonsNFTUnlisted is emitted on the unlisting of Pons NFTs from the marketplace */    pub event PonsNFTUnlisted(
        nftId: String,
        serialNumber: UInt64,
        editionLabel: String,
        price: PonsUtils.FlowUnits
    ...
    /* PonsNFTSold is emitted when a Pons NFT is sold */    pub event PonsNFTSold(
        nftId: String,
        serialNumber: UInt64,
        editionLabel: String,
        price: PonsUtils.FlowUnits
    ...
    /* PonsNFTSold is emitted when a Pons NFT is sold, and the new owner address is known */    pub event PonsNFTOwns(
        owner: Address,
        nftId: String,
        serialNumber: UInt64,
        editionLabel: String,
    ...
    /* Allow the PonsNft events to be emitted by all implementations of Pons NFTs from the same account */    access(account) fun emitPonsNFTListed(
        nftId: String,
        serialNumber: UInt64,
        editionLabel: String,
        price: PonsUtils.FlowUnits
    ...
    access(account) fun emitPonsNFTUnlisted(
        nftId: String,
        serialNumber: UInt64,
        editionLabel: String,
        price: PonsUtils.FlowUnits
    ...
    access(account) fun emitPonsNFTSold(
        nftId: String,
        serialNumber: UInt64,
        editionLabel: String,
        price: PonsUtils.FlowUnits
    ...
    access(account) fun emitPonsNFTOwns(
        owner: Address,
        nftId: String,
        serialNumber: UInt64,
        editionLabel: String,
    ...
    */
    ...
    pub resource interface PonsNftMarket{ 
        ...
        /* List a Pons NFT on the marketplace for sale */        pub fun listForSale(
        ...
        /* Purchase a Pons NFT from the marketplace */        pub fun purchase(
            nftId: String,
        ...
        /* Unlist a Pons NFT from the marketplace */        pub fun unlist(
    ...
    */
    ...
    pub resource PonsListingCertificateCollection{ 
        ...
        /* API to add listing certificates to a listing certificate collection */        pub fun appendListingCertificate(
            _ listingCertificate: @{PonsListingCertificate}
        ): Void{ 
            self.listingCertificates.append(<-listingCertificate)
        }
        
        /* API to remove listing certificates from a listing certificate collection */        pub fun removeListingCertificate(at index: Int): @{
            PonsListingCertificate
        }{ 
    ...
    /* Checks whether all the listing certificates provided belong to the market *///	pub fun certificatesOwnedByMarket (_ listingCertificatesRef : &[{PonsListingCertificate}]) : Bool {
    //		var index = 0
    ...
    /* API to get the nftIds on the market for sale */    pub fun getForSaleIds(): [String]{ 
        return PonsNftMarketContract.ponsMarket.getForSaleIds()
    }
    
    /* API to get the price of an NFT on the market */    pub fun getPrice(nftId: String): PonsUtils.FlowUnits?{ 
        return PonsNftMarketContract.ponsMarket.getPrice(nftId: nftId)
    }
    
    /* API to borrow an NFT for browsing */    pub fun borrowNft(nftId: String): &PonsNftContractInterface.NFT?{ 
        return PonsNftMarketContract.ponsMarket.borrowNft(nftId: nftId)
    }
    
    /* API to borrow the active Pons market instance */    pub fun borrowPonsMarket(): &{PonsNftMarket}{ 
        return &self.ponsMarket as &{PonsNftMarket}
    }
    
    /* A list recording all previously active instances of PonsNftMarket */    access(account) var historicalPonsMarkets: @[{PonsNftMarket}]
    
    /* The currently active instance of PonsNftMarket */    access(account) var ponsMarket: @{PonsNftMarket}
    
    /* Updates the currently active PonsNftMarket */    access(account) fun setPonsMarket(_ ponsMarket: @{PonsNftMarket}): Void{ 
    ...
    init(){ 
        ...
        self
            .PonsListingCertificateCollectionStoragePath = /storage/listingCertificateCollection
        
        // Emit the PonsNftMarket initialisation event
        emit PonsMarketContractInit()
    }
    
    /* An trivial instance of PonsNftMarket which panics on all calls, used on initialization of the PonsNftMarket contract. */    pub resource InvalidPonsNftMarket: PonsNftMarket{ 
        ...
        pub fun mintForSale(_ artistCertificate: &PonsNftContract.PonsArtistCertificate, metadata:{ String: String}, quantity: Int, basePrice: PonsUtils.FlowUnits, incrementalPrice: PonsUtils.FlowUnits, _ royaltyRatio: PonsUtils.Ratio, _ receivePaymentCap: Capability<&{FungibleToken.Receiver}>): @[{PonsListingCertificate}]{ 
            panic("not implemented")
        }
