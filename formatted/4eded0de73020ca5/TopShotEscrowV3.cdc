...
pub contract TopShotEscrowV3{ 
    ...
    pub resource EscrowItem: EscrowItemPublic{ 
        ...
        priv var NFTCollection: [UInt64]?
        
        pub let startTime: UFix64
        
        priv var duration: UFix64
        
        pub var redeemed: Bool
        
        priv let receiverCap: Capability<&{TopShot.MomentCollectionPublic}>
        
        priv var lock: Bool
        
        init(_NFTCollection: @TopShot.Collection, _duration: UFix64, _receiverCap: Capability<&{TopShot.MomentCollectionPublic}>){ 
            ...
            self.redeemed = false
            self.lock = false
            let adminTopShotReceiverRef = TopShotEscrowV3.account.getCapability(/public/MomentCollection).borrow<&{NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, TopShot.MomentCollectionPublic}>() ?? panic("Cannot borrow collection")
        ...
        pub fun redeem(NFTIds: [UInt64]){ 
            ...
            let providerTopShotProviderRef: &TopShot.Collection? = TopShotEscrowV3.account.borrow<&TopShot.Collection>(from: /storage/MomentCollection) ?? panic("Cannot borrow collection")
    ...
    pub resource interface EscrowCollectionPublic{ 
        ...
        pub fun borrowEscrow(escrowID: UInt64): &EscrowItem{
            EscrowItemPublic
        }?{} 
    ...
    pub resource EscrowCollection: EscrowCollectionPublic{ 
        ...
        // Escrow Items
        priv var escrowItems: @{UInt64: EscrowItem}
        ...
        pub fun createEscrow(NFTCollection: @TopShot.Collection, duration: UFix64, receiverCap: Capability<&{TopShot.MomentCollectionPublic}>){ 
    ...
    pub fun createEscrow(
        ...
        let escrowCollectionRef =
            self.account.borrow<&TopShotEscrowV3.EscrowCollection>(
    ...
    pub fun batchRedeem(_ escrowIDs: [UInt64]){ 
        for escrowID in escrowIDs{ 
            let escrowCollectionRef = self.account.borrow<&TopShotEscrowV3.EscrowCollection>(from: self.escrowStoragePath) ?? panic("Couldn't borrow escrow collection")
            let escrowRef = escrowCollectionRef.borrowEscrow(escrowID: escrowID)!
    ...
    init(){ 
        ...
        self.account.link<
            &TopShotEscrowV3.EscrowCollection{
                TopShotEscrowV3.EscrowCollectionPublic
            }
        >(self.escrowPublicPath, target: self.escrowStoragePath)
    }
}
