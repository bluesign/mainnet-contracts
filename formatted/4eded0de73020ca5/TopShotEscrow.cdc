...
pub contract TopShotEscrow{ 
    ...
    pub resource EscrowItem: EscrowItemPublic{ 
        ...
        init(_NFTCollection: @TopShot.Collection, _duration: UFix64, _receiverCap: Capability<&{NonFungibleToken.Receiver}>){ 
            ...
            self.redeemed = false
            let adminTopShotReceiverRef = TopShotEscrow.account.getCapability(/public/MomentCollection).borrow<&{NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, TopShot.MomentCollectionPublic}>() ?? panic("Cannot borrow collection")
        ...
        pub fun redeem(NFTIds: [UInt64]){ 
            ...
            let providerTopShotProviderRef: &TopShot.Collection? = TopShotEscrow.account.borrow<&TopShot.Collection>(from: /storage/MomentCollection) ?? panic("Cannot borrow collection")
    ...
    pub fun createEscrow(
        ...
        let escrowCollectionRef =
            self.account.borrow<&TopShotEscrow.EscrowCollection>(
    ...
    pub fun batchRedeem(_ escrowIDs: [UInt64]){ 
        for escrowID in escrowIDs{ 
            let escrowCollectionRef = self.account.borrow<&TopShotEscrow.EscrowCollection>(from: self.escrowStoragePath) ?? panic("Couldn't borrow escrow collection")
            let escrowRef = escrowCollectionRef.borrowEscrow(escrowID: escrowID)!
    ...
    init(){ 
        ...
        self.account.link<
            &TopShotEscrow.EscrowCollection{
                TopShotEscrow.EscrowCollectionPublic
            }
