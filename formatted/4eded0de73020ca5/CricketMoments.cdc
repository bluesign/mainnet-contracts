...
pub contract CricketMoments: NonFungibleToken{ 
    ...
    pub fun fetch(_ from: Address, id: UInt64): &CricketMoments.NFT?{ 
        let collection = getAccount(from).getCapability(CricketMoments.CollectionPublicPath).borrow<&CricketMoments.Collection{CricketMoments.CricketMomentsCollectionPublic}>() ?? panic("Couldn't get collection")
