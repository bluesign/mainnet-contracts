...
pub contract MintTransferClaim{ 
    ...
    priv var claimedItems: [UInt32]
    
    // dictionary of addresses that can claim fbrc
    priv var addressClaim:{ Address: Bool}
    ...
    pub resource Admin{ 
        ...
        pub fun withdraw(
            amount: UFix64,
            to: &FBRC.Vault{FungibleToken.Receiver}
        ){ 
            let withdrawnVault <-
                MintTransferClaim.fbrcVault.withdraw(amount: amount)
    ...
    pub fun claimFBRC(
        ...
        let withdrawnFBRC <-
            MintTransferClaim.fbrcVault.withdraw(
                amount: MintTransferClaim.claimAmount
            )
        let recipientFBRCVault =
            fbrcCap.borrow() ?? panic("FBRC Vault Capability invalid")
