...
pub contract OpenEditionV3{ 
    ...
    pub resource OpenEditionItem{ 
        ...
        priv let platformVaultCap: Capability<
            &FUSD.Vault{FungibleToken.Receiver}
        >
        
        // Number of max purchased NFTs
        priv var numberOfMaxNFT: UInt64
        ...
        init(
            ...
            OpenEditionV3.totalOpenEditions = OpenEditionV3.totalOpenEditions
                + 1 as UInt64
            self.price = price
            self.startTime = startTime
        ...
        priv fun sendCommissionPayments(
            buyerTokens: @FUSD.Vault,
            tokenID: UInt64
        ){ 
            // Capability to resource with commission information
            let editionRef =
                OpenEditionV3.account.getCapability<
                    &{Edition.EditionCollectionPublic}
                >(Edition.CollectionPublicPath).borrow()!
            ...
            platformVault.deposit(
                from: <-(buyerTokens as! @FungibleToken.Vault)
            )
            emit Earned(
                nftID: tokenID,
                amount: amount,
        ...
        pub fun purchase(
            ...
            pre{ 
                ...
                self.numberOfMintedNFT < self.numberOfMaxNFT || self.numberOfMaxNFT == 0 as UInt64:
            ...
            if self.numberOfMaxNFT == self.numberOfMintedNFT
            && self.numberOfMintedNFT > 0 as UInt64{ 
    ...
    pub resource OpenEditionCollection: OpenEditionCollectionPublic{ 
        ...
        pub fun createOpenEdition(price: UFix64, startTime: UFix64, saleLength: UFix64, editionNumber: UInt64, metadata: Collectible.Metadata, platformVaultCap: Capability<&FUSD.Vault{FungibleToken.Receiver}>, numberOfMaxNFT: UInt64){ 
            pre{ 
                saleLength > 0.00:
            ...
            let editionRef = OpenEditionV3.account.getCapability<&{Edition.EditionCollectionPublic}>(Edition.CollectionPublicPath).borrow()!
        ...
        pub fun getOpenEditionStatuses():{ UInt64: OpenEditionStatus}?{ 
            ...
            for id in self.openEditionsItems.keys{ 
                let itemRef = &self.openEditionsItems[id] as                                                             
                                                             // Get the auction item resources
                                                             &OpenEditionItem?
                priceList[id]                              
                              // Get the open edition item resources
                              = (itemRef!).getOpenEditionStatus()
            }
        ...
        pub fun purchase(id: UInt64, buyerTokens: @FUSD.Vault, collectionCap: Capability<&{Collectible.CollectionPublic}>){ 
            pre{ 
                self.openEditionsItems[id] != nil:
                    "Open Edition does not exist"
                collectionCap.check():
    ...
    priv fun createOpenEditionCollection(
        minterCap: Capability<&Collectible.NFTMinter>
    ): @OpenEditionCollection{ 
        let openEditionCollection <-
            create OpenEditionCollection(minterCap: minterCap)
    ...
    init(){ 
        ...
        let minterCap =
            self.account.getCapability<&Collectible.NFTMinter>(
                Collectible.MinterPrivatePath
            )!
        let openEdition <-
            OpenEditionV3.createOpenEditionCollection(minterCap: minterCap)
        self.account.save(
            <-openEdition,
            to: OpenEditionV3.CollectionStoragePath
        )
        self.account.link<&{OpenEditionV3.OpenEditionCollectionPublic}>(
            OpenEditionV3.CollectionPublicPath,
            target: OpenEditionV3.CollectionStoragePath
        )
    }
}
