...
pub contract Auction{ 
    ...
    pub event Extend(
        auctionID: UInt64,
        auctionLengthFrom: UFix64,
        auctionLengthTo: UFix64
    )
    ...
    pub resource AuctionItem{ 
        ...
        priv var recipientVaultCap: Capability<
        ...
        priv fun sendNFT(
            _ capability: Capability<&{Collectible.CollectionPublic}>
        ){ 
    ...
    pub resource AuctionCollection: AuctionCollectionPublic{ 
        ...
        pub fun createAuction(minimumBidIncrement: UFix64, auctionLength: UFix64, extendedLength: UFix64, remainLengthToExtend: UFix64, auctionStartTime: UFix64, startPrice: UFix64, platformVaultCap: Capability<&FUSD.Vault{FungibleToken.Receiver}>, editionCap: Capability<&{Edition.EditionCollectionPublic}>): UInt64{ 
            pre{ 
                auctionLength > 0.00:
    ...
    init(){ 
        ...
        self.account.link<&{Auction.AuctionCollectionPublic}>(
            Auction.CollectionPublicPath,
            target: Auction.CollectionStoragePath
        )
    }
}
