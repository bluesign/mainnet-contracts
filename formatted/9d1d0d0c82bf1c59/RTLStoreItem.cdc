...
pub contract RTLStoreItem: NonFungibleToken{ 
    ...
    // -----------------------------------------------------------------------
    // RTLStoreItem contract-level fields.
    ...
    // -----------------------------------------------------------------------
    // RTLStoreItem contract-level Composite Type definitions
    // -----------------------------------------------------------------------
    ...
    pub struct EditionData{ 
        ...
        init(editionID: UInt32){ 
            if RTLStoreItem.editions[editionID] == nil{ 
                panic("the editionID was not found")
            }
            let editionToRead = (&RTLStoreItem.editions[editionID] as &Edition?)!
    ...
    pub resource Edition{ 
        ...
        init(merchantID: UInt32, metadata:{ String: String}, name: String, printingLimit: UInt32?){ 
            ...
            self.editionID = RTLStoreItem.nextEditionID
            ...
            RTLStoreItem.nextEditionID = RTLStoreItem.nextEditionID + 1 as UInt32
    ...
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver{ 
        ...
        init(merchantID: UInt32, editionID: UInt32, editionNumber: UInt32){ 
            ...
            RTLStoreItem.totalSupply = RTLStoreItem.totalSupply + 1 as UInt64
            self.id = RTLStoreItem.totalSupply
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                ...
                case Type<MetadataViews.ExternalURL>():
                    let url = "https://wallet.sisi-nft.de/"
                    return MetadataViews.ExternalURL(url)
                case Type<MetadataViews.Royalties>():
                    let royaltiesDictionary = RTLStoreItem.royaltiesForSpecificEdition[self.data.editionID] ?? RTLStoreItem.defaultRoyalties
                ...
                case Type<MetadataViews.NFTCollectionDisplay>():
                    let mediaSquare = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://dkuxa1i6sgo8h.cloudfront.net/sisi/Sisi-square-whitebackground-small.png"), mediaType: "image/png")
                    let mediaBanner = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://dkuxa1i6sgo8h.cloudfront.net/sisi/Sisi+banner.png"), mediaType: "image/png")
                    let url = "https://wallet.sisi-nft.de/"
                    let description = ""
                    return MetadataViews.NFTCollectionDisplay(name: "RTLStoreItem", description: description, externalURL: MetadataViews.ExternalURL(url), squareImage: mediaSquare, bannerImage: mediaBanner, socials:{} )
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: RTLStoreItem.CollectionStoragePath, publicPath: RTLStoreItem.CollectionPublicPath, providerPath: /private/RTLStoreItemCollection, publicCollection: Type<&RTLStoreItem.Collection{RTLStoreItem.RTLStoreItemCollectionPublic}>(), publicLinkedType: Type<&RTLStoreItem.Collection{RTLStoreItemCollectionPublic, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&RTLStoreItem.Collection{RTLStoreItemCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
                            return <-RTLStoreItem.createEmptyCollection()
                        })
    ...
    pub resource Admin{ 
        ...
        pub fun createEdition(merchantID: UInt32, metadata:{ String: String}, name: String, printingLimit: UInt32?){ 
            ...
            RTLStoreItem.editions[newID] <-! newEdition
        }
        
        // borrowEdition returns a reference to an edition in the RTLStoreItem
        ...
        pub fun borrowEdition(editionID: UInt32): &Edition{ 
            ...
            return (&RTLStoreItem.editions[editionID] as &Edition?)!
        }
        
        // updateEditionMetadata returns a reference to an edition in the RTLStoreItem
        // contract so that the admin can call methods on it
        //
        // Parameters: 
        ...
        pub fun updateEditionMetadata(editionID: UInt32, updates:{ String: String}, suffix: String): UInt32{ 
            ...
            let editionRef = &RTLStoreItem.editions[editionID] as &Edition?
        ...
        pub fun setDefaultRoyaltyByName(name: String, royalty: MetadataViews.Royalty){ 
            ...
            // verify total
            let totalCut = RTLStoreItem.getDefaultRoyaltyTotalRate()
        ...
        pub fun removeDefaultRoyaltyByName(name: String){ 
            ...
            RTLStoreItem.defaultRoyalties.remove(key: name)
        ...
        pub fun setEditionRoyaltyByName(editionID: UInt32, name: String, royalty: MetadataViews.Royalty){ 
            ...
            (             //let royaltiesForSpecificEdition = RTLStoreItem.royaltiesForSpecificEdition[editionID]!
             //royaltiesForSpecificEdition.insert(key: name, royalty);
             RTLStoreItem.royaltiesForSpecificEdition[editionID]!).insert(key: name, royalty)
            let totalCut = RTLStoreItem.getEditionRoyaltyTotalRate(editionID: editionID)
            assert(totalCut <= 1.0, message: "Sum of cutInfos multipliers should not be greater than 1.0")
        ...
        pub fun removeEditionRoyaltyByName(editionID: UInt32, name: String){ 
            ...
            (RTLStoreItem.royaltiesForSpecificEdition[editionID]!).remove(key: name)
        ...
        pub fun revertRoyaltiesForEditionToDefault(editionID: UInt32){ 
            ...
            RTLStoreItem.royaltiesForSpecificEdition.remove(key: editionID)
        ...
        pub fun createNewAdmin(): @Admin{ 
            ...
            // Increment the ID so that it isn't used again
            RTLStoreItem.nextAdminID = RTLStoreItem.nextAdminID + 1 as UInt32
            return <-create Admin(id: newID)
        }
    }
    
    // This is the interface that users can cast their RTLStoreItem Collection as
    // to allow others to deposit RTLStoreItems into their Collection. It also allows for reading
    // the IDs of RTLStoreItems in the Collection.
    pub resource interface RTLStoreItemCollectionPublic{ 
        ...
        pub fun borrowRTLStoreItem(id: UInt64): &RTLStoreItem.NFT?{ 
            // If the result isn't nil, the id of the returned reference
            // should be the same as the argument to the function
            post{ 
                result == nil || result?.id == id:
                    "Cannot borrow RTLStoreItem reference: The ID of the returned reference is incorrect"
            }
        }
    }
    ...
    //
    pub resource Collection: RTLStoreItemCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection{ 
        // Dictionary of RTLStoreItem conforming tokens
        // NFT is a resource type with a UInt64 ID field
        pub var ownedNFTs: @{UInt64: NonFungibleToken.NFT}
        
        // pub var ownedNFTs: @{UInt64: RTLStoreItem.NFT}
        
        init(){ 
            self.ownedNFTs <-{} 
        }
        
        // withdraw removes a RTLStoreItem from the Collection and moves it to the caller
        //
        ...
        pub fun withdraw(withdrawID: UInt64): @NonFungibleToken.NFT{ 
            ...
            let token <- self.ownedNFTs.remove(key: withdrawID) ?? panic("Cannot withdraw: RTLStoreItem does not exist in the collection")
        ...
        // deposit takes a RTLStoreItem and adds it to the Collections dictionary
        //
        ...
        pub fun deposit(token: @NonFungibleToken.NFT){ 
            ...
            let token <- token as! @RTLStoreItem.NFT
        ...
        // borrowNFT Returns a borrowed reference to a RTLStoreItem in the Collection
        ...
        // not any RTLStoreItem specific data. Please use borrowRTLStoreItem to 
        // read RTLStoreItem data.
        //
        pub fun borrowNFT(id: UInt64): &NonFungibleToken.NFT{ 
            return (&self.ownedNFTs[id] as &NonFungibleToken.NFT?)!
        }
        
        // borrowRTLStoreItem returns a borrowed reference to a RTLStoreItem
        ...
        pub fun borrowRTLStoreItem(id: UInt64): &RTLStoreItem.NFT?{ 
            if self.ownedNFTs[id] != nil{ 
                ...
                return ref as! &RTLStoreItem.NFT
            } else{ 
                return nil
            }
        }
        ...
        pub fun borrowViewResolver(id: UInt64): &AnyResource{MetadataViews.Resolver}{ 
            ...
            let RTLStoreItemNFT = nft as! &RTLStoreItem.NFT
            return RTLStoreItemNFT as &AnyResource{MetadataViews.Resolver}
        }
    ...
    // -----------------------------------------------------------------------
    // RTLStoreItem contract-level function definitions
    // -----------------------------------------------------------------------
    
    // createEmptyCollection creates a new, empty Collection object so that
    // a user can store it in their account storage.
    // Once they have a Collection in their storage, they are able to receive
    // RTLStoreItems in transactions.
    //
    pub fun createEmptyCollection(): @NonFungibleToken.Collection{ 
        return <-create RTLStoreItem.Collection()
    }
    
    pub fun createEmptyRTLStoreItemCollection(): @RTLStoreItem.Collection{ 
        return <-create RTLStoreItem.Collection()
    }
    ...
    init(){ 
        ...
        self.CollectionStoragePath = /storage/RTLStoreItemCollection
        self.CollectionPublicPath = /public/RTLStoreItemCollection
        self.AdminStoragePath = /storage/RTLStoreItemItemAdmin
        
        // Put a new Collection in storage
        self.account.save<@Collection>(<-create Collection(), to: self.CollectionStoragePath)
        
        // Create a public capability for the Collection
        self.account.link<&{RTLStoreItemCollectionPublic}>(self.CollectionPublicPath, target: self.CollectionStoragePath)
