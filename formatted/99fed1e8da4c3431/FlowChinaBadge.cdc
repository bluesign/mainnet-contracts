...
pub contract FlowChinaBadge: NonFungibleToken{ 
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &FlowChinaBadge.NFT?{ 
        let collection = (getAccount(from).getCapability(FlowChinaBadge.CollectionPublicPath)!).borrow<&{FlowChinaBadge.FlowChinaBadgeCollectionPublic}>() ?? panic("Couldn't get collection")
