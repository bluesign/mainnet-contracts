...
pub contract XvsX: NonFungibleToken{ 
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                case Type<MetadataViews.Display>():
                    return MetadataViews.Display(name: "XvsX NFT", description: "XvsX NFT #".concat(self.id.toString()), thumbnail: MetadataViews.HTTPFile(url: "https://ipfs.tenzingai.com/ipfs/".concat(self.file.cid)))
                case Type<MetadataViews.Editions>():
                    // There is no max number of NFTs that can be minted from this contract
                    // so the max edition field value is set to nil
                    return MetadataViews.Editions([MetadataViews.Edition(name: "XvsX NFT Edition", number: self.id, max: nil)])
                case Type<MetadataViews.Serial>():
                    return MetadataViews.Serial(self.id)
                case Type<MetadataViews.Royalties>():
                    let receivers:{ Address: UFix64} ={ 0x61a56aa81654c8a7: 0.005}
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: XvsX.CollectionStoragePath, publicPath: XvsX.CollectionPublicPath, providerPath: /private/XvsXCollection, publicCollection: Type<&XvsX.Collection{NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection}>(), publicLinkedType: Type<&XvsX.Collection{NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&XvsX.Collection{NonFungibleToken.CollectionPublic, NonFungibleToken.Provider, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
                            return <-XvsX.createEmptyCollection()
                        })
                case Type<MetadataViews.NFTCollectionDisplay>():
                    let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://ipfs.tenzingai.com/ipfs/QmV1HKNKBriM3s2E9K73StS4u2A6ZKAEAZS1W8bQAKxBFk"), mediaType: "image/png")
                    return MetadataViews.NFTCollectionDisplay(name: "The XvsX NFT Collection", description: "", externalURL: MetadataViews.ExternalURL("https://mpc.xvsxsports.com"), squareImage: media, bannerImage: media, socials:{} )
    ...
    pub resource NFTMinter{ 
        ...
        priv fun transformMetadata(_ metadata:{ String: String}):{ String: String}{ 
            ...
            return{ "nftType": metadata["nftType"]!}
        }
    ...
    init(){ 
        ...
        self.account.link<&XvsX.Collection{NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection}>(self.CollectionPublicPath, target: self.CollectionStoragePath)
