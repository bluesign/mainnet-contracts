...
pub contract AAExchange{ 
    ...
    pub fun listForSale(
        ...
        assert(
            ownerAmount >= minimumRequire,
            message: "Minimum require not pass"
        )
        addPayment(
            type: "Reward",
        ...
        self.affiliateAmounts[listingResourceID] = AAFeeManager.affiliateRate
            * listingPrice
    ...
    pub fun acceptListing(
        ...
        let recipient =
            receiverCap.borrow()
            ?? panic("Collection to receive NFT not setup correctly")
