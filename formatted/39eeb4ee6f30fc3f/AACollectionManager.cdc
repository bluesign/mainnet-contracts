...
pub contract AACollectionManager{ 
    priv let collections: @{UInt64: Collection}
    
    pub let AdminStoragePath: StoragePath
    
    // A map from nftType-nftID -> Collection ID
    priv let items:{ String: UInt64}
    ...
    pub event CollectionItemRemove(
        collectionID: UInt64,
        type: Type,
        nftID: UInt64
    )
    ...
    pub fun getCollectionCuts(type: Type, nftID: UInt64): [
        AACommon.PaymentCut
    ]?{ 
        if let collectionID =
            self.items[AACommon.itemIdentifier(type: type, id: nftID)]{ 
    ...
    pub resource Administrator{ 
        pub fun createCollection(name: String, cuts: [AACommon.PaymentCut]){ 
            ...
            let old
                <- AACollectionManager.collections[collectionID]
                <- collection
            destroy old
            emit CollectionCreated(collectionID: collectionID, name: name)
        }
        
        pub fun addItemToCollection(
            collectionID: UInt64,
            type: Type,
            nftID: UInt64
        ){ 
        ...
        pub fun removeItemFromCollection(
            collectionID: UInt64,
            type: Type,
            nftID: UInt64
        ){ 
