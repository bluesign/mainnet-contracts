...
pub contract AAAuction{ 
    ...
    pub resource Bid{ 
        access(contract) let recipient: Capability<
            &{NonFungibleToken.CollectionPublic}
        >
        
        priv let vault: @FungibleToken.Vault
        ...
        pub fun doIncrease(affiliate: Address?, from: @FungibleToken.Vault){ 
        ...
        pub fun payout(cuts: [AACommon.PaymentCut], for seller: Address){ 
            ...
            let cap =
                fun (_ addr: Address): Capability<&{FungibleToken.Receiver}>{ 
            ...
            let sellerRecipient =
                cap(seller).borrow() ?? panic("Seller vault broken")
            sellerRecipient.deposit(
                from: <-self.vault.withdraw(amount: self.vault.balance)
            )
        ...
        destroy(){ 
            pre{ 
                self.vault.balance == 0.0:
                    "Can't destroy a bid with non-empty vault"
            }
            destroy self.vault
        }
    }
    ...
    pub resource Auction: AuctionPublic{ 
        priv let details: AuctionDetails
        ...
        pub fun placeBid(recipient: Capability<&{NonFungibleToken.CollectionPublic}>, refund: Capability<&{FungibleToken.Receiver}>, vault: @FungibleToken.Vault, affiliate: Address?){ 
            pre{ 
                refund.check():
                    "Refund broken"
                recipient.check():
        ...
        priv fun increaseBid(vault: @FungibleToken.Vault, affiliate: Address?){ 
            pre{ 
                self.bid != nil:
                    "Invalid call"
        ...
        priv fun createBid(recipient: Capability<&{NonFungibleToken.CollectionPublic}>, refund: Capability<&{FungibleToken.Receiver}>, vault: @FungibleToken.Vault, affiliate: Address?){ 
            pre{ 
                self.isOpen():
                    "Auction not open"
        ...
        priv fun addUnclaimedBid(bid: @Bid){ 
        ...
        priv fun sendNFT(to receiver: Capability<&{NonFungibleToken.CollectionPublic}>){ 
    ...
    pub resource AuctionStore: AuctionStorePublic{ 
        priv let auctions: @{UInt64: Auction}
        
        pub fun createAuction(nftProviderCapability: Capability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>, nftType: Type, nftID: UInt64, startAt: UFix64, endAt: UFix64?, startPrice: UFix64, increment: UFix64, bidType: Type): UInt64{ 
            pre{ 
        ...
        destroy(){ 
            pre{ 
                self.auctions.length == 0:
                    "Can't not destroy non-empty auction center"
            }
    ...
    access(contract) fun getSaleCuts(
        seller: Address,
        nftType: Type,
        nftID: UInt64,
        affiliate: Address?
        ...
        if let collectionCuts =
            AACollectionManager.getCollectionCuts(type: nftType, nftID: nftID){ 
            cuts.appendAll(collectionCuts)
        }
        return cuts
    }
    
    priv fun getUnclaimBidIDs(): [UInt64]{ 
