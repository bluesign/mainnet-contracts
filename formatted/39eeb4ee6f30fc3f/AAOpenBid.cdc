...
pub contract AAOpenBid{ 
    ...
    pub resource Bid: BidPublic{ 
        ...
        priv let details: BidDetails
        
        priv let vaultProviderCapability: Capability<&{FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance}>
        
        priv let recipientCapability: Capability<&{NonFungibleToken.CollectionPublic}>
        ...
        destroy(){ 
            ...
            if !self.details.purchased{ 
                emit BidCompleted(bidResourceID: self.uuid, openBidResourceID: self.details.openBidID, purchased: self.details.purchased, nftType: self.details.nftType, nftID: self.details.nftID, payments: nil)
            }
        }
    ...
    pub resource OpenBid: OpenBidManager, OpenBidPublic{ 
        ...
        priv var bids: @{UInt64: Bid}
        
        pub fun createBid(vaultProviderCapability: Capability<&{FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance}>, nftType: Type, nftID: UInt64, bidPrice: UFix64, recipientCapability: Capability<&{NonFungibleToken.CollectionPublic}>, affiliate: Address?): UInt64{ 
        ...
        pub fun borrowBid(bidResourceID: UInt64): &Bid{BidPublic}?{ 
            if self.bids[bidResourceID] != nil{ 
                return (&self.bids[bidResourceID] as                                                     
