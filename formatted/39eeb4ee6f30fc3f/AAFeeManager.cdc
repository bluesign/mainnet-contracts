...
pub contract AAFeeManager{ 
    ...
    access(contract) let basePhysicalCuts:{ 
        PhysicalCutFor: [
            AACommon.PaymentCut
        ]
    }
    ...
    pub event BaseFeePhysicalSetting(
        firstPurchased: Bool,
        cuts: [
            AACommon.PaymentCut
        ]
    ...
    pub resource Administrator{ 
        ...
        pub fun setBasePhysicalCuts(
            ...
            emit BaseFeePhysicalSetting(
                firstPurchased: forPurchased,
                cuts: cuts
            )
        }
    ...
    pub fun getNonPhysicalsPaymentCuts(type: Type, nftID: UInt64): [
        AACommon.PaymentCut
    ]{ 
    ...
    pub fun getPhysicalPaymentCuts(nftID: UInt64): [AACommon.PaymentCut]{ 
        let cuts: [AACommon.PaymentCut] = []
        let id =
            AACommon.itemIdentifier(type: Type<@AAPhysical.NFT>(), id: nftID)
        let data = AAFeeManager.extradata[id]
        if data?.royalty != nil{ 
            cuts.append((data!).royalty!)
        }
        ...
        let cutFor =
            purchased ? PhysicalCutFor.Purchased : PhysicalCutFor.firstOrder
    ...
    pub fun getPlatformCuts(referralReceiver: Address?, affiliate: Address?): [
        AACommon.PaymentCut
    ]?{ 
    ...
    pub fun paidAffiliateFee(
        ...
        let vaultRef =
            self.account.borrow<&FungibleToken.Vault>(from: (path!).storagePath)
        ...
        if let recipient =
            getAccount(recipientAddress!).getCapability<
                &{FungibleToken.Receiver}
            >((path!).publicPath).borrow(){ 
