...
pub contract toddmain2_NFT: NonFungibleToken{ 
    ...
    pub resource Series{ 
        ...
        //
        pub fun minttoddmain2_NFT(recipient: &{NonFungibleToken.CollectionPublic}, tokenId: UInt64, setId: UInt32){ 
            pre{ 
            ...
            recipient.deposit(token: <-create toddmain2_NFT.NFT(tokenId: tokenId, setId: setId, editionNum: editionNum))
        ...
        pub fun batchMinttoddmain2_NFT(recipient: &{NonFungibleToken.CollectionPublic}, setId: UInt32, tokenIds: [UInt64]){ 
            pre{ 
                tokenIds.length > 0:
                    "Number of token Ids must be > 0"
            }
            for tokenId in tokenIds{ 
                self.minttoddmain2_NFT(recipient: recipient, tokenId: tokenId, setId: setId)
            }
        }
    ...
    pub fun fetch(_ from: Address, id: UInt64): &toddmain2_NFT.NFT?{ 
        let collection = getAccount(from).getCapability(toddmain2_NFT.CollectionPublicPath).borrow<&toddmain2_NFT.Collection{toddmain2_NFT.toddmain2_NFTCollectionPublic}>() ?? panic("Couldn't get collection")
    ...
    init(){ 
        ...
        self.account.link<&toddmain2_NFT.Admin>(self.AdminPrivatePath, target: self.AdminStoragePath) ?? panic("Could not get a capability to the admin")
