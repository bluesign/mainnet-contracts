...
pub contract Fomopoly: FungibleToken{ 
    ...
    pub fun stakingInscription(flowVault: @FungibleToken.Vault, collectionRef: auth &Inscription.Collection, stakeIds: [UInt64]){ 
        pre{ 
    ...
    pub fun claimStakingReward(identityCollectionRef: auth &Fomopoly.Vault, inscriptionCollectionRef: auth &Inscription.Collection){ 
        pre{ 
            getCurrentBlock().timestamp >= self.stakingEndTime:
    ...
    pub fun bridgeTokens(flowVault: @FungibleToken.Vault, vault: &Fomopoly.Vault, to: String){ 
    ...
    pub resource Administrator{ 
        ...
        pub fun compensate(receiver: Address, burnedIds: [UInt64], txHash: String): @Fomopoly.Vault{ 
