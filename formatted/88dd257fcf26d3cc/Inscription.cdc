...
pub contract Inscription: NonFungibleToken, ViewResolver{ 
    ...
    pub event BatchWithdraw(ids: [UInt64], from: Address?)
    ...
    pub event BatchDeposit(ids: [UInt64], to: Address?)
    ...
    pub event BatchBurn(ids: [UInt64], from: Address?)
    ...
    pub resource Collection: InscriptionCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, InscriptionMetadata.ResolverCollection{ 
        ...
        /// @return The Inscription resource that has been taken out of the collection
        ///
        pub fun batchWithdraw(withdrawIDs: [UInt64]): @Collection{ 
            var tokens: @Collection <- Inscription.createEmptyCollection()
    ...
    ///
    pub fun createEmptyCollection(): @Inscription.Collection{ 
    ...
    ///
    pub fun mintInscription(recipient: &{NonFungibleToken.CollectionPublic}, amount: UInt64){ 
        pre{ 
            amount == UInt64(1000):
    ...
    init(){ 
        ...
        self.account.link<&Inscription.Collection{NonFungibleToken.CollectionPublic, Inscription.InscriptionCollectionPublic, InscriptionMetadata.ResolverCollection}>(self.CollectionPublicPath, target: self.CollectionStoragePath)
