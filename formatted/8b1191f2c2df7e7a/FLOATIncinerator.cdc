...
pub contract FLOATIncinerator{ 
    ...
    pub resource Incinerator: IncineratorPublic{ 
        ...
        pub fun burn(collection: &MusicPeaksSocialToken.Collection, ids: [UInt64]){ 
            let length = ids.length
            for id in ids{ 
                let float: &MusicPeaksSocialToken.NFT = collection.borrowFLOAT(id: id) ?? panic("This FLOAT does not exist.")
    ...
    init(){ 
        self
            .IncineratorStoragePath = /storage/MusicPeaksSocialTokenIncineratorStoragePath
        self
            .IncineratorPublicPath = /public/MusicPeaksSocialTokenIncineratorPublicPath
