...
pub contract MonoPaymentMinter{ 
    ...
    pub fun paymentMintGold(
        ...
        let fusdReceiver =
            self.account.getCapability<&FUSD.Vault{FungibleToken.Receiver}>(
                /public/fusdReceiver
            )!
        let receiver = fusdReceiver.borrow()!
        receiver.deposit(from: <-payment)
        let minter =
            self.account.borrow<&MonoGold.NFTMinter>(
                from: MonoGold.MinterStoragePath
            )!
    ...
    pub fun paymentMintSilver(
        ...
        receiver.deposit(from: <-payment)
        let minter =
            self.account.borrow<&MonoSilver.NFTMinter>(
                from: MonoSilver.MinterStoragePath
            )!
        let metadata:{ String: String} ={} 
