...
pub contract StarlyTokenVesting: NonFungibleToken{ 
    pub event TokensVested(id: UInt64, beneficiary: Address, amount: UFix64)
    
    pub event TokensReleased(id: UInt64, beneficiary: Address, amount: UFix64, remainingAmount: UFix64)
    
    pub event VestingBurned(id: UInt64, beneficiary: Address, amount: UFix64)
    
    pub event TokensBurned(id: UInt64, amount: UFix64)
    ...
    pub resource PeriodVestingSchedule: IVestingSchedule{ 
        ...
        priv let schedule:{ UFix64: UFix64}
    ...
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver, VestingPublic{ 
        ...
        init(id: UInt64, beneficiary: Address, vestedVault: @StarlyToken.Vault, vestingSchedule: @AnyResource{StarlyTokenVesting.IVestingSchedule}){ 
            self.id = id
            self.beneficiary = beneficiary
    ...
    pub resource NFTMinter{ 
        pub fun mintLinear(beneficiary: Address, vestedVault: @StarlyToken.Vault, startTimestamp: UFix64, endTimestamp: UFix64): @StarlyTokenVesting.NFT{ 
        ...
        pub fun mintPeriod(beneficiary: Address, vestedVault: @StarlyToken.Vault, schedule:{ UFix64: UFix64}): @StarlyTokenVesting.NFT{ 
            let vestingSchedule <- create PeriodVestingSchedule(schedule: schedule)
            return <-self.mintInternal(beneficiary: beneficiary, vestedVault: <-vestedVault, vestingSchedule: <-vestingSchedule)
        }
        
        priv fun mintInternal(beneficiary: Address, vestedVault: @StarlyToken.Vault, vestingSchedule: @AnyResource{StarlyTokenVesting.IVestingSchedule}): @StarlyTokenVesting.NFT{ 
            pre{ 
    ...
    pub resource NFTBurner{ 
        ...
        pub fun release(vestingRef: &StarlyTokenVesting.NFT){ 
            let receiverRef = getAccount(vestingRef.beneficiary).getCapability(StarlyToken.TokenPublicReceiverPath).borrow<&{FungibleToken.Receiver}>() ?? panic("Could not borrow StarlyToken receiver reference to the beneficiary's vault!")
    ...
    pub resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, CollectionPublic, CollectionPrivate, MetadataViews.ResolverCollection{ 
    ...
    init(){ 
        ...
        if self.account.borrow<&StarlyToken.Vault>(from: StarlyToken.TokenStoragePath) == nil{ 
            ...
            self.account.link<&StarlyToken.Vault{FungibleToken.Receiver}>(StarlyToken.TokenPublicReceiverPath, target: StarlyToken.TokenStoragePath)
            self.account.link<&StarlyToken.Vault{FungibleToken.Balance}>(StarlyToken.TokenPublicBalancePath, target: StarlyToken.TokenStoragePath)
        }
