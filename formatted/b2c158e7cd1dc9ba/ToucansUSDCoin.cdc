...
pub contract ToucansUSDCoin: FungibleToken, ViewResolver{ 
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, MetadataViews.Resolver{ 
        ...
        pub fun withdraw(amount: UFix64): @FungibleToken.Vault{ 
            ...
            if let owner: Address = self.owner?.address{ 
                ToucansUSDCoin.setBalance(address: owner, balance: self.balance)
            }
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                case Type<FungibleTokenMetadataViews.FTView>():
                    return ToucansUSDCoin.resolveView(view)
        ...
        destroy(){ 
            if self.balance > 0.0{ 
                ...
                ToucansUSDCoin.totalSupply = ToucansUSDCoin.totalSupply - self.balance
            }
        }
    }
    ...
    pub resource Minter: Toucans.Minter{ 
        pub fun mint(amount: UFix64): @Vault{ 
            post{ 
                ToucansUSDCoin.maxSupply == nil || ToucansUSDCoin.totalSupply <= ToucansUSDCoin.maxSupply!:
                    "Exceeded the max supply of tokens allowd."
            }
            ToucansUSDCoin.totalSupply = ToucansUSDCoin.totalSupply + amount
    ...
    pub fun resolveView(_ view: Type): AnyStruct?{ 
        switch view{ 
            ...
            case Type<FungibleTokenMetadataViews.FTDisplay>():
                ...
                return FungibleTokenMetadataViews.FTDisplay(name: "Toucans USD Coin", symbol: "TUSDC", description: "Toucans USD Coin", externalURL: MetadataViews.ExternalURL("ecdao.org"), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("emerald_dao"), "discord": MetadataViews.ExternalURL("emeraldcity")})
            case Type<FungibleTokenMetadataViews.FTVaultData>():
                return FungibleTokenMetadataViews.FTVaultData(storagePath: ToucansUSDCoin.VaultStoragePath, receiverPath: ToucansUSDCoin.ReceiverPublicPath, metadataPath: ToucansUSDCoin.VaultPublicPath, providerPath: /private/ToucansUSDCoinVault, receiverLinkedType: Type<&Vault{FungibleToken.Receiver}>(), metadataLinkedType: Type<&Vault{FungibleToken.Balance, MetadataViews.Resolver}>(), providerLinkedType: Type<&Vault{FungibleToken.Provider}>(), createEmptyVaultFunction: fun (): @Vault{ 
                        return <-ToucansUSDCoin.createEmptyVault()
                    })
        }
        return nil
    }
    ...
    init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
        ...
        // Paths
        self.VaultStoragePath = /storage/ToucansUSDCoinVault
        self.ReceiverPublicPath = /public/ToucansUSDCoinReceiver
        self.VaultPublicPath = /public/ToucansUSDCoinMetadata
        self.MinterStoragePath = /storage/ToucansUSDCoinMinter
        self.AdministratorStoragePath = /storage/ToucansUSDCoinAdmin
