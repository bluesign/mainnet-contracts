...
pub contract JadeContracts{ 
    ...
    pub fun getUserJadeCollections(user: Address): [String]{ 
        let collections: &JadeContracts.ContractsBook{
            JadeContracts.ContractsBookPublic
        } =
            getAccount(user).getCapability(
                JadeContracts.ContractsBookPublicPath
            ).borrow<
                &JadeContracts.ContractsBook{JadeContracts.ContractsBookPublic}
            >()
            ?? panic(
                "Could not borrow JadeContracts.ContractsBookPublic from user account"
            )
        return collections.getContracts()
    }
    
    pub fun getGlobalContractsBook(): &GlobalContractsBook{
        GlobalContractsBookPublic
    }{ 
    ...
    init(){ 
        ...
        self.account.link<&GlobalContractsBook{GlobalContractsBookPublic}>(
            JadeContracts.GlobalContractsBookPublicPath,
            target: JadeContracts.GlobalContractsBookStoragePath
        )
    }
}
