...
pub contract Necryptolis: NonFungibleToken{ 
    ...
    pub event PlotSalesInfoChanged(squarePixelPrice: UFix64, candlePrice: UFix64, trimPrice: UFix64, maxPlotHeight: UInt16, maxPlotWidth: UInt16, minPlotHeight: UInt16, minPlotWidth: UInt16, vaultAddress: Address, vaultType: Type)
    ...
    pub resource Collection: NecryptolisCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection{ 
        ...
        pub fun borrowCemeteryPlot(id: UInt64): &Necryptolis.NFT?{ 
            if self.ownedNFTs[id]                                  // lightCandle adds a candle to candles list of the NFT
                                  //
                                  // Parameters: 
                                  ...
                                  // buyerAddress: the address of user buying the candle
                                  != nil{ 
                let ref = (                           // trimCemeteryPlot trims the plot and resets lastTrimDate to current block timestamp
                           //
                           // Parameters: 
                           // cemeteryPlotId: id of the NFT
                           // buyerPayment: payment vault of the buyer
                           ...
                           &self.ownedNFTs[id] as auth                                                       // Allows the Cemetery plot owner to bury an NFT
                                                       //
                                                       &NonFungibleToken.NFT                                                                            // A resource which a user who wishes to add gravestones or 
    ...
    pub resource interface GravestoneCreator{ 
        pub fun createGravestone(necryptolisProviderCapability: Capability<&Necryptolis.Collection{NecryptolisCollectionPublic}>, nftID: UInt64, graveData: GraveData){} 
    }
    
    pub resource interface BurialProvider{ 
        pub fun buryNFT(necryptolisProviderCapability: Capability<&Necryptolis.Collection{NecryptolisCollectionPublic}>, plotID: UInt64, nftProviderCapability: Capability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>, nftType: Type, nftID: UInt64){} 
    }
