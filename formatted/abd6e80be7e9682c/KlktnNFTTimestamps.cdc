pub contract KlktnNFTTimestamps{ 
    ...
    pub event NFTTemplateTimestampCreated(
        typeID: UInt64,
        timestamps: KlktnNFTTemplateTimestamps
    )
    
    pub let AdminStoragePath: StoragePath
    
    priv var KlktnNFTTimestampsSet:{ UInt64: KlktnNFTTemplateTimestamps}
    
    pub struct KlktnNFTTemplateTimestamps{ 
        pub let typeID: UInt64
        
        priv var timestamps:{ String: UFix64}
    ...
    pub resource Admin{ 
        pub fun createKlktnNFTTemplateTimestamps(
            typeID: UInt64,
            initTimestamps:{ 
                String: UFix64
            }
            ...
            let newNFTTimestamps =
                KlktnNFTTemplateTimestamps(
                    initTypeID: typeID,
                    initTimestamps: initTimestamps
                )
            KlktnNFTTimestamps.KlktnNFTTimestampsSet[typeID] = newNFTTimestamps
            emit NFTTemplateTimestampCreated(
                typeID: typeID,
                timestamps: newNFTTimestamps
            )
        }
        
        pub fun updateKlktnNFTTemplateTimestamps(
            typeID: UInt64,
            newTimestamps:{ 
                String: UFix64
            }
            ...
            (KlktnNFTTimestamps.KlktnNFTTimestampsSet[typeID]!)
                .updateTimestamps(newTimestamps: newTimestamps)
            return KlktnNFTTimestamps.KlktnNFTTimestampsSet[typeID]!
        }
    }
    
    pub fun getNFTTemplateTimestamps(
        typeID: UInt64
    ): KlktnNFTTemplateTimestamps{ 
