...
import
    ...
    from "../0xcfdd90d4a00f7b5b/TeleportedTetherToken.cdc"
...
pub contract BloctoIdo{ 
    ...
    pub struct Activity{ 
        ...
        access(contract) fun addUserInfo(address: Address, userInfo: UserInfo){ 
            self.userInfos.insert(key: address, userInfo)
        }
        
        access(contract) fun updateUserInfo(
            address: Address,
            userInfo: UserInfo
        ){ 
        ...
        access(contract) fun updateTotalValidStake(
            poolType: String,
            totalValidStake: UFix64
        ){ 
            self.totalValidStake[poolType] = totalValidStake
        }
        
        access(contract) fun addKycList(address: Address){ 
    ...
    pub resource Admin{ 
        ...
        pub fun addKycList(_ name: String, _ addrList: [Address]){ 
            ...
            for addr in addrList{ 
                // check ido user
                let idoUserPublicRefOpt = getAccount(addr).getCapability(BloctoIdo.BloctoIdoUserPublicPath).borrow<&{BloctoIdo.UserPublic}>()
        ...
        pub fun updateStakeInfo(
            ...
            (BloctoIdo.activities[name]!).addUserInfo(
                address: addr,
                userInfo: userInfo
            )
        }
        ...
        pub fun distribute(_ name: String, start: UInt64, num: UInt64){ 
            let activity =
                BloctoIdo.activities[name] ?? panic("ido does not exist")
            let interval =
                activity.schedule["DISTRIBUTE"] ?? panic("stage does not exist")
            let current = getCurrentBlock().timestamp
            if current < interval[0] || current > interval[1]{ 
                panic("stage closed")
            }
            ...
            while curr < end{ 
                ...
                if userInfo.poolType != "" && userInfo.deposited != 0.0{ 
                    ...
                    let vaultRef = BloctoIdo.account.borrow<&FungibleToken.Vault>(from: activity.tokenInfo.storagePath) ?? panic("failed to get vault")
                    let receiverRefOpt = getAccount(address).getCapability(activity.tokenInfo.receiverPath).borrow<&{FungibleToken.Receiver}>()
        ...
        pub fun withdrawFromFee(amount: UFix64): @BloctoToken.Vault{ 
            return <-(
                BloctoIdo.fee.withdraw(amount: amount) as! @BloctoToken.Vault
            )
        }
        
        pub fun withdrawFromVault(
            amount: UFix64
        ): @TeleportedTetherToken.Vault{ 
            return <-(
                BloctoIdo.vault.withdraw(amount: amount)
                as!
                @TeleportedTetherToken.Vault
            )
        }
