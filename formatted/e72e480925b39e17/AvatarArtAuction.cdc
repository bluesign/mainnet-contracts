...
pub contract AvatarArtAuction{ 
    ...
    priv var priceSteps:{ UFix64: UFix64}
    
    priv var feeReference: Capability<
        &{AvatarArtTransactionInfo.PublicFeeInfo}
    >?
    
    priv var feeRecepientReference: Capability<
        &{AvatarArtTransactionInfo.PublicTransactionAddress}
    >?
    ...
    pub resource AuctionItem: AuctionPublic{ 
        ...
        priv var lastPrice: UFix64
        
        // The capablity to send the escrow bidVault affiliate 
        priv var affiliateVaultCap: Capability<&{FungibleToken.Receiver}>?
        
        // The capability that points to the resource where you want the NFT transfered to if you win this bid. 
        priv var recipientCollectionCap: Capability<&{AvatarArtNFT.CollectionPublic}>?
        ...
        priv let ownerCollectionCap: Capability<&{AvatarArtNFT.CollectionPublic}>
        ...
        priv let storeID: UInt64
        
        init(nft: @AvatarArtNFT.NFT, bidVault: @FungibleToken.Vault, startTime: UFix64, endTime: UFix64, startPrice: UFix64, minimumBidIncrement: UFix64, ownerCollectionCap: Capability<&{AvatarArtNFT.CollectionPublic}>, ownerVaultCap: Capability<&{FungibleToken.Receiver}>, storeID: UInt64){ 
            pre{ 
                bidVault.balance == 0.0:
                    "AuctionItem init: Bid Vault should be empty (balance = 0.0)"
                endTime > startTime:
        ...
        priv fun sendNFT(_ capability: Capability<&{AvatarArtNFT.CollectionPublic}>){ 
        ...
        priv fun sendBidTokens(_ capability: Capability<&{FungibleToken.Receiver}>){ 
        ...
        pub fun placeBid(price: UFix64, affiliateVaultCap: Capability<&{FungibleToken.Receiver}>?, vaultCap: Capability<&{FungibleToken.Receiver}>, collectionCap: Capability<&{AvatarArtNFT.CollectionPublic}>, bidTokens: @FungibleToken.Vault){ 
            pre{ 
                self.nft != nil:
                    "The selling NFT doesnot exist"
            }
            ...
            if biddingAmount < minNextBid{ 
                panic("bid amount + (your current bid) must be larger or equal to the current price + minimum bid increment ".concat(biddingAmount.toString()).concat(" < ").concat(minNextBid.toString()))
            }
            ...
            emit Bid(storeID: self.storeID, nftID: self.nft?.id!, auctionID: self.uuid, bidderAddress: bidderAddress, bidPrice: self.lastPrice, time: getCurrentBlock().timestamp)
        }
        
        priv fun settleFee(nftID: UInt64){ 
            ...
            let feeRecepientOp = ((AvatarArtAuction.feeRecepientReference!).borrow()!).getAddress(tokenId: nftID, payType: self.bidVault.getType())
            ...
            // Affiliate
            if fee.affiliate != nil && fee.affiliate > 0.0 && self.affiliateVaultCap != nil && (self.affiliateVaultCap!).check(){ 
            ...
            // Author
            if fee.author != nil && fee.author > 0.0 && feeRecepient.author != nil && (feeRecepient.author!).check(){ 
        ...
        access(contract) fun settleAuction(){ 
            pre{ 
                getCurrentBlock().timestamp > self.endTime:
        ...
        pub fun getDetails(): AuctionDetail{ 
            ...
            return AuctionDetail(id: self.uuid, currentPrice: self.lastPrice, bids: self.numberOfBids, active: !self.auctionCompleted && !self.isAuctionExpired(), timeRemaining: self.timeRemaining(), nftID: self.nft?.id, minimumBidIncrement: self.minimumBidIncrement, owner: self.ownerVaultCap.address, startTime: self.startTime, endTime: self.endTime, minNextBid: self.minNextBid(), completed: self.auctionCompleted, expired: self.isAuctionExpired(), leader: leader)
        }
    ...
    pub resource interface AuctionStorePublic{ 
        ...
        pub fun borrowAuction(auctionID: UInt64): &AuctionItem{
            AuctionPublic
        }?{} 
    }
    ...
    pub resource AuctionStore: AuctionStorePublic, AuctionStoreManager{ 
        priv let auctions: @{UInt64: AuctionItem}
        
        pub fun createStandaloneAuction(token: @AvatarArtNFT.NFT, bidVault: @FungibleToken.Vault, startTime: UFix64, endTime: UFix64, startPrice: UFix64, collectionCap: Capability<&{AvatarArtNFT.CollectionPublic}>, vaultCap: Capability<&{FungibleToken.Receiver}>): UInt64{ 
            pre{ 
            ...
            let auctionItem <- create AuctionItem(nft: <-token, bidVault: <-bidVault, startTime: startTime, endTime: endTime, startPrice: startPrice, minimumBidIncrement: AvatarArtAuction.getPriceStep(price: startPrice), ownerCollectionCap: collectionCap, ownerVaultCap: vaultCap, storeID: self.uuid)
            let auctionId = auctionItem.uuid
            let oldAuc <- self.auctions[auctionId] <- auctionItem
            destroy oldAuc
            emit AuctionAvailable(storeAddress: self.owner?.address!, auctionID: auctionId, nftType: nftType, nftID: nftID, ftVaultType: ftType, startPrice: startPrice, startTime: startTime, endTime: endTime)
            return auctionId
        }
        ...
        pub fun cancelAuction(auctionID: UInt64){ 
            let auction <- self.auctions.remove(key                                                   
                                                   // TODO: Remove suffix
                                                   : auctionID) ?? panic("could not find auction with given id")
    ...
    pub resource Administrator{ 
        ...
        pub fun setFeePreference(
            feeReference: Capability<
                &AvatarArtTransactionInfo.FeeInfo{
                    AvatarArtTransactionInfo.PublicFeeInfo
                }
            >,
            feeRecepientReference: Capability<
                &AvatarArtTransactionInfo.TransactionAddress{
                    AvatarArtTransactionInfo.PublicTransactionAddress
                }
            >
        ){ 
    ...
    priv fun getPriceStep(price: UFix64): UFix64{ 
