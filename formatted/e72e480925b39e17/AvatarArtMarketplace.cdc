...
pub contract AvatarArtMarketplace{ 
    ...
    pub resource SaleCollection: SalePublic{ 
        ...
        priv var listing:{ UInt64: ListingDetails}
        
        priv var ownerCollection: Capability<&AvatarArtNFT.Collection>
        
        // The fungible token vault of the owner of this sale.
        // When someone buys a token, this will be used to deposit
        // tokens into the owner's account.
        
        init(ownerCollection: Capability<&AvatarArtNFT.Collection>){ 
            pre{ 
                ownerCollection.check():
                    "Invalid owner's collection capability"
            }
            self.listing ={} 
        ...
        //
        pub fun listForSale(nftID: UInt64, price: UFix64, paymentType: Type, receiver: Capability<&{FungibleToken.Receiver}>){ 
            pre{ 
                (self.ownerCollection.borrow()!).borrowAvatarArtNFT(id: nftID) != nil:
                    "NFT does not exist in your collection"
                price > 0.0:
                    "Cannot list a NFT for 0.0"
                AvatarArtTransactionInfo.isCurrencyAccepted(type: paymentType):
                    "Payment type is not allow"
                receiver.check():
                    "can not borrow receiver"
            }
            
            // Set sale price
            self.listing[nftID] = ListingDetails(salePaymentVaultType: paymentType, salePrice: price, receiver: receiver)
            emit TokenListed(nftID: nftID, price: price, seller: self.owner?.address, paymentType: paymentType)
        }
        ...
        //
        pub fun unlistSale(tokenID: UInt64){ 
            pre{ 
        ...
        priv fun cutFee(vault: &FungibleToken.Vault, salePrice: UFix64, artId: UInt64, paymentType: Type, affiliateVaultCap: Capability<&{FungibleToken.Receiver}>?){ 
        ...
        //
        pub fun purchase(tokenID: UInt64, buyTokens: @FungibleToken.Vault, receiverCap: Capability<&{NonFungibleToken.CollectionPublic}>, affiliateVaultCap: Capability<&{FungibleToken.Receiver}>?){ 
            pre{ 
            ...
            let vaultRef = details.receiver.borrow() ?? panic("Could not borrow reference to seller token vault")
            ...
            let nft <- (self.ownerCollection.borrow()!).withdraw(withdrawID: tokenID)
            let receiver = receiverCap.borrow() ?? panic("Can not borrow a reference to buyer collection")
    ...
    init(){ 
        ...
        self.AdminStoragePath = /storage/AvatarArtMarketplaceAdmin05
        self
            .SaleCollectionStoragePath = /storage/AvatarArtMarketplaceSaleCollection05
        self
            .SaleCollectionPublicPath = /public/AvatarArtMarketplaceSaleCollection05
