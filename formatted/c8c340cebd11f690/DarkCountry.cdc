...
pub contract DarkCountry: NonFungibleToken{ 
    ...
    priv var itemTemplates:{ UInt64: ItemTemplate}
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &DarkCountry.NFT?{ 
        let collection = getAccount(from).getCapability(DarkCountry.CollectionPublicPath).borrow<&DarkCountry.Collection{DarkCountry.DarkCountryCollectionPublic}>() ?? panic("Couldn't get collection")
