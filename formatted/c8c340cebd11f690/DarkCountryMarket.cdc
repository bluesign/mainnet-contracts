...
pub contract DarkCountryMarket{ 
    ...
    pub resource SaleOffer: SaleOfferPublicView{ 
        ...
        priv let sellerItemProvider: Capability<&DarkCountry.Collection{NonFungibleToken.Provider}>
        
        // The FlowToken vault that will receive that payment if the sale completes successfully.
        priv let sellerPaymentReceiver: Capability<&FlowToken.Vault{FungibleToken.Receiver}>
        ...
        //
        pub fun accept(buyerCollection: &DarkCountry.Collection{NonFungibleToken.Receiver}, buyerPayment: @FungibleToken.Vault){ 
            pre{ 
        ...
        //
        init(sellerItemProvider: Capability<&DarkCountry.Collection{NonFungibleToken.Provider}>, itemID: UInt64, sellerPaymentReceiver: Capability<&FlowToken.Vault{FungibleToken.Receiver}>, price: UFix64, isPreOrdersOnly: Bool){ 
            pre{ 
            ...
            let collectionBorrow = (saleOwner.getCapability(DarkCountry.CollectionPublicPath)!).borrow<&{DarkCountry.DarkCountryCollectionPublic}>() ?? panic("Could not borrow DarkCountryCollectionPublic")
    ...
    pub fun createSaleOffer(
        sellerItemProvider: Capability<
            &DarkCountry.Collection{NonFungibleToken.Provider}
        >,
        itemID: UInt64,
        sellerPaymentReceiver: Capability<
    ...
    pub resource Admin{ 
        ...
        pub fun setPreOrdersForAddress(
            userAddress: Address,
            newPreOrders:{ 
                UInt64: UInt64
            }
    ...
    init(){ 
        ...
        self.beneficiaryCapability = self.account.getCapability(
