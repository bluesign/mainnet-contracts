...
This is the contract for dinosteam NFTs!
...
pub contract dinosteam: NonFungibleToken, ViewResolver{ 
    ...
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver, NiftoryNonFungibleToken.NFTPublic{ 
        ...
        init(setId: Int, templateId: Int, serial: UInt64){ 
            self.id = dinosteam.totalSupply
            dinosteam.totalSupply = dinosteam.totalSupply + 1
            self.setId = setId
    ...
    pub resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection, NiftoryNonFungibleToken.CollectionPublic, NiftoryNonFungibleToken.CollectionPrivate{ 
        ...
        pub fun deposit(token: @NonFungibleToken.NFT){ 
            let token <- token as! @dinosteam.NFT
        ...
        pub fun depositBulk(tokens: @[NonFungibleToken.NFT]){ 
            while tokens.length > 0{ 
                let token <- tokens.removeLast() as! @dinosteam.NFT
    ...
    pub resource Manager: NiftoryNonFungibleToken.ManagerPublic, NiftoryNonFungibleToken.ManagerPrivate{ 
        ...
        pub fun metadata(): AnyStruct?{ 
            return dinosteam.metadata
        }
        
        pub fun getSetManagerPublic(): &MutableMetadataSetManager.Manager{MutableMetadataSetManager.Public}{ 
            return NiftoryNFTRegistry.getSetManagerPublic(dinosteam.REGISTRY_ADDRESS, dinosteam.REGISTRY_BRAND)
        }
        
        pub fun getMetadataViewsManagerPublic(): &MetadataViewsManager.Manager{MetadataViewsManager.Public}{ 
            return NiftoryNFTRegistry.getMetadataViewsManagerPublic(dinosteam.REGISTRY_ADDRESS, dinosteam.REGISTRY_BRAND)
        }
        
        pub fun getNFTCollectionData(): MetadataViews.NFTCollectionData{ 
            return NiftoryNFTRegistry.buildNFTCollectionData(dinosteam.REGISTRY_ADDRESS, dinosteam.REGISTRY_BRAND, fun (): @NonFungibleToken.Collection{ 
                    return <-dinosteam.createEmptyCollection()
                })
        }
        ...
        pub fun modifyContractMetadata(): auth &AnyStruct{ 
            ...
            let maybeMetadata = dinosteam.metadata
            if maybeMetadata == nil{ 
                let blankMetadata:{ String: String} ={} 
                dinosteam.metadata = blankMetadata
            }
            return (&dinosteam.metadata as auth &AnyStruct?)!
        }
        
        pub fun replaceContractMetadata(_ metadata: AnyStruct?){ 
            emit ContractMetadataUpdated()
            dinosteam.metadata = metadata
        }
        ...
        priv fun _getMetadataViewsManagerPrivate(): &MetadataViewsManager.Manager{MetadataViewsManager.Private}{ 
            let record = NiftoryNFTRegistry.getRegistryRecord(dinosteam.REGISTRY_ADDRESS, dinosteam.REGISTRY_BRAND)
            let manager = dinosteam.account.getCapability<&MetadataViewsManager.Manager{MetadataViewsManager.Private}>(record.metadataViewsManager.paths.private).borrow()!
            return manager
        }
        ...
        priv fun _getSetManagerPrivate(): &MutableMetadataSetManager.Manager{MutableMetadataSetManager.Public, MutableMetadataSetManager.Private}{ 
            let record = NiftoryNFTRegistry.getRegistryRecord(dinosteam.REGISTRY_ADDRESS, dinosteam.REGISTRY_BRAND)
            let setManager = dinosteam.account.getCapability<&MutableMetadataSetManager.Manager{MutableMetadataSetManager.Public, MutableMetadataSetManager.Private}>(record.setManager.paths.private).borrow()!
            return setManager
        }
    ...
    pub fun contract(): &{NiftoryNonFungibleToken.ManagerPublic}{ 
        return NiftoryNFTRegistry.getNFTManagerPublic(dinosteam.REGISTRY_ADDRESS, dinosteam.REGISTRY_BRAND)
    }
    ...
    init(nftManagerProxy: &{NiftoryNonFungibleTokenProxy.Public, NiftoryNonFungibleTokenProxy.Private}){ 
        let record = NiftoryNFTRegistry.generateRecord(account: self.account.address, project: "clbu7s6u40000ky0grxxj0xa3_dinosteam")
        self.REGISTRY_ADDRESS = 0x32d62d5c43ad1038
        self.REGISTRY_BRAND = "clbu7s6u40000ky0grxxj0xa3_dinosteam"
        ...
        self.account.save<@MutableMetadataSetManager.Manager>(<-MutableMetadataSetManager.create(name: "dinosteam", description: "The set manager for dinosteam."), to: record.setManager.paths.storage)
