...
pub contract BulkSales{ 
    access(           
           /// AllowBulkPurchasing
           /// Toggle to control execution of new bulk purchases
           contract) var AllowBulkPurchasing: Bool
    ...
    pub let CommissionReaderCapability: Capability<
        &BulkSalesAdmin{CommissionReader}
    >
    
    /// ContractCommissionRate
    /// Facilitator commission rate applied to new listings
    pub var ContractCommissionRate: UFix64
    ...
    pub var SecondaryCommissionRate: UFix64
    ...
    pub struct BulkPurchaseOrder{ 
        access(contract) let orders: [PurchaseOrder]
        
        access(contract) let storefrontV1Refs:{ 
            Address: &NFTStorefront.Storefront{NFTStorefront.StorefrontPublic}
        }
        
        access(contract) let storefrontV2Refs:{ 
            Address: &NFTStorefrontV2.Storefront{
        ...
        priv fun addToPriceTotal(
            vaultIdentifier: String,
            _ additionalAmount: UFix64
        ){ 
        ...
        priv fun tabulateV1Listing(
            _ listing: &NFTStorefront.Listing{NFTStorefront.ListingPublic},
            order: PurchaseOrder
        ){ 
        ...
        priv fun tabulateV2Listing(
            _ listing: &NFTStorefrontV2.Listing{NFTStorefrontV2.ListingPublic},
            order: PurchaseOrder
        ){ 
            let listingDetails = listing.getDetails()
            assert(
    ...
    pub struct ListingOrder{ 
        ...
        init(
            ...
            self.nftType = CompositeType(nftTypeIdentifier)
                ?? panic(
                    "unable to cast type; must be a valid NFT type reference"
                )
            self.nftID = nftID
            self.salePrice = salePrice
            self.royalties = royalties
            self.expirationDays = expirationDays
                ?? BulkSales.DefaultExpirationDays
        }
    }
    
    /// SalesAdmin
    /// Private capability to toggle sales
    pub resource interface SalesAdmin{ 
        pub fun toggleBulkPurchasing(_ value: Bool){} 
        
        pub fun toggleBulkListing(_ value: Bool){} 
    }
    
    /// CommissionAdmin
    /// Private capability to adjust commission settings
    pub resource interface CommissionAdmin{ 
        pub fun addContractCommissionReceiver(
            _ receiver: Capability<&AnyResource{FungibleToken.Receiver}>
        ){} 
        
        pub fun setCommissionRate(_ rate: UFix64, secondary: Bool){} 
        
        pub fun addMarketplaceCommissionReceiver(address: Address){} 
        
        pub fun removeMarketplaceCommissionReceiver(address: Address){} 
    }
    
    /// CommissionReader
    /// Public capability to get the contract and marketplace commission receivers
    pub resource interface CommissionReader{ 
        pub fun getContractCommissionReceiver(_ identifier: String): Capability<
            &AnyResource{FungibleToken.Receiver}
        >?{} 
        
        pub fun getMarketplaceCommissionReceivers(vaultPath: PublicPath): [
    ...
    pub resource BulkSalesAdmin: SalesAdmin, CommissionAdmin, CommissionReader{ 
        ...
        priv var marketplaceCommissionAddresses: [Address]
        ...
        pub fun getContractCommissionReceiver(_ identifier: String): Capability<&AnyResource{FungibleToken.Receiver}>?{ 
        ...
        pub fun getMarketplaceCommissionReceivers(vaultPath: PublicPath): [Capability<&{FungibleToken.Receiver}>]{ 
        ...
        pub fun addContractCommissionReceiver(_ receiver: Capability<&AnyResource{FungibleToken.Receiver}>){ 
    ...
    pub fun getCatalogEntryForNFT(
        nftTypeIdentifier: String,
        ownerAddress: Address?,
        nftID: UInt64?
    ): NFTCatalog.NFTCatalogMetadata?{ 
        let nftCatalogCollections:{ String: Bool}? =
            NFTCatalog.getCollectionsForType(
                nftTypeIdentifier: nftTypeIdentifier
            )
        if nftCatalogCollections == nil
        || (nftCatalogCollections!).keys.length < 1{ 
    ...
    pub fun purchaseNFTs(
        ...
        nftReceiverCapabilities:{ 
            String: Capability<&AnyResource{NonFungibleToken.Receiver}>
        },
        expectedTotals:{ 
            String: UFix64
        }?,
    ...
    pub fun listNFTs(
        ...
        let saleVaultCapability =
            (storefront.owner!).getCapability<
                &AnyResource{FungibleToken.Receiver}
            >(saleVaultPath)
        let saleVaultRef =
            saleVaultCapability.borrow()
            ?? panic(
                "seller receiver vault invalid or missing for ".concat(
                    saleVaultPath.toString()
                )
            )
    ...
    access(contract) fun listNFT(
        ...
        assert(
            ...
            message: "could not verify ownership for ".concat(
                listingOrder.nftID.toString()
            )
        )
        ...
        // add seller cut
        assert(
            totalCutAmount < listingOrder.salePrice,
            message: "no remaining cut for seller"
        )
        saleCutsV2.append(
        ...
        // get expiration
        let expirationOffset: UInt64 =
            listingOrder.expirationDays * 24 * 60 * 60 * 1000
        ...
        var commissionCapabilities: [Capability<&{FungibleToken.Receiver}>]? =
            nil
        if BulkSales.ContractCommissionRate > 0.0{ 
    ...
    init(){ 
        ...
        self.account.save(
            <-create BulkSalesAdmin(),
            to: self.BulkSalesAdminStoragePath
        )
        self.account.link<&BulkSalesAdmin{CommissionAdmin}>(
            self.CommissionAdminPrivatePath,
            target: self.BulkSalesAdminStoragePath
        )
        self.CommissionReaderCapability = self.account.link<
                &BulkSalesAdmin{CommissionReader}
            >(
                self.CommissionReaderPublicPath,
                target: self.BulkSalesAdminStoragePath
            )!
    }
}
