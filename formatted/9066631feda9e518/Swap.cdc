...
pub contract Swap{ 
    ...
    pub let SwapFeeReaderPublicPath: PublicPath
    ...
    pub resource SwapCollection: SwapCollectionManager, SwapCollectionPublic{ 
        ...
        pub fun createProposal(leftUserOffer: UserOffer, rightUserOffer: UserOffer, leftUserCapabilities: UserCapabilities, expirationOffsetMinutes: UFix64?): String{ 
    ...
    pub resource interface SwapProposalManager{ 
        ...
        access(account) fun getProposalCreationStatus(): Bool{} 
    }
    ...
    pub resource interface SwapFeeReader{ 
        pub fun getFees(): [Fee]{} 
    }
    
    // SwapAdmin
    // This object provides admin controls for swap proposals
    pub resource SwapAdmin: SwapProposalManager, SwapFeeManager, SwapFeeReader{ 
    ...
    init(){ 
        ...
        self.account.link<&{SwapProposalManager, SwapFeeManager}>(
            self.SwapAdminPrivatePath,
            target: self.SwapAdminStoragePath
        )
        self.account.link<&{SwapFeeReader}>(
            self.SwapFeeReaderPublicPath,
            target: self.SwapAdminStoragePath
        )
    }
}
