...
pub contract FLOATTreasuryStrategies{ 
    ...
    pub event FLOATStrategyGoalAccomplished(
        strategyIdentifier: String,
        strategyUuid: UInt64,
        user: Address
    )
    
    pub event FLOATStrategyLotteryDrawn(
        strategyIdentifier: String,
        winners: [
            Address
        ]
    ...
    pub resource LotteryStrategy: FLOATEventSeries.ITreasuryStrategy{ 
        ...
        priv let params:{ String: AnyStruct}
        
        priv let minimiumValid: UInt64
        
        priv let ending:{ FLOATEventSeries.StrategyState: UFix64}
        
        priv var valid: [Address]
        
        priv var winners: [Address]
    ...
    access(contract) fun parseStrategyTime(_ params:{ String: AnyStruct}):{ 
        FLOATEventSeries.StrategyState: UFix64
    }{ 
    ...
    pub fun createStrategy(
        ...
        switch type{ 
            case StrategyType.Lottery:
                return <-create LotteryStrategy(
                    controller: <-controller,
                    params: params
                )
            case StrategyType.ClaimingQueue:
                return <-create ClaimingQueueStrategy(
                    controller: <-controller,
                    params: params
                )
            default:
