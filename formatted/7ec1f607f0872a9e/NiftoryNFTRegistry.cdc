...
pub contract NiftoryNFTRegistry{ 
    ...
    pub resource Registry: Public, Private{ 
        ...
        priv let _registry:{ String: Record}
    ...
    pub fun generatePaths(prefix: String, suffix: String): Paths{ 
    ...
    pub fun _notFoundError(
        ...
        return entity.concat(" not found for registry at ").concat(
    ...
    pub fun getRegistryRecord(
        ...
        let registry =
            getAccount(registryAddress).getCapability<&{Public}>(
                self.PUBLIC_PATH
            ).borrow()
            ?? panic(
                self._notFoundError(registryAddress, brand, "Registry Record")
            )
    ...
    pub fun getNFTManagerPublic(_ registryAddress: Address, _ brand: String): &{
        ...
        let record = self.getRegistryRecord(registryAddress, brand)
        let manager =
            getAccount(record.nftManager.account).getCapability<
                &{NiftoryNonFungibleToken.ManagerPublic}
    ...
    pub fun getSetManagerPublic(
        ...
        let record = self.getRegistryRecord(registryAddress, brand)
        let manager =
            getAccount(record.setManager.account).getCapability<
    ...
    pub fun getMetadataViewsManagerPublic(
        ...
        let record = self.getRegistryRecord(registryAddress, brand)
        let manager =
            getAccount(record.metadataViewsManager.account).getCapability<
                &MetadataViewsManager.Manager{MetadataViewsManager.Public}
            >(record.metadataViewsManager.paths.public)
        return manager.borrow()
        ?? panic(
            self._notFoundError(registryAddress, brand, "MetadataViewsManager")
        )
    }
