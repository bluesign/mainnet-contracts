...
pub contract FlovatarPack{ 
    ...
    access(account) fun createPack(
        ...
        emit Created(
            id: newPack.id,
            prefix: randomString.slice(from: 0, upTo: 4)
        )
        return <-newPack
    }
    ...
    init(){ 
        ...
        self.totalSupply = 0
        self.account.save<@FlovatarPack.Collection>(
            <-FlovatarPack.createEmptyCollection(ownerVault: wallet),
            to: FlovatarPack.CollectionStoragePath
        )
        self.account.link<&{FlovatarPack.CollectionPublic}>(
            FlovatarPack.CollectionPublicPath,
            target: FlovatarPack.CollectionStoragePath
        )
