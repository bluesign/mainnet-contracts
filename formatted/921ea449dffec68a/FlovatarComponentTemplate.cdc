...
pub contract FlovatarComponentTemplate{ 
    ...
    pub resource interface CollectionPublic{ 
        ...
        pub fun borrowComponentTemplate(id: UInt64): &{
            FlovatarComponentTemplate.Public
        }?{} 
    }
    ...
    pub struct ComponentTemplateData{ 
        ...
        init(
            ...
            self.series = series
            self.maxMintableComponents = maxMintableComponents
            self.totalMintedComponents = FlovatarComponentTemplate
                    .getTotalMintedComponents(id: id)!
            self.lastComponentMintedAt = FlovatarComponentTemplate
    ...
    pub fun getComponentTemplates(): [ComponentTemplateData]{ 
        ...
        if let componentTemplateCollection =
            self.account.getCapability(self.CollectionPublicPath).borrow<
                &{FlovatarComponentTemplate.CollectionPublic}
            >(){ 
    ...
    pub fun getComponentTemplate(id: UInt64): ComponentTemplateData?{ 
        if let componentTemplateCollection =
            self.account.getCapability(self.CollectionPublicPath).borrow<
                &{FlovatarComponentTemplate.CollectionPublic}
            >(){ 
            if let componentTemplate =
                componentTemplateCollection.borrowComponentTemplate(id: id){ 
                return ComponentTemplateData(
                    id: id,
                    ...
                    maxMintableComponents: (componentTemplate!)
    ...
    init(){ 
        ...
        self
            .CollectionStoragePath = /storage/FlovatarComponentTemplateCollection
        
        // Initialize the total supply
        self.totalSupply = 0
        self.totalMintedComponents ={} 
        self.lastComponentMintedAt ={} 
        self.account.save<@FlovatarComponentTemplate.Collection>(
            <-FlovatarComponentTemplate.createEmptyCollection(),
            to: FlovatarComponentTemplate.CollectionStoragePath
        )
        self.account.link<&{FlovatarComponentTemplate.CollectionPublic}>(
            FlovatarComponentTemplate.CollectionPublicPath,
            target: FlovatarComponentTemplate.CollectionStoragePath
        )
