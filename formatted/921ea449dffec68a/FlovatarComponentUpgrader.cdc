...
pub contract FlovatarComponentUpgrader{ 
    ...
    pub event FlovatarComponentUpgraded(
        newId: UInt64,
        rarity: String,
        category: String,
        burnedIds: [
    ...
    pub struct RandomInt{ 
        ...
        pub fun getValue(): UInt64{ 
            ...
            let h: [UInt8] =
                HashAlgorithm.SHA3_256.hash(self.uuid.toBigEndianBytes())
            ...
            self.value = self.minValue
                + random % (self.maxValue - self.minValue)
    ...
    pub fun upgradeFlovatarComponent(
        components: @[
        ...
        if let upgraderCollection =
            self.account.borrow<&FlovatarComponentUpgrader.Collection>(
            ...
            if let inboxCollection =
                ...
                inboxCollection.depositComponentToWallet(
                    address: address,
                    component: <-component
                )
            } else{ 
    ...
    init(){ 
        ...
        self
            .CollectionStoragePath = /storage/FlovatarComponentUpgraderCollection
        self.account.save<@FlovatarComponentUpgrader.Collection>(
            <-FlovatarComponentUpgrader.createEmptyCollection(),
            to: FlovatarComponentUpgrader.CollectionStoragePath
        )
        self.account.link<&{FlovatarComponentUpgrader.CollectionPublic}>(
            FlovatarComponentUpgrader.CollectionPublicPath,
            target: FlovatarComponentUpgrader.CollectionStoragePath
        )
