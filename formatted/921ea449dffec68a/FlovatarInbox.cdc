...
pub contract FlovatarInbox{ 
    ...
    pub event FlovatarWithdrawComponent(
        flovatarId: UInt64,
        componentId: UInt64,
        to: Address
    )
    ...
    pub event FlovatarClaimedCommunityDust(
        id: UInt64,
        amount: UFix64,
        to: Address
    )
    ...
    pub resource Container{ 
        ...
        access(contract) let flovatarComponents: @{
            UInt64: FlovatarComponent.NFT
        }
        
        priv var blockHeight: UInt64
        ...
        pub fun insertComponent(component: @FlovatarComponent.NFT){ 
            let oldComponent
                <- self.flovatarComponents[component.id]
                <- component
        ...
        pub fun withdrawComponent(id: UInt64): @FlovatarComponent.NFT{ 
            ...
            let token <-
                self.flovatarComponents.remove(key: id) ?? panic("missing NFT")
            return <-token
        }
    ...
    pub resource interface CollectionPublic{ 
        pub fun depositDustToFlovatar(
            id: UInt64,
            vault: @FungibleToken.Vault
        ){} 
        
        pub fun depositDustToWallet(
            address: Address,
            vault: @FungibleToken.Vault
        ){} 
        
        pub fun depositComponentToFlovatar(
            id: UInt64,
            component: @FlovatarComponent.NFT
        ){} 
        
        pub fun depositComponentToWallet(
            address: Address,
            component: @FlovatarComponent.NFT
        ){} 
    ...
    pub fun getFlovatarDustBalance(id: UInt64): UFix64{ 
        if let inboxCollection =
            self.account.getCapability(self.CollectionPublicPath).borrow<
                &{FlovatarInbox.CollectionPublic}
            >(){ 
    ...
    pub fun withdrawFlovatarComponent(id: UInt64, address: Address){ 
        ...
        if let inboxCollection =
            self.account.borrow<&FlovatarInbox.Collection>(
                from: self.CollectionStoragePath
            ){ 
            if let flovatar =
                Flovatar.getFlovatar(address: address, flovatarId: id){ 
                let receiverAccount = getAccount(address)
                let flovatarComponentReceiverCollection =
                    receiverAccount.getCapability<
                        &{FlovatarComponent.CollectionPublic}
    ...
    pub fun getClaimableFlovatarCommunityDust(
        id: UInt64,
        address: Address
    ): ClaimableDust?{ 
        if let flovatarScore: UFix64 =
            ...
            if let inboxCollection =
                self.account.getCapability(self.CollectionPublicPath).borrow<
                    &{FlovatarInbox.CollectionPublic}
                >(){ 
                let lastClaimed: UFix64 =
                    inboxCollection.getLastClaimedDust(id: id)
    ...
    priv fun setLastClaimedDust(id: UInt64, days: UInt64){ 
        if let inboxCollection =
            self.account.borrow<&FlovatarInbox.Collection>(
                from: self.CollectionStoragePath
            ){ 
            let lastClaimed: UFix64 = inboxCollection.getLastClaimedDust(id: id)
            inboxCollection.setLastClaimedDust(
                id: id,
                value: lastClaimed + UFix64(days * 86400)
            )
        }
    }
    ...
    pub fun claimFlovatarCommunityDust(id: UInt64, address: Address){ 
        ...
        if let claimableDust: ClaimableDust =
            ...
            if claimableDust.amount > UFix64(0.0){ 
                let receiverAccount = getAccount(address)
                let receiverRef = (receiverAccount.getCapability(FlovatarDustToken.VaultReceiverPath)!).borrow<&{FungibleToken.Receiver}>() ?? panic("Could not borrow receiver reference to the recipient's Vault")
    ...
    pub fun claimFlovatarCommunityDustFromChild(
        id: UInt64,
        parent: Address,
        child: Address
    ){ 
        ...
        let manager =
            getAccount(parent).getCapability<
                &HybridCustody.Manager{HybridCustody.ManagerPublic}
            >(HybridCustody.ManagerPublicPath).borrow()
    ...
    init(){ 
        ...
        self.account.save<@FlovatarInbox.Collection>(
            <-FlovatarInbox.createEmptyCollection(),
            to: FlovatarInbox.CollectionStoragePath
        )
        self.account.link<&{FlovatarInbox.CollectionPublic}>(
            FlovatarInbox.CollectionPublicPath,
            target: FlovatarInbox.CollectionStoragePath
        )
        
        // Put the Admin resource in storage
        self.account.save<@Admin>(
            <-create Admin(),
            to: /storage/FlovatarInboxAdmin
        )
