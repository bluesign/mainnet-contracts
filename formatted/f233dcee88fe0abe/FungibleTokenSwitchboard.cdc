    ...
    pub resource Switchboard: FungibleToken.Receiver, SwitchboardPublic{ 
        ...
        pub fun addNewVault(capability: Capability<&{FungibleToken.Receiver}>){ 
            ...
            if self.receiverCapabilities[vaultRef.getType()] == nil{ 
                ...
                // added
                emit VaultCapabilityAdded(type: vaultRef.getType(), switchboardOwner: self.owner?.address, capabilityOwner: capability.address)
            } else{ 
            ...
            for path in paths{ 
                ...
                if let vaultRef = capability.borrow(){ 
                    ...
                    if self.receiverCapabilities[(vaultRef!).getType()] == nil{ 
                        ...
                        emit VaultCapabilityAdded(type: vaultRef.getType(), switchboardOwner: self.owner?.address, capabilityOwner: address)
                    }
                }
            }
        }
        ...
        pub fun addNewVaultWrapper(capability: Capability<&{FungibleToken.Receiver}>, type: Type){ 
            ...
            // emit the event that indicates that a new capability has been 
            // added
            emit VaultCapabilityAdded(type: type, switchboardOwner: self.owner?.address, capabilityOwner: capability.address)
        }
        ...
        pub fun removeVault(capability: Capability<&{FungibleToken.Receiver}>){ 
            ...
            // Emit the event that indicates that a new capability has been 
            // removed
            emit VaultCapabilityRemoved(type: vaultRef.getType(), switchboardOwner: self.owner?.address, capabilityOwner: capability.address)
        }
        ...
        pub fun deposit(from: @FungibleToken.Vault){ 
            ...
            let depositedVaultCapability = self.receiverCapabilities[from.getType()] ?? panic("The deposited vault is not available on this switchboard")
            // Borrow the reference to the desired vault
            let vaultRef = depositedVaultCapability.borrow() ?? panic("Can not borrow a reference to the the vault")
