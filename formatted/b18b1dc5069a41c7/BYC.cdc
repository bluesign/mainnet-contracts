...
pub contract BYC{ 
    ...
    access(contract) var feeReceiverCapByIdentifier:{ 
    ...
    //
    pub struct FTAsset{ 
        pub(set) var providerCap: Capability<
            &{FungibleToken.Provider, FungibleToken.Balance}
        >?
        
        pub(set) var receiverCap: Capability<
        ...
        pub fun providerIsValid(): Bool{ 
            ...
            assert(
                ...
                message: "Provider has insufficient tokens! Requested:".concat(
                    ...
                    (self.providerCap?.borrow()!!).balance.toString().concat(
                        " available!"
                    )
                )
            )
        ...
        pub fun receiverIsValid(): Bool{ 
            assert(
                self.receiverCap?.borrow() != nil,
                message: "invalid receiver capability"
            )
            return true
        }
    ...
    pub struct NFTAsset{ 
        ...
        pub fun providerIsValid(): Bool{ 
            ...
            assert(
                ...
                message: "Provider does not have the requested NFT! Requested:"
        ...
        pub fun doesAddressOwnNFT(_ address: Address): Bool{ 
            ...
            let collectionRef =
                account.getCapability(self.collectionPublicPath!).borrow<
                    &{NonFungibleToken.CollectionPublic}
                >()
        ...
        access(contract) fun transfer(){ 
            (self.receiverCap?.borrow()!!).deposit(
                token: <-(self.providerCap?.borrow()!!).withdraw(
                    withdrawID: self.id!
                )
            )
        }
    ...
    pub resource Barter: BarterPublic, IRestricted, MetadataViews.Resolver{ 
        ...
        //
        access(contract) fun acceptBarter(offeredNFTReceiverCaps: [Capability<&AnyResource{NonFungibleToken.CollectionPublic}>], requestedNFTProviderCaps: [Capability<&AnyResource{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>], offeredFTReceiverCaps: [Capability<&AnyResource{FungibleToken.Receiver, FungibleToken.Balance}>], requestedFTProviderCaps: [Capability<&AnyResource{FungibleToken.Provider, FungibleToken.Balance}>], feeCapability: Capability<&AnyResource{FungibleToken.Provider}>){ 
            pre{ 
    ...
    pub resource interface BarterCollectionPublic{ 
        ...
        pub fun acceptBarter(
            ...
            offeredNFTReceiverCaps: [
                Capability<&AnyResource{NonFungibleToken.CollectionPublic}>
            ],
            ...
            offeredFTReceiverCaps: [
                Capability<
                    &AnyResource{FungibleToken.Receiver, FungibleToken.Balance}
                >
            ],
            requestedFTProviderCaps: [
                Capability<
                    &AnyResource{FungibleToken.Provider, FungibleToken.Balance}
                >
            ],
    ...
    //
    pub resource BarterCollection:
        BarterCollectionPublic,
        ...
        pub fun acceptBarter(
            ...
            barter.acceptBarter(
                offeredNFTReceiverCaps: offeredNFTReceiverCaps,
            ...
            let linkedCollection =
                getAccount(barter.getOfferAddress()).getCapability(
                    BYC.BarterCollectionPublicPath
                ).borrow<&{BYC.BarterCollectionPublic}>()!
            linkedCollection.clean(
                barterRef: &barter as &Barter{IRestricted},
                id: barter.linkedID!
            )
            destroy barter
        }
        ...
        pub fun counterBarter(
            ...
            let barterCollectionCap =
                getAccount(barterAddress).getCapability(
                    BYC.BarterCollectionPublicPath
                )
            let barterCollectionRef =
                barterCollectionCap.borrow<&{BarterCollectionPublic}>()!
            let barterRef = barterCollectionRef.borrowBarter(id: barterID)!
            var counterpartyAddress = barterRef.getOfferAddress()
            let counterpartyCollection =
                getAccount(counterpartyAddress!).getCapability(
                    BYC.BarterCollectionPublicPath
                ).borrow<&{BYC.BarterCollectionPublic}>()!
    ...
    pub resource Admin{ 
        //
        pub fun updateFeeByIdentifier(
            identifier: String,
            fee: UFix64,
            feeCap: Capability<&AnyResource{FungibleToken.Receiver}>
        ){ 
