...
/************************************************************************/pub contract DAAM: NonFungibleToken{ 
    ...
    pub resource MetadataGenerator: MetadataGeneratorPublic, MetadataGeneratorMint{ 
        ...
        pub fun addMetadata(name: String, max: UInt64?, categories: [Categories.Category], description: String, misc: String, thumbnail:{ String:{ MetadataViews.File}}, file:{ String: MetadataViews.Media}, interact: AnyStruct?): UInt64{ 
            pre{ 
                self.grantee == (self.owner!).address:
                    "Account: ".concat((self.owner!).address.toString()).concat(" Permission Denied")
                DAAM.creators.containsKey(self.grantee):
    ...
    pub resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, CollectionPublic, MetadataViews.ResolverCollection, MetadataViews.Resolver{ 
        ...
        pub fun addCollection(name: String, description: String, externalURL: MetadataViews.ExternalURL, squareImage: MetadataViews.Media, bannerImage: MetadataViews.Media, socials:{ String: MetadataViews.ExternalURL}){ 
        ...
        pub fun borrowDAAM(id: UInt64): &DAAM.NFT{ 
            ...
            let ref = (&self.ownedNFTs[id] as auth &NonFungibleToken.NFT // Destructor /************************************************************************/// Agent interface. List of all powers belonging to the Agent
// the current status of the Admin // Admin invites a new creator        // Admin or Agent change Creator status         // Admin or Agenct can change MID copyright status // Admin or Agent can change Metadata Status // Admin or Agent can remove CAmiRajpal@hotmail.cometadata Status // Create Request Generator?) /************************************************************************/// The Admin Resource deletgates permissions between Founders and Agents
// The current status of the Admin // Default Admin status: True                                                                          // Used only when genreating a new Admin. Creates a Resource Generator for Negoiations.
                                                                          !
            let daam // status variable may be Depreicated // TODO check  // return new Request // Admin invite a new Admin // Admin account is setup but not active untill accepted. // Admin ivites new Agent // Admin account is setup but not active untill accepted. // Agent account is setup but not active untill accepted. // Admin or Agent invite a new creator, agentCut = nil no agent = ref // Creator account is setup but not active untill accepted. // Admin invites a new Minter (Key) // Minter Key is setup but not active untill accepted. // Two Admin to Remove Admin // TODO change to x // Append removal list // If votes is 3 or greater // {To Remove : Total Votes} // Talley Votes
// get To Remove // increment counter
// Remove all with a vote of 3 or greater
// Does To Remove have enough votes to be removed // Reset DAAM.Remove // Remove selected Admin // end if // Admin removes selected Agent by Address // Unreachable // Remove Agent from list // Remove Agent from list // Admin removes selected Creator by Address // Verify Agent or Admin // Unreachable // Remove Creator from list // Remove Metadata Capability from list // Admin removes selected Agent by Address // Unreachable // Remove Agent from list // Admin can Change Agent status 
// Unreachable // status changed // Admin or Agent can Change Creator status 
// Verify Agent or Admin // Unreachable // status changed // Admin can Change Minter status 
// Unreachable // status changed // Admin or Agent can change a Metadata status.
// Verify Agent or Admin // change to a new Metadata status // Admin or Agent can change a MIDs copyright status.
// Verify Agent or Admin // Unreachable // Change to new copyright // Create Metadata // copy of MetadataGenerator.saveMID function below
// a metadata ID for Admin approval, currently unapproved (false) // default copyright setting // TODO REMOVE AUTO-APPROVE AFTER DEVELOPMENT // end of copy
/************************************************************************/// Agent First Sale // nil = invited, false = frozen, true = active /************************************************************************/// The Creator Resource (like Admin/Agent) is a permissions Resource. This allows the Creator
...
// Insert new Minter in minter list. // Get MID // Create NFT // Update Request, if last remove.
// Get Request using MID // if last destroy request, Request not needed. Counter has reached limit. // Mark NFT as new // return NFT // Removes token from 'new' list. 'new' is defines as newly Mited. Age is not a consideration.
// Unreachable // start the conter // cycle through 'new' list // if Token ID is found // remove from 'new' list // increment counter // end for // Add NFT to 'new' list
// Append 'new' list /************************************************************************/// Is a Marketplace, Access Approvd, minterStatus is always true // Access is Frozen or Invitation has not been acccepted. // is an Agent, verify creator & mid relationship to agent // Verify MID belongs to Creator && Verify Agent is Creators'
...
// Counter varibbles
// Initialize the total supply of NFTs // Incremental Serial Number for the MetadataGenerator                           // Setup Up Default Admins
                           as! &DAAM.NFT
            return daam
        }
    ...
    pub resource interface Agent{ 
        ...
        pub fun inviteCreator(_ creator: Address, agentCut: UFix64){} 
        ...
        pub fun createMetadata(creator: Address, name: String, max: UInt64?, categories: [Categories.Category], description: String, misc: String, thumbnail:{ String:{ MetadataViews.File}}, file:{ String: MetadataViews.Media}, interact: AnyStruct?): @Metadata{} 
    }
    ...
    pub resource Admin: Agent{ 
        ...
        pub fun newRequestGenerator(): @RequestGenerator{ 
            pre{ 
                ...
                self.grantee == (self.owner!).address:
                    "Account: ".concat((self.owner!).address.toString()).concat(" Permission Denied")
                self.status:
                    "Account: ".concat((self.owner!).address.toString()).concat(" Access has been Frozen.")
            }
            return <-create RequestGenerator(self.grantee)
        }
        
        pub fun inviteAdmin(_ admin: Address){ 
            pre{ 
                DAAM.admins[(self.owner!).address] == true:
                    "Account: ".concat((self.owner!).address.toString()).concat(" Permission Denied")
                self.grantee == (self.owner!).address:
            ...
            DAAM.admins.insert(key: admin, false)
        ...
        pub fun inviteAgent(_ agent: Address){ 
            ...
            DAAM.admins.insert(key: agent, false)
        ...
        pub fun inviteCreator(_ creator: Address, agentCut: UFix64){ 
            ...
            DAAM.creators.insert(key: creator, creatorInfo)
        ...
        pub fun inviteMinter(_ minter: Address){ 
            ...
            DAAM.minters.insert(key: minter, false)
        ...
        pub fun removeAdmin(admin: Address){ 
            ...
            let vote = 5 as Int
            DAAM.remove.insert(key: self.grantee, admin)
            if DAAM.remove.length >= vote{ 
                var counter:{ Address: Int} ={} 
                for a in DAAM.remove.keys{ 
                    let remove = DAAM.remove[a]!
        ...
        pub fun removeAgent(agent: Address){ 
            pre{ 
                DAAM.admins[(self.owner!).address] == true:
                    "Permission Denied"
                self.grantee == (self.owner!).address:
            ...
            DAAM.admins.remove(key: agent)
            DAAM.agents.remove(key: agent)
            log("Removed Agent")
        ...
        pub fun removeCreator(creator: Address){ 
            pre{ 
                ...
                DAAM.isAgent((self.owner!).address) == nil || (DAAM.agentHistory[(self.owner!).address]!).contains(creator):
                    "Access Denied!"
            }
            post{ 
                !DAAM.creators.containsKey(creator):
            ...
            DAAM.metadataCap.remove(key: creator)
            log("Removed Creator")
            emit CreatorRemoved(creator: creator)
        }
        
        pub fun removeMinter(minter: Address){ 
            pre{ 
            ...
            DAAM.minters.remove(key: minter)
            log("Removed Minter")
        ...
        pub fun changeCreatorStatus(creator: Address, status: Bool){ 
            ...
            (DAAM.creators[creator]!).setStatus(status)
        ...
        pub fun createMetadata(creator: Address, name: String, max: UInt64?, categories: [Categories.Category], description: String, misc: String, thumbnail:{ String:{ MetadataViews.File}}, file:{ String: MetadataViews.Media}, interact: AnyStruct?): @Metadata{ 
            ...
            let metadata <- create Metadata(creator: DAAM.creators[creator], name: name, max: max, categories: categories, description: description, misc: misc, thumbnail: thumbnail, interact: interact, file: file, metadata: nil)
            let mid = metadata.mid
            DAAM.metadata.insert(key: mid, false)
            DAAM.copyright.insert(key: mid, CopyrightStatus.UNVERIFIED)
            DAAM.metadata[mid] = true
            log("Metadata Generatated ID: ".concat(mid.toString()))
            emit AddMetadata(creator: self.grantee, mid: mid)
            return <-metadata
        }
        ...
        pub fun bargin(creator: Address, mid: UInt64, percentage: UFix64){ 
            ...
            let royalties = [MetadataViews.Royalty(receiver: getAccount(creator).getCapability<&AnyResource{FungibleToken.Receiver}>(MetadataViews.getRoyaltyReceiverPublicPath()), cut: percentage, description: "Creator Royalty")]
    ...
    pub struct CreatorInfo{ 
        ...
        access(contract) fun setStatus(_ status: Bool?){ 
            self.status = status
        }
    }
    ...
    pub resource Creator{ 
        ...
        pub fun newMetadataGenerator(): @MetadataGenerator{ 
            ...
            return <-create MetadataGenerator(self.grantee)
        }
        ...
        pub fun bargin(mid: UInt64, percentage: UFix64){ 
            ...
            let ref = &DAAM.request[mid] as &Request?
            let royalties = [MetadataViews.Royalty(receiver: getAccount(self.grantee).getCapability<&AnyResource{FungibleToken.Receiver}>(MetadataViews.getRoyaltyReceiverPublicPath()), cut: percentage, description: "Creator Royalty")]
            let royalty = MetadataViews.Royalties(royalties)
            let request <- DAAM.request.remove(key: mid)!
    ...
    pub resource Minter{ 
        ...
        init(_ minter: Address){ 
            self.grantee = minter
            DAAM.minters.insert(key: minter, true)
        }
        ...
        pub fun mintNFT(metadata: @Metadata): @DAAM.NFT{ 
            ...
            let mid = metadata.mid
            let nft <- create NFT(metadata: <-metadata, request: &DAAM.request[mid] as &Request?)
            if isLast{ 
                let request <- DAAM.request.remove(key: mid)!
                destroy request
            }
        ...
        pub fun notNew(tokenID: UInt64){ 
            ...
            var counter: UInt64 = 0 as UInt64
    ...
    pub fun answerAdminInvite(newAdmin: AuthAccount, submit: Bool): @Admin?{ 
        ...
        if !submit{ 
            DAAM.admins.remove(key: newAdminAddress)
            return nil
        }
        DAAM.admins[newAdminAddress] = submit
        log("Admin: ".concat(newAdminAddress.toString()).concat(" added to DAAM"))
        emit NewAdmin(admin: newAdminAddress)
        return <-create Admin(newAdminAddress)
    }
    ...
    pub fun answerAgentInvite(newAgent: AuthAccount, submit: Bool): @Admin{Agent}?{ 
        ...
        if !submit{ 
            DAAM.admins.remove(key: newAgentAddress)
            DAAM.agents.remove(key: newAgentAddress)
            return nil
        }
        DAAM.admins[newAgentAddress] = submit
        DAAM.agents[newAgentAddress] = submit
        DAAM.agentHistory[newAgentAddress] = []
        log("Agent: ".concat(newAgentAddress.toString()).concat(" added to DAAM"))
        emit NewAgent(agent: newAgentAddress)
        return <-(create Admin(newAgentAddress))!
    }
    ...
    pub fun answerCreatorInvite(newCreator: AuthAccount, submit: Bool): @Creator?{ 
        ...
        if !submit{ 
            DAAM.creators.remove(key: newCreatorAddress)
            return nil
        }
        (DAAM.creators[newCreatorAddress]!).setStatus(submit)
        ...
        return <-(create Creator(newCreatorAddress))!
    }
    ...
    pub fun getAgentCreators(agent: Address): [Address]?{ 
    ...
    pub fun isNFTNew(id: UInt64): Bool{ 
        return self.newNFTs.contains(id)
    }
    ...
    pub fun isAgent(_ agent: Address): Bool?{ 
        return self.agents[agent]
    }
    
    pub fun isMinter(_ minter: Address): Bool?{ 
    ...
    init(founders:{ Address: UFix64}, company: Address, defaultAdmins: [Address]){ 
        ...
        for founder in founders.keys{ 
            royalty_list.append(MetadataViews.Royalty(receiver: getAccount(founder).getCapability<&{FungibleToken.Receiver}>(MetadataViews.getRoyaltyReceiverPublicPath()), cut: founders[founder]!, description: "Founder: ".concat(founder.toString()).concat("Percentage: ").concat((founders[founder]!).toString())))
            totalCut = totalCut + founders[founder]!
        }
        self.agency = MetadataViews.Royalties(royalty_list)
        self.company = MetadataViews.Royalty(receiver: getAccount(company).getCapability<&{FungibleToken.Receiver}>(MetadataViews.getRoyaltyReceiverPublicPath()), cut: 1.0, description: "Comapny Holding")
        self.admins ={} 
        self.remove ={} 
        self.request <-{} 
        self.copyright ={} 
