...
pub contract UFC_FAN_NFT: NonFungibleToken{ 
    ...
    pub resource Series{ 
        ...
        //
        pub fun mintUFC_FAN_NFT(recipient: &{NonFungibleToken.CollectionPublic}, setId: UInt32){ 
            pre{ 
        ...
        // and deposits it in the recipients collection using their collection reference
        //
        pub fun mintEditionUFC_FAN_NFT(recipient: &{NonFungibleToken.CollectionPublic}, setId: UInt32, edition: UInt32){ 
            pre{ 
                self.numberEditionsMintedPerSet[setId] != nil:
                    "The Set does not exist."
                self.numberEditionsMintedPerSet[setId]! <= UFC_FAN_NFT.getSetMaxEditions(setId: setId)!:
        ...
        pub fun batchMintUFC_FAN_NFT(recipient: &{NonFungibleToken.CollectionPublic}, setId: UInt32, batch: UInt64){ 
            pre{ 
                batch > 0:
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                    ...
                    for metadataKey in (UFC_FAN_NFT.getSetMetadata(setId: self.setId)!).keys{ 
                        ...
                        if metadataKey.length >= 13{ 
                            if metadataKey.slice(from: 0, upTo: 13) == "royalty_addr_"{ 
                                ...
                                if cutValue != 0.0{ 
                                    royalties.append(MetadataViews.Royalty(receiver: getAccount(royaltyAddress).getCapability<&FungibleToken.Vault{FungibleToken.Receiver}>(royaltyReceiver), cut: cutValue, description: UFC_FAN_NFT.getSetMetadataByField(setId: self.setId, field: "royalty_desc_".concat(royaltyName))!))
                                }
                            }
                        }
                    }
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: UFC_FAN_NFT.CollectionStoragePath, publicPath: UFC_FAN_NFT.CollectionPublicPath, providerPath: /private/UFC_FAN_NFT, publicCollection: Type<&UFC_FAN_NFT.Collection{UFC_FAN_NFT.UFC_FAN_NFTCollectionPublic, NonFungibleToken.CollectionPublic}>(), publicLinkedType: Type<&UFC_FAN_NFT.Collection{UFC_FAN_NFT.UFC_FAN_NFTCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&UFC_FAN_NFT.Collection{UFC_FAN_NFT.UFC_FAN_NFTCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
                ...
                case Type<MetadataViews.Medias>():
                    return MetadataViews.Medias(items: [MetadataViews.Media(file: MetadataViews.HTTPFile(url: UFC_FAN_NFT.getSetMetadataByField(setId: self.setId, field: "image")!), mediaType: self.getMimeType())])
            }
            return nil
        }
    ...
    pub fun fetch(_ from: Address, id: UInt64): &UFC_FAN_NFT.NFT?{ 
        let collection = getAccount(from).getCapability(UFC_FAN_NFT.CollectionPublicPath).borrow<&UFC_FAN_NFT.Collection{UFC_FAN_NFT.UFC_FAN_NFTCollectionPublic}>() ?? panic("Couldn't get collection")
    ...
    init(){ 
        ...
        self.account.link<&UFC_FAN_NFT.Admin>(self.AdminPrivatePath, target: self.AdminStoragePath) ?? panic("Could not get a capability to the admin")
