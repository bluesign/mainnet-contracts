...
pub contract LendingOracle{ 
    ...
    pub resource OracleReaders: LendingInterfaces.OraclePublic{ 
        priv let feeds: [Address]
        
        priv let oracleAddrDict:{ Address: Address}
        ...
        pub fun latestResult(pool: Address): [UFix64; 2]{ 
            ...
            let oraclePublicInterface_ReaderRef = getAccount(oracleAddr).getCapability<&{OracleInterface.OraclePublicInterface_Reader}>(OracleConfig.OraclePublicInterface_ReaderPath).borrow() ?? panic("Lost oracle public capability at ".concat(oracleAddr.toString()))
            let priceReaderSuggestedPath = oraclePublicInterface_ReaderRef.getPriceReaderStoragePath()
            let priceReaderRef = LendingOracle.account.borrow<&OracleInterface.PriceReader>(from: priceReaderSuggestedPath) ?? panic("Lost local price reader resource.")
    ...
    pub resource Admin{ 
        ...
        pub fun addPriceFeed(
            oracleRef: &OracleReaders,
            pool: Address,
            oracleAddr: Address
        ){ 
    ...
    init(){ 
        ...
        self.account.link<&{LendingInterfaces.OraclePublic}>(
