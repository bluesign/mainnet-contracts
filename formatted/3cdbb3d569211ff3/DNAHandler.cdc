...
pub contract DNAHandler{ 
    pub resource Handler: FlowtyListingCallback.Handler{ 
        priv let recordedDNA:{ UInt64: String}
        ...
        priv fun handleCreate(listing: &{FlowtyListingCallback.Listing}, nft: &NonFungibleToken.NFT?): Bool{ 
            if nft == nil{ 
                return true
            }
        ...
        priv fun handleFilled(listing: &{FlowtyListingCallback.Listing}, nft: &NonFungibleToken.NFT): Bool{ 
        ...
        priv fun handleCompleted(listing: &{FlowtyListingCallback.Listing}, nft: &NonFungibleToken.NFT?): Bool{ 
            if let ref = nft{ 
                if let prev = self.recordedDNA[listing.uuid]{ 
        ...
        priv fun handleDestroyed(listing: &{FlowtyListingCallback.Listing}, nft: &NonFungibleToken.NFT?): Bool{ 
            self.recordedDNA.remove(key: listing.uuid)
            return true
        }
