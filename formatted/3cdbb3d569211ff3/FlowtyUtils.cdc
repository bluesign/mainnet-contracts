...
pub contract FlowtyUtils{ 
    ...
    pub resource FlowtyUtilsAdmin{ 
        pub fun setBalancePath(key: String, path: PublicPath): Bool{ 
            ...
            return (FlowtyUtils.Attributes["balancePaths"]! as! BalancePaths)
        ...
        pub fun addSupportedTokenType(tokenInfo: TokenInfo){ 
            ...
            self.setBalancePath(
                key: tokenInfo.tokenType.identifier,
                path: tokenInfo.balancePath
            )
        }
    ...
    priv fun getDepositor(): &LostAndFound.Depositor{ 
        return self.account.borrow<&LostAndFound.Depositor>(
    ...
    pub fun metadataRoyaltiesToRoyaltyCuts(
        tokenInfo: TokenInfo,
        mdRoyalties: [
    ...
    pub fun distributeRoyaltiesWithDepositor(
        royaltyCuts: [
            RoyaltyCut
        ],
    ...
    pub fun trySendFungibleTokenVault(
        vault: @FungibleToken.Vault,
    ...
    pub struct BalancePaths{ 
        priv var paths:{ String: PublicPath}
    ...
    pub fun getTokenBalance(address: Address, vaultType: Type): UFix64{ 
        ...
        assert(
            balancePath != nil,
            message: "No balance path configured for ".concat(
    ...
    pub fun getRoyaltyRate(_ nft: &NonFungibleToken.NFT): UFix64{ 
        ...
        let royalties =
            nft.resolveView(Type<MetadataViews.Royalties>())
            as!
            MetadataViews.Royalties?
        if royalties == nil{ 
            return 0.0
    ...
    init(){ 
        ...
        if self.account.borrow<&LostAndFound.Depositor>(
            from: LostAndFound.DepositorStoragePath
        )
        == nil{ 
            ...
            let depositor <-
                LostAndFound.createDepositor(
                    flowTokenReceiver,
                    lowBalanceThreshold: 10.0
                )
            self.account.save(
                <-depositor,
                to: LostAndFound.DepositorStoragePath
            )
        }
    }
}
