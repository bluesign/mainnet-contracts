...
pub contract Permitted{ 
    ...
    pub event PermittedType(
        _ type: String,
        _ permitted: Bool,
        _ message: String
    )
    
    pub event PermittedUUID(
        _ uuid: UInt64,
        _ permitted: Bool,
        _ message: String
    )
    ...
    pub resource Manager{ 
        ...
        pub fun isPermitted(_ nft: &NonFungibleToken.NFT): Bool{ 
            ...
            && (
                self.permittedUUID[nft.uuid] == nil
                || self.permittedUUID[nft.uuid]!
            )
        }
    ...
    pub fun isPermitted(_ nft: &NonFungibleToken.NFT): Bool{ 
        return (self.account.borrow<&Manager>(from: Permitted.StoragePath)!)
            .isPermitted(nft)
    }
    ...
    pub fun getReason(_ t: Type): String?{ 
        return (
            self.account.borrow<&PermitReasonManager>(
    ...
    access(account) fun getReasonManager(): &PermitReasonManager{ 
        return self.account.borrow<&PermitReasonManager>(
            from: Permitted.getReasonManagerStoragePath()
        )!
    }
