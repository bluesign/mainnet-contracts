...
pub contract NFTStorefrontV2{ 
    ...
    pub resource Listing: ListingPublic, FlowtyListingCallback.Listing{ 
        ...
        pub fun purchase(payment: @FungibleToken.Vault, commissionRecipient: Capability<&{FungibleToken.Receiver}>?, privateListingAcceptor: &Storefront{PrivateListingAcceptor}): @NonFungibleToken.NFT{ 
            pre{ 
                ...
                self.details.buyer == nil || self.details.buyer! == (privateListingAcceptor.owner!).address:
                    "incorrect buyer for private listing"
                commissionRecipient == nil || (commissionRecipient!).address == NFTStorefrontV2.account.address:
                    "invalid commission recipient"
            }
            post{ 
                Permitted.isPermitted(result):
                    "type of nft is not permitted"
            }
            ...
            let provider = self.nftProviderCapability.borrow() ?? panic("nft provider capability is invalid")
            ...
            let storeFrontPublicRef = (self.owner!).getCapability<&NFTStorefrontV2.Storefront{NFTStorefrontV2.StorefrontPublic}>(NFTStorefrontV2.StorefrontPublicPath).borrow() ?? panic("Unable to borrow the storeFrontManager resource")
            ...
            emit ListingCompleted(listingResourceID: self.uuid, storefrontResourceID: self.details.storefrontID, storefrontAddress: self.owner?.address, purchased: self.details.purchased, nftType: self.details.nftType.identifier, nftUUID: self.details.nftUUID, nftID: self.details.nftID, salePaymentVaultType: self.details.salePaymentVaultType.identifier, salePrice: self.details.salePrice, customID: self.details.customID, commissionAmount: self.details.commissionAmount, commissionReceiver: self.details.commissionAmount != 0.0 ? (commissionRecipient!).address : nil, expiry: self.details.expiry, buyer: privateListingAcceptor.owner?.address)
        ...
        destroy(){ 
            ...
            if !listingDetails.purchased{ 
                ...
                emit ListingCompleted(listingResourceID: self.uuid, storefrontResourceID: listingDetails.storefrontID, storefrontAddress: self.owner?.address, purchased: listingDetails.purchased, nftType: listingDetails.nftType.identifier, nftUUID: listingDetails.nftUUID, nftID: listingDetails.nftID, salePaymentVaultType: listingDetails.salePaymentVaultType.identifier, salePrice: listingDetails.salePrice, customID: listingDetails.customID, commissionAmount: listingDetails.commissionAmount, commissionReceiver: nil, expiry: listingDetails.expiry, buyer: nil)
            }
        }
        
        /// initializer
        ///
        init(nftProviderCapability: Capability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>, nftType: Type, nftUUID: UInt64, nftID: UInt64, salePaymentVaultType: Type, saleCuts: [SaleCut], marketplacesCapability: [Capability<&{FungibleToken.Receiver}>]?, storefrontID: UInt64, customID: String?, commissionAmount: UFix64, expiry: UInt64, buyer: Address?){ 
            // Store the sale information
            self.details = ListingDetails(nftType: nftType, nftUUID: nftUUID, nftID: nftID, salePaymentVaultType: salePaymentVaultType, saleCuts: saleCuts, storefrontID: storefrontID, customID: customID, commissionAmount: commissionAmount, expiry: expiry, buyer: buyer)
    ...
    pub resource interface StorefrontPublic{ 
        ...
        pub fun getDuplicateListingIDs(
            nftType: Type,
            nftID: UInt64,
            listingID: UInt64
        ): [
    ...
    pub resource Storefront:
        ...
        pub fun createListing(
            ...
            pre{ 
                paymentReceiver.check():
                    "payment receiver is invalid"
            }
            ...
            assert(
                price <= 10_000_000.0,
                message: "price must be less than 10 million"
            )
            let commission =
                NFTStorefrontV2.getFee(p: price, t: salePaymentVaultType)
            let marketplacesCapability =
                [NFTStorefrontV2.getCommissionReceiver(t: salePaymentVaultType)]
            ...
            assert(
                Permitted.isPermitted(nftRef),
                message: "type of nft is not permitted"
            )
            let cuts =
                NFTStorefrontV2.getPaymentCuts(
                    r: paymentReceiver,
                    n: nftRef,
                    p: price,
                    tokenType: salePaymentVaultType
                )
            ...
            self.addDuplicateListing(
                nftIdentifier: nftType.identifier,
                nftID: nftID,
        ...
        ///
        access(contract) fun addDuplicateListing(
            ...
            } else if !(self.listedNFTs[nftIdentifier]!).containsKey(nftID){ 
        ...
        ///
        access(contract) fun removeDuplicateListing(
            nftIdentifier: String,
            nftID: UInt64,
            listingResourceID: UInt64
        ){ 
            ...
            let listingIndex =
                ((self.listedNFTs[nftIdentifier]!)[nftID]!).firstIndex(
                    of: listingResourceID
                )
                ?? panic("Should contain the index")
        ...
        pub fun removeListing(listingResourceID: UInt64){ 
            ...
            self.removeDuplicateListing(
                nftIdentifier: listingDetails.nftType.identifier,
        ...
        pub fun getExistingListingIDs(nftType: Type, nftID: UInt64): [UInt64]{ 
            if self.listedNFTs[nftType.identifier] == nil
            || (self.listedNFTs[nftType.identifier]!)[nftID] == nil{ 
                return []
            }
        ...
        pub fun getDuplicateListingIDs(
            ...
            var listingIDs =
                self.getExistingListingIDs(nftType: nftType, nftID: nftID)
        ...
        pub fun cleanupExpiredListings(fromIndex: UInt64, toIndex: UInt64){ 
            pre{ 
                ...
                Int(toIndex - fromIndex) < self.getListingIDs().length:
                    "Provided range is out of bound"
            }
            var index = fromIndex
        ...
        pub fun cleanupInvalidListing(listingResourceID: UInt64){ 
            ...
            assert(
                !listing.isValid(),
                message: "listing is valid and cannot be removed"
            )
            let listingDetails = listing.getDetails()
            self.removeDuplicateListing(
                nftIdentifier: listingDetails.nftType.identifier,
            ...
            if !listingDetails.purchased{ 
                emit ListingCompleted(listingResourceID: listingResourceID, storefrontResourceID: listingDetails.storefrontID, storefrontAddress: self.owner?.address, purchased: listingDetails.purchased, nftType: listingDetails.nftType.identifier, nftUUID: listingDetails.nftUUID, nftID: listingDetails.nftID, salePaymentVaultType: listingDetails.salePaymentVaultType.identifier, salePrice: listingDetails.salePrice, customID: listingDetails.customID, commissionAmount: listingDetails.commissionAmount, commissionReceiver: nil, expiry: listingDetails.expiry, buyer: nil)
            }
            destroy listing
        }
    ...
    pub resource Admin{ 
        pub fun removeListing(addr: Address, listingResourceID: UInt64){ 
            let s =
                getAccount(addr).getCapability<&Storefront{StorefrontPublic}>(
    ...
    pub fun getCommissionReceiver(t: Type): Capability<
        &{FungibleToken.Receiver}
    >{ 
        let tokenInfo =
            FlowtyUtils.getTokenInfo(t) ?? panic("invalid token type")
        return self.account.getCapability<&{FungibleToken.Receiver}>(
            tokenInfo.receiverPath
        )
    }
    ...
    pub fun getPaymentCuts(
        r: Capability<&{FungibleToken.Receiver}>,
        n: &NonFungibleToken.NFT,
        p: UFix64,
        tokenType: Type
    ): [
        SaleCut
        ...
        let t = n.getType()
        let ti =
            FlowtyUtils.getTokenInfo(tokenType)
    ...
    init(){ 
        let pathIdentifier =
            "NFTStorefrontV2".concat(self.account.address.toString())
        let adminIdentifier =
            "NFTStorefrontV2Admin".concat(self.account.address.toString())
        ...
        NFTStorefrontV2.account.save(
            <-create Admin(),
            to: self.AdminStoragePath
        )
        if self.account.borrow<&AnyResource>(
            from: FlowtyListingCallback.ContainerStoragePath
        )
        == nil{ 
            let dnaHandler <- DNAHandler.createHandler()
            let listingHandler <-
                FlowtyListingCallback.createContainer(
                    defaultHandler: <-dnaHandler
                )
            self.account.save(
                <-listingHandler,
