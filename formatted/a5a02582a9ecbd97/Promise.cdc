pub contract Promise{ 
    priv var templates:{ UInt32: Template}
    
    priv var lastTemplateId: UInt32
    
    priv let reward: UInt64
    ...
    pub fun createProfile(name: String, termsAcceptedAt: UInt64): @Profile{ 
        let profile <-
            create Profile(name: name, termsAcceptedAt: termsAcceptedAt)
        return <-profile
    }
    ...
    pub fun createTemplate(author: Address, content: String): @NFT{ 
        ...
        let vault =
            account.getCapability<&{PublicVault}>(self.vaultPublicPath).borrow()
            ?? panic("Could not borrow Vault")
        let userProfile =
            account.getCapability<&{Promise.PublicProfile}>(
                Promise.profilePublicPath
            ).borrow()
