...
pub contract Melody{ 
    ...
    pub event TicketCached(
        paymentId: UInt64,
        ticketId: UInt64,
        receiver: Address
    )
    
    pub event TicketClaimed(
        paymentId: UInt64,
        ticketId: UInt64,
        receiver: Address
    )
    ...
    pub event PaymentRevoked(
        paymentId: UInt64,
        amount: UFix64,
        operator: Address
    )
    
    pub event PaymentStatusUpdated(
        paymentId: UInt64,
        oldStatus: UInt8,
        newStatus: UInt8
    )
    
    pub event PaymentTypeChanged(
        paymentId: UInt64,
        oldType: UInt8,
        newType: UInt8
    )
    ...
    pub event VaultWithdrawn(
        identifier: String,
        amount: UFix64,
        balance: UFix64
    )
    
    pub event PaymentRecordsUpdated(
        address: Address,
        before: [
            UInt64
        ],
    ...
    pub resource Payment{ 
        ...
        pub fun getRevocable(): Bool{ 
            return self.type == Melody.PaymentType.REVOCABLE_STREAM
            || self.type == Melody.PaymentType.REVOCABLE_VESTING
        }
        ...
        access(contract) fun getClaimable(): UFix64{ 
            ...
            var claimable = 0.0
            if self.status == PaymentStatus.COMPLETE
            || self.status == PaymentStatus.CANCELED{ 
                return 0.0
            }
            if self.type == PaymentType.STREAM
            || self.type == PaymentType.REVOCABLE_STREAM{ 
                ...
                let streamed =
                    timeDelta / (endTimestamp - startTimestamp) * amount
                claimable = streamed
            } else{ 
        ...
        // === write funcs ===
        // revoke payment
        pub fun revokePayment(
            userCertificateCap: Capability<&{Melody.IdentityCertificate}>
        ): @FungibleToken.Vault{ 
            pre{ 
                self.status != PaymentStatus.COMPLETE && self.status != PaymentStatus.CANCELED:
                    MelodyError.errorEncode(msg: "Cannot cancel close payment", err: MelodyError.ErrorCode.WRONG_LIFE_CYCLE_STATE)
                self.creator == ((userCertificateCap.borrow()!).owner!).address:
                    MelodyError.errorEncode(msg: "Only owner can revokePayment", err: MelodyError.ErrorCode.ACCESS_DENIED)
                self.type == PaymentType.REVOCABLE_STREAM || self.type == Melody.PaymentType.REVOCABLE_VESTING:
                    MelodyError.errorEncode(msg: "Only revocable payment can be revoked", err: MelodyError.ErrorCode.PAYMENT_NOT_REVOKABLE)
            }
            let balance = self.vault.balance
            self.status = PaymentStatus.CANCELED
            Melody.updateTicketMetadata(
                id: self.id,
                key: "status",
                value: PaymentStatus.CANCELED.rawValue
            )
            emit PaymentRevoked(
                paymentId: self.id,
                amount: balance,
        ...
        access(contract) fun chacheTicket(ticket: @MelodyTicket.NFT){ 
            pre{ 
                self.ticket == nil:
                    MelodyError.errorEncode(msg: "Ticket already cached", err: MelodyError.ErrorCode.ALREADY_EXIST)
            }
        ...
        access(contract) fun updateConfig(_ key: String, value: AnyStruct){ 
            pre{ 
                self.config[key] != nil:
                    MelodyError.errorEncode(msg: "Not set vaule", err: MelodyError.ErrorCode.NOT_EXIST)
            }
        ...
        access(contract) fun changeRevocable(){ 
            pre{ 
                self.status != PaymentStatus.COMPLETE && self.status != PaymentStatus.CANCELED:
                    MelodyError.errorEncode(msg: "Cannot change close payment ", err: MelodyError.ErrorCode.WRONG_LIFE_CYCLE_STATE)
            }
            ...
            assert(
                oldType != type,
                message: MelodyError.errorEncode(
                    msg: "Canot set same type",
                    err: MelodyError.ErrorCode.INVALID_PARAMETERS
                )
            )
            self.type = type
            Melody.updateTicketMetadata(
            ...
            emit PaymentTypeChanged(
                paymentId: self.id,
                oldType: oldType.rawValue,
                newType: type.rawValue
            )
        }
        ...
        access(contract) fun withdraw(_ amount: UFix64): @FungibleToken.Vault{ 
            pre{ 
                self.status != PaymentStatus.COMPLETE && self.status != PaymentStatus.CANCELED:
                    MelodyError.errorEncode(msg: "Cannot update close payment", err: MelodyError.ErrorCode.WRONG_LIFE_CYCLE_STATE)
            }
        ...
        priv fun updateStatus(){ 
            ...
            || self.type == PaymentType.REVOCABLE_STREAM{ 
                let endTimestamp = (self.config["endTimestamp"] as? UFix64?)!!
                if self.status == PaymentStatus.UPCOMING
        ...
        destroy(){ 
            pre{ 
                self.status == PaymentStatus.COMPLETE && self.status == PaymentStatus.CANCELED:
                    MelodyError.errorEncode(msg: "Cannot destroy active payment", err: MelodyError.ErrorCode.WRONG_LIFE_CYCLE_STATE)
                self.vault.balance > 0.0:
                    MelodyError.errorEncode(msg: "Please withdraw the remaining funds", err: MelodyError.ErrorCode.WRONG_LIFE_CYCLE_STATE)
            }
    ...
    pub resource Admin{ 
        ...
        priv var vaults: @{String: FungibleToken.Vault}
        
        priv let payments: @{UInt64: Payment}
        ...
        pub fun setPause(_ flag: Bool){ 
            pre{ 
                Melody.pause != flag:
                    MelodyError.errorEncode(msg: "Set pause state faild, the state is same", err: MelodyError.ErrorCode.SAME_BOOL_STATE)
            }
            Melody.pause = flag
            emit PauseStateChanged(
                pauseFlag: flag,
                operator: (self.owner!).address
            )
        }
        
        pub fun setCommision(_ commision: UFix64){ 
            emit CommisionChanged(
                before: Melody.melodyCommision,
                after: commision
            )
        ...
        pub fun setMinimumPayment(_ min: UFix64){ 
            emit MinimumPaymentChanged(
                before: Melody.minimumPayment,
                after: min
            )
        ...
        pub fun setGraceDuration(_ duration: UFix64){ 
            emit GraceDurationChanged(
                before: Melody.graceDuration,
                after: duration
            )
        ...
        pub fun getPayment(_ id: UInt64): &Payment{ 
            pre{ 
                self.payments[id] != nil:
                    MelodyError.errorEncode(msg: "Payment not found", err: MelodyError.ErrorCode.NOT_EXIST)
            }
        ...
        pub fun savePayment(_ payment: @Payment){ 
            pre{ 
                self.payments[payment.id] == nil:
                    MelodyError.errorEncode(msg: "Payment already exists", err: MelodyError.ErrorCode.ALREADY_EXIST)
            }
        ...
        pub fun withdraw(_ key: String?, amount: UFix64?): @{
    ...
    access(account) fun updateTicketMetadata(
        id: UInt64,
        key: String,
        value: AnyStruct
    ){ 
        pre{ 
            MelodyTicket.getMetadata(id) != nil:
                MelodyError.errorEncode(msg: "Ticket not found", err: MelodyError.ErrorCode.NOT_EXIST)
        }
        MelodyTicket.updateMetadata(id: id, key: key, value: value)
    }
    
    // set ticket metadata
    access(account) fun setTicketMetadata(
        id: UInt64,
        metadata:{ 
            String: AnyStruct
        }
        ...
        pre{ 
            MelodyTicket.getMetadata(id) == nil:
                MelodyError.errorEncode(msg: "Ticket already exist", err: MelodyError.ErrorCode.ALREADY_EXIST)
        }
    ...
    pub fun createStream(
        ...
        pre{ 
            vault.balance >= Melody.minimumPayment:
                MelodyError.errorEncode(msg: "Vault balance must be greater than ".concat(Melody.minimumPayment.toString()), err: MelodyError.ErrorCode.INVALID_PARAMETERS)
            self.pause == false:
                MelodyError.errorEncode(msg: "Create stream is paused", err: MelodyError.ErrorCode.PAUSED)
        }
        ...
        let recipient =
            getAccount(receiver).getCapability<
                &{NonFungibleToken.CollectionPublic}
            >(MelodyTicket.CollectionPublicPath).borrow()
        let balance = vault.balance
        let currentTimestamp = getCurrentBlock().timestamp
        let startTimestamp =
            (config["startTimestamp"] as? UFix64?)!
            ?? panic(
                MelodyError.errorEncode(
                    msg: "Start timestamp invalid",
                    err: MelodyError.ErrorCode.INVALID_PARAMETERS
                )
            )
        let endTimestamp =
            (config["endTimestamp"] as? UFix64?)!
            ?? panic(
                MelodyError.errorEncode(
                    msg: "Endt timestamp invalid",
                    err: MelodyError.ErrorCode.INVALID_PARAMETERS
                )
            )
        let transferable = (config["transferable"] as? Bool?)! ?? true
        let vaultIdentifier =
            (config["vaultIdentifier"] as? String?)!
            ?? panic(
                MelodyError.errorEncode(
                    msg: "Vault identifier invalid",
                    err: MelodyError.ErrorCode.INVALID_PARAMETERS
                )
            )
        assert(
            vaultIdentifier != "",
            message: MelodyError.errorEncode(
                msg: "Must have vaultIdentifier",
                err: MelodyError.ErrorCode.INVALID_PARAMETERS
            )
        )
        assert(
            currentTimestamp + Melody.graceDuration <= startTimestamp,
            message: MelodyError.errorEncode(
                msg: "Start time must be greater than current time, currentTimestamp: "
                    .concat(currentTimestamp.toString()).concat(
                    " startTimestamp: "
                ).concat(startTimestamp.toString()),
                err: MelodyError.ErrorCode.INVALID_PARAMETERS
            )
        )
        assert(
            endTimestamp > startTimestamp + Melody.graceDuration,
            message: MelodyError.errorEncode(
                msg: "End time must be greater than current time",
                err: MelodyError.ErrorCode.INVALID_PARAMETERS
            )
        )
        if recipient == nil{ 
        ...
        let ticketMinter =
            account.borrow<&MelodyTicket.NFTMinter>(
                from: MelodyTicket.MinterStoragePath
            )!
        let name =
            "Melody".concat(" stream ticket#").concat(paymentId.toString())
        // todo
        let metadata:{ String: AnyStruct} ={} 
    ...
    pub fun createVesting(
        ...
        pre{ 
            vault.balance > Melody.minimumPayment:
                MelodyError.errorEncode(msg: "Vault balance must be greater than 0", err: MelodyError.ErrorCode.CAN_NOT_BE_ZERO)
            self.pause == false:
                MelodyError.errorEncode(msg: "Create stream is paused", err: MelodyError.ErrorCode.PAUSED)
        }
        let account = self.account
        ...
        let cliffAmount = (config["cliffAmount"] as? UFix64?)! ?? 0.0
        let stepDuration =
            (config["stepDuration"] as? UFix64?)!
            ?? panic(
                MelodyError.errorEncode(
                    msg: "Step duration invalid",
                    err: MelodyError.ErrorCode.INVALID_PARAMETERS
                )
            )
        let steps =
            (config["steps"] as? Int8?)!
            ?? panic(
                MelodyError.errorEncode(
                    msg: "Step invalid",
                    err: MelodyError.ErrorCode.INVALID_PARAMETERS
        ...
        let stepAmount =
            (config["stepAmount"] as? UFix64?)!
            ?? panic(
                MelodyError.errorEncode(
                    msg: "Step amount invalid",
                    err: MelodyError.ErrorCode.INVALID_PARAMETERS
                )
            )
        let transferable = (config["transferable"] as? Bool?)! ?? true
        ...
        assert(
            stepAmount >= Melody.minimumPayment,
            message: MelodyError.errorEncode(
                msg: "Step amount must be greater than minimum payment",
                err: MelodyError.ErrorCode.INVALID_PARAMETERS
            )
        )
        assert(
        ...
        assert(
            balance >= totalAmount,
            message: MelodyError.errorEncode(
                msg: "Valut balance not enougth - balance: ".concat(
                    balance.toString()
                ).concat("required: ").concat(totalAmount.toString()),
                err: MelodyError.ErrorCode.INVALID_PARAMETERS
            )
        )
        assert(
            currentTimestamp + Melody.graceDuration <= startTimestamp,
            message: MelodyError.errorEncode(
                msg: "Start time must be greater than current time with grace period, currentTimestamp: "
                    .concat(currentTimestamp.toString()).concat(
                    " startTimestamp: "
                ).concat(startTimestamp.toString()),
                err: MelodyError.ErrorCode.INVALID_PARAMETERS
        ...
        assert(
            stepDuration >= Melody.graceDuration,
            message: MelodyError.errorEncode(
                msg: "Step duration must be greater than grace period",
                err: MelodyError.ErrorCode.INVALID_PARAMETERS
            )
        )
        assert(
            stepDuration % 1.0 == 0.0,
            message: MelodyError.errorEncode(
                msg: "Step duration can not be a decimal ",
                err: MelodyError.ErrorCode.INVALID_PARAMETERS
            )
        )
        if recipient == nil{ 
        ...
        let name =
            "Melody".concat(" vesting ticket#").concat(paymentId.toString())
        let metadata:{ String: AnyStruct} ={} 
        metadata["paymentInfo"] = config
        metadata["paymentType"] = type.rawValue
        metadata["paymentId"] = paymentId
    ...
    pub fun revokePayment(
        ...
        pre{ 
            (self.paymentsRecords[((userCertificateCap.borrow()!).owner!).address]!).contains(paymentId):
                MelodyError.errorEncode(msg: "Access denied when cancle payment", err: MelodyError.ErrorCode.ACCESS_DENIED)
        }
        let paymentRef =
            (self.account.borrow<&Admin>(from: self.AdminStoragePath)!)
                .getPayment(paymentId)
        assert(
            paymentRef.status != PaymentStatus.CANCELED
            || paymentRef.status != PaymentStatus.COMPLETE,
            message: MelodyError.errorEncode(
                msg: "Payment already canceled",
                err: MelodyError.ErrorCode.WRONG_LIFE_CYCLE_STATE
            )
        )
        assert(
            paymentRef.type != PaymentType.VESTING
            && paymentRef.type != PaymentType.STREAM,
            message: MelodyError.errorEncode(
                msg: "Cannot cancel non-revoked payment",
                err: MelodyError.ErrorCode.WRONG_LIFE_CYCLE_STATE
            )
        )
        return <-paymentRef.revokePayment(
            userCertificateCap: userCertificateCap
        )
    }
    
    // change payment revocable to non-revocable
    pub fun changeRevocable(
        userCertificateCap: Capability<&{Melody.IdentityCertificate}>,
        paymentId: UInt64
    ){ 
        pre{ 
            (self.paymentsRecords[((userCertificateCap.borrow()!).owner!).address]!).contains(paymentId):
                MelodyError.errorEncode(msg: "Access denied when update payment info", err: MelodyError.ErrorCode.ACCESS_DENIED)
        }
        let paymentRef =
            (self.account.borrow<&Admin>(from: self.AdminStoragePath)!)
                .getPayment(paymentId)
        assert(
            ...
            || paymentRef.status != PaymentStatus.COMPLETE,
            message: MelodyError.errorEncode(
                msg: "Cannot change revocable with canceled payment",
                err: MelodyError.ErrorCode.WRONG_LIFE_CYCLE_STATE
            )
        )
        assert(
            ...
            && paymentRef.type != PaymentType.STREAM,
            message: MelodyError.errorEncode(
                msg: "Cannot change revocable with non-revoked payment",
                err: MelodyError.ErrorCode.WRONG_LIFE_CYCLE_STATE
            )
        )
    ...
    pub fun changeTransferable(
        ...
        let paymentRef =
            (self.account.borrow<&Admin>(from: self.AdminStoragePath)!)
                .getPayment(paymentId)
        let transferable =
            (paymentRef.config["transferable"] as? Bool?)! ?? true
        assert(
            paymentRef.status != PaymentStatus.CANCELED
            || paymentRef.status != PaymentStatus.COMPLETE,
            message: MelodyError.errorEncode(
                msg: "Cannot change transferable with canceled payment",
                err: MelodyError.ErrorCode.WRONG_LIFE_CYCLE_STATE
            )
        )
        assert(
            transferable == false,
            message: MelodyError.errorEncode(
                msg: "Only allow non-transferable to transferable",
                err: MelodyError.ErrorCode.WRONG_LIFE_CYCLE_STATE
            )
        )
    ...
    pub fun claimTicket(
        ...
        pre{ 
            (self.getUserTicketRecords(((userCertificateCap.borrow()!).owner!).address)!).contains(paymentId):
                MelodyError.errorEncode(msg: "Access denied when claim ticket", err: MelodyError.ErrorCode.ACCESS_DENIED)
        }
        let paymentRef =
            (self.account.borrow<&Admin>(from: self.AdminStoragePath)!)
                .getPayment(paymentId)
        assert(
            paymentRef.status != PaymentStatus.CANCELED,
            message: MelodyError.errorEncode(
                msg: "Cannot claim ticket from canceled payment",
                err: MelodyError.ErrorCode.WRONG_LIFE_CYCLE_STATE
            )
        )
        ...
        assert(
            recipient == ((userCertificateCap.borrow()!).owner!).address,
            message: MelodyError.errorEncode(
                msg: "Cannot claim ticket from wrong receiver",
                err: MelodyError.ErrorCode.ACCESS_DENIED
            )
        )
        self.updateUserTicketsRecord(
            address: recipient!,
            id: paymentRef.id,
            isDelete: true
        )
    ...
    pub fun withdraw(
        userCertificateCap: Capability<&{Melody.IdentityCertificate}>,
        ticket: &MelodyTicket.NFT
    ): @FungibleToken.Vault{ 
        pre{ 
            ((userCertificateCap.borrow()!).owner!).address == (ticket.owner!).address:
                MelodyError.errorEncode(msg: "Withdraw ", err: MelodyError.ErrorCode.ACCESS_DENIED)
        }
        // todo
        let paymentRef =
            (self.account.borrow<&Admin>(from: self.AdminStoragePath)!)
        ...
        if paymentRef.type == PaymentType.VESTING
        || paymentRef.type == PaymentType.REVOCABLE_VESTING{ 
    ...
    access(contract) fun withdrawStream(
        ticket: &MelodyTicket.NFT
    ): @FungibleToken.Vault{ 
        let paymentRef =
            (self.account.borrow<&Admin>(from: self.AdminStoragePath)!)
        ...
        let config = paymentRef.config
        let vaultRef = &paymentRef.vault as                                            
                                            // commision cut
                                            &FungibleToken.Vault
        let withdrawn                      
                      // vesting payment withdraw
                      = paymentRef.withdrawn
        assert(
            paymentType == PaymentType.STREAM
            || paymentType == PaymentType.REVOCABLE_STREAM,
            message: MelodyError.errorEncode(
                msg: "Can only withdraw from stream payment",
                err: MelodyError.ErrorCode.TYPE_MISMATCH
            )
        )
        assert(
            ...
            message: MelodyError.errorEncode(
                msg: "Can withdraw with wrong status",
                err: MelodyError.ErrorCode.WRONG_LIFE_CYCLE_STATE
            )
        )
        let currentTimestamp = getCurrentBlock().timestamp
        ...
        assert(
            ...
            message: MelodyError.errorEncode(
                msg:                     
                     ...
                     "Can not withdraw before start",
                err: MelodyError.ErrorCode.WRONG_LIFE_CYCLE_STATE
            )
        )
        let streamed = paymentRef.getClaimable()
        let canClaimAmount = streamed                                      
        ...
        assert(
            streamed >= withdrawn,
            message: MelodyError.errorEncode(
                msg: "Steamed amount must greater than withdraw amount",
                err: MelodyError.ErrorCode.WRONG_LIFE_CYCLE_STATE
            )
        )
        assert(
            canClaimAmount <= vaultBalance,
            message: MelodyError.errorEncode(
                msg: "Withdraw amount must lower than vault balance",
                err: MelodyError.ErrorCode.WRONG_LIFE_CYCLE_STATE
            )
        )
        let withdrawVault <- paymentRef.withdraw(canClaimAmount)
        self.cutCommision(
            &withdrawVault as &FungibleToken.Vault,
            paymentId: paymentRef.id
        )
        emit PaymentWithdrawn(
            paymentId: paymentRef.id,
            type: paymentType.rawValue,
            status: paymentStatus.rawValue,
            amount: canClaimAmount
        )
        return <-withdrawVault
    }
    
    access(contract) fun withdrawVesting(
        ticket: &MelodyTicket.NFT
    ): @FungibleToken.Vault{ 
        let paymentRef =
            (self.account.borrow<&Admin>(from: self.AdminStoragePath)!)
        ...
        assert(
            paymentType == PaymentType.VESTING
            || paymentType == PaymentType.REVOCABLE_VESTING,
            message: MelodyError.errorEncode(
                msg: "Can only withdraw from vesting payment",
                err: MelodyError.ErrorCode.TYPE_MISMATCH
            )
        )
        assert(
        ...
        assert(
            currentTimestamp >= startTimestamp,
            message: MelodyError.errorEncode(
                msg: "Can withdraw before start".concat(
        ...
        assert(
            claimable >= withdrawn,
            message: MelodyError.errorEncode(
                msg: "Vesting claimable amount must greater than withdraw amount",
                err: MelodyError.ErrorCode.WRONG_LIFE_CYCLE_STATE
            )
        )
        assert(
            canClaimAmount > 0.0,
            message: MelodyError.errorEncode(
                msg: "No amount can claim",
                err: MelodyError.ErrorCode.CAN_NOT_BE_ZERO
            )
        )
        let withdrawVault <- paymentRef.withdraw(canClaimAmount)
        self.cutCommision(
    ...
    access(contract) fun cutCommision(
        _ vaultRef: &FungibleToken.Vault,
        paymentId: UInt64
    ){ 
    ...
    access(contract) fun updateUserTicketsRecord(
        address: Address,
        id: UInt64,
        isDelete: Bool
    ){ 
        if isDelete == true
        && (Melody.userTicketRecords[address]!).contains(id) == false{ 
            panic(
                MelodyError.errorEncode(
                    msg: "Delete failed: record not existed",
                    err: MelodyError.ErrorCode.INVALID_PARAMETERS
                )
            )
        }
        ...
        emit TicketRecordChanged(
            address: address,
            before: userTicketRecords,
            after: newRecords
        )
    }
    ...
    init(){ 
        ...
        account.save(
            ...
            to: self.UserCertificateStoragePath
        )
    }
}
