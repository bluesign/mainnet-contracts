...
pub contract Ezy3dItems: NonFungibleToken{ 
    ...
    pub event Minted(id: UInt64, TaskId: String, TaskName: String, ImgId: String, TaskFrame: String, TaskHolder: String, TaskDetail: String)
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                case Type<MetadataViews.Display>():
                    return MetadataViews.Display(name: self.TaskName, description: self.description(), taskId: self.TaskId, thumbnail: self.ImgId)
            }
            return nil
        }
    }
    ...
    pub resource NFTMinter{ 
        ...
        //
        pub fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, TaskId: String, TaskName: String, ImgId: String, taskFrame: String, taskHolder: String, taskDetail: String){ 
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &Ezy3dItems.NFT?{ 
        let collection = (getAccount(from).getCapability(Ezy3dItems.CollectionPublicPath)!).borrow<&Ezy3dItems.Collection{Ezy3dItems.Ezy3dItemsCollectionPublic}>() ?? panic("Couldn't get collection")
