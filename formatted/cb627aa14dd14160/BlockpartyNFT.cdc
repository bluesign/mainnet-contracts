...
pub contract BlockpartyNFT: NonFungibleToken{ 
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            ...
            switch view{ 
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: BlockpartyNFT.BNFTCollectionStoragePath, publicPath: BlockpartyNFT.BNFTCollectionPublicPath, providerPath: /private/NFTCollection, publicCollection: Type<&BlockpartyNFT.Collection{BlockpartyNFT.BNFTCollectionPublic}>(), publicLinkedType: Type<&BlockpartyNFT.Collection{BlockpartyNFT.BNFTCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&BlockpartyNFT.Collection{BlockpartyNFT.BNFTCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Provider, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
    ...
    pub resource NFTMinter{ 
        priv var BNFTCollectionPublicPath: PublicPath
        
        init(collectionPublicPath: PublicPath){ 
            self.BNFTCollectionPublicPath = collectionPublicPath
        }
        
        pub fun mintNFTByIssuance(requests: [IssuanceMintMsg], metadatas: [{String: String}]){ 
            let minterOwner = self.owner ?? panic("could not get minter owner")
            let minterOwnerCollection = minterOwner.getCapability(self.BNFTCollectionPublicPath).borrow<&{NonFungibleToken.CollectionPublic}>() ?? panic("Could not get reference to the service account's NFT Collection")
        ...
        pub fun mintNFT(withdrawRequestID: UInt64, detailsURL: String, metadataMap:{ String: String}, receiver: Address){ 
            // Borrow the recipient's public NFT collection reference
            let recipientAccount = getAccount(receiver)
            let recipientCollection = recipientAccount.getCapability(self.BNFTCollectionPublicPath).borrow<&{NonFungibleToken.CollectionPublic}>() ?? panic("Could not get receiver reference to the NFT Collection")
