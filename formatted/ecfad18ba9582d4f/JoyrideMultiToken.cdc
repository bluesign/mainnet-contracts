...
pub contract JoyrideMultiToken{ 
    ...
    access(contract) fun getPlatformBalance(
        vault: Vaults,
        tokenContext: String
    ): UFix64{ 
        return self.account.borrow<&Vault>(
            from: self.TokenStoragePaths[vault.rawValue]!
        )?.balanceOf(tokenContext: tokenContext)
        ?? 0.0
    }
    
    access(contract) fun doPlatformWithdraw(
        vault: Vaults,
        tokenContext: String,
        amount: UFix64
    ): @FungibleToken.Vault?{ 
        let path = self.TokenStoragePaths[vault.rawValue]
        emit JoyrideMultiTokenInfoEvent(
            notes: "vault Index".concat(vault.rawValue.toString())
        )
        if path == nil{ 
            return nil
        }
        ...
        return <-(vault!).withdrawToken(
            tokenContext: tokenContext,
            amount: amount
        )
    }
    
    access(contract) fun doPlatformDeposit(
        vault: Vaults,
        from: @FungibleToken.Vault
    ){ 
        (
            self.account.borrow<&Vault>(
                from: self.TokenStoragePaths[vault.rawValue]!
            )!
    ...
    pub resource PlatformAdmin:
        PlatformBalance,
        PlatformWithdraw,
        PlatformDeposit{
    
        pub fun balance(vault: Vaults, tokenContext: String): UFix64{ 
            return JoyrideMultiToken.getPlatformBalance(
                vault: vault,
                tokenContext: tokenContext
            )
        }
        ...
        pub fun withdraw(
            ...
            return <-JoyrideMultiToken.doPlatformWithdraw(
                vault: vault,
                tokenContext: tokenContext,
                amount: amount
            )
    ...
    init(){ 
        ...
        self.TokenPublicPaths[
            Vaults.treasury.rawValue
        ] = /public/JoyrideMultiToken_PlatformTreasury
        self.TokenStoragePaths[
            Vaults.reserve.rawValue
        ] = /storage/JoyrideMultiToken_PlatformReserve
        self.TokenPublicPaths[
            Vaults.reserve.rawValue
        ] = /public/JoyrideMultiToken_PlatformReserve
        ...
        self.account.save(
            <-treasury,
            to: self.TokenStoragePaths[Vaults.treasury.rawValue]!
        )
        ...
        self.account.save(
            <-reserve,
            to: self.TokenStoragePaths[Vaults.reserve.rawValue]!
        )
        self.account.link<&{FungibleToken.Receiver, FungibleToken.Balance}>(
        ...
        self.account.save(
            <-platformAdmin,
            to: /storage/JoyrideMultiToken_PlatformAdmin
        )
