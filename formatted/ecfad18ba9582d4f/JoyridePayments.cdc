...
pub contract JoyridePayments{ 
    access(contract) var accountsCapability: Capability<
        &{
            JoyrideAccounts.GrantsTokenRewards,
            JoyrideAccounts.SharesProfits,
            JoyrideAccounts.PlayerAccounts
        }
    >?
    
    access(contract) var treasuryCapability: Capability<
        &JoyrideMultiToken.PlatformAdmin
    >?
    ...
    pub event DebitTransactionCreate(
        playerID: String,
        txID: String,
        tokenContext: String,
        amount: UFix64,
    ...
    pub event CreditTransactionCreate(
        playerID: String,
        txID: String,
        tokenContext: String,
        amount: UFix64,
    ...
    pub event DebitTransactionReverted(
        playerID: String,
        txID: String,
        tokenContext: String,
        amount: UFix64,
    ...
    pub event DebitTransactionFinalized(
        playerID: String,
        txID: String,
        tokenContext: String,
        amount: UFix64,
    ...
    pub event TxFailed_ByTxTypeAndTxID(
        txID: String,
        txType: String,
        notes: String
    )
    ...
    init(){ 
        ...
        self.account.link<&PaymentsAdmin{WalletAdmin}>(
            /private/PaymentsAdmin,
            target: /storage/PaymentsAdmin
        )
    }
    ...
    pub fun linkAccountsCapability(
        accountsCapability: Capability<
            &JoyrideAccounts.JoyrideAccountsAdmin{
                JoyrideAccounts.GrantsTokenRewards,
                JoyrideAccounts.SharesProfits,
                JoyrideAccounts.PlayerAccounts
            }
    ...
    //Pretty sure this is safe to be public, since a valid Capability<&JRXToken.AdminVault.{JRXToken.ConvertsRewards}> can only be created by the JRXToken contract account.
    pub fun linkTreasuryCapability(
        treasuryCapability: Capability<&JoyrideMultiToken.PlatformAdmin>
    ){ 
    ...
    pub fun getPlay2EarnCapabilities(account: AuthAccount): Capability<
        &PaymentsAdmin{WalletAdmin}
    >{ 
        if account.address == self.account.address
        || self.authorizedBackendAccounts.containsKey(account.address){ 
            return self.account.getCapability<&PaymentsAdmin{WalletAdmin}>(
                /private/PaymentsAdmin
            )
        }
    ...
    pub resource interface WalletAdmin{ 
        ...
        pub fun FinalizeTransactionWithDevPercentage(
            txID: String,
            profit: UFix64,
    ...
    pub resource Transaction{ 
        ...
        init(
            ...
            gameEscrowMarker.depositToEscrowVault(
                gameID: gameID,
                vault: <-vault
            )
        ...
        priv fun doFinalize(
            ...
            let treasury = (JoyridePayments.treasuryCapability!).borrow()
            (treasury!).deposit(
                vault: JoyrideMultiToken.Vaults.treasury,
                from: <-vault
            )
    ...
    pub resource PaymentsAdmin: WalletAdmin{ 
        priv let pendingTransactions: @{String: Transaction}
