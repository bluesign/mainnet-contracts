...
pub contract NFTLendingPlace{ 
    ...
    pub resource interface LendingPublic{ 
        ...
        pub fun forcedRedeem(
            uuid: UInt64,
            lendticket: &LenderTicket
        ): @NonFungibleToken.NFT{} 
    ...
    pub resource LendingCollection: LendingPublic, LendingManager{ 
        ...
        pub fun repay(uuid: UInt64, repayAmount: @FlowToken.Vault): @NonFungibleToken.NFT{ 
            ...
            if let vaultRef = getAccount(self.lenders[uuid]!).getCapability(/public/flowTokenReceiver).borrow<&FlowToken.Vault{FungibleToken.Receiver}>(){ 
                vaultRef.deposit(from: <-repayAmount)
            } else{ 
                let vaultRef = getAccount(0xd5613003fe383df9).getCapability(/public/flowTokenReceiver).borrow<&FlowToken.Vault{FungibleToken.Receiver}>() ?? panic("Could not borrow reference to admin token vault")
                vaultRef.deposit(from: <-repayAmount)
            }
            self.lenders[uuid] = nil
            self.beginningTime[uuid] = nil
    ...
    pub fun createLendingCollection(
