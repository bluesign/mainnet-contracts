...
pub contract FindRulesCache{ 
    ...
    // Tenant Find Rules
    access(account) fun setTenantFindRulesCache(
        tenant: String,
        ruleId: String,
        result: ActionResult
    ){ 
        ...
        panic(
            "There is already a cache for this find rule. RuleId : ".concat(
                ruleId
            )
        )
    }
    ...
    // Tenant Tenant Rules
    access(account) fun setTenantTenantRulesCache(
        tenant: String,
        ruleId: String,
        result: ActionResult
    ){ 
        ...
        panic(
            "There is already a cache for this tenant rule. RuleId : ".concat(
                ruleId
            )
        )
    }
    
    pub fun getTenantTenantRules(
        tenant: String,
        ruleId: String
    ): ActionResult?{ 
        if self.tenantTenantRules[tenant] == nil{ 
    ...
    access(account) fun setTenantCutCache(
        tenant: String,
        ruleId: String,
        cut: TenantCuts
    ){ 
        ...
        panic(
            "There is already a cache for this tenant cut information. RuleId : "
                .concat(ruleId)
        )
    }
