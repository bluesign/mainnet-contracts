...
pub contract FindMarketCut{ 
    ...
    pub fun getRuleId(listingType: Type, nftType: Type, ftType: Type): String{ 
        let s: [String] =
            [listingType.identifier, nftType.identifier, ftType.identifier]
    ...
    access(contract) fun borrowContract(
        ...
        return self.account.contracts.borrow<&FindMarketCutInterface>(
            name: identifier
        )
        ?? panic("Cannor borrow contract with identifier : ".concat(identifier))
    }
    
    access(account) fun setTenantCuts(
        tenant: String,
        types: [
            Type
        ],
        ...
        let contractName =
            ...
            ?? panic(
                "Category is not set to link with contract. Category ".concat(
                    category
                )
            )
        let con = self.borrowContract(contractName)
    ...
    access(account) fun removeTenantCuts(
        tenant: String,
        types: [
            Type
        ],
    ...
    access(account) fun setTenantRulesCache(
        tenant: String,
        ruleId: String,
        category: String,
    ...
    access(account) fun resetTenantRulesCache(
        tenant: String,
        category: String
    ){ 
        let contractName =
