...
pub contract FindLeaseMarket{ 
    ...
    /* Get Sale Reports and Sale Item */    pub fun assertOperationValid(
        ...
        let address =
            FIND.lookupAddress(name)
            ?? panic("Name is not owned by anyone. Name : ".concat(name))
        let collectionCap =
    ...
    pub fun getSaleItems(tenant: Address, name: String, getLeaseInfo: Bool):{ 
        String: FindLeaseMarket.SaleItemCollectionReport
    }{ 
        let address =
            FIND.lookupAddress(name)
    ...
    access(contract) fun checkSaleInformation(
        ...
        return FindLeaseMarket.SaleItemCollectionReport(
            items: info,
            ghosts: ghost
        )
    }
    ...
    access(contract) fun checkBidInformation(
        ...
        return FindLeaseMarket.BidItemCollectionReport(
            items: info,
            ghosts: ghost
        )
    }
    ...
    pub struct interface LeasePointer{ 
        ...
        access(contract) fun borrow(): &FIND.LeaseCollection{
            FIND.LeaseCollectionPublic
        }{} 
    }
    
    pub struct ReadLeasePointer: LeasePointer{ 
        priv let cap: Capability<&FIND.LeaseCollection{FIND.LeaseCollectionPublic}>
    ...
    access(account) fun pay(
        ...
        let oldProfile =
            FindMarket.getPaymentWallet(
                oldProfileCap,
                ftInfo,
                panicOnFailCheck: true
            )
    ...
    access(contract) fun getNetwork(): &FIND.Network{ 
        return FindLeaseMarket.account.borrow<&FIND.Network>(
            from: FIND.NetworkStoragePath
        )
        ?? panic("Network is not up")
    }
    
    /* Admin Function */    access(account) fun addSaleItemType(_ type: Type){ 
