...
pub contract FindThoughts{ 
    ...
    pub struct ThoughtPointer{ 
        pub let cap: Capability<
            &FindThoughts.Collection{FindThoughts.CollectionPublic}
        >
        ...
        init(creator: Address, id: UInt64){ 
            let cap =
                getAccount(creator).getCapability<
                    &FindThoughts.Collection{FindThoughts.CollectionPublic}
    ...
    pub resource Thought: ThoughtPublic, MetadataViews.Resolver{ 
        ...
        priv let stringTags:{ String: String}
        
        priv let scalars:{ String: UFix64}
        
        priv let extras:{ String: AnyStruct}
    ...
    pub resource interface CollectionPublic{ 
        ...
        pub fun borrowThoughtPublic(_ id: UInt64): &FindThoughts.Thought{
            FindThoughts.ThoughtPublic
        }{} 
    }
    ...
    pub resource Collection:
        ...
        priv let ownedThoughts: @{UInt64: FindThoughts.Thought}
        
        priv let sequence: [UInt64]
        ...
        pub fun publish(
            ...
            media: MetadataViews.Media?,
            nftPointer: FindViews.ViewReadPointer?,
            quote: FindThoughts.ThoughtPointer?
        ){ 
        ...
        pub fun react(user: Address, id: UInt64, reaction: String?){ 
            ...
            let ref =
                cap.borrow()
                ?? panic(
                    "Cannot borrow reference to Find Thoughts Collection from user : "
                        .concat(user.toString())
                )
            let thought = ref.borrowThoughtPublic(id)
            thought.internal_react(
    ...
    pub fun getFindThoughtsCapability(_ user: Address): Capability<
        &FindThoughts.Collection{FindThoughts.CollectionPublic}
    >{ 
