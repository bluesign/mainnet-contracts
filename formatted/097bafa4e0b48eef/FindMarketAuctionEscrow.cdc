...
pub contract FindMarketAuctionEscrow{ 
    ...
    pub resource interface SaleItemCollectionPublic{ 
        ...
        access(contract) fun registerBid(
            item: FindViews.ViewReadPointer,
            callback: Capability<
                &MarketBidCollection{MarketBidCollectionPublic}
            >,
            vaultType: Type
        ){} 
    ...
    pub resource SaleItemCollection:
        SaleItemCollectionPublic,
        FindMarket.SaleItemCollectionPublic{
    
        //is this the best approach now or just put the NFT inside the saleItem?
        access(contract) var items: @{UInt64: SaleItem}
        
        access(contract) let tenantCapability: Capability<
            &FindMarket.Tenant{FindMarket.TenantPublic}
        >
        
        init(
            _ tenantCapability: Capability<
                &FindMarket.Tenant{FindMarket.TenantPublic}
            >
        ){ 
            self.items <-{} 
        ...
        priv fun getTenant(): &FindMarket.Tenant{FindMarket.TenantPublic}{ 
        ...
        priv fun addBid(
            ...
            let actionResult =
                tenant.allowedAction(
                    listingType: Type<@FindMarketAuctionEscrow.SaleItem>(),
                    nftType: nftType,
                    ftType: ftType,
                    action: FindMarket.MarketAction(
                        listing: false,
                        name: "add bid in auction"
                    ),
                    seller: (self.owner!).address,
                    buyer: newOffer.address
                )
            ...
            let newOfferBalance =
                newOffer.borrow()?.getBalance(id)
                ?? panic("The new offer bid capability is invalid.")
            let previousOffer = saleItem.offerCallback
            var minBid = oldBalance + saleItem.auctionMinBidIncrement
            if previousOffer != nil
            && newOffer.address != (previousOffer!).address{ 
                minBid = ((previousOffer!).borrow()!).getBalance(id)
                    + saleItem.auctionMinBidIncrement
            }
            ...
            let suggestedEndTime =
                timestamp + saleItem.auctionExtensionOnLateBid
            ...
            emit EnglishAuction(
                ...
                seller: seller,
                sellerName: FIND.reverseLookup(seller),
                amount: newOfferBalance,
                auctionReservePrice: saleItem.auctionReservePrice,
                status: status,
                vaultType: saleItem.vaultType.identifier,
                nft: nftInfo,
                buyer: buyer,
                buyerName: buyerName,
                buyerAvatar: profile.getAvatar(),
                startsAt: saleItem.auctionStartedAt,
                endsAt: saleItem.auctionEndsAt,
        ...
        access(contract) fun registerIncreasedBid(
            ...
            self.addBid(
                id: id,
                newOffer: saleItem.offerCallback!,
        ...
        access(contract) fun registerBid(
            ...
            let balance =
                callback.borrow()?.getBalance(id)
                ?? panic(
                    "Bidder unlinked bid collection capability. bidder address : "
                        .concat(callback.address.toString())
                )
        ...
        pub fun relist(_ id: UInt64){ 
            ...
            self.listForAuction(
                ...
                auctionExtensionOnLateBid: auctionExtensionOnLateBid,
                minimumBidIncrement: minimumBidIncrement,
                auctionStartTime: auctionStartedAt,
                auctionValidUntil: auctionValidUntil,
                saleItemExtraField: saleItemExtraField
            )
        }
        
        priv fun internalCancelAuction(saleItem: &SaleItem, status: String){ 
            let status = status
            let ftType = saleItem.getFtType()
        ...
        pub fun listForAuction(
            ...
            let saleItem <-
                create SaleItem(
                    ...
                    auctionReservePrice: auctionReservePrice,
                    auctionDuration: auctionDuration,
                    extentionOnLateBid: auctionExtensionOnLateBid,
                    minimumBidIncrement: minimumBidIncrement,
                    auctionValidUntil: auctionValidUntil,
                    saleItemExtraField: saleItemExtraField
                )
            ...
            emit EnglishAuction(
                tenant: tenant.name,
                id: id,
                saleID: saleItemRef.uuid,
                seller: seller,
                sellerName: FIND.reverseLookup(seller),
                amount: balance,
                auctionReservePrice: saleItemRef.auctionReservePrice,
                status: status,
                ...
                buyerName: nil,
                buyerAvatar: nil,
                startsAt: saleItemRef.auctionStartedAt,
                endsAt: saleItemRef.auctionEndsAt,
                previousBuyer: nil,
                previousBuyerName: nil
            )
        }
    ...
    pub resource MarketBidCollection:
        MarketBidCollectionPublic,
        FindMarket.MarketBidCollectionPublic{
        ...
        pub fun bid(
            ...
            let tenant = self.getTenant()
            let from =
                getAccount(item.owner()).getCapability<
                    &SaleItemCollection{SaleItemCollectionPublic}
                >(tenant.getPublicPath(Type<@SaleItemCollection>()))
            ...
            let saleItemCollection =
                from.borrow()
                ?? panic(
                    "Could not borrow sale item for id=".concat(uuid.toString())
                )
            let callbackCapability =
                (self.owner!).getCapability<
                    &MarketBidCollection{MarketBidCollectionPublic}
                >(tenant.getPublicPath(Type<@MarketBidCollection>()))
        ...
        pub fun increaseBid(id: UInt64, vault: @FungibleToken.Vault){ 
            ...
            (bid.from.borrow()!).registerIncreasedBid(
                id,
                oldBalance: oldBalance
            )
        }
        ...
        access(contract) fun cancelBidFromSaleItem(_ id: UInt64){ 
            ...
            (self.receiver.borrow()!).deposit(
                from: <-vaultRef.withdraw(amount: vaultRef.balance)
            )
            destroy bid
        }
    ...
    pub fun createEmptySaleItemCollection(
        _ tenantCapability: Capability<
            &FindMarket.Tenant{FindMarket.TenantPublic}
        >
    ): @SaleItemCollection{ 
    ...
    pub fun createEmptyMarketBidCollection(
        receiver: Capability<&{FungibleToken.Receiver}>,
        tenantCapability: Capability<
            &FindMarket.Tenant{FindMarket.TenantPublic}
        >
    ): @MarketBidCollection{ 
        return <-create MarketBidCollection(
            receiver: receiver,
            tenantCapability: tenantCapability
        )
    }
    
    pub fun getSaleItemCapability(
        marketplace: Address,
        user: Address
    ): Capability<
        &SaleItemCollection{
    ...
    >?{ 
        ...
        if let tenant =
            (FindMarket.getTenantCapability(marketplace)!).borrow(){ 
            return getAccount(user).getCapability<
                &SaleItemCollection{
                    SaleItemCollectionPublic,
                    FindMarket.SaleItemCollectionPublic
                }
    ...
    pub fun getBidCapability(marketplace: Address, user: Address): Capability<
        &MarketBidCollection{
            MarketBidCollectionPublic,
            FindMarket.MarketBidCollectionPublic
        }
