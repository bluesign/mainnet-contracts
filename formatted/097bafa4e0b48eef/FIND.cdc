...
pub contract FIND{ 
    ...
    pub fun getLeases(): [NetworkLease]{ 
        if let network =
            self.account.borrow<&Network>(from: FIND.NetworkStoragePath){ 
    ...
    pub fun depositWithTagAndMessage(
        to: String,
        message: String,
        tag: String,
    ...
    pub resource Lease{ 
        ...
        access(contract) var offerCallback: Capability<
            &BidCollection{BidCollectionPublic}
        >?
        ...
        pub fun setCallback(
            _ callback: Capability<&BidCollection{BidCollectionPublic}>?
        ){ 
        ...
        pub fun extendLease(_ vault: @FUSD.Vault){ 
            let network =
                self.networkCap.borrow() ?? panic("The network is not up")
            network.renew(name: self.name, vault: <-vault)
        }
        
        pub fun extendLeaseDapper(
        ...
        access(contract) fun move(profile: Capability<&{Profile.Public}>){ 
            ...
            // receiver
            let receiver =
                profile.borrow() ?? panic("The profile capability is invalid")
    ...
    /* An Auction for a lease */    pub resource Auction{ 
        ...
        access(contract) var latestBidCallback: Capability<
            &BidCollection{BidCollectionPublic}
        >
        ...
        pub fun getBalance(): UFix64{ 
            let cb =
                ...
                ?? panic(
                    "The bidder has unlinked the capability. bidder address: "
                        .concat(self.latestBidCallback.address.toString())
                )
            return cb.getBalance(self.name)
        }
        
        pub fun addBid(
            callback: Capability<&BidCollection{BidCollectionPublic}>,
            timestamp: UFix64,
            lease: &Lease
        ){ 
            ...
            let profile =
                getAccount(bidder).getCapability<&{Profile.Public}>(
                    Profile.publicPath
                ).borrow()
            if profile == nil{ 
    ...
    pub resource interface LeaseCollectionPublic{ 
        ...
        access(account) fun move(
            name: String,
            profile: Capability<&{Profile.Public}>,
            to: Capability<&LeaseCollection{LeaseCollectionPublic}>
        ){} 
    ...
    pub fun createEmptyLeaseCollection(): @FIND.LeaseCollection{ 
        if let network =
            ...
            return <-create LeaseCollection(
                networkCut: network.secondaryCut,
                networkWallet: network.wallet
            )
        }
        panic("Network is not set up")
    }
    ...
    pub resource Network{ 
        ...
        init(
            ...
            self.addonPrices ={ "forge": 50.0, // will have to run transactions on this when update on mainnet. "premiumForge": 1000.0}
        ...
        access(contract) fun renewDapper(
            merchAccount: Address,
            name: String,
            vault: @DapperUtilityCoin.Vault
        ){ 
        ...
        pub fun registerDapper(
            ...
            let wallet =
                getAccount(merchAccount).getCapability<
                    &{FungibleToken.Receiver}
                >(/public/dapperUtilityCoinReceiver)
            let walletRef =
                wallet.borrow()
                ?? panic(
                    "Cannot borrow reference to Dapper Merch Account receiver. Address : "
                        .concat(merchAccount.toString())
                )
            walletRef.deposit(from: <-vault)
            self.internal_register(name: name, profile: profile, leases: leases)
        }
        
        access(account) fun internal_register(
            name: String,
            profile: Capability<&{Profile.Public}>,
            leases: Capability<&LeaseCollection{LeaseCollectionPublic}>
        ){ 
            ...
            let lease =
                NetworkLease(
                    ...
                    lockedUntil: Clock.time() + self.leasePeriod
                    + self.lockPeriod,
                    profile: profile,
                    name: name
                )
            ...
            (leases.borrow()!).deposit(
                token: <-create Lease(
                    name: name,
                    networkCap: FIND.account.getCapability<&Network>(
                        FIND.NetworkPrivatePath
                    )
                )
            )
        }
    ...
    pub resource Bid{ 
        access(contract) let from: Capability<
    ...
    pub resource interface BidCollectionPublic{ 
        ...
        access(contract) fun fulfillLease(
            _ token: @FIND.Lease
        ): @FungibleToken.Vault{} 
    ...
    pub fun createEmptyBidCollection(
    ...
    init(){ 
        ...
        let wallet =
            self.account.getCapability<&{FungibleToken.Receiver}>(
                /public/fusdReceiver
            )
        ...
        self.account.link<&Network>(
            FIND.NetworkPrivatePath,
            target: FIND.NetworkStoragePath
        )
    }
}
