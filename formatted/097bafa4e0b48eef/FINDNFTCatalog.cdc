...
pub contract FINDNFTCatalog{ 
    ...
    pub event ProposalEntryUpdated(
        proposalID: UInt64,
        collectionIdentifier: String,
        message: String,
        status: String,
    ...
    priv let catalog:{ String: NFTCatalog.NFTCatalogMetadata} // { collectionIdentifier -> Metadata }
    
    priv let catalogTypeData:{ String:{ String: Bool}} // Additional view to go from { NFT Type Identifier -> {Collection Identifier : Bool } }
    
    priv let catalogProposals:{ UInt64: NFTCatalog.NFTCatalogProposal} // { ProposalID : Metadata }
    
    priv var totalProposals: UInt64
    ...
    pub fun getCatalogEntry(
        collectionIdentifier: String
    ): NFTCatalog.NFTCatalogMetadata?{ 
        if let dapper =
            NFTCatalog.getCatalogEntry(
    ...
    pub fun getCollectionDataForType(
        nftTypeIdentifier: String
    ): NFTCatalog.NFTCollectionData?{ 
        if let collections =
            FINDNFTCatalog.getCollectionsForType(
                nftTypeIdentifier: nftTypeIdentifier
            ){ 
            ...
            if let collection =
                FINDNFTCatalog.getCatalogEntry(
                    collectionIdentifier: collections.keys[0]
                ){ 
    ...
    pub fun getMetadataFromType(_ t: Type): NFTCatalog.NFTCatalogMetadata?{ 
        let collectionIdentifier =
            self.getCollectionsForType(nftTypeIdentifier: t.identifier)
        if collectionIdentifier == nil || (collectionIdentifier!).length < 1{ 
            return nil
        }
        return self.getCatalogEntry(
            collectionIdentifier: (collectionIdentifier!).keys[0]
        )
    }
    ...
    pub fun getFINDCatalogEntry(
        collectionIdentifier: String
    ): NFTCatalog.NFTCatalogMetadata?{ 
        return self.catalog[collectionIdentifier]
    }
    
    pub fun getFINDCollectionsForType(nftTypeIdentifier: String):{ 
        String: Bool
    }?{ 
        return self.catalogTypeData[nftTypeIdentifier]
    }
    ...
    pub fun proposeNFTMetadata(
        ...
        let proposerManagerCap =
            getAccount(proposer).getCapability<
                &NFTCatalog.NFTCatalogProposalManager{
                    NFTCatalog.NFTCatalogProposalManagerPublic
                }
            >(NFTCatalog.ProposalManagerPublicPath)
        assert(
            proposerManagerCap.check(),
            message: "Proposer needs to set up a manager"
        )
        let proposerManagerRef = proposerManagerCap.borrow()!
        assert(
            proposerManagerRef.getCurrentProposalEntry()!
            == collectionIdentifier,
            message: "Expected proposal entry does not match entry for the proposer"
        )
        let catalogProposal =
        ...
        emit ProposalEntryAdded(
            proposalID: self.totalProposals,
            collectionIdentifier: collectionIdentifier,
            message: catalogProposal.message,
            status: catalogProposal.status,
            proposer: catalogProposal.proposer
        )
    ...
    pub fun getCatalogProposalEntry(
        proposalID: UInt64
    ): NFTCatalog.NFTCatalogProposal?{ 
    ...
    access(account) fun addCatalogEntry(
        collectionIdentifier: String,
        metadata: NFTCatalog.NFTCatalogMetadata
    ){ 
        pre{ 
            self.catalog[collectionIdentifier] == nil:
                "The nft name has already been added to the catalog"
        }
        self.addCatalogTypeEntry(
            collectionIdentifier: collectionIdentifier,
            metadata: metadata
        )
    ...
    access(account) fun updateCatalogEntry(
        collectionIdentifier: String,
        metadata: NFTCatalog.NFTCatalogMetadata
    ){ 
        pre{ 
    ...
    access(account) fun removeCatalogEntry(collectionIdentifier: String){ 
        ...
        self.removeCatalogTypeEntry(
            collectionIdentifier: collectionIdentifier,
            metadata: self.catalog[collectionIdentifier]!
        )
        self.catalog.remove(key: collectionIdentifier)
        emit EntryRemoved(collectionIdentifier: collectionIdentifier)
    }
    
    access(account) fun updateCatalogProposal(
        proposalID: UInt64,
        proposalMetadata: NFTCatalog.NFTCatalogProposal
    ){ 
        self.catalogProposals[proposalID] = proposalMetadata
        emit ProposalEntryUpdated(
            proposalID: proposalID,
            collectionIdentifier: proposalMetadata.collectionIdentifier,
            message: proposalMetadata.message,
            status: proposalMetadata.status,
            proposer: proposalMetadata.proposer
        )
    }
    ...
    access(contract) fun addCatalogTypeEntry(
        collectionIdentifier: String,
        metadata: NFTCatalog.NFTCatalogMetadata
    ){ 
    ...
    access(contract) fun removeCatalogTypeEntry(
        collectionIdentifier: String,
        metadata: NFTCatalog.NFTCatalogMetadata
    ){ 
        let prevMetadata = self.catalog[collectionIdentifier]!
        let prevCollectionsForType =
            self.catalogTypeData[prevMetadata.nftType.identifier]!
