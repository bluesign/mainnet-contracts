...
pub contract FindForge{ 
    ...
    pub struct MinterPlatform{ 
        ...
        pub fun getMinterFTReceiver(): Capability<&{FungibleToken.Receiver}>{ 
            return getAccount(self.minter).getCapability<
    ...
    pub fun setMinterPlatform(
        ...
        panic(
            "Please purchase forge addon to start forging. Name: ".concat(
                lease.getName()
            )
        )
    }
    
    access(account) fun adminSetMinterPlatform(
        leaseName: String,
        forgeType: Type,
        minterCut: UFix64?,
        description: String,
        ...
        let user =
            FIND.lookupAddress(leaseName)
            ?? panic("Cannot find lease owner. Lease : ".concat(leaseName))
        let leaseCollection =
            getAccount(user).getCapability<
                &FIND.LeaseCollection{FIND.LeaseCollectionPublic}
            >(FIND.LeasePublicPath).borrow()
            ?? panic(
                "Cannot borrow reference to lease collection of user : ".concat(
                    leaseName
                )
            )
        ...
        panic(
            "Please give the user forge addon to start forging. Name: ".concat(
                leaseName
            )
        )
    }
    ...
    access(account) fun mintAdmin(
        leaseName: String,
        forgeType: Type,
        data: AnyStruct,
        receiver: &{NonFungibleToken.Receiver, MetadataViews.ResolverCollection}
    ){ 
        if !FindForge.minterPlatforms.containsKey(forgeType){ 
            panic("The minter platform is not set. Please set up properly before mint.")
        }
        let minterPlatform =
            (FindForge.minterPlatforms[forgeType]!)[leaseName]
            ?? panic(
                "The minter platform is not set. Please set up properly before mint."
            )
        ...
        let forge =
            FindForge.borrowForge(forgeType)
            ?? panic(
                "The forge type passed in is not supported. Forge Type : "
                    .concat(forgeType.identifier)
            )
        let nft <-
            forge.mint(platform: minterPlatform, data: data, verifier: verifier)
        let id = nft.id
        let uuid = nft.uuid
    ...
    pub fun orderForge(
        ...
        if !lease.checkAddon(addon: "forge")
        && !lease.checkAddon(addon: "premiumForge"){ 
            panic(
                "Please purchase forge addon to start forging. Name: ".concat(
                    lease.getName()
                )
    ...
    access(account) fun adminOrderForge(
        leaseName: String,
        mintType: String,
        minterCut: UFix64?,
        collectionDisplay: MetadataViews.NFTCollectionDisplay
    ...
    access(account) fun cancelForgeOrder(leaseName: String, mintType: String){ 
        FindForgeOrder.cancelForgeOrder(
            leaseName: leaseName,
            mintType: mintType
        )
    }
    
    access(account) fun fulfillForgeOrder(
        _ contractName: String,
        forgeType: Type
    ): MetadataViews.NFTCollectionDisplay{ 
        let order =
            FindForgeOrder.fulfillForgeOrder(contractName, forgeType: forgeType)
        let c = order.collectionDisplay
        let s:{ String: String} ={} 
        for social in c.socials.keys{ 
            s[social] = (c.socials[social]!).url
        }
        FindForge.adminSetMinterPlatform(
            leaseName: order.leaseName,
            forgeType: forgeType,
    ...
    access(account) fun adminMint(
        ...
        let address =
            FIND.lookupAddress(lease)
            ?? panic("This name is not owned by anyone. Name : ".concat(lease))
        let leaseCol =
            getAccount(address).getCapability<
                &FIND.LeaseCollection{FIND.LeaseCollectionPublic}
            >(FIND.LeasePublicPath).borrow()
            ?? panic(
                "Cannot borrow lease collection to lease owner. Owner : "
                    .concat(address.toString())
            )
        if !leaseCol.checkAddon(name: lease, addon: "forge")
        && !leaseCol.checkAddon(name: lease, addon: "premiumForge"){ 
            panic(
                "Please purchase forge addon to start forging. Name: ".concat(
                    lease
                )
        ...
        let minterPlatform =
            (FindForge.minterPlatforms[forgeType]!)[lease]
            ?? panic(
                "The minter platform is not set. Please set up properly before mint."
            )
        if minterPlatform.description == ""{ 
        ...
        let vr = receiver.borrowViewResolver(id: id)
        let view =
            vr.resolveView(Type<MetadataViews.Display>())
            ?? panic(
                "The minting nft should implement MetadataViews Display view."
            )
        let display = view as! MetadataViews.Display
        let nftName = display.name
        let thumbnail = display.thumbnail.uri()
    ...
    pub fun addContractData(
        ...
        FindForge.adminAddContractData(
            lease: lease.getName(),
            forgeType: forgeType,
            data: data
        )
    ...
    access(account) fun adminAddContractData(
        lease: String,
        forgeType: Type,
        data: AnyStruct
    ){ 
        ...
        let minterPlatform =
            (FindForge.minterPlatforms[forgeType]!)[lease]
            ?? panic("The minter platform is not set. Please set up properly.")
        if minterPlatform.description == ""{ 
    ...
    access(account) fun addPrivateForgeType(name: String, forgeType: Type){ 
        ...
        let receiverCap =
            FindForge.account.getCapability<&{FungibleToken.Receiver}>(
                Profile.publicReceiverPath
            )
        let minterPlatform =
            MinterPlatform(
                name: name,
                platform: receiverCap,
                platformPercentCut: FindForge.platformCut,
                minterCut: nil,
                description: "",
                externalURL: "",
                squareImage: "",
        ...
        (FindForge.minterPlatforms[forgeType]!).insert(
            key: name,
            minterPlatform
        )
    }
    
    access(account) fun adminRemoveMinterPlatform(
        name: String,
        forgeType: Type
    ){ 
        if !FindForge.minterPlatforms.containsKey(forgeType){ 
    ...
    pub resource ForgeAdminProxy: ForgeAdminProxyClient{ 
        priv var capability: Capability<&FIND.Network>?
