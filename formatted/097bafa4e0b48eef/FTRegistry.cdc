pub contract FTRegistry{ 
    ...
    /* getters */    pub fun getFTInfoByTypeIdentifier(_ typeIdentifier: String): FTInfo?{ 
    ...
    /* setters */    access(account) fun setFTInfo(
        alias: String,
        type: Type,
        tag: [
            String
    ...
    access(account) fun removeFTInfoByTypeIdentifier(
        _ typeIdentifier: String
    ): FTInfo{ 
        let info =
            FTRegistry.fungibleTokenList.remove(key: typeIdentifier)
            ?? panic(
                "Cannot find this Fungible Token Registry. Type : ".concat(
                    typeIdentifier
                )
            )
    ...
    access(account) fun removeFTInfoByAlias(_ alias: String): FTInfo{ 
        let typeIdentifier =
            ...
            ?? panic(
                "Cannot find type identifier from this alias. Alias : ".concat(
                    alias
                )
            )
