...
pub contract FindLeaseMarketAuctionSoft{ 
    ...
    pub resource SaleItem: FindLeaseMarket.SaleItem{ 
        ...
        pub fun acceptNonEscrowedBid(){ 
            pre{ 
                self.offerCallback != nil:
                    "There is no bid offer to the item."
                (self.offerCallback!).check():
                    "Bidder unlinked bid collection capability."
            }
        ...
        pub fun getAuction(): FindLeaseMarket.AuctionItem?{ 
            return FindLeaseMarket.AuctionItem(startPrice: self.auctionStartPrice, currentPrice: self.getBalance(), minimumBidIncrement: self.auctionMinBidIncrement, reservePrice: self.auctionReservePrice, extentionOnLateBid: self.auctionExtensionOnLateBid, auctionEndsAt: self.auctionEndsAt, timestamp: Clock.time())
        }
    ...
    pub resource SaleItemCollection:
        ...
        priv fun addBid(
            ...
            let newOfferBalance =
                newOffer.borrow()?.getBalance(name)
        ...
        access(contract) fun registerIncreasedBid(
            ...
            pre{ 
                self.items.containsKey(name):
                    "Invalid lease name=".concat(name)
            }
            let saleItem = self.borrow(name)
        ...
        pub fun cancel(_ name: String){ 
            ...
            let actionResult =
                self.getTenant().allowedAction(
                    ...
                    action: FindMarket.MarketAction(
                        listing: false,
                        name: "delist item from soft-auction"
                    ),
                    seller: nil,
                    buyer: nil
                )
            ...
            && (saleItem.offerCallback!).check(){ 
                ((saleItem.offerCallback!).borrow()!).cancelBidFromSaleItem(
                    name
                )
            }
        ...
        pub fun listForAuction(
            ...
            assert(
                self.items[pointer.name] == nil,
                message: "Auction listing for this item is already created."
            )
    ...
    pub resource MarketBidCollection:
        ...
        pub fun bid(
            ...
            let from =
                getAccount(FIND.status(name).owner!).getCapability<
            ...
            let saleItemCollection =
                from.borrow()
                ?? panic(
                    "Could not borrow sale item for lease name=".concat(name)
                )
        ...
        pub fun borrowBid(_ name: String): &Bid{ 
            pre{ 
                self.bids.containsKey(name):
                    "This name lease bid does not exist.".concat(name)
            }
