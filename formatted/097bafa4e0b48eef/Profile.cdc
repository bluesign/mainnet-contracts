...
pub contract Profile{ 
    ...
    pub resource User: Public, Owner, FungibleToken.Receiver{ 
        ...
        priv var findName: String
        
        priv var createdAt: String
        
        priv var gender: String
        
        priv var description: String
        
        priv var avatar: String
        
        priv var tags: [String]
        
        priv var followers:{ Address: FriendStatus}
        
        priv var bans:{ Address: Bool}
        
        priv var following:{ Address: FriendStatus}
        
        priv var collections:{ String: ResourceCollection}
        
        priv var wallets: [Wallet]
        
        priv var links:{ String: Link}
        
        priv var allowStoringFollowers: Bool
        
        //this is just a bag of properties if we need more fields here, so that we can do it with contract upgrade
        priv var additionalProperties:{ String: String}
    ...
    pub fun findReceiverCapability(
        address: Address,
        path: PublicPath,
        type: Type
        ...
        let cap =
            getAccount(address).getCapability<&{FungibleToken.Receiver}>(path)
        if cap.check(){ 
            return cap
        }
        return nil
    ...
    pub fun findWalletCapability(_ address: Address): Capability<
        &{FungibleToken.Receiver}
    >{ 
        return getAccount(address).getCapability<&{FungibleToken.Receiver}>(
