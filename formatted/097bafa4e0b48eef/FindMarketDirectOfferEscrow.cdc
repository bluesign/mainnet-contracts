...
pub contract FindMarketDirectOfferEscrow{ 
    ...
    pub resource SaleItemCollection:
        ...
        access(contract) fun registerIncreasedBid(_ id: UInt64){ 
            ...
            let actionResult =
                tenant.allowedAction(
                    listingType: Type<@FindMarketDirectOfferEscrow.SaleItem>(),
                    nftType: nftType,
                    ftType: ftType,
                    action: FindMarket.MarketAction(
                        listing: true,
                        name: "add bid in direct offer"
                    ),
        ...
        access(contract) fun registerBid(
            ...
            Debug.log(
                "currentBalance=".concat(currentBalance.toString()).concat(
                    " new bid is at="
            ...
            let previousCB =
                saleItem.offerCallback.borrow()
                ?? panic(
                    "Previous bidder unlinked the bid collection capability. bidder address : "
                        .concat(previousBuyer.toString())
                )
        ...
        pub fun acceptDirectOffer(_ pointer: FindViews.AuthNFTPointer){ 
            ...
            let saleItem = self.borrow(id)
            if saleItem.validUntil != nil
            && saleItem.validUntil! < Clock.time(){ 
            ...
            FindMarket.pay(
                ...
                nftInfo: nftInfo,
                cuts: cuts,
                resolver: fun (address: Address): String?{ 
                    return FIND.reverseLookup(address)
                },
