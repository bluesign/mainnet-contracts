...
pub contract FindForgeOrder{ 
    ...
    access(account) fun orderForge(
        ...
        let col =
            FindForgeOrder.account.borrow<&FindForgeOrder.Collection>(
    ...
    access(account) fun cancelForgeOrder(leaseName: String, mintType: String){ 
        let contractName =
            "Find".concat(FindUtils.firstUpperLetter(leaseName)).concat(
                mintType
            )
        let id =
    ...
    init(){ 
        ...
        self.account.save(
            <-queuedCollection,
            to: self.QueuedCollectionStoragePath
        )
        
        // create a public capability for the collection
        self.account.link<
            &FindForgeOrder.Collection{MetadataViews.ResolverCollection}
        >(
        ...
        self.account.save(
            <-completedCollection,
            to: self.CompletedCollectionStoragePath
        )
