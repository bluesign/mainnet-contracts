...
pub contract FindMarketSale{ 
        ...
        pub fun buy(
            ...
            let actionResult =
                tenant.allowedAction(
                    listingType: Type<@FindMarketSale.SaleItem>(),
                    nftType: nftType,
                    ftType: ftType,
                    action: FindMarket.MarketAction(
                        listing: false,
                        name: "buy item for sale"
                    ),
                    seller: (self.owner!).address,
                    buyer: nftCap.address
                )
            ...
            emit Sale(
                ...
                amount: saleItem.getBalance(),
                status: "sold",
                vaultType: ftType.identifier,
                nft: nftInfo,
                buyer: buyer,
                buyerName: buyerName,
                buyerAvatar: Profile.find(nftCap.address).getAvatar(),
        ...
        pub fun listForSale(
            ...
            let saleItem <-
                create SaleItem(
                    pointer: pointer,
                    vaultType: vaultType,
                    price: directSellPrice,
                    validUntil: validUntil,
                    saleItemExtraField: extraField
                )
            let tenant = self.getTenant()
            ...
            emit Sale(
                ...
                amount: saleItem.salePrice,
                status: "active_listed",
                vaultType: vaultType.identifier,
                nft: saleItem.toNFTInfo(true),
                buyer: nil,
                buyerName: nil,
                buyerAvatar: nil,
                endsAt: saleItem.validUntil
    ...
    pub fun getSaleItemCapability(
        ...
        &FindMarketSale.SaleItemCollection{
            FindMarketSale.SaleItemCollectionPublic,
            FindMarket.SaleItemCollectionPublic
        }
    >?{ 
