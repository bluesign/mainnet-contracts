...
import
    ...
    from "../0xf233dcee88fe0abe/FungibleTokenSwitchboard.cdc"
...
pub contract FindMarket{ 
    ...
    pub event RoyaltyCouldNotBePaid(
        tenant: String,
        id: UInt64,
        saleID: UInt64,
        address: Address,
    ...
    pub struct TenantCuts{ 
        ...
        init(
            findCut: MetadataViews.Royalty?,
            tenantCut: MetadataViews.Royalty?
        ){ 
    ...
    pub fun getPublicPath(_ type: Type, name: String): PublicPath{ 
        ...
        return PublicPath(identifier: path)
    ...
    /* Get Tenant */    pub fun getTenant(_ tenant: Address): &FindMarket.Tenant{
        FindMarket.TenantPublic
    }{ 
    ...
    /* Get SaleItemCollections */    pub fun getSaleItemCollectionTypes(): [Type]{ 
        return self.saleItemCollectionTypes
    }
    
    pub fun getSaleItemCollectionCapabilities(
        tenantRef: &FindMarket.Tenant{FindMarket.TenantPublic},
        address: Address
    ): [
    ...
    pub fun getSaleItemCollectionCapability(
        tenantRef: &FindMarket.Tenant{FindMarket.TenantPublic},
        marketOption: String,
        address: Address
    ): Capability<&{FindMarket.SaleItemCollectionPublic}>{ 
    ...
    /* Get Sale Reports and Sale Item */    pub fun assertOperationValid(
        tenant: Address,
        address: Address,
        marketOption: String,
        id: UInt64
    ...
    pub fun getRoyaltiesChangedIds(tenant: Address, address: Address):{ 
        String: [
            UInt64
        ]
    }{ 
    ...
    pub fun getRoyaltiesChangedItems(tenant: Address, address: Address):{ 
    ...
    /* Get Sale Reports and Sale Item */    pub fun getSaleInformation(
        tenant: Address,
        address: Address,
        marketOption: String,
        id: UInt64,
        getNFTInfo: Bool
    ): FindMarket.SaleItemInformation?{ 
        let tenantRef = self.getTenant(tenant)
        let info =
    ...
    access(contract) fun checkSaleInformation(
        tenantRef: &FindMarket.Tenant{FindMarket.TenantPublic},
        marketOption: String,
        address: Address,
        itemId: UInt64?,
        getGhost: Bool,
        getNFTInfo: Bool,
        getRoyaltyChanged: Bool
    ): FindMarket.SaleItemCollectionReport{ 
    ...
    /* Get Bid Collections */    pub fun getMarketBidTypes(): [Type]{ 
    ...
    pub fun getMarketBidCollectionCapabilities(
        tenantRef: &FindMarket.Tenant{FindMarket.TenantPublic},
        address: Address
    ): [
    ...
    pub fun getMarketBidCollectionCapability(
        tenantRef: &FindMarket.Tenant{FindMarket.TenantPublic},
        marketOption: String,
        address: Address
    ): Capability<&{FindMarket.MarketBidCollectionPublic}>{ 
    ...
    access(contract) fun checkBidInformation(
        tenantRef: &FindMarket.Tenant{FindMarket.TenantPublic},
        marketOption: String,
        address: Address,
        itemId: UInt64?,
        getGhost: Bool,
        getNFTInfo: Bool
    ): FindMarket.BidItemCollectionReport{ 
        let ghost: [FindMarket.GhostListing] = []
    ...
    pub fun assertBidOperationValid(
        tenant: Address,
        address: Address,
        marketOption: String,
        id: UInt64
    ...
    /* Helper Function */    pub fun getMarketOptionFromType(_ type: Type): String{ 
    ...
    /* Admin Function */    access(account) fun addPathMap(_ type: Type){ 
    ...
    pub struct TenantRule{ 
        ...
        init(name: String, types: [Type], ruleType: String, allow: Bool){ 
            if !(
                ruleType == "nft" || ruleType == "ft" || ruleType == "listing"
            ){ 
    ...
    pub resource Tenant: TenantPublic{ 
        priv let findSaleItems:{ String: TenantSaleItem}
        
        priv let tenantSaleItems:{ String: TenantSaleItem}
        
        priv let findCuts:{ String: TenantSaleItem}
        ...
        access(account) fun alterMarketOption(name: String, status: String){ 
            pre{ 
                self.tenantSaleItems[name] != nil:
                    "This saleItem does not exist. Item : ".concat(name)
            }
        ...
        pub fun getFindCut(name: String, listingType: Type, nftType: Type, ftType: Type): FindMarketCutStruct.Cuts?{ 
            ...
            if let cache = FindRulesCache.getTenantCut(tenant: self.name, ruleId: findRuleId){ 
                ...
                if let findCut = cache.findCut{ 
                    returningCut = FindMarketCutStruct.Cuts([FindMarketCutStruct.GeneralCut(name: findCut.description, cap: findCut.receiver, cut: findCut.cut, description: findCut.description)])
                }
                return returningCut
            }
            ...
            for findCut in self.findCuts.values{ 
                ...
                if valid && findCut.cut != nil{ 
                    ...
                    returningCut = FindMarketCutStruct.Cuts([FindMarketCutStruct.GeneralCut(name: (findCut.cut!).description, cap: (findCut.cut!).receiver, cut: (findCut.cut!).cut, description: (findCut.cut!).description)])
                    break
                }
            }
            
            // store that to cache
            let cacheCut = FindRulesCache.TenantCuts(findCut: cacheFindCut, tenantCut: nil)
        ...
        pub fun getTenantCut(name: String, listingType: Type, nftType: Type, ftType: Type): FindMarketCutStruct.Cuts?{ 
            ...
            if let cache = FindRulesCache.getTenantCut(tenant: self.name, ruleId: tenantRuleId){ 
                ...
                if let tenantCut = cache.tenantCut{ 
                    returningCut = FindMarketCutStruct.Cuts([FindMarketCutStruct.GeneralCut(name: tenantCut.description, cap: tenantCut.receiver, cut: tenantCut.cut, description: tenantCut.description)])
                }
                return returningCut
            }
            ...
            for item in self.tenantSaleItems.values{ 
                ...
                if valid && item.cut != nil{ 
                    ...
                    returningCut = FindMarketCutStruct.Cuts([FindMarketCutStruct.GeneralCut(name: (item.cut!).description, cap: (item.cut!).receiver, cut: (item.cut!).cut, description: (item.cut!).description)])
                    break
                }
            }
            
            // store that to cache
            let cacheCut = FindRulesCache.TenantCuts(findCut: nil, tenantCut: cacheTenantCut)
        ...
        pub fun allowedAction(listingType: Type, nftType: Type, ftType: Type, action: MarketAction, seller: Address?, buyer: Address?): FindRulesCache.ActionResult{ 
            /* Check for Honour Banning */            let profile = getAccount(FindMarket.tenantNameAddress[self.name]!).getCapability<&Profile.User{Profile.Public}>(Profile.publicPath).borrow() ?? panic("Cannot get reference to Profile to check honour banning. Tenant Name : ".concat(self.name))
    ...
    pub resource TenantClient: TenantClientPublic{ 
        priv var capability: Capability<&Tenant>?
    ...
    access(account) fun createFindMarket(
        name: String,
        address: Address,
        findCutSaleItem: TenantSaleItem?
    ): Capability<&Tenant>{ 
        let account = FindMarket.account
    ...
    pub fun getTenantCapability(_ marketplace: Address): Capability<
        &Tenant{TenantPublic}
    >?{ 
        if !self.tenantAddressName.containsKey(marketplace){ 
            "tenant is not registered in registry"
        }
        return FindMarket.account.getCapability<&Tenant{TenantPublic}>(
            PublicPath(identifier: self.getTenantPathForAddress(marketplace))!
        )
    }
    
    access(account) fun pay(
        tenant: String,
        id: UInt64,
        saleItem: &{SaleItem},
        vault: @FungibleToken.Vault,
        royalty: MetadataViews.Royalties,
        nftInfo: NFTInfo,
        cuts:{ 
            String: FindMarketCutStruct.Cuts
        },
        ...
        /* Residual Royalty */        var payInFUT = false
        var payInDUC = false
        let ftInfo = FTRegistry.getFTInfoByTypeIdentifier(ftType.identifier)! // If this panic, there is sth wrong in FT set up
        let oldProfileCap =
            getAccount(seller).getCapability<&{FungibleToken.Receiver}>(
                Profile.publicReceiverPath
            )
        let oldProfile =
            self.getPaymentWallet(oldProfileCap, ftInfo, panicOnFailCheck: true)
        /* Check the total royalty to prevent changing of royalties */        let royalties = royalty.getRoyalties()
        if royalties.length != 0{ 
            ...
            for royaltyItem in royalties{ 
                ...
                /* If the royalty receiver check failed */                if (wallet.owner!).address == FindMarket.residualAddress{ 
    ...
    access(account) fun getPaymentWallet(
        _ cap: Capability<&{FungibleToken.Receiver}>,
        _ ftInfo: FTRegistry.FTInfo,
        panicOnFailCheck: Bool
        ...
        tempCap = getAccount(cap.address).getCapability<
                &{FungibleToken.Receiver}
            >(ftInfo.receiverPath)
        if tempCap.check(){ 
        ...
        let msg =
            "User ".concat(cap.address.toString()).concat(
                " does not have any usable links set up for vault type "
            ).concat(ftInfo.typeIdentifier)
        panic(msg)
    }
    ...
    pub struct NFTInfo{ 
        ...
        init(_ item: &{MetadataViews.Resolver}, id: UInt64, detail: Bool){ 
            ...
            let display =
                MetadataViews.getDisplay(item)
                ?? panic("cannot get MetadataViews.Display View")
