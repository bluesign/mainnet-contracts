...
pub contract FindLostAndFoundWrapper{ 
    ...
    pub event UserStorageSubsidized(
        receiver: Address,
        receiverName: String?,
        sender: Address,
        senderName: String?,
    ...
    pub event TicketDeposited(
        ...
        collectionName: String?,
        collectionImage: String?,
        flowStorageFee: UFix64
    )
    
    pub event TicketRedeemed(
        receiver: Address,
        receiverName: String?,
        ticketID: UInt64,
        type: String
    ...
    pub fun depositNFT(
        ...
        let collectionDisplay =
            MetadataViews.getNFTCollectionDisplay(item.getViewResolver())
        let id = item.id
        let uuid = item.uuid
        ...
        let requiredStorage =
            senderStorageBeforeSend - getAccount(sender).storageUsed
        let receiverAvailableStorage =
            getAccount(receiverAddress).storageCapacity
            - getAccount(receiverAddress).storageUsed
        
        // Try to send before using Lost & FIND
        let receiverCap =
            getAccount(receiverAddress).getCapability<
        ...
        let vault <-
            storagePayment.withdraw(amount: estimate.storageFee + 0.00005)
        ...
        emit TicketDeposited(
            ...
            receiverName: FIND.reverseLookup(receiverAddress),
            sender: sender,
            ...
            collectionImage: collectionDisplay?.squareImage?.file?.uri(),
            flowStorageFee: flowStorageFee
        )
        destroy estimate
        return ticketID
    }
    ...
    pub fun redeemNFT(
        ...
        let shelf =
            LostAndFound.borrowShelfManager().borrowShelf(
                redeemer: receiverAddress
            )
            ?? panic(
                "No items to redeem for this user: ".concat(
                    receiverAddress.toString()
                )
            )
        let bin =
            shelf.borrowBin(type: type)
            ?? panic(
                "No items to redeem for this user: ".concat(
                    receiverAddress.toString()
                )
        ...
        let nftID =
            ticket.getNonFungibleTokenID()
            ?? panic("The item you are trying to redeem is not an NFT")
        ...
        shelf.redeem(
            type: type,
            ticketID: ticketID,
            receiver: receiverCap.check() ? receiverCap : collectionPublicCap
        )
    ...
    // Check 
    pub fun getTickets(user: Address, specificType: Type?):{ 
        String: [
            LostAndFoundHelper.Ticket
        ]
    }{ 
    ...
    pub fun getTicketIDs(user: Address, specificType: Type?):{ 
        String: [
            UInt64
        ]
    }{ 
    ...
    pub fun getSpecificRedeemableTypes(user: Address, specificType: Type?): [
        Type
    ]{ 
        let allTypes: [Type] = []
        if specificType != nil{ 
    ...
    // Helper function
    access(contract) fun depositVault(
    ...
    access(contract) fun destroyVault(
        _ uuid: UInt64,
        cap: Capability<&FlowToken.Vault{FungibleToken.Receiver}>
    ){ 
        let vault <-
            self.storagePaymentVaults.remove(key: uuid)
            ?? panic("Invalid vault UUID. UUID: ".concat(uuid.toString()))
    ...
    access(contract) fun subsidizeUserStorage(
        requiredStorage: UInt64,
        receiverAvailableStorage: UInt64,
        receiver: Address,
        ...
        let subsidizeAmount =
            FlowStorageFees.storageCapacityToFlow(
                FlowStorageFees.convertUInt64StorageBytesToUFix64Megabytes(
                    subsidizeCapacity
                )
            )
        let flowReceiverCap =
            getAccount(receiver).getCapability<&{FungibleToken.Receiver}>(
        ...
        emit UserStorageSubsidized(
            receiver: receiver,
            receiverName: FIND.reverseLookup(receiver),
            sender: sender,
            senderName: FIND.reverseLookup(sender),
            forUUID: uuid,
            storageFee: subsidizeAmount
        )
        return true
    }
