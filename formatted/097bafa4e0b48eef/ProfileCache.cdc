...
pub contract ProfileCache{ 
    ...
    access(account) fun setAddressLeaseNameCache(
        address: Address,
        leaseName: String?,
        validUntil: UFix64
    ){ 
    ...
    access(account) fun setNameAddressCache(
        address: Address?,
        leaseName: String,
        validUntil: UFix64
    ){ 
        ...
        panic(
            "There is already a cache for this name. Name : ".concat(leaseName)
        )
    }
    ...
    access(account) fun setWalletIndexCache(
        address: Address,
        walletType: Type,
        index: Int
    ){ 
        ...
        panic(
            "There is already a cache for this wallet. User : ".concat(
                address.toString()
            ).concat(". Wallet : ").concat(walletType.identifier)
        )
    }
