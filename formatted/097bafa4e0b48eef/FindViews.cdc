...
pub contract FindViews{ 
    ...
    pub struct ViewReadPointer: Pointer{ 
        priv let cap: Capability<&{MetadataViews.ResolverCollection}>
    ...
    pub struct AuthNFTPointer: Pointer, AuthPointer{ 
        priv let cap: Capability<&{MetadataViews.ResolverCollection, NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>
    ...
    pub fun checkSoulBound(_ viewResolver: &{MetadataViews.Resolver}): Bool{ 
        if let soulBound =
            viewResolver.resolveView(Type<FindViews.SoulBound>()){ 
