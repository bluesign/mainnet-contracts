...
pub contract FindAirdropper{ 
    ...
    pub event AirdroppedToLostAndFound(
        from: Address,
        fromName: String?,
        to: Address,
        toName: String?,
    ...
    pub fun safeAirdrop(
        ...
        let receiverCap =
            getAccount(receiver).getCapability<&{NonFungibleToken.Receiver}>(
                path
            )
        ...
        let requiredStorage =
            senderStorageBeforeSend - getAccount(from).storageUsed
        let receiverAvailableStorage =
            getAccount(receiver).storageCapacity
            - getAccount(receiver).storageUsed
