...
pub contract EnvironmentAct: NonFungibleToken{ 
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: EnvironmentAct.CollectionStoragePath, publicPath: EnvironmentAct.CollectionPublicPath, providerPath: /private/environmentActCollectionProviderPath, publicCollection: Type<&EnvironmentAct.Collection{EnvironmentAct.EnvironmentActCollectionPublic}>(), publicLinkedType: Type<&EnvironmentAct.Collection{EnvironmentAct.EnvironmentActCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&EnvironmentAct.Collection{EnvironmentAct.EnvironmentActCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Provider, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
                ...
                case Type<MetadataViews.Medias>():
                    return MetadataViews.Medias(items: [MetadataViews.Media(file: MetadataViews.HTTPFile(url: self.mediumimage()), mediaType: "image/png")])
            }
            return nil
        }
    }
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &EnvironmentAct.NFT?{ 
        let collection = getAccount(from).getCapability(EnvironmentAct.CollectionPublicPath).borrow<&EnvironmentAct.Collection{EnvironmentAct.EnvironmentActCollectionPublic}>() ?? panic("Couldn't get collection")
