...
pub contract ByteNextStaking{ 
    ...
    pub event WithdrawRewardByOrder(
        stageId: UInt64,
        user: Address?,
        orderId: Integer,
        rewardAmount: UFix64,
    ...
    pub resource StageStaking: StageStakingPublic{ 
        ...
        access(contract) fun withdrawByOrder(orderId: UInt64, receiverReward: Capability<&{FungibleToken.Receiver}>, receiverStaking: Capability<&{FungibleToken.Receiver}>, receiverNftBoost: Capability<&{ByteNextMedalNFT.CollectionPublic}>){ 
            pre{ 
                receiverReward.check():
                    "Recipient reward ref invalid"
        ...
        priv fun _calculateInterestOrder(order: StakingOrder):{ String: UFix64}{ 
            if order.isUnstake{ 
        ...
        priv fun _calculatePendingEarned(annualProfit: UFix64, userStakedAmount: UFix64, pendingTime: UFix64): UFix64{ 
    ...
    pub resource Administrator{ 
        pub fun freeze(stageId: UInt64){ 
            ...
            let stage =
                (&ByteNextStaking.stageStaking[stageId] as &StageStaking?)!
        ...
        pub fun createStage(
            ...
            let oldStage
                <- ByteNextStaking.stageStaking[ByteNextStaking.stakingCount]
                <- stage
            destroy oldStage
            emit CreateNewStage(
    ...
    pub resource StakingProxy{ 
        ...
        pub fun withdrawRewardByOrder(
            ...
            let stage =
                (&ByteNextStaking.stageStaking[stageId] as &StageStaking?)!
            stage.withdrawRewardByOrder(
                orderId: orderId,
                receiverReward: receiver
            )
        }
    ...
    pub fun borrowStage(stageId: UInt64): &StageStaking{StageStakingPublic}?{ 
        return (
            &self.stageStaking[stageId] as &StageStaking{StageStakingPublic}?
        )!
    }
