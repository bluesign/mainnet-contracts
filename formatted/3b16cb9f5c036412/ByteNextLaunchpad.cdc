...
pub contract ByteNextLaunchpad{ 
    ...
    pub resource Launchpad: LaunchpadPublic{ 
        ...
        //PUBLIC FUNCTIONS
        pub fun getLaunchpadInfo():{ String: AnyStruct}{ 
            return{ "totalSale": self.totalSell, "totalSold": self.totalSold, "tokenType": self.tokenType, "tokenAddress": self.tokenAddress, "minterCapability": self.minterCapability, "nftFormatTemp": self.nftFormatTemp, "totalRevice": self.totalRevice, "totalStage": self.totalStage}
        }
    ...
    pub resource Administrator{ 
        priv fun getLaunchpad(launchpadId: UInt64): &Launchpad{ 
            pre{ 
                ByteNextLaunchpad.launchpads.containsKey(launchpadId):
        ...
        pub fun createLaunchpad(
            ...
            let launchpad <-
                create Launchpad(
                    ...
                    nftFormatTemp: nftFormatTemp
                )
            let oldLaunchpad
                <- ByteNextLaunchpad.launchpads[
                    ByteNextLaunchpad.launchpadCount
                ]
                <- launchpad
            destroy oldLaunchpad
            emit NewLaunchpadCreated(
                id: ByteNextLaunchpad.launchpadCount,
                totalSell: totalSell,
                tokenType: tokenType
            )
            ByteNextLaunchpad.launchpadCount = ByteNextLaunchpad.launchpadCount
                + 1
        }
    ...
    pub resource LaunchpadProxy{ 
        pub fun swapTicketForPoint(tickets: @[ByteNextMedalNFT.NFT]){ 
            ...
            let currentPoint =
                ByteNextLaunchpad.userPoints.remove(key: (self.owner!).address)
            let amountPoint = (currentPoint ?? 0) + totalPointReceive
            ByteNextLaunchpad.userPoints.insert(
        ...
        pub fun registJoin(launchpadId: UInt64, stageId: UInt8){ 
            pre{ 
                ...
                self.owner != nil:
                    "Owner should not be nil"
                ByteNextLaunchpad.userPoints[(self.owner!).address]! > 0:
                    "Not enough points to participate"
            }
            let launchpad =
                (&ByteNextLaunchpad.launchpads[launchpadId] as &Launchpad?)!
