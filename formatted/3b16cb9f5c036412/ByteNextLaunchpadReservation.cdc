...
pub contract ByteNextLaunchpadReservation{ 
    ...
    pub resource Launchpad: LaunchpadPublic{ 
        ...
        init(totalSell: UInt64, nftType: Type, startTime: UFix64, endTime: UFix64, claimTime: UFix64, price: UFix64, tokenPerUser: UInt64, fundVault: @FungibleToken.Vault){ 
            pre{ 
                totalSell > 0:
        ...
        pub fun setPriceToken(price: UFix64, fundVault: @FungibleToken.Vault){ 
            pre{ 
                self.fundVault.balance == 0.0:
                    "Fund vault must be empty"
            }
            self.price = price
        ...
        pub fun depositFund(depVault: @FungibleToken.Vault){ 
            pre{ 
                depVault.getType() == self.fundVault.getType():
                    "Token deposit must be same type token IDO"
            }
        ...
        pub fun withdrawFund(amount: UFix64): @FungibleToken.Vault{ 
            pre{ 
                amount <= self.fundVault.balance:
                    "Amount withdraw is invalid"
            }
        ...
        //PUBLIC FUNCTIONS
        pub fun getLaunchpadInfo():{ String: AnyStruct}{ 
            return{ "isFrozen": self.isFrozen, "totalSell": self.totalSell, "comboPrice": self.comboPrice, "comboSell": self.comboSell, "comboSold": self.comboSold, "comboEndTime": self.comboEndTime, "nftType": self.nftType, "startTime": self.startTime, "endTime": self.endTime, "claimTime": self.claimTime, "price": self.price, "tokenPerUser": self.tokenPerUser, "totalSold": self.totalSold, "totalClaim": self.totalClaim, "totalRefund": self.totalRefund, "amountNft": self.nftVaults.length, "fundBalance": self.fundVault.balance}
        }
        ...
        pub fun buy(launchpadId: UInt64, account: Address, quantity: UInt64, paymentVault: @FungibleToken.Vault, recipientCapability: Capability<&{NonFungibleToken.CollectionPublic}>){ 
            pre{ 
                paymentVault.getType() == self.fundVault.getType():
        ...
        pub fun buyCombo(launchpadId: UInt64, account: Address, comboType: UInt64, quantity: UInt64, paymentVault: @FungibleToken.Vault, recipientCapability: Capability<&{NonFungibleToken.CollectionPublic}>){ 
            pre{ 
                paymentVault.getType() == self.fundVault.getType():
                    "Payment token is not allowed"
            }
        ...
        pub fun claim(launchpadId: UInt64, account: Address){ 
            pre{ 
                self.userCapReceivers[account] != nil:
                    "The account were not join this launchpad"
            }
            assert(!self.isFrozen, message: "Launchpad is frozen")
            let now = getCurrentBlock().timestamp
        ...
        pub fun refund(launchpadId: UInt64, account: Address): @FungibleToken.Vault{ 
            pre{ 
                self.userBoughts.containsKey(account):
                    "The account were not join this launchpad"
            }
            assert(!self.isFrozen, message: "Launchpad is frozen")
            let now = getCurrentBlock().timestamp
    ...
    pub resource Administrator{ 
        pub fun borrowLaunchpad(launchpadId: UInt64): &Launchpad{ 
            pre{ 
                ByteNextLaunchpadReservation.launchpads.containsKey(launchpadId):
                    "Launchpad is invalid"
                self.owner != nil:
                    "Owner should not be nil"
            }
            return (
                &ByteNextLaunchpadReservation.launchpads[launchpadId]
                as
                &Launchpad?
            )!
        }
        ...
        pub fun createLaunchpad(
            ...
            let oldLaunchpad
                <- ByteNextLaunchpadReservation.launchpads[
                    ByteNextLaunchpadReservation.launchpadCount
                ]
                <- launchpad
            destroy oldLaunchpad
            emit NewLaunchpadCreated(
                id: ByteNextLaunchpadReservation.launchpadCount,
                totalSell: totalSell,
                nftType: nftType,
                startTime: startTime,
                endTime: endTime,
            ...
            ByteNextLaunchpadReservation
                .launchpadCount = ByteNextLaunchpadReservation.launchpadCount
                + 1
        }
    }
    ...
    init(){ 
        ...
        self
            .LaunchpadProxyStoragePath = /storage/ByteNextLaunchpadReservationProxy
