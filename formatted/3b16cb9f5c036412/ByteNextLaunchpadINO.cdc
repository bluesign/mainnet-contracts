...
import
    NonFungibleTokenMinter
    from "../0x3b16cb9f5c036412;/NonFungibleTokenMinter.cdc"

pub contract ByteNextLaunchpadINO{ 
    ...
    pub struct LaunchpadStage{ 
        ...
        init(
            ...
            pre{ 
                ...
                paymentType == (tokenReceiver.borrow()!).getType():
                    "Should type receiver same type of payment"
            }
            self.isFrozen = false
    ...
    pub resource interface LaunchpadPublic{ 
        ...
        pub fun getNFTForMint():{ UInt64:{ String: String}}{} 
        ...
        pub fun setMinterCapability(
            capability: Capability<&{NonFungibleTokenMinter.MinterProvider}>
        ){} 
    }
    
    pub resource Launchpad: LaunchpadPublic{ 
        priv var totalStage: UInt8
        
        priv let stages:{ UInt8: LaunchpadStage}
        
        priv var totalSell: UInt64
        
        priv var mintByTemp: Bool
        
        // format template to build metadata for nft
        priv var nftFormatTemp: NFTFormatTemplate?
        
        // list type nft special for mint
        priv var nftForMints:{ UInt64:{ String: String}}
        
        // Type of token NFT to INO
        priv var tokenType: Type
        
        priv var tokenAddress: Address
        
        priv var totalSold: UInt64
        
        priv var totalRevice: UFix64
        
        priv var minterCapability: Capability<&{NonFungibleTokenMinter.MinterProvider}>?
        
        init(totalSell: UInt64, tokenType: Type, tokenAddress: Address, mintByTemp: Bool){ 
            pre{ 
                totalSell > 0:
                    "total sell token is invalid"
            }
            self.totalSell = totalSell
            self.mintByTemp = mintByTemp
            self.nftFormatTemp = nil
            self.nftForMints ={} 
        ...
        access(contract) fun setMintByTemp(mintByTemp: Bool){ 
            self.mintByTemp = mintByTemp
        }
        ...
        access(contract) fun setTypeContract(type: Type){ 
            self.tokenType = type
        }
        ...
        pub fun setMinterCapability(capability: Capability<&{NonFungibleTokenMinter.MinterProvider}>){ 
            pre{ 
                capability.address == self.tokenAddress:
                    "You not owner of contract nft"
            }
        ...
        access(contract) fun setListNFTForMint(tokenIds: [UInt64], metadatas: [{String: String}]){ 
            pre{ 
                tokenIds.length == metadatas.length:
                    "Params data not same length"
            }
            var index: UInt64 = 0
            for tokenId in tokenIds{ 
                self.nftForMints.insert(key: tokenId, metadatas[index])
                index = index + 1
            }
        }
        
        access(contract) fun removeNFTForMint(tokenIds: [UInt64]){ 
            for tokenId in tokenIds{ 
                if self.nftForMints.containsKey(tokenId){ 
                    self.nftForMints.remove(key: tokenId)
                }
            }
        }
        
        access(contract) fun createNewStage(launchpadId: UInt64, isPublic: Bool, tokenPerUser: UInt64, acceptingPoint: Bool, totalAllocation: UInt64, startTime: UFix64, endTime: UFix64, price: UFix64, paymentType: Type, tokenReceiver: Capability<&{FungibleToken.Receiver}>){ 
            pre{ 
                totalAllocation <= self.totalSell - self.totalSold:
                    "Allocation has exceeded the total raise"
            }
            ...
            self.stages[self.totalStage] = LaunchpadStage(isPublic: isPublic, tokenPerUser: tokenPerUser, acceptingPoint: acceptingPoint, totalAllocation: totalAllocation, startTime: startTime, endTime: endTime, price: price, paymentType: paymentType, tokenReceiver: tokenReceiver)
            emit NewStageCreated(launchpadId: launchpadId, id: self.totalStage, isPublic: isPublic, tokenPerUser: tokenPerUser, totalAllocation: totalAllocation, startTime: startTime, endTime: endTime, price: price, paymentType: paymentType)
            self.totalStage = self.totalStage + 1
        }
        
        access(contract) fun setFrozen(stageId: UInt8, isFrozen: Bool){ 
            pre{ 
                self.stages.containsKey(stageId):
                    "Stage is invalid"
            }
        ...
        access(contract) fun registJoin(stageId: UInt8, user: Address){ 
            pre{ 
                self.stages.containsKey(stageId):
                    "Stage is invalid"
                !(self.stages[stageId]!).isPublic:
                    "Stage must be private sale"
            }
            let stageInfo = self.stages[stageId]!
            ...
            let currentPoint = ByteNextLaunchpadINO.userPoints.remove(key: user)
            let amountPoint = (currentPoint ?? 0) - 1
            ByteNextLaunchpadINO.userPoints.insert(key: user, amountPoint)
        }
        ...
        access(contract) fun setUserAllocation(stageId: UInt8, whitelist: [Whitelist]){ 
            ...
            for data in whitelist{ 
                ...
                if stageInfo.userAllocations.containsKey(userAddress){ 
                    stageInfo.totalAllocated = stageInfo.totalAllocated - stageInfo.userAllocations[userAddress]!
                    stageInfo.userAllocations.remove(key: userAddress)
                }
                stageInfo.totalAllocated = stageInfo.totalAllocated + userAllocation
        ...
        //PUBLIC FUNCTIONS
        pub fun getLaunchpadInfo():{ String: AnyStruct}{ 
            return{ "totalSale": self.totalSell, "totalSold": self.totalSold, "tokenType": self.tokenType, "tokenAddress": self.tokenAddress, "mintByTemp": self.mintByTemp, "minterCapability": self.minterCapability, "nftFormatTemp": self.nftFormatTemp, "totalRevice": self.totalRevice, "totalStage": self.totalStage}
        }
        
        pub fun getNFTForMint():{ UInt64:{ String: String}}{ 
            return self.nftForMints
        }
        ...
        access(contract) fun join(launchpadId: UInt64, stageId: UInt8, account: Address, paymentVault: @FungibleToken.Vault, recipient: &{NonFungibleToken.CollectionPublic}){ 
            pre{ 
                self.stages.containsKey(stageId):
                    "Stage is invalid"
                paymentVault.isInstance(self.getStageInfo(stageId: stageId).paymentType):
        ...
        priv fun _mintNFT(account: Address, recipient: &{NonFungibleToken.CollectionPublic}, amount: UInt64){ 
            let minterRef = (self.minterCapability!).borrow() ?? panic("Can not borrow minter")
            if self.mintByTemp{ 
                assert(self.nftFormatTemp != nil, message: "Template nft unset")
                var minted: UInt64 = 0
                while minted < amount{ 
                    ...
                    minterRef.mintNFT(id: tokenId, recipient: recipient, metadata: metadata)
                    minted = minted + 1
                }
            } else{ 
                let tokenIds = self.nftForMints.keys
                assert(UInt64(tokenIds.length) >= amount, message: "NFT is not enough to mint")
                var minted: UInt64 = 0
                while minted < amount{ 
                    let tokenId = tokenIds[minted]
                    let metadata = self.nftForMints[tokenId]!
                    minterRef.mintNFT(id: tokenId, recipient: recipient, metadata: metadata)
                    minted = minted + 1
                    self.nftForMints.remove(key: tokenId)
                }
        ...
        priv fun _buildMetadataMint(tokenId: UInt64, formatTemplate: NFTFormatTemplate):{ String: String}{ 
    ...
    pub resource Administrator{ 
        priv fun getLaunchpad(launchpadId: UInt64): &Launchpad{ 
            pre{ 
                ByteNextLaunchpadINO.launchpads.containsKey(launchpadId):
                    "Launchpad is invalid"
                self.owner != nil:
                    "Owner should not be nil"
            }
            return (
                &ByteNextLaunchpadINO.launchpads[launchpadId] as &Launchpad?
            )!
        }
        ...
        pub fun createLaunchpad(
            ...
            let launchpad <-
                create Launchpad(
                    ...
                    mintByTemp: mintByTemp
                )
            let oldLaunchpad
                <- ByteNextLaunchpadINO.launchpads[
                    ByteNextLaunchpadINO.launchpadCount
                ]
                <- launchpad
            destroy oldLaunchpad
            emit NewLaunchpadCreated(
                id: ByteNextLaunchpadINO.launchpadCount,
                totalSell: totalSell,
                tokenType: tokenType
            )
            ByteNextLaunchpadINO.launchpadCount = ByteNextLaunchpadINO
                    .launchpadCount
                + 1
        }
        ...
        pub fun setTypeContract(launchpadId: UInt64, type: Type){ 
            let launchpad = self.getLaunchpad(launchpadId: launchpadId)
            launchpad.setTypeContract(type: type)
        }
        
        pub fun setMintByTemp(launchpadId: UInt64, mintByTemp: Bool){ 
            let launchpad = self.getLaunchpad(launchpadId: launchpadId)
            launchpad.setMintByTemp(mintByTemp: mintByTemp)
        }
        
        pub fun setNFTFormatTemplate(
            launchpadId: UInt64,
            nftFormatTemp: NFTFormatTemplate
        ){ 
            let launchpad = self.getLaunchpad(launchpadId: launchpadId)
            launchpad.setNFTFormatTemplate(nftFormatTemp: nftFormatTemp)
        }
        
        pub fun setListNFTForMint(
            launchpadId: UInt64,
            tokenIds: [
                UInt64
            ],
            ...
            let launchpad = self.getLaunchpad(launchpadId: launchpadId)
            launchpad.setListNFTForMint(
                tokenIds: tokenIds,
                metadatas: metadatas
            )
        }
        
        pub fun removeNFTForMint(launchpadId: UInt64, tokenIds: [UInt64]){ 
            let launchpad = self.getLaunchpad(launchpadId: launchpadId)
            launchpad.removeNFTForMint(tokenIds: tokenIds)
        }
        ...
        pub fun setTotalAllocationStage(
            launchpadId: UInt64,
            stageId: UInt8,
            amount: UInt64
        ){ 
        ...
        pub fun setUserAllocationStage(
            launchpadId: UInt64,
            stageId: UInt8,
            whitelist: [
                Whitelist
        ...
        pub fun removeUserAllocationStage(
            launchpadId: UInt64,
            stageId: UInt8,
            accounts: [
                Address
    ...
    pub resource LaunchpadProxy{ 
        pub fun swapTicketForPoint(tickets: @[ByteNextMedalNFT.NFT]){ 
            ...
            while i < tickets.length{ 
                ...
                assert(ByteNextLaunchpadINO.configTicketPoints.containsKey(metadata["level"]!), message: "Data Ticket not match")
                let pointReceive = ByteNextLaunchpadINO.configTicketPoints[metadata["level"]!]
                totalPointReceive = totalPointReceive + (pointReceive ?? 0)
                ByteNextLaunchpadINO.swapHistory.insert(key: tokenId, (self.owner!).address)
                ticketIds.append(tokenId)
                i = i + 1
            }
            let currentPoint =
                ByteNextLaunchpadINO.userPoints.remove(
                    key: (self.owner!).address
                )
            let amountPoint = (currentPoint ?? 0) + totalPointReceive
            ByteNextLaunchpadINO.userPoints.insert(
                key: (self.owner!).address,
                amountPoint
            )
            emit SwapTicket(
        ...
        pub fun registJoin(launchpadId: UInt64, stageId: UInt8){ 
            pre{ 
                ...
                self.owner != nil:
                    "Owner should not be nil"
                ByteNextLaunchpadINO.userPoints[(self.owner!).address]! > 0:
                    "Not enough points to participate"
            }
            let launchpad =
                (&ByteNextLaunchpadINO.launchpads[launchpadId] as &Launchpad?)!
    ...
    init(){ 
        ...
        self.configTicketPoints ={ "0": 1, "1": 2, "2": 3}
        self.swapHistory ={} 
        self.launchpadCount = 0
        self.AdminStoragePath = /storage/ByteNextLaunchpadINOAdmin
        self.LaunchpadProxyStoragePath = /storage/ByteNextLaunchpadINOProxy
