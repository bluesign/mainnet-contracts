...
pub contract ByteNextBridge{ 
    ...
    pub event NonFungibleRequested(
        account: Address,
    ...
    pub event UserFungibleVerified(
        account: Address,
        tokenType: String,
        amount: UFix64,
        hash: String
    ...
    pub event UserNonFungibleVerified(
        account: Address,
        collectionType: String,
        ids: [
            UInt64
    ...
    pub event FungibleFulfilled(
        account: Address,
        tokenType: String,
        amount: UFix64
    )
    
    pub event NonFungibleFulfilled(
        account: Address,
        collectionType: String,
        ids: [
            UInt64
    ...
    priv let allowedChains:{ UInt64: Bool}
    
    priv let allowedFungibleTokens:{ String: Bool}
    
    priv let allowedNonFungibleTokens:{ String: Bool}
    
    priv let chainAddressLengths:{ UInt64: UInt64}
    
    priv let addedHashes:{ String: Bool}
    
    priv var feeTokenType: String
    
    priv let nonFungibleFees:{ UInt64: UFix64}
    
    priv var maxNftCountPerTransaction: UInt64
    
    priv var feeTokenReceiver: Capability<&AnyResource{FungibleToken.Receiver}>?
    
    priv let fungibleTokenReceiver:{ 
        String: Capability<&AnyResource{FungibleToken.Receiver}>
    }
    
    priv let nonFungibleTokenReceiver:{ 
    ...
    pub resource BridgeStore: BridgeStorePublic{ 
        priv let userVaults: @{String: FungibleToken.Vault}
        
        priv let userCollections: @{String: NonFungibleToken.Collection}
        ...
        pub fun depositNonFungible(destinationChain: UInt64, collection: @NonFungibleToken.Collection, feeTokens: @FungibleToken.Vault, recipient: String){ 
            pre{ 
                ByteNextBridge.allowedChains[destinationChain] == true:
                    "Destination chain is not supported"
                ByteNextBridge.chainAddressLengths[destinationChain] == UInt64(recipient.length):
        ...
        priv fun getTokenType(type: Type): String{ 
            return type.identifier
        }
        
        priv fun _depositBridgeVault(tokenType: String, tokens: @FungibleToken.Vault){ 
            if self.userVaults.containsKey(tokenType){ 
        ...
        priv fun _depositNonFungible(collectionType: String, collection: @NonFungibleToken.Collection){ 
            if self.userCollections.containsKey(collectionType){ 
    ...
    pub resource Administrator{ 
        ...
        pub fun setFeeTokenType(
            feeTokenType: String,
            feeTokenReceiver: Capability<&AnyResource{FungibleToken.Receiver}>
        ){ 
            ByteNextBridge.feeTokenType = feeTokenType
            ByteNextBridge.feeTokenReceiver = feeTokenReceiver
        }
        
        pub fun setFungibleReceiver(
            tokenType: String,
            fungibleTokenReceiver: Capability<
                &AnyResource{FungibleToken.Receiver}
            >
        ){ 
            ByteNextBridge.fungibleTokenReceiver[
                tokenType
            ] = fungibleTokenReceiver
        }
        
        pub fun setNonFungibleReceiver(
            collectionType: String,
            nonFungibleTokenReceiver: Capability<
                &AnyResource{NonFungibleToken.Receiver}
            >
        ){ 
            ByteNextBridge.nonFungibleTokenReceiver[
                collectionType
        ...
        pub fun setAllowedNonFungibleToken(
            collectionTypes: [
                String
            ],
            value: Bool
    ...
    init(){ 
        ...
        self.account.link<&BridgeStore{BridgeStorePublic}>(
            self.UserPublicPath,
            target: self.UserStoragePath
        )
    }
}
