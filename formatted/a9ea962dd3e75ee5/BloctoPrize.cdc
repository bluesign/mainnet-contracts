...
pub contract BloctoPrize{ 
    ...
    pub resource Admin{ 
        ...
        pub fun addWinners(id: Int, addresses: [Address], prizeIndex: Int){ 
            BloctoPrize.campaigns[id].addWinners(
                addresses: addresses,
        ...
        pub fun removeWinners(id: Int, addresses: [Address], prizeIndex: Int){ 
            BloctoPrize.campaigns[id].removeWinners(
                addresses: addresses,
                prizeIndex: prizeIndex
            )
        }
        ...
        pub fun removeToken(tokenKey: String){ 
            ...
            destroy BloctoPrize.account.load<@AnyResource>(
                from: token.vaultPath
            )
        ...
        pub fun addToken(
            ...
            BloctoPrize.account.link<
                &FungibleToken.Vault{FungibleToken.Balance}
            >(balancePath, target: vaultPath)
            BloctoPrize.account.link<
                &FungibleToken.Vault{FungibleToken.Receiver}
            >(receiverPath, target: vaultPath)
        }
    }
    ...
    pub struct Campaign{ 
        ...
        pub fun claimPrizes(address: Address){ 
            ...
            for index in prizeIndexes{ 
                ...
                let vaultRef = BloctoPrize.account.borrow<&FungibleToken.Vault>(from: prize.token.vaultPath) ?? panic("Could not borrow reference to the owner's Vault!")
                let sentVault <- vaultRef.withdraw(amount: prize.amount)
                let recipient = getAccount(address)
                let receiverRef = recipient.getCapability(prize.token.receiverPath).borrow<&{FungibleToken.Receiver}>() ?? panic("Could not borrow receiver reference to the recipient's Vault")
