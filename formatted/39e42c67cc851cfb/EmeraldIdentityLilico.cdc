...
pub contract EmeraldIdentityLilico{ 
    ...
    pub resource Administrator{ 
        ...
        pub fun removeByDiscord(discordID: String){ 
            let account =
                EmeraldIdentityLilico.getAccountFromDiscord(
                    discordID: discordID
                )
                ?? panic("This EmeraldID does not exist!")
            self.remove(account: account, discordID: discordID)
        ...
        pub fun createAdministrator(): Capability<&Administrator>{ 
            return EmeraldIdentityLilico.account.getCapability<&Administrator>(
    ...
    /*** USE THE BELOW FUNCTIONS FOR SECURE VERIFICATION OF ID ***/    pub fun getDiscordFromAccount(account: Address): String?{ 
        let admin =
            EmeraldIdentityLilico.account.borrow<&Administrator>(
                from: EmeraldIdentityLilico.AdministratorStoragePath
            )!
    ...
    init(){ 
        ...
        self.account.save(
            ...
            to: EmeraldIdentityLilico.AdministratorStoragePath
        )
        self.account.link<&Administrator>(
            EmeraldIdentityLilico.AdministratorPrivatePath,
            target: EmeraldIdentityLilico.AdministratorStoragePath
        )
    }
}
