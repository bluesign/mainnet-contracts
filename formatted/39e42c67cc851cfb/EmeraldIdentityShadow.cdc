...
pub contract EmeraldIdentityShadow{ 
    ...
    pub resource Administrator{ 
        ...
        pub fun removeByDiscord(discordID: String){ 
            let account =
                EmeraldIdentityShadow.getAccountFromDiscord(
                    discordID: discordID
                )
                ?? panic("This EmeraldID does not exist!")
            self.remove(account: account, discordID: discordID)
        ...
        pub fun createAdministrator(): Capability<&Administrator>{ 
            return EmeraldIdentityShadow.account.getCapability<&Administrator>(
    ...
    /*** USE THE BELOW FUNCTIONS FOR SECURE VERIFICATION OF ID ***/    pub fun getDiscordFromAccount(account: Address): String?{ 
        let admin =
            EmeraldIdentityShadow.account.borrow<&Administrator>(
    ...
    init(){ 
        ...
        self.account.link<&Administrator>(
            EmeraldIdentityShadow.AdministratorPrivatePath,
            target: EmeraldIdentityShadow.AdministratorStoragePath
        )
    }
}
