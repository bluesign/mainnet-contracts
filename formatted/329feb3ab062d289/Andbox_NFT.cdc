...
pub contract Andbox_NFT: NonFungibleToken{ 
    ...
    // Andbox_NFT Events
    //
    // Emitted when the Andbox_NFT contract is created
    ...
    // A Series is special resource type that contains functions to mint Andbox_NFT NFTs, 
    ...
    pub resource Series{ 
        ...
        init(seriesId: UInt32, metadata:{ String: String}){ 
            ...
            self.setIds = []
            self.setSealedState ={} 
            Andbox_NFT.seriesData[seriesId] = SeriesData(seriesId: seriesId, metadata: metadata)
        ...
        //
        pub fun mintAndbox_NFT(recipient: &{NonFungibleToken.CollectionPublic}, tokenId: UInt64, setId: UInt32){ 
            pre{ 
            ...
            recipient.deposit(token: <-create Andbox_NFT.NFT(tokenId: tokenId, setId: setId, editionNum: editionNum))
        ...
        // mintEditionAndbox_NFT
        // Mints a new NFT with a new ID and specific edition Num (random open edition)
        // and deposits it in the recipients collection using their collection reference
        //
        pub fun mintEditionAndbox_NFT(recipient: &{NonFungibleToken.CollectionPublic}, tokenId: UInt64, setId: UInt32, edition: UInt32){ 
            pre{ 
                self.numberEditionsMintedPerSet[setId] != nil:
                    "The Set does not exist."
                self.numberEditionsMintedPerSet[setId]! <= Andbox_NFT.getSetMaxEditions(setId: setId)!:
            ...
            // deposit it in the recipient's account using their reference
            recipient.deposit(token: <-create Andbox_NFT.NFT(tokenId: tokenId, setId: setId, editionNum: edition))
        ...
        pub fun batchMintAndbox_NFT(recipient: &{NonFungibleToken.CollectionPublic}, setId: UInt32, tokenIds: [UInt64]){ 
            pre{ 
                tokenIds.length > 0:
                    "Number of token Ids must be > 0"
            }
            for tokenId in tokenIds{ 
                self.mintAndbox_NFT(recipient: recipient, tokenId: tokenId, setId: setId)
            }
        }
    ...
    // A resource that represents the Andbox_NFT NFT
    //
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver{ 
        ...
        init(tokenId: UInt64, setId: UInt32, editionNum: UInt32){ 
            ...
            let seriesId = Andbox_NFT.getSetSeriesId(setId: setId)!
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                case Type<MetadataViews.Display>():
                    return MetadataViews.Display(name: Andbox_NFT.getSetMetadataByField(setId: self.setId, field: "name")!, description: Andbox_NFT.getSetMetadataByField(setId: self.setId, field: "description")!, thumbnail: MetadataViews.HTTPFile(url: Andbox_NFT.getSetMetadataByField(setId: self.setId, field: "preview")!))
                case Type<MetadataViews.Serial>():
                    return MetadataViews.Serial(self.id)
                case Type<MetadataViews.Editions>():
                    let maxEditions = Andbox_NFT.setData[self.setId]?.maxEditions ?? 0
                    let editionName = Andbox_NFT.getSetMetadataByField(setId: self.setId, field: "name")!
                ...
                case Type<MetadataViews.ExternalURL>():
                    if let externalBaseURL = Andbox_NFT.getSetMetadataByField(setId: self.setId, field: "external_token_base_url"){ 
                ...
                case Type<MetadataViews.Royalties>():
                    ...
                    for metadataKey in (Andbox_NFT.getSetMetadata(setId: self.setId)!).keys{ 
                        ...
                        if metadataKey.length >= 13{ 
                            if metadataKey.slice(from: 0, upTo: 13) == "royalty_addr_"{ 
                                ...
                                let royaltyAddress = Andbox_NFT.convertStringToAddress(Andbox_NFT.getSetMetadataByField(setId: self.setId, field: "royalty_addr_".concat(royaltyName))!)!
                                let royaltyReceiver: PublicPath = PublicPath(identifier: Andbox_NFT.getSetMetadataByField(setId: self.setId, field: "royalty_rcv_".concat(royaltyName))!)!
                                let royaltyCut = Andbox_NFT.getSetMetadataByField(setId: self.setId, field: "royalty_cut_".concat(royaltyName))!
                                let cutValue: UFix64 = Andbox_NFT.royaltyCutStringToUFix64(royaltyCut)
                                if cutValue != 0.0{ 
                                    royalties.append(MetadataViews.Royalty(receiver: getAccount(royaltyAddress).getCapability<&FungibleToken.Vault{FungibleToken.Receiver}>(royaltyReceiver), cut: cutValue, description: Andbox_NFT.getSetMetadataByField(setId: self.setId, field: "royalty_desc_".concat(royaltyName))!))
                                }
                            }
                        }
                    }
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: Andbox_NFT.CollectionStoragePath, publicPath: Andbox_NFT.CollectionPublicPath, providerPath: /private/Andbox_NFT, publicCollection: Type<&Andbox_NFT.Collection{Andbox_NFT.Andbox_NFTCollectionPublic, NonFungibleToken.CollectionPublic}>(), publicLinkedType: Type<&Andbox_NFT.Collection{Andbox_NFT.Andbox_NFTCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&Andbox_NFT.Collection{Andbox_NFT.Andbox_NFTCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
                            return <-Andbox_NFT.createEmptyCollection()
                        })
                case Type<MetadataViews.NFTCollectionDisplay>():
                    let squareImage = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://media.gigantik.io/andbox/square.png"), mediaType: "image/png")
                    let bannerImage = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://media.gigantik.io/andbox/banner.png"), mediaType: "image/png")
                    ...
                    for metadataKey in (Andbox_NFT.getSetMetadata(setId: self.setId)!).keys{ 
                        ...
                        if metadataKey.length >= 18{ 
                            if metadataKey.slice(from: 0, upTo: 18) == "collection_social_"{ 
                                ...
                                socials.insert(key: metadataKey.slice(from: 18, upTo: metadataKey.length), MetadataViews.ExternalURL(Andbox_NFT.getSetMetadataByField(setId: self.setId, field: metadataKey)!))
                            }
                        }
                    }
                    return MetadataViews.NFTCollectionDisplay(name: Andbox_NFT.getSetMetadataByField(setId: self.setId, field: "collection_name") ?? "", description: Andbox_NFT.getSetMetadataByField(setId: self.setId, field: "collection_description") ?? "", externalURL: MetadataViews.ExternalURL(Andbox_NFT.getSetMetadataByField(setId: self.setId, field: "external_url") ?? ""), squareImage: squareImage, bannerImage: bannerImage, socials: socials)
                    ...
                    for metadataKey in (Andbox_NFT.getSetMetadata(setId: self.setId)!).keys{ 
                        ...
                        if metadataKey.length >= 6{ 
                            if metadataKey.slice(from: 0, upTo: 6) == "trait_"{ 
                                ...
                                traitDictionary.insert(key: metadataKey.slice(from: 6, upTo: metadataKey.length), Andbox_NFT.getSetMetadataByField(setId: self.setId, field: metadataKey)!)
                            }
                        }
                    }
                    return MetadataViews.dictToTraits(dict: traitDictionary, excludedNames: [])
                case Type<MetadataViews.Medias>():
                    return MetadataViews.Medias(items: [MetadataViews.Media(file: MetadataViews.HTTPFile(url: Andbox_NFT.getSetMetadataByField(setId: self.setId, field: "image")!), mediaType: self.getMimeType())])
            }
            return nil
        }
        
        pub fun getMimeType(): String{ 
            var metadataFileType = (Andbox_NFT.getSetMetadataByField(setId: self.setId, field: "image_file_type")!).toLower()
    ...
    //
    pub resource Collection: Andbox_NFTCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection{ 
        ...
        pub fun borrowViewResolver(id: UInt64): &AnyResource{MetadataViews.Resolver}{ 
            ...
            let Andbox_NFTNft = nft as! &Andbox_NFT.NFT
            return Andbox_NFTNft as &AnyResource{MetadataViews.Resolver}
        }
    ...
    pub fun fetch(_ from: Address, id: UInt64): &Andbox_NFT.NFT?{ 
        let collection = getAccount(from).getCapability(Andbox_NFT.CollectionPublicPath).borrow<&Andbox_NFT.Collection{Andbox_NFT.Andbox_NFTCollectionPublic}>() ?? panic("Couldn't get collection")
    ...
    init(){ 
        ...
        self.account.link<&Andbox_NFT.Admin>(self.AdminPrivatePath, target: self.AdminStoragePath) ?? panic("Could not get a capability to the admin")
