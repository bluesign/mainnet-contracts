...
pub contract RaceDay_NFT: NonFungibleToken{ 
    ...
    // RaceDay_NFT Events
    //
    // Emitted when the RaceDay_NFT contract is created
    ...
    // A Series is special resource type that contains functions to mint RaceDay_NFT NFTs, 
    ...
    pub resource Series{ 
        ...
        init(seriesId: UInt32, metadata:{ String: String}){ 
            ...
            self.setIds = []
            self.setSealedState ={} 
            RaceDay_NFT.seriesData[seriesId] = SeriesData(seriesId: seriesId, metadata: metadata)
        ...
        //
        pub fun mintRaceDay_NFT(recipient: &{NonFungibleToken.CollectionPublic}, tokenId: UInt64, setId: UInt32){ 
            pre{ 
            ...
            recipient.deposit(token: <-create RaceDay_NFT.NFT(tokenId: tokenId, setId: setId, editionNum: editionNum))
        ...
        // mintEditionRaceDay_NFT
        // Mints a new NFT with a new ID and specific edition Num (random open edition)
        // and deposits it in the recipients collection using their collection reference
        //
        pub fun mintEditionRaceDay_NFT(recipient: &{NonFungibleToken.CollectionPublic}, tokenId: UInt64, setId: UInt32, edition: UInt32){ 
            pre{ 
                self.numberEditionsMintedPerSet[setId] != nil:
                    "The Set does not exist."
                self.numberEditionsMintedPerSet[setId]! <= RaceDay_NFT.getSetMaxEditions(setId: setId)!:
            ...
            // deposit it in the recipient's account using their reference
            recipient.deposit(token: <-create RaceDay_NFT.NFT(tokenId: tokenId, setId: setId, editionNum: edition))
        ...
        pub fun batchMintRaceDay_NFT(recipient: &{NonFungibleToken.CollectionPublic}, setId: UInt32, tokenIds: [UInt64]){ 
            pre{ 
                tokenIds.length > 0:
                    "Number of token Ids must be > 0"
            }
            for tokenId in tokenIds{ 
                self.mintRaceDay_NFT(recipient: recipient, tokenId: tokenId, setId: setId)
            }
        }
    ...
    // A resource that represents the RaceDay_NFT NFT
    //
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver{ 
        ...
        init(tokenId: UInt64, setId: UInt32, editionNum: UInt32){ 
            ...
            let seriesId = RaceDay_NFT.getSetSeriesId(setId: setId)!
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                case Type<MetadataViews.Display>():
                    return MetadataViews.Display(name: RaceDay_NFT.getSetMetadataByField(setId: self.setId, field: "name")!, description: RaceDay_NFT.getSetMetadataByField(setId: self.setId, field: "description")!, thumbnail: MetadataViews.HTTPFile(url: RaceDay_NFT.getSetMetadataByField(setId: self.setId, field: "preview")!))
                case Type<MetadataViews.Serial>():
                    return MetadataViews.Serial(self.id)
                case Type<MetadataViews.Editions>():
                    let maxEditions = RaceDay_NFT.setData[self.setId]?.maxEditions ?? 0
                    let editionName = RaceDay_NFT.getSetMetadataByField(setId: self.setId, field: "name")!
                ...
                case Type<MetadataViews.ExternalURL>():
                    if let externalBaseURL = RaceDay_NFT.getSetMetadataByField(setId: self.setId, field: "external_token_base_url"){ 
                ...
                case Type<MetadataViews.Royalties>():
                    ...
                    for metadataKey in (RaceDay_NFT.getSetMetadata(setId: self.setId)!).keys{ 
                        ...
                        if metadataKey.length >= 13{ 
                            if metadataKey.slice(from: 0, upTo: 13) == "royalty_addr_"{ 
                                ...
                                let royaltyAddress = RaceDay_NFT.convertStringToAddress(RaceDay_NFT.getSetMetadataByField(setId: self.setId, field: "royalty_addr_".concat(royaltyName))!)!
                                let royaltyReceiver: PublicPath = PublicPath(identifier: RaceDay_NFT.getSetMetadataByField(setId: self.setId, field: "royalty_rcv_".concat(royaltyName))!)!
                                let royaltyCut = RaceDay_NFT.getSetMetadataByField(setId: self.setId, field: "royalty_cut_".concat(royaltyName))!
                                let cutValue: UFix64 = RaceDay_NFT.royaltyCutStringToUFix64(royaltyCut)
                                if cutValue != 0.0{ 
                                    royalties.append(MetadataViews.Royalty(receiver: getAccount(royaltyAddress).getCapability<&FungibleToken.Vault{FungibleToken.Receiver}>(royaltyReceiver), cut: cutValue, description: RaceDay_NFT.getSetMetadataByField(setId: self.setId, field: "royalty_desc_".concat(royaltyName))!))
                                }
                            }
                        }
                    }
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: RaceDay_NFT.CollectionStoragePath, publicPath: RaceDay_NFT.CollectionPublicPath, providerPath: /private/RaceDay_NFT, publicCollection: Type<&RaceDay_NFT.Collection{RaceDay_NFT.RaceDay_NFTCollectionPublic, NonFungibleToken.CollectionPublic}>(), publicLinkedType: Type<&RaceDay_NFT.Collection{RaceDay_NFT.RaceDay_NFTCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&RaceDay_NFT.Collection{RaceDay_NFT.RaceDay_NFTCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
                            return <-RaceDay_NFT.createEmptyCollection()
                        })
                case Type<MetadataViews.NFTCollectionDisplay>():
                    let squareImage = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://media.gigantik.io/raceday/square.png"), mediaType: "image/png")
                    let bannerImage = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://media.gigantik.io/raceday/banner.png"), mediaType: "image/png")
                    ...
                    for metadataKey in (RaceDay_NFT.getSetMetadata(setId: self.setId)!).keys{ 
                        ...
                        if metadataKey.length >= 18{ 
                            if metadataKey.slice(from: 0, upTo: 18) == "collection_social_"{ 
                                ...
                                socials.insert(key: metadataKey.slice(from: 18, upTo: metadataKey.length), MetadataViews.ExternalURL(RaceDay_NFT.getSetMetadataByField(setId: self.setId, field: metadataKey)!))
                            }
                        }
                    }
                    return MetadataViews.NFTCollectionDisplay(name: RaceDay_NFT.getSetMetadataByField(setId: self.setId, field: "collection_name") ?? "", description: RaceDay_NFT.getSetMetadataByField(setId: self.setId, field: "collection_description") ?? "", externalURL: MetadataViews.ExternalURL(RaceDay_NFT.getSetMetadataByField(setId: self.setId, field: "external_url") ?? ""), squareImage: squareImage, bannerImage: bannerImage, socials: socials)
                    ...
                    for metadataKey in (RaceDay_NFT.getSetMetadata(setId: self.setId)!).keys{ 
                        ...
                        if metadataKey.length >= 6{ 
                            if metadataKey.slice(from: 0, upTo: 6) == "trait_"{ 
                                ...
                                traitDictionary.insert(key: metadataKey.slice(from: 6, upTo: metadataKey.length), RaceDay_NFT.getSetMetadataByField(setId: self.setId, field: metadataKey)!)
                            }
                        }
                    }
                    return MetadataViews.dictToTraits(dict: traitDictionary, excludedNames: [])
                case Type<MetadataViews.Medias>():
                    return MetadataViews.Medias(items: [MetadataViews.Media(file: MetadataViews.HTTPFile(url: RaceDay_NFT.getSetMetadataByField(setId: self.setId, field: "image")!), mediaType: self.getMimeType())])
            }
            return nil
        }
        
        pub fun getMimeType(): String{ 
            var metadataFileType = (RaceDay_NFT.getSetMetadataByField(setId: self.setId, field: "image_file_type")!).toLower()
    ...
    //
    pub resource Collection: RaceDay_NFTCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection{ 
        ...
        pub fun borrowViewResolver(id: UInt64): &AnyResource{MetadataViews.Resolver}{ 
            ...
            let RaceDay_NFTNft = nft as! &RaceDay_NFT.NFT
            return RaceDay_NFTNft as &AnyResource{MetadataViews.Resolver}
        }
    ...
    pub fun fetch(_ from: Address, id: UInt64): &RaceDay_NFT.NFT?{ 
        let collection = getAccount(from).getCapability(RaceDay_NFT.CollectionPublicPath).borrow<&RaceDay_NFT.Collection{RaceDay_NFT.RaceDay_NFTCollectionPublic}>() ?? panic("Couldn't get collection")
    ...
    init(){ 
        ...
        self.account.link<&RaceDay_NFT.Admin>(self.AdminPrivatePath, target: self.AdminStoragePath) ?? panic("Could not get a capability to the admin")
