...
pub contract BreakingT_NFT: NonFungibleToken{ 
    ...
    // BreakingT_NFT Events
    //
    // Emitted when the BreakingT_NFT contract is created
    ...
    // A Series is special resource type that contains functions to mint BreakingT_NFT NFTs, 
    ...
    pub resource Series{ 
        ...
        init(seriesId: UInt32, metadata:{ String: String}){ 
            ...
            self.setIds = []
            self.setSealedState ={} 
            BreakingT_NFT.seriesData[seriesId] = SeriesData(seriesId: seriesId, metadata: metadata)
        ...
        //
        pub fun mintBreakingT_NFT(recipient: &{NonFungibleToken.CollectionPublic}, tokenId: UInt64, setId: UInt32){ 
            pre{ 
            ...
            recipient.deposit(token: <-create BreakingT_NFT.NFT(tokenId: tokenId, setId: setId, editionNum: editionNum))
        ...
        // and deposits it in the recipients collection using their collection reference
        //
        pub fun mintEditionBreakingT_NFT(recipient: &{NonFungibleToken.CollectionPublic}, tokenId: UInt64, setId: UInt32, edition: UInt32){ 
            pre{ 
                self.numberEditionsMintedPerSet[setId] != nil:
                    "The Set does not exist."
                self.numberEditionsMintedPerSet[setId]! <= BreakingT_NFT.getSetMaxEditions(setId: setId)!:
            ...
            // deposit it in the recipient's account using their reference
            recipient.deposit(token: <-create BreakingT_NFT.NFT(tokenId: tokenId, setId: setId, editionNum: edition))
        ...
        pub fun batchMintBreakingT_NFT(recipient: &{NonFungibleToken.CollectionPublic}, setId: UInt32, tokenIds: [UInt64]){ 
            pre{ 
                tokenIds.length > 0:
                    "Number of token Ids must be > 0"
            }
            for tokenId in tokenIds{ 
                self.mintBreakingT_NFT(recipient: recipient, tokenId: tokenId, setId: setId)
            }
        }
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                case Type<MetadataViews.Display>():
                    return MetadataViews.Display(name: BreakingT_NFT.getSetMetadataByField(setId: self.setId, field: "name")!, description: BreakingT_NFT.getSetMetadataByField(setId: self.setId, field: "description")!, thumbnail: MetadataViews.HTTPFile(url: BreakingT_NFT.getSetMetadataByField(setId: self.setId, field: "preview")!))
                    ...
                    for metadataKey in (BreakingT_NFT.getSetMetadata(setId: self.setId)!).keys{ 
                        ...
                        if metadataKey.length >= 13{ 
                            if metadataKey.slice(from: 0, upTo: 13) == "royalty_addr_"{ 
                                ...
                                if cutValue != 0.0{ 
                                    royalties.append(MetadataViews.Royalty(receiver: getAccount(royaltyAddress).getCapability<&FungibleToken.Vault{FungibleToken.Receiver}>(royaltyReceiver), cut: cutValue, description: BreakingT_NFT.getSetMetadataByField(setId: self.setId, field: "royalty_desc_".concat(royaltyName))!))
                                }
                            }
                        }
                    }
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: BreakingT_NFT.CollectionStoragePath, publicPath: BreakingT_NFT.CollectionPublicPath, providerPath: /private/BreakingT_NFT, publicCollection: Type<&BreakingT_NFT.Collection{BreakingT_NFT.BreakingT_NFTCollectionPublic, NonFungibleToken.CollectionPublic}>(), publicLinkedType: Type<&BreakingT_NFT.Collection{BreakingT_NFT.BreakingT_NFTCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&BreakingT_NFT.Collection{BreakingT_NFT.BreakingT_NFTCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
                            return <-BreakingT_NFT.createEmptyCollection()
                        })
                case Type<MetadataViews.NFTCollectionDisplay>():
                    let squareImage = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://media.gigantik.io/breakingt/square.png"), mediaType: "image/png")
                    let bannerImage = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://media.gigantik.io/breakingt/banner.png"), mediaType: "image/png")
                    ...
                    return MetadataViews.NFTCollectionDisplay(name: BreakingT_NFT.getSetMetadataByField(setId: self.setId, field: "collection_name") ?? "", description: BreakingT_NFT.getSetMetadataByField(setId: self.setId, field: "collection_description") ?? "", externalURL: MetadataViews.ExternalURL(BreakingT_NFT.getSetMetadataByField(setId: self.setId, field: "external_url") ?? ""), squareImage: squareImage, bannerImage: bannerImage, socials: socials)
                ...
                case Type<MetadataViews.Medias>():
                    return MetadataViews.Medias(items: [MetadataViews.Media(file: MetadataViews.HTTPFile(url: BreakingT_NFT.getSetMetadataByField(setId: self.setId, field: "image")!), mediaType: self.getMimeType())])
            }
            return nil
        }
    ...
    pub fun fetch(_ from: Address, id: UInt64): &BreakingT_NFT.NFT?{ 
        let collection = getAccount(from).getCapability(BreakingT_NFT.CollectionPublicPath).borrow<&BreakingT_NFT.Collection{BreakingT_NFT.BreakingT_NFTCollectionPublic}>() ?? panic("Couldn't get collection")
    ...
    init(){ 
        ...
        self.account.link<&BreakingT_NFT.Admin>(self.AdminPrivatePath, target: self.AdminStoragePath) ?? panic("Could not get a capability to the admin")
