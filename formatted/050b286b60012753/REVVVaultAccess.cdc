...
pub contract REVVVaultAccess{ 
    ...
    pub resource VaultGuard{ 
        ...
        //
        priv let vaultCapability: Capability<
            &REVV.Vault{FungibleToken.Provider}
        >
        ...
        //
        init(
            vaultCapability: Capability<&REVV.Vault{FungibleToken.Provider}>,
            max: UFix64
        ){ 
            pre{ 
                max > 0.0:
    ...
    pub fun createVaultGuard(
        ...
        let vaultCapability =
            self.account.getCapability<&REVV.Vault{FungibleToken.Provider}>(
                REVV.RevvVaultPrivatePath
            )
        let guard <-
            create VaultGuard(vaultCapability: vaultCapability, max: maxAmount)
        self.account.save(<-guard, to: guardStoragePath)
        self.account.link<&VaultGuard>(
            guardPrivatePath,
            target: guardStoragePath
        )
        let pathObject =
            VaultGuardPaths(
                storagePath: guardStoragePath,
                privatePath: guardPrivatePath
            )
    ...
    pub fun getMaxAmountForAccount(vaultProxyAddress: Address): UFix64{ 
        ...
        let capability =
            self.account.getCapability<&REVVVaultAccess.VaultGuard>(
                paths.privatePath
            )
    ...
    pub fun revokeVaultGuard(adminRef: &Admin, vaultProxyAddress: Address){ 
        ...
        let paths =
            self.getVaultGuardPaths(vaultProxyAddress: vaultProxyAddress)!
        ...
        //delete guard
        let guard <-
            self.account.load<@REVVVaultAccess.VaultGuard>(
                from: paths.storagePath
            )!
    ...
    pub resource VaultProxy: VaultProxyPublic{ 
        priv var vaultGuardCap: Capability<&REVVVaultAccess.VaultGuard>?
