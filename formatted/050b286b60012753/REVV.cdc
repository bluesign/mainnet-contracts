...
pub contract REVV: FungibleToken{ 
    ...
    init(){ 
        ...
        //
        self.account.link<&REVV.Vault{FungibleToken.Receiver}>(self.RevvReceiverPublicPath, target: self.RevvVaultStoragePath)
        
        // Create a public capability to the stored Vault that only exposes
        // the `balance` field through the `Balance` interface
        //
        self.account.link<&REVV.Vault{FungibleToken.Balance}>(self.RevvBalancePublicPath, target: self.RevvVaultStoragePath)
        self.account.link<&REVV.Vault{FungibleToken.Provider}>(self.RevvVaultPrivatePath, target: self.RevvVaultStoragePath)
