...
pub contract FrontRowStorefront{ 
    ...
    pub struct SaleOfferDetails{ 
        ...
        init(
            ...
            let blueprint: FrontRow.Blueprint? =
                FrontRow.getBlueprint(id: blueprintId)
    ...
    pub resource SaleOffer: SaleOfferPublic{ 
        ...
        priv let details: SaleOfferDetails
        ...
        access(contract) let nftProviderCapability: Capability<&{NonFungibleToken.Provider, FrontRow.CollectionPublic}>
        ...
        pub fun purchase(payment: @FUSD.Vault): @FrontRow.NFT{ 
            ...
            assert(payment.balance == self.details.price, message: "Payment vault doesn't have enough funds.")
            ...
            let nft: @FrontRow.NFT <- (self.minterCapability.borrow()!).mintNFT(blueprintId: blueprint.id)
            ...
            assert(nft.isInstance(Type<@FrontRow.NFT>()), message: "Withdrawn NFT is not of FrontRow.NFT type.")
            
            // NFT minted: check the blueprint ID and the serial number
            assert(nft.blueprintId == blueprint.id && nft.serialNumber == nftSerialNumber, message: "Selling incorrect NFT: serial number or/and blueprint are incorrect.")
            ...
            emit Purchase(blueprintId: self.details.blueprintId, serialNumber: nftSerialNumber,                                                                                                // If all minted NFTs for a bluprint are purchased, the offer is soldOut.
                                                                                                soldOut: nftSerialNumber == blueprint.maxQuantity)
            return <-nft
        }
        
        // Initialize resource fields
        init(nftProviderCapability: Capability<&{NonFungibleToken.Provider, FrontRow.CollectionPublic}>, blueprintId: UInt32, price: UFix64, beneficiary: Capability<&{FungibleToken.Receiver}>, minterCapability: Capability<&{FrontRow.Minter}>){ 
            //
            ...
            self.details = SaleOfferDetails(blueprintId: blueprintId, price: price, beneficiary: beneficiary)
    ...
    pub resource interface StorefrontPublic{ 
        ...
        //
        pub fun borrowSaleOffer(blueprintId: UInt32): &SaleOffer{
            SaleOfferPublic
        }?{} 
    }
    ...
    pub resource Storefront: StorefrontManager, StorefrontPublic{ 
        ...
        priv var saleOffers: @{UInt32: SaleOffer}
        ...
        //
        pub fun createSaleOffer(nftProviderCapability: Capability<&{NonFungibleToken.Provider, FrontRow.CollectionPublic}>, blueprintId: UInt32, price: UFix64, beneficiary: Capability<&{FungibleToken.Receiver}>, minterCapability: Capability<&{FrontRow.Minter}>){ 
            //
            let blueprint: FrontRow.Blueprint? = FrontRow.getBlueprint(id: blueprintId)
            ...
            // Create a sale offer
            let saleOffer <- create SaleOffer(nftProviderCapability: nftProviderCapability, blueprintId: blueprintId, price: price, beneficiary: beneficiary, minterCapability: minterCapability)
            ...
            destroy oldOffer
            emit SaleOfferAvailable(storefrontAddress: self.owner?.address!, blueprintId: blueprintId, price: price)
        }
        ...
        pub fun removeSaleOffer(blueprintId: UInt32){ 
            let blueprint: FrontRow.Blueprint? = FrontRow.getBlueprint(id: blueprintId)
            assert(blueprint != nil, message: "Blueprint doesn't exist.")
            let offer <- self.saleOffers.remove(key: blueprintId) ?? panic("Missing sale offer.")
            ...
            emit SaleOfferRemoved(blueprintId: blueprintId, serialNumber: saleOfferDetails.sold, soldOut: saleOfferDetails.sold == (blueprint!).maxQuantity)
        }
    ...
    init(){ 
        ...
        self.account.link<
            &FrontRowStorefront.Storefront{FrontRowStorefront.StorefrontPublic}
        >(
