...
pub contract IncrementPointsV2{ 
    ...
    //
    priv let _balances:{ Address: UFix64}
    
    // 
    priv let _balancesHistorySnapshot:{ Address: UFix64}
    
    // 在黑名单里的地址，不会再有积分累计，积分暂时不会清零
    priv let _userBlacklist:{ Address: Bool}
    
    // 
    priv let _swapPoolWhitelist:{ Address: Bool} // {PoolAddress}
    ...
    pub fun balanceOf(_ userAddr: Address): UFix64{ 
        return (
            self._balances.containsKey(userAddr)
                ? self._balances[userAddr]!
                : 0.0
        )
        + (
            self._balancesHistorySnapshot.containsKey(userAddr)
                ? self._balances[userAddr]!
                : 0.0
        )
    }
    ...
    // 估算当前实时的points数量
    pub fun balanceOfRealTime(_ userAddr: Address): UFix64{ 
        return self.balanceOf(userAddr) // base points
        + self.calculateNewPointsSinceLastUpdate(
            userAddr: userAddr,
            ifLog: false
        )
        as!
        UFix64 // accured points
    }
    
    // 
    pub fun calculateNewPointsSinceLastUpdate(
        userAddr: Address,
        ifLog: Bool
    ): AnyStruct{ 
        ...
        // Accrued new points
        if lastUpdateTimestamp != 0.0
        && currUpdateTimestamp - lastUpdateTimestamp > 0.0{ 
            let duration = currUpdateTimestamp - lastUpdateTimestamp
            let timeRate = duration
            
            // Lending Points
            let currTotalSupplyAmountInUsd =
                (self._userStates[userAddr]!)["LendingTotalSupplyInUsd"]!
            let currTotalBorrowAmountInUsd =
                (self._userStates[userAddr]!)["LendingTotalBorrowInUsd"]!
            let mintAmountByLendingSupply =
                currTotalSupplyAmountInUsd
                * self.getPointsRatePerDay_LendingSupply(
                    amount: currTotalSupplyAmountInUsd
                )
                / self._secondsPerDay
                * timeRate
            let mintAmountbyLendingBorrow =
                currTotalBorrowAmountInUsd
                * self.getPointsRatePerDay_LendingBorrow(
                    amount: currTotalBorrowAmountInUsd
                )
                / self._secondsPerDay
                * timeRate
            if mintAmountByLendingSupply > 0.0 && ifLog{ 
                res["LendingSupply"] ={ "AccuredPoints": mintAmountByLendingSupply, "SupplyUsdValue": currTotalSupplyAmountInUsd, "Duration": duration}
            }
            if mintAmountbyLendingBorrow > 0.0 && ifLog{ 
                res["LendingBorrow"] ={ "AccuredPoints": mintAmountbyLendingBorrow, "BorrowUsdValue": currTotalBorrowAmountInUsd, "Duration": duration}
            }
            
            // Liquid Staking Points
            let stFlowHoldingBalance: UFix64 =
                self.getUserState_stFlowHoldingBalance(userAddr: userAddr)
            let mintAmountByHoldingStFlow =
                stFlowHoldingBalance
                * self.getPointsRatePerDay_LiquidStaking(
                    amount: stFlowHoldingBalance
                )
                / self._secondsPerDay
                * timeRate
            if mintAmountByHoldingStFlow > 0.0 && ifLog{ 
                res["stFlowHolding"] ={ "AccuredPoints": mintAmountByHoldingStFlow, "stFlowHoldingBalance": stFlowHoldingBalance, "Duration": duration}
            }
            
            // Accured amount to mint
            totalAccuredPoints = mintAmountByLendingSupply
                + mintAmountbyLendingBorrow
                + mintAmountByHoldingStFlow
            res["TotalAccuredPoints"] = totalAccuredPoints
        }
        if ifLog{ 
            return res
        } else{ 
            return totalAccuredPoints
        }
    }
    ...
    pub fun updateUserState(userAddr: Address){ 
        ...
        //
        let accuredRes:{ String: AnyStruct} =
            self.calculateNewPointsSinceLastUpdate(
                userAddr: userAddr,
                ifLog: true
            )
        ...
        if accuredRes.containsKey("LendingSupply"){ 
            ...
            emit PointsMinted(userAddr: userAddr, amount: logInfo["AccuredPoints"]! as! UFix64, source: "LendingSupply", param:{ "SupplyUsdValue": (logInfo["SupplyUsdValue"]! as! UFix64).toString(), "Duration": (logInfo["Duration"]! as! UFix64).toString()})
        }
        if accuredRes.containsKey("LendingBorrow"){ 
            let logInfo = accuredRes["LendingBorrow"]! as!{ String: AnyStruct}
            emit PointsMinted(userAddr: userAddr, amount: logInfo["AccuredPoints"]! as! UFix64, source: "LendingBorrow", param:{ "BorrowUsdValue": (logInfo["BorrowUsdValue"]! as! UFix64).toString(), "Duration": (logInfo["Duration"]! as! UFix64).toString()})
        }
        if accuredRes.containsKey("stFlowHolding"){ 
            let logInfo = accuredRes["stFlowHolding"]! as!{ String: AnyStruct}
            emit PointsMinted(userAddr: userAddr, amount: logInfo["AccuredPoints"]! as! UFix64, source: "stFlowHolding", param:{ "stFlowHoldingBalance": (logInfo["stFlowHoldingBalance"]! as! UFix64).toString(), "Duration": (logInfo["Duration"]! as! UFix64).toString()})
        }
        ...
        || stFlowTotalBalance > 0.0{ 
            if self._userStates.containsKey(userAddr) == false{ 
                self._userStates[userAddr] ={ "LastMintTimestamp": 0.0, "LendingTotalSupplyInUsd": 0.0, "LendingTotalBorrowInUsd": 0.0, "stFlowHoldingBalance": 0.0 // 包含账户余额，farm池子等}
            }
            ...
            self.setUserState_stFlowHoldingBalance(
                userAddr: userAddr,
                stFlowBalance: stFlowTotalBalance
            )
            (self._userStates[userAddr]!).insert(
                key: "LastMintTimestamp",
    ...
    pub fun endVolumeTrackingAndMintPoints(userAddr: Address){ 
        ...
        let mintAmountBySwapVolume =
            volumeUsd * self.getPointsRatePerDay_SwapVolume()
    ...
    pub fun getBalanceLength(): Int{ 
        return self._balances.length
    }
    
    pub fun getSlicedBalances(from: UInt64, to: UInt64):{ Address: UFix64}{ 
    ...
    pub fun getPointsRatePerDay_LiquidStaking(amount: UFix64): UFix64{ 
    ...
    pub fun getPointsRatePerDay_LendingSupply(amount: UFix64): UFix64{ 
        return self.calculateTierRateByAmount(
            amount: amount,
    ...
    pub fun getPointsRatePerDay_LendingBorrow(amount: UFix64): UFix64{ 
        return self.calculateTierRateByAmount(
            amount: amount,
    ...
    pub fun getPointsRatePerDay_SwapLP(): UFix64{ 
        return self._pointsRatePerDay["SwapLP"]! as! UFix64
    }
    
    pub fun getPointsRatePerDay_SwapVolume(): UFix64{ 
    ...
    pub fun getUserState_stFlowHoldingBalance(userAddr: Address): UFix64{ 
        return (self._userStates[userAddr]!).containsKey("stFlowHoldingBalance")
            ? (self._userStates[userAddr]!)["stFlowHoldingBalance"]!
            : 0.0
    }
    
    priv fun setUserState_stFlowHoldingBalance(
        userAddr: Address,
        stFlowBalance: UFix64
    ){ 
        (self._userStates[userAddr]!).insert(
            key: "stFlowHoldingBalance",
            stFlowBalance
        )
    }
    ...
    ///
    pub resource Admin{ 
        pub fun setPointsRatePerDay_LiquidStakingTier(
            tierRate:{ 
                UFix64: UFix64
            }
        ){ 
            emit PointsTierRateChanged(
                ...
                ori: IncrementPointsV2._pointsRatePerDay["LiquidStaking"]!
                as!{
        ...
        pub fun setPointsRatePerDay_LendingSupplyTier(
            tierRate:{ 
                UFix64: UFix64
            }
        ){ 
            emit PointsTierRateChanged(
                ...
                ori: IncrementPointsV2._pointsRatePerDay["LendingSupply"]!
                as!{
        ...
        pub fun setPointsRatePerDay_LendingBorrowTier(
            tierRate:{ 
                UFix64: UFix64
            }
        ){ 
            emit PointsTierRateChanged(
                ...
                ori: IncrementPointsV2._pointsRatePerDay["LendingBorrow"]!
                as!{
        ...
        pub fun setPointsRatePerDay_SwapLP(rate: UFix64){ 
            emit PointsRateChanged(
                source: "SwapLP",
                ori: IncrementPointsV2._pointsRatePerDay["SwapLP"]! as! UFix64,
                new: rate
            )
            IncrementPointsV2._pointsRatePerDay["SwapLP"] = rate
        }
        
        pub fun setPointsRatePerDay_SwapVolume(rate: UFix64){ 
            emit PointsRateChanged(
                source: "SwapVolume",
                ori: IncrementPointsV2._pointsRatePerDay["SwapVolume"]!
                as!
                UFix64,
                new: rate
            )
        ...
        pub fun setHistorySnapshotPointsBalance(
            userAddr: Address,
            amount: UFix64
        ){ 
            if IncrementPointsV2._balancesHistorySnapshot.containsKey(userAddr)
            == false{ 
            ...
            emit PointsMinted(
                ...
                param:{ 
                    "PreAmount":
                    (IncrementPointsV2._balancesHistorySnapshot[userAddr]!)
                        .toString(),
                    "NewAmount": amount.toString()
                }
            )
            IncrementPointsV2._totalSupply = IncrementPointsV2._totalSupply
                - IncrementPointsV2._balancesHistorySnapshot[userAddr]!
                + amount
