...
pub contract NFTMetadataUtility{ 
    ...
    priv fun getMetadataFromNFTRef(
        nftRef: &NonFungibleToken.NFT,
        owner: Address
    ): CollectionItem{ 
        let displayView =
            nftRef.resolveView(Type<MetadataViews.Display>())!
            as!
            MetadataViews.Display
        let externalURLView =
            nftRef.resolveView(Type<MetadataViews.ExternalURL>())!
            as!
        ...
        let royaltyView =
            nftRef.resolveView(Type<MetadataViews.Royalties>())!
            as!
        ...
        return CollectionItem(
            ...
            collectionSquareImage: (collectionDisplayView!).squareImage.file
                .uri(),
            collectionBannerImage: (collectionDisplayView!).bannerImage.file
                .uri(),
            collectionSocials: (collectionDisplayView!).socials
        )
    }
    
    pub fun getStorefrontV2NFTRef(
        owner: Address,
        listingResourceID: UInt64
    ): &NonFungibleToken.NFT?{ 
        let storefrontRef =
            getAccount(owner).getCapability<
    ...
    pub fun getStorefrontV2ListingMetadata(
        owner: Address,
        listingResourceID: UInt64
    ): StorefrontItem{ 
        let storefrontRef =
        ...
        let collectionItem =
            NFTMetadataUtility.getMetadataFromNFTRef(
                nftRef: nftRef!,
                owner: owner
            )
    ...
    pub fun getStorefrontV2FlowtyNFTRef(
        owner: Address,
        listingResourceID: UInt64
    ): &NonFungibleToken.NFT?{ 
    ...
    pub fun getStorefrontV2FlowtyListingMetadata(
        owner: Address,
        listingResourceID: UInt64
    ): StorefrontItem{ 
        let storefrontRef = FlowtyStorefront.getStorefrontRef(owner: owner)
    ...
    pub fun getStorefrontV1ListingMetadata(
        owner: Address,
        listingResourceID: UInt64
    ): StorefrontItem{ 
        let storefrontRef =
    ...
    pub fun getTopShotNFTRef(
        owner: Address,
        nftID: UInt64
    ): &NonFungibleToken.NFT?{ 
        let collectionRef =
            getAccount(owner).getCapability(/public/MomentCollection).borrow<
    ...
    pub fun getTopShotMetadata(owner: Address, nftID: UInt64): CollectionItem{ 
        let nftRef =
            NFTMetadataUtility.getTopShotNFTRef(owner: owner, nftID: nftID)
        return NFTMetadataUtility.getMetadataFromNFTRef(
            nftRef: nftRef!,
            owner: owner
        )
    }
