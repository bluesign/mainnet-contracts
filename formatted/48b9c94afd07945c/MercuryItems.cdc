...
pub contract MercuryItems: NonFungibleToken{ 
    ...
    pub event Minted(id: UInt64, tokenURI: String)
    ...
    pub resource NFTMinter{ 
        ...
        //
        pub fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, tokenURI: String){ 
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &MercuryItems.NFT?{ 
        let collection = (getAccount(from).getCapability(MercuryItems.CollectionPublicPath)!).borrow<&MercuryItems.Collection{MercuryItems.MercuryItemsCollectionPublic}>() ?? panic("Couldn't get collection")
