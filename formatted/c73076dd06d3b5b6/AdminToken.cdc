...
pub contract AdminToken{ 
    ...
    pub fun checkAuthorizedAdmin(_ adminTokenRef: &AdminToken.Token?){ 
        ...
        if (AdminToken.adminRegistry[((adminTokenRef!).owner!).address]!)
            .expires
        <= Clock.getTime(){ 
    ...
    pub resource TokenMinter{ 
        pub fun mintToken(recipient: &{AdminToken.AdminTokenVaultPublic}){ 
            var newToken <-
                create Token(
    ...
    pub resource SuperAdminManager{ 
        pub fun addNAdmin(address: Address, expires: UFix64){ 
            ...
            AdminToken.adminRegistry.insert(
                key: address,
                AdminToken.AdminDetails(expires: expires)
            )
    ...
    init(){ 
        ...
        self.account.save(
            <-superAdminManager,
            to: self.SuperAdminManagerStoragePath
        )
