...
pub contract ElvnFeeTreasury{ 
    ...
    priv fun getVault(): &Elvn.Vault{ 
        return self.account.borrow<&Elvn.Vault>(from: /storage/elvnVault)
    ...
    pub fun getReceiver(): Capability<&Elvn.Vault{FungibleToken.Receiver}>{ 
        return self.account.getCapability<&Elvn.Vault{FungibleToken.Receiver}>(
            /public/elvnReceiver
        )
    }
    ...
    init(){ 
        ...
        if self.account.borrow<&Elvn.Vault>(from: /storage/elvnVault) == nil{ 
            ...
            self.account.link<&Elvn.Vault{FungibleToken.Receiver}>(/public/elvnReceiver, target: /storage/elvnVault)
            self.account.link<&Elvn.Vault{FungibleToken.Balance}>(/public/elvnBalance, target: /storage/elvnVault)
        }
        emit Initialize()
    }
}
