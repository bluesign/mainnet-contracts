...
pub contract RockPeaksClipCard: NonFungibleToken{ 
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &RockPeaksClipCard.NFT?{ 
        let collection = (getAccount(from).getCapability(RockPeaksClipCard.CollectionPublicPath)!).borrow<&RockPeaksClipCard.Collection{RockPeaksClipCard.RockPeaksClipCardCollectionPublic}>() ?? panic("Couldn't get collection")
