...
pub contract GovernanceToken: NonFungibleToken{ 
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &GovernanceToken.NFT?{ 
        let collection = (getAccount(from).getCapability(GovernanceToken.CollectionPublicPath)!).borrow<&GovernanceToken.Collection{GovernanceToken.GovernanceTokenCollectionPublic}>() ?? panic("Couldn't get collection")
