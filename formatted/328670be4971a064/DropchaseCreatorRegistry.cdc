...
pub contract DropchaseCreatorRegistry{ 
    ...
    priv var cutPercentages:{ String: UFix64}
    
    // The mappings from the name of an creator to the cut percentage
    // that they are supposed to receive.
    priv var cutPackPercentages:{ String: UFix64}
    ...
    pub resource Admin{ 
        ...
        pub fun setCutPackPercentage(name: String, cutPackPercentage: UFix64?){ 
            DropchaseCreatorRegistry.cutPackPercentages[
                name
            ] = cutPackPercentage
            emit CutPackPercentageUpdated(
                name: name,
                cutPackPercentage: cutPackPercentage
            )
        }
        ...
        pub fun setDefaultPackCutPercentage(cutPackPercentage: UFix64){ 
            DropchaseCreatorRegistry
                .defaultCutPackPercentage = cutPackPercentage
            emit DefaultCutPackPercentageUpdated(
                cutPackPercentage: cutPackPercentage
            )
        }
    }
