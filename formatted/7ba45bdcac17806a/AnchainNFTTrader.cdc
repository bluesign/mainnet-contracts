...
pub contract AnchainNFTTrader{ 
    ...
    pub resource Trader: TradeManager, TraderPublic{ 
        ...
        priv var trades: @{UInt64: Trade}
        
        /// insert
        /// Create and publish a Trade for an NFT.
        ///
        pub fun createTrade(nftProviderCapability: Capability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>, nftType: Type, nftID: UInt64, nftReceiverCapability: Capability<&{NonFungibleToken.Receiver}>, requestedNftType: Type, requestedNftID: UInt64?): UInt64{ 
        ...
        pub fun borrowTrade(tradeResourceID: UInt64): &Trade{TradePublic}?{ 
            if self.trades[tradeResourceID] != nil{ 
                return &self.trades[tradeResourceID] as                                                        
                                                        /// cleanup
