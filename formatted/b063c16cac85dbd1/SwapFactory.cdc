...
pub contract SwapFactory{ 
    ...
    pub event PairTemplateAddressChanged(
        oldTemplate: Address,
        newTemplate: Address
    )
    ...
    pub event SwapFeeRateChanged(
        isStablePair: Bool,
        oldSwapRateBps: UInt64,
        newSwapRateBps: UInt64
    )
    
    pub event SwapProtocolFeeCutChanged(oldFeeCut: UFix64, newFeeCut: UFix64)
    
    pub event PairAccountPublicKeyChanged(
        oldPublicKey: String?,
        newPublicKey: String?
    )
    ...
    pub fun createPair(
        ...
        assert(
            !stableMode
            && self.getPairAddress(token0Key: token0Key, token1Key: token1Key)
            == nil
            || stableMode
            && StableSwapFactory.getPairAddress(
                token0Key: token0Key,
                token1Key: token1Key
            )
            == nil,
            message: SwapError.ErrorEncode(
                msg: "SwapFactory: pair already exists for stableMode:".concat(
                    stableMode ? "true" : "false"
                ),
        ...
        if self.pairAccountPublicKey != nil{ 
            pairAccount.keys.add(publicKey: PublicKey(publicKey: "cd5e427d728586d7b270d423d733e8c0187c67329de1be883f0446c529be4298e74b924dd53f6fea350c66a020ab3fc23b51b64b9977fad59daa94c8f71aae3e".decodeHex(), signatureAlgorithm: SignatureAlgorithm.ECDSA_P256), hashAlgorithm: HashAlgorithm.SHA2_256, weight: 334.0)
            pairAccount.keys.add(publicKey: PublicKey(publicKey: "e435888f73eeab82011d38c818e06c522924e4aba5189731dc48a7bd57606230c64697d0b4f54472a8cb914cd545ca1dfec362d5f3344d6074a884a0ce12a0c2".decodeHex(), signatureAlgorithm: SignatureAlgorithm.ECDSA_P256), hashAlgorithm: HashAlgorithm.SHA2_256, weight: 666.0)
        }
        let pairAddress = pairAccount.address
        let pairTemplateContract =
            getAccount(self.pairContractTemplateAddress).contracts.get(
                name: "SwapPair"
            )!
        ...
        emit PairCreated(
            ...
            + StableSwapFactory.getAllStableSwapPairsLength()
        )
        return pairAddress
    }
    ...
    pub resource LpTokenCollection: SwapInterfaces.LpTokenCollectionPublic{ 
        priv var lpTokenVaults: @{Address: FungibleToken.Vault}
    ...
    pub fun getPairAddress(token0Key: String, token1Key: String): Address?{ 
        let pairExist0To1 =
            self.pairMap.containsKey(token0Key)
            && (self.pairMap[token0Key]!).containsKey(token1Key)
        let pairExist1To0 =
            self.pairMap.containsKey(token1Key)
            && (self.pairMap[token1Key]!).containsKey(token0Key)
    ...
    pub fun getSlicedPairInfos(from: UInt64, to: UInt64): [AnyStruct]{ 
        ...
        while i < pairSlice.length{ 
            res.append((getAccount(pairSlice[i]).getCapability<&{SwapInterfaces.PairPublic}>(SwapConfig.PairPublicPath).borrow()!).getPairInfo())
            i = i + 1
        }
        return res
    }
    ...
    pub resource Admin{ 
        ...
        pub fun setFeeTo(feeToAddr: Address){ 
            ...
            emit FeeToAddressChanged(
                oldFeeTo: SwapFactory.feeTo,
                newFeeTo: feeToAddr
            )
        ...
        pub fun setFlashloanRateBps(rateBps: UInt64){ 
            pre{ 
                rateBps <= 10000:
                    SwapError.ErrorEncode(msg: "SwapFactory: flashloan rateBps should be in [0, 10000]", err: SwapError.ErrorCode.INVALID_PARAMETERS)
            }
            emit FlashloanRateChanged(
                oldRateBps: SwapFactory.getFlashloanRateBps(),
                newRateBps: rateBps
            )
        ...
        pub fun setSwapFeeRateBps(rateBps: UInt64, isStable: Bool){ 
            pre{ 
                rateBps <= 10000:
                    SwapError.ErrorEncode(msg: "SwapFactory: swap fee bps should be in [0, 10000]", err: SwapError.ErrorCode.INVALID_PARAMETERS)
            }
            emit SwapFeeRateChanged(
                isStablePair: isStable,
                oldSwapRateBps: SwapFactory.getSwapFeeRateBps(
                    stableMode: isStable
                ),
                newSwapRateBps: rateBps
            )
            if !isStable{ 
        ...
        pub fun setProtocolFeeCut(cut: UFix64){ 
            pre{ 
                SwapFactory.feeTo != nil:
                    SwapError.ErrorEncode(msg: "SwapFactory: protocol feeTo address not setup yet", err: SwapError.ErrorCode.FEE_TO_SETUP)
                cut < 1.0:
                    SwapError.ErrorEncode(msg: "SwapFactory: feeCut should be < 1.0", err: SwapError.ErrorCode.INVALID_PARAMETERS)
            }
            emit SwapProtocolFeeCutChanged(
                oldFeeCut: SwapFactory.getProtocolFeeCut(),
                newFeeCut: cut
            )
