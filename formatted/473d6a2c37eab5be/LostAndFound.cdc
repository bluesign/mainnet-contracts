...
pub contract LostAndFound{ 
    ...
    pub event DepositorBalanceLow(
        uuid: UInt64,
        threshold: UFix64,
        balance: UFix64
    )
    
    pub event DepositorTokensAdded(
        uuid: UInt64,
        tokens: UFix64,
        balance: UFix64
    )
    
    pub event DepositorTokensWithdrawn(
        uuid: UInt64,
        tokens: UFix64,
        balance: UFix64
    )
    ...
    pub resource DepositEstimate{ 
        ...
        destroy(){ 
            pre{ 
                self.item == nil:
                    "cannot destroy with non-null item"
            }
            destroy self.item
        }
    }
    ...
    pub resource Ticket{ 
        ...
        access(contract) let flowTokenRepayment: Capability<
        ...
        destroy(){ 
            pre{ 
                ...
                self.item == nil:
                    "can only destroy if not holding any item"
            }
    ...
    pub resource Bin{ 
        ...
        access(contract) fun withdrawTicket(
            ticketID: UInt64
        ): @LostAndFound.Ticket{ 
    ...
    pub resource Shelf{ 
        priv let bins: @{String: Bin}
        
        priv let identifierToType:{ String: Type}
        
        priv let redeemer: Address
        ...
        pub fun redeem(type: Type, ticketID: UInt64, receiver: Capability){ 
            ...
            && LostAndFound.storageFees[uuid] != nil{ 
                ...
                let repaymentVault <-
                    refundProvider.withdraw(
                        amount: LostAndFound.storageFees[uuid]!
                    )
    ...
    access(contract) fun getFlowProvider(): &FlowToken.Vault{
        ...
        return self.account.borrow<&FlowToken.Vault{FungibleToken.Provider}>(
    ...
    pub resource ShelfManager{ 
        priv let shelves: @{Address: Shelf}
        
        init(){ 
            self.shelves <-{} 
        }
        
        access(contract) fun ensureShelf(
            _ addr: Address,
            flowTokenRepayment: Capability<
                &FlowToken.Vault{FungibleToken.Receiver}
            >?
        ...
        pub fun deposit(
            ...
            let receiver =
                LostAndFound.account.getCapability<
                    &FlowToken.Vault{FungibleToken.Receiver}
            ...
            if LostAndFound.account.storageUsed != storageBeforeShelf
            && LostAndFound.storageFees[shelf.uuid] != nil{ 
                receiver.deposit(
                    from: <-storagePayment.withdraw(
                        amount: LostAndFound.storageFees[shelf.uuid]!
                    )
                )
            }
            ...
            let storageFee =
                FeeEstimator.storageUsedToFlowAmount(
                    storageUsedAfter - storageBefore
                )
            LostAndFound.storageFees[uuid] = storageFee
            let storagePaymentVault <-
                storagePayment.withdraw(amount: storageFee)
        ...
        pub fun deleteShelf(_ addr: Address){ 
            ...
            && LostAndFound.storageFees[uuid] != nil{ 
                let provider = LostAndFound.getFlowProvider()
                let vault <-
                    provider.withdraw(amount: LostAndFound.storageFees[uuid]!)
                ((flowTokenRepayment!).borrow()!).deposit(from: <-vault)
            }
            destroy shelf
    ...
    pub resource Depositor: DepositorPublic{ 
        priv let flowTokenVault: @FlowToken.Vault
        
        pub let flowTokenRepayment: Capability<&FlowToken.Vault{FungibleToken.Receiver}>
        
        priv var lowBalanceThreshold: UFix64?
        
        priv fun checkForLowBalance(): Bool{ 
        ...
        pub fun deposit(redeemer: Address, item: @AnyResource, memo: String?, display: MetadataViews.Display?): UInt64{ 
            let receiver = LostAndFound.account.getCapability<&FlowToken.Vault{FungibleToken.Receiver}>(/public/flowTokenReceiver).borrow()!
            let storageBeforeShelf = LostAndFound.account.storageUsed
        ...
        pub fun trySendResource(item: @AnyResource, cap: Capability, memo: String?, display: MetadataViews.Display?){ 
    ...
    pub fun createDepositor(
        _ flowTokenRepayment: Capability<
            &FlowToken.Vault{FungibleToken.Receiver}
        >,
    ...
    pub fun borrowShelfManager(): &LostAndFound.ShelfManager{ 
        return self.account.getCapability<&LostAndFound.ShelfManager>(
            LostAndFound.LostAndFoundPublicPath
        ).borrow()!
    }
    
    pub fun borrowAllTicketsByType(addr: Address, type: Type): [
        &LostAndFound.Ticket
    ]{ 
    ...
    init(){ 
        ...
        self.account.link<&LostAndFound.ShelfManager>(
            self.LostAndFoundPublicPath,
            target: self.LostAndFoundStoragePath
        )
    }
}
