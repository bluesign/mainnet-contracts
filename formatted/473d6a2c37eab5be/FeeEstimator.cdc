...
pub contract FeeEstimator{ 
    ...
    pub fun estimateDeposit(item: @AnyResource): @DepositEstimate{ 
        ...
        let loadedItem <-
            FeeEstimator.account.load<@AnyResource>(from: /storage/temp)!
        let estimate <-
            create DepositEstimate(item: <-loadedItem, storageFee: storageFee)
        return <-estimate
    }
