...
pub contract ItemsWithOrg: NonFungibleToken{ 
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                case Type<MetadataViews.Display>():
                    return MetadataViews.Display(name: self.metadata["title"]!, description: self.metadata["description"]!, thumbnail: MetadataViews.HTTPFile(self.metadata["image"]!!))
            }
            return nil
        }
    }
    ...
    pub resource NFTMinter: NFTMinterPublic{ 
        priv var mintedNFTs: UInt64
        
        priv var orgAccount: Address
        ...
        pub fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, metadata:{ String: String}){ 
            ...
            emit Minted(id: ItemsWithOrg.totalSupply, metadata: metadata, orgAccount: self.orgAccount)
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &ItemsWithOrg.NFT?{ 
        let collection = (getAccount(from).getCapability(ItemsWithOrg.CollectionPublicPath)!).borrow<&ItemsWithOrg.Collection{ItemsWithOrg.ItemsWithOrgCollectionPublic}>() ?? panic("Couldn't get collection")
