...
pub contract Items: NonFungibleToken{ 
    ...
    pub resource NFTMinter: NFTMinterPublic{ 
        ...
        pub fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, metadata:{ String: String}){ 
            ...
            emit Minted(id: Items.totalSupply, metadata: metadata)
    ...
    pub fun fetch(_ from: Address, itemID: UInt64): &Items.NFT?{ 
        let collection = (getAccount(from).getCapability(Items.CollectionPublicPath)!).borrow<&Items.Collection{Items.ItemsCollectionPublic}>() ?? panic("Couldn't get collection")
