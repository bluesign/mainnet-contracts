...
pub contract OpenlockerNFT: NonFungibleToken{ 
    ...
    pub resource NFTMinter{ 
        priv var ONFTCollectionPublicPath: PublicPath
        
        init(collectionPublicPath: PublicPath){ 
            self.ONFTCollectionPublicPath = collectionPublicPath
        }
        
        pub fun mintNFTByIssuance(requests: [IssuanceMintMsg]){ 
            let minterOwner = self.owner ?? panic("could not get minter owner")
            let minterOwnerCollection = minterOwner.getCapability(self.ONFTCollectionPublicPath).borrow<&{NonFungibleToken.CollectionPublic}>() ?? panic("Could not get reference to the service account's NFT Collection")
        ...
        pub fun mintNFT(withdrawRequestID: UInt64, detailsURL: String, receiver: Address){ 
            // Borrow the recipient's public NFT collection reference
            let recipientAccount = getAccount(receiver)
            let recipientCollection = recipientAccount.getCapability(self.ONFTCollectionPublicPath).borrow<&{NonFungibleToken.CollectionPublic}>() ?? panic("Could not get receiver reference to the NFT Collection")
