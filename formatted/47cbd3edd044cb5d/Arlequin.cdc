...
pub contract Arlequin{ 
    ...
    // Query Functions
    /* For ArleePartner */    pub fun checkArleePartnerNFT(addr: Address): Bool{ 
    ...
    pub fun getArleePartnerRoyaltiesByPartner(
        partner: String
    ): ArleePartner.Royalty?{ 
    ...
    /* For ArleeScene */    pub fun getArleeSceneNFTIDs(addr: Address): [UInt64]?{ 
    ...
    pub resource ArleePartnerAdmin{ 
        ...
        pub fun setSpecificPartnerNFTMintable(partner: String, mintable: Bool){ 
            ArleePartner.setSpecificPartnerNFTMintable(
                partner: partner,
                mintable: mintable
            )
        }
        ...
        pub fun adminMintArleePartnerNFT(partner: String){ 
            ...
            let recipientCap =
                getAccount(Arlequin.account.address).getCapability<
                    &ArleePartner.Collection{ArleePartner.CollectionPublic}
                >(ArleePartner.CollectionPublicPath)
            let recipient =
                recipientCap.borrow()
                ?? panic("Cannot borrow Arlequin's Collection Public")
            // deposit
            ArleePartner.adminMintArleePartnerNFT(
                recipient: recipient,
                partner: partner
            )
        }
    ...
    pub resource ArleeSceneAdmin{ 
        ...
        pub fun addFreeMintAcctQuota(addr: Address, additionalMint: UInt64){ 
            ArleeScene.addFreeMintAcctQuota(
                addr: addr,
                additionalMint: additionalMint
            )
        }
        ...
        pub fun mintSceneNFT(
            ...
            let recipientCap =
                getAccount(buyer).getCapability<
                    &ArleeScene.Collection{ArleeScene.CollectionPublic}
                >(ArleeScene.CollectionPublicPath)
            let recipient =
                recipientCap.borrow()
                ?? panic("Cannot borrow recipient's Collection Public")
            ArleeScene.mintSceneNFT(
                recipient: recipient,
                cid: cid,
                metadata: metadata
            )
    ...
    /* Public Minting for ArleePartnerNFT */    pub fun mintArleePartnerNFT(
        buyer: Address,
        partner: String,
        ...
        // get all merchant receiving vault references 
        let arlequinVault =
            self.account.borrow<&FlowToken.Vault{FungibleToken.Receiver}>(
        ...
        let partnerVaultCap =
            getAccount(partnerAddr).getCapability<
                &FlowToken.Vault{FungibleToken.Receiver}
            >(/public/flowTokenReceiver)
        let partnerVault =
            partnerVaultCap.borrow()
            ?? panic("Cannot borrow partner's receiving vault reference")
        let recipientCap =
            getAccount(buyer).getCapability<
                &ArleePartner.Collection{ArleePartner.CollectionPublic}
            >(ArleePartner.CollectionPublicPath)
        ...
        let toPartnerVault <-
            paymentVault.withdraw(
                amount: paymentVault.balance * Arlequin.partnerSplitRatio
            )
        // deposit
    ...
    /* Public Minting for ArleeSceneNFT */    pub fun mintSceneNFT(
        buyer: Address,
        cid: String,
        metadata:{ 
            String: String
    ...
    /* Free Minting for ArleeSceneNFT */    pub fun mintSceneFreeMintNFT(
        buyer: Address,
        cid: String,
        metadata:{ 
            String: String
    ...
    /* Public Minting ArleeSceneVoucher NFT */    pub fun mintVoucherNFT(
        buyer: Address,
        species: String,
        paymentVault: @FungibleToken.Vault,
        adminRef: &ArleeSceneAdmin
        ...
        let recipientRef =
            getAccount(buyer).getCapability<
                &ArleeSceneVoucher.Collection{
                    ArleeSceneVoucher.CollectionPublic
                }
            >(ArleeSceneVoucher.CollectionPublicPath).borrow()
            ?? panic("Cannot borrow recipient's collection")
        arlequinVault.deposit(from: <-paymentVault)
        ArleeSceneVoucher.mintVoucherNFT(
            recipient: recipientRef,
            species: species
        )
    }
    
    /* Minting from ArleeSceneNFT from ArleeSceneVoucher (doesn't allow possibility to change cid, metadata etc. only validate on backend */    pub fun mintSceneFromVoucher(
        buyer: Address,
        cid: String,
        metadata:{ 
            String: String
        ...
        let recipientRef =
            getAccount(buyer).getCapability<
                &ArleeScene.Collection{ArleeScene.CollectionPublic}
            >(ArleeScene.CollectionPublicPath).borrow()
    ...
    /* Redeem Voucher - general purpose voucher consumption function, backend can proceed to mint once voucher is redeemed */    pub fun redeemVoucher(
        address: Address,
        voucher: @NonFungibleToken.NFT,
        adminRef: &ArleeSceneAdmin
    ){ 
    ...
    /* Upgrade Arlee */    pub fun updateArleeCID(
    ...
    init(){ 
        ...
        destroy <-self.account.load<@AnyResource>(
            from: Arlequin.ArleeSceneAdminStoragePath
        )
        self.account.save(
