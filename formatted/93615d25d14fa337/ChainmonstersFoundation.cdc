...
pub contract ChainmonstersFoundation{ 
    ...
    priv fun pickBundle(tier: Tier): @NonFungibleToken.NFT{ 
        ...
        assert(
            ids.length > 0,
            message: "No bundles available for tier ".concat(
                tier.rawValue.toString()
            )
        )
        let rng <-
            PRNG.createFrom(
                blockHeight: getCurrentBlock().height,
                uuid: UInt64(ids.length)
            )
    ...
    priv fun pickReservedNFT(
        ...
        let collection =
            ...
            ?? panic(
                "Could not borrow collection for tier ".concat(
                    tier.rawValue.toString()
                )
            )
        let ids = collection.getIDs()
        assert(
            ids.length > 0,
            message: "No reserved NFTs available for tier ".concat(
                tier.rawValue.toString()
            )
        )
        let index = UInt64(rng.range(0, UInt256(ids.length - 1)))
    ...
    pub resource Admin{ 
        ...
        pub fun redeemBundle(
            nft: @ChainmonstersRewards.NFT
            ...
            let tier =
                ChainmonstersFoundation.bundleRewardTierMapping[
                    nft.data.rewardID
                ]!
            let tokens <-
            ...
            switch tier{ 
                ...
                case Tier.RARE:
                    tokens.deposit(
                        token: <-ChainmonstersFoundation.rollForUpgrade(
                            rng: rng,
                            tier: Tier.RARE
                        )
                    )
        ...
        pub fun manualRaffle(
            rng: &PRNG.Generator,
            tier: Tier
        ): @NonFungibleToken.NFT?{ 
            return <-ChainmonstersFoundation.rollForUpgrade(
                rng: rng,
                tier: tier
            )
        }
    ...
    init(
        ...
        self
            .ReservedTiersCollectionStoragePath = /storage/cmfReservedTiersCollection
