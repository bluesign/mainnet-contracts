...
pub contract NowWhereContract{ 
    ...
    pub event DropPurchasedWithFlow(
        dropId: UInt64,
        templateId: UInt64,
        mintNumbers: UInt64,
        receiptAddress: Address,
    ...
    access(contract) let adminRef: Capability<
        &{NFTContract.NFTMethodsCapability}
    >
    
    // Variable size dictionary of Drop structs
    priv var allDrops:{ UInt64: Drop}
    ...
    pub struct Drop{ 
        ...
        pub fun updateDrop(
            ...
            pre{ 
                startDate == nil || self.startDate > getCurrentBlock().timestamp && startDate! >= getCurrentBlock().timestamp:
                    "can't update start date"
                endDate == nil || endDate! > getCurrentBlock().timestamp:
                    "can't update end date"
                templates == nil || (templates!).keys.length != 0 && self.startDate > getCurrentBlock().timestamp:
                    "can't update templates"
                !(startDate == nil && endDate == nil && templates == nil):
                    "All values are nil"
            }
    ...
    pub resource DropAdmin{ 
        access(contract) var ownerVault: Capability<
            &AnyResource{FungibleToken.Receiver}
        >?
        
        pub fun addOwnerVault(
            _ownerVault: Capability<&AnyResource{FungibleToken.Receiver}>
        ){ 
        ...
        pub fun updateDrop(
            ...
            pre{ 
                dropId != nil:
                    "invalid drop id"
                NowWhereContract.allDrops[dropId] != nil:
                    "drop id does not exists"
                startDate != 0.0 || endDate != 0.0:
            ...
            (NowWhereContract.allDrops[dropId]!).updateDrop(
                startDate: startDate,
                endDate: endDate,
                templates: templates
            )
        ...
        pub fun removeDrop(dropId: UInt64){ 
            pre{ 
                ...
                getCurrentBlock().timestamp < (NowWhereContract.allDrops[dropId]!).startDate:
                    "Drop sale is started"
            }
