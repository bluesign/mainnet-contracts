pub contract VouchersSaleManagerHelper{ 
    ...
    access(account) fun setMintedGoatVoucherEditionToMinted(_ edition: UInt64){ 
        self.mintedGoatVoucherEditions[edition] = true
    }
    
    access(account) fun hasGoatVoucherEdition(_ edition: UInt64): Bool{ 
        return self.mintedGoatVoucherEditions.containsKey(edition)
    }
    
    access(account) fun hasGoatPackEdition(_ edition: UInt64): Bool{ 
        return self.mintedPackVoucherEditions.containsKey(edition)
    }
    
    access(account) fun setMintedPackVoucherEditionToMinted(_ edition: UInt64){ 
        self.mintedPackVoucherEditions[edition] = true
    }
    
    access(account) fun updateGoatVoucherSale(
    ...
    access(account) fun updatePackVoucherSale(
        quantityPerMint: UInt64,
        price: UFix64,
        startTime: UFix64
    ){ 
