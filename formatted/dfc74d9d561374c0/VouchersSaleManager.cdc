...
pub contract VouchersSaleManager{ 
    ...
    // Emitted when any info about sale logistics has been modified
    pub event UpdateGoatVoucherSaleInfo(
        saleStartTime: UFix64,
        salePrice: UFix64,
        maxQuantityPerMint: UInt64
    )
    ...
    // Emitted when any info about sale logistics has been modified
    pub event UpdatePackVoucherSaleInfo(
        saleStartTime: UFix64,
        salePrice: UFix64,
        maxQuantityPerMint: UInt64
    )
    ...
    // -----------------------------------------------------------------------
    priv let mintedEditions:{ UInt64: Bool}
    
    priv var sequentialMintMin: UInt64
    ...
    pub resource Manager{ 
        pub fun updateGoatVoucherSaleInfo(
            quantityPerMint: UInt64,
            price: UFix64,
            startTime: UFix64
        ){ 
        ...
        pub fun updatePackVoucherSaleInfo(
            quantityPerMint: UInt64,
            price: UFix64,
            startTime: UFix64
        ){ 
        ...
        pub fun updateGoatedGoatsCollectionMetadata(
            metadata:{ 
                String: String
            }
        ){ 
        ...
        pub fun updateGoatedGoatsEditionMetadata(
            editionNumber: UInt64,
            metadata:{ 
                String: String
            }
        ){ 
            GoatedGoatsVouchers.setEditionMetadata(
        ...
        pub fun updateTraitsPacksEditionMetadata(
            editionNumber: UInt64,
            metadata:{ 
                String: String
            }
        ){ 
            TraitPacksVouchers.setEditionMetadata(
                editionNumber: editionNumber,
                metadata: metadata
            )
        ...
        pub fun mintGoatVoucherAtEdition(
            edition: UInt64
        ...
        pub fun mintPackVoucherAtEdition(
            edition: UInt64
        ): @NonFungibleToken.NFT{ 
    ...
    access(contract) fun mintGoatVoucher(
        edition: UInt64
    ): @NonFungibleToken.NFT{ 
        pre{ 
    ...
    priv fun mintSequentialGoatVoucher(): @NonFungibleToken.NFT{ 
        var curEditionNumber =
            VouchersSaleManagerHelper.curSequentialGoatEditionNumber
        while self.mintedEditions.containsKey(UInt64(curEditionNumber))
        || VouchersSaleManagerHelper.hasGoatVoucherEdition(
            UInt64(curEditionNumber)
        ){ 
            curEditionNumber = curEditionNumber + 1
        }
        VouchersSaleManagerHelper.setSequentialGoatEditionNumber(
            curEditionNumber
        )
    ...
    // Mint a voucher for goated goats
    access(contract) fun mintPackVoucher(
        edition: UInt64
    ): @NonFungibleToken.NFT{ 
        pre{ 
            edition >= 1 && edition <= self.maxSupply:
    ...
    // Look for the next available voucher, and mint there
    priv fun mintSequentialPackVoucher(): @NonFungibleToken.NFT{ 
        var curEditionNumber =
            VouchersSaleManagerHelper.curSequentialPackEditionNumber
        while self.mintedEditions.containsKey(UInt64(curEditionNumber))
        ...
        VouchersSaleManagerHelper.setSequentialPackEditionNumber(
            curEditionNumber
        )
    ...
    pub fun publicBatchMintSequentialGoatVouchers(
        ...
        // -- Receive Payments --
        let totalPrice =
            VouchersSaleManagerHelper.goatVoucherSalePrice * UFix64(quantity)
        // Ensure that the provided balance is equal to our expected price for the NFTs
        assert(
            totalPrice == buyVault.balance,
            message: "Invalid amount of Flow provided"
        )
    ...
    // Accepts payment for vouchers, payment is moved to the `self.paymentReceiver` capability field
    pub fun publicBatchMintSequentialPackVouchers(
        buyVault: @FungibleToken.Vault,
        quantity: UInt64
    ): @NonFungibleToken.Collection{ 
        pre{ 
        ...
        // -- Receive Payments --
        let totalPrice =
            VouchersSaleManagerHelper.packVoucherSalePrice * UFix64(quantity)
        // Ensure that the provided balance is equal to our expected price for the NFTs
        assert(
            totalPrice == buyVault.balance,
            message: "Invalid amount of Flow provided"
    ...
    access(contract) fun mintVouchers(edition: UInt64): @[
        NonFungibleToken.NFT
    ]{ 
        pre{ 
            edition >= 1 && edition <= self.maxSupply:
    ...
    priv fun mintSequentialVouchers(): @[NonFungibleToken.NFT]{ 
    ...
    pub fun publicBatchMintSequentialVouchers(
        buyVault: @FungibleToken.Vault,
        quantity: UInt64
    ): @[
        NonFungibleToken.Collection
    ...
    init(){ 
        ...
        self.paymentReceiver = self.account.getCapability<
