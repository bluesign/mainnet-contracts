...
pub contract MatchContract{ 
    ...
    // Events
    pub event registered(host: Address, index: UInt32)
    
    pub event matched(host: Address, challenger: Address, index: UInt32)
    ...
    access(account) fun match(
        ...
        let roleAddressMap = self.indexAddressMap[index] ??{} 
        assert(
            roleAddressMap[MatchRole.host] != nil,
            message: "Host not found for this index."
        )
        assert(
            roleAddressMap[MatchRole.challenger] == nil,
        ...
        assert(
            ...
            message: hostAddress.toString().concat(
                "'s waiting group length should over 0"
            )
        )
    ...
    priv fun moveMatchedToFinished(for index: UInt32, key: String){ 
        ...
        let matchedGroup: [UInt32] = addressGroup[MatchStatus.matched] ?? []
        assert(
            matchedGroup.contains(index),
            message: "Index not found in matchedGroup."
        )
