...
pub contract Gomoku{ 
    ...
    pub event CompositionCreated(host: Address, currency: String)
    ...
    pub event RoundSwitch(compositionId: UInt32, previous: UInt8, next: UInt8)
    
    pub resource Admin{ 
        pub fun manualFinalizeByTimeout(index: UInt32){ 
            if let compositionRef =
                Gomoku.getCompositionRef(by: index) as &Gomoku.Composition?{ 
                let participants = Gomoku.getParticipants(by: index)
                assert(
                    participants.length == 2,
                    message: "Composition not matched."
                )
                for participant in participants{ 
                    if let identityCollectionRef = getAccount(participant).getCapability<&GomokuIdentity.IdentityCollection>(GomokuIdentity.CollectionPublicPath).borrow(){ 
        ...
        pub fun recycleBets(){ 
            let capability =
                Gomoku.account.getCapability<
                    &FlowToken.Vault{FungibleToken.Receiver}
                >(/public/flowTokenReceiver)
            let flowReceiverReference =
                capability.borrow()
                ?? panic(
                    "Could not borrow a reference to the Flow token receiver capability"
                )
    ...
    init(){ 
        ...
        if self.account.borrow<&FlowToken.Vault>(from: /storage/flowTokenVault)
        == nil{ 
            let flowVault <- FlowToken.createEmptyVault()
            self.account.save(<-flowVault, to: /storage/flowTokenVault)
        }
        if self.account.getCapability<&FlowToken.Vault{FungibleToken.Receiver}>(
            /public/flowTokenReceiver
        )
        == nil{ 
        ...
        if self.account.getCapability<&FlowToken.Vault{FungibleToken.Balance}>(
    ...
    pub fun getCompositionRef(by index: UInt32): &Gomoku.Composition?{ 
        if let host = MatchContract.getHostAddress(by: index){ 
            if let collectionRef = getAccount(host).getCapability<&Gomoku.CompositionCollection>(self.CollectionPublicPath).borrow(){ 
    ...
    pub fun getFinalizedBets(by index: UInt32): [UFix64]?{ 
        if let roundWinners =
            self.getCompositionRef(by: index)?.getRoundWinners(){ 
            if roundWinners.length != 2{ 
                return nil
            }
            return [
    ...
    pub fun matchOpponent(
        ...
        if let matchedHost =
            MatchContract.match(index: index, challengerAddress: challenger){ 
            assert(
                matchedHost != challenger,
                message: "You can't play with yourself."
            )
            if let compositionCollectionRef =
                getAccount(matchedHost).getCapability<
                    &Gomoku.CompositionCollection
                >(self.CollectionPublicPath).borrow(){ 
                let hostBet =
                    self.hostOpeningBetMap[index]?.balance ?? UFix64(0)
                assert(
                    hostBet == bet.balance,
                    message: "Opening bets not equal."
                )
                self.hostRaisedBetMap[index] <-! FlowToken.createEmptyVault()
                    as!
                    @FlowToken.Vault
                self.challengerRaisedBetMap[index] <-! FlowToken
                        .createEmptyVault()
                    as!
                    @FlowToken.Vault
                if let compositionRef =
                    compositionCollectionRef.borrow(id: index){ 
                    ...
                    let challengerOpeningBet =
                        self.getChallengerOpeningBet(by: index)
                    assert(
                        hostOpeningBet != nil,
                        message: "Host opening bet should not be 0."
                    )
                    assert(
                        challengerOpeningBet != nil,
                        message: "Challenger opening bet should not be 0."
                    )
                    assert(
                        hostOpeningBet! == challengerOpeningBet!,
                        message: "Opening bet not match."
                    )
    ...
    pub resource Composition{ 
        ...
        pub fun makeMove(
            ...
            let identityTokenRef =
                identityCollectionRef.borrow(id: self.id)
                as
                &GomokuIdentity.IdentityToken?
            assert(
                identityTokenRef != nil,
                message: "Identity token ref not found."
            )
            assert(
            ...
            assert(
                Int(self.currentRound) + 1 > self.roundWinners.length,
                message: "Game Over."
            )
            ...
            switch currentRole{ 
                case GomokuType.Role.host:
                    assert(
                        identityToken.address == self.host,
                        message: "It's not you turn yet!"
                    )
                    var emptyBet: @FlowToken.Vault <-
                        FlowToken.createEmptyVault() as! @FlowToken.Vault
                    var hostRaisedBet
                        <- Gomoku.hostRaisedBetMap[self.id]
                        <- emptyBet
                ...
                case GomokuType.Role.challenger:
                    ...
                    var emptyBet: @FlowToken.Vault <-
                        FlowToken.createEmptyVault() as! @FlowToken.Vault
                    var hostRaisedBet
                        <- Gomoku.challengerRaisedBetMap[self.id]
                        <- emptyBet
                    if let oldBet <- hostRaisedBet{ 
                        oldBet.deposit(from: <-raisedBet)
        ...
        priv fun finalize(identityToken: @GomokuIdentity.IdentityToken){ 
            ...
            let devFlowTokenReceiver =
                Gomoku.account.getCapability<
                    &FlowToken.Vault{FungibleToken.Receiver}
                >(/public/flowTokenReceiver).borrow()
                ?? panic(
                    "Could not borrow a reference to the dev flowTokenReceiver capability."
                )
            ...
            let challengerFlowTokenReceiver =
                getAccount(self.challenger!).getCapability<
                    &FlowToken.Vault{FungibleToken.Receiver}
                >(/public/flowTokenReceiver).borrow()
                ?? panic(
                    "Could not borrow a reference to the dev flowTokenReceiver capability."
            ...
            let hostOpeningBet: @FlowToken.Vault?
                ...
                <- FlowToken.createEmptyVault() as! @FlowToken.Vault
            Gomoku.hostFinalizedOpeningBetMap[identityToken.id] = hostOpeningBet
                    ?.balance
                ?? UFix64(0)
            ...
            let challengerOpeningBet: @FlowToken.Vault?
                ...
                <- FlowToken.createEmptyVault() as! @FlowToken.Vault
            Gomoku.challengerFinalizedOpeningBetMap[
                identityToken.id
            ] = challengerOpeningBet?.balance ?? UFix64(0)
            ...
            let hostRaisedBet: @FlowToken.Vault?
                ...
                <- FlowToken.createEmptyVault() as! @FlowToken.Vault
            Gomoku.hostFinalizedRaisedBetMap[identityToken.id] = hostRaisedBet
                    ?.balance
                ?? UFix64(0)
            let challengerRaisedBet: @FlowToken.Vault?
                <- Gomoku.challengerRaisedBetMap[identityToken.id]
                <- FlowToken.createEmptyVault() as! @FlowToken.Vault
            Gomoku.challengerFinalizedRaisedBetMap[
                identityToken.id
            ] = challengerRaisedBet?.balance ?? UFix64(0)
            ...
            switch result{ 
                case GomokuType.Result.hostWins:
                    ...
                    let devRewardBalance =
                        tatalVault.balance * UFix64(5) / UFix64(100)
                    let hostRewardBalance =
                        tatalVault.balance * UFix64(1) / UFix64(100)
                    let winnerRewardBalance =
                        tatalVault.balance - devRewardBalance
                        - hostRewardBalance
                    devReward.deposit(
                        from: <-tatalVault.withdraw(amount: devRewardBalance)
                    )
                    hostReward.deposit(
                        from: <-tatalVault.withdraw(amount: hostRewardBalance)
                    )
                    winnerReward.deposit(
                        from: <-tatalVault.withdraw(amount: winnerRewardBalance)
                    )
                    destroy tatalVault
                ...
                case GomokuType.Result.draw:
                    ...
                    // each player get 49%.
                    let hostRewardBalance =
                        tatalVault.balance * UFix64(49) / UFix64(100)
                    let challengerRewardBalance =
                        tatalVault.balance * UFix64(49) / UFix64(100)
                    let devRewardBalance =
                        tatalVault.balance - hostRewardBalance
                        - challengerRewardBalance
                    devReward.deposit(
                        from: <-tatalVault.withdraw(amount: devRewardBalance)
                    )
                    hostReward.deposit(
                    ...
                    challengerReward.deposit(
                        from: <-tatalVault.withdraw(
                            amount: challengerRewardBalance
                        )
                    )
                    destroy tatalVault
                default:
            ...
            if identityToken.address == self.host{ 
                if let identityCollectionRef = getAccount(self.challenger!).getCapability<&GomokuIdentity.IdentityCollection>(GomokuIdentity.CollectionPublicPath).borrow(){ 
                ...
                if let identityCollectionRef = getAccount(self.host).getCapability<&GomokuIdentity.IdentityCollection>(GomokuIdentity.CollectionPublicPath).borrow(){ 
        ...
        priv fun mintCompositionResults(
            ...
            if let winnerResultCollectionCapability =
                getAccount(winnerAddress).getCapability<
                    &GomokuResult.ResultCollection
                >(GomokuResult.CollectionPublicPath).borrow(){ 
                winnerResultCollectionCapability.deposit(
                    token: <-winnerResultToken
                )
            } else{ 
            ...
            if let losserResultCollectionCapability =
                getAccount(losserAddress).getCapability<
                    &GomokuResult.ResultCollection
                >(GomokuResult.CollectionPublicPath).borrow(){ 
                losserResultCollectionCapability.deposit(
                    token: <-losserResultToken
                )
            } else{ 
        ...
        priv fun switchRound(){ 
            ...
            let hostIdentityCollectionCapability =
                ...
                >(GomokuIdentity.CollectionPublicPath).borrow()
                ?? panic("Could not borrow a reference to the host capability.")
            if let identityToken =
                hostIdentityCollectionCapability.borrow(id: self.id){ 
            ...
            let challengerIdentityCollectionCapability =
                ...
                >(GomokuIdentity.CollectionPublicPath).borrow()
                ?? panic(
                    "Could not borrow a reference to the challenger capability."
                )
            if let identityToken =
                challengerIdentityCollectionCapability.borrow(id: self.id){ 
                identityToken.switchIdentity()
            } else{ 
                panic("Could not borrow a reference to identityToken.")
            }
        ...
        priv fun verifyAndStoreStone(stone: @Stone){ 
            ...
            let roundSteps =
                &self.steps[self.currentRound]
                as
                &[
                    AnyResource{GomokuType.Stoning}
                ]
            ...
            assert(
                isOnBoard,
                message: "Stone location".concat(stone.location.description())
                    .concat(" is invalid.")
            )
        ...
        priv fun checkWinner(
            ...
            switch direction{ 
                case GomokuType.VerifyDirection.vertical:
                    while !isFinished && shift <= Int8(4)
                    && center.x - shift >= Int8(0){ 
                        let currentCheckedLocation =
                            GomokuType.StoneLocation(
                                x: center.x - shift,
                                y: center.y
                            )
                        if let color =
    ...
    pub resource CompositionCollection{ 
        ...
        access(account) fun withdraw(by id: UInt32): @Gomoku.Composition{ 
            let token <-
                self.ownedCompositionMap.remove(key: id)
    ...
    pub resource Stone: GomokuType.Stoning{ 
        ...
        pub init(color: GomokuType.StoneColor, location: GomokuType.StoneLocation){ 
            self.color = color
            self.location = location
        }
