pub contract YDYProfile{ 
    ...
    pub resource interface Public{ 
        ...
        access(contract) fun updateWorkoutData(
            id: String,
            key: String,
            value: String
        ){} 
    ...
    pub resource Admin{ 
        pub fun updateDataOfUser(
            receiverCollectionCapability: Capability<
                &YDYProfile.User{YDYProfile.Public}
            >,
            key: String,
            value: String
        ){ 
            let receiver =
                receiverCollectionCapability.borrow() ?? panic("Cannot borrow")
            receiver.updateUserData(key: key, value: value)
        }
        
        pub fun addWorkoutDataToUser(
            receiverCollectionCapability: Capability<
                &YDYProfile.User{YDYProfile.Public}
            >,
            data:{ 
        ...
        pub fun updateWorkoutDataOfUser(
            receiverCollectionCapability: Capability<
                &YDYProfile.User{YDYProfile.Public}
            >,
            id: String,
