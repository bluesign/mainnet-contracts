...
pub contract YDYNFT: NonFungibleToken{ 
        ...
        pub fun resolveView(_ view: Type): AnyStruct?{ 
            switch view{ 
                ...
                case Type<MetadataViews.NFTCollectionData>():
                    return MetadataViews.NFTCollectionData(storagePath: YDYNFT.CollectionStoragePath, publicPath: YDYNFT.CollectionPublicPath, providerPath: /private/ydyNFTCollection, publicCollection: Type<&YDYNFT.Collection{YDYNFT.YDYNFTCollectionPublic}>(), publicLinkedType: Type<&YDYNFT.Collection{YDYNFT.YDYNFTCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver, MetadataViews.ResolverCollection}>(), providerLinkedType: Type<&YDYNFT.Collection{YDYNFT.YDYNFTCollectionPublic, NonFungibleToken.CollectionPublic, NonFungibleToken.Provider, MetadataViews.ResolverCollection}>(), createEmptyCollectionFunction: fun (): @NonFungibleToken.Collection{ 
                ...
                case Type<MetadataViews.Royalties>():
                    return MetadataViews.Royalties([MetadataViews.Royalty(recepient: getAccount(YDYNFT.account.address).getCapability<&{FungibleToken.Receiver}>(/public/flowTokenReceiver), cut: 0.075, description: "This is the royalty receiver for YDY NFTs")])
            }
            return nil
        }
    }
    ...
    pub fun buy(collectionCapability: Capability<&Collection{YDYNFT.YDYNFTCollectionPublic}>, payment: @FlowToken.Vault, quantity: UInt64){ 
        ...
        let ydyCollection = self.account.getCapability(YDYNFT.CollectionPublicPath).borrow<&Collection{YDYNFT.YDYNFTCollectionPublic, YDYNFT.YDYNFTCollectionPrivate}>() ?? panic("Can't get YDY's collection.")
    ...
    pub resource Admin{ 
        ...
        pub fun transferNFT(collectionCapability: Capability<&Collection{YDYNFT.YDYNFTCollectionPublic}>): UInt64{ 
            let ydyCollection = YDYNFT.account.getCapability(YDYNFT.CollectionPublicPath).borrow<&Collection{YDYNFT.YDYNFTCollectionPublic, YDYNFT.YDYNFTCollectionPrivate}>() ?? panic("Can't get YDY's collection.")
            let availableNFTs = ydyCollection.getIDs()
            if availableNFTs.length > 0{ 
                let receiver = collectionCapability.borrow() ?? panic("Cannot borrow")
                let randomInt = unsafeRandom() % UInt64(availableNFTs.length)
