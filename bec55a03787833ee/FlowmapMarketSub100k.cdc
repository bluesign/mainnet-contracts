...
access(all) contract FlowmapMarketSub100k {
    ...
    access(all) event FlowmapListed(id: UInt64, price: UFix64, listingFee: UFix64)
    access(all) event FlowmapListingCancelled(id: UInt64)
    access(all) event FlowmapPurchased(id: UInt64, price: UFix64, purchaseFee: UFix64)
    access(all) event FlowmapListingPurged(id: UInt64, price: UFix64)
    ...
    access(all) var purchaseFee: UFix64
    access(all) var listingDuration: UFix64
    access(all) var marketPaused: Bool
    access(all) var totalVolume: UFix64
    access(all) var totalSales: UInt64
    access(self) var sellers: [Address]
    access(self) var sales: {UInt64: Sale}

    access(all) struct Sale {
        access(all) let id: UInt64
        access(all) let flowmapID: UInt64
        access(all) let seller: Address
        access(all) let buyer: Address
        access(all) let purchaseDate: UFix64
        access(all) let price: UFix64
        access(all) let expirationDate: UFix64
    ...
    access(all) resource interface SalePublic {
        access(all) fun purchase(tokenID: UInt64, buyTokens: @FlowToken.Vault, buyer: Address): @Flowmap.NFT
        access(all) fun getPrice(tokenID: UInt64): UFix64?
        access(all) fun getExpirationDate(tokenID: UInt64): UFix64?
        access(all) fun getPrices(): {UInt64: UFix64}
        access(all) fun getExpirationDates(): {UInt64: UFix64}
        access(all) fun getIDs(): [UInt64]
        access(all) fun checkCapability(): Bool
        access(all) fun purgeExpiredListings()
        access(all) fun purgeGhostListings()
    }

    access(all) resource SaleCollection: SalePublic {
        access(self) var prices: {UInt64: UFix64}
        access(self) var expirationDates: {UInt64: UFix64}
        access(self) var ownerCollection: Capability<&Flowmap.Collection>

        init(
            ownerCollection: Capability<&Flowmap.Collection>,
            ) {
            pre {
                ownerCollection.check(): "Owner's Flowmap Collection Capability is invalid!"
            }
        ...
        access(all) fun listForSale(tokenID: UInt64, price: UFix64, listingFee: @FlowToken.Vault) {
            pre {
                tokenID <= 99999: "Can't list for sale: ID must be less than 100,000"
                listingFee.balance == FlowmapMarketSub100k.listingFee: "Can't list for sale: listingFee payment doesn't match contract listingFee"
                price >= FlowmapMarketSub100k.purchaseFee: "Can't list for sale: price must be greater than purchaseFee"
                self.ownerCollection.borrow()!.borrowFlowmap(id: tokenID) != nil: "Can't list for sale: ID doesn't exist in owner's Flowmap Collection"
                FlowmapMarketSub100k.marketPaused == false: "Can't list for sale: Market is paused"
            }
        ...
        access(all) fun cancelSale(tokenID: UInt64) {
            pre {
                self.prices[tokenID] != nil: "Can't cancel Sale: ID doesn't exist in this SaleCollection"
            }
        ...
        access(all) fun purchase(tokenID: UInt64, buyTokens: @FlowToken.Vault, buyer: Address): @Flowmap.NFT {
            pre {
                self.prices[tokenID] != nil: "Can't purchase: ID doesn't exist in this SaleCollection"
                self.expirationDates[tokenID]! > getCurrentBlock().timestamp: "Can't purchase: Sale has expired"
                buyTokens.balance == self.prices[tokenID]!: "Can't purchase: buyTokens balance must match the price"
                FlowmapMarketSub100k.marketPaused == false: "Can't purchase: Market is paused"
            }
            ...
            let sale = Sale(
                id: FlowmapMarketSub100k.totalSales,
                flowmapID: tokenID,
                seller: self.owner!.address,
                buyer: buyer,
                purchaseDate: getCurrentBlock().timestamp,
                price: price,
                expirationDate: self.expirationDates[tokenID]!
            )
    ...
    access(all) resource Admin {
        access(all) fun setListingFee(fee: UFix64) {
            FlowmapMarketSub100k.listingFee = fee
        }

        access(all) fun setPurchaseFee(fee: UFix64) {
            FlowmapMarketSub100k.purchaseFee = fee
        }

        access(all) fun setListingDuration(duration: UFix64) {
            FlowmapMarketSub100k.listingDuration = duration
        }

        access(all) fun pauseMarket() {
            FlowmapMarketSub100k.marketPaused = true
        }

        access(all) fun unpauseMarket() {
    ...
    access(all) fun createSaleCollection(ownerCollection: Capability<&Flowmap.Collection>): @SaleCollection {
    ...
    access(all) fun getSellers(): [Address] {
        return self.sellers
    }

    access(all) fun getSales(): {UInt64: Sale} {
        return self.sales
    }

    access(all) fun getSale(id: UInt64): Sale? {
        return self.sales[id]
    }

    access(all) fun getSaleIDs(): [UInt64] {
    ...
    init() {
        ...
        self.account.link<&FlowmapMarketSub100k.Admin>(self.AdminPrivatePath, target: self.AdminStoragePath) 
