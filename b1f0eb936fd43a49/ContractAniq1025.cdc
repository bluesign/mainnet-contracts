...
pub contract ContractAniq1025: NonFungibleToken, Anique {
    // -----------------------------------------------------------------------
    // ContractAniq1025 contract Events
    // -----------------------------------------------------------------------

    // Events for Contract-Related actions
    //
    // Emitted when the ContractAniq1025 contract is created
    ...
    // -----------------------------------------------------------------------
    // ContractAniq1025 contract-level fields.
    ...
    // -----------------------------------------------------------------------
    // ContractAniq1025 contract-level Composite Type definitions
    // -----------------------------------------------------------------------

    // The structure that represents Item
    // each digital content which ContractAniq1025 deal with on Flow
    //
    pub struct ItemData {
        ...
        init(itemID: UInt32) {
            let item = (&ContractAniq1025.items[itemID] as &Item?)!
    ...
    pub resource Item {
        ...
        init(metadata: {String: String}) {
            ...
            self.itemID = ContractAniq1025.nextItemID
            self.metadata = metadata
            self.numberMintedPerItem = 0

            // increment the nextItemID so that it isn't used again
            ContractAniq1025.nextItemID = ContractAniq1025.nextItemID + 1
    ...
    pub resource NFT: NonFungibleToken.INFT, Anique.INFT {
        ...
        init(serialNumber: UInt32, itemID: UInt32) {
            ...
            ContractAniq1025.totalSupply = ContractAniq1025.totalSupply + 1

            // set id
            self.id = ContractAniq1025.totalSupply
    ...
    // interface that represents ContractAniq1025 collections to public
    ...
    pub resource interface CollectionPublic {
        ...
        // borrow NFT as ContractAniq1025 token
        pub fun borrowContractAniq1025Collectible(id: UInt64): auth &NFT
    }
    ...
    pub resource Collection: CollectionPublic, NonFungibleToken.Receiver, NonFungibleToken.Provider, NonFungibleToken.CollectionPublic {
        ...
        pub fun deposit(token: @NonFungibleToken.NFT) {
            ...
            // Cast the deposited token as an ContractAniq1025 NFT to make sure
            // it is the correct type
            let token <- token as! @ContractAniq1025.NFT
        ...
        // not any ContractAniq1025 specific data. Please use borrowCollectible to
        ...
        // borrowContractAniq1025Collectible returns a borrowed reference
        // to an ContractAniq1025 Collectible
        pub fun borrowContractAniq1025Collectible(id: UInt64): auth &NFT {
            pre {
    ...
    pub resource Admin {
        ...
        // and stores it in the Items dictionary field in the ContractAniq1025 smart contract
        //
        ...
        pub fun createItem(metadata: {String: String}): UInt32 {
            ...
            ContractAniq1025.items[newItem.itemID] <-! newItem

            return itemId
        }

        // borrowItem returns a reference to a Item in the ContractAniq1025
        ...
        pub fun borrowItem(itemID: UInt32): &Item {
            pre {
                ContractAniq1025.items[itemID] != nil: "Cannot borrow Item: The Item doesn't exist"
            }

            return (&ContractAniq1025.items[itemID] as &Item?)!
        }
    ...
    // -----------------------------------------------------------------------
    // ContractAniq1025 contract-level function definitions
    // -----------------------------------------------------------------------
    ...
    pub fun createEmptyCollection(): @NonFungibleToken.Collection {
        return <-create ContractAniq1025.Collection()
    }
    ...
    // -----------------------------------------------------------------------
    // ContractAniq1025 initialization function
    // -----------------------------------------------------------------------
    //
    init() {
        ...
        self.collectionStoragePath     = /storage/ContractAniq1025Collection
        self.collectionPublicPath      =  /public/ContractAniq1025Collection
        self.collectionPrivatePath     = /private/ContractAniq1025Collection
        self.adminStoragePath          = /storage/ContractAniq1025Admin
        self.saleCollectionStoragePath = /storage/ContractAniq1025SaleCollection
        self.saleCollectionPublicPath  =  /public/ContractAniq1025SaleCollection
