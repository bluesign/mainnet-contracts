...
pub contract PPPV2 {
    ...
    */
    ...
    access(all) view fun calculateNewPointsSinceLastUpdate(userAddr: Address): AnyStruct {
        ...
        return (accuredLendingSupplyPoints + accuredLendingBorrowPoints + accuredStFlowHoldingPoints + accuredSwapLPPoints) * (referrerAddr==nil? 1.0 : 1.0+self.getPointsRate_RefereeUp())
    }
        ...
        if duration > 0.0 {
            ...
            if (accuredSwapLPPoints > 0.0) {
                emit PointsMinted(
                    ...
                    source: "SwapLP",
                    param: {
                        "SwapLPUsdValue": self.getUserState_SwapLP(userAddr: userAddr).toString(),
                        "Duration": durationStr
                    }
                )
            }
    ...
    access(all) view fun getLatestUserInfo(_ userAddr: Address): [UFix64] {
        let userLatestStates: [UFix64] = self.fetchOnchainUserStates(userAddr: userAddr)
        return [
    ...
    access(all) view fun getSlicedBalances(from: Int, to: Int): {Address: UFix64} {
    ...
    access(all) view fun getSlicedPointsAndTimestamp(from: Int, to: Int): {Address: [UFix64; 3]} {
        let len = self._pointsbase.length
        let endIndex = to > len ? len : to
        var curIndex = from
    ...
    // Get Points Rate
    access(all) view fun getPointsRate(): {String: AnyStruct} { return self._pointsRatePerDay }
    access(all) view fun getPointsRate_LendingSupply(amount: UFix64): UFix64 { return self.calculateTierRateByAmount(amount: amount, tier: self._pointsRatePerDay["LendingSupply"]! as! {UFix64: UFix64}) }
    access(all) view fun getPointsRate_LendingBorrow(amount: UFix64): UFix64 { return self.calculateTierRateByAmount(amount: amount, tier: self._pointsRatePerDay["LendingBorrow"]! as! {UFix64: UFix64}) }
    access(all) view fun getPointsRate_stFlowHolding(amount: UFix64): UFix64 { return self.calculateTierRateByAmount(amount: amount, tier: self._pointsRatePerDay["stFlowHolding"]! as! {UFix64: UFix64}) }
    access(all) view fun getPointsRate_SwapLP(amount: UFix64): UFix64 { return self.calculateTierRateByAmount(amount: amount, tier: self._pointsRatePerDay["SwapLP"]! as! {UFix64: UFix64}) }
    access(all) view fun getPointsRate_SwapVolume(): UFix64 { return self._pointsRatePerDay["SwapVolume"]! as! UFix64 }
    access(all) view fun getPointsRate_ReferrerUp(): UFix64 { return self._pointsRatePerDay["ReferrerUp"]! as! UFix64 }
    access(all) view fun getPointsRate_RefereeUp(): UFix64 { return self._pointsRatePerDay["RefereeUp"]! as! UFix64 }
    ...
    access(all) view fun getUserState_SwapLP(userAddr: Address): UFix64 { return self._userStates[userAddr]!.containsKey("SwapLpUsd")? self._userStates[userAddr]!["SwapLpUsd"]! : 0.0 }
    ...
    access(self) fun setUserState_SwapLP(userAddr: Address, lpAmount: UFix64) { self._userStates[userAddr]!.insert(key: "SwapLpUsd", lpAmount) }
    ...
    access(all) resource Admin {
        ...
        access(all) fun setPointsRate_stFlowHoldingTier(tierRate: {UFix64: UFix64}) {
            emit PointsTierRateChanged(source: "stFlowHolding", ori: PPPV2._pointsRatePerDay["stFlowHolding"]! as! {UFix64: UFix64}, new: tierRate)
            PPPV2._pointsRatePerDay["stFlowHolding"] = tierRate
        }
        access(all) fun setPointsRate_LendingSupplyTier(tierRate: {UFix64: UFix64}) {
            emit PointsTierRateChanged(source: "LendingSupply", ori: PPPV2._pointsRatePerDay["LendingSupply"]! as! {UFix64: UFix64}, new: tierRate)
            PPPV2._pointsRatePerDay["LendingSupply"] = tierRate
        }
        access(all) fun setPointsRate_LendingBorrowTier(tierRate: {UFix64: UFix64}) {
            emit PointsTierRateChanged(source: "LendingBorrow", ori: PPPV2._pointsRatePerDay["LendingBorrow"]! as! {UFix64: UFix64}, new: tierRate)
            PPPV2._pointsRatePerDay["LendingBorrow"] = tierRate
        }
        access(all) fun setPointsRate_SwapLPTier(tierRate: {UFix64: UFix64}) {
            emit PointsTierRateChanged(source: "SwapLP", ori: PPPV2._pointsRatePerDay["SwapLP"]! as! {UFix64: UFix64}, new: tierRate)
            PPPV2._pointsRatePerDay["SwapLP"] = tierRate
        }
        access(all) fun setPointsRate_SwapVolume(rate: UFix64) {
            emit PointsRateChanged(source: "SwapVolume", ori: PPPV2._pointsRatePerDay["SwapVolume"]! as! UFix64, new: rate)
            PPPV2._pointsRatePerDay["SwapVolume"] = rate
        }
        access(all) fun setPointsRate_ReferrerUp(rate: UFix64) {
            emit PointsRateChanged(source: "ReferrerUp", ori: PPPV2._pointsRatePerDay["ReferrerUp"]! as! UFix64, new: rate)
            PPPV2._pointsRatePerDay["ReferrerUp"] = rate
        }
        access(all) fun setPointsRate_RefereeUp(rate: UFix64) {
            emit PointsRateChanged(source: "RefereeUp", ori: PPPV2._pointsRatePerDay["RefereeUp"]! as! UFix64, new: rate)
            PPPV2._pointsRatePerDay["RefereeUp"] = rate
        }

        // Add Swap Pool in Whiltelist
        access(all) fun addSwapPoolInWhiltelist(poolAddr: Address) {
            PPPV2._swapPoolWhitelist[poolAddr] = true
        }
        // Remove Swap Pool in Whitelist
        access(all) fun removeSwapPoolInWhiltelist(poolAddr: Address) {
            PPPV2._swapPoolWhitelist.remove(key: poolAddr)
        }

        // Set history snapshot points
        access(all) fun setHistorySnapshotPoints(userAddr: Address, newSnapshotBalance: UFix64) {
            if PPPV2._pointsHistorySnapshot.containsKey(userAddr) == false {
                PPPV2._pointsHistorySnapshot[userAddr] = 0.0
            }
            if PPPV2._pointsbase.containsKey(userAddr) == false {
                PPPV2._pointsbase[userAddr] = 0.0
            }
            let preSnapshotBalance = PPPV2._pointsHistorySnapshot[userAddr]!
            ...
            PPPV2._totalSupply = PPPV2._totalSupply - preSnapshotBalance + newSnapshotBalance
            PPPV2._pointsHistorySnapshot[userAddr] = newSnapshotBalance
        }

        // Ban user
        access(all) fun addUserBlackList(userAddr: Address) {
            PPPV2._userBlacklist[userAddr] = true
        }
        access(all) fun removeUserBlackList(userAddr: Address) {
            PPPV2._userBlacklist.remove(key: userAddr)
        }
        access(all) fun reconcileBasePoints(userAddr: Address, newBasePoints: UFix64) {
            ...
            let preBasePoints = PPPV2._pointsbase[userAddr]!
            ...
            PPPV2._totalSupply = PPPV2._totalSupply - preBasePoints + newBasePoints
            PPPV2._pointsbase[userAddr] = newBasePoints
        }        
    }
