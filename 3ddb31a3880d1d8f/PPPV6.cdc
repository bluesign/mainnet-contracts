    ...
    */
    ...
    access(all) resource Admin {
        ...
        access(all) fun setPointsRate_stFlowHoldingTier(tierRate: {UFix64: UFix64}) {
            emit PointsTierRateChanged(source: "stFlowHolding", ori: PPPV6._pointsRate["stFlowHolding"]! as! {UFix64: UFix64}, new: tierRate)
            PPPV6._pointsRate["stFlowHolding"] = tierRate
        }
        access(all) fun setPointsRate_LendingSupplyTier(tierRate: {UFix64: UFix64}) {
            emit PointsTierRateChanged(source: "LendingSupply", ori: PPPV6._pointsRate["LendingSupply"]! as! {UFix64: UFix64}, new: tierRate)
            PPPV6._pointsRate["LendingSupply"] = tierRate
        }
        access(all) fun setPointsRate_LendingBorrowTier(tierRate: {UFix64: UFix64}) {
            emit PointsTierRateChanged(source: "LendingBorrow", ori: PPPV6._pointsRate["LendingBorrow"]! as! {UFix64: UFix64}, new: tierRate)
            PPPV6._pointsRate["LendingBorrow"] = tierRate
        }
        access(all) fun setPointsRate_SwapLPTier(tierRate: {UFix64: UFix64}) {
            emit PointsTierRateChanged(source: "SwapLP", ori: PPPV6._pointsRate["SwapLP"]! as! {UFix64: UFix64}, new: tierRate)
            PPPV6._pointsRate["SwapLP"] = tierRate
        }
        access(all) fun setPointsRate_SwapVolumeTier(tierRate: {UFix64: UFix64}) {
            emit PointsTierRateChanged(source: "SwapVolume", ori: PPPV6._pointsRate["SwapVolume"]! as! {UFix64: UFix64}, new: tierRate)
            PPPV6._pointsRate["SwapVolume"] = tierRate
        }
        access(all) fun setPointsRate_ReferrerUp(rate: UFix64) {
            emit PointsRateChanged(source: "ReferrerUp", ori: PPPV6._pointsRate["ReferrerUp"]! as! UFix64, new: rate)
            PPPV6._pointsRate["ReferrerUp"] = rate
        }
        access(all) fun setPointsRate_RefereeUp(rate: UFix64) {
            emit PointsRateChanged(source: "RefereeUp", ori: PPPV6._pointsRate["RefereeUp"]! as! UFix64, new: rate)
            PPPV6._pointsRate["RefereeUp"] = rate
        }
        access(all) fun setPointsRate_CoreMember(rate: UFix64) {
            emit PointsRateChanged(source: "CoreMember", ori: PPPV6._pointsRate["CoreMember"]! as! UFix64, new: rate)
            PPPV6._pointsRate["CoreMember"] = rate
        }

        // Add Swap Pool in Whiltelist
        access(all) fun addSwapPoolInWhiltelist(poolAddr: Address) {
            PPPV6._swapPoolWhitelist[poolAddr] = true
        }
        // Remove Swap Pool in Whitelist
        access(all) fun removeSwapPoolInWhiltelist(poolAddr: Address) {
            PPPV6._swapPoolWhitelist.remove(key: poolAddr)
        }

        // Set history snapshot points
        access(all) fun setHistorySnapshotPoints(userAddr: Address, newSnapshotBalance: UFix64) {
            if PPPV6._pointsHistorySnapshot.containsKey(userAddr) == false {
                PPPV6._pointsHistorySnapshot[userAddr] = 0.0
            }
            if PPPV6._pointsBase.containsKey(userAddr) == false {
                PPPV6._pointsBase[userAddr] = 0.0
            }
            let preSnapshotBalance = PPPV6._pointsHistorySnapshot[userAddr]!
            ...
            PPPV6._totalSupply = PPPV6._totalSupply - preSnapshotBalance + newSnapshotBalance
            PPPV6._pointsHistorySnapshot[userAddr] = newSnapshotBalance
        }
        access(all) fun setClaimEndTimestamp(endTime: UFix64) {
            PPPV6._claimEndTimestamp = endTime
        }

        // Ban user
        access(all) fun addUserBlackList(userAddr: Address) {
            PPPV6._userBlacklist[userAddr] = true
        }
        access(all) fun removeUserBlackList(userAddr: Address) {
            PPPV6._userBlacklist.remove(key: userAddr)
        }

        access(all) fun addVolumePoints(userAddr: Address, volumePoints: UFix64, volumeUsd: UFix64) {
            PPPV6._mint(targetAddr: userAddr, amount: volumePoints)

            if PPPV6._userStates.containsKey(userAddr) == false {
                PPPV6._userStates[userAddr] = {}
            }
            PPPV6.setUserState_SwapVolume(userAddr: userAddr, volume: PPPV6.getUserState_SwapVolume(userAddr: userAddr) + volumeUsd)
            emit PointsMinted(
                userAddr: userAddr,
                amount: volumePoints,
                source: "SwapVolume", 
            ...
            emit StateUpdated(userAddr: userAddr, state: PPPV6._userStates[userAddr]!)
        }
        ...
        access(all) fun reconcileBasePoints(userAddr: Address, newBasePoints: UFix64) {
            ...
            let preBasePoints = PPPV6._pointsBase[userAddr]!
            ...
            PPPV6._totalSupply = PPPV6._totalSupply - preBasePoints + newBasePoints
            PPPV6._pointsBase[userAddr] = newBasePoints
        }

        access(all) fun updateTopUsers(addrs: [Address]) {
            emit TopUsersChanged(ori: PPPV6._topUsers, new: addrs)
            PPPV6._topUsers = addrs
        }     
    }
