...
pub contract SGKCLDR_destro016: NonFungibleToken, Anique {
    // -----------------------------------------------------------------------
    // SGKCLDR_destro016 contract Events
    // -----------------------------------------------------------------------

    // Events for Contract-Related actions
    //
    // Emitted when the SGKCLDR_destro016 contract is created
    ...
    // -----------------------------------------------------------------------
    // SGKCLDR_destro016 contract-level fields.
    ...
    // -----------------------------------------------------------------------
    // SGKCLDR_destro016 contract-level Composite Type definitions
    // -----------------------------------------------------------------------

    // The structure that represents Item
    // each digital content which SGKCLDR_destro016 deal with on Flow
    //
    pub struct ItemData {
        ...
        init(itemID: UInt32) {
            let item = (&SGKCLDR_destro016.items[itemID] as &Item?)!
    ...
    pub resource Item {
        ...
        init(metadata: {String: String}) {
            ...
            self.itemID = SGKCLDR_destro016.nextItemID
            self.metadata = metadata
            self.numberMintedPerItem = 0

            // increment the nextItemID so that it isn't used again
            SGKCLDR_destro016.nextItemID = SGKCLDR_destro016.nextItemID + 1
    ...
    pub resource NFT: NonFungibleToken.INFT, Anique.INFT {
        ...
        init(serialNumber: UInt32, itemID: UInt32) {
            ...
            SGKCLDR_destro016.totalSupply = SGKCLDR_destro016.totalSupply + 1

            // set id
            self.id = SGKCLDR_destro016.totalSupply
    ...
    // interface that represents SGKCLDR_destro016 collections to public
    ...
    pub resource interface CollectionPublic {
        ...
        // borrow NFT as SGKCLDR_destro016 token
        pub fun borrowSGKCLDR_destro016Collectible(id: UInt64): auth &NFT
    }
    ...
    pub resource Collection: CollectionPublic, NonFungibleToken.Receiver, NonFungibleToken.Provider, NonFungibleToken.CollectionPublic {
        ...
        pub fun deposit(token: @NonFungibleToken.NFT) {
            ...
            // Cast the deposited token as an SGKCLDR_destro016 NFT to make sure
            // it is the correct type
            let token <- token as! @SGKCLDR_destro016.NFT
        ...
        // not any SGKCLDR_destro016 specific data. Please use borrowCollectible to
        ...
        // borrowSGKCLDR_destro016Collectible returns a borrowed reference
        // to an SGKCLDR_destro016 Collectible
        pub fun borrowSGKCLDR_destro016Collectible(id: UInt64): auth &NFT {
            pre {
    ...
    pub resource Admin {
        ...
        // and stores it in the Items dictionary field in the SGKCLDR_destro016 smart contract
        //
        ...
        pub fun createItem(metadata: {String: String}): UInt32 {
            ...
            SGKCLDR_destro016.items[newItem.itemID] <-! newItem

            return itemId
        }

        // borrowItem returns a reference to a Item in the SGKCLDR_destro016
        ...
        pub fun borrowItem(itemID: UInt32): &Item {
            pre {
                SGKCLDR_destro016.items[itemID] != nil: "Cannot borrow Item: The Item doesn't exist"
            }

            return (&SGKCLDR_destro016.items[itemID] as &Item?)!
        }
    ...
    // -----------------------------------------------------------------------
    // SGKCLDR_destro016 contract-level function definitions
    // -----------------------------------------------------------------------
    ...
    pub fun createEmptyCollection(): @NonFungibleToken.Collection {
        return <-create SGKCLDR_destro016.Collection()
    }
    ...
    // -----------------------------------------------------------------------
    // SGKCLDR_destro016 initialization function
    // -----------------------------------------------------------------------
    //
    init() {
        ...
        self.collectionStoragePath     = /storage/SGKCLDR_destro016Collection
        self.collectionPublicPath      =  /public/SGKCLDR_destro016Collection
        self.collectionPrivatePath     = /private/SGKCLDR_destro016Collection
        self.adminStoragePath          = /storage/SGKCLDR_destro016Admin
        self.saleCollectionStoragePath = /storage/SGKCLDR_destro016SaleCollection
        self.saleCollectionPublicPath  =  /public/SGKCLDR_destro016SaleCollection
