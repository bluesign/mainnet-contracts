...
pub contract Flowty {
    ...
    pub struct ListingDetails {
        ...
        init (
            ...
            let fundingFeeAmount = self.amount * self.interestRate * Flowty.FundingFee
            ...
            assert(cutsAmount + fundingFeeAmount == amount, message: "The cut amount of the borrower is invalid, the funding fee is not satisfied")
        }
    }
    ...
    pub resource Listing: ListingPublic {
        ...
        pub fun fund(payment: @FungibleToken.Vault, 
            ...
            lenderNFTCollection: Capability<&AnyResource{NonFungibleToken.CollectionPublic}>) {
            pre {
                self.isFundingEnabled(): "Funding is not enabled"
                ...
                payment.balance == self.details.amount: "payment vault does not contain requested amount"
            }
    ...
    init () {
        ...
        self.ListingFee = 0.25 // Fixed FUSD
        self.FundingFee = 0.05 // Percentage of the interest, a number between 0 and 1.
