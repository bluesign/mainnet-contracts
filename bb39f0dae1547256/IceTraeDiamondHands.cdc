...
pub contract IceTraeDiamondHands: NonFungibleToken {
			...
			switch view {
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(
						storagePath: IceTraeDiamondHands.CollectionStoragePath,
						publicPath: IceTraeDiamondHands.CollectionPublicPath,
						providerPath: IceTraeDiamondHands.CollectionPrivatePath,
						...
						createEmptyCollectionFunction: (fun (): @NonFungibleToken.Collection {
								return <- IceTraeDiamondHands.createEmptyCollection()
						})
					)
          ...
          return MetadataViews.ExternalURL("https://touchstone.city/discover/".concat(self.owner!.address.toString()).concat("/IceTraeDiamondHands"))
				case Type<MetadataViews.NFTCollectionDisplay>():
					let squareMedia = MetadataViews.Media(
						file: IceTraeDiamondHands.getCollectionAttribute(key: "image") as! MetadataViews.IPFSFile,
						mediaType: "image"
					)
					...
					return MetadataViews.NFTCollectionDisplay(
						name: IceTraeDiamondHands.getCollectionAttribute(key: "name") as! String,
						description: IceTraeDiamondHands.getCollectionAttribute(key: "description") as! String,
						externalURL: MetadataViews.ExternalURL("https://touchstone.city/discover/".concat(self.owner!.address.toString()).concat("/IceTraeDiamondHands")),
						squareImage: squareMedia,
						bannerImage: bannerMedia ?? squareMedia,
						socials: IceTraeDiamondHands.getCollectionAttribute(key: "socials") as! {String: MetadataViews.ExternalURL}
					)
	...
	pub resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection {
		...
		pub fun claim() {
			if let storage = &IceTraeDiamondHands.nftStorage[self.owner!.address] as &{UInt64: NFT}? {
	...
	pub resource Administrator {
		pub fun createNFTMetadata(name: String, description: String, imagePath: String, thumbnailPath: String?, ipfsCID: String, price: UFix64?, extra: {String: AnyStruct}) {
			IceTraeDiamondHands.metadatas[IceTraeDiamondHands.nextMetadataId] = NFTMetadata(
				_name: name,
		...
		pub fun mintNFT(metadataId: UInt64, recipient: Address) {
			pre {
				EmeraldPass.isActive(user: IceTraeDiamondHands.account.address): "You must have an active Emerald Pass subscription to airdrop NFTs. You can purchase Emerald Pass at https://pass.ecdao.org/"
			}
			let nft <- create NFT(_metadataId: metadataId, _recipient: recipient)
			if let recipientCollection = getAccount(recipient).getCapability(IceTraeDiamondHands.CollectionPublicPath).borrow<&IceTraeDiamondHands.Collection{NonFungibleToken.CollectionPublic}>() {
				recipientCollection.deposit(token: <- nft)
			} else {
				if let storage = &IceTraeDiamondHands.nftStorage[recipient] as &{UInt64: NFT}? {
					storage[nft.id] <-! nft
				} else {
	...
	pub fun getClaimableNFTs(user: Address): {UInt64: NFTMetadata} {
		...
		if let storage = &IceTraeDiamondHands.nftStorage[user] as &{UInt64: NFT}? {
			for id in storage.keys {
