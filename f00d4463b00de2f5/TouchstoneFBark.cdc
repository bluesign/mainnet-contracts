...
pub contract TouchstoneFBark: NonFungibleToken {
			...
			switch view {
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(
						storagePath: TouchstoneFBark.CollectionStoragePath,
						publicPath: TouchstoneFBark.CollectionPublicPath,
						providerPath: TouchstoneFBark.CollectionPrivatePath,
          ...
          return MetadataViews.ExternalURL("https://touchstone.city/discover/".concat(self.owner!.address.toString()).concat("/TouchstoneFBark"))
				case Type<MetadataViews.NFTCollectionDisplay>():
					let squareMedia = MetadataViews.Media(
						file: TouchstoneFBark.getCollectionAttribute(key: "image") as! MetadataViews.IPFSFile,
						mediaType: "image"
					)
					...
					return MetadataViews.NFTCollectionDisplay(
						name: TouchstoneFBark.getCollectionAttribute(key: "name") as! String,
						description: TouchstoneFBark.getCollectionAttribute(key: "description") as! String,
						externalURL: MetadataViews.ExternalURL("https://touchstone.city/discover/".concat(self.owner!.address.toString()).concat("/TouchstoneFBark")),
						squareImage: squareMedia,
						bannerImage: bannerMedia ?? squareMedia,
						socials: TouchstoneFBark.getCollectionAttribute(key: "socials") as! {String: MetadataViews.ExternalURL}
					)
	...
	pub resource Administrator {
		pub fun createNFTMetadata(name: String, description: String, imagePath: String, thumbnailPath: String?, ipfsCID: String, price: UFix64?, extra: {String: AnyStruct}) {
			TouchstoneFBark.metadatas[TouchstoneFBark.nextMetadataId] = NFTMetadata(
				_name: name,
		...
		pub fun mintNFT(metadataId: UInt64, recipient: Address) {
			pre {
				EmeraldPass.isActive(user: TouchstoneFBark.account.address): "You must have an active Emerald Pass subscription to airdrop NFTs. You can purchase Emerald Pass at https://pass.ecdao.org/"
			}
			...
			if let recipientCollection = getAccount(recipient).getCapability(TouchstoneFBark.CollectionPublicPath).borrow<&TouchstoneFBark.Collection{NonFungibleToken.CollectionPublic}>() {
				...
				if let storage = &TouchstoneFBark.nftStorage[recipient] as &{UInt64: NFT}? {
					storage[nft.id] <-! nft
				} else {
