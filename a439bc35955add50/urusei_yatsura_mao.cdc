...
pub contract urusei_yatsura_mao: NonFungibleToken, Anique {
    // -----------------------------------------------------------------------
    // urusei_yatsura_mao contract Events
    // -----------------------------------------------------------------------

    // Events for Contract-Related actions
    //
    // Emitted when the urusei_yatsura_mao contract is created
    ...
    // -----------------------------------------------------------------------
    // urusei_yatsura_mao contract-level fields.
    ...
    // -----------------------------------------------------------------------
    // urusei_yatsura_mao contract-level Composite Type definitions
    // -----------------------------------------------------------------------

    // The structure that represents Item
    // each digital content which urusei_yatsura_mao deal with on Flow
    //
    pub struct ItemData {
        ...
        init(itemID: UInt32) {
            let item = (&urusei_yatsura_mao.items[itemID] as &Item?)!
    ...
    pub resource Item {
        ...
        init(metadata: {String: String}) {
            ...
            self.itemID = urusei_yatsura_mao.nextItemID
            self.metadata = metadata
            self.numberMintedPerItem = 0

            // increment the nextItemID so that it isn't used again
            urusei_yatsura_mao.nextItemID = urusei_yatsura_mao.nextItemID + 1
    ...
    pub resource NFT: NonFungibleToken.INFT, Anique.INFT {
        ...
        init(serialNumber: UInt32, itemID: UInt32) {
            ...
            urusei_yatsura_mao.totalSupply = urusei_yatsura_mao.totalSupply + 1

            // set id
            self.id = urusei_yatsura_mao.totalSupply
    ...
    // interface that represents urusei_yatsura_mao collections to public
    ...
    pub resource interface CollectionPublic {
        ...
        // borrow NFT as urusei_yatsura_mao token
        pub fun borrowurusei_yatsura_maoCollectible(id: UInt64): auth &NFT
    }
    ...
    pub resource Collection: CollectionPublic, NonFungibleToken.Receiver, NonFungibleToken.Provider, NonFungibleToken.CollectionPublic {
        ...
        pub fun deposit(token: @NonFungibleToken.NFT) {
            ...
            // Cast the deposited token as an urusei_yatsura_mao NFT to make sure
            // it is the correct type
            let token <- token as! @urusei_yatsura_mao.NFT
        ...
        // not any urusei_yatsura_mao specific data. Please use borrowCollectible to
        ...
        // borrowurusei_yatsura_maoCollectible returns a borrowed reference
        // to an urusei_yatsura_mao Collectible
        pub fun borrowurusei_yatsura_maoCollectible(id: UInt64): auth &NFT {
            pre {
    ...
    pub resource Admin {
        ...
        // and stores it in the Items dictionary field in the urusei_yatsura_mao smart contract
        //
        ...
        pub fun createItem(metadata: {String: String}): UInt32 {
            ...
            urusei_yatsura_mao.items[newItem.itemID] <-! newItem

            return itemId
        }

        // borrowItem returns a reference to a Item in the urusei_yatsura_mao
        ...
        pub fun borrowItem(itemID: UInt32): &Item {
            pre {
                urusei_yatsura_mao.items[itemID] != nil: "Cannot borrow Item: The Item doesn't exist"
            }

            return (&urusei_yatsura_mao.items[itemID] as &Item?)!
        }
    ...
    // -----------------------------------------------------------------------
    // urusei_yatsura_mao contract-level function definitions
    // -----------------------------------------------------------------------
    ...
    pub fun createEmptyCollection(): @NonFungibleToken.Collection {
        return <-create urusei_yatsura_mao.Collection()
    }
    ...
    // -----------------------------------------------------------------------
    // urusei_yatsura_mao initialization function
    // -----------------------------------------------------------------------
    //
    init() {
        ...
        self.collectionStoragePath     = /storage/urusei_yatsura_maoCollection
        self.collectionPublicPath      =  /public/urusei_yatsura_maoCollection
        self.collectionPrivatePath     = /private/urusei_yatsura_maoCollection
        self.adminStoragePath          = /storage/urusei_yatsura_maoAdmin
        self.saleCollectionStoragePath = /storage/urusei_yatsura_maoSaleCollection
        self.saleCollectionPublicPath  =  /public/urusei_yatsura_maoSaleCollection
