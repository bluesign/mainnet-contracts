...
pub contract guiguikuru: NonFungibleToken, Anique {
    // -----------------------------------------------------------------------
    // guiguikuru contract Events
    // -----------------------------------------------------------------------

    // Events for Contract-Related actions
    //
    // Emitted when the guiguikuru contract is created
    ...
    // -----------------------------------------------------------------------
    // guiguikuru contract-level fields.
    ...
    // -----------------------------------------------------------------------
    // guiguikuru contract-level Composite Type definitions
    // -----------------------------------------------------------------------

    // The structure that represents Item
    // each digital content which guiguikuru deal with on Flow
    //
    pub struct ItemData {
        ...
        init(itemID: UInt32) {
            let item = (&guiguikuru.items[itemID] as &Item?)!
    ...
    pub resource NFT: NonFungibleToken.INFT, Anique.INFT {
        ...
        init(serialNumber: UInt32, itemID: UInt32) {
            ...
            guiguikuru.totalSupply = guiguikuru.totalSupply + 1

            // set id
            self.id = guiguikuru.totalSupply
    ...
    // interface that represents guiguikuru collections to public
    ...
    pub resource interface CollectionPublic {
        ...
        // borrow NFT as guiguikuru token
        pub fun borrowguiguikuruCollectible(id: UInt64): auth &NFT
    }
    ...
    pub resource Collection: CollectionPublic, NonFungibleToken.Receiver, NonFungibleToken.Provider, NonFungibleToken.CollectionPublic {
        ...
        pub fun deposit(token: @NonFungibleToken.NFT) {
            ...
            // Cast the deposited token as an guiguikuru NFT to make sure
            // it is the correct type
            let token <- token as! @guiguikuru.NFT
        ...
        // not any guiguikuru specific data. Please use borrowCollectible to
        ...
        // borrowguiguikuruCollectible returns a borrowed reference
        // to an guiguikuru Collectible
        pub fun borrowguiguikuruCollectible(id: UInt64): auth &NFT {
            pre {
    ...
    pub resource Admin {
        ...
        // and stores it in the Items dictionary field in the guiguikuru smart contract
        //
        ...
        pub fun createItem(metadata: {String: String}): UInt32 {
            ...
            guiguikuru.items[newItem.itemID] <-! newItem

            return itemId
        }

        // borrowItem returns a reference to a Item in the guiguikuru
        ...
        pub fun borrowItem(itemID: UInt32): &Item {
            pre {
                guiguikuru.items[itemID] != nil: "Cannot borrow Item: The Item doesn't exist"
            }

            return (&guiguikuru.items[itemID] as &Item?)!
        }
    ...
    // -----------------------------------------------------------------------
    // guiguikuru contract-level function definitions
    // -----------------------------------------------------------------------
    ...
    pub fun createEmptyCollection(): @NonFungibleToken.Collection {
        return <-create guiguikuru.Collection()
    }
    ...
    // -----------------------------------------------------------------------
    // guiguikuru initialization function
    // -----------------------------------------------------------------------
    //
    init() {
        ...
        self.collectionStoragePath     = /storage/guiguikuruCollection
        self.collectionPublicPath      =  /public/guiguikuruCollection
        self.collectionPrivatePath     = /private/guiguikuruCollection
        self.adminStoragePath          = /storage/guiguikuruAdmin
        self.saleCollectionStoragePath = /storage/guiguikuruSaleCollection
        self.saleCollectionPublicPath  =  /public/guiguikuruSaleCollection
