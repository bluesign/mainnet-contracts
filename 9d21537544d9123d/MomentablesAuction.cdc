...
pub contract MomentablesAuction {
    ...
    pub event Created(tokenID: UInt64, owner: Address, startPrice: UFix64, startTime: UFix64, endTime: Fix64, timeRemaining:Fix64, itemID: UInt64, momentableId:String)
    ...
    pub resource AuctionItem {
        ...
        priv var recipientCollectionCap: Capability<&{NonFungibleToken.CollectionPublic}>?
        ...
        priv let ownerCollectionCap: Capability<&{NonFungibleToken.CollectionPublic}>
    ...
    pub resource AuctionCollection: AuctionPublic {
        ...
        pub fun createAuction(
            ...
            let item <- MomentablesAuction.createStandaloneAuction(
                token: <-token,
        ...
        pub fun getAuctionStatuses(): {UInt64: AnyStruct} {
            ...
            for id in self.auctionItems.keys {
                ...
                priceList.insert(key: id, {
                  "auctionDetails": itemRef.getAuctionStatus(),
                 "momentableDetails": {
                 "momentableId":self.auctionItems[id]?.NFT?.momentableId,
                 "name":self.auctionItems[id]?.NFT?.name,
                 "description":self.auctionItems[id]?.NFT?.description,
                 "itemID":self.auctionItems[id]?.NFT?.id
                 }})
            }
