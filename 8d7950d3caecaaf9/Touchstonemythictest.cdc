...
pub contract Touchstonemythictest: NonFungibleToken {
			...
			switch view {
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(
						storagePath: Touchstonemythictest.CollectionStoragePath,
						publicPath: Touchstonemythictest.CollectionPublicPath,
						providerPath: Touchstonemythictest.CollectionPrivatePath,
          ...
          return MetadataViews.ExternalURL("https://touchstone.city/discover/".concat(self.owner!.address.toString()).concat("/Touchstonemythictest"))
				case Type<MetadataViews.NFTCollectionDisplay>():
					let squareMedia = MetadataViews.Media(
						file: Touchstonemythictest.getCollectionAttribute(key: "image") as! MetadataViews.IPFSFile,
						mediaType: "image"
					)
					...
					return MetadataViews.NFTCollectionDisplay(
						name: Touchstonemythictest.getCollectionAttribute(key: "name") as! String,
						description: Touchstonemythictest.getCollectionAttribute(key: "description") as! String,
						externalURL: MetadataViews.ExternalURL("https://touchstone.city/discover/".concat(self.owner!.address.toString()).concat("/Touchstonemythictest")),
						squareImage: squareMedia,
						bannerImage: bannerMedia ?? squareMedia,
						socials: Touchstonemythictest.getCollectionAttribute(key: "socials") as! {String: MetadataViews.ExternalURL}
					)
	...
	pub resource Administrator {
		pub fun createNFTMetadata(name: String, description: String, imagePath: String, thumbnailPath: String?, ipfsCID: String, price: UFix64?, extra: {String: AnyStruct}, supply: UInt64) {
			Touchstonemythictest.metadatas[Touchstonemythictest.nextMetadataId] = NFTMetadata(
				_name: name,
		...
		pub fun mintNFT(metadataId: UInt64, serial: UInt64, recipient: Address) {
			...
			if let recipientCollection = getAccount(recipient).getCapability(Touchstonemythictest.CollectionPublicPath).borrow<&Touchstonemythictest.Collection{NonFungibleToken.CollectionPublic}>() {
				...
				if let storage = &Touchstonemythictest.nftStorage[recipient] as &{UInt64: NFT}? {
					storage[nft.id] <-! nft
				} else {
