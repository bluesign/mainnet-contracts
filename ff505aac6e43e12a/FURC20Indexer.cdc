    ...
    access(all) resource InscriptionIndexer: IndexerPublic {
        ...
        fun getTokenDisplay(tick: String): FungibleTokenMetadataViews.FTDisplay? {
            ...
            let svgStr = "data:image/svg+xml;utf8,"
                ...
                .concat("<text style=\"dominant-baseline: middle; fill: rgb(80, 213, 155); font-family: system-ui, sans-serif; text-anchor: middle;\" fill-opacity=\"0.2\" y=\"-12\" font-size=\"420\">ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ</text>")
        ...
        fun mint(ins: &FlowUp.Inscription, flowToCreator: @FlowToken.Vault, flowToInviter: @FlowToken.Vault, inviterAddress: Address) {
            ...
            assert(
                flowToCreatorAmount >= 70.0,
                message: "The flowToCreatorAmount should be greater than 70.0"
            )

            let flowToInviterAmount = flowToInviter.balance
            assert(
                flowToInviterAmount >= 30.0,
                message: "The flowToInviterAmount should be greater than 30.0"
            )
            ...
            let creatorRecipientVaultRef = getAccount(creatorRecipient)
            .getCapability(/public/flowTokenReceiver)
            .borrow<&FlowToken.Vault{FungibleToken.Receiver}>()
            ?? panic("Could not get creator receiver reference")
            ...
            if inviterFlowUpBalance < 1.0 || fromAddr == inviterAddress {
                ...
                let inviterRecipientVaultRef = getAccount(inviterAddress)
                .getCapability(/public/flowTokenReceiver)
                .borrow<&FlowToken.Vault{FungibleToken.Receiver}>()
                ?? panic("Could not get inviter receiver reference")
        ...
        fun transfer(ins: &FlowUp.Inscription, flowToCreator: @FlowToken.Vault) {
            ...
            let flowToCreatorAmount = flowToCreator.balance
            assert(
                flowToCreatorAmount >= 0.5,
                message: "The flowToCreatorAmount should be greater than 0.5"
            )
