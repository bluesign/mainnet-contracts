...
pub contract TestContract1019: NonFungibleToken, Anique {
    // -----------------------------------------------------------------------
    // TestContract1019 contract Events
    // -----------------------------------------------------------------------

    // Events for Contract-Related actions
    //
    // Emitted when the TestContract1019 contract is created
    ...
    // -----------------------------------------------------------------------
    // TestContract1019 contract-level fields.
    ...
    // -----------------------------------------------------------------------
    // TestContract1019 contract-level Composite Type definitions
    // -----------------------------------------------------------------------

    // The structure that represents Item
    // each digital content which TestContract1019 deal with on Flow
    //
    pub struct ItemData {
    ...
    pub resource Item {
        ...
        init(metadata: {String: String}) {
            ...
            self.itemID = TestContract1019.nextItemID
            self.metadata = metadata
            self.numberMintedPerItem = 0

            // increment the nextItemID so that it isn't used again
            TestContract1019.nextItemID = TestContract1019.nextItemID + 1
    ...
    pub resource NFT: NonFungibleToken.INFT, Anique.INFT {
        ...
        init(serialNumber: UInt32, itemID: UInt32) {
            ...
            TestContract1019.totalSupply = TestContract1019.totalSupply + 1

            // set id
            self.id = TestContract1019.totalSupply
    ...
    // interface that represents TestContract1019 collections to public
    ...
    pub resource interface CollectionPublic {
        ...
        pub fun borrowTestContract1019Collectible(id: UInt64): auth &NFT
    }
    ...
    pub resource Collection: CollectionPublic, NonFungibleToken.Receiver, NonFungibleToken.Provider, NonFungibleToken.CollectionPublic {
        ...
        pub fun deposit(token: @NonFungibleToken.NFT) {
            ...
            let token <- token as! @TestContract1019.NFT
        ...
        // not any TestContract1019 specific data. Please use borrowCollectible to
    ...
    pub resource Admin {
        ...
        // and stores it in the Items dictionary field in the TestContract1019 smart contract
        //
        ...
        pub fun createItem(metadata: {String: String}): UInt32 {
            ...
            TestContract1019.items[newItem.itemID] <-! newItem

            return itemId
        }

        // borrowItem returns a reference to a Item in the TestContract1019
        ...
        pub fun borrowItem(itemID: UInt32): &Item {
            pre {
                TestContract1019.items[itemID] != nil: "Cannot borrow Item: The Item doesn't exist"
            }

            return (&TestContract1019.items[itemID] as &Item?)!
        }
    ...
    // -----------------------------------------------------------------------
    // TestContract1019 contract-level function definitions
    // -----------------------------------------------------------------------
    ...
    pub fun createEmptyCollection(): @NonFungibleToken.Collection {
        return <-create TestContract1019.Collection()
    }
    ...
    // -----------------------------------------------------------------------
    // TestContract1019 initialization function
    // -----------------------------------------------------------------------
    //
    init() {
        ...
        self.collectionStoragePath     = /storage/TestContract1019Collection
        self.collectionPublicPath      =  /public/TestContract1019Collection
        self.collectionPrivatePath     = /private/TestContract1019Collection
        self.adminStoragePath          = /storage/TestContract1019Admin
        self.saleCollectionStoragePath = /storage/TestContract1019SaleCollection
        self.saleCollectionPublicPath  =  /public/TestContract1019SaleCollection
