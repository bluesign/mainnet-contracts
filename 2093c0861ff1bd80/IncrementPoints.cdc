...
pub contract IncrementPoints {
    ...
    access(self) let _swapPoolWhitelist: {Address: Bool}
    ...
    */
    access(self) let _pointsRate: {String: AnyStruct}

    // The duration in seconds for each points rate session, affecting how points accumulate over time.
    access(self) let _secondsPerPointsRateSession: UFix64
    ...
    access(self) let _ifClaimHistorySnapshot: {Address: Bool}
    access(self) var _claimEndTimestamp: UFix64

    access(self) var _pointsMintEndTimestamp: UFix64
    
    // A list of user addresses ranked by their total points, representing the top users.
    access(self) var _topUsers: [Address]

    access(self) var _lastVolumeAndRefereePointsUpdateTimestamp: UFix64
    ...
    access(all) event PointsBurned(userAddr: Address, amount: UFix64, source: String, param: {String: String})
    ...
    access(all) event ClaimSnapshotPoints(userAddr: Address, amount: UFix64)
    access(all) event TopUsersChanged(ori: [Address], new: [Address])
    access(all) event SetSnapshotPoints(userAddr: Address, pre: UFix64, new: UFix64)
    access(all) event SpecificSwapLpUsdRatioChange(poolAddr: Address, usdRatio: UFix64)
    ...
    access(all) view fun balanceOf(_ userAddr: Address): UFix64 {
        ...
        return self.getBasePoints(userAddr)
               ...
               self.getPointsAsCoreMember(userAddr)
                +
               self.calculateNewPointsSinceLastUpdate(userAddr: userAddr) * (1.0 + self.getBasePointsBoostRate(userAddr))  // accured points
    }
    ...
    access(self) fun _mint(targetAddr: Address, amount: UFix64) {
        ...
        if referrerAddr != nil {
            ...
            if boostAmountForReferrer > 0.0 && self._userBlacklist.containsKey(referrerAddr!) == false {
                ...
                emit PointsMinted(
                    userAddr: referrerAddr!,
                    amount: boostAmountForReferrer,
                    source: "AsReferrer",
                    param: {}
                )
        ...
        if self.isCoreMember(targetAddr) {
            ...
            if boostAmountAsCoreMember > 0.0 {
                ...
                emit PointsMinted(
                    userAddr: targetAddr,
                    amount: boostAmountAsCoreMember,
                    source: "AsCoreMember",
                    param: {}
                )
    ...
    access(all) view fun calculateNewPointsSinceLastUpdate(userAddr: Address): UFix64 {
        let lastUpdateTimestamp = self.getUserState_LastUpdateTimestamp(userAddr: userAddr)
        if lastUpdateTimestamp == 0.0 { return 0.0 }
        if self._userBlacklist.containsKey(userAddr) { return 0.0 }
        ...
        return accuredLendingSupplyPoints + accuredLendingBorrowPoints + accuredStFlowHoldingPoints + accuredSwapLPPoints
    ...
    access(all) fun updateUserState(userAddr: Address) {
        ...
        let lastUpdateTimestamp = self.getUserState_LastUpdateTimestamp(userAddr: userAddr)
        var curTimestamp = (getCurrentBlock().timestamp<self._pointsMintEndTimestamp)? getCurrentBlock().timestamp:self._pointsMintEndTimestamp
        ...
        if duration > 0.0 {
            ...
            if (accuredLendingSupplyPoints > 0.0) {
                emit PointsMinted(
                    userAddr: userAddr,
                    amount: accuredLendingSupplyPoints,
                    source: "LendingSupply",
                    param: {
                        "SupplyUsdValue": self.getUserState_LendingSupply(userAddr: userAddr).toString(),
            ...
            if (accuredLendingBorrowPoints > 0.0) {
                emit PointsMinted(
                    userAddr: userAddr,
                    amount: accuredLendingBorrowPoints,
                    source: "LendingBorrow",
                    param: {
                        "BorrowUsdValue": self.getUserState_LendingBorrow(userAddr: userAddr).toString(),
                        "Duration": durationStr
                    }
                )
            }
            ...
            if (accuredStFlowHoldingPoints > 0.0) {
                emit PointsMinted(
                    userAddr: userAddr,
                    amount: accuredStFlowHoldingPoints,
                    source: "stFlowHolding",
                    param: {
                        "stFlowHoldingBalance": self.getUserState_stFlowHolding(userAddr: userAddr).toString(),
                        "Duration": durationStr
                    }
                )
            }
            ...
            if (accuredSwapLPPoints > 0.0) {
                emit PointsMinted(
                    userAddr: userAddr,
                    amount: accuredSwapLPPoints,
                    source: "SwapLP",
                    param: {
                        "SwapLPUsdValue": self.getUserState_SwapLPUsd(userAddr: userAddr).toString(),
                        "Duration": durationStr
                    }
                )
            }
            ...
            let accuredPointsToMint = 
                accuredLendingSupplyPoints +
                accuredLendingBorrowPoints +
            ...
            || totalLpBalanceUsd > 0.0
        {
            ...
            self.setUserState_LendingSupply(userAddr: userAddr, supplyAmount: totalSupplyAmountInUsd)
            self.setUserState_LendingBorrow(userAddr: userAddr, borrowAmount: totalBorrowAmountInUsd)
            ...
            self.setUserState_LastUpdateTimestamp(userAddr: userAddr, timestamp: getCurrentBlock().timestamp)
    ...
    access(all) view fun fetchOnchainUserStates(userAddr: Address): [UFix64] {
        // Oracle Price
    ...
    access(all) fun claimSnapshotPoints(userCertificateCap: Capability<&{LendingInterfaces.IdentityCertificate}>) {
        pre {
            self._claimEndTimestamp > getCurrentBlock().timestamp: "The claim period has concluded."
        }
        ...
        emit ClaimSnapshotPoints(userAddr: userAddr, amount: self.getHistorySnapshotPoints(userAddr))
    }

    access(all) view fun getBasePointsLength(): Int {
        return self._pointsBase.length
    }

    access(all) view fun getSlicedBasePointsAddrs(from: Int, to: Int): [Address] {
    ...
    access(all) view fun getBasePoints(_ userAddr: Address): UFix64 {
        return self._pointsBase.containsKey(userAddr)? self._pointsBase[userAddr]! : 0.0
    }
    
    access(all) view fun getHistorySnapshotPoints(_ userAddr: Address): UFix64 {
        return self._pointsHistorySnapshot.containsKey(userAddr)? self._pointsHistorySnapshot[userAddr]! : 0.0
    }

    access(all) view fun getPointsAsReferrer(_ userAddr: Address): UFix64 {
        return self._totalPointsAsReferrer.containsKey(userAddr)? self._totalPointsAsReferrer[userAddr]!: 0.0
    }

    access(all) view fun getPointsAsReferee(_ userAddr: Address): UFix64 {
        return self._pointsAsReferee.containsKey(userAddr)? self._pointsAsReferee[userAddr]!: 0.0
    }

    access(all) view fun getLastestPointsAsReferee(_ userAddr: Address): UFix64 {
    ...
    access(all) view fun getPointsAsCoreMember(_ userAddr: Address): UFix64 {
        return self._pointsAsCoreMember.containsKey(userAddr)? self._pointsAsCoreMember[userAddr]!: 0.0
    }

    access(all) view fun getLastestPointsAsCoreMember(_ userAddr: Address): UFix64 {
    ...
    access(all) view fun getPointsAndTimestamp(_ userAddr: Address): [UFix64; 3] {
        return [self.balanceOf(userAddr), self.getPointsAsReferrer(userAddr), self.getUserState_LastUpdateTimestamp(userAddr: userAddr)]
    }

    access(all) view fun getUserInfo(_ userAddr: Address): [UFix64] {
        return [
    ...
    access(all) view fun getUserInfosLength(): Int {
        return self._userStates.length
    }

    access(all) view fun getSlicedUserInfos(from: Int, to: Int): {Address: [UFix64]} {
        let len = self._userStates.length
        let endIndex = to > len ? len : to
        var curIndex = from
    ...
    access(all) view fun isCoreMember(_ userAddr: Address): Bool {
        let coreMemberEventID: UInt64 = 326723707
        let floatCollection = getAccount(userAddr).getCapability(FLOAT.FLOATCollectionPublicPath).borrow<&FLOAT.Collection{FLOAT.CollectionPublic}>()
        if floatCollection == nil {
            return false
        }
        let nftID: [UInt64] = floatCollection!.ownedIdsFromEvent(eventId: coreMemberEventID)
        if floatCollection!.getType().identifier == "A.2d4c3caffbeab845.FLOAT.Collection" && nftID.length > 0{
            return true
        }
        let poolCollectionCap = getAccount(StakingNFT.address).getCapability<&{StakingNFT.PoolCollectionPublic}>(StakingNFT.CollectionPublicPath).borrow()!
    ...
    access(all) view fun ifClaimHistorySnapshot(_ userAddr: Address): Bool {
    ...
    access(all) view fun getBasePointsBoostRate(_ userAddr: Address): UFix64 {
        let referrerAddr = IncrementReferral.getReferrerByReferee(referee: userAddr)
    ...
    access(all) view fun calculateNewPointsSinceLastUpdate_LendingSupply(userAddr: Address): UFix64 {
        let lastUpdateTimestamp = self.getUserState_LastUpdateTimestamp(userAddr: userAddr)
        var accuredPoints = 0.0
        if lastUpdateTimestamp == 0.0 { return 0.0 }
        let currUpdateTimestamp = (getCurrentBlock().timestamp<self._pointsMintEndTimestamp)? getCurrentBlock().timestamp:self._pointsMintEndTimestamp
    ...
    access(all) view fun calculateNewPointsSinceLastUpdate_LendingBorrow(userAddr: Address): UFix64 {
        let lastUpdateTimestamp = self.getUserState_LastUpdateTimestamp(userAddr: userAddr)
        var accuredPoints = 0.0
        if lastUpdateTimestamp == 0.0 { return 0.0 }
        let currUpdateTimestamp = (getCurrentBlock().timestamp<self._pointsMintEndTimestamp)? getCurrentBlock().timestamp:self._pointsMintEndTimestamp
    ...
    access(self) view fun calculateNewPointsSinceLastUpdate_stFlowHolding(userAddr: Address): UFix64 {
        let lastUpdateTimestamp = self.getUserState_LastUpdateTimestamp(userAddr: userAddr)
        var accuredPoints = 0.0
        if lastUpdateTimestamp == 0.0 { return 0.0 }
        let currUpdateTimestamp = (getCurrentBlock().timestamp<self._pointsMintEndTimestamp)? getCurrentBlock().timestamp:self._pointsMintEndTimestamp
    ...
    // Accure Swap LP
    access(self) view fun calculateNewPointsSinceLastUpdate_SwapLP(userAddr: Address): UFix64 {
        let lastUpdateTimestamp = self.getUserState_LastUpdateTimestamp(userAddr: userAddr)
        var accuredPoints = 0.0
        if lastUpdateTimestamp == 0.0 { return 0.0 }
        let currUpdateTimestamp = (getCurrentBlock().timestamp<self._pointsMintEndTimestamp)? getCurrentBlock().timestamp:self._pointsMintEndTimestamp
    ...
    access(all) view fun getTopUsers(): [Address] { return self._topUsers }
    ...
    // Get Points Rate
    access(all) view fun getPointsRate(): {String: AnyStruct} { return self._pointsRate }
    access(all) view fun getPointsRate_LendingSupply(amount: UFix64): UFix64 { return self.calculateTierRateByAmount(amount: amount, tier: self._pointsRate["LendingSupply"]! as! {UFix64: UFix64}) }
    access(all) view fun getPointsRate_LendingBorrow(amount: UFix64): UFix64 { return self.calculateTierRateByAmount(amount: amount, tier: self._pointsRate["LendingBorrow"]! as! {UFix64: UFix64}) }
    access(all) view fun getPointsRate_stFlowHolding(amount: UFix64): UFix64 { return self.calculateTierRateByAmount(amount: amount, tier: self._pointsRate["stFlowHolding"]! as! {UFix64: UFix64}) }
    access(all) view fun getPointsRate_SwapLP(amount: UFix64): UFix64 { return self.calculateTierRateByAmount(amount: amount, tier: self._pointsRate["SwapLP"]! as! {UFix64: UFix64}) }
    access(all) view fun getPointsRate_SpecificSwapLPUsdRatio(poolAddr: Address): UFix64 {
        if IncrementPoints._pointsRate.containsKey("SpecificSwapLpUsdRatio") == true
            &&
           (IncrementPoints._pointsRate["SpecificSwapLpUsdRatio"]! as! {Address: UFix64}).containsKey(poolAddr) == true
        {
            let usdRatio = (IncrementPoints._pointsRate["SpecificSwapLpUsdRatio"]! as! {Address: UFix64})[poolAddr]!
            return usdRatio

        }
        return 1.0
    }
    access(all) view fun getPointsRate_SwapVolume(amount: UFix64): UFix64 { return self.calculateTierRateByAmount(amount: amount, tier: self._pointsRate["SwapVolume"]! as! {UFix64: UFix64}) }
    access(all) view fun getPointsRate_ReferrerUp(): UFix64 { return self._pointsRate["ReferrerUp"]! as! UFix64 }
    access(all) view fun getPointsRate_RefereeUp(): UFix64 { return self._pointsRate["RefereeUp"]! as! UFix64 }
    access(all) view fun getPointsRate_CoreMember(): UFix64 { return self._pointsRate["CoreMember"]! as! UFix64 }

    // Get User State
    access(all) view fun getUserState(userAddr: Address): {String: UFix64} { return self._userStates.containsKey(userAddr)? self._userStates[userAddr]! : {} }
    access(all) view fun getUserState_LastUpdateTimestamp(userAddr: Address): UFix64 { return self._userStates.containsKey(userAddr)? (self._userStates[userAddr]!.containsKey("LastUpdateTimestamp")? self._userStates[userAddr]!["LastUpdateTimestamp"]! : 0.0): 0.0 }
    access(all) view fun getUserState_LendingSupply(userAddr: Address): UFix64 { return self._userStates.containsKey(userAddr)? (self._userStates[userAddr]!.containsKey("LendingTotalSupplyUsd")? self._userStates[userAddr]!["LendingTotalSupplyUsd"]! : 0.0): 0.0 }
    access(all) view fun getUserState_LendingBorrow(userAddr: Address): UFix64 { return self._userStates.containsKey(userAddr)? (self._userStates[userAddr]!.containsKey("LendingTotalBorrowUsd")? self._userStates[userAddr]!["LendingTotalBorrowUsd"]! : 0.0): 0.0 }
    access(all) view fun getUserState_stFlowHolding(userAddr: Address): UFix64 { return self._userStates.containsKey(userAddr)? (self._userStates[userAddr]!.containsKey("stFlowHolding")? self._userStates[userAddr]!["stFlowHolding"]! : 0.0): 0.0 }
    access(all) view fun getUserState_SwapLPUsd(userAddr: Address): UFix64 { return self._userStates.containsKey(userAddr)? (self._userStates[userAddr]!.containsKey("SwapLpUsd")? self._userStates[userAddr]!["SwapLpUsd"]! : 0.0): 0.0 }
    access(all) view fun getUserState_SwapLPAmount(userAddr: Address): UFix64 { return self._userStates.containsKey(userAddr)? (self._userStates[userAddr]!.containsKey("SwapLpAmount")? self._userStates[userAddr]!["SwapLpAmount"]! : 0.0): 0.0 }
    access(all) view fun getUserState_SwapVolume(userAddr: Address): UFix64 { return self._userStates.containsKey(userAddr)? (self._userStates[userAddr]!.containsKey("SwapVolumeUsd")? self._userStates[userAddr]!["SwapVolumeUsd"]! : 0.0): 0.0 }
    
    access(self) fun setUserState_LastUpdateTimestamp(userAddr: Address, timestamp: UFix64) { self._userStates[userAddr]!.insert(key: "LastUpdateTimestamp", timestamp) }
    access(self) fun setUserState_LendingSupply(userAddr: Address, supplyAmount: UFix64) { self._userStates[userAddr]!.insert(key: "LendingTotalSupplyUsd", supplyAmount) }
    access(self) fun setUserState_LendingBorrow(userAddr: Address, borrowAmount: UFix64) { self._userStates[userAddr]!.insert(key: "LendingTotalBorrowUsd", borrowAmount) }
    access(self) fun setUserState_stFlowHolding(userAddr: Address, stFlowBalance: UFix64) { self._userStates[userAddr]!.insert(key: "stFlowHolding", stFlowBalance) }
    access(self) fun setUserState_SwapLPUsd(userAddr: Address, lpUsd: UFix64) { self._userStates[userAddr]!.insert(key: "SwapLpUsd", lpUsd) }
    access(self) fun setUserState_SwapLPAmount(userAddr: Address, lpAmount: UFix64) { self._userStates[userAddr]!.insert(key: "SwapLpAmount", lpAmount) }
    access(self) fun setUserState_SwapVolume(userAddr: Address, volume: UFix64) { self._userStates[userAddr]!.insert(key: "SwapVolumeUsd", volume) }
    ...
    access(all) view fun getClaimEndTimestamp(): UFix64 {
        return self._claimEndTimestamp
    }

    access(all) view fun getPointsMintEndTimestamp(): UFix64 {
        return self._pointsMintEndTimestamp
    }

    access(all) view fun getLastVolumeAndRefereePointsUpdateTimestamp(): UFix64 {
    ...
    access(all) resource Admin {
        // Set Points Rate
        access(all) fun setPointsRate_stFlowHoldingTier(tierRate: {UFix64: UFix64}) {
            emit PointsTierRateChanged(source: "stFlowHolding", ori: IncrementPoints._pointsRate["stFlowHolding"]! as! {UFix64: UFix64}, new: tierRate)
            IncrementPoints._pointsRate["stFlowHolding"] = tierRate
        }
        access(all) fun setPointsRate_LendingSupplyTier(tierRate: {UFix64: UFix64}) {
            emit PointsTierRateChanged(source: "LendingSupply", ori: IncrementPoints._pointsRate["LendingSupply"]! as! {UFix64: UFix64}, new: tierRate)
            IncrementPoints._pointsRate["LendingSupply"] = tierRate
        }
        access(all) fun setPointsRate_LendingBorrowTier(tierRate: {UFix64: UFix64}) {
            emit PointsTierRateChanged(source: "LendingBorrow", ori: IncrementPoints._pointsRate["LendingBorrow"]! as! {UFix64: UFix64}, new: tierRate)
            IncrementPoints._pointsRate["LendingBorrow"] = tierRate
        }
        access(all) fun setPointsRate_SwapLPTier(tierRate: {UFix64: UFix64}) {
            emit PointsTierRateChanged(source: "SwapLP", ori: IncrementPoints._pointsRate["SwapLP"]! as! {UFix64: UFix64}, new: tierRate)
            IncrementPoints._pointsRate["SwapLP"] = tierRate
        }
        access(all) fun setPointsRate_SwapVolumeTier(tierRate: {UFix64: UFix64}) {
            emit PointsTierRateChanged(source: "SwapVolume", ori: IncrementPoints._pointsRate["SwapVolume"]! as! {UFix64: UFix64}, new: tierRate)
            IncrementPoints._pointsRate["SwapVolume"] = tierRate
        }
        //
        access(all) fun setPointsRate_SpecificSwapLPUsdRatio(poolAddr: Address, usdRatio: UFix64) {
            ...
            (IncrementPoints._pointsRate["SpecificSwapLpUsdRatio"]! as! {Address: UFix64}).insert(key: poolAddr, usdRatio)
            emit SpecificSwapLpUsdRatioChange(poolAddr: poolAddr, usdRatio: usdRatio)
        }

        access(all) fun setPointsRate_ReferrerUp(rate: UFix64) {
            emit PointsRateChanged(source: "ReferrerUp", ori: IncrementPoints._pointsRate["ReferrerUp"]! as! UFix64, new: rate)
            IncrementPoints._pointsRate["ReferrerUp"] = rate
        }
        access(all) fun setPointsRate_RefereeUp(rate: UFix64) {
            emit PointsRateChanged(source: "RefereeUp", ori: IncrementPoints._pointsRate["RefereeUp"]! as! UFix64, new: rate)
            IncrementPoints._pointsRate["RefereeUp"] = rate
        }
        access(all) fun setPointsRate_CoreMember(rate: UFix64) {
            emit PointsRateChanged(source: "CoreMember", ori: IncrementPoints._pointsRate["CoreMember"]! as! UFix64, new: rate)
        ...
        access(all) fun setHistorySnapshotPoints(userAddr: Address, newSnapshotBalance: UFix64) {
            ...
            emit SetSnapshotPoints(userAddr: userAddr, pre: preSnapshotBalance, new: newSnapshotBalance)
            ...
            IncrementPoints._totalSupply = IncrementPoints._totalSupply - preSnapshotBalance + newSnapshotBalance
            IncrementPoints._pointsHistorySnapshot[userAddr] = newSnapshotBalance
        }
        access(all) fun setClaimEndTimestamp(endTime: UFix64) {
            IncrementPoints._claimEndTimestamp = endTime
        }

        access(all) fun setPointsMintEndTimestamp(endTime: UFix64) {
        ...
        access(all) fun addVolumePoints(userAddr: Address, volumePoints: UFix64, volumeUsd: UFix64) {
            ...
            IncrementPoints.setUserState_SwapVolume(userAddr: userAddr, volume: IncrementPoints.getUserState_SwapVolume(userAddr: userAddr) + volumeUsd)
            emit PointsMinted(
                userAddr: userAddr,
                amount: volumePoints,
                source: "SwapVolume", 
        ...
        access(all) fun setLastVolumeAndRefereePointsUpdateTimestamp(timestamp: UFix64) {
            IncrementPoints._lastVolumeAndRefereePointsUpdateTimestamp = timestamp
        }

        access(all) fun reconcileBasePoints(userAddr: Address, newBasePoints: UFix64) {
            if IncrementPoints._pointsBase.containsKey(userAddr) == false {
                IncrementPoints._pointsBase[userAddr] = 0.0
            }
            ...
            IncrementPoints._totalSupply = IncrementPoints._totalSupply - preBasePoints + newBasePoints
            IncrementPoints._pointsBase[userAddr] = newBasePoints
        }

        access(all) fun updateTopUsers(addrs: [Address]) {
    ...
    init() {
        ...
        self._pointsRate = {
            ...
            "SpecificSwapLpUsdRatio": {
                "0xc353b9d685ec427d": 2.0
            },
            "SwapVolume": {
                0.0: 0.0,
                1.0: 0.001
