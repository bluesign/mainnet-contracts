...
pub contract TouchstoneHomesFourHoopsFoundation: NonFungibleToken {
			...
			switch view {
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(
						storagePath: TouchstoneHomesFourHoopsFoundation.CollectionStoragePath,
						publicPath: TouchstoneHomesFourHoopsFoundation.CollectionPublicPath,
						providerPath: TouchstoneHomesFourHoopsFoundation.CollectionPrivatePath,
          ...
          return MetadataViews.ExternalURL("https://touchstone.city/discover/".concat(self.owner!.address.toString()).concat("/TouchstoneHomesFourHoopsFoundation"))
				case Type<MetadataViews.NFTCollectionDisplay>():
					let squareMedia = MetadataViews.Media(
						file: TouchstoneHomesFourHoopsFoundation.getCollectionAttribute(key: "image") as! MetadataViews.IPFSFile,
						mediaType: "image"
					)
					...
					return MetadataViews.NFTCollectionDisplay(
						name: TouchstoneHomesFourHoopsFoundation.getCollectionAttribute(key: "name") as! String,
						description: TouchstoneHomesFourHoopsFoundation.getCollectionAttribute(key: "description") as! String,
						externalURL: MetadataViews.ExternalURL("https://touchstone.city/discover/".concat(self.owner!.address.toString()).concat("/TouchstoneHomesFourHoopsFoundation")),
						squareImage: squareMedia,
						bannerImage: bannerMedia ?? squareMedia,
						socials: TouchstoneHomesFourHoopsFoundation.getCollectionAttribute(key: "socials") as! {String: MetadataViews.ExternalURL}
					)
	...
	pub resource Administrator {
		pub fun createNFTMetadata(name: String, description: String, imagePath: String, thumbnailPath: String?, ipfsCID: String, price: UFix64?, extra: {String: AnyStruct}, supply: UInt64) {
			TouchstoneHomesFourHoopsFoundation.metadatas[TouchstoneHomesFourHoopsFoundation.nextMetadataId] = NFTMetadata(
				_name: name,
		...
		pub fun mintNFT(metadataId: UInt64, serial: UInt64, recipient: Address) {
			pre {
				EmeraldPass.isActive(user: TouchstoneHomesFourHoopsFoundation.account.address): "You must have an active Emerald Pass subscription to airdrop NFTs. You can purchase Emerald Pass at https://pass.ecdao.org/"
			}
			...
			if let recipientCollection = getAccount(recipient).getCapability(TouchstoneHomesFourHoopsFoundation.CollectionPublicPath).borrow<&TouchstoneHomesFourHoopsFoundation.Collection{NonFungibleToken.CollectionPublic}>() {
				...
				if let storage = &TouchstoneHomesFourHoopsFoundation.nftStorage[recipient] as &{UInt64: NFT}? {
					storage[nft.id] <-! nft
				} else {
