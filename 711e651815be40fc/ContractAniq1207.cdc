...
pub contract ContractAniq1207: NonFungibleToken, Anique {
    // -----------------------------------------------------------------------
    // ContractAniq1207 contract Events
    // -----------------------------------------------------------------------

    // Events for Contract-Related actions
    //
    // Emitted when the ContractAniq1207 contract is created
    ...
    // -----------------------------------------------------------------------
    // ContractAniq1207 contract-level fields.
    ...
    // -----------------------------------------------------------------------
    // ContractAniq1207 contract-level Composite Type definitions
    // -----------------------------------------------------------------------

    // The structure that represents Item
    // each digital content which ContractAniq1207 deal with on Flow
    //
    pub struct ItemData {
        ...
        init(itemID: UInt32) {
            let item = (&ContractAniq1207.items[itemID] as &Item?)!
    ...
    pub resource Item {
        ...
        init(metadata: {String: String}) {
            ...
            self.itemID = ContractAniq1207.nextItemID
            self.metadata = metadata
            self.numberMintedPerItem = 0

            // increment the nextItemID so that it isn't used again
            ContractAniq1207.nextItemID = ContractAniq1207.nextItemID + 1
    ...
    pub resource NFT: NonFungibleToken.INFT, Anique.INFT {
        ...
        init(serialNumber: UInt32, itemID: UInt32) {
            ...
            ContractAniq1207.totalSupply = ContractAniq1207.totalSupply + 1

            // set id
            self.id = ContractAniq1207.totalSupply
    ...
    // interface that represents ContractAniq1207 collections to public
    ...
    pub resource interface CollectionPublic {
        ...
        // borrow NFT as ContractAniq1207 token
        pub fun borrowContractAniq1207Collectible(id: UInt64): auth &NFT
    }
    ...
    pub resource Collection: CollectionPublic, NonFungibleToken.Receiver, NonFungibleToken.Provider, NonFungibleToken.CollectionPublic {
        ...
        pub fun deposit(token: @NonFungibleToken.NFT) {
            ...
            // Cast the deposited token as an ContractAniq1207 NFT to make sure
            // it is the correct type
            let token <- token as! @ContractAniq1207.NFT
        ...
        // not any ContractAniq1207 specific data. Please use borrowCollectible to
        ...
        // borrowContractAniq1207Collectible returns a borrowed reference
        // to an ContractAniq1207 Collectible
        pub fun borrowContractAniq1207Collectible(id: UInt64): auth &NFT {
            pre {
    ...
    pub resource Admin {
        ...
        // and stores it in the Items dictionary field in the ContractAniq1207 smart contract
        //
        ...
        pub fun createItem(metadata: {String: String}): UInt32 {
            ...
            ContractAniq1207.items[newItem.itemID] <-! newItem

            return itemId
        }

        // borrowItem returns a reference to a Item in the ContractAniq1207
        ...
        pub fun borrowItem(itemID: UInt32): &Item {
            pre {
                ContractAniq1207.items[itemID] != nil: "Cannot borrow Item: The Item doesn't exist"
            }

            return (&ContractAniq1207.items[itemID] as &Item?)!
        }
    ...
    // -----------------------------------------------------------------------
    // ContractAniq1207 contract-level function definitions
    // -----------------------------------------------------------------------
    ...
    pub fun createEmptyCollection(): @NonFungibleToken.Collection {
        return <-create ContractAniq1207.Collection()
    }
    ...
    // -----------------------------------------------------------------------
    // ContractAniq1207 initialization function
    // -----------------------------------------------------------------------
    //
    init() {
        ...
        self.collectionStoragePath     = /storage/ContractAniq1207Collection
        self.collectionPublicPath      =  /public/ContractAniq1207Collection
        self.collectionPrivatePath     = /private/ContractAniq1207Collection
        self.adminStoragePath          = /storage/ContractAniq1207Admin
        self.saleCollectionStoragePath = /storage/ContractAniq1207SaleCollection
        self.saleCollectionPublicPath  =  /public/ContractAniq1207SaleCollection
