...
pub contract SGKCLDR_sjmook_bp_tn: NonFungibleToken, Anique {
    // -----------------------------------------------------------------------
    // SGKCLDR_sjmook_bp_tn contract Events
    // -----------------------------------------------------------------------

    // Events for Contract-Related actions
    //
    // Emitted when the SGKCLDR_sjmook_bp_tn contract is created
    ...
    // -----------------------------------------------------------------------
    // SGKCLDR_sjmook_bp_tn contract-level fields.
    ...
    // -----------------------------------------------------------------------
    // SGKCLDR_sjmook_bp_tn contract-level Composite Type definitions
    // -----------------------------------------------------------------------

    // The structure that represents Item
    // each digital content which SGKCLDR_sjmook_bp_tn deal with on Flow
    //
    pub struct ItemData {
        ...
        init(itemID: UInt32) {
            let item = (&SGKCLDR_sjmook_bp_tn.items[itemID] as &Item?)!
    ...
    pub resource Item {
        ...
        init(metadata: {String: String}) {
            ...
            self.itemID = SGKCLDR_sjmook_bp_tn.nextItemID
            self.metadata = metadata
            self.numberMintedPerItem = 0

            // increment the nextItemID so that it isn't used again
            SGKCLDR_sjmook_bp_tn.nextItemID = SGKCLDR_sjmook_bp_tn.nextItemID + 1
    ...
    pub resource NFT: NonFungibleToken.INFT, Anique.INFT {
        ...
        init(serialNumber: UInt32, itemID: UInt32) {
            ...
            SGKCLDR_sjmook_bp_tn.totalSupply = SGKCLDR_sjmook_bp_tn.totalSupply + 1

            // set id
            self.id = SGKCLDR_sjmook_bp_tn.totalSupply
    ...
    // interface that represents SGKCLDR_sjmook_bp_tn collections to public
    ...
    pub resource interface CollectionPublic {
        ...
        // borrow NFT as SGKCLDR_sjmook_bp_tn token
        pub fun borrowSGKCLDR_sjmook_bp_tnCollectible(id: UInt64): auth &NFT
    }
    ...
    pub resource Collection: CollectionPublic, NonFungibleToken.Receiver, NonFungibleToken.Provider, NonFungibleToken.CollectionPublic {
        ...
        pub fun deposit(token: @NonFungibleToken.NFT) {
            ...
            // Cast the deposited token as an SGKCLDR_sjmook_bp_tn NFT to make sure
            // it is the correct type
            let token <- token as! @SGKCLDR_sjmook_bp_tn.NFT
        ...
        // not any SGKCLDR_sjmook_bp_tn specific data. Please use borrowCollectible to
        ...
        // borrowSGKCLDR_sjmook_bp_tnCollectible returns a borrowed reference
        // to an SGKCLDR_sjmook_bp_tn Collectible
        pub fun borrowSGKCLDR_sjmook_bp_tnCollectible(id: UInt64): auth &NFT {
            pre {
    ...
    pub resource Admin {
        ...
        // and stores it in the Items dictionary field in the SGKCLDR_sjmook_bp_tn smart contract
        //
        ...
        pub fun createItem(metadata: {String: String}): UInt32 {
            ...
            SGKCLDR_sjmook_bp_tn.items[newItem.itemID] <-! newItem

            return itemId
        }

        // borrowItem returns a reference to a Item in the SGKCLDR_sjmook_bp_tn
        ...
        pub fun borrowItem(itemID: UInt32): &Item {
            pre {
                SGKCLDR_sjmook_bp_tn.items[itemID] != nil: "Cannot borrow Item: The Item doesn't exist"
            }

            return (&SGKCLDR_sjmook_bp_tn.items[itemID] as &Item?)!
        }
    ...
    // -----------------------------------------------------------------------
    // SGKCLDR_sjmook_bp_tn contract-level function definitions
    // -----------------------------------------------------------------------
    ...
    pub fun createEmptyCollection(): @NonFungibleToken.Collection {
        return <-create SGKCLDR_sjmook_bp_tn.Collection()
    }
    ...
    // -----------------------------------------------------------------------
    // SGKCLDR_sjmook_bp_tn initialization function
    // -----------------------------------------------------------------------
    //
    init() {
        ...
        self.collectionStoragePath     = /storage/SGKCLDR_sjmook_bp_tnCollection
        self.collectionPublicPath      =  /public/SGKCLDR_sjmook_bp_tnCollection
        self.collectionPrivatePath     = /private/SGKCLDR_sjmook_bp_tnCollection
        self.adminStoragePath          = /storage/SGKCLDR_sjmook_bp_tnAdmin
        self.saleCollectionStoragePath = /storage/SGKCLDR_sjmook_bp_tnSaleCollection
        self.saleCollectionPublicPath  =  /public/SGKCLDR_sjmook_bp_tnSaleCollection
