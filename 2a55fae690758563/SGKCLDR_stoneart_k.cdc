...
pub contract SGKCLDR_stoneart_k: NonFungibleToken, Anique {
    // -----------------------------------------------------------------------
    // SGKCLDR_stoneart_k contract Events
    // -----------------------------------------------------------------------

    // Events for Contract-Related actions
    //
    // Emitted when the SGKCLDR_stoneart_k contract is created
    ...
    // -----------------------------------------------------------------------
    // SGKCLDR_stoneart_k contract-level fields.
    ...
    // -----------------------------------------------------------------------
    // SGKCLDR_stoneart_k contract-level Composite Type definitions
    // -----------------------------------------------------------------------

    // The structure that represents Item
    // each digital content which SGKCLDR_stoneart_k deal with on Flow
    //
    pub struct ItemData {
        ...
        init(itemID: UInt32) {
            let item = (&SGKCLDR_stoneart_k.items[itemID] as &Item?)!
    ...
    pub resource Item {
        ...
        init(metadata: {String: String}) {
            ...
            self.itemID = SGKCLDR_stoneart_k.nextItemID
            self.metadata = metadata
            self.numberMintedPerItem = 0

            // increment the nextItemID so that it isn't used again
            SGKCLDR_stoneart_k.nextItemID = SGKCLDR_stoneart_k.nextItemID + 1
    ...
    pub resource NFT: NonFungibleToken.INFT, Anique.INFT {
        ...
        init(serialNumber: UInt32, itemID: UInt32) {
            ...
            SGKCLDR_stoneart_k.totalSupply = SGKCLDR_stoneart_k.totalSupply + 1

            // set id
            self.id = SGKCLDR_stoneart_k.totalSupply
    ...
    // interface that represents SGKCLDR_stoneart_k collections to public
    ...
    pub resource interface CollectionPublic {
        ...
        // borrow NFT as SGKCLDR_stoneart_k token
        pub fun borrowSGKCLDR_stoneart_kCollectible(id: UInt64): auth &NFT
    }
    ...
    pub resource Collection: CollectionPublic, NonFungibleToken.Receiver, NonFungibleToken.Provider, NonFungibleToken.CollectionPublic {
        ...
        pub fun deposit(token: @NonFungibleToken.NFT) {
            ...
            // Cast the deposited token as an SGKCLDR_stoneart_k NFT to make sure
            // it is the correct type
            let token <- token as! @SGKCLDR_stoneart_k.NFT
        ...
        // not any SGKCLDR_stoneart_k specific data. Please use borrowCollectible to
        ...
        // borrowSGKCLDR_stoneart_kCollectible returns a borrowed reference
        // to an SGKCLDR_stoneart_k Collectible
        pub fun borrowSGKCLDR_stoneart_kCollectible(id: UInt64): auth &NFT {
            pre {
    ...
    pub resource Admin {
        ...
        // and stores it in the Items dictionary field in the SGKCLDR_stoneart_k smart contract
        //
        ...
        pub fun createItem(metadata: {String: String}): UInt32 {
            ...
            SGKCLDR_stoneart_k.items[newItem.itemID] <-! newItem

            return itemId
        }

        // borrowItem returns a reference to a Item in the SGKCLDR_stoneart_k
        ...
        pub fun borrowItem(itemID: UInt32): &Item {
            pre {
                SGKCLDR_stoneart_k.items[itemID] != nil: "Cannot borrow Item: The Item doesn't exist"
            }

            return (&SGKCLDR_stoneart_k.items[itemID] as &Item?)!
        }
    ...
    // -----------------------------------------------------------------------
    // SGKCLDR_stoneart_k contract-level function definitions
    // -----------------------------------------------------------------------
    ...
    pub fun createEmptyCollection(): @NonFungibleToken.Collection {
        return <-create SGKCLDR_stoneart_k.Collection()
    }
    ...
    // -----------------------------------------------------------------------
    // SGKCLDR_stoneart_k initialization function
    // -----------------------------------------------------------------------
    //
    init() {
        ...
        self.collectionStoragePath     = /storage/SGKCLDR_stoneart_kCollection
        self.collectionPublicPath      =  /public/SGKCLDR_stoneart_kCollection
        self.collectionPrivatePath     = /private/SGKCLDR_stoneart_kCollection
        self.adminStoragePath          = /storage/SGKCLDR_stoneart_kAdmin
        self.saleCollectionStoragePath = /storage/SGKCLDR_stoneart_kSaleCollection
        self.saleCollectionPublicPath  =  /public/SGKCLDR_stoneart_kSaleCollection
