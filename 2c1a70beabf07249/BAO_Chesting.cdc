...
//
pub contract BAO_Chesting: NonFungibleToken {
	...
	pub resource NFTMinter {
		...
		pub fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, mugenNFTID: UInt64) {
            ...
            if BAO_Chesting.totalSupply == BAO_Chesting.maxSupply {
			...
			recipient.deposit(token: <-create BAO_Chesting.NFT(initID: mugenNFTID))

            BAO_Chesting.mintedIds[mugenNFTID] = true

            BAO_Chesting.totalSupply = BAO_Chesting.totalSupply + (1 as UInt64)
		}
	}
	...
	init() {
        // Mugen contract name
        // Set our named paths
        self.CollectionStoragePath = /storage/BAO_ChestingCollection
        self.CollectionPublicPath = /public/BAO_ChestingCollection
        self.MinterStoragePath = /storage/BAO_Chesting
