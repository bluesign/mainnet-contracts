...
//
pub contract MugenProductCNFT: NonFungibleToken {
    ...
    pub resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic {
        ...
        pub fun deposit(token: @NonFungibleToken.NFT) {
            let token <- token as! @MugenProductCNFT.NFT
	...
	pub resource NFTMinter {
		...
		pub fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, mugenNFTID: UInt64) {
            ...
            if MugenProductCNFT.mintedIds[mugenNFTID] == true {
                emit MintFailDuplicateId(id: mugenNFTID)
                panic("Duplicate token id")
            }

            if MugenProductCNFT.totalSupply == MugenProductCNFT.maxSupply {
			...
			recipient.deposit(token: <-create MugenProductCNFT.NFT(initID: mugenNFTID))

            MugenProductCNFT.mintedIds[mugenNFTID] = true

            MugenProductCNFT.totalSupply = MugenProductCNFT.totalSupply + (1 as UInt64)
		}
	}
	...
	init() {
        // Mugen contract name
        // Set our named paths
        self.CollectionStoragePath = /storage/MugenProductCNFTCollection
        self.CollectionPublicPath = /public/MugenProductCNFTCollection
        self.MinterStoragePath = /storage/MugenProductCNFT
