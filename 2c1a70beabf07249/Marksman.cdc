...
//
pub contract Marksman: NonFungibleToken {
    ...
    pub resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic {
        ...
        pub fun deposit(token: @NonFungibleToken.NFT) {
            let token <- token as! @Marksman.NFT
	...
	pub resource NFTMinter {
		...
		pub fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, mugenNFTID: UInt64) {
            ...
            if Marksman.mintedIds[mugenNFTID] == true {
                emit MintFailDuplicateId(id: mugenNFTID)
                panic("Duplicate token id")
            }

            if Marksman.totalSupply == Marksman.maxSupply {
			...
			recipient.deposit(token: <-create Marksman.NFT(initID: mugenNFTID))

            Marksman.mintedIds[mugenNFTID] = true

            Marksman.totalSupply = Marksman.totalSupply + (1 as UInt64)
		}
	}
	...
	init() {
        // Mugen contract name
        // Set our named paths
        self.CollectionStoragePath = /storage/MarksmanCollection
        self.CollectionPublicPath = /public/MarksmanCollection
        self.MinterStoragePath = /storage/Marksman
