...
access(all)
contract NuclearDAO: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				NuclearDAO.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return NuclearDAO.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				NuclearDAO.maxSupply == nil || NuclearDAO.totalSupply <= NuclearDAO.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			NuclearDAO.totalSupply = NuclearDAO.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreiac4eyy6squsq7er2qib24pwnrpefe37is5vu75g7in6a5kmhmnpq"), mediaType: "image")
				let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreihbulqvjxp3s5c5v5nqwj2awbpoqumtmuwf73jdhmjkdfwwbr5p4y"), mediaType: "image")
				let medias = MetadataViews.Medias([media, bannerMedia])
				return FungibleTokenMetadataViews.FTDisplay(name: "Nuclear DAO", symbol: "NUKE", description: "Welcome to the Wasteland.", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("https://twitter.com/NuclearDA0"), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: NuclearDAO.VaultStoragePath, receiverPath: NuclearDAO.ReceiverPublicPath, metadataPath: NuclearDAO.VaultPublicPath, receiverLinkedType: /private/NuclearDAOVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-NuclearDAO.createEmptyVault(vaultType: Type<@NuclearDAO.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/NuclearDAOVault
		self.ReceiverPublicPath = /public/NuclearDAOReceiver
		self.VaultPublicPath = /public/NuclearDAOMetadata
		self.MinterStoragePath = /storage/NuclearDAOMinter
		self.AdministratorStoragePath = /storage/NuclearDAOAdmin
