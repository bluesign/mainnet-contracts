...
access(all)
contract SwaychainNFT: NonFungibleToken{ 
	access(all)
	...
	access(all)
	resource interface SwaychainNFTCollectionPublic{ 
		access(all)
		...
		fun borrowSwaychainNFT(id: UInt64): &SwaychainNFT.NFT?{ 
			post{ 
	...
	access(all)
	resource Collection: SwaychainNFTCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		...
		view fun borrowViewResolver(id: UInt64): &{ViewResolver.Resolver}?{ 
			...
			return swaychainNFT as &{ViewResolver.Resolver}
		}
	...
	resource NFTMinter{ 
		...
		fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, name: String, description: String, thumbnail: String){ 
			...
			// create a new NFT
			var newNFT <- create NFT(id: SwaychainNFT.totalSupply, name: name, description: description, thumbnail: thumbnail)
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&SwaychainNFT.Collection>(self.CollectionStoragePath)
