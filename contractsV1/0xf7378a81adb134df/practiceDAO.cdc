...
access(all)
contract practiceDAO: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				practiceDAO.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return practiceDAO.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				practiceDAO.maxSupply == nil || practiceDAO.totalSupply <= practiceDAO.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			practiceDAO.totalSupply = practiceDAO.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreieiwk4kdznclh5o25d3dkay2regf6fdqhxwsscssv6jmfdc2ijvuq"), mediaType: "image")
				let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreieiwk4kdznclh5o25d3dkay2regf6fdqhxwsscssv6jmfdc2ijvuq"), mediaType: "image")
				let medias = MetadataViews.Medias([media, bannerMedia])
				return FungibleTokenMetadataViews.FTDisplay(name: "practiceDAO", symbol: "PDAO", description: "This is just a test DAO", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("inspiredbymamba"), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: practiceDAO.VaultStoragePath, receiverPath: practiceDAO.ReceiverPublicPath, metadataPath: practiceDAO.VaultPublicPath, receiverLinkedType: /private/practiceDAOVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-practiceDAO.createEmptyVault(vaultType: Type<@practiceDAO.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/practiceDAOVault
		self.ReceiverPublicPath = /public/practiceDAOReceiver
		self.VaultPublicPath = /public/practiceDAOMetadata
		self.MinterStoragePath = /storage/practiceDAOMinter
		self.AdministratorStoragePath = /storage/practiceDAOAdmin
