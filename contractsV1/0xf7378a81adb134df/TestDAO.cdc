...
access(all)
contract TestDAO: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				TestDAO.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return TestDAO.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				TestDAO.maxSupply == nil || TestDAO.totalSupply <= TestDAO.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			TestDAO.totalSupply = TestDAO.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "TestDAO", symbol: "TSDO", description: "This is a test DAO, feel free to disregard this.", externalURL: MetadataViews.ExternalURL("justatest"), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: TestDAO.VaultStoragePath, receiverPath: TestDAO.ReceiverPublicPath, metadataPath: TestDAO.VaultPublicPath, receiverLinkedType: /private/TestDAOVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-TestDAO.createEmptyVault(vaultType: Type<@TestDAO.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/TestDAOVault
		self.ReceiverPublicPath = /public/TestDAOReceiver
		self.VaultPublicPath = /public/TestDAOMetadata
		self.MinterStoragePath = /storage/TestDAOMinter
		self.AdministratorStoragePath = /storage/TestDAOAdmin
