...
contract PopsyclAuction{ 
	...
	resource AuctionItem{ 
		...
		access(self)
		var recipientCollectionCap: Capability<&{Popsycl.PopsyclCollectionPublic}>
		...
		access(self)
		let ownerCollectionCap: Capability<&{Popsycl.PopsyclCollectionPublic}>
		...
		access(contract)
		fun sendNFT(_ capability: Capability<&{Popsycl.PopsyclCollectionPublic}>){ 
	...
	resource AuctionCollection: AuctionPublic{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun addTokenToAuctionItems(token: @{NonFungibleToken.NFT}, minimumBidIncrement: UFix64, startPrice: UFix64, bidVault: @{FungibleToken.Vault}, collectionCap: Capability<&{Popsycl.PopsyclCollectionPublic}>, vaultCap: Capability<&{FungibleToken.Receiver}>, endTime: Fix64){ 
			pre{ 
		...
		fun getAuctionStatuses():{ UInt64: AuctionStatus}{ 
			...
			for id in self.auctionItems.keys{ 
				let itemRef = &self.auctionItems[id] as &PopsyclAuction.AuctionItem?
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun placeBid(id: UInt64, bidTokens: @{FungibleToken.Vault}, vaultCap: Capability<&{FungibleToken.Receiver}>, collectionCap: Capability<&{Popsycl.PopsyclCollectionPublic}>){ 
			pre{ 
				self.auctionItems[id] != nil:
					"Auction doesn't exist"
			}
