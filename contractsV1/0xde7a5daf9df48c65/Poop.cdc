...
access(all)
contract Poop: FungibleToken{ 
	
	// -----------------------------------------------------------------------
	// Poop contract Events
	// -----------------------------------------------------------------------
	...
	// -----------------------------------------------------------------------
	// Poop contract Named Paths
	// -----------------------------------------------------------------------
	// Defines poop vault storage path
	access(all)
	let VaultStoragePath: StoragePath
	
	// Defines poop vault public balance path
	access(all)
	let BalancePublicPath: PublicPath
	
	// Defines poop vault public receiver path
	access(all)
	let ReceiverPublicPath: PublicPath
	
	// Defines poop admin storage path
	access(all)
	let AdminStoragePath: StoragePath
	
	// Defines poop minter storage path
	access(all)
	let MinterStoragePath: StoragePath
	
	// Defines poop minters' MinterProxy storage path
	access(all)
	let MinterProxyStoragePath: StoragePath
	
	// Defines poop minters' MinterProxy capability public path
	access(all)
	let MinterProxyPublicPath: PublicPath
	
	// -----------------------------------------------------------------------
	// Poop contract fields
	// These contain actual values that are stored in the smart contract
	// -----------------------------------------------------------------------
	// Total supply of Poop in existence
	access(all)
	...
	resource Minter{ 
		...
		//
		access(TMP_ENTITLEMENT_OWNER)
		fun mintTokens(amount: UFix64): @Poop.Vault{ 
			pre{ 
				amount > 0.0:
					"Amount minted must be greater than zero"
			}
			Poop.totalSupply = Poop.totalSupply + amount
	...
	resource interface MinterProxyPublic{ 
		access(TMP_ENTITLEMENT_OWNER)
		fun setMinterCapability(cap: Capability<&Poop.Minter>): Void
	}
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&Poop.Vault>(self.VaultStoragePath)
		self.account.capabilities.publish(capability_1, at: self.ReceiverPublicPath)
		
		// Create a public capability to the stored Vault that only exposes
		// the `balance` field through the `Balance` interface
		var capability_2 = self.account.capabilities.storage.issue<&Poop.Vault>(self.VaultStoragePath)
