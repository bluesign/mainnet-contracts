...
access(all)
contract Gweela: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				Gweela.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return Gweela.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				Gweela.maxSupply == nil || Gweela.totalSupply <= Gweela.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			Gweela.totalSupply = Gweela.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "Gweela", symbol: "GWE", description: "A decentralized community of kids telling their own story, in their own words.", externalURL: MetadataViews.ExternalURL("gweela.com"), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("gweelakids"), "discord": MetadataViews.ExternalURL("gweela")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: Gweela.VaultStoragePath, receiverPath: Gweela.ReceiverPublicPath, metadataPath: Gweela.VaultPublicPath, receiverLinkedType: /private/GweelaVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-Gweela.createEmptyVault(vaultType: Type<@Gweela.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/GweelaVault
		self.ReceiverPublicPath = /public/GweelaReceiver
		self.VaultPublicPath = /public/GweelaMetadata
		self.MinterStoragePath = /storage/GweelaMinter
		self.AdministratorStoragePath = /storage/GweelaAdmin
