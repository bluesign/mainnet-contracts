...
access(all)
contract GarmentNFT: NonFungibleToken{ 
	...
	access(all)
	event GarmentDataCreated(garmentDataID: UInt32, mainImage: String, images: [String], name: String, artist: String, description: String)
	...
	struct GarmentData{ 
		...
		init(mainImage: String, images: [String], name: String, artist: String, description: String){ 
	...
	resource Admin{ 
		...
		fun createGarmentData(mainImage: String, images: [String], name: String, artist: String, description: String): UInt32{ 
			// Create the new GarmentData
			var newGarment = GarmentData(mainImage: mainImage, images: images, name: name, artist: artist, description: description)
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun mintNFT(garmentDataID: UInt32, royaltyVault: Capability<&FBRC.Vault>): @NFT{ 
			pre{ 
				royaltyVault.check():
			...
			let numInGarment = GarmentNFT.numberMintedPerGarment[garmentDataID] ?? panic("Cannot mint Garment. garmentData not found")
			let newGarment: @NFT <- create NFT(serialNumber: numInGarment + 1, garmentDataID: garmentDataID, royaltyVault: royaltyVault)
			return <-newGarment
		}
		
		access(TMP_ENTITLEMENT_OWNER)
		fun batchMintNFT(garmentDataID: UInt32, royaltyVault: Capability<&FBRC.Vault>, quantity: UInt64): @Collection{ 
		...
		fun retireGarmentData(garmentDataID: UInt32){ 
			pre{ 
				GarmentNFT.isGarmentDataRetired[garmentDataID] != nil:
	...
	resource interface GarmentCollectionPublic{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowGarment(id: UInt64): &GarmentNFT.NFT?{ 
	...
	fun getGarmentNumberMinted(id: UInt32): UInt32{ 
		let numberMinted = GarmentNFT.numberMintedPerGarment[id] ?? panic("garmentDataID not found")
		return numberMinted
	}
	...
	fun getGarmentData(id: UInt32): GarmentData{ 
		let garmentData = GarmentNFT.garmentDatas[id] ?? panic("garmentDataID not found")
		return garmentData
	}
	...
	fun getGarmentDataRetired(garmentDataID: UInt32): Bool{ 
		let isGarmentDataRetired = GarmentNFT.isGarmentDataRetired[garmentDataID] ?? panic("garmentDataID not found")
