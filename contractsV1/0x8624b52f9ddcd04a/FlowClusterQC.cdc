...
contract FlowClusterQC{ 
	...
	struct Cluster{ 
		...
		view fun voteThreshold(): UInt64{ 
			...
			let floorOneThird = self.totalWeight / UInt64(3)
	...
	struct Vote{ 
		...
		init(nodeID: String, clusterIndex: UInt16, voteWeight: UInt64){ 
			pre{ 
				nodeID.length == 64:
					"Voter ID must be a valid length node ID"
			}
			self.signature = nil
			self.message = nil
			self.nodeID = nodeID
	...
	resource Voter{ 
		...
		init(nodeID: String, stakingKey: String){ 
			pre{ 
				!FlowClusterQC.voterIsClaimed(nodeID):
					"Cannot create a Voter resource for a node ID that has already been claimed"
			}
			self.nodeID = nodeID
		...
		fun vote(voteSignature: String, voteMessage: String){ 
			pre{ 
				FlowClusterQC.inProgress:
					"Voting phase is not in progress"
				voteSignature.length > 0:
					"Vote signature must not be empty"
				voteMessage.length > 0:
					"Vote message must not be empty"
				!FlowClusterQC.nodeHasVoted(self.nodeID):
			...
			// Assert the validity
			assert(isValid, message: "Vote Signature cannot be verified")
			
			// Get the cluster that this node belongs to
			let clusterIndex =
				FlowClusterQC.nodeCluster[self.nodeID]
		...
		fun stopVoting(){ 
			pre{ 
				FlowClusterQC.votingCompleted():
					"Voting must be complete before it can be stopped"
			}
