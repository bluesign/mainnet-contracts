...
contract FlowEpoch{ 
	...
	struct EpochMetadata{ 
		...
		access(account)
		fun setTotalRewards(_ newRewards: UFix64){ 
			self.totalRewards = newRewards
		}
		
		access(account)
		fun setRewardAmounts(_ rewardBreakdown: [FlowIDTableStaking.RewardsBreakdown]){ 
			self.rewardAmounts = rewardBreakdown
		}
		
		access(account)
		fun setRewardsPaid(_ rewardsPaid: Bool){ 
			self.rewardsPaid = rewardsPaid
		}
		
		access(account)
		fun setClusterQCs(qcs: [FlowClusterQC.ClusterQC]){ 
			self.clusterQCs = qcs
		}
		
		access(account)
		fun setDKGGroupKey(keys: [String]){ 
			self.dkgKeys = keys
		}
	}
	...
	struct Config{ 
		...
		access(all)
		var FLOWsupplyIncreasePercentage: UFix64
	...
	access(contract)
	let configurableMetadata: Config
	...
	/// 
	access(contract)
	fun saveEpochMetadata(_ newMetadata: EpochMetadata){ 
		pre{ 
			self.currentEpochCounter == 0 as UInt64 || newMetadata.counter >= self.currentEpochCounter - 1 as UInt64 && newMetadata.counter <= self.proposedEpochCounter():
		...
		if let metadataDictionary =
			...
			if let metadata = metadataDictionary[newMetadata.counter]{ 
				assert(metadata.counter == newMetadata.counter, message: "Cannot save metadata with mismatching epoch counters")
			}
	...
	access(all)
	var currentEpochCounter: UInt64
	
	/// The current phase that the epoch is in
	access(all)
	var currentEpochPhase: EpochPhase
	...
	access(all)
	let heartbeatStoragePath: StoragePath
	...
	resource Admin{ 
		...
		fun updateEpochViews(_ newEpochViews: UInt64){ 
			pre{ 
				FlowEpoch.currentEpochPhase == EpochPhase.STAKINGAUCTION:
					"Can only update fields during the staking auction"
				FlowEpoch.isValidPhaseConfiguration(FlowEpoch.configurableMetadata.numViewsInStakingAuction, FlowEpoch.configurableMetadata.numViewsInDKGPhase, newEpochViews):
					"New Epoch Views must be greater than the sum of staking and DKG Phase views"
			}
		...
		fun updateAuctionViews(_ newAuctionViews: UInt64){ 
			pre{ 
				FlowEpoch.currentEpochPhase == EpochPhase.STAKINGAUCTION:
					"Can only update fields during the staking auction"
				FlowEpoch.isValidPhaseConfiguration(newAuctionViews, FlowEpoch.configurableMetadata.numViewsInDKGPhase, FlowEpoch.configurableMetadata.numViewsInEpoch):
					"Epoch Views must be greater than the sum of new staking and DKG Phase views"
			}
		...
		fun updateDKGPhaseViews(_ newPhaseViews: UInt64){ 
			pre{ 
				FlowEpoch.currentEpochPhase == EpochPhase.STAKINGAUCTION:
					"Can only update fields during the staking auction"
				FlowEpoch.isValidPhaseConfiguration(FlowEpoch.configurableMetadata.numViewsInStakingAuction, newPhaseViews, FlowEpoch.configurableMetadata.numViewsInEpoch):
					"Epoch Views must be greater than the sum of staking and new DKG Phase views"
			}
		...
		fun updateEpochTimingConfig(_ newConfig: EpochTimingConfig){ 
			pre{ 
				FlowEpoch.currentEpochCounter >= newConfig.refCounter:
					"Reference epoch must be before next epoch"
			}
		...
		fun updateFLOWSupplyIncreasePercentage(_ newPercentage: UFix64){ 
			pre{ 
				FlowEpoch.currentEpochPhase == EpochPhase.STAKINGAUCTION:
					"Can only update fields during the staking auction"
				newPercentage <= 1.0 as UFix64:
					"New value must be between zero and one"
			}
	...
	resource Heartbeat{ 
		...
		fun endStakingAuction(){ 
			pre{ 
				FlowEpoch.currentEpochPhase == EpochPhase.STAKINGAUCTION:
					"Can only end staking auction during the staking auction"
			}
		...
		fun startEpochCommit(){ 
			pre{ 
				FlowEpoch.currentEpochPhase == EpochPhase.EPOCHSETUP:
					"Can only end Epoch Setup during Epoch Setup"
			}
		...
		fun endEpoch(){ 
			pre{ 
				FlowEpoch.currentEpochPhase == EpochPhase.EPOCHCOMMIT:
					"Can only end epoch during Epoch Commit"
			}
			FlowEpoch.startNewEpoch()
		}
	...
	fun calculateAndSetRewards(){ 
		...
		if FlowEpoch.automaticRewardsEnabled(){ 
			...
			let proposedMetadata = self.getEpochMetadata(self.proposedEpochCounter()) ?? panic("Cannot set rewards for the next epoch becuase it hasn't been proposed yet")
	...
	access(account)
	fun payRewardsForPreviousEpoch(){ 
		if let previousEpochMetadata =
			self.getEpochMetadata(self.currentEpochCounter - 1 as UInt64){ 
	...
	access(account)
	fun startEpochSetup(proposedNodeIDs: [String], randomSource: String){ 
		...
		let proposedEpochMetadata =
			EpochMetadata(
				...
				stakingEndView: currentEpochMetadata.endView
				+ self.configurableMetadata.numViewsInStakingAuction,
		...
		let proposedTargetEndTime =
			timingConfig.getTargetEndTimeForEpoch(self.proposedEpochCounter())
		self.saveEpochMetadata(proposedEpochMetadata)
		self.currentEpochPhase = EpochPhase.EPOCHSETUP
		emit EpochSetup(
			counter: proposedEpochMetadata.counter,
			nodeInfo: nodeInfoArray,
			firstView: proposedEpochMetadata.startView,
			finalView: proposedEpochMetadata.endView,
			collectorClusters: collectorClusters,
			randomSource: randomSource,
			DKGPhase1FinalView: proposedEpochMetadata.startView
			+ self.configurableMetadata.numViewsInStakingAuction
			+ self.configurableMetadata.numViewsInDKGPhase
			- 1 as UInt64,
			DKGPhase2FinalView: proposedEpochMetadata.startView
			+ self.configurableMetadata.numViewsInStakingAuction
			+ 2 as UInt64 * self.configurableMetadata.numViewsInDKGPhase
			- 1 as UInt64,
			DKGPhase3FinalView: proposedEpochMetadata.startView
			+ self.configurableMetadata.numViewsInStakingAuction
			+ 3 as UInt64 * self.configurableMetadata.numViewsInDKGPhase
			- 1 as UInt64,
	...
	fun startEpochCommit(){ 
		...
		for cluster in clusters{ 
			var certificate = cluster.generateQuorumCertificate() ?? panic("Could not generate the quorum certificate for this cluster")
	...
	access(contract)
	fun borrowStakingAdmin(): &FlowIDTableStaking.Admin{ 
		let adminCapability =
			self.account.storage.copy<Capability>(from: /storage/flowStakingAdminEpochOperations)
			?? panic("Could not get capability from account storage")
		
		// borrow a reference to the staking admin object
		let adminRef =
			adminCapability.borrow<&FlowIDTableStaking.Admin>()
	...
	access(contract)
	fun borrowClusterQCAdmin(): &FlowClusterQC.Admin{ 
		let adminCapability =
			self.account.storage.copy<Capability>(from: /storage/flowQCAdminEpochOperations)
			?? panic("Could not get capability from account storage")
		
		// borrow a reference to the QC admin object
		let adminRef =
			adminCapability.borrow<&FlowClusterQC.Admin>()
	...
	access(contract)
	fun borrowDKGAdmin(): &FlowDKG.Admin{ 
		let adminCapability =
			self.account.storage.copy<Capability>(from: /storage/flowDKGAdminEpochOperations)
			?? panic("Could not get capability from account storage")
		
		// borrow a reference to the dkg admin object
		let adminRef =
			adminCapability.borrow<&FlowDKG.Admin>()
	...
	fun createCollectorClusters(nodeIDs: [String]): [FlowClusterQC.Cluster]{ 
		pre{ 
			UInt16(nodeIDs.length) >= self.configurableMetadata.numCollectorClusters:
				"Cannot have less collector nodes than clusters"
		}
	...
	init(
		...
		self.configurableMetadata = Config(
		...
		var capability_1 =
			self.account.capabilities.storage.issue<&FlowIDTableStaking.Admin>(
				FlowIDTableStaking.StakingAdminStoragePath
			)
		self.account.capabilities.publish(
			capability_1,
			at: /private/flowStakingAdminEpochOperations
		)
		let stakingAdminCapability = capability_1
		self.account.storage.save<Capability<&FlowIDTableStaking.Admin>>(
			stakingAdminCapability,
			to: /storage/flowStakingAdminEpochOperations
		)
		
		// Create a private capability to the qc admin
		// and store it in a different path
		var capability_2 =
			self.account.capabilities.storage.issue<&FlowClusterQC.Admin>(
				FlowClusterQC.AdminStoragePath
			)
		self.account.capabilities.publish(capability_2, at: /private/flowQCAdminEpochOperations)
		let qcAdminCapability = capability_2
		self.account.storage.save<Capability<&FlowClusterQC.Admin>>(
			qcAdminCapability,
			to: /storage/flowQCAdminEpochOperations
		)
		
		// Create a private capability to the dkg admin
		// and store it in a different path
		var capability_3 =
			self.account.capabilities.storage.issue<&FlowDKG.Admin>(FlowDKG.AdminStoragePath)
		self.account.capabilities.publish(capability_3, at: /private/flowDKGAdminEpochOperations)
		let dkgAdminCapability = capability_3
		self.account.storage.save<Capability<&FlowDKG.Admin>>(
			dkgAdminCapability,
			to: /storage/flowDKGAdminEpochOperations
		)
		self.borrowStakingAdmin().startStakingAuction()
		let currentBlock = getCurrentBlock()
		let firstEpochMetadata =
			EpochMetadata(
				counter: self.currentEpochCounter,
				seed: randomSource,
				startView: currentBlock.view,
				endView: currentBlock.view + self.configurableMetadata.numViewsInEpoch
				- 1 as UInt64,
				stakingEndView: currentBlock.view
				+ self.configurableMetadata.numViewsInStakingAuction
				- 1 as UInt64,
				totalRewards: FlowIDTableStaking.getEpochTokenPayout(),
				collectorClusters: collectorClusters,
