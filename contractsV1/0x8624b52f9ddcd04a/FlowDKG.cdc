...
contract FlowDKG{ 
	...
	access(all)
	event EndDKG(finalSubmission: [String?]?)
	
	/// Emitted when a consensus node has posted a message to the DKG whiteboard
	access(all)
	event BroadcastMessage(nodeID: String, content: String)
	...
	access(account)
	var finalSubmissionByNodeID:{ String: [String?]}
	...
	access(account)
	var uniqueFinalSubmissions: [[String?]]
	
	/// Tracks how many submissions have been sent
	/// for each unique final submission
	access(account)
	var uniqueFinalSubmissionCount:{ Int: UInt64}
	...
	access(all)
	let ParticipantStoragePath: StoragePath
	
	access(all)
	let ParticipantPublicPath: PublicPath
	...
	resource Admin: EpochOperations{ 
		...
		fun setSafeSuccessThreshold(newThresholdPercentage: UFix64?){ 
			pre{ 
				!FlowDKG.dkgEnabled:
					"Cannot set the dkg success threshold while the DKG is enabled"
				newThresholdPercentage == nil || newThresholdPercentage! < 1.0:
					"The threshold percentage must be in [0,1)"
			}
		...
		fun startDKG(nodeIDs: [String]){ 
			pre{ 
				FlowDKG.dkgEnabled == false:
					"Cannot start the DKG when it is already running"
			}
		...
		fun endDKG(){ 
			pre{ 
				FlowDKG.dkgEnabled == true:
					"Cannot end the DKG when it is already disabled"
				FlowDKG.dkgCompleted() != nil:
					"Cannot end the DKG until enough final arrays have been submitted"
			}
			FlowDKG.dkgEnabled = false
	...
	access(account)
	fun submissionsEqual(_ existingSubmission: [String?], _ submission: [String?]): Bool{ 
