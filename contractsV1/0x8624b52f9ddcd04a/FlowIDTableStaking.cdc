...
contract FlowIDTableStaking{ 
	...
	access(all)
	event NewEpoch(totalStaked: UFix64, totalRewardPayout: UFix64, newEpochCounter: UInt64)
	
	access(all)
	event EpochTotalRewardsPaid(
		total: UFix64,
		fromFees: UFix64,
		minted: UFix64,
		feesBurned: UFix64,
	...
	access(all)
	event NodeTokensRequestedToUnstake(nodeID: String, amount: UFix64)
	
	access(all)
	event TokensUnstaking(nodeID: String, amount: UFix64)
	...
	access(all)
	event RewardsPaid(nodeID: String, amount: UFix64, epochCounter: UInt64)
	
	access(all)
	event UnstakedTokensWithdrawn(nodeID: String, amount: UFix64)
	
	access(all)
	event RewardTokensWithdrawn(nodeID: String, amount: UFix64)
	
	access(all)
	event NetworkingAddressUpdated(nodeID: String, newAddress: String)
	
	access(all)
	event NodeWeightChanged(nodeID: String, newWeight: UInt64)
	...
	access(all)
	event DelegatorTokensCommitted(nodeID: String, delegatorID: UInt32, amount: UFix64)
	
	access(all)
	event DelegatorTokensStaked(nodeID: String, delegatorID: UInt32, amount: UFix64)
	
	access(all)
	event DelegatorTokensRequestedToUnstake(nodeID: String, delegatorID: UInt32, amount: UFix64)
	
	access(all)
	event DelegatorTokensUnstaking(nodeID: String, delegatorID: UInt32, amount: UFix64)
	
	access(all)
	event DelegatorTokensUnstaked(nodeID: String, delegatorID: UInt32, amount: UFix64)
	
	access(all)
	event DelegatorRewardsPaid(
		nodeID: String,
		delegatorID: UInt32,
		amount: UFix64,
		epochCounter: UInt64
	...
	access(all)
	event DelegatorUnstakedTokensWithdrawn(nodeID: String, delegatorID: UInt32, amount: UFix64)
	...
	access(all)
	event NewStakingMinimums(newMinimums:{ UInt8: UFix64})
	
	access(all)
	event NewDelegatorStakingMinimum(newMinimum: UFix64)
	...
	access(all)
	let NodeStakerStoragePath: StoragePath
	
	access(all)
	let NodeStakerPublicPath: PublicPath
	...
	resource NodeRecord{ 
		...
		access(all)
		var tokensUnstaking: @FlowToken.Vault
		...
		init(
			...
			pre{ 
				...
				FlowIDTableStaking.isValidNodeID(id):
					"The node ID must have only numbers and lowercase hex characters"
				...
				networkingAddress.length > 0 && networkingAddress.length <= 510:
					"The networkingAddress must be less than 510 characters"
				networkingKey.length == 128:
					"The networkingKey length must be exactly 64 bytes (128 hex characters)"
				stakingKey.length == 192:
					"The stakingKey length must be exactly 96 bytes (192 hex characters)"
				!FlowIDTableStaking.getNetworkingAddressClaimed(address: networkingAddress):
					"The networkingAddress cannot have already been claimed"
				!FlowIDTableStaking.getNetworkingKeyClaimed(key: networkingKey):
					"The networkingKey cannot have already been claimed"
				!FlowIDTableStaking.getStakingKeyClaimed(key: stakingKey):
					"The stakingKey cannot have already been claimed"
			}
			let stakeKey =
				PublicKey(
			...
			FlowIDTableStaking.updateClaimed(
				path: /storage/networkingAddressesClaimed,
				networkingAddress,
				claimed: true
			)
			FlowIDTableStaking.updateClaimed(
				path: /storage/networkingKeysClaimed,
				networkingKey,
				claimed: true
			)
			FlowIDTableStaking.updateClaimed(
				path: /storage/stakingKeysClaimed,
				stakingKey,
				claimed: true
			)
			...
			self.tokensStaked <- FlowToken.createEmptyVault(vaultType: Type<@FlowToken.Vault>())
				as!
				@FlowToken.Vault
			self.tokensUnstaking <- FlowToken.createEmptyVault(vaultType: Type<@FlowToken.Vault>())
				as!
				@FlowToken.Vault
		...
		access(account)
		fun nodeFullCommittedBalance(): UFix64{ 
			if self.tokensCommitted.balance + self.tokensStaked.balance
		...
		access(account)
		fun setDelegator(delegatorID: UInt32, delegator: @DelegatorRecord){ 
	...
	resource DelegatorRecord{ 
		...
		access(all)
		let tokensUnstaked: @FlowToken.Vault
		...
		access(contract)
		fun delegatorFullCommittedBalance(): UFix64{ 
			if self.tokensCommitted.balance + self.tokensStaked.balance
			< self.tokensRequestedToUnstake{ 
				return 0.0
			} else{ 
	...
	access(all)
	resource interface NodeStakerPublic{ 
		access(all)
		let id: String
	}
	...
	access(all)
	resource NodeStaker: NodeStakerPublic{ 
		...
		access(self)
		fun isEligibleForCandidateNodeStatus(_ nodeRecord: &FlowIDTableStaking.NodeRecord): Bool{ 
			...
			return nodeRecord.tokensStaked.balance == 0.0 && FlowIDTableStaking.isGreaterThanMinimumForRole(numTokens: nodeRecord.tokensCommitted.balance, role: nodeRecord.role)
		}
		...
		fun updateNetworkingAddress(_ newAddress: String){ 
			pre{ 
				FlowIDTableStaking.stakingEnabled():
					"Cannot update networking address if the staking auction isn't in progress"
				newAddress.length > 0 && newAddress.length <= 510:
					"The networkingAddress must be less than 510 characters"
				!FlowIDTableStaking.getNetworkingAddressClaimed(address: newAddress):
					"The networkingAddress cannot have already been claimed"
			}
		...
		fun stakeNewTokens(_ tokens: @{FungibleToken.Vault}){ 
			pre{ 
				FlowIDTableStaking.stakingEnabled():
					"Cannot stake if the staking auction isn't in progress"
			}
		...
		fun requestUnstaking(amount: UFix64){ 
			...
			assert(nodeRecord.tokensStaked.balance + nodeRecord.tokensCommitted.balance >= amount + nodeRecord.tokensRequestedToUnstake, message: "Not enough tokens to unstake!")
			...
			assert(nodeRecord.delegators.length == 0 || FlowIDTableStaking.isGreaterThanMinimumForRole(numTokens: FlowIDTableStaking.NodeInfo(nodeID: nodeRecord.id).totalCommittedWithoutDelegators() - amount, role: nodeRecord.role), message: "Cannot unstake below the minimum if there are delegators")
	...
	access(all)
	resource interface NodeDelegatorPublic{ 
		access(all)
		let id: UInt32
	...
	access(all)
	resource NodeDelegator: NodeDelegatorPublic{ 
		access(all)
		let id: UInt32
		...
		fun delegateNewTokens(from: @{FungibleToken.Vault}){ 
			pre{ 
				FlowIDTableStaking.stakingEnabled():
					"Cannot delegate if the staking auction isn't in progress"
			}
		...
		fun requestUnstaking(amount: UFix64){ 
			pre{ 
				FlowIDTableStaking.stakingEnabled():
					"Cannot request unstaking if the staking auction isn't in progress"
			}
			let nodeRecord = FlowIDTableStaking.borrowNodeRecord(self.nodeID)
			let delRecord = nodeRecord.borrowDelegatorRecord(self.id)
			
			// The delegator must have enough tokens to unstake
			assert(delRecord.tokensStaked.balance + delRecord.tokensCommitted.balance >= amount + delRecord.tokensRequestedToUnstake, message: "Not enough tokens to unstake!")
	...
	access(all)
	struct EpochRewardsSummary{ 
		access(all)
		let totalRewards: UFix64
		
		access(all)
		let breakdown: [RewardsBreakdown]
	...
	struct RewardsBreakdown{ 
		...
		access(all)
		var nodeRewards: UFix64
		
		access(all)
		let delegatorRewards:{ UInt32: UFix64}
	...
	resource Admin: EpochOperations{ 
		...
		fun setSlotLimits(slotLimits:{ UInt8: UInt16}){ 
			pre{ 
				slotLimits.keys.length == 5:
					"Slot Limits Dictionary can only have 5 entries"
				slotLimits[1] != nil:
					"Need to have a limit set for collector nodes"
				slotLimits[2] != nil:
					"Need to have a limit set for consensus nodes"
				slotLimits[3] != nil:
					"Need to have a limit set for execution nodes"
				slotLimits[4] != nil:
					"Need to have a limit set for verification nodes"
				slotLimits[5] != nil:
					"Need to have a limit set for access nodes"
			}
		...
		fun setOpenNodeSlots(openSlots:{ UInt8: UInt16}){ 
			pre{ 
				openSlots[5] != nil:
					"Need to have a value set for access nodes"
			}
		...
		access(self)
		fun unsafeSetApprovedList(_ newApproveList:{ String: Bool}){ 
			let currentApproveList = FlowIDTableStaking.account.storage.load<{String: Bool}>(from: /storage/idTableApproveList) ?? panic("Could not load the current approve list from storage")
		...
		access(self)
		fun unsafeRemoveAndRefundNodeRecord(_ nodeID: String){ 
			let nodeRecord = FlowIDTableStaking.borrowNodeRecord(nodeID)
			...
			var movesPendingList = FlowIDTableStaking.account.storage.borrow<&{String:{ UInt32: Bool}}>(from: /storage/idTableMovesPendingList) ?? panic("No moves pending list in account storage")
		...
		fun removeInvalidNodes():{ String: Bool}{ 
			...
			let participantList = FlowIDTableStaking.getParticipantNodeList() ?? panic("Could not copy participant list from storage")
		...
		fun payRewards(forEpochCounter: UInt64, rewardsSummary: EpochRewardsSummary){ 
			...
			if mintedRewards > 0.0{ 
				let flowTokenMinter = FlowIDTableStaking.account.storage.borrow<&FlowToken.Minter>(from: /storage/flowTokenMinter) ?? panic("Could not borrow minter reference")
		...
		fun calculateRewards(): EpochRewardsSummary{ 
			...
			for nodeID in nonOperationalNodes.keys{ 
				...
				for delegator in nodeRecord.delegators.keys{ 
					...
					if delegatorRewardsAfterWithholding == 0.0{ 
						continue
					}
			...
			for nodeID in stakedNodeIDs.keys{ 
				if nonOperationalNodes[nodeID] != nil{ 
					continue
				}
				let nodeRecord = FlowIDTableStaking.borrowNodeRecord(nodeID)
				var nodeRewardAmount = nodeRecord.tokensStaked.balance * totalRewardsPlusWithheld
				if nodeRewardAmount == 0.0 || nodeRecord.role == UInt8(5){ 
					continue
				}
				let rewardsBreakdown = FlowIDTableStaking.RewardsBreakdown(nodeID: nodeID)
		...
		fun moveTokens(newEpochCounter: UInt64){ 
			pre{ 
				!FlowIDTableStaking.stakingEnabled():
					"Cannot move tokens if the staking auction is still in progress"
			}
			let approvedNodeIDs = FlowIDTableStaking.getApprovedList() ?? panic("Could not read the approve list from storage")
			let movesPendingNodeIDs = FlowIDTableStaking.account.storage.load<{String:{ UInt32: Bool}}>(from: /storage/idTableMovesPendingList) ?? panic("No moves pending list in account storage")
			...
			let newMovesPendingList = FlowIDTableStaking.account.storage.borrow<&{String:{ UInt32: Bool}}>(from: /storage/idTableMovesPendingList) ?? panic("No moves pending list in account storage")
			let stakedNodeIDs:{ String: Bool} = FlowIDTableStaking.getParticipantNodeList()!
			...
			emit NewEpoch(totalStaked: FlowIDTableStaking.getTotalStaked(), totalRewardPayout: FlowIDTableStaking.epochTokenPayout, newEpochCounter: newEpochCounter)
		}
	}
	...
	fun addNodeRecord(
		...
		pre{ 
			FlowIDTableStaking.stakingEnabled():
				"Cannot register a node operator if the staking auction isn't in progress"
		}
		let newNode <-
			create NodeRecord(
				id: id,
		...
		assert(
			...
			message: "Tokens committed for registration is not above the minimum (".concat(
				minimum.toString()
			).concat(") for the chosen node role (".concat(role.toString()).concat(")"))
		)
	...
	fun registerNewDelegator(
		...
		assert(
			...
			message: "Tokens committed for delegator registration is not above the minimum ("
				.concat(minimum.toString()).concat(")")
		)
		assert(
			FlowIDTableStaking.isGreaterThanMinimumForRole(
				numTokens: nodeRecord.nodeFullCommittedBalance(),
		...
		nodeRecord.setDelegator(
			delegatorID: nodeRecord.delegatorIDCounter,
			delegator: <-create DelegatorRecord()
		)
		...
		let newDelegator <-
			create NodeDelegator(id: nodeRecord.delegatorIDCounter, nodeID: nodeRecord.id)
	...
	access(account)
	fun borrowFeesAdmin(): &FlowFees.Administrator{ 
		let feesAdmin =
			self.account.storage.borrow<&FlowFees.Administrator>(from: /storage/flowFeesAdmin)
	...
	access(account)
	fun updateClaimed(path: StoragePath, _ key: String, claimed: Bool){ 
	...
	access(contract)
	fun setParticipantNodeList(_ nodeIDs:{ String: Bool}){ 
	...
	fun modifyNewMovesPending(
		...
		let movesPendingList =
			existingList
			?? self.account.storage.borrow<&{String:{ UInt32: Bool}}>(
	...
	access(contract)
	fun addToCandidateNodeList(nodeID: String, roleToAdd: UInt8){ 
		pre{ 
			roleToAdd >= UInt8(1) && roleToAdd <= UInt8(5):
				"The role must be 1, 2, 3, 4, or 5"
		}
		var candidateNodes =
			FlowIDTableStaking.account.storage.borrow<&{UInt8:{ String: Bool}}>(
				from: /storage/idTableCandidateNodes
			)!
		var candidateNodesForRole = candidateNodes[roleToAdd]!
		if UInt64(candidateNodesForRole.keys.length)
		>= (self.getCandidateNodeLimits()!)[roleToAdd]!{ 
	...
	access(contract)
	fun removeFromCandidateNodeList(nodeID: String, role: UInt8){ 
		pre{ 
			role >= UInt8(1) && role <= UInt8(5):
				"The role must be 1, 2, 3, 4, or 5"
		}
	...
	fun getCurrentRoleNodeCounts():{ UInt8: UInt16}{ 
		if let currentCounts =
			FlowIDTableStaking.account.storage.copy<{UInt8: UInt16}>(
				from: /storage/flowStakingRoleNodeCounts
			){ 
	...
	fun getProposedNodeIDs(): [String]{ 
		...
		let approvedNodeIDs:{ String: Bool} =
			FlowIDTableStaking.getApprovedList()
		...
		for nodeID in nodeIDs{ 
			...
			//  - Old ANs must be approved, but are allowed to have zero stake
			//  - New ANs may be unapproved, but must have submitted sufficient stake
			if nodeRecord.role == UInt8(5) && (greaterThanMin || nodeIsApproved) && nodeWeight > 0{ 
				proposedNodeIDs[nodeID] = true
				continue
			}
		}
	...
	access(account)
	view fun getClaimed(path: StoragePath, key: String): Bool{ 
		let claimedDictionary =
			self.account.storage.borrow<&{String: Bool}>(from: path)
			?? panic("Invalid path for dictionary")
	...
	init(_ epochTokenPayout: UFix64, _ rewardCut: UFix64, _ candidateNodeLimits:{ UInt8: UInt64}){ 
		...
		self.account.storage.save<{String: UFix64}>(
			nonOperationalList,
			to: /storage/idTableNonOperationalNodesList
		)
		let movesPendingList:{ String:{ UInt32: Bool}} ={} 
		self.account.storage.save<{String:{ UInt32: Bool}}>(
			movesPendingList,
			to: /storage/idTableMovesPendingList
		)
		let emptyCandidateNodes:{ UInt8:{ String: Bool}} ={ 1:{} , 2:{} , 3:{} , 4:{} , 5:{} }
		FlowIDTableStaking.account.storage.save(
			emptyCandidateNodes,
			to: /storage/idTableCandidateNodes
		)
		
		// Save the candidate nodes limit
		FlowIDTableStaking.account.storage.save<{UInt8: UInt64}>(
			candidateNodeLimits,
			to: /storage/idTableCandidateNodeLimits
		)
