...
contract SwapConfig{ 
	...
	access(all)
	resource interface getAmountOutOutside{ 
		access(TMP_ENTITLEMENT_OWNER)
		fun setPoolIn(poolIn: AnyStruct): Void
		
		access(TMP_ENTITLEMENT_OWNER)
		fun setPoolOut(poolOut: AnyStruct)
	}
	
	access(all)
	resource getAmountOutLocal: getAmountOutOutside{ 
		access(all)
		var poolIn: AnyStruct
		
		access(all)
		var poolOut: AnyStruct
		
		access(TMP_ENTITLEMENT_OWNER)
		fun setPoolIn(poolIn: AnyStruct){ 
			if self.poolIn == nil{ 
				self.poolIn = poolIn
			}
		}
	...
	access(TMP_ENTITLEMENT_OWNER)
	fun getAmountOutLocaly(): @getAmountOutLocal{ 
