...
access(all)
contract TMCAFR: NonFungibleToken{ 
	access(all)
	...
	access(all)
	resource interface TMCAFRCollectionPublic{ 
		access(all)
		...
		fun borrowTMCAFR(id: UInt64): &TMCAFR.NFT?{ 
			post{ 
	...
	access(all)
	resource Collection: TMCAFRCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic{ 
		access(all)
