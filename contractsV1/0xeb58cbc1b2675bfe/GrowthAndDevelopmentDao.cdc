...
access(all)
contract GrowthAndDevelopmentDao: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				GrowthAndDevelopmentDao.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return GrowthAndDevelopmentDao.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				GrowthAndDevelopmentDao.maxSupply == nil || GrowthAndDevelopmentDao.totalSupply <= GrowthAndDevelopmentDao.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			GrowthAndDevelopmentDao.totalSupply = GrowthAndDevelopmentDao.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "Growth And Development Dao", symbol: "GDN", description: "A Generational Dao to bring those of growth and development to help propel web3 into the communities of the people", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: GrowthAndDevelopmentDao.VaultStoragePath, receiverPath: GrowthAndDevelopmentDao.ReceiverPublicPath, metadataPath: GrowthAndDevelopmentDao.VaultPublicPath, receiverLinkedType: /private/GrowthAndDevelopmentDaoVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-GrowthAndDevelopmentDao.createEmptyVault(vaultType: Type<@GrowthAndDevelopmentDao.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/GrowthAndDevelopmentDaoVault
		self.ReceiverPublicPath = /public/GrowthAndDevelopmentDaoReceiver
		self.VaultPublicPath = /public/GrowthAndDevelopmentDaoMetadata
		self.MinterStoragePath = /storage/GrowthAndDevelopmentDaoMinter
		self.AdministratorStoragePath = /storage/GrowthAndDevelopmentDaoAdmin
