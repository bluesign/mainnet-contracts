...
access(all)
contract Utopian: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				Utopian.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return Utopian.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				Utopian.maxSupply == nil || Utopian.totalSupply <= Utopian.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			Utopian.totalSupply = Utopian.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "Utopian", symbol: "UTO", description: "A DAO for people who care about truth-seeking technology news", externalURL: MetadataViews.ExternalURL("medium.com/utopian"), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("utopianhq"), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: Utopian.VaultStoragePath, receiverPath: Utopian.ReceiverPublicPath, metadataPath: Utopian.VaultPublicPath, receiverLinkedType: /private/UtopianVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-Utopian.createEmptyVault(vaultType: Type<@Utopian.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/UtopianVault
		self.ReceiverPublicPath = /public/UtopianReceiver
		self.VaultPublicPath = /public/UtopianMetadata
		self.MinterStoragePath = /storage/UtopianMinter
		self.AdministratorStoragePath = /storage/UtopianAdmin
