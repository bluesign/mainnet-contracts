...
//
access(all)
contract SportsIconNFTStorefront{ 
	...
	resource interface StorefrontManager{ 
		...
		fun createListing(
			...
			salePaymentVaultType: Type,
			saleCuts: [
				SportsIconNFTStorefront.SaleCut
			],
			startTime: UFix64,
			endTime: UFix64
		): UInt64
	...
	resource Storefront: StorefrontManager, StorefrontPublic{ 
		...
		//
		access(TMP_ENTITLEMENT_OWNER)
		fun createListing(nftProviderCapability: Capability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>, nftType: Type, nftID: UInt64, salePaymentVaultType: Type, saleCuts: [SaleCut], startTime: UFix64, endTime: UFix64): UInt64{ 
