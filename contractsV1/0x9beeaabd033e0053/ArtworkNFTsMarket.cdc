...
contract ArtworkNFTsMarket{ 
	...
	resource SaleOffer: SaleOfferPublicView{ 
		...
		access(self)
		let sellerItemProvider: Capability<&ArtworkNFTs.Collection>
		
		// The PigCoin vault that will receive that payment if teh sale completes successfully.
		access(self)
		let sellerPaymentReceiver: Capability<&PigCoin.Vault>
		...
		//
		init(sellerItemProvider: Capability<&ArtworkNFTs.Collection>, itemID: UInt64, typeID: UInt64, hashCode: String, sellerPaymentReceiver: Capability<&PigCoin.Vault>, price: UFix64){ 
			pre{ 
	...
	fun createSaleOffer(
		...
		typeID: UInt64,
		hashCode: String,
		sellerPaymentReceiver: Capability<&PigCoin.Vault>,
		price: UFix64
	): @SaleOffer{ 
	...
	resource Collection: CollectionManager, CollectionPurchaser, CollectionPublic{ 
		...
		fun insert(offer: @ArtworkNFTsMarket.SaleOffer){ 
			...
			destroy oldOffer
			emit CollectionInsertedSaleOffer(itemID: itemID, typeID: typeID, hashCode: hashCode, owner: self.owner?.address!, price: price)
		}
		...
		fun borrowSaleItem(itemID: UInt64): &SaleOffer?{ 
			if self.saleOffers[itemID] == nil{ 
				...
				return &self.saleOffers[itemID] as &ArtworkNFTsMarket.SaleOffer?
			}
		}
