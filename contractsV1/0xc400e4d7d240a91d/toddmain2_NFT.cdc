...
contract toddmain2_NFT: NonFungibleToken{ 
	...
	//
	access(all)
	resource Collection: toddmain2_NFTCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic{ 
	...
	fun fetch(_ from: Address, id: UInt64): &toddmain2_NFT.NFT?{ 
		let collection = (getAccount(from).capabilities.get<&toddmain2_NFT.Collection>(toddmain2_NFT.CollectionPublicPath)!).borrow() ?? panic("Couldn't get collection")
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&toddmain2_NFT.Admin>(self.AdminStoragePath)
