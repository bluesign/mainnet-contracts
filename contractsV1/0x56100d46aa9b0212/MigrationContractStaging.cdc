...
contract MigrationContractStaging{ 
	...
	access(self)
	let capsulePathPrefix: String
	
	access(all)
	let HostStoragePath: StoragePath
	...
	access(self)
	let stagedContracts:{ Address: [String]}
	...
	access(all)
	event EmulatedMigrationResultCommitted(
	...
	access(all)
	event StagingCutoffUpdated(old: UInt64?, new: UInt64?)
	...
	///
	access(all)
	fun stageContract(host: &Host, name: String, code: String){ 
		pre{ 
			self.isStagingPeriodActive():
				"Staging period has ended"
		}
		let capsulePath =
			self.deriveCapsuleStoragePath(contractAddress: host.address(), contractName: name)
		...
		if let contractIndex = (self.stagedContracts[host.address()]!).firstIndex(of: name){ 
			...
			let capsule = self.account.storage.borrow<&Capsule>(from: capsulePath) ?? panic("Could not borrow existing Capsule from storage for staged contract")
	...
	///
	access(all)
	fun unstageContract(host: &Host, name: String){ 
		pre{ 
			self.isStagingPeriodActive():
				"Staging period has ended"
		}
	...
	///
	access(all)
	view fun isStagingPeriodActive(): Bool{ 
	...
	///
	access(all)
	view fun isStaged(address: Address, name: String): Bool{ 
	...
	///
	access(all)
	view fun isValidated(address: Address, name: String): Bool?{ 
	...
	///
	access(all)
	view fun getStagedContractNames(forAddress: Address): [String]{ 
	...
	///
	access(all)
	view fun getStagedContractCode(address: Address, name: String): String?{ 
	...
	///
	access(all)
	view fun getStagedContractUpdate(address: Address, name: String): ContractUpdate?{ 
		let capsulePath =
			self.deriveCapsuleStoragePath(contractAddress: address, contractName: name)
	...
	///
	access(all)
	view fun getAllStagedContractHosts(): [Address]{ 
	...
	///
	access(all)
	view fun getAllStagedContractCode(forAddress: Address):{ String: String}{ 
	...
	///
	access(all)
	view fun getAllStagedContracts():{ Address: [String]}{ 
	...
	access(all)
	view fun deriveCapsuleStoragePath(contractAddress: Address, contractName: String): StoragePath{ 
		let identifier =
			self.capsulePathPrefix.concat(self.delimiter).concat(contractAddress.toString()).concat(
				self.delimiter
			).concat(contractName)
	...
	///
	access(all)
	struct EmulatedMigrationResult{ 
	...
	struct ContractUpdate{ 
		...
		///
		access(all)
		view fun identifier(): String{ 
			let sans0x =
				self.address.toString().slice(from: 2, upTo: self.address.toString().length)
	...
	resource Capsule{ 
		...
		///
		access(all)
		view fun getContractUpdate(): ContractUpdate{ 
			return self.update
		}
	...
	resource Admin{ 
		...
		///
		access(all)
		fun commitMigrationResults(snapshot: UFix64, failed: [String]){ 
			...
			emit EmulatedMigrationResultCommitted(
				...
				committedTimestamp: (MigrationContractStaging.lastEmulatedMigrationResult!)
					.committed,
				failedContracts: failed
			)
		}
	...
	///
	access(self)
	fun createCapsule(host: &Host, name: String, code: String): @Capsule{ 
	...
	///
	access(self)
	fun removeStagedContract(address: Address, name: String): UInt64?{ 
	...
	///
	access(self)
	fun destroyCapsule(address: Address, name: String): UInt64?{ 
		let capsulePath =
			self.deriveCapsuleStoragePath(contractAddress: address, contractName: name)
	...
	init(){ 
		...
		self.HostStoragePath = StoragePath(
				identifier: "MigrationContractStagingHost".concat(self.delimiter).concat(
		...
		self.capsulePathPrefix = "MigrationContractStagingCapsule".concat(self.delimiter).concat(
				self.account.address.toString()
			)
