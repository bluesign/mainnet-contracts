...
access(all)
contract stFlowToken: FungibleToken{ 
	...
	access(all)
	let tokenProviderPath: PrivatePath
	...
	//
	access(account)
	fun mintTokens(amount: UFix64): @stFlowToken.Vault{ 
		pre{ 
			amount > 0.0:
	...
	init(){ 
		...
		//
		var capability_1 = self.account.capabilities.storage.issue<&stFlowToken.Vault>(self.tokenVaultPath)
		self.account.capabilities.publish(capability_1, at: self.tokenReceiverPath)
		
		// Create a public capability to the stored Vault that only exposes
		// the `balance` field through the `Balance` interface
		//
		var capability_2 = self.account.capabilities.storage.issue<&stFlowToken.Vault>(self.tokenVaultPath)
		self.account.capabilities.publish(capability_2, at: self.tokenBalancePath)
