...
access(all)
contract LiquidStakingConfig{ 
	
	/// Minimum limit of staking
	access(all)
	var minStakingAmount: UFix64
	
	/// Pauses
	access(all)
	var isStakingPaused: Bool
	
	access(all)
	var isUnstakingPaused: Bool
	
	access(all)
	var isMigratingPaused: Bool
	
	/// Max staking amount
	access(all)
	var stakingCap: UFix64
	
	// Windows size before auction stage end, left for bots to handle unstaking requests
	access(all)
	var windowSizeBeforeStakingEnd: UInt64
	
	/// Fee of quick unstaking from reserved committed vault
	access(all)
	var quickUnstakeFee: UFix64
	
	/// Cut of staking interests reserved as protocol fees
	access(all)
	var protocolFee: UFix64
	...
	/// events
	access(all)
	event ConfigMinStakingAmount(newValue: UFix64, oldValue: UFix64)
	
	access(all)
	event ConfigStakingCap(newValue: UFix64, oldValue: UFix64)
	
	access(all)
	event ConfigStakingPause(newValue: Bool, oldValue: Bool)
	
	access(all)
	event ConfigUnstakingPause(newValue: Bool, oldValue: Bool)
	
	access(all)
	event ConfigMigratingPause(newValue: Bool, oldValue: Bool)
	
	access(all)
	event ConfigQuickUnstakeFee(newValue: UFix64, oldValue: UFix64)
	
	access(all)
	event ConfigProtocolFee(newValue: UFix64, oldValue: UFix64)
	
	access(all)
	event ConfigWindowSize(newValue: UInt64, oldValue: UInt64)
	...
	fun calcEstimatedStakingPayout(stakedAmount: UFix64): UFix64{ 
		...
		let scaledSystemTotalStaked =
			self.UFix64ToScaledUInt256(FlowIDTableStaking.getTotalStaked())
		let scaledEpochTokenPayout =
			self.UFix64ToScaledUInt256(FlowIDTableStaking.getEpochTokenPayout())
		if scaledSystemTotalStaked == 0{ 
			return 0.0
		}
		var scaledRewardAmount =
			scaledStakedAmount * scaledEpochTokenPayout / scaledSystemTotalStaked
		let scaledNodeCutAmount =
			scaledRewardAmount
			* self.UFix64ToScaledUInt256(FlowIDTableStaking.getRewardCutPercentage())
			/ self.scaleFactor
		// Deducting node cut
		scaledRewardAmount = scaledRewardAmount - scaledNodeCutAmount
		let scaledProtocolCutAmount =
			scaledRewardAmount * self.UFix64ToScaledUInt256(self.protocolFee) / self.scaleFactor
	...
	resource Admin{ 
		...
		fun setMinStakingAmount(minStakingAmount: UFix64){ 
			emit ConfigMinStakingAmount(
				...
				oldValue: LiquidStakingConfig.minStakingAmount
			)
		...
		fun setQuickUnstakeFee(quickUnstakeFee: UFix64){ 
			pre{ 
				quickUnstakeFee < 1.0:
					"Invalid quick unstake fee"
			}
			emit ConfigQuickUnstakeFee(
				newValue: quickUnstakeFee,
				oldValue: LiquidStakingConfig.quickUnstakeFee
			)
		...
		fun setWindowSize(windowSize: UInt64){ 
			emit ConfigWindowSize(
				newValue: windowSize,
				oldValue: LiquidStakingConfig.windowSizeBeforeStakingEnd
			)
