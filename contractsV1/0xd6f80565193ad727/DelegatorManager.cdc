...
contract DelegatorManager{ 
	...
	access(self)
	let migratedDelegatorIDs:{ String:{ UInt32: UInt64}}
	...
	access(all)
	var defaultNodeIDToStake: String
	...
	access(all)
	var requestedToUnstake: UFix64
	
	/// Collect and aggregate all rewards & unstaked tokens from all delegators at the beginning of each epoch
	access(self)
	let totalRewardedVault: @FlowToken.Vault
	
	access(self)
	let totalUnstakedVault: @FlowToken.Vault
	
	/// All epoch snapshot history
	/// {epoch index -> snapshot}
	access(self)
	let epochSnapshotHistory:{ UInt64: EpochSnapshot}
	
	/// Vault of protocol fees
	access(self)
	let protocolFeeVault: @FlowToken.Vault
	...
	access(all)
	let delegationStrategyPath: StoragePath
	
	/// Events
	access(all)
	event NewQuoteEpoch(epoch: UInt64)
	
	access(all)
	event RegisterNewDelegator(nodeID: String, delegatorID: UInt32)
	
	access(all)
	event RewardsInfoCheckpointed(
		currEpoch: UInt64,
		received: UFix64,
		estimated: UFix64,
		estimateNextReward: UFix64
	)
	
	access(all)
	event DepositProtocolFees(amount: UFix64, purpose: String)
	
	access(all)
	event ExternalRewardsContributed(amount: UFix64)
	
	access(all)
	event RestakeCanceledTokens(amount: UFix64, type: String, fromProtocolEpoch: UInt64)
	
	access(all)
	event RedelegateTokens(amount: UFix64, currEpoch: UInt64)
	
	access(all)
	event CompoundReward(rewardAmount: UFix64, protocolEpoch: UInt64)
	
	access(all)
	event DelegatorsCollected(startIndex: Int, endIndex: Int, uncollectedCount: Int)
	
	access(all)
	event StrategyTransferCommittedTokens(from: String, to: String, amount: UFix64)
	
	access(all)
	event StrategyProcessUnstakeRequest(
		amount: UFix64,
		nodeIDToUnstake: String,
		delegatorIDToUnstake: UInt32,
		leftoverAmount: UFix64
	)
	
	access(all)
	event DelegatorRemoved(nodeID: String, delegatorID: UInt32, uuid: UInt64)
	
	access(all)
	event SetApprovedNodeList(nodeIDs:{ String: UFix64}, defaultNodeIDToStake: String)
	
	access(all)
	event UpsertApprovedNode(nodeID: String, oldWeight: UFix64?, newWeight: UFix64)
	
	access(all)
	event SetDefaultStakeNode(from: String, to: String)
	
	access(all)
	event ApprovedNodeCanceled(nodeID: String)
	
	access(all)
	event ApprovedNodeRemoved(nodeID: String)
	
	access(all)
	event RedelegateRequested(
		nodeID: String,
		delegatorID: UInt32,
		redelegateCommittedAmount: UFix64,
		redelegateRequestToUnstake: UFix64
	)
	...
	struct EpochSnapshot{ 
		...
		access(all)
		var scaledQuoteStFlowFlow: UInt256
		
		/// Price: Flow to stFlow (<= 1.0)
		access(all)
		var scaledQuoteFlowStFlow: UInt256
		
		/// Total staked amount of all delegators
		access(all)
		var allDelegatorStaked: UFix64
		
		/// Total committed amount of all delegators
		access(all)
		var allDelegatorCommitted: UFix64
		
		/// Total requests to unstake of all delegators
		access(all)
		var allDelegatorRequestedToUnstake: UFix64
		
		/// After-fee total rewards received of current protocol epoch
		access(all)
		var receivedReward: UFix64
		
		/// Estimated rewards to be received
		access(all)
		var estimatedReward: UFix64
		
		/// Snapshotted delegator infos
		/// {nodeID -> {delegatorID -> DelegatorInfo}}
		access(self)
		let delegatorInfoDict:{ String:{ UInt32: FlowIDTableStaking.DelegatorInfo}}
		
		/// { delegator uuid -> collected? }
		access(self)
		let delegatorCollected:{ UInt64: Bool}
		...
		access(all)
		var canceledCommittedTokens: UFix64
		...
		access(all)
		var canceledStakedTokens: UFix64
		
		/// Tokens that are requested to unstake, will be in unstaking mode in the next protocol epoch
		access(all)
		var redelegatedTokensRequestToUnstake: UFix64
		
		/// Tokens in unstaking mode, will be turned to unstaked (and available to be restaked) in the next protocol epoch
		access(all)
		var redelegatedTokensUnderUnstaking: UFix64
		
		/// Start time of new quote epoch
		access(all)
		var quoteEpochStartTimestamp: UFix64
		
		access(all)
		var quoteEpochStartBlockHeight: UInt64
		
		access(all)
		var quoteEpochStartBlockView: UInt64
		
		/// Update or insert the snapshotted delegator info
		access(contract)
		fun upsertDelegatorInfo(nodeID: String, delegatorID: UInt32){ 
			let delegatorInfo =
				FlowIDTableStaking.DelegatorInfo(nodeID: nodeID, delegatorID: delegatorID)
			if self.delegatorInfoDict.containsKey(nodeID) == false{ 
				self.delegatorInfoDict[nodeID] ={} 
			} else if (self.delegatorInfoDict[nodeID]!).containsKey(delegatorID) == true{ 
			...
			self.allDelegatorRequestedToUnstake = self.allDelegatorRequestedToUnstake
		...
		fun borrowDelegatorInfo(
			...
			return &(self.delegatorInfoDict[nodeID]!)[delegatorID]
			as
			&FlowIDTableStaking.DelegatorInfo?
		}
		...
		fun getDelegatorIDListOnNode(nodeID: String): [UInt32]{ 
			return self.delegatorInfoDict[nodeID] == nil
				? []
				: (self.delegatorInfoDict[nodeID]!).keys
		}
		
		access(contract)
		fun markDelegatorCollected(uuid: UInt64){ 
			self.delegatorCollected.insert(key: uuid, true)
		}
		
		access(contract)
		fun removeDelegatorInfo(nodeID: String, delegatorID: UInt32){ 
			if self.delegatorInfoDict.containsKey(nodeID)
			&& (self.delegatorInfoDict[nodeID]!).containsKey(delegatorID){ 
				let delegatorInfo =
					FlowIDTableStaking.DelegatorInfo(nodeID: nodeID, delegatorID: delegatorID)
				assert(
					delegatorInfo.totalTokensInRecord() + delegatorInfo.tokensRequestedToUnstake
					== 0.0,
	...
	fun advanceEpoch(){ 
		pre{ 
			FlowEpoch.currentEpochCounter > self.quoteEpochCounter:
				"can only advance protocol epoch after a new chain epoch starts"
		}
	...
	fun stFlowQuote(){ 
		pre{ 
			FlowEpoch.currentEpochCounter > self.quoteEpochCounter:
				"stFlow price can only be checkpointed after new chain epoch and before new protocol epoch start"
		}
		...
		let totalStaked =
			nextEpochSnapshot.allDelegatorStaked + nextEpochSnapshot.canceledStakedTokens
	...
	access(account)
	fun depositToCommitted(flowVault: @FlowToken.Vault){ 
		let defaultDelegator =
			self.borrowApprovedDelegatorFromNode(self.defaultNodeIDToStake)
	...
	access(account)
	fun withdrawFromCommitted(amount: UFix64): @FlowToken.Vault{ 
		...
		let defaultDelegator =
			self.borrowApprovedDelegatorFromNode(self.defaultNodeIDToStake)
			?? panic("delegator object not found from the defaultNodeToStake")
		let defaultDelegatorInfo =
			FlowIDTableStaking.DelegatorInfo(
				nodeID: defaultDelegator.nodeID,
				delegatorID: defaultDelegator.id
			)
		assert(
			defaultDelegatorInfo.tokensCommitted >= amount,
			message: "Not enough committed tokens to withdraw"
		)
		...
		// Update snapshot
		self.borrowCurrentQuoteEpochSnapshot().upsertDelegatorInfo(
			nodeID: defaultDelegator.nodeID,
			delegatorID: defaultDelegator.id
		)
	...
	access(account)
	fun requestWithdrawFromStaked(amount: UFix64){ 
		...
		assert(
			...
			>= amount + currentEpochSnapshot.allDelegatorRequestedToUnstake
			+ self.requestedToUnstake,
	...
	access(account)
	fun withdrawFromUnstaked(amount: UFix64): @FlowToken.Vault{ 
	...
	access(account)
	fun migrateDelegator(delegator: @FlowIDTableStaking.NodeDelegator){ 
	...
	access(self)
	fun restakeCanceledTokens(){ 
		pre{ 
			FlowEpoch.currentEpochCounter > self.quoteEpochCounter:
				"restake canceled tokens can only happen after new chain epoch and before new protocol epoch start"
		}
		// Protocol epoch N
	...
	access(self)
	fun redelegateTokens(){ 
		pre{ 
			FlowEpoch.currentEpochCounter > self.quoteEpochCounter:
				"redelegate requests can only be processed after new chain epoch and before new protocol epoch start"
		}
		let currEpochSnapshot = self.borrowCurrentQuoteEpochSnapshot()
		let nextEpochSnapshot = self.borrowCurrentChainEpochSnapshot()
	...
	fun collectDelegatorsOnEpochStart(startIndex: Int, endIndex: Int, ifAdvanceEpoch: Bool){ 
		pre{ 
			FlowEpoch.currentEpochCounter > self.quoteEpochCounter:
				"No need to collect, chain epoch not advanced yet"
			startIndex <= endIndex:
				"Invalid index"
		}
		...
		if FlowEpoch.automaticRewardsEnabled() == true{ 
			if self.quoteEpochCounter > 0{ 
				assert((FlowEpoch.getEpochMetadata(self.quoteEpochCounter)!).rewardsPaid == true, message: LiquidStakingError.ErrorEncode(msg: "Rewards has not been paid yet for protocol epoch ".concat(self.quoteEpochCounter.toString()), err: LiquidStakingError.ErrorCode.STAKING_REWARD_NOT_PAID))
			}
		}
		...
		while index <= endIndex && index < delegatorLength{ 
			...
			if currDelegatorInfo != nil{ 
				...
				if latestDelegatorInfo.tokensStaked == 0.0 && ((currDelegatorInfo!).tokensCommitted > 0.0 || (currDelegatorInfo!).tokensStaked - (currDelegatorInfo!).tokensRequestedToUnstake > 0.0){ 
		...
		emit DelegatorsCollected(
			startIndex: startIndex,
			endIndex: endIndex,
			uncollectedCount: self.allDelegators.length - collectedCount
		)
	}
	
	access(contract)
	fun insertMigratedDelegatorUUID(nodeID: String, delegatorID: UInt32, uuid: UInt64){ 
		if self.migratedDelegatorIDs.containsKey(nodeID) == false{ 
			self.migratedDelegatorIDs[nodeID] ={} 
		}
		assert(
			(self.migratedDelegatorIDs[nodeID]!).containsKey(delegatorID) == false,
			message: "Reinsert delegator uuid"
		)
	...
	access(self)
	fun snapshotRewardsInfo(){ 
		pre{ 
			FlowEpoch.currentEpochCounter > self.quoteEpochCounter:
				"Rewards info can only be snapshotted after new chain epoch and before new protocol epoch start"
		}
		// Protocol epoch N
		let currEpochSnapshot = self.borrowCurrentQuoteEpochSnapshot()
		// Chain epoch N+1
		...
		let estimatedNextEpochAmount =
			LiquidStakingConfig.calcEstimatedStakingPayout(
				stakedAmount: nextEpochSnapshot.allDelegatorStaked
			)
		nextEpochSnapshot.setEstimatedReward(estimated: estimatedNextEpochAmount)
		emit RewardsInfoCheckpointed(
			currEpoch: self.quoteEpochCounter,
			received: currEpochSnapshot.receivedReward,
			estimated: currEpochSnapshot.estimatedReward,
			estimateNextReward: nextEpochSnapshot.estimatedReward
		)
	}
	
	/// Check protocol's approved nodes against FlowIDTableStaking.getStakedNodeIDs()
	access(self)
	fun filterApprovedNodeListOnEpochStart(){ 
	...
	access(self)
	fun borrowManagedDelegator(uuid: UInt64): &FlowIDTableStaking.NodeDelegator?{ 
		return &self.allDelegators[uuid] as &FlowIDTableStaking.NodeDelegator?
	}
	
	access(self)
	fun removeManagedDelegator(uuid: UInt64){ 
		let tmpDelegator <- self.allDelegators[uuid] <- nil
		destroy tmpDelegator
	}
	
	access(self)
	fun borrowApprovedDelegatorFromNode(_ nodeID: String): &FlowIDTableStaking.NodeDelegator?{ 
		if (
			self.approvedNodeIDList.containsKey(nodeID)
			&& self.approvedDelegatorIDs.containsKey(nodeID)
		)
		== false{ 
			return nil
		}
	...
	access(self)
	fun registerApprovedDelegator(_ nodeID: String, _ initialCommit: @{FungibleToken.Vault}){ 
		pre{ 
			FlowIDTableStaking.getStakedNodeIDs().contains(nodeID):
				"Cannot stake to the inactive node: ".concat(nodeID)
			self.approvedNodeIDList.containsKey(nodeID):
				"Cannot register delegator on nodes out of approved list"
			!self.approvedDelegatorIDs.containsKey(nodeID):
				"Delegator object on the given node has already existed"
			initialCommit.balance >= FlowIDTableStaking.getDelegatorMinimumStakeRequirement():
				"Initial commitment for delegator registration < system threshold"
		}
		let nodeDelegator <-
			FlowIDTableStaking.registerNewDelegator(
				nodeID: nodeID,
				tokensCommitted: <-initialCommit
			)
	...
	fun removeApprovedNodeID(nodeID: String){ 
		pre{ 
			self.approvedNodeIDList.containsKey(nodeID):
				"Nonexistent nodeID to remove"
			self.defaultNodeIDToStake != nodeID:
				"Default commit node should be reappointed before removing"
		}
		...
		assert(
			delegatorInfo.tokensCommitted == 0.0,
			message: "Committed tokens should be moved out before removing"
		)
		self.approvedNodeIDList.remove(key: nodeID)
		...
		self.insertMigratedDelegatorUUID(
			nodeID: delegatorRef.nodeID,
			delegatorID: delegatorRef.id,
			uuid: uuid
		)
	...
	fun addReward(rewardedVault: @FlowToken.Vault){ 
		pre{ 
			FlowEpoch.currentEpochCounter == DelegatorManager.quoteEpochCounter:
				"Cannot addReward until protocol epoch syncs"
		}
	...
	resource DelegationStrategy{ 
		...
		fun transferCommittedTokens(fromNodeID: String, toNodeID: String, amount: UFix64){ 
			pre{ 
				FlowEpoch.currentEpochCounter == DelegatorManager.quoteEpochCounter:
					"Cannot transfer comitted tokens until protocol epoch syncs"
				fromNodeID != toNodeID:
					"Cannot transfer tokens among same nodes"
			}
			let fromDelegator =
				DelegatorManager.borrowApprovedDelegatorFromNode(fromNodeID)
				?? panic("cannot borrow from approved delegator of fromNode")
			let toDelegator =
				DelegatorManager.borrowApprovedDelegatorFromNode(toNodeID)
				?? panic("cannot borrow from approved delegator of toNode")
			let fromDelegatorInfo =
				FlowIDTableStaking.DelegatorInfo(nodeID: fromNodeID, delegatorID: fromDelegator.id)
			assert(
				fromDelegatorInfo.tokensCommitted >= amount,
				message: "try to transfer more than fromNode.committed"
			)
			...
			// update snapshot
			DelegatorManager.borrowCurrentQuoteEpochSnapshot().upsertDelegatorInfo(
				nodeID: fromDelegator.nodeID,
				delegatorID: fromDelegator.id
			)
			DelegatorManager.borrowCurrentQuoteEpochSnapshot().upsertDelegatorInfo(
				nodeID: toDelegator.nodeID,
				delegatorID: toDelegator.id
			)
		...
		fun processUnstakeRequest(requestUnstakeAmount: UFix64, delegatorUUID: UInt64){ 
			pre{ 
				FlowEpoch.currentEpochCounter == DelegatorManager.quoteEpochCounter:
					"Cannot process unstake request until protocol epoch syncs"
				DelegatorManager.requestedToUnstake > 0.0:
					"No pending unstake request to handle"
			}
			...
			let tokensStakedLeft =
				delegatorInfo.tokensStaked - delegatorInfo.tokensRequestedToUnstake
			...
			assert(
				DelegatorManager.requestedToUnstake >= unstakeAmount,
				message: "Handle unstake requests out of limit"
			)
			...
			DelegatorManager.requestedToUnstake = DelegatorManager.requestedToUnstake
				- unstakeAmount
			...
			emit StrategyProcessUnstakeRequest(
				amount: requestUnstakeAmount,
				nodeIDToUnstake: delegator.nodeID,
				delegatorIDToUnstake: delegator.id,
				leftoverAmount: DelegatorManager.requestedToUnstake
			)
		}
		...
		fun cleanDelegators(delegatorUUID: UInt64){ 
			pre{ 
				FlowEpoch.currentEpochCounter == DelegatorManager.quoteEpochCounter:
					"Cannot cleanup delegators until protocol epoch syncs"
			}
			let delegator = DelegatorManager.borrowManagedDelegator(uuid: delegatorUUID)!
			let nodeID = delegator.nodeID
			let delegatorID = delegator.id
			let delegatorInfo =
				FlowIDTableStaking.DelegatorInfo(nodeID: nodeID, delegatorID: delegatorID)
			assert(
				delegatorInfo.tokensCommitted + delegatorInfo.tokensStaked
				+ delegatorInfo.tokensUnstaking
				+ delegatorInfo.tokensRewarded
				+ delegatorInfo.tokensUnstaked
				+ delegatorInfo.tokensRequestedToUnstake
				== 0.0,
				message: "Please clean up before deleting"
			)
			var removed = false
		...
		fun compoundRewards(){ 
			pre{ 
				FlowEpoch.currentEpochCounter == DelegatorManager.quoteEpochCounter:
					"Cannot compound until protocol epoch syncs"
			}
	...
	resource Admin{ 
		...
		fun initApprovedNodeIDList(nodeIDs:{ String: UFix64}, defaultNodeIDToStake: String){ 
			pre{ 
				nodeIDs.containsKey(defaultNodeIDToStake):
					"Default staking node id must be in the list"
				DelegatorManager.approvedNodeIDList.length == 0:
		...
		fun setDefaultNodeIDToStake(nodeID: String){ 
			pre{ 
				DelegatorManager.approvedNodeIDList.containsKey(nodeID):
					"Default staking node id must be in approved node list"
			}
		...
		fun redelegate(nodeID: String, delegatorID: UInt32, amount: UFix64){ 
			pre{ 
				FlowEpoch.currentEpochCounter == DelegatorManager.quoteEpochCounter:
					"Cannot redelegate until protocol epoch syncs"
			}
			let uuid =
				DelegatorManager.getDelegatorUUIDByID(nodeID: nodeID, delegatorID: delegatorID)!
			...
			DelegatorManager.borrowCurrentQuoteEpochSnapshot().addRedelegatedTokensRequestToUnstake(
				amount: amount
			)
			...
			emit RedelegateRequested(
				nodeID: nodeID,
				delegatorID: delegatorID,
				redelegateCommittedAmount: redelegateCommittedAmount,
				redelegateRequestToUnstake: amount
			)
		}
	...
	init(){ 
		...
		self.protocolFeeVault <- FlowToken.createEmptyVault(vaultType: Type<@FlowToken.Vault>())
			as!
			@FlowToken.Vault
		self.totalUnstakedVault <- FlowToken.createEmptyVault(vaultType: Type<@FlowToken.Vault>())
			as!
			@FlowToken.Vault
		self.totalRewardedVault <- FlowToken.createEmptyVault(vaultType: Type<@FlowToken.Vault>())
			as!
			@FlowToken.Vault
		...
		self.account.storage.save(<-create Admin(), to: self.adminPath)
	}
}
