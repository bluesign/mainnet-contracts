...
contract LiquidStaking{ 
	...
	/// Paths
	access(all)
	let WithdrawVoucherCollectionPath: StoragePath
	
	access(all)
	let WithdrawVoucherCollectionPublicPath: PublicPath
	
	/// Events
	access(all)
	event Stake(flowAmountIn: UFix64, stFlowAmountOut: UFix64, currProtocolEpoch: UInt64)
	...
	access(all)
	event UnstakeQuickly(stFlowAmountIn: UFix64, flowAmountOut: UFix64, currProtocolEpoch: UInt64)
	
	access(all)
	event MigrateDelegator(uuid: UInt64, migratedFlowIn: UFix64, stFlowOut: UFix64)
	
	access(all)
	event BurnWithdrawVoucher(uuid: UInt64, amountFlowCashedout: UFix64, currProtocolEpoch: UInt64)
	...
	access(all)
	resource WithdrawVoucher{ 
		
		/// Flow token amount to be unlocked
		access(all)
		let lockedFlowAmount: UFix64
		
		/// Unlock epoch
		access(all)
		let unlockEpoch: UInt64
	...
	fun unstake(stFlowVault: @stFlowToken.Vault): @WithdrawVoucher{ 
		...
		let stakingEndView =
			(FlowEpoch.getEpochMetadata(FlowEpoch.currentEpochCounter)!).stakingEndView
		...
		let unstakeVoucher <-
			create WithdrawVoucher(lockedFlowAmount: flowAmountToUnstake, unlockEpoch: unlockEpoch)
		emit Unstake(
	...
	fun unstakeQuickly(stFlowVault: @stFlowToken.Vault): @FlowToken.Vault{ 
		...
		let feeVault <-
			flowVault.withdraw(amount: LiquidStakingConfig.quickUnstakeFee * flowAmountToUnstake)
	...
	fun migrate(delegator: @FlowIDTableStaking.NodeDelegator): @stFlowToken.Vault{ 
		...
		let delegatroInfo =
			FlowIDTableStaking.DelegatorInfo(nodeID: delegator.nodeID, delegatorID: delegator.id)
		assert(
			LiquidStakingConfig.stakingCap
			>= delegatroInfo.tokensStaked + DelegatorManager.getTotalValidStakingAmount(),
	...
	access(all)
	resource WithdrawVoucherCollection: WithdrawVoucherCollectionPublic{ 
