...
contract AniqueMarket{ 
	...
	access(all)
	event CollectibleListed(id: UInt64, price: UFix64, seller: Address?)
	
	// emitted when a Collectible is purchased from the market
	access(all)
	event CollectiblePurchased(id: UInt64, price: UFix64, seller: Address?)
	
	// emitted when a Collectible has been withdrawn from the sale
	access(all)
	event CollectibleSaleCancelled(id: UInt64, owner: Address?)
	
	// emitted when the cut rate of the sale has been changed by the owner
	access(all)
	event CutRateChanged(newRate: UFix64, seller: Address?)
	...
	resource SaleCollection: SalePublic{ 
		...
		access(self)
		var ownerCollection: Capability<&{Anique.Collection}>
		...
		init(ownerCollection: Capability<&{Anique.Collection}>, ownerCapability: Capability<&{AniqueCredit.Receiver}>, beneficiaryCapability: Capability<&{AniqueCredit.Receiver}>, cutRate: UFix64){ 
			pre{ 
		...
		fun purchase(tokenID: UInt64, buyTokens: @AniqueCredit.Vault, admin: &AniqueCredit.Admin): @{Anique.NFT}{ 
			...
			(			 
			 ...
			 self.beneficiaryCapability.borrow<&{AniqueCredit.Receiver}>()!).deposit(from: <-beneficiaryCut)
			(			 
			 // deposit the remaining tokens into the owners vault
			 self.ownerCapability.borrow<&{AniqueCredit.Receiver}>()!).deposit(from: <-buyTokens)
