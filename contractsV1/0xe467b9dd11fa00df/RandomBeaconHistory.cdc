...
contract RandomBeaconHistory{ 
	...
	resource Heartbeat{ 
		...
		///
		access(all)
		fun heartbeat(randomSourceHistory: [UInt8]){ 
	...
	struct RandomSource{ 
		...
		view init(blockHeight: UInt64, value: [UInt8]){ 
	...
	///
	access(all)
	struct RandomSourceHistoryPage{ 
		access(all)
		let page: UInt64
	...
	///
	access(all)
	fun sourceOfRandomness(atBlockHeight blockHeight: UInt64): RandomSource{ 
		pre{ 
			self.lowestHeight != nil:
				"History has not yet been initialized"
			blockHeight >= self.lowestHeight!:
				"Requested block height precedes recorded history"
			blockHeight < getCurrentBlock().height:
				"Source of randomness not yet recorded"
		}
	...
	///
	access(all)
	view fun getRandomSourceHistoryPage(_ page: UInt64, perPage: UInt64): RandomSourceHistoryPage{ 
		pre{ 
			self.lowestHeight != nil:
				"History has not yet been initialized"
		}
		...
		for i, block in self.randomSourceHistory.slice(from: Int(startIndex), upTo: Int(endIndex)){ 
			values.append(RandomSource(blockHeight: lowestHeight + startIndex + UInt64(i), value: self.randomSourceHistory[startIndex + UInt64(i)]))
		}
	...
	///
	access(all)
	view fun getLowestHeight(): UInt64{ 
