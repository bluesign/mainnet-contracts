...
contract NodeVersionBeacon{ 
	...
	access(all)
	event VersionBeacon(versionBoundaries: [VersionBoundary], sequence: UInt64)
	...
	access(all)
	event NodeVersionBoundaryFreezePeriodChanged(freezePeriod: UInt64)
	...
	resource Admin{ 
		...
		fun setVersionBoundary(versionBoundary: VersionBoundary){ 
			...
			while i > 1
			&& NodeVersionBeacon.versionBoundaryBlockList[i - 1] > versionBoundary.blockHeight{ 
				i = i - 1
			}
		...
		fun setVersionBoundaryFreezePeriod(newFreezePeriod: UInt64){ 
			...
			let nextBlockBoundary =
				NodeVersionBeacon.versionBoundaryBlockList[NodeVersionBeacon.firstUpcomingBoundary!]
			
			// Ensure that the we're not currently within the old or new freeze period
			// of the next block height boundary
			assert(
				currentBlockHeight + NodeVersionBeacon.versionBoundaryFreezePeriod
				< nextBlockBoundary
	...
	resource Heartbeat{ 
		...
		fun heartbeat(){ 
			...
			self.emitVersionBeaconEvent(
				versionBoundaries: NodeVersionBeacon.getCurrentVersionBoundaries()
			)
		}
		
		access(self)
		fun emitVersionBeaconEvent(versionBoundaries: [VersionBoundary]){ 
			emit VersionBeacon(
		...
		access(self)
		fun checkFirstUpcomingBoundary(){ 
			if NodeVersionBeacon.firstUpcomingBoundary == nil{ 
				return
			}
			let currentBlockHeight = getCurrentBlock().height
	...
	access(contract)
	fun searchForClosestHistoricalBlockBoundary(blockHeight: UInt64): UInt64{ 
