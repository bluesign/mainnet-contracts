...
contract FlowStorageFees{ 
	...
	access(all)
	event StorageMegaBytesPerReservedFLOWChanged(_ storageMegaBytesPerReservedFLOW: UFix64)
	
	// Emitted when the minimum amount of Flow tokens that an account needs to have reserved for storage capacity changes.
	access(all)
	event MinimumStorageReservationChanged(_ minimumStorageReservation: UFix64)
	...
	access(all)
	var storageMegaBytesPerReservedFLOW: UFix64
	...
	access(all)
	var minimumStorageReservation: UFix64
	...
	fun defaultTokenReservedBalance(_ accountAddress: Address): UFix64{ 
		...
		var reserved =
			self.storageCapacityToFlow(
				self.convertUInt64StorageBytesToUFix64Megabytes(acct.storage.used)
			)
