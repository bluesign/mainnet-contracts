...
contract OffersV2{ 
	...
	struct OfferDetails{ 
		...
		access(all)
		let offerParamsUFix64:{ String: UFix64}
		
		access(all)
		let offerParamsUInt64:{ String: UInt64}
	...
	resource Offer: OfferPublic{ 
		...
		fun accept(item: @{NonFungibleToken.INFT, ViewResolver.Resolver}, receiverCapability: Capability<&{FungibleToken.Receiver}>): Void{ 
			...
			let resolverResult = resolverCapability.checkOfferResolver(item: &item as &{NonFungibleToken.INFT, ViewResolver.Resolver}, offerParamsString: self.details.offerParamsString, offerParamsUInt64: self.details.offerParamsUInt64, offerParamsUFix64: self.details.offerParamsUFix64)
			if !resolverResult{ 
