...
access(all)
contract ClioDerma: NonFungibleToken, ViewResolver{ 
	...
	resource Manager: NiftoryNonFungibleToken.ManagerPublic, NiftoryNonFungibleToken.ManagerPrivate{ 
		...
		fun getNFTCollectionData(): MetadataViews.NFTCollectionData{ 
			return NiftoryNFTRegistry.buildNFTCollectionData(ClioDerma.REGISTRY_ADDRESS, ClioDerma.REGISTRY_BRAND, fun (): @{NonFungibleToken.Collection}{ 
					return <-ClioDerma.createEmptyCollection(nftType: Type<@ClioDerma.Collection>())
				})
		}
		...
		fun modifyContractMetadata(): &AnyStruct{ 
			...
			return (&ClioDerma.metadata as &AnyStruct?)!
		}
	...
	init(nftManagerProxy: &{NiftoryNonFungibleTokenProxy.Public, NiftoryNonFungibleTokenProxy.Private}){ 
		...
		self.account.storage.save<@MutableMetadataSetManager.Manager>(<-MutableMetadataSetManager._create(name: "ClioDerma", description: "The set manager for ClioDerma."), to: record.setManager.paths.storage)
