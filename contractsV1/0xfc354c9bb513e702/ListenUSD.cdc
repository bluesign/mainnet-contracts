...
access(all)
contract ListenUSD: FungibleToken{ 
	// TokensInitialized
	//
	...
	// Total supply of ListenUSDs in existence
	access(all)
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance{ 
		...
		fun deposit(from: @{FungibleToken.Vault}): Void{ 
			let vault <- from as! @ListenUSD.Vault
	...
	resource Minter{ 
		...
		//
		access(TMP_ENTITLEMENT_OWNER)
		fun mintTokens(amount: UFix64): @ListenUSD.Vault{ 
			pre{ 
				amount > 0.0:
			...
			ListenUSD.totalSupply = ListenUSD.totalSupply + amount
	...
	init(){ 
		// Set our named paths.
		self.VaultStoragePath = /storage/ListenUSDVault
		self.ReceiverPublicPath = /public/ListenUSDReceiver
		self.BalancePublicPath = /public/ListenUSDBalance
		self.AdminStoragePath = /storage/ListenUSDAdmin
		
		// Initialize contract state.
		self.totalSupply = 0.0
		
		// Create the Admin object and deposit it into the contract account.
		let admin = self.account.storage.borrow<&ListenUSD.Administrator>(from: self.AdminStoragePath)
		if admin == nil{ 
