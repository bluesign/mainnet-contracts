...
access(all)
contract ChessCombo: NonFungibleToken{ 
	...
	access(all)
	event CombinationCreated(id: UInt32, metadata:{ String: String})
	...
	access(all)
	event CompilationCreated(compilationId: UInt32, series: UInt32, name: String)
	
	// Emitted when a new Combination is added to a Compilation
	access(all)
	event CombinationAddedToCompilation(compilationId: UInt32, combinationId: UInt32)
	
	// Emitted when a Combination is retired from a Compilation and cannot be used to mint
	access(all)
	event CombinationRetiredFromCompilation(compilationId: UInt32, combinationId: UInt32, numCombos: UInt32)
	
	// Emitted when a Compilation is locked, meaning Combination cannot be added
	access(all)
	event CompilationLocked(compilationId: UInt32)
	
	// Emitted when a Combo is minted from a Compilation
	access(all)
	event ComboMinted(comboId: UInt64, combinationId: UInt32, compilationId: UInt32, serialNumber: UInt32)
	...
	access(all)
	event ComboDestroyed(id: UInt64)
	...
	access(all)
	let ChessComboAdminStoragePath: StoragePath
	...
	access(self)
	var combinationDatas:{ UInt32: CombinationData}
	
	// Variable size dictionary of CompilationData structs
	access(self)
	var compilationDatas:{ UInt32: CompilationData}
	
	// Variable size dictionary of Compilation resources
	access(self)
	var compilations: @{UInt32: Compilation}
	
	// The ID that is used to create Combination. 
	access(all)
	var nextCombinationId: UInt32
	...
	struct CompilationData{ 
		...
		init(name: String){ 
			pre{ 
				name.length > 0:
					"New Compilation name cannot be empty"
				ChessCombo.currentSeries > 0:
	...
	resource Compilation{ 
		...
		access(all)
		var numberMintedPerCombination:{ UInt32: UInt32}
		...
		fun addCombination(combinationId: UInt32){ 
			pre{ 
				ChessCombo.combinationDatas[combinationId] != nil:
					"Cannot add the Combination to Compilation: Combination doesn't exist."
				!self.locked:
					"Cannot add the Combination to the Compilation after the Compilation has been locked."
				self.numberMintedPerCombination[combinationId] == nil:
					"The Combination has already beed added to the Compilation."
			}
		...
		fun retireCombination(combinationId: UInt32){ 
			pre{ 
				self.retired[combinationId] != nil:
					"Cannot retire the Combination: Combination doesn't exist in this Compilation!"
			}
			if !self.retired[combinationId]!{ 
				self.retired[combinationId] = true
				emit CombinationRetiredFromCompilation(compilationId: self.id, combinationId: combinationId, numCombos: self.numberMintedPerCombination[combinationId]!)
			}
		}
		...
		fun mintCombo(combinationId: UInt32): @NFT{ 
			pre{ 
				self.retired[combinationId] != nil:
					"Cannot mint the combo: This Combination doesn't exist."
				!self.retired[combinationId]!:
					"Cannot mint the Combo from this Combination: This Combination has been retired."
			}
			...
			// Mint the new Combo
			let newCombo: @NFT <- create NFT(serialNumber: combinationMintsCount + 1 as UInt32, combinationId: combinationId, compilationId: self.id)
		...
		fun borrowCompilation(compilationId: UInt32): &Compilation{ 
			pre{ 
				ChessCombo.compilations[compilationId] != nil:
					"Cannot borrow Compilation: The Compilation doesn't exist"
			}
			return &ChessCombo.compilations[compilationId] as &ChessCombo.Compilation?
		}
	...
	//
	access(all)
	resource Collection: ComboCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic{ 
		...
		fun withdraw(withdrawID: UInt64): @{NonFungibleToken.NFT}{ 
			...
			let token <- self.ownedNFTs.remove(key: withdrawID) ?? panic("Cannot withdraw: Combo does not exist in the collection")
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&{ComboCollectionPublic}>(self.CollectionStoragePath)
		self.account.capabilities.publish(capability_1, at: self.CollectionPublicPath)
		
		// Put the Minter in storage
		self.account.storage.save<@Admin>(<-create Admin(), to: self.ChessComboAdminStoragePath)
