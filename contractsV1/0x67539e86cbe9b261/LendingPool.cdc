...
contract LendingPool{ 
	...
	access(all)
	event FlashloanOpen(isOpen: Bool)
	
	/// Event emitted when the flashloan is executed
	access(all)
	event Flashloan(executor: Address, executorType: Type, originator: Address, amount: UFix64)
	...
	fun flashloan(
		...
		let tokenIn <-
			(executorCap.borrow()!).executeAndRepay(loanedToken: <-tokenOut, params: params)
		assert(
		...
		assert(
			tokenIn.balance
			>= requestedAmount * (1.0 + UFix64(self.getFlashloanRateBps()) / 10000.0),
			message: LendingError.ErrorEncode(
	...
	resource PoolPublic: LendingInterfaces.PoolPublic{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun flashloan(executorCap: Capability<&{LendingInterfaces.FlashLoanExecutor}>, requestedAmount: UFix64, params:{ String: AnyStruct}){ 
	...
	///
	access(all)
	resource PoolAdmin: LendingInterfaces.PoolAdminPublic{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun initializePool(reserveFactor: UFix64, poolSeizeShare: UFix64, interestRateModelAddress: Address){ 
			pre{ 
	...
	init(){ 
		...
		self.account.capabilities.publish(capability_1, at: LendingConfig.PoolPublicPublicPath)
	}
}
