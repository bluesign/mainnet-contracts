...
access(all)
contract BabyShark: NonFungibleToken{ 
	/// Events
	///
	access(all)
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				...
				case Type<MetadataViews.HTTPFile>():
					return MetadataViews.HTTPFile(url: BabyShark.getSetMetadata(setId: self.setId).httpFile)
				case Type<MetadataViews.Serial>():
					return MetadataViews.Serial(self.id)
				case Type<MetadataViews.ExternalURL>():
					return MetadataViews.ExternalURL(BabyShark.getSetMetadata(setId: self.setId).externalUrl)
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: BabyShark.CollectionStoragePath, publicPath: BabyShark.CollectionPublicPath, publicCollection: Type<&BabyShark.Collection>(), publicLinkedType: Type<&BabyShark.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-BabyShark.createEmptyCollection(nftType: Type<@BabyShark.Collection>())
						})
				...
				case Type<MetadataViews.Royalties>():
					let royaltyReceiver: Capability<&{FungibleToken.Receiver}> = getAccount(BabyShark.royaltyAddress).capabilities.get<&{FungibleToken.Receiver}>(MetadataViews.getRoyaltyReceiverPublicPath())!
					return MetadataViews.Royalties([MetadataViews.Royalty(receiver: royaltyReceiver, cut: BabyShark.royaltyCut, description: "Flickplay Royalty")])
	...
	resource interface IAdminSafeShare{ 
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowSeries(): &BabyShark.Series
	...
	resource interface FlickplaySeriesCollectionPublic{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowFlickplaySeries(id: UInt64): &BabyShark.NFT?{ 
			post{ 
	...
	access(TMP_ENTITLEMENT_OWNER)
	fun getSetMetadata(setId: UInt32): BabyShark.NFTSetMetadata{ 
	...
	access(TMP_ENTITLEMENT_OWNER)
	fun getAllSets(): [BabyShark.NFTSetMetadata]{ 
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&BabyShark.Admin>(self.AdminStoragePath)
