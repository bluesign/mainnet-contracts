...
access(all)
contract Driverz: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				Driverz.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return Driverz.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				Driverz.maxSupply == nil || Driverz.totalSupply <= Driverz.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			Driverz.totalSupply = Driverz.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "Driverz World", symbol: "VROOM", description: "Our token is hosted outside of Toucans.Check out $VROOM token on https://driverz.world/.", externalURL: MetadataViews.ExternalURL("driverz.world/"), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("Driverz.World"), "discord": MetadataViews.ExternalURL("driverz")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: Driverz.VaultStoragePath, receiverPath: Driverz.ReceiverPublicPath, metadataPath: Driverz.VaultPublicPath, receiverLinkedType: /private/DriverzVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-Driverz.createEmptyVault(vaultType: Type<@Driverz.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/DriverzVault
		self.ReceiverPublicPath = /public/DriverzReceiver
		self.VaultPublicPath = /public/DriverzMetadata
		self.MinterStoragePath = /storage/DriverzMinter
		self.AdministratorStoragePath = /storage/DriverzAdmin
