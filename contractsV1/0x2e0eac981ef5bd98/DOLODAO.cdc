...
access(all)
contract DOLODAO: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				DOLODAO.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return DOLODAO.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				DOLODAO.maxSupply == nil || DOLODAO.totalSupply <= DOLODAO.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			DOLODAO.totalSupply = DOLODAO.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "DOLO DAO", symbol: "DOLO", description: "Maybe something. Probably nothing.", externalURL: MetadataViews.ExternalURL("https://twitter.com/dolodood"), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("dolodood"), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: DOLODAO.VaultStoragePath, receiverPath: DOLODAO.ReceiverPublicPath, metadataPath: DOLODAO.VaultPublicPath, receiverLinkedType: /private/DOLODAOVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-DOLODAO.createEmptyVault(vaultType: Type<@DOLODAO.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/DOLODAOVault
		self.ReceiverPublicPath = /public/DOLODAOReceiver
		self.VaultPublicPath = /public/DOLODAOMetadata
		self.MinterStoragePath = /storage/DOLODAOMinter
		self.AdministratorStoragePath = /storage/DOLODAOAdmin
