...
access(all)
contract VideoGameNation: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				VideoGameNation.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return VideoGameNation.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				VideoGameNation.maxSupply == nil || VideoGameNation.totalSupply <= VideoGameNation.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			VideoGameNation.totalSupply = VideoGameNation.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "VideoGameNation", symbol: "VGN", description: "DAO for Video Game Nation, a sub-community of NBA Top Shot.", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("t5d6pQ9g")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: VideoGameNation.VaultStoragePath, receiverPath: VideoGameNation.ReceiverPublicPath, metadataPath: VideoGameNation.VaultPublicPath, receiverLinkedType: /private/VideoGameNationVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-VideoGameNation.createEmptyVault(vaultType: Type<@VideoGameNation.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/VideoGameNationVault
		self.ReceiverPublicPath = /public/VideoGameNationReceiver
		self.VaultPublicPath = /public/VideoGameNationMetadata
		self.MinterStoragePath = /storage/VideoGameNationMinter
		self.AdministratorStoragePath = /storage/VideoGameNationAdmin
