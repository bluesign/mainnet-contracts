...
contract Bl0x: NonFungibleToken{ 
	...
	access(all)
	resource interface Bl0xCollectionPublic{ 
		access(all)
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowBl0x(id: UInt64): &Bl0x.NFT?{ 
			post{ 
	...
	access(all)
	resource Collection: Bl0xCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		access(all)
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&Bl0x.Collection>(self.CollectionStoragePath)
