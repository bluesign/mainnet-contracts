...
contract OpenEditionV2{ 
	...
	resource OpenEditionItem{ 
		...
		fun sendCommissionPayments(buyerTokens: @FUSD.Vault, tokenID: UInt64){ 
			...
			let editionRef =
				OpenEditionV2.account.capabilities.get<&{Edition.EditionCollectionPublic}>(
	...
	resource interface OpenEditionCollectionPublic{ 
		access(TMP_ENTITLEMENT_OWNER)
		fun getOpenEditionStatuses():{ UInt64: OpenEditionV2.OpenEditionStatus}?
	...
	resource OpenEditionCollection: OpenEditionCollectionPublic{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun createOpenEdition(price: UFix64, startTime: UFix64, saleLength: UFix64, editionNumber: UInt64, metadata: Collectible.Metadata, platformVaultCap: Capability<&FUSD.Vault>){ 
			pre{ 
				saleLength > 0.00:
			...
			let editionRef = OpenEditionV2.account.capabilities.get<&{Edition.EditionCollectionPublic}>(Edition.CollectionPublicPath).borrow()!
		...
		fun getOpenEditionStatuses():{ UInt64: OpenEditionStatus}?{ 
			...
			for id in self.openEditionsItems.keys{ 
				let itemRef = &self.openEditionsItems[id] as &OpenEditionV2.OpenEditionItem?
	...
	init(){ 
		...
		self.account.storage.save(<-openEdition, to: OpenEditionV2.CollectionStoragePath)
		var capability_1 =
			self.account.capabilities.storage.issue<&{OpenEditionV2.OpenEditionCollectionPublic}>(
				OpenEditionV2.CollectionStoragePath
			)
		self.account.capabilities.publish(capability_1, at: OpenEditionV2.CollectionPublicPath)
	}
}
