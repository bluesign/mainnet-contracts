...
access(all)
contract Meowow: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				Meowow.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return Meowow.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				Meowow.maxSupply == nil || Meowow.totalSupply <= Meowow.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			Meowow.totalSupply = Meowow.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafybeidmyttrxfeufx7nnix4szbnjpldv3viqw5jk4hlhielcks7m6rfay"), mediaType: "image")
				let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafybeigsudeidkvy7kx4h36liea7cdu3oav3silh3kbn3uddohsbpxreca"), mediaType: "image")
				let medias = MetadataViews.Medias([media, bannerMedia])
				return FungibleTokenMetadataViews.FTDisplay(name: "Meowow", symbol: "MEOW", description: "Welcome to the world of Meowow! Meowow is a unique and playful meme coin on the Flow blockchain", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: Meowow.VaultStoragePath, receiverPath: Meowow.ReceiverPublicPath, metadataPath: Meowow.VaultPublicPath, receiverLinkedType: /private/MeowowVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-Meowow.createEmptyVault(vaultType: Type<@Meowow.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/MeowowVault
		self.ReceiverPublicPath = /public/MeowowReceiver
		self.VaultPublicPath = /public/MeowowMetadata
		self.MinterStoragePath = /storage/MeowowMinter
		self.AdministratorStoragePath = /storage/MeowowAdmin
