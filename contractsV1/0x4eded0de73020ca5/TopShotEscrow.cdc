...
contract TopShotEscrow{ 
	...
	resource EscrowItem: EscrowItemPublic{ 
		...
		init(_NFTCollection: @TopShot.Collection, _duration: UFix64, _receiverCap: Capability<&{NonFungibleToken.Receiver}>){ 
			...
			self.redeemed = false
			let adminTopShotReceiverRef = (TopShotEscrow.account.capabilities.get<&{NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, TopShot.MomentCollectionPublic}>(/public/MomentCollection)!).borrow() ?? panic("Cannot borrow collection")
		...
		fun redeem(NFTIds: [UInt64]){ 
			...
			let providerTopShotProviderRef: &TopShot.Collection? = TopShotEscrow.account.storage.borrow<&TopShot.Collection>(from: /storage/MomentCollection) ?? panic("Cannot borrow collection")
	...
	resource EscrowCollection: EscrowCollectionPublic{ 
		...
		access(all)
		fun withdraw(escrowID: UInt64): @TopShotEscrow.EscrowItem{ 
	...
	fun createEscrow(
		...
		let escrowCollectionRef =
			self.account.storage.borrow<&TopShotEscrow.EscrowCollection>(
	...
	fun batchRedeem(_ escrowIDs: [UInt64]){ 
		for escrowID in escrowIDs{ 
			let escrowCollectionRef = self.account.storage.borrow<&TopShotEscrow.EscrowCollection>(from: self.escrowStoragePath) ?? panic("Couldn't borrow escrow collection")
			let escrowRef = escrowCollectionRef.borrowEscrow(escrowID: escrowID)!
	...
	init(){ 
		...
		var capability_1 =
			self.account.capabilities.storage.issue<&TopShotEscrow.EscrowCollection>(
