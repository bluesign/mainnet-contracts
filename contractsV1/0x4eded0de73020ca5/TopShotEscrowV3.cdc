...
contract TopShotEscrowV3{ 
	...
	resource EscrowItem: EscrowItemPublic{ 
		...
		access(self)
		let receiverCap: Capability<&{TopShot.MomentCollectionPublic}>
		...
		init(_NFTCollection: @TopShot.Collection, _duration: UFix64, _receiverCap: Capability<&{TopShot.MomentCollectionPublic}>){ 
			...
			self.redeemed = false
			self.lock = false
			let adminTopShotReceiverRef = (TopShotEscrowV3.account.capabilities.get<&{NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, TopShot.MomentCollectionPublic}>(/public/MomentCollection)!).borrow() ?? panic("Cannot borrow collection")
		...
		fun redeem(NFTIds: [UInt64]){ 
			...
			let providerTopShotProviderRef: &TopShot.Collection? = TopShotEscrowV3.account.storage.borrow<&TopShot.Collection>(from: /storage/MomentCollection) ?? panic("Cannot borrow collection")
	...
	resource EscrowCollection: EscrowCollectionPublic{ 
		...
		access(all)
		fun withdraw(escrowID: UInt64): @TopShotEscrowV3.EscrowItem{ 
		...
		access(all)
		fun createEscrow(NFTCollection: @TopShot.Collection, duration: UFix64, receiverCap: Capability<&{TopShot.MomentCollectionPublic}>){ 
	...
	fun createEscrow(
		...
		let escrowCollectionRef =
			self.account.storage.borrow<&TopShotEscrowV3.EscrowCollection>(
	...
	fun batchRedeem(_ escrowIDs: [UInt64]){ 
		for escrowID in escrowIDs{ 
			let escrowCollectionRef = self.account.storage.borrow<&TopShotEscrowV3.EscrowCollection>(from: self.escrowStoragePath) ?? panic("Couldn't borrow escrow collection")
			let escrowRef = escrowCollectionRef.borrowEscrow(escrowID: escrowID)!
	...
	init(){ 
		...
		var capability_1 =
			self.account.capabilities.storage.issue<&TopShotEscrowV3.EscrowCollection>(
