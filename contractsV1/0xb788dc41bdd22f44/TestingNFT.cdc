...
access(all)
contract TestingNFT: NonFungibleToken{ 
	access(all)
	...
	access(all)
	resource interface TestingNFTCollectionPublic{ 
		access(all)
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowTestingNFT(id: UInt64): &TestingNFT.NFT?{ 
			post{ 
	...
	access(all)
	resource Collection: TestingNFTCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		access(all)
	...
	resource NFTMinter{ 
		...
		fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, metadata:{ String: String}): &{NonFungibleToken.NFT}{ 
			...
			// create a new NFT
			var newNFT <- create NFT(id: TestingNFT.totalSupply, creator: creator, metadata: metadata)
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&TestingNFT.Collection>(self.CollectionStoragePath)
