...
access(all)
contract SORAAI: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				SORAAI.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return SORAAI.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				SORAAI.maxSupply == nil || SORAAI.totalSupply <= SORAAI.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			SORAAI.totalSupply = SORAAI.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "SORA AI", symbol: "SORA", description: "SORA Ai On Flow ", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: SORAAI.VaultStoragePath, receiverPath: SORAAI.ReceiverPublicPath, metadataPath: SORAAI.VaultPublicPath, receiverLinkedType: /private/SORAAIVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-SORAAI.createEmptyVault(vaultType: Type<@SORAAI.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/SORAAIVault
		self.ReceiverPublicPath = /public/SORAAIReceiver
		self.VaultPublicPath = /public/SORAAIMetadata
		self.MinterStoragePath = /storage/SORAAIMinter
		self.AdministratorStoragePath = /storage/SORAAIAdmin
