...
access(all)
contract SORAOnFlow: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				SORAOnFlow.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return SORAOnFlow.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				SORAOnFlow.maxSupply == nil || SORAOnFlow.totalSupply <= SORAOnFlow.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			SORAOnFlow.totalSupply = SORAOnFlow.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafybeia52yey437r2bigtsc7jv4rx52arv6ckf3fw5ejusb6lfwzbff72e"), mediaType: "image")
				let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreiaefdlsnqdqn5hf4n4ua5xtrbbxdageu7w2ht5jvdruhwuwtqlxtm"), mediaType: "image")
				let medias = MetadataViews.Medias([media, bannerMedia])
				return FungibleTokenMetadataViews.FTDisplay(name: "SORA On Flow", symbol: "SORA", description: "SORA On Flow Blockchain", externalURL: MetadataViews.ExternalURL("sora.ai "), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("sora"), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: SORAOnFlow.VaultStoragePath, receiverPath: SORAOnFlow.ReceiverPublicPath, metadataPath: SORAOnFlow.VaultPublicPath, receiverLinkedType: /private/SORAOnFlowVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-SORAOnFlow.createEmptyVault(vaultType: Type<@SORAOnFlow.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/SORAOnFlowVault
		self.ReceiverPublicPath = /public/SORAOnFlowReceiver
		self.VaultPublicPath = /public/SORAOnFlowMetadata
		self.MinterStoragePath = /storage/SORAOnFlowMinter
		self.AdministratorStoragePath = /storage/SORAOnFlowAdmin
