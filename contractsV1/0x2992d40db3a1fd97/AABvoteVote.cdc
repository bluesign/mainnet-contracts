...
contract AABvoteVote{ 
	access(all)
	let votes:{ Address: [Vote]}
	
	access(all)
	let star:{ Address: UInt}
	
	access(all)
	var allowVote: Bool
	
	access(all)
	event NewVote(voter: Address, candidateId: String, nfts: [UInt64], star: UInt64)
	
	access(all)
	let AdminVoteStoragePath: StoragePath
		...
		fun vote(voter: Address, candidateId: String, nfts: [UInt64], type: UInt8){ 
			pre{ 
				voter != nil:
					"Invalid voter"
				candidateId != nil:
					"Cannot vote for a category that doesn't exist"
				nfts.length > 0 && nfts.length <= 3:
					"Nfts length must be greater than 0 and less than 4"
			}
			if !AABvoteVote.allowVote{ 
				panic("Now unable to vote. Please try again!")
			}
			let rarityCond:{ UInt8: Int} ={
				AABvoteNFT.Rarity.common.rawValue: 0, AABvoteNFT.Rarity.iconic.rawValue: 0}
			...
			if type == AABvoteNFT.Rarity.common.rawValue
			&& rarityCond[AABvoteNFT.Rarity.common.rawValue] != nfts.length
			|| type == AABvoteNFT.Rarity.iconic.rawValue
			&& rarityCond[AABvoteNFT.Rarity.iconic.rawValue] != nfts.length
			|| rarityCond[AABvoteNFT.Rarity.common.rawValue] != nfts.length
			&& rarityCond[AABvoteNFT.Rarity.iconic.rawValue] != nfts.length{ 
