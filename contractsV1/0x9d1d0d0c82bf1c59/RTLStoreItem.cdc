...
access(all)
contract RTLStoreItem: NonFungibleToken{ 
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: RTLStoreItem.CollectionStoragePath, publicPath: RTLStoreItem.CollectionPublicPath, publicCollection: Type<&RTLStoreItem.Collection>(), publicLinkedType: Type<&RTLStoreItem.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-RTLStoreItem.createEmptyCollection(nftType: Type<@RTLStoreItem.Collection>())
						})
	...
	//
	access(all)
	resource Collection: RTLStoreItemCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		...
		view fun borrowViewResolver(id: UInt64): &{ViewResolver.Resolver}?{ 
			...
			return RTLStoreItemNFT as &{ViewResolver.Resolver}
		}
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&{RTLStoreItemCollectionPublic}>(self.CollectionStoragePath)
