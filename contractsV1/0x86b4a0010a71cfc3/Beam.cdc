...
access(all)
contract Beam: NonFungibleToken{ 
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				...
				case Type<MetadataViews.Royalties>():
					let receiver = Beam.account.capabilities.get<&{FungibleToken.Receiver}>(/public/flowTokenReceiver)
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: Beam.CollectionStoragePath, publicPath: Beam.CollectionPublicPath, publicCollection: Type<&Beam.Collection>(), publicLinkedType: Type<&Beam.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-Beam.createEmptyCollection(nftType: Type<@Beam.Collection>())
						})
				case Type<MetadataViews.NFTCollectionDisplay>():
					return MetadataViews.NFTCollectionDisplay(name: "Frightclub", description: "Frightclub collection", externalURL: MetadataViews.ExternalURL("https://frightclub.io"), squareImage: MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://frightclub.niftory.com/fc_black_logo_round.svg"), mediaType: "image/svg+xml"), bannerImage: MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://frightclub.niftory.com/fc_black_logo_round.svg"), mediaType: "image/svg+xml"), socials:{} )
				case Type<MetadataViews.ExternalURL>():
					return MetadataViews.ExternalURL("https://frightclub.io")
			}
			return nil
		}
	...
	access(all)
	resource interface BeamCollectionPublic{ 
		access(all)
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowCollectible(id: UInt64): &Beam.NFT?{ 
	...
	access(all)
	resource Collection: BeamCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
	...
	access(TMP_ENTITLEMENT_OWNER)
	fun getAllCollectibleItems(): [Beam.CollectibleItem]{ 
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&{BeamCollectionPublic}>(self.CollectionStoragePath)
