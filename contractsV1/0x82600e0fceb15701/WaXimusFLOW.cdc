...
access(all)
contract WaXimusFLOW: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				WaXimusFLOW.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return WaXimusFLOW.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				WaXimusFLOW.maxSupply == nil || WaXimusFLOW.totalSupply <= WaXimusFLOW.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			WaXimusFLOW.totalSupply = WaXimusFLOW.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreics7ybtjxdv23khf64dypq5qld6gszl26dc5ndkc5exowutui3rju"), mediaType: "image")
				let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreics7ybtjxdv23khf64dypq5qld6gszl26dc5ndkc5exowutui3rju"), mediaType: "image")
				let medias = MetadataViews.Medias([media, bannerMedia])
				return FungibleTokenMetadataViews.FTDisplay(name: "WaXimus FLOW", symbol: "WAX", description: "Living every week like its #SharkWeek...NFA", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("@WaximusW"), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: WaXimusFLOW.VaultStoragePath, receiverPath: WaXimusFLOW.ReceiverPublicPath, metadataPath: WaXimusFLOW.VaultPublicPath, receiverLinkedType: /private/WaXimusFLOWVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-WaXimusFLOW.createEmptyVault(vaultType: Type<@WaXimusFLOW.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/WaXimusFLOWVault
		self.ReceiverPublicPath = /public/WaXimusFLOWReceiver
		self.VaultPublicPath = /public/WaXimusFLOWMetadata
		self.MinterStoragePath = /storage/WaXimusFLOWMinter
		self.AdministratorStoragePath = /storage/WaXimusFLOWAdmin
