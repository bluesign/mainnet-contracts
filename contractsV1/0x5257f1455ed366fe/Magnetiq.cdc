...
access(all)
contract Magnetiq: NonFungibleToken{ 
	// -----------------------------------------------------------------------
	...
	access(all)
	event MagnetCreated(id: String, metadata:{ String: String})
	
	// Events for Brand-Related actions
	//
	// Emitted when a new Brand is created
	access(all)
	event BrandCreated(brandID: String)
	
	// Emitted when a new Magnet is added to a Brand
	access(all)
	event MagnetAddedToBrand(brandID: String, magnetID: String)
	
	// Emitted when a Magnet is retired from a Brand and cannot be used to mint
	access(all)
	event MagnetRetiredFromBrand(brandID: String, magnetID: String, numTokens: UInt32)
	
	// Emitted when a Brand is locked, meaning Magnets cannot be added
	access(all)
	event BrandLocked(brandID: String)
	
	// Emitted when a Tokens is minted from a Brand
	access(all)
	event TokensMinted(tokenID: UInt64, tokenType: String, magnetiqID: String, brandID: String, serialNumber: UInt32)
	...
	access(all)
	event TokensDestroyed(id: UInt64)
	
	// Emitted when a Memento is created
	access(all)
	event MementoCreated(mementoID: String, magnetID: String)
	
	// Emitted when a token mark claimed
	access(all)
	event TokenClaimed(tokenID: UInt64)
	
	// Emitted when magnet metadata updated
	access(all)
	event MagnetMetadataUpdated(id: String, metadata:{ String: String})
	
	// Emitted when memento metadata updated
	access(all)
	event MementoMetadataUpdated(id: String, metadata:{ String: String})
	...
	access(all)
	var royalityReceiver: Address
	
	// variable for royality percentage
	access(all)
	var royalityPercentage: UFix64
	...
	access(self)
	var magnetData:{ String: Magnet}
	
	// Variable size dictionary of Memento structs
	access(self)
	var mementoData:{ String: Memento}
	
	access(self)
	var magnetiqTokenExtraInfo:{ UInt64:{ String: AnyStruct}}
	
	// Variable size dictionary of BrandData structs
	access(self)
	var brandData:{ String: BrandData}
	...
	access(self)
	var allowedSellersForNonSellableNFT: [Address?]
	...
	struct Magnet{ 
		...
		access(contract)
		var numberMintedPerMemento:{ String: UInt32}
		
		init(metadata:{ String: String}, magnetID: String){ 
			pre{ 
				metadata.length != 0:
					"New Magnet metadata cannot be empty"
			}
	...
	struct BrandData{ 
		...
		init(name: String, brandID: String){ 
			pre{ 
				name.length > 0:
					"New Brand name cannot be empty"
			}
	...
	resource Brand{ 
		...
		access(contract)
		var numberMintedPerMagnet:{ String: UInt32}
		...
		fun addMagnet(magnetID: String){ 
			pre{ 
				Magnetiq.magnetData[magnetID] != nil:
					"Cannot add the Magnet to Brand: Magnet doesn't exist."
				!self.locked:
					"Cannot add the magnet to the Brand after the brand has been locked."
				self.numberMintedPerMagnet[magnetID] == nil:
					"The magnet has already beed added to the brand."
			}
		...
		fun retireMagnet(magnetID: String){ 
			pre{ 
				self.retired[magnetID] != nil:
					"Cannot retire the Magnet: Magnet doesn't exist in this brand!"
			}
		...
		fun mintToken(magnetiqID: String, tokenType: String): @NFT{ 
			...
			// Mint the new token
			let newTokens: @NFT <- create NFT(serialNumber: numInMagnetMemento! + UInt32(1), magnetiqID: magnetiqID, brandID: self.brandID, tokenType: tokenType)
	...
	// there will be 
	access(all)
	struct MagnetiqTokenMetadataView{ 
		access(all)
		let name: String?
		...
		access(all)
		let numTokensInEdition: UInt32?
		
		access(all)
		let is_claimed: AnyStruct?
		
		access(all)
		let is_sellable: AnyStruct?
		
		access(all)
		let is_claimable: AnyStruct?
		
		access(all)
		let is_visible: AnyStruct?
		
		init(name: String?, tokenType: String?, brandName: String?, serialNumber: UInt32, magnetID: String, mementoID: String?, brandID: String, numTokensInEdition: UInt32?, is_claimed: AnyStruct?, is_sellable: AnyStruct?, is_claimable: AnyStruct?, is_visible: AnyStruct?){ 
			self.name = name
			self.tokenType = tokenType
	...
	// there will be 
	access(all)
	struct MagnetTokenMetadataView{ 
		access(all)
		let name: String?
		...
		init(name: String?, brandName: String?, serialNumber: UInt32, magnetID: String, brandID: String, numTokensInEdition: UInt32?, is_sellable: AnyStruct?){ 
			self.name = name
			self.brandName = brandName
			self.serialNumber = serialNumber
			self.magnetID = magnetID
	...
	// there will be 
	access(all)
	struct MementoTokenMetadataView{ 
		access(all)
		let name: String?
		...
		init(name: String?, brandName: String?, serialNumber: UInt32, magnetID: String, mementoID: String, brandID: String, numTokensInEdition: UInt32?, is_claimed: AnyStruct?, is_sellable: AnyStruct?, is_claimable: AnyStruct?, is_visible: AnyStruct?){ 
			self.name = name
			self.brandName = brandName
			self.serialNumber = serialNumber
			self.magnetID = magnetID
	...
	resource NFT: NonFungibleToken.NFT, ViewResolver.Resolver{ 
		...
		fun name(): String{ 
			...
			return fullName.concat(" (").concat(tokType).concat(")")
		}
		...
		view fun getViews(): [Type]{ 
			return [Type<MetadataViews.Display>(), Type<MagnetiqTokenMetadataView>(), // keep the common view, and dont add individual magnet/memento view																					  
																					  Type<MagnetTokenMetadataView>(), Type<MementoTokenMetadataView>(), Type<MetadataViews.Royalties>(), Type<MetadataViews.Editions>(), Type<MetadataViews.ExternalURL>(), Type<MetadataViews.NFTCollectionData>(), Type<MetadataViews.NFTCollectionDisplay>(), Type<MetadataViews.Serial>(), Type<MetadataViews.Traits>(), Type<MetadataViews.Medias>()]
		}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				...
				case Type<MagnetTokenMetadataView>():
					return MagnetTokenMetadataView(name: Magnetiq.getMagnetMetaDataByField(magnetiqID: self.data.magnetiqID, field: "name"), brandName: Magnetiq.getBrandName(brandID: self.data.brandID), serialNumber: self.data.serialNumber, magnetID: self.data.magnetiqID, brandID: self.data.brandID, numTokensInEdition: Magnetiq.getNumTokensInEdition(brandID: self.data.brandID, magnetiqID: self.data.magnetiqID, tokenType: self.data.tokenType), is_sellable: Magnetiq.getMagnetMetaDataByField(magnetiqID: self.data.magnetiqID, field: "is_sellable"))
				
				// metadata view for memento tokens
				case Type<MementoTokenMetadataView>():
					return MementoTokenMetadataView(name: Magnetiq.getMementoMetaDataByField(magnetiqID: self.data.magnetiqID, field: "name"), brandName: Magnetiq.getBrandName(brandID: self.data.brandID), serialNumber: self.data.serialNumber, magnetID: Magnetiq.mementoData[self.data.magnetiqID]?.magnetID!, mementoID: self.data.magnetiqID, brandID: self.data.brandID, numTokensInEdition: Magnetiq.getNumTokensInEdition(brandID: self.data.brandID, magnetiqID: self.data.magnetiqID, tokenType: self.data.tokenType), is_claimed: (Magnetiq.magnetiqTokenExtraInfo[self.id]!)["claimed"] ?? "", is_sellable: Magnetiq.getMementoMetaDataByField(magnetiqID: self.data.magnetiqID, field: "is_sellable"), is_claimable: Magnetiq.getMementoMetaDataByField(magnetiqID: self.data.magnetiqID, field: "is_claimable"), is_visible: Magnetiq.getMementoMetaDataByField(magnetiqID: self.data.magnetiqID, field: "is_visible"))
				...
				case Type<MagnetiqTokenMetadataView>():
					...
					return MagnetiqTokenMetadataView(name: Magnetiq.getMementoMetaDataByField(magnetiqID: self.data.magnetiqID, field: "name"), tokenType: self.data.tokenType, brandName: Magnetiq.getBrandName(brandID: self.data.brandID), serialNumber: self.data.serialNumber, magnetID: magnetID, mementoID: mementoID, brandID: self.data.brandID, numTokensInEdition: Magnetiq.getNumTokensInEdition(brandID: self.data.brandID, magnetiqID: self.data.magnetiqID, tokenType: self.data.tokenType), is_claimed: is_claimed, is_sellable: is_sellable, is_claimable: is_claimable, is_visible: is_visible)
				...
				case Type<MetadataViews.Royalties>():
					let royaltyReceiver: Capability<&{FungibleToken.Receiver}> = getAccount(Magnetiq.RoyaltyAddress()).capabilities.get<&{FungibleToken.Receiver}>(MetadataViews.getRoyaltyReceiverPublicPath())!
					return MetadataViews.Royalties([MetadataViews.Royalty(receiver: royaltyReceiver, cut: Magnetiq.royalityPercentage, description: "Magnetiq marketplace royalty")])
				case Type<MetadataViews.ExternalURL>():
					return MetadataViews.ExternalURL(self.getTokensURL())
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: /storage/MagnetiqTokensCollection, publicPath: /public/MagnetiqTokensCollection, publicCollection: Type<&Magnetiq.Collection>(), publicLinkedType: Type<&Magnetiq.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-Magnetiq.createEmptyCollection(nftType: Type<@Magnetiq.Collection>())
						})
				case Type<MetadataViews.NFTCollectionDisplay>():
					let bannerImage = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://magnetiq-static.s3.amazonaws.com/media/public/MAGNETIQ_banner.png"), mediaType: "image/png")
					let squareImage = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://magnetiq-static.s3.amazonaws.com/media/public/MAGNETIQ_Square_Logo.png"), mediaType: "image/png")
					return MetadataViews.NFTCollectionDisplay(name: "MAGNETIQ", description: "MAGNETIQ is making managing brand community engagement easy and efficient with a plug and play, blockchain powered platform.  MAGNETIQ NFTs represent your membership in brand communities.", externalURL: MetadataViews.ExternalURL("https://www.magnetiq.xyz/"), squareImage: squareImage, bannerImage: bannerImage, socials:{ "twitter": MetadataViews.ExternalURL("https://twitter.com/magnetiq_xyz"), "discord": MetadataViews.ExternalURL("https://discord.com/invite/Magnetiq"), "instagram": MetadataViews.ExternalURL("https://www.instagram.com/magnetiq_xyz")})
				case Type<MetadataViews.Traits>():
					// sports radar team id
					let excludedNames: [String] = ["TeamAtTokensID"]
					// non magnet specific traits
					let traitDictionary:{ String: AnyStruct} ={ "BrandName": Magnetiq.getBrandName(brandID: self.data.brandID), "SerialNumber": self.data.serialNumber}
	...
	resource Admin{ 
		...
		fun createMemento(mementoID: String, magnetID: String, metadata:{ String: String}): String{ 
			pre{ 
				Magnetiq.magnetData[magnetID] != nil:
					"Cannot create Memento: Magnet Doesn't exist"
			}
		...
		fun borrowBrand(brandID: String): &Brand{ 
			pre{ 
				Magnetiq.brands[brandID] != nil:
					"Cannot borrow Brand: The Brand doesn't exist"
			}
	...
	resource Collection: TokenCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		...
		fun withdraw(withdrawID: UInt64): @{NonFungibleToken.NFT}{ 
			...
			let token <- self.ownedNFTs.remove(key: withdrawID) ?? panic("Cannot withdraw: Token does not exist in the collection")
	...
	fun getNumTokensInEdition(brandID: String, magnetiqID: String, tokenType: String): UInt32?{ 
		if tokenType == "magnet"{ 
		  ...
		  else if let magnet = Magnetiq.magnetData[magnetiqID]{ 
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&{TokenCollectionPublic}>(/storage/MagnetiqTokensCollection)
		self.account.capabilities.publish(capability_1, at: /public/MagnetiqTokensCollection)
