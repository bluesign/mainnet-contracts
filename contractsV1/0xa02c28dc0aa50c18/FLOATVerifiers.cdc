...
contract FLOATVerifiers{ 
	...
	access(all)
	struct Timelock: MusicPeaksMembershipToken.IVerifier{ 
	...
	access(all)
	struct Secret: MusicPeaksMembershipToken.IVerifier{ 
	...
	// delete their FLOATs.
	access(all)
	struct Limited: MusicPeaksMembershipToken.IVerifier{ 
		access(all)
		var capacity: UInt64
		
		access(TMP_ENTITLEMENT_OWNER)
		fun verify(_ params:{ String: AnyStruct}){ 
			let _event = params["event"]! as! &MusicPeaksMembershipToken.FLOATEvent
	...
	// from the list.
	access(all)
	struct MultipleSecret: MusicPeaksMembershipToken.IVerifier{ 
		access(self)
	...
	access(all)
	struct SecretV2: MusicPeaksMembershipToken.IVerifier{ 
		access(all)
