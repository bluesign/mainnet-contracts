...
access(all)
contract LBJDAO: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				LBJDAO.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return LBJDAO.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				LBJDAO.maxSupply == nil || LBJDAO.totalSupply <= LBJDAO.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			LBJDAO.totalSupply = LBJDAO.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafybeiap65dolssr775a366jk4kl63exbxcpeehwo23jccacxndscxccne"), mediaType: "image")
				let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreiad4drc6vsjm3zy34ph6ymgnb5yiwp7gmyojosndjwppzkjfyjssa"), mediaType: "image")
				let medias = MetadataViews.Medias([media, bannerMedia])
				return FungibleTokenMetadataViews.FTDisplay(name: "LBJ DAO", symbol: "LBJ", description: "An unofficial DAO for everyone who saw LBJ's gorgeous play.", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: LBJDAO.VaultStoragePath, receiverPath: LBJDAO.ReceiverPublicPath, metadataPath: LBJDAO.VaultPublicPath, receiverLinkedType: /private/LBJDAOVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-LBJDAO.createEmptyVault(vaultType: Type<@LBJDAO.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/LBJDAOVault
		self.ReceiverPublicPath = /public/LBJDAOReceiver
		self.VaultPublicPath = /public/LBJDAOMetadata
		self.MinterStoragePath = /storage/LBJDAOMinter
		self.AdministratorStoragePath = /storage/LBJDAOAdmin
