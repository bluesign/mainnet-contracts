...
access(all)
contract NftEggsStore{ 
	access(all)
	event NftEggsStoreInitialized()
	...
	struct ListingDetails{ 
		...
		access(all)
		let saleCuts:{ String: NftEggsNFT.Royalty}
		...
		access(contract)
		fun setBid(endTime: UFix64, bidPrice: UFix64, bidAddress: Address){ 
	...
	resource Listing: ListingPublic{ 
		...
		//当前投标用户钱包引用
		access(self)
		var bidVault: @{FungibleToken.Vault}
