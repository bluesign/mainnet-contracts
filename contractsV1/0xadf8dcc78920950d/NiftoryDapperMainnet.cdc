...
access(all)
contract NiftoryDapperMainnet: NonFungibleToken, ViewResolver{ 
	...
	resource NFT: NonFungibleToken.NFT, ViewResolver.Resolver, NiftoryNonFungibleToken.NFTPublic{ 
		...
		view fun _contract(): &{NiftoryNonFungibleToken.ManagerPublic}{ 
			return NiftoryDapperMainnet._contract()
		}
	...
	resource Manager: NiftoryNonFungibleToken.ManagerPublic, NiftoryNonFungibleToken.ManagerPrivate{ 
		...
		fun getNFTCollectionData(): MetadataViews.NFTCollectionData{ 
			return NiftoryNFTRegistry.buildNFTCollectionData(NiftoryDapperMainnet.REGISTRY_ADDRESS, NiftoryDapperMainnet.REGISTRY_BRAND, fun (): @{NonFungibleToken.Collection}{ 
					return <-NiftoryDapperMainnet.createEmptyCollection(nftType: Type<@NiftoryDapperMainnet.Collection>())
				})
		}
		...
		fun modifyContractMetadata(): &AnyStruct{ 
			...
			return (&NiftoryDapperMainnet.metadata as &AnyStruct?)!
		}
		...
		fun _getMetadataViewsManagerPrivate(): &MetadataViewsManager.Manager{ 
			...
			let manager = NiftoryDapperMainnet.account.capabilities.get<&MetadataViewsManager.Manager>(record.metadataViewsManager.paths.private).borrow()!
			return manager
		}
		...
		fun _getSetManagerPrivate(): &MutableMetadataSetManager.Manager{ 
			let record = NiftoryNFTRegistry.getRegistryRecord(NiftoryDapperMainnet.REGISTRY_ADDRESS, NiftoryDapperMainnet.REGISTRY_BRAND)
			let setManager = NiftoryDapperMainnet.account.capabilities.get<&MutableMetadataSetManager.Manager>(record.setManager.paths.private).borrow()!
			return setManager
		}
	...
	init(nftManagerProxy: &{NiftoryNonFungibleTokenProxy.Public, NiftoryNonFungibleTokenProxy.Private}){ 
		...
		self.account.storage.save<@MutableMetadataSetManager.Manager>(<-MutableMetadataSetManager._create(name: "NiftoryDapperMainnet", description: "The set manager for NiftoryDapperMainnet."), to: record.setManager.paths.storage)
