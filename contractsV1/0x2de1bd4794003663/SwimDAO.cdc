...
access(all)
contract SwimDAO: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				SwimDAO.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return SwimDAO.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				SwimDAO.maxSupply == nil || SwimDAO.totalSupply <= SwimDAO.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			SwimDAO.totalSupply = SwimDAO.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreidv75hz6sabfmne6mxjptqzim4sl2jehknawmnp7rrjt4gbts4hqi"), mediaType: "image")
				let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreifug52jcqbnqbwkie4clixb45jbrgwb6kpetw5dlvb6hbn5mhiw24"), mediaType: "image")
				let medias = MetadataViews.Medias([media, bannerMedia])
				return FungibleTokenMetadataViews.FTDisplay(name: "SwimDAO", symbol: "SWIM", description: "A  Scalable Entrepreneurship Ecosystem ", externalURL: MetadataViews.ExternalURL("swimdao.org"), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("swimdao"), "discord": MetadataViews.ExternalURL("swimdao")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: SwimDAO.VaultStoragePath, receiverPath: SwimDAO.ReceiverPublicPath, metadataPath: SwimDAO.VaultPublicPath, receiverLinkedType: /private/SwimDAOVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-SwimDAO.createEmptyVault(vaultType: Type<@SwimDAO.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/SwimDAOVault
		self.ReceiverPublicPath = /public/SwimDAOReceiver
		self.VaultPublicPath = /public/SwimDAOMetadata
		self.MinterStoragePath = /storage/SwimDAOMinter
		self.AdministratorStoragePath = /storage/SwimDAOAdmin
