...
access(all)
contract JollyRoger: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				JollyRoger.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return JollyRoger.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				JollyRoger.maxSupply == nil || JollyRoger.totalSupply <= JollyRoger.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			JollyRoger.totalSupply = JollyRoger.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreiesj3rzq4wvflxi5u7effyrcwe6tqct2nxdc44hutgwz6kewqaccy"), mediaType: "image")
				let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreicwr6jvrvdcgl7k7ryeosamwcyfjwb4g2z6wbbeo37cbcsup6psjq"), mediaType: "image")
				let medias = MetadataViews.Medias([media, bannerMedia])
				return FungibleTokenMetadataViews.FTDisplay(name: "Jolly Roger", symbol: "JOLLY", description: "Hoist the flag! Doubloons for all... ", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: JollyRoger.VaultStoragePath, receiverPath: JollyRoger.ReceiverPublicPath, metadataPath: JollyRoger.VaultPublicPath, receiverLinkedType: /private/JollyRogerVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-JollyRoger.createEmptyVault(vaultType: Type<@JollyRoger.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/JollyRogerVault
		self.ReceiverPublicPath = /public/JollyRogerReceiver
		self.VaultPublicPath = /public/JollyRogerMetadata
		self.MinterStoragePath = /storage/JollyRogerMinter
		self.AdministratorStoragePath = /storage/JollyRogerAdmin
