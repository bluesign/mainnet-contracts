...
access(all)
contract ArkDao: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				ArkDao.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return ArkDao.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				ArkDao.maxSupply == nil || ArkDao.totalSupply <= ArkDao.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			ArkDao.totalSupply = ArkDao.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreicsbhxw2nijqtyi4xxy66a7cwhkv6qdu6ddjlc67mblfouzewfgxq"), mediaType: "image")
				let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreid2o6jvbteh7y66an5e6h4nqh4vwwczoatpl2nqwel3ul3mw6qr5u"), mediaType: "image")
				let medias = MetadataViews.Medias([media, bannerMedia])
				return FungibleTokenMetadataViews.FTDisplay(name: "Ark Dao", symbol: "ARK", description: "Board the 'Ark' \u{1f680} - Let's sail past the moon p.s. Noah ate the animals", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: ArkDao.VaultStoragePath, receiverPath: ArkDao.ReceiverPublicPath, metadataPath: ArkDao.VaultPublicPath, receiverLinkedType: /private/ArkDaoVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-ArkDao.createEmptyVault(vaultType: Type<@ArkDao.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/ArkDaoVault
		self.ReceiverPublicPath = /public/ArkDaoReceiver
		self.VaultPublicPath = /public/ArkDaoMetadata
		self.MinterStoragePath = /storage/ArkDaoMinter
		self.AdministratorStoragePath = /storage/ArkDaoAdmin
