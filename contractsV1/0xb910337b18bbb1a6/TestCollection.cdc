...
access(all)
contract TestCollection: NonFungibleToken{ 
	access(all)
	...
	access(all)
	resource Collection: TestCollectionCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		access(all)
		...
		view fun borrowViewResolver(id: UInt64): &{ViewResolver.Resolver}?{ 
			...
			return TestCollection as &{ViewResolver.Resolver}
		}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: TestCollection.CollectionStoragePath, publicPath: TestCollection.CollectionPublicPath, publicCollection: Type<&TestCollection.Collection>(), publicLinkedType: Type<&TestCollection.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-TestCollection.createEmptyCollection(nftType: Type<@TestCollection.Collection>())
						})
			}
			return nil
		}
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&TestCollection.Collection>(self.CollectionStoragePath)
