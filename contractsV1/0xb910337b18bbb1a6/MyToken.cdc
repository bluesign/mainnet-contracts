...
contract MyToken: FungibleToken{ 
	...
	init(){ 
		...
		self.account.storage.save(<-create Minter(), to: MyToken.TokenMinterStoragePath)
		
		//
		// Create an Empty Vault for the Minter
		//
		self.account.storage.save(<-MyToken.initialMint(initialMintValue: self.totalSupply), to: MyToken.TokenVaultStoragePath)
		var capability_1 = self.account.capabilities.storage.issue<&MyToken.Vault>(MyToken.TokenVaultStoragePath)
		self.account.capabilities.publish(capability_1, at: MyToken.TokenVaultPublicPath)
	}
}
