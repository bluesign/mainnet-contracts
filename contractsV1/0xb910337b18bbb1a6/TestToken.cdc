...
access(all)
contract TestToken: FungibleToken{ 
	access(all)
	...
	init(){ 
		...
		self.account.storage.save(<-create Minter(), to: TestToken.TokenMinterStoragePath)
		
		//
		// Create an Empty Vault for the Minter
		//
		self.account.storage.save(<-TestToken.initialMint(initialMintValue: self.totalSupply), to: TestToken.TokenVaultStoragePath)
		var capability_1 = self.account.capabilities.storage.issue<&TestToken.Vault>(TestToken.TokenVaultStoragePath)
		self.account.capabilities.publish(capability_1, at: TestToken.TokenVaultPublicPath)
	}
}
