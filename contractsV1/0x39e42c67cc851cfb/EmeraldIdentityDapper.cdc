...
access(all)
contract EmeraldIdentityDapper{ 
	...
	resource Administrator{ 
		...
		fun removeByDiscord(discordID: String){ 
			let account =
				EmeraldIdentityDapper.getAccountFromDiscord(discordID: discordID)
				?? panic("This EmeraldID does not exist!")
			self.remove(account: account, discordID: discordID)
		}
		...
		fun createAdministrator(): Capability<&Administrator>{ 
			return EmeraldIdentityDapper.account.capabilities.get<&Administrator>(
	...
	view fun getDiscordFromAccount(account: Address): String?{ 
		let admin =
			EmeraldIdentityDapper.account.storage.borrow<&Administrator>(
	...
	init(){ 
		...
		var capability_1 =
			self.account.capabilities.storage.issue<&Administrator>(
				EmeraldIdentityDapper.AdministratorStoragePath
			)
		self.account.capabilities.publish(
			capability_1,
			at: EmeraldIdentityDapper.AdministratorPrivatePath
		)
	}
}
