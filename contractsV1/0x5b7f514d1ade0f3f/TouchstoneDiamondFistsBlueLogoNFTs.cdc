...
access(all)
contract TouchstoneDiamondFistsBlueLogoNFTs: NonFungibleToken{ 
	...
	resource NFT: NonFungibleToken.NFT, ViewResolver.Resolver{ 
			...
			switch view{ 
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: TouchstoneDiamondFistsBlueLogoNFTs.CollectionStoragePath, publicPath: TouchstoneDiamondFistsBlueLogoNFTs.CollectionPublicPath, publicCollection: Type<&Collection>(), publicLinkedType: Type<&Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-TouchstoneDiamondFistsBlueLogoNFTs.createEmptyCollection(nftType: Type<@TouchstoneDiamondFistsBlueLogoNFTs.Collection>())
						})
		...
		init(_metadataId: UInt64, _serial: UInt64, _recipient: Address){ 
			...
			if let buyersRef = &TouchstoneDiamondFistsBlueLogoNFTs.primaryBuyers[_recipient] as auth(Mutate) &{UInt64: [UInt64]}?{ 
	...
	resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun claim(){ 
			if let storage = &TouchstoneDiamondFistsBlueLogoNFTs.nftStorage[(self.owner!).address] as auth(Mutate) &{UInt64: NFT}?{ 
	...
	resource Administrator{ 
		...
		fun mintNFT(metadataId: UInt64, serial: UInt64, recipient: Address){ 
			...
			if let recipientCollection = getAccount(recipient).capabilities.get<&TouchstoneDiamondFistsBlueLogoNFTs.Collection>(TouchstoneDiamondFistsBlueLogoNFTs.CollectionPublicPath).borrow<&TouchstoneDiamondFistsBlueLogoNFTs.Collection>(){ 
				recipientCollection.deposit(token: <-nft)
			} else if let storage = &TouchstoneDiamondFistsBlueLogoNFTs.nftStorage[recipient] as auth(Mutate) &{UInt64: NFT}?{ 
				storage[nft.id] <-! nft
			} else{ 
	...
	fun getClaimableNFTs(user: Address):{ UInt64: NFTMetadata}{ 
		...
		if let storage = &TouchstoneDiamondFistsBlueLogoNFTs.nftStorage[user] as auth(Mutate) &{UInt64: NFT}?{ 
			for id in storage.keys{ 
				let nftRef = (storage[id] as &TouchstoneDiamondFistsBlueLogoNFTs.NFT?)!
