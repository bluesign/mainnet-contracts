...
access(all)
contract TouchstoneSpecialEditionFloFightersxDiamondFists: NonFungibleToken{ 
	...
	struct NFTMetadata{ 
		...
		init(_name: String, _description: String, _image: MetadataViews.IPFSFile, _thumbnail: MetadataViews.IPFSFile?, _price: UFix64?, _extra:{ String: AnyStruct}, _supply: UInt64){ 
			self.metadataId = TouchstoneSpecialEditionFloFightersxDiamondFists.nextMetadataId
			self.name = _name
	...
	resource NFT: NonFungibleToken.NFT, ViewResolver.Resolver{ 
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: TouchstoneSpecialEditionFloFightersxDiamondFists.CollectionStoragePath, publicPath: TouchstoneSpecialEditionFloFightersxDiamondFists.CollectionPublicPath, publicCollection: Type<&Collection>(), publicLinkedType: Type<&Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-TouchstoneSpecialEditionFloFightersxDiamondFists.createEmptyCollection(nftType: Type<@TouchstoneSpecialEditionFloFightersxDiamondFists.Collection>())
						})
		...
		init(_metadataId: UInt64, _serial: UInt64, _recipient: Address){ 
			...
			if let buyersRef = &TouchstoneSpecialEditionFloFightersxDiamondFists.primaryBuyers[_recipient] as auth(Mutate) &{UInt64: [UInt64]}?{ 
	...
	resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun claim(){ 
			if let storage = &TouchstoneSpecialEditionFloFightersxDiamondFists.nftStorage[(self.owner!).address] as auth(Mutate) &{UInt64: NFT}?{ 
	...
	fun mintNFT(metadataId: UInt64, recipient: &{NonFungibleToken.Receiver}, payment: @FlowToken.Vault, serial: UInt64): UInt64{ 
		...
		if let royalty = TouchstoneSpecialEditionFloFightersxDiamondFists.getOptionalCollectionAttribute(key: "royalty") as! MetadataViews.Royalty?{ 
	...
	resource Administrator{ 
		...
		fun createNFTMetadata(name: String, description: String, imagePath: String, thumbnailPath: String?, ipfsCID: String, price: UFix64?, extra:{ String: AnyStruct}, supply: UInt64){ 
			TouchstoneSpecialEditionFloFightersxDiamondFists.metadatas[TouchstoneSpecialEditionFloFightersxDiamondFists.nextMetadataId] = NFTMetadata(_name: name, _description: description, _image: MetadataViews.IPFSFile(cid: ipfsCID, path: imagePath), _thumbnail: thumbnailPath == nil ? nil : MetadataViews.IPFSFile(cid: ipfsCID, path: thumbnailPath), _price: price, _extra: extra, _supply: supply)
			TouchstoneSpecialEditionFloFightersxDiamondFists.nextMetadataId = TouchstoneSpecialEditionFloFightersxDiamondFists.nextMetadataId + 1
		}
		...
		fun mintNFT(metadataId: UInt64, serial: UInt64, recipient: Address){ 
			...
			if let recipientCollection = getAccount(recipient).capabilities.get<&TouchstoneSpecialEditionFloFightersxDiamondFists.Collection>(TouchstoneSpecialEditionFloFightersxDiamondFists.CollectionPublicPath).borrow<&TouchstoneSpecialEditionFloFightersxDiamondFists.Collection>(){ 
				recipientCollection.deposit(token: <-nft)
			} else if let storage = &TouchstoneSpecialEditionFloFightersxDiamondFists.nftStorage[recipient] as auth(Mutate) &{UInt64: NFT}?{ 
				storage[nft.id] <-! nft
			} else{ 
		...
		fun changeField(key: String, value: AnyStruct){ 
			TouchstoneSpecialEditionFloFightersxDiamondFists.collectionInfo[key] = value
		}
	}
	...
	view fun getPriceOfNFT(_ metadataId: UInt64): UFix64?{ 
		if let metadata: TouchstoneSpecialEditionFloFightersxDiamondFists.NFTMetadata = self.getNFTMetadata(metadataId){ 
	...
	fun getClaimableNFTs(user: Address):{ UInt64: NFTMetadata}{ 
		...
		if let storage = &TouchstoneSpecialEditionFloFightersxDiamondFists.nftStorage[user] as auth(Mutate) &{UInt64: NFT}?{ 
			for id in storage.keys{ 
				let nftRef = (storage[id] as &TouchstoneSpecialEditionFloFightersxDiamondFists.NFT?)!
	...
	init(_name: String, _description: String, _imagePath: String, _bannerImagePath: String?, _minting: Bool, _royalty: MetadataViews.Royalty?, _defaultPrice: UFix64, _paymentType: String, _ipfsCID: String, _lotteryBuying: Bool, _socials:{ String: MetadataViews.ExternalURL}, _mintVerifiers: [{MintVerifiers.IVerifier}]){ 
		...
		self.CollectionStoragePath = /storage/TouchstoneSpecialEditionFloFightersxDiamondFistsCollection_0x5b7f514d1ade0f3f
		self.CollectionPublicPath = /public/TouchstoneSpecialEditionFloFightersxDiamondFistsCollection_0x5b7f514d1ade0f3f
		self.CollectionPrivatePath = /private/TouchstoneSpecialEditionFloFightersxDiamondFistsCollection_0x5b7f514d1ade0f3f
		self.AdministratorStoragePath = /storage/TouchstoneSpecialEditionFloFightersxDiamondFistsAdministrator_0x5b7f514d1ade0f3f
