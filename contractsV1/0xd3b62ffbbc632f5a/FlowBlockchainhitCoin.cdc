...
access(all)
contract FlowBlockchainhitCoin: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				FlowBlockchainhitCoin.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return FlowBlockchainhitCoin.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				FlowBlockchainhitCoin.maxSupply == nil || FlowBlockchainhitCoin.totalSupply <= FlowBlockchainhitCoin.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			FlowBlockchainhitCoin.totalSupply = FlowBlockchainhitCoin.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreihgkcx73e444ehgfwdwthhs7ghvhwv7hnb65csuwsvskk7v6fhsu4"), mediaType: "image")
				let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreifniuyvoqtjwi3jnwnmwbo7vfvwzqkyxcet27dahu2mbp2qjvejye"), mediaType: "image")
				let medias = MetadataViews.Medias([media, bannerMedia])
				return FungibleTokenMetadataViews.FTDisplay(name: "Flow Blockchain $hit Coin", symbol: "HIT", description: "Just an original shitcoin, powered by the FLOW blockchain", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: FlowBlockchainhitCoin.VaultStoragePath, receiverPath: FlowBlockchainhitCoin.ReceiverPublicPath, metadataPath: FlowBlockchainhitCoin.VaultPublicPath, receiverLinkedType: /private/FlowBlockchainhitCoinVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-FlowBlockchainhitCoin.createEmptyVault(vaultType: Type<@FlowBlockchainhitCoin.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/FlowBlockchainhitCoinVault
		self.ReceiverPublicPath = /public/FlowBlockchainhitCoinReceiver
		self.VaultPublicPath = /public/FlowBlockchainhitCoinMetadata
		self.MinterStoragePath = /storage/FlowBlockchainhitCoinMinter
		self.AdministratorStoragePath = /storage/FlowBlockchainhitCoinAdmin
