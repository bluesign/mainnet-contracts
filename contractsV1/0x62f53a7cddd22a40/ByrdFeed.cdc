...
access(all)
contract ByrdFeed: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				ByrdFeed.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return ByrdFeed.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				ByrdFeed.maxSupply == nil || ByrdFeed.totalSupply <= ByrdFeed.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			ByrdFeed.totalSupply = ByrdFeed.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "Byrd Feed", symbol: "BYRD", description: "Feed for the byrds", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: ByrdFeed.VaultStoragePath, receiverPath: ByrdFeed.ReceiverPublicPath, metadataPath: ByrdFeed.VaultPublicPath, receiverLinkedType: /private/ByrdFeedVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-ByrdFeed.createEmptyVault(vaultType: Type<@ByrdFeed.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/ByrdFeedVault
		self.ReceiverPublicPath = /public/ByrdFeedReceiver
		self.VaultPublicPath = /public/ByrdFeedMetadata
		self.MinterStoragePath = /storage/ByrdFeedMinter
		self.AdministratorStoragePath = /storage/ByrdFeedAdmin
