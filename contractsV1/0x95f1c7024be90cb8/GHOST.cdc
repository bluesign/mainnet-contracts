...
access(all)
contract GHOST: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				GHOST.setBalance(address: owner, balance: self.balance)
			}
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				GHOST.maxSupply == nil || GHOST.totalSupply <= GHOST.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			GHOST.totalSupply = GHOST.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreibx7gmgudp734t6bih3vfrecoryfhuuie4hwz35n44f2kxtghaske"), mediaType: "image")
				let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreibx7gmgudp734t6bih3vfrecoryfhuuie4hwz35n44f2kxtghaske"), mediaType: "image")
				let medias = MetadataViews.Medias([media, bannerMedia])
				return FungibleTokenMetadataViews.FTDisplay(name: "$GHOST", symbol: "GHOST", description: "The GHOST emerges as a luminous beacon,", externalURL: MetadataViews.ExternalURL("ninjapanda.io/"), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("https://twitter.com/N1NJ4P4ND410"), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: GHOST.VaultStoragePath, receiverPath: GHOST.ReceiverPublicPath, metadataPath: GHOST.VaultPublicPath, receiverLinkedType: /private/GHOSTVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-GHOST.createEmptyVault(vaultType: Type<@GHOST.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/GHOSTVault
		self.ReceiverPublicPath = /public/GHOSTReceiver
		self.VaultPublicPath = /public/GHOSTMetadata
		self.MinterStoragePath = /storage/GHOSTMinter
		self.AdministratorStoragePath = /storage/GHOSTAdmin
