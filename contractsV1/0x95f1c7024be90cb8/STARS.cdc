...
access(all)
contract STARS: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				STARS.setBalance(address: owner, balance: self.balance)
			}
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				STARS.maxSupply == nil || STARS.totalSupply <= STARS.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			STARS.totalSupply = STARS.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreigultlrtfrx64556pqcauvz2rgtusvgglpwwpqbgqwegeur4idpeu"), mediaType: "image")
				let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreigultlrtfrx64556pqcauvz2rgtusvgglpwwpqbgqwegeur4idpeu"), mediaType: "image")
				let medias = MetadataViews.Medias([media, bannerMedia])
				return FungibleTokenMetadataViews.FTDisplay(name: "STAR$", symbol: "STARS", description: "Forever reaching for the stars", externalURL: MetadataViews.ExternalURL("ninjapanda.io"), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("https://twitter.com/N1NJ4P4ND410"), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: STARS.VaultStoragePath, receiverPath: STARS.ReceiverPublicPath, metadataPath: STARS.VaultPublicPath, receiverLinkedType: /private/STARSVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-STARS.createEmptyVault(vaultType: Type<@STARS.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/STARSVault
		self.ReceiverPublicPath = /public/STARSReceiver
		self.VaultPublicPath = /public/STARSMetadata
		self.MinterStoragePath = /storage/STARSMinter
		self.AdministratorStoragePath = /storage/STARSAdmin
