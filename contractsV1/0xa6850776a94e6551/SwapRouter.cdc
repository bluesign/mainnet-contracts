...
contract SwapRouter{ 
	...
	fun swapWithPath(
		...
		while index < tokenKeyPath.length - 1{ 
			var _in <- curVaultOut.withdraw(amount: curVaultOut.balance)
			var exactAmountOut: UFix64? = nil
			if exactAmounts != nil{ 
				exactAmountOut = (exactAmounts!)[index + 1]
			}
			var out <- self.swapWithPair(vaultIn: <-_in, exactAmountOut: exactAmountOut, token0Key: tokenKeyPath[index], token1Key: tokenKeyPath[index + 1])
			curVaultOut <-> out
			destroy out
			index = index + 1
		}
