...
access(all)
contract MatrixMarket: NonFungibleToken{ 
	access(all)
	...
	access(all)
	resource interface MatrixMarketCollectionPublic{ 
		access(all)
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowMatrixMarket(id: UInt64): &MatrixMarket.NFT?{ 
			post{ 
	...
	access(all)
	resource Collection: MatrixMarketCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
	...
	resource NFTMinter{ 
		...
		fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, subCollectionId: String, metadata:{ String: String}): &{NonFungibleToken.NFT}{ 
			...
			// create a new NFT
			var newNFT <- create NFT(id: MatrixMarket.totalSupply, creator: creator, subCollectionId: subCollectionId, metadata: metadata)
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&MatrixMarket.Collection>(self.CollectionStoragePath)
