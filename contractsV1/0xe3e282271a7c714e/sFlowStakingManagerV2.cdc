...
access(all)
contract sFlowStakingManagerV2{ 
	access(contract)
	var unstakeRequests: [{String: AnyStruct}]
		...
		fun unstakeAll(nodeId: String, delegatorId: UInt32){ 
			...
			for info in delegatingInfo{ 
				if info.nodeID == nodeId && info.id == delegatorId{ 
					let stakingCollectionRef: &FlowStakingCollection.StakingCollection = sFlowStakingManagerV2.account.storage.borrow<&FlowStakingCollection.StakingCollection>(from: FlowStakingCollection.StakingCollectionStoragePath) ?? panic("Could not borrow ref to StakingCollection")
	...
	init(nodeID: String, delegatorID: UInt32){ 
		...
		self.account.storage.save(<-create Manager(), to: /storage/sFlowStakingManagerV2)
		var capability_1 =
			self.account.capabilities.storage.issue<&sFlowStakingManagerV2.Manager>(
				/storage/sFlowStakingManagerV2
			)
		self.account.capabilities.publish(capability_1, at: /private/sFlowStakingManagerV2)
