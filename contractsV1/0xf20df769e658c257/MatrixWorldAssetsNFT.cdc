...
contract MatrixWorldAssetsNFT: NonFungibleToken, LicensedNFT{ 
	...
	access(all)
	resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, LicensedNFT.CollectionPublic, Metadata{ 
		access(all)
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&{Metadata, NonFungibleToken.CollectionPublic, NonFungibleToken.Receiver}>(self.collectionStoragePath)
