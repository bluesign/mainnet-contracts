...
access(all)
contract GummyBear: FungibleToken{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				GummyBear.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				...
				case Type<FungibleTokenMetadataViews.FTDisplay>():
					let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreibliwqzbojquctxwuxmxrshahhoyqawh5mcp77dhedbjvvditkkxe"), mediaType: "image")
					let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreigbouixabpk44wkx6lzqgh5zyfvonlltsuo4ddihmcmpjqtwot6hm"), mediaType: "image")
					let medias = MetadataViews.Medias([media, bannerMedia])
					return FungibleTokenMetadataViews.FTDisplay(name: "Gummy Bear", symbol: "GBEAR", description: "Black Gummy Bear is a dark and negative meme coin", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("@_GreenBitcoin"), "discord": MetadataViews.ExternalURL("")})
				case Type<FungibleTokenMetadataViews.FTVaultData>():
					return FungibleTokenMetadataViews.FTVaultData(storagePath: GummyBear.VaultStoragePath, receiverPath: GummyBear.ReceiverPublicPath, metadataPath: GummyBear.VaultPublicPath, receiverLinkedType: /private/GummyBearVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
							return <-GummyBear.createEmptyVault(vaultType: Type<@GummyBear.Vault>())
						})
			}
			return nil
		}
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				GummyBear.maxSupply == nil || GummyBear.totalSupply <= GummyBear.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			GummyBear.totalSupply = GummyBear.totalSupply + amount
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/GummyBearVault
		self.ReceiverPublicPath = /public/GummyBearReceiver
		self.VaultPublicPath = /public/GummyBearMetadata
		self.MinterStoragePath = /storage/GummyBearMinter
		self.AdministratorStoragePath = /storage/GummyBearAdmin
