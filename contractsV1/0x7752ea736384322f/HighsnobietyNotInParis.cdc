...
contract HighsnobietyNotInParis: NonFungibleToken, TheFabricantNFTStandardV2, RevealableV2{ 
	...
	resource Admin{ 
		...
		fun distributeDirectlyViaAccessList(receiver: &{NonFungibleToken.CollectionPublic}, publicMinterPathString: String, variantId: UInt64){ 
			...
			let publicMinterCap = (getAccount((self.owner!).address).capabilities.get<&HighsnobietyNotInParis.PublicMinter>(publicPath)!).borrow() ?? panic("Couldn't get publicMinter ref or pathString is wrong: ".concat(publicMinterPathString))
