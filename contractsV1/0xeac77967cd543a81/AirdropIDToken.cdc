...
access(all)
contract AirdropIDToken: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				AirdropIDToken.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return AirdropIDToken.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				AirdropIDToken.maxSupply == nil || AirdropIDToken.totalSupply <= AirdropIDToken.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			AirdropIDToken.totalSupply = AirdropIDToken.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "AirdropID Token", symbol: "AEIDT", description: "Welcome Crypto AirdropID Bounty Sharing ChannelYou can get free Cryptocurrency Airdrop everyday here Airdrop - Testnet - Retroactive And How to earn money is simple.", externalURL: MetadataViews.ExternalURL("t.me/DropAID"), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("AirdropID_ID"), "discord": MetadataViews.ExternalURL("DropAID")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: AirdropIDToken.VaultStoragePath, receiverPath: AirdropIDToken.ReceiverPublicPath, metadataPath: AirdropIDToken.VaultPublicPath, receiverLinkedType: /private/AirdropIDTokenVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-AirdropIDToken.createEmptyVault(vaultType: Type<@AirdropIDToken.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/AirdropIDTokenVault
		self.ReceiverPublicPath = /public/AirdropIDTokenReceiver
		self.VaultPublicPath = /public/AirdropIDTokenMetadata
		self.MinterStoragePath = /storage/AirdropIDTokenMinter
		self.AdministratorStoragePath = /storage/AirdropIDTokenAdmin
