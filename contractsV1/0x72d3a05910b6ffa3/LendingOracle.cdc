...
contract LendingOracle{ 
	...
	access(all)
	let OraclePublicPath: PublicPath
	...
	access(all)
	event PriceFeedAdded(_for pool: Address, oracleAddr: Address)
	
	access(all)
	event PriceFeedRemoved(from pool: Address)
	
	access(all)
	resource OracleReaders: LendingInterfaces.OraclePublic{ 
		access(self)
		let feeds: [Address]
		
		access(self)
		let oracleAddrDict:{ Address: Address}
		...
		access(contract)
		fun addPriceFeed(_for pool: Address, oracleAddr: Address){ 
			if !self.feeds.contains(pool){ 
				/// 1. Append new feed
				...
				if LendingOracle.account.storage.borrow<&OracleInterface.PriceReader>(from: priceReaderSuggestedPath) == nil{ 
					...
					LendingOracle.account.storage.save(<-priceReader, to: priceReaderSuggestedPath)
				}
				emit PriceFeedAdded(_for: pool, oracleAddr: oracleAddr)
			}
		}
		
		access(contract)
		fun removePriceFeed(pool: Address){ 
			if self.feeds.contains(pool){ 
	...
	init(){ 
		...
		var adminRef =
			self.account.storage.borrow<&LendingOracle.Admin>(from: self.OracleAdminStoragePath)!
		destroy <-self.account.storage.load<@AnyResource>(from: self.OracleStoragePath)
		// Create and store a new Oracle resource
		self.account.storage.save(<-adminRef.createOracleResource(), to: self.OracleStoragePath)
		...
		var capability_1 =
			self.account.capabilities.storage.issue<&{LendingInterfaces.OraclePublic}>(
				self.OracleStoragePath
			)
		self.account.capabilities.publish(capability_1, at: self.OraclePublicPath)
		self._reservedFields ={} 
	}
}
