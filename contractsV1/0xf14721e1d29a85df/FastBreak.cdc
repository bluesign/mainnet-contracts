...
access(all)
contract FastBreak: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				FastBreak.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return FastBreak.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				FastBreak.maxSupply == nil || FastBreak.totalSupply <= FastBreak.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			FastBreak.totalSupply = FastBreak.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafybeibl6oqxri2bbnfwh7k5ybzjd2spy6yj65qkvzofxins5pp3fxm5be"), mediaType: "image")
				let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafybeicvc4bffmgp3l3hti2kve23imawtpo37tvnxvs4z3223ztd6hvkfe"), mediaType: "image")
				let medias = MetadataViews.Medias([media, bannerMedia])
				return FungibleTokenMetadataViews.FTDisplay(name: "FastBreak", symbol: "FBRK", description: "This is a DAO for NBA Top Shot Fast Break (unofficial)", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: FastBreak.VaultStoragePath, receiverPath: FastBreak.ReceiverPublicPath, metadataPath: FastBreak.VaultPublicPath, receiverLinkedType: /private/FastBreakVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-FastBreak.createEmptyVault(vaultType: Type<@FastBreak.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/FastBreakVault
		self.ReceiverPublicPath = /public/FastBreakReceiver
		self.VaultPublicPath = /public/FastBreakMetadata
		self.MinterStoragePath = /storage/FastBreakMinter
		self.AdministratorStoragePath = /storage/FastBreakAdmin
