...
contract Collectibles: CapsuleNFT{ 
	...
	access(all)
	event CollectibleMinted(id: String, name: String, collection: String, type: String, rarity: String, edition: String, mediaUri: String, mintedTime: String, resourceId: UInt64)
	
	access(all)
	event CollectibleDestroyed(id: String)
	...
	access(all)
	resource Collection: CollectiblesCollectionPublic, CapsuleNFT.Provider, CapsuleNFT.Receiver, CapsuleNFT.CollectionPublic{ 
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&Collectibles.Collection>(self.CollectionStoragePath)
