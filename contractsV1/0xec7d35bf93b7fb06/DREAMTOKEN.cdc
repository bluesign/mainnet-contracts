...
access(all)
contract DREAMTOKEN: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				DREAMTOKEN.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return DREAMTOKEN.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				DREAMTOKEN.maxSupply == nil || DREAMTOKEN.totalSupply <= DREAMTOKEN.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			DREAMTOKEN.totalSupply = DREAMTOKEN.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "DREAM TOKEN", symbol: "DREAM", description: "NATIVE TOKEN FOR THE DREAM DAO COMMUNITY", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("dreamdaofnd"), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: DREAMTOKEN.VaultStoragePath, receiverPath: DREAMTOKEN.ReceiverPublicPath, metadataPath: DREAMTOKEN.VaultPublicPath, receiverLinkedType: /private/DREAMTOKENVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-DREAMTOKEN.createEmptyVault(vaultType: Type<@DREAMTOKEN.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/DREAMTOKENVault
		self.ReceiverPublicPath = /public/DREAMTOKENReceiver
		self.VaultPublicPath = /public/DREAMTOKENMetadata
		self.MinterStoragePath = /storage/DREAMTOKENMinter
		self.AdministratorStoragePath = /storage/DREAMTOKENAdmin
