...
contract AToken: FungibleToken{ 
	...
	resource Minter{ 
		...
		//
		access(TMP_ENTITLEMENT_OWNER)
		fun mintTokens(amount: UFix64): @AToken.Vault{ 
			pre{ 
				amount > 0.0:
	...
	init(){ 
		...
		adminAccount.storage.save(<-vault, to: /storage/ATokenVault)
		
		// Create a public capability to the stored Vault that only exposes
		// the `deposit` method through the `Receiver` interface
		//
		adminAccount.link<&AToken.Vault>(/public/flowTokenReceiver, target: /storage/ATokenVault)
		
		// Create a public capability to the stored Vault that only exposes
		// the `balance` field through the `Balance` interface
		//
		adminAccount.link<&AToken.Vault>(/public/flowTokenBalance, target: /storage/ATokenVault)
		let admin <- create Administrator()
		adminAccount.storage.save(<-admin, to: /storage/ATokenAdmin)
