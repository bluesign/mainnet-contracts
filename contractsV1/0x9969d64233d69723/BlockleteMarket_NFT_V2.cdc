...
import Blockletes_NFT_V2 from "./Blockletes_NFT_V2.cdc"
...
access(all)
contract BlockleteMarket_NFT_V2{ 
	...
	access(all)
	event BlockleteListed(id: UInt64, price: UFix64, seller: Address?)
	
	// emitted when the price of a listed blocklete has changed
	access(all)
	event BlockletePriceChanged(id: UInt64, newPrice: UFix64, seller: Address?)
	
	// emitted when a token is purchased from the market
	access(all)
	event BlockletePurchased(id: UInt64, price: UFix64, seller: Address?)
	
	// emitted when a Blocklete has been withdrawn from the sale
	access(all)
	event BlockleteWithdrawn(id: UInt64, owner: Address?)
	...
	resource interface SalePublic{ 
		...
		fun purchase(tokenID: UInt64, buyTokens: @{FungibleToken.Vault}): @Blockletes_NFT_V2.NFT{ 
			post{ 
				result.id == tokenID:
	...
	resource SaleCollection: SalePublic{ 
		...
		access(self)
		var blockletesForSale: @Blockletes_NFT_V2.Collection
		...
		access(self)
		var beneficiaryCapability: Capability<&FUSD.Vault>
		...
		init(ownerCapability: Capability<&FUSD.Vault>, beneficiaryCapability: Capability<&FUSD.Vault>, cutPercentage: UFix64){ 
			pre{ 
			...
			self.blockletesForSale <- Blockletes_NFT_V2.createEmptyCollection(nftType: Type<@Blockletes_NFT_V2.Collection>()) as! @Blockletes_NFT_V2.Collection
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun listForSale(token: @Blockletes_NFT_V2.NFT, price: UFix64){ 
			pre{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun withdraw(tokenID: UInt64): @Blockletes_NFT_V2.NFT{ 
