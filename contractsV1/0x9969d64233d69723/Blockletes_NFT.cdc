...
access(all)
contract Blockletes_NFT: NonFungibleToken{ 
	...
	fun fetch(_ from: Address, blockleteId: UInt64): &Blockletes_NFT.NFT?{ 
		let collection = (getAccount(from).capabilities.get<&Blockletes_NFT.Collection>(Blockletes_NFT.CollectionPublicPath)!).borrow() ?? panic("Couldn't get collection")
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&Blockletes_NFT.Admin>(self.AdminStoragePath)
