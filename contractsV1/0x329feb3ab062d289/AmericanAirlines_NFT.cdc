...
contract AmericanAirlines_NFT: NonFungibleToken{ 
	...
	fun fetch(_ from: Address, id: UInt64): &AmericanAirlines_NFT.NFT?{ 
		let collection = (getAccount(from).capabilities.get<&AmericanAirlines_NFT.Collection>(AmericanAirlines_NFT.CollectionPublicPath)!).borrow() ?? panic("Couldn't get collection")
