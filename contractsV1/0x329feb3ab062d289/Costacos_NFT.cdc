...
access(all)
contract Costacos_NFT: NonFungibleToken{ 
	...
	resource Series{ 
		...
		//
		access(TMP_ENTITLEMENT_OWNER)
		fun mintCostacos_NFT(recipient: &{NonFungibleToken.CollectionPublic}, tokenId: UInt64, setId: UInt32){ 
			pre{ 
		...
		//
		access(TMP_ENTITLEMENT_OWNER)
		fun mintEditionCostacos_NFT(recipient: &{NonFungibleToken.CollectionPublic}, tokenId: UInt64, setId: UInt32, edition: UInt32){ 
			pre{ 
				self.numberEditionsMintedPerSet[setId] != nil:
					"The Set does not exist."
				self.numberEditionsMintedPerSet[setId]! <= Costacos_NFT.getSetMaxEditions(setId: setId)!:
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun batchMintCostacos_NFT(recipient: &{NonFungibleToken.CollectionPublic}, setId: UInt32, tokenIds: [UInt64]){ 
			pre{ 
				tokenIds.length > 0:
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
					...
					for metadataKey in (Costacos_NFT.getSetMetadata(setId: self.setId)!).keys{ 
						...
						if metadataKey.length >= 13{ 
							if metadataKey.slice(from: 0, upTo: 13) == "royalty_addr_"{ 
								...
								if cutValue != 0.0{ 
									royalties.append(MetadataViews.Royalty(receiver: getAccount(royaltyAddress).capabilities.get<&{FungibleToken.Vault}>(royaltyReceiver), cut: cutValue, description: Costacos_NFT.getSetMetadataByField(setId: self.setId, field: "royalty_desc_".concat(royaltyName))!))
								}
							}
						}
					}
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: Costacos_NFT.CollectionStoragePath, publicPath: Costacos_NFT.CollectionPublicPath, publicCollection: Type<&Costacos_NFT.Collection>(), publicLinkedType: Type<&Costacos_NFT.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-Costacos_NFT.createEmptyCollection(nftType: Type<@Costacos_NFT.Collection>())
						})
				...
				case Type<MetadataViews.Medias>():
					return MetadataViews.Medias([MetadataViews.Media(file: MetadataViews.HTTPFile(url: Costacos_NFT.getSetMetadataByField(setId: self.setId, field: "image")!), mediaType: self.getMimeType())])
			}
			return nil
		}
	...
	access(all)
	resource interface Costacos_NFTCollectionPublic{ 
		access(all)
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowCostacos_NFT(id: UInt64): &Costacos_NFT.NFT?{ 
	...
	//
	access(all)
	resource Collection: Costacos_NFTCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		...
		view fun borrowViewResolver(id: UInt64): &{ViewResolver.Resolver}?{ 
			...
			return Costacos_NFTNft as &{ViewResolver.Resolver}
		}
	...
	//
	access(TMP_ENTITLEMENT_OWNER)
	fun fetch(_ from: Address, id: UInt64): &Costacos_NFT.NFT?{ 
		let collection = getAccount(from).capabilities.get<&Costacos_NFT.Collection>(Costacos_NFT.CollectionPublicPath).borrow<&Costacos_NFT.Collection>() ?? panic("Couldn't get collection")
	...
	access(TMP_ENTITLEMENT_OWNER)
	fun getAllSeries(): [Costacos_NFT.SeriesData]{ 
	...
	access(TMP_ENTITLEMENT_OWNER)
	fun getAllSets(): [Costacos_NFT.NFTSetData]{ 
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&Costacos_NFT.Admin>(self.AdminStoragePath)
