...
// VictoryARCollectible
// NFT items for Victory Collection
//
access(all)
contract VictoryARCollectible: NonFungibleToken{ 
	...
	access(all)
	event Minted(id: UInt64, typeID: UInt64, brandID: UInt64, seriesID: UInt64, dropID: UInt64)
	...
	// totalSupply
	// The total number of VictoryARCollectible that have been minted
	//
	access(all)
	...
	resource NFT: NonFungibleToken.NFT{ 
		...
		// initializer
		//
		init(initOwner: Address, initID: UInt64, initTypeID: UInt64, initBrandID: UInt64, initSeriesID: UInt64, initDropID: UInt64, initHash: UInt256, initIssueNum: UInt32, initMaxIssueNum: UInt32){ 
	...
	// This is the interface that users can cast their VictoryARCollectible Collection as
	// to allow others to deposit VictoryARCollectible into their Collection. It also allows for reading
	// the details of VictoryARCollectible in the Collection.
	access(all)
	resource interface VictoryARCollectibleCollectionPublic{ 
		access(all)
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowVictoryItem(id: UInt64): &VictoryARCollectible.NFT?{ 
	...
	access(all)
	resource interface VictoryARCollectibleBundle{ 
		access(TMP_ENTITLEMENT_OWNER)
	...
	//
	access(all)
	resource Collection: VictoryARCollectibleCollectionPublic, VictoryARCollectibleBundle, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic{ 
		...
		access(all)
		fun deposit(token: @{NonFungibleToken.NFT}): Void{ 
			let token <- token as! @VictoryARCollectible.NFT
		...
		fun borrowVictoryItem(id: UInt64): &VictoryARCollectible.NFT?{ 
			if self.ownedNFTs[id] != nil{ 
				...
				return ref as! &VictoryARCollectible.NFT?
			} else{ 
				return nil
			}
		}
	...
	resource NFTMinter{ 
		...
		//
		access(TMP_ENTITLEMENT_OWNER)
		fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, owner: Address, typeID: UInt64, brandID: UInt64, seriesID: UInt64, dropID: UInt64, contentHash: UInt256, startIssueNum: UInt32, maxIssueNum: UInt32, totalIssueNum: UInt32){ 
			var i: UInt32 = 0
			while i < maxIssueNum{ 
				emit Minted(id: VictoryARCollectible.totalSupply, typeID: typeID, brandID: brandID, seriesID: seriesID, dropID: dropID)
				
				// deposit it in the recipient's account using their reference
				recipient.deposit(token: <-create VictoryARCollectible.NFT(initOwner: owner, initID: VictoryARCollectible.totalSupply, initTypeID: typeID, initBrandID: brandID, initSeriesID: seriesID, initDropID: dropID, initHash: contentHash, initIssueNum: i + startIssueNum, initMaxIssueNum: totalIssueNum))
				VictoryARCollectible.totalSupply = VictoryARCollectible.totalSupply + 1 as UInt64
				i = i + 1 as UInt32
			}
		}
	}
	...
	// If an account does not have a VictoryARCollectible.Collection, panic.
	// If it has a collection but does not contain the itemID, return nil.
	// If it has a collection and that collection contains the itemID, return a reference to that.
	//
	access(TMP_ENTITLEMENT_OWNER)
	fun fetch(_ from: Address, itemID: UInt64): &VictoryARCollectible.NFT?{ 
		let collection = (getAccount(from).capabilities.get<&VictoryARCollectible.Collection>(VictoryARCollectible.CollectionPublicPath)!).borrow() ?? panic("Couldn't get collection")
		// We trust VictoryARCollectible.Collection.borowVictoryItem to get the correct itemID
	...
	init(){ 
		// Set our named paths
		self.CollectionStoragePath = /storage/VictoryARCollectibleCollection
		self.CollectionPublicPath = /public/VictoryARCollectibleCollection
		self.MinterStoragePath = /storage/VictoryARCollectibleMinter
