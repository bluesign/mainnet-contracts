...
//
access(all)
contract VictoryCollectible: NonFungibleToken{ 
	...
	access(all)
	resource interface VictoryCollectibleCollectionPublic{ 
		access(all)
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowVictoryItem(id: UInt64): &VictoryCollectible.NFT?{ 
	...
	access(all)
	resource interface VictoryCollectibleBundle{ 
		access(TMP_ENTITLEMENT_OWNER)
	...
	//
	access(all)
	resource Collection: VictoryCollectibleCollectionPublic, VictoryCollectibleBundle, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic{ 
	...
	resource NFTMinter{ 
		...
		fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, owner: Address, typeID: UInt64, brandID: UInt64, seriesID: UInt64, dropID: UInt64, contentHash: UInt256, startIssueNum: UInt32, maxIssueNum: UInt32, totalIssueNum: UInt32){ 
			...
			while i < maxIssueNum{ 
				...
				recipient.deposit(token: <-create VictoryCollectible.NFT(initOwner: owner, initID: VictoryCollectible.totalSupply, initTypeID: typeID, initBrandID: brandID, initSeriesID: seriesID, initDropID: dropID, initHash: contentHash, initIssueNum: i + startIssueNum, initMaxIssueNum: totalIssueNum))
	...
	//
	access(TMP_ENTITLEMENT_OWNER)
	fun fetch(_ from: Address, itemID: UInt64): &VictoryCollectible.NFT?{ 
		let collection = (getAccount(from).capabilities.get<&VictoryCollectible.Collection>(VictoryCollectible.CollectionPublicPath)!).borrow() ?? panic("Couldn't get collection")
