...
access(all)
contract VictoryARCollectibleSaleOffer{ 
	// SaleOffer events.
	//
	...
	resource SaleOffer: SaleOfferPublicView{ 
		...
		access(self)
		let sellerItemProvider: Capability<&VictoryARCollectible.Collection>
		...
		fun accept(buyerCollection: &VictoryARCollectible.Collection, buyerPayment: @{FungibleToken.Vault}, ownerPaymentReceiver: Capability<&{FungibleToken.Receiver}>){ 
			pre{ 
		...
		//
		init(sellerItemProvider: Capability<&VictoryARCollectible.Collection>, bundleID: UInt64, sellerPaymentReceiver: Capability<&{FungibleToken.Receiver}>, price: UFix64, saleType: UInt8, startTime: UInt32, endTime: UInt32, targetPrice: UFix64, royaltyPaymentReceiver: Capability<&{FungibleToken.Receiver}>, royaltyFactor: UFix64){ 
			pre{ 
	...
	resource interface CollectionManager{ 
		access(TMP_ENTITLEMENT_OWNER)
		fun createSaleOffer(
			sellerItemProvider: Capability<&VictoryARCollectible.Collection>,
			bundleID: UInt64,
		...
		): @VictoryARCollectibleSaleOffer.SaleOffer
		
		access(TMP_ENTITLEMENT_OWNER)
		fun insert(offer: @VictoryARCollectibleSaleOffer.SaleOffer)
	...
	resource Collection: CollectionManager, CollectionPurchaser, CollectionPublic{ 
		...
		//
		access(TMP_ENTITLEMENT_OWNER)
		fun createSaleOffer(sellerItemProvider: Capability<&VictoryARCollectible.Collection>, bundleID: UInt64, sellerPaymentReceiver: Capability<&{FungibleToken.Receiver}>, price: UFix64, saleType: UInt8, startTime: UInt32, endTime: UInt32, targetPrice: UFix64, royaltyPaymentReceiver: Capability<&{FungibleToken.Receiver}>, royaltyFactor: UFix64): @SaleOffer{ 
		...
		//
		access(TMP_ENTITLEMENT_OWNER)
		fun purchase(bundleID: UInt64, buyerCollection: &VictoryARCollectible.Collection, buyerPayment: @{FungibleToken.Vault}, ownerPaymentReceiver: Capability<&{FungibleToken.Receiver}>){ 
			pre{ 
	...
	init(){ 
		self.CollectionStoragePath = /storage/VictoryARCollectibleSaleOfferCollection
		self.CollectionPublicPath = /public/VictoryARCollectibleSaleOfferCollection
	}
}
