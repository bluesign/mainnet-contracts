...
access(all)
contract FlowUtilityToken: FungibleToken{ 
	...
	resource Minter{ 
		...
		//
		access(TMP_ENTITLEMENT_OWNER)
		fun mintTokens(amount: UFix64): @FlowUtilityToken.Vault{ 
			pre{ 
				amount > UFix64(0):
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&FlowUtilityToken.Vault>(/storage/flowUtilityTokenVault)
		self.account.capabilities.publish(capability_1, at: /public/flowUtilityTokenBalance)
		
		// Create a public capability to the stored Vault that only exposes
		// the deposit method through the Receiver interface
		var capability_2 = self.account.capabilities.storage.issue<&{FungibleToken.Receiver}>(/storage/flowUtilityTokenVault)
		self.account.capabilities.publish(capability_2, at: /public/flowUtilityTokenReceiver)
