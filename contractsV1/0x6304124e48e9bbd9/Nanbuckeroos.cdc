...
access(all)
contract Nanbuckeroos: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				Nanbuckeroos.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return Nanbuckeroos.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				Nanbuckeroos.maxSupply == nil || Nanbuckeroos.totalSupply <= Nanbuckeroos.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			Nanbuckeroos.totalSupply = Nanbuckeroos.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "Nanbuckeroos", symbol: "NBUCK", description: "A DAO of meaningless Nanbuckeroos holders", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: Nanbuckeroos.VaultStoragePath, receiverPath: Nanbuckeroos.ReceiverPublicPath, metadataPath: Nanbuckeroos.VaultPublicPath, receiverLinkedType: /private/NanbuckeroosVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-Nanbuckeroos.createEmptyVault(vaultType: Type<@Nanbuckeroos.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/NanbuckeroosVault
		self.ReceiverPublicPath = /public/NanbuckeroosReceiver
		self.VaultPublicPath = /public/NanbuckeroosMetadata
		self.MinterStoragePath = /storage/NanbuckeroosMinter
		self.AdministratorStoragePath = /storage/NanbuckeroosAdmin
