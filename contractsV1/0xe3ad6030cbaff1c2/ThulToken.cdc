...
access(all)
contract ThulToken: FungibleToken{ 
	...
	resource Minter{ 
		...
		///
		access(TMP_ENTITLEMENT_OWNER)
		fun mintTokens(amount: UFix64): @ThulToken.Vault{ 
			pre{ 
				amount > 0.0:
	...
	init(){ 
		...
		//
		var capability_2 = self.account.capabilities.storage.issue<&ThulToken.Vault>(self.VaultStoragePath)
