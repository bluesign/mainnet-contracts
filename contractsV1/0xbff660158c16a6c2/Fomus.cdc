...
access(all)
contract Fomus: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				Fomus.setBalance(address: owner, balance: self.balance)
			}
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				Fomus.maxSupply == nil || Fomus.totalSupply <= Fomus.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			Fomus.totalSupply = Fomus.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "Fomus", symbol: "FOM", description: "Dao for real holders of the Flow token. The future is in our hands.", externalURL: MetadataViews.ExternalURL("twitter.com/timon4room"), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("@timon4room"), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: Fomus.VaultStoragePath, receiverPath: Fomus.ReceiverPublicPath, metadataPath: Fomus.VaultPublicPath, receiverLinkedType: /private/FomusVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-Fomus.createEmptyVault(vaultType: Type<@Fomus.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/FomusVault
		self.ReceiverPublicPath = /public/FomusReceiver
		self.VaultPublicPath = /public/FomusMetadata
		self.MinterStoragePath = /storage/FomusMinter
		self.AdministratorStoragePath = /storage/FomusAdmin
