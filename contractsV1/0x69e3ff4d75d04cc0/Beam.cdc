...
contract Beam: NonFungibleToken{ 
	...
	struct CurrSeriesData{ 
		...
		init(){ 
			var referencedSeries = &Beam.seriesDatas[Beam.currentSeriesID] as &Beam.Series?
	...
	struct SetData{ 
		...
		init(setID: UInt32){ 
			var referencedSet = &Beam.sets[setID] as &Beam.Set?
	...
	resource Admin{ 
		...
		fun borrowSet(setID: UInt32): &Set{ 
			...
			return &Beam.sets[setID] as &Beam.Set?
		}
	...
	access(all)
	resource Collection: BeamCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic{ 
