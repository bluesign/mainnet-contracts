...
access(all)
contract Flowix: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				Flowix.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return Flowix.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				Flowix.maxSupply == nil || Flowix.totalSupply <= Flowix.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			Flowix.totalSupply = Flowix.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "Flowix", symbol: "FOX", description: "Flowix - token created for the Flow community.", externalURL: MetadataViews.ExternalURL("twitter.com/roomsterteam"), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("roomsterteam"), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: Flowix.VaultStoragePath, receiverPath: Flowix.ReceiverPublicPath, metadataPath: Flowix.VaultPublicPath, receiverLinkedType: /private/FlowixVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-Flowix.createEmptyVault(vaultType: Type<@Flowix.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/FlowixVault
		self.ReceiverPublicPath = /public/FlowixReceiver
		self.VaultPublicPath = /public/FlowixMetadata
		self.MinterStoragePath = /storage/FlowixMinter
		self.AdministratorStoragePath = /storage/FlowixAdmin
