...
contract Shard: NonFungibleToken{ 
	...
	resource Admin{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, clipID: UInt32){ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun batchMintNFT(recipient: &{NonFungibleToken.CollectionPublic}, clipID: UInt32, quantity: UInt64){ 
			var i: UInt64 = 0
			while i < quantity{ 
	...
	init(){ 
		...
		self.account.storage.save(<-collection, to: /storage/EternalShardCollection)
		
		// Create an Admin resource and save it to storage
		let admin <- create Admin()
		self.account.storage.save(<-admin, to: /storage/EternalShardAdmin)
		
		// Create a public capability for the collection
		var capability_1 = self.account.capabilities.storage.issue<&{NonFungibleToken.CollectionPublic}>(/storage/EternalShardCollection)
