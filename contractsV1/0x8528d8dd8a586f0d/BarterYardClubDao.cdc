...
access(all)
contract BarterYardClubDao: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				BarterYardClubDao.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return BarterYardClubDao.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				BarterYardClubDao.maxSupply == nil || BarterYardClubDao.totalSupply <= BarterYardClubDao.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			BarterYardClubDao.totalSupply = BarterYardClubDao.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreiaurtq3hlqp5covqafm43ibknrft27blwb3xaep4u7clag4ywiuka"), mediaType: "image")
				let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafybeia33yz2rawsswuw4ysctjj2zo3hjbp6kjus7u4d2hnbc7jxhpanoq"), mediaType: "image")
				let medias = MetadataViews.Medias([media, bannerMedia])
				return FungibleTokenMetadataViews.FTDisplay(name: "Barter Yard Club", symbol: "BYC", description: "Barter Yard Club is a DAO creating the next generation of internet tools.", externalURL: MetadataViews.ExternalURL("www.barteryard.club"), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("barteryard"), "discord": MetadataViews.ExternalURL("cq7YsnE6Dt")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: BarterYardClubDao.VaultStoragePath, receiverPath: BarterYardClubDao.ReceiverPublicPath, metadataPath: BarterYardClubDao.VaultPublicPath, receiverLinkedType: /private/BarterYardClubDaoVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-BarterYardClubDao.createEmptyVault(vaultType: Type<@BarterYardClubDao.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/BarterYardClubDaoVault
		self.ReceiverPublicPath = /public/BarterYardClubDaoReceiver
		self.VaultPublicPath = /public/BarterYardClubDaoMetadata
		self.MinterStoragePath = /storage/BarterYardClubDaoMinter
		self.AdministratorStoragePath = /storage/BarterYardClubDaoAdmin
