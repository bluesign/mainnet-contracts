...
access(all)
contract Baminerry: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				Baminerry.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return Baminerry.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				Baminerry.maxSupply == nil || Baminerry.totalSupply <= Baminerry.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			Baminerry.totalSupply = Baminerry.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "Baminerry", symbol: "BAMI", description: "This is a community project that is building based on community opinion", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: Baminerry.VaultStoragePath, receiverPath: Baminerry.ReceiverPublicPath, metadataPath: Baminerry.VaultPublicPath, receiverLinkedType: /private/BaminerryVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-Baminerry.createEmptyVault(vaultType: Type<@Baminerry.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/BaminerryVault
		self.ReceiverPublicPath = /public/BaminerryReceiver
		self.VaultPublicPath = /public/BaminerryMetadata
		self.MinterStoragePath = /storage/BaminerryMinter
		self.AdministratorStoragePath = /storage/BaminerryAdmin
