...
access(all)
contract wfqerwgqergqerg: NonFungibleToken, ViewResolver{ 
	...
	resource NFT: NonFungibleToken.NFT, ViewResolver.Resolver, NiftoryNonFungibleToken.NFTPublic{ 
		...
		view fun _contract(): &{NiftoryNonFungibleToken.ManagerPublic}{ 
			return wfqerwgqergqerg._contract()
		}
	...
	resource Manager: NiftoryNonFungibleToken.ManagerPublic, NiftoryNonFungibleToken.ManagerPrivate{ 
		...
		fun getNFTCollectionData(): MetadataViews.NFTCollectionData{ 
			return NiftoryNFTRegistry.buildNFTCollectionData(wfqerwgqergqerg.REGISTRY_ADDRESS, wfqerwgqergqerg.REGISTRY_BRAND, fun (): @{NonFungibleToken.Collection}{ 
					return <-wfqerwgqergqerg.createEmptyCollection(nftType: Type<@wfqerwgqergqerg.Collection>())
				})
		}
		...
		fun modifyContractMetadata(): &AnyStruct{ 
			...
			return (&wfqerwgqergqerg.metadata as &AnyStruct?)!
		}
		...
		fun _getMetadataViewsManagerPrivate(): &MetadataViewsManager.Manager{ 
			...
			let manager = wfqerwgqergqerg.account.capabilities.get<&MetadataViewsManager.Manager>(record.metadataViewsManager.paths.private).borrow()!
			return manager
		}
		...
		fun _getSetManagerPrivate(): &MutableMetadataSetManager.Manager{ 
			let record = NiftoryNFTRegistry.getRegistryRecord(wfqerwgqergqerg.REGISTRY_ADDRESS, wfqerwgqergqerg.REGISTRY_BRAND)
			let setManager = wfqerwgqergqerg.account.capabilities.get<&MutableMetadataSetManager.Manager>(record.setManager.paths.private).borrow()!
			return setManager
		}
	...
	init(nftManagerProxy: &{NiftoryNonFungibleTokenProxy.Public, NiftoryNonFungibleTokenProxy.Private}){ 
		...
		self.account.storage.save<@MutableMetadataSetManager.Manager>(<-MutableMetadataSetManager._create(name: "wfqerwgqergqerg", description: "The set manager for wfqerwgqergqerg."), to: record.setManager.paths.storage)
