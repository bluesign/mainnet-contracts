...
access(all)
contract SPIN: NonFungibleToken{ 
	access(all)
	...
	access(all)
	resource interface SPINCollectionPublic{ 
		access(all)
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowSPIN(id: UInt64): &SPIN.NFT?{ 
			post{ 
	...
	access(all)
	resource Collection: SPINCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		access(all)
	...
	resource NFTMinter{ 
		...
		fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, metadata:{ String: String}): &{NonFungibleToken.NFT}{ 
			...
			// create a new NFT
			var newNFT <- create NFT(id: SPIN.totalSupply, creator: creator, metadata: metadata)
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&SPIN.Collection>(self.CollectionStoragePath)
