...
access(all)
contract WATCHTOWER: NonFungibleToken{ 
	access(all)
	...
	access(all)
	resource interface WATCHTOWERCollectionPublic{ 
		access(all)
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowWATCHTOWER(id: UInt64): &WATCHTOWER.NFT?{ 
			post{ 
	...
	access(all)
	resource Collection: WATCHTOWERCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		access(all)
	...
	resource NFTMinter{ 
		...
		fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, metadata:{ String: String}): &{NonFungibleToken.NFT}{ 
			...
			// create a new NFT
			var newNFT <- create NFT(id: WATCHTOWER.totalSupply, creator: creator, metadata: metadata)
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&WATCHTOWER.Collection>(self.CollectionStoragePath)
