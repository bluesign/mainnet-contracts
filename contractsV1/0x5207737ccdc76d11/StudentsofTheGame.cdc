...
access(all)
contract StudentsofTheGame: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				StudentsofTheGame.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return StudentsofTheGame.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				StudentsofTheGame.maxSupply == nil || StudentsofTheGame.totalSupply <= StudentsofTheGame.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			StudentsofTheGame.totalSupply = StudentsofTheGame.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreie37b56et5kweaztkahef5iqkxuvniaskuldfh2fppbbdpmwhshje"), mediaType: "image")
				let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafybeido5dy5yhxl6qi2vajgwstbnqjwiy3ntu3p3gl4fbjaw67xu2mzhu"), mediaType: "image")
				let medias = MetadataViews.Medias([media, bannerMedia])
				return FungibleTokenMetadataViews.FTDisplay(name: "Students of The Game", symbol: "GAME", description: "The Students of The Game", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: StudentsofTheGame.VaultStoragePath, receiverPath: StudentsofTheGame.ReceiverPublicPath, metadataPath: StudentsofTheGame.VaultPublicPath, receiverLinkedType: /private/StudentsofTheGameVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-StudentsofTheGame.createEmptyVault(vaultType: Type<@StudentsofTheGame.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/StudentsofTheGameVault
		self.ReceiverPublicPath = /public/StudentsofTheGameReceiver
		self.VaultPublicPath = /public/StudentsofTheGameMetadata
		self.MinterStoragePath = /storage/StudentsofTheGameMinter
		self.AdministratorStoragePath = /storage/StudentsofTheGameAdmin
