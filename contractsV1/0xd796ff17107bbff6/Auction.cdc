...
contract Auction{ 
	...
	resource AuctionItem{ 
		...
		access(self)
		var recipientCollectionCap: Capability<&{Art.CollectionPublic}>?
		...
		access(self)
		let ownerCollectionCap: Capability<&{Art.CollectionPublic}>
		...
		access(contract)
		fun sendNFT(_ capability: Capability<&{Art.CollectionPublic}>){ 
	...
	resource AuctionCollection: AuctionPublic{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun createAuction(token: @Art.NFT, minimumBidIncrement: UFix64, auctionLength: UFix64, auctionStartTime: UFix64, startPrice: UFix64, collectionCap: Capability<&{Art.CollectionPublic}>, vaultCap: Capability<&{FungibleToken.Receiver}>){ 
			
			// create a new auction items resource container
			let item <- Auction.createStandaloneAuction(token: <-token, minimumBidIncrement: minimumBidIncrement, auctionLength: auctionLength, auctionStartTime: auctionStartTime, startPrice: startPrice, collectionCap: collectionCap, vaultCap: vaultCap)
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun placeBid(id: UInt64, bidTokens: @{FungibleToken.Vault}, vaultCap: Capability<&{FungibleToken.Receiver}>, collectionCap: Capability<&{Art.CollectionPublic}>){ 
			pre{ 
				self.auctionItems[id] != nil:
