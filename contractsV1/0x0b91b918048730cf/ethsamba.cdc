...
access(all)
contract ethsamba: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				ethsamba.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return ethsamba.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				ethsamba.maxSupply == nil || ethsamba.totalSupply <= ethsamba.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			ethsamba.totalSupply = ethsamba.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafybeiccp2t2gyldxunfzbr7qtp5dpu3fuakwdcwbs23ocga53ka2p3qbm"), mediaType: "image")
				let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafybeid2cwhtjp7hn4okpbwvam6llmblweggkxvrpyo7thub3y7zowdzyq"), mediaType: "image")
				let medias = MetadataViews.Medias([media, bannerMedia])
				return FungibleTokenMetadataViews.FTDisplay(name: "ethsamba", symbol: "SAMBA", description: "ETHSamba d\u{e1} as boas-vindas a entusiastas e vision\u{e1}rios do Ethereum de todo o Brasil para experimentar a magia da tecnologia blockchain atrav\u{e9}s de eventos emocionantes.", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: ethsamba.VaultStoragePath, receiverPath: ethsamba.ReceiverPublicPath, metadataPath: ethsamba.VaultPublicPath, receiverLinkedType: /private/ethsambaVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-ethsamba.createEmptyVault(vaultType: Type<@ethsamba.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/ethsambaVault
		self.ReceiverPublicPath = /public/ethsambaReceiver
		self.VaultPublicPath = /public/ethsambaMetadata
		self.MinterStoragePath = /storage/ethsambaMinter
		self.AdministratorStoragePath = /storage/ethsambaAdmin
