...
access(all)
contract ADUToken: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				ADUToken.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return ADUToken.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				ADUToken.maxSupply == nil || ADUToken.totalSupply <= ADUToken.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			ADUToken.totalSupply = ADUToken.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "All Day Unlimited", symbol: "ADU", description: "All Day Unlimited community DAO.", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("alldayunlimited")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: ADUToken.VaultStoragePath, receiverPath: ADUToken.ReceiverPublicPath, metadataPath: ADUToken.VaultPublicPath, receiverLinkedType: /private/ADUTokenVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-ADUToken.createEmptyVault(vaultType: Type<@ADUToken.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/ADUTokenVault
		self.ReceiverPublicPath = /public/ADUTokenReceiver
		self.VaultPublicPath = /public/ADUTokenMetadata
		self.MinterStoragePath = /storage/ADUTokenMinter
		self.AdministratorStoragePath = /storage/ADUTokenAdmin
