...
access(all)
contract MCVentureCapital: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				MCVentureCapital.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return MCVentureCapital.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				MCVentureCapital.maxSupply == nil || MCVentureCapital.totalSupply <= MCVentureCapital.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			MCVentureCapital.totalSupply = MCVentureCapital.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "MC Venture Capital", symbol: "MCVT", description: "Support MC Venture Capital Flow Incubator, a project that will create and launch several apps on the Flow blockchain, a fast, scalable, and user-friendly platform. Join us and help us make the web more open, decentralized, and accessible!", externalURL: MetadataViews.ExternalURL("www.mcventurecapital.com/incubator.php"), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("MCventureCap"), "discord": MetadataViews.ExternalURL("6skTVaZk3G")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: MCVentureCapital.VaultStoragePath, receiverPath: MCVentureCapital.ReceiverPublicPath, metadataPath: MCVentureCapital.VaultPublicPath, receiverLinkedType: /private/MCVentureCapitalVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-MCVentureCapital.createEmptyVault(vaultType: Type<@MCVentureCapital.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/MCVentureCapitalVault
		self.ReceiverPublicPath = /public/MCVentureCapitalReceiver
		self.VaultPublicPath = /public/MCVentureCapitalMetadata
		self.MinterStoragePath = /storage/MCVentureCapitalMinter
		self.AdministratorStoragePath = /storage/MCVentureCapitalAdmin
