...
access(all)
contract beta3LilaiNFT: NonFungibleToken, ViewResolver{ 
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: beta3LilaiNFT.CollectionStoragePath, publicPath: beta3LilaiNFT.CollectionPublicPath, publicCollection: Type<&beta3LilaiNFT.Collection>(), publicLinkedType: Type<&beta3LilaiNFT.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-beta3LilaiNFT.createEmptyCollection(nftType: Type<@beta3LilaiNFT.Collection>())
						})
	...
	///
	access(all)
	resource interface beta3LilaiNFTCollectionPublic{ 
		access(all)
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowbeta3LilaiNFT(id: UInt64): &beta3LilaiNFT.NFT?{ 
			post{ 
	...
	///
	access(all)
	resource Collection: beta3LilaiNFTCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		access(all)
		...
		view fun borrowViewResolver(id: UInt64): &{ViewResolver.Resolver}?{ 
			...
			return beta3LilaiNFT as &{ViewResolver.Resolver}
		}
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&beta3LilaiNFT.Collection>(self.CollectionStoragePath)
		self.account.capabilities.publish(capability_1, at: self.CollectionPublicPath)
		// Create and store the minter
		let minter <- create NFTMinter()
		self.account.storage.save(<-minter, to: self.MinterStoragePath)
		// Create a public capability for the minter
		var capability_2 = self.account.capabilities.storage.issue<&beta3LilaiNFT.NFTMinter>(self.MinterStoragePath)
