...
contract Elvn: FungibleToken{ 
	...
	resource Minter{ 
		...
		//
		access(TMP_ENTITLEMENT_OWNER)
		fun mintTokens(amount: UFix64): @Elvn.Vault{ 
			pre{ 
				amount > UFix64(0):
	...
	init(){ 
		...
		self.account.storage.save(<-vault, to: /storage/elvnVault)
		...
		self.account.storage.save(<-admin, to: /storage/elvnAdmin)
