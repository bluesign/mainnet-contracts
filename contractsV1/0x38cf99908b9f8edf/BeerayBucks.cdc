...
access(all)
contract BeerayBucks: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				BeerayBucks.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return BeerayBucks.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				BeerayBucks.maxSupply == nil || BeerayBucks.totalSupply <= BeerayBucks.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			BeerayBucks.totalSupply = BeerayBucks.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "Beeray Bucks", symbol: "BRAY", description: "A useless DAO with a worthless coin", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: BeerayBucks.VaultStoragePath, receiverPath: BeerayBucks.ReceiverPublicPath, metadataPath: BeerayBucks.VaultPublicPath, receiverLinkedType: /private/BeerayBucksVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-BeerayBucks.createEmptyVault(vaultType: Type<@BeerayBucks.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/BeerayBucksVault
		self.ReceiverPublicPath = /public/BeerayBucksReceiver
		self.VaultPublicPath = /public/BeerayBucksMetadata
		self.MinterStoragePath = /storage/BeerayBucksMinter
		self.AdministratorStoragePath = /storage/BeerayBucksAdmin
