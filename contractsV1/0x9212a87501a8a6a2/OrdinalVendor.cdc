...
contract OrdinalVendor{ 
	...
	access(contract)
	var restrictedIDs:{ UInt64: Bool}
	
	access(all)
	event ContractInitialized()
	
	access(all)
	event OrdinalPurchased(
		id: UInt64,
		type: String,
	...
	access(all)
	event OrdinalRestricted(id: UInt64)
	
	access(all)
	resource interface IMinter{ 
		access(all)
		fun mint(creator: Address, type: String, data: String): @{NonFungibleToken.NFT}
	}
	...
	struct AdminSignedPayload{ 
		...
		view fun toString(): String{ 
			return self.type.concat("-").concat(self.creator.toString()).concat("-").concat(
				self.expiration.toString()
			)
		}
	}
	
	access(all)
	resource interface VendorPublic{ 
		access(all)
		fun purchaseDomain(
		...
		access(all)
		fun getPaymentReceivers():{ String: Address}
	}
	
	access(all)
	resource Vendor: VendorPublic{ 
		access(self)
		...
		access(self)
		var paymentReceiverCaps:{ String: Capability<&{FungibleToken.Receiver}>}
		
		init(minterCap: Capability<&{IMinter}>, prices:{ String: PriceData}, paymentReceiverCaps:{ String: Capability<&{FungibleToken.Receiver}>}){ 
		...
		access(all)
		fun setPaymentReceiver(salePaymentVaultType: String, paymentReceiverCap: Capability<&{FungibleToken.Receiver}>){ 
			pre{ 
		...
		access(all)
		fun getPaymentReceiverAddress(salePaymentVaultType: String): Address?{ 
		...
		access(self)
		fun validateDomain(domain: String, ownerAddress: Address){ 
			var i = 0
			var index = -1
			...
			assert(domainExtension == "flow" || domainExtension == "flowverse" || domainExtension == "socks" || domainExtension == "shirt", message: "domain extension must be .flow, .flowverse, .socks, or .shirt")
			...
			if domainExtension == "flowverse"{ 
				let mysteryPassCollectionRef = getAccount(ownerAddress).capabilities.get<&{FlowversePass.CollectionPublic}>(FlowversePass.CollectionPublicPath).borrow<&{FlowversePass.CollectionPublic}>() ?? panic("FlowversePass Collection reference not found")
			...
			if domainExtension == "socks"{ 
				let socksCollectionRef = getAccount(ownerAddress).capabilities.get<&{FlowverseSocks.FlowverseSocksCollectionPublic}>(FlowverseSocks.CollectionPublicPath).borrow<&{FlowverseSocks.FlowverseSocksCollectionPublic}>() ?? panic("FlowverseSocks Collection reference not found")
			...
			if domainExtension == "shirt"{ 
				let shirtCollectionRef = getAccount(ownerAddress).capabilities.get<&{FlowverseShirt.CollectionPublic}>(FlowverseShirt.CollectionPublicPath).borrow<&{FlowverseShirt.CollectionPublic}>() ?? panic("FlowverseShirt Collection reference not found")
				assert(shirtCollectionRef.getIDs().length > 0, message: "ineligible for .shirt domain as user does not own a Flowverse Shirt")
			}
		}
		
		access(all)
		fun purchaseDomain(payment: @{FungibleToken.Vault}, data: PurchaseData, adminSignedPayload: AdminSignedPayload, signature: String){ 
			pre{ 
				data.type == "domain":
					"Invalid type (must be domain)"
				...
				self.verifyAdminSignedPayload(signedPayloadData: adminSignedPayload, signature: signature):
					"failed to validate signature for the purchase"
			}
		...
		access(all)
		fun purchaseText(payment: @{FungibleToken.Vault}, data: PurchaseData, adminSignedPayload: AdminSignedPayload, signature: String){ 
			pre{ 
				data.type == "text":
				...
				self.verifyAdminSignedPayload(signedPayloadData: adminSignedPayload, signature: signature):
					"failed to validate signature for the purchase"
				data.inscriptionData.length > 0:
					"text size must be greater than 0"
				data.inscriptionData.length <= 300000:
					"text must be less than or equal to 300KB"
			}
			self.handlePurchase(payment: <-payment, data: data)
		}
		
		access(all)
		fun purchaseImage(payment: @{FungibleToken.Vault}, data: PurchaseData, adminSignedPayload: AdminSignedPayload, signature: String){ 
			pre{ 
				data.type == "image":
					"Invalid type (must be image)"
				self.minterCap.borrow() != nil:
					"cannot borrow minter"
				adminSignedPayload.expiration >= UInt64(getCurrentBlock().timestamp):
					"expired signature"
		...
		access(self)
		fun handlePurchase(payment: @{FungibleToken.Vault}, data: PurchaseData){ 
			pre{ 
				data.type == "image" || data.type == "text" || data.type == "domain":
					"Invalid type (must be either image, text or domain)"
				self.minterCap.borrow() != nil:
					"cannot borrow minter"
				self.paymentReceiverCaps.containsKey(payment.getType().identifier):
					"payment receiver capability does not exist"
			}
		...
		access(all)
		fun updatePaymentReceiver(salePaymentVaultType: String, paymentReceiverCap: Capability<&{FungibleToken.Receiver}>){ 
			pre{ 
				paymentReceiverCap.borrow() != nil:
					"Could not borrow payment receiver capability"
			}
	...
	resource Admin{ 
		...
		access(all)
		fun addRestrictedID(id: UInt64){ 
			pre{ 
				!OrdinalVendor.restrictedIDs.containsKey(id):
					"ID already restricted"
			}
	...
	fun purchaseOrdinal(
		...
		pre{ 
			OrdinalVendor.vendor != nil:
				"Vendor has not been initialised"
	...
	access(all)
	fun getPrice(size: UInt64, type: String, salePaymentVaultType: String): UFix64{ 
		pre{ 
			size > 0:
	...
	access(all)
	fun getDomainSize(domain: String): UInt64{ 
		var i = 0
		for c in domain{ 
			if c == "."{ 
				break
	...
	access(all)
	fun checkDomainAvailability(domain: String): Bool{ 
		return OrdinalVendor.domains[domain] == nil
	}
	
	access(all)
	fun checkOrdinalRestricted(id: UInt64): Bool{ 
		return OrdinalVendor.restrictedIDs.containsKey(id)
	}
	
	access(all)
	struct OrdinalVendorInfo{ 
		access(all)
		let domains:{ String: UInt64}
		...
		access(all)
		let restrictedIDs:{ UInt64: Bool}
		
		access(all)
		let prices:{ String: OrdinalVendor.PriceData}
		...
		init(
			...
			prices:{ 
				String: OrdinalVendor.PriceData
			},
			paymentReceivers:{ 
				String: Address
			}
	...
	fun getInfo(): OrdinalVendorInfo?{ 
		if OrdinalVendor.vendor != nil{ 
			let vendor = (&OrdinalVendor.vendor as &Vendor?)!
			return OrdinalVendorInfo(domains: OrdinalVendor.domains, textIDs: OrdinalVendor.textIDs, imageIDs: OrdinalVendor.imageIDs, restrictedIDs: OrdinalVendor.restrictedIDs, prices: vendor.getPrices(), paymentReceivers: vendor.getPaymentReceivers())
		}
		return nil
	}
