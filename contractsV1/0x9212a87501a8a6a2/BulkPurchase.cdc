...
contract BulkPurchase{ 
	access(all)
	event BulkPurchaseCompleted(completedOrders: [CompletedPurchaseOrder])
	
	access(all)
	enum Storefront: UInt8{ 
		access(all)
		case StorefrontV1
	...
	access(all)
	fun getStorefrontFromIdentifier(_ identifier: String): Storefront{ 
		switch identifier{ 
	...
	access(all)
	fun getStorefrontIdentifier(_ storefront: Storefront): String{ 
		switch storefront{ 
	...
	struct CompletedPurchaseOrder{ 
		...
		access(all)
		var salePaymentVaultType: String
	...
	access(contract)
	fun getFlovatarMarketRef(address: Address): &FlovatarMarketplace.SaleCollection?{ 
		return (
	...
	fun processStorefrontV1Order(
		...
		let receiverCapability =
			...
			?? panic(
				"failed to get nft receiver capability for Storefront V1, NFT type: ".concat(
		...
		let receiver =
			...
			?? panic(
				"failed to borrow receiver for Storefront V1, NFT type: ".concat(
					listingDetails.nftType.identifier
				)
			)
	...
	fun processStorefrontV2Order(
		...
		let receiverCapability =
			nftReceiverCapabilities[listingDetails.nftType.identifier]
			?? panic(
				"failed to get nft receiver capability for Storefront V2, NFT type: ".concat(
					listingDetails.nftType.identifier
				)
			)
		let receiver =
			receiverCapability.borrow()
			?? panic(
				"failed to borrow receiver for Storefront V2, NFT type: ".concat(
					listingDetails.nftType.identifier
				)
			)
		...
		let payment <- paymentVaultRef.withdraw(amount: listingDetails.salePrice)
		let nft <- (listing!).purchase(payment: <-payment, commissionRecipient: commissionRecipient)
		receiver.deposit(token: <-nft)
		order.setSalePrice(listingDetails.salePrice)
		order.setSalePaymentVaultType(listingDetails.salePaymentVaultType)
		order.setNFTType(listingDetails.nftType)
	...
	fun processTopshotOrder(
		...
		let receiver = receiverCapability.borrow() ?? panic("failed to borrow receiver for TopShot")
		let buyTokens <- paymentVaultRef.withdraw(amount: salePrice) as! @DapperUtilityCoin.Vault
		let purchasedItem <-
			(topShotMarket!).purchase(tokenID: order.listingID, buyTokens: <-buyTokens)
	...
	fun purchase(
		...
		let topShotV1MarketRefs:{ Address: &Market.SaleCollection} ={} 
		let topShotV3MarketRefs:{ Address: &TopShotMarketV3.SaleCollection} ={} 
		let flovatarMarketRefs:{ Address: &FlovatarMarketplace.SaleCollection} ={} 
