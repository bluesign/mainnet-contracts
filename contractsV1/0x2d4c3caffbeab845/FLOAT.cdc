...
access(all)
contract FLOAT: NonFungibleToken, ViewResolver{ 
	/***********************************************/
	/******************** PATHS ********************/
	/***********************************************/
	access(all)
	...
	access(all)
	event FLOATPurchased(id: UInt64, eventHost: Address, eventId: UInt64, recipient: Address, serial: UInt64)
	...
	resource FLOATEvent: FLOATEventPublic, ViewResolver.Resolver{ 
		...
		access(self)
		fun setUserClaim(serial: UInt64, address: Address, floatId: UInt64){ 
			...
			let e = (&self.extraMetadata["userClaims"] as &AnyStruct?)!
		...
		// Type: Contract Setter
		access(self)
		fun setExtraFloatMetadata(serial: UInt64, metadata:{ String: AnyStruct}){ 
			...
			let e = (&self.extraMetadata["extraFloatMetadatas"] as &AnyStruct?)!
		...
		// Type: Contract Setter
		access(self)
		fun setSpecificExtraFloatMetadata(serial: UInt64, key: String, value: AnyStruct){ 
			if self.extraMetadata["extraFloatMetadatas"] == nil{ 
				let extraFloatMetadatas:{ UInt64: AnyStruct} ={} 
				self.extraMetadata["extraFloatMetadatas"] = extraFloatMetadatas
			}
			...
			let f = (extraFloatMetadatas[serial] as &AnyStruct?)!
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			case Type<MetadataViews.NFTCollectionData>():
				return MetadataViews.NFTCollectionData(storagePath: FLOAT.FLOATCollectionStoragePath, publicPath: FLOAT.FLOATCollectionPublicPath, publicCollection: Type<&Collection>(), publicLinkedType: Type<&Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
						return <-FLOAT.createEmptyCollection(nftType: Type<@FLOAT.Collection>())
					})
			case Type<MetadataViews.NFTCollectionDisplay>():
	...
	init(){ 
		...
		if self.account.storage.borrow<&FLOAT.Collection>(from: FLOAT.FLOATCollectionStoragePath) == nil{ 
			self.account.storage.save(<-FLOAT.createEmptyCollection(nftType: Type<@FLOAT.Collection>()), to: FLOAT.FLOATCollectionStoragePath)
			var capability_1 = self.account.capabilities.storage.issue<&FLOAT.Collection>(FLOAT.FLOATCollectionStoragePath)
			self.account.capabilities.publish(capability_1, at: FLOAT.FLOATCollectionPublicPath)
		}
		if self.account.storage.borrow<&FLOAT.FLOATEvents>(from: FLOAT.FLOATEventsStoragePath) == nil{ 
			...
			var capability_2 = self.account.capabilities.storage.issue<&FLOAT.FLOATEvents>(FLOAT.FLOATEventsStoragePath)
			self.account.capabilities.publish(capability_2, at: FLOAT.FLOATEventsPublicPath)
		}
		let FLOATEvents = self.account.storage.borrow<&FLOAT.FLOATEvents>(from: FLOAT.FLOATEventsStoragePath) ?? panic("Could not borrow the FLOATEvents from the signer.")
