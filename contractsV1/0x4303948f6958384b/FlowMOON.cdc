...
access(all)
contract FlowMoon: FungibleToken{ 
	access(all)
	...
	init(){ 
		...
		self.account.storage.save(<-create Minter(), to: FlowMoon.TokenMinterStoragePath)
		
		//
		// Create an Empty Vault for the Minter
		//
		self.account.storage.save(<-FlowMoon.initialMint(initialMintValue: self.totalSupply), to: FlowMoon.TokenVaultStoragePath)
		var capability_1 = self.account.capabilities.storage.issue<&FlowMoon.Vault>(FlowMoon.TokenVaultStoragePath)
		self.account.capabilities.publish(capability_1, at: FlowMoon.TokenVaultPublicPath)
	}
}
