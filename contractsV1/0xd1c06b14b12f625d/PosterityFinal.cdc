...
access(all)
contract PosterityFinal: NonFungibleToken, ViewResolver{ 
	...
	access(all)
	event Minted(id: UInt64, recipient: Address, randomID: UInt64)
	
	access(all)
	event SVGSuccess(randomID: UInt64, userAddress: Address)
	
	access(all)
	event StringAdded(randomID: UInt64, stringPiece: String)
	...
	resource SVGStorage{ 
		...
		access(all)
		fun addToSVG(stringPiece: String){ 
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<MetadataViews.Display>():
					return MetadataViews.Display(name: "metadata.creatorUsername", description: "Posterity description", thumbnail: MetadataViews.HTTPFile(url: " "))
	...
	access(all)
	resource Collection: PosterityFinalCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		access(all)
	...
	resource Administrator{ 
		...
		fun createSVGPath(_randomID: UInt64, _userAddress: Address){ 
			...
			PosterityFinal.account.storage.save(<-svg, to: path)
		...
		fun addToSVG(_randomID: UInt64, _stringPiece: String){ 
			let identifier = "PosteritySVGStorage_".concat(_randomID.toString())
			let path = StoragePath(identifier: identifier)!
			let SVGStorage = PosterityFinal.account.storage.borrow<&PosterityFinal.SVGStorage>(from: path)!
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			case Type<MetadataViews.NFTCollectionData>():
				return MetadataViews.NFTCollectionData(storagePath: PosterityFinal.CollectionStoragePath, publicPath: PosterityFinal.CollectionPublicPath, publicCollection: Type<&PosterityFinal.Collection>(), publicLinkedType: Type<&PosterityFinal.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
						return <-PosterityFinal.createEmptyCollection(nftType: Type<@PosterityFinal.Collection>())
					})
			case Type<MetadataViews.NFTCollectionDisplay>():
