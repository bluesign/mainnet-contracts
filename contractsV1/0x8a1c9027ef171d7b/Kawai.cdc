...
access(all)
contract Kawai: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				Kawai.setBalance(address: owner, balance: self.balance)
			}
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				Kawai.maxSupply == nil || Kawai.totalSupply <= Kawai.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			Kawai.totalSupply = Kawai.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "Kawai", symbol: "KAT", description: "Anime fan lovers", externalURL: MetadataViews.ExternalURL("tiktok.com/@kawaiianimemix"), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: Kawai.VaultStoragePath, receiverPath: Kawai.ReceiverPublicPath, metadataPath: Kawai.VaultPublicPath, receiverLinkedType: /private/KawaiVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-Kawai.createEmptyVault(vaultType: Type<@Kawai.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/KawaiVault
		self.ReceiverPublicPath = /public/KawaiReceiver
		self.VaultPublicPath = /public/KawaiMetadata
		self.MinterStoragePath = /storage/KawaiMinter
		self.AdministratorStoragePath = /storage/KawaiAdmin
