...
access(all)
contract NonboringDAO: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				NonboringDAO.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return NonboringDAO.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				NonboringDAO.maxSupply == nil || NonboringDAO.totalSupply <= NonboringDAO.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			NonboringDAO.totalSupply = NonboringDAO.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreihgq3gga2s3cnlw4mvvfc6cqbrpq5y4qoxsjeg6nd2noc3xjngb24"), mediaType: "image")
				let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreieq4ukswd77ho5yi55k6krlncl2tlhkuor5jsq54d6y37myvuzrry"), mediaType: "image")
				let medias = MetadataViews.Medias([media, bannerMedia])
				return FungibleTokenMetadataViews.FTDisplay(name: "Nonboring DAO", symbol: "NONBO", description: "A non-boring DAO for non-ordinal people", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: NonboringDAO.VaultStoragePath, receiverPath: NonboringDAO.ReceiverPublicPath, metadataPath: NonboringDAO.VaultPublicPath, receiverLinkedType: /private/NonboringDAOVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-NonboringDAO.createEmptyVault(vaultType: Type<@NonboringDAO.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/NonboringDAOVault
		self.ReceiverPublicPath = /public/NonboringDAOReceiver
		self.VaultPublicPath = /public/NonboringDAOMetadata
		self.MinterStoragePath = /storage/NonboringDAOMinter
		self.AdministratorStoragePath = /storage/NonboringDAOAdmin
