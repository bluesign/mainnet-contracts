...
contract Marketplace{ 
	...
	fun addListing(
		...
		let indexToInsertListingIDByTime =
			self.getIndexToAddListingIDByTime(item: item, items: self.listingIDsByTime)
		
		// all by price
		let indexToInsertListingIDByPrice =
			self.getIndexToAddListingIDByPrice(item: item, items: self.listingIDsByPrice)
	...
	fun removeItem(_ item: Item){ 
		let indexToRemoveListingIDByTime =
			self.getIndexToRemoveListingIDByTime(item: item, items: self.listingIDsByTime)
		let indexToRemoveListingIDByPrice =
			self.getIndexToRemoveListingIDByPrice(item: item, items: self.listingIDsByPrice)
	...
	access(contract)
	fun getIndexToAddListingIDByPrice(item: Item, items: [UInt64]): Int?{ 
		var startIndex = 0
	...
	access(contract)
	fun getIndexToAddListingIDByTime(item: Item, items: [UInt64]): Int{ 
	...
	access(contract)
	fun getIndexToRemoveListingIDByTime(item: Item, items: [UInt64]): Int?{ 
		var startIndex = 0
		var endIndex = items.length
		while startIndex < endIndex{ 
			var midIndex = startIndex + (endIndex - startIndex) / 2
	...
	// Run binary search to find the listing ID
	access(contract)
	fun getIndexToRemoveListingIDByPrice(item: Item, items: [UInt64]): Int?{ 
		var startIndex = 0
		var endIndex = items.length
		while startIndex < endIndex{ 
			var midIndex = startIndex + (endIndex - startIndex) / 2
	...
	access(contract)
	fun checkValidIndexToInsertByTime(item: Item, index: Int, items: [UInt64]){ 
		if index > 0{ 
	...
	access(contract)
	fun checkValidIndexToInsertByPrice(item: Item, index: Int, items: [UInt64]){ 
		if index > 0{ 
			let previousListingID = items[index - 1]
			let previousItem = self.listingIDItems[previousListingID]!
