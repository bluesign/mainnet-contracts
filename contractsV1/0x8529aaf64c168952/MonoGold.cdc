...
access(all)
contract MonoGold: NonFungibleToken{ 
	access(all)
	...
	access(all)
	resource interface MonoGoldCollectionPublic{ 
		access(all)
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowMonoGold(id: UInt64): &MonoGold.NFT?{ 
			post{ 
	...
	access(all)
	resource Collection: MonoGoldCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
	...
	resource NFTMinter{ 
		...
		fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, metadata:{ String: String}): &{NonFungibleToken.NFT}{ 
			...
			// create a new NFT
			var newNFT <- create NFT(id: MonoGold.totalSupply, metadata: metadata)
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&MonoGold.Collection>(self.CollectionStoragePath)
