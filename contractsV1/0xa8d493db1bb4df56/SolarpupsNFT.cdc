...
access(all)
contract SolarpupsNFT: NonFungibleToken{ 
	access(all)
	let SolarpupsNFTPublicPath: PublicPath
	
	access(all)
	let SolarpupsNFTPrivatePath: PrivatePath
	
	access(all)
	let SolarpupsNFTStoragePath: StoragePath
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			...
			switch view{ 
				...
				case Type<MetadataViews.Royalties>():
					...
					var royaltyReceiver = getAccount(0xcf99ac7bc594366c).capabilities.get<&{FungibleToken.Receiver}>(RECEIVER_PATH)
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: SolarpupsNFT.SolarpupsNFTStoragePath, publicPath: SolarpupsNFT.SolarpupsNFTPublicPath, publicCollection: Type<&SolarpupsNFT.Collection>(), publicLinkedType: Type<&SolarpupsNFT.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-SolarpupsNFT.createEmptyCollection(nftType: Type<@SolarpupsNFT.Collection>())
						})
				case Type<MetadataViews.NFTCollectionDisplay>():
					let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://cdn.solarpups.com/web/assets/img/solar-pups/logo.png"), mediaType: "image/png")
	...
	resource AssetRegistry{ 
		...
		fun store(asset: Asset){ 
			pre{ 
				SolarpupsNFT.assets[asset.assetId] == nil:
		...
		fun setMaxSupply(assetId: String){ 
			pre{ 
				SolarpupsNFT.assets[assetId] != nil:
					"asset not found"
			}
	...
	resource Collection: CollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowSolarpupsNFT(id: UInt64): &SolarpupsNFT.NFT{ 
			pre{ 
				self.ownedNFTs[id] != nil:
					"this NFT is nil"
			}
	...
	resource interface CollectionPublic{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowSolarpupsNFT(id: UInt64): &{NonFungibleToken.NFT}?{ 
			post{ 
	...
	access(account)
	fun getAsset(assetId: String): &SolarpupsNFT.Asset?{ 
		pre{ 
	...
	init(){ 
		...
		self.account.storage.save(<-create Collection(), to: self.SolarpupsNFTStoragePath)
		
		// create a public capability for the collection	
		var capability_1 = self.account.capabilities.storage.issue<&SolarpupsNFT.Collection>(self.SolarpupsNFTStoragePath)
		self.account.capabilities.publish(capability_1, at: self.SolarpupsNFTPublicPath)
