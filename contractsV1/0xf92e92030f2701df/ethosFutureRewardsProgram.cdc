...
access(all)
contract ethosFutureRewardsProgram: NonFungibleToken{ 
	...
	access(all)
	event Minted(id: UInt64, metadataId: UInt64)
	
	access(all)
	event BatchMint(metadataIds: [UInt64])
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: ethosFutureRewardsProgram.CollectionStoragePath, publicPath: ethosFutureRewardsProgram.CollectionPublicPath, publicCollection: Type<&Collection>(), publicLinkedType: Type<&Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-ethosFutureRewardsProgram.createEmptyCollection(nftType: Type<@ethosFutureRewardsProgram.Collection>())
						})
	...
	init(_collectionName: String, _description: String, _imagePath: String, _bannerImagePath: String?, _minting: Bool, _ipfsCID: String){ 
		...
		var capability_2 = self.account.capabilities.storage.issue<&ethosFutureRewardsProgram.Admin>(self.AdminStoragePath)
