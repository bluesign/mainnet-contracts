...
contract BloctoTokenStaking{ 
	...
	resource StakerRecord{ 
		...
		init(id: UInt64){ 
			...
			self.tokensCommitted <- BloctoToken.createEmptyVault(
					vaultType: Type<@BloctoToken.Vault>()
				)
				as!
				@BloctoToken.Vault
	...
	resource Admin{ 
		...
		fun payRewards(_ stakerIDs: [UInt64]){ 
			...
			let BloctoTokenMinter =
				BloctoTokenStaking.account.storage.borrow<&BloctoToken.Minter>(
			...
			var stakingRewardRecordsRefOpt =
				BloctoTokenStaking.account.storage.borrow<&{String: Bool}>(from: epochPath)
			if stakingRewardRecordsRefOpt == nil{ 
				BloctoTokenStaking.account.storage.save<{String: Bool}>({} as{ String: Bool}, to: epochPath)
				stakingRewardRecordsRefOpt = BloctoTokenStaking.account.storage.borrow<&{String: Bool}>(from: epochPath)
			}
	...
	access(account)
	fun borrowStakerRecord(_ stakerID: UInt64): &StakerRecord{ 
		pre{ 
	...
	fun getStakingRewardPath(epoch: UInt64): StoragePath{ 
		...
		return StoragePath(
			identifier: "bloctoTokenStakingStakingRewardRecords".concat("_").concat(
				epoch.toString()
			)
		)!
	}
