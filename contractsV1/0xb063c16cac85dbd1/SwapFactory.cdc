...
contract SwapFactory{ 
	...
	access(all)
	event FlashloanRateChanged(oldRateBps: UInt64, newRateBps: UInt64)
	
	access(all)
	event SwapFeeRateChanged(isStablePair: Bool, oldSwapRateBps: UInt64, newSwapRateBps: UInt64)
	
	access(all)
	event SwapProtocolFeeCutChanged(oldFeeCut: UFix64, newFeeCut: UFix64)
	...
	fun createPair(
		...
		assert(
			!stableMode && self.getPairAddress(token0Key: token0Key, token1Key: token1Key) == nil
			|| stableMode
			&& StableSwapFactory.getPairAddress(token0Key: token0Key, token1Key: token1Key) == nil,
			message: SwapError.ErrorEncode(
		...
		emit PairCreated(
			...
			stableMode: stableMode,
			numPairs: self.pairs.length + StableSwapFactory.getAllStableSwapPairsLength()
		)
		return pairAddress
	}
	...
	resource Admin{ 
		...
		fun setSwapFeeRateBps(rateBps: UInt64, isStable: Bool){ 
			...
			emit SwapFeeRateChanged(
				...
				oldSwapRateBps: SwapFactory.getSwapFeeRateBps(stableMode: isStable),
