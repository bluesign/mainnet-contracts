...
access(all)
contract LegaciCollectible: NonFungibleToken{ 
	
	// -----------------------------------------------------------------------
	// Legaci Collecible contract Events
	// -----------------------------------------------------------------------
	
	// Emitted when the Legaci Collectibles contract is created
	access(all)
	event ContractInitialized()
	
	// Emitted when a Legaci Collecible is minted
	access(all)
	event LegaciCollectibleMinted(legaciCollectibleId: UInt64, brandId: String, collectionId: String, unitId: String)
	...
	access(all)
	event LegaciCollectibleDestroyed(id: UInt64)
	...
	access(all)
	let LegaciCollectibleCollectionStoragePath: StoragePath
	
	// Path to store the Legaci collectibles collection
	// in a user's public account storage
	access(all)
	let LegaciCollectibleCollectionPublicPath: PublicPath
	
	// Path to admin storage for Legaci collectibles
	// currently serving to store the minter functionality
	access(all)
	let LegaciCollectibleAdminStoragePath: StoragePath
	...
	resource Brand{ 
		...
		access(all)
		var numberOfMintedCollectibles: UInt64
	...
	resource NFT: NonFungibleToken.NFT{ 
		...
		access(all)
		let data: LegaciCollectibleData
	...
	//
	access(all)
	resource Collection: LegaciCollectibleCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic{ 
		...
		fun withdraw(withdrawID: UInt64): @{NonFungibleToken.NFT}{ 
			...
			let token <- self.ownedNFTs.remove(key: withdrawID) ?? panic("Cannot withdraw: Legaci Collectible does not exist in the collection")
	...
	// -----------------------------------------------------------------------
	// Legaci Collectibles initialization function
	// -----------------------------------------------------------------------
	//
	init(){ 
		...
		self.account.storage.save<@Collection>(<-create Collection(), to: self.LegaciCollectibleCollectionStoragePath)
		
		// Create a public capability for the Collection
		var capability_1 = self.account.capabilities.storage.issue<&{LegaciCollectibleCollectionPublic}>(self.LegaciCollectibleCollectionStoragePath)
		self.account.capabilities.publish(capability_1, at: self.LegaciCollectibleCollectionPublicPath)
		
		// Put the Minter in storage
		self.account.storage.save<@Admin>(<-create Admin(), to: self.LegaciCollectibleAdminStoragePath)
