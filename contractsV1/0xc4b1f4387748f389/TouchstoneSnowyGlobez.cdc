...
access(all)
contract TouchstoneSnowyGlobez: NonFungibleToken{ 
	...
	resource NFT: NonFungibleToken.INFT, ViewResolver.Resolver{ 
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: TouchstoneSnowyGlobez.CollectionStoragePath, publicPath: TouchstoneSnowyGlobez.CollectionPublicPath, publicCollection: Type<&Collection>(), publicLinkedType: Type<&Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-TouchstoneSnowyGlobez.createEmptyCollection(nftType: Type<@TouchstoneSnowyGlobez.Collection>())
						})
		...
		init(_metadataId: UInt64, _serial: UInt64, _recipient: Address){ 
			...
			if let buyersRef = &TouchstoneSnowyGlobez.primaryBuyers[_recipient] as auth(Mutate) &{UInt64: [UInt64]}?{ 
	...
	resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		...
		access(all)
		fun claim(){ 
			if let storage = &TouchstoneSnowyGlobez.nftStorage[(self.owner!).address] as auth(Mutate) &{UInt64: NFT}?{ 
	...
	resource Administrator{ 
		...
		fun mintNFT(metadataId: UInt64, serial: UInt64, recipient: Address){ 
			...
			if let recipientCollection = (getAccount(recipient).capabilities.get<&TouchstoneSnowyGlobez.Collection>(TouchstoneSnowyGlobez.CollectionPublicPath)!).borrow(){ 
				recipientCollection.deposit(token: <-nft)
			} else if let storage = &TouchstoneSnowyGlobez.nftStorage[recipient] as auth(Mutate) &{UInt64: NFT}?{ 
				storage[nft.id] <-! nft
			} else{ 
	...
	fun getClaimableNFTs(user: Address):{ UInt64: NFTMetadata}{ 
		...
		if let storage = &TouchstoneSnowyGlobez.nftStorage[user] as auth(Mutate) &{UInt64: NFT}?{ 
			for id in storage.keys{ 
				let nftRef = (storage[id] as &TouchstoneSnowyGlobez.NFT?)!
