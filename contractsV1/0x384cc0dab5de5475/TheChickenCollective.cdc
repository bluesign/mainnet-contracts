...
access(all)
contract TheChickenCollective: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				TheChickenCollective.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return TheChickenCollective.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				TheChickenCollective.maxSupply == nil || TheChickenCollective.totalSupply <= TheChickenCollective.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			TheChickenCollective.totalSupply = TheChickenCollective.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafybeiepxnyi42l67jh4mcl3eodh2clrqrk44xys5aoyewzfeyoukvtv6e"), mediaType: "image")
				let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafybeiepxnyi42l67jh4mcl3eodh2clrqrk44xys5aoyewzfeyoukvtv6e"), mediaType: "image")
				let medias = MetadataViews.Medias([media, bannerMedia])
				return FungibleTokenMetadataViews.FTDisplay(name: "The Chicken Collective", symbol: "EGG", description: "A Farm Land Collective.For the $EGG", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: TheChickenCollective.VaultStoragePath, receiverPath: TheChickenCollective.ReceiverPublicPath, metadataPath: TheChickenCollective.VaultPublicPath, receiverLinkedType: /private/TheChickenCollectiveVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-TheChickenCollective.createEmptyVault(vaultType: Type<@TheChickenCollective.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/TheChickenCollectiveVault
		self.ReceiverPublicPath = /public/TheChickenCollectiveReceiver
		self.VaultPublicPath = /public/TheChickenCollectiveMetadata
		self.MinterStoragePath = /storage/TheChickenCollectiveMinter
		self.AdministratorStoragePath = /storage/TheChickenCollectiveAdmin
