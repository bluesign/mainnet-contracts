...
access(all)
contract FlowtyRentals{ 
	
	// FlowtyRentalsInitialized
	// This contract has been deployed
	access(all)
	event FlowtyRentalsInitialized()
	...
	//
	access(all)
	event FlowtyRentalsStorefrontInitialized(flowtyRentalsStorefrontResourceID: UInt64)
	...
	//
	access(all)
	event FlowtyRentalsStorefrontDestroyed(flowtyRentalsStorefrontResourceID: UInt64)
	...
	//
	access(all)
	event FlowtyRentalsMarketplaceInitialized(flowtyRentalsMarketplaceResourceID: UInt64)
	...
	//
	access(all)
	event FlowtyRentalsMarketplaceDestroyed(flowtyRentalsMarketplaceResourceID: UInt64)
	...
	access(all)
	let FlowtyRentalsStorefrontStoragePath: StoragePath
	
	access(all)
	let FlowtyRentalsMarketplaceStoragePath: StoragePath
	
	access(all)
	let FlowtyRentalsStorefrontPublicPath: PublicPath
	
	access(all)
	let FlowtyRentalsMarketplacePublicPath: PublicPath
	
	access(all)
	let FlowtyRentalsAdminStoragePath: StoragePath
	...
	struct ListingDetails{ 
		...
		access(all)
		fun getPaymentCut(): Flowty.PaymentCut{ 
	...
	resource interface ListingPublic{ 
		...
		fun rent(
			...
			renterNFTCollection: Capability<&{NonFungibleToken.CollectionPublic}>,
			renterNFTProvider: Capability<
		...
		//
		access(all)
		view fun remainingTimeToRent(): Fix64
		
		// isRentingEnabled
		// checks if a listing can be rented or not.
		//
		access(all)
		view fun isRentingEnabled(): Bool
	}
	...
	resource Listing: ListingPublic, FlowtyListingCallback.Listing{ 
		...
		access(contract)
		let ownerFungibleTokenReceiver: Capability<&{FungibleToken.Receiver}>
		...
		fun borrowNFT(): &{NonFungibleToken.NFT}{ 
			pre{ 
				self.nftProviderCapability.check():
					"provider capability failed check"
			}
		...
		// to be returned.
		access(all)
		fun rent(payment: @{FungibleToken.Vault}, renterFungibleTokenReceiver: Capability<&{FungibleToken.Receiver}>, renterNFTCollection: Capability<&{NonFungibleToken.CollectionPublic}>, renterNFTProvider: Capability<&{NonFungibleToken.CollectionPublic, NonFungibleToken.Provider}>?){ 
			pre{ 
				self.isRentingEnabled():
					"Renting is not enabled or this listing has expired"
				!self.details.rented:
					"listing has already been rented"
				payment.getType() == self.details.paymentVaultType:
				...
				renterNFTCollection.check():
					"renterNFTCollection failed check"
				renterNFTProvider == nil || (renterNFTProvider!).check():
					"renterNFTProvider failed check"
			}
			...
			let rental = marketplace.createRental(storefrontID: self.details.flowtyStorefrontID, listingResourceID: listingResourceID, nftID: self.details.nftID, nftType: self.details.nftType, paymentVaultType: self.details.paymentVaultType, term: self.details.term, listingDetails: self.details, ownerNFTCollectionPublic: self.nftPublicCollectionCapability, ownerFungibleTokenReceiver: self.ownerFungibleTokenReceiver, renterFungibleTokenReceiver: renterFungibleTokenReceiver, depositedFungibleTokens: <-depositCut, renterNFTCollection: renterNFTCollection, renterNFTProvider: renterNFTProvider)
			if let callback = FlowtyRentals.borrowCallbackContainer(){ 
				callback.handle(stage: FlowtyListingCallback.Stage.Filled, listing: &self as &{FlowtyListingCallback.Listing}, nft: ref)
				callback.handle(stage: FlowtyListingCallback.Stage.Created, listing: rental, nft: ref)
			}
			...
			let depositor = FlowtyRentals.account.storage.borrow<&LostAndFound.Depositor>(from: LostAndFound.DepositorStoragePath)!
			if royalties != nil{ 
				...
				FlowtyUtils.distributeRoyaltiesWithDepositor(royaltyCuts: royaltyCuts, depositor: depositor, vault: <-royaltyFeeCut)
			} else{ 
			...
			emit ListingRented(flowtyStorefrontAddress: (self.owner!).address, flowtyStorefrontID: self.details.flowtyStorefrontID, renterAddress: renterNFTCollection.address, listingResourceID: listingResourceID, rentalResourceID: rental.uuid, nftID: self.details.nftID, nftType: self.details.nftType.identifier, amount: self.details.amount, deposit: self.details.deposit, enabledAutomaticReturn: enabledAutomaticReturn)
		}
		...
		init(nftProviderCapability: Capability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>, nftPublicCollectionCapability: Capability<&{NonFungibleToken.CollectionPublic}>, ownerFungibleTokenReceiver: Capability<&{FungibleToken.Receiver}>, nftType: Type, nftID: UInt64, amount: UFix64, deposit: UFix64, term: UFix64, paymentVaultType: Type, paymentCut: Flowty.PaymentCut, storefrontID: UInt64, expiresAfter: UFix64, renter: Address?){ 
			pre{ 
				nftProviderCapability.check():
					"nftProviderCapability failed check"
				nftPublicCollectionCapability.check():
					"nftPublicCollectionCapability failed check"
				ownerFungibleTokenReceiver.check():
					"ownerFungibleTokenReceiver failed check"
			}
			...
			let nft = provider.borrowNFT(nftID)
			...
			self.details = ListingDetails(nftType: nftType, nftID: nftID, amount: amount, deposit: deposit, term: term, paymentVaultType: paymentVaultType, storefrontID: storefrontID, paymentCut: paymentCut, expiresAfter: expiresAfter, renter: renter, royaltyRate: royaltyRate)
			if let callback = FlowtyRentals.borrowCallbackContainer(){ 
				callback.handle(stage: FlowtyListingCallback.Stage.Created, listing: &self as &{FlowtyListingCallback.Listing}, nft: nft)
			}
		}
	}
	...
	access(all)
	resource Rental: RentalPublic, FlowtyListingCallback.Listing{ 
		...
		access(contract)
		var depositedFungibleTokens: @{FungibleToken.Vault}?
		
		// reference to the original owner of the nft which was rented
		access(contract)
		let ownerNFTCollectionPublic: Capability<&{NonFungibleToken.CollectionPublic}>
		...
		access(contract)
		let renterFungibleTokenReceiver: Capability<&{FungibleToken.Receiver}>
		
		// Capability used to transfer the rented nft to our renter
		access(contract)
		let renterNFTCollection: Capability<&{NonFungibleToken.CollectionPublic}>
		
		// optional capability to automatically return the asset on settlement
		access(contract)
		let renterNFTProvider: Capability<&{NonFungibleToken.CollectionPublic, NonFungibleToken.Provider}>?
		
		init(			 // Rental details
			 storefrontID: UInt64, listingResourceID: UInt64, paymentVaultType: Type, term: UFix64, nftID: UInt64, nftType: Type, listingDetails: ListingDetails,																																								  
																																								  // Rental resource
																																								  ownerNFTCollectionPublic: Capability<&{NonFungibleToken.CollectionPublic}>, renterFungibleTokenReceiver: Capability<&{FungibleToken.Receiver}>, ownerFungibleTokenReceiver: Capability<&{FungibleToken.Receiver}>, depositedFungibleTokens: @{FungibleToken.Vault}?, renterNFTCollection: Capability<&{NonFungibleToken.CollectionPublic}>, renterNFTProvider: Capability<&{NonFungibleToken.CollectionPublic, NonFungibleToken.Provider}>?){ 
			...
			self.details = RentalDetails(flowtyStorefrontID: storefrontID, listingResourceID: listingResourceID, paymentVaultType: paymentVaultType, nftType: nftType, nftID: nftID, term: term)
		}
		...
		fun returnNFT(nft: @{NonFungibleToken.NFT}){ 
			pre{ 
				!self.isRentalExpired():
					"rental has expired"
				!self.details.returned:
					"rental has been returned"
				nft.getType() == self.details.nftType:
					"returned nft has the wrong type"
			...
			let depositor = FlowtyRentals.account.storage.borrow<&LostAndFound.Depositor>(from: LostAndFound.DepositorStoragePath)!
			if let callback = FlowtyRentals.borrowCallbackContainer(){ 
				callback.handle(stage: FlowtyListingCallback.Stage.Completed, listing: &self as &{FlowtyListingCallback.Listing}, nft: &nft as &{NonFungibleToken.NFT})
			}
			
			// get the fungibleToken vault capable of receiving the deposit so it can be returned
			FlowtyUtils.trySendFungibleTokenVault(vault: <-deposit!, receiver: self.renterFungibleTokenReceiver, depositor: depositor)
			
			// return the nft to the owner's collection
			FlowtyUtils.trySendNFT(nft: <-nft, receiver: self.ownerNFTCollectionPublic, depositor: depositor)
			RoyaltiesLedger.remove(self.details.listingResourceID)
			emit RentalReturned(flowtyStorefrontAddress: self.ownerFungibleTokenReceiver.address, flowtyStorefrontID: self.details.flowtyStorefrontID, renterAddress: self.renterFungibleTokenReceiver.address, listingResourceID: self.details.listingResourceID, rentalResourceID: self.uuid, nftID: self.details.nftID, nftType: self.details.nftType.identifier)
		}
		...
		fun settleRental(){ 
			pre{ 
				self.isRentalExpired():
					"rental hasn't expired"
				self.details.returned == false:
					"rental has already been returned"
				self.details.settled == false:
					"rental has already been settled"
			}
			let depositor = FlowtyRentals.account.storage.borrow<&LostAndFound.Depositor>(from: LostAndFound.DepositorStoragePath)!
			...
			if self.renterNFTProvider != nil{ 
				...
				if (self.renterNFTProvider!).check(){ 
					...
					if ids.contains(self.details.nftID){ 
						let borrowedNFT = renterNFTProvider.borrowNFT(self.details.nftID)
						if borrowedNFT != nil && borrowedNFT.getType() == self.details.nftType && borrowedNFT.id == self.details.nftID{ 
							...
							if nft.getType() == self.details.nftType && nft.id == self.details.nftID{ 
								...
								// receiver is setup properly to receive the borrowed item back or not so we should just make a lostandfound 
								// item for them and move on. If they can mess with a borrowed item returning a type that is different than 
								// the actual withdrawn nft, they can handle redeeming their item back from the lostandfound.
								FlowtyUtils.depositToLostAndFound(redeemer: self.renterNFTCollection.address, item: <-nft, memo: nil, display: nil, depositor: depositor)
							}
						}
					}
				}
			...
			if self.depositedFungibleTokens != nil{ 
				...
				FlowtyUtils.trySendFungibleTokenVault(vault: <-v, receiver: self.listingDetails.getPaymentCut().receiver, depositor: depositor)
			}
			RoyaltiesLedger.remove(self.details.listingResourceID)
			if let callback = FlowtyRentals.borrowCallbackContainer(){ 
				callback.handle(stage: FlowtyListingCallback.Stage.Completed, listing: &self as &{FlowtyListingCallback.Listing}, nft: nil)
			}
			emit RentalSettled(rentalResourceID: self.uuid, listingResourceID: self.details.listingResourceID, renter: self.renterFungibleTokenReceiver.address, lender: self.ownerFungibleTokenReceiver.address, nftID: self.details.nftID, nftType: self.details.nftType.identifier, deposit: self.listingDetails.deposit)
		}
	...
	//
	access(all)
	resource interface FlowtyRentalsMarketplaceManager{ 
		// createFunding
		...
		fun createRental(
			...
			ownerNFTCollectionPublic: Capability<&{NonFungibleToken.CollectionPublic}>,
		...
		//
		access(all)
		fun removeRental(rentalResourceID: UInt64)
		
		access(all)
		fun borrowPrivateRental(rentalResourceID: UInt64): &Rental?
	}
	
	access(all)
	resource interface FlowtyRentalsMarketplacePublic{ 
		access(all)
		fun getRentalIDs(): [UInt64]
		
		access(all)
		fun borrowRental(rentalResourceID: UInt64): &Rental?
	}
	
	access(all)
	resource FlowtyRentalsMarketplace:
		FlowtyRentalsMarketplaceManager,
		FlowtyRentalsMarketplacePublic{
	
		access(self)
		var rentals: @{UInt64: Rental}
		...
		access(all)
		fun removeRental(rentalResourceID: UInt64){ 
			let rental <- self.rentals.remove(key: rentalResourceID) ?? panic("missing Rental")
			if let callback = FlowtyRentals.borrowCallbackContainer(){ 
				callback.handle(stage: FlowtyListingCallback.Stage.Destroyed, listing: &rental as &{FlowtyListingCallback.Listing}, nft: nil)
			}
			assert(
				rental.getDetails().returned == true || rental.getDetails().settled == true,
				message: "rental is not returned or settled"
			)
			destroy rental
		}
	...
	access(all)
	resource interface FlowtyRentalsStorefrontManager{ 
		access(all)
		fun createListing(
	...
	access(all)
	resource interface FlowtyRentalsStorefrontPublic{ 
		access(all)
	...
	access(all)
	resource FlowtyRentalsStorefront:
		FlowtyRentalsStorefrontManager,
		FlowtyRentalsStorefrontPublic{
		...
		fun createListing(
			...
			pre{ 
				...
				paymentCut.receiver.check() && (paymentCut.receiver.borrow()!).getType() == paymentVaultType:
					"paymentCut receiver type and paymentVaultType do not match"
				nftProviderCapability.check():
					"invalid nft provider"
			}
			let ref = (nftProviderCapability.borrow()!).borrowNFT(nftID)
			assert(FlowtyUtils.isSupported(ref!), message: "nft type is not supported")
			let royalties =
				ref.resolveView(Type<MetadataViews.Royalties>()) as! MetadataViews.Royalties?
		...
		fun cleanup(listingResourceID: UInt64){ 
			...
			assert(
				listing.getDetails().rented == true,
				message: "listing is not rented, only admin can remove"
			)
			destroy listing
		}
	...
	access(all)
	fun createStorefront(): @FlowtyRentalsStorefront{ 
		return <-create FlowtyRentalsStorefront()
	}
	
	access(account)
	fun borrowMarketplace(): &FlowtyRentals.FlowtyRentalsMarketplace{ 
		return self.account.storage.borrow<&FlowtyRentals.FlowtyRentalsMarketplace>(
			from: FlowtyRentals.FlowtyRentalsMarketplaceStoragePath
		)!
	}
	
	access(all)
	fun borrowMarketplacePublic(): &FlowtyRentalsMarketplace{ 
		let mp =
			self.account.storage.borrow<&FlowtyRentalsMarketplace>(
				from: FlowtyRentals.FlowtyRentalsMarketplaceStoragePath
			)!
		return mp
	}
	...
	access(all)
	fun settleRental(rentalResourceID: UInt64){ 
		let marketplace = FlowtyRentals.borrowMarketplace()
	...
	access(all)
	resource FlowtyRentalsAdmin{ 
		access(all)
		fun setFees(rentalFee: UFix64){ 
			pre{ 
				rentalFee <= 1.0:
	...
	init(){ 
		...
		self.SuspendedFundingPeriod = 1.0 // Period in seconds until the listing is valid
		...
		var capability_1 =
			self.account.capabilities.storage.issue<&FlowtyRentals.FlowtyRentalsMarketplace>(
				FlowtyRentals.FlowtyRentalsMarketplaceStoragePath
			)
		self.account.capabilities.publish(
			capability_1,
			at: FlowtyRentals.FlowtyRentalsMarketplacePublicPath
		)
