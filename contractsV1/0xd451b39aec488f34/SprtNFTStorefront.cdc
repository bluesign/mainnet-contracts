...
contract SprtNFTStorefront{ 
	...
	resource Storefront: StorefrontManager, StorefrontPublic{ 
		...
		fun cleanup(listingResourceID: UInt64){ 
			...
			for key in self.listings.keys{ 
				let otherListing = (&self.listings[key] as &Listing?)!
				if self.cmpNFT(listing: &listing as &Listing, listing2: otherListing){ 
	...
	struct FeeInfo{ 
		access(all)
		let minimumPrice: UFix64
		
		access(all)
		let maximumPrice: UFix64
		
		access(all)
		let priceTickSize: UFix64
		
		access(all)
		let baseFeeRate: UFix64
		
		access(all)
		let feeRateTickSize: UFix64
