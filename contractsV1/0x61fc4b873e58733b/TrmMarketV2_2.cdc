/**
	...
	- A TrmAssetV2_2.Collection capability that allows their sale to withdraw an asset when it is purchased.
...
contract TrmMarketV2_2{ 
	...
	// Event that emitted when the sale collection is initialized
	access(all)
	event SaleCollectionInitialized(userAccountAddress: Address)
	
	/// Emitted when an Asset is listed for transfer
	access(all)
	event AssetListedForTransfer(
		assetTokenID: UInt64,
		salePrice: UFix64?,
		auctionPrice: UFix64?,
	...
	/// Emitted when an Asset is listed for transfer
	access(all)
	event AssetBatchListedForTransfer(
		assetTokenIDs: [
			UInt64
		],
		salePrice: UFix64?,
	...
	access(all)
	event AssetTransferListingChanged(
		assetTokenID: UInt64,
		salePrice: UFix64?,
		auctionPrice: UFix64?,
		auctionStartTimestamp: UFix64?,
	...
	/// Emitted when the sale price of a listed asset has changed
	access(all)
	event AssetBatchTransferListingChanged(
		assetTokenIDs: [
			UInt64
		],
		salePrice: UFix64?,
	...
	/// Emitted when the rent price of a listed asset has changed
	access(all)
	event AssetBatchRentListingChanged(
		assetTokenIDs: [
			UInt64
		],
		price: UFix64?,
	...
	access(all)
	event AssetDelistedForTransfer(assetTokenID: UInt64, owner: Address?)
	
	/// Emitted when a token has been delisted for sale
	access(all)
	event AssetBatchDelistedForTransfer(assetTokenIDs: [UInt64], owner: Address?)
	
	/// Emitted when a token has been delisted for rent
	access(all)
	event AssetDelistedForRent(assetTokenID: UInt64, owner: Address?)
	
	/// Emitted when a token has been delisted for rent
	access(all)
	event AssetBatchDelistedForRent(assetTokenIDs: [UInt64], owner: Address?)
	...
	access(all)
	let marketPrivatePath: PrivatePath
	...
	access(all)
	var minterCapability: Capability<&TrmRentV2_2.Minter>
	
	access(all)
	resource RentListing{ 
		access(all)
		var price: UFix64
		
		access(all)
		var rentalPeriodSeconds: UFix64
		
		access(contract)
		fun setRentalPeriodSeconds(seconds: UFix64){ 
	...
	resource interface SalePublic{ 
		...
		access(contract)
		fun cancelTransfer(tokenID: UInt64)
		
		access(contract)
		fun cancelRent(tokenID: UInt64)
		
		access(contract)
		fun batchCancelTransfer(tokenIDs: [UInt64])
		...
		fun transfer(
			tokenID: UInt64,
			adminAccount: &TrmAssetV2_2.Admin,
		...
		fun rent(
			tokenID: UInt64,
			price: UFix64,
			recipient: Capability<&TrmRentV2_2.Collection>,
			paymentID: String
		): UInt64
	...
	resource SaleCollection: SalePublic{ 
		...
		access(self)
		var ownerCollection: Capability<&TrmAssetV2_2.Collection>
		
		/// Dictionary of the transfer prices for each NFT by ID
		access(self)
		var transferListings: @{UInt64: TransferListing}
		
		/// Dictionary of the rent prices for each NFT by ID
		access(self)
		var rentListings: @{UInt64: RentListing}
		
		init(ownerCollection: Capability<&TrmAssetV2_2.Collection>){ 
			pre{ 
		...
		access(contract)
		fun listForTransfer(tokenID: UInt64, salePrice: UFix64?, auctionPrice: UFix64?, auctionStartTimestamp: UFix64?, auctionPeriodSeconds: UFix64?){ 
			pre{ 
		...
		///			 auctionPeriodSeconds: The period for which auction will last
		access(contract)
		fun batchListForTransfer(tokenIDs: [UInt64], salePrice: UFix64?, auctionPrice: UFix64?, auctionStartTimestamp: UFix64?, auctionPeriodSeconds: UFix64?){ 
		...
		///			 paymentID
		access(contract)
		fun transfer(tokenID: UInt64, adminAccount: &TrmAssetV2_2.Admin, recipientAddress: Address, price: UFix64, paymentID: String){ 
			pre{ 
		...
		access(contract)
		fun rent(tokenID: UInt64, price: UFix64, recipient: Capability<&TrmRentV2_2.Collection>, paymentID: String): UInt64{ 
			pre{ 
			...
			let minterRef = TrmMarketV2_2.minterCapability.borrow()!
	...
	resource Admin{ 
			...
			let saleCollectionCapability =
				(
					getAccount(saleCollectionAddress).capabilities.get<
						&TrmMarketV2_2.SaleCollection
					>(TrmMarketV2_2.marketPublicPath)!
				).borrow()
				?? panic(
					"Could not borrow sale collection capability from provided sale collection address"
				)
			saleCollectionCapability.listForTransfer(
				tokenID: tokenID,
				salePrice: salePrice,
				auctionPrice: auctionPrice,
				auctionStartTimestamp: auctionStartTimestamp,
				auctionPeriodSeconds: auctionPeriodSeconds
			)
		}
		...
		fun listForRent(
			...
			saleCollectionCapability.listForRent(
				tokenID: tokenID,
				price: price,
		...
		fun batchListForTransfer(
			...
			saleCollectionCapability.batchListForTransfer(
				tokenIDs: tokenIDs,
				salePrice: salePrice,
				auctionPrice: auctionPrice,
				auctionStartTimestamp: auctionStartTimestamp,
		...
		fun batchListForRent(
			...
			saleCollectionCapability.batchListForRent(
				tokenIDs: tokenIDs,
				price: price,
				rentalPeriodSeconds: rentalPeriodSeconds
			)
		...
		fun transfer(
			saleCollectionAddress: Address,
			tokenID: UInt64,
			assetAdminResource: &TrmAssetV2_2.Admin,
			recipientAddress: Address,
			price: UFix64,
			paymentID: String
		){ 
			...
			saleCollectionCapability.transfer(
				tokenID: tokenID,
				adminAccount: assetAdminResource,
		...
		fun rent(
			...
			return saleCollectionCapability.rent(
				tokenID: tokenID,
				price: price,
				recipient: recipient,
				paymentID: paymentID
	...
	init(){ 
		// Settings paths
		self.marketStoragePath = /storage/TrmMarketV2_2SaleCollection
		self.marketPublicPath = /public/TrmMarketV2_2SaleCollection
		self.marketPrivatePath = /private/TrmMarketV2_2SaleCollection
		self.adminStoragePath = /storage/TrmMarketV2_2Admin
		...
		self.minterCapability = self.account.capabilities.get<&TrmRentV2_2.Minter>(
				TrmRentV2_2.minterPrivatePath
			)!
