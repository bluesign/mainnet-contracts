...
contract TrmMarketV2_1_1{ 
	...
	access(all)
	var minterCapability: Capability<&TrmRentV2_1.Minter>
	...
	resource interface SalePublic{ 
		...
		fun transfer(
			tokenID: UInt64,
			adminAccount: &TrmAssetV2_1.Admin,
		...
		fun getTransferListing(tokenID: UInt64): &TrmMarketV2_1_1.TransferListing?
	...
	resource SaleCollection: SalePublic{ 
		...
		access(self)
		var ownerCollection: Capability<&TrmAssetV2_1.Collection>
		...
		///			 paymentID
		access(contract)
		fun transfer(tokenID: UInt64, adminAccount: &TrmAssetV2_1.Admin, recipientAddress: Address, price: UFix64, paymentID: String){ 
			pre{ 
		...
		access(contract)
		fun rent(tokenID: UInt64, price: UFix64, recipient: Capability<&{TrmRentV2_1.CollectionPublic}>, paymentID: String): UInt64{ 
			pre{ 
	...
	fun createSaleCollection(
		ownerCollection: Capability<&TrmAssetV2_1.Collection>
	): @SaleCollection{ 
	...
	init(){ 
		...
		self.minterCapability = self.account.capabilities.get<&TrmRentV2_1.Minter>(
				TrmRentV2_1.minterPrivatePath
			)!
