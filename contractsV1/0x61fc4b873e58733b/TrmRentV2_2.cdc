...
access(all)
contract TrmRentV2_2: NonFungibleToken{ 
	// -----------------------------------------------------------------------
	...
	access(all)
	event RentCollectionInitialized(userAccountAddress: Address)
	...
	access(TMP_ENTITLEMENT_OWNER)
	fun emitCreateEmptyRentCollectionEvent(userAccountAddress: Address){ 
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&TrmRentV2_2.Minter>(TrmRentV2_2.minterStoragePath)
