...
access(all)
contract LemonToken: FungibleToken{ 
	...
	resource Minter{ 
		...
		///
		access(TMP_ENTITLEMENT_OWNER)
		fun mintTokens(amount: UFix64): @LemonToken.Vault{ 
			pre{ 
				amount > 0.0:
	...
	init(){ 
		...
		//
		var capability_2 = self.account.capabilities.storage.issue<&LemonToken.Vault>(self.VaultStoragePath)
