...
access(all)
contract UJNFTAC: NonFungibleToken{ 
	access(all)
	...
	access(all)
	resource interface UJNFTACCollectionPublic{ 
		access(all)
		...
		fun borrowUJNFTAC(id: UInt64): &UJNFTAC.NFT?{ 
			post{ 
	...
	access(all)
	resource Collection: UJNFTACCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic{ 
	...
	resource NFTMinter{ 
		...
		fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, name: String, description: String, thumbnail: String){ 
			...
			// create a new NFT
			var newNFT <- create NFT(id: UJNFTAC.totalSupply, name: name, description: description, thumbnail: thumbnail)
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&UJNFTAC.Collection>(self.CollectionStoragePath)
