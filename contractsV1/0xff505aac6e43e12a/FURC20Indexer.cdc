...
contract FURC20Indexer{ 
	...
	access(all)
	event FURC20Deployed(tick: String, max: UFix64, limit: UFix64, deployer: Address)
	
	/// Event emitted when a FURC20 token is minted
	access(all)
	event FURC20Minted(tick: String, amount: UFix64, to: Address)
	
	/// Event emitted when the owner of an inscription is updated
	access(all)
	event FURC20Transfer(tick: String, from: Address, to: Address, amount: UFix64)
	
	/// Event emitted when a FURC20 token is burned
	access(all)
	event FURC20Burned(tick: String, amount: UFix64, from: Address, flowExtracted: UFix64)
	
	/// Event emitted when a FURC20 token is withdrawn as change
	access(all)
	event FURC20WithdrawnAsChange(tick: String, amount: UFix64, from: Address)
	
	/// Event emitted when a FURC20 token is deposited from change
	access(all)
	event FURC20DepositedFromChange(tick: String, amount: UFix64, to: Address, from: Address)
	
	/// Event emitted when a FURC20 token is set to be burnable
	access(all)
	event FURC20BurnableSet(tick: String, burnable: Bool)
	
	/// Event emitted when a FURC20 token is burned unsupplied tokens
	access(all)
	event FURC20UnsuppliedBurned(tick: String, amount: UFix64)
	...
	resource interface IndexerPublic{ 
		...
		access(all)
		view fun getTokenMeta(tick: String): FURC20Meta?
		...
		access(all)
		view fun isValidFURC20Inscription(ins: &FlowUp.Inscription): Bool
		...
		access(all)
		fun transfer(ins: &FlowUp.Inscription, flowToCreator: @FlowToken.Vault)
		
		/// Burn a FURC20 token
		access(all)
		fun burn(ins: &FlowUp.Inscription, flowToCreator: @FlowToken.Vault): @FlowToken.Vault
		
		/** ---- Account Methods for readonly ---- */
		/// Parse the metadata of a FURC20 inscription
		access(account)
		view fun parseMetadata(_ data: &FlowUp.InscriptionData):{ String: String}
	...
	resource InscriptionIndexer: IndexerPublic{ 
		...
		///
		access(all)
		fun sponsorship(amount: UFix64, to: Capability<&FlowToken.Vault>, forTick: String){ 
			pre{ 
				amount > 0.0:
		...
		///
		access(all)
		fun mint(ins: &FlowUp.Inscription, flowToCreator: @FlowToken.Vault, flowToInviter: @FlowToken.Vault, inviterAddress: Address){ 
			pre{ 
				ins.isExtractable():
					"The inscription is not extractable"
				self.isValidFURC20Inscription(ins: ins):
		...
		/// Apply a listed order, maker and taker should be the same token and the same amount
		access(account)
		fun applyBuyNowOrder(makerIns: &FlowUp.Inscription, takerIns: &FlowUp.Inscription, maxAmount: UFix64, change: @FURC20FTShared.Change): @FURC20FTShared.Change{ 
			pre{ 
		...
		/// Apply a listed order, maker and taker should be the same token and the same amount
		access(account)
		fun applySellNowOrder(makerIns: &FlowUp.Inscription, takerIns: &FlowUp.Inscription, maxAmount: UFix64, change: @FURC20FTShared.Change, _ distributeFlowTokenFunc: fun (UFix64, @FlowToken.Vault): Bool): @FURC20FTShared.Change{ 
			pre{ 
				makerIns.isExtractable():
					"The MAKER inscription is not extractable"
				takerIns.isExtractable():
		...
		fun extractFlowVaultChangeFromInscription(_ ins: &FlowUp.Inscription, amount: UFix64): @FURC20FTShared.Change{ 
			...
			// return the change
			return <-FURC20FTShared.createChange(tick: "", // empty tick means this change is a FLOW token change														   
														   from: (ins.owner!).address, // Pay $FLOW to the buyer and get the FURC20 token																					   
																					   balance: nil, ftVault: <-vault)
		}
		...
		fun _buildFURC20SaleCuts(sellerAddress: Address?): [FURC20FTShared.SaleCut]{ 
			...
			ret.append(FURC20FTShared.SaleCut(type: FURC20FTShared.SaleCutType.TokenTreasury, ratio: salesFee * treasuryPoolCut, receiver: nil))
			ret.append(FURC20FTShared.SaleCut(type: FURC20FTShared.SaleCutType.PlatformTreasury, ratio: salesFee * platformTreasuryCut, receiver: nil))
			ret.append(FURC20FTShared.SaleCut(type: FURC20FTShared.SaleCutType.PlatformStakers, ratio: salesFee * platformStakersCut, receiver: nil))
			ret.append(FURC20FTShared.SaleCut(type: FURC20FTShared.SaleCutType.MarketplacePortion, ratio: salesFee * marketplacePortionCut, receiver: nil))
			...
			if sellerAddress != nil{ 
				...
				ret.append(FURC20FTShared.SaleCut(type: FURC20FTShared.SaleCutType.BuyTaker, ratio: 1.0 - salesFee, receiver: nil))
			}
		...
		///
		access(self)
		fun _withdrawToTokenChange(tick: String, fromAddr: Address, amt: UFix64): @FURC20FTShared.Change{ 
			post{ 
		...
		///
		access(self)
		fun _depositFromTokenChange(change: @FURC20FTShared.Change, to: Address){ 
			pre{ 
				change.isBackedByVault() == false:
					"The change should not be backed by a vault"
			}
		...
		///
		access(self)
		view fun _isOwnedByIndexer(_ ins: &FlowUp.Inscription): Bool{ 
		...
		///
		access(self)
		view fun borrowTokenMeta(tick: String): &FURC20Meta{ 
