...
access(all)
contract TicalUniverse: NonFungibleToken{ 
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: TicalUniverse.CollectionStoragePath, publicPath: TicalUniverse.CollectionPublicPath, publicCollection: Type<&TicalUniverse.Collection>(), publicLinkedType: Type<&TicalUniverse.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-TicalUniverse.createEmptyCollection(nftType: Type<@TicalUniverse.Collection>())
						})
	...
	access(all)
	resource Collection: TicalUniverseCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
	...
	fun getCollectionDisplay(): MetadataViews.NFTCollectionDisplay{ 
		...
		return MetadataViews.NFTCollectionDisplay(name: "Tical Universe", description: "The Genesis NFT drop represents the dawn of Method Man\u{2019}s Tical Universe and the birth and introduction of the original Tical Universe heroes and villains.", externalURL: MetadataViews.ExternalURL("https://www.tunegonft.com/collection-details/95487a67-a66a-43d5-913d-46fa8a644f4c"), squareImage: media, bannerImage: media, socials: socials)
	}
	...
	init(){ 
		...
		self.account.storage.save<@Collection>(<-create Collection(), to: TicalUniverse.CollectionStoragePath)
		
		// Create a public capability for the Collection
		var capability_1 = self.account.capabilities.storage.issue<&{TicalUniverseCollectionPublic}>(TicalUniverse.CollectionStoragePath)
		self.account.capabilities.publish(capability_1, at: TicalUniverse.CollectionPublicPath)
		
		// Put the Admin in storage
		self.account.storage.save<@Admin>(<-create Admin(), to: TicalUniverse.AdminStoragePath)
