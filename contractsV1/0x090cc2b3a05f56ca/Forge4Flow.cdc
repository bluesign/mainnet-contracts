...
access(all)
contract Forge4Flow: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				Forge4Flow.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return Forge4Flow.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				Forge4Flow.maxSupply == nil || Forge4Flow.totalSupply <= Forge4Flow.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			Forge4Flow.totalSupply = Forge4Flow.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "Forge4Flow", symbol: "FORGE", description: "Forge Secure Web3 DApp's On Flow", externalURL: MetadataViews.ExternalURL("forge4flow.com"), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("forge4flow"), "discord": MetadataViews.ExternalURL("z7GCDeVhYG")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: Forge4Flow.VaultStoragePath, receiverPath: Forge4Flow.ReceiverPublicPath, metadataPath: Forge4Flow.VaultPublicPath, receiverLinkedType: /private/Forge4FlowVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-Forge4Flow.createEmptyVault(vaultType: Type<@Forge4Flow.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/Forge4FlowVault
		self.ReceiverPublicPath = /public/Forge4FlowReceiver
		self.VaultPublicPath = /public/Forge4FlowMetadata
		self.MinterStoragePath = /storage/Forge4FlowMinter
		self.AdministratorStoragePath = /storage/Forge4FlowAdmin
