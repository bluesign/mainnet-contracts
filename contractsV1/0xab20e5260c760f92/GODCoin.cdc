...
access(all)
contract GODCoin: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				GODCoin.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return GODCoin.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				GODCoin.maxSupply == nil || GODCoin.totalSupply <= GODCoin.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			GODCoin.totalSupply = GODCoin.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "GOD Coin", symbol: "GOD", description: "\u{1f680} Dive into the cosmic hilarity with $GOD Coin on Flow Blockchain \u{1f310} \u{2013} Your gateway to decentralized laughter and interstellar fun! \u{1f52e} Join the memevolution \u{2013} grab $GOD now for a universe of good vibes! \u{1f30c}", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("FlowGOD23"), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: GODCoin.VaultStoragePath, receiverPath: GODCoin.ReceiverPublicPath, metadataPath: GODCoin.VaultPublicPath, receiverLinkedType: /private/GODCoinVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-GODCoin.createEmptyVault(vaultType: Type<@GODCoin.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/GODCoinVault
		self.ReceiverPublicPath = /public/GODCoinReceiver
		self.VaultPublicPath = /public/GODCoinMetadata
		self.MinterStoragePath = /storage/GODCoinMinter
		self.AdministratorStoragePath = /storage/GODCoinAdmin
