...
contract Gomoku{ 
	...
	// Bets
	access(account)
	let hostOpeningBetMap: @{UInt32: FlowToken.Vault}
	
	access(account)
	let challengerOpeningBetMap: @{UInt32: FlowToken.Vault}
	
	access(account)
	let hostRaisedBetMap: @{UInt32: FlowToken.Vault}
	
	access(account)
	let challengerRaisedBetMap: @{UInt32: FlowToken.Vault}
	
	// Bets value when finalize
	access(account)
	let hostFinalizedOpeningBetMap:{ UInt32: UFix64}
	
	access(account)
	let challengerFinalizedOpeningBetMap:{ UInt32: UFix64}
	
	access(account)
	let hostFinalizedRaisedBetMap:{ UInt32: UFix64}
	
	access(account)
	let challengerFinalizedRaisedBetMap:{ UInt32: UFix64}
	
	// Events
	// Event be emitted when the composition is created
	access(all)
	event HostOpeningBet(balance: UFix64)
	
	// Event be emitted when the contract is created
	access(all)
	event CompositionMatched(
		host: Address,
		challenger: Address,
		currency: String,
		openingBet: UFix64
	...
	access(all)
	event CompositionCreated(host: Address, currency: String)
	...
	access(all)
	event Surrender(id: UInt32, from: Address)
	
	access(all)
	event CollectionNotFound(type: Type, path: Path, address: Address)
	
	access(all)
	event ResourceNotFound(id: UInt32, type: Type, address: Address)
	
	access(all)
	event MakeMove(compositionId: UInt32, locationX: Int8, locationY: Int8, stoneColor: UInt8)
	
	access(all)
	event RoundSwitch(compositionId: UInt32, previous: UInt8, next: UInt8)
	...
	resource Admin{ 
		...
		fun recycleBets(){ 
			...
			for key in Gomoku.hostOpeningBetMap.keys{ 
				var hostOpeningBet: @FlowToken.Vault? <- FlowToken.createEmptyVault(vaultType: Type<@FlowToken.Vault>()) as! @FlowToken.Vault
			...
			for key in Gomoku.challengerOpeningBetMap.keys{ 
				var challengerOpeningBet: @FlowToken.Vault? <- FlowToken.createEmptyVault(vaultType: Type<@FlowToken.Vault>()) as! @FlowToken.Vault
			...
			for key in Gomoku.hostRaisedBetMap.keys{ 
				var hostRaisedBet: @FlowToken.Vault? <- FlowToken.createEmptyVault(vaultType: Type<@FlowToken.Vault>()) as! @FlowToken.Vault
			...
			for key in Gomoku.challengerRaisedBetMap.keys{ 
				var challengerRaisedBet: @FlowToken.Vault? <- FlowToken.createEmptyVault(vaultType: Type<@FlowToken.Vault>()) as! @FlowToken.Vault
	...
	fun getFinalizedBets(by index: UInt32): [UFix64]?{ 
		if let roundWinners = self.getCompositionRef(by: index)?.getRoundWinners(){ 
			...
			return [self.hostFinalizedOpeningBetMap[index] ?? UFix64(0), self.challengerFinalizedOpeningBetMap[index] ?? UFix64(0), self.hostFinalizedRaisedBetMap[index] ?? UFix64(0), self.challengerFinalizedRaisedBetMap[index] ?? UFix64(0)]
		} else{ 
			return nil
		}
	}
	...
	fun register(
		...
		pre{ 
			identityCollectionRef.owner?.address == host:
				"You are not authorized to move other's Gomoku identity collection."
			resultCollectionRef.owner?.address == host:
				"You are not authorized to move other's Gomoku result collection."
			compositionCollectionRef.owner?.address == host:
				"You are not authorized to move other's Gomoku composition collection."
		}
		...
		if self.hostOpeningBetMap.keys.contains(index){ 
			...
			var tempVault: @FlowToken.Vault <- FlowToken.createEmptyVault(vaultType: Type<@FlowToken.Vault>()) as! @FlowToken.Vault
		...
		let composition <-
			Gomoku.createComposition(id: index, host: host, boardSize: 15, totalRound: 2)
	...
	fun matchOpponent(
		...
		challenger: Address,
		bet: @FlowToken.Vault,
		recycleBetVaultRef: &FlowToken.Vault,
		identityCollectionRef: &GomokuIdentity.IdentityCollection,
		resultCollectionRef: &GomokuResult.ResultCollection
	): Bool{ 
		if let matchedHost = MatchContract.match(index: index, challengerAddress: challenger){ 
			...
			if let compositionCollectionRef = getAccount(matchedHost).capabilities.get<&Gomoku.CompositionCollection>(self.CollectionPublicPath).borrow(){ 
				...
				self.challengerRaisedBetMap[index] <-! FlowToken.createEmptyVault(vaultType: Type<@FlowToken.Vault>()) as! @FlowToken.Vault
				if let compositionRef = compositionCollectionRef.borrow(id: index){ 
					self.challengerOpeningBetMap[index] <-! bet
					compositionRef.match(identityCollectionRef: identityCollectionRef, challenger: challenger)
					...
					emit CompositionMatched(host: matchedHost, challenger: challenger, currency: Type<FlowToken>().identifier, openingBet: hostOpeningBet! + challengerOpeningBet!)
					return true
				} else{ 
	...
	resource Composition{ 
		...
		access(all)
		var blockHeightTimeout: UInt64
		
		access(self)
		var winner: GomokuType.Role?
		
		access(self)
		var host: Address
		
		access(self)
		var challenger: Address?
		
		access(self)
		var roundWinners: [GomokuType.Result]
		
		access(self)
		var steps: @[[Stone]]
		
		access(self)
		var locationStoneMaps: [{String: GomokuType.StoneColor}]
		
		access(self)
		var destroyable: Bool
		
		init(id: UInt32, host: Address, boardSize: UInt8, totalRound: UInt8){ 
			pre{ 
				totalRound >= 2:
					"Total round should be 2 to take turns to make first move (black stone) for fairness."
				totalRound % 2 == 0:
					"Total round should be event number to take turns to make first move (black stone) for fairness."
			}
			self.id = id
			self.host = host
			...
			emit CompositionCreated(host: host, currency: Type<FlowToken>().identifier)
		}
		...
		fun getStoneData(_for round: UInt8): [StoneData]{ 
			pre{ 
				self.steps.length > Int(round):
					"Round ".concat(round.toString()).concat(" not exist.")
			}
			var placeholderArray: @[Stone] <- []
			self.steps[round] <-> placeholderArray
			var placeholderStone <-
				create Stone(
		...
		fun makeMove(
			...
			// check identity
			let identityTokenRef =
				identityCollectionRef.borrow(id: self.id) as &GomokuIdentity.IdentityToken?
			assert(identityTokenRef != nil, message: "Identity token ref not found.")
			assert(
				identityTokenRef?.owner?.address == identityTokenRef?.address,
				message: "Identity token should not be transfer to other."
			)
			let identityToken <-
				identityCollectionRef.withdraw(by: self.id)
				?? panic("You are not suppose to make this move.")
			assert(Int(self.currentRound) + 1 > self.roundWinners.length, message: "Game Over.")
			
			// check result
			assert(
				resultCollectionRef.owner?.address == identityTokenRef?.address,
				message: "Gomoku result collection not found."
			)
			let stone <- create Stone(color: identityToken.stoneColor, location: location)
			
			// check raise bet type
			assert(
				raisedBet.getType() == Type<@FlowToken.Vault>(),
				message: "You can onlty raise bet with the same token of opening bet: ".concat(
					raisedBet.getType().identifier
				)
			)
			...
			switch currentRole{ 
				...
				case GomokuType.Role.challenger:
					...
					assert(
						identityToken.address == self.challenger!,
						message: "It's not you turn yet!"
					)
					var emptyBet: @FlowToken.Vault <-
						FlowToken.createEmptyVault(vaultType: Type<@FlowToken.Vault>())
						as!
			...
			let hasRoundWinner =
				self.checkWinnerInAllDirection(
			...
			} else if UInt8(self.steps[self.currentRound].length) == self.boardSize * self.boardSize{ 
		...
		fun surrender(
			...
			pre{ 
				self.roundWinners.length == Int(self.currentRound):
					"Current round index should be equal to number of round winners."
			}
			...
			let identityToken <-
				identityCollectionRef.withdraw(by: self.id)
				?? panic("Identity token ref not found.")
			switch identityToken.role{ 
				case GomokuType.Role.host:
		...
		access(account)
		fun match(identityCollectionRef: &GomokuIdentity.IdentityCollection, challenger: Address){ 
			pre{ 
				self.challenger == nil:
					"Already matched."
			}
		...
		access(account)
		fun finalizeByTimeout(
			identityToken: @GomokuIdentity.IdentityToken
		): @GomokuIdentity.IdentityToken?{ 
			pre{ 
				getCurrentBlock().height > self.getTimeout():
					"Let's give opponent more time to think......"
			}
		...
		// Private Method
		access(self)
		fun finalize(identityToken: @GomokuIdentity.IdentityToken){ 
			pre{ 
				self.roundWinners.length == Int(self.totalRound):
					"Game not over yet!"
				self.challenger != nil:
					"Challenger not found."
				Gomoku.hostOpeningBetMap.keys.contains(identityToken.id):
					"Host's OpeningBet not found."
				Gomoku.challengerOpeningBetMap.keys.contains(identityToken.id):
					"Challenger's OpeningBet not found."
				Gomoku.hostRaisedBetMap.keys.contains(identityToken.id):
					"Host's RaisedBet not found."
				Gomoku.challengerRaisedBetMap.keys.contains(identityToken.id):
					"Challenger's RaisedBet not found."
			}
			...
			// withdraw reward
			let tatalVault <-
				FlowToken.createEmptyVault(vaultType: Type<@FlowToken.Vault>()) as! @FlowToken.Vault
			let hostOpeningBet: @FlowToken.Vault?
				<- Gomoku.hostOpeningBetMap[identityToken.id]
				<- FlowToken.createEmptyVault(vaultType: Type<@FlowToken.Vault>())
				as!
				@FlowToken.Vault
			Gomoku.hostFinalizedOpeningBetMap[identityToken.id] = hostOpeningBet?.balance
				?? UFix64(0)
			...
			let challengerOpeningBet: @FlowToken.Vault?
				<- Gomoku.challengerOpeningBetMap[identityToken.id]
				<- FlowToken.createEmptyVault(vaultType: Type<@FlowToken.Vault>())
				as!
				@FlowToken.Vault
			Gomoku.challengerFinalizedOpeningBetMap[identityToken.id] = challengerOpeningBet
					?.balance
				?? UFix64(0)
			...
			let hostRaisedBet: @FlowToken.Vault?
				<- Gomoku.hostRaisedBetMap[identityToken.id]
				<- FlowToken.createEmptyVault(vaultType: Type<@FlowToken.Vault>())
				as!
				@FlowToken.Vault
			Gomoku.hostFinalizedRaisedBetMap[identityToken.id] = hostRaisedBet?.balance ?? UFix64(0)
			let challengerRaisedBet: @FlowToken.Vault?
				<- Gomoku.challengerRaisedBetMap[identityToken.id]
				<- FlowToken.createEmptyVault(vaultType: Type<@FlowToken.Vault>())
				as!
				@FlowToken.Vault
			Gomoku.challengerFinalizedRaisedBetMap[identityToken.id] = challengerRaisedBet?.balance
				?? UFix64(0)
			...
			let devReward: @FlowToken.Vault <-
				FlowToken.createEmptyVault(vaultType: Type<@FlowToken.Vault>()) as! @FlowToken.Vault
			let hostReward: @FlowToken.Vault <-
				FlowToken.createEmptyVault(vaultType: Type<@FlowToken.Vault>()) as! @FlowToken.Vault
			let challengerReward: @FlowToken.Vault <-
				FlowToken.createEmptyVault(vaultType: Type<@FlowToken.Vault>()) as! @FlowToken.Vault
			let winnerReward: @FlowToken.Vault <-
				FlowToken.createEmptyVault(vaultType: Type<@FlowToken.Vault>()) as! @FlowToken.Vault
			let losserReward: @FlowToken.Vault <-
				FlowToken.createEmptyVault(vaultType: Type<@FlowToken.Vault>()) as! @FlowToken.Vault
			...
			switch result{ 
				case GomokuType.Result.hostWins:
					...
					let winnerRewardBalance =
						tatalVault.balance - devRewardBalance - hostRewardBalance
				...
				case GomokuType.Result.draw:
					...
					let devRewardBalance =
						tatalVault.balance - hostRewardBalance - challengerRewardBalance
					devReward.deposit(from: <-tatalVault.withdraw(amount: devRewardBalance))
					hostReward.deposit(from: <-tatalVault.withdraw(amount: hostRewardBalance))
					challengerReward.deposit(
						from: <-tatalVault.withdraw(amount: challengerRewardBalance)
					)
					destroy tatalVault
				default:
					panic("Should not be the case.")
			...
			if identityToken.address == self.host{ 
				if let identityCollectionRef = getAccount(self.challenger!).capabilities.get<&GomokuIdentity.IdentityCollection>(GomokuIdentity.CollectionPublicPath).borrow(){ 
					if let challengerIdentityToken <- identityCollectionRef.withdraw(by: identityTokenId){ 
						...
						emit ResourceNotFound(id: identityTokenId, type: Type<@GomokuIdentity.IdentityToken>(), address: self.challenger!)
					}
				} else{ 
				...
				if let identityCollectionRef = getAccount(self.host).capabilities.get<&GomokuIdentity.IdentityCollection>(GomokuIdentity.CollectionPublicPath).borrow(){ 
					if let hostIdentityToken <- identityCollectionRef.withdraw(by: identityTokenId){ 
						...
						emit ResourceNotFound(id: identityTokenId, type: Type<@GomokuIdentity.IdentityToken>(), address: self.host)
					}
				} else{ 
					emit CollectionNotFound(type: Type<@GomokuIdentity.IdentityCollection>(), path: GomokuIdentity.CollectionPublicPath, address: self.host)
				}
			}
		...
		fun mintCompositionResults(
			...
			while index < self.totalRound{ 
				steps.append(self.getStoneData(_for: index))
				index = index + UInt8(1)
			}
			...
			switch result{ 
				case GomokuType.Result.hostWins:
					...
					let winnerResultToken <-
						GomokuResult.createResult(
							id: id,
		...
		fun getRole(): GomokuType.Role{ 
			...
			} else				   // first move is host if index is odd
				   if roundSteps.length % 2 == 0{ 
				// step for host
				return GomokuType.Role.host
			} else{ 
		...
		fun switchRound(){ 
			pre{ 
				self.roundWinners[self.currentRound] != nil:
					"Current round winner not decided."
				self.totalRound > self.currentRound + 1:
					"Next round should not over totalRound."
			}
			post{ 
				self.currentRound == before(self.currentRound) + 1:
					"fatal error."
				self.roundWinners.length == Int(self.currentRound):
					"Should not have winner right after switching rounds."
			}
			...
			emit RoundSwitch(compositionId: self.id, previous: previous, next: self.currentRound)
		}
		
		access(self)
		fun verifyAndStoreStone(stone: @Stone){ 
			pre{ 
				self.steps.length == 2:
					"Steps length should be 2."
				Int(self.currentRound) <= 1:
					"Composition only has 2 round each."
			}
			let roundSteps = &self.steps[self.currentRound] as &[{GomokuType.Stoning}]
			
			// check stone location is within board.
			let isOnBoard = self.verifyOnBoard(location: stone.location)
			assert(
				isOnBoard,
				message: "Stone location".concat(stone.location.description()).concat(
					" is invalid."
				)
			)
			...
			assert(
				self.locationStoneMaps[self.currentRound][stone.key()] == nil,
				message: "This place had been taken."
			)
			if roundSteps.length % 2 == 0{ 
				// black stone move
		...
		access(self)
		fun verifyOnBoard(location: GomokuType.StoneLocation): Bool{ 
		...
		fun checkWinner(
			...
			switch direction{ 
				case GomokuType.VerifyDirection.vertical:
					while !isFinished && shift <= Int8(4) && center.x - shift >= Int8(0){ 
				...
				case GomokuType.VerifyDirection.horizontal:
					while !isFinished && shift <= Int8(4) && center.y - shift >= Int8(0){ 
				...
				case GomokuType.VerifyDirection.diagonal:
					...
					&& center.y - shift >= Int8(0){ 
						let currentCheckedLocation =
							GomokuType.StoneLocation(x: center.x - shift, y: center.y - shift)
						if let color =
							self.locationStoneMaps[self.currentRound][
								currentCheckedLocation.key()
							]{ 
							if color == targetColor{ 
								countInRow = countInRow + UInt8(1)
							} else{ 
		...
		access(self)
		fun getWinnerResult(): GomokuType.Result{ 
			pre{ 
				self.roundWinners.length == Int(self.totalRound):
					"Game not over yet!"
			}
	...
	fun createComposition(
		...
		let Composition <-
			create Composition(id: id, host: host, boardSize: boardSize, totalRound: totalRound)
		return <-Composition
	}
	
	access(all)
	resource CompositionCollection{ 
		access(all)
		...
		access(self)
		var ownedCompositionMap: @{UInt32: Gomoku.Composition}
		...
		access(account)
		fun withdraw(by id: UInt32): @Gomoku.Composition{ 
		...
		access(account)
		fun deposit(token: @Gomoku.Composition){ 
			let token <- token
	...
	access(all)
	struct StoneData: GomokuType.StoneDataing{ 
		access(all)
		let color: GomokuType.StoneColor
	...
	access(all)
	resource Stone: GomokuType.Stoning{ 
		access(all)
		let color: GomokuType.StoneColor
