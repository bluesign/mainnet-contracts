...
access(all)
contract TixologiTickets: NonFungibleToken{ 
	...
	resource Event{ 
		...
		fun mintTicket(ticketTypeID: UInt32, metadata: String?): @NFT{ 
			...
			// Mint the new ticket
			let newTicket: @NFT <- create NFT(serialNumber: numInTicketType + UInt32(1), ticketTypeID: ticketTypeID, eventID: self.eventID, metadata: metadata)
	...
	struct QueryEventData{ 
		...
		init(eventID: UInt32){ 
			...
			let _event = (&TixologiTickets.events[eventID] as &Event?)!
	...
	//
	access(all)
	struct TixologiTicketMetadataView{ 
		access(all)
	...
	resource NFT: NonFungibleToken.NFT, ViewResolver.Resolver{ 
		...
		view fun getViews(): [Type]{ 
			return [Type<MetadataViews.Display>(), Type<TixologiTicketMetadataView>()]
		}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				...
				case Type<TixologiTicketMetadataView>():
					return TixologiTicketMetadataView(eventName: TixologiTickets.getEventName(eventID: self.data.eventID), ticketTypeName: TixologiTickets.getTicketTypeName(ticketTypeID: self.data.ticketTypeID), venue: TixologiTickets.getEventVenue(eventID: self.data.eventID), startDate: TixologiTickets.getEventStartDate(eventID: self.data.eventID), serialNumber: self.data.serialNumber, ticketTypeID: self.data.ticketTypeID, eventID: self.data.eventID, numTicketsInTicketType: TixologiTickets.getNumTicketsInTicketType(eventID: self.data.eventID, ticketTypeID: self.data.ticketTypeID))
			}
			return nil
		}
	...
	//
	access(all)
	resource Collection: TixologiTicketsCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		...
		view fun borrowViewResolver(id: UInt64): &{ViewResolver.Resolver}?{ 
			...
			return tixologiTicketNFT as &{ViewResolver.Resolver}
		}
	...
	init(){ 
		...
		self.account.storage.save<@Collection>(<-create Collection(), to: /storage/TixologiTicketsCollection)
		
		// Create a public capability for the Collection
		var capability_1 = self.account.capabilities.storage.issue<&{TixologiTicketsCollectionPublic}>(/storage/TixologiTicketsCollection)
		self.account.capabilities.publish(capability_1, at: /public/TixologiTicketsCollection)
		
		// Put the Minter in storage
		self.account.storage.save<@Admin>(<-create Admin(), to: /storage/TixologiTicketAdmin)
