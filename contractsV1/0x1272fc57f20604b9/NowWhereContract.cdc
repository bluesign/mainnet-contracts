...
contract NowWhereContract{ 
	...
	access(all)
	event DropUpdated(dropId: UInt64, startDate: UFix64, endDate: UFix64)
	...
	struct Drop{ 
		...
		access(all)
		var startDate: UFix64
		
		access(all)
		var endDate: UFix64
		
		access(contract)
		var templates:{ UInt64: AnyStruct}
		...
		fun updateDrop(startDate: UFix64?, endDate: UFix64?, templates:{ UInt64: AnyStruct}?){ 
			pre{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun getDropTemplates():{ UInt64: AnyStruct}{ 
