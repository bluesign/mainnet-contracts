...
access(all)
contract AeraPack: NonFungibleToken{ 
	// Events
	access(all)
	...
	access(all)
	event PackReveal(packId: UInt64, address: Address, packTypeId: UInt64, rewardId: UInt64, rewardType: String, rewardFields:{ String: String}, nftPerPack: Int, packTier: String)
	...
	resource NFT: NonFungibleToken.NFT, ViewResolver.Resolver{ 
		...
		view fun getViews(): [Type]{ 
			return [Type<MetadataViews.Display>(), Type<MetadataViews.NFTCollectionData>(), Type<MetadataViews.Traits>(), Type<Metadata>(), Type<String>()]
		}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			...
			switch view{ 
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: AeraPack.CollectionStoragePath, publicPath: AeraPack.CollectionPublicPath, publicCollection: Type<&AeraPack.Collection>(), publicLinkedType: Type<&AeraPack.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-AeraPack.createEmptyCollection(nftType: Type<@AeraPack.Collection>())
						})
				case Type<MetadataViews.Traits>():
					return MetadataViews.Traits([MetadataViews.Trait(name: "pack_type_id", value: self.typeId, displayType: "Number", rarity: nil)])
				case Type<String>():
		...
		fun claimFreePackWithReservation(packId: UInt64, signature: String, collectionCapability: Capability<&Collection>){ 
			pre{ 
				(self.owner!).address == AeraPack.account.address:
					"You can only buy pack directly from the AeraPack account"
			}
	...
	fun fulfill(packId: UInt64, rewardIds: [UInt64], salt: String){ 
		let openedPacksCollection = AeraPack.account.storage.borrow<&AeraPack.Collection>(from: AeraPack.OpenedCollectionStoragePath)!
		...
		for reward in rewardIds{ 
			...
			emit PackReveal(packId: packId, address: receiver.address, packTypeId: pack.getTypeID(), rewardId: reward, rewardType: token.getType().identifier, rewardFields: fields, nftPerPack: AeraPackExtraData.getItemsPerPackType(pack.getTypeID())!, packTier: AeraPackExtraData.getTierPerPackType(pack.getTypeID())!)
	...
	fun transferToDLQ(_ pack: @NFT){ 
		let dlq = AeraPack.account.storage.borrow<&AeraPack.Collection>(from: AeraPack.DLQCollectionStoragePath)!
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&AeraPack.Collection>(AeraPack.OpenedCollectionStoragePath)
		self.account.capabilities.publish(capability_1, at: AeraPack.OpenedCollectionPublicPath)
		...
		var capability_2 = self.account.capabilities.storage.issue<&AeraPack.Collection>(AeraPack.DLQCollectionStoragePath)
		self.account.capabilities.publish(capability_2, at: AeraPack.DLQCollectionPublicPath)
		...
		var capability_3 = self.account.capabilities.storage.issue<&AeraPack.Collection>(AeraPack.CollectionStoragePath)
		self.account.capabilities.publish(capability_3, at: AeraPack.CollectionPublicPath)
