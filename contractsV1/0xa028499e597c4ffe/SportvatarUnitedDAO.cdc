...
access(all)
contract SportvatarUnitedDAO: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				SportvatarUnitedDAO.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return SportvatarUnitedDAO.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				SportvatarUnitedDAO.maxSupply == nil || SportvatarUnitedDAO.totalSupply <= SportvatarUnitedDAO.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			SportvatarUnitedDAO.totalSupply = SportvatarUnitedDAO.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafybeidz6cdofpqjjmtvlod4olmf5pzuyob7ianscbj7nyyistfo7f7lri"), mediaType: "image")
				let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreicwrruushz5t5pjy5jlf7eqvovexbzhna43l4v2iuu3iyinzme5z4"), mediaType: "image")
				let medias = MetadataViews.Medias([media, bannerMedia])
				return FungibleTokenMetadataViews.FTDisplay(name: "Sportvatar United DAO", symbol: "SUD", description: "A DAO for Sportvatar holders to test their managerial wits through joint ownership in the Metaverse Football League", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: SportvatarUnitedDAO.VaultStoragePath, receiverPath: SportvatarUnitedDAO.ReceiverPublicPath, metadataPath: SportvatarUnitedDAO.VaultPublicPath, receiverLinkedType: /private/SportvatarUnitedDAOVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-SportvatarUnitedDAO.createEmptyVault(vaultType: Type<@SportvatarUnitedDAO.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/SportvatarUnitedDAOVault
		self.ReceiverPublicPath = /public/SportvatarUnitedDAOReceiver
		self.VaultPublicPath = /public/SportvatarUnitedDAOMetadata
		self.MinterStoragePath = /storage/SportvatarUnitedDAOMinter
		self.AdministratorStoragePath = /storage/SportvatarUnitedDAOAdmin
