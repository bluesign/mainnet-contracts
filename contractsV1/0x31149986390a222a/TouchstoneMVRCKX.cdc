...
access(all)
contract TouchstoneMVRCKX: NonFungibleToken{ 
	...
	resource NFT: NonFungibleToken.NFT, ViewResolver.Resolver{ 
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: TouchstoneMVRCKX.CollectionStoragePath, publicPath: TouchstoneMVRCKX.CollectionPublicPath, publicCollection: Type<&Collection>(), publicLinkedType: Type<&Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-TouchstoneMVRCKX.createEmptyCollection(nftType: Type<@TouchstoneMVRCKX.Collection>())
						})
		...
		init(_metadataId: UInt64, _serial: UInt64, _recipient: Address){ 
			...
			if let buyersRef = &TouchstoneMVRCKX.primaryBuyers[_recipient] as auth(Mutate) &{UInt64: [UInt64]}?{ 
	...
	resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun claim(){ 
			if let storage = &TouchstoneMVRCKX.nftStorage[(self.owner!).address] as auth(Mutate) &{UInt64: NFT}?{ 
	...
	resource Administrator{ 
		...
		fun mintNFT(metadataId: UInt64, serial: UInt64, recipient: Address){ 
			...
			if let recipientCollection = getAccount(recipient).capabilities.get<&TouchstoneMVRCKX.Collection>(TouchstoneMVRCKX.CollectionPublicPath).borrow<&TouchstoneMVRCKX.Collection>(){ 
				recipientCollection.deposit(token: <-nft)
			} else if let storage = &TouchstoneMVRCKX.nftStorage[recipient] as auth(Mutate) &{UInt64: NFT}?{ 
				storage[nft.id] <-! nft
			} else{ 
	...
	fun getClaimableNFTs(user: Address):{ UInt64: NFTMetadata}{ 
		...
		if let storage = &TouchstoneMVRCKX.nftStorage[user] as auth(Mutate) &{UInt64: NFT}?{ 
			for id in storage.keys{ 
				let nftRef = (storage[id] as &TouchstoneMVRCKX.NFT?)!
