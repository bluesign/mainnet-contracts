...
//
access(all)
contract TransactionGenerationUtils{ 
	access(all)
	struct interface TokenSchema{ 
		access(all)
	...
	struct FTSchemaV2: TokenSchema{ 
		...
		access(all)
		let receiverStoragePath: String?
	...
	fun createStaticTypeFromType(_ type: Type): String{ 
		...
		let replaceableType = ArrayUtils.mapStrings(types, fun (str: String): String{ 
