...
access(all)
contract BIFF2THEFUTURE: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				BIFF2THEFUTURE.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return BIFF2THEFUTURE.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				BIFF2THEFUTURE.maxSupply == nil || BIFF2THEFUTURE.totalSupply <= BIFF2THEFUTURE.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			BIFF2THEFUTURE.totalSupply = BIFF2THEFUTURE.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreiaarpv4kblnlzapj4dmgvdv2zxorwcajn4iwnzabplzipfpuyf4pa"), mediaType: "image")
				let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreiawrb37pnq2vayycgnilkrwg3sgmt6v56qtl4ajby5zw6ktaqq3ua"), mediaType: "image")
				let medias = MetadataViews.Medias([media, bannerMedia])
				return FungibleTokenMetadataViews.FTDisplay(name: "BIFF2THEFUTURE", symbol: "BIFF", description: "Since you're new here, I'm gonna cut you a break, today. So, why don't you make like a tree and get outta here? ", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: BIFF2THEFUTURE.VaultStoragePath, receiverPath: BIFF2THEFUTURE.ReceiverPublicPath, metadataPath: BIFF2THEFUTURE.VaultPublicPath, receiverLinkedType: /private/BIFF2THEFUTUREVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-BIFF2THEFUTURE.createEmptyVault(vaultType: Type<@BIFF2THEFUTURE.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/BIFF2THEFUTUREVault
		self.ReceiverPublicPath = /public/BIFF2THEFUTUREReceiver
		self.VaultPublicPath = /public/BIFF2THEFUTUREMetadata
		self.MinterStoragePath = /storage/BIFF2THEFUTUREMinter
		self.AdministratorStoragePath = /storage/BIFF2THEFUTUREAdmin
