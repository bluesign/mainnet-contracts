...
access(all)
contract GrandmallworldNFT: NonFungibleToken, RedevNFT{ 
	access(all)
	...
	access(all)
	resource interface GrandmallworldNFTCollectionPublic{ 
		access(all)
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun getMetadata(id: UInt64): GrandmallworldNFT.Metadata
		
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowGrandmallworldItem(id: UInt64): &GrandmallworldNFT.NFT?{ 
	...
	access(all)
	resource Collection: GrandmallworldNFTCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, RedevNFT.CollectionPublic{ 
		access(all)
	...
	resource Minter{ 
		...
		fun mintTo(creator: Capability<&{NonFungibleToken.Receiver}>, name: String, artist: String, description: String, dedicace: String, type: String, ipfs: String, collection: String, nomSerie: String, edition: UInt64, nbrEdition: UInt64, royalties: [{RedevNFT.Royalty}]): &{NonFungibleToken.NFT}{ 
			...
			let token <- create NFT(id: GrandmallworldNFT.totalSupply, creator: creator.address, metadata: metadata, royalties: royalties)
