...
///
access(all)
contract OfferStorefront{ 
	access(all)
	event OfferStorefrontInitialized()
	...
	struct ListingDetails{ 
		...
		/// initializer
		///
		init(nftType: Type, nftID: UInt64, walletClient: Address,																  //salePaymentVaultType: Type,
																  //saleCuts: [SaleCut],
																  storefrontID: UInt64){ 
	...
	resource interface ListingPublic{ 
		...
		///
		access(TMP_ENTITLEMENT_OWNER)
		fun purchase(wallet: Capability<&{NonFungibleToken.Receiver}>): @{NonFungibleToken.NFT}
	...
	resource Listing: ListingPublic{ 
		...
		fun purchase(wallet: Capability<&{NonFungibleToken.Receiver}>): @{NonFungibleToken.NFT}{ 
			pre{ 
				...
				self.details.walletClient == wallet.address:
					"wallet address is wrong"
		...
		/// initializer
		///
		init(nftProviderCapability: Capability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>, nftType: Type, nftID: UInt64, walletClient: Address, storefrontID: UInt64){ 
			// Store the sale information
			self.details = ListingDetails(nftType: nftType, nftID: nftID, walletClient: walletClient, storefrontID: storefrontID)
	...
	resource Storefront: StorefrontManager, StorefrontPublic{ 
		...
		///
		access(TMP_ENTITLEMENT_OWNER)
		fun createListing(nftProviderCapability: Capability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>, nftType: Type, nftID: UInt64, walletClient: Address): UInt64{ 
			let listing <- create Listing(nftProviderCapability: nftProviderCapability, nftType: nftType, nftID: nftID, walletClient: walletClient, storefrontID: self.uuid)
			...
			destroy oldListing
			emit ListingAvailable(storefrontAddress: self.owner?.address!, listingResourceID: listingResourceID, nftType: nftType, nftID: nftID, walletClient: listingWalletAddress)
