...
access(all)
contract Trianons: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				Trianons.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return Trianons.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				Trianons.maxSupply == nil || Trianons.totalSupply <= Trianons.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			Trianons.totalSupply = Trianons.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "Trianons", symbol: "FIDE", description: "Trianons DAO: Revolutionizing Marketing in the Web3 Era", externalURL: MetadataViews.ExternalURL("www.trianons.com.br"), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("julianokimura"), "discord": MetadataViews.ExternalURL("xPAeS77uXa")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: Trianons.VaultStoragePath, receiverPath: Trianons.ReceiverPublicPath, metadataPath: Trianons.VaultPublicPath, receiverLinkedType: /private/TrianonsVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-Trianons.createEmptyVault(vaultType: Type<@Trianons.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/TrianonsVault
		self.ReceiverPublicPath = /public/TrianonsReceiver
		self.VaultPublicPath = /public/TrianonsMetadata
		self.MinterStoragePath = /storage/TrianonsMinter
		self.AdministratorStoragePath = /storage/TrianonsAdmin
