...
contract Swap{ 
	...
	struct ProposedTradeAsset: Readable{ 
		...
		access(all)
		let collectionData: Utils.StorableNFTCollectionData
	...
	struct UserOffer: Readable{ 
		...
		access(all)
		fun toFullyQualifiedIdentifiers(): [String]{ 
			let response: [String] = []
	...
	struct SwapProposal{ 
		...
		init(
			...
			assert(
				leftUserOffer.proposedNfts.length > 0 || rightUserOffer.proposedNfts.length > 0,
	...
	access(all)
	resource interface SwapStatsManager{ 
		access(all)
	...
	access(all)
	resource SwapAdmin: SwapProposalManager, SwapFeeManager, SwapStatsManager{ 
