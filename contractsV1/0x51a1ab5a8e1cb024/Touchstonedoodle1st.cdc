...
access(all)
contract Touchstonedoodle1st: NonFungibleToken{ 
	...
	resource NFT: NonFungibleToken.NFT, ViewResolver.Resolver{ 
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: Touchstonedoodle1st.CollectionStoragePath, publicPath: Touchstonedoodle1st.CollectionPublicPath, publicCollection: Type<&Collection>(), publicLinkedType: Type<&Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-Touchstonedoodle1st.createEmptyCollection(nftType: Type<@Touchstonedoodle1st.Collection>())
						})
		...
		init(_metadataId: UInt64, _serial: UInt64, _recipient: Address){ 
			...
			if let buyersRef = &Touchstonedoodle1st.primaryBuyers[_recipient] as auth(Mutate) &{UInt64: [UInt64]}?{ 
	...
	resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun claim(){ 
			if let storage = &Touchstonedoodle1st.nftStorage[(self.owner!).address] as auth(Mutate) &{UInt64: NFT}?{ 
	...
	resource Administrator{ 
		...
		fun mintNFT(metadataId: UInt64, serial: UInt64, recipient: Address){ 
			...
			if let recipientCollection = getAccount(recipient).capabilities.get<&Touchstonedoodle1st.Collection>(Touchstonedoodle1st.CollectionPublicPath).borrow<&Touchstonedoodle1st.Collection>(){ 
				recipientCollection.deposit(token: <-nft)
			} else if let storage = &Touchstonedoodle1st.nftStorage[recipient] as auth(Mutate) &{UInt64: NFT}?{ 
				storage[nft.id] <-! nft
			} else{ 
	...
	fun getClaimableNFTs(user: Address):{ UInt64: NFTMetadata}{ 
		...
		if let storage = &Touchstonedoodle1st.nftStorage[user] as auth(Mutate) &{UInt64: NFT}?{ 
			for id in storage.keys{ 
				let nftRef = (storage[id] as &Touchstonedoodle1st.NFT?)!
