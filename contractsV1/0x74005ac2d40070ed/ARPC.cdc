...
access(all)
contract ARPC: NonFungibleToken, NonFungibleTokenMinter{ 
	access(all)
	...
	resource interface CollectionPublic{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowNFTARPC(id: UInt64): &ARPC.NFT?{ 
			post{ 
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&ARPC.Collection>(self.CollectionStoragePath)
