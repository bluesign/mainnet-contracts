...
access(all)
contract MCity: NonFungibleToken, NonFungibleTokenMinter{ 
	access(all)
	...
	access(all)
	event Minted(to: Address, tokenId: UInt64, metadata:{ String: String})
	...
	resource interface CollectionPublic{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowNFTDream(id: UInt64): &MCity.NFT?{ 
			post{ 
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&MCity.Collection>(self.CollectionStoragePath)
