...
access(all)
contract DoanDucManh: NonFungibleToken, NonFungibleTokenMinter{ 
	access(all)
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&DoanDucManh.Collection>(self.CollectionStoragePath)
