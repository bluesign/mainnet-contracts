...
access(all)
contract wolFOAD: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				wolFOAD.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return wolFOAD.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				wolFOAD.maxSupply == nil || wolFOAD.totalSupply <= wolFOAD.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			wolFOAD.totalSupply = wolFOAD.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "wolF OAD", symbol: "WOLF", description: "it's just a revers", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("sergeyr188"), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: wolFOAD.VaultStoragePath, receiverPath: wolFOAD.ReceiverPublicPath, metadataPath: wolFOAD.VaultPublicPath, receiverLinkedType: /private/wolFOADVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-wolFOAD.createEmptyVault(vaultType: Type<@wolFOAD.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/wolFOADVault
		self.ReceiverPublicPath = /public/wolFOADReceiver
		self.VaultPublicPath = /public/wolFOADMetadata
		self.MinterStoragePath = /storage/wolFOADMinter
		self.AdministratorStoragePath = /storage/wolFOADAdmin
