...
access(all)
contract Freely: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				Freely.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return Freely.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				Freely.maxSupply == nil || Freely.totalSupply <= Freely.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			Freely.totalSupply = Freely.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "Freely", symbol: "FRLY", description: "A DAO for Christians", externalURL: MetadataViews.ExternalURL("freely.com"), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("freely"), "discord": MetadataViews.ExternalURL("q2maASKC")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: Freely.VaultStoragePath, receiverPath: Freely.ReceiverPublicPath, metadataPath: Freely.VaultPublicPath, receiverLinkedType: /private/FreelyVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-Freely.createEmptyVault(vaultType: Type<@Freely.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/FreelyVault
		self.ReceiverPublicPath = /public/FreelyReceiver
		self.VaultPublicPath = /public/FreelyMetadata
		self.MinterStoragePath = /storage/FreelyMinter
		self.AdministratorStoragePath = /storage/FreelyAdmin
