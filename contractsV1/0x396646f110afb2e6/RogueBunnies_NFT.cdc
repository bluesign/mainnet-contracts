...
access(all)
contract RogueBunnies_NFT: NonFungibleToken{ 
	...
	resource Series{ 
		...
		//
		access(TMP_ENTITLEMENT_OWNER)
		fun mintRogueBunnies_NFT(recipient: &{NonFungibleToken.CollectionPublic}, tokenId: UInt64, setId: UInt32){ 
			pre{ 
		...
		//
		access(TMP_ENTITLEMENT_OWNER)
		fun mintEditionRogueBunnies_NFT(recipient: &{NonFungibleToken.CollectionPublic}, tokenId: UInt64, setId: UInt32, edition: UInt32){ 
			pre{ 
				self.numberEditionsMintedPerSet[setId] != nil:
					"The Set does not exist."
				self.numberEditionsMintedPerSet[setId]! <= RogueBunnies_NFT.getSetMaxEditions(setId: setId)!:
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun batchMintRogueBunnies_NFT(recipient: &{NonFungibleToken.CollectionPublic}, setId: UInt32, tokenIds: [UInt64]){ 
			pre{ 
				tokenIds.length > 0:
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
					...
					for metadataKey in (RogueBunnies_NFT.getSetMetadata(setId: self.setId)!).keys{ 
						...
						if metadataKey.length >= 13{ 
							if metadataKey.slice(from: 0, upTo: 13) == "royalty_addr_"{ 
								...
								if cutValue != 0.0{ 
									royalties.append(MetadataViews.Royalty(receiver: getAccount(royaltyAddress).capabilities.get<&{FungibleToken.Vault}>(royaltyReceiver), cut: cutValue, description: RogueBunnies_NFT.getSetMetadataByField(setId: self.setId, field: "royalty_desc_".concat(royaltyName))!))
								}
							}
						}
					}
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: RogueBunnies_NFT.CollectionStoragePath, publicPath: RogueBunnies_NFT.CollectionPublicPath, publicCollection: Type<&RogueBunnies_NFT.Collection>(), publicLinkedType: Type<&RogueBunnies_NFT.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-RogueBunnies_NFT.createEmptyCollection(nftType: Type<@RogueBunnies_NFT.Collection>())
						})
				...
				case Type<MetadataViews.Medias>():
					return MetadataViews.Medias([MetadataViews.Media(file: MetadataViews.HTTPFile(url: RogueBunnies_NFT.getSetMetadataByField(setId: self.setId, field: "image")!), mediaType: self.getMimeType())])
			}
			return nil
		}
	...
	access(all)
	resource interface RogueBunnies_NFTCollectionPublic{ 
		access(all)
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowRogueBunnies_NFT(id: UInt64): &RogueBunnies_NFT.NFT?{ 
	...
	//
	access(all)
	resource Collection: RogueBunnies_NFTCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		...
		view fun borrowViewResolver(id: UInt64): &{ViewResolver.Resolver}?{ 
			...
			return RogueBunnies_NFTNft as &{ViewResolver.Resolver}
		}
	...
	//
	access(TMP_ENTITLEMENT_OWNER)
	fun fetch(_ from: Address, id: UInt64): &RogueBunnies_NFT.NFT?{ 
		let collection = getAccount(from).capabilities.get<&RogueBunnies_NFT.Collection>(RogueBunnies_NFT.CollectionPublicPath).borrow<&RogueBunnies_NFT.Collection>() ?? panic("Couldn't get collection")
	...
	access(TMP_ENTITLEMENT_OWNER)
	fun getAllSeries(): [RogueBunnies_NFT.SeriesData]{ 
	...
	access(TMP_ENTITLEMENT_OWNER)
	fun getAllSets(): [RogueBunnies_NFT.NFTSetData]{ 
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&RogueBunnies_NFT.Admin>(self.AdminStoragePath)
