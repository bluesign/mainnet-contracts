...
access(all)
contract TMAUNFT: NonFungibleToken{ 
	access(all)
	...
	access(all)
	resource interface TMAUNFTCollectionPublic{ 
		access(all)
		...
		fun borrowTMAUNFT(id: UInt64): &TMAUNFT.NFT?{ 
			post{ 
	...
	access(all)
	resource Collection: TMAUNFTCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic{ 
		access(all)
