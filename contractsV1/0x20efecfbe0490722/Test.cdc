...
access(all)
contract Test: NonFungibleToken{ 
	access(all)
	...
	access(all)
	resource interface TestCollectionPublic{ 
		access(all)
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowTest(id: UInt64): &Test.NFT?{ 
			post{ 
	...
	access(all)
	resource Collection: TestCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		access(all)
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&Test.Collection>(self.CollectionStoragePath)
