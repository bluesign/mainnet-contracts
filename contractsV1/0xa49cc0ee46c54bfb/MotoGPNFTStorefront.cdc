...
contract MotoGPNFTStorefront{ 
	...
	//
	access(all)
	event SaleOfferRemoved(saleOfferResourceID: UInt64)
	...
	access(contract)
	var commissionReceiver: Capability<&{FungibleToken.Receiver}>?
	...
	resource SaleOffer: SaleOfferPublic{ 
		...
		// initializer
		//
		init(nftProviderCapability: Capability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>, nftType: Type, nftID: UInt64, salePaymentVaultType: Type, price: UFix64, // GK: pass in price UFix64,																																														  
