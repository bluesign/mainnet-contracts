...
contract SalesContract{ 
	...
	access(contract)
	let serialMap:{ UInt64:{ UInt64: Bool}} // Used like so : { packType : { serial: true/false } 
	...
	struct SKU{ 
		...
		access(contract)
		fun setStartTime(startTime: UInt64){ 
			self.startTime = startTime
		}
		
		access(contract)
		fun setEndTime(endTime: UInt64){ 
			self.endTime = endTime
		}
		...
		access(contract)
		fun setMaxPerBuyer(maxPerBuyer: UInt64){ 
			self.maxPerBuyer = maxPerBuyer
		}
		
		access(contract)
		fun setPayoutAddress(payoutAddress: Address){ 
			self.payoutAddress = payoutAddress
		}
		...
		access(contract)
		fun increaseSupply(supplyList: [UInt64]){ 
	...
	access(contract)
	fun isValidSignature(signature: String, message: String): Bool{ 
		let pk =
			PublicKey(
	...
	fun buyPack(
		...
		recipientCollectionRef: &MotoGPPack.Collection
	){ 
		pre{ 
			paymentVaultRef.balance >= (self.skuMap[skuName]!).price:
				"paymentVaultRef's balance is lower than price"
			self.isActiveSKU(name: skuName) == true:
				"SKU is not active"
			self.getRemainingSupplyForSKU(name: skuName) > UInt64(0):
				"No remaining supply for SKU"
			(self.skuMap[skuName]!).price >= UFix64(0.0):
				"Price is zero. Admin needs to set the price"
			(self.skuMap[skuName]!).packType == packType:
				"Supplied packType doesn't match SKU packType"
		}
		post{ 
			self.nonceMap[recipient]! == before(self.nonceMap[recipient] ?? UInt64(0)) + UInt64(1):
				"Nonce hasn't increased by one"
			(self.skuMap[skuName]!).buyerCountMap[recipient]! == before((self.skuMap[skuName]!).buyerCountMap[recipient] ?? UInt64(0)) + UInt64(1):
				"buyerCountMap hasn't increased by one"
			(self.skuMap[skuName]!).buyerCountMap[recipient]! <= (self.skuMap[skuName]!).maxPerBuyer:
				"Max pack purchase count per buyer exceeded"
			paymentVaultRef.balance == before(paymentVaultRef.balance) - (self.skuMap[skuName]!).price:
				"Decrease in buyer vault balance doesn't match the price"
		}
		let sku = self.skuMap[skuName]!
		let recipientStr = self.convertAddressToString(address: recipient)
		let message =
			skuName.concat(recipientStr).concat(nonce.toString()).concat(packType.toString())
		...
		let tempCollection <-
			MotoGPPack.createEmptyCollection(nftType: Type<@MotoGPPack.Collection>())
		...
		MotoGPTransfer.transferPacks(
			fromCollection: <-tempCollection,
			toCollection: recipientCollectionRef,
			toAddress: recipient
		)
	}
