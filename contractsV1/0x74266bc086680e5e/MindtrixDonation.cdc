...
contract MindtrixDonation{ 
	...
	access(self)
	var episodeGuidToDonations:{ String:{ Address: [MindtrixViews.NFTIdentifier]}}
	...
	//}
	access(self)
	var showGuidToRoyalties:{ String:{ String: [MetadataViews.Royalty]}}
	...
	access(account)
	fun getNFTEditionFromDonationDicByEpisodeGuid(episodeGuid: String): UInt64{ 
		if self.episodeGuidToDonations[episodeGuid] == nil{ 
			return 0
		} else{ 
	...
	access(account)
	fun replaceShowGuidToRoyalties(
		showGuid: String,
		creatorAddress: Address,
		primaryRoyalties: [
	...
	fun mintNFTFromDonation(
		...
		let royaltiesFromShowGuid =
			MindtrixDonation.showGuidToRoyalties[showGuid]! as{ String: [MetadataViews.Royalty]}
		log("showGuid:")
		log(showGuid)
		log("mintNFTFromDonation royaltiesFromShowGuid:")
		log(royaltiesFromShowGuid)
		var mindtrixPrimaryCut = 0.2
		var MindtrixVault: &{FungibleToken.Receiver}? = nil
		var CreatorVault: &{FungibleToken.Receiver}? = nil
		...
		let mintedEdition =
			self.getNFTEditionFromDonationDicByEpisodeGuid(episodeGuid: episodeGuid) + 1
		...
		let nftIdentifier =
			MindtrixViews.NFTIdentifier(uuid: nftId, serial: mintedEdition, holder: donorAddress)
		self.updateDonorDic(
