...
access(all)
contract TouchstoneHalloweencreatedbyAi: NonFungibleToken{ 
	...
	resource NFT: NonFungibleToken.NFT, ViewResolver.Resolver{ 
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: TouchstoneHalloweencreatedbyAi.CollectionStoragePath, publicPath: TouchstoneHalloweencreatedbyAi.CollectionPublicPath, publicCollection: Type<&Collection>(), publicLinkedType: Type<&Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-TouchstoneHalloweencreatedbyAi.createEmptyCollection(nftType: Type<@TouchstoneHalloweencreatedbyAi.Collection>())
						})
		...
		init(_metadataId: UInt64, _serial: UInt64, _recipient: Address){ 
			...
			if let buyersRef = &TouchstoneHalloweencreatedbyAi.primaryBuyers[_recipient] as auth(Mutate) &{UInt64: [UInt64]}?{ 
	...
	resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun claim(){ 
			if let storage = &TouchstoneHalloweencreatedbyAi.nftStorage[(self.owner!).address] as auth(Mutate) &{UInt64: NFT}?{ 
	...
	// is currently active.
	access(TMP_ENTITLEMENT_OWNER)
	fun mintNFT(metadataId: UInt64, recipient: &{NonFungibleToken.Receiver}, payment: @FUSD.Vault, serial: UInt64): UInt64{ 
		pre{ 
			self.canMint():
		...
		let EmeraldCityTreasury = getAccount(0x5643fd47a29770e7).capabilities.get<&FUSD.Vault>(/public/fusdReceiver).borrow<&FUSD.Vault>()!
		...
		let paymentRecipient = self.account.capabilities.get<&FUSD.Vault>(/public/fusdReceiver).borrow<&FUSD.Vault>()!
	...
	resource Administrator{ 
		...
		fun mintNFT(metadataId: UInt64, serial: UInt64, recipient: Address){ 
			...
			if let recipientCollection = getAccount(recipient).capabilities.get<&TouchstoneHalloweencreatedbyAi.Collection>(TouchstoneHalloweencreatedbyAi.CollectionPublicPath).borrow<&TouchstoneHalloweencreatedbyAi.Collection>(){ 
				recipientCollection.deposit(token: <-nft)
			} else if let storage = &TouchstoneHalloweencreatedbyAi.nftStorage[recipient] as auth(Mutate) &{UInt64: NFT}?{ 
				storage[nft.id] <-! nft
			} else{ 
	...
	fun getClaimableNFTs(user: Address):{ UInt64: NFTMetadata}{ 
		...
		if let storage = &TouchstoneHalloweencreatedbyAi.nftStorage[user] as auth(Mutate) &{UInt64: NFT}?{ 
			for id in storage.keys{ 
				let nftRef = (storage[id] as &TouchstoneHalloweencreatedbyAi.NFT?)!
