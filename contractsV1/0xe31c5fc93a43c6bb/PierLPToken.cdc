...
access(all)
contract PierLPToken: MultiFungibleToken{ 
	...
	access(all)
	event TokensMinted(tokenId: UInt64, amount: UFix64)
	
	// Event that is emitted when tokens are burned
	access(all)
	event TokensBurned(tokenId: UInt64, amount: UFix64)
	...
	access(all)
	event TokenMasterCreated(tokenId: UInt64)
	...
	access(contract)
	let totalSupply:{ UInt64: UFix64}
	
	// PierLPToken Vault
	access(all)
	resource Vault: MultiFungibleToken.Receiver, MultiFungibleToken.View{ 
	...
	access(all)
	resource Collection: MultiFungibleToken.Provider, MultiFungibleToken.Receiver, MultiFungibleToken.CollectionPublic{ 
		// A mapping from token id (pool id) to LP token Vault
		access(all)
		var vaults: @{UInt64: PierLPToken.Vault}
	...
	resource TokenMaster{ 
		...
		fun mintTokens(amount: UFix64): @PierLPToken.Vault{ 
			pre{ 
				amount > 0.0:
					"Metapier PierLPToken: Amount to mint must be greater than zero"
			}
		...
		fun burnTokens(vault: @PierLPToken.Vault){ 
			pre{ 
				self.tokenId == vault.tokenId:
					"Metapier PierLPToken: Cannot burn other LP tokens"
			}
		...
		fun initNewLPToken(tokenId: UInt64): @TokenMaster{ 
			pre{ 
				!PierLPToken.totalSupply.containsKey(tokenId):
					"Metapier PierLPToken: LP Token already exists"
			}
