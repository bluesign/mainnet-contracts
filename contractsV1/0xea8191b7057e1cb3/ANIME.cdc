...
access(all)
contract ANIME: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				ANIME.setBalance(address: owner, balance: self.balance)
			}
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				ANIME.maxSupply == nil || ANIME.totalSupply <= ANIME.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			ANIME.totalSupply = ANIME.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "ANIME", symbol: "ANIM", description: "WIBU WIBU WIBU WIBU", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: ANIME.VaultStoragePath, receiverPath: ANIME.ReceiverPublicPath, metadataPath: ANIME.VaultPublicPath, receiverLinkedType: /private/ANIMEVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-ANIME.createEmptyVault(vaultType: Type<@ANIME.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/ANIMEVault
		self.ReceiverPublicPath = /public/ANIMEReceiver
		self.VaultPublicPath = /public/ANIMEMetadata
		self.MinterStoragePath = /storage/ANIMEMinter
		self.AdministratorStoragePath = /storage/ANIMEAdmin
