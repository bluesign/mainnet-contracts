...
access(all)
contract KrumpKommunity: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				KrumpKommunity.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return KrumpKommunity.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				KrumpKommunity.maxSupply == nil || KrumpKommunity.totalSupply <= KrumpKommunity.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			KrumpKommunity.totalSupply = KrumpKommunity.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "Krump Kommunity", symbol: "KRUMP", description: "Pioneering Krump Ownership Economy Worldwide", externalURL: MetadataViews.ExternalURL("bit.ly/DeDan"), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("krumpverse"), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: KrumpKommunity.VaultStoragePath, receiverPath: KrumpKommunity.ReceiverPublicPath, metadataPath: KrumpKommunity.VaultPublicPath, receiverLinkedType: /private/KrumpKommunityVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-KrumpKommunity.createEmptyVault(vaultType: Type<@KrumpKommunity.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/KrumpKommunityVault
		self.ReceiverPublicPath = /public/KrumpKommunityReceiver
		self.VaultPublicPath = /public/KrumpKommunityMetadata
		self.MinterStoragePath = /storage/KrumpKommunityMinter
		self.AdministratorStoragePath = /storage/KrumpKommunityAdmin
