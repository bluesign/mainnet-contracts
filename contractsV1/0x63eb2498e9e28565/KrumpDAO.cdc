...
access(all)
contract KrumpDAO: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				KrumpDAO.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return KrumpDAO.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				KrumpDAO.maxSupply == nil || KrumpDAO.totalSupply <= KrumpDAO.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			KrumpDAO.totalSupply = KrumpDAO.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "Krump DAO", symbol: "BUCK", description: "Pioneering Decentralized Dance Worldwide", externalURL: MetadataViews.ExternalURL("bit.ly/DeDan"), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("krumpverse"), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: KrumpDAO.VaultStoragePath, receiverPath: KrumpDAO.ReceiverPublicPath, metadataPath: KrumpDAO.VaultPublicPath, receiverLinkedType: /private/KrumpDAOVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-KrumpDAO.createEmptyVault(vaultType: Type<@KrumpDAO.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/KrumpDAOVault
		self.ReceiverPublicPath = /public/KrumpDAOReceiver
		self.VaultPublicPath = /public/KrumpDAOMetadata
		self.MinterStoragePath = /storage/KrumpDAOMinter
		self.AdministratorStoragePath = /storage/KrumpDAOAdmin
