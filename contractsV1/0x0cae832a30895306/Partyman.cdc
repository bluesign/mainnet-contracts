...
access(all)
contract Partyman: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				Partyman.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return Partyman.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				Partyman.maxSupply == nil || Partyman.totalSupply <= Partyman.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			Partyman.totalSupply = Partyman.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreif27isitasik7iafmv6d6x2vhhponxsfogp5h45kprnl53ote3aei"), mediaType: "image")
				let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreifvxiggm6jbajbfttptaqnyzgdoijjq56jmv7nhe7bdsmm4xe6lqu"), mediaType: "image")
				let medias = MetadataViews.Medias([media, bannerMedia])
				return FungibleTokenMetadataViews.FTDisplay(name: "Partyman", symbol: "DANCE", description: "All hail, the new king in town.", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: Partyman.VaultStoragePath, receiverPath: Partyman.ReceiverPublicPath, metadataPath: Partyman.VaultPublicPath, receiverLinkedType: /private/PartymanVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-Partyman.createEmptyVault(vaultType: Type<@Partyman.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/PartymanVault
		self.ReceiverPublicPath = /public/PartymanReceiver
		self.VaultPublicPath = /public/PartymanMetadata
		self.MinterStoragePath = /storage/PartymanMinter
		self.AdministratorStoragePath = /storage/PartymanAdmin
