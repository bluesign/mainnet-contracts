...
access(all)
contract VOARJ: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				VOARJ.setBalance(address: owner, balance: self.balance)
			}
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				VOARJ.maxSupply == nil || VOARJ.totalSupply <= VOARJ.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			VOARJ.totalSupply = VOARJ.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafybeicpmiywiyskzctsanfk4u2ypwzhfumuv7wndcthywfz5puhmiwrpa"), mediaType: "image")
				let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafybeic6qhjl32pwgghlkvubcicqp2rkhkjbzwe7zentvt46buydpzfxoi"), mediaType: "image")
				let medias = MetadataViews.Medias([media, bannerMedia])
				return FungibleTokenMetadataViews.FTDisplay(name: "VOARJ", symbol: "VOARJ", description: "Passeios de helic\u{f3}ptero no Rio de Janeiro \u{1f681}", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: VOARJ.VaultStoragePath, receiverPath: VOARJ.ReceiverPublicPath, metadataPath: VOARJ.VaultPublicPath, receiverLinkedType: /private/VOARJVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-VOARJ.createEmptyVault(vaultType: Type<@VOARJ.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/VOARJVault
		self.ReceiverPublicPath = /public/VOARJReceiver
		self.VaultPublicPath = /public/VOARJMetadata
		self.MinterStoragePath = /storage/VOARJMinter
		self.AdministratorStoragePath = /storage/VOARJAdmin
