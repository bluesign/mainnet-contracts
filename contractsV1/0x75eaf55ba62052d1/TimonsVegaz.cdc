...
access(all)
contract TimonsVegaz: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				TimonsVegaz.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return TimonsVegaz.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				TimonsVegaz.maxSupply == nil || TimonsVegaz.totalSupply <= TimonsVegaz.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			TimonsVegaz.totalSupply = TimonsVegaz.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreieqlnkl65lbxowd766yvkm5xdgqkeswmexrr4xui5ynkxadvh3h6a"), mediaType: "image")
				let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreiafkl4cjlbak3oqu4hmtwnahjfkjebsl52ptinbzcinlxsxqjzc6a"), mediaType: "image")
				let medias = MetadataViews.Medias([media, bannerMedia])
				return FungibleTokenMetadataViews.FTDisplay(name: "Timon's Vegaz", symbol: "VEGAZ", description: "Vegaz - memecoin for flow community.", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: TimonsVegaz.VaultStoragePath, receiverPath: TimonsVegaz.ReceiverPublicPath, metadataPath: TimonsVegaz.VaultPublicPath, receiverLinkedType: /private/TimonsVegazVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-TimonsVegaz.createEmptyVault(vaultType: Type<@TimonsVegaz.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/TimonsVegazVault
		self.ReceiverPublicPath = /public/TimonsVegazReceiver
		self.VaultPublicPath = /public/TimonsVegazMetadata
		self.MinterStoragePath = /storage/TimonsVegazMinter
		self.AdministratorStoragePath = /storage/TimonsVegazAdmin
