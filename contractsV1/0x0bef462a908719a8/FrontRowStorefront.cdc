...
contract FrontRowStorefront{ 
	...
	access(all)
	event SaleOfferAvailable(storefrontAddress: Address, blueprintId: UInt32, price: UFix64)
	...
	//
	access(all)
	event SaleOfferRemoved(blueprintId: UInt32, serialNumber: UInt32, soldOut: Bool)
	...
	//
	access(all)
	event Purchase(blueprintId: UInt32, serialNumber: UInt32, soldOut: Bool)
	...
	resource SaleOffer: SaleOfferPublic{ 
		...
		access(contract)
		let nftProviderCapability: Capability<&{NonFungibleToken.Provider, FrontRow.CollectionPublic}>
	...
	resource Storefront: StorefrontManager, StorefrontPublic{ 
		...
		access(self)
		var saleOffers: @{UInt32: SaleOffer}
	...
	init(){ 
		...
		self.account.storage.save<@Storefront>(
			<-create Storefront(),
		...
		var capability_1 =
			self.account.capabilities.storage.issue<&FrontRowStorefront.Storefront>(
				FrontRowStorefront.StorefrontStoragePath
			)
		self.account.capabilities.publish(capability_1, at: FrontRowStorefront.StorefrontPublicPath)
