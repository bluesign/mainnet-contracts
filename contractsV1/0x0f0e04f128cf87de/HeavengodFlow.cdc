...
access(all)
contract HeavengodFlow: FungibleToken, ViewResolver{ 
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: HeavengodFlow.VaultStoragePath, receiverPath: HeavengodFlow.ReceiverPublicPath, metadataPath: HeavengodFlow.VaultPublicPath, receiverLinkedType: /private/HeavengodFlowVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-HeavengodFlow.createEmptyVault(vaultType: Type<@HeavengodFlow.Vault>())
					})
		}
		return nil
	}
