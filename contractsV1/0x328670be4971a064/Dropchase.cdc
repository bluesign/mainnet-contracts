...
access(all)
contract Dropchase: NonFungibleToken{ 
	...
	access(all)
	event StatCreated(id: UInt32, metadata:{ String: String})
	...
	access(all)
	event StatAddedToSet(setID: UInt32, statID: UInt32)
	
	// Emitted when a Stat is retired from a Set and cannot be used to mint
	access(all)
	event StatRetiredFromSet(setID: UInt32, statID: UInt32, numItems: UInt32)
	...
	access(all)
	event ItemMinted(ItemID: UInt64, statID: UInt32, setID: UInt32, serialNumber: UInt32)
	...
	access(all)
	event ItemBurned(itemID: UInt64, from: Address?)
	...
	struct Stat{ 
		...
		init(metadata:{ String: String}){ 
			pre{ 
				metadata.length != 0:
					"New Stat metadata cannot be empty"
			}
	...
	struct SetData{ 
		...
		access(all)
		var numberMintedPerStat:{ UInt32: UInt32}
		
		init(setID: UInt32){ 
			let set = &Dropchase.sets[setID] as &Dropchase.Set?
	...
	resource Set{ 
		...
		fun addStat(statID: UInt32){ 
			pre{ 
				Dropchase.statDatas[statID] != nil:
					"Cannot add the Stat to Set: Stat doesn't exist."
				!self.locked:
					"Cannot add the stat to the Set after the set has been locked."
				self.numberMintedPerStat[statID] == nil:
					"The stat has already beed added to the set."
			}
		...
		fun retireStat(statID: UInt32){ 
			pre{ 
				self.retired[statID] != nil:
					"Cannot retire the Stat: Stat doesn't exist in this set!"
			}
		...
		fun mintItem(statID: UInt32): @NFT{ 
			pre{ 
				self.retired[statID] != nil:
					"Cannot mint the Item: This stat doesn't exist."
				!self.retired[statID]!:
					"Cannot mint the Item from this stat: This stat has been retired."
			}
			...
			// Mint the new Item
			let newItem: @NFT <- create NFT(serialNumber: numInStat + 1 as UInt32, statID: statID, setID: self.setID)
	...
	resource Admin{ 
		...
		fun borrowSet(setID: UInt32): &Set{ 
			...
			return &Dropchase.sets[setID] as &Dropchase.Set?
		}
	...
	init(){ 
		...
		self.account.storage.save<@Collection>(<-create Collection(), to: /storage/DropchaseItemCollection)
		
		// Create a public capability for the Collection
		var capability_1 = self.account.capabilities.storage.issue<&{ItemCollectionPublic}>(/storage/DropchaseItemCollection)
		self.account.capabilities.publish(capability_1, at: /public/DropchaseItemCollection)
		
		// Put the Minter in storage
		self.account.storage.save<@Admin>(<-create Admin(), to: /storage/DropchaseAdmin)
