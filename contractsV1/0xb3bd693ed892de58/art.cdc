...
contract art: NonFungibleToken{ 
	...
	resource interface _NFT_NAME_CollectionPublic{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun borrow_NFT_NAME_(id: UInt64): &art.NFT?{ 
			post{ 
	...
	resource NFTMinter{ 
		...
		fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, metadata:{ String: String}): &{NonFungibleToken.NFT}{ 
			...
			// create a new NFT
			var newNFT <- create NFT(id: art.totalSupply, creator: creator, metadata: metadata)
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&art.Collection>(self.CollectionStoragePath)
