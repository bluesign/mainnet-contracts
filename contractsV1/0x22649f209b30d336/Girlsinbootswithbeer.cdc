...
access(all)
contract Girlsinbootswithbeer: NonFungibleToken, ViewResolver{ 
	...
	resource NFT: NonFungibleToken.NFT, ViewResolver.Resolver, NiftoryNonFungibleToken.NFTPublic{ 
		...
		view fun _contract(): &{NiftoryNonFungibleToken.ManagerPublic}{ 
			return Girlsinbootswithbeer._contract()
		}
	...
	resource Manager: NiftoryNonFungibleToken.ManagerPublic, NiftoryNonFungibleToken.ManagerPrivate{ 
		...
		fun getNFTCollectionData(): MetadataViews.NFTCollectionData{ 
			return NiftoryNFTRegistry.buildNFTCollectionData(Girlsinbootswithbeer.REGISTRY_ADDRESS, Girlsinbootswithbeer.REGISTRY_BRAND, fun (): @{NonFungibleToken.Collection}{ 
					return <-Girlsinbootswithbeer.createEmptyCollection(nftType: Type<@Girlsinbootswithbeer.Collection>())
				})
		}
		...
		fun modifyContractMetadata(): &AnyStruct{ 
			...
			return (&Girlsinbootswithbeer.metadata as &AnyStruct?)!
		}
	...
	init(nftManagerProxy: &{NiftoryNonFungibleTokenProxy.Public, NiftoryNonFungibleTokenProxy.Private}){ 
		...
		self.account.storage.save<@MutableMetadataSetManager.Manager>(<-MutableMetadataSetManager._create(name: "Girlsinbootswithbeer", description: "The set manager for Girlsinbootswithbeer."), to: record.setManager.paths.storage)
