...
access(all)
contract ShebaHopeGrows: NonFungibleToken{ 
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: ShebaHopeGrows.CollectionStoragePath, publicPath: ShebaHopeGrows.CollectionPublicPath, publicCollection: Type<&ShebaHopeGrows.Collection>(), publicLinkedType: Type<&ShebaHopeGrows.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-ShebaHopeGrows.createEmptyCollection(nftType: Type<@ShebaHopeGrows.Collection>())
						})
	...
	resource Admin{ 
		...
		fun changeDefaultRoyaltyRate(name: String, rate: UFix64){ 
			...
			ShebaHopeGrows.defaultRoyalties[name] = MetadataViews.Royalty(receiver: previousRecipientAddress, cut: UFix64(rate), description: "ShebaHopeGrows Royalties")
		...
		fun changeRoyaltyRateForPFP(tokenID: UInt64, name: String, rate: UFix64){ 
			...
			let newRoyalty = MetadataViews.Royalty(receiver: previousRoyalty.receiver, cut: UFix64(rate), description: "ShebaHopeGrows Royalties")
	...
	//
	access(all)
	resource Collection: ShebaHopeGrowsCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		...
		view fun borrowViewResolver(id: UInt64): &{ViewResolver.Resolver}?{ 
			...
			return ShebaHopeGrowsNFT as &{ViewResolver.Resolver}
		}
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&{ShebaHopeGrowsCollectionPublic}>(self.CollectionStoragePath)
