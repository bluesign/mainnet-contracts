...
access(all)
contract MondoPixo_CyberYak: NonFungibleToken{ 
	access(all)
	...
	access(all)
	resource interface _NFT_NAME_CollectionPublic{ 
		access(all)
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun borrow_NFT_NAME_(id: UInt64): &MondoPixo_CyberYak.NFT?{ 
			post{ 
	...
	access(all)
	resource Collection: _NFT_NAME_CollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		access(all)
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&MondoPixo_CyberYak.Collection>(self.CollectionStoragePath)
