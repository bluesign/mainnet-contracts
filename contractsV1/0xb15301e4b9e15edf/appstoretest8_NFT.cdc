...
access(all)
contract appstoretest8_NFT: NonFungibleToken{ 
	...
	resource Series{ 
		...
		//
		access(all)
		fun mintEditionappstoretest8_NFT(recipient: &{NonFungibleToken.CollectionPublic}, tokenId: UInt64, setId: UInt32, edition: UInt32){ 
			pre{ 
				self.numberEditionsMintedPerSet[setId] != nil:
					"The Set does not exist."
				self.numberEditionsMintedPerSet[setId]! <= appstoretest8_NFT.getSetMaxEditions(setId: setId)!:
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
					...
					for metadataKey in (appstoretest8_NFT.getSetMetadata(setId: self.setId)!).keys{ 
						...
						if metadataKey.length >= 13{ 
							if metadataKey.slice(from: 0, upTo: 13) == "royalty_addr_"{ 
								...
								if cutValue != 0.0{ 
									royalties.append(MetadataViews.Royalty(receiver: getAccount(royaltyAddress).capabilities.get<&{FungibleToken.Vault}>(royaltyReceiver)!, cut: cutValue, description: appstoretest8_NFT.getSetMetadataByField(setId: self.setId, field: "royalty_desc_".concat(royaltyName))!))
								}
							}
						}
					}
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: appstoretest8_NFT.CollectionStoragePath, publicPath: appstoretest8_NFT.CollectionPublicPath, publicCollection: Type<&appstoretest8_NFT.Collection>(), publicLinkedType: Type<&appstoretest8_NFT.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-appstoretest8_NFT.createEmptyCollection(nftType: Type<@appstoretest8_NFT.Collection>())
						})
				...
				case Type<MetadataViews.Medias>():
					return MetadataViews.Medias([MetadataViews.Media(file: MetadataViews.HTTPFile(url: appstoretest8_NFT.getSetMetadataByField(setId: self.setId, field: "image")!), mediaType: self.getMimeType())])
			}
			return nil
		}
	...
	//
	access(all)
	resource Collection: appstoretest8_NFTCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		...
		view fun borrowViewResolver(id: UInt64): &{ViewResolver.Resolver}?{ 
			...
			return appstoretest8_NFTNft as &{ViewResolver.Resolver}
		}
	...
	fun fetch(_ from: Address, id: UInt64): &appstoretest8_NFT.NFT?{ 
		let collection = (getAccount(from).capabilities.get<&appstoretest8_NFT.Collection>(appstoretest8_NFT.CollectionPublicPath)!).borrow() ?? panic("Couldn't get collection")
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&appstoretest8_NFT.Admin>(self.AdminStoragePath)
