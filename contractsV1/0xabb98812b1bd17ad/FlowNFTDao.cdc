...
access(all)
contract FlowNFTDao: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				FlowNFTDao.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return FlowNFTDao.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				FlowNFTDao.maxSupply == nil || FlowNFTDao.totalSupply <= FlowNFTDao.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			FlowNFTDao.totalSupply = FlowNFTDao.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "Flow NFT Dao", symbol: "FNFT", description: "Flow NFT Dao Community", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: FlowNFTDao.VaultStoragePath, receiverPath: FlowNFTDao.ReceiverPublicPath, metadataPath: FlowNFTDao.VaultPublicPath, receiverLinkedType: /private/FlowNFTDaoVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-FlowNFTDao.createEmptyVault(vaultType: Type<@FlowNFTDao.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/FlowNFTDaoVault
		self.ReceiverPublicPath = /public/FlowNFTDaoReceiver
		self.VaultPublicPath = /public/FlowNFTDaoMetadata
		self.MinterStoragePath = /storage/FlowNFTDaoMinter
		self.AdministratorStoragePath = /storage/FlowNFTDaoAdmin
