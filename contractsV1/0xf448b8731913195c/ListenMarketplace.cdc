...
contract ListenMarketplace{ 
	...
	event ListingCompleted(
		listingResourceID: UInt64,
		listenStorefrontResourceID: UInt64,
		purchased: Bool
	)
	...
	access(all)
	let ListenStorefrontStoragePath: StoragePath
	
	// ListenStorefrontPublicPath
	// The public location for a ListenStorefront link.
	access(all)
	let ListenStorefrontPublicPath: PublicPath
	...
	resource interface ListingPublic{ 
		...
		fun borrowNFTs(): [&{NonFungibleToken.NFT}]
		...
		//
		access(TMP_ENTITLEMENT_OWNER)
		fun purchase(payment: @{FungibleToken.Vault}): @{NonFungibleToken.Collection}
	...
	resource Listing: ListingPublic{ 
		...
		fun borrowNFTs(): [&{NonFungibleToken.NFT}]{ 
			...
			for id in self.getDetails().nftIDs{ 
				let ref = (self.nftProviderCapability.borrow()!).borrowNFT(id)
		...
		fun purchase(payment: @{FungibleToken.Vault}): @{NonFungibleToken.Collection}{ 
			...
			let collection <- ListenNFT.createEmptyCollection(nftType: Type<@ListenNFT.Collection>())
			...
			emit ListingCompleted(listingResourceID: self.uuid, listenStorefrontResourceID: self.details.ListenStorefrontID, purchased: self.details.purchased)
			return <-collection
		}
		...
		// initializer
		//
		init(nftProviderCapability: Capability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>, nftType: Type, nftIDs: [UInt64], salePaymentVaultType: Type, saleCuts: [SaleCut], listenStorefrontID: UInt64){ 
			// Store the sale information
			self.details = ListingDetails(nftType: nftType, nftIDs: nftIDs, salePaymentVaultType: salePaymentVaultType, saleCuts: saleCuts, listenStorefrontID: listenStorefrontID)
	...
	//
	access(all)
	resource interface ListenStorefrontManager{ 
		// createListing
	...
	//
	access(all)
	resource interface ListenStorefrontPublic{ 
		access(TMP_ENTITLEMENT_OWNER)
	...
	//
	access(all)
	resource ListenStorefront: ListenStorefrontManager, ListenStorefrontPublic{ 
		...
		fun createListing(nftProviderCapability: Capability<&{NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}>, nftType: Type, nftIDs: [UInt64], salePaymentVaultType: Type, ftReceiverCap: Capability<&{FungibleToken.Receiver}>, salePrice: UFix64)																																																																   // saleCuts: [SaleCut]
			...
			let listing <- create Listing(nftProviderCapability: nftProviderCapability, nftType: nftType, nftIDs: nftIDs, salePaymentVaultType: salePaymentVaultType, saleCuts: [royalty, saleCut], listenStorefrontID: self.uuid)
			...
			destroy oldListing
			emit ListingAvailable(listenStorefrontAddress: self.owner?.address!, listingResourceID: listingResourceID, nftType: nftType, nftIDs: nftIDs, ftVaultType: salePaymentVaultType, price: listingPrice)
		...
		fun borrowListing(listingResourceID: UInt64): &Listing?{ 
			if self.listings[listingResourceID] != nil{ 
				return &self.listings[listingResourceID] as &ListenMarketplace.Listing?
			} else{ 
				return nil
			}
		}
	...
	//
	access(TMP_ENTITLEMENT_OWNER)
	fun createListenStorefront(): @ListenStorefront{ 
	...
	init(){ 
		...
		let listenUSD_Vault =
			self.account.storage.borrow<&ListenUSD.Vault>(from: ListenUSD.VaultStoragePath)
		if listenUSD_Vault == nil{ 
			self.account.storage.save(<-ListenUSD.createEmptyVault(vaultType: Type<@ListenUSD.Vault>()), to: ListenUSD.VaultStoragePath)
		}
		var capability_1 =
			self.account.capabilities.storage.issue<&{FungibleToken.Receiver}>(
				ListenUSD.VaultStoragePath
			)
		self.account.capabilities.publish(capability_1, at: ListenUSD.ReceiverPublicPath)
		var capability_2 =
			self.account.capabilities.storage.issue<&{FungibleToken.Balance}>(
				ListenUSD.VaultStoragePath
			)
		self.account.capabilities.publish(capability_2, at: ListenUSD.BalancePublicPath)
