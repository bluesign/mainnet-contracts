...
access(all)
contract CryptoUnderManagement: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				CryptoUnderManagement.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return CryptoUnderManagement.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				CryptoUnderManagement.maxSupply == nil || CryptoUnderManagement.totalSupply <= CryptoUnderManagement.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			CryptoUnderManagement.totalSupply = CryptoUnderManagement.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafybeiedx2kw6n546lei2x5lcqg5547uq4gnpgjxsuhqlntvwzlh3ijxci"), mediaType: "image")
				let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreibt5mhlpy3xd2shifc7eevza24pcyihbbe26pbwnci76vkxteaade"), mediaType: "image")
				let medias = MetadataViews.Medias([media, bannerMedia])
				return FungibleTokenMetadataViews.FTDisplay(name: "Crypto Under Management", symbol: "CUM", description: "CUM (Crypto Under Management) is a Decentralized Autonomous Organization (DAO) revolutionizing asset management in the cryptocurrency space. Through transparent, community-driven governance, CUM empowers token holders to collectively manage digital assets and make informed investment decisions.", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: CryptoUnderManagement.VaultStoragePath, receiverPath: CryptoUnderManagement.ReceiverPublicPath, metadataPath: CryptoUnderManagement.VaultPublicPath, receiverLinkedType: /private/CryptoUnderManagementVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-CryptoUnderManagement.createEmptyVault(vaultType: Type<@CryptoUnderManagement.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/CryptoUnderManagementVault
		self.ReceiverPublicPath = /public/CryptoUnderManagementReceiver
		self.VaultPublicPath = /public/CryptoUnderManagementMetadata
		self.MinterStoragePath = /storage/CryptoUnderManagementMinter
		self.AdministratorStoragePath = /storage/CryptoUnderManagementAdmin
