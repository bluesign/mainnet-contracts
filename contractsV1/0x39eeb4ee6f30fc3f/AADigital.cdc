...
access(all)
contract AADigital: NonFungibleToken{ 
	access(all)
	...
	access(all)
	resource interface AADigitalCollectionPublic{ 
		access(all)
		...
		fun borrowAADigital(id: UInt64): &AADigital.NFT?{ 
			post{ 
	...
	access(all)
	resource Collection: AADigitalCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
	...
	resource NFTMinter{ 
		...
		fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, name: String, description: String, thumbnail: String){ 
			...
			// create a new NFT
			var newNFT <- create NFT(id: AADigital.totalSupply + 100, name: name, description: description, thumbnail: thumbnail)
			let id = newNFT.id
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&AADigital.Collection>(self.CollectionStoragePath)
