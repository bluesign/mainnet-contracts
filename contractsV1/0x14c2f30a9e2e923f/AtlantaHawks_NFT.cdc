...
access(all)
contract AtlantaHawks_NFT: NonFungibleToken{ 
	...
	resource Series{ 
		...
		//
		access(TMP_ENTITLEMENT_OWNER)
		fun mintAtlantaHawks_NFT(recipient: &{NonFungibleToken.CollectionPublic}, tokenId: UInt64, setId: UInt32){ 
			pre{ 
		...
		//
		access(TMP_ENTITLEMENT_OWNER)
		fun mintEditionAtlantaHawks_NFT(recipient: &{NonFungibleToken.CollectionPublic}, tokenId: UInt64, setId: UInt32, edition: UInt32){ 
			pre{ 
				self.numberEditionsMintedPerSet[setId] != nil:
					"The Set does not exist."
				self.numberEditionsMintedPerSet[setId]! <= AtlantaHawks_NFT.getSetMaxEditions(setId: setId)!:
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun batchMintAtlantaHawks_NFT(recipient: &{NonFungibleToken.CollectionPublic}, setId: UInt32, tokenIds: [UInt64]){ 
			pre{ 
				tokenIds.length > 0:
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
					...
					for metadataKey in (AtlantaHawks_NFT.getSetMetadata(setId: self.setId)!).keys{ 
						...
						if metadataKey.length >= 13{ 
							if metadataKey.slice(from: 0, upTo: 13) == "royalty_addr_"{ 
								...
								if cutValue != 0.0{ 
									royalties.append(MetadataViews.Royalty(receiver: getAccount(royaltyAddress).capabilities.get<&{FungibleToken.Vault}>(royaltyReceiver), cut: cutValue, description: AtlantaHawks_NFT.getSetMetadataByField(setId: self.setId, field: "royalty_desc_".concat(royaltyName))!))
								}
							}
						}
					}
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: AtlantaHawks_NFT.CollectionStoragePath, publicPath: AtlantaHawks_NFT.CollectionPublicPath, publicCollection: Type<&AtlantaHawks_NFT.Collection>(), publicLinkedType: Type<&AtlantaHawks_NFT.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-AtlantaHawks_NFT.createEmptyCollection(nftType: Type<@AtlantaHawks_NFT.Collection>())
						})
				...
				case Type<MetadataViews.Medias>():
					return MetadataViews.Medias([MetadataViews.Media(file: MetadataViews.HTTPFile(url: AtlantaHawks_NFT.getSetMetadataByField(setId: self.setId, field: "image")!), mediaType: self.getMimeType())])
			}
			return nil
		}
	...
	access(all)
	resource interface AtlantaHawks_NFTCollectionPublic{ 
		access(all)
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowAtlantaHawks_NFT(id: UInt64): &AtlantaHawks_NFT.NFT?{ 
	...
	//
	access(all)
	resource Collection: AtlantaHawks_NFTCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		...
		view fun borrowViewResolver(id: UInt64): &{ViewResolver.Resolver}?{ 
			...
			return AtlantaHawks_NFTNft as &{ViewResolver.Resolver}
		}
	...
	//
	access(TMP_ENTITLEMENT_OWNER)
	fun fetch(_ from: Address, id: UInt64): &AtlantaHawks_NFT.NFT?{ 
		let collection = getAccount(from).capabilities.get<&AtlantaHawks_NFT.Collection>(AtlantaHawks_NFT.CollectionPublicPath).borrow<&AtlantaHawks_NFT.Collection>() ?? panic("Couldn't get collection")
	...
	access(TMP_ENTITLEMENT_OWNER)
	fun getAllSeries(): [AtlantaHawks_NFT.SeriesData]{ 
	...
	access(TMP_ENTITLEMENT_OWNER)
	fun getAllSets(): [AtlantaHawks_NFT.NFTSetData]{ 
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&AtlantaHawks_NFT.Admin>(self.AdminStoragePath)
