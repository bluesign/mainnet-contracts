...
access(all)
contract SpaceToursAgency: NonFungibleToken{ 
	access(all)
	...
	access(all)
	resource interface SpaceToursAgencyCollectionPublic{ 
		access(all)
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowSpaceToursAgency(id: UInt64): &SpaceToursAgency.NFT?{ 
			post{ 
	...
	access(all)
	resource Collection: SpaceToursAgencyCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		access(all)
	...
	resource NFTMinter{ 
		...
		fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, metadata:{ String: String}): &{NonFungibleToken.NFT}{ 
			...
			// create a new NFT
			var newNFT <- create NFT(id: SpaceToursAgency.totalSupply, creator: creator, metadata: metadata)
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&SpaceToursAgency.Collection>(self.CollectionStoragePath)
