...
import TKNZToken from "./TKNZToken.cdc"
...
contract TKNZMarket{ 
	...
	access(all)
	event NFTListed(id: UInt64, price: UFix64, seller: Address?)
	
	// emitted when the price of a listed NFT has changed
	access(all)
	event NFTPriceChanged(id: UInt64, newPrice: UFix64, seller: Address?)
	
	// emitted when a token is purchased from the market
	access(all)
	event NFTPurchased(id: UInt64, price: UFix64, seller: Address?)
	
	// emitted when a NFT has been withdrawn from the sale
	access(all)
	event NFTWithdrawn(id: UInt64, owner: Address?)
	...
	resource SaleCollection: SalePublic{ 
		...
		access(self)
		var ownerCollection: Capability<&TKNZ.Collection>
		...
		fun changePercentage(_ newPercent: UFix64){ 
			pre{ 
				newPercent <= 1.0:
					"Cannot drop cut percentage to greater than 100%"
			}
