...
contract FLOATVerifiers{ 
	...
	access(all)
	struct Timelock: MusicPeaksAttendanceToken.IVerifier{ 
	...
	access(all)
	struct Secret: MusicPeaksAttendanceToken.IVerifier{ 
	...
	// delete their FLOATs.
	access(all)
	struct Limited: MusicPeaksAttendanceToken.IVerifier{ 
		access(all)
		var capacity: UInt64
	...
	// from the list.
	access(all)
	struct MultipleSecret: MusicPeaksAttendanceToken.IVerifier{ 
		access(self)
	...
	access(all)
	struct SecretV2: MusicPeaksAttendanceToken.IVerifier{ 
		access(all)
