...
access(all)
contract MomentableArt: NonFungibleToken, ViewResolver{ 
	...
	resource NFT: NonFungibleToken.NFT, ViewResolver.Resolver, NiftoryNonFungibleToken.NFTPublic{ 
		...
		view fun _contract(): &{NiftoryNonFungibleToken.ManagerPublic}{ 
			return MomentableArt._contract()
		}
	...
	resource Manager: NiftoryNonFungibleToken.ManagerPublic, NiftoryNonFungibleToken.ManagerPrivate{ 
		...
		fun getNFTCollectionData(): MetadataViews.NFTCollectionData{ 
			return NiftoryNFTRegistry.buildNFTCollectionData(MomentableArt.REGISTRY_ADDRESS, MomentableArt.REGISTRY_BRAND, fun (): @{NonFungibleToken.Collection}{ 
					return <-MomentableArt.createEmptyCollection(nftType: Type<@MomentableArt.Collection>())
				})
		}
		...
		fun modifyContractMetadata(): &AnyStruct{ 
			...
			return (&MomentableArt.metadata as &AnyStruct?)!
		}
		...
		fun _getMetadataViewsManagerPrivate(): &MetadataViewsManager.Manager{ 
			...
			let manager = MomentableArt.account.capabilities.get<&MetadataViewsManager.Manager>(record.metadataViewsManager.paths.private).borrow()!
			return manager
		}
		...
		fun _getSetManagerPrivate(): &MutableMetadataSetManager.Manager{ 
			let record = NiftoryNFTRegistry.getRegistryRecord(MomentableArt.REGISTRY_ADDRESS, MomentableArt.REGISTRY_BRAND)
			let setManager = MomentableArt.account.capabilities.get<&MutableMetadataSetManager.Manager>(record.setManager.paths.private).borrow()!
			return setManager
		}
	...
	init(nftManagerProxy: &{NiftoryNonFungibleTokenProxy.Public, NiftoryNonFungibleTokenProxy.Private}){ 
		...
		self.account.storage.save<@MutableMetadataSetManager.Manager>(<-MutableMetadataSetManager._create(name: "MomentableArt", description: "The set manager for MomentableArt."), to: record.setManager.paths.storage)
