...
access(all)
contract UntoldPerksNFT: NonFungibleToken, ViewResolver{ 
	...
	resource NFT: NonFungibleToken.NFT, ViewResolver.Resolver, NiftoryNonFungibleToken.NFTPublic{ 
		...
		view fun _contract(): &{NiftoryNonFungibleToken.ManagerPublic}{ 
			return UntoldPerksNFT._contract()
		}
	...
	resource Manager: NiftoryNonFungibleToken.ManagerPublic, NiftoryNonFungibleToken.ManagerPrivate{ 
		...
		fun getNFTCollectionData(): MetadataViews.NFTCollectionData{ 
			return NiftoryNFTRegistry.buildNFTCollectionData(UntoldPerksNFT.REGISTRY_ADDRESS, UntoldPerksNFT.REGISTRY_BRAND, fun (): @{NonFungibleToken.Collection}{ 
					return <-UntoldPerksNFT.createEmptyCollection(nftType: Type<@UntoldPerksNFT.Collection>())
				})
		}
		...
		fun modifyContractMetadata(): &AnyStruct{ 
			...
			return (&UntoldPerksNFT.metadata as &AnyStruct?)!
		}
		...
		fun _getMetadataViewsManagerPrivate(): &MetadataViewsManager.Manager{ 
			...
			let manager = UntoldPerksNFT.account.capabilities.get<&MetadataViewsManager.Manager>(record.metadataViewsManager.paths.private).borrow()!
			return manager
		}
		...
		fun _getSetManagerPrivate(): &MutableMetadataSetManager.Manager{ 
			let record = NiftoryNFTRegistry.getRegistryRecord(UntoldPerksNFT.REGISTRY_ADDRESS, UntoldPerksNFT.REGISTRY_BRAND)
			let setManager = UntoldPerksNFT.account.capabilities.get<&MutableMetadataSetManager.Manager>(record.setManager.paths.private).borrow()!
			return setManager
		}
	...
	init(nftManagerProxy: &{NiftoryNonFungibleTokenProxy.Public, NiftoryNonFungibleTokenProxy.Private}){ 
		...
		self.account.storage.save<@MutableMetadataSetManager.Manager>(<-MutableMetadataSetManager._create(name: "UntoldPerksNFT", description: "The set manager for UntoldPerksNFT."), to: record.setManager.paths.storage)
