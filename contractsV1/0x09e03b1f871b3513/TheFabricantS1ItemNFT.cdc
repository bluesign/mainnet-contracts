...
access(all)
contract TheFabricantS1ItemNFT: NonFungibleToken{ 
	...
	access(all)
	event ItemDataCreated(itemDataID: UInt32, coCreator: Address, metadatas:{ String: TheFabricantS1ItemNFT.Metadata})
	...
	struct ItemData{ 
		...
		// other metadata
		access(self)
		let metadatas:{ String: TheFabricantS1ItemNFT.Metadata}
	...
	resource NFT: NonFungibleToken.NFT{ 
		...
		// Royalty struct
		access(all)
		let royaltyVault: TheFabricantS1ItemNFT.Royalty
		...
		access(self)
		var garment: @TheFabricantS1GarmentNFT.NFT?
		
		// this is where the material nft is stored, it cannot be moved out
		access(self)
		var material: @TheFabricantS1MaterialNFT.NFT?
		...
		access(contract)
		fun split(garmentCap: Capability<&{TheFabricantS1GarmentNFT.GarmentCollectionPublic}>, materialCap: Capability<&{TheFabricantS1MaterialNFT.MaterialCollectionPublic}>){ 
			pre{ 
				!self.isDead:
		...
		fun borrowGarment(): &TheFabricantS1GarmentNFT.NFT?{ 
			let garmentOptional <- self.garment <- nil
			let garment <- garmentOptional!
			let garmentRef = &garment as &TheFabricantS1GarmentNFT.NFT
			self.garment <-! garment
			return garmentRef
		}
		...
		fun borrowMaterial(): &TheFabricantS1MaterialNFT.NFT?{ 
			let materialOptional <- self.material <- nil
			let material <- materialOptional!
			let materialRef = &material as &TheFabricantS1MaterialNFT.NFT
			self.material <-! material
			return materialRef
		}
