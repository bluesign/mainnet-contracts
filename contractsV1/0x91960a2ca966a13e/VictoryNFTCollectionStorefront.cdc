...
access(all)
contract VictoryNFTCollectionStorefront{ 
	// SaleOffer events.
	//
	...
	resource SaleOffer: SaleOfferPublicView{ 
		...
		access(self)
		let sellerItemProvider: Capability<&VictoryNFTCollectionItem.Collection>
		...
		//
		init(sellerItemProvider: Capability<&VictoryNFTCollectionItem.Collection>, bundleID: UInt64, sellerPaymentReceiver: Capability<&{FungibleToken.Receiver}>, price: UFix64, saleType: UInt8, startTime: UInt32, endTime: UInt32, targetPrice: UFix64, royaltyPaymentReceiver: Capability<&{FungibleToken.Receiver}>, royaltyFactor: UFix64){ 
			pre{ 
	...
	resource Collection: CollectionManager, CollectionPurchaser, CollectionPublic{ 
		...
		fun borrowSaleItem(bundleID: UInt64): &SaleOffer?{ 
			if self.saleOffers[bundleID] == nil{ 
				...
				return &self.saleOffers[bundleID] as &VictoryNFTCollectionStorefront.SaleOffer?
			}
		}
