...
access(all)
contract DefLeppard: NonFungibleToken, TiblesApp, TiblesNFT, TiblesProducer{ 
	access(all)
	let appId: String
	...
	resource NFT: NonFungibleToken.NFT, TiblesNFT.INFT, ViewResolver.Resolver{ 
		...
		fun displayData():{ String: String}{ 
			...
			if metadata.containsKey("pack"){ 
				return{ "name": "DefLeppard pack", "description": "A DefLeppard pack", "imageUrl": "https://i.tibles.com/m/leppardvault-flow-icon.png"}
			}
		...
		fun nftCollectionData(): MetadataViews.NFTCollectionData{ 
			return MetadataViews.NFTCollectionData(storagePath: DefLeppard.CollectionStoragePath, publicPath: DefLeppard.PublicCollectionPath, publicCollection: Type<&DefLeppard.Collection>(), publicLinkedType: Type<&DefLeppard.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
					return <-DefLeppard.createEmptyCollection(nftType: Type<@DefLeppard.Collection>())
				})
		}
		
		access(TMP_ENTITLEMENT_OWNER)
		fun nftCollectionDisplay(): MetadataViews.NFTCollectionDisplay{ 
			let squareMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://i.tibles.com/m/leppardvault-flow-icon.png"), mediaType: "image/svg+xml")
			let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://i.tibles.com/m/leppardvault-flow-collection-banner.png"), mediaType: "image/png")
			...
			return MetadataViews.NFTCollectionDisplay(name: "Def Leppard Collection by Leppard Vault and Tibles", description: "Leppard Vault Tibles is a digital trading card collecting experience by Tibles, made just for Def Leppard fans, backed by the FLOW blockchain.", externalURL: MetadataViews.ExternalURL("https://leppardvault.tibles.com"), squareImage: squareMedia, bannerImage: bannerMedia, socials: socials)
		}
	...
	access(contract)
	fun createNewPackMinter(id: String, contentCapability: Capability, acctAddress: Address): @PackMinter{ 
