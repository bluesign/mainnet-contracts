...
contract AlchemyMetadataWrapperMainnetShard1{ 
	...
	fun getCnnNFT(owner: &Account, id: UInt64): NFTData?{ 
		...
		let col =
			(
				owner.capabilities.get<&{CNN_NFT.CNN_NFTCollectionPublic}>(
	...
	fun getBeam(owner: &Account, id: UInt64): NFTData?{ 
		...
		let col =
			(owner.capabilities.get<&{Beam.BeamCollectionPublic}>(Beam.CollectionPublicPath)!)
				.borrow()
		if col == nil{ 
			return nil
		}
	...
	fun getEternalShard(owner: &Account, id: UInt64): NFTData?{ 
		...
		let col =
			(
				owner.capabilities.get<&{Shard.ShardCollectionPublic}>(
	...
	fun getMatrixWorldFlowFest(owner: &Account, id: UInt64): NFTData?{ 
		...
		let col =
			(
				...
				>(MatrixWorldFlowFestNFT.CollectionPublicPath)!
			).borrow()
		if col == nil{ 
			return nil
		}
	...
	fun getNFTIDs(ownerAddress: Address):{ String: [UInt64]}{ 
		...
		if let col =
			(owner.capabilities.get<&{Gaia.CollectionPublic}>(Gaia.CollectionPublicPath)!)
				.borrow(){ 
			ids["Gaia"] = col.getIDs()
		}
		if let col =
