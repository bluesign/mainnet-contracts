...
contract FlowStakingCollection{ 
	...
	/// Account paths
	access(all)
	let StakingCollectionStoragePath: StoragePath
	
	access(all)
	let StakingCollectionPrivatePath: PrivatePath
	
	access(all)
	let StakingCollectionPublicPath: PublicPath
	
	/// Events
	access(all)
	event NodeAddedToStakingCollection(
		nodeID: String,
		role: UInt8,
	...
	access(all)
	event DelegatorAddedToStakingCollection(
		nodeID: String,
		delegatorID: UInt32,
		amountCommitted: UFix64,
		address: Address?
	...
	access(all)
	event NodeRemovedFromStakingCollection(nodeID: String, role: UInt8, address: Address?)
	
	access(all)
	event DelegatorRemovedFromStakingCollection(
		nodeID: String,
		delegatorID: UInt32,
		address: Address?
	)
	...
	access(all)
	event MachineAccountCreated(nodeID: String, role: UInt8, address: Address)
	
	/// Struct that stores delegator ID info
	access(all)
	struct DelegatorIDs{ 
		access(all)
		let delegatorNodeID: String
	...
	struct MachineAccountInfo{ 
		...
		access(contract)
		let machineAccountVaultProvider: Capability<&FlowToken.Vault>
	...
	access(all)
	resource interface StakingCollectionPublic{ 
		access(all)
		var lockedTokensUsed: UFix64
	...
	access(all)
	resource StakingCollection: StakingCollectionPublic{ 
		
		/// unlocked vault
		access(self)
		var unlockedVault: Capability<&FlowToken.Vault>
		
		/// locked vault
		/// will be nil if the account has no corresponding locked account
		access(self)
		var lockedVault: Capability<&FlowToken.Vault>?
		...
		access(self)
		var nodeStakers: @{String: FlowIDTableStaking.NodeStaker}
		
		access(self)
		var nodeDelegators: @{String: FlowIDTableStaking.NodeDelegator}
		...
		access(self)
		var tokenHolder: Capability<&LockedTokens.TokenHolder>?
		...
		access(self)
		var machineAccounts:{ String: MachineAccountInfo}
		...
		access(self)
		fun getTokens(amount: UFix64): @{FungibleToken.Vault}{ 
		...
		access(self)
		fun depositTokens(from: @{FungibleToken.Vault}){ 
			pre{ 
		...
		access(self)
		fun registerMachineAccount(nodeReference: &FlowIDTableStaking.NodeStaker, payer: AuthAccount): AuthAccount?{ 
			let nodeInfo = FlowIDTableStaking.NodeInfo(nodeID: nodeReference.id)
		...
		access(self)
		view fun borrowNode(_ nodeID: String): &FlowIDTableStaking.NodeStaker?{ 
			if self.nodeStakers[nodeID] != nil{ 
		...
		access(self)
		view fun borrowDelegator(nodeID: String, delegatorID: UInt32): &FlowIDTableStaking.NodeDelegator?{ 
			if self.nodeDelegators[nodeID] != nil{ 
				let delegatorRef = (&self.nodeDelegators[nodeID] as &FlowIDTableStaking.NodeDelegator?)!
				if delegatorRef.id == delegatorID{ 
					return delegatorRef
