...
access(all)
contract UntoldTest: NonFungibleToken, ViewResolver{ 
	...
	resource Manager: NiftoryNonFungibleToken.ManagerPublic, NiftoryNonFungibleToken.ManagerPrivate{ 
		...
		fun getNFTCollectionData(): MetadataViews.NFTCollectionData{ 
			return NiftoryNFTRegistry.buildNFTCollectionData(UntoldTest.REGISTRY_ADDRESS, UntoldTest.REGISTRY_BRAND, fun (): @{NonFungibleToken.Collection}{ 
					return <-UntoldTest.createEmptyCollection(nftType: Type<@UntoldTest.Collection>())
				})
		}
		...
		fun modifyContractMetadata(): &AnyStruct{ 
			...
			return (&UntoldTest.metadata as &AnyStruct?)!
		}
		...
		fun _getMetadataViewsManagerPrivate(): &MetadataViewsManager.Manager{ 
			...
			let manager = UntoldTest.account.capabilities.get<&MetadataViewsManager.Manager>(record.metadataViewsManager.paths.private).borrow()!
			return manager
		}
		...
		fun _getSetManagerPrivate(): &MutableMetadataSetManager.Manager{ 
			let record = NiftoryNFTRegistry.getRegistryRecord(UntoldTest.REGISTRY_ADDRESS, UntoldTest.REGISTRY_BRAND)
			let setManager = UntoldTest.account.capabilities.get<&MutableMetadataSetManager.Manager>(record.setManager.paths.private).borrow()!
			return setManager
		}
	...
	init(nftManagerProxy: &{NiftoryNonFungibleTokenProxy.Public, NiftoryNonFungibleTokenProxy.Private}){ 
		...
		self.account.storage.save<@MutableMetadataSetManager.Manager>(<-MutableMetadataSetManager._create(name: "UntoldTest", description: "The set manager for UntoldTest."), to: record.setManager.paths.storage)
