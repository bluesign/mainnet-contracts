...
access(all)
contract Eclipse: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				Eclipse.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return Eclipse.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				Eclipse.maxSupply == nil || Eclipse.totalSupply <= Eclipse.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			Eclipse.totalSupply = Eclipse.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreiaxmzssokqqovpsf7zsou3yjwzipchyphjnz22cxsymzvzcadt3um"), mediaType: "image")
				let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreiaxmzssokqqovpsf7zsou3yjwzipchyphjnz22cxsymzvzcadt3um"), mediaType: "image")
				let medias = MetadataViews.Medias([media, bannerMedia])
				return FungibleTokenMetadataViews.FTDisplay(name: "Eclipse", symbol: "ECLPS", description: "solar eclipse  ", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: Eclipse.VaultStoragePath, receiverPath: Eclipse.ReceiverPublicPath, metadataPath: Eclipse.VaultPublicPath, receiverLinkedType: /private/EclipseVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-Eclipse.createEmptyVault(vaultType: Type<@Eclipse.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/EclipseVault
		self.ReceiverPublicPath = /public/EclipseReceiver
		self.VaultPublicPath = /public/EclipseMetadata
		self.MinterStoragePath = /storage/EclipseMinter
		self.AdministratorStoragePath = /storage/EclipseAdmin
