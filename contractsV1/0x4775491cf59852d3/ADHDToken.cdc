...
access(all)
contract ADHDToken: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				ADHDToken.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return ADHDToken.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				ADHDToken.maxSupply == nil || ADHDToken.totalSupply <= ADHDToken.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			ADHDToken.totalSupply = ADHDToken.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "ADHD DAO", symbol: "ADHD", description: "A DAO for those that suffer with ADHD.", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: ADHDToken.VaultStoragePath, receiverPath: ADHDToken.ReceiverPublicPath, metadataPath: ADHDToken.VaultPublicPath, receiverLinkedType: /private/ADHDTokenVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-ADHDToken.createEmptyVault(vaultType: Type<@ADHDToken.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/ADHDTokenVault
		self.ReceiverPublicPath = /public/ADHDTokenReceiver
		self.VaultPublicPath = /public/ADHDTokenMetadata
		self.MinterStoragePath = /storage/ADHDTokenMinter
		self.AdministratorStoragePath = /storage/ADHDTokenAdmin
