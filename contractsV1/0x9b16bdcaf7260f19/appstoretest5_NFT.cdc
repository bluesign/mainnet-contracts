...
contract appstoretest5_NFT: NonFungibleToken{ 
	...
	fun fetch(_ from: Address, id: UInt64): &appstoretest5_NFT.NFT?{ 
		let collection = (getAccount(from).capabilities.get<&appstoretest5_NFT.Collection>(appstoretest5_NFT.CollectionPublicPath)!).borrow() ?? panic("Couldn't get collection")
