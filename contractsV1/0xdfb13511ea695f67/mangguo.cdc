...
access(all)
contract mangguo: NonFungibleToken{ 
	access(all)
	...
	access(all)
	resource interface mangguoCollectionPublic{ 
		access(all)
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowmangguo(id: UInt64): &mangguo.NFT?{ 
			post{ 
	...
	access(all)
	resource Collection: mangguoCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		access(all)
	...
	resource NFTMinter{ 
		...
		fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, metadata:{ String: String}): &{NonFungibleToken.NFT}{ 
			...
			// create a new NFT
			var newNFT <- create NFT(id: mangguo.totalSupply, creator: creator, metadata: metadata)
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&mangguo.Collection>(self.CollectionStoragePath)
