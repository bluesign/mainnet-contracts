...
contract EMSwap{ 
	...
	access(account)
	let SwapProposalAdminStoragePath: StoragePath
	
	// SwapProposalAdminPrivatePath
	// Storage directory used to store SwapProposalAdmin object
	access(account)
	let SwapProposalAdminPrivatePath: PrivatePath
	...
	struct UserOffer: Readable{ 
		...
		access(all)
		fun getReadable():{ String: [{String: String}]}{ 
	...
	struct SwapProposal{ 
		...
		init(
			...
			assert(
				expirationOffsetMinutes >= EMSwap.SwapProposalMinExpirationMinutes,
				message: "expirationOffsetMinutes must be greater than or equal to EMSwap.SwapProposalMinExpirationMinutes"
			)
			assert(
				expirationOffsetMinutes <= EMSwap.SwapProposalMaxExpirationMinutes,
		...
		access(contract)
		fun getReadableSwapProposal(): ReadableSwapProposal{ 
	...
	resource interface SwapCollectionManager{ 
		...
		access(all)
		fun getAllProposals():{ String: ReadableSwapProposal}
	...
	resource interface SwapCollectionPublic{ 
		access(all)
		fun getProposal(id: String): ReadableSwapProposal
		
		access(all)
		fun getUserOffer(proposalId: String, leftOrRight: String): UserOffer
	...
	resource SwapCollection: SwapCollectionManager, SwapCollectionPublic{ 
		...
		access(all)
		fun createProposal(leftUserOffer: UserOffer, rightUserOffer: UserOffer, leftUserCapabilities: UserCapabilities, fees: [Fee]?, expirationOffsetMinutes: UFix64?): String{ 
	...
	resource interface SwapProposalManager{ 
		...
		access(all)
		fun getProposalCreationStatus(): Bool
	}
	
	// SwapProposalAdmin
	// This object provides admin controls for swap proposals
	access(all)
	resource SwapProposalAdmin: SwapProposalManager{ 
	...
	init(){ 
		...
		var capability_1 =
			self.account.capabilities.storage.issue<&{SwapProposalManager}>(
				self.SwapProposalAdminStoragePath
			)
		self.account.capabilities.publish(capability_1, at: self.SwapProposalAdminPrivatePath)
	}
}
