...
import InfluencerRegistry from "./InfluencerRegistry.cdc"
...
contract Market{ 
	...
	access(all)
	event MomentListed(id: UInt64, price: UFix64, seller: Address?)
	
	// emitted when the price of a listed moment has changed
	access(all)
	event MomentPriceChanged(id: UInt64, newPrice: UFix64, seller: Address?)
	
	// emitted when a token is purchased from the market
	access(all)
	event MomentPurchased(id: UInt64, price: UFix64, seller: Address?)
	
	// emitted when a moment has been withdrawn from the sale
	access(all)
	event MomentWithdrawn(id: UInt64, owner: Address?)
	...
	access(all)
	event InfluencerCutReceived(name: String, ftType: Type, cut: UFix64)
	...
	resource SaleCollection: SalePublic{ 
		...
		access(self)
		var forSale: @Eternal.Collection
		...
		init(ftType: Type, ownerCapability: Capability, beneficiaryCapability: Capability, cutPercentage: UFix64){ 
			...
			self.forSale <- Eternal.createEmptyCollection(nftType: Type<@Eternal.Collection>()) as! @Eternal.Collection
		...
		fun purchase(tokenID: UInt64, buyTokens: @{FungibleToken.Vault}): @Eternal.NFT{ 
			pre{ 
				...
				buyTokens.isInstance(self.ftType):
			...
			let influencerCap = InfluencerRegistry.getCapability(name: influencerName, ftType: self.ftType) ?? panic("Cannot find the influencer in the registry")
			let influencerReceiverRef = influencerCap.borrow<&{FungibleToken.Receiver}>() ?? panic("Cannot find a token receiver for the influencer")
		...
		access(self)
		fun getInfluencerNameFromMoment(moment: &Eternal.NFT): String{ 
	...
	fun createSaleCollection(
		...
		beneficiaryCapability: Capability,
