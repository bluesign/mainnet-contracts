...
access(all)
contract SwoleDoge: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				SwoleDoge.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return SwoleDoge.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				SwoleDoge.maxSupply == nil || SwoleDoge.totalSupply <= SwoleDoge.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			SwoleDoge.totalSupply = SwoleDoge.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "SwoleDoge", symbol: "SWOGE", description: "The number 1 dog coin on flow!Forget $DOGE, forget $BONK, foget $WIF, forget $SHIB, now is the time for $SWOGE!Liquidity pool to be created on full funding.", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: SwoleDoge.VaultStoragePath, receiverPath: SwoleDoge.ReceiverPublicPath, metadataPath: SwoleDoge.VaultPublicPath, receiverLinkedType: /private/SwoleDogeVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-SwoleDoge.createEmptyVault(vaultType: Type<@SwoleDoge.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/SwoleDogeVault
		self.ReceiverPublicPath = /public/SwoleDogeReceiver
		self.VaultPublicPath = /public/SwoleDogeMetadata
		self.MinterStoragePath = /storage/SwoleDogeMinter
		self.AdministratorStoragePath = /storage/SwoleDogeAdmin
