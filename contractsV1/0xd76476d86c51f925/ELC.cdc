...
contract ELC: FungibleToken{ 
	...
	resource Minter{ 
		...
		//
		access(TMP_ENTITLEMENT_OWNER)
		fun mintTokens(amount: UFix64): @ELC.Vault{ 
			pre{ 
				amount > UFix64(0):
	...
	init(){ 
		...
		self.account.storage.save(<-admin, to: /storage/elcAdmin)
		
		// mint tokens
		let tokenVault <- minter.mintTokens(amount: self.totalSupply)
		self.account.storage.save(<-tokenVault, to: /storage/elcVault)
		destroy minter
		
		// Create a public capability to the stored Vault that only exposes
		// the `balance` field through the `Balance` interface
		var capability_1 = self.account.capabilities.storage.issue<&ELC.Vault>(/storage/elcVault)
		self.account.capabilities.publish(capability_1, at: /public/elcBalance)
		
		// Create a public capability to the stored Vault that only exposes
		// the `deposit` method through the `Receiver` interface
		var capability_2 = self.account.capabilities.storage.issue<&{FungibleToken.Receiver}>(/storage/elcVault)
		self.account.capabilities.publish(capability_2, at: /public/elcReceiver)
