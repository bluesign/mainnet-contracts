...
access(all)
contract TetrisObjects{ 
	// GameObject for a TetrisPiece
	access(all)
	struct TetrisPiece: GameEngine.GameObject{ 
		// Standard fields
		access(all)
		var id: UInt64
		...
		fun toMap():{ String: String}{ 
			...
			return{ "id": self.id.toString(), "type": self.type, "doesTick": doesTick, "x": (self.referencePoint[0]!).toString(), "y": (self.referencePoint[1]!).toString(), "shape": self.shape, "rotation": (self.rotation!).toString(), "color": self.color, "dropRate": self.dropRate.toString(), "lastDropTick": self.lastDropTick.toString()}
		}
		...
		fun tick(tickCount: UInt64, events: [GameEngine.PlayerEvent], level:{ GameEngine.Level}, callbacks:{ String: fun (AnyStruct?): AnyStruct?}){ 
			...
			if needsRedraw{ 
				...
				if collisionMap.length > 0{ 
					...
					(callbacks["redraw"]!)({"prev": prevTetrimone, "new": self})
				}
			}
		}
	}
	...
	access(all)
	struct LockedInTetrisPiece: GameEngine.GameObject{ 
		// Standard fields
		access(all)
		var id: UInt64
