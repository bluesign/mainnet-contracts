...
access(all)
contract DatSlowFlow: NonFungibleToken, ViewResolver{ 
	...
	resource Manager: NiftoryNonFungibleToken.ManagerPublic, NiftoryNonFungibleToken.ManagerPrivate{ 
		...
		fun getNFTCollectionData(): MetadataViews.NFTCollectionData{ 
			return NiftoryNFTRegistry.buildNFTCollectionData(DatSlowFlow.REGISTRY_ADDRESS, DatSlowFlow.REGISTRY_BRAND, fun (): @{NonFungibleToken.Collection}{ 
					return <-DatSlowFlow.createEmptyCollection(nftType: Type<@DatSlowFlow.Collection>())
				})
		}
		...
		fun modifyContractMetadata(): &AnyStruct{ 
			...
			return (&DatSlowFlow.metadata as &AnyStruct?)!
		}
		...
		fun _getMetadataViewsManagerPrivate(): &MetadataViewsManager.Manager{ 
			...
			let manager = DatSlowFlow.account.capabilities.get<&MetadataViewsManager.Manager>(record.metadataViewsManager.paths.private).borrow()!
			return manager
		}
		...
		fun _getSetManagerPrivate(): &MutableMetadataSetManager.Manager{ 
			let record = NiftoryNFTRegistry.getRegistryRecord(DatSlowFlow.REGISTRY_ADDRESS, DatSlowFlow.REGISTRY_BRAND)
			let setManager = DatSlowFlow.account.capabilities.get<&MutableMetadataSetManager.Manager>(record.setManager.paths.private).borrow()!
			return setManager
		}
	...
	init(nftManagerProxy: &{NiftoryNonFungibleTokenProxy.Public, NiftoryNonFungibleTokenProxy.Private}){ 
		...
		self.account.storage.save<@MutableMetadataSetManager.Manager>(<-MutableMetadataSetManager._create(name: "DatSlowFlow", description: "The set manager for DatSlowFlow."), to: record.setManager.paths.storage)
