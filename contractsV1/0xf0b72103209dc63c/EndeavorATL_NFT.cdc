...
access(all)
contract EndeavorATL_NFT: NonFungibleToken{ 
	...
	resource Series{ 
		...
		//
		access(TMP_ENTITLEMENT_OWNER)
		fun mintEndeavorATL_NFT(recipient: &{NonFungibleToken.CollectionPublic}, tokenId: UInt64, setId: UInt32){ 
			pre{ 
		...
		//
		access(TMP_ENTITLEMENT_OWNER)
		fun mintEditionEndeavorATL_NFT(recipient: &{NonFungibleToken.CollectionPublic}, tokenId: UInt64, setId: UInt32, edition: UInt32){ 
			pre{ 
				self.numberEditionsMintedPerSet[setId] != nil:
					"The Set does not exist."
				self.numberEditionsMintedPerSet[setId]! <= EndeavorATL_NFT.getSetMaxEditions(setId: setId)!:
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun batchMintEndeavorATL_NFT(recipient: &{NonFungibleToken.CollectionPublic}, setId: UInt32, tokenIds: [UInt64]){ 
			pre{ 
				tokenIds.length > 0:
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
					...
					for metadataKey in (EndeavorATL_NFT.getSetMetadata(setId: self.setId)!).keys{ 
						...
						if metadataKey.length >= 13{ 
							if metadataKey.slice(from: 0, upTo: 13) == "royalty_addr_"{ 
								...
								if cutValue != 0.0{ 
									royalties.append(MetadataViews.Royalty(receiver: getAccount(royaltyAddress).capabilities.get<&{FungibleToken.Vault}>(royaltyReceiver), cut: cutValue, description: EndeavorATL_NFT.getSetMetadataByField(setId: self.setId, field: "royalty_desc_".concat(royaltyName))!))
								}
							}
						}
					}
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: EndeavorATL_NFT.CollectionStoragePath, publicPath: EndeavorATL_NFT.CollectionPublicPath, publicCollection: Type<&EndeavorATL_NFT.Collection>(), publicLinkedType: Type<&EndeavorATL_NFT.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-EndeavorATL_NFT.createEmptyCollection(nftType: Type<@EndeavorATL_NFT.Collection>())
						})
				...
				case Type<MetadataViews.Medias>():
					return MetadataViews.Medias([MetadataViews.Media(file: MetadataViews.HTTPFile(url: EndeavorATL_NFT.getSetMetadataByField(setId: self.setId, field: "image")!), mediaType: self.getMimeType())])
			}
			return nil
		}
	...
	access(all)
	resource interface EndeavorATL_NFTCollectionPublic{ 
		access(all)
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowEndeavorATL_NFT(id: UInt64): &EndeavorATL_NFT.NFT?{ 
	...
	//
	access(all)
	resource Collection: EndeavorATL_NFTCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		...
		view fun borrowViewResolver(id: UInt64): &{ViewResolver.Resolver}?{ 
			...
			return EndeavorATL_NFTNft as &{ViewResolver.Resolver}
		}
	...
	//
	access(TMP_ENTITLEMENT_OWNER)
	fun fetch(_ from: Address, id: UInt64): &EndeavorATL_NFT.NFT?{ 
		let collection = getAccount(from).capabilities.get<&EndeavorATL_NFT.Collection>(EndeavorATL_NFT.CollectionPublicPath).borrow<&EndeavorATL_NFT.Collection>() ?? panic("Couldn't get collection")
	...
	access(TMP_ENTITLEMENT_OWNER)
	fun getAllSeries(): [EndeavorATL_NFT.SeriesData]{ 
	...
	access(TMP_ENTITLEMENT_OWNER)
	fun getAllSets(): [EndeavorATL_NFT.NFTSetData]{ 
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&EndeavorATL_NFT.Admin>(self.AdminStoragePath)
