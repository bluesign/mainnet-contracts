...
access(all)
contract DIGITALDAWGPOUND: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				DIGITALDAWGPOUND.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return DIGITALDAWGPOUND.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				DIGITALDAWGPOUND.maxSupply == nil || DIGITALDAWGPOUND.totalSupply <= DIGITALDAWGPOUND.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			DIGITALDAWGPOUND.totalSupply = DIGITALDAWGPOUND.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafybeiftplvxejdamrbnv6emqge7tbvetcnl6s5j5kgzpxfdjyp3v6xqgu"), mediaType: "image")
				let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreibf5e7yeo6b4n7j4gzmryt36ba7jquv6s5fobzoojacwrw465oxca"), mediaType: "image")
				let medias = MetadataViews.Medias([media, bannerMedia])
				return FungibleTokenMetadataViews.FTDisplay(name: "Hamiltyne Bulldogs MFC", symbol: "BYTE", description: "Welcome to the DIGITALDAWGPOUND! Earn $BYTE tokens to unleash your team decision-making powers, one BITE at a time. Join our Toucans DAO for Hamiltyne Bulldogs MFC on Flow blockchain and be a part of revolutionizing sports governance!", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("HamiltyneMFC"), "discord": MetadataViews.ExternalURL("8yAc9F4CFX")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: DIGITALDAWGPOUND.VaultStoragePath, receiverPath: DIGITALDAWGPOUND.ReceiverPublicPath, metadataPath: DIGITALDAWGPOUND.VaultPublicPath, receiverLinkedType: /private/DIGITALDAWGPOUNDVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-DIGITALDAWGPOUND.createEmptyVault(vaultType: Type<@DIGITALDAWGPOUND.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/DIGITALDAWGPOUNDVault
		self.ReceiverPublicPath = /public/DIGITALDAWGPOUNDReceiver
		self.VaultPublicPath = /public/DIGITALDAWGPOUNDMetadata
		self.MinterStoragePath = /storage/DIGITALDAWGPOUNDMinter
		self.AdministratorStoragePath = /storage/DIGITALDAWGPOUNDAdmin
