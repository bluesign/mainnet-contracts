...
contract WLD: FungibleToken{ 
	...
	resource Minter{ 
		...
		//
		access(TMP_ENTITLEMENT_OWNER)
		fun mintTokens(amount: UFix64): @WLD.Vault{ 
			pre{ 
				amount > UFix64(0):
			...
			WLD.totalSupply = WLD.totalSupply + amount
	...
	init(){ 
		...
		self.account.storage.save(<-admin, to: /storage/wldAdmin)
		
		// mint tokens
		let tokenVault <- minter.mintTokens(amount: self.totalSupply)
		self.account.storage.save(<-tokenVault, to: /storage/wldVault)
		destroy minter
		
		// Create a public capability to the stored Vault that only exposes
		// the `balance` field through the `Balance` interface
		var capability_1 = self.account.capabilities.storage.issue<&WLD.Vault>(/storage/wldVault)
		self.account.capabilities.publish(capability_1, at: /public/wldBalance)
		
		// Create a public capability to the stored Vault that only exposes
		// the `deposit` method through the `Receiver` interface
		var capability_2 = self.account.capabilities.storage.issue<&{FungibleToken.Receiver}>(/storage/wldVault)
		self.account.capabilities.publish(capability_2, at: /public/wldReceiver)
