...
access(all)
contract Crave: NonFungibleToken{ 
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				...
				case Type<MetadataViews.Royalties>():
					let receiver = Crave.account.capabilities.get<&{FungibleToken.Receiver}>(/public/flowTokenReceiver)
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: Crave.CollectionStoragePath, publicPath: Crave.CollectionPublicPath, publicCollection: Type<&Crave.Collection>(), publicLinkedType: Type<&Crave.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-Crave.createEmptyCollection(nftType: Type<@Crave.Collection>())
						})
			}
			return nil
		}
	...
	access(all)
	resource Collection: CraveCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&{CraveCollectionPublic}>(self.CollectionStoragePath)
