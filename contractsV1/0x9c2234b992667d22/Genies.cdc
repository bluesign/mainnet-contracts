...
contract Genies: NonFungibleToken{ 
	...
	resource GeniesCollection{ 
		...
		fun retireEdition(editionID: UInt32){ 
			...
			let edition = &Genies.editionByID[editionID] as &Genies.Edition?
			edition.retire()
			self.editionsActive = self.editionsActive - 1 as UInt32
		}
	...
	resource NFT: NonFungibleToken.NFT{ 
		...
		init(id: UInt64, editionID: UInt32, serialNumber: UInt32){ 
			pre{ 
				Genies.editionByID[editionID] != nil:
					"no such editionID"
				(&Genies.editionByID[editionID] as &Genies.Edition?).open:
	...
	//
	access(all)
	resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, GeniesNFTCollectionPublic{ 
