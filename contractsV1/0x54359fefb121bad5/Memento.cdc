...
contract Memento: NonFungibleToken, ViewResolver{ 
	...
	resource NFT: NonFungibleToken.NFT, ViewResolver.Resolver{ 
		...
		init(_creatorID: UInt64, _description: String, _recipient: Address){ 
			...
			let metadatas <- Memento.account.storage.load<@Memento.MetadataStorage>(from: Memento.MetadataStoragePath)!
			...
			Memento.account.storage.save(<-metadatas, to: Memento.MetadataStoragePath)
		}
	}
