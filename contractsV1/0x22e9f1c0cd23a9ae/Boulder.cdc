...
access(all)
contract Boulder: NonFungibleToken{ 
	access(all)
	let version: String
	...
	access(all)
	resource Collection: BoulderCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
	...
	fun initAdmin(admin: AuthAccount){ 
		...
		let collection <- Boulder.createEmptyCollection(nftType: Type<@Boulder.Collection>())
		...
		admin.link<&Boulder.Collection>(Boulder.CollectionPublicPath, target: Boulder.CollectionStoragePath)
