...
contract MetayaBeneficiaryCut{ 
	...
	access(all)
	event CopyrightOwnerCapabilityUpdated(name: String, address: Address?)
	...
	access(all)
	event StoreCutPercentagesUpdated(
		saleID: UInt32,
	...
	/// If the copyrightOwnerAndCutPercentage is nil, that means it has been removed.
	access(all)
	event PackCutPercentagesUpdated(
		packID: UInt32,
		copyrightOwnerAndCutPercentage:{ 
			String: UFix64
		}?
	...
	/// If the copyrightOwnerAndCutPercentage is nil, that means it has been removed.
	access(all)
	event MarketCutPercentagesUpdated(
		playID: UInt32,
		copyrightOwnerAndCutPercentage:{ 
			String: UFix64
		}?
	...
	access(all)
	event MetayaCapabilityUpdated(address: Address)
	
	/// Emitted when the market cutPercentage of Metaya service has been updated
	access(all)
	event MetayaMarketCutPercentageUpdated(cutPercentage: UFix64)
	
	/// Emitted when the commonweal organization has been updated
	access(all)
	event CommonwealUpdated(name: String, address: Address?, cutPercentage: UFix64)
	...
	access(self)
	var copyrightOwnerCapabilities:{ String: Capability<&{FungibleToken.Receiver}>}
	...
	access(self)
	var storeCutPercentages:{ UInt32:{ String: UFix64}}
	...
	/// the cut percentage that they are supposed to receive.
	access(self)
	var packCutPercentages:{ UInt32:{ String: UFix64}}
	...
	/// the cut percentage that they are supposed to receive.
	access(self)
	var marketCutPercentages:{ UInt32:{ String: UFix64}}
	
	/// The capability of Metaya service
	access(all)
	var metayaCapability: Capability<&{FungibleToken.Receiver}>
	
	/// The market cutPercentage of Metaya service
	access(all)
	var metayaMarketCutPercentage: UFix64
	
	/// Commonweal organization capabilities
	access(self)
	var commonwealCapabilities:{ String: Capability<&{FungibleToken.Receiver}>}
	...
	resource Admin{ 
		...
		fun setMetayaCapability(capability: Capability<&{FungibleToken.Receiver}>){ 
			...
			let addr =
				(
					(capability.borrow() ?? panic("Capability is empty.")).owner
					?? panic("Capability owner is empty.")
				).address
		...
		fun setMetayaMarketCutPercentage(cutPercentage: UFix64){ 
			pre{ 
				cutPercentage < 1.0:
					"The cutPercentage must be less than 1.0."
			}
		...
		fun setCommonweal(
			...
			pre{ 
				cutPercentage <= 1.0:
					"The cutPercentage can not be greater than 1.0."
			}
			if let cap = capability{ 
	...
	init(){ 
		...
		self.metayaCapability = self.account.capabilities.get<&{FungibleToken.Receiver}>(
				/public/MetayaUtilityCoinReceiver
			)!
