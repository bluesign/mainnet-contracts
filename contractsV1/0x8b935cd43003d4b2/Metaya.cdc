...
access(all)
contract Metaya: NonFungibleToken{ 
	...
	access(all)
	event MomentMinted(momentID: UInt64, setID: UInt32, playID: UInt32, serialNumber: UInt32, soundlinksDIDHash: String)
	...
	struct QuerySetData{ 
		...
		init(setID: UInt32){ 
			...
			let setRef = &Metaya.sets[setID] as &Metaya.Set?
			self.setID = setID
	...
	resource Set{ 
		...
		fun mintMoment(playID: UInt32, soundlinksDID: @SoundlinksDID.NFT): @NFT{ 
			...
			// Mint the new moment
			let newMoment: @NFT <- create NFT(setID: self.setID, playID: playID, serialNumber: numInPlay + 1 as UInt32, soundlinksDID: <-soundlinksDID)
		...
		fun batchMintMoment(playID: UInt32, quantity: UInt32, soundlinksDIDCollection: @SoundlinksDID.Collection): @Collection{ 
			pre{ 
				quantity == UInt32(soundlinksDIDCollection.getIDs().length):
					"The DIDs in the Collection must be the same as the quantity parameter."
			}
	...
	resource Admin{ 
		...
		fun borrowSet(setID: UInt32): &Set{ 
			...
			return &Metaya.sets[setID] as &Metaya.Set?
		}
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&{MomentCollectionPublic}>(self.CollectionStoragePath)
