...
access(all)
contract PepeV2: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				PepeV2.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return PepeV2.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				PepeV2.maxSupply == nil || PepeV2.totalSupply <= PepeV2.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			PepeV2.totalSupply = PepeV2.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "PepeV2", symbol: "PEPEF", description: "PepeV2 or (PEPEF) its the flow people token ,aint gonna be here writing a book so... we're here to make the token scarcer and give a happy ending to our holders. We gonna have a plenty surprises and giveaways ,We're holding Piggos,Pirates ,BYC's and etc we the collectors we the fam ,so i think u know what to expect.", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("Daddy44Block"), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: PepeV2.VaultStoragePath, receiverPath: PepeV2.ReceiverPublicPath, metadataPath: PepeV2.VaultPublicPath, receiverLinkedType: /private/PepeV2Vault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-PepeV2.createEmptyVault(vaultType: Type<@PepeV2.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/PepeV2Vault
		self.ReceiverPublicPath = /public/PepeV2Receiver
		self.VaultPublicPath = /public/PepeV2Metadata
		self.MinterStoragePath = /storage/PepeV2Minter
		self.AdministratorStoragePath = /storage/PepeV2Admin
