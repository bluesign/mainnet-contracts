...
contract MysteryBox{ 
	...
	/// @return
	access(all)
	event RebateToUser(token: String, user: Address, amount: UFix64)
	...
	/// @return
	access(all)
	event Invitation(user: Address, referrer: Address)
	...
	/// @return
	access(all)
	event UpdatePrizePool(tokenName: String, amount: UFix64)
	...
	/// @return
	access(all)
	event PrizePoolReward(round: UInt64, addresses: [Address], amounts: [UFix64], total: UFix64)
	
	// event of nft provider resources destroyed
	access(all)
	event nftProviderResourcesDestroyed()
	
	//path
	access(all)
	let MysteryBoxControllerPublicPath: PublicPath
	
	access(all)
	let MysteryBoxControllerPrivatePath: PrivatePath
	
	access(all)
	let MysteryBoxControllerStoragePath: StoragePath
	...
	struct accountItem{ 
		...
		access(all)
		var promotes:{ String: UFix64}
		
		access(all)
		var earned:{ String: UFix64}
	...
	access(self)
	var prizePool:{ String: UFix64}
	
	//account list
	//mapping user address => accountItem
	access(self)
	var accountList:{ Address: accountItem}
	
	//mystery box type list
	//mapping type id => mysteryBoxTypeItem
	access(self)
	var mysteryBoxTypeList:{ UInt64: MysteryBoxType}
	...
	access(all)
	resource interface MysteryBoxControllerPrivate{ 
	...
	access(all)
	resource interface MysteryBoxControllerPublic{ 
	...
	access(all)
	resource MysteryBoxController: MysteryBoxControllerPrivate, MysteryBoxControllerPublic{ 
		...
		access(self)
		var mainFusdReceiverAddr: Address
		...
		fun checkInviteRelationAndSaveIt(buyerAddr: Address, referrerAddr: Address){ 
			...
			if (buyerAccount!).referrerAddr == MysteryBox.ZERO_ADDRESS && referrerAddr != MysteryBox.ZERO_ADDRESS{ 
				MysteryBox.accountList[buyerAddr] = MysteryBox.accountItem(referrerAddr: referrerAddr, active: (MysteryBox.accountList[buyerAddr]!).active, inviteAmount: (MysteryBox.accountList[buyerAddr]!).inviteAmount, promotes: (MysteryBox.accountList[buyerAddr]!).promotes, earned: (MysteryBox.accountList[buyerAddr]!).earned)
		...
		fun checkStockAndSendNFTToBuyer(buyerAddr: Address, mysteryBoxTypeId: UInt64, mysteryBoxAmount: UInt64): [UInt64]{ 
			...
			MysteryBox.mysteryBoxTypeList[mysteryBoxTypeId] = MysteryBox.MysteryBoxType(typeId: (MysteryBox.mysteryBoxTypeList[mysteryBoxTypeId]!).typeId, describe: (MysteryBox.mysteryBoxTypeList[mysteryBoxTypeId]!).describe, stock: stock, unitPrice: (MysteryBox.mysteryBoxTypeList[mysteryBoxTypeId]!).unitPrice)
	...
	init(){ 
		...
		self.account.storage.save(<-mysteryBoxController, to: self.MysteryBoxControllerStoragePath)
		var capability_1 =
			self.account.capabilities.storage.issue<&MysteryBox.MysteryBoxController>(
				self.MysteryBoxControllerStoragePath
			)
		self.account.capabilities.publish(capability_1, at: self.MysteryBoxControllerPrivatePath)
		var capability_2 =
			self.account.capabilities.storage.issue<&MysteryBox.MysteryBoxController>(
				self.MysteryBoxControllerStoragePath
			)
		self.account.capabilities.publish(capability_2, at: self.MysteryBoxControllerPublicPath)
	}
}
