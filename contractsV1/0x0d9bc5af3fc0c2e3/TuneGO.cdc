...
access(all)
contract TuneGO: NonFungibleToken{ 
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: TuneGO.CollectionStoragePath, publicPath: TuneGO.CollectionPublicPath, publicCollection: Type<&TuneGO.Collection>(), publicLinkedType: Type<&TuneGO.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-TuneGO.createEmptyCollection(nftType: Type<@TuneGO.Collection>())
						})
	...
	access(all)
	resource interface TuneGOCollectionPublic{ 
		access(all)
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowCollectible(id: UInt64): &TuneGO.NFT?{ 
	...
	access(all)
	resource Collection: TuneGOCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
	...
	init(){ 
		...
		self.account.storage.save<@Collection>(<-create Collection(), to: TuneGO.CollectionStoragePath)
		
		// Create a public capability for the Collection
		var capability_1 = self.account.capabilities.storage.issue<&{TuneGOCollectionPublic}>(TuneGO.CollectionStoragePath)
		self.account.capabilities.publish(capability_1, at: TuneGO.CollectionPublicPath)
		
		// Put the Admin in storage
		self.account.storage.save<@Admin>(<-create Admin(), to: TuneGO.AdminStoragePath)
