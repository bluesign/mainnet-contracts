...
access(all)
contract DoodlesFans: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				DoodlesFans.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return DoodlesFans.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				DoodlesFans.maxSupply == nil || DoodlesFans.totalSupply <= DoodlesFans.maxSupply!:
					"Excedeu o fornecimento m\u{e1}ximo de tokens permitido."
			}
			DoodlesFans.totalSupply = DoodlesFans.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "Doodles Fans", symbol: "DOODLES", description: "Doodle Fans Coin", externalURL: MetadataViews.ExternalURL("www.doodles.app"), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("doodles"), "discord": MetadataViews.ExternalURL("doodles")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: DoodlesFans.VaultStoragePath, receiverPath: DoodlesFans.ReceiverPublicPath, metadataPath: DoodlesFans.VaultPublicPath, receiverLinkedType: /private/DoodlesFansVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-DoodlesFans.createEmptyVault(vaultType: Type<@DoodlesFans.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/DoodlesFansVault
		self.ReceiverPublicPath = /public/DoodlesFansReceiver
		self.VaultPublicPath = /public/DoodlesFansMetadata
		self.MinterStoragePath = /storage/DoodlesFansMinter
		self.AdministratorStoragePath = /storage/DoodlesFansAdmin
