...
access(all)
contract OzoneToken: FungibleToken{ 
	...
	access(all)
	let OzoneTokenVaultPath: StoragePath
	
	// The public path for ozone token receiver
	access(all)
	let OzoneTokenReceiverPath: PublicPath
	
	// The public path for ozone token balance
	access(all)
	let OzoneTokenBalancePath: PublicPath
	...
	init(){ 
		...
		self.account.storage.save(<-mintedVault, to: self.OzoneTokenVaultPath)
		var capability_1 = self.account.capabilities.storage.issue<&OzoneToken.Vault>(self.OzoneTokenVaultPath)
		self.account.capabilities.publish(capability_1, at: self.OzoneTokenReceiverPath)
		var capability_2 = self.account.capabilities.storage.issue<&OzoneToken.Vault>(self.OzoneTokenVaultPath)
		self.account.capabilities.publish(capability_2, at: self.OzoneTokenBalancePath)
	}
}
