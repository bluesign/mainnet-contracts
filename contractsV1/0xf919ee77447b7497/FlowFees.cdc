...
contract FlowFees{ 
	...
	access(all)
	event FeesDeducted(amount: UFix64, inclusionEffort: UFix64, executionEffort: UFix64)
	
	// Event that is emitted when fee parameters change
	access(all)
	event FeeParametersChanged(
		surgeFactor: UFix64,
	...
	struct VerifyPayerBalanceResult{ 
		...
		access(all)
		let maximumTransactionFees: UFix64
	...
	fun verifyPayersBalanceForTransactionExecution(
		...
		var maxTransactionFee =
			self.computeFees(inclusionEffort: inclusionEffort, executionEffort: maxExecutionEffort)
	...
	fun deductTransactionFee(
		...
		var feeAmount =
			self.computeFees(inclusionEffort: inclusionEffort, executionEffort: executionEffort)
