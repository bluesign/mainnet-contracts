...
access(all)
contract Trash: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				Trash.setBalance(address: owner, balance: self.balance)
			}
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				Trash.maxSupply == nil || Trash.totalSupply <= Trash.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			Trash.totalSupply = Trash.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreifi6y2qbnaq4zm3gz2npgvvunrtbrjxqufav7fyxzxkiws2rfk3wa"), mediaType: "image")
				let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreifi6y2qbnaq4zm3gz2npgvvunrtbrjxqufav7fyxzxkiws2rfk3wa"), mediaType: "image")
				let medias = MetadataViews.Medias([media, bannerMedia])
				return FungibleTokenMetadataViews.FTDisplay(name: "Trash", symbol: "TRASH", description: "Everyone has some trash.", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: Trash.VaultStoragePath, receiverPath: Trash.ReceiverPublicPath, metadataPath: Trash.VaultPublicPath, receiverLinkedType: /private/TrashVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-Trash.createEmptyVault(vaultType: Type<@Trash.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/TrashVault
		self.ReceiverPublicPath = /public/TrashReceiver
		self.VaultPublicPath = /public/TrashMetadata
		self.MinterStoragePath = /storage/TrashMinter
		self.AdministratorStoragePath = /storage/TrashAdmin
