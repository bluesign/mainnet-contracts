...
access(all)
contract Cryptoyonqui: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				Cryptoyonqui.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return Cryptoyonqui.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				Cryptoyonqui.maxSupply == nil || Cryptoyonqui.totalSupply <= Cryptoyonqui.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			Cryptoyonqui.totalSupply = Cryptoyonqui.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafybeibma5gmus4q4bxiceh23i2xvjydqbuwwexcjytfhuy7gsp7foszqm"), mediaType: "image")
				let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafybeih4j2ezt7gmtuzrbkus22252yee75xo5tfw5ursugatbkv5zgjjxy"), mediaType: "image")
				let medias = MetadataViews.Medias([media, bannerMedia])
				return FungibleTokenMetadataViews.FTDisplay(name: "Cryptoyonqui", symbol: "CYQ", description: "DAO for all the cryptoyonquis", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: Cryptoyonqui.VaultStoragePath, receiverPath: Cryptoyonqui.ReceiverPublicPath, metadataPath: Cryptoyonqui.VaultPublicPath, receiverLinkedType: /private/CryptoyonquiVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-Cryptoyonqui.createEmptyVault(vaultType: Type<@Cryptoyonqui.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/CryptoyonquiVault
		self.ReceiverPublicPath = /public/CryptoyonquiReceiver
		self.VaultPublicPath = /public/CryptoyonquiMetadata
		self.MinterStoragePath = /storage/CryptoyonquiMinter
		self.AdministratorStoragePath = /storage/CryptoyonquiAdmin
