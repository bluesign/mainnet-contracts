...
access(all)
contract NCTRDAO: FungibleToken, ViewResolver{ 
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "NCTR DAO", symbol: "NCTR", description: "The NCTR DAO is committed to support NCTR, the world's first global loyalty point.", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: NCTRDAO.VaultStoragePath, receiverPath: NCTRDAO.ReceiverPublicPath, metadataPath: NCTRDAO.VaultPublicPath, receiverLinkedType: /private/NCTRDAOVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-NCTRDAO.createEmptyVault(vaultType: Type<@NCTRDAO.Vault>())
					})
		}
		return nil
	}
