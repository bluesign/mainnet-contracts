...
contract FLOATVerifiers{ 
	...
	access(all)
	struct Timelock: MusicPeaksSocialToken.IVerifier{ 
	...
	access(all)
	struct Secret: MusicPeaksSocialToken.IVerifier{ 
	...
	// delete their FLOATs.
	access(all)
	struct Limited: MusicPeaksSocialToken.IVerifier{ 
		access(all)
		var capacity: UInt64
		
		access(TMP_ENTITLEMENT_OWNER)
		fun verify(_ params:{ String: AnyStruct}){ 
			let _event = params["event"]! as! &MusicPeaksSocialToken.FLOATEvent
	...
	// from the list.
	access(all)
	struct MultipleSecret: MusicPeaksSocialToken.IVerifier{ 
		access(self)
	...
	access(all)
	struct SecretV2: MusicPeaksSocialToken.IVerifier{ 
		access(all)
