...
contract MusicPeaksSocialToken: NonFungibleToken{ 
	...
	resource FLOATEvent: FLOATEventPublic, ViewResolver.Resolver{ 
		...
		fun getCurrentHolder(serial: UInt64): TokenIdentifier?{ 
			...
			let collection = (getAccount(data.address).capabilities.get<&Collection>(MusicPeaksSocialToken.FLOATCollectionPublicPath)!).borrow()
	...
	resource FLOATEvents: FLOATEventsPublic, ViewResolver.ResolverCollection{ 
		...
		fun borrowSharedRef(fromHost: Address): &FLOATEvents{ 
			...
			let otherFLOATEvents = (getAccount(fromHost).capabilities.get<&FLOATEvents>(MusicPeaksSocialToken.FLOATEventsPublicPath)!).borrow() ?? panic("Could not borrow the public FLOATEvents.")
