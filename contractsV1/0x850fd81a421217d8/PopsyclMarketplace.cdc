...
contract PopsyclMarketplace{ 
	...
	resource SaleCollection: SalePublic{ 
		...
		// Dictionary of the prices for each NFT by ID
		access(self)
		let creators:{ UInt64: Address?}
		
		// Dictionary of the prices for each NFT by ID
		access(self)
		let influencer:{ UInt64: Address?}
		
		// Dictionary of sale type (either sale/resale)
		access(self)
		let resale:{ UInt64: Bool}
		
		access(self)
		let roylty:{ UInt64: UFix64}
		
		// Dictionary of token owner
		access(self)
		let seller:{ UInt64: Address?}
	...
	init(){ 
		...
		self.account.storage.save(<-create Admin(), to: /storage/PopsyclMarketAdmin)
	}
}
