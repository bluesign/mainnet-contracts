...
contract OnChainMultiSig{ 
	...
	// ------- Events ------- 
	access(all)
	event NewPayloadAdded(resourceId: UInt64, txIndex: UInt64)
	
	access(all)
	event NewPayloadSigAdded(resourceId: UInt64, txIndex: UInt64)
	...
	resource PayloadDetails{ 
		...
		fun verifySigners(
			...
			assert(
				pks.length == sigs.length,
				message: "Cannot verify signatures without corresponding public keys"
			)
			...
			while i < pks.length{ 
				...
				let pk = PublicKey(publicKey: pks[i].decodeHex(), signatureAlgorithm: SignatureAlgorithm(rawValue: (currentKeyList[pks[i]]!).sigAlgo) ?? panic("Invalid signature algo"))
				let keyListSig = Crypto.KeyListSignature(keyIndex: keyIndex, signature: sigs[i])
				keyListSignatures.append(keyListSig)
				keyList.add(pk, hashAlgorithm: HashAlgorithm.SHA3_256, weight: (currentKeyList[pks[i]]!).weight)
				totalAuthorizedWeight = totalAuthorizedWeight + (currentKeyList[pks[i]]!).weight
				i = i + 1
				keyIndex = keyIndex + 1
			}
			let isValid =
				keyList.verify(signatureSet: keyListSignatures, signedData: payloadInBytes)
			if isValid{ 
	...
	access(all)
	resource Manager: SignatureManager{ 
