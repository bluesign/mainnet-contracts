...
access(all)
contract TouchstoneManekiPlanets: NonFungibleToken{ 
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: TouchstoneManekiPlanets.CollectionStoragePath, publicPath: TouchstoneManekiPlanets.CollectionPublicPath, publicCollection: Type<&Collection>(), publicLinkedType: Type<&Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-TouchstoneManekiPlanets.createEmptyCollection(nftType: Type<@TouchstoneManekiPlanets.Collection>())
						})
	...
	resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun claim(){ 
			if let storage = &TouchstoneManekiPlanets.nftStorage[(self.owner!).address] as auth(Mutate) &{UInt64: NFT}?{ 
	...
	resource Administrator{ 
		...
		fun mintNFT(metadataId: UInt64, recipient: Address){ 
			...
			if let recipientCollection = getAccount(recipient).capabilities.get<&TouchstoneManekiPlanets.Collection>(TouchstoneManekiPlanets.CollectionPublicPath).borrow<&TouchstoneManekiPlanets.Collection>(){ 
				recipientCollection.deposit(token: <-nft)
			} else if let storage = &TouchstoneManekiPlanets.nftStorage[recipient] as auth(Mutate) &{UInt64: NFT}?{ 
				storage[nft.id] <-! nft
			} else{ 
	...
	fun getClaimableNFTs(user: Address):{ UInt64: NFTMetadata}{ 
		...
		if let storage = &TouchstoneManekiPlanets.nftStorage[user] as auth(Mutate) &{UInt64: NFT}?{ 
			for id in storage.keys{ 
				let nftRef = (storage[id] as &TouchstoneManekiPlanets.NFT?)!
