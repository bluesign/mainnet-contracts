...
contract TouchstoneIconicPlanets: NonFungibleToken{ 
	...
	resource Administrator{ 
		...
		fun mintNFT(metadataId: UInt64, recipient: Address){ 
			...
			if let recipientCollection = (getAccount(recipient).capabilities.get<&TouchstoneIconicPlanets.Collection>(TouchstoneIconicPlanets.CollectionPublicPath)!).borrow(){ 
