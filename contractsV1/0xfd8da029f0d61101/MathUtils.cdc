...
access(all)
contract MathUtils: NonFungibleToken{ 
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: MathUtils.CollectionStoragePath, publicPath: MathUtils.CollectionPublicPath, publicCollection: Type<&MathUtils.Collection>(), publicLinkedType: Type<&MathUtils.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-MathUtils.createEmptyCollection(nftType: Type<@MathUtils.Collection>())
						})
	...
	resource Admin{ 
		...
		fun changeDefaultRoyaltyRate(name: String, rate: UFix64){ 
			...
			MathUtils.defaultRoyalties[name] = MetadataViews.Royalty(receiver: previousRecipientAddress, cut: UFix64(rate), description: "MathUtils Royalties")
		...
		fun changeRoyaltyRateForPFP(tokenID: UInt64, name: String, rate: UFix64){ 
			...
			let newRoyalty = MetadataViews.Royalty(receiver: previousRoyalty.receiver, cut: UFix64(rate), description: "MathUtils Royalties")
	...
	//
	access(all)
	resource Collection: MathUtilsCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		...
		view fun borrowViewResolver(id: UInt64): &{ViewResolver.Resolver}?{ 
			...
			return MathUtilsNFT as &{ViewResolver.Resolver}
		}
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&{MathUtilsCollectionPublic}>(self.CollectionStoragePath)
