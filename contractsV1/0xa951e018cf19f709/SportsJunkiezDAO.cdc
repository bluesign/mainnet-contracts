...
access(all)
contract SportsJunkiezDAO: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				SportsJunkiezDAO.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return SportsJunkiezDAO.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				SportsJunkiezDAO.maxSupply == nil || SportsJunkiezDAO.totalSupply <= SportsJunkiezDAO.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			SportsJunkiezDAO.totalSupply = SportsJunkiezDAO.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreif4l3hxaqm3oblt3gt7sbgaoxkvnvp7fy6fvrcwufddmf4vmiosty"), mediaType: "image")
				let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreicxyz46wzdajd4tpzl5r53j4wpjokmg7nkwyuf52pow5lyksjfxim"), mediaType: "image")
				let medias = MetadataViews.Medias([media, bannerMedia])
				return FungibleTokenMetadataViews.FTDisplay(name: "Sports Junkiez DAO", symbol: "SJNKZ", description: "A DAO For Sports Junkiez", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("https://twitter.com/JunkiezSports"), "discord": MetadataViews.ExternalURL("SportsJunkiez")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: SportsJunkiezDAO.VaultStoragePath, receiverPath: SportsJunkiezDAO.ReceiverPublicPath, metadataPath: SportsJunkiezDAO.VaultPublicPath, receiverLinkedType: /private/SportsJunkiezDAOVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-SportsJunkiezDAO.createEmptyVault(vaultType: Type<@SportsJunkiezDAO.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/SportsJunkiezDAOVault
		self.ReceiverPublicPath = /public/SportsJunkiezDAOReceiver
		self.VaultPublicPath = /public/SportsJunkiezDAOMetadata
		self.MinterStoragePath = /storage/SportsJunkiezDAOMinter
		self.AdministratorStoragePath = /storage/SportsJunkiezDAOAdmin
