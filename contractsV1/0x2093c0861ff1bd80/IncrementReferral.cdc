...
contract IncrementReferral{ 
	...
	access(self)
	let _referrerToReferees:{ Address: [RefereeInfo]}
	...
	fun bind(
		...
		(self._referrerToReferees[referrer]!).append(
			RefereeInfo(referee, getCurrentBlock().timestamp)
		)
	...
	access(all)
	view fun getSlicedRefereesByReferrer(referrer: Address, from: Int, to: Int): [RefereeInfo]{ 
		if self._referrerToReferees.containsKey(referrer) == false{ 
			return []
		}
	...
	init(){ 
		...
		destroy <-self.account.storage.load<@AnyResource>(from: /storage/referralAdmin)
		self.account.storage.save(<-create Admin(), to: /storage/referralAdmin)
	}
}
