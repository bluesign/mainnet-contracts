...
contract OpenEdition{ 
	...
	resource OpenEditionItem{ 
		...
		fun purchase(
			...
			buyerCollectionCap: Capability<&Collectible.Collection>,
	...
	resource interface OpenEditionCollectionPublic{ 
		access(TMP_ENTITLEMENT_OWNER)
		fun getOpenEditionStatuses():{ UInt64: OpenEdition.OpenEditionStatus}?
	...
	resource OpenEditionCollection: OpenEditionCollectionPublic{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun purchase(id: UInt64, buyerTokens: @FUSD.Vault, collectionCap: Capability<&Collectible.Collection>){ 
			pre{ 
				self.openEditionsItems[id] != nil:
					"Open Edition does not exist"
				collectionCap.check():
	...
	init(){ 
		...
		self.account.storage.save(<-openEdition, to: OpenEdition.CollectionStoragePath)
		var capability_1 =
			self.account.capabilities.storage.issue<&{OpenEdition.OpenEditionCollectionPublic}>(
				OpenEdition.CollectionStoragePath
			)
		self.account.capabilities.publish(capability_1, at: OpenEdition.CollectionPublicPath)
	}
}
