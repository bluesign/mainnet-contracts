...
contract Auction{ 
	...
	resource AuctionItem{ 
		...
		access(self)
		var recipientCollectionCap: Capability<&Collectible.Collection>?
		...
		access(self)
		fun sendNFT(_ capability: Capability<&Collectible.Collection>){ 
		...
		fun placeBid(
			...
			collectionCap: Capability<&Collectible.Collection>
		){ 
			pre{ 
				vaultCap.check():
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun reclaimSendNFT(collectionCap: Capability<&Collectible.Collection>){ 
			pre{ 
				self.auctionCompleted:
	...
	resource interface AuctionCollectionPublic{ 
		access(TMP_ENTITLEMENT_OWNER)
		fun getAuctionStatuses():{ UInt64: Auction.AuctionStatus}
	...
	resource AuctionCollection: AuctionCollectionPublic{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun createAuction(minimumBidIncrement: UFix64, auctionLength: UFix64, extendedLength: UFix64, remainLengthToExtend: UFix64, auctionStartTime: UFix64, startPrice: UFix64, platformVaultCap: Capability<&FUSD.Vault>, editionCap: Capability<&{Edition.EditionCollectionPublic}>): UInt64{ 
			pre{ 
				auctionLength > 0.00:
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun placeBid(id: UInt64, bidTokens: @FUSD.Vault, vaultCap: Capability<&FUSD.Vault>, collectionCap: Capability<&Collectible.Collection>){ 
			pre{ 
				self.auctionItems[id] != nil:
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun reclaimSendNFT(id: UInt64, collectionCap: Capability<&Collectible.Collection>){ 
			pre{ 
				self.auctionItems[id] != nil:
					"Auction does not exist"
			}
	...
	init(){ 
		...
		self.account.storage.save(<-sale, to: Auction.CollectionStoragePath)
		var capability_1 =
			self.account.capabilities.storage.issue<&{Auction.AuctionCollectionPublic}>(
				Auction.CollectionStoragePath
			)
		self.account.capabilities.publish(capability_1, at: Auction.CollectionPublicPath)
	}
}
