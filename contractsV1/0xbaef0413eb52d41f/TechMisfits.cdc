...
access(all)
contract TechMisfits: NonFungibleToken{ 
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: TechMisfits.CollectionStoragePath, publicPath: TechMisfits.CollectionPublicPath, publicCollection: Type<&TechMisfits.Collection>(), publicLinkedType: Type<&TechMisfits.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-TechMisfits.createEmptyCollection(nftType: Type<@TechMisfits.Collection>())
						})
	...
	resource Admin{ 
		...
		fun changeDefaultRoyaltyRate(name: String, rate: UFix64){ 
			...
			TechMisfits.defaultRoyalties[name] = MetadataViews.Royalty(receiver: previousRecipientAddress, cut: UFix64(rate), description: "TechMisfits Royalties")
		...
		fun changeRoyaltyRateForPFP(tokenID: UInt64, name: String, rate: UFix64){ 
			...
			let newRoyalty = MetadataViews.Royalty(receiver: previousRoyalty.receiver, cut: UFix64(rate), description: "TechMisfits Royalties")
	...
	access(all)
	resource interface TechMisfitsCollectionPublic{ 
		access(all)
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowTechMisfits(id: UInt64): &TechMisfits.NFT?{ 
	...
	//
	access(all)
	resource Collection: TechMisfitsCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
	...
	access(TMP_ENTITLEMENT_OWNER)
	fun createEmptyMintPFPsCollection(): @TechMisfits.Collection{ 
		return <-create TechMisfits.Collection()
	}
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&{TechMisfitsCollectionPublic}>(self.CollectionStoragePath)
