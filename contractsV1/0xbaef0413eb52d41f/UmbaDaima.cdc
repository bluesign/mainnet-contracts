...
access(all)
contract UmbaDaima: NonFungibleToken{ 
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: UmbaDaima.CollectionStoragePath, publicPath: UmbaDaima.CollectionPublicPath, publicCollection: Type<&UmbaDaima.Collection>(), publicLinkedType: Type<&UmbaDaima.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-UmbaDaima.createEmptyCollection(nftType: Type<@UmbaDaima.Collection>())
						})
	...
	//
	access(all)
	resource Collection: UmbaDaimaCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		...
		view fun borrowViewResolver(id: UInt64): &{ViewResolver.Resolver}?{ 
			...
			return UmbaDaimaNFT as &{ViewResolver.Resolver}
		}
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&{UmbaDaimaCollectionPublic}>(self.CollectionStoragePath)
