...
contract Questing{ 
	...
	access(all)
	event RewardPerSecondChanged(questID: UInt64, typeIdentifier: String, rewardPerSecond: UFix64)
	
	access(all)
	event RewardRevealed(
		questID: UInt64,
		typeIdentifier: String,
		questingResourceID: UInt64,
	...
	access(all)
	event AdjustedQuestingStartDateUpdated(
		questID: UInt64,
		typeIdentifier: String,
		questingResourceID: UInt64,
	...
	access(all)
	event QuestCreated(questID: UInt64, type: Type, questCreator: Address)
	
	access(all)
	event QuestDeposited(
		questID: UInt64,
		type: Type,
	...
	access(all)
	event QuestDestroyed(questID: UInt64, type: Type, questCreator: Address)
	
	access(all)
	event MinterDeposited(minterID: UInt64, name: String, receiverAddress: Address?)
	
	access(all)
	event MinterWithdrawn(minterID: UInt64, name: String, providerAddress: Address?)
	
	// -----------------------------------------------------------------------
	// Paths
	// -----------------------------------------------------------------------
	access(all)
	let QuestManagerStoragePath: StoragePath
	
	access(all)
	let QuestManagerPublicPath: PublicPath
	
	access(all)
	let QuestManagerPrivatePath: PrivatePath
	
	access(all)
	let QuesterStoragePath: StoragePath
	...
	resource interface Public{ 
		...
		access(all)
		let questCreator: Address
		
		access(all)
		var rewardPerSecond: UFix64
		
		access(contract)
		fun quest(questingResource: @AnyResource, address: Address): @AnyResource
		
		access(contract)
		fun unquest(questingResource: @AnyResource, address: Address): @AnyResource
		
		access(contract)
		fun revealQuestReward(questingResource: @AnyResource, questRewardID: UInt64): @AnyResource
		
		access(all)
		fun getQuesters(): [Address]
		
		access(all)
		fun getAllQuestingStartDates():{ UInt64: UFix64}
		
		access(all)
		fun getQuestingStartDate(questingResourceID: UInt64): UFix64?
		
		access(all)
		fun getAllAdjustedQuestingStartDates():{ UInt64: UFix64}
		
		access(all)
		fun getAdjustedQuestingStartDate(questingResourceID: UInt64): UFix64?
		
		access(all)
		fun getQuestingResourceIDs(): [UInt64]
		
		access(all)
		fun borrowRewardCollection(questingResourceID: UInt64): &QuestReward.Collection?
	}
	
	access(all)
	resource Quest: Public{ 
		access(all)
		let id: UInt64
		...
		access(self)
		var questers: [Address]
		
		access(self)
		var questingStartDates:{ UInt64: UFix64}
		
		access(self)
		var adjustedQuestingStartDates:{ UInt64: UFix64}
		...
		access(self)
		var rewards: @{UInt64: QuestReward.Collection}
		...
		fun quest(questingResource: @AnyResource, address: Address): @AnyResource{ 
			...
			if container[0]?.isInstance(Type<@{NonFungibleToken.NFT}>()) == true{ 
				let ref = &container[0] as &AnyResource?
				let _resource = (ref as! &{NonFungibleToken.NFT}?)!
				uuid = _resource.uuid
			}
		...
		access(all)
		fun unquestResource(questingResourceID: UInt64){ 
			// remove timers
		...
		access(all)
		fun addReward(questingResourceID: UInt64, minter: &QuestReward.Minter, rewardAlgo: &{RewardAlgorithm.Algorithm}, rewardMapping:{ Int: UInt32}){ 
			...
			if let adjustedQuestingStartDate = self.adjustedQuestingStartDates[questingResourceID]{ 
				...
				if timeQuested >= self.rewardPerSecond{ 
					...
					if toRef == nil{ 
						let newCollection <- QuestReward.createEmptyCollection(nftType: Type<@QuestReward.Collection>())
		...
		access(all)
		fun burnReward(questingResourceID: UInt64, rewardID: UInt64){ 
		...
		access(all)
		fun moveReward(fromID: UInt64, toID: UInt64, rewardID: UInt64){ 
		...
		access(all)
		fun changeRewardPerSecond(seconds: UFix64){ 
		...
		access(all)
		fun revealReward(questingResourceID: UInt64, questRewardID: UInt64){ 
			let collectionRef = &self.rewards[questingResourceID] as &QuestReward.Collection?
		...
		access(contract)
		fun updateAdjustedQuestingStartDate(questingResourceID: UInt64, rewardPerSecond: UFix64){ 
	...
	access(all)
	resource interface MinterReceiver{ 
		access(all)
		fun depositMinter(minter: @QuestReward.Minter)
	}
	
	access(all)
	resource interface QuestReceiver{ 
		access(all)
		fun depositQuest(quest: @Quest)
	}
	
	access(all)
	resource interface QuestManagerPublic{ 
		access(all)
		fun getIDs(): [UInt64]
		
		access(all)
		fun getMinterIDs(): [UInt64]
		
		access(all)
		fun borrowQuest(id: UInt64): &Quest?{ 
			post{ 
		...
		access(all)
		fun borrowMinter(id: UInt64): &QuestReward.Minter?{ 
			post{ 
				result == nil || result?.id == id:
	...
	access(all)
	resource QuestManager: QuestManagerPublic, QuestReceiver, MinterReceiver{ 
		access(self)
		var quests: @{UInt64: Quest}
		
		access(self)
		var minters: @{UInt64: QuestReward.Minter}
		...
		access(all)
		fun withdrawQuest(id: UInt64): @Quest{ 
		...
		access(all)
		fun borrowQuest(id: UInt64): &Questing.Quest?{ 
			return &self.quests[id] as &Questing.Quest?
		}
		
		access(all)
		fun borrowEntireQuest(id: UInt64): &Questing.Quest?{ 
			return &self.quests[id] as &Questing.Quest?
		}
		...
		access(all)
		fun withdrawMinter(id: UInt64): @QuestReward.Minter{ 
		...
		access(all)
		fun borrowEntireMinter(id: UInt64): &QuestReward.Minter?{ 
			return &self.minters[id] as &QuestReward.Minter?
		}
	}
	...
	access(all)
	fun createQuestManager(): @QuestManager{ 
		return <-create QuestManager()
	}
	
	access(all)
	fun createQuester(): @Quester{ 
