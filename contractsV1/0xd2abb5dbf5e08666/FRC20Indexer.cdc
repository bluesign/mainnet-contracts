...
contract FRC20Indexer{ 
	...
	access(all)
	event PlatformTreasurySponsorship(amount: UFix64, to: Address, forTick: String)
	
	/// Event emitted when the Treasury Withdrawn invoked
	access(all)
	event TokenTreasuryWithdrawn(tick: String, amount: UFix64, byInsId: UInt64, reason: String)
	
	/// Event emitted when a FRC20 token is deployed
	access(all)
	event FRC20Deployed(tick: String, max: UFix64, limit: UFix64, deployer: Address)
	
	/// Event emitted when a FRC20 token is minted
	access(all)
	event FRC20Minted(tick: String, amount: UFix64, to: Address)
	
	/// Event emitted when the owner of an inscription is updated
	access(all)
	event FRC20Transfer(tick: String, from: Address, to: Address, amount: UFix64)
	
	/// Event emitted when a FRC20 token is burned
	access(all)
	event FRC20Burned(tick: String, amount: UFix64, from: Address, flowExtracted: UFix64)
	
	/// Event emitted when a FRC20 token is withdrawn as change
	access(all)
	event FRC20WithdrawnAsChange(tick: String, amount: UFix64, from: Address)
	
	/// Event emitted when a FRC20 token is deposited from change
	access(all)
	event FRC20DepositedFromChange(tick: String, amount: UFix64, to: Address, from: Address)
	
	/// Event emitted when a FRC20 token is set to be burnable
	access(all)
	event FRC20BurnableSet(tick: String, burnable: Bool)
	
	/// Event emitted when a FRC20 token is burned unsupplied tokens
	access(all)
	event FRC20UnsuppliedBurned(tick: String, amount: UFix64)
	...
	access(all)
	resource interface IndexerPublic{ 
		/* --- read-only --- */
		/// Get all the tokens
		access(all)
		...
		access(all)
		fun borrowTokenTreasuryReceiver(tick: String): &FlowToken.Vault
		
		/// Borrow the platform treasury $FLOW receiver
		access(all)
		fun borowPlatformTreasuryReceiver(): &FlowToken.Vault
	...
	///
	access(all)
	resource InscriptionIndexer: IndexerPublic{ 
		...
		fun borrowTokenTreasuryReceiver(tick: String): &FlowToken.Vault{ 
			...
			return pool as &FlowToken.Vault
		}
		...
		///
		access(all)
		fun sponsorship(amount: UFix64, to: Capability<&{FungibleToken.Receiver}>, forTick: String){ 
			pre{ 
				amount > 0.0:
		...
		fun deploy(ins: &Fixes.Inscription){ 
			...
			self.pool[tick] <-! FlowToken.createEmptyVault(vaultType: Type<@FlowToken.Vault>()) as! @FlowToken.Vault // init the pool
		...
		/// Apply a listed order, maker and taker should be the same token and the same amount
		access(account)
		fun applyBuyNowOrder(makerIns: &Fixes.Inscription, takerIns: &Fixes.Inscription, maxAmount: UFix64, change: @FRC20FTShared.Change): @FRC20FTShared.Change{ 
			pre{ 
		...
		/// Apply a listed order, maker and taker should be the same token and the same amount
		access(account)
		fun applySellNowOrder(makerIns: &Fixes.Inscription, takerIns: &Fixes.Inscription, maxAmount: UFix64, change: @FRC20FTShared.Change, _ distributeFlowTokenFunc: fun (UFix64, @FlowToken.Vault): Bool): @FRC20FTShared.Change{ 
			pre{ 
				makerIns.isExtractable():
					"The MAKER inscription is not extractable"
				takerIns.isExtractable():
		...
		fun _buildFRC20SaleCuts(sellerAddress: Address?): [FRC20FTShared.SaleCut]{ 
			...
			ret.append(FRC20FTShared.SaleCut(type: FRC20FTShared.SaleCutType.TokenTreasury, ratio: salesFee * treasuryPoolCut, receiver: nil))
			ret.append(FRC20FTShared.SaleCut(type: FRC20FTShared.SaleCutType.PlatformTreasury, ratio: salesFee * platformTreasuryCut, receiver: nil))
			ret.append(FRC20FTShared.SaleCut(type: FRC20FTShared.SaleCutType.PlatformStakers, ratio: salesFee * platformStakersCut, receiver: nil))
			ret.append(FRC20FTShared.SaleCut(type: FRC20FTShared.SaleCutType.MarketplacePortion, ratio: salesFee * marketplacePortionCut, receiver: nil))
			...
			if sellerAddress != nil{ 
				...
				ret.append(FRC20FTShared.SaleCut(type: FRC20FTShared.SaleCutType.BuyTaker, ratio: 1.0 - salesFee, receiver: nil))
			}
		...
		///
		access(self)
		fun _transferToken(tick: String, fromAddr: Address, to: Address, amt: UFix64){ 
		...
		///
		access(self)
		fun _withdrawToTokenChange(tick: String, fromAddr: Address, amt: UFix64): @FRC20FTShared.Change{ 
			post{ 
		...
		///
		access(self)
		fun _depositFromTokenChange(change: @FRC20FTShared.Change, to: Address){ 
			pre{ 
				change.isBackedByVault() == false:
					"The change should not be backed by a vault"
			}
		...
		///
		access(self)
		view fun borrowTokenMeta(tick: String): &FRC20Meta{ 
		...
		///
		access(self)
		view fun _borrowBalancesRef(tick: String): &{Address: UFix64}{ 
		...
		///
		access(self)
		view fun _borrowTokenTreasury(tick: String): &FlowToken.Vault{ 
	...
	init(){ 
		...
		var capability_1 =
			self.account.capabilities.storage.issue<&InscriptionIndexer>(self.IndexerStoragePath)
		self.account.capabilities.publish(capability_1, at: self.IndexerPublicPath)
	}
}
