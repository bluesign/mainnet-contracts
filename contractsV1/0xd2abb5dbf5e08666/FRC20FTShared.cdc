...
contract FRC20FTShared{ 
	...
	access(all)
	event TransactionHooksOnHeartbeat(
		hooksOwner: Address,
		executedHookType: Type,
		deltaTime: UFix64
	)
	...
	///
	access(account)
	fun createStakedChange(ref: &Change, issuer: Address): @Change{ 
		pre{ 
	...
	access(account)
	fun createEmptyChange(tick: String, from: Address): @Change{ 
		if tick == ""{ 
			return <-self.createChange(tick: "", from: from, balance: nil, ftVault: <-FlowToken.createEmptyVault(vaultType: Type<@FlowToken.Vault>()))
		} else{ 
	...
	///
	access(account)
	fun createEmptyFlowChange(from: Address): @Change{ 
	...
	///
	access(account)
	fun wrapFungibleVaultChange(ftVault: @{FungibleToken.Vault}, from: Address): @Change{ 
		let tick = ftVault.isInstance(Type<@FlowToken.Vault>()) ? "" : ftVault.getType().identifier
		return <-self.createChange(tick: tick, from: from, balance: nil, ftVault: <-ftVault)
	}
	...
	///
	access(account)
	fun depositToChange(receiver: &Change, change: @Change){ 
		pre{ 
	...
	enum ConfigType: UInt8{ 
		...
		access(all)
		case GameLotteryTicketPrice
		
		access(all)
		case GameLotteryEpochInterval
		
		access(all)
		case GameLotteryAutoStart
		
		access(all)
		case GameLotteryServiceFee
	}
	...
	///
	access(all)
	resource Hooks: TransactionHook, FixesHeartbeat.IHeartbeatHook{ 
		access(self)
	...
	///
	access(account)
	fun borrowTransactionHookWithHeartbeat(_ address: Address): &{
		TransactionHook,
	...
	init(){ 
		// Transaction Hook
		let hookIdentifier =
			"FRC20FTShared_".concat(self.account.address.toString()).concat("_transactionHook")
