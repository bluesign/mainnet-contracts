...
contract FRC20VoteCommands{ 
	...
	enum CommandType: UInt8{ 
		...
		access(all)
		case MoveTreasuryToLotteryJackpot
		
		access(all)
		case MoveTreasuryToStakingReward
	}
	
	/// The interface of FRC20 vote command struct.
	///
	access(all)
	struct interface IVoteCommand{ 
		access(all)
		...
		///
		access(contract)
		view fun borrowSystemInscriptionWritableRefs(): [&Fixes.Inscription]{ 
			let store = FRC20VoteCommands.borrowSystemInscriptionsStore()
	...
	access(all)
	struct CommandNone: IVoteCommand{ 
		access(all)
		let inscriptionIds: [UInt64]
	...
	access(all)
	struct CommandSetBurnable: IVoteCommand{ 
		access(all)
		let inscriptionIds: [UInt64]
	...
	access(all)
	struct CommandBurnUnsupplied: IVoteCommand{ 
		access(all)
		let inscriptionIds: [UInt64]
	...
	access(all)
	struct CommandMoveTreasuryToLotteryJackpot: IVoteCommand{ 
		access(all)
		let inscriptionIds: [UInt64]
	...
	access(all)
	struct CommandMoveTreasuryToStakingReward: IVoteCommand{ 
		access(all)
		let inscriptionIds: [UInt64]
			...
			case CommandType.BurnUnsupplied:
				return [
					FixesInscriptionFactory.buildVoteCommandBurnUnsupplied(
						tick: meta["tick"] ?? panic("Missing tick in params"),
						percent: UFix64.fromString(meta["perc"] ?? panic("Missing perc in params"))
						?? panic("Invalid perc")
					)
				]
			case CommandType.MoveTreasuryToLotteryJackpot:
				return [
					FixesInscriptionFactory.buildVoteCommandMoveTreasuryToLotteryJackpot(
						tick: meta["tick"] ?? panic("Missing tick in params"),
						amount: UFix64.fromString(meta["amt"] ?? panic("Missing amt in params"))
						?? panic("Invalid amt")
					)
				]
				...
				return [
					FixesInscriptionFactory.buildVoteCommandMoveTreasuryToStaking(
						...
						vestingInterval: UFix64.fromString(
							meta["interval"] ?? panic("Missing interval in params")
						)
