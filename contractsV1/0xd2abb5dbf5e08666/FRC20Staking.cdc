...
contract FRC20Staking{ 
	...
	access(all)
	event StakingInitialized(pool: Address, tick: String)
	
	/// Event emitted when the reward strategy is created
	access(all)
	event RewardStrategyInitialized(
		pool: Address,
		stakeTick: String,
		rewardTick: String,
		ftVaultType: String?
	...
	access(all)
	event RewardStrategyAdded(pool: Address, stakeTick: String, rewardTick: String)
	
	/// Event emitted when the reward income is added
	access(all)
	event RewardIncomeAdded(pool: Address, tick: String, amount: UFix64, from: Address)
	
	/// Event emitted when the delegator record is added
	access(all)
	event DelegatorRecordAdded(
		pool: Address,
		tick: String,
		delegatorID: UInt32,
	...
	access(all)
	event DelegatorUnStakingLocked(
		pool: Address,
		tick: String,
		delegatorID: UInt32,
		delegatorAddress: Address,
	...
	access(all)
	event DelegatorUnStakingUnlocked(
		pool: Address,
		tick: String,
		delegatorID: UInt32,
		delegatorAddress: Address,
	...
	access(all)
	event DelegatorStakedTokenDeposited(
		tick: String,
		pool: Address,
		receiver: Address,
		amount: UFix64,
	...
	resource interface PoolPublic{ 
		...
		/// Unstake FRC20 token
		access(account)
		fun unstake(_ semiNFTCol: &FRC20SemiNFT.Collection, nftId: UInt64)
		
		/// Claim all unlocked staked changes
		access(account)
		fun claimUnlockedUnstakingChange(delegator: Address): @FRC20FTShared.Change?
	...
	resource Pool: PoolPublic{ 
		...
		///
		access(contract)
		view fun borrowTotalStaked(): &FRC20FTShared.Change{ 
	...
	struct DelegatorUnstakingInfo{ 
		...
		access(all)
		let unstakingEntriesNFTIds: [UInt64]
		
		access(all)
		let totalUnstakingBalance: UFix64
		
		access(all)
		let totalUnlockedClaimableBalance: UFix64
	...
	///
	access(all)
	resource interface UnstakingEntryPublic{ 
		access(all)
	...
	///
	access(all)
	resource UnstakingEntry: UnstakingEntryPublic{ 
		access(all)
		let unlockTime: UInt64
	...
	resource DelegatorRecord{ 
		...
		///
		access(contract)
		fun addUnstakingEntry(_ unstakingNFT: @FRC20SemiNFT.NFT){ 
			pre{ 
				unstakingNFT.getOriginalTick() == self.stakeTick:
					"Unstaking NFT tick must match"
			}
			let unlockTime =
				UInt64(getCurrentBlock().timestamp) + FRC20Staking.getUnstakingLockTime()
			let tick = unstakingNFT.getOriginalTick()
			let amount = unstakingNFT.getBalance()
			self.unstakingEntries.append(
				<-create UnstakingEntry(unlockTime: unlockTime, unstakingNFT: <-unstakingNFT)
			)
		...
		///
		access(contract)
		fun borrowDelegatorRef(): &Delegator{ 
	...
	resource RewardStrategy{ 
		...
		init(pool: Capability<&Pool>, rewardTick: String){ 
			pre{ 
				pool.check():
					"Pool must be valid"
			}
		...
		///
		access(account)
		fun claim(byNft: &FRC20SemiNFT.NFT): @FRC20FTShared.Change{ 
			pre{ 
				self.poolCap.check():
					"Pool must be valid"
				self.owner?.address == self.poolCap.address:
			...
			let delegatorRewardChange <-
				FRC20FTShared.createEmptyChange(tick: self.rewardTick, from: delegator)
			
			// delegator info
			let delegatorRef =
				FRC20Staking.borrowDelegator(delegator) ?? panic("Delegator must exist")
			...
			let yieldReward =
				(self.globalYieldRate - delegatorLastGlobalYieldRate) * delegatorStakedToken
			assert(
	...
	///
	access(all)
	resource interface DelegatorPublic{ 
		/** ---- Public methods ---- */
		...
		access(contract)
		fun onFRC20Staked(stakedChange: @FRC20FTShared.Change)
	...
	///
	access(all)
	resource Delegator: DelegatorPublic{ 
		access(self)
		let semiNFTcolCap: Capability<&FRC20SemiNFT.Collection>
		
		init(_ semiNFTCol: Capability<&FRC20SemiNFT.Collection>){ 
			pre{ 
				semiNFTCol.check():
		...
		///
		access(contract)
		fun onClaimingReward(reward: &RewardStrategy, byNftId: UInt64, amount: UFix64, currentGlobalYieldRate: UFix64){ 
		...
		///
		access(self)
		view fun borrowSemiNFTCollection(): &FRC20SemiNFT.Collection{ 
	...
	///
	access(all)
	fun borrowPool(_ addr: Address): &Pool?{ 
	...
	///
	access(all)
	fun getPoolCap(_ addr: Address): Capability<&Pool>{ 
		return getAccount(addr).capabilities.get<&Pool>(self.StakingPoolPublicPath)!
	}
	...
	///
	access(all)
	fun createDelegator(_ semiNFTCol: Capability<&FRC20SemiNFT.Collection>): @Delegator{ 
		pre{ 
			semiNFTCol.check():
				"SemiNFT Collection must be valid"
		}
