...
contract FRC20StakingVesting{ 
	...
	struct VestingInfo{ 
		...
		access(all)
		let vestedAmount: UFix64
		
		access(all)
		let totalBatches: UInt32
		
		access(all)
		let vestedBatchAmount: UInt32
		
		access(all)
		let vestingInterval: UFix64
		
		access(all)
		let lastVestedAt: UFix64
	...
	///
	access(all)
	resource interface VestingVaultPublic{ 
	...
	resource Vault:
		...
		fun addVesting(
			...
			let stakingPool =
				FRC20Staking.borrowPool(poolAddr) ?? panic("The staking pool is not found")
	...
	fun getVaultCap(_ addr: Address): Capability<&Vault>{ 
		return getAccount(addr).capabilities.get<&Vault>(self.publicPath)!
	}
