...
access(all)
contract FRC20StakingForwarder{ 
	/* --- Events --- */
	...
	access(all)
	resource interface ForwarderPublic{ 
	...
	access(all)
	resource Forwarder: FungibleToken.Receiver, ForwarderPublic{ 
		/// The capability of staking pool
		access(self)
		let pool: Capability<&FRC20Staking.Pool>
