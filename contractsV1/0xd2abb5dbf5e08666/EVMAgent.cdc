...
contract EVMAgent{ 
	...
	access(all)
	event NewAgencySetup(agency: Address)
	
	/// Event emitted when a new agency manager is created
	access(all)
	event NewAgencyManagerCreated(forAgency: Address)
	
	/// Event emitted when a new entrusted account is created.
	access(all)
	event NewEntrustedAccountCreated(
		evmAddress: String,
	...
	access(all)
	event EntrustedAccountVerified(
		evmAddress: String,
		entrustedAccount: Address,
		byAgency: Address,
		message: String,
	...
	access(all)
	resource interface IEntrustedStatus{ 
		access(all)
		let key: String
		
		/// Borrow the agency capability
		access(all)
		fun borrowAgency(): &Agency
	...
	///
	access(all)
	resource EntrustedStatus: IEntrustedStatus{ 
		access(all)
		let key: String
		
		// Capability to the agency
		access(self)
		let agency: Capability<&Agency>
		
		// record the flow spent by the entrusted account
		access(self)
		var feeSpent: UFix64
		
		init(key: String, _ agency: Capability<&Agency>){ 
			self.key = key
			self.agency = agency
			self.feeSpent = 0.0
		}
	...
	///
	access(all)
	resource AgencyManager{ 
		// Capability to the agency
		access(self)
		let agency: Capability<&Agency>
		
		init(_ agency: Capability<&Agency>){ 
			self.agency = agency
		}
		...
		fun withdraw(amt: UFix64): @FlowToken.Vault{ 
			...
			assert(agency.getFlowBalance() >= amt, message: "Insufficient balance")
	...
	struct AgencyStatus{ 
		...
		access(all)
		var managingEntrustedAccounts: UInt64
		
		access(all)
		var spentFlowAmount: UFix64
		
		access(all)
		var earnedFlowAmount: UFix64
	...
	///
	access(all)
	resource interface AgencyPublic{ 
	...
	///
	access(all)
	resource interface AgencyPrivate{ 
	...
	///
	access(all)
	resource Agency: AgencyPublic, AgencyPrivate{ 
		access(all)
		let creator: Address
		...
		/// Setup the agency
		access(all)
		fun setup(_ acctCap: Capability<&AuthAccount>){ 
			pre{ 
			...
			if authAcct.getCapability<&Agency>(EVMAgent.evmAgencyPublicPath) != nil{ 
				authAcct.unlink(EVMAgent.evmAgencyPublicPath)
				authAcct.link<&Agency>(EVMAgent.evmAgencyPublicPath, target: EVMAgent.evmAgencyStoragePath)
			}
			...
			if authAcct.getCapability<&Agency>(privPath) != nil{ 
				authAcct.unlink(privPath)
				authAcct.link<&Agency>(privPath, target: EVMAgent.evmAgencyStoragePath)
			}
		...
		///
		access(all)
		fun createEntrustedAccount(hexPublicKey: String, hexSignature: String, timestamp: UInt64, _ acctCap: Capability<&AuthAccount>): @FlowToken.Vault{ 
			pre{ 
				acctCap.check():
			...
			let entrustedAcctFlowBalanceRef = entrustedAcct.getCapability(/public/flowTokenBalance).borrow<&FlowToken.Vault>() ?? panic("Could not borrow Balance reference to the Vault")
			// Reference to the recipient's receiver
			let entrustedAcctFlowRecipientRef = entrustedAcct.getCapability(/public/flowTokenReceiver).borrow<&FlowToken.Vault>() ?? panic("Could not borrow receiver reference to the recipient's Vault")
		...
		///
		access(all)
		fun verifyAndBorrowEntrustedAccount(methodFingerprint: String, params: [String], hexPublicKey: String, hexSignature: String, timestamp: UInt64): &AuthAccount{ 
		...
		///
		access(self)
		fun _verifyAndBorrowEntrustedAccount(message: String, hexPublicKey: String, hexSignature: String, timestamp: UInt64): &AuthAccount{ 
			let evmAddress = ETHUtils.getETHAddressFromPublicKey(hexPublicKey: hexPublicKey)
			...
			if let entrustedStatus = entrustedAcct.borrow<&EntrustedStatus>(from: EVMAgent.entrustedStatusStoragePath){ 
		...
		fun _ensureEntrustedAcctResources(_ key: String): Bool{ 
			...
			if childAcctRef.borrow<&AnyResource>(from: EVMAgent.entrustedStatusStoragePath) == nil{ 
				...
				childAcctRef.link<&EVMAgent.EntrustedStatus>(EVMAgent.entrustedStatusPublicPath, target: EVMAgent.entrustedStatusStoragePath)
		...
		///
		access(self)
		fun _getSelfPrivCap(): Capability<&Agency>{ 
			// get the agency account
			...
			let privPathId = EVMAgent.getIdentifierPrefix().concat("_agency")
			let privPath = PrivatePath(identifier: privPathId)!
			return authAcct.getCapability<&Agency>(privPath)
		}
	...
	///
	access(all)
	resource interface AgencyCenterPublic{ 
		/// Get the agencies
		access(all)
		...
		access(all)
		fun borrowAgencyByEVMAddress(_ evmAddress: String): &Agency?
	...
	///
	access(all)
	resource AgencyCenter: AgencyCenterPublic{ 
		access(self)
	...
	///
	access(all)
	fun getAgencyPublicCap(_ addr: Address): Capability<&Agency>{ 
		return getAccount(addr).capabilities.get<&Agency>(self.evmAgencyPublicPath)!
	}
	
	/// Borrow the reference to agency public
	///
	access(all)
	fun borrowAgency(_ addr: Address): &Agency?{ 
	...
	///
	access(all)
	fun borrowAgencyByEVMPublicKey(_ hexPubKey: String): &Agency?{ 
	...
	init(){ 
		...
		self.account.capabilities.publish(capability_1, at: self.evmAgencyCenterPublicPath)
