...
contract FGameLotteryRegistry{ 
	...
	access(all)
	event RegistryWhitelistUpdated(address: Address, isWhitelisted: Bool)
	
	/// Event emitted when a lottery pool is enabled
	access(all)
	event LotteryPoolEnabled(
		name: String,
		tick: String,
		ticketPrice: UFix64,
	...
	access(all)
	event LotteryPoolResourcesUpdated(name: String, address: Address, by: Address)
	...
	access(all)
	let registryControllerStoragePath: StoragePath
	
	/* --- Interfaces & Resources --- */
	/// Resource inferface for the Lottery registry
	///
	access(all)
	resource interface RegistryPublic{ 
		access(all)
	...
	///
	access(all)
	resource Registry: RegistryPublic{ 
		access(self)
	...
	resource RegistryController{ 
		...
		fun ensureResourcesAvailable(
			...
			if childAcctRef.getCapability<&FGameLottery.LotteryPool>(
				FGameLottery.lotteryPoolPublicPath
			).borrow()
			== nil{ 
				childAcctRef.unlink(FGameLottery.lotteryPoolPublicPath)
				childAcctRef.link<&FGameLottery.LotteryPool>(
					FGameLottery.lotteryPoolPublicPath,
			...
			if childAcctRef.borrow<&AnyResource>(from: FRC20FTShared.SharedStoreStoragePath)
			== nil{ 
			...
			// link the resource to the public path
			// @deprecated after Cadence 1.0
			if childAcctRef.getCapability<&FRC20FTShared.SharedStore>(
				FRC20FTShared.SharedStorePublicPath
			).borrow()
			== nil{ 
				childAcctRef.unlink(FRC20FTShared.SharedStorePublicPath)
				childAcctRef.link<&FRC20FTShared.SharedStore>(
					FRC20FTShared.SharedStorePublicPath,
			...
			if !FixesHeartbeat.hasHook(scope: heartbeatScope, hookAddr: childAcctRef.address){ 
				FixesHeartbeat.addHook(scope: heartbeatScope, hookAddr: childAcctRef.address, hookPath: FGameLottery.lotteryPoolPublicPath)
				isUpdated = true || isUpdated
			}
			if isUpdated{ 
	...
	init(){ 
		...
		self.account.capabilities.publish(capability_1, at: self.registryPublicPath)
