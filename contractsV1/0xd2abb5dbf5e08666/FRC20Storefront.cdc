...
access(all)
contract FRC20Storefront{ 
	/* --- Events --- */
	access(all)
	event StorefrontInitialized(uuid: UInt64)
	...
	enum ListingType: UInt8{ 
		access(all)
		case FixedPriceBuyNow
		
		access(all)
		case FixedPriceSellNow
	}
	...
	resource interface ListingPublic{ 
		...
		///
		access(all)
		fun takeBuyNow(ins: &Fixes.Inscription, commissionRecipient: Capability<&FlowToken.Vault>?)
		
		/// Purchase the listing, selling the token.
		/// This pays the beneficiaries and returns the token to the buyer.
		access(all)
		fun takeSellNow(ins: &Fixes.Inscription, commissionRecipient: Capability<&FlowToken.Vault>?)
	...
	resource Listing: ListingPublic{ 
		...
		/// initializer
		///
		init(storefrontId: UInt64, listIns: &Fixes.Inscription, commissionRecipientCaps: [Capability<&FlowToken.Vault>]?, customID: String?){ 
		...
		///
		access(self)
		fun _onTransactionDeal(storefrontAddress: Address, seller: Address, buyer: Address, transactedAmt: UFix64, transactedPrice: UFix64, commissionAmount: UFix64, commissionRecipientAddress: Address?){ 
			// Some singleton resources
			let frc20Indexer = FRC20Indexer.getIndexer()
		...
		///
		access(self)
		fun _payToSaleCuts(payment: @FlowToken.Vault, commissionRecipient: Capability<&FlowToken.Vault>?, paymentRecipient: &{FungibleToken.Receiver}?): UFix64{ 
			// Some singleton resources
			let frc20Indexer = FRC20Indexer.getIndexer()
			let acctsPool = FRC20AccountsPool.borrowAccountsPool()
		...
		///
		access(self)
		fun _borrowStorefront(): &Storefront{ 
	...
	resource interface StorefrontPublic{ 
		...
		access(all)
		fun borrowListing(_ listingResourceID: UInt64): &Listing?
	...
	resource Storefront: StorefrontManager, StorefrontPublic{ 
		...
		///
		access(all)
		fun createListing(ins: @Fixes.Inscription, marginVault: @FlowToken.Vault?, commissionRecipientCaps: [Capability<&FlowToken.Vault>]?, customID: String?): UInt64{ 
			pre{ 
				self.owner != nil:
					"Resource doesn't have the assigned owner"
			}
