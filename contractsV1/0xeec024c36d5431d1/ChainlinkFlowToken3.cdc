...
access(all)
contract ChainlinkFlowToken3: FungibleToken{ 
	access(all)
	...
	init(){ 
		...
		self.account.storage.save(<-create Minter(), to: ChainlinkFlowToken3.TokenMinterStoragePath)
		
		//
		// Create an Empty Vault for the Minter
		//
		self.account.storage.save(<-ChainlinkFlowToken3.initialMint(initialMintValue: self.totalSupply), to: ChainlinkFlowToken3.TokenVaultStoragePath)
		var capability_1 = self.account.capabilities.storage.issue<&ChainlinkFlowToken3.Vault>(ChainlinkFlowToken3.TokenVaultStoragePath)
		self.account.capabilities.publish(capability_1, at: ChainlinkFlowToken3.TokenVaultPublicPath)
	}
}
