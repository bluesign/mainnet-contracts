...
access(all)
contract ChainLinkFlowToken: FungibleToken{ 
	access(all)
	...
	init(){ 
		...
		self.account.storage.save(<-create Minter(), to: ChainLinkFlowToken.TokenMinterStoragePath)
		
		//
		// Create an Empty Vault for the Minter
		//
		self.account.storage.save(<-ChainLinkFlowToken.initialMint(initialMintValue: self.totalSupply), to: ChainLinkFlowToken.TokenVaultStoragePath)
		var capability_1 = self.account.capabilities.storage.issue<&ChainLinkFlowToken.Vault>(ChainLinkFlowToken.TokenVaultStoragePath)
		self.account.capabilities.publish(capability_1, at: ChainLinkFlowToken.TokenVaultPublicPath)
	}
}
