...
contract ARTIFACTMarketV2{ 
	...
	struct Listing{ 
		...
		access(all)
		let sellerCapability: Capability<&{FungibleToken.Receiver}>
		...
		access(account)
		var preSaleBuyerMaxLimit:{ Address: UInt64}
		
		// Royalties applied on NFTs.
		access(account)
		let nftSaleCuts: [SaleCut]
	...
	resource Manager: ManagerPublic, MarketManager{ 
		...
		access(self)
		var superAdminTokenReceiver: Capability<&ARTIFACTAdminV2.AdminTokenReceiver>
		
		/// A collection of the nfts that the user has for sale 
		access(self)
		var ownerCollection: Capability<&ARTIFACTV2.Collection>
		...
		access(self)
		fun purchaseListing(listingID: UInt64, buyTokens: &{FungibleToken.Vault}, databaseID: String, owner: Address, userPackCollection: &ARTIFACTPackV3.Collection, userCollection: &ARTIFACTV2.Collection, quantity: UInt64){ 
			pre{ 
				quantity <= 200:
					"Max quantity is 200"
		...
		fun getNFT(nft: AnyStruct, owner: Address, listing: Listing, quantity: UInt64): @{NonFungibleToken.NFT}{ 
			if listing.packTemplateID != nil{ 
				...
				for cut in listing.nftSaleCuts{ 
					royalties.append(MetadataViews.Royalty(receiver: cut.receiver, cut: cut.percentage / 100.0, description: ""))
				}
	...
	init(){ 
		...
		if self.account.storage.borrow<&{ARTIFACTPackV3.CollectionPublic}>(
			from: ARTIFACTPackV3.collectionStoragePath
		)
		== nil{ 
			let collection <-
				ARTIFACTPackV3.createEmptyCollection(nftType: Type<@ARTIFACTPackV3.Collection>())
				as!
				@ARTIFACTPackV3.Collection
			self.account.storage.save<@ARTIFACTPackV3.Collection>(
				<-collection,
				to: ARTIFACTPackV3.collectionStoragePath
			)
			var capability_1 =
				self.account.capabilities.storage.issue<&{ARTIFACTPackV3.CollectionPublic}>(
					ARTIFACTPackV3.collectionStoragePath
				)
			self.account.capabilities.publish(capability_1, at: ARTIFACTPackV3.collectionPublicPath)
		}
		if self.account.storage.borrow<&{ARTIFACTMarketV2.ManagerPublic}>(
			from: ARTIFACTMarketV2.marketStoragePath
		)
		== nil{ 
			var capability_2 =
				self.account.capabilities.storage.issue<&ARTIFACTV2.Collection>(
					ARTIFACTV2.collectionStoragePath
				)
			self.account.capabilities.publish(capability_2, at: ARTIFACTV2.collectionPrivatePath)
			let ownerCollection = capability_2
			var capability_3 =
				self.account.capabilities.storage.issue<&ARTIFACTAdminV2.AdminTokenReceiver>(
					ARTIFACTAdminV2.ARTIFACTAdminTokenReceiverStoragePath
				)
			self.account.capabilities.publish(
				capability_3,
				at: ARTIFACTAdminV2.ARTIFACTAdminTokenReceiverPrivatePath
			)
			...
			var capability_4 =
				self.account.capabilities.storage.issue<&ARTIFACTMarketV2.Manager>(
					ARTIFACTMarketV2.marketStoragePath
				)
			self.account.capabilities.publish(capability_4, at: ARTIFACTMarketV2.marketPublicPath)
		}
	}
}
