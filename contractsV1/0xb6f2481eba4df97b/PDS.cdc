...
contract PDS{ 
	...
	resource SharedCapabilities{ 
		...
		fun releaseEscrow(
			...
			let pdsCollection =
				PDS.account.capabilities.get<&{NonFungibleToken.Provider}>(collectionProviderPath)
		...
		access(self)
		fun fulfillFromIssuer(nftIds: [UInt64], recvCap: &{NonFungibleToken.CollectionPublic}){ 
			let issuerCollection =
				self.withdrawCap.borrow() ?? panic("Unable to borrow withdrawCap")
			var i = 0
			while i < nftIds.length{ 
	...
	resource PackIssuer: PackIssuerCapReciever{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun changeDistributionSharedCap(distIds: [UInt64], withdrawCap: Capability<&{NonFungibleToken.Provider}>, operatorCap: Capability<&{IPackNFT.IOperator}>){ 
