...
access(all)
contract RichEntitledFuck: NonFungibleToken{ 
	access(all)
	...
	access(all)
	resource interface RichEntitledFuckCollectionPublic{ 
		access(all)
		...
		fun borrowRichEntitledFuck(id: UInt64): &RichEntitledFuck.NFT?{ 
			post{ 
	...
	access(all)
	resource Collection: RichEntitledFuckCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		...
		view fun borrowViewResolver(id: UInt64): &{ViewResolver.Resolver}?{ 
			...
			return RichEntitledFuck as &{ViewResolver.Resolver}
		}
	...
	resource NFTMinter{ 
		...
		fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}){ 
			...
			// create a new NFT
			var newNFT <- create NFT(id: RichEntitledFuck.totalSupply)
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&RichEntitledFuck.Collection>(self.CollectionStoragePath)
