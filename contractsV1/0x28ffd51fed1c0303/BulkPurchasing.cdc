...
access(all)
contract BulkPurchasing: IBulkSales{ 
	
	/// BulkPurchaseExecuted
	/// Event to notify when a user has executed a bulk purchase
	access(all)
	event BulkPurchaseExecuted(bulkOrder: ReadableBulkPurchaseOrder)
	...
	access(all)
	let CommissionReaderCapability: Capability<&{IBulkSales.ICommissionReader}>
	...
	access(all)
	struct PurchaseOrder: IBulkSales.IReadable{ 
		access(contract)
		...
		access(all)
		view fun getStorefrontType(): Type?{ 
		...
		access(contract)
		fun setReceiverAddress(_ receiver: Address){ 
		...
		access(contract)
		fun addSaleCut(recipient: Address?, amount: UFix64){ 
			pre{ 
				self.salePrice != nil:
		...
		access(contract)
		fun setVaultType(_ vaultType: Type){ 
			pre{ 
	...
	/// A human-readable struct meant for use in the BulkPurchaseExecuted event
	access(all)
	struct ReadableBulkPurchaseOrder{ 
		access(all)
		let orders: [ReadablePurchaseOrder]
		
		// actual price totals of all executed orders
		access(all)
		let priceTotals:{ String: String}
	...
	access(all)
	struct BulkPurchaseOrder: IBulkSales.IReadable{ 
		access(contract)
		let orders: [PurchaseOrder]
		
		access(contract)
		let topShotMarketV1Refs:{ Address: &Market.SaleCollection}
		
		access(contract)
		let topShotMarketV3Refs:{ Address: &TopShotMarketV3.SaleCollection}
		...
		access(self)
		fun tabulateV1Listing(_ listing: &NFTStorefront.Listing, order: PurchaseOrder): PurchaseOrder{ 
		...
		access(self)
		fun tabulateV2Listing(_ listing: &NFTStorefrontV2.Listing, order: PurchaseOrder): PurchaseOrder{ 
			let listingDetails = listing.getDetails()
			assert(!listingDetails.purchased, message: "listing has already been purchased for ".concat(listingDetails.nftType.identifier))
			self.addToPriceTotal(vaultIdentifier: listingDetails.salePaymentVaultType.identifier, listingDetails.salePrice)
		...
		access(self)
		fun tabulateTopShotListing(nftID: UInt64, salePublic: &{Market.SalePublic}, v3: Bool, order: PurchaseOrder): PurchaseOrder{ 
			pre{ 
	...
	access(all)
	struct Royalty: IBulkSales.IRoyalty, IBulkSales.IReadable{ 
		access(all)
	...
	access(all)
	resource interface PurchasingAdmin{ 
		access(all)
	...
	access(all)
	resource BulkPurchasingAdmin: PurchasingAdmin, IBulkSales.ICommissionAdmin, IBulkSales.ICommissionReader{ 
	...
	access(all)
	view fun getCatalogEntryForNFT(nftTypeIdentifier: String, ownerAddress: Address?, nftID: UInt64?): NFTCatalog.NFTCatalogMetadata?{ 
	...
	access(all)
	fun purchaseNFTs(bulkOrder: BulkPurchaseOrder, paymentVaultRefs:{ String: &{FungibleToken.Vault}}, nftReceiverCapabilities:{ String: Capability<&{NonFungibleToken.Receiver}>}, expectedTotals:{ String: UFix64}?, preferredCommissionReceiverAddresses: [Address]?, unsafeMode: Bool?){ 
		pre{ 
	...
	init(){ 
		...
		self.account.storage.save(<-create BulkPurchasingAdmin(), to: self.AdminStoragePath)
		var capability_1 = self.account.capabilities.storage.issue<&BulkPurchasingAdmin>(self.AdminStoragePath)
		self.account.capabilities.publish(capability_1, at: self.CommissionAdminPrivatePath)
		var capability_2 = self.account.capabilities.storage.issue<&BulkPurchasingAdmin>(self.AdminStoragePath)
