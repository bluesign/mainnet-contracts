...
contract TopShotMarketV3{ 
	...
	access(all)
	resource SaleCollection: Market.SalePublic{ 
		
		/// A collection of the moments that the user has for sale
		access(self)
		var ownerCollection: Capability<&TopShot.Collection>
		
		/// Capability to point at the V1 sale collection
		access(self)
		var marketV1Capability: Capability<&Market.SaleCollection>?
		...
		init(ownerCollection: Capability<&TopShot.Collection>, ownerCapability: Capability<&{FungibleToken.Receiver}>, beneficiaryCapability: Capability<&{FungibleToken.Receiver}>, cutPercentage: UFix64, marketV1Capability: Capability<&Market.SaleCollection>?){ 
			pre{ 
		...
		fun cancelSale(tokenID: UInt64){ 
			...
			// If not found in this SaleCollection, check V1
				...
				if v1MarketRef.getPrice(tokenID: tokenID) != nil{ 
					...
					let ownerCollectionRef = self.ownerCollection.borrow() ?? panic("Could not borrow owner collection reference")
