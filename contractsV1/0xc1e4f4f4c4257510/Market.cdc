...
contract Market{ 
	...
	resource SaleCollection: SalePublic{ 
		...
		access(self)
		var forSale: @TopShot.Collection
		...
		init(ownerCapability: Capability, beneficiaryCapability: Capability, cutPercentage: UFix64){ 
			...
			self.forSale <- TopShot.createEmptyCollection(nftType: Type<@TopShot.Collection>()) as! @TopShot.Collection
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun listForSale(token: @TopShot.NFT, price: UFix64){ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun withdraw(tokenID: UInt64): @TopShot.NFT{ 
