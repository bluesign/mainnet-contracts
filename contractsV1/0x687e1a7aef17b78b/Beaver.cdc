...
access(all)
contract Beaver: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				Beaver.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return Beaver.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				Beaver.maxSupply == nil || Beaver.totalSupply <= Beaver.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			Beaver.totalSupply = Beaver.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafybeid4icgfrn46k3ze2f6vbus53zk7mnkofjpqgs3oyrdiispr5hfe7e"), mediaType: "image")
				let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafybeiblasflqd3geg7eribz7vshayqkay2drlhgr3zg4slndum4i6ernm"), mediaType: "image")
				let medias = MetadataViews.Medias([media, bannerMedia])
				return FungibleTokenMetadataViews.FTDisplay(name: "Beaver", symbol: "PUSS", description: "Hey, nice beaver", externalURL: MetadataViews.ExternalURL("beavercoin.io"), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("beavercoinflow"), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: Beaver.VaultStoragePath, receiverPath: Beaver.ReceiverPublicPath, metadataPath: Beaver.VaultPublicPath, receiverLinkedType: /private/BeaverVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-Beaver.createEmptyVault(vaultType: Type<@Beaver.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/BeaverVault
		self.ReceiverPublicPath = /public/BeaverReceiver
		self.VaultPublicPath = /public/BeaverMetadata
		self.MinterStoragePath = /storage/BeaverMinter
		self.AdministratorStoragePath = /storage/BeaverAdmin
