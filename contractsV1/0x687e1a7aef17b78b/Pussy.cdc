...
access(all)
contract Pussy: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				Pussy.setBalance(address: owner, balance: self.balance)
			}
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				Pussy.maxSupply == nil || Pussy.totalSupply <= Pussy.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			Pussy.totalSupply = Pussy.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreidf6272p52ry3j2fu4ab77j4f3mjokamugqcryrtwvw5mv7njxvnm"), mediaType: "image")
				let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafkreidf6272p52ry3j2fu4ab77j4f3mjokamugqcryrtwvw5mv7njxvnm"), mediaType: "image")
				let medias = MetadataViews.Medias([media, bannerMedia])
				return FungibleTokenMetadataViews.FTDisplay(name: "Pussy", symbol: "PUSSY", description: "$Pussy on Flow ", externalURL: MetadataViews.ExternalURL("pussyonflow.com"), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: Pussy.VaultStoragePath, receiverPath: Pussy.ReceiverPublicPath, metadataPath: Pussy.VaultPublicPath, receiverLinkedType: /private/PussyVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-Pussy.createEmptyVault(vaultType: Type<@Pussy.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/PussyVault
		self.ReceiverPublicPath = /public/PussyReceiver
		self.VaultPublicPath = /public/PussyMetadata
		self.MinterStoragePath = /storage/PussyMinter
		self.AdministratorStoragePath = /storage/PussyAdmin
