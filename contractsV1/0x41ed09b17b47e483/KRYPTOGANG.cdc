...
access(all)
contract KRYPTOGANG: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				KRYPTOGANG.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return KRYPTOGANG.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				KRYPTOGANG.maxSupply == nil || KRYPTOGANG.totalSupply <= KRYPTOGANG.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			KRYPTOGANG.totalSupply = KRYPTOGANG.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "KRYPTO GANG", symbol: "KPG", description: "OUR VALUES WILL NEVER DIMINISH", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("BABAKRYPTO__"), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: KRYPTOGANG.VaultStoragePath, receiverPath: KRYPTOGANG.ReceiverPublicPath, metadataPath: KRYPTOGANG.VaultPublicPath, receiverLinkedType: /private/KRYPTOGANGVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-KRYPTOGANG.createEmptyVault(vaultType: Type<@KRYPTOGANG.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/KRYPTOGANGVault
		self.ReceiverPublicPath = /public/KRYPTOGANGReceiver
		self.VaultPublicPath = /public/KRYPTOGANGMetadata
		self.MinterStoragePath = /storage/KRYPTOGANGMinter
		self.AdministratorStoragePath = /storage/KRYPTOGANGAdmin
