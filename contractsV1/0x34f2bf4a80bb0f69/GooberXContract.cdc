...
contract GooberXContract: NonFungibleToken{ 
	...
	access(all)
	event Minted(id: UInt64, uri: String, price: UFix64)
	...
	access(all)
	event NamedGoober(id: UInt64, name: String)
	...
	access(all)
	var firstGooberInPoolIndex: UInt64
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				...
				case Type<MetadataViews.ExternalURL>():
					return MetadataViews.ExternalURL("https://partymansion.io/gooberz/".concat(self.id.toString()))
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: GooberXContract.CollectionStoragePath, publicPath: GooberXContract.CollectionPublicPath, publicCollection: Type<&GooberXContract.Collection>(), publicLinkedType: Type<&GooberXContract.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-GooberXContract.createEmptyCollection(nftType: Type<@GooberXContract.Collection>())
						})
	...
	access(all)
	resource interface GooberCollectionPublic{ 
	...
	//
	access(all)
	resource Collection: GooberCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		...
		fun listUsersGoobers():{ UInt64: GooberStruct}{ 
			...
			for key in self.ownedNFTs.keys{ 
				let el = (&self.ownedNFTs[key] as &{NonFungibleToken.NFT}?)!
