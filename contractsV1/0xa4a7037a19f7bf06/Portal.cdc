...
access(all)
contract Portal: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				Portal.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return Portal.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				Portal.maxSupply == nil || Portal.totalSupply <= Portal.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			Portal.totalSupply = Portal.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "Portal", symbol: "PORT", description: "A Flow DAO for the people", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: Portal.VaultStoragePath, receiverPath: Portal.ReceiverPublicPath, metadataPath: Portal.VaultPublicPath, receiverLinkedType: /private/PortalVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-Portal.createEmptyVault(vaultType: Type<@Portal.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/PortalVault
		self.ReceiverPublicPath = /public/PortalReceiver
		self.VaultPublicPath = /public/PortalMetadata
		self.MinterStoragePath = /storage/PortalMinter
		self.AdministratorStoragePath = /storage/PortalAdmin
