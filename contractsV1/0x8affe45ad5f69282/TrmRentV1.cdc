...
access(all)
contract TrmRentV1: NonFungibleToken{ 
	// -----------------------------------------------------------------------
	...
	resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, CollectionPublic{ 
		...
		// withdraw removes an NFT from the collection and moves it to the caller
		access(TMP_ENTITLEMENT_OWNER)
		fun withdrawUsingAssetTokenID(assetTokenID: UInt64): @{NonFungibleToken.NFT}{ 
	...
	resource Minter{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun mintNFT(assetTokenID: UInt64, kID: String, assetURL: String, expiryTimestamp: UFix64, recipient: &{CollectionPublic}): UInt64{ 
			// pre {
			//	 expiryTimestamp > getCurrentBlock().timestamp:
			//		 "Expiry timestamp should be greater than current timestamp"
			// }
	...
	init(){ 
		...
		if self.account.storage.borrow<&TrmRentV1.Minter>(from: self.minterStoragePath) == nil{ 
			
			// Put the minter in storage with access only to admin
			self.account.storage.save(<-create Minter(), to: self.minterStoragePath)
		}
		var capability_1 = self.account.capabilities.storage.issue<&TrmRentV1.Minter>(TrmRentV1.minterStoragePath)
