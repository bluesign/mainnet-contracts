...
contract TrmMarketV1{ 
	...
	access(all)
	event AssetListedForSale(assetTokenID: UInt64, price: UFix64, seller: Address?)
	...
	access(all)
	event AssetSaleListingChanged(assetTokenID: UInt64, price: UFix64, seller: Address?)
	...
	access(all)
	event AssetPurchased(assetTokenID: UInt64, price: UFix64, seller: Address?, buyer: Address?)
	...
	access(all)
	event AssetDelistedForSale(assetTokenID: UInt64, owner: Address?)
	...
	access(all)
	var minterCapability: Capability<&TrmRentV1.Minter>
	...
	resource SaleCollection: SalePublic{ 
		...
		access(self)
		var ownerCollection: Capability<&TrmAssetV1.Collection>
		
		/// Dictionary of the sale prices for each NFT by ID
		access(self)
		var saleListings:{ UInt64: SaleListing}
		...
		/// The percentage that is taken from every purchase for the beneficiary
		/// For example, if the percentage is 15%, cutPercentage = 0.15
		init(ownerCollection: Capability<&TrmAssetV1.Collection>, ownerCapability: Capability<&{FungibleToken.Receiver}>){ 
			pre{ 
		...
		fun purchase(tokenID: UInt64, buyTokens: @FUSD.Vault, recipient: &{TrmAssetV1.CollectionPublic}){ 
			...
			(			 
			 ...
			 TrmMarketV1.beneficiaryCapability.borrow()!).deposit(from: <-beneficiaryCut)
			(			 
		...
		fun rent(tokenID: UInt64, buyTokens: @FUSD.Vault, recipient: &{TrmRentV1.CollectionPublic}): UInt64{ 
			...
			(			 
			 // Deposit the remaining tokens into the owners vault
			 self.ownerCapability.borrow()!).deposit(from: <-buyTokens)
			let ownerCollectionRef = self.ownerCollection.borrow() ?? panic("Could not borrow reference to owner collection vault")
			...
			let minterRef = TrmMarketV1.minterCapability.borrow() ?? panic("Could not borrow Rent Minter Reference")
	...
	fun setupBeneficiaryFusdVault(): Capability<&{FungibleToken.Receiver}>{ 
		...
		self.account.storage.save(
			<-FUSD.createEmptyVault(vaultType: Type<@FUSD.Vault>()),
			to: /storage/fusdVault
		)
	...
	init(){ 
		...
		self.minterCapability = self.account.capabilities.get<&TrmRentV1.Minter>(
				TrmRentV1.minterPrivatePath
			)!
