...
contract StrikeNow: NonFungibleToken{ 
	...
	access(all)
	event MintClaimSubmitted(by: Address)
	
	access(all)
	event MintOnDemandPurchased()
	...
	access(self)
	var pendingMints: [StrikeNowData.MintSet]
	
	access(self)
	var failedMints: [StrikeNowData.MintSet]
	...
	init(){ 
		...
		self.account.storage.save(<-ducForwarder, to: /storage/dapperUtilityCoinReceiver)
		
		//Publish Receiver capability linked to the DUC Forwarder
		var capability_3 = self.account.capabilities.storage.issue<&{FungibleToken.Receiver}>(/storage/dapperUtilityCoinReceiver)
		self.account.capabilities.publish(capability_3, at: self.VaultPublicPath) ?? panic("Could not set up capability link for token receiver")
