...
access(all)
contract TwoSegmentsInterestRateModel{ 
	/// The storage path for the Admin resource
	access(all)
	let InterestRateModelAdminStoragePath: StoragePath
	
	/// The storage path for the InterestRateModel resource
	access(all)
	let InterestRateModelStoragePath: StoragePath
	
	/// The private path for the capability to InterestRateModel which is for admin to update model parameters
	access(all)
	let InterestRateModelPrivatePath: PrivatePath
	...
	event InterestRateModelUpdated(
		_ oldBlocksPerYear: UInt256,
		_ newBlocksPerYear: UInt256,
		_ oldScaledBaseRatePerBlock: UInt256,
		_ newScaledBaseRatePerBlock: UInt256,
		_ oldScaledBaseMultiplierPerBlock: UInt256,
		_ newScaledBaseMultiplierPerBlock: UInt256,
		_ oldScaledJumpMultiplierPerBlock: UInt256,
		_ newScaledJumpMultiplierPerBlock: UInt256,
		_ oldScaledCriticalUtilRate: UInt256,
		_ newScaledCriticalUtilRate: UInt256
	)
	
	access(all)
	resource InterestRateModel: LendingInterfaces.InterestRateModelPublic{ 
		access(self)
		let modelName: String
		
		/// See: https://docs.onflow.org/cadence/measuring-time/#time-on-the-flow-blockchain
		access(self)
		var blocksPerYear: UInt256
		
		/// The base borrow interest rate per block when utilization rate is 0 (the y-intercept)
		access(self)
		var scaledBaseRatePerBlock: UInt256
		
		/// The multiplier of utilization rate that gives the base slope of the borrow interest rate when utilRate% <= criticalUtilRate%
		access(self)
		var scaledBaseMultiplierPerBlock: UInt256
		
		/// The multiplier of utilization rate that gives the jump slope of the borrow interest rate when utilRate% > criticalUtilRate%
		access(self)
		var scaledJumpMultiplierPerBlock: UInt256
		
		/// The critical point of utilization rate beyond which the jumpMultiplierPerBlock is applied
		access(self)
		var scaledCriticalUtilRate: UInt256
		...
		access(contract)
		fun setInterestRateModelParams(_ newBlocksPerYear: UInt256, _ newScaledZeroUtilInterestRatePerYear: UInt256, _ newScaledCriticalUtilInterestRatePerYear: UInt256, _ newScaledFullUtilInterestRatePerYear: UInt256, _ newScaledCriticalUtilPoint: UInt256){ 
			pre{ 
				newScaledCriticalUtilPoint < LendingConfig.scaleFactor:
					"newScaledCriticalUtilRate should be within (0.0, 1.0) x newScaleFactor"
				newScaledZeroUtilInterestRatePerYear <= newScaledCriticalUtilInterestRatePerYear && newScaledCriticalUtilInterestRatePerYear <= newScaledFullUtilInterestRatePerYear:
					"Invalid InterestRateModel Parameters"
			}
			let scaleFactor = LendingConfig.scaleFactor
			...
			emit InterestRateModelUpdated(_blocksPerYear, self.blocksPerYear, _scaledBaseRatePerBlock, self.scaledBaseRatePerBlock, _scaledBaseMultiplierPerBlock, self.scaledBaseMultiplierPerBlock, _scaledJumpMultiplierPerBlock, self.scaledJumpMultiplierPerBlock, _scaledCriticalUtilRate, self.scaledCriticalUtilRate)
		}
		...
		init(modelName: String, blocksPerYear: UInt256, scaledZeroUtilInterestRatePerYear: UInt256, scaledCriticalUtilInterestRatePerYear: UInt256, scaledFullUtilInterestRatePerYear: UInt256, scaledCriticalUtilPoint: UInt256){ 
			pre{ 
				scaledCriticalUtilPoint < LendingConfig.scaleFactor:
					"criticalUtilRate should be within (0.0, 1.0) x scaleFactor"
				scaledZeroUtilInterestRatePerYear <= scaledCriticalUtilInterestRatePerYear && scaledCriticalUtilInterestRatePerYear <= scaledFullUtilInterestRatePerYear:
					"Invalid InterestRateModel Parameters"
			}
			let scaleFactor = LendingConfig.scaleFactor
			...
			emit InterestRateModelUpdated(0, self.blocksPerYear, 0, self.scaledBaseRatePerBlock, 0, self.scaledBaseMultiplierPerBlock, 0, self.scaledJumpMultiplierPerBlock, 0, self.scaledCriticalUtilRate)
		}
	}
	...
	init(){ 
		...
		destroy <-self.account.storage.load<@AnyResource>(
			from: self.InterestRateModelAdminStoragePath
		)
		self.account.storage.save(<-create Admin(), to: self.InterestRateModelAdminStoragePath)
	}
}
