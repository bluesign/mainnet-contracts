...
access(all)
contract GigantikEvents_NFT: NonFungibleToken{ 
	...
	resource Series{ 
		...
		//
		access(TMP_ENTITLEMENT_OWNER)
		fun mintGigantikEvents_NFT(recipient: &{NonFungibleToken.CollectionPublic}, tokenId: UInt64, setId: UInt32){ 
			pre{ 
		...
		//
		access(TMP_ENTITLEMENT_OWNER)
		fun mintEditionGigantikEvents_NFT(recipient: &{NonFungibleToken.CollectionPublic}, tokenId: UInt64, setId: UInt32, edition: UInt32){ 
			pre{ 
				self.numberEditionsMintedPerSet[setId] != nil:
					"The Set does not exist."
				self.numberEditionsMintedPerSet[setId]! <= GigantikEvents_NFT.getSetMaxEditions(setId: setId)!:
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun batchMintGigantikEvents_NFT(recipient: &{NonFungibleToken.CollectionPublic}, setId: UInt32, tokenIds: [UInt64]){ 
			pre{ 
				tokenIds.length > 0:
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
					...
					for metadataKey in (GigantikEvents_NFT.getSetMetadata(setId: self.setId)!).keys{ 
						...
						if metadataKey.length >= 13{ 
							if metadataKey.slice(from: 0, upTo: 13) == "royalty_addr_"{ 
								...
								if cutValue != 0.0{ 
									royalties.append(MetadataViews.Royalty(receiver: getAccount(royaltyAddress).capabilities.get<&{FungibleToken.Vault}>(royaltyReceiver), cut: cutValue, description: GigantikEvents_NFT.getSetMetadataByField(setId: self.setId, field: "royalty_desc_".concat(royaltyName))!))
								}
							}
						}
					}
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: GigantikEvents_NFT.CollectionStoragePath, publicPath: GigantikEvents_NFT.CollectionPublicPath, publicCollection: Type<&GigantikEvents_NFT.Collection>(), publicLinkedType: Type<&GigantikEvents_NFT.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-GigantikEvents_NFT.createEmptyCollection(nftType: Type<@GigantikEvents_NFT.Collection>())
						})
				...
				case Type<MetadataViews.Medias>():
					return MetadataViews.Medias([MetadataViews.Media(file: MetadataViews.HTTPFile(url: GigantikEvents_NFT.getSetMetadataByField(setId: self.setId, field: "image")!), mediaType: self.getMimeType())])
			}
			return nil
		}
	...
	access(all)
	resource interface GigantikEvents_NFTCollectionPublic{ 
		access(all)
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowGigantikEvents_NFT(id: UInt64): &GigantikEvents_NFT.NFT?{ 
	...
	//
	access(all)
	resource Collection: GigantikEvents_NFTCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		...
		view fun borrowViewResolver(id: UInt64): &{ViewResolver.Resolver}?{ 
			...
			return GigantikEvents_NFTNft as &{ViewResolver.Resolver}
		}
	...
	//
	access(TMP_ENTITLEMENT_OWNER)
	fun fetch(_ from: Address, id: UInt64): &GigantikEvents_NFT.NFT?{ 
		let collection = getAccount(from).capabilities.get<&GigantikEvents_NFT.Collection>(GigantikEvents_NFT.CollectionPublicPath).borrow<&GigantikEvents_NFT.Collection>() ?? panic("Couldn't get collection")
	...
	access(TMP_ENTITLEMENT_OWNER)
	fun getAllSeries(): [GigantikEvents_NFT.SeriesData]{ 
	...
	access(TMP_ENTITLEMENT_OWNER)
	fun getAllSets(): [GigantikEvents_NFT.NFTSetData]{ 
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&GigantikEvents_NFT.Admin>(self.AdminStoragePath)
