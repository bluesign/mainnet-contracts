...
contract breakingttest_NFT: NonFungibleToken{ 
	...
	fun fetch(_ from: Address, id: UInt64): &breakingttest_NFT.NFT?{ 
		let collection = (getAccount(from).capabilities.get<&breakingttest_NFT.Collection>(breakingttest_NFT.CollectionPublicPath)!).borrow() ?? panic("Couldn't get collection")
