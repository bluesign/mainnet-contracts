...
contract HybridCustody{ 
	...
	//
	access(all)
	let OwnedAccountStoragePath: StoragePath
	
	access(all)
	let OwnedAccountPublicPath: PublicPath
	
	access(all)
	let OwnedAccountPrivatePath: PrivatePath
	...
	access(all)
	let ManagerPrivatePath: PrivatePath
	
	access(all)
	let LinkedAccountPrivatePath: PrivatePath
	
	access(all)
	let BorrowableAccountPrivatePath: PrivatePath
	
	/* --- Events --- */
	//
	/// Manager creation event
	access(all)
	event CreatedManager(id: UInt64)
	
	/// OwnedAccount creation event
	access(all)
	event CreatedOwnedAccount(id: UInt64, child: Address)
	
	/// ChildAccount added/removed from Manager
	///	 active  : added to Manager
	///	 !active : removed from Manager
	access(all)
	event AccountUpdated(id: UInt64?, child: Address, parent: Address, active: Bool)
	...
	access(all)
	event AccountSealed(id: UInt64, address: Address, parents: [Address])
	
	/// An OwnedAccount shares the BorrowableAccount capability to itelf with ChildAccount resources
	///
	access(all)
	resource interface BorrowableAccount{ 
		access(contract)
		fun borrowAccount(): &AuthAccount
	...
	resource interface OwnedAccountPrivate{ 
		...
		access(all)
		fun removeParent(parent: Address): Bool
		...
		fun publishToParent(
			...
			pre{ 
				factory.check():
					"Invalid CapabilityFactory.Getter Capability provided"
				filter.check():
					"Invalid CapabilityFilter Capability provided"
			}
		}
		...
		access(all)
		fun setCapabilityFactoryForParent(
			parent: Address,
		...
		access(all)
		fun setCapabilityFilterForParent(
			parent: Address,
			cap: Capability<&{CapabilityFilter.Filter}>
		){ 
			pre{ 
				cap.check():
					"Invalid CapabilityFilter Capability provided"
		...
		access(all)
		fun addCapabilityToDelegator(parent: Address, cap: Capability, isPublic: Bool){ 
			pre{ 
		...
		access(all)
		fun removeCapabilityFromDelegator(parent: Address, cap: Capability)
		...
		access(all)
		fun isChildOf(_ addr: Address): Bool
	...
	///
	access(all)
	resource interface AccountPublic{ 
		access(all)
		fun getPublicCapability(path: PublicPath, type: Type): Capability?
		
		access(all)
		fun getPublicCapFromDelegator(type: Type): Capability?
		
		access(all)
		fun getAddress(): Address
		
		access(all)
		fun getCapabilityFactoryManager(): &{CapabilityFactory.Getter}?
		
		access(all)
		fun getCapabilityFilter(): &{CapabilityFilter.Filter}?
	}
	
	/// Methods accessible to the designated parent of a ChildAccount
	///
	access(all)
	resource interface AccountPrivate{ 
		access(all)
		fun getCapability(path: CapabilityPath, type: Type): Capability?{ 
			post{ 
		...
		access(all)
		view fun getManagerCapabilityFilter(): &{CapabilityFilter.Filter}?
		
		access(all)
		fun getPublicCapFromDelegator(type: Type): Capability?
		
		access(all)
		fun getPrivateCapFromDelegator(type: Type): Capability?{ 
			post{ 
				result == nil || [true, nil].contains(self.getManagerCapabilityFilter()?.allowed(cap: result!)):
					"Capability is not allowed by this account's Parent"
			}
		...
		access(contract)
		fun redeemedCallback(_ addr: Address)
		
		access(contract)
		fun setManagerCapabilityFilter(
			_ managerCapabilityFilter: Capability<&{CapabilityFilter.Filter}>?
		){ 
			pre{ 
				managerCapabilityFilter == nil || (managerCapabilityFilter!).check():
					"Invalid Manager Capability Filter"
			}
		}
		
		access(contract)
		fun parentRemoveChildCallback(parent: Address)
	}
	...
	resource interface ManagerPrivate{ 
		access(all)
		fun addAccount(cap: Capability<&{AccountPrivate, AccountPublic, ViewResolver.Resolver}>)
		
		access(all)
		fun borrowAccount(addr: Address): &{AccountPrivate, AccountPublic, ViewResolver.Resolver}?
		
		access(all)
		fun removeChild(addr: Address)
		
		access(all)
		fun addOwnedAccount(
			cap: Capability<&{OwnedAccountPrivate, OwnedAccountPublic, ViewResolver.Resolver}>
		)
		
		access(all)
		fun borrowOwnedAccount(addr: Address): &{
			OwnedAccountPrivate,
			OwnedAccountPublic,
	...
	resource interface ManagerPublic{ 
		access(all)
		fun borrowAccountPublic(addr: Address): &{AccountPublic, ViewResolver.Resolver}?
		
		access(all)
		fun getChildAddresses(): [Address]
		
		access(all)
		fun getOwnedAddresses(): [Address]
		
		access(all)
		fun getChildAccountDisplay(address: Address): MetadataViews.Display?
		
		access(contract)
		fun removeParentCallback(child: Address)
	}
	...
	///
	access(all)
	resource Manager: ManagerPrivate, ManagerPublic, ViewResolver.Resolver{ 
		
		/// Mapping of restricted access child account Capabilities indexed by their address
		access(all)
		let childAccounts:{ Address: Capability<&{AccountPrivate, AccountPublic, ViewResolver.Resolver}>}
		
		/// Mapping of unrestricted owned account Capabilities indexed by their address
		access(all)
		let ownedAccounts:{ Address: Capability<&{OwnedAccountPrivate, OwnedAccountPublic, ViewResolver.Resolver}>}
		...
		access(all)
		var filter: Capability<&{CapabilityFilter.Filter}>?
		
		// display metadata for a child account exists on its parent
		access(all)
		let childAccountDisplays:{ Address: MetadataViews.Display}
		
		/// Sets the Display on the ChildAccount. If nil, the display is removed.
		///
		access(all)
		fun setChildAccountDisplay(address: Address, _ d: MetadataViews.Display?){ 
			pre{ 
				self.childAccounts[address] != nil:
					"There is no child account with this address"
			}
			if d == nil{ 
		...
		fun addAccount(cap: Capability<&{AccountPrivate, AccountPublic, ViewResolver.Resolver}>){ 
			pre{ 
				self.childAccounts[cap.address] == nil:
					"There is already a child account with this address"
			}
			let acct = cap.borrow() ?? panic("child account capability could not be borrowed")
		...
		///
		access(all)
		fun setDefaultManagerCapabilityFilter(cap: Capability<&{CapabilityFilter.Filter}>?){ 
			pre{ 
				cap == nil || (cap!).check():
					"supplied capability must be nil or check must pass"
			}
			self.filter = cap
		}
		...
		///
		access(all)
		fun setManagerCapabilityFilter(cap: Capability<&{CapabilityFilter.Filter}>?, childAddress: Address){ 
			let acct = self.borrowAccount(addr: childAddress) ?? panic("child account not found")
		...
		access(all)
		fun removeChild(addr: Address){ 
			let cap = self.childAccounts.remove(key: addr) ?? panic("child account not found")
		...
		///
		access(all)
		fun addOwnedAccount(cap: Capability<&{OwnedAccountPrivate, OwnedAccountPublic, ViewResolver.Resolver}>){ 
			pre{ 
				self.ownedAccounts[cap.address] == nil:
					"There is already an owned account with this address"
			}
			let acct = cap.borrow() ?? panic("owned account capability could not be borrowed")
		...
		///
		access(all)
		fun borrowOwnedAccount(addr: Address): &{OwnedAccountPrivate, OwnedAccountPublic, ViewResolver.Resolver}?{ 
		...
		///
		access(all)
		fun giveOwnership(addr: Address, to: Address){ 
			let acct = self.ownedAccounts.remove(key: addr) ?? panic("account not found")
	...
	/// 
	access(all)
	resource ChildAccount: AccountPrivate, AccountPublic, ViewResolver.Resolver{ 
		/// A Capability providing access to the underlying child account
		access(self)
		let childCap: Capability<&{BorrowableAccount, OwnedAccountPublic, ViewResolver.Resolver}>
		...
		access(all)
		var factory: Capability<&CapabilityFactory.Manager>
		...
		access(all)
		let delegator: Capability<&CapabilityDelegator.Delegator>
		...
		access(self)
		var managerCapabilityFilter: Capability<&{CapabilityFilter.Filter}>?
		...
		///
		access(contract)
		fun setManagerCapabilityFilter(_ managerCapabilityFilter: Capability<&{CapabilityFilter.Filter}>?){ 
		...
		///
		access(all)
		fun setCapabilityFactory(cap: Capability<&CapabilityFactory.Manager>){ 
			self.factory = cap
		}
		...
		///
		access(all)
		fun setCapabilityFilter(cap: Capability<&{CapabilityFilter.Filter}>){ 
			self.filter = cap
		}
		...
		fun getCapability(path: CapabilityPath, type: Type): Capability?{ 
			...
			if path.getType() == Type<PrivatePath>() && ((self.filter.borrow()!).allowed(cap: cap) == false || self.getManagerCapabilityFilter()?.allowed(cap: cap) ?? true == false){ 
				return nil
			}
			return cap
		}
		...
		///
		access(all)
		fun borrowCapabilityDelegator(): &CapabilityDelegator.Delegator?{ 
			let path = HybridCustody.getCapabilityDelegatorIdentifier(self.parent)
			return (self.childCap.borrow()!).borrowAccount().borrow<&CapabilityDelegator.Delegator>(from: StoragePath(identifier: path)!)
		}
		...
		init(_ childCap: Capability<&{BorrowableAccount, OwnedAccountPublic, ViewResolver.Resolver}>, _ factory: Capability<&CapabilityFactory.Manager>, _ filter: Capability<&{CapabilityFilter.Filter}>, _ delegator: Capability<&CapabilityDelegator.Delegator>, _ parent: Address){ 
			pre{ 
				childCap.check():
					"Provided childCap Capability is invalid"
				factory.check():
					"Provided factory Capability is invalid"
				filter.check():
					"Provided filter Capability is invalid"
				delegator.check():
					"Provided delegator Capability is invalid"
			}
	...
	resource OwnedAccount:
		...
		access(self)
		var display: MetadataViews.Display?
		...
		fun setRedeemed(_ addr: Address){ 
			pre{ 
				self.parents[addr] != nil:
					"address is not waiting to be redeemed"
			}
		...
		fun setOwnerCallback(_ addr: Address){ 
			pre{ 
				self.pendingOwner == addr:
					"Address does not match pending owner!"
			}
		...
		fun publishToParent(
			...
			pre{ 
				self.parents[parentAddress] == nil:
					"Address pending or already redeemed as parent"
			}
			let capDelegatorIdentifier =
				HybridCustody.getCapabilityDelegatorIdentifier(parentAddress)
			...
			assert(
				...
				message: "conflicting resource found in capability delegator storage slot for parentAddress"
			)
			assert(
				acct.borrow<&AnyResource>(from: childAccountStorage) == nil,
				message: "conflicting resource found in child account storage slot for parentAddress"
			)
			...
			acct.link<&CapabilityDelegator.Delegator>(
				capDelegatorPublic,
			...
			let delegator = acct.getCapability<&CapabilityDelegator.Delegator>(capDelegatorPrivate)
			assert(
				delegator.check(),
				message: "failed to setup capability delegator for parent address"
			)
			let borrowableCap =
				self.borrowAccount().getCapability<
					&{BorrowableAccount, OwnedAccountPublic, MetadataViews.Resolver}
				>(HybridCustody.OwnedAccountPrivatePath)
			let childAcct <-
				create ChildAccount(borrowableCap, factory, filter, delegator, parentAddress)
			let childAccountPrivatePath = PrivatePath(identifier: identifier)!
			acct.save(<-childAcct, to: childAccountStorage)
			acct.link<&ChildAccount>(childAccountPrivatePath, target: childAccountStorage)
			let delegatorCap = acct.getCapability<&ChildAccount>(childAccountPrivatePath)
		...
		fun removeParent(parent: Address): Bool{ 
			...
			destroy <-acct.load<@AnyResource>(
				from: StoragePath(identifier: capDelegatorIdentifier)!
			)
		...
		fun giveOwnership(to: Address){ 
			...
			if self.acctOwner != nil{ 
				acct.unlink(PrivatePath(identifier: HybridCustody.getOwnerIdentifier(self.acctOwner!))!)
			}
			// Link a Capability for the new owner, retrieve & publish
			let identifier = HybridCustody.getOwnerIdentifier(to)
			let cap =
				acct.link<&{OwnedAccountPrivate, OwnedAccountPublic, MetadataViews.Resolver}>(
		...
		fun rotateAuthAccount(){ 
			...
			let authAcctPath =
				"HybridCustodyRelinquished_".concat(HybridCustody.account.address.toString())
					.concat(getCurrentBlock().height.toString()).concat(
					revertibleRandom<UInt64>().toString()
				) // ensure that the path is different from the previous one
		...
		///
		access(all)
		fun borrowChildAccount(parent: Address): &ChildAccount?{ 
			let identifier = HybridCustody.getChildAccountIdentifier(parent)
			return self.borrowAccount().borrow<&ChildAccount>(
				from: StoragePath(identifier: identifier)!
			)
		}
		...
		fun setCapabilityFactoryForParent(
			...
			let p =
				self.borrowChildAccount(parent: parent) ?? panic("could not find parent address")
		...
		///
		access(all)
		fun borrowCapabilityDelegatorForParent(parent: Address): &CapabilityDelegator.Delegator?{ 
		...
		fun addCapabilityToDelegator(parent: Address, cap: Capability, isPublic: Bool){ 
			let p =
				self.borrowChildAccount(parent: parent) ?? panic("could not find parent address")
			let delegator =
				self.borrowCapabilityDelegatorForParent(parent: parent)
		...
		///
		access(all)
		fun setDisplay(_ d: MetadataViews.Display){ 
			self.display = d
		}
		
		init(_ acct: Capability<&AuthAccount>){ 
			self.acct = acct
			self.parents ={} 
			self.pendingOwner = nil
			self.acctOwner = nil
	...
	///
	access(all)
	fun getChildAccountIdentifier(_ addr: Address): String{ 
	...
	///
	access(all)
	fun getCapabilityDelegatorIdentifier(_ addr: Address): String{ 
	...
	///
	access(all)
	fun getOwnerIdentifier(_ addr: Address): String{ 
		return "HybridCustodyOwnedAccount_".concat(HybridCustody.account.address.toString()).concat(
			addr.toString()
		)
	}
	...
	///
	access(all)
	fun createOwnedAccount(acct: Capability<&AuthAccount>): @OwnedAccount{ 
		pre{ 
			acct.check():
				"invalid auth account capability"
		}
	...
	///
	access(all)
	fun createManager(filter: Capability<&{CapabilityFilter.Filter}>?): @Manager{ 
		pre{ 
			filter == nil || (filter!).check():
				"Invalid CapabilityFilter Filter capability provided"
		}
	...
	init(){ 
		...
		self.LinkedAccountPrivatePath = PrivatePath(
				identifier: "LinkedAccountPrivatePath_".concat(identifier)
			)!
		self.BorrowableAccountPrivatePath = PrivatePath(
				identifier: "BorrowableAccountPrivatePath_".concat(identifier)
			)!
