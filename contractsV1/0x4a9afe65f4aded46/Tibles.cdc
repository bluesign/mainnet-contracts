...
access(all)
contract Tibles: NonFungibleToken, TiblesApp, TiblesNFT, TiblesProducer{ 
	access(all)
	let appId: String
	...
	resource NFT: NonFungibleToken.NFT, TiblesNFT.INFT, ViewResolver.Resolver{ 
		...
		fun nftCollectionData(): MetadataViews.NFTCollectionData{ 
			return MetadataViews.NFTCollectionData(storagePath: Tibles.CollectionStoragePath, publicPath: Tibles.PublicCollectionPath, publicCollection: Type<&Tibles.Collection>(), publicLinkedType: Type<&Tibles.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
					return <-Tibles.createEmptyCollection(nftType: Type<@Tibles.Collection>())
				})
		}
