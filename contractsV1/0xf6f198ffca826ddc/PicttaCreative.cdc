...
access(all)
contract PicttaCreative: NonFungibleToken{ 
	...
	resource Admin{ 
		...
		fun borrowSet(setId: UInt32): &Set{ 
			...
			return &PicttaCreative.sets[setId] as &PicttaCreative.Set?
		}
	...
	access(all)
	resource Collection: PicttaCreativeCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic{ 
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&{PicttaCreativeCollectionPublic}>(self.CollectionStoragePath)
