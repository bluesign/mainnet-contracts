...
contract pUSD: FungibleToken{ 
	...
	resource Minter{ 
		...
		fun mintTokens(amount: UFix64): @pUSD.Vault{ 
			pre{ 
				amount > 0.0:
	...
	resource interface MinterProxyPublic{ 
		access(TMP_ENTITLEMENT_OWNER)
		fun setMinterCapability(cap: Capability<&pUSD.Minter>): Void
	}
	...
	init(){ 
		...
		self.account.storage.save(<-mintedVault, to: self.VaultStoragePath)
		var capability_1 = self.account.capabilities.storage.issue<&pUSD.Vault>(self.VaultStoragePath)
		self.account.capabilities.publish(capability_1, at: self.ReceiverPublicPath)
		var capability_2 = self.account.capabilities.storage.issue<&pUSD.Vault>(self.VaultStoragePath)
