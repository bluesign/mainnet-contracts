...
access(all)
contract FAHCoin: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				FAHCoin.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return FAHCoin.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				FAHCoin.maxSupply == nil || FAHCoin.totalSupply <= FAHCoin.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			FAHCoin.totalSupply = FAHCoin.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "Flow Against Humanity", symbol: "FAH", description: "Flow Against Humanity is a Web3 card game based on Cards Against Humanity. Play now at http://fah.boiseitguru.com", externalURL: MetadataViews.ExternalURL("fah.boiseitguru.com"), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("boise_it_guru"), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: FAHCoin.VaultStoragePath, receiverPath: FAHCoin.ReceiverPublicPath, metadataPath: FAHCoin.VaultPublicPath, receiverLinkedType: /private/FAHCoinVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-FAHCoin.createEmptyVault(vaultType: Type<@FAHCoin.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/FAHCoinVault
		self.ReceiverPublicPath = /public/FAHCoinReceiver
		self.VaultPublicPath = /public/FAHCoinMetadata
		self.MinterStoragePath = /storage/FAHCoinMinter
		self.AdministratorStoragePath = /storage/FAHCoinAdmin
