...
contract IncrementPointsV2{ 
	...
	// 
	access(all)
	view fun calculateNewPointsSinceLastUpdate(userAddr: Address, ifLog: Bool): AnyStruct{ 
	...
	fun updateUserState(userAddr: Address){ 
		...
		let accuredRes:{ String: AnyStruct} =
			self.calculateNewPointsSinceLastUpdate(userAddr: userAddr, ifLog: true)
			as!{
	...
	access(all)
	view fun getPointsRatePerDay_LiquidStaking(amount: UFix64): UFix64{ 
	...
	access(all)
	view fun getPointsRatePerDay_SwapLP(): UFix64{ 
	...
	access(all)
	view fun getUserState_stFlowHoldingBalance(userAddr: Address): UFix64{ 
	...
	access(self)
	fun setUserState_stFlowHoldingBalance(userAddr: Address, stFlowBalance: UFix64){ 
	...
	resource Admin{ 
		...
		fun setPointsRatePerDay_LiquidStakingTier(tierRate:{ UFix64: UFix64}){ 
			emit PointsTierRateChanged(
				...
				ori: IncrementPointsV2._pointsRatePerDay["LiquidStaking"]! as!{ UFix64: UFix64},
				new: tierRate
			)
		...
		fun setPointsRatePerDay_LendingSupplyTier(tierRate:{ UFix64: UFix64}){ 
			emit PointsTierRateChanged(
				...
				ori: IncrementPointsV2._pointsRatePerDay["LendingSupply"]! as!{ UFix64: UFix64},
				new: tierRate
			)
		...
		fun setPointsRatePerDay_LendingBorrowTier(tierRate:{ UFix64: UFix64}){ 
			emit PointsTierRateChanged(
				...
				ori: IncrementPointsV2._pointsRatePerDay["LendingBorrow"]! as!{ UFix64: UFix64},
				new: tierRate
			)
		...
		fun setPointsRatePerDay_SwapVolume(rate: UFix64){ 
			emit PointsRateChanged(
				source: "SwapVolume",
				ori: IncrementPointsV2._pointsRatePerDay["SwapVolume"]! as! UFix64,
				new: rate
			)
		...
		access(all)
		fun setHistorySnapshotPointsBalance(userAddr: Address, amount: UFix64){ 
			...
			emit PointsMinted(
				...
				param:{ 
					"PreAmount": (IncrementPointsV2._balancesHistorySnapshot[userAddr]!).toString(),
