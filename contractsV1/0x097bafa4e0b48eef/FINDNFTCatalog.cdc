...
contract FINDNFTCatalog{ 
	...
	access(all)
	event EntryRemoved(collectionIdentifier: String)
	
	// ProposalEntryAdded
	// A new proposal to make an addtion to the catalog has been made
	access(all)
	event ProposalEntryAdded(
		proposalID: UInt64,
		collectionIdentifier: String,
		message: String,
		status: String,
	...
	access(all)
	event ProposalEntryRemoved(proposalID: UInt64)
	
	access(all)
	let ProposalManagerStoragePath: StoragePath
	
	access(all)
	let ProposalManagerPublicPath: PublicPath
	
	access(self)
	let catalog:{ String: NFTCatalog.NFTCatalogMetadata} // { collectionIdentifier -> Metadata }
	
	
	access(self)
	let catalogTypeData:{ String:{ String: Bool}} // Additional view to go from { NFT Type Identifier -> {Collection Identifier : Bool } }
	
	
	access(self)
	let catalogProposals:{ UInt64: NFTCatalog.NFTCatalogProposal} // { ProposalID : Metadata }
	...
	fun getCollectionDataForType(nftTypeIdentifier: String): NFTCatalog.NFTCollectionData?{ 
		if let collections =
			...
			if let collection =
				FINDNFTCatalog.getCatalogEntry(collectionIdentifier: collections.keys[0]){ 
	...
	fun proposeNFTMetadata(
		...
		assert(
			proposerManagerRef.getCurrentProposalEntry()! == collectionIdentifier,
	...
	fun withdrawNFTProposal(proposalID: UInt64){ 
		...
		let proposerManagerRef = proposerManagerCap.borrow()!
		assert(
			proposerManagerRef.getCurrentProposalEntry()! == proposal.collectionIdentifier,
			message: "Expected proposal entry does not match entry for the proposer"
		)
	...
	access(account)
	fun updateCatalogEntry(collectionIdentifier: String, metadata: NFTCatalog.NFTCatalogMetadata){ 
		pre{ 
			self.catalog[collectionIdentifier] != nil:
	...
	access(account)
	fun updateCatalogProposal(proposalID: UInt64, proposalMetadata: NFTCatalog.NFTCatalogProposal){ 
	...
	access(contract)
	fun addCatalogTypeEntry(collectionIdentifier: String, metadata: NFTCatalog.NFTCatalogMetadata){ 
