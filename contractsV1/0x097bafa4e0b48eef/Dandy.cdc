...
access(all)
contract Dandy: NonFungibleToken{ 
	access(all)
	...
	access(account)
	var viewConverters:{ String: [{ViewConverter}]}
	...
	access(all)
	event Minted(id: UInt64, minter: String, name: String, description: String)
	...
	resource NFT: NonFungibleToken.NFT, ViewResolver.Resolver{ 
		...
		access(self)
		var primaryCutPaid: Bool
		
		access(contract)
		let schemas:{ String: ViewInfo}
		...
		access(contract)
		let platform: FindForge.MinterPlatform
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			...
			if type == Type<MetadataViews.NFTCollectionData>(){ 
				return MetadataViews.NFTCollectionData(storagePath: Dandy.CollectionStoragePath, publicPath: Dandy.CollectionPublicPath, publicCollection: Type<&Dandy.Collection>(), publicLinkedType: Type<&Dandy.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
						return <-Dandy.createEmptyCollection(nftType: Type<@Dandy.Collection>())
					})
			}
	...
	resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection, CollectionPublic{ 
		...
		access(self)
		let nftIndex:{ String:{ UInt64: Bool}}
	...
	access(account)
	fun mintNFT(name: String, description: String, thumbnail: MetadataViews.Media, platform: FindForge.MinterPlatform, schemas: [AnyStruct], externalUrlPrefix: String?): @NFT{ 
	...
	access(all)
	struct interface ViewConverter{ 
		access(all)
		let to: Type
