...
access(all)
contract FindMarketDirectOfferEscrow{ 
	access(all)
	event DirectOffer(
		tenant: String,
		id: UInt64,
	...
	resource SaleItemCollection: SaleItemCollectionPublic, FindMarket.SaleItemCollectionPublic{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun acceptDirectOffer(_ pointer: FindViews.AuthNFTPointer){ 
	...
	resource MarketBidCollection: MarketBidCollectionPublic, FindMarket.MarketBidCollectionPublic{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun bid(item: FindViews.ViewReadPointer, vault: @{FungibleToken.Vault}, nftCap: Capability<&{NonFungibleToken.Receiver}>, validUntil: UFix64?, saleItemExtraField:{ String: AnyStruct}, bidExtraField:{ String: AnyStruct}){ 
