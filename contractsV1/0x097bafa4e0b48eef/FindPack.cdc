...
access(all)
contract FindPack: NonFungibleToken{ 
	// Events
	access(all)
	...
	access(all)
	event Opened(packTypeName: String, packTypeId: UInt64, packId: UInt64, address: Address, packFields:{ String: String}, packNFTTypes: [String])
	
	access(all)
	event Fulfilled(packTypeName: String, packTypeId: UInt64, packId: UInt64, address: Address, packFields:{ String: String}, packNFTTypes: [String])
	
	access(all)
	event PackReveal(packTypeName: String, packTypeId: UInt64, packId: UInt64, address: Address, rewardId: UInt64, rewardType: String, rewardFields:{ String: String}, packFields:{ String: String}, packNFTTypes: [String])
	
	access(all)
	event Purchased(packTypeName: String, packTypeId: UInt64, packId: UInt64, address: Address, amount: UFix64, packFields:{ String: String}, packNFTTypes: [String])
	
	access(all)
	event MetadataRegistered(packTypeName: String, packTypeId: UInt64)
	
	access(all)
	event FulfilledError(packTypeName: String, packTypeId: UInt64, packId: UInt64, address: Address?, reason: String)
	...
	access(contract)
	let packMetadata:{ String:{ UInt64: Metadata}}
	...
	struct PackRegisterInfo{ 
		...
		access(all)
		let packFields:{ String: String}
		
		access(all)
		let primaryRoyalty: [Royalty]
		
		access(all)
		let secondaryRoyalty: [Royalty]
		...
		access(all)
		let saleInfo: [PackRegisterSaleInfo]
		
		access(all)
		let extra:{ String: AnyStruct}
		
		init(forge: String, name: String, description: String, typeId: UInt64, externalURL: String, squareImageHash: String, bannerHash: String, socials:{ String: String}, paymentAddress: Address, paymentType: String, openTime: UFix64, packFields:{ String: String}, primaryRoyalty: [Royalty], secondaryRoyalty: [Royalty], requiresReservation: Bool, nftTypes: [String], storageRequirement: UInt64, saleInfo: [PackRegisterSaleInfo]){ 
			self.forge = forge
			self.name = name
	...
	struct PackRegisterSaleInfo{ 
		...
		access(all)
		let verifiers: [{FindVerifier.Verifier}]
		...
		init(name: String, startTime: UFix64, price: UFix64, verifiers: [{FindVerifier.Verifier}], verifyAll: Bool, extra:{ String: AnyStruct}){ 
			self.name = name
		...
		fun generateSaleInfo(): SaleInfo{ 
			...
			return SaleInfo(name: self.name, startTime: self.startTime, endTime: endTime, price: self.price, purchaseLimit: purchaseLimit, verifiers: self.verifiers, verifyAll: self.verifyAll)
		}
	}
	...
	struct SaleInfo{ 
		...
		access(all)
		let purchaseLimit: UInt64?
		
		access(all)
		let purchaseRecord:{ Address: UInt64}
		...
		access(contract)
		fun verify(_ addr: Address): Bool{ 
		...
		access(contract)
		fun generateParam(_ addr: Address):{ String: AnyStruct}{ 
			return{ "address": addr}
		}
	}
	...
	struct MintPackData{ 
		...
		access(all)
		let verifierRef: &FindForge.Verifier
	...
	struct Metadata{ 
		...
		access(contract)
		let primarySaleRoyalties: MetadataViews.Royalties
		...
		access(contract)
		fun borrowSaleInfo(_ i: Int): &SaleInfo{ 
			return &self.saleInfos[i] as &FindPack.SaleInfo
		}
	}
	
	access(account)
	fun registerMetadata(packTypeName: String, typeId: UInt64, metadata: Metadata){ 
		emit MetadataRegistered(packTypeName: packTypeName, packTypeId: typeId)
		let _mapping = self.packMetadata[packTypeName] ??{} //<- if this is empty then setup the storage slot for this pack type 
		
		
		// first time we create this type ID, if its not there then we create it.
		if _mapping[typeId] == nil{ 
			...
			FindPack.account.storage.save<@{NonFungibleToken.Collection}>(<-FindPack.createEmptyCollection(nftType: Type<@FindPack.Collection>()), to: storagePath)
			FindPack.account.link<&FindPack.Collection>(publicPath, target: storagePath)
		}
		_mapping[typeId] = metadata
	...
	resource NFT: NonFungibleToken.NFT, ViewResolver.Resolver{ 
		...
		access(self)
		var openedBy:{ Type: Capability<&{NonFungibleToken.Receiver}>}
		...
		access(contract)
		fun setOpenedBy(_ cap:{ Type: Capability<&{NonFungibleToken.Receiver}>}){ 
			if self.openedBy.length != 0{ 
		...
		view fun getViews(): [Type]{ 
			return [Type<MetadataViews.Display>(), Type<Metadata>(), Type<String>(), Type<MetadataViews.ExternalURL>(), Type<MetadataViews.Royalties>(), Type<MetadataViews.NFTCollectionData>(), Type<MetadataViews.NFTCollectionDisplay>()]
		}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			...
			switch view{ 
				case Type<MetadataViews.Display>():
					return MetadataViews.Display(name: metadata.name, description: metadata.description, thumbnail: metadata.getThumbnail())
				case Type<String>():
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: FindPack.CollectionStoragePath, publicPath: FindPack.CollectionPublicPath, publicCollection: Type<&FindPack.Collection>(), publicLinkedType: Type<&FindPack.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-FindPack.createEmptyCollection(nftType: Type<@FindPack.Collection>())
						})
		...
		fun buyWithSignature(packId: UInt64, signature: String, vault: @{FungibleToken.Vault}, collectionCapability: Capability<&Collection>){ 
			pre{ 
				(self.owner!).address == FindPack.account.address:
					"You can only buy pack directly from the FindPack account"
			}
	...
	access(account)
	fun mintNFT(packTypeName: String, typeId: UInt64, hash: String, royalties: [MetadataViews.Royalty]): @{NonFungibleToken.NFT}{ 
	...
	access(account)
	fun fulfill(packId: UInt64, types: [Type], rewardIds: [UInt64], salt: String){ 
		...
		for i, type in types{ 
			...
			if nft.getViews().contains(viewType){ 
				...
				fields = view.data
			} else if let display = MetadataViews.getDisplay(nft){ 
			...
			emit PackReveal(packTypeName: packTypeName, packTypeId: packTypeId, packId: packId, address: (receiver[type]!).address, rewardId: id, rewardType: token.getType().identifier, rewardFields: fields, packFields: packFields, packNFTTypes: packNFTTypes)
	...
	access(account)
	fun getPacksCollectionPath(packTypeName: String, packTypeId: UInt64): String{ 
	...
	access(contract)
	fun borrowSaleInfo(packTypeName: String, packTypeId: UInt64, index: Int): &FindPack.SaleInfo{ 
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&FindPack.Collection>(FindPack.OpenedCollectionStoragePath)
		self.account.capabilities.publish(capability_1, at: FindPack.OpenedCollectionPublicPath)
		...
		var capability_2 = self.account.capabilities.storage.issue<&FindPack.Collection>(FindPack.DLQCollectionStoragePath)
		self.account.capabilities.publish(capability_2, at: FindPack.DLQCollectionPublicPath)
