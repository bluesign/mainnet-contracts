...
access(all)
contract FindLeaseMarketAuctionSoft{ 
	access(all)
	...
	resource SaleItem: FindLeaseMarket.SaleItem{ 
		access(contract)
		var pointer: FindLeaseMarket.AuthLeasePointer
	...
	resource interface SaleItemCollectionPublic{ 
		...
		access(contract)
		fun registerIncreasedBid(_ name: String, oldBalance: UFix64)
		
		//place a bid on a token
		access(contract)
		fun registerBid(name: String, callback: Capability<&MarketBidCollection>, vaultType: Type)
		
		//only buyer can fulfill auctions since he needs to send funds for this type
		access(contract)
		fun fulfillAuction(name: String, vault: @{FungibleToken.Vault})
	}
	...
	resource SaleItemCollection:
		...
		access(self)
		fun addBid(name: String, newOffer: Capability<&MarketBidCollection>, oldBalance: UFix64){ 
	...
	resource Bid: FindLeaseMarket.Bid{ 
		...
		access(contract)
		fun setBalance(_ balance: UFix64){ 
			self.balance = balance
		}
	...
	resource MarketBidCollection:
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun bid(name: String, amount: UFix64, vaultType: Type, bidExtraField:{ String: AnyStruct}){ 
			pre{ 
			...
			let saleItemCollection =
				from.borrow() ?? panic("Could not borrow sale item for lease name=".concat(name))
