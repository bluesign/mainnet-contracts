...
contract FindForgeOrder{ 
	...
	access(all)
	let QueuedCollectionStoragePath: StoragePath
	
	access(all)
	let QueuedCollectionPublicPath: PublicPath
	
	access(all)
	let CompletedCollectionStoragePath: StoragePath
	
	access(all)
	let CompletedCollectionPublicPath: PublicPath
	
	access(all)
	let mintTypes: [String]
	
	// contractName : Resource UUID
	access(all)
	let contractNames:{ String: UInt64}
	
	access(all)
	resource Order: ViewResolver.Resolver{ 
		access(all)
		let id: UInt64
		...
		access(all)
		let minterCut: UFix64?
		
		access(all)
		let collectionDisplay: MetadataViews.NFTCollectionDisplay
		
		init(lease: String, mintType: String, minterCut: UFix64?, collectionDisplay: MetadataViews.NFTCollectionDisplay){ 
			pre{ 
				collectionDisplay.name.toLower() == lease:
					"Collection Display Name must equal to lease Name"
				FindForgeOrder.mintTypes.contains(mintType):
					"MintType is not supported at the moment"
			}
			self.id = self.uuid
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<MetadataViews.Display>():
					return MetadataViews.Display(name: self.collectionDisplay.name, description: self.collectionDisplay.description, thumbnail: self.collectionDisplay.squareImage.file)
	...
	access(all)
	resource Collection: ViewResolver.ResolverCollection{ 
		access(all)
		let orders: @{UInt64: FindForgeOrder.Order}
	...
	fun orderForge(
		...
		emit ForgeOrdered(
			...
			collectionExternalURL: c.externalURL.url,
			collectionSquareImage: c.squareImage.file.uri(),
			collectionBannerImage: c.bannerImage.file.uri(),
			collectionSocials: s
		)
		FindForgeOrder.contractNames[order.contractName] = order.id
		let col =
			FindForgeOrder.account.storage.borrow<&FindForgeOrder.Collection>(
				from: FindForgeOrder.QueuedCollectionStoragePath
			)!
	...
	fun cancelForgeOrder(leaseName: String, mintType: String){ 
		...
		let id =
			FindForgeOrder.contractNames[contractName]
			?? panic("Forge is not ordered. identifier : ".concat(contractName))
		let queuedCol =
			FindForgeOrder.account.storage.borrow<&FindForgeOrder.Collection>(
				from: FindForgeOrder.QueuedCollectionStoragePath
			)!
		...
		emit ForgeOrderCancelled(
			lease: order.leaseName,
			mintType: order.mintType,
			collectionDescription: c.description,
			collectionExternalURL: c.externalURL.url,
			collectionSquareImage: c.squareImage.file.uri(),
			collectionBannerImage: c.bannerImage.file.uri(),
	...
	access(account)
	fun fulfillForgeOrder(_ contractName: String, forgeType: Type): &FindForgeOrder.Order{ 
		let id =
			FindForgeOrder.contractNames[contractName]
			?? panic("Forge is not ordered. identifier : ".concat(contractName))
		let queuedCol =
		...
		let completedCol =
			FindForgeOrder.account.storage.borrow<&FindForgeOrder.Collection>(
				from: FindForgeOrder.CompletedCollectionStoragePath
			)!
	...
	access(account)
	fun addMintType(_ mintType: String){ 
		pre{ 
			!self.mintTypes.contains(mintType):
				"Mint type is already there : ".concat(mintType)
		}
		self.mintTypes.append(mintType)
	}
	
	access(account)
	fun removeMintType(_ mintType: String){ 
		pre{ 
			self.mintTypes.contains(mintType):
				"Mint type not there : ".concat(mintType)
		}
	...
	init(){ 
		...
		var capability_1 =
			self.account.capabilities.storage.issue<&FindForgeOrder.Collection>(
				self.QueuedCollectionStoragePath
			)
		self.account.capabilities.publish(capability_1, at: self.QueuedCollectionPublicPath)
		...
		var capability_2 =
			self.account.capabilities.storage.issue<&FindForgeOrder.Collection>(
				self.CompletedCollectionStoragePath
			)
		self.account.capabilities.publish(capability_2, at: self.CompletedCollectionPublicPath)
	}
}
