...
contract FIND{ 
	...
	access(all)
	event TokensCanNotBeRewarded()
	
	//event when FT is sent
	access(all)
	event FungibleTokenSent(
		from: Address,
		fromName: String?,
		name: String,
		toAddress: Address,
	...
	access(TMP_ENTITLEMENT_OWNER)
	fun resolve(_ input: String): Address?{ 
	...
	access(TMP_ENTITLEMENT_OWNER)
	fun lookup(_ input: String): &{Profile.Public}?{ 
	...
	access(TMP_ENTITLEMENT_OWNER)
	fun reverseLookupFN(): fun (Address): String?{ 
	...
	access(TMP_ENTITLEMENT_OWNER)
	fun reverseLookup(_ address: Address): String?{ 
	...
	resource Lease{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun checkAddon(addon: String): Bool{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun extendLeaseDapper(merchAccount: Address, vault: @DapperUtilityCoin.Vault){ 
			let network = self.networkCap.borrow() ?? panic("The network is not up")
		...
		fun move(profile: Capability<&{Profile.Public}>){ 
			...
			if sender.getFindName() == self.name{ 
				...
				let leaseCol = getAccount(senderAddress).capabilities.get<&FIND.LeaseCollection>(FIND.LeasePublicPath).borrow()!
				let nameLeases = leaseCol.getNames()
				for nameLease in nameLeases{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun getLeaseLockedUntil(): UFix64{ 
			let network = self.networkCap.borrow() ?? panic("The network is not up")
	...
	resource Auction{ 
		...
		fun getBalance(): UFix64{ 
			let cb =
				self.latestBidCallback.borrow()
				?? panic(
					"The bidder has unlinked the capability. bidder address: ".concat(
						self.latestBidCallback.address.toString()
					)
				)
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun addBid(callback: Capability<&BidCollection>, timestamp: UFix64, lease: &Lease){ 
			...
			let profile =
				getAccount(bidder).capabilities.get<&{Profile.Public}>(Profile.publicPath).borrow()
			if profile == nil{ 
			...
			emit EnglishAuction(
				...
				amount: offer.getBalance(self.name),
				auctionReservePrice: lease.auctionReservePrice!,
				status: "active_ongoing",
				vaultType: Type<@FUSD.Vault>().identifier,
				buyer: bidder,
				buyerName: bidderName,
				buyerAvatar: bidderAvatar,
				endsAt: self.endsAt,
				validUntil: lease.getLeaseExpireTime(),
				lockedUntil: lease.getLeaseLockedUntil(),
	...
	resource interface LeaseCollectionPublic{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun getInvalidatedLeases(): [String]
		...
		access(contract)
		fun cancelUserBid(_ name: String)
		
		access(contract)
		fun increaseBid(_ name: String, balance: UFix64)
		
		//place a bid on a token
		access(contract)
		fun registerBid(name: String, callback: Capability<&BidCollection>)
		...
		access(account)
		fun adminAddAddon(name: String, addon: String)
		
		access(TMP_ENTITLEMENT_OWNER)
		fun getAddon(name: String): [String]
		
		access(TMP_ENTITLEMENT_OWNER)
		fun checkAddon(name: String, addon: String): Bool
		
		access(account)
		fun getNames(): [String]
		
		access(account)
		fun containsName(_ name: String): Bool
		
		access(account)
		fun move(
			name: String,
			profile: Capability<&{Profile.Public}>,
			to: Capability<&LeaseCollection>
		)
		
		access(TMP_ENTITLEMENT_OWNER)
		fun getLeaseUUID(_ name: String): UInt64
	}
	...
	resource LeaseCollection: LeaseCollectionPublic{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun buyAddonDapper(merchAccount: Address, name: String, addon: String, vault: @DapperUtilityCoin.Vault){ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun registerUSDC(name: String, vault: @FiatToken.Vault){ 
		...
		//This has to be here since you can only get this from a auth account and thus we ensure that you cannot use wrong paths
		access(TMP_ENTITLEMENT_OWNER)
		fun registerDapper(merchAccount: Address, name: String, vault: @DapperUtilityCoin.Vault){ 
			let profileCap = (self.owner!).capabilities.get<&{Profile.Public}>(Profile.publicPath)
			let leases = (self.owner!).capabilities.get<&LeaseCollection>(FIND.LeasePublicPath)
			let network = FIND.account.storage.borrow<&Network>(from: FIND.NetworkStoragePath)!
			if !network.publicEnabled{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun cleanUpInvalidatedLease(_ name: String){ 
			let lease = self.borrow(name)
			if lease.validate(){ 
	...
	resource Network{ 
		...
		init(
			...
			self.addonPrices ={ "forge": 50.0, // will have to run transactions on this when update on mainnet.											   
											   "premiumForge": 1000.0}
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun getLease(_ name: String): NetworkLease?{ 
		...
		access(contract)
		fun renewDapper(merchAccount: Address, name: String, vault: @DapperUtilityCoin.Vault){ 
			FIND.checkMerchantAddress(merchAccount)
			if let lease = self.profiles[name]{ 
				let cost = self.calculateCost(name)
				if vault.balance != cost{ 
		...
		access(account)
		fun internal_renew(name: String){ 
			if let lease = self.profiles[name]{ 
				var newTime = 0.0
		...
		access(account)
		fun getLeaseLockedUntil(_ name: String): UFix64{ 
			if let lease = self.profiles[name]{ 
		...
		fun registerUSDC(
			...
			let account = getAccount(address)
			let usdcCap =
				account.capabilities.get<&{FungibleToken.Receiver}>(FiatToken.VaultReceiverPubPath)
			let usdcReceiver =
				usdcCap.borrow()
				?? panic(
					"cound not find usdc vault receiver for address".concat(
						self.wallet.address.toString()
					)
				)
		...
		fun registerDapper(
			...
			let wallet =
				getAccount(merchAccount).capabilities.get<&{FungibleToken.Receiver}>(
					/public/dapperUtilityCoinReceiver
				)
			let walletRef =
				wallet.borrow()
				?? panic(
					"Cannot borrow reference to Dapper Merch Account receiver. Address : ".concat(
						merchAccount.toString()
					)
				)
			walletRef.deposit(from: <-vault)
			self.internal_register(name: name, profile: profile, leases: leases)
	...
	access(TMP_ENTITLEMENT_OWNER)
	fun getFindNetworkAddress(): Address{ 
	...
	resource interface BidCollectionPublic{ 
		...
		access(contract)
		fun fulfillLease(_ token: @FIND.Lease): @{FungibleToken.Vault}
	...
	access(TMP_ENTITLEMENT_OWNER)
	fun validateFindName(_ value: String): Bool{ 
	...
	access(TMP_ENTITLEMENT_OWNER)
	fun validateAlphanumericLowerDash(_ value: String): Bool{ 
		let lowerA: UInt8 = 97
	...
	access(TMP_ENTITLEMENT_OWNER)
	fun trimFindSuffix(_ name: String): String{ 
		return FindUtils.trimSuffix(name, suffix: ".find")
	}
	
	access(contract)
	fun checkMerchantAddress(_ merchAccount: Address){ 
		...
		} else			   // otherwise falls into emulator and user dapper
	...
	access(account)
	fun getMerchantAddress(): Address{ 
		// If only find can sign the trxns and call this function, then we do not have to check the address passed in.
		// Otherwise, would it be wiser if we hard code the address here?
		if FIND.account.address == 0x097bafa4e0b48eef{ 
			// This is for mainnet
