...
contract FindMarket{ 
	...
	access(all)
	event FindTenantRemoved(tenant: String, address: Address)
	
	//Residual Royalty
	access(all)
	var residualAddress: Address
	
	// Tenant information
	access(all)
	let TenantClientPublicPath: PublicPath
	
	access(all)
	let TenantClientStoragePath: StoragePath
	
	access(contract)
	let tenantPathPrefix: String
	
	access(account)
	let tenantNameAddress:{ String: Address}
	
	access(account)
	let tenantAddressName:{ Address: String}
	...
	access(all)
	struct AllowedListing{ 
		access(all)
		let listingType: Type
	...
	struct TenantSaleItem{ 
		...
		access(contract)
		fun removeRules(_ index: Int): TenantRule{ 
			return self.rules.remove(at: index)
		}
		
		access(contract)
		fun addRules(_ rule: TenantRule){ 
			self.rules.append(rule)
		}
		
		access(contract)
		fun alterStatus(_ status: String){ 
			self.status = status
		}
		
		access(contract)
		fun isValid(nftType: Type, ftType: Type, listingType: Type): Bool{ 
	...
	access(all)
	resource Tenant: TenantPublic{ 
		access(self)
		let findSaleItems:{ String: TenantSaleItem}
		
		access(self)
		let tenantSaleItems:{ String: TenantSaleItem}
		
		access(self)
		let findCuts:{ String: TenantSaleItem}
		...
		access(account)
		fun checkFindCuts(_ cutName: String): Bool{ 
			return self.findCuts.containsKey(cutName)
		}
		
		access(account)
		fun alterMarketOption(name: String, status: String){ 
			pre{ 
		...
		fun getFindCut(name: String, listingType: Type, nftType: Type, ftType: Type): FindMarketCutStruct.Cuts?{ 
			...
			if let cache = FindRulesCache.getTenantCut(tenant: self.name, ruleId: findRuleId){ 
				...
				if let findCut = cache.findCut{ 
					returningCut = FindMarketCutStruct.Cuts(cuts: [FindMarketCutStruct.GeneralCut(name: findCut.description, cap: findCut.receiver, cut: findCut.cut, description: findCut.description)])
				}
				return returningCut
			}
			...
			for findCut in self.findCuts.values{ 
				...
				if valid && findCut.cut != nil{ 
					...
					returningCut = FindMarketCutStruct.Cuts(cuts: [FindMarketCutStruct.GeneralCut(name: (findCut.cut!).description, cap: (findCut.cut!).receiver, cut: (findCut.cut!).cut, description: (findCut.cut!).description)])
					break
				}
			}
		...
		fun getTenantCut(name: String, listingType: Type, nftType: Type, ftType: Type): FindMarketCutStruct.Cuts?{ 
			...
			if let cache = FindRulesCache.getTenantCut(tenant: self.name, ruleId: tenantRuleId){ 
				...
				if let tenantCut = cache.tenantCut{ 
					returningCut = FindMarketCutStruct.Cuts(cuts: [FindMarketCutStruct.GeneralCut(name: tenantCut.description, cap: tenantCut.receiver, cut: tenantCut.cut, description: tenantCut.description)])
				}
				return returningCut
			}
			...
			for item in self.tenantSaleItems.values{ 
				...
				if valid && item.cut != nil{ 
					...
					returningCut = FindMarketCutStruct.Cuts(cuts: [FindMarketCutStruct.GeneralCut(name: (item.cut!).description, cap: (item.cut!).receiver, cut: (item.cut!).cut, description: (item.cut!).description)])
					break
				}
			}
		...
		access(account)
		fun addSaleItem(_ item: TenantSaleItem, type: String){ 
			if type == "find"{ 
		...
		access(account)
		fun removeSaleItem(_ name: String, type: String): TenantSaleItem{ 
			if type == "find"{ 
		...
		access(contract)
		fun emitRulesEvent(item: TenantSaleItem, type: String, status: String?){ 
	...
	access(all)
	resource TenantClient: TenantClientPublic{ 
		access(self)
	...
	fun createFindMarket(
		...
		account.link<&Tenant>(pubp, target: sp)
	...
	fun getPaymentWallet(
		...
		tempCap = getAccount(cap.address).capabilities.get<&{FungibleToken.Receiver}>(
				ftInfo.receiverPath
			)!
		if tempCap.check(){ 
	...
	struct NFTInfo{ 
		...
		access(all)
		var totalInEdition: UInt64?
		
		access(all)
		var scalars:{ String: UFix64}
		...
		init(_ item: &{ViewResolver.Resolver}, id: UInt64, detail: Bool){ 
			...
			let display =
				MetadataViews.getDisplay(item) ?? panic("cannot get MetadataViews.Display View")
	...
	resource interface MarketBidCollectionPublic{ 
		...
		access(account)
		fun borrowBidItem(_ id: UInt64): &{Bid}
	}
	
	access(all)
	struct BidItemCollectionReport{ 
		access(all)
		let items: [FindMarket.BidInfo]
