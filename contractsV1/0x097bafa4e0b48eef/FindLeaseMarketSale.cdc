		...
		fun buy(name: String, vault: @{FungibleToken.Vault}, to: Address){ 
			...
			let actionResult =
				self.getTenant().allowedAction(
					...
					action: FindMarket.MarketAction(listing: false, name: "buy lease for sale"),
		...
		fun listForSale(
			...
			let actionResult =
				self.getTenant().allowedAction(
					...
					nftType: saleItem.getItemType(),
					ftType: saleItem.getFtType(),
					action: FindMarket.MarketAction(listing: true, name: "list lease for sale"),
					seller: (self.owner!).address,
					buyer: nil
				)
			if !actionResult.allowed{ 
