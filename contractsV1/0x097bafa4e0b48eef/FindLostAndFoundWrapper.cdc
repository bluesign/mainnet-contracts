...
access(all)
contract FindLostAndFoundWrapper{ 
	access(all)
	event NFTDeposited(
		receiver: Address,
		receiverName: String?,
	...
	access(all)
	event TicketRedeemed(receiver: Address, receiverName: String?, ticketID: UInt64, type: String)
	
	access(all)
	event TicketRedeemFailed(
		receiver: Address,
		receiverName: String?,
		ticketID: UInt64,
		type: String,
	...
	fun depositNFT(
		...
		let receiverAvailableStorage =
			getAccount(receiverAddress).storage.capacity - getAccount(receiverAddress).storage.used
	...
	fun redeemNFT(
		...
		let shelf =
			LostAndFound.borrowShelfManager().borrowShelf(redeemer: receiverAddress)
			?? panic("No items to redeem for this user: ".concat(receiverAddress.toString()))
		let bin =
	...
	// Helper function
	access(contract)
	fun depositVault(_ vault: @{FungibleToken.Vault}): &{FungibleToken.Vault}{ 
		let uuid = vault.uuid
	...
	access(contract)
	fun destroyVault(_ uuid: UInt64, cap: Capability<&FlowToken.Vault>){ 
		let vault <-
	...
	fun subsidizeUserStorage(
		...
		let subsidizeAmount =
			FlowStorageFees.storageCapacityToFlow(
				FlowStorageFees.convertUInt64StorageBytesToUFix64Megabytes(subsidizeCapacity)
			)
		let flowReceiverCap =
