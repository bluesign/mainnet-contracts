...
contract ToucansActions{ 
	...
	access(all)
	struct BatchWithdrawToken: Action{ 
		access(all)
		let vaultType: Type
	...
	struct WithdrawNFTs: Action{ 
		...
		init(_ collectionType: Type, _ nftIDs: [UInt64], _ recipientCollection: Capability<&{NonFungibleToken.Receiver}>, _ recipientCollectionBackup: Capability<&{NonFungibleToken.CollectionPublic}>, _ message: String){ 
			pre{ 
				recipientCollection.check() || recipientCollectionBackup.check():
					"Invalid recipient capability."
			}
			...
			self.extra ={ "backupReceiver": recipientCollectionBackup, "message": message}
		}
	}
	
	// Mint `amount` tokens to `recipientVault`
	access(all)
	struct MintTokens: Action{ 
		access(all)
		let recipientVault: Capability<&{FungibleToken.Receiver}>
	...
	access(all)
	struct MintTokensToTreasury: Action{ 
		access(all)
		let amount: UFix64
	...
	access(all)
	struct UpdateTreasuryThreshold: Action{ 
		access(all)
	...
	access(all)
	struct UnstakeFlow: Action{ 
		access(all)
		let stFlowAmount: UFix64
		
		access(all)
		let readableAmount: String
		
		access(all)
		let flowAmountOutMin: UFix64
