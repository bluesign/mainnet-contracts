...
contract Toucans{ 
	...
	access(all)
	event StakeFlow(projectId: String, by: Address, amountIn: UFix64, amountOut: UFix64)
	
	access(all)
	event UnstakeFlow(projectId: String, by: Address, amountIn: UFix64, amountOut: UFix64)
	
	access(all)
	event AddSigner(projectId: String, signer: Address)
	
	access(all)
	event RemoveSigner(projectId: String, signer: Address)
	
	access(all)
	event UpdateThreshold(projectId: String, newThreshold: UInt64)
	
	access(all)
	struct CycleTimeFrame{ 
		access(all)
	...
	access(all)
	struct FundingCycleDetails{ 
		access(all)
		let cycleId: UInt64
		
		// nil if the funding target is infinity
		access(all)
		let fundingTarget: UFix64?
		
		access(all)
		let issuanceRate: UFix64
		
		// a tax on purchases
		access(all)
		let reserveRate: UFix64
		
		access(all)
		let timeframe: CycleTimeFrame
		...
		access(all)
		let catalogCollectionIdentifier: String?
		...
		init(
			...
			pre{ 
				reserveRate <= 1.0:
					"You must provide a reserve rate value between 0.0 and 1.0"
			}
	...
	resource interface ProjectPublic{ 
		...
		access(all)
		var projectTokenInfo: ToucansTokens.TokenInfo
		
		access(all)
		let paymentTokenInfo: ToucansTokens.TokenInfo
		
		access(all)
		var totalFunding: UFix64
		
		access(all)
		var editDelay: UFix64
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun proposeBatchWithdraw(
			vaultType: Type,
			recipientVaults:{ 
				Address: Capability<&{FungibleToken.Receiver}>
			},
			amounts:{ 
				Address: UFix64
			}
		...
		fun proposeWithdrawNFTs(
			...
			message: String,
			_ recipientCollectionBackup: Capability<&{NonFungibleToken.CollectionPublic}>
		)
		
		access(TMP_ENTITLEMENT_OWNER)
		fun proposeMint(recipientVault: Capability<&{FungibleToken.Receiver}>, amount: UFix64)
		
		access(TMP_ENTITLEMENT_OWNER)
		fun proposeBatchMint(
			recipientVaults:{ 
				Address: Capability<&{FungibleToken.Receiver}>
			},
			amounts:{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun proposeMintToTreasury(amount: UFix64)
		
		access(TMP_ENTITLEMENT_OWNER)
		fun proposeBurn(tokenType: Type, amount: UFix64)
		
		access(TMP_ENTITLEMENT_OWNER)
		fun proposeAddSigner(signer: Address)
		
		access(TMP_ENTITLEMENT_OWNER)
		fun proposeRemoveSigner(signer: Address)
		
		access(TMP_ENTITLEMENT_OWNER)
		fun proposeUpdateThreshold(threshold: UInt64)
		
		access(TMP_ENTITLEMENT_OWNER)
		fun proposeLockTokens(
			recipient: Address,
			tokenType: Type,
			amount: UFix64,
			unlockTime: UFix64
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun proposeStakeFlow(flowAmount: UFix64, stFlowAmountOutMin: UFix64)
		
		access(TMP_ENTITLEMENT_OWNER)
		fun proposeUnstakeFlow(stFlowAmount: UFix64, flowAmountOutMin: UFix64)
		
		// If the action is ready to execute, then allow anyone to do it.
		access(TMP_ENTITLEMENT_OWNER)
		fun finalizeAction(actionUUID: UInt64)
		
		access(TMP_ENTITLEMENT_OWNER)
		fun donateToTreasury(vault: @{FungibleToken.Vault}, payer: Address, message: String)
		
		access(TMP_ENTITLEMENT_OWNER)
		fun donateNFTToTreasury(
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun transferProjectTokenToTreasury(
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun claimOverflow(tokenVault: @{FungibleToken.Vault}, receiver: &{FungibleToken.Receiver})
		
		access(TMP_ENTITLEMENT_OWNER)
		fun claimLockedTokens(lockedVaultUuid: UInt64, recipientVault: &{FungibleToken.Receiver})
		
		// Getters
		access(TMP_ENTITLEMENT_OWNER)
		fun getCurrentIssuanceRate(): UFix64?
		
		access(TMP_ENTITLEMENT_OWNER)
		fun getCurrentFundingCycle(): FundingCycle?
		
		access(TMP_ENTITLEMENT_OWNER)
		fun getCurrentFundingCycleId(): UInt64?
		
		access(TMP_ENTITLEMENT_OWNER)
		fun getFundingCycle(cycleIndex: UInt64): FundingCycle
		
		access(TMP_ENTITLEMENT_OWNER)
		fun getFundingCycles(): [FundingCycle]
		
		access(TMP_ENTITLEMENT_OWNER)
		fun getVaultTypesInTreasury(): [Type]
		
		access(TMP_ENTITLEMENT_OWNER)
		fun getVaultBalanceInTreasury(vaultType: Type): UFix64?
		
		access(TMP_ENTITLEMENT_OWNER)
		fun getExtra():{ String: AnyStruct}
		
		access(TMP_ENTITLEMENT_OWNER)
		fun getCompletedActionIds():{ UInt64: Bool}
		
		access(TMP_ENTITLEMENT_OWNER)
		fun getFunders():{ Address: UFix64}
		
		access(TMP_ENTITLEMENT_OWNER)
		fun getOverflowBalance(): UFix64
		
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowManagerPublic(): &Manager
		
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowLockTokensManagerPublic(): &ToucansLockTokens.Manager?
		
		access(TMP_ENTITLEMENT_OWNER)
		view fun hasTokenContract(): Bool
		
		access(TMP_ENTITLEMENT_OWNER)
		fun getCollectionTypesInTreasury(): [Type]
		
		access(TMP_ENTITLEMENT_OWNER)
		fun getNFTRefs(collectionType: Type): [&{NonFungibleToken.NFT}]
		
		access(TMP_ENTITLEMENT_OWNER)
		fun getNFTRefsByIDs(collectionType: Type, ids: [UInt64]): [&{NonFungibleToken.NFT}]
		
		access(TMP_ENTITLEMENT_OWNER)
		fun getNFTIDs(collectionType: Type): [UInt64]
	}
	...
	resource Project: ProjectPublic{ 
		...
		access(self)
		let fundingCycles: [FundingCycle]
		
		access(self)
		let treasury: @{Type:{ FungibleToken.Vault}}
		
		access(self)
		let multiSignManager: @Manager
		
		access(self)
		let overflow: @{FungibleToken.Vault}
		...
		//								  |___/	   
		access(TMP_ENTITLEMENT_OWNER)
		fun proposeWithdraw(vaultType: Type, recipientVault: Capability<&{FungibleToken.Receiver}>, amount: UFix64){ 
			let tokenInfo = self.getTokenInfo(inputVaultType: vaultType) ?? panic("Unsupported token type for withdrawing.")
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun proposeBatchWithdraw(vaultType: Type, recipientVaults:{ Address: Capability<&{FungibleToken.Receiver}>}, amounts:{ Address: UFix64}){ 
			let tokenInfo = self.getTokenInfo(inputVaultType: vaultType) ?? panic("Unsupported token type for withdrawing.")
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun proposeWithdrawNFTs(collectionType: Type, recipientCollection: Capability<&{NonFungibleToken.Receiver}>, nftIDs: [UInt64], message: String, _ recipientCollectionBackup: Capability<&{NonFungibleToken.CollectionPublic}>){ 
			let specificNFTTreasury = self.borrowSpecificNFTTreasuryCollection(type: collectionType) ?? panic("This collection type does not exist in the NFT Treasury.")
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun proposeBurn(tokenType: Type, amount: UFix64){ 
			let tokenInfo = self.getTokenInfo(inputVaultType: tokenType) ?? panic("Unsupported token type for burning.")
			let action = ToucansActions.BurnTokens(amount, tokenInfo.symbol)
			self.multiSignManager.createMultiSign(action: action)
		}
		
		access(TMP_ENTITLEMENT_OWNER)
		fun proposeBatchMint(recipientVaults:{ Address: Capability<&{FungibleToken.Receiver}>}, amounts:{ Address: UFix64}){ 
			pre{ 
				self.minting:
					"Minting is turned off."
				self.hasTokenContract():
		...
		fun proposeAddSigner(signer: Address){ 
			pre{ 
				!self.multiSignManager.getSigners().contains(signer):
					"This wallet is already a signer."
			}
		...
		fun proposeRemoveSigner(signer: Address){ 
			pre{ 
				self.multiSignManager.getSigners().length > 1:
					"Cannot remove a signer if it will bring the signers to 0."
				self.multiSignManager.getSigners().contains(signer):
					"This wallet is not already a signer."
			}
		...
		fun proposeUpdateThreshold(threshold: UInt64){ 
			pre{ 
				self.multiSignManager.getSigners().length >= Int(threshold):
					"Threshold cannot be above number of signers."
				threshold > 0:
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun proposeLockTokens(recipient: Address, tokenType: Type, amount: UFix64, unlockTime: UFix64){ 
			let tokenInfo = self.getTokenInfo(inputVaultType: tokenType) ?? panic("Unsupported token type for locking tokens.")
			...
			if actionState == ActionState.ACCEPTED{ 
				...
				switch action.getType(){ 
					...
					case Type<ToucansActions.WithdrawNFTs>():
						...
						self.withdrawNFTsFromTreasury(collectionType: withdraw.collectionType, collectionReceiver: withdraw.recipientCollection.borrow(), nftIDs: withdraw.nftIDs, collectionIdentifier: withdraw.collectionIdentifier, collectionName: withdraw.collectionName, collectionExternalURL: withdraw.collectionExternalURL, recipientAddr: recipientAddr, message: message, backupReceiver)
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun configureFundingCycle(fundingTarget: UFix64?, issuanceRate: UFix64, reserveRate: UFix64, timeframe: CycleTimeFrame, payouts: [Payout], allowOverflow: Bool, allowedAddresses: [Address]?, catalogCollectionIdentifier: String?, extra:{ String: AnyStruct}){ 
			pre{ 
				getCurrentBlock().timestamp + self.editDelay <= timeframe.startTime:
					"You cannot configure a new cycle to start within the edit delay."
				timeframe.startTime >= getCurrentBlock().timestamp:
					"Start time must be now or in the future."
				self.hasTokenContract():
					"There is no token to mint."
			}
			let newFundingCycle: FundingCycle = FundingCycle(details: FundingCycleDetails(cycleId: self.nextCycleId, fundingTarget: fundingTarget, issuanceRate: issuanceRate, reserveRate: reserveRate, timeframe: timeframe, payouts: payouts, allowOverflow: allowOverflow, allowedAddresses: allowedAddresses, catalogCollectionIdentifier: catalogCollectionIdentifier, extra))
			...
			emit NewFundingCycle(projectId: self.projectId, projectOwner: (self.owner!).address, newCycleId: self.nextCycleId, fundingTarget: fundingTarget, issuanceRate: issuanceRate, reserveRate: reserveRate, timeframe: timeframe)
			self.nextCycleId = self.nextCycleId + 1
		}
		
		// Allows you to edit a cycle that has not happened yet
		access(TMP_ENTITLEMENT_OWNER)
		fun editUpcomingCycle(cycleIndex: UInt64, details: FundingCycleDetails){ 
			...
			assert(self.editDelay == 0.0 || currentTime + self.editDelay <= details.timeframe.startTime, message: "You are no longer allowed to edit this upcoming cycle because of your edit delay.")
			assert(fundingCycle.details.timeframe.startTime == details.timeframe.startTime || currentTime <= fundingCycle.details.timeframe.startTime, message: "You can only change the start time if the round hasn't started yet.")
			assert(fundingCycle.details.timeframe.startTime == details.timeframe.startTime || currentTime <= details.timeframe.startTime, message: "New start time must be greater than now.")
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun togglePurchasing(){ 
			self.purchasing = !self.purchasing
		}
		
		access(TMP_ENTITLEMENT_OWNER)
		fun purchase(paymentTokens: @{FungibleToken.Vault}, projectTokenReceiver: &{FungibleToken.Receiver}, message: String){ 
			pre{ 
				paymentTokens.getType() == self.paymentTokenInfo.tokenType:
					"This is not the correct payment."
				self.purchasing:
					"Purchasing is turned off at the moment."
				self.hasTokenContract():
					"There is no token to purchase."
			}
			...
			if let allowedAddresses: [Address] = *fundingCycleRef.details.allowedAddresses{ 
				assert(allowedAddresses.contains(payer), message: "This account is not allowed to participate in this round.")
			}
			...
			if let catalogCollectionIdentifier: String = fundingCycleRef.details.catalogCollectionIdentifier{ 
				assert(ToucansUtils.ownsNFTFromCatalogCollectionIdentifier(collectionIdentifier: catalogCollectionIdentifier, user: payer), message: "User does not own a requried NFT for participating in the round.")
			}
			...
			emit Purchase(projectId: self.projectId, projectOwner: (self.owner!).address, currentCycle: fundingCycleRef.details.cycleId, tokenSymbol: self.paymentTokenInfo.symbol, amount: paymentAfterTax, by: payer, message: message)
		}
		...
		//				|_|			  
		access(self)
		fun depositToTreasury(vault: @{FungibleToken.Vault}){ 
		...
		access(self)
		fun depositToOverflow(vault: @{FungibleToken.Vault}){ 
			pre{ 
				vault.getType() == self.paymentTokenInfo.tokenType:
					"Not payment token type."
			}
			self.overflow.deposit(from: <-vault)
		}
		
		access(account)
		fun borrowManager(): &Manager{ 
			return &self.multiSignManager as &Manager
		}
		
		access(TMP_ENTITLEMENT_OWNER)
		view fun getTokenInfo(inputVaultType: Type): ToucansTokens.TokenInfo?{ 
		...
		access(self)
		fun withdrawFromTreasury(vaultType: Type, vault: &{FungibleToken.Receiver}, amount: UFix64, tokenSymbol: String){ 
			emit Withdraw(projectId: self.projectId, projectOwner: (self.owner!).address, currentCycle: self.getCurrentFundingCycleId(), tokenSymbol: tokenSymbol, amount: amount, to: (vault.owner!).address)
			vault.deposit(from: <-(self.treasury[vaultType]?.withdraw!)(amount: amount))
		}
		
		access(self)
		fun batchWithdrawFromTreasury(vaultType: Type, vaults:{ Address: Capability<&{FungibleToken.Receiver}>}, amounts:{ Address: UFix64}, tokenSymbol: String){ 
			let failed: [Address] = []
			...
			emit BatchWithdraw(projectId: self.projectId, projectOwner: (self.owner!).address, currentCycle: self.getCurrentFundingCycleId(), tokenSymbol: tokenSymbol, amounts: amounts, amount: totalAmount, failed: failed)
		}
		
		access(TMP_ENTITLEMENT_OWNER)
		fun donateToTreasury(vault: @{FungibleToken.Vault}, payer: Address, message: String){ 
			let tokenInfo = self.getTokenInfo(inputVaultType: vault.getType()) ?? panic("Unsupported token type for donating.")
			...
			emit Donate(projectId: self.projectId, projectOwner: (self.owner!).address, currentCycle: self.getCurrentFundingCycleId(), amount: vault.balance, tokenSymbol: tokenInfo.symbol, by: payer, message: message)
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun transferProjectTokenToTreasury(vault: @{FungibleToken.Vault}, payer: Address, message: String){ 
			pre{ 
				vault.getType() == self.projectTokenInfo.tokenType:
					"The received vault is not the project's token type."
				self.hasTokenContract():
					"There is no project token."
			}
			emit Donate(projectId: self.projectId, projectOwner: (self.owner!).address, currentCycle: self.getCurrentFundingCycleId(), amount: vault.balance, tokenSymbol: self.projectTokenInfo.symbol, by: payer, message: message)
			self.depositToTreasury(vault: <-vault)
		}
		
		access(self)
		fun markCompletedAction(actionUUID: UInt64, mark: Bool){ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun addAllowedNFTCollections(collectionIdentifiers: [String]){ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun removeAllowedNFTCollections(collectionIdentifiers: [String]){ 
			for collectionIdentifier in collectionIdentifiers{ 
		...
		//							  |___/ 
		access(account)
		fun mint(recipientVault: &{FungibleToken.Receiver}, amount: UFix64){ 
			pre{ 
				self.minting:
					"Minting is off. You cannot do this."
				self.hasTokenContract():
					"There is no token to mint."
			}
			let tokens <- self.minter.mint(amount: amount)
			recipientVault.deposit(from: <-tokens)
			emit Mint(projectId: self.projectId, by: (self.owner!).address, currentCycle: self.getCurrentFundingCycleId(), tokenSymbol: self.projectTokenInfo.symbol, to: (recipientVault.owner!).address, amount: amount)
		}
		
		access(account)
		fun batchMint(vaults:{ Address: Capability<&{FungibleToken.Receiver}>}, amounts:{ Address: UFix64}){ 
			pre{ 
				self.minting:
					"Minting is off. You cannot do this."
				self.hasTokenContract():
			...
			emit BatchMint(projectId: self.projectId, by: (self.owner!).address, currentCycle: self.getCurrentFundingCycleId(), tokenSymbol: self.projectTokenInfo.symbol, amounts: amounts, amount: totalAmount, failed: failed)
		}
		...
		//  |_| \_|_|	   |_|   
		access(TMP_ENTITLEMENT_OWNER)
		fun donateNFTToTreasury(collection: @{NonFungibleToken.Collection}, sender: Address, message: String){ 
			...
			if nftTreasury[collection.getType()] == nil{ 
				...
				nftTreasury[collection.getType()] <-! nftContract.createEmptyCollection(nftType: Type<@nftContract.Collection>())
			}
			...
			destroy collection
			emit DonateNFT(projectId: self.projectId, projectOwner: (self.owner!).address, amount: UInt64(donatedUUIDs.length), uuids: donatedUUIDs, collectionIdentifier: nftCatalogCollectionIdentifier, collectionName: nftCatalogEntry.collectionDisplay.name, collectionExternalURL: nftCatalogEntry.collectionDisplay.externalURL.url, by: sender, message: message)
		}
		
		access(self)
		fun withdrawNFTsFromTreasury(collectionType: Type, collectionReceiver: &{NonFungibleToken.Receiver}?, nftIDs: [UInt64], collectionIdentifier: String, collectionName: String, collectionExternalURL: String, recipientAddr: Address, message: String, _ backupReceiver: &{NonFungibleToken.CollectionPublic}?){ 
			emit WithdrawNFTs(projectId: self.projectId, projectOwner: (self.owner!).address, collectionIdentifier: collectionIdentifier, collectionName: collectionName, collectionExternalURL: collectionExternalURL, amount: UInt64(nftIDs.length), to: recipientAddr, message: message)
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun getAllowedNFTCollections(): [String]{ 
		...
		access(account)
		fun sendToLock(recipient: Address, tokenInfo: ToucansTokens.TokenInfo, amount: UFix64, unlockTime: UFix64){ 
			...
			emit LockTokens(projectId: self.projectId, by: (self.owner!).address, to: recipient, currentCycle: self.getCurrentFundingCycleId(), tokenSymbol: tokenInfo.symbol, amount: amount, unlockTime: unlockTime)
		}
		...
		access(account)
		fun stakeFlow(flowAmount: UFix64, stFlowAmountOutMin: UFix64){ 
			...
			assert(outVault.balance >= stFlowAmountOutMin, message: SwapError.ErrorEncode(msg: "SLIPPAGE_OFFSET_TOO_LARGE expect min ".concat(stFlowAmountOutMin.toString()).concat(" got ").concat(outVault.balance.toString()), err: SwapError.ErrorCode.SLIPPAGE_OFFSET_TOO_LARGE))
			emit StakeFlow(projectId: self.projectId, by: (self.owner!).address, amountIn: flowAmount, amountOut: outVault.balance)
			self.depositToTreasury(vault: <-outVault)
		}
		
		// DISCLAIMER: Only works on Mainnet.
		access(account)
		fun unstakeFlow(stFlowAmount: UFix64, flowAmountOutMin: UFix64){ 
			...
			assert(outVault.balance >= flowAmountOutMin, message: SwapError.ErrorEncode(msg: "SLIPPAGE_OFFSET_TOO_LARGE expect min ".concat(flowAmountOutMin.toString()).concat(" got ").concat(outVault.balance.toString()), err: SwapError.ErrorCode.SLIPPAGE_OFFSET_TOO_LARGE))
			emit UnstakeFlow(projectId: self.projectId, by: (self.owner!).address, amountIn: stFlowAmount, amountOut: outVault.balance)
			self.depositToTreasury(vault: <-outVault)
		}
		...
		//  |____/ \__,_|_|  |_| |_|
		access(account)
		fun burn(tokenType: Type, tokenSymbol: String, amount: UFix64){ 
			let tokens <- (self.treasury[tokenType]?.withdraw!)(amount: amount)
			destroy tokens
			emit Burn(projectId: self.projectId, by: (self.owner!).address, currentCycle: self.getCurrentFundingCycleId(), tokenSymbol: tokenSymbol, amount: amount)
		}
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun transferOverflowToCurrentRound(amount: UFix64){ 
			...
			assert(cycle.details.fundingTarget == nil || cycle.raisedTowardsGoal <= cycle.details.fundingTarget!, message: "Transferred Overflow cannot put the current funding round over its goal.")
		}
		
		access(TMP_ENTITLEMENT_OWNER)
		fun claimOverflow(tokenVault: @{FungibleToken.Vault}, receiver: &{FungibleToken.Receiver}){ 
			pre{ 
				tokenVault.getType() == self.projectTokenInfo.tokenType:
					"This is not the project's token."
			}
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun getCurrentFundingCycleIndex(): Int?{ 
			var i: Int = self.fundingCycles.length - 1
		...
		access(self)
		fun borrowFundingCycleRef(cycleIndex: UInt64): &FundingCycle{ 
			return &self.fundingCycles[cycleIndex] as &FundingCycle
		}
		
		access(self)
		fun borrowCurrentFundingCycleRef(): &FundingCycle?{ 
			let index: Int? = self.getCurrentFundingCycleIndex()
			if index == nil{ 
				return nil
			}
		...
		access(self)
		fun borrowLockTokensManager(): &ToucansLockTokens.Manager?{ 
		...
		access(self)
		fun borrowNFTTreasury(): &{Type:{ NonFungibleToken.Collection}}?{ 
		...
		access(self)
		fun borrowSpecificNFTTreasuryCollection(type: Type): &{NonFungibleToken.Collection}?{ 
		...
		init(projectId: String, projectTokenInfo: ToucansTokens.TokenInfo, paymentTokenInfo: ToucansTokens.TokenInfo, minter: @{Minter}, editDelay: UFix64, initialSigners: [Address], initialThreshold: UInt64, minting: Bool, initialTreasurySupply: UFix64, initialAllowedNFTCollections: [String], extra:{ String: AnyStruct}){ 
			pre{ 
				ToucansTokens.getTokenInfo(tokenType: paymentTokenInfo.tokenType) != nil:
					"Unsupported token type for payment."
			}
			...
			let emptyPaymentVault <- paymentContract.createEmptyVault(vaultType: Type<@paymentContract.Vault>())
			...
			self.extra = extra
			self.extra ={ "completedActionIds":{}  as{ UInt64: Bool}, "allowedNFTCollections": allowedNFTCollections}
			self.fundingCycles = []
			self.minter <- minter
			self.funders ={} 
			self.editDelay = editDelay
			...
			self.additions <-{ "lockedTokensManager": <-ToucansLockTokens.createManager(), "nftTreasury": <-({} as @{Type:{ NonFungibleToken.Collection}})}
			self.overflow <- paymentContract.createEmptyVault(vaultType: Type<@paymentContract.Vault>())
	...
	resource interface CollectionPublic{ 
		access(TMP_ENTITLEMENT_OWNER)
		fun getProjectIds(): [String]
		
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowProjectPublic(projectId: String): &Project?
	}
	...
	resource Collection: CollectionPublic{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun createProjectNoToken(projectId: String, paymentTokenInfo: ToucansTokens.TokenInfo, initialAllowedNFTCollections: [String], extra:{ String: AnyStruct}){ 
			let project: @Project <- create Project(projectId: projectId, projectTokenInfo: paymentTokenInfo, // use the payment token, or "preferred currency", for this																											   
																											  paymentTokenInfo: paymentTokenInfo, minter: <-create DummyMinter(), editDelay: 0.0, initialSigners: [(self.owner!).address], initialThreshold: 1, minting: false, initialTreasurySupply: 0.0, initialAllowedNFTCollections: initialAllowedNFTCollections, extra: extra)
			self.projects[projectId] <-! project
			emit ProjectCreated(projectId: projectId, tokenTypeIdentifier: nil, by: (self.owner!).address)
		}
		
		access(TMP_ENTITLEMENT_OWNER)
		fun createProject(projectTokenInfo: ToucansTokens.TokenInfo, paymentTokenInfo: ToucansTokens.TokenInfo, minter: @{Minter}, editDelay: UFix64, minting: Bool, initialTreasurySupply: UFix64, extra:{ String: AnyStruct}){ 
			...
			let project: @Project <- create Project(projectId: projectId, projectTokenInfo: projectTokenInfo, paymentTokenInfo: paymentTokenInfo, minter: <-minter, editDelay: editDelay, initialSigners: [(self.owner!).address], initialThreshold: 1, minting: minting, initialTreasurySupply: initialTreasurySupply, initialAllowedNFTCollections: initialAllowedNFTCollections, extra: extra)
			self.projects[projectId] <-! project
			emit ProjectCreated(projectId: projectId, tokenTypeIdentifier: projectTokenInfo.tokenType.identifier, by: (self.owner!).address)
		}
		
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowProject(projectId: String): &Project?{ 
	...
	resource MultiSignAction{ 
		...
		access(self)
		let votes:{ Address: Bool}
		
		access(all)
		let threshold: UInt64
		
		access(contract)
		fun vote(acctAddress: Address, vote: Bool){ 
			pre{ 
				self.signers.contains(acctAddress):
		...
		access(TMP_ENTITLEMENT_OWNER)
		view fun getSigners(): [Address]{ 
			return self.signers
		}
		
		// Only returns people who have actually voted
		access(TMP_ENTITLEMENT_OWNER)
		fun getVotes():{ Address: Bool}{ 
			return self.votes
		}
		
		access(TMP_ENTITLEMENT_OWNER)
		fun getAccepted(): UInt64{ 
			var count: UInt64 = 0
	...
	resource interface ManagerPublic{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowAction(actionUUID: UInt64): &Toucans.MultiSignAction
		
		access(TMP_ENTITLEMENT_OWNER)
		fun getActionState(actionUUID: UInt64): ActionState
		
		access(TMP_ENTITLEMENT_OWNER)
		fun readyToFinalize(actionUUID: UInt64): Bool
	...
	resource Manager: ManagerPublic{ 
		...
		access(self)
		let actions: @{UInt64: MultiSignAction}
		
		access(account)
		fun createMultiSign(action:{ ToucansActions.Action}){ 
		...
		access(account)
		fun destroyAction(actionUUID: UInt64){ 
		...
		access(account)
		fun removeSigner(signer: Address){ 
			pre{ 
				self.signers.contains(signer):
					"This wallet is not already a signer."
			}
		...
		access(account)
		fun updateThreshold(newThreshold: UInt64){ 
			self.threshold = newThreshold
			self.assertValidTreasury()
		}
		
		access(TMP_ENTITLEMENT_OWNER)
		fun borrowAction(actionUUID: UInt64): &MultiSignAction{ 
	...
	access(TMP_ENTITLEMENT_OWNER)
	fun createMultiSigManager(signers: [Address], threshold: UInt64): @Manager{ 
	...
	access(TMP_ENTITLEMENT_OWNER)
	fun assertNonConflictingCycles(
		earlierCycle: FundingCycleDetails,
		laterCycle: FundingCycleDetails
	){ 
		let earlierCycleStartsEarlier =
			earlierCycle.timeframe.startTime < laterCycle.timeframe.startTime
		let earlierCycleEndsBeforeLaterStarts =
			earlierCycle.timeframe.endTime == nil
			|| earlierCycle.timeframe.endTime! < laterCycle.timeframe.startTime
		assert(
			earlierCycleStartsEarlier && earlierCycleEndsBeforeLaterStarts,
			message: "Conflicting cycles!"
		)
	}
