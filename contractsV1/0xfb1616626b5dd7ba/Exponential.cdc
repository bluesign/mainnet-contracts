...
contract Exponential{ 
	...
	access(all)
	event BidPlaced(auctionID: UInt64, bidDetails: BidMeta, premiumPaid: UFix64)
	...
	access(all)
	event WinnerPremiumReturned(address: Address, amount: UFix64)
	
	access(all)
	event WinnerPremiumPaid(address: Address, amount: UFix64)
	
	access(all)
	event SellerPremiumPaid(address: Address, amount: UFix64)
	
	access(all)
	event PlatformPaid(address: Address, amount: UFix64)
	
	access(all)
	event SellerPaid(address: Address, amount: UFix64)
	
	access(all)
	event BatchProcessed(id: UInt64, total: UInt64)
	
	access(contract)
	var auctions: @{UInt64: Auction} // Dictionary of live Auctions
	...
	access(account)
	var premiumVaults: @{String:{ FungibleToken.Vault}}
	...
	access(account)
	var nfts: @{String:{ NonFungibleToken.Collection}}
	
	access(all)
	var adminStoragePath: StoragePath
	
	access(all)
	var adminPrivatePath: PrivatePath
	
	access(contract)
	var BATCH_SIZE: Int // used for batch sending NFTs 
	
	
	access(contract)
	var POWER: Int // Exponential Power for premium curve (set to 12 but can be updated if needed)
	
	
	access(all)
	resource Auction{ 
		access(contract)
		let id: UInt64 // unique ID for auction set in creation function
		
		
		access(contract)
		var isSettled: Bool // set to true once the auction has been paid out
		
		
		access(contract)
		let singleBidMode: Bool // if true each address can have 1 bid they can add to otherwise they have multiple bids they can't add to 
		...
		access(contract)
		var bidValues: [UFix64] // Sorted bids values 
		
		
		access(contract)
		var totalBidByAddress:{ Address: UFix64} // used to lookup bid amount from array
		
		
		access(contract)
		var totalPremiumsPaidByAddresses:{ Address: UFix64} // total premium paid by user....  one version return the winner their premium 
		
		
		access(contract)
		var sellerFTReceiverCap: Capability<&{FungibleToken.Receiver}>
		
		access(contract)
		var nftCollection: @{NonFungibleToken.Collection}
		
		access(contract)
		var minStartingBid: UFix64 // minStartingBid can be set at start of auction and doesn't include any premium....  
		
		
		access(contract)
		var startTime: UFix64 // when the auction opens for bidding
		
		
		access(contract)
		var endTime: UFix64 // end time of the auction unixtime
		
		
		access(contract)
		var duration: UFix64 // length of the auction .... 
		
		
		access(contract)
		var sellerNftReceiverCap: Capability<&{NonFungibleToken.Receiver}>
		
		access(contract)
		var premiumVault: @{FungibleToken.Vault} // vault for collecting any premiums paid	
		
		
		access(contract)
		var bidReturnsProcessed: UInt64 // used for batching bid returns
		
		
		access(all)
		let createdAtBlockHeight: UInt64
		...
		fun sendNFTsToWinners(){ 
			...
			while i < batchTotal{ 
				...
				if nftReceiverRef != nil && (nftReceiverRef!).getType() == collection.getType(){ 
					( // if receiver is still correct type					  
		...
		access(contract)
		fun sendWinningBidsToSeller(){ 
			if self.bidReturnsProcessed == UInt64(self.bids.length){ 
				return
			} // early return if all bids are processed
			
			var ownersVaultRef =
				self.sellerFTReceiverCap.borrow() ?? &self.premiumVault as &{FungibleToken.Vault} // fallback to using auctions premiumVault
		...
		//
		access(contract)
		fun returnWinnerCutOfPremium(fraction: UFix64){ 
		...
		fun withdrawRemainingPremium(){ 
			...
			emit PlatformPaid(address: Exponential.account.address, amount: remainingBalance)
		}
	...
	struct AuctionMeta{ 
		...
		access(all)
		var currentPremium: UFix64
		
		access(all)
		var singleBidMode: Bool
		
		access(all)
		var minStartingBid: UFix64
		
		access(all)
		var createdAtBlockHeight: UInt64
		...
		access(all)
		var totalPremiumsPaidByAddresses:{ Address: UFix64}
		
		access(all)
		var premiumVaultBalance: UFix64
	...
	resource Bidder{ 
		...
		access(contract)
		var depositCap: Capability<&{FungibleToken.Receiver}>
		
		access(contract)
		var nftReceiverCap: Capability<&{NonFungibleToken.Receiver}>
	...
	fun placeBid(
		...
		let bidUID = bid.uuid
		let bids = auctionRef.bids as &[Exponential.Bid]
		...
		auctionRef.totalBidByAddress[address] = auctionRef.totalBidByAddress[address] == nil
				? balance
		...
		&& auctionRef.bids.length > 0{ 
			...
			var ownersVaultRef =
				...
				?? auctionRef.premiumVault as &{FungibleToken.Vault} // if the capability is unlinked funds are sent to the premium vault
	...
	fun getAllAuctionsMeta(): [AuctionMeta]{ 
		...
		for id in auctionIDs{ 
			let a = &Exponential.auctions[id] as &Exponential.Auction?
	...
	access(contract)
	fun power(value: UFix64, power: UInt64): UFix64{ 
		var v = value
		var n = power
		while n != 1{ 
			v = v * value
	...
	init(){ 
		...
		let defaultPremiumVault <- FlowToken.createEmptyVault(vaultType: Type<@FlowToken.Vault>())
		let vaultType = defaultPremiumVault.getType().identifier
		self.premiumVaults <-{ vaultType: <-defaultPremiumVault}
		let adminResource <- create Admin()
		let adminCapability =
			Exponential.account.link<&Admin>(self.adminPrivatePath, target: self.adminStoragePath)
		Exponential.account.storage.save(<-adminResource, to: self.adminStoragePath)
