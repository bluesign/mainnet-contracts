...
contract Memento: NonFungibleToken, ViewResolver{ 
	...
	access(all)
	event Minted(id: UInt64, serial: UInt64, recipient: Address, creatorID: String)
	
	access(all)
	event MetadataSuccess(creatorID: String, description: String)
	...
	resource MetadataStorage: MetadataStoragePublic{ 
		// List of Creator 
		access(all)
		var creatorsIds:{ String: [NFTMetadata]}
		...
		access(account)
		fun creatorExist(_ creatorId: String){ 
		...
		access(account)
		fun metadataIsNew(_ creatorId: String, _ description: String): Bool{ 
		...
		access(account)
		fun addMetadata(_ creatorId: String, _ metadata: NFTMetadata){ 
			if self.creatorsIds[creatorId] == nil{ 
				self.creatorsIds[creatorId] = []
			}
		...
		access(account)
		fun updateMinted(_ creatorId: String, _ description: String){ 
		...
		fun findMetadataRef(_ creatorId: String, _ description: String): &Memento.NFTMetadata?{ 
		...
		// Public Functions
		access(TMP_ENTITLEMENT_OWNER)
		view fun findMetadata(_ creatorId: String, _ description: String): Memento.NFTMetadata?{ 
			let metadatas = self.creatorsIds[creatorId]!
			var i = metadatas.length - 1
			while i >= 0{ 
				if metadatas[i].description == description{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun getTimeRemaining(_ creatorID: String, _ description: String): UFix64?{ 
			let metadata = self.findMetadata(creatorID, description)!
	...
	resource Administrator{ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun createNFTMetadata(channel: String, creatorID: String, creatorUsername: String, creatorAddress: Address, sourceURL: String, description: String, MementoCreationDate: String, contentCreationDate: String, lockdownOption: Int, supplyOption: UInt64, imgUrl: String, embededHTML: String){ 
		...
		access(TMP_ENTITLEMENT_OWNER)
		fun mintNFT(creatorId: String, description: String, recipient: Address){ 
			pre{ 
		...
		access(account)
		view fun isMintingAvailable(_ creatorId: String, _ description: String): Bool{ 
	...
	access(TMP_ENTITLEMENT_OWNER)
	view fun getNFTMetadata(_ creatorId: String, _ description: String): Memento.NFTMetadata?{ 
