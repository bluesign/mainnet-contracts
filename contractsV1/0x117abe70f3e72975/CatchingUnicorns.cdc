...
access(all)
contract CatchingUnicorns: NonFungibleToken{ 
	access(all)
	...
	access(all)
	resource interface CatchingUnicornsCollectionPublic{ 
		access(all)
	...
	access(all)
	resource Collection: CatchingUnicornsCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		access(all)
		...
		view fun borrowViewResolver(id: UInt64): &{ViewResolver.Resolver}?{ 
			...
			return CatchingUnicorns as &{ViewResolver.Resolver}
		}
	...
	fun mintNFT(recipient: &{NonFungibleToken.CollectionPublic}, name: String, description: String, thumbnail: String){ 
		var newNFT <- create NFT(id: CatchingUnicorns.totalSupply, name: name, description: description, thumbnail: thumbnail)
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&CatchingUnicorns.Collection>(self.CollectionStoragePath)
