...
access(all)
contract ConfrariaWeb3: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				ConfrariaWeb3.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return ConfrariaWeb3.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				ConfrariaWeb3.maxSupply == nil || ConfrariaWeb3.totalSupply <= ConfrariaWeb3.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			ConfrariaWeb3.totalSupply = ConfrariaWeb3.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				...
				return FungibleTokenMetadataViews.FTDisplay(name: "ConfrariaWeb3", symbol: "EGGS", description: "Comer, beber e Conectar", externalURL: MetadataViews.ExternalURL("www.confrariaweb3.com.br"), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL("confrariaweb3"), "discord": MetadataViews.ExternalURL("xPAeS77uXa")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: ConfrariaWeb3.VaultStoragePath, receiverPath: ConfrariaWeb3.ReceiverPublicPath, metadataPath: ConfrariaWeb3.VaultPublicPath, receiverLinkedType: /private/ConfrariaWeb3Vault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-ConfrariaWeb3.createEmptyVault(vaultType: Type<@ConfrariaWeb3.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/ConfrariaWeb3Vault
		self.ReceiverPublicPath = /public/ConfrariaWeb3Receiver
		self.VaultPublicPath = /public/ConfrariaWeb3Metadata
		self.MinterStoragePath = /storage/ConfrariaWeb3Minter
		self.AdministratorStoragePath = /storage/ConfrariaWeb3Admin
