...
access(all)
contract GUACAMOLE: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				GUACAMOLE.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return GUACAMOLE.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				GUACAMOLE.maxSupply == nil || GUACAMOLE.totalSupply <= GUACAMOLE.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			GUACAMOLE.totalSupply = GUACAMOLE.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafybeidqzlabeeexzcchyfyuy7x7zwoxqfcu3pcwihnjrdfadm3akzaeaa"), mediaType: "image")
				let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafybeig2n5ibboqhw2u7qhzvaf2pwpa2f2d7hfmm6zogmzly5lzzyggsl4"), mediaType: "image")
				let medias = MetadataViews.Medias([media, bannerMedia])
				return FungibleTokenMetadataViews.FTDisplay(name: "GUACAMOLE", symbol: "GUAC", description: "BUY THE DIP! BUY THE DIP!", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: GUACAMOLE.VaultStoragePath, receiverPath: GUACAMOLE.ReceiverPublicPath, metadataPath: GUACAMOLE.VaultPublicPath, receiverLinkedType: /private/GUACAMOLEVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-GUACAMOLE.createEmptyVault(vaultType: Type<@GUACAMOLE.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/GUACAMOLEVault
		self.ReceiverPublicPath = /public/GUACAMOLEReceiver
		self.VaultPublicPath = /public/GUACAMOLEMetadata
		self.MinterStoragePath = /storage/GUACAMOLEMinter
		self.AdministratorStoragePath = /storage/GUACAMOLEAdmin
