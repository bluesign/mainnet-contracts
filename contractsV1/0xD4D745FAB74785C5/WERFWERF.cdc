...
access(all)
contract WERFWERF: FungibleToken, ViewResolver{ 
	...
	resource Vault: FungibleToken.Vault, FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, ViewResolver.Resolver{ 
		...
		fun withdraw(amount: UFix64): @{FungibleToken.Vault}{ 
			...
			if let owner: Address = self.owner?.address{ 
				WERFWERF.setBalance(address: owner, balance: self.balance)
			}
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				case Type<FungibleTokenMetadataViews.FTView>():
					return WERFWERF.resolveView(view)
	...
	resource Minter: Toucans.Minter{ 
		...
		fun mint(amount: UFix64): @Vault{ 
			post{ 
				WERFWERF.maxSupply == nil || WERFWERF.totalSupply <= WERFWERF.maxSupply!:
					"Exceeded the max supply of tokens allowd."
			}
			WERFWERF.totalSupply = WERFWERF.totalSupply + amount
	...
	fun resolveView(_ view: Type): AnyStruct?{ 
		switch view{ 
			...
			case Type<FungibleTokenMetadataViews.FTDisplay>():
				let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafybeihppkuy3dhzuqz2ekfxs4jusxf5562vb6hpzrjzokm6rzoifvyymi"), mediaType: "image")
				let bannerMedia = MetadataViews.Media(file: MetadataViews.HTTPFile(url: "https://nftstorage.link/ipfs/bafybeifkqnfqrerhrgoja62n6yxvgcmdskjrgvjkl6jsqooalo2ragoeuu"), mediaType: "image")
				let medias = MetadataViews.Medias([media, bannerMedia])
				return FungibleTokenMetadataViews.FTDisplay(name: "WERFWERF", symbol: "WERF", description: "Fuck $BEAVER, choose $WERF instead.@werfonflow", externalURL: MetadataViews.ExternalURL(""), logos: medias, socials:{ "twitter": MetadataViews.ExternalURL(""), "discord": MetadataViews.ExternalURL("")})
			case Type<FungibleTokenMetadataViews.FTVaultData>():
				return FungibleTokenMetadataViews.FTVaultData(storagePath: WERFWERF.VaultStoragePath, receiverPath: WERFWERF.ReceiverPublicPath, metadataPath: WERFWERF.VaultPublicPath, receiverLinkedType: /private/WERFWERFVault, metadataLinkedType: Type<&Vault>(), createEmptyVaultFunction: Type<&Vault>(), providerLinkedType: Type<&Vault>(), createEmptyVaultFunction: fun (): @Vault{ 
						return <-WERFWERF.createEmptyVault(vaultType: Type<@WERFWERF.Vault>())
					})
		}
		return nil
	}
	...
	init(_paymentTokenInfo: ToucansTokens.TokenInfo, _editDelay: UFix64, _minting: Bool, _initialTreasurySupply: UFix64, _maxSupply: UFix64?, _extra:{ String: AnyStruct}){ 
		...
		// Paths
		self.VaultStoragePath = /storage/WERFWERFVault
		self.ReceiverPublicPath = /public/WERFWERFReceiver
		self.VaultPublicPath = /public/WERFWERFMetadata
		self.MinterStoragePath = /storage/WERFWERFMinter
		self.AdministratorStoragePath = /storage/WERFWERFAdmin
