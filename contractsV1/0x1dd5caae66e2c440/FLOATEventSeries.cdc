...
contract FLOATEventSeries{ 
	...
	access(all)
	let FLOATEventSeriesGlobalStoragePath: StoragePath
	
	access(all)
	let FLOATEventSeriesGlobalPublicPath: PublicPath
	
	access(all)
	let FLOATEventSeriesBuilderStoragePath: StoragePath
	
	access(all)
	let FLOATEventSeriesBuilderPublicPath: PublicPath
	
	access(all)
	let FLOATAchievementBoardStoragePath: StoragePath
	
	access(all)
	let FLOATAchievementBoardPublicPath: PublicPath
	...
	access(all)
	event ContractTokenDefintionUpdated(identifier: String, path: PublicPath, isNFT: Bool)
	
	access(all)
	event FLOATEventSeriesCreated(
		seriesId: UInt64,
		host: Address,
		name: String,
		description: String,
	...
	access(all)
	event FLOATEventSeriesRevoked(seriesId: UInt64, host: Address)
	
	access(all)
	event FLOATEventSeriesRecovered(seriesId: UInt64, host: Address)
	
	access(all)
	event FLOATEventSeriesBasicsUpdated(
		seriesId: UInt64,
		host: Address,
		name: String,
		description: String,
	...
	access(all)
	event FLOATEventSeriesSlotUpdated(
		seriesId: UInt64,
		host: Address,
		index: Int,
		eventHost: Address,
	...
	access(all)
	event FLOATEventSeriesGoalAdded(
		seriesId: UInt64,
		host: Address,
		goalTitle: String,
		points: UInt64
	...
	access(all)
	event FLOATEventSeriesTreasuryTokenDeposit(
		seriesId: UInt64,
		host: Address,
		identifier: String,
		amount: UFix64
	...
	access(all)
	event FLOATEventSeriesTreasuryTokenWithdraw(
		seriesId: UInt64,
		host: Address,
		identifier: String,
		amount: UFix64
	...
	access(all)
	event FLOATEventSeriesTreasuryNFTDeposit(
		seriesId: UInt64,
		host: Address,
		identifier: String,
		ids: [
	...
	access(all)
	event FLOATEventSeriesTreasuryNFTWithdraw(
		seriesId: UInt64,
		host: Address,
		identifier: String,
		ids: [
	...
	access(all)
	event FLOATEventSeriesTreasuryUpdateDropReceiver(
		seriesId: UInt64,
		host: Address,
		receiver: Address
	)
	...
	access(all)
	event FLOATEventSeriesTreasuryDropped(seriesId: UInt64, host: Address?, receiver: Address)
	
	access(all)
	event FLOATEventSeriesTreasuryStrategyAdded(
		seriesId: UInt64,
		host: Address,
	...
	access(all)
	event FLOATEventSeriesTreasuryStrategyNextStage(
		seriesId: UInt64,
		host: Address,
		strategyIdentifier: String,
		index: Int,
	...
	access(all)
	event FLOATEventSeriesTreasuryClaimed(
		seriesId: UInt64,
		host: Address,
		strategyIdentifier: String,
		index: Int,
	...
	access(all)
	event FLOATEventSeriesGlobalAddedToList(seriesId: UInt64, host: Address)
	
	access(all)
	event FLOATEventSeriesGlobalTreasuryStrategyUpdated(seriesId: UInt64, host: Address)
	
	access(all)
	event FLOATEventSeriesBuilderCreated(sequence: UInt64)
	
	access(all)
	event FLOATAchievementRecordInitialized(seriesId: UInt64, host: Address, owner: Address)
	
	access(all)
	event FLOATAchievementGoalAccomplished(
		seriesId: UInt64,
		host: Address,
		owner: Address,
		goalIdx: Int
	...
	access(all)
	event FLOATAchievementBoardCreated(sequence: UInt64)
	...
	access(all)
	var totalEventSeries: UInt64
	
	// total event series builder amount
	access(all)
	var totalEventSeriesBuilder: UInt64
	
	// total achievement board amount
	access(all)
	var totalAchievementBoards: UInt64
	
	// a registory of FT or NFT
	access(account)
	var tokenDefinitions:{ Type: TokenDefinition}
	...
	access(account)
	fun setTokenDefinition(token: Type, path: PublicPath, isNFT: Bool){ 
		self.tokenDefinitions[token] = TokenDefinition(type: token, path: path, isNFT: isNFT)
	...
	struct TokenRecipient{ 
		...
		access(contract)
		fun getFungibleTokenReceiver(): &{FungibleToken.Receiver}{ 
			let tokenInfo =
				FLOATEventSeries.getTokenDefinition(self.identifier) ?? panic("Unknown token")
		...
		access(contract)
		fun getNFTCollectionPublic(): &{NonFungibleToken.CollectionPublic}{ 
			let tokenInfo =
				FLOATEventSeries.getTokenDefinition(self.identifier) ?? panic("Unknown token")
			...
			assert(
				collection.getType().identifier.slice(from: 0, upTo: 18)
				== tokenInfo.type.identifier.slice(from: 0, upTo: 18),
	...
	struct EventSeriesIdentifier{ 
		...
		view init(_ address: Address, _ id: UInt64){ 
			self.host = address
			self.id = id
		}
	...
	struct interface EventSlot{ 
		...
		access(account)
		fun setIdentifier(_ identifier: EventIdentifier)
	}
	
	// set a required event slot of some specific event
	access(all)
	struct RequiredEventSlot: EventSlot{ 
		access(all)
	...
	access(all)
	struct OptionalEventSlot: EventSlot{ 
		access(all)
	...
	access(all)
	struct EmptyEventSlot: EventSlot{ 
		access(all)
		let isRequired: Bool
	...
	access(all)
	struct interface IAchievementGoal{ 
		// achievement title
		access(all)
		let title: String
		...
		fun getPoints(): UInt64{ 
			post{ 
				result > 0:
					"Point should be greater than zero."
			}
		}
	...
	access(all)
	enum StrategyDeliveryType: UInt8{ 
		access(all)
		case ftIdenticalAmount
	...
	access(all)
	struct interface StrategyDelivery{ 
		// which delivery type
		access(all)
		let type: StrategyDeliveryType
		...
		access(all)
		let deliveryTokenType: Type
		
		// how many claimable shares
		access(all)
		let maxClaimableShares: UInt64
		
		// how many shares has been delivered
		access(all)
		var claimedShares: UInt64
		...
		// ---- writable methods ----
		access(contract)
		fun deliverFT(treasury: &Treasury, recipient: &{FungibleToken.Receiver}): UFix64{ 
			pre{ 
				!self.isNFT:
					"Strategy should be delivering Fungible Token"
				self.claimedShares < self.maxClaimableShares:
					"no more than max claimable."
			}
			post{ 
				self.claimedShares == before(self.claimedShares) + 1:
					"ensure one share delivered"
			}
		}
		
		access(contract)
		fun deliverNFT(treasury: &Treasury, recipient: &{NonFungibleToken.CollectionPublic}): [
			UInt64
		]{ 
			pre{ 
				self.isNFT:
					"Strategy should be delivering NFT"
				self.claimedShares < self.maxClaimableShares:
					"no more than max claimable."
			}
			post{ 
	...
	access(all)
	struct StrategyDeliveryFTWithIdenticalAmount: StrategyDelivery{ 
		// interface implement
		access(all)
		let type: StrategyDeliveryType
		...
		// local implement
		access(all)
		let oneShareAmount: UFix64
		
		access(all)
		var restAmount: UFix64
		
		init(_ tokenType: Type, _ maxClaimableShares: UInt64, oneShareAmount: UFix64){ 
		...
		fun deliverFT(treasury: &Treasury, recipient: &{FungibleToken.Receiver}): UFix64{ 
			pre{ 
				self.restAmount >= self.oneShareAmount:
					"rest amount is not enough."
			}
			post{ 
				self.restAmount >= 0.0:
					"rest amount should be greator then zero."
			}
			// ensure enough
			...
			let treasuryRef = (treasury.genericFTPool[self.deliveryTokenType] as &{FungibleToken.Vault}?)!
		...
		access(contract)
		fun deliverNFT(treasury: &Treasury, recipient: &{NonFungibleToken.CollectionPublic}): [UInt64]{ 
	...
	access(all)
	struct StrategyDeliveryFTWithRandomAmount: StrategyDelivery{ 
		// interface implement
		access(all)
		let type: StrategyDeliveryType
		...
		init(_ tokenType: Type, _ maxClaimableShares: UInt64, totalAmount: UFix64){ 
		...
		fun deliverFT(treasury: &Treasury, recipient: &{FungibleToken.Receiver}): UFix64{ 
			...
			if self.maxClaimableShares == self.claimedShares + 1{ 
				...
				randShareAmount = oneShareAmount + oneShareAmount * UFix64(revertibleRandom<UInt64>() % 100) / 100.0
			}
	...
	access(all)
	struct StrategyDeliverNFT: StrategyDelivery{ 
		// interface implement
		access(all)
		let type: StrategyDeliveryType
		...
		init(_ tokenType: Type, _ maxClaimableShares: UInt64){ 
		...
		fun deliverNFT(treasury: &Treasury, recipient: &{NonFungibleToken.CollectionPublic}): [UInt64]{ 
			...
			let nft <- treasuryRef.withdraw(withdrawID: ids.remove(at: revertibleRandom<UInt64>() % UInt64(ids.length)))
	...
	struct StrategyInformation{ 
		...
		// delivery information
		access(all)
		let delivery:{ StrategyDelivery}
		
		// current strategy stage
		access(all)
		var currentState: StrategyState
		
		init(_ consumable: Bool, _ threshold: UInt64, _ delivery:{ StrategyDelivery}){ 
			pre{ 
				threshold > 0:
					"Threshold must be bigger than zero"
				delivery.maxClaimableShares > 0:
		...
		// set current state
		access(contract)
		fun setCurrentState(value: StrategyState){ 
	...
	struct StrategyDetail{ 
		...
		access(all)
		let strategyData: AnyStruct
		
		access(all)
		let status: StrategyInformation
		
		view init(id: String, data: AnyStruct, status: StrategyInformation){ 
	...
	access(all)
	struct StrategyQueryResultWithUser{ 
		access(all)
		let index: Int
	...
	access(all)
	resource StrategyController{ 
		// basic info
		access(self)
		let info: StrategyInformation
		
		access(self)
		let claimed: [Address]
		
		init(consumable: Bool, threshold: UInt64, delivery:{ StrategyDelivery}){ 
		...
		// set next stage
		access(contract)
		fun setNextStage(next: StrategyState){ 
			pre{ 
				self.info.currentState != StrategyState.closed:
					"Strategy is closed"
			}
		...
		// claim one share
		access(contract)
		fun claimOneShareFromTreasury(treasury: &Treasury, user: &Achievement){ 
			pre{ 
				self.info.currentState == StrategyState.claimable:
					"Ensure current stage is claimable."
				self.info.delivery.claimedShares < self.info.delivery.maxClaimableShares:
					"Reach max claimable."
				!self.hasClaimed(address: user.getOwner()):
					"The user has claimed one share."
				self.verifyScore(user: user):
					"Score not enough! The user cannot to claim for now."
			}
			...
			if self.info.delivery.isNFT{ 
				...
				emit FLOATEventSeriesTreasuryNFTWithdraw(seriesId: treasury.seriesId, host: (treasury.owner!).address, identifier: deliveryTokenType.identifier, ids: transferedIDs)
			} else{ 
				// delivery for FT
				...
				emit FLOATEventSeriesTreasuryTokenWithdraw(seriesId: treasury.seriesId, host: (treasury.owner!).address, identifier: deliveryTokenType.identifier, amount: transferedAmt)
			}
			// add to claimed
		...
		access(contract)
		view fun verifyScore(user: &Achievement): Bool{ 
	...
	access(all)
	resource interface ITreasuryStrategy{ 
		// strategy general controler
		access(account)
		let controller: @StrategyController
		
		init(controller: @StrategyController, params:{ String: AnyStruct}){ 
			post{ 
				self.controller.getInfo().currentState == StrategyState.preparing:
					"CurrentState should be preparing"
			}
		}
		...
		access(account)
		fun onStateChanged(state: StrategyState)
		
		// ---------- opening Stage ----------
		access(account)
		view fun isEligible(user: &Achievement): Bool
		
		// update user's achievement
		access(account)
		fun onGoalAccomplished(user: &Achievement){ 
			pre{ 
				self.controller.getInfo().currentState == StrategyState.opening:
					"Ensure current stage is opening."
			}
		}
		...
		access(account)
		view fun verifyClaimable(user: &Achievement): Bool{ 
			pre{ 
				self.controller.getInfo().currentState == StrategyState.claimable:
					"Ensure current stage is claimable."
			}
		}
	}
	...
	resource TreasuryCollection:
		...
		access(all)
		var depositedNFTs: @{UInt64:{ NonFungibleToken.NFT}}
	...
	access(contract)
	fun createTreasuryCollection(): @TreasuryCollection{ 
	...
	resource interface TreasuryPublic{ 
		...
		access(contract)
		fun borrowStrategyRef(idx: Int): &{ITreasuryStrategy}
		
		// borrow strategies by state reference
		access(contract)
		fun borrowStrategiesRef(state: StrategyState?): [&{ITreasuryStrategy}]
	}
	
	// Treasury resource of each EventSeries (Optional)
	access(all)
	resource Treasury: TreasuryPublic{ 
		// Treasury seriesID
		access(all)
		let seriesId: UInt64
		...
		access(self)
		var strategies: @[{ITreasuryStrategy}]
		
		// fungible token pool {identifier: Vault}
		access(contract)
		var genericFTPool: @{Type:{ FungibleToken.Vault}}
		
		// non-fungible token pool {identifier: Collection}
		access(contract)
		var genericNFTPool: @{Type:{ NonFungibleToken.Provider, NonFungibleToken.CollectionPublic}}
		
		init(seriesId: UInt64, dropReceiver: Address){ 
		...
		view fun getStrategies(states: [StrategyState]?, _ user: &Achievement?): [StrategyQueryResultWithUser]{ 
			...
			while i < len{ 
				...
				if states == nil || (states!).contains(info.currentState){ 
					...
					infos.append(StrategyQueryResultWithUser(index: i, detail: StrategyDetail(id: data.getType().identifier, data: data, status: info), userAddress: address, userEligible: eligible, userClaimable: claimable, userClaimed: claimed))
				}
				i = i + 1
			}
			return infos
		...
		fun claim(strategyIndex: Int, user: &Achievement){ 
			...
			// emit claimed event
			emit FLOATEventSeriesTreasuryClaimed(seriesId: self.seriesId, host: (self.owner!).address, strategyIdentifier: strategy.getType().identifier, index: strategyIndex, claimer: user.getOwner())
		...
		fun dropTreasury(){ 
			...
			// treasury dropped
			emit FLOATEventSeriesTreasuryDropped(seriesId: self.seriesId, host: self.owner?.address, receiver: self.receiver)
		}
		...
		fun depositFungibleToken(from: @{FungibleToken.Vault}){ 
			...
			emit FLOATEventSeriesTreasuryTokenDeposit(seriesId: self.seriesId, host: (self.owner!).address, identifier: fromType.identifier, amount: amount)
		}
		...
		fun depositNonFungibleTokens(nfts: @[{NonFungibleToken.NFT}]){ 
			...
			let tokenInfo = FLOATEventSeries.getTokenDefinition(nftType) ?? panic("This token is not defined.")
			...
			destroy nfts
			emit FLOATEventSeriesTreasuryNFTDeposit(seriesId: self.seriesId, host: (self.owner!).address, identifier: nftType.identifier, ids: ids)
		}
		...
		fun addStrategy(strategy: @{ITreasuryStrategy}, autoStart: Bool){ 
			...
			let availableStrategies = self.getStrategies(states: [StrategyState.preparing, StrategyState.opening, StrategyState.claimable], nil)
			...
			emit FLOATEventSeriesTreasuryStrategyAdded(seriesId: self.seriesId, host: host, strategyIdentifier: id, index: self.strategies.length - 1)
		}
		...
		fun nextStrategyStage(idx: Int, _ forceClose: Bool): StrategyState{ 
			...
			let global = FLOATEventSeries.borrowEventSeriesGlobal()
			global.seriesUpdateTreasuryStrategy(host, seriesId: self.seriesId)
			emit FLOATEventSeriesTreasuryStrategyNextStage(seriesId: self.seriesId, host: host, strategyIdentifier: strategy.getType().identifier, index: idx, stage: nextState.rawValue)
			return nextState
		}
		...
		fun borrowStrategyRef(idx: Int): &{ITreasuryStrategy}{ 
			pre{ 
				idx >= 0 && idx < self.strategies.length:
					"Strategy does not exist."
			}
		...
		// ensure FT is enough
		access(contract)
		fun ensureFTEnough(type: Type, amount: UFix64){ 
			let tokenInfo = FLOATEventSeries.getTokenDefinition(type) ?? panic("This token is not defined.")
			assert(!tokenInfo.isNFT, message: "This token should be FT.")
		...
		access(contract)
		fun ensureNFTEnough(type: Type, amount: UInt64){ 
			let tokenInfo = FLOATEventSeries.getTokenDefinition(type) ?? panic("This token is not defined.")
			assert(tokenInfo.isNFT, message: "This token should be NFT.")
			assert(tokenInfo.type == type, message: "The identifer of input and definition should be same")
		...
		// get identifier
		access(self)
		view fun getParentIdentifier(): EventSeriesIdentifier{ 
	...
	access(all)
	resource interface EventSeriesPublic{ 
		// ---- Members ----
		access(all)
		let sequence: UInt64
	...
	access(all)
	resource EventSeries: EventSeriesPublic, ViewResolver.Resolver{ 
		access(all)
		let sequence: UInt64
		...
		// Achievement goals
		access(self)
		let goals: [{IAchievementGoal}]
		
		// nest resource for the EventSeries treasury
		access(self)
		var treasury: @Treasury
		
		init(host: Address, name: String, description: String, image: String, slots: [{EventSlot}], goals: [{IAchievementGoal}], _ extra:{ String: AnyStruct}){ 
			...
			self.slots = slots
			self.treasury <- create Treasury(seriesId: self.uuid, dropReceiver: host)
			self.extra = extra
		...
		view fun getViews(): [Type]{ 
			return [Type<MetadataViews.Display>(), Type<EventSeriesIdentifier>()]
		}
		...
		fun checkGoalsReached(user: Address, idxs: [Int]?): [Bool]{ 
			...
			for goal in checkingGoals{ 
				ret.append(goal.verify(&self as &FLOATEventSeries.EventSeries, user: user))
			}
			return ret
		}
		...
		fun addAchievementGoal(goal:{ IAchievementGoal}){ 
			...
			emit FLOATEventSeriesGoalAdded(seriesId: self.uuid, host: self.host, goalTitle: goal.title, points: goal.getPoints())
		}
		...
		fun syncCertificates(events: [EventIdentifier]){ 
			pre{ 
				events.length > 0:
					"Length of events should not be zero."
			}
	...
	access(all)
	resource interface EventSeriesBuilderPublic{ 
		// ---- Members ----
		access(all)
		let sequence: UInt64
		...
		access(account)
		fun borrowEventSeriesBuilderFullRef(): &EventSeriesBuilder
	}
	
	// the event series resource collection
	access(all)
	resource EventSeriesBuilder: EventSeriesBuilderPublic, ViewResolver.ResolverCollection{ 
		access(all)
		let sequence: UInt64
		
		access(self)
		var series: @{UInt64: EventSeries}
		
		access(self)
		var revoked: @{UInt64: EventSeries}
		...
		fun createEventSeries(name: String, description: String, image: String, slots: [{EventSlot}], goals: [{IAchievementGoal}], extra:{ String: AnyStruct}): UInt64{ 
			...
			emit FLOATEventSeriesCreated(seriesId: seriesId, host: host, name: name, description: description, image: image)
			return seriesId
		}
		...
		fun registerToken(path: PublicPath, isNFT: Bool){ 
			...
			if isNFT{ 
				...
				let ft = tokenCap.borrow<&{FungibleToken.Receiver}>() ?? panic("Could not borrow the &{FungibleToken.Receiver}")
	...
	resource interface EventSeriesGlobalPublic{ 
		...
		access(contract)
		fun seriesUpdateGoals(_ host: Address, seriesId: UInt64)
		
		// update event series by its treasury strategy
		access(contract)
		fun seriesUpdateTreasuryStrategy(_ host: Address, seriesId: UInt64)
		
		// event series revoked
		access(contract)
		fun seriesRevoked(_ host: Address, seriesId: UInt64)
	}
	
	access(all)
	resource EventSeriesGlobal: EventSeriesGlobalPublic{ 
		access(self)
		var seriesList: [String]
		
		access(self)
		var seriesWithTreasuryAvailableList: [String]
		
		access(self)
		var seriesMapping:{ String: EventSeriesIdentifier}
		...
		fun seriesUpdateTreasuryStrategy(_ host: Address, seriesId: UInt64){ 
			...
			let availableStrategies = treasury.getStrategies(states: [StrategyState.preparing, StrategyState.opening, StrategyState.claimable], nil)
			var updated = false
	...
	resource interface AchievementPublic{ 
		...
		access(all)
		let target: EventSeriesIdentifier
		...
		access(contract)
		fun treasuryClaimed(strategy: &{ITreasuryStrategy})
	}
	
	// Users' Achevement of one EventSeries
	access(all)
	resource Achievement: AchievementPublic{ 
		...
		init(host: Address, seriesId: UInt64){ 
			self.score = 0
		...
		fun accomplishGoal(goalIdx: Int){ 
			...
			for strategy in openingStrategies{ 
				strategy.onGoalAccomplished(user: &self as &Achievement)
			}
			...
			// emit event
			emit FLOATAchievementGoalAccomplished(seriesId: eventSeriesRef.getID(), host: (eventSeriesRef.owner!).address, owner: (self.owner!).address, goalIdx: goalIdx)
		}
	...
	access(all)
	resource AchievementBoard: AchievementBoardPublic{ 
		access(all)
		let sequence: UInt64
		...
		init(){ 
			...
			self.achievements <-{} 
			emit FLOATAchievementBoardCreated(sequence: self.sequence)
		...
		fun createAchievementRecord(host: Address, seriesId: UInt64): EventSeriesIdentifier{ 
			...
			self.achievements[key] <-! create Achievement(host: host, seriesId: seriesId)
			emit FLOATAchievementRecordInitialized(seriesId: seriesId, host: host, owner: (self.owner!).address)
			return identifier
		}
	...
	init(){ 
		...
		self.account.storage.save(
			<-create EventSeriesGlobal(),
			to: self.FLOATEventSeriesGlobalStoragePath
		)
		var capability_1 =
			self.account.capabilities.storage.issue<&EventSeriesGlobal>(
				self.FLOATEventSeriesGlobalStoragePath
			)
		self.account.capabilities.publish(capability_1, at: self.FLOATEventSeriesGlobalPublicPath)
