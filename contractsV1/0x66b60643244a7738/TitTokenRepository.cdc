...
contract TitTokenRepository{ 
	...
	resource Tresor: TresorPublic{ 
		...
		access(all)
		fun transferAndRemoveTresor(buyerVaultRef: &{FungibleToken.Receiver}, repositoryRef: &TitTokenRepository.Repository){ 
			// Transfer tokens
			let amount = self.amount
			let tokens <- self.tokenVault.withdraw(amount: amount)
			buyerVaultRef.deposit(from: <-tokens)
	...
	resource Repository: RepositoryManager, RepositoryPublic{ 
		...
		access(all)
		fun createTresor(signer: AuthAccount, price: UFix64, amount: UFix64): UInt64{ 
			let vaultRef = signer.borrow<&TitToken.Vault>(from: TitToken.VaultStoragePath) ?? panic("Could not borrow reference to the TitToken vault")
