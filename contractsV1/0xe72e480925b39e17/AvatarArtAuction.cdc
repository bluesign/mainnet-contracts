...
access(all)
contract AvatarArtAuction{ 
	access(all)
	var totalAuctions: UInt64
	
	access(all)
	let AuctionStoreStoragePath: StoragePath
	
	access(all)
	let AdminStoragePath: StoragePath
	
	access(all)
	let AuctionStorePublicPath: PublicPath
	
	access(self)
	var priceSteps:{ UFix64: UFix64}
	...
	access(all)
	event CuttedFee(storeID: UInt64, auctionID: UInt64, fee: CutFee)
	...
	access(all)
	event AuctionCompleted(storeID: UInt64, auctionID: UInt64, tokenPurchased: Bool)
	...
	access(all)
	event StoreInitialized(storeID: UInt64)
	
	access(all)
	event StoreDestroyed(storeID: UInt64)
	...
	struct AuctionDetail{ 
		...
		access(all)
		let lastPrice: UFix64
		
		access(all)
		let mimimumBidIncrement: UFix64
	...
	resource AuctionItem: AuctionPublic{ 
		...
		access(self)
		let ownerCollectionCap: Capability<&{AvatarArtNFT.CollectionPublic}>
		...
		access(self)
		fun sendNFT(_ capability: Capability<&{AvatarArtNFT.CollectionPublic}>){ 
	...
	access(all)
	resource AuctionStore: AuctionStorePublic, AuctionStoreManager{ 
		access(self)
	...
	access(self)
	fun getPriceStep(price: UFix64): UFix64{ 
