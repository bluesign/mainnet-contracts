...
access(all)
contract YoungBoysBern: NonFungibleToken{ 
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: YoungBoysBern.CollectionStoragePath, publicPath: YoungBoysBern.CollectionPublicPath, publicCollection: Type<&YoungBoysBern.Collection>(), publicLinkedType: Type<&YoungBoysBern.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-YoungBoysBern.createEmptyCollection(nftType: Type<@YoungBoysBern.Collection>())
						})
	...
	//
	access(all)
	resource Collection: YoungBoysBernCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		...
		view fun borrowViewResolver(id: UInt64): &{ViewResolver.Resolver}?{ 
			...
			return YoungBoysBernNFT as &{ViewResolver.Resolver}
		}
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&{YoungBoysBernCollectionPublic}>(self.CollectionStoragePath)
