...
access(all)
contract MintStoreItem: NonFungibleToken{ 
	...
	access(all)
	event MerchantCreated(merchantID: UInt32, name: String)
	
	// Emitted when a new Merchant is updated
	access(all)
	event MerchantUpdated(merchantID: UInt32, name: String)
		...
		fun resolveView(_ view: Type): AnyStruct?{ 
			switch view{ 
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(storagePath: MintStoreItem.CollectionStoragePath, publicPath: MintStoreItem.CollectionPublicPath, publicCollection: Type<&MintStoreItem.Collection>(), publicLinkedType: Type<&MintStoreItem.Collection>(), createEmptyCollectionFunction: fun (): @{NonFungibleToken.Collection}{ 
							return <-MintStoreItem.createEmptyCollection(nftType: Type<@MintStoreItem.Collection>())
						})
				case Type<MetadataViews.NFTCollectionDisplay>():
					let edition = EditionData(editionID: self.data.editionID)
					let merchantName = MintStoreItem.merchants[self.data.merchantID]!
					let thumbnail = edition.metadata["thumbnail"] ?? ""
					let media = MetadataViews.Media(file: MetadataViews.HTTPFile(url: thumbnail), mediaType: "image/png")
					let url = edition.metadata["externalUrl"] ?? ""
					let description = edition.metadata["description"] ?? ""
					return MetadataViews.NFTCollectionDisplay(name: merchantName, description: description, externalURL: MetadataViews.ExternalURL(url), squareImage: media, bannerImage: media, socials:{} )
	...
	resource Admin{ 
		...
		fun createMerchant(merchantName: String): UInt32{ 
			pre{ 
				merchantName != nil:
					"Cannot create the merchant: merchantName is nil"
			}
		...
		fun updateMerchant(merchantID: UInt32, merchantName: String): UInt32{ 
			pre{ 
				MintStoreItem.merchants[merchantID] != nil:
					"Cannot upate the merchant: merchantID is not initialized"
				merchantName != nil:
					"Cannot upate the merchant: merchantName is nil"
			}
	...
	//
	access(all)
	resource Collection: MintStoreItemCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.Collection, NonFungibleToken.CollectionPublic, ViewResolver.ResolverCollection{ 
		...
		fun withdraw(withdrawID: UInt64): @{NonFungibleToken.NFT}{ 
			...
			let token <- self.ownedNFTs.remove(key: withdrawID) ?? panic("Cannot withdraw: MintStoreItem does not exist in the collection")
		...
		view fun borrowViewResolver(id: UInt64): &{ViewResolver.Resolver}?{ 
			...
			return MintStoreItemNFT as &{ViewResolver.Resolver}
		}
	...
	init(){ 
		...
		var capability_1 = self.account.capabilities.storage.issue<&{MintStoreItemCollectionPublic}>(self.CollectionStoragePath)
