...
pub contract TouchstoneSpecialEditionFloFightersxDiamondFists: NonFungibleToken {
			...
			switch view {
				...
				case Type<MetadataViews.NFTCollectionData>():
					return MetadataViews.NFTCollectionData(
						storagePath: TouchstoneSpecialEditionFloFightersxDiamondFists.CollectionStoragePath,
						publicPath: TouchstoneSpecialEditionFloFightersxDiamondFists.CollectionPublicPath,
						providerPath: TouchstoneSpecialEditionFloFightersxDiamondFists.CollectionPrivatePath,
          ...
          return MetadataViews.ExternalURL("https://touchstone.city/discover/".concat(self.owner!.address.toString()).concat("/TouchstoneSpecialEditionFloFightersxDiamondFists"))
				case Type<MetadataViews.NFTCollectionDisplay>():
					let squareMedia = MetadataViews.Media(
						file: TouchstoneSpecialEditionFloFightersxDiamondFists.getCollectionAttribute(key: "image") as! MetadataViews.IPFSFile,
						mediaType: "image"
					)
					...
					return MetadataViews.NFTCollectionDisplay(
						name: TouchstoneSpecialEditionFloFightersxDiamondFists.getCollectionAttribute(key: "name") as! String,
						description: TouchstoneSpecialEditionFloFightersxDiamondFists.getCollectionAttribute(key: "description") as! String,
						externalURL: MetadataViews.ExternalURL("https://touchstone.city/discover/".concat(self.owner!.address.toString()).concat("/TouchstoneSpecialEditionFloFightersxDiamondFists")),
						squareImage: squareMedia,
						bannerImage: bannerMedia ?? squareMedia,
						socials: TouchstoneSpecialEditionFloFightersxDiamondFists.getCollectionAttribute(key: "socials") as! {String: MetadataViews.ExternalURL}
					)
	...
	pub resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection {
		...
		pub fun claim() {
			if let storage = &TouchstoneSpecialEditionFloFightersxDiamondFists.nftStorage[self.owner!.address] as &{UInt64: NFT}? {
	...
	pub resource Administrator {
		pub fun createNFTMetadata(name: String, description: String, imagePath: String, thumbnailPath: String?, ipfsCID: String, price: UFix64?, extra: {String: AnyStruct}, supply: UInt64) {
			TouchstoneSpecialEditionFloFightersxDiamondFists.metadatas[TouchstoneSpecialEditionFloFightersxDiamondFists.nextMetadataId] = NFTMetadata(
				_name: name,
		...
		pub fun mintNFT(metadataId: UInt64, serial: UInt64, recipient: Address) {
			pre {
				EmeraldPass.isActive(user: TouchstoneSpecialEditionFloFightersxDiamondFists.account.address): "You must have an active Emerald Pass subscription to airdrop NFTs. You can purchase Emerald Pass at https://pass.ecdao.org/"
			}
			let nft <- create NFT(_metadataId: metadataId, _serial: serial, _recipient: recipient)
			if let recipientCollection = getAccount(recipient).getCapability(TouchstoneSpecialEditionFloFightersxDiamondFists.CollectionPublicPath).borrow<&TouchstoneSpecialEditionFloFightersxDiamondFists.Collection{NonFungibleToken.CollectionPublic}>() {
				recipientCollection.deposit(token: <- nft)
			} else {
				if let storage = &TouchstoneSpecialEditionFloFightersxDiamondFists.nftStorage[recipient] as &{UInt64: NFT}? {
					storage[nft.id] <-! nft
				} else {
	...
	pub fun getClaimableNFTs(user: Address): {UInt64: NFTMetadata} {
		...
		if let storage = &TouchstoneSpecialEditionFloFightersxDiamondFists.nftStorage[user] as &{UInt64: NFT}? {
			for id in storage.keys {
