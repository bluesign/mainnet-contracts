...
pub contract SGKCLDRpuniru: NonFungibleToken, Anique {
    // -----------------------------------------------------------------------
    // SGKCLDRpuniru contract Events
    // -----------------------------------------------------------------------

    // Events for Contract-Related actions
    //
    // Emitted when the SGKCLDRpuniru contract is created
    ...
    // -----------------------------------------------------------------------
    // SGKCLDRpuniru contract-level fields.
    ...
    // -----------------------------------------------------------------------
    // SGKCLDRpuniru contract-level Composite Type definitions
    // -----------------------------------------------------------------------

    // The structure that represents Item
    // each digital content which SGKCLDRpuniru deal with on Flow
    //
    pub struct ItemData {
        ...
        init(itemID: UInt32) {
            let item = (&SGKCLDRpuniru.items[itemID] as &Item?)!
    ...
    pub resource Item {
        ...
        init(metadata: {String: String}) {
            ...
            self.itemID = SGKCLDRpuniru.nextItemID
            self.metadata = metadata
            self.numberMintedPerItem = 0

            // increment the nextItemID so that it isn't used again
            SGKCLDRpuniru.nextItemID = SGKCLDRpuniru.nextItemID + 1
    ...
    pub resource NFT: NonFungibleToken.INFT, Anique.INFT {
        ...
        init(serialNumber: UInt32, itemID: UInt32) {
            ...
            SGKCLDRpuniru.totalSupply = SGKCLDRpuniru.totalSupply + 1

            // set id
            self.id = SGKCLDRpuniru.totalSupply
    ...
    // interface that represents SGKCLDRpuniru collections to public
    ...
    pub resource interface CollectionPublic {
        ...
        // borrow NFT as SGKCLDRpuniru token
        pub fun borrowSGKCLDRpuniruCollectible(id: UInt64): auth &NFT
    }
    ...
    pub resource Collection: CollectionPublic, NonFungibleToken.Receiver, NonFungibleToken.Provider, NonFungibleToken.CollectionPublic {
        ...
        pub fun deposit(token: @NonFungibleToken.NFT) {
            ...
            // Cast the deposited token as an SGKCLDRpuniru NFT to make sure
            // it is the correct type
            let token <- token as! @SGKCLDRpuniru.NFT
        ...
        // not any SGKCLDRpuniru specific data. Please use borrowCollectible to
        ...
        // borrowSGKCLDRpuniruCollectible returns a borrowed reference
        // to an SGKCLDRpuniru Collectible
        pub fun borrowSGKCLDRpuniruCollectible(id: UInt64): auth &NFT {
            pre {
    ...
    pub resource Admin {
        ...
        // and stores it in the Items dictionary field in the SGKCLDRpuniru smart contract
        //
        ...
        pub fun createItem(metadata: {String: String}): UInt32 {
            ...
            SGKCLDRpuniru.items[newItem.itemID] <-! newItem

            return itemId
        }

        // borrowItem returns a reference to a Item in the SGKCLDRpuniru
        ...
        pub fun borrowItem(itemID: UInt32): &Item {
            pre {
                SGKCLDRpuniru.items[itemID] != nil: "Cannot borrow Item: The Item doesn't exist"
            }

            return (&SGKCLDRpuniru.items[itemID] as &Item?)!
        }
    ...
    // -----------------------------------------------------------------------
    // SGKCLDRpuniru contract-level function definitions
    // -----------------------------------------------------------------------
    ...
    pub fun createEmptyCollection(): @NonFungibleToken.Collection {
        return <-create SGKCLDRpuniru.Collection()
    }
    ...
    // -----------------------------------------------------------------------
    // SGKCLDRpuniru initialization function
    // -----------------------------------------------------------------------
    //
    init() {
        ...
        self.collectionStoragePath     = /storage/SGKCLDRpuniruCollection
        self.collectionPublicPath      =  /public/SGKCLDRpuniruCollection
        self.collectionPrivatePath     = /private/SGKCLDRpuniruCollection
        self.adminStoragePath          = /storage/SGKCLDRpuniruAdmin
        self.saleCollectionStoragePath = /storage/SGKCLDRpuniruSaleCollection
        self.saleCollectionPublicPath  =  /public/SGKCLDRpuniruSaleCollection
